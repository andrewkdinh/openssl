...
int ssl_release_record(SSL_CONNECTION *s, TLS_RECORD *rr, size_t length)
{
    ...
    if (rr->rechandle != NULL) {
        ...
        if (HANDLE_RLAYER_READ_RETURN(s,
                                      s->rlayer.rrlmethod->release_record(
    ...
    if (s->rlayer.curr_rec >= s->rlayer.num_recs) {
        ...
        do {
            ...
            ret = HANDLE_RLAYER_READ_RETURN(
                s,
                s->rlayer.rrlmethod->read_record(
    ...
    *set = OSSL_PARAM_construct_end();
    ...
    for (;;) {
        ...
        rlret = meth->new_record_layer(
            sctx->libctx, sctx->propq, version, s->server, direction, level,
            epoch, secret, secretlen, key, keylen, iv, ivlen, mackey, mackeylen,
            ciph, taglen, mactype, md, compm, kdfdigest, prev, thisbio, next,
            NULL, NULL, settings, options, rlayer_dispatch_tmp, s,
