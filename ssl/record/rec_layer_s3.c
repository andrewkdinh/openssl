...
int RECORD_LAYER_reset(RECORD_LAYER *rl) {
    ...
    OSSL_RECORD_DIRECTION_READ, OSSL_RECORD_PROTECTION_LEVEL_NONE, NULL, 0,
    NULL, 0, NULL, 0, NULL, 0, NULL, 0, NID_undef, NULL, NULL, NULL);

    ret &= ssl_set_new_record_layer(
    rl->s, SSL_CONNECTION_IS_DTLS(rl->s) ? DTLS_ANY_VERSION : TLS_ANY_VERSION,
    OSSL_RECORD_DIRECTION_WRITE, OSSL_RECORD_PROTECTION_LEVEL_NONE, NULL, 0,
    NULL, 0, NULL, 0, NULL, 0, NULL, 0, NID_undef, NULL, NULL, NULL);
...
int ssl_release_record(SSL_CONNECTION *s, TLS_RECORD *rr, size_t length) {
    ...
    if (rr->rechandle != NULL) {
        ...
        if (HANDLE_RLAYER_READ_RETURN(
            s, s->rlayer.rrlmethod->release_record(s->rlayer.rrl, rr->rechandle,
                                                   length)) <= 0) {
    ...
    if (s->rlayer.curr_rec >= s->rlayer.num_recs) {
        ...
        do {
            ...
            s, s->rlayer.rrlmethod->read_record(
               s->rlayer.rrl, &rr->rechandle, &rr->version, &rr->type,
               &rr->data, &rr->length, NULL, NULL));
            if (ret <= 0) {
                /* SSLfatal() already called if appropriate */
                return ret;
            }
...
int ssl_set_new_record_layer(SSL_CONNECTION *s, int version, int direction,
    ...
    *set = OSSL_PARAM_construct_end();
    ...
    for (;;) {
        ...
        secret, secretlen, key, keylen, iv, ivlen, mackey, mackeylen, ciph,
        taglen, mactype, md, compm, kdfdigest, prev, thisbio, next, NULL, NULL,
        settings, options, rlayer_dispatch_tmp, s, s->rlayer.rlarg, &newrl);
        BIO_free(prev);
        switch (rlret) {
