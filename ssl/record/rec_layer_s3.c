...
void RECORD_LAYER_init(RECORD_LAYER *rl, SSL_CONNECTION *s) { rl->s = s; }
...
int RECORD_LAYER_reset(RECORD_LAYER *rl) {
  ...
  ret &= ssl_set_new_record_layer(
  rl->s, SSL_CONNECTION_IS_DTLS(rl->s) ? DTLS_ANY_VERSION : TLS_ANY_VERSION,
  OSSL_RECORD_DIRECTION_READ, OSSL_RECORD_PROTECTION_LEVEL_NONE, NULL, 0, NULL,
  0, NULL, 0, NULL, 0, NULL, 0, NID_undef, NULL, NULL, NULL);

  ret &= ssl_set_new_record_layer(
  rl->s, SSL_CONNECTION_IS_DTLS(rl->s) ? DTLS_ANY_VERSION : TLS_ANY_VERSION,
  OSSL_RECORD_DIRECTION_WRITE, OSSL_RECORD_PROTECTION_LEVEL_NONE, NULL, 0, NULL,
  0, NULL, 0, NULL, 0, NULL, 0, NID_undef, NULL, NULL, NULL);
...
int ssl3_write_bytes(SSL *ssl, uint8_t type, const void *buf_, size_t len,
  ...
  if (i < 0) {
    ...
    s, s->rlayer.wrlmethod->retry_write_records(s->rlayer.wrl));
    if (i <= 0) {
      s->rlayer.wnum = tot;
      return i;
    }
  ...
  for (;;) {
    ...
    s->rlayer.wrl, type, n, max_send_fragment, &split_send_fragment);
    /*
    ...
    i = HANDLE_RLAYER_WRITE_RETURN(
    s, s->rlayer.wrlmethod->write_records(s->rlayer.wrl, tmpls, maxpipes));
    if (i <= 0) {
      /* SSLfatal() already called if appropriate */
      s->rlayer.wnum = tot;
      return i;
...
int ssl_release_record(SSL_CONNECTION *s, TLS_RECORD *rr, size_t length) {
  ...
  if (rr->rechandle != NULL) {
    ...
    if (HANDLE_RLAYER_READ_RETURN(s, s->rlayer.rrlmethod->release_record(
...
start:
  ...
  if (s->rlayer.curr_rec >= s->rlayer.num_recs) {
    ...
    do {
      ...
      s, s->rlayer.rrlmethod->read_record(s->rlayer.rrl, &rr->rechandle,
  ...
  if (s->rlayer.handshake_fragment_len > 0 && rr->type != SSL3_RT_HANDSHAKE &&
  ...
  if (type == rr->type ||
      (rr->type == SSL3_RT_CHANGE_CIPHER_SPEC && type == SSL3_RT_HANDSHAKE &&
       recvd_type != NULL && !is_tls13)) {
    /*
...
int ssl_set_new_record_layer(SSL_CONNECTION *s, int version, int direction,
  ...
  *opts++ = OSSL_PARAM_construct_uint64(OSSL_LIBSSL_RECORD_LAYER_PARAM_OPTIONS,
                                        &s->options);
  *opts++ =
  OSSL_PARAM_construct_uint32(OSSL_LIBSSL_RECORD_LAYER_PARAM_MODE, &s->mode);
  if (direction == OSSL_RECORD_DIRECTION_READ) {
    *opts++ = OSSL_PARAM_construct_size_t(
    OSSL_LIBSSL_RECORD_LAYER_READ_BUFFER_LEN, &s->rlayer.default_read_buf_len);
    *opts++ = OSSL_PARAM_construct_int(
    OSSL_LIBSSL_RECORD_LAYER_PARAM_READ_AHEAD, &s->rlayer.read_ahead);
  } else {
    *opts++ = OSSL_PARAM_construct_size_t(
    OSSL_LIBSSL_RECORD_LAYER_PARAM_BLOCK_PADDING, &s->rlayer.block_padding);
    *opts++ = OSSL_PARAM_construct_size_t(
    OSSL_LIBSSL_RECORD_LAYER_PARAM_HS_PADDING, &s->rlayer.hs_padding);
  }
  *opts = OSSL_PARAM_construct_end();
  ...
  if (use_etm)
    *set++ =
    OSSL_PARAM_construct_int(OSSL_LIBSSL_RECORD_LAYER_PARAM_USE_ETM, &use_etm);
  ...
  if (tlstree)
    *set++ =
    OSSL_PARAM_construct_int(OSSL_LIBSSL_RECORD_LAYER_PARAM_TLSTREE, &tlstree);
  ...
  *set = OSSL_PARAM_construct_end();
  ...
  for (;;) {
    ...
    sctx->libctx, sctx->propq, version, s->server, direction, level, epoch,
    secret, secretlen, key, keylen, iv, ivlen, mackey, mackeylen, ciph, taglen,
    mactype, md, compm, kdfdigest, prev, thisbio, next, NULL, NULL, settings,
    options, rlayer_dispatch_tmp, s, s->rlayer.rlarg, &newrl);
    BIO_free(prev);
    switch (rlret) {
