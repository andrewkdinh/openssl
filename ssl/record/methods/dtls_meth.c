...
static int dtls_record_replay_check(OSSL_RECORD_LAYER *rl,
                                    DTLS_BITMAP *bitmap) {
  int cmp;
  unsigned int shift;
...
again:
  ...
  if ((rl->rstate != SSL_ST_READ_BODY) ||
    ...
    if (rl->msg_callback != NULL)
      rl->msg_callback(0, rr->rec_version, SSL3_RT_HEADER, rl->packet,
                       DTLS1_RT_HEADER_LENGTH, rl->cbarg);
    ...
    if (ssl_major != (rl->version == DTLS_ANY_VERSION ? DTLS1_VERSION_MAJOR
                                                      : rl->version >> 8)) {
  ...
  if (is_next_epoch) {
    if (rl->in_init) {
      if (dtls_rlayer_buffer_record(rl, rl->unprocessed_rcds, rr->seq_num) <
          0) {
        /* RLAYERfatal() already called */
        return OSSL_RECORD_RETURN_FATAL;
      }
  ...
  if (rl->funcs->post_process_record &&
      !rl->funcs->post_process_record(rl, rr)) {
    /* RLAYERfatal already called */
    return OSSL_RECORD_RETURN_FATAL;
  }
...
static int dtls_free(OSSL_RECORD_LAYER *rl) {
  ...
  if (rl->unprocessed_rcds != NULL) {
    while ((item = pqueue_pop(rl->unprocessed_rcds)) != NULL) {
      ...
      ret &=
      BIO_write_ex(rl->next, rdata->packet, rdata->packet_length, &written);
      OPENSSL_free(rdata->rbuf.buf);
      OPENSSL_free(item->data);
      pitem_free(item);
    }
...
static int dtls_new_record_layer(
...
int dtls_post_encryption_processing(OSSL_RECORD_LAYER *rl, size_t mac_size,
  ...
  if (!tls_post_encryption_processing_default(rl, mac_size, thistempl, thispkt,
                                              thiswr)) {
    /* RLAYERfatal() already called */
    return 0;
  }
...
static size_t dtls_get_max_record_overhead(OSSL_RECORD_LAYER *rl) {
   ...
   */
  assert(rl->enc_ctx == NULL ||
         ((blocksize == 0 || rl->eivlen > 0) && rl->taglen > 0));
