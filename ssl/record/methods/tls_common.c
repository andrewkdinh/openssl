...
int tls_do_compress(OSSL_RECORD_LAYER *rl, TLS_RL_RECORD *wr)
{
    ...
    i = COMP_compress_block(rl->compctx,
                            wr->data,
...
int tls_read_record(OSSL_RECORD_LAYER    *rl,
                    void                **rechandle,
                    int                  *rversion,
                    uint8_t              *type,
...
int tls_int_new_record_layer(OSSL_LIB_CTX        *libctx,
                             const char          *propq,
                             int                  vers,
                             int                  role,
                             int                  direction,
...
static int tls_new_record_layer(OSSL_LIB_CTX        *libctx,
                                const char          *propq,
                                int                  vers,
                                int                  role,
                                int                  direction,
                                ...
                                OSSL_RECORD_LAYER  **retrl)
{
    ...
    ret = tls_int_new_record_layer(libctx,
                                   propq,
                                   vers,
                                   role,
                                   direction,
    ...
    ret = (*retrl)->funcs->set_crypto_state(*retrl,
                                            level,
                                            key,
                                            keylen,
                                            iv,
...
int tls_retry_write_records(OSSL_RECORD_LAYER *rl)
{
    ...
    for (;;) {
        ...
        if (rl->bio != NULL) {
            ...
            i = BIO_write(rl->bio,
                          (char *)&(TLS_BUFFER_get_buf(thiswb)[TLS_BUFFER_get_offset(thiswb)]),
