...
static int tls_new_record_layer(
    ...
    unsigned char *mackey, size_t mackeylen, const EVP_CIPHER *ciph,
                                           ...
                                           TLS_RL_RECORD *thiswr)
{
    ...
    if (!WPACKET_get_length(thispkt, &origlen)
        ...
        || (thiswr->length > origlen
            && !WPACKET_allocate_bytes(thispkt, thiswr->length - origlen,
                                       NULL))) {
        RLAYERfatal(rl, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        return 0;
    }
                              ...
                              OSSL_RECORD_TEMPLATE *templates, size_t numtempl)
{
    ...
    ret = 1;
err:
    for (j = 0; j < wpinited; j++) WPACKET_cleanup(&pkt[j]);
    return ret;
}
