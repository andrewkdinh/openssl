...
int tls_setup_write_buffer(OSSL_RECORD_LAYER *rl, size_t numwpipes,
    ...
    if (firstlen == 0 || (numwpipes > 1 && nextlen == 0)) {
        ...
        if ((rl->options & SSL_OP_DONT_INSERT_EMPTY_FRAGMENTS) == 0)
            defltlen +=
            headerlen + maxalign + SSL3_RT_SEND_MAX_ENCRYPTED_OVERHEAD;
    }
...
int tls_setup_read_buffer(OSSL_RECORD_LAYER *rl) {
    ...
    if (b->buf == NULL) {
        len = rl->max_frag_len + SSL3_RT_MAX_ENCRYPTED_OVERHEAD + headerlen +
              maxalign;
#ifndef OPENSSL_NO_COMP
        if (tls_allow_compression(rl))
...
static int rlayer_early_data_count_ok(OSSL_RECORD_LAYER *rl, size_t length,
    ...
    if (max_early_data == 0) {
        RLAYERfatal(rl,
                    send ? SSL_AD_INTERNAL_ERROR : SSL_AD_UNEXPECTED_MESSAGE,
...
int tls_get_more_records(OSSL_RECORD_LAYER *rl) {
    ...
    do {
        ...
        if ((rl->rstate != SSL_ST_READ_BODY) ||
            ...
            if (rl->role == OSSL_RECORD_ROLE_SERVER && rl->is_first_record &&
                ...
                if (thisrr->length >
                    TLS_BUFFER_get_len(rbuf) - SSL2_RT_HEADER_LENGTH) {
                ...
                if (rl->msg_callback != NULL)
                    rl->msg_callback(0, version, SSL3_RT_HEADER, p, 5,
                                     rl->cbarg);
    ...
    if (enc_err == 0) {
        ...
        if (num_recs == 1 && rl->skip_early_data != NULL &&
            ...
            if (!rlayer_early_data_count_ok(
                rl, thisrr->length, EARLY_DATA_CIPHERTEXT_OVERHEAD, 0)) {
                /* RLAYERfatal() already called */
                goto end;
            }
...
int tls_set_options(OSSL_RECORD_LAYER *rl, const OSSL_PARAM *options) {
    ...
    p =
    OSSL_PARAM_locate_const(options, OSSL_LIBSSL_RECORD_LAYER_PARAM_OPTIONS);
    ...
    if (rl->direction == OSSL_RECORD_DIRECTION_READ) {
        ...
        options, OSSL_LIBSSL_RECORD_LAYER_PARAM_BLOCK_PADDING);
    ...
    *retrl = NULL;
    ...
    if (settings != NULL) {
        for (p = settings; p->key != NULL; p++) {
            ...
            } else if (strcmp(p->key, OSSL_LIBSSL_RECORD_LAYER_PARAM_TLSTREE) ==
                       0) {
...
void *cbarg, void *rlarg, OSSL_RECORD_LAYER **retrl) {
    ...
    ret = tls_int_new_record_layer(libctx, propq, vers, role, direction, level,
                                   ciph, taglen, md, comp, prev, transport,
...
int tls_retry_write_records(OSSL_RECORD_LAYER *rl) {
    ...
    for (;;) {
        ...
        if (rl->bio != NULL) {
            ...
            i = BIO_write(rl->bio,
                          (char *)&(TLS_BUFFER_get_buf(
                          thiswb)[TLS_BUFFER_get_offset(thiswb)]),
