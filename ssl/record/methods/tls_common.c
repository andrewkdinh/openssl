...
int ossl_set_tls_provider_parameters(OSSL_RECORD_LAYER *rl, EVP_CIPHER_CTX *ctx,
  ...
  *pprm++ =
  OSSL_PARAM_construct_int(OSSL_CIPHER_PARAM_TLS_VERSION, &rl->version);
  *pprm++ =
  OSSL_PARAM_construct_size_t(OSSL_CIPHER_PARAM_TLS_MAC_SIZE, &macsize);
...
int tls_setup_read_buffer(OSSL_RECORD_LAYER *rl) {
  ...
  if (b->buf == NULL) {
    len =
    rl->max_frag_len + SSL3_RT_MAX_ENCRYPTED_OVERHEAD + headerlen + maxalign;
#ifndef OPENSSL_NO_COMP
    if (tls_allow_compression(rl))
...
int tls_get_more_records(OSSL_RECORD_LAYER *rl) {
  ...
  do {
    ...
    if ((rl->rstate != SSL_ST_READ_BODY) ||
      ...
      rret =
      rl->funcs->read_n(rl, SSL3_RT_HEADER_LENGTH, TLS_BUFFER_get_len(rbuf), 0,
                        num_recs == 0 ? 1 : 0, &n);
       ...
       */
      if (rl->role == OSSL_RECORD_ROLE_SERVER && rl->is_first_record &&
    ...
    rl->is_first_record = 0;
           ...
           RLAYER_USE_EXPLICIT_IV(rl) && rl->enc_ctx != NULL &&
  ...
  if (enc_err == 0) {
    ...
    if (num_recs == 1 && rl->skip_early_data != NULL &&
...
int tls13_common_post_process_record(OSSL_RECORD_LAYER *rl,
                                     TLS_RL_RECORD *rec) {
  if (rec->type != SSL3_RT_APPLICATION_DATA && rec->type != SSL3_RT_ALERT &&
  ...
  if (rl->msg_callback != NULL)
    rl->msg_callback(0, rl->version, SSL3_RT_INNER_CONTENT_TYPE, &rec->type, 1,
                     rl->cbarg);
...
int tls_set_options(OSSL_RECORD_LAYER *rl, const OSSL_PARAM *options) {
  ...
  if (rl->direction == OSSL_RECORD_DIRECTION_READ) {
    p =
    OSSL_PARAM_locate_const(options, OSSL_LIBSSL_RECORD_LAYER_READ_BUFFER_LEN);
    ...
    p =
    OSSL_PARAM_locate_const(options, OSSL_LIBSSL_RECORD_LAYER_PARAM_HS_PADDING);
  ...
  if (rl->level == OSSL_RECORD_PROTECTION_LEVEL_APPLICATION) {
     ...
     */
    p =
    OSSL_PARAM_locate_const(options, OSSL_LIBSSL_RECORD_LAYER_PARAM_READ_AHEAD);
...
const OSSL_PARAM *settings, const OSSL_PARAM *options, const OSSL_DISPATCH *fns,
void *cbarg, OSSL_RECORD_LAYER **retrl) {
  ...
  *retrl = NULL;
  ...
  if (settings != NULL) {
    for (p = settings; p->key != NULL; p++) {
      ...
      } else if (strcmp(p->key, OSSL_LIBSSL_RECORD_LAYER_PARAM_MAX_FRAG_LEN) ==
                 0) {
      ...
      } else if (strcmp(p->key, OSSL_LIBSSL_RECORD_LAYER_PARAM_STREAM_MAC) ==
                 0) {
  ...
  if ((rl->options & SSL_OP_DONT_INSERT_EMPTY_FRAGMENTS) == 0 &&
      rl->version <= TLS1_VERSION && !EVP_CIPHER_is_a(ciph, "NULL") &&
...
static int tls_new_record_layer(
OSSL_LIB_CTX *libctx, const char *propq, int vers, int role, int direction,
int level, uint16_t epoch, unsigned char *secret, size_t secretlen,
...
int mactype, const EVP_MD *md, COMP_METHOD *comp, const EVP_MD *kdfdigest,
BIO *prev, BIO *transport, BIO *next, BIO_ADDR *local, BIO_ADDR *peer,
const OSSL_PARAM *settings, const OSSL_PARAM *options, const OSSL_DISPATCH *fns,
void *cbarg, void *rlarg, OSSL_RECORD_LAYER **retrl) {
  int ret;

  ret = tls_int_new_record_layer(libctx, propq, vers, role, direction, level,
                                 ciph, taglen, md, comp, prev, transport, next,
                                 settings, options, fns, cbarg, retrl);
  ...
  ret = (*retrl)->funcs->set_crypto_state(*retrl, level, key, keylen, iv, ivlen,
...
int tls_prepare_for_encryption_default(OSSL_RECORD_LAYER *rl, size_t mac_size,
   ...
   * MAC
   */
  if (!WPACKET_reserve_bytes(
      thispkt, SSL3_RT_SEND_MAX_ENCRYPTED_OVERHEAD - mac_size, NULL)
      /*
...
int tls_post_encryption_processing_default(OSSL_RECORD_LAYER *rl,
  ...
  size_t origlen, len;
  size_t headerlen =
  rl->isdtls ? DTLS1_RT_HEADER_LENGTH : SSL3_RT_HEADER_LENGTH;

  /* Allocate bytes for the encryption overhead */
  if (!WPACKET_get_length(thispkt, &origlen)
      /* Check we allowed enough room for the encryption growth */
      ||
      !ossl_assert(origlen + SSL3_RT_SEND_MAX_ENCRYPTED_OVERHEAD - mac_size >=
                   thiswr->length)
...
int tls_write_records_default(OSSL_RECORD_LAYER *rl,
  ...
  if (!rl->funcs->initialise_write_packets(
      rl, templates, numtempl, &prefixtempl, pkt, rl->wbuf, &wpinited)) {
    /* RLAYERfatal() already called */
    goto err;
  }
  ...
  for (j = 0; j < numtempl + prefix; j++) {
    ...
    if (rl->funcs->add_record_padding != NULL &&
        !rl->funcs->add_record_padding(rl, thistempl, thispkt, thiswr)) {
      /* RLAYERfatal() already called */
      goto err;
    }
  ...
  for (j = 0; j < numtempl + prefix; j++) {
    ...
    if (!rl->funcs->post_encryption_processing(rl, mac_size, thistempl, thispkt,
                                               thiswr)) {
      /* RLAYERfatal() already called */
      goto err;
    }
...
int tls_get_alert_code(OSSL_RECORD_LAYER *rl) { return rl->alert; }
...
int tls_free_buffers(OSSL_RECORD_LAYER *rl) {
  if (rl->direction == OSSL_RECORD_DIRECTION_WRITE) {
    if (rl->nextwbuf < rl->numwpipes) {
       ...
       */
      if (rl->nextwbuf != 0 || rl->numwpipes != 1 ||
  ...
  if (rl->curr_rec < rl->num_recs || rl->curr_rec != rl->num_released ||
      TLS_BUFFER_get_left(&rl->rbuf) != 0 || rl->rstate == SSL_ST_READ_BODY)
    return 0;
