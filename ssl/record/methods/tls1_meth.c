...
static int tls1_set_crypto_state(OSSL_RECORD_LAYER *rl, int level,
  ...
  if ((EVP_CIPHER_get_flags(ciph) & EVP_CIPH_FLAG_AEAD_CIPHER) == 0) {
    if (mactype == EVP_PKEY_HMAC) {
      mac_key = EVP_PKEY_new_raw_private_key_ex(rl->libctx, "HMAC", rl->propq,
                                                mackey, mackeylen);
    } else {
      /*
    ...
    if (mac_key == NULL ||
        EVP_DigestSignInit_ex(rl->md_ctx, NULL, EVP_MD_get0_name(md),
                              rl->libctx, rl->propq, mac_key, NULL) <= 0) {
  ...
  if (EVP_CIPHER_get_mode(ciph) == EVP_CIPH_GCM_MODE) {
    if (!EVP_CipherInit_ex(ciph_ctx, ciph, NULL, key, NULL, enc) ||
        EVP_CIPHER_CTX_ctrl(ciph_ctx, EVP_CTRL_GCM_SET_IV_FIXED, (int)ivlen,
                            iv) <= 0) {
      ERR_raise(ERR_LIB_SSL, ERR_R_INTERNAL_ERROR);
      return OSSL_RECORD_RETURN_FATAL;
    }
    ...
    if (!EVP_CipherInit_ex(ciph_ctx, ciph, NULL, NULL, NULL, enc) ||
        ...
        EVP_CIPHER_CTX_ctrl(ciph_ctx, EVP_CTRL_AEAD_SET_TAG, (int)taglen,
                            NULL) <= 0 ||
        EVP_CIPHER_CTX_ctrl(ciph_ctx, EVP_CTRL_CCM_SET_IV_FIXED, (int)ivlen,
                            iv) <= 0 ||
  ...
  if ((EVP_CIPHER_get_flags(ciph) & EVP_CIPH_FLAG_AEAD_CIPHER) != 0 &&
      mackeylen != 0 &&
      EVP_CIPHER_CTX_ctrl(ciph_ctx, EVP_CTRL_AEAD_SET_MAC_KEY, (int)mackeylen,
                          mackey) <= 0) {
    ERR_raise(ERR_LIB_SSL, ERR_R_INTERNAL_ERROR);
    return OSSL_RECORD_RETURN_FATAL;
  }
...
static int tls1_cipher(OSSL_RECORD_LAYER *rl, TLS_RL_RECORD *recs,
  ...
  if (n_recs > 1) {
    ...
    if (EVP_CIPHER_CTX_ctrl(ds, EVP_CTRL_SET_PIPELINE_OUTPUT_BUFS, (int)n_recs,
                            data) <= 0) {
      RLAYERfatal(rl, SSL_AD_INTERNAL_ERROR, SSL_R_PIPELINE_FAILURE);
      return 0;
    }
    ...
    if (EVP_CIPHER_CTX_ctrl(ds, EVP_CTRL_SET_PIPELINE_INPUT_BUFS, (int)n_recs,
                            data) <= 0 ||
        EVP_CIPHER_CTX_ctrl(ds, EVP_CTRL_SET_PIPELINE_INPUT_LENS, (int)n_recs,
                            reclen) <= 0) {
      RLAYERfatal(rl, SSL_AD_INTERNAL_ERROR, SSL_R_PIPELINE_FAILURE);
      return 0;
    }
    ...
    if (!sending) {
      for (ctr = 0; ctr < n_recs; ctr++) {
        ...
        if (!tls1_cbc_remove_padding_and_mac(
            &recs[ctr].length, recs[ctr].orig_len, recs[ctr].data,
            (macs != NULL) ? &macs[ctr].mac : NULL,
            (macs != NULL) ? &macs[ctr].alloced : NULL, bs,
            pad ? (size_t)pad : macsize,
            (EVP_CIPHER_get_flags(enc) & EVP_CIPH_FLAG_AEAD_CIPHER) != 0,
            rl->libctx))
          return 0;
      }
    }
...
static int tls1_mac(OSSL_RECORD_LAYER *rl, TLS_RL_RECORD *rec,
  ...
  if (!sending && !rl->use_etm &&
    ...
    *p++ =
    OSSL_PARAM_construct_size_t(OSSL_MAC_PARAM_TLS_DATA_SIZE, &rec->orig_len);
    *p++ = OSSL_PARAM_construct_end();
...
int tls1_allocate_write_buffers(OSSL_RECORD_LAYER *rl,
  ...
  *prefix =
  rl->need_empty_fragments && templates[0].type == SSL3_RT_APPLICATION_DATA;
...
int tls1_initialise_write_packets(OSSL_RECORD_LAYER *rl,
  ...
  rl, templates, numtempl, NULL, pkt + prefix, bufs + prefix, wpinited);
}
...
tls1_set_crypto_state, tls1_cipher, tls1_mac, tls_default_set_protocol_version,
tls_default_read_n, dtls_get_more_records, NULL, NULL, NULL,
...
tls_initialise_write_packets_default, NULL, dtls_prepare_record_header, NULL,
tls_prepare_for_encryption_default, dtls_post_encryption_processing, NULL};
