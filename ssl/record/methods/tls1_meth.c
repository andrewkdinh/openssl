    ...
    if ((EVP_CIPHER_get_flags(ciph) & EVP_CIPH_FLAG_AEAD_CIPHER) == 0) {
        ...
        if (mac_key == NULL
            || EVP_DigestSignInit_ex(rl->md_ctx, NULL, EVP_MD_get0_name(md),
                                     rl->libctx, rl->propq, mac_key, NULL)
                <= 0) {
                       ...
                       size_t macsize)
{
        ...
        if (ivlen > 1) {
            for (ctr = 0; ctr < n_recs; ctr++) {
                if (recs[ctr].data != recs[ctr].input) {
                    ...
                    return 0;
                } else if (RAND_bytes_ex(rl->libctx, recs[ctr].input, ivlen, 0)
                           <= 0) {
                    RLAYERfatal(rl, SSL_AD_INTERNAL_ERROR,
                                ERR_R_INTERNAL_ERROR);
                    return 0;
    ...
    if (n_recs > 1) {
        ...
        for (ctr = 0; ctr < n_recs; ctr++) data[ctr] = recs[ctr].data;
        ...
        for (ctr = 0; ctr < n_recs; ctr++) data[ctr] = recs[ctr].input;
...
const struct record_functions_st dtls_1_funcs = {
    ...
    tls_default_set_protocol_version, tls_default_read_n, dtls_get_more_records,
    NULL, NULL, NULL, tls_write_records_default,
    /*
    ...
    NULL, tls_prepare_for_encryption_default, dtls_post_encryption_processing,
    NULL};
