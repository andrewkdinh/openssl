...
static int tls1_set_crypto_state(OSSL_RECORD_LAYER *rl, int level,
    ...
    if ((EVP_CIPHER_get_flags(ciph) & EVP_CIPH_FLAG_AEAD_CIPHER) == 0) {
        if (mactype == EVP_PKEY_HMAC) {
            ...
            rl->libctx, "HMAC", rl->propq, mackey, mackeylen);
        } else {
            /*
             ...
             */
            mac_key =
            EVP_PKEY_new_mac_key(mactype, NULL, mackey, (int)mackeylen);
        }
        if (mac_key == NULL ||
    ...
    if (EVP_CIPHER_get_mode(ciph) == EVP_CIPH_GCM_MODE) {
        ...
        if (!EVP_CipherInit_ex(ciph_ctx, ciph, NULL, NULL, NULL, enc) ||
            EVP_CIPHER_CTX_ctrl(ciph_ctx, EVP_CTRL_AEAD_SET_IVLEN, 12, NULL) <=
            0 ||
...
static int tls1_cipher(OSSL_RECORD_LAYER *rl, TLS_RL_RECORD *recs,
        ...
        if (ivlen > 1) {
            for (ctr = 0; ctr < n_recs; ctr++) {
                if (recs[ctr].data != recs[ctr].input) {
                    ...
                    return 0;
                } else if (RAND_bytes_ex(rl->libctx, recs[ctr].input, ivlen,
                                         0) <= 0) {
                    RLAYERfatal(rl, SSL_AD_INTERNAL_ERROR,
                                ERR_R_INTERNAL_ERROR);
                    return 0;
    ...
    if (provided) {
        ...
        if (!sending) {
            ...
            if (macs != NULL) {
                ...
                *p++ = OSSL_PARAM_construct_octet_ptr(
                OSSL_CIPHER_PARAM_TLS_MAC, (void **)&macs[0].mac, macsize);
        ...
        tmpr =
        EVP_Cipher(ds, recs[0].data, recs[0].input, (unsigned int)reclen[0]);
        if ((EVP_CIPHER_get_flags(EVP_CIPHER_CTX_get0_cipher(ds)) &
