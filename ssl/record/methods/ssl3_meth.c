...
static int ssl3_cipher(OSSL_RECORD_LAYER *rl, TLS_RL_RECORD *inrecs,
  ...
  if (provided) {
    ...
    if (!sending)
      ...
      &rec->length, rec->orig_len, rec->data, (mac != NULL) ? &mac->mac : NULL,
...
static int ssl3_mac(OSSL_RECORD_LAYER *rl, TLS_RL_RECORD *rec,
  ...
  if (!sending && EVP_CIPHER_CTX_get_mode(rl->enc_ctx) == EVP_CIPH_CBC_MODE &&
    ...
    if (ssl3_cbc_digest_record(EVP_MD_CTX_get0_md(hash), md, &md_size, header,
                               rec->input, rec->length, rec->orig_len, mac_sec,
                               md_size, 1) <= 0)
      return 0;
#endif
  } else {
...
ssl3_set_crypto_state, ssl3_cipher, ssl3_mac, tls_default_set_protocol_version,
tls_default_read_n, tls_get_more_records, tls_default_validate_record_header,
...
tls_prepare_record_header_default, NULL, tls_prepare_for_encryption_default,
