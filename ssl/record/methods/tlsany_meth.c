...
static int tls_any_set_crypto_state(OSSL_RECORD_LAYER *rl, int level, unsigned char *key, size_t keylen,
...
static int tls_any_cipher(OSSL_RECORD_LAYER *rl, TLS_RL_RECORD *recs, size_t n_recs, int sending, SSL_MAC_BUF *macs,
                          size_t macsize)
{
    return 1;
}
        ...
        if (rl->version == TLS_ANY_VERSION) {
            if ((rec->rec_version >> 8) != SSL3_VERSION_MAJOR) {
                if (rl->is_first_record) {
                     ...
                     */
                    p = rl->packet;
                    if (HAS_PREFIX((char *)p, "GET ") || HAS_PREFIX((char *)p, "POST ")
                        || HAS_PREFIX((char *)p, "HEAD ") || HAS_PREFIX((char *)p, "PATCH")
                        || HAS_PREFIX((char *)p, "OPTIO") || HAS_PREFIX((char *)p, "DELET")
                        || HAS_PREFIX((char *)p, "TRACE") || HAS_PREFIX((char *)p, "PUT ")) {
                        RLAYERfatal(rl, SSL_AD_NO_ALERT, SSL_R_HTTP_REQUEST);
                        return 0;
                    } else if (HAS_PREFIX((char *)p, "CONNE")) {
                        RLAYERfatal(rl, SSL_AD_NO_ALERT, SSL_R_HTTPS_PROXY_REQUEST);
                        return 0;
                    }
...
static int tls_any_prepare_for_encryption(OSSL_RECORD_LAYER *rl, size_t mac_size, WPACKET *thispkt,
...
const struct record_functions_st tls_any_funcs = {tls_any_set_crypto_state,
                                                  tls_any_cipher,
                                                  NULL,
...
const struct record_functions_st dtls_any_funcs = {tls_any_set_crypto_state,
                                                   tls_any_cipher,
                                                   NULL,
