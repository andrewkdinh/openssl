 ...
 */
static int ktls_int_check_supported_cipher(OSSL_RECORD_LAYER *rl, const EVP_CIPHER *c, const EVP_MD *md, size_t taglen)
{
    switch (rl->version) {
    case TLS1_VERSION:
    case TLS1_1_VERSION:
...
# endif
    ...
    if (EVP_CIPHER_is_a(c, "AES-128-GCM") || EVP_CIPHER_is_a(c, "AES-256-GCM")
...
# endif
    ...
    if (EVP_MD_is_a(md, "SHA1") || EVP_MD_is_a(md, "SHA2-256") || EVP_MD_is_a(md, "SHA2-384"))
        return 1;
...
static int ktls_configure_crypto(OSSL_LIB_CTX *libctx, int version, const EVP_CIPHER *c, EVP_MD *md, void *rl_sequence,
                                 ktls_crypto_info_t *crypto_info, int is_tx, unsigned char *iv, size_t ivlen,
                                 unsigned char *key, size_t keylen, unsigned char *mac_key, size_t mac_secret_size)
{
...
/* Function to configure kernel TLS structure */
static int ktls_configure_crypto(OSSL_LIB_CTX *libctx, int version, const EVP_CIPHER *c, const EVP_MD *md,
                                 void *rl_sequence, ktls_crypto_info_t *crypto_info, int is_tx, unsigned char *iv,
                                 size_t ivlen, unsigned char *key, size_t keylen, unsigned char *mac_key,
...
# endif
    ...
    if (EVP_CIPHER_get_mode(c) == EVP_CIPH_GCM_MODE || EVP_CIPHER_get_mode(c) == EVP_CIPH_CCM_MODE) {
        if (!ossl_assert(EVP_GCM_TLS_FIXED_IV_LEN == EVP_CCM_TLS_FIXED_IV_LEN)
            || !ossl_assert(EVP_GCM_TLS_EXPLICIT_IV_LEN == EVP_CCM_TLS_EXPLICIT_IV_LEN))
            return 0;
        if (version == TLS1_2_VERSION) {
            if (!ossl_assert(ivlen == EVP_GCM_TLS_FIXED_IV_LEN))
                return 0;
            if (is_tx) {
                if (RAND_bytes_ex(libctx, geniv, EVP_GCM_TLS_EXPLICIT_IV_LEN, 0) <= 0)
                    return 0;
            } else {
...
static int ktls_set_crypto_state(OSSL_RECORD_LAYER *rl, int level, unsigned char *key, size_t keylen, unsigned char *iv,
                                 ...
                                 size_t taglen, int mactype, const EVP_MD *md, COMP_METHOD *comp)
{
    ...
    if (!ktls_configure_crypto(rl->libctx, rl->version, ciph, md, rl->sequence, &crypto_info,
                               rl->direction == OSSL_RECORD_DIRECTION_WRITE, iv, ivlen, key, keylen, mackey, mackeylen))
        return OSSL_RECORD_RETURN_NON_FATAL_ERR;
...
static int ktls_read_n(OSSL_RECORD_LAYER *rl, size_t n, size_t max, int extend, int clearold, size_t *readbytes)
{
    int ret;
...
static int ktls_cipher(OSSL_RECORD_LAYER *rl, TLS_RL_RECORD *inrecs, size_t n_recs, int sending, SSL_MAC_BUF *mac,
                       size_t macsize)
{
    return 1;
}
...
static int ktls_new_record_layer(OSSL_LIB_CTX *libctx, const char *propq, int vers, int role, int direction, int level,
...
static int ktls_allocate_write_buffers(OSSL_RECORD_LAYER *rl, OSSL_RECORD_TEMPLATE *templates, size_t numtempl,
                                       size_t *prefix)
{
...
static int ktls_initialise_write_packets(OSSL_RECORD_LAYER *rl, OSSL_RECORD_TEMPLATE *templates, size_t numtempl,
...
static int ktls_prepare_record_header(OSSL_RECORD_LAYER *rl, WPACKET *thispkt, OSSL_RECORD_TEMPLATE *templ,
...
static int ktls_prepare_for_encryption(OSSL_RECORD_LAYER *rl, size_t mac_size, WPACKET *thispkt, TLS_RL_RECORD *thiswr)
{
    /* No encryption, so nothing to do */
    return 1;
}

static int ktls_post_encryption_processing(OSSL_RECORD_LAYER *rl, size_t mac_size, OSSL_RECORD_TEMPLATE *templ,
...
static struct record_functions_st ossl_ktls_funcs = {ktls_set_crypto_state,
                                                     ktls_cipher,
                                                     NULL,
