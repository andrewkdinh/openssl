...
start:
  ...
  if (sc->rlayer.curr_rec >= sc->rlayer.num_recs) {
    ...
    do {
      ...
      sc, sc->rlayer.rrlmethod->read_record(
          sc->rlayer.rrl, &rr->rechandle, &rr->version, &rr->type, &rr->data,
  ...
  if (rr->type == SSL3_RT_ALERT) {
    ...
    if (alert_level == SSL3_AL_WARNING) {
      ...
      SSLfatal_data(sc, SSL_AD_NO_ALERT, SSL_AD_REASON_OFFSET + alert_descr,
  ...
  case SSL3_RT_APPLICATION_DATA:
     ...
     */
    if (sc->s3.in_read_app_data && (sc->s3.total_renegotiations != 0) &&
...
int do_dtls1_write(SSL_CONNECTION *sc, uint8_t type, const unsigned char *buf,
  ...
  ret = HANDLE_RLAYER_WRITE_RETURN(
  sc, sc->rlayer.wrlmethod->write_records(sc->rlayer.wrl, &tmpl, 1));
