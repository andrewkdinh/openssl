                    ...
                    size_t olen, int fatal)
{
    ...
    kdf =
        EVP_KDF_fetch(SSL_CONNECTION_GET_CTX(s)->libctx, OSSL_KDF_NAME_TLS1_PRF,
...
int tls1_change_cipher_state(SSL_CONNECTION *s, int which)
{
    ...
    if (!ssl_set_new_record_layer(
            ...
            NULL, 0, key, cl, iv, (size_t)k, mac_secret, mac_secret_size, c,
            taglen, mac_type, m, comp, NULL)) {
                                ...
                                int use_context)
{
     ...
     */
    if (memcmp(val, TLS_MD_CLIENT_FINISH_CONST, TLS_MD_CLIENT_FINISH_CONST_SIZE)
        == 0)
        goto err1;
    if (memcmp(val, TLS_MD_SERVER_FINISH_CONST, TLS_MD_SERVER_FINISH_CONST_SIZE)
        == 0)
        goto err1;
    if (memcmp(val, TLS_MD_MASTER_SECRET_CONST, TLS_MD_MASTER_SECRET_CONST_SIZE)
        == 0)
        goto err1;
    if (memcmp(val, TLS_MD_EXTENDED_MASTER_SECRET_CONST,
        ...
        == 0)
        goto err1;
    if (memcmp(val, TLS_MD_KEY_EXPANSION_CONST, TLS_MD_KEY_EXPANSION_CONST_SIZE)
        == 0)
        goto err1;
