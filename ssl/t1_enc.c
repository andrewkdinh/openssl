...
static int tls1_PRF(SSL_CONNECTION *s, const void *seed1, size_t seed1_len,
  ...
  kdf = EVP_KDF_fetch(SSL_CONNECTION_GET_CTX(s)->libctx, OSSL_KDF_NAME_TLS1_PRF,
  ...
  *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SECRET,
                                           (unsigned char *)sec, (size_t)slen);
  *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SEED, (void *)seed1,
                                           (size_t)seed1_len);
  *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SEED, (void *)seed2,
                                           (size_t)seed2_len);
  *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SEED, (void *)seed3,
                                           (size_t)seed3_len);
  *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SEED, (void *)seed4,
                                           (size_t)seed4_len);
  *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SEED, (void *)seed5,
                                           (size_t)seed5_len);
...
static int tls1_generate_key_block(SSL_CONNECTION *s, unsigned char *km,
  ...
  ret = tls1_PRF(s, TLS_MD_KEY_EXPANSION_CONST, TLS_MD_KEY_EXPANSION_CONST_SIZE,
...
int tls1_change_cipher_state(SSL_CONNECTION *s, int which) {
  ...
  if (!ssl_set_new_record_layer(
      s, s->version, direction, OSSL_RECORD_PROTECTION_LEVEL_APPLICATION, NULL,
      0, key, cl, iv, (size_t)k, mac_secret, mac_secret_size, c, taglen,
...
int tls1_setup_key_block(SSL_CONNECTION *s) {
  ...
  if (!ssl_cipher_get_evp(SSL_CONNECTION_GET_CTX(s), s->session, &c, &hash,
                          &mac_type, &mac_secret_size, &comp, s->ext.use_etm)) {
...
int tls1_generate_master_secret(SSL_CONNECTION *s, unsigned char *out,
  ...
  if (s->session->flags & SSL_SESS_FLAG_EXTMS) {
    ...
    if (!tls1_PRF(s, TLS_MD_EXTENDED_MASTER_SECRET_CONST,
                  TLS_MD_EXTENDED_MASTER_SECRET_CONST_SIZE, hash, hashlen, NULL,
                  0, NULL, 0, NULL, 0, p, len, out, SSL3_MASTER_SECRET_SIZE,
                  1)) {
      /* SSLfatal() already called */
      return 0;
    }
    ...
    if (!tls1_PRF(
        s, TLS_MD_MASTER_SECRET_CONST, TLS_MD_MASTER_SECRET_CONST_SIZE,
        s->s3.client_random, SSL3_RANDOM_SIZE, NULL, 0, s->s3.server_random,
  ...
  OSSL_TRACE_BEGIN(TLS) {
    ...
    BIO_dump_indent(trc_out, s->session->master_key, SSL3_MASTER_SECRET_SIZE,
                    4);
  }
  OSSL_TRACE_END(TLS);
...
int tls1_export_keying_material(SSL_CONNECTION *s, unsigned char *out,
  ...
  rv =
  tls1_PRF(s, val, vallen, NULL, 0, NULL, 0, NULL, 0, NULL, 0,
           s->session->master_key, s->session->master_key_length, out, olen, 0);
