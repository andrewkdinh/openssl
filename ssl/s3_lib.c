...
static int sslcon_undefined_function_1(SSL_CONNECTION *sc, unsigned char *r,
                                       ...
                                       const unsigned char *v, size_t w,
                                       int x) {
  (void)r;
  (void)s;
  (void)t;
...
long ssl3_ctrl(SSL *s, int cmd, long larg, void *parg) {
  ...
  case SSL_CTRL_SET_TMP_ECDH: {
    ...
    &sc->ext.supportedgroups, &sc->ext.supportedgroups_len, &sc->ext.keyshares,
...
long ssl3_ctx_ctrl(SSL_CTX *ctx, int cmd, long larg, void *parg) {
  ...
  case SSL_CTRL_GET_TLSEXT_TICKET_KEYS: {
    ...
    long tick_keylen =
    (sizeof(ctx->ext.tick_key_name) + sizeof(ctx->ext.secure->tick_hmac_key) +
  ...
  case SSL_CTRL_SET_GROUPS:
    return tls1_set_groups(&ctx->ext.supportedgroups,
                           &ctx->ext.supportedgroups_len, &ctx->ext.keyshares,
                           &ctx->ext.keyshares_len, &ctx->ext.tuples,
                           &ctx->ext.tuples_len, parg, larg);

  case SSL_CTRL_SET_GROUPS_LIST:
    return tls1_set_groups_list(ctx, &ctx->ext.supportedgroups,
                                &ctx->ext.supportedgroups_len,
                                &ctx->ext.keyshares, &ctx->ext.keyshares_len,
                                &ctx->ext.tuples, &ctx->ext.tuples_len, parg);

  case SSL_CTRL_GET0_IMPLEMENTED_GROUPS:
    return tls1_get0_implemented_groups(ctx->min_proto_version,
                                        ctx->max_proto_version, ctx->group_list,
...
long ssl3_ctx_callback_ctrl(SSL_CTX *ctx, int cmd, void (*fp)(void)) {
  ...
  case SSL_CTRL_SET_TLSEXT_TICKET_KEY_CB:
    ctx->ext.ticket_key_cb = (int (*)(SSL *, unsigned char *, unsigned char *,
                                      EVP_CIPHER_CTX *, HMAC_CTX *, int))fp;
    break;
#endif
