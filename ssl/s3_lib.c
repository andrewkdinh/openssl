...
long ssl3_ctrl(SSL *s, int cmd, long larg, void *parg)
{
    ...
    case SSL_CTRL_SET_TMP_ECDH: {
        ...
        return ssl_set_tmp_ecdh_groups(
            &sc->ext.supportedgroups, &sc->ext.supportedgroups_len,
            &sc->ext.keyshares, &sc->ext.keyshares_len, &sc->ext.tuples,
            &sc->ext.tuples_len, parg);
    }
    ...
    case SSL_CTRL_SET_GROUPS:
        return tls1_set_groups(&sc->ext.supportedgroups,
                               &sc->ext.supportedgroups_len, &sc->ext.keyshares,
                               &sc->ext.keyshares_len, &sc->ext.tuples,
                               &sc->ext.tuples_len, parg, larg);

    case SSL_CTRL_SET_GROUPS_LIST:
        return tls1_set_groups_list(s->ctx, &sc->ext.supportedgroups,
                                    &sc->ext.supportedgroups_len,
                                    &sc->ext.keyshares, &sc->ext.keyshares_len,
                                    &sc->ext.tuples, &sc->ext.tuples_len, parg);
...
long ssl3_ctx_ctrl(SSL_CTX *ctx, int cmd, long larg, void *parg)
{
    ...
    case SSL_CTRL_SET_TMP_ECDH: {
        ...
        return ssl_set_tmp_ecdh_groups(
            &ctx->ext.supportedgroups, &ctx->ext.supportedgroups_len,
            &ctx->ext.keyshares, &ctx->ext.keyshares_len, &ctx->ext.tuples,
            &ctx->ext.tuples_len, parg);
    }
#endif                          /* !OPENSSL_NO_DEPRECATED_3_0 */
    ...
    case SSL_CTRL_SET_GROUPS:
        return tls1_set_groups(
            &ctx->ext.supportedgroups, &ctx->ext.supportedgroups_len,
            &ctx->ext.keyshares, &ctx->ext.keyshares_len, &ctx->ext.tuples,
            &ctx->ext.tuples_len, parg, larg);

    case SSL_CTRL_SET_GROUPS_LIST:
        return tls1_set_groups_list(
            ctx, &ctx->ext.supportedgroups, &ctx->ext.supportedgroups_len,
            &ctx->ext.keyshares, &ctx->ext.keyshares_len, &ctx->ext.tuples,
            &ctx->ext.tuples_len, parg);

    case SSL_CTRL_GET0_IMPLEMENTED_GROUPS:
        return tls1_get0_implemented_groups(
            ctx->min_proto_version, ctx->max_proto_version, ctx->group_list,
...
const SSL_CIPHER *ssl3_get_cipher_by_std_name(const char *stdname)
{
    ...
    size_t i, j,
        tblsize[] = {TLS13_NUM_CIPHERS, SSL3_NUM_CIPHERS, SSL3_NUM_SCSVS};
 ...
 */
const SSL_CIPHER *ssl3_choose_cipher(SSL_CONNECTION *s,
    ...
    for (i = 0; i < sk_SSL_CIPHER_num(prio); i++) {
        ...
        if (!SSL_CONNECTION_IS_TLS13(s)) {
            ...
            OSSL_TRACE7(TLS_CIPHER, "%d:[%08lX:%08lX:%08lX:%08lX]%p:%s\n", ok,
                        alg_k, alg_a, mask_k, mask_a, (void *)c, c->name);
        ...
        if (ii >= 0) {
            ...
            if (prefer_sha256) {
                ...
                const EVP_MD *md =
                    ssl_md(SSL_CONNECTION_GET_CTX(s), tmp->algorithm2);
 ...
 */
int ssl_fill_hello_random(SSL_CONNECTION *s, int server, unsigned char *result,
...
EVP_PKEY *ssl_generate_pkey_group(SSL_CONNECTION *s, uint16_t id)
{
    ...
    pctx =
        EVP_PKEY_CTX_new_from_name(sctx->libctx, ginf->algorithm, sctx->propq);
...
int ssl_gensecret(SSL_CONNECTION *s, unsigned char *pms, size_t pmslen)
{
    ...
    if (SSL_CONNECTION_IS_TLS13(s)) {
         ...
         */
        if (!s->hit)
            rv = tls13_generate_secret(s, ssl_handshake_md(s), NULL, NULL, 0,
...
int ssl_derive(SSL_CONNECTION *s, EVP_PKEY *privkey, EVP_PKEY *pubkey,
               int gensecret)
{
    int rv = 0;
...
int ssl_decapsulate(SSL_CONNECTION *s, EVP_PKEY *privkey,
                    const unsigned char *ct, size_t ctlen, int gensecret)
{
    int rv = 0;
    unsigned char *pms = NULL;
    size_t pmslen = 0;
...
int ssl_encapsulate(SSL_CONNECTION *s, EVP_PKEY *pubkey, unsigned char **ctp,
                    size_t *ctlenp, int gensecret)
{
    int rv = 0;
