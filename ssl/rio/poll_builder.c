...
int ossl_rio_poll_builder_poll(RIO_POLL_BUILDER *rpb, OSSL_TIME deadline) {
    ...
    do {
        ...
        if (ossl_time_is_infinite(deadline))
             ...
             */
            timeout =
            ossl_time_to_timeval(ossl_time_subtract(deadline, ossl_time_now()));

        rc =
        select(rpb->hwm_fd + 1, &rpb->rfd, &rpb->wfd, &rpb->efd, p_timeout);
    ...
    do {
        ...
        if (ossl_time_is_infinite(deadline))
            ...
            timeout_ms =
            ossl_time2ms(ossl_time_subtract(deadline, ossl_time_now()));
