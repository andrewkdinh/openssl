...
ASN1_SEQUENCE(SSL_SESSION_ASN1)
    = {ASN1_EMBED(SSL_SESSION_ASN1, version, UINT32),
       ...
       ASN1_EXP_OPT_EMBED(SSL_SESSION_ASN1, tlsext_max_fragment_len_mode,
                          ZUINT32, 17),
       ASN1_EXP_OPT(SSL_SESSION_ASN1, ticket_appdata, ASN1_OCTET_STRING, 18),
       ASN1_EXP_OPT_EMBED(SSL_SESSION_ASN1, kex_group, UINT32, 19),
       ASN1_EXP_OPT(SSL_SESSION_ASN1, peer_rpk, ASN1_OCTET_STRING,
...
/* Initialise OCTET STRING from buffer and length */
        ...
        static void
        ssl_session_oinit(ASN1_OCTET_STRING * *dest, ASN1_OCTET_STRING * os,
                          const unsigned char *data, size_t len)
    {
        os->data =
            (unsigned char *)data; /* justified cast: data is not modified */
        os->length = (int)len;
        os->flags = 0;
        *dest = os;
    }
...
/* Initialise OCTET STRING from string */
    static void ssl_session_sinit(ASN1_OCTET_STRING * *dest,
                                  ASN1_OCTET_STRING * os, const char *data)
    {
        if (data != NULL)
...
/* Copy an OCTET STRING, return error if it exceeds maximum length */
    ...
    /* ASN.1 code returns suitable error */
         ...
         */
            ret->peer_rpk =
                d2i_PUBKEY_ex(NULL, &data, as->peer_rpk->length, libctx, propq);
