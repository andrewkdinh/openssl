...
extract_keyshares(SSL_CONNECTION *s, PACKET *key_share_list,
  ...
  *encoded_pubkey_arr =
  ...
  while (PACKET_remaining(key_share_list) > 0) {
     ...
     */
    if (s->s3.group_id != 0 &&
        (group_id != s->s3.group_id || PACKET_remaining(key_share_list) != 0)) {
    ...
    if (*keyshares_cnt == *keyshares_max) {
      PACKET *tmp_pkt;
      uint16_t *tmp =
      OPENSSL_realloc(*keyshares_arr, (*keyshares_max + GROUPLIST_INCREMENT) *
...
int tls_parse_ctos_key_share(SSL_CONNECTION *s, PACKET *pkt,
   ...
   * checks) */
  ks_extraction_result = extract_keyshares(
  s, &key_share_list, clntgroups, clnt_num_groups, srvrgroups, srvr_num_groups,
  &keyshares_arr, &encoded_pubkey_arr, &keyshares_cnt, &keyshares_max);
   ...
   * encoded_pubkey_arr[keyshares_cnt] From server: srvrgroups[srvr_num_groups],
   * srvrtuples[srvr_num_tuples]
   *
  ...
  for (current_tuple = 0; current_tuple < srvr_num_tuples; current_tuple++) {
    ...
    if (s->options & SSL_OP_CIPHER_SERVER_PREFERENCE) {
      ...
      if (group_id_candidate >
         ...
         */
        check_overlap(s, first_group_in_tuple, number_of_groups_in_tuple,
                      clntgroups, clnt_num_groups, &prio_group_idx,
                      &candidate_group_idx, &group_id_candidate);
      ...
      if (group_id_candidate > 0) {
        ...
        check_overlap(s, clntgroups, clnt_num_groups, first_group_in_tuple,
                      number_of_groups_in_tuple, &prio_group_idx,
                      &candidate_group_idx, &group_id_candidate);
        if (group_id_candidate > 0) {
          s->s3.group_id_candidate = group_id_candidate;
...
int tls_parse_ctos_psk(SSL_CONNECTION *s, PACKET *pkt, unsigned int context,
  ...
  for (id = 0; PACKET_remaining(&identities) != 0; id++) {
    ...
    if (s->psk_find_session_cb != NULL &&
        !s->psk_find_session_cb(ussl, PACKET_data(&identity), idlen, &sess)) {
    ...
    if (sess == NULL && s->psk_server_callback != NULL &&
      ...
      if (pskdatalen > PSK_MAX_PSK_LEN) {
         ...
         */
        cipher =
        SSL_CIPHER_find(SSL_CONNECTION_GET_SSL(s), tls13_aes128gcmsha256_id);
        if (cipher == NULL) {
    ...
    if (sess != NULL) {
      ...
      if (id == 0 && ossl_time_compare(sess->timeout, t) >= 0 &&
          ...
          ossl_time_compare(ossl_time_add(age, TICKET_AGE_ALLOWANCE), expire) >=
          0) {
        /*
    ...
    if (!EVP_MD_is_a(
        md, EVP_MD_get0_name(ssl_md(sctx, s->s3.tmp.new_cipher->algorithm2)))) {
...
int tls_parse_ctos_client_cert_type(SSL_CONNECTION *sc, PACKET *pkt,
  ...
  reconcile_cert_type(data, len, sc->client_cert_type, sc->client_cert_type_len,
...
int tls_parse_ctos_server_cert_type(SSL_CONNECTION *sc, PACKET *pkt,
  ...
  reconcile_cert_type(sc->server_cert_type, sc->server_cert_type_len, data, len,
