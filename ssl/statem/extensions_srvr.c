...
#define MAX_COOKIE_SIZE                                                        \
    (2 + 2 + 2 + 2 + 2 + 1 + 8 + 2 + EVP_MAX_MD_SIZE + 1 + SSL_COOKIE_LENGTH + \
...
int tls_parse_ctos_ec_pt_formats(SSL_CONNECTION *s, PACKET *pkt,
    ...
    if (!s->hit) {
        if (!PACKET_memdup(&ec_point_format_list, &s->ext.peer_ecpointformats,
...
int tls_parse_ctos_status_request(SSL_CONNECTION *s, PACKET *pkt,
    ...
    while (PACKET_remaining(&responder_id_list) > 0) {
        ...
        id =
        d2i_OCSP_RESPID(NULL, &id_data, (int)PACKET_remaining(&responder_id));
        if (id == NULL) {
            SSLfatal(s, SSL_AD_DECODE_ERROR, SSL_R_BAD_EXTENSION);
            return 0;
        }
...
/*
 * Use function tls_parse_ctos_key_share with helper functions
 * extract_keyshares, check_overlap and tls_accept_ksgroup to parse the
 * key_share extension(s) received in the ClientHello and to select the group
 * used of the key exchange
 */
 ...
 */
static int tls_accept_ksgroup(SSL_CONNECTION *s, uint16_t ksgroup,
    ...
    if (tls13_set_encoded_pub_key(s->s3.peer_tmp, PACKET_data(encoded_pubkey),
...
#define GROUPLIST_INCREMENT                                                    \
    32 /* Memory allocation chunk size (nominally 64 Bytes chunks) */
...
static KS_EXTRACTION_RESULT
extract_keyshares(SSL_CONNECTION *s, PACKET *key_share_list,
    ...
    *encoded_pubkey_arr =
    OPENSSL_malloc(*keyshares_max * sizeof(**encoded_pubkey_arr));
    ...
    while (PACKET_remaining(key_share_list) > 0) {
        ...
        if (!check_in_list(s, group_id, srvrgroups, srvr_num_groups, 1, NULL) ||
            ...
            !tls_valid_group(s, group_id, TLS1_3_VERSION, TLS1_3_VERSION, 0,
                             NULL)) {
 ...
 * assign to selected_group and also set the related index in the candidate
 * group list, or set selected_group to 0 if no overlap
 */
#ifndef OPENSSL_NO_TLS1_3
static void check_overlap(SSL_CONNECTION *s, const uint16_t *prio_groups,
                          size_t prio_num_groups,
                          const uint16_t *candidate_groups,
                          size_t candidate_num_groups, int *prio_group_idx,
    ...
    *selected_group = 0;
    ...
    for (current_group = 0; current_group < candidate_num_groups;
         current_group++) {
...
int tls_parse_ctos_key_share(SSL_CONNECTION *s, PACKET *pkt,
    ...
    /* We parse the key share extension and memorize the entries (after some
     * checks) */
    ks_extraction_result =
    extract_keyshares(s, &key_share_list, clntgroups, clnt_num_groups,
                      srvrgroups, srvr_num_groups, &keyshares_arr,
                      &encoded_pubkey_arr, &keyshares_cnt, &keyshares_max);

    if (ks_extraction_result ==
        EXTRACTION_FAILURE) /* Fatal error during tests */
     ...
     * encoded_pubkey_arr[keyshares_cnt] From server:
     * srvrgroups[srvr_num_groups], srvrtuples[srvr_num_tuples]
     *
     * Group selection algorithm:
     *    For all tuples do:
     *      key share group(s) overlapping with current tuple?
     *         --> Yes: accept group_id for SH
     *        --> No: is any of the client supported_groups overlapping with
     * current tuple?
    ...
    for (current_tuple = 0; current_tuple < srvr_num_tuples; current_tuple++) {
        ...
        if (s->options & SSL_OP_CIPHER_SERVER_PREFERENCE) {
            ...
            check_overlap(s, first_group_in_tuple, number_of_groups_in_tuple,
                          keyshares_arr, keyshares_cnt, &prio_group_idx,
                          &candidate_group_idx, &group_id_candidate);
            if (group_id_candidate >
                0) { /* Overlap found -> accept the key share group */
                if (!tls_accept_ksgroup(
                    s, group_id_candidate,
                ...
                /*
                 * There's no overlap with a key share, but is there at least a
                 * client supported_group overlapping with the current tuple?
                 */
                check_overlap(s, first_group_in_tuple,
                              number_of_groups_in_tuple, clntgroups,
                              clnt_num_groups, &prio_group_idx,
                              &candidate_group_idx, &group_id_candidate);
                if (group_id_candidate > 0) {
                    /*
                     * We did not have a key share overlap, but at least the
                     * supported groups overlap hence we can stop searching (and
                     * report group_id_candidate 'upward' for HRR)
                     */
            ...
            check_overlap(s, keyshares_arr, keyshares_cnt, first_group_in_tuple,
                          number_of_groups_in_tuple, &prio_group_idx,
                          &candidate_group_idx, &group_id_candidate);
            if (group_id_candidate > 0) {
                if (!tls_accept_ksgroup(s, group_id_candidate,
                                        &encoded_pubkey_arr[prio_group_idx]))
                    goto err;
                goto end;
            } else {
                check_overlap(s, clntgroups, clnt_num_groups,
                              first_group_in_tuple, number_of_groups_in_tuple,
...
int tls_parse_ctos_cookie(SSL_CONNECTION *s, PACKET *pkt, unsigned int context,
    ...
    sctx->libctx, "HMAC", sctx->propq, s->session_ctx->ext.cookie_hmac_key,
    ...
    if (EVP_DigestSignInit_ex(hctx, NULL, "SHA2-256", sctx->libctx, sctx->propq,
                              pkey, NULL) <= 0 ||
    ...
    if (!create_synthetic_message_hash(
        s, PACKET_data(&chhash), PACKET_remaining(&chhash), hrr, hrrlen)) {
...
int tls_parse_ctos_supported_groups(SSL_CONNECTION *s, PACKET *pkt,
    ...
    if (!s->hit || SSL_CONNECTION_IS_TLS13(s)) {
        ...
        if (!tls1_save_u16(&supported_groups_list, &s->ext.peer_supportedgroups,
...
static SSL_TICKET_STATUS
tls_get_stateful_ticket(SSL_CONNECTION *s, PACKET *tick, SSL_SESSION **sess) {
    ...
    tmpsess =
    lookup_sess_in_cache(s, PACKET_data(tick), SSL_MAX_SSL_SESSION_ID_LENGTH);
...
int tls_parse_ctos_psk(SSL_CONNECTION *s, PACKET *pkt, unsigned int context,
    ...
    for (id = 0; PACKET_remaining(&identities) != 0; id++) {
        ...
        if (sess == NULL && s->psk_server_callback != NULL &&
            ...
            pskdatalen =
            s->psk_server_callback(ussl, pskid, pskdata, sizeof(pskdata));
            OPENSSL_free(pskid);
        ...
        if (sess != NULL) {
            ...
            if ((s->options & SSL_OP_NO_TICKET) != 0 ||
                ...
                ret =
                tls_decrypt_ticket(s, PACKET_data(&identity),
            ...
            if (id == 0 && ossl_time_compare(sess->timeout, t) >= 0 &&
        ...
        if (!EVP_MD_is_a(md, EVP_MD_get0_name(
                             ssl_md(sctx, s->s3.tmp.new_cipher->algorithm2)))) {
...
EXT_RETURN tls_construct_stoc_key_share(SSL_CONNECTION *s, WPACKET *pkt,
    ...
    if (!ginf->is_kem) {
        ...
        encoded_pubkey_len =
        EVP_PKEY_get1_encoded_public_key(skey, &encoded_pubkey);
...
int tls_parse_ctos_client_cert_type(SSL_CONNECTION *sc, PACKET *pkt,
    ...
    reconcile_cert_type(data, len, sc->client_cert_type,
                        sc->client_cert_type_len, &sc->ext.client_cert_type);
...
int tls_parse_ctos_server_cert_type(SSL_CONNECTION *sc, PACKET *pkt,
    ...
    reconcile_cert_type(sc->server_cert_type, sc->server_cert_type_len, data,
                        len, &sc->ext.server_cert_type);
    if (sc->ext.server_cert_type_ctos == OSSL_CERT_TYPE_CTOS_GOOD)
        return 1;
