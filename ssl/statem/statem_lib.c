...
int ssl3_do_write(SSL_CONNECTION *s, uint8_t type) {
    ...
    if (type == SSL3_RT_HANDSHAKE)
        ...
        if (!SSL_CONNECTION_IS_TLS13(s) ||
            ...
            if (!ssl3_finish_mac(
                s, (unsigned char *)&s->init_buf->data[s->init_off], written))
                return -1;
...
static int get_cert_verify_tbs_data(SSL_CONNECTION *s, unsigned char *tls13tbs,
    ...
    /* ASCII: "TLS 1.3, server CertificateVerify", in hex for EBCDIC
     * compatibility
     */
    static const char servercontext[] =
    "\x54\x4c\x53\x20\x31\x2e\x33\x2c\x20\x73\x65\x72"
    "\x76\x65\x72\x20\x43\x65\x72\x74\x69\x66\x69\x63\x61\x74\x65\x56\x65\x72"
    "\x69"
    "\x66\x79";
    /* ASCII: "TLS 1.3, client CertificateVerify", in hex for EBCDIC
     * compatibility
     */
    static const char clientcontext[] =
    "\x54\x4c\x53\x20\x31\x2e\x33\x2c\x20\x63\x6c\x69"
    "\x65\x6e\x74\x20\x43\x65\x72\x74\x69\x66\x69\x63\x61\x74\x65\x56\x65\x72"
    "\x69"
    "\x66\x79";
...
int tls_process_rpk(SSL_CONNECTION *sc, PACKET *pkt, EVP_PKEY **peer_rpk) {
    ...
    if (SSL_CONNECTION_IS_TLS13(sc)) {
        ...
        if (!tls_collect_extensions(sc, &extensions,
                                    SSL_EXT_TLS1_3_RAW_PUBLIC_KEY, &rawexts,
                                    NULL, 1)) {
            /* SSLfatal already called */
            goto err;
        }
...
#ifndef OPENSSL_NO_SCTP
            /*
             * RFC6083: SCTP provides a reliable and in-sequence transport
             * service for DTLS messages that require it. Therefore, DTLS
             * procedures for retransmissions MUST NOT be used. Hence the
             * init_buf can be cleared when DTLS over SCTP as transport is used.
             */
...
int ssl_version_supported(const SSL_CONNECTION *s, int version,
    ...
    for (vent = table;
         ...
         ++vent) {
        const SSL_METHOD *(*thismeth)(void) =
        s->server ? vent->smeth : vent->cmeth;
