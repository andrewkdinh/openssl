...
int ssl3_do_write(SSL_CONNECTION *s, uint8_t type) {
   ...
   */
  if (s->statem.mutate_handshake_cb != NULL && !s->statem.write_in_progress &&
      type == SSL3_RT_HANDSHAKE && s->init_num >= SSL3_HM_HEADER_LENGTH) {
    unsigned char *msg;
    size_t msglen;

    if (!s->statem.mutate_handshake_cb((unsigned char *)s->init_buf->data,
                                       s->init_num, &msg, &msglen,
                                       s->statem.mutatearg))
      return -1;
  ...
  if (type == SSL3_RT_HANDSHAKE)
    ...
    if (!SSL_CONNECTION_IS_TLS13(s) ||
      ...
      if (!ssl3_finish_mac(s, (unsigned char *)&s->init_buf->data[s->init_off],
                           written))
        return -1;
...
int tls_setup_handshake(SSL_CONNECTION *s) {
  ...
  if (sctx->ssl_digest_methods[SSL_MD_MD5_SHA1_IDX] == NULL) {
    ...
    int md5sha1_needed_maxversion =
    SSL_CONNECTION_IS_DTLS(s) ? DTLS1_VERSION : TLS1_1_VERSION;
    ...
    SSL_CONNECTION_IS_DTLS(s) ? DTLS1_2_VERSION : TLS1_2_VERSION;
  ...
  if (s->server) {
    ...
    for (i = 0; i < sk_SSL_CIPHER_num(ciphers); i++) {
      ...
      int cipher_minprotover =
      SSL_CONNECTION_IS_DTLS(s) ? c->min_dtls : c->min_tls;
      int cipher_maxprotover =
      SSL_CONNECTION_IS_DTLS(s) ? c->max_dtls : c->max_tls;
...
static int get_cert_verify_tbs_data(SSL_CONNECTION *s, unsigned char *tls13tbs,
  ...
  "\x76\x65\x72\x20\x43\x65\x72\x74\x69\x66\x69\x63\x61\x74\x65\x56\x65\x72\x69"
  "\x66\x79";
  ...
  "\x65\x6e\x74\x20\x43\x65\x72\x74\x69\x66\x69\x63\x61\x74\x65\x56\x65\x72\x69"
  "\x66\x79";
...
CON_FUNC_RETURN tls_construct_finished(SSL_CONNECTION *s, WPACKET *pkt) {
  ...
  finish_md_len =
  ssl->method->ssl3_enc->final_finish_mac(s, sender, slen, s->s3.tmp.finish_md);
  if (finish_md_len == 0) {
    /* SSLfatal() already called */
    return CON_FUNC_ERROR;
  }
...
int tls_process_rpk(SSL_CONNECTION *sc, PACKET *pkt, EVP_PKEY **peer_rpk) {
  ...
  if (SSL_CONNECTION_IS_TLS13(sc)) {
    ...
    if (!tls_parse_all_extensions(sc, SSL_EXT_TLS1_3_RAW_PUBLIC_KEY, rawexts,
                                  NULL, 0, 1)) {
      /* SSLfatal already called */
      goto err;
    }
...
unsigned long tls_output_rpk(SSL_CONNECTION *sc, WPACKET *pkt, CERT_PKEY *cpk) {
  ...
  if (SSL_CONNECTION_IS_TLS13(sc)) {
     ...
     */
    if (!tls_construct_extensions(sc, pkt, SSL_EXT_TLS1_3_RAW_PUBLIC_KEY, x509,
                                  0)) {
      /* SSLfatal() already called */
      goto err;
    }
...
#ifndef OPENSSL_NO_SCTP
        /*
         * RFC6083: SCTP provides a reliable and in-sequence transport service
         * for DTLS messages that require it. Therefore, DTLS procedures for
         * retransmissions MUST NOT be used. Hence the init_buf can be cleared
         * when DTLS over SCTP as transport is used.
         */
        || BIO_dgram_is_sctp(SSL_get_wbio(SSL_CONNECTION_GET_SSL(s)))
#endif
    ) {
      /*
       * We don't do this in DTLS over UDP because we may still need the
       * init_buf in case there are any unexpected retransmits
       */
...
int tls_get_message_header(SSL_CONNECTION *s, int *mt) {
  ...
  do {
    while (s->init_num < SSL3_HM_HEADER_LENGTH) {
      ...
      ssl, SSL3_RT_HANDSHAKE, &recvd_type, &p[s->init_num],
        ...
        if (p[1] == 0 && p[2] == 0 && p[3] == 0) {
          ...
          if (s->msg_callback)
            s->msg_callback(0, s->version, SSL3_RT_HANDSHAKE, p,
                            SSL3_HM_HEADER_LENGTH, ussl, s->msg_callback_arg);
        }
  } while (skip_message);
  /* s->init_num == SSL3_HM_HEADER_LENGTH */
...
#define SERVER_HELLO_RANDOM_OFFSET (SSL3_HM_HEADER_LENGTH + 2)
    ...
    if (!SSL_CONNECTION_IS_TLS13(s) ||
      ...
      if (s->s3.tmp.message_type != SSL3_MT_SERVER_HELLO ||
          ...
          memcmp(hrrrandom, s->init_buf->data + SERVER_HELLO_RANDOM_OFFSET,
                 SSL3_RANDOM_SIZE) != 0) {
...
int ssl_version_supported(const SSL_CONNECTION *s, int version,
  ...
  for (vent = table;
    ...
    if (thismeth != NULL && ssl_version_cmp(s, version, vent->version) == 0 &&
...
int check_in_list(SSL_CONNECTION *s, uint16_t group_id, const uint16_t *groups,
  ...
  for (i = 0; i < num_groups; i++) {
    ...
    if (group_id == group &&
        (!checkallow || tls_group_allowed(s, group, SSL_SECOP_CURVE_CHECK))) {
      if (pos != NULL)
        *pos = i;
      return 1;
    }
...
MSG_PROCESS_RETURN tls13_process_compressed_certificate(SSL_CONNECTION *sc,
  ...
  if (!BUF_MEM_grow(buf, expected_length) ||
      ...
      COMP_expand_block(comp, (unsigned char *)buf->data, expected_length,
                        ...
                        comp_length) != (int)expected_length) {
    SSLfatal(sc, SSL_AD_BAD_CERTIFICATE, SSL_R_BAD_DECOMPRESSION);
    goto err;
  }
  ret = MSG_PROCESS_CONTINUE_PROCESSING;
