...
static const unsigned char bitmask_start_values[] = {0xff, 0xfe, 0xfc, 0xf8,
                                                     0xf0, 0xe0, 0xc0, 0x80};
static const unsigned char bitmask_end_values[] = {0xff, 0x01, 0x03, 0x07,
                                                   0x0f, 0x1f, 0x3f, 0x7f};
...
int dtls1_do_write(SSL_CONNECTION *s, uint8_t type) {
  ...
  while (s->init_num > 0) {
    ...
    ret =
    dtls1_write_bytes(s, type, &s->init_buf->data[s->init_off], len, &written);
    if (ret <= 0) {
      /*
       ...
       */
      if (retry && BIO_ctrl(SSL_get_wbio(ssl), BIO_CTRL_DGRAM_MTU_EXCEEDED, 0,
                            NULL) > 0) {
...
again:
  ...
  *mt = s->s3.tmp.message_type;
  ...
  if (*mt == SSL3_MT_CHANGE_CIPHER_SPEC) {
    if (s->msg_callback) {
      s->msg_callback(0, s->version, SSL3_RT_CHANGE_CIPHER_SPEC, p, 1,
...
static int dtls1_retrieve_buffered_fragment(SSL_CONNECTION *s, size_t *len) {
  ...
  do {
    ...
    if (frag->msg_header.seq < s->d1->handshake_read_seq) {
      ...
      if (!s->server || frag->msg_header.seq != 0 ||
...
static int dtls1_reassemble_fragment(SSL_CONNECTION *s,
  ...
  if (frag->reassembly == NULL) {
    ...
    while (frag_len) {
      ...
      ssl, SSL3_RT_HANDSHAKE, NULL, devnull,
...
static int
dtls1_process_out_of_seq_message(SSL_CONNECTION *s,
                                 const struct hm_header_st *msg_hdr) {
  int i = -1;
  hm_fragment *frag = NULL;
  pitem *item = NULL;
  ...
  if (msg_hdr->seq <= s->d1->handshake_read_seq ||
    ...
    if (frag_len) {
       ...
       */
      i = ssl->method->ssl_read_bytes(ssl, SSL3_RT_HANDSHAKE, NULL,
                                      frag->fragment, frag_len, 0, &readbytes);
      if (i <= 0 || readbytes != frag_len)
        i = -1;
      if (i <= 0)
        goto err;
...
redo:
   ...
   */
  if (msg_hdr.seq != s->d1->handshake_read_seq) {
    if (!s->server || msg_hdr.seq != 0 || s->d1->handshake_read_seq != 1 ||
  ...
  if (!s->server && s->d1->r_msg_hdr.frag_off == 0 &&
      s->statem.hand_state != TLS_ST_OK && p[0] == SSL3_MT_HELLO_REQUEST) {
    /*
    ...
    if (p[1] == 0 && p[2] == 0 && p[3] == 0) {
      if (s->msg_callback)
        s->msg_callback(0, s->version, SSL3_RT_HANDSHAKE, p,
                        DTLS1_HM_HEADER_LENGTH, ussl, s->msg_callback_arg);
  ...
  if (frag_len > 0) {
    ...
    i = ssl->method->ssl_read_bytes(ssl, SSL3_RT_HANDSHAKE, NULL, &p[frag_off],
...
int dtls1_buffer_message(SSL_CONNECTION *s, int is_ccs) {
  ...
  if (is_ccs) {
    ...
    if (!ossl_assert(s->d1->w_msg_hdr.msg_len + ((s->version == DTLS1_BAD_VER)
                                                 ? 3
