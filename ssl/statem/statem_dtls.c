...
int dtls1_do_write(SSL_CONNECTION *s, uint8_t type)
{
    ...
    while (s->init_num > 0) {
        ...
        if (ret <= 0) {
             ...
             */
            if (retry
                && BIO_ctrl(SSL_get_wbio(ssl), BIO_CTRL_DGRAM_MTU_EXCEEDED, 0,
                            NULL)
                    > 0) {
...
int dtls1_retransmit_message(SSL_CONNECTION *s, unsigned short seq, int *found)
{
    ...
    *found = 1;
    ...
    ret = dtls1_do_write(s,
                         frag->msg_header.is_ccs ? SSL3_RT_CHANGE_CIPHER_SPEC
                                                 : SSL3_RT_HANDSHAKE);
