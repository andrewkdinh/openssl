 ...
 */
static int custom_ext_add_old_cb_wrap(SSL *s, unsigned int ext_type, unsigned int context, const unsigned char **out,
...
static void custom_ext_free_old_cb_wrap(SSL *s, unsigned int ext_type, unsigned int context, const unsigned char *out,
                                        void *add_arg)
{
    custom_ext_add_cb_wrap *add_cb_wrap = (custom_ext_add_cb_wrap *)add_arg;
...
static int custom_ext_parse_old_cb_wrap(SSL *s, unsigned int ext_type, unsigned int context, const unsigned char *in,
...
custom_ext_method *custom_ext_find(const custom_ext_methods *exts, ENDPOINT role, unsigned int ext_type, size_t *idx)
{
    ...
    for (i = 0; i < exts->meths_count; i++, meth++) {
        if (ext_type == meth->ext_type
            && (role == ENDPOINT_BOTH || role == meth->role || meth->role == ENDPOINT_BOTH)) {
            if (idx != NULL)
                *idx = i;
            return meth;
        }
...
int custom_ext_parse(SSL_CONNECTION *s, unsigned int context, unsigned int ext_type, const unsigned char *ext_data,
                     size_t ext_size, X509 *x, size_t chainidx)
{
    ...
    if ((context & (SSL_EXT_TLS1_2_SERVER_HELLO | SSL_EXT_TLS1_3_SERVER_HELLO | SSL_EXT_TLS1_3_ENCRYPTED_EXTENSIONS))
        != 0) {
        /*
    ...
    if (meth->parse_cb(SSL_CONNECTION_GET_USER_SSL(s), ext_type, context, ext_data, ext_size, x, chainidx, &al,
                       meth->parse_arg)
        <= 0) {
 ...
 * buffer.
 */
int custom_ext_add(SSL_CONNECTION *s, int context, WPACKET *pkt, X509 *x, size_t chainidx, int maxversion)
{
    custom_ext_methods *exts = &s->cert->custext;
    custom_ext_method *meth;
    size_t i;
    ...
    for (i = 0; i < exts->meths_count; i++) {
        ...
        if ((context
             & (SSL_EXT_TLS1_2_SERVER_HELLO | SSL_EXT_TLS1_3_SERVER_HELLO | SSL_EXT_TLS1_3_ENCRYPTED_EXTENSIONS
                | SSL_EXT_TLS1_3_CERTIFICATE | SSL_EXT_TLS1_3_RAW_PUBLIC_KEY | SSL_EXT_TLS1_3_HELLO_RETRY_REQUEST))
            != 0) {
        ...
        if (meth->add_cb != NULL) {
            int cb_retval = meth->add_cb(SSL_CONNECTION_GET_USER_SSL(s), meth->ext_type, context, &out, &outlen, x,
        ...
        if (!WPACKET_put_bytes_u16(pkt, meth->ext_type) || !WPACKET_start_sub_packet_u16(pkt)
            || (outlen > 0 && !WPACKET_memcpy(pkt, out, outlen)) || !WPACKET_close(pkt)) {
...
static void custom_ext_copy_old_cb(custom_ext_method *methdst, const custom_ext_method *methsrc, int *err)
{
    ...
    if (src->meths_count > 0) {
        ...
        if (meths_count > 0) {
            custom_ext_method *methdst =
                OPENSSL_realloc(dst->meths, (dst->meths_count + meths_count) * sizeof(custom_ext_method));
                                   ...
                                   SSL_custom_ext_parse_cb_ex parse_cb, void *parse_arg)
{
     ...
     */
    if (ext_type == TLSEXT_TYPE_signed_certificate_timestamp && (context & SSL_EXT_CLIENT_HELLO) != 0 && ctx != NULL
...
static int add_old_custom_ext(SSL_CTX *ctx, ENDPOINT role, unsigned int ext_type, unsigned int context,
                              custom_ext_add_cb add_cb, custom_ext_free_cb free_cb, void *add_arg,
                              custom_ext_parse_cb parse_cb, void *parse_arg)
{
    ...
    ret = ossl_tls_add_custom_ext_intern(ctx, NULL, role, ext_type, context, custom_ext_add_old_cb_wrap,
                                         custom_ext_free_old_cb_wrap, add_cb_wrap, custom_ext_parse_old_cb_wrap,
                                         parse_cb_wrap);
...
int SSL_CTX_add_custom_ext(SSL_CTX *ctx, unsigned int ext_type, unsigned int context, SSL_custom_ext_add_cb_ex add_cb,
                           SSL_custom_ext_free_cb_ex free_cb, void *add_arg, SSL_custom_ext_parse_cb_ex parse_cb,
                           void *parse_arg)
{
    return ossl_tls_add_custom_ext_intern(ctx, NULL, ENDPOINT_BOTH, ext_type, context, add_cb, free_cb, add_arg,
                                          parse_cb, parse_arg);
}
