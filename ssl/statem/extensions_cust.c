...
int custom_ext_add(SSL_CONNECTION *s, int context, WPACKET *pkt, X509 *x,
  ...
  for (i = 0; i < exts->meths_count; i++) {
    ...
    if ((context & (SSL_EXT_TLS1_2_SERVER_HELLO | SSL_EXT_TLS1_3_SERVER_HELLO |
                    ...
                    SSL_EXT_TLS1_3_CERTIFICATE | SSL_EXT_TLS1_3_RAW_PUBLIC_KEY |
    ...
    if (meth->add_cb != NULL) {
      int cb_retval =
      meth->add_cb(SSL_CONNECTION_GET_USER_SSL(s), meth->ext_type, context,
                   &out, &outlen, x, chainidx, &al, meth->add_arg);
    ...
    if (!WPACKET_put_bytes_u16(pkt, meth->ext_type) ||
      ...
      if (meth->free_cb != NULL)
        meth->free_cb(SSL_CONNECTION_GET_USER_SSL(s), meth->ext_type, context,
                      out, meth->add_arg);
      if (!for_comp)
...
int custom_exts_copy(custom_ext_methods *dst, const custom_ext_methods *src) {
  ...
  if (src->meths_count > 0) {
    ...
    for (i = 0; i < src->meths_count; i++) {
      ...
      methdst->parse_arg =
      OPENSSL_memdup(methsrc->parse_arg, sizeof(custom_ext_parse_cb_wrap));
