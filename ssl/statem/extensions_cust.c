                     ...
                     size_t ext_size, X509 *x, size_t chainidx)
{
    ...
    if (meth->parse_cb(SSL_CONNECTION_GET_USER_SSL(s), ext_type, context,
                       ext_data, ext_size, x, chainidx, &al, meth->parse_arg)
        <= 0) {
    ...
    for (i = 0; i < exts->meths_count; i++) {
        ...
        if (meth->add_cb != NULL) {
            int cb_retval = meth->add_cb(SSL_CONNECTION_GET_USER_SSL(s),
                                         meth->ext_type, context, &out, &outlen,
            ...
            if (!ossl_assert((meth->ext_flags & SSL_EXT_FLAG_SENT) == 0)) {
                if (meth->free_cb != NULL)
                    meth->free_cb(SSL_CONNECTION_GET_USER_SSL(s),
                                  meth->ext_type, context, out, meth->add_arg);
                if (!for_comp)
                    SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
                return 0;
            }
...
{
    return custom_ext_find(&ctx->cert->custext, ENDPOINT_CLIENT, ext_type, NULL)
        != NULL;
}
