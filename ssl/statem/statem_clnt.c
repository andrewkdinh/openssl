...
#error Internal DTLS version error
            ...
            if (!SSL_CONNECTION_IS_DTLS(s) &&
                 ...
                 * message here after the swap, or do it after the
                 * clientFinished?
                 */
    ...
    case TLS_ST_CW_CLNT_HELLO:
        ...
        if (SSL_CONNECTION_IS_DTLS(s)) {
            ...
            if (!ssl_set_new_record_layer(
                ...
                OSSL_RECORD_PROTECTION_LEVEL_NONE, NULL, 0, NULL, 0, NULL, 0,
                NULL, 0, NULL, 0, NID_undef, NULL, NULL, NULL)) {
                /* SSLfatal already called */
                return WORK_ERROR;
            }
        }
...
WORK_STATE ossl_statem_client_post_work(SSL_CONNECTION *s, WORK_STATE wst) {
    ...
    case TLS_ST_CW_CHANGE:
        ...
        if (SSL_CONNECTION_IS_DTLS(s) && s->hit) {
             ...
             * no SCTP used.
             */
            BIO_ctrl(SSL_get_wbio(ssl), BIO_CTRL_DGRAM_SCTP_NEXT_AUTH_KEY, 0,
                     NULL);
        }
#endif
        break;
...
CON_FUNC_RETURN tls_construct_client_hello(SSL_CONNECTION *s, WPACKET *pkt) {
    ...
    if (s->new_session || s->session->ssl_version == TLS1_3_VERSION) {
        if (s->version == TLS1_3_VERSION &&
            ...
            if (s->hello_retry_request == SSL_HRR_NONE &&
                RAND_bytes_ex(sctx->libctx, s->tmp_session_id, sess_id_len,
                              0) <= 0) {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
                return CON_FUNC_ERROR;
            }
...
MSG_PROCESS_RETURN tls_process_server_hello(SSL_CONNECTION *s, PACKET *pkt) {
    ...
    if (compression == 0)
        ...
        comp =
        ssl3_comp_find(SSL_CONNECTION_GET_CTX(s)->comp_methods, compression);
    }
...
MSG_PROCESS_RETURN tls_process_server_certificate(SSL_CONNECTION *s,
    ...
    for (chainidx = 0; PACKET_remaining(pkt); chainidx++) {
        ...
        if (d2i_X509(&x, (const unsigned char **)&certbytes, cert_len) ==
            NULL) {
...
static int tls_process_ske_srp(SSL_CONNECTION *s, PACKET *pkt,
    ...
    if ((s->srp_ctx.N = BN_bin2bn(
         PACKET_data(&prime), (int)PACKET_remaining(&prime), NULL)) == NULL ||
...
MSG_PROCESS_RETURN tls_process_key_exchange(SSL_CONNECTION *s, PACKET *pkt) {
    ...
    if (pkey != NULL) {
        ...
        if (EVP_DigestVerifyInit_ex(
            md_ctx, &pctx, md == NULL ? NULL : EVP_MD_get0_name(md),
...
MSG_PROCESS_RETURN tls_process_new_session_ticket(SSL_CONNECTION *s,
    ...
    if (SSL_CONNECTION_IS_TLS13(s)) {
        ...
        if (!tls_collect_extensions(
            s, &extpkt, SSL_EXT_TLS1_3_NEW_SESSION_TICKET, &exts, NULL, 1) ||
    ...
    if (wst == WORK_MORE_B) {
        ...
        if (i == 0) {
            if (s->version == SSL3_VERSION) {
                ...
                s->s3.tmp.cert_req = 2;
                s->ext.compress_certificate_from_peer[0] =
...
#error Max cipher length too short
    ...
    if (totlen == 0 || !maxverok) {
        ...
        !maxverok ? "No ciphers enabled for max supported SSL/TLS version"
                  : NULL;
