...
static int ssl_cipher_list_to_bytes(SSL_CONNECTION *s,
                                    STACK_OF(SSL_CIPHER) * sk, WPACKET *pkt);
...
static WRITE_TRAN ossl_statem_client13_write_transition(SSL_CONNECTION *s) {
  ...
  case TLS_ST_CW_CERT:
    ...
    st->hand_state =
    (s->s3.tmp.cert_req == 1) ? TLS_ST_CW_CERT_VRFY : TLS_ST_CW_FINISHED;
    return WRITE_TRAN_CONTINUE;
  ...
  case TLS_ST_CW_CLNT_HELLO:
    ...
    if (SSL_CONNECTION_IS_DTLS(s)) {
      ...
      if (!ssl_set_new_record_layer(
          s, TLS_ANY_VERSION, OSSL_RECORD_DIRECTION_WRITE,
          OSSL_RECORD_PROTECTION_LEVEL_NONE, NULL, 0, NULL, 0, NULL, 0, NULL, 0,
 ...
 *   0: Error
 */
int ossl_statem_client_construct_message(SSL_CONNECTION *s, confunc_f *confunc,
                                         int *mt) {
  OSSL_STATEM *st = &s->statem;
...
MSG_PROCESS_RETURN tls_process_server_hello(SSL_CONNECTION *s, PACKET *pkt) {
  ...
  if (!hrr) {
    if (!tls_collect_extensions(
        s, &extpkt, SSL_EXT_TLS1_2_SERVER_HELLO | SSL_EXT_TLS1_3_SERVER_HELLO,
        &extensions, NULL, 1)) {
      /* SSLfatal() already called */
      goto err;
    }
  ...
  if (SSL_CONNECTION_IS_TLS13(s) || hrr) {
    ...
    if (session_id_len != s->tmp_session_id_len ||
        memcmp(PACKET_data(&session_id), s->tmp_session_id, session_id_len) !=
        0) {
  ...
  if (SSL_CONNECTION_IS_TLS13(s)) {
    ...
    if (s->version >= TLS1_VERSION && s->ext.session_secret_cb != NULL &&
      ...
      if (s->ext.session_secret_cb(ussl, s->session->master_key,
        ...
        s->session->cipher =
        pref_cipher ? pref_cipher : ssl_get_cipher_by_char(s, cipherchars, 0);
      } else {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        goto err;
      }
...
static MSG_PROCESS_RETURN tls_process_as_hello_retry_request(SSL_CONNECTION *s,
   ...
   */
  if (s->early_data_state == SSL_EARLY_DATA_FINISHED_WRITING &&
      !ssl_set_new_record_layer(s, TLS_ANY_VERSION, OSSL_RECORD_DIRECTION_WRITE,
                                OSSL_RECORD_PROTECTION_LEVEL_NONE, NULL, 0,
                                NULL, 0, NULL, 0, NULL, 0, NULL, 0, NID_undef,
                                NULL, NULL, NULL)) {
    /* SSLfatal already called */
    goto err;
  }
...
MSG_PROCESS_RETURN tls_process_server_certificate(SSL_CONNECTION *s,
  ...
  if ((SSL_CONNECTION_IS_TLS13(s) && !PACKET_get_1(pkt, &context)) ||
      context != 0 || !PACKET_get_net_3(pkt, &cert_list_len) ||
    ...
    if (SSL_CONNECTION_IS_TLS13(s)) {
      ...
      if (!tls_collect_extensions(s, &extensions, SSL_EXT_TLS1_3_CERTIFICATE,
          ...
          !tls_parse_all_extensions(s, SSL_EXT_TLS1_3_CERTIFICATE, rawexts, x,
                                    chainidx, PACKET_remaining(pkt) == 0)) {
...
#ifndef OPENSSL_NO_COMP_ALG
MSG_PROCESS_RETURN tls_process_server_compressed_certificate(SSL_CONNECTION *sc,
                                                             PACKET *pkt) {
...
static int tls_process_ske_srp(SSL_CONNECTION *s, PACKET *pkt,
                               EVP_PKEY **pkey) {
...
static int tls_process_ske_dhe(SSL_CONNECTION *s, PACKET *pkt,
                               EVP_PKEY **pkey) {
  ...
  g =
  BN_bin2bn(PACKET_data(&generator), (int)PACKET_remaining(&generator), NULL);
  bnpub_key =
  ...
  if (!ssl_security(s, SSL_SECOP_TMP_DH, EVP_PKEY_get_security_bits(peer_tmp),
                    0, peer_tmp)) {
...
static int tls_process_ske_ecdhe(SSL_CONNECTION *s, PACKET *pkt,
                                 EVP_PKEY **pkey) {
  PACKET encoded_pt;
  ...
  if (EVP_PKEY_set1_encoded_public_key(s->s3.peer_tmp, PACKET_data(&encoded_pt),
...
MSG_PROCESS_RETURN tls_process_certificate_request(SSL_CONNECTION *s,
  ...
  if (SSL_CONNECTION_IS_TLS13(s)) {
    ...
    if (!tls_collect_extensions(s, &extensions,
                                SSL_EXT_TLS1_3_CERTIFICATE_REQUEST, &rawexts,
                                NULL, 1) ||
...
MSG_PROCESS_RETURN tls_process_new_session_ticket(SSL_CONNECTION *s,
  ...
  if (SSL_CONNECTION_IS_TLS13(s)) {
    ...
    if (!tls_collect_extensions(s, &extpkt, SSL_EXT_TLS1_3_NEW_SESSION_TICKET,
                                &exts, NULL, 1) ||
        !tls_parse_all_extensions(s, SSL_EXT_TLS1_3_NEW_SESSION_TICKET, exts,
                                  NULL, 0, 1)) {
      /* SSLfatal() already called */
      goto err;
    }
...
static int tls_construct_cke_psk_preamble(SSL_CONNECTION *s, WPACKET *pkt) {
  ...
  psklen = s->psk_client_callback(SSL_CONNECTION_GET_USER_SSL(s),
                                  s->session->psk_identity_hint, identity,
...
int ossl_gost_ukm(const SSL_CONNECTION *s, unsigned char *dgst_buf) {
  ...
  SSL_CTX *sctx = SSL_CONNECTION_GET_CTX(s);
  const EVP_MD *md =
  ssl_evp_md_fetch(sctx->libctx, NID_id_GostR3411_2012_256, sctx->propq);
...
static int tls_construct_cke_gost18(SSL_CONNECTION *s, WPACKET *pkt) {
  ...
  if (EVP_PKEY_CTX_ctrl(pkey_ctx, -1, EVP_PKEY_OP_ENCRYPT, EVP_PKEY_CTRL_CIPHER,
                        cipher_nid, NULL) <= 0) {
    SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_R_LIBRARY_BUG);
    goto err;
  }
...
CON_FUNC_RETURN tls_construct_client_certificate(SSL_CONNECTION *s,
   ...
   */
  if (SSL_CONNECTION_IS_TLS13(s) && SSL_IS_FIRST_HANDSHAKE(s) &&
      (s->early_data_state != SSL_EARLY_DATA_NONE ||
       (s->options & SSL_OP_ENABLE_MIDDLEBOX_COMPAT) != 0) &&
      (!ssl->method->ssl3_enc->change_cipher_state(
      s, SSL3_CC_HANDSHAKE | SSL3_CHANGE_CIPHER_CLIENT_WRITE))) {
...
CON_FUNC_RETURN tls_construct_client_compressed_certificate(SSL_CONNECTION *sc,
  ...
  if (sc->pha_context == NULL) {
    ...
    if (!WPACKET_put_bytes_u8(&tmppkt, 0))
      goto err;
  } else if (!WPACKET_sub_memcpy_u8(&tmppkt, sc->pha_context,
                                    sc->pha_context_len))
    goto err;
  ...
  if (SSL_IS_FIRST_HANDSHAKE(sc) &&
      ...
      (!ssl->method->ssl3_enc->change_cipher_state(
      sc, SSL3_CC_HANDSHAKE | SSL3_CHANGE_CIPHER_CLIENT_WRITE))) {
    /*
...
static MSG_PROCESS_RETURN tls_process_encrypted_extensions(SSL_CONNECTION *s,
  ...
  if (!tls_collect_extensions(
      s, &extensions, SSL_EXT_TLS1_3_ENCRYPTED_EXTENSIONS, &rawexts, NULL, 1) ||
      !tls_parse_all_extensions(s, SSL_EXT_TLS1_3_ENCRYPTED_EXTENSIONS, rawexts,
                                NULL, 0, 1)) {
    /* SSLfatal() already called */
    goto err;
  }
...
int ssl_cipher_list_to_bytes(SSL_CONNECTION *s, STACK_OF(SSL_CIPHER) * sk,
  ...
  int empty_reneg_info_scsv =
  !s->renegotiate && !SSL_CONNECTION_IS_DTLS(s) &&
...
#error Max cipher length too short
  ...
  if (totlen == 0 || !maxverok) {
    ...
    !maxverok ? "No ciphers enabled for max supported SSL/TLS version" : NULL;
...
CON_FUNC_RETURN tls_construct_end_of_early_data(SSL_CONNECTION *s,
                                                WPACKET *pkt) {
  if (s->early_data_state != SSL_EARLY_DATA_WRITE_RETRY &&
      s->early_data_state != SSL_EARLY_DATA_FINISHED_WRITING) {
