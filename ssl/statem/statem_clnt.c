...
MSG_PROCESS_RETURN tls_process_key_exchange(SSL_CONNECTION *s, PACKET *pkt)
{
    ...
    if (pkey != NULL) {
         ...
         */
        if (!PACKET_get_sub_packet(&save_param_start,
                                   &params,
        ...
        if (EVP_DigestVerifyInit_ex(md_ctx,
                                    &pctx,
...
CON_FUNC_RETURN tls_construct_client_compressed_certificate(SSL_CONNECTION *sc, WPACKET *pkt)
{
    ...
    comp_len =
        COMP_compress_block(comp, WPACKET_get_curr(pkt), (int)max_length, (unsigned char *)buf->data, (int)length);
    if (comp_len <= 0)
        goto err;
