...
#error Internal DTLS version error
    ...
    if (!WPACKET_put_bytes_u16(pkt, TLSEXT_TYPE_renegotiate) ||
        !WPACKET_start_sub_packet_u16(pkt) || !WPACKET_put_bytes_u8(pkt, 0) ||
        !WPACKET_close(pkt)) {
...
static int add_key_share(SSL_CONNECTION *s, WPACKET *pkt, unsigned int group_id,
  ...
  /* We ensure in t1_lib.c that the loop number does not exceed
  ...
  if (s->s3.group_id != 0 && s->s3.tmp.pkey == NULL) {
    ...
    for (i = 0; i < num_groups; i++) {
      if (!tls_group_allowed(s, pgroups[i], SSL_SECOP_CURVE_SUPPORTED))
        continue;
      if (!tls_valid_group(s, pgroups[i], TLS1_3_VERSION, TLS1_3_VERSION, 0,
                           NULL))
        continue;
...
EXT_RETURN tls_construct_ctos_padding(SSL_CONNECTION *s, WPACKET *pkt,
  ...
  if (s->session->ssl_version == TLS1_3_VERSION &&
    ...
    const EVP_MD *md =
    ssl_md(SSL_CONNECTION_GET_CTX(s), s->session->cipher->algorithm2);
...
dopsksess:
  ...
  if (s->psksession != NULL) {
    if (!WPACKET_sub_memcpy_u16(pkt, s->psksession_id, s->psksession_id_len) ||
...
int tls_parse_stoc_ec_pt_formats(SSL_CONNECTION *s, PACKET *pkt,
  ...
  if (!s->hit) {
    ...
    if (!PACKET_copy_bytes(&ecptformatlist, s->ext.peer_ecpointformats,
                           ecpointformats_len)) {
      SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
      return 0;
    }
...
int tls_parse_stoc_session_ticket(SSL_CONNECTION *s, PACKET *pkt,
  ...
  if (s->ext.session_ticket_cb != NULL &&
      !s->ext.session_ticket_cb(ssl, PACKET_data(pkt), PACKET_remaining(pkt),
...
int tls_parse_stoc_sct(SSL_CONNECTION *s, PACKET *pkt, unsigned int context,
  ...
  if (s->ct_validation_callback != NULL) {
    ...
    if (!custom_ext_parse(s, context, TLSEXT_TYPE_signed_certificate_timestamp,
                          PACKET_data(pkt), PACKET_remaining(pkt), x,
                          chainidx)) {
      /* SSLfatal already called */
      return 0;
    }
...
int tls_parse_stoc_key_share(SSL_CONNECTION *s, PACKET *pkt,
  ...
  if ((ginf = tls1_group_id_lookup(SSL_CONNECTION_GET_CTX(s), group_id)) ==
      NULL) {
    SSLfatal(s, SSL_AD_ILLEGAL_PARAMETER, SSL_R_BAD_KEY_SHARE);
    return 0;
  }
