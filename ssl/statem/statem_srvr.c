...
MSG_PROCESS_RETURN tls_process_client_hello(SSL_CONNECTION *s, PACKET *pkt)
{
    ...
    if (!PACKET_copy_all(&compression,
...
static int tls_early_post_process_client_hello(SSL_CONNECTION *s)
{
    ...
    if (SSL_CONNECTION_IS_DTLS(s)) {
        ...
        if (SSL_get_options(ssl) & SSL_OP_COOKIE_EXCHANGE) {
            if (sctx->app_verify_cookie_cb != NULL) {
                if (sctx->app_verify_cookie_cb(ussl,
                                               clienthello->dtls_cookie,
    ...
    if (!s->hit && s->version >= TLS1_VERSION && !SSL_CONNECTION_IS_TLS13(s) && !SSL_CONNECTION_IS_DTLS(s)
        ...
        if (s->ext.session_secret_cb(ussl,
            ...
            if (!s->hit) {
                ...
                if (s->not_resumable_session_cb != NULL)
                    s->session->not_resumable = s->not_resumable_session_cb(
                        ussl,
                        ((s->s3.tmp.new_cipher->algorithm_mkey & (SSL_kDHE | SSL_kECDHE)) != 0));
...
CON_FUNC_RETURN tls_construct_server_hello(SSL_CONNECTION *s, WPACKET *pkt)
{
    ...
    if (!WPACKET_put_bytes_u16(pkt, version)
         ...
         */
        || !WPACKET_memcpy(pkt,
                           s->hello_retry_request == SSL_HRR_PENDING ? hrrrandom : s->s3.server_random,
                           SSL3_RANDOM_SIZE)) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        return CON_FUNC_ERROR;
    }
...
CON_FUNC_RETURN tls_construct_server_key_exchange(SSL_CONNECTION *s, WPACKET *pkt)
{
    ...
    if (lu != NULL) {
        ...
        if (EVP_DigestSignInit_ex(md_ctx,
                                  &pctx,
...
static CON_FUNC_RETURN
construct_stateless_ticket(SSL_CONNECTION *s, WPACKET *pkt, uint32_t age_add, unsigned char *tick_nonce)
{
