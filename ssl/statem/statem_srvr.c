...
static void ssl_check_for_safari(SSL_CONNECTION *s,
    ...
    ext_len =
    TLS1_get_client_version(SSL_CONNECTION_GET_SSL(s)) >= TLS1_2_VERSION
...
MSG_PROCESS_RETURN tls_process_client_hello(SSL_CONNECTION *s, PACKET *pkt) {
    ...
    if (clienthello->isv2) {
        ...
        if (SSL_CONNECTION_IS_DTLS(s)) {
             ...
             */
            if (SSL_get_options(SSL_CONNECTION_GET_SSL(s)) &
                SSL_OP_COOKIE_EXCHANGE) {
...
static int tls_process_cke_rsa(SSL_CONNECTION *s, PACKET *pkt) {
    ...
    *p++ = OSSL_PARAM_construct_uint(OSSL_ASYM_CIPHER_PARAM_TLS_CLIENT_VERSION,
    ...
    if ((s->options & SSL_OP_TLS_ROLLBACK_BUG) != 0)
        *p++ =
        OSSL_PARAM_construct_uint(OSSL_ASYM_CIPHER_PARAM_TLS_NEGOTIATED_VERSION,
    ...
    if (wst == WORK_MORE_A) {
        if (SSL_CONNECTION_IS_DTLS(s)) {
            ...
            if (SSL_export_keying_material(
                SSL_CONNECTION_GET_SSL(s), sctpauthkey, sizeof(sctpauthkey),
...
static CON_FUNC_RETURN construct_stateless_ticket(SSL_CONNECTION *s,
...
#endif
    {
        ...
        if (tctx->ext.ticket_key_evp_cb != NULL)
            ret = tctx->ext.ticket_key_evp_cb(
            ssl, key_name, iv, ctx, ssl_hmac_get0_EVP_MAC_CTX(hctx), 1);
#ifndef OPENSSL_NO_DEPRECATED_3_0
...
CON_FUNC_RETURN tls_construct_new_session_ticket(SSL_CONNECTION *s,
    ...
    if (SSL_CONNECTION_IS_TLS13(s) &&
        ...
        tmpret =
        construct_stateless_ticket(s, pkt, age_add_u.age_add, tick_nonce);
