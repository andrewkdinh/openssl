...
tls_construct_encrypted_extensions(SSL_CONNECTION * s, WPACKET *pkt);
...
WORK_STATE ossl_statem_server_post_work(SSL_CONNECTION *s, WORK_STATE wst) {
  ...
  case TLS_ST_SW_SRVR_HELLO:
    ...
    if (SSL_CONNECTION_IS_DTLS(s) && s->hit) {
      ...
      if (SSL_export_keying_material(ssl, sctpauthkey, sizeof(sctpauthkey),
  ...
  case TLS_ST_SW_FINISHED:
    ...
    if (SSL_CONNECTION_IS_TLS13(s)) {
      ...
      if (!ssl->method->ssl3_enc->generate_master_secret(
          s, s->master_secret, s->handshake_secret, 0, &dummy) ||
          !ssl->method->ssl3_enc->change_cipher_state(
          s, SSL3_CC_APPLICATION | SSL3_CHANGE_CIPHER_SERVER_WRITE))
        /* SSLfatal() already called */
        return WORK_ERROR;
    }
    break;
 ...
 *   0: Error
 */
int ossl_statem_server_construct_message(SSL_CONNECTION *s, confunc_f *confunc,
                                         int *mt) {
  OSSL_STATEM *st = &s->statem;
...
static void ssl_check_for_safari(SSL_CONNECTION *s,
  ...
  ext_len = TLS1_get_client_version(SSL_CONNECTION_GET_SSL(s)) >= TLS1_2_VERSION
...
MSG_PROCESS_RETURN tls_process_client_hello(SSL_CONNECTION *s, PACKET *pkt) {
  ...
  if (clienthello->isv2) {
    ...
    if (!PACKET_copy_bytes(
        &challenge, clienthello->random + SSL3_RANDOM_SIZE - challenge_len,
        challenge_len)
...
static int tls_early_post_process_client_hello(SSL_CONNECTION *s) {
  ...
  if (memchr(clienthello->compressions, 0, clienthello->compressions_len) ==
      NULL) {
  ...
  if (!s->hit && s->version >= TLS1_VERSION && !SSL_CONNECTION_IS_TLS13(s) &&
      !SSL_CONNECTION_IS_DTLS(s) && s->ext.session_secret_cb != NULL) {
...
static int tls_handle_status_request(SSL_CONNECTION *s) {
  ...
  if (s->ext.status_type != TLSEXT_STATUSTYPE_nothing && sctx != NULL &&
    ...
    if (s->s3.tmp.cert != NULL) {
      ...
      ret =
      sctx->ext.status_cb(SSL_CONNECTION_GET_USER_SSL(s), sctx->ext.status_arg);
      switch (ret) {
...
int tls_handle_alpn(SSL_CONNECTION *s) {
  ...
  if (sctx->ext.alpn_select_cb != NULL && s->s3.alpn_proposed != NULL) {
    int r = sctx->ext.alpn_select_cb(SSL_CONNECTION_GET_USER_SSL(s), &selected,
                                     &selected_len, s->s3.alpn_proposed,
    ...
    if (!s->hit || SSL_CONNECTION_IS_TLS13(s)) {
      ...
      if (!s->hit) {
        ...
        if (s->not_resumable_session_cb != NULL)
          ...
          ussl, ((s->s3.tmp.new_cipher->algorithm_mkey &
...
CON_FUNC_RETURN tls_construct_server_hello(SSL_CONNECTION *s, WPACKET *pkt) {
  ...
  int usetls13 =
  SSL_CONNECTION_IS_TLS13(s) || s->hello_retry_request == SSL_HRR_PENDING;
  ...
  if (!WPACKET_sub_memcpy_u8(pkt, session_id, sl) ||
      !SSL_CONNECTION_GET_SSL(s)->method->put_cipher_by_char(
      s->s3.tmp.new_cipher, pkt, &len) ||
...
CON_FUNC_RETURN tls_construct_server_key_exchange(SSL_CONNECTION *s,
  ...
  if (type & (SSL_kPSK | SSL_kRSAPSK)) {
    ...
    if (type & (SSL_kDHE | SSL_kDHEPSK)) {
      ...
      if (!ssl_security(s, SSL_SECOP_TMP_DH, EVP_PKEY_get_security_bits(pkdhp),
                        0, pkdhp)) {
      ...
      if (!EVP_PKEY_get_bn_param(s->s3.tmp.pkey, OSSL_PKEY_PARAM_FFC_P,
          ...
          !EVP_PKEY_get_bn_param(s->s3.tmp.pkey, OSSL_PKEY_PARAM_FFC_G,
                                 &r[1]) ||
          !EVP_PKEY_get_bn_param(s->s3.tmp.pkey, OSSL_PKEY_PARAM_PUB_KEY,
                                 &r[2])) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        goto err;
      }
      ...
      encodedlen =
      EVP_PKEY_get1_encoded_public_key(s->s3.tmp.pkey, &encodedPoint);
      if (encodedlen == 0) {
  ...
  if (type & (SSL_kECDHE | SSL_kECDHEPSK)) {
    ...
    if (!WPACKET_put_bytes_u8(pkt, NAMED_CURVE_TYPE) ||
        !WPACKET_put_bytes_u8(pkt, 0) || !WPACKET_put_bytes_u8(pkt, curve_id) ||
...
CON_FUNC_RETURN tls_construct_certificate_request(SSL_CONNECTION *s,
  ...
  if (SSL_CONNECTION_IS_TLS13(s)) {
    ...
    if (s->post_handshake_auth == SSL_PHA_REQUEST_PENDING) {
      ...
      if (RAND_bytes_ex(SSL_CONNECTION_GET_CTX(s)->libctx, s->pha_context,
...
static int tls_process_cke_psk_preamble(SSL_CONNECTION *s, PACKET *pkt) {
  ...
  psklen = s->psk_server_callback(SSL_CONNECTION_GET_USER_SSL(s),
                                  s->session->psk_identity, psk, sizeof(psk));
  ...
  *p++ = OSSL_PARAM_construct_uint(OSSL_ASYM_CIPHER_PARAM_TLS_CLIENT_VERSION,
  ...
  if ((s->options & SSL_OP_TLS_ROLLBACK_BUG) != 0)
    *p++ = OSSL_PARAM_construct_uint(
    OSSL_ASYM_CIPHER_PARAM_TLS_NEGOTIATED_VERSION, (unsigned int *)&s->version);
  *p++ = OSSL_PARAM_construct_end();

  if (!EVP_PKEY_CTX_set_params(ctx, params) ||
      EVP_PKEY_decrypt(ctx, rsa_decrypt, &outlen, PACKET_data(&enc_premaster),
...
static int tls_process_cke_gost18(SSL_CONNECTION *s, PACKET *pkt) {
  ...
  if (EVP_PKEY_CTX_ctrl(pkey_ctx, -1, EVP_PKEY_OP_DECRYPT, EVP_PKEY_CTRL_CIPHER,
                        cipher_nid, NULL) <= 0) {
    SSLfatal(s, SSL_AD_INTERNAL_ERROR, SSL_R_LIBRARY_BUG);
    goto err;
  }
...
WORK_STATE tls_post_process_client_key_exchange(SSL_CONNECTION *s,
  ...
  if (wst == WORK_MORE_A) {
    if (SSL_CONNECTION_IS_DTLS(s)) {
      ...
      if (SSL_export_keying_material(SSL_CONNECTION_GET_SSL(s), sctpauthkey,
                                     sizeof(sctpauthkey), labelbuffer, labellen,
                                     NULL, 0, 0) <= 0) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        return WORK_ERROR;
      }

      BIO_ctrl(s->wbio, BIO_CTRL_DGRAM_SCTP_ADD_AUTH_KEY, sizeof(sctpauthkey),
               sctpauthkey);
    }
  }
#endif
...
MSG_PROCESS_RETURN tls_process_client_certificate(SSL_CONNECTION *s,
  ...
  for (chainidx = 0; PACKET_remaining(&spkt) > 0; chainidx++) {
    ...
    if (SSL_CONNECTION_IS_TLS13(s)) {
      ...
      if (!tls_collect_extensions(s, &extensions, SSL_EXT_TLS1_3_CERTIFICATE,
                                  &rawexts, NULL, chainidx == 0) ||
          !tls_parse_all_extensions(s, SSL_EXT_TLS1_3_CERTIFICATE, rawexts, x,
                                    chainidx, PACKET_remaining(&spkt) == 0)) {
  ...
  /* Save the current hash state for when we receive the CertificateVerify */
  if (SSL_CONNECTION_IS_TLS13(s)) {
    if (!ssl_handshake_hash(s, s->cert_verify_hash, sizeof(s->cert_verify_hash),
...
#ifndef OPENSSL_NO_COMP_ALG
MSG_PROCESS_RETURN tls_process_client_compressed_certificate(SSL_CONNECTION *sc,
                                                             PACKET *pkt) {
  MSG_PROCESS_RETURN ret = MSG_PROCESS_ERROR;
  PACKET tmppkt;
  BUF_MEM *buf = BUF_MEM_new();

  if (tls13_process_compressed_certificate(sc, pkt, &tmppkt, buf) !=
      MSG_PROCESS_ERROR)
...
CON_FUNC_RETURN tls_construct_server_certificate(SSL_CONNECTION *s,
                                                 WPACKET *pkt) {
...
#ifndef OPENSSL_NO_COMP_ALG
CON_FUNC_RETURN tls_construct_server_compressed_certificate(SSL_CONNECTION *sc,
                                                            WPACKET *pkt) {
  ...
  if (!WPACKET_put_bytes_u16(pkt, alg) ||
      ...
      !WPACKET_memcpy(pkt, cc->data, cc->len) || !WPACKET_close(pkt))
    return 0;
...
static CON_FUNC_RETURN construct_stateless_ticket(SSL_CONNECTION *s,
  ...
  const_p = senc;
  sess =
  d2i_SSL_SESSION_ex(NULL, &const_p, slen_full, sctx->libctx, sctx->propq);
  if (sess == NULL) {
    SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
    goto err;
  }
...
#endif
  {
    ...
    if (tctx->ext.ticket_key_evp_cb != NULL)
      ret = tctx->ext.ticket_key_evp_cb(ssl, key_name, iv, ctx,
                                        ssl_hmac_get0_EVP_MAC_CTX(hctx), 1);
#ifndef OPENSSL_NO_DEPRECATED_3_0
    ...
    EVP_CIPHER *cipher =
    EVP_CIPHER_fetch(sctx->libctx, "AES-256-CBC", sctx->propq);
    ...
    if (iv_len < 0 || RAND_bytes_ex(sctx->libctx, iv, iv_len, 0) <= 0 ||
        !EVP_EncryptInit_ex(ctx, cipher, NULL, tctx->ext.secure->tick_aes_key,
                            iv) ||
  ...
  if (!WPACKET_get_total_written(pkt, &macoffset)
      ...
      !ssl_hmac_update(hctx, (unsigned char *)s->init_buf->data + macoffset,
                       macendoffset - macoffset) ||
      !WPACKET_reserve_bytes(pkt, EVP_MAX_MD_SIZE, &macdata1) ||
      !ssl_hmac_final(hctx, macdata1, &hlen, EVP_MAX_MD_SIZE) ||
      hlen > EVP_MAX_MD_SIZE || !WPACKET_allocate_bytes(pkt, hlen, &macdata2) ||
      macdata1 != macdata2) {
    SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
    goto err;
  }
...
CON_FUNC_RETURN tls_construct_new_session_ticket(SSL_CONNECTION *s,
                                                 WPACKET *pkt) {
  SSL_CTX *tctx = s->session_ctx;
  ...
  if (SSL_CONNECTION_IS_TLS13(s)) {
    ...
    if (RAND_bytes_ex(SSL_CONNECTION_GET_CTX(s)->libctx, age_add_u.age_add_c,
    ...
    if (!tls13_hkdf_expand(s, md, s->resumption_master_secret, nonce_label,
                           ...
                           TICKET_NONCE_SIZE, s->session->master_key, hashlen,
                           1)) {
      /* SSLfatal() already called */
      goto err;
    }
