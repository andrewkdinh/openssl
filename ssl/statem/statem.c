...
int SSL_in_before(const SSL *s)
{
     ...
     */
    return (sc->statem.hand_state == TLS_ST_BEFORE) && (sc->statem.state == MSG_FLOW_UNINITED);
}
...
static int state_machine(SSL_CONNECTION *s, int server)
{
    ...
    if (SSL_CONNECTION_IS_DTLS(s) && BIO_dgram_is_sctp(SSL_get_wbio(ssl))) {
         ...
         */
        BIO_ctrl(SSL_get_wbio(ssl), BIO_CTRL_DGRAM_SCTP_SET_IN_HANDSHAKE, st->in_handshake, NULL);
    }
#endif
...
static SUB_STATE_RETURN read_state_machine(SSL_CONNECTION *s)
{
    ...
    while (1) {
        ...
        case READ_STATE_HEADER:
            ...
            if (!SSL_CONNECTION_IS_DTLS(s) && s->s3.tmp.message_size > 0
...
static SUB_STATE_RETURN write_state_machine(SSL_CONNECTION *s)
{
    ...
    int (*get_construct_message_f)(SSL_CONNECTION *s, CON_FUNC_RETURN (**confunc)(SSL_CONNECTION *s, WPACKET *pkt),
                                   int *mt);
    void (*cb)(const SSL *ssl, int type, int val) = NULL;
