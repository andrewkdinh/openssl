...
int tls13_hkdf_expand(SSL_CONNECTION *s, const EVP_MD *md,
                      const unsigned char *secret, const unsigned char *label,
  ...
  ret = tls13_hkdf_expand_ex(sctx->libctx, sctx->propq, md, secret, label,
                             labellen, data, datalen, out, outlen, !fatal);
  if (ret == 0 && fatal)
...
int tls13_derive_key(SSL_CONNECTION *s, const EVP_MD *md,
  ...
  return tls13_hkdf_expand(s, md, secret, keylabel, sizeof(keylabel) - 1, NULL,
                           0, key, keylen, 1);
}
...
int tls13_derive_iv(SSL_CONNECTION *s, const EVP_MD *md,
  ...
  return tls13_hkdf_expand(s, md, secret, ivlabel, sizeof(ivlabel) - 1, NULL, 0,
                           iv, ivlen, 1);
}
...
int tls13_derive_finishedkey(SSL_CONNECTION *s, const EVP_MD *md,
  ...
  static const unsigned char finishedlabel[] =
  "\x66\x69\x6E\x69\x73\x68\x65\x64";
  ...
  *p++ =
  ...
  if (insecret != NULL)
    *p++ = OSSL_PARAM_construct_octet_string(
    OSSL_KDF_PARAM_KEY, (unsigned char *)insecret, insecretlen);
  if (prevsecret != NULL)
    *p++ = OSSL_PARAM_construct_octet_string(
    OSSL_KDF_PARAM_SALT, (unsigned char *)prevsecret, mdlen);
  *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_PREFIX,
                                           (unsigned char *)label_prefix,
                                           sizeof(label_prefix) - 1);
  *p++ = OSSL_PARAM_construct_octet_string(
  OSSL_KDF_PARAM_LABEL, (unsigned char *)derived_secret_label,
...
size_t tls13_final_finish_mac(SSL_CONNECTION *s, const char *str, size_t slen,
  ...
  *p = OSSL_PARAM_construct_end();
  ...
  if (str ==
      SSL_CONNECTION_GET_SSL(s)->method->ssl3_enc->server_finished_label) {
    key = s->server_finished_secret;
  } else if (SSL_IS_FIRST_HANDSHAKE(s)) {
    key = s->client_finished_secret;
  } else {
    if (!tls13_derive_finishedkey(s, md, s->client_app_traffic_secret,
                                  finsecret, hashlen))
      goto err;
    key = finsecret;
  }

  if (!EVP_Q_mac(sctx->libctx, "HMAC", sctx->propq, mdname, params, key,
...
static int derive_secret_key_and_iv(
SSL_CONNECTION *s, const EVP_MD *md, const EVP_CIPHER *ciph, int mac_type,
...
int tls13_change_cipher_state(SSL_CONNECTION *s, int which) {
  ...
  if (((which & SSL3_CC_CLIENT) && (which & SSL3_CC_WRITE)) ||
    ...
    if ((which & SSL3_CC_EARLY) != 0) {
      ...
      if (((EVP_CIPHER_flags(cipher) & EVP_CIPH_FLAG_AEAD_CIPHER) == 0) &&
          (!ssl_cipher_get_evp_md_mac(sctx, sslcipher, &mac_md, &mac_pkey_type,
                                      NULL))) {
        SSLfatal_alert(s, SSL_AD_INTERNAL_ERROR);
        goto err;
      }
      ...
      if (!tls13_hkdf_expand(s, md, insecret, early_exporter_master_secret,
                             sizeof(early_exporter_master_secret) - 1, hashval,
                             hashlen, s->early_exporter_master_secret, hashlen,
                             1)) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        goto err;
      }
  ...
  if (label == client_application_traffic) {
    ...
    if (!tls13_hkdf_expand(s, ssl_handshake_md(s), insecret,
                           ...
                           sizeof(resumption_master_secret) - 1, hashval,
                           hashlen, s->resumption_master_secret, hashlen, 1)) {
      /* SSLfatal() already called */
      goto err;
    }
  }
  ...
  if (!derive_secret_key_and_iv(s, md, cipher, mac_pkey_type, mac_md, insecret,
                                hash, label, labellen, secret, key, &keylen,
                                &iv, &ivlen, &taglen)) {
    /* SSLfatal() already called */
    goto err;
  }
  ...
  if (label == server_application_traffic) {
    ...
    if (!tls13_hkdf_expand(s, ssl_handshake_md(s), insecret,
                           ...
                           sizeof(exporter_master_secret) - 1, hash, hashlen,
  ...
  if (!ssl_set_new_record_layer(
      s, s->version, direction, level, secret, hashlen, key, keylen, iv, ivlen,
      NULL, 0, cipher, taglen, mac_pkey_type, mac_md, NULL, md)) {
    /* SSLfatal already called */
    goto err;
  }
...
int tls13_update_key(SSL_CONNECTION *s, int sending) {
  ...
  int ret = 0, l;
  int direction =
  sending ? OSSL_RECORD_DIRECTION_WRITE : OSSL_RECORD_DIRECTION_READ;
  unsigned char iv_intern[EVP_MAX_IV_LENGTH];
  unsigned char *iv = iv_intern;
  ...
  if (!ssl_set_new_record_layer(
      s, s->version, direction, OSSL_RECORD_PROTECTION_LEVEL_APPLICATION,
      insecret, hashlen, key, keylen, iv, ivlen, NULL, 0, s->s3.tmp.new_sym_enc,
  ...
  log_label = s->server == sending ? SERVER_APPLICATION_N_LABEL
                                   : CLIENT_APPLICATION_N_LABEL;
  if (!ssl_log_secret(s, log_label, secret, hashlen)) {
    /* SSLfatal() already called */
    goto err;
  }
...
int tls13_export_keying_material(SSL_CONNECTION *s, unsigned char *out,
                                 size_t olen, const char *label, size_t llen,
                                 const unsigned char *context,
                                 size_t contextlen, int use_context) {
  unsigned char exportsecret[EVP_MAX_MD_SIZE];
  /* ASCII: "exporter", in hex for EBCDIC compatibility */
  static const unsigned char exporterlabel[] =
  "\x65\x78\x70\x6F\x72\x74\x65\x72";
...
int tls13_export_keying_material_early(SSL_CONNECTION *s, unsigned char *out,
   ...
   */
  if (md == NULL || EVP_DigestInit_ex(ctx, md, NULL) <= 0 ||
      EVP_DigestUpdate(ctx, context, contextlen) <= 0 ||
      EVP_DigestFinal_ex(ctx, hash, &hashsize) <= 0 ||
      EVP_DigestInit_ex(ctx, md, NULL) <= 0 ||
      EVP_DigestFinal_ex(ctx, data, &datalen) <= 0 ||
