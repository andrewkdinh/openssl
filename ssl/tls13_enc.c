    ...
    *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_LABEL,
    ...
    if (data != NULL)
        *p++ = OSSL_PARAM_construct_octet_string(
            OSSL_KDF_PARAM_DATA, (unsigned char *)data, datalen);
...
static int derive_secret_key_and_iv(
    ...
    const EVP_MD *mac_md, const unsigned char *insecret,
    const unsigned char *hash, const unsigned char *label, size_t labellen,
    unsigned char *secret, unsigned char *key, size_t *keylen,
    unsigned char **iv, size_t *ivlen, size_t *taglen)
{
    ...
    if (EVP_CIPHER_is_a(ciph, "NULL") && mac_md != NULL
        && mac_type == NID_hmac) {
...
int tls13_change_cipher_state(SSL_CONNECTION *s, int which)
{
    ...
    if (label == server_application_traffic) {
        ...
        if (!tls13_hkdf_expand(
                s, ssl_handshake_md(s), insecret, exporter_master_secret,
    ...
    if (!ssl_set_new_record_layer(
            s, s->version, direction, level, secret, hashlen, key, keylen, iv,
            ivlen, NULL, 0, cipher, taglen, mac_pkey_type, mac_md, NULL, md)) {
        /* SSLfatal already called */
        goto err;
    }
...
int tls13_update_key(SSL_CONNECTION *s, int sending)
{
    ...
    if (!derive_secret_key_and_iv(
            s, md, s->s3.tmp.new_sym_enc, s->s3.tmp.new_mac_pkey_type,
            s->s3.tmp.new_hash, insecret, NULL, application_traffic,
            sizeof(application_traffic) - 1, secret, key, &keylen, &iv, &ivlen,
            &taglen)) {
        /* SSLfatal() already called */
        goto err;
    }
    ...
    if (!ssl_set_new_record_layer(
            ...
            s->s3.tmp.new_sym_enc, taglen, NID_undef, NULL, NULL, md)) {
        /* SSLfatal already called */
        goto err;
    }
                                 ...
                                 size_t contextlen, int use_context)
{
    ...
    if (EVP_DigestInit_ex(ctx, md, NULL) <= 0
        ...
        || !tls13_hkdf_expand(s, md, exportsecret, exporterlabel,
                              sizeof(exporterlabel) - 1, hash, hashsize, out,
                              olen, 0))
        goto err;
