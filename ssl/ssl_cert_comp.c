...
__owur static OSSL_COMP_CERT *
OSSL_COMP_CERT_from_uncompressed_data(unsigned char *data, size_t len,
                                      int alg) {
    OSSL_COMP_CERT *ret = NULL;
    size_t max_length;
    int comp_length;
    ...
    if ((max_length = ossl_calculate_comp_expansion(alg, len)) == 0 ||
        method == NULL || (comp_ctx = COMP_CTX_new(method)) == NULL ||
...
static int ssl_set_cert_comp_pref(int *prefs, int *algs, size_t len) {
    ...
    /* Include only those algorithms we support, ignoring duplicates and
     * unknowns
     */
    for (i = 0; i < len; i++) {
...
static int ssl_compress_certs(SSL *ssl, CERT_PKEY *cpks, int alg_in) {
    ...
    for (i = 0; i < TLSEXT_comp_cert_limit; i++) {
        /*
         * alg = 0 means compress for everything, but only for algorithms
         * enabled alg != 0 means compress for that algorithm if enabled
         */
