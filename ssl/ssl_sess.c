...
int SSL_CTX_add_session(SSL_CTX *ctx, SSL_SESSION *c) {
    ...
    /* Adjust last used time, and add back into the cache at the appropriate
     * spot
     */
    ...
    if (s == NULL) {
         ...
         * delete cache entry *before* add, so we don't remove the one we're
         * adding!
         */
...
EVP_PKEY *SSL_SESSION_get0_peer_rpk(SSL_SESSION *s) { return s->peer_rpk; }
