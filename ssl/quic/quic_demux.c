...
QUIC_DEMUX *ossl_quic_demux_new(BIO *net_bio, size_t short_conn_id_len, OSSL_TIME (*now)(void *arg), void *now_arg)
{
    QUIC_DEMUX *demux;
    ...
    if (net_bio != NULL && BIO_dgram_get_local_addr_cap(net_bio) && BIO_dgram_set_local_addr_enable(net_bio, 1))
...
static QUIC_URXE *demux_resize_urxe(QUIC_DEMUX *demux, QUIC_URXE *e, size_t new_alloc_len)
{
    QUIC_URXE *e2, *prev;
...
static QUIC_URXE *demux_reserve_urxe(QUIC_DEMUX *demux, QUIC_URXE *e, size_t alloc_len)
{
...
static int demux_recv(QUIC_DEMUX *demux)
{
     ...
     */
    for (i = 0; i < (ossl_ssize_t)OSSL_NELEM(msg); ++i, urxe = ossl_list_urxe_next(urxe)) {
        if (urxe == NULL) {
...
static int demux_identify_conn_id(QUIC_DEMUX *demux, QUIC_URXE *e, QUIC_CONN_ID *dst_conn_id)
{
    return ossl_quic_wire_get_pkt_hdr_dst_conn_id(ossl_quic_urxe_data(e), e->data_len, demux->short_conn_id_len,
                                                  dst_conn_id);
}
...
static int demux_process_pending_urxe(QUIC_DEMUX *demux, QUIC_URXE *e)
{
    ...
    if (demux->default_cb != NULL) {
        ...
        demux->default_cb(e, demux->default_cb_arg, dst_conn_id_ok ? &dst_conn_id : NULL);
    } else {
        /* Discard. */
        ossl_list_urxe_insert_tail(&demux->urx_free, e);
        e->demux_state = URXE_DEMUX_STATE_FREE;
...
int ossl_quic_demux_inject(QUIC_DEMUX *demux, const unsigned char *buf, size_t buf_len, const BIO_ADDR *peer,
                           const BIO_ADDR *local)
{
    ...
    urxe->time = demux->now != NULL ? demux->now(demux->now_arg) : ossl_time_zero();
