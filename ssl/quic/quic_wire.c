...
int ossl_quic_frame_ack_contains_pn(const OSSL_QUIC_FRAME_ACK *ack,
                                    QUIC_PN pn) {
  size_t i;
...
int ossl_quic_wire_encode_frame_streams_blocked(WPACKET *pkt, char is_uni,
                                                uint64_t max_streams) {
  if (!encode_frame_hdr(pkt, is_uni
...
int ossl_quic_wire_encode_frame_conn_close(
WPACKET *pkt, const OSSL_QUIC_FRAME_CONN_CLOSE *f) {
  if (!encode_frame_hdr(pkt, f->is_app
...
int ossl_quic_wire_peek_frame_ack_num_ranges(const PACKET *orig_pkt,
  ...
  if (!expect_frame_header_mask(&pkt, OSSL_QUIC_FRAME_TYPE_ACK_WITHOUT_ECN, 1,
                                NULL) ||
...
int ossl_quic_wire_decode_frame_max_streams(PACKET *pkt,
  ...
  if (!expect_frame_header_mask(pkt, OSSL_QUIC_FRAME_TYPE_MAX_STREAMS_BIDI, 1,
                                NULL) ||
...
int ossl_quic_wire_decode_frame_new_conn_id(PACKET *pkt,
  ...
  if (!expect_frame_header(pkt, OSSL_QUIC_FRAME_TYPE_NEW_CONN_ID) ||
      ...
      f->seq_num < f->retire_prior_to || !PACKET_get_1(pkt, &len) || len < 1 ||
...
int ossl_quic_wire_decode_transport_param_int(PACKET *pkt, uint64_t *id,
  ...
  sub.curr =
  ossl_quic_wire_decode_transport_param_bytes(pkt, id, &sub.remaining);
  if (sub.curr == NULL)
    return 0;
...
PACKET *pkt, QUIC_PREFERRED_ADDR *p) {
  const unsigned char *body;
  uint64_t id;
  size_t len = 0;
  PACKET pkt2;
  ...
  if (!PACKET_copy_bytes(&pkt2, p->ipv4, sizeof(p->ipv4)) ||
      ...
      !PACKET_get_net_2(&pkt2, &ipv6_port) || !PACKET_get_1(&pkt2, &cidl) ||
