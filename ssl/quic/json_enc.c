...
static int wbuf_flush(struct json_write_buf *wbuf, int full) {
  ...
  while (total_written < wbuf->cur) {
    if (!BIO_write_ex(wbuf->bio, wbuf->buf + total_written,
                      wbuf->cur - total_written, &written)) {
...
static int json_push(OSSL_JSON_ENC *json, unsigned int v) {
  ...
  if (json->stack_end_byte >= json->stack_bytes) {
    size_t new_size = (json->stack_bytes == 0) ? OSSL_NELEM(json->stack_small)
                                               : (json->stack_bytes * 2);
...
int ossl_json_in_error(OSSL_JSON_ENC *json) { return json->error; }
...
static void json_raise_error(OSSL_JSON_ENC *json) { json->error = 1; }
 ...
 */
void ossl_json_object_end(OSSL_JSON_ENC *json) { composite_end(json, 0, '}'); }
...
void ossl_json_array_end(OSSL_JSON_ENC *json) { composite_end(json, 1, ']'); }
...
void ossl_json_u64(OSSL_JSON_ENC *json, uint64_t v) { json_u64(json, v, 0); }
...
void ossl_json_i64(OSSL_JSON_ENC *json, int64_t value) {
  ...
  uv =
  (value == INT64_MIN) ? ((uint64_t)-(INT64_MIN + 1)) + 1 : (uint64_t)-value;
...
static ossl_inline void json_write_qstring_inner(OSSL_JSON_ENC *json,
                                                 const char *str,
                                                 size_t str_len, int nul_term) {
  char c, *o, obuf[7];
  unsigned char *u_str;
  int i;
  size_t j;
  ...
  for (j = nul_term ? strlen(str) : str_len; j > 0; str++, j--) {
    ...
    default:
      ...
      if (u_str[0] >= 0xc2 && u_str[0] <= 0xdf && j >= 2 && u_str[1] >= 0x80 &&
          u_str[1] <= 0xbf) {
        memcpy(obuf, str, 2);
        obuf[2] = '\0';
        str++, j--;
      ...
      if (u_str[0] >= 0xe0 && u_str[0] <= 0xef && j >= 3 && u_str[1] >= 0x80 &&
          u_str[1] <= 0xbf && u_str[2] >= 0x80 && u_str[2] <= 0xbf &&
      ...
      if (u_str[0] >= 0xf0 && u_str[0] <= 0xf4 && j >= 4 && u_str[1] >= 0x80 &&
          u_str[1] <= 0xbf && u_str[2] >= 0x80 && u_str[2] <= 0xbf &&
...
static void json_write_qstring_len(OSSL_JSON_ENC *json, const char *str,
                                   size_t str_len) {
