...
int ossl_quic_obj_can_support_blocking(const QUIC_OBJ *obj)
{
    ...
    if ((obj->domain_flags & (SSL_DOMAIN_FLAG_LEGACY_BLOCKING | SSL_DOMAIN_FLAG_BLOCKING)) == 0)
        return 0;
...
int ossl_quic_obj_desires_blocking(const QUIC_OBJ *obj)
{
    ...
    assert(obj != NULL);
    for (; (req_blocking_mode = obj->req_blocking_mode) == QUIC_BLOCKING_MODE_INHERIT && obj->parent_obj != NULL;
         obj = obj->parent_obj)
        ;
