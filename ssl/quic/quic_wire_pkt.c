...
int ossl_quic_hdr_protector_init(QUIC_HDR_PROTECTOR *hpr, OSSL_LIB_CTX *libctx, const char *propq, uint32_t cipher_id,
                                 const unsigned char *quic_hp_key, size_t quic_hp_key_len)
{
    ...
    if (hpr->cipher == NULL || quic_hp_key_len != (size_t)EVP_CIPHER_get_key_length(hpr->cipher)) {
        ERR_raise(ERR_LIB_SSL, ERR_R_EVP_LIB);
        goto err;
    }

    if (!EVP_CipherInit_ex(hpr->cipher_ctx, hpr->cipher, NULL, quic_hp_key, NULL, 1)) {
        ERR_raise(ERR_LIB_SSL, ERR_R_EVP_LIB);
        goto err;
    }
...
static int hdr_generate_mask(QUIC_HDR_PROTECTOR *hpr, const unsigned char *sample, size_t sample_len,
                             unsigned char *mask)
{
    int l = 0;
    ...
    if (hpr->cipher_id == QUIC_HDR_PROT_CIPHER_AES_128 || hpr->cipher_id == QUIC_HDR_PROT_CIPHER_AES_256) {
        if (sample_len < 16) {
        ...
        if (!EVP_CipherInit_ex(hpr->cipher_ctx, NULL, NULL, NULL, sample, 1)
            || !EVP_CipherUpdate(hpr->cipher_ctx, mask, &l, zeroes, sizeof(zeroes))) {
            ERR_raise(ERR_LIB_SSL, ERR_R_EVP_LIB);
            return 0;
        }
    } else {
...
{
    return ossl_quic_hdr_protector_decrypt_fields(hpr, ptrs->raw_sample, ptrs->raw_sample_len, ptrs->raw_start,
                                                  ptrs->raw_pn);
}

int ossl_quic_hdr_protector_decrypt_fields(QUIC_HDR_PROTECTOR *hpr, const unsigned char *sample, size_t sample_len,
                                           unsigned char *first_byte, unsigned char *pn_bytes)
{
...
{
    return ossl_quic_hdr_protector_encrypt_fields(hpr, ptrs->raw_sample, ptrs->raw_sample_len, ptrs->raw_start,
                                                  ptrs->raw_pn);
}

int ossl_quic_hdr_protector_encrypt_fields(QUIC_HDR_PROTECTOR *hpr, const unsigned char *sample, size_t sample_len,
                                           unsigned char *first_byte, unsigned char *pn_bytes)
{
    unsigned char mask[5], pn_len, i;
...
int ossl_quic_wire_decode_pkt_hdr(PACKET *pkt, size_t short_conn_id_len, int partial, int nodata, QUIC_PKT_HDR *hdr,
                                  QUIC_PKT_HDR_PTRS *ptrs, uint64_t *fail_cause)
{
    ...
    if ((b0 & 0x80) == 0) {
        ...
        if (!PACKET_get_1(pkt, &dst_conn_id_len) || dst_conn_id_len > QUIC_MAX_CONN_ID_LEN
            || !PACKET_copy_bytes(pkt, hdr->dst_conn_id.id, dst_conn_id_len) || !PACKET_get_1(pkt, &src_conn_id_len)
            || src_conn_id_len > QUIC_MAX_CONN_ID_LEN || !PACKET_copy_bytes(pkt, hdr->src_conn_id.id, src_conn_id_len))
            return 0;
        ...
        if (version == 0) {
            ...
            if (hdr->type == QUIC_PKT_TYPE_INITIAL) {
                ...
                if (!PACKET_get_quic_vlint(pkt, &token_len) || token_len > SIZE_MAX
            ...
            if (hdr->type == QUIC_PKT_TYPE_RETRY) {
                ...
                if (!PACKET_get_quic_vlint(pkt, &len) || len < sizeof(hdr->pn))
                    return 0;
                                  ...
                                  QUIC_PKT_HDR_PTRS *ptrs)
{
    ...
    if (hdr->partial || (hdr->type == QUIC_PKT_TYPE_1RTT && hdr->dst_conn_id.id_len != short_conn_id_len))
        return 0;
    ...
    if (hdr->type == QUIC_PKT_TYPE_1RTT) {
         ...
         * invalid PN length.
         */
        if (hdr->dst_conn_id.id_len != short_conn_id_len || short_conn_id_len > QUIC_MAX_CONN_ID_LEN || hdr->pn_len < 1
            || hdr->pn_len > 4)
            return 0;

        b0 = (hdr->spin_bit << 5) | (hdr->key_phase << 2) | (hdr->pn_len - 1) | (hdr->reserved << 3)
            | 0x40; /* fixed bit */

        if (!WPACKET_put_bytes_u8(pkt, b0) || !WPACKET_memcpy(pkt, hdr->dst_conn_id.id, short_conn_id_len)
            || !WPACKET_get_total_written(pkt, &off_pn) || !WPACKET_memcpy(pkt, hdr->pn, hdr->pn_len))
            return 0;
    } else {
        /* Long header. */
        unsigned int raw_type;

        if (hdr->dst_conn_id.id_len > QUIC_MAX_CONN_ID_LEN || hdr->src_conn_id.id_len > QUIC_MAX_CONN_ID_LEN)
            return 0;

        if (ossl_quic_pkt_type_has_pn(hdr->type) && (hdr->pn_len < 1 || hdr->pn_len > 4))
            return 0;
        ...
        if (!WPACKET_put_bytes_u8(pkt, b0) || !WPACKET_put_bytes_u32(pkt, hdr->version)
            || !WPACKET_put_bytes_u8(pkt, hdr->dst_conn_id.id_len)
            || !WPACKET_memcpy(pkt, hdr->dst_conn_id.id, hdr->dst_conn_id.id_len)
            || !WPACKET_put_bytes_u8(pkt, hdr->src_conn_id.id_len)
            || !WPACKET_memcpy(pkt, hdr->src_conn_id.id, hdr->src_conn_id.id_len))
            return 0;
        ...
        if (hdr->type == QUIC_PKT_TYPE_INITIAL) {
            if (!WPACKET_quic_write_vlint(pkt, hdr->token_len) || !WPACKET_memcpy(pkt, hdr->token, hdr->token_len))
                return 0;
        }
        ...
        if (!WPACKET_quic_write_vlint(pkt, hdr->len + hdr->pn_len) || !WPACKET_get_total_written(pkt, &off_pn)
    ...
    if (ptrs != NULL) {
        ...
        ptrs->raw_sample_len = WPACKET_get_curr(pkt) + hdr->len - ptrs->raw_sample;
...
int ossl_quic_wire_get_pkt_hdr_dst_conn_id(const unsigned char *buf, size_t buf_len, size_t short_conn_id_len,
                                           QUIC_CONN_ID *dst_conn_id)
{
    unsigned char b0;
    size_t blen;

    if (buf_len < QUIC_MIN_VALID_PKT_LEN || short_conn_id_len > QUIC_MAX_CONN_ID_LEN)
        return 0;
    ...
    if ((b0 & 0x80) != 0) {
        ...
        if (blen > QUIC_MAX_CONN_ID_LEN || buf_len < QUIC_MIN_VALID_PKT_LEN + blen)
            return 0;
...
int ossl_quic_wire_decode_pkt_hdr_pn(const unsigned char *enc_pn, size_t enc_pn_len, QUIC_PN largest_pn,
                                     QUIC_PN *res_pn)
{
    ...
    case 2:
        ...
        break;
    case 3:
        truncated_pn = ((QUIC_PN)enc_pn[0] << 16) | ((QUIC_PN)enc_pn[1] << 8) | (QUIC_PN)enc_pn[2];
        break;
    case 4:
        truncated_pn =
            ((QUIC_PN)enc_pn[0] << 24) | ((QUIC_PN)enc_pn[1] << 16) | ((QUIC_PN)enc_pn[2] << 8) | (QUIC_PN)enc_pn[3];
        break;
    default:
        return 0;
    }
    ...
    if (candidate_pn <= expected_pn - pn_hwin && candidate_pn < (((int64_t)1) << 62) - pn_win)
...
{
    uint64_t num_unacked = (largest_acked == QUIC_PN_INVALID) ? pn + 1 : pn - largest_acked;
...
int ossl_quic_validate_retry_integrity_tag(OSSL_LIB_CTX *libctx, const char *propq, const QUIC_PKT_HDR *hdr,
                                           const QUIC_CONN_ID *client_initial_dcid)
{
    ...
    if (!ossl_quic_calculate_retry_integrity_tag(libctx, propq, hdr, client_initial_dcid, expected_tag))
        return 0;

    actual_tag = hdr->data + hdr->len - QUIC_RETRY_INTEGRITY_TAG_LEN;

    return !CRYPTO_memcmp(expected_tag, actual_tag, QUIC_RETRY_INTEGRITY_TAG_LEN);
}

/* RFC 9001 s. 5.8 */
static const unsigned char retry_integrity_key[] = {0xbe, 0x0c, 0x69, 0x0b, 0x9f, 0x66, 0x57, 0x5a,
                                                    0x1d, 0x76, 0x6b, 0x54, 0xe3, 0x68, 0xc8, 0x4e};

static const unsigned char retry_integrity_nonce[] = {0x46, 0x15, 0x99, 0xd3, 0x5d, 0x63,
                                                      0x2b, 0xf2, 0x23, 0x98, 0x25, 0xbb};

int ossl_quic_calculate_retry_integrity_tag(OSSL_LIB_CTX *libctx, const char *propq, const QUIC_PKT_HDR *hdr,
                                            const QUIC_CONN_ID *client_initial_dcid, unsigned char *tag)
{
    ...
    if (hdr->type != QUIC_PKT_TYPE_RETRY || hdr->version == 0 || hdr->len < QUIC_RETRY_INTEGRITY_TAG_LEN
        || hdr->data == NULL || client_initial_dcid == NULL || tag == NULL
    ...
    if (!WPACKET_put_bytes_u8(&wpkt, client_initial_dcid->id_len)
        || !WPACKET_memcpy(&wpkt, client_initial_dcid->id, client_initial_dcid->id_len)) {
        ERR_raise(ERR_LIB_SSL, ERR_R_CRYPTO_LIB);
        goto err;
    }

    /* Encode main retry header. */
    if (!ossl_quic_wire_encode_pkt_hdr(&wpkt, hdr2.dst_conn_id.id_len, &hdr2, NULL))
        goto err;
    ...
    if (!EVP_CipherInit_ex(cctx, cipher, NULL, retry_integrity_key, retry_integrity_nonce, /*enc=*/1)) {
        ERR_raise(ERR_LIB_SSL, ERR_R_EVP_LIB);
        goto err;
    }
    ...
    if (EVP_CipherUpdate(cctx, NULL, &l, hdr->data, hdr->len - QUIC_RETRY_INTEGRITY_TAG_LEN) != 1) {
        ERR_raise(ERR_LIB_SSL, ERR_R_EVP_LIB);
        goto err;
    }
    ...
    if (EVP_CIPHER_CTX_ctrl(cctx, EVP_CTRL_AEAD_GET_TAG, QUIC_RETRY_INTEGRITY_TAG_LEN, tag) != 1) {
        ERR_raise(ERR_LIB_SSL, ERR_R_EVP_LIB);
        goto err;
    }
