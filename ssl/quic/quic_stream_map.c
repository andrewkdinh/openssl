...
static void list_insert_tail(QUIC_STREAM_LIST_NODE *l,
  ...
  /* Must not be in list. */
  assert(n->prev == NULL && n->next == NULL && l->prev != NULL &&
         l->next != NULL);
...
static QUIC_STREAM *list_next(QUIC_STREAM_LIST_NODE *l,
                              QUIC_STREAM_LIST_NODE *n, size_t off) {
  assert(n->prev != NULL && n->next != NULL &&
         (n == l || (n->prev != n && n->next != n)) && l->prev != NULL &&
         l->next != NULL);
...
#define active_next(l, s)                                                      \
  list_next((l), &(s)->active_node, offsetof(QUIC_STREAM, active_node))
#define accept_next(l, s)                                                      \
  list_next((l), &(s)->accept_node, offsetof(QUIC_STREAM, accept_node))
#define ready_for_gc_next(l, s)                                                \
  list_next((l), &(s)->ready_for_gc_node,                                      \
            offsetof(QUIC_STREAM, ready_for_gc_node))
#define accept_head(l) list_next((l), (l), offsetof(QUIC_STREAM, accept_node))
#define ready_for_gc_head(l)                                                   \
  list_next((l), (l), offsetof(QUIC_STREAM, ready_for_gc_node))
...
int ossl_quic_stream_map_init(
QUIC_STREAM_MAP *qsm, uint64_t (*get_stream_limit_cb)(int uni, void *arg),
...
QUIC_STREAM *ossl_quic_stream_map_alloc(QUIC_STREAM_MAP *qsm,
  ...
  s->send_state =
  (ossl_quic_stream_is_local_init(s) || ossl_quic_stream_is_bidi(s))
...
static int qsm_ready_for_gc(QUIC_STREAM_MAP *qsm, QUIC_STREAM *qs) {
   ...
   */
  assert(!qs->deleted || !ossl_quic_stream_has_send(qs) ||
         ossl_quic_stream_send_is_reset(qs) ||
         ossl_quic_stream_send_get_final_size(qs, NULL));

  return qs->deleted &&
         (!ossl_quic_stream_has_recv(qs) || recv_stream_fully_drained ||
...
QUIC_STREAM_MAP *qsm, uint64_t stream_ordinal, int is_uni) {
...
void ossl_quic_stream_map_update_state(QUIC_STREAM_MAP *qsm, QUIC_STREAM *s) {
  ...
  if (ossl_quic_stream_is_server_init(s) == qsm->is_server) {
    ...
    ossl_quic_stream_map_is_local_allowed_by_stream_limit(qsm, stream_ordinal,
                                                          is_uni);
  }
  ...
  ((ossl_quic_stream_has_recv(s) && !ossl_quic_stream_recv_is_reset(s) &&
   ...
   s->want_stop_sending || s->want_reset_stream ||
...
static QUIC_RXFC *qsm_get_max_streams_rxfc(QUIC_STREAM_MAP *qsm,
                                           QUIC_STREAM *s) {
  return ossl_quic_stream_is_bidi(s) ? qsm->max_streams_bidi_rxfc
