...
QUIC_TXFC *ossl_quic_txfc_get_parent(QUIC_TXFC *txfc) { return txfc->parent; }
...
uint64_t ossl_quic_txfc_get_cwm(QUIC_TXFC *txfc) { return txfc->cwm; }

uint64_t ossl_quic_txfc_get_swm(QUIC_TXFC *txfc) { return txfc->swm; }
...
int ossl_quic_rxfc_init(QUIC_RXFC *rxfc, QUIC_RXFC *conn_rxfc,
                        uint64_t initial_window_size, uint64_t max_window_size,
                        OSSL_TIME (*now)(void *now_arg), void *now_arg) {
...
int ossl_quic_rxfc_init_standalone(QUIC_RXFC *rxfc,
    ...
    if (!ossl_quic_rxfc_init(rxfc, NULL, initial_window_size,
                             initial_window_size, now, now_arg))
        return 0;
...
QUIC_RXFC *ossl_quic_rxfc_get_parent(QUIC_RXFC *rxfc) { return rxfc->parent; }
...
int ossl_quic_rxfc_on_rx_stream_frame(QUIC_RXFC *rxfc, uint64_t end,
                                      int is_fin) {
    uint64_t delta;
...
static int rxfc_cwm_bump_desired(QUIC_RXFC *rxfc) {
    ...
    uint64_t threshold = safe_muldiv_uint64_t(
    rxfc->cur_window_size, WINDOW_THRESHOLD_NUM, WINDOW_THRESHOLD_DEN, &err);
...
static void rxfc_adjust_window_size(QUIC_RXFC *rxfc, uint64_t min_window_size,
    ...
    if (new_window_size >
        rxfc->max_window_size) /* takes precedence over min size */
...
static int rxfc_on_retire(QUIC_RXFC *rxfc, uint64_t num_bytes,
                          uint64_t min_window_size, OSSL_TIME rtt) {
...
int ossl_quic_rxfc_on_retire(QUIC_RXFC *rxfc, uint64_t num_bytes,
                             OSSL_TIME rtt) {
...
uint64_t ossl_quic_rxfc_get_cwm(const QUIC_RXFC *rxfc) { return rxfc->cwm; }

uint64_t ossl_quic_rxfc_get_swm(const QUIC_RXFC *rxfc) { return rxfc->swm; }

uint64_t ossl_quic_rxfc_get_rwm(const QUIC_RXFC *rxfc) { return rxfc->rwm; }
