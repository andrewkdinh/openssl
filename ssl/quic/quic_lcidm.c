...
QUIC_LCIDM *ossl_quic_lcidm_new(OSSL_LIB_CTX *libctx, size_t lcid_len) {
  ...
  if ((lcidm->conns =
       lh_QUIC_LCIDM_CONN_new(lcidm_conn_hash, lcidm_conn_comp)) == NULL)
    goto err;
...
static QUIC_LCIDM_CONN *lcidm_upsert_conn(const QUIC_LCIDM *lcidm,
                                          void *opaque) {
...
static int lcidm_generate(QUIC_LCIDM *lcidm, void *opaque, unsigned int type,
                          QUIC_CONN_ID *lcid_out, uint64_t *seq_num) {
  QUIC_LCIDM_CONN *conn;
...
int ossl_quic_lcidm_generate(QUIC_LCIDM *lcidm, void *opaque,
  ...
  return lcidm_generate(lcidm, opaque, LCID_TYPE_NCID, &ncid_frame->conn_id,
                        &ncid_frame->seq_num);
}
