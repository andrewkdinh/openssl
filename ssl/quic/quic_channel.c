...
static int ch_init(QUIC_CHANNEL *ch)
{
    ...
    if (!ossl_quic_rxfc_init(&ch->conn_rxfc,
                             NULL,
    ...
    if (!ossl_quic_stream_map_init(&ch->qsm,
                                   get_stream_limit,
                                   ch,
...
int ossl_quic_channel_schedule_new_token(QUIC_CHANNEL *ch, const unsigned char *token, size_t token_len)
{
    ...
    cfq_item = ossl_quic_cfq_add_frame(ch->cfq,
                                       1,
                                       QUIC_PN_SPACE_APP,
 ...
 */
enum rxku_decision { DECISION_RXKU_ONLY, DECISION_PROTOCOL_VIOLATION, DECISION_SOLICITED_TXKU };
...
static int ch_generate_transport_params(QUIC_CHANNEL *ch)
{
    ...
    if (!ossl_quic_wire_encode_transport_param_int(&wpkt,
...
static void ch_rx_handle_packet(QUIC_CHANNEL *ch, int channel_only)
{
    ...
    ossl_qlog_event_transport_packet_received(ch_get_qlog(ch),
                                              ch->qrx_pkt->hdr,
                                              ch->qrx_pkt->pn,
                                              &iovec,
                                              1,
...
static int ch_restart(QUIC_CHANNEL *ch)
{
     ...
     */
    return ossl_ackm_mark_packet_pseudo_lost(ch->ackm,
...
static void ch_start_terminating(QUIC_CHANNEL *ch, const QUIC_TERMINATE_CAUSE *tcause, int force_immediate)
{
    ...
    case QUIC_CHANNEL_STATE_ACTIVE:
        ...
        if (!force_immediate) {
            ch_record_state_transition(ch,
                                       tcause->remote ? QUIC_CHANNEL_STATE_TERMINATING_DRAINING
...
static int ch_enqueue_retire_conn_id(QUIC_CHANNEL *ch, uint64_t seq_num)
{
    ...
    if (ossl_quic_cfq_add_frame(ch->cfq,
                                1,
                                QUIC_PN_SPACE_APP,
 ...
 */
static int ch_on_new_conn_common(QUIC_CHANNEL       *ch,
                                 const BIO_ADDR     *peer,
...
int ossl_quic_channel_on_new_conn(QUIC_CHANNEL       *ch,
                                  const BIO_ADDR     *peer,
                                  const QUIC_CONN_ID *peer_scid,
                                  const QUIC_CONN_ID *peer_dcid)
{
 ...
 */
int ossl_quic_bind_channel(QUIC_CHANNEL       *ch,
                           const BIO_ADDR     *peer,
                           const QUIC_CONN_ID *peer_scid,
                           const QUIC_CONN_ID *peer_dcid,
                           const QUIC_CONN_ID *peer_odcid)
...
static int ch_init_new_stream(QUIC_CHANNEL *ch, QUIC_STREAM *qs, int can_send, int can_recv)
{
    ...
    if (!ossl_quic_rxfc_init(&qs->rxfc,
                             &ch->conn_rxfc,
                             rxfc_wnd,
...
QUIC_STREAM *ossl_quic_channel_new_stream_remote(QUIC_CHANNEL *ch, uint64_t stream_id)
{
    ...
    qs     = ossl_quic_stream_map_alloc(&ch->qsm,
                                    stream_id,
