...
static int ch_retry(QUIC_CHANNEL *ch, const unsigned char *retry_token, size_t retry_token_len,
                    const QUIC_CONN_ID *retry_scid, int drop_later_pn);
...
static int ch_on_transport_params(const unsigned char *params, size_t params_len, void *arg);
static int ch_on_handshake_alert(void *arg, unsigned char alert_code);
static int ch_on_handshake_complete(void *arg);
static int ch_on_handshake_yield_secret(uint32_t prot_level, int direction, uint32_t suite_id, EVP_MD *md,
                                        const unsigned char *secret, size_t secret_len, void *arg);
static int ch_on_crypto_recv_record(const unsigned char **buf, size_t *bytes_read, void *arg);
static int ch_on_crypto_release_record(size_t bytes_read, void *arg);
static int crypto_ensure_empty(QUIC_RSTREAM *rstream);
static int ch_on_crypto_send(const unsigned char *buf, size_t buf_len, size_t *consumed, void *arg);
...
static int ch_discard_el(QUIC_CHANNEL *ch, uint32_t enc_level);
static void ch_on_idle_timeout(QUIC_CHANNEL *ch);
static void ch_update_idle(QUIC_CHANNEL *ch);
static void ch_update_ping_deadline(QUIC_CHANNEL *ch);
static void ch_on_terminating_timeout(QUIC_CHANNEL *ch);
static void ch_start_terminating(QUIC_CHANNEL *ch, const QUIC_TERMINATE_CAUSE *tcause, int force_immediate);
static void ch_on_txp_ack_tx(const OSSL_QUIC_FRAME_ACK *ack, uint32_t pn_space, void *arg);
...
static int ch_init(QUIC_CHANNEL *ch)
{
    ...
    if (!ch->is_server && !ossl_quic_gen_rand_conn_id(ch->port->engine->libctx, tx_init_dcid_len, &ch->init_dcid))
        goto err;
    ...
    if (!ossl_quic_rxfc_init(&ch->conn_rxfc, NULL, DEFAULT_INIT_CONN_RXFC_WND,
                             DEFAULT_CONN_RXFC_MAX_WND_MUL * DEFAULT_INIT_CONN_RXFC_WND, get_time, ch))
        goto err;

    for (pn_space = QUIC_PN_SPACE_INITIAL; pn_space < QUIC_PN_SPACE_NUM; ++pn_space)
        if (!ossl_quic_rxfc_init_standalone(&ch->crypto_rxfc[pn_space], INIT_CRYPTO_RECV_BUF_LEN, get_time, ch))
            goto err;

    if (!ossl_quic_rxfc_init_standalone(&ch->max_streams_bidi_rxfc, DEFAULT_INIT_CONN_MAX_STREAMS, get_time, ch))
        goto err;

    if (!ossl_quic_rxfc_init_standalone(&ch->max_streams_uni_rxfc, DEFAULT_INIT_CONN_MAX_STREAMS, get_time, ch))
        goto err;
    ...
    if ((ch->ackm = ossl_ackm_new(get_time, ch, &ch->statm, ch->cc_method, ch->cc_data)) == NULL)
        goto err;

    if (!ossl_quic_stream_map_init(&ch->qsm, get_stream_limit, ch, &ch->max_streams_bidi_rxfc,
                                   &ch->max_streams_uni_rxfc, ch->is_server))
        goto err;

    ch->have_qsm = 1;

    if (!ch->is_server && !ossl_quic_lcidm_generate_initial(ch->lcidm, ch, &ch->init_scid))
        goto err;
    ...
    if (ch->qrx != NULL) {
         ...
         */
        if (!ossl_qrx_set_late_validation_cb(ch->qrx, rx_late_validate, ch))
            goto err;

        if (!ossl_qrx_set_key_update_cb(ch->qrx, rxku_detected, ch))
            goto err;
    }
...
void ossl_quic_channel_bind_qrx(QUIC_CHANNEL *tserver_ch, OSSL_QRX *qrx)
{
    if (tserver_ch->qrx == NULL && tserver_ch->is_tserver_ch == 1) {
        tserver_ch->qrx = qrx;
        ossl_qrx_set_late_validation_cb(tserver_ch->qrx, rx_late_validate, tserver_ch);
        ossl_qrx_set_key_update_cb(tserver_ch->qrx, rxku_detected, tserver_ch);
    }
}
...
int ossl_quic_channel_set_mutator(QUIC_CHANNEL *ch, ossl_mutate_packet_cb mutatecb,
...
int ossl_quic_channel_schedule_new_token(QUIC_CHANNEL *ch, const unsigned char *token, size_t token_len)
{
    int rc = 0;
    ...
    if (!ossl_quic_wire_encode_frame_new_token(&wpkt, token, token_len)) {
    ...
    cfq_item = ossl_quic_cfq_add_frame(ch->cfq, 1, QUIC_PN_SPACE_APP, OSSL_QUIC_FRAME_TYPE_NEW_TOKEN, 0,
...
QUIC_STREAM *ossl_quic_channel_get_stream_by_id(QUIC_CHANNEL *ch, uint64_t stream_id)
{
...
{
    return ossl_quic_channel_is_closing(ch) || ossl_quic_channel_is_draining(ch);
}
...
{
    return ossl_quic_channel_is_terminating(ch) || ossl_quic_channel_is_terminated(ch);
}

const QUIC_TERMINATE_CAUSE *ossl_quic_channel_get_terminate_cause(const QUIC_CHANNEL *ch)
{
...
{
    return ossl_quic_demux_has_pending(ch->port->demux) || ossl_qrx_processed_read_pending(ch->qrx);
}
...
{
    uint64_t next_pn = ossl_quic_tx_packetiser_get_next_pn(ch->txp, QUIC_PN_SPACE_APP);

    if (!ossl_quic_pn_valid(next_pn) || !ossl_qtx_trigger_key_update(ch->qtx)) {
        ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_INTERNAL_ERROR, 0, "key update");
        return;
    }
...
{
    if (ch->txku_in_progress && ossl_ackm_get_largest_acked(ch->ackm, QUIC_PN_SPACE_APP) >= ch->txku_pn) {
        ...
        ch->txku_cooldown_deadline = ossl_time_add(get_time(ch), ossl_time_multiply(pto, 3));
    }
...
static int txku_allowed(QUIC_CHANNEL *ch)
{
    return ch->tx_enc_level == QUIC_ENC_LEVEL_1RTT /* Sanity check. */
        ...
        && ch->handshake_confirmed && !txku_in_progress(ch);
}
...
static int txku_recommendable(QUIC_CHANNEL *ch)
{
    ...
    return
        ...
        && !ch->rxku_in_progress && !ch->rxku_pending_confirm;
}
...
static void rxku_detected(QUIC_PN pn, void *arg)
{
    ...
    if (decision == DECISION_PROTOCOL_VIOLATION) {
        ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_KEY_UPDATE_ERROR, 0, "RX key update again too soon");
        return;
    }
...
{
    if (!ch->rxku_in_progress || ossl_time_compare(get_time(ch), ch->rxku_update_end_deadline) < 0)
        return;
    ...
    if (!ossl_qrx_key_update_timeout(ch->qrx, /*normal=*/1))
        ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_INTERNAL_ERROR, 0, "RXKU cooldown internal error");
}
...
static int ch_on_crypto_recv_record(const unsigned char **buf, size_t *bytes_read, void *arg)
{
    ...
    for (i = QUIC_ENC_LEVEL_INITIAL; i < ch->rx_enc_level; ++i)
        if (i != QUIC_ENC_LEVEL_0RTT && !crypto_ensure_empty(ch->crypto_recv[ossl_quic_enc_level_to_pn_space(i)])) {
            ...
            ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_PROTOCOL_VIOLATION, OSSL_QUIC_FRAME_TYPE_CRYPTO,
    ...
    return ossl_quic_rstream_get_record(rstream, buf, bytes_read, &is_fin);
}
...
static int ch_on_crypto_release_record(size_t bytes_read, void *arg)
{
    ...
    if (!ossl_quic_rxfc_on_retire(&ch->crypto_rxfc[rx_pn_space], bytes_read, rtt_info.smoothed_rtt))
        return 0;
                                        ...
                                        const unsigned char *secret, size_t secret_len, void *arg)
{
    ...
    if (direction) {
        ...
        if (!ossl_qtx_provide_secret(ch->qtx, enc_level, suite_id, md, secret, secret_len))
            return 0;
        ...
        for (i = QUIC_ENC_LEVEL_INITIAL; i < enc_level; ++i)
            if (!crypto_ensure_empty(ch->crypto_recv[ossl_quic_enc_level_to_pn_space(i)])) {
                ...
                ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_PROTOCOL_VIOLATION,
                                                       OSSL_QUIC_FRAME_TYPE_CRYPTO, "crypto stream data in wrong EL");
                return 0;
            }

        if (!ossl_qrx_provide_secret(ch->qrx, enc_level, suite_id, md, secret, secret_len))
            return 0;
...
static int ch_on_handshake_complete(void *arg)
{
    ...
    if (!ch->got_remote_transport_params) {
         ...
         * params.
         */
        ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_CRYPTO_MISSING_EXT, OSSL_QUIC_FRAME_TYPE_CRYPTO,
    ...
    if (ch->pending_new_token != NULL) {
         ...
         */
        ossl_quic_channel_schedule_new_token(ch, ch->pending_new_token, ch->pending_new_token_len);
        OPENSSL_free(ch->pending_new_token);
...
static int ch_on_handshake_alert(void *arg, unsigned char alert_code)
{
     ...
     */
    if (alert_code == SSL_AD_UNEXPECTED_MESSAGE && ch->handshake_complete && ossl_quic_tls_is_cert_request(ch->qtls))
     ...
     * PROTOCOL_VIOLATION.
     */
    else if (alert_code == SSL_AD_ILLEGAL_PARAMETER && ch->handshake_complete
             && ossl_quic_tls_has_bad_max_early_data(ch->qtls))
        ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_PROTOCOL_VIOLATION, 0, "Bad max_early_data received");
    else
        ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_CRYPTO_ERR_BEGIN + alert_code, 0, "handshake alert");
...
{
    if (!ossl_quic_stream_is_bidi(s) || ossl_quic_stream_is_server_init(s))
        return;
...
static int ch_on_transport_params(const unsigned char *params, size_t params_len, void *arg)
{
    ...
    if (!PACKET_buf_init(&pkt, params, params_len)) {
        ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_INTERNAL_ERROR, 0, "internal error (packet buf init)");
        return 0;
    }
    ...
    while (PACKET_remaining(&pkt) > 0) {
        ...
        case QUIC_TPARAM_ACK_DELAY_EXP:
            ...
            if (!ossl_quic_wire_decode_transport_param_int(&pkt, &id, &v) || v > QUIC_MAX_ACK_DELAY_EXP) {
        ...
        case QUIC_TPARAM_MAX_ACK_DELAY:
            ...
            if (!ossl_quic_wire_decode_transport_param_int(&pkt, &id, &v) || v >= (((uint64_t)1) << 14)) {
        ...
        case QUIC_TPARAM_INITIAL_MAX_STREAMS_BIDI:
            ...
            if (!ossl_quic_wire_decode_transport_param_int(&pkt, &id, &v) || v > (((uint64_t)1) << 60)) {
        ...
        case QUIC_TPARAM_MAX_IDLE_TIMEOUT:
            ...
            ch->max_idle_timeout = min_u64_ignore_0(ch->max_idle_timeout_local_req, ch->max_idle_timeout_remote_req);
        ...
        case QUIC_TPARAM_MAX_UDP_PAYLOAD_SIZE:
            ...
            if (!ossl_quic_wire_decode_transport_param_int(&pkt, &id, &v) || v < QUIC_MIN_INITIAL_DGRAM_LEN) {
        ...
        case QUIC_TPARAM_ACTIVE_CONN_ID_LIMIT:
            ...
            if (!ossl_quic_wire_decode_transport_param_int(&pkt, &id, &v) || v < QUIC_MIN_ACTIVE_CONN_ID_LIMIT) {
        ...
        case QUIC_TPARAM_STATELESS_RESET_TOKEN:
            ...
            if (!ossl_quic_srtm_add(ch->srtm, ch, ch->cur_remote_seq_num, (const QUIC_STATELESS_RESET_TOKEN *)body)) {
    ...
    if (got_initial_scid)
        QLOG_CID("original_source_connection_id", &ch->init_dcid);
    if (got_retry_scid)
        QLOG_CID("retry_source_connection_id", &ch->retry_scid);
    if (got_initial_max_data)
        QLOG_U64("initial_max_data", ossl_quic_txfc_get_cwm(&ch->conn_txfc));
    if (got_initial_max_stream_data_bidi_local)
        QLOG_U64("initial_max_stream_data_bidi_local", ch->rx_init_max_stream_data_bidi_local);
    if (got_initial_max_stream_data_bidi_remote)
        QLOG_U64("initial_max_stream_data_bidi_remote", ch->rx_init_max_stream_data_bidi_remote);
    if (got_initial_max_stream_data_uni)
        QLOG_U64("initial_max_stream_data_uni", ch->rx_init_max_stream_data_uni);
    if (got_initial_max_streams_bidi)
        QLOG_U64("initial_max_streams_bidi", ch->max_local_streams_bidi);
    if (got_initial_max_streams_uni)
        QLOG_U64("initial_max_streams_uni", ch->max_local_streams_uni);
    if (got_ack_delay_exp)
    ...
    if (got_stateless_reset_token)
        QLOG_BIN("stateless_reset_token", stateless_reset_token_p, QUIC_STATELESS_RESET_TOKEN_LEN);
    if (got_preferred_addr) {
        ...
        QLOG_BIN("stateless_reset_token", pfa.stateless_reset.token, sizeof(pfa.stateless_reset.token));
    ...
    if (got_initial_max_data || got_initial_max_stream_data_bidi_remote || got_initial_max_streams_bidi
    ...
    if (ch->is_server && !ch_generate_transport_params(ch)) {
        ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_INTERNAL_ERROR, 0, "internal error");
        return 0;
    }
...
malformed:
    ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_TRANSPORT_PARAMETER_ERROR, 0, reason);
    return 0;
}
...
static int ch_generate_transport_params(QUIC_CHANNEL *ch)
{
    ...
    if (ossl_quic_wire_encode_transport_param_bytes(&wpkt, QUIC_TPARAM_DISABLE_ACTIVE_MIGRATION, NULL, 0) == NULL)
        goto err;

    if (ch->is_server) {
        if (!ossl_quic_wire_encode_transport_param_cid(&wpkt, QUIC_TPARAM_ORIG_DCID, id_to_use))
            goto err;

        if (!ossl_quic_wire_encode_transport_param_cid(&wpkt, QUIC_TPARAM_INITIAL_SCID, &ch->cur_local_cid))
            goto err;
        if (ch->odcid.id_len != 0)
            if (!ossl_quic_wire_encode_transport_param_cid(&wpkt, QUIC_TPARAM_RETRY_SCID, &ch->init_dcid))
                goto err;
    } else {
        if (!ossl_quic_wire_encode_transport_param_cid(&wpkt, QUIC_TPARAM_INITIAL_SCID, &ch->init_scid))
            goto err;
    }

    if (!ossl_quic_wire_encode_transport_param_int(&wpkt, QUIC_TPARAM_MAX_IDLE_TIMEOUT, ch->max_idle_timeout_local_req))
        goto err;

    if (!ossl_quic_wire_encode_transport_param_int(&wpkt, QUIC_TPARAM_MAX_UDP_PAYLOAD_SIZE, QUIC_MIN_INITIAL_DGRAM_LEN))
        goto err;
    ...
    if (ch->tx_max_ack_delay != QUIC_DEFAULT_MAX_ACK_DELAY
        && !ossl_quic_wire_encode_transport_param_int(&wpkt, QUIC_TPARAM_MAX_ACK_DELAY, ch->tx_max_ack_delay))
        goto err;
    ...
    if (!ossl_quic_tls_set_transport_params(ch->qtls, ch->local_transport_params, buf_len))
        goto err;
    ...
    QLOG_U64("initial_max_stream_data_bidi_local", ch->tx_init_max_stream_data_bidi_local);
    QLOG_U64("initial_max_stream_data_bidi_remote", ch->tx_init_max_stream_data_bidi_remote);
    QLOG_U64("initial_max_stream_data_uni", ch->tx_init_max_stream_data_uni);
    QLOG_U64("initial_max_streams_bidi", ossl_quic_rxfc_get_cwm(&ch->max_streams_bidi_rxfc));
    QLOG_U64("initial_max_streams_uni", ossl_quic_rxfc_get_cwm(&ch->max_streams_uni_rxfc));
    QLOG_EVENT_END()
#endif
 ...
 * "loudly".
 */
void ossl_quic_channel_subtick(QUIC_CHANNEL *ch, QUIC_TICK_RESULT *res, uint32_t flags)
{
     ...
     */
    if (ch->state == QUIC_CHANNEL_STATE_IDLE || ossl_quic_channel_is_terminated(ch)) {
    ...
    res->net_write_desired = (!ossl_quic_channel_is_terminated(ch) && ossl_qtx_get_queue_len_datagrams(ch->qtx) > 0);
...
static int ch_tick_tls(QUIC_CHANNEL *ch, int channel_only, int *notify_other_threads)
{
    ...
    if (ossl_quic_tls_get_error(ch->qtls, &error_code, &error_msg, &error_state)) {
        ossl_quic_channel_raise_protocol_error_state(ch, error_code, 0, error_msg, error_state);
...
static void ch_rx_check_forged_pkt_limit(QUIC_CHANNEL *ch)
{
    ...
    for (enc_level = QUIC_ENC_LEVEL_INITIAL; enc_level < QUIC_ENC_LEVEL_NUM; ++enc_level) {
        /*
    ...
    ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_AEAD_LIMIT_REACHED, 0, "forgery limit");
}
...
static int ch_rx(QUIC_CHANNEL *ch, int channel_only, int *notify_other_threads)
{
    ...
    for (;;) {
        ...
        if (closing)
            ossl_quic_tx_packetiser_record_received_closing_bytes(ch->txp, ch->qrx_pkt->hdr->len);
...
static int bio_addr_eq(const BIO_ADDR *a, const BIO_ADDR *b)
{
    ...
    case AF_INET:
        return !memcmp(&a->s_in.sin_addr, &b->s_in.sin_addr, sizeof(a->s_in.sin_addr))
            && a->s_in.sin_port == b->s_in.sin_port;
#if OPENSSL_USE_IPV6
    case AF_INET6:
        return !memcmp(&a->s_in6.sin6_addr, &b->s_in6.sin6_addr, sizeof(a->s_in6.sin6_addr))
...
static void ch_rx_handle_packet(QUIC_CHANNEL *ch, int channel_only)
{
     ...
     * address.
     */
    if (!ch->is_server && ch->qrx_pkt->peer != NULL
    ...
    if (!ch->is_server && ch->have_received_enc_pkt && ossl_quic_pkt_type_has_scid(ch->qrx_pkt->hdr->type)) {
        /*
         ...
         */
        if (!ossl_quic_conn_id_eq(&ch->qrx_pkt->hdr->src_conn_id, &ch->init_scid))
            return;
    }

    if (ossl_quic_pkt_type_has_version(ch->qrx_pkt->hdr->type) && ch->qrx_pkt->hdr->version != QUIC_VERSION_1)
        /*
    ...
    if (ch->qrx_pkt->hdr->type == QUIC_PKT_TYPE_VERSION_NEG) {
         ...
         * version
         */
        if (!PACKET_buf_init(&vpkt, ch->qrx_pkt->hdr->data, ch->qrx_pkt->hdr->len))
            return;
            ...
            if (supported_ver == QUIC_VERSION_1) {
                ...
                if (!ch_restart(ch))
                    ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_INTERNAL_ERROR, 0,
                                                           "handling ver negotiation packet");
                return;
            }
        }
         ...
         */
        ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_CONNECTION_REFUSED, 0, "unsupported protocol version");
        return;
    }
     ...
     */
    if (ossl_quic_pkt_type_is_encrypted(ch->qrx_pkt->hdr->type) && ch->qrx_pkt->hdr->reserved != 0) {
        ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_PROTOCOL_VIOLATION, 0, "packet header reserved bits");
        return;
    }

    iovec.buf = ch->qrx_pkt->hdr->data;
    iovec.buf_len = ch->qrx_pkt->hdr->len;
    ossl_qlog_event_transport_packet_received(ch_get_qlog(ch), ch->qrx_pkt->hdr, ch->qrx_pkt->pn, &iovec, 1,
    ...
    case QUIC_PKT_TYPE_RETRY:
         ...
         */
        if (!ossl_quic_validate_retry_integrity_tag(ch->port->engine->libctx, ch->port->engine->propq, ch->qrx_pkt->hdr,
                                                    &ch->init_dcid))
            /* Malformed retry packet, ignore. */
            return;

        if (!ch_retry(ch, ch->qrx_pkt->hdr->data, ch->qrx_pkt->hdr->len - QUIC_RETRY_INTEGRITY_TAG_LEN,
                      &ch->qrx_pkt->hdr->src_conn_id, old_have_processed_any_pkt))
            ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_INTERNAL_ERROR, 0, "handling retry packet");
        break;
    ...
    case QUIC_PKT_TYPE_1RTT:
        ...
        if (ch->rxku_in_progress && ch->qrx_pkt->hdr->type == QUIC_PKT_TYPE_1RTT
            && ch->qrx_pkt->pn >= ch->rxku_trigger_pn && ch->qrx_pkt->key_epoch < ossl_qrx_get_key_epoch(ch->qrx)) {
            /*
             ...
             */
            ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_KEY_UPDATE_ERROR, 0, "new packet with old keys");
            break;
        }

        if (!ch->is_server && ch->qrx_pkt->hdr->type == QUIC_PKT_TYPE_INITIAL && ch->qrx_pkt->hdr->token_len > 0) {
            /*
...
static OSSL_TIME ch_determine_next_tick_deadline(QUIC_CHANNEL *ch)
{
    ...
    for (i = 0; i < QUIC_ENC_LEVEL_NUM; i++) {
        if (ossl_qtx_is_enc_level_provisioned(ch->qtx, i)) {
            deadline =
                ossl_time_min(deadline, ossl_ackm_get_ack_deadline(ch->ackm, ossl_quic_enc_level_to_pn_space(i)));
        }
    }
    ...
    deadline = ossl_time_min(deadline, ossl_quic_tx_packetiser_get_deadline(ch->txp));

    /* Is the terminating timer armed? */
    if (ossl_quic_channel_is_terminating(ch))
        deadline = ossl_time_min(deadline, ch->terminate_deadline);
    else if (!ossl_time_is_infinite(ch->idle_deadline))
        deadline = ossl_time_min(deadline, ch->idle_deadline);
...
static void ch_record_state_transition(QUIC_CHANNEL *ch, uint32_t new_state)
{
    ...
    ossl_qlog_event_connectivity_connection_state_updated(ch_get_qlog(ch), old_state, new_state, ch->handshake_complete,
                                                          ch->handshake_confirmed);
}

static void free_peer_token(const unsigned char *token, size_t token_len, void *arg)
{
...
int ossl_quic_channel_start(QUIC_CHANNEL *ch)
{
     ...
     */
    if (!ch->is_server && ossl_quic_get_peer_token(ch->port->channel_ctx, &ch->cur_peer_addr, &token)
        && !ossl_quic_tx_packetiser_set_initial_token(ch->txp, token->token, token->token_len, free_peer_token, token))
        free_peer_token(NULL, 0, token);

    /* Plug in secrets for the Initial EL. */
    if (!ossl_quic_provide_initial_secret(ch->port->engine->libctx, ch->port->engine->propq, &ch->init_dcid,
                                          ch->is_server, ch->qrx, ch->qtx))
        return 0;
     ...
     */
    if (!ch->is_server && !ch->got_local_transport_params && !ch_generate_transport_params(ch))
        return 0;
    ...
    ossl_qlog_event_connectivity_connection_started(ch_get_qlog(ch), &ch->init_dcid);
...
void ossl_quic_channel_local_close(QUIC_CHANNEL *ch, uint64_t app_error_code, const char *app_reason)
{
    QUIC_TERMINATE_CAUSE tcause = {0};
                    ...
                    const QUIC_CONN_ID *retry_scid, int drop_later_pn)
{
    ...
    if (!ossl_quic_tx_packetiser_set_initial_token(ch->txp, buf, retry_token_len, free_token, NULL)) {
        /*
         * This may fail if the token we receive is too big for us to ever be
         * able to transmit in an outgoing Initial packet.
         */
        ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_INVALID_TOKEN, 0, "received oversize token");
        OPENSSL_free(buf);
        return 0;
    }
     ...
     * repeated retries.
     */
    if (!ossl_ackm_mark_packet_pseudo_lost(ch->ackm, QUIC_PN_SPACE_INITIAL, pn))
        return 0;
     ...
     */
    if (!ossl_quic_provide_initial_secret(ch->port->engine->libctx, ch->port->engine->propq, &ch->retry_scid,
                                          /*is_server=*/0, ch->qrx, ch->qtx))
        return 0;
...
static int ch_discard_el(QUIC_CHANNEL *ch, uint32_t enc_level)
{
    ...
    if (enc_level != QUIC_ENC_LEVEL_0RTT) {
        ...
        if (!ossl_assert(ch->crypto_send[pn_space] != NULL) || !ossl_assert(ch->crypto_recv[pn_space] != NULL))
            return 0;
 ...
 */
static void copy_tcause(QUIC_TERMINATE_CAUSE *dst, const QUIC_TERMINATE_CAUSE *src)
{
    ...
    case QUIC_CHANNEL_STATE_ACTIVE:
        ...
        if (!force_immediate) {
            ch_record_state_transition(
                ch, tcause->remote ? QUIC_CHANNEL_STATE_TERMINATING_DRAINING : QUIC_CHANNEL_STATE_TERMINATING_CLOSING);
            /*
            ...
            ch->terminate_deadline =
                ossl_time_add(get_time(ch), ossl_time_multiply(ossl_ackm_get_pto_duration(ch->ackm), 3));
...
/* For RXDP use. */
void ossl_quic_channel_on_remote_conn_close(QUIC_CHANNEL *ch, OSSL_QUIC_FRAME_CONN_CLOSE *f)
{
    QUIC_TERMINATE_CAUSE tcause = {0};
...
static int ch_enqueue_retire_conn_id(QUIC_CHANNEL *ch, uint64_t seq_num)
{
    ...
    if (ossl_quic_cfq_add_frame(ch->cfq, 1, QUIC_PN_SPACE_APP, OSSL_QUIC_FRAME_TYPE_RETIRE_CONN_ID, 0,
                                (unsigned char *)buf_mem->data, l, free_frame_data, NULL)
        == NULL)
        goto err;
...
err:
    ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_INTERNAL_ERROR, OSSL_QUIC_FRAME_TYPE_NEW_CONN_ID,
                                           "internal error enqueueing retire conn id");
    BUF_MEM_free(buf_mem);
    return 0;
}

void ossl_quic_channel_on_new_conn_id(QUIC_CHANNEL *ch, OSSL_QUIC_FRAME_NEW_CONN_ID *f)
{
    ...
    if (ch->cur_remote_dcid.id_len == 0) {
        ...
        ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_PROTOCOL_VIOLATION, OSSL_QUIC_FRAME_TYPE_NEW_CONN_ID,
    ...
    if (new_remote_seq_num - new_retire_prior_to > 1) {
        ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_CONNECTION_ID_LIMIT_ERROR,
                                               OSSL_QUIC_FRAME_TYPE_NEW_CONN_ID, "active_connection_id limit violated");
        return;
    }
    ...
    if (new_remote_seq_num > ch->cur_remote_seq_num) {
        ...
        if (!ossl_quic_srtm_add(ch->srtm, ch, new_remote_seq_num, &f->stateless_reset)) {
            ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_CONNECTION_ID_LIMIT_ERROR,
                                                   OSSL_QUIC_FRAME_TYPE_NEW_CONN_ID,
...
void ossl_quic_channel_raise_protocol_error_loc(QUIC_CHANNEL *ch, uint64_t error_code, uint64_t frame_type,
                                                const char *reason, ERR_STATE *err_state, const char *src_file,
                                                int src_line, const char *src_func)
{
    QUIC_TERMINATE_CAUSE tcause = {0};
    int err_reason = error_code == OSSL_QUIC_ERR_INTERNAL_ERROR ? ERR_R_INTERNAL_ERROR : SSL_R_QUIC_PROTOCOL_ERROR;
    ...
    if (frame_type != 0) {
        ...
        ERR_raise_data(ERR_LIB_SSL, err_reason,
                       ...
                       (unsigned long long)error_code, err_str_pfx, err_str, err_str_sfx,
                       (unsigned long long)frame_type, ft_str_pfx, ft_str, ft_str_sfx, reason);
    } else {
        ERR_raise_data(ERR_LIB_SSL, err_reason, "QUIC error code: 0x%llx%s%s%s, reason: \"%s\"",
                       (unsigned long long)error_code, err_str_pfx, err_str, err_str_sfx, reason);
    }
...
static OSSL_TIME ch_get_effective_idle_timeout_duration(QUIC_CHANNEL *ch)
{
     ...
     */
    pto = ossl_ackm_get_pto_duration(ch->ackm);
    return ossl_time_max(ossl_ms2time(ch->max_idle_timeout), ossl_time_multiply(pto, 3));
}
 ...
 */
static int ch_on_new_conn_common(QUIC_CHANNEL *ch, const BIO_ADDR *peer, const QUIC_CONN_ID *peer_scid,
                                 const QUIC_CONN_ID *peer_dcid, const QUIC_CONN_ID *peer_odcid)
{
...
int ossl_quic_channel_on_new_conn(QUIC_CHANNEL *ch, const BIO_ADDR *peer, const QUIC_CONN_ID *peer_scid,
 ...
 */
int ossl_quic_bind_channel(QUIC_CHANNEL *ch, const BIO_ADDR *peer, const QUIC_CONN_ID *peer_scid,
                           const QUIC_CONN_ID *peer_dcid, const QUIC_CONN_ID *peer_odcid)
{
    if (peer_dcid == NULL)
        return 0;
...
static int ch_init_new_stream(QUIC_CHANNEL *ch, QUIC_STREAM *qs, int can_send, int can_recv)
{
    uint64_t rxfc_wnd;
    ...
    if (!ossl_quic_rxfc_init(&qs->rxfc, &ch->conn_rxfc, rxfc_wnd, DEFAULT_STREAM_RXFC_MAX_WND_MUL * rxfc_wnd, get_time,
                             ch))
        goto err;
...
static uint64_t *ch_get_local_stream_next_ordinal_ptr(QUIC_CHANNEL *ch, int is_uni)
{
    return is_uni ? &ch->next_local_stream_ordinal_uni : &ch->next_local_stream_ordinal_bidi;
}

static const uint64_t *ch_get_local_stream_max_ptr(const QUIC_CHANNEL *ch, int is_uni)
{
    return is_uni ? &ch->max_local_streams_uni : &ch->max_local_streams_bidi;
}

static const QUIC_RXFC *ch_get_remote_stream_count_rxfc(const QUIC_CHANNEL *ch, int is_uni)
{
    return is_uni ? &ch->max_streams_uni_rxfc : &ch->max_streams_bidi_rxfc;
}
...
int ossl_quic_channel_is_new_local_stream_admissible(QUIC_CHANNEL *ch, int is_uni)
{
    ...
    return ossl_quic_stream_map_is_local_allowed_by_stream_limit(&ch->qsm, *p_next_ordinal, is_uni);
}

uint64_t ossl_quic_channel_get_local_stream_count_avail(const QUIC_CHANNEL *ch, int is_uni)
{
    const uint64_t *p_next_ordinal, *p_max;

    p_next_ordinal = ch_get_local_stream_next_ordinal_ptr((QUIC_CHANNEL *)ch, is_uni);
    p_max = ch_get_local_stream_max_ptr(ch, is_uni);

    return *p_max - *p_next_ordinal;
}

uint64_t ossl_quic_channel_get_remote_stream_count_avail(const QUIC_CHANNEL *ch, int is_uni)
{
...
QUIC_STREAM *ossl_quic_channel_new_stream_local(QUIC_CHANNEL *ch, int is_uni)
{
    ...
    type = ch->is_server ? QUIC_STREAM_INITIATOR_SERVER : QUIC_STREAM_INITIATOR_CLIENT;
...
QUIC_STREAM *ossl_quic_channel_new_stream_remote(QUIC_CHANNEL *ch, uint64_t stream_id)
{
    uint64_t peer_role;
    int is_uni;
    QUIC_STREAM *qs;
...
{
    ossl_quic_stream_map_stop_sending_recv_part(&ch->qsm, qs, ch->incoming_stream_auto_reject_aec);

    ossl_quic_stream_map_reset_stream_send_part(&ch->qsm, qs, ch->incoming_stream_auto_reject_aec);
    qs->deleted = 1;
...
int ossl_quic_channel_replace_local_cid(QUIC_CHANNEL *ch, const QUIC_CONN_ID *conn_id)
{
    ...
    if (!ossl_quic_lcidm_debug_add(ch->lcidm, ch, &ch->cur_local_cid, 100))
        return 0;
    return 1;
}
...
void ossl_quic_channel_set_msg_callback(QUIC_CHANNEL *ch, ossl_msg_cb msg_callback, SSL *msg_callback_ssl)
{
    ...
    ossl_quic_tx_packetiser_set_msg_callback(ch->txp, msg_callback, msg_callback_ssl);
    /*
...
void ossl_quic_channel_set_msg_callback_arg(QUIC_CHANNEL *ch, void *msg_callback_arg)
{
...
void ossl_quic_channel_set_txku_threshold_override(QUIC_CHANNEL *ch, uint64_t tx_pkt_threshold)
{
