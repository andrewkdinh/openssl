...
static int ch_init(QUIC_CHANNEL *ch) {
  ...
  if (!ossl_quic_rxfc_init(
      &ch->conn_rxfc, NULL, DEFAULT_INIT_CONN_RXFC_WND,
  ...
  for (pn_space = QUIC_PN_SPACE_INITIAL; pn_space < QUIC_PN_SPACE_NUM;
    ...
    if (!ossl_quic_rxfc_init_standalone(&ch->crypto_rxfc[pn_space],
                                        INIT_CRYPTO_RECV_BUF_LEN, get_time, ch))
      goto err;

  if (!ossl_quic_rxfc_init_standalone(
      &ch->max_streams_bidi_rxfc, DEFAULT_INIT_CONN_MAX_STREAMS, get_time, ch))
    goto err;
...
static int ch_tick_tls(QUIC_CHANNEL *ch, int channel_only,
  ...
  if (ossl_quic_tls_get_error(ch->qtls, &error_code, &error_msg,
                              &error_state)) {
    ossl_quic_channel_raise_protocol_error_state(ch, error_code, 0, error_msg,
                                                 error_state);
...
static void ch_rx_handle_packet(QUIC_CHANNEL *ch, int channel_only) {
  ...
  case QUIC_PKT_TYPE_1RTT:
    ...
    if (!ch->is_server && ch->qrx_pkt->hdr->type == QUIC_PKT_TYPE_INITIAL &&
       ...
       */
      ossl_quic_channel_raise_protocol_error(
      ch, OSSL_QUIC_ERR_PROTOCOL_VIOLATION, 0, "client received initial token");
      break;
    }
...
static int ch_tx(QUIC_CHANNEL *ch, int *notify_other_threads) {
  ...
  do {
    ...
    if (!res) {
       ...
       */
      ossl_quic_channel_raise_protocol_error(
      ch, OSSL_QUIC_ERR_INTERNAL_ERROR, 0, "internal error (txp generate)");
      break;
    }
...
static OSSL_TIME ch_determine_next_tick_deadline(QUIC_CHANNEL *ch) {
  ...
  for (i = 0; i < QUIC_ENC_LEVEL_NUM; i++) {
    if (ossl_qtx_is_enc_level_provisioned(ch->qtx, i)) {
      ...
      deadline,
      ossl_ackm_get_ack_deadline(ch->ackm, ossl_quic_enc_level_to_pn_space(i)));
    }
  }
...
int ossl_quic_channel_start(QUIC_CHANNEL *ch) {
  ...
  if (!ossl_quic_provide_initial_secret(ch->port->engine->libctx,
                                        ch->port->engine->propq, &ch->init_dcid,
...
static int ch_retry(QUIC_CHANNEL *ch, const unsigned char *retry_token,
  ...
  if (!ossl_quic_tx_packetiser_set_initial_token(ch->txp, buf, retry_token_len,
                                                 free_token, NULL)) {
    /*
...
void ossl_quic_channel_on_new_conn_id(QUIC_CHANNEL *ch,
  ...
  if (new_retire_prior_to - ch->cur_retire_prior_to > 10) {
    ossl_quic_channel_raise_protocol_error(
    ch, OSSL_QUIC_ERR_CONNECTION_ID_LIMIT_ERROR,
    OSSL_QUIC_FRAME_TYPE_NEW_CONN_ID, "retiring connection id limit violated");
...
const char *src_func) {
  ...
  if (frame_type != 0) {
    ...
    ERR_raise_data(ERR_LIB_SSL, err_reason,
                   ...
                   err_str_sfx, (unsigned long long)frame_type, ft_str_pfx,
                   ft_str, ft_str_sfx, reason);
  } else {
    ERR_raise_data(
    ERR_LIB_SSL, err_reason, "QUIC error code: 0x%llx%s%s%s, reason: \"%s\"",
...
static int ch_init_new_stream(QUIC_CHANNEL *ch, QUIC_STREAM *qs, int can_send,
  ...
  if (!ossl_quic_rxfc_init(&qs->rxfc, &ch->conn_rxfc, rxfc_wnd,
                           DEFAULT_STREAM_RXFC_MAX_WND_MUL * rxfc_wnd, get_time,
                           ch))
    goto err;
