...
static void ackm_on_pkts_lost(OSSL_ACKM *ackm, int pkt_space,
    ...
    for (p = lpkt; p != NULL; p = pnext) {
        ...
        if (p->is_inflight) {
            ...
            if (!pseudo) {
                /*
                 * If this is pseudo-loss (e.g. during connection retry) we do
                 * not inform the CC as it is not a real loss and not reflective
    ...
    for (; apkt != NULL; apkt = anext) {
        if (apkt->is_inflight) {
            ...
            if (apkt->largest_acked != QUIC_PN_INVALID)
                ...
                get_rx_history(ackm, apkt->pkt_space), apkt->largest_acked + 1);
        }
