        ...
        if (i == QUIC_PN_SPACE_APP) {
            ...
            if (!ossl_time_is_infinite(ackm->rx_max_ack_delay)) {
                ...
                duration = ossl_time_add(
                    duration,
                    ossl_time_multiply(ackm->rx_max_ack_delay, factor));
            }
        }
        ...
        if (apkt->is_inflight) {
            ...
            if (apkt->largest_acked != QUIC_PN_INVALID)
                ...
                rx_pkt_history_bump_watermark(
                    get_rx_history(ackm, apkt->pkt_space),
...
err:
    while (--i >= 0) tx_pkt_history_destroy(&ackm->tx_history[i]);
...
int ossl_ackm_on_pkt_space_discarded(OSSL_ACKM *ackm, int pkt_space)
{
    ...
    for (pkt = ossl_list_tx_history_head(
             &get_tx_history(ackm, pkt_space)->packets);
         pkt != NULL; pkt = pnext) {
        pnext = ossl_list_tx_history_next(pkt);
        if (pkt->is_inflight) {
            ackm->bytes_in_flight -= pkt->num_bytes;
