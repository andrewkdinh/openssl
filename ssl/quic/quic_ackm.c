...
static int tx_pkt_history_add_actual(struct tx_pkt_history_st *h,
...
static int tx_pkt_history_add(struct tx_pkt_history_st *h,
                              OSSL_ACKM_TX_PKT *pkt) {
...
static OSSL_ACKM_TX_PKT *tx_pkt_history_by_pkt_num(struct tx_pkt_history_st *h,
                                                   uint64_t pkt_num) {
...
static int tx_pkt_history_remove(struct tx_pkt_history_st *h,
                                 uint64_t pkt_num) {
 ...
 * receive another RX PN acknowledging that TX packet. This means that a given
 * RX PN can only become provably ACKed at least 1 RTT after it is received; it
 * is unlikely that any reordered datagrams will still be "in the network" (and
 * not lost) by this time. If this does occur for whatever reason and a late PN
 * is received, the packet will be discarded unprocessed and the PN is simply
...
static void rx_pkt_history_trim_range_count(struct rx_pkt_history_st *h) {
  ...
  while (ossl_list_uint_set_num(&h->set) > MAX_RX_ACK_RANGES) {
    ...
    highest =
    (highest == QUIC_PN_INVALID) ? r.end : ossl_quic_pn_max(highest, r.end);
 ...
 * discarded.
 */
static struct tx_pkt_history_st *get_tx_history(OSSL_ACKM *ackm,
                                                int pkt_space) {
 ...
 * discarded.
 */
static struct rx_pkt_history_st *get_rx_history(OSSL_ACKM *ackm,
                                                int pkt_space) {
  assert(!ackm->discarded[pkt_space]);
 ...
 */
static OSSL_ACKM_TX_PKT *ackm_detect_and_remove_newly_acked_pkts(
OSSL_ACKM *ackm, const OSSL_QUIC_FRAME_ACK *ack, int pkt_space) {
...
static OSSL_ACKM_TX_PKT *ackm_detect_and_remove_lost_pkts(OSSL_ACKM *ackm,
  ...
  loss_delay = ossl_time_multiply(
  ossl_time_max(rtt.latest_rtt, rtt.smoothed_rtt), K_TIME_THRESHOLD_NUM);
    ...
    if (ossl_time_compare(pkt->time, lost_send_time) <= 0 ||
      ...
      *fixup = NULL;
      ...
      if (ossl_time_is_zero(ackm->loss_time[pkt_space]))
        ...
        ackm->loss_time[pkt_space] = ossl_time_min(
        ackm->loss_time[pkt_space], ossl_time_add(pkt->time, loss_delay));
    }
  }
...
static OSSL_TIME ackm_get_pto_time_and_space(OSSL_ACKM *ackm, int *space) {
  ...
  duration = ossl_time_add(
  rtt.smoothed_rtt, ossl_time_max(ossl_time_multiply(rtt.rtt_variance, 4),
                                  ossl_ticks2time(K_GRANULARITY)));

  duration = ossl_time_multiply(
  duration, (uint64_t)1 << min_u32(ackm->pto_count, MAX_PTO_COUNT));

  /* Anti-deadlock PTO starts from the current time. */
  if (ackm_ack_eliciting_bytes_in_flight(ackm) == 0) {
    assert(!ackm->peer_completed_addr_validation);

    *space = ackm->discarded[QUIC_PN_SPACE_INITIAL] ? QUIC_PN_SPACE_HANDSHAKE
  ...
  for (i = QUIC_PN_SPACE_INITIAL; i < QUIC_PN_SPACE_NUM; ++i) {
    ...
    if (i == QUIC_PN_SPACE_APP) {
      ...
      if (!ossl_time_is_infinite(ackm->rx_max_ack_delay)) {
        uint64_t factor = (uint64_t)1
                          << min_u32(ackm->pto_count, MAX_PTO_COUNT);

        duration = ossl_time_add(
        duration, ossl_time_multiply(ackm->rx_max_ack_delay, factor));
      }
    }
  ...
  for (; apkt != NULL; apkt = anext) {
    if (apkt->is_inflight) {
      ...
      if (apkt->largest_acked != QUIC_PN_INVALID)
         ...
         * next time.
         */
        rx_pkt_history_bump_watermark(get_rx_history(ackm, apkt->pkt_space),
...
int ossl_ackm_on_rx_ack_frame(OSSL_ACKM *ackm, const OSSL_QUIC_FRAME_ACK *ack,
  ...
  if (ackm->largest_acked_pkt[pkt_space] == QUIC_PN_INVALID)
    ...
    ackm->largest_acked_pkt[pkt_space] = ossl_quic_pn_max(
    ackm->largest_acked_pkt[pkt_space], ack->ack_ranges[0].end);
...
int ossl_ackm_on_pkt_space_discarded(OSSL_ACKM *ackm, int pkt_space) {
  ...
  for (pkt =
       ossl_list_tx_history_head(&get_tx_history(ackm, pkt_space)->packets);
       pkt != NULL; pkt = pnext) {
    pnext = ossl_list_tx_history_next(pkt);
    if (pkt->is_inflight) {
      ackm->bytes_in_flight -= pkt->num_bytes;
...
static void ackm_on_rx_ack_eliciting(OSSL_ACKM *ackm, OSSL_TIME rx_time,
                                     int pkt_space, int was_missing) {
  OSSL_TIME tx_max_ack_delay;
...
int ossl_ackm_mark_packet_pseudo_lost(OSSL_ACKM *ackm, int pkt_space,
                                      QUIC_PN pn) {
