...
void ossl_statm_update_rtt(OSSL_STATM *statm, OSSL_TIME ack_delay,
  ...
  if (ossl_time_compare(latest_rtt, ossl_time_add(statm->min_rtt, ack_delay)) >=
      0)
    adjusted_rtt = ossl_time_subtract(latest_rtt, ack_delay);

  statm->rtt_variance = ossl_time_divide(
  ossl_time_add(ossl_time_multiply(statm->rtt_variance, 3),
                ossl_time_abs_difference(statm->smoothed_rtt, adjusted_rtt)),
  4);
  statm->smoothed_rtt = ossl_time_divide(
  ossl_time_add(ossl_time_multiply(statm->smoothed_rtt, 7), adjusted_rtt), 8);
}
...
void ossl_statm_destroy(OSSL_STATM *statm) { /* No-op. */ }
