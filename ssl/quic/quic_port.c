...
static int port_init(QUIC_PORT *port) {
  ...
  ossl_quic_demux_set_default_handler(port->demux, port_default_packet_handler,
                                      port);

  if ((port->srtm =
       ossl_quic_srtm_new(port->engine->libctx, port->engine->propq)) == NULL)
    goto err;

  if ((port->lcidm =
       ossl_quic_lcidm_new(port->engine->libctx, rx_short_dcid_len)) == NULL)
    goto err;
  ...
  if ((port->token_ctx = EVP_CIPHER_CTX_new()) == NULL ||
      (cipher = EVP_CIPHER_fetch(port->engine->libctx, "AES-256-GCM", NULL)) ==
      NULL ||
...
QUIC_DEMUX *ossl_quic_port_get0_demux(QUIC_PORT *port) { return port->demux; }
...
BIO *ossl_quic_port_get_net_rbio(QUIC_PORT *port) { return port->net_rbio; }

BIO *ossl_quic_port_get_net_wbio(QUIC_PORT *port) { return port->net_wbio; }
...
static SSL *port_new_handshake_layer(QUIC_PORT *port, QUIC_CHANNEL *ch) {
  ...
  if (ql != NULL && ql->obj.ssl.ctx->new_pending_conn_cb != NULL)
    if (!ql->obj.ssl.ctx->new_pending_conn_cb(
        ql->obj.ssl.ctx, user_ssl, ql->obj.ssl.ctx->new_pending_conn_arg)) {
      SSL_free(tls);
      SSL_free(user_ssl);
      if (qc != NULL)
        qc->tls = NULL;
...
static int port_try_handle_stateless_reset(QUIC_PORT *port,
                                           const QUIC_URXE *e) {
  size_t i;
...
static void port_send_retry(QUIC_PORT *port, BIO_ADDR *peer,
  ...
  port->engine->libctx, port->engine->propq, &hdr, &client_hdr->dst_conn_id,
...
static int port_validate_token(QUIC_PKT_HDR *hdr, QUIC_PORT *port,
  ...
  *gen_new_token = 0;
  ...
  if (!decrypt_validation_token(port, hdr->token, hdr->token_len, NULL,
      ...
      !decrypt_validation_token(port, hdr->token, hdr->token_len, dec_token,
                                &dec_token_len) ||
...
static void port_default_packet_handler(QUIC_URXE *e, void *arg,
  ...
  /*
   * TODO(QUIC FUTURE): there should be some logic similar to accounting
   * half-open states in TCP. If we reach certain threshold, then we want to
   * validate clients.
   */
   ...
   * validate here
   */
  if (hdr.token != NULL && port_validate_token(&hdr, port, &e->peer, &odcid,
                                               &scid, &gen_new_token) == 0) {
    /*
     * RFC 9000 s 8.1.3
