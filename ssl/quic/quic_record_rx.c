...
static int qrx_decrypt_pkt_body(OSSL_QRX *qrx, unsigned char *dst,
                                ...
                                size_t *dec_len, const unsigned char *aad,
                                size_t aad_len, QUIC_PN pn, uint32_t enc_level,
...
void ossl_qrx_inject_urxe(OSSL_QRX *qrx, QUIC_URXE *urxe) {
    ...
    if (qrx->msg_callback != NULL)
        qrx->msg_callback(0, OSSL_QUIC1_VERSION, SSL3_RT_QUIC_DATAGRAM,
                          urxe + 1, urxe->data_len, qrx->msg_callback_ssl,
...
static int qrx_validate_initial_pkt(OSSL_QRX *qrx, QUIC_URXE *urxe,
    ...
    if (ossl_qrl_enc_level_set_have_el(&qrx->el_set, QUIC_ENC_LEVEL_INITIAL) !=
        1)
        goto malformed;
    ...
    if (ossl_quic_wire_decode_pkt_hdr(&pkt, 0, 0, 0, &rxe->hdr, NULL, NULL) !=
        1)
        goto malformed;
    ...
    if (!qrx_decrypt_pkt_body(qrx, dst, rxe->hdr.data, rxe->hdr.len, &dec_len,
                              sop, aad_len, rxe->pn, QUIC_ENC_LEVEL_INITIAL,
...
int ossl_qrx_provide_secret(OSSL_QRX *qrx, uint32_t enc_level,
    ...
    if (!ossl_qrl_enc_level_set_provide_secret(
        &qrx->el_set, qrx->libctx, qrx->propq, enc_level, suite_id, md, secret,
...
static int qrx_validate_hdr_early(OSSL_QRX *qrx, RXE *rxe,
    ...
    if (first_dcid != NULL &&
        !ossl_quic_pkt_type_can_share_dgram(rxe->hdr.type))
        return 0;
...
static size_t qrx_get_cipher_ctx_idx(OSSL_QRX *qrx, OSSL_QRL_ENC_LEVEL *el,
                                     uint32_t enc_level,
                                     unsigned char key_phase_bit,
                                     uint64_t *rx_key_epoch, int *is_old_key) {
    size_t idx;
...
static int qrx_decrypt_pkt_body(OSSL_QRX *qrx, unsigned char *dst,
    ...
    if (EVP_CIPHER_CTX_ctrl(cctx, EVP_CTRL_AEAD_SET_TAG, el->tag_len,
...
static ossl_inline void ignore_res(int x) { /* No-op. */ }
...
static int qrx_process_pkt(OSSL_QRX *qrx, QUIC_URXE *urxe, PACKET *pkt,
                           size_t pkt_idx, QUIC_CONN_ID *first_dcid,
                           size_t datagram_len) {
    RXE *rxe;
    ...
    *pkt = orig_pkt;
    ...
    if (need_second_decode) {
        ...
        /* Decode the now unprotected header. */
        if (ossl_quic_wire_decode_pkt_hdr(pkt, qrx->short_conn_id_len, 0, 0,
                                          &rxe->hdr, NULL, NULL) != 1)
            goto malformed;
    }
...
static int qrx_process_one_urxe(OSSL_QRX *qrx, QUIC_URXE *e) {
     ...
     */
    was_deferred =
    qrx_process_datagram(qrx, e, ossl_quic_urxe_data(e), e->data_len);
...
uint64_t ossl_qrx_get_key_epoch(OSSL_QRX *qrx) {
    OSSL_QRL_ENC_LEVEL *el =
    ossl_qrl_enc_level_set_get(&qrx->el_set, QUIC_ENC_LEVEL_1RTT, 1);
