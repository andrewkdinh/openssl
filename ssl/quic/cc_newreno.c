...
static void newreno_free(OSSL_CC_DATA *cc) { OPENSSL_free(cc); }
...
static int newreno_set_input_params(OSSL_CC_DATA *cc,
...
static int newreno_bind_diagnostic(OSSL_CC_DATA *cc, OSSL_PARAM *params) {
  ...
  if (!bind_diag(params, OSSL_CC_OPTION_MAX_DGRAM_PAYLOAD_LEN, sizeof(size_t),
      ...
      !bind_diag(params, OSSL_CC_OPTION_CUR_STATE, sizeof(uint32_t),
...
static void newreno_cong(OSSL_CC_NEWRENO *nr, OSSL_TIME tx_time) {
  ...
  safe_muldiv_u64(nr->cong_wnd, nr->k_loss_reduction_factor_num,
...
static int newreno_is_cong_limited(OSSL_CC_NEWRENO *nr) {
   ...
   */
  return (nr->cong_wnd < nr->slow_start_thresh &&
          wnd_rem <= nr->cong_wnd / 2) ||
