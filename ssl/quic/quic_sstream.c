...
int ossl_quic_sstream_get_stream_frame(QUIC_SSTREAM *qss, size_t skip,
    ...
    for (i = 0;; ++i) {
        ...
        if (!ring_buf_get_buf_at(&qss->ring_buf, range->range.start + total_len,
                                 &src, &src_len))
            return 0;
    ...
    hdr->len = total_len;
    hdr->is_fin =
    qss->have_final_size && hdr->offset + hdr->len == qss->ring_buf.head_offset;
...
int ossl_quic_sstream_mark_lost(QUIC_SSTREAM *qss, uint64_t start,
     ...
     * so that they are returned once more by
