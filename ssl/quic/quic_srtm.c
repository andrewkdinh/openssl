...
/*
 * The SRTM is implemented using two LHASH instances, one matching opaque
 * pointers to an item structure, and another matching a SRT-derived value to an
 * item structure. Multiple items with different seq_num values under a given
 * opaque, and duplicate SRTs, are handled using sorted singly-linked lists.
 *
...
static void check_mark(SRTM_ITEM *item, void *arg) {
    ...
    while (item != NULL) {
        if (have_prev) {
            assert(
            !(item->opaque == prev_opaque && item->seq_num == prev_seq_num));
            if (!arg_->mode)
