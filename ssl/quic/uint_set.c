 ...
 *            integers.
 */
void ossl_uint_set_init(UINT_SET *s) { ossl_list_uint_set_init(s); }
...
static uint64_t u64_min(uint64_t x, uint64_t y) { return x < y ? x : y; }

static uint64_t u64_max(uint64_t x, uint64_t y) { return x > y ? x : y; }
