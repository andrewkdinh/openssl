...
static void on_acked(void *arg) {
    ...
    for (i = 0; i < num_chunks; ++i) {
        sstream =
        fifd->get_sstream_by_id(chunks[i].stream_id, pkt->ackm_pkt.pkt_space,
        ...
        if (chunks[i].end >= chunks[i].start)
            ...
            ossl_quic_sstream_mark_acked(sstream, chunks[i].start,
                                         chunks[i].end);
...
static void on_lost(void *arg) {
    ...
    for (i = 0; i < num_chunks; ++i) {
        ...
        if (chunks[i].end >= chunks[i].start) {
             ...
             */
            ossl_quic_sstream_mark_lost(sstream, chunks[i].start,
                                        chunks[i].end);
            sstream_updated = 1;
        }
        ...
        if (chunks[i].has_stop_sending && chunks[i].stream_id != UINT64_MAX)
            fifd->regen_frame(OSSL_QUIC_FRAME_TYPE_STOP_SENDING,
                              chunks[i].stream_id, pkt, fifd->regen_frame_arg);
    ...
    if (pkt->had_max_streams_bidi_frame)
        fifd->regen_frame(OSSL_QUIC_FRAME_TYPE_MAX_STREAMS_BIDI, UINT64_MAX,
