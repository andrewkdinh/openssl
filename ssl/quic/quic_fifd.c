...
static void on_acked(void *arg) {
  ...
  for (i = 0; i < num_chunks; ++i) {
    sstream = fifd->get_sstream_by_id(
    chunks[i].stream_id, pkt->ackm_pkt.pkt_space, fifd->get_sstream_by_id_arg);
    if (sstream == NULL)
      continue;
    ...
    if (chunks[i].has_stop_sending && chunks[i].stream_id != UINT64_MAX)
      fifd->confirm_frame(OSSL_QUIC_FRAME_TYPE_STOP_SENDING,
                          chunks[i].stream_id, pkt, fifd->confirm_frame_arg);
...
static void on_lost(void *arg) {
  ...
  for (i = 0; i < num_chunks; ++i) {
    ...
    if (chunks[i].has_stop_sending && chunks[i].stream_id != UINT64_MAX)
      fifd->regen_frame(OSSL_QUIC_FRAME_TYPE_STOP_SENDING, chunks[i].stream_id,
                        pkt, fifd->regen_frame_arg);

    if (chunks[i].has_reset_stream && chunks[i].stream_id != UINT64_MAX)
      fifd->regen_frame(OSSL_QUIC_FRAME_TYPE_RESET_STREAM, chunks[i].stream_id,
                        pkt, fifd->regen_frame_arg);
     ...
     * handling loss.
     */
    fifd->regen_frame(OSSL_QUIC_FRAME_TYPE_MAX_STREAM_DATA, chunks[i].stream_id,
                      pkt, fifd->regen_frame_arg);
  ...
  if (pkt->had_handshake_done_frame)
    fifd->regen_frame(OSSL_QUIC_FRAME_TYPE_HANDSHAKE_DONE, UINT64_MAX, pkt,
                      fifd->regen_frame_arg);

  if (pkt->had_max_data_frame)
    fifd->regen_frame(OSSL_QUIC_FRAME_TYPE_MAX_DATA, UINT64_MAX, pkt,
                      fifd->regen_frame_arg);

  if (pkt->had_max_streams_bidi_frame)
    fifd->regen_frame(OSSL_QUIC_FRAME_TYPE_MAX_STREAMS_BIDI, UINT64_MAX, pkt,
                      fifd->regen_frame_arg);

  if (pkt->had_max_streams_uni_frame)
    fifd->regen_frame(OSSL_QUIC_FRAME_TYPE_MAX_STREAMS_UNI, UINT64_MAX, pkt,
                      fifd->regen_frame_arg);
  ...
  if (pkt->had_ack_frame)
     ...
     */
    fifd->regen_frame(OSSL_QUIC_FRAME_TYPE_ACK_WITH_ECN, UINT64_MAX, pkt,
                      fifd->regen_frame_arg);
...
int ossl_quic_fifd_pkt_commit(QUIC_FIFD *fifd, QUIC_TXPIM_PKT *pkt) {
  ...
  for (i = 0; i < num_chunks; ++i) {
    ...
    if (chunks[i].end >= chunks[i].start &&
        !ossl_quic_sstream_mark_transmitted(sstream, chunks[i].start,
                                            chunks[i].end))
      return 0;

    if (chunks[i].has_fin &&
        !ossl_quic_sstream_mark_transmitted_fin(sstream, chunks[i].end + 1))
      return 0;
  }
