...
static int quic_write_records(OSSL_RECORD_LAYER *rl,
    ...
    case SSL3_RT_HANDSHAKE:
        ...
        if (!rl->qtls->args.crypto_send_cb(
            template->buf + rl->written, template->buflen - rl->written,
            &consumed, rl->qtls->args.crypto_send_cb_arg)) {
            QUIC_TLS_FATAL(rl, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
            return OSSL_RECORD_RETURN_FATAL;
        }
        /*
...
static int quic_release_record(OSSL_RECORD_LAYER *rl, void *rechandle,
    ...
    if (rl->recunreleased == length) {
        if (!rl->qtls->args.crypto_release_rcd_cb(
            rl->recread, rl->qtls->args.crypto_release_rcd_cb_arg)) {
            QUIC_TLS_FATAL(rl, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
            return OSSL_RECORD_RETURN_FATAL;
        }
        rl->recread = 0;
...
static int quic_get_alert_code(OSSL_RECORD_LAYER *rl) { return rl->alert; }
...
static void quic_set_max_pipelines(OSSL_RECORD_LAYER *rl,
...
#define RAISE_ERROR(qtls, error_code, error_msg)                               \
    raise_error((qtls), (error_code), (error_msg), OPENSSL_FILE, OPENSSL_LINE, \
                OPENSSL_FUNC)
