...
int ossl_quic_reactor_init(QUIC_REACTOR *rtor,
                           void (*tick_cb)(QUIC_TICK_RESULT *res, void *arg,
                                           uint32_t flags),
                           void *tick_cb_arg, CRYPTO_MUTEX *mutex,
                           OSSL_TIME initial_tick_deadline, uint64_t flags) {
...
const BIO_POLL_DESCRIPTOR *
ossl_quic_reactor_get_poll_r(const QUIC_REACTOR *rtor) {
  return &rtor->poll_r;
}

const BIO_POLL_DESCRIPTOR *
ossl_quic_reactor_get_poll_w(const QUIC_REACTOR *rtor) {
  return &rtor->poll_w;
}

int ossl_quic_reactor_can_support_poll_descriptor(
const QUIC_REACTOR *rtor, const BIO_POLL_DESCRIPTOR *d) {
 ...
 */
static int poll_two_fds(int rfd, int rfd_want_read, int wfd, int wfd_want_write,
                        int notify_rfd, OSSL_TIME deadline,
                        CRYPTO_MUTEX *mutex) {
  ...
  if (rfd == wfd) {
    pfds[npfd].fd = rfd;
    pfds[npfd].events =
    (rfd_want_read ? POLLIN : 0) | (wfd_want_write ? POLLOUT : 0);
...
static int poll_two_descriptors(const BIO_POLL_DESCRIPTOR *r, int r_want_read,
  ...
  return poll_two_fds(rfd, r_want_read, wfd, w_want_write, notify_rfd, deadline,
                      mutex);
}
...
int ossl_quic_reactor_block_until_pred(QUIC_REACTOR *rtor,
  ...
  notifier_fd = (rtor->have_notifier ? ossl_rio_notifier_as_fd(&rtor->notifier)
                                     : INVALID_SOCKET);
  ...
  for (;;) {
    ...
    res =
    poll_two_descriptors(ossl_quic_reactor_get_poll_r(rtor), net_read_desired,
                         ossl_quic_reactor_get_poll_w(rtor), net_write_desired,
                         notifier_fd, tick_deadline, rtor->mutex);
