...
static int poll_two_descriptors(const BIO_POLL_DESCRIPTOR *r, int r_want_read,
    ...
    return poll_two_fds(rfd, r_want_read, wfd, w_want_write, notify_rfd,
                        deadline, mutex);
}
...
int ossl_quic_reactor_block_until_pred(QUIC_REACTOR *rtor,
    ...
    for (;;) {
        ...
        res = poll_two_descriptors(
        ossl_quic_reactor_get_poll_r(rtor), net_read_desired,
        ossl_quic_reactor_get_poll_w(rtor), net_write_desired, notifier_fd,
        tick_deadline, rtor->mutex);
         ...
         *   T3 predicate is not satisfied, cur_blocking_waiters -> 3, block
         * again
         *
