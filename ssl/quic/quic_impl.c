...
#define QUIC_RAISE_NON_NORMAL_ERROR(ctx, reason, msg)                          \
  quic_raise_non_normal_error((ctx), OPENSSL_FILE, OPENSSL_LINE, OPENSSL_FUNC, \
                              (reason), (msg))
/*
...
static int quic_write_nonblocking_aon(QCTX *ctx, const void *buf, size_t len,
  ...
  quic_post_write(xso, actual_written > 0, actual_written == actual_len, flags,
...
SSL *ossl_quic_accept_connection(SSL *ssl, uint64_t flags) {
  ...
  if (new_ch == NULL && ossl_quic_port_is_running(ctx.ql->port)) {
    if (!no_block && qctx_blocking(&ctx)) {
      ret =
      block_until_pred(&ctx, quic_accept_connection_wait, ctx.ql->port, 0);
      if (ret < 1)
        goto out;
    } else {
      qctx_maybe_autotick(&ctx);
...
SSL *ossl_quic_new_domain(SSL_CTX *ctx, uint64_t flags) {
  ...
  if (!ossl_quic_obj_init(&qd->obj, ctx, SSL_TYPE_QUIC_DOMAIN, NULL, qd->engine,
                          NULL))
    goto err;
