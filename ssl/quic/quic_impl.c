 ...
 */
static int
quic_raise_non_normal_error(QCTX *ctx, const char *file, int line, const char *func, int reason, const char *fmt, ...)
{
    va_list args;
...
static int qc_try_create_default_xso_for_write(QCTX *ctx)
{
    ...
    qc_set_default_xso(qc,
                       (QUIC_XSO *)quic_conn_stream_new(ctx,
                                                        flags,
                                                        /*needs_lock=*/0),
                       /*touch=*/0);
...
QUIC_TAKES_LOCK
static int
qc_get_stream_write_buf_stat(QCTX *ctx, uint32_t class_, uint64_t *p_value_out, size_t (*getter)(QUIC_SSTREAM *sstream))
{
    int    ret   = 0;
    size_t value = 0;
...
SSL *ossl_quic_accept_stream(SSL *s, uint64_t flags)
{
    ...
    } else if ((flags & SSL_ACCEPT_STREAM_BIDI) && !(flags & SSL_ACCEPT_STREAM_UNI)) {
 ...
 */
static void
quic_classify_stream(QUIC_CONNECTION *qc, QUIC_STREAM *qs, int is_write, int *state, uint64_t *app_error_code)
{
    int      local_init;
    uint64_t final_size;
...
{
    return ossl_quic_stream_map_get_accept_queue_len(ossl_quic_channel_get_qsm(qc->ch), is_uni) > 0;
}
