...
QLOG *qlog, uint32_t old_state, uint32_t new_state, int handshake_complete,
int handshake_confirmed) {
#ifndef OPENSSL_NO_QLOG
  const char *state_s;
  ...
  state_s =
  map_state_to_qlog(new_state, handshake_complete, handshake_confirmed);
...
static int log_frame_actual(QLOG *qlog_instance, PACKET *pkt,
  ...
  case OSSL_QUIC_FRAME_TYPE_MAX_STREAMS_UNI: {
    ...
    QLOG_STR("stream_type", frame_type == OSSL_QUIC_FRAME_TYPE_MAX_STREAMS_BIDI
                            ? "bidirectional"
                            : "unidirectional");
    QLOG_U64("maximum", x);
  } break;
...
static int log_frames(QLOG *qlog_instance, const OSSL_QTX_IOVEC *iovec,
  ...
  for (i = 0; i < num_iovec; ++i) {
    ...
    while (PACKET_remaining(&pkt) > 0) {
      ...
      QLOG_BEGIN(NULL) { log_frame(qlog_instance, &pkt, &need_skip); }
      QLOG_END()
    }
  }
...
QLOG *qlog, const QUIC_PKT_HDR *hdr, QUIC_PN pn, const OSSL_QTX_IOVEC *iovec,
