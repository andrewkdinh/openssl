...
void ossl_qlog_event_connectivity_connection_started(QLOG *qlog, const QUIC_CONN_ID *init_dcid)
{
...
#ifndef OPENSSL_NO_QLOG
static const char *map_state_to_qlog(uint32_t state, int handshake_complete, int handshake_confirmed)
{
    switch (state) {
    default:
...
void ossl_qlog_event_connectivity_connection_state_updated(QLOG *qlog, uint32_t old_state, uint32_t new_state,
                                                           int handshake_complete, int handshake_confirmed)
{
    ...
    state_s = map_state_to_qlog(new_state, handshake_complete, handshake_confirmed);
...
void ossl_qlog_event_connectivity_connection_closed(QLOG *qlog, const QUIC_TERMINATE_CAUSE *tcause)
{
#ifndef OPENSSL_NO_QLOG
    ...
    if (tcause->app) {
        ...
        if (tcause->error_code >= OSSL_QUIC_ERR_CRYPTO_ERR_BEGIN
            ...
            BIO_snprintf(ce, sizeof(ce), "crypto_error_0x%03llx", (unsigned long long)tcause->error_code);
            m = ce;
        }
...
void ossl_qlog_event_recovery_packet_lost(QLOG *qlog, const QUIC_TXPIM_PKT *tpkt)
{
#ifndef OPENSSL_NO_QLOG
...
static int log_frame_actual(QLOG *qlog_instance, PACKET *pkt, size_t *need_skip)
{
    ...
    case OSSL_QUIC_FRAME_TYPE_STREAM_OFF_LEN_FIN: {
        ...
        if (f.is_fin)
            QLOG_BOOL("fin", 1);
        *need_skip = f.has_explicit_len ? *need_skip + (size_t)f.len : SIZE_MAX;
    } break;
    ...
    case OSSL_QUIC_FRAME_TYPE_MAX_STREAMS_UNI: {
        ...
        QLOG_STR("stream_type",
                 frame_type == OSSL_QUIC_FRAME_TYPE_MAX_STREAMS_BIDI ? "bidirectional" : "unidirectional");
        QLOG_U64("maximum", x);
    } break;
    case OSSL_QUIC_FRAME_TYPE_MAX_STREAM_DATA: {
        uint64_t stream_id, max_data;

        if (!ossl_quic_wire_decode_frame_max_stream_data(pkt, &stream_id, &max_data))
            goto unknown;
    ...
    case OSSL_QUIC_FRAME_TYPE_NEW_CONN_ID: {
        ...
        QLOG_BIN("stateless_reset_token", f.stateless_reset.token, sizeof(f.stateless_reset.token));
    } break;
    ...
    case OSSL_QUIC_FRAME_TYPE_STREAM_DATA_BLOCKED: {
        ...
        if (!ossl_quic_wire_decode_frame_stream_data_blocked(pkt, &stream_id, &x))
            goto unknown;
    ...
    case OSSL_QUIC_FRAME_TYPE_STREAMS_BLOCKED_UNI: {
        ...
        QLOG_STR("stream_type",
                 frame_type == OSSL_QUIC_FRAME_TYPE_STREAMS_BLOCKED_BIDI ? "bidirectional" : "unidirectional");
        QLOG_U64("limit", x);
    } break;
...
unknown:
         ...
         */
        QLOG_BEGIN("raw");
        QLOG_BIN("data", PACKET_data(&orig_pkt), PACKET_remaining(&orig_pkt));
        QLOG_END();
...
static int log_frames(QLOG *qlog_instance, const OSSL_QTX_IOVEC *iovec, size_t num_iovec)
{
    size_t i;
    PACKET pkt;
    size_t need_skip = 0;
...
static void log_packet(QLOG *qlog_instance, const QUIC_PKT_HDR *hdr, QUIC_PN pn, const OSSL_QTX_IOVEC *iovec,
                       size_t num_iovec, uint64_t datagram_id)
{
    const char *type_s;
...
void ossl_qlog_event_transport_packet_sent(QLOG *qlog, const QUIC_PKT_HDR *hdr, QUIC_PN pn, const OSSL_QTX_IOVEC *iovec,
                                           size_t num_iovec, uint64_t datagram_id)
{
#ifndef OPENSSL_NO_QLOG
...
void ossl_qlog_event_transport_packet_received(QLOG *qlog, const QUIC_PKT_HDR *hdr, QUIC_PN pn,
