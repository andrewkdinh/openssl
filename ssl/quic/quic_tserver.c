...
static int alpn_select_cb(SSL *ssl, const unsigned char **out,
    ...
    static const unsigned char alpndeflt[] = {8,   'o', 's', 's', 'l',
                                              't', 'e', 's', 't'};
    ...
    if (SSL_select_next_proto((unsigned char **)out, outlen, alpn, alpnlen, in,
...
QUIC_TSERVER *ossl_quic_tserver_new(const QUIC_TSERVER_ARGS *args,
    ...
    if (args->ctx != NULL)
        ...
        srv->ctx =
        SSL_CTX_new_ex(srv->args.libctx, srv->args.propq, TLS_method());
    if (srv->ctx == NULL)
        goto err;
    ...
    if ((srv->port = ossl_quic_engine_create_port(srv->engine, &port_args)) ==
        NULL)
        goto err;
...
void *mutatearg) {
    return ossl_statem_set_mutator(ossl_quic_channel_get0_ssl(srv->ch),
                                   mutate_handshake_cb,
                                   finish_mutate_handshake_cb, mutatearg);
}
...
int ossl_quic_tserver_read(QUIC_TSERVER *srv, uint64_t stream_id,
    ...
    qs = ossl_quic_stream_map_get_by_id(ossl_quic_channel_get_qsm(srv->ch),
                                        stream_id);
    if (qs == NULL) {
        int is_client_init = ((stream_id & QUIC_STREAM_INITIATOR_MASK) ==
    ...
    if (is_fin)
        ...
        ossl_quic_channel_get_qsm(srv->ch), qs);
...
int ossl_quic_tserver_has_read_ended(QUIC_TSERVER *srv, uint64_t stream_id) {
     ...
     */
    if (!ossl_quic_rstream_peek(qs->rstream, buf, sizeof(buf), &bytes_read,
                                &is_fin))
        return 0;
    ...
    if (is_fin && bytes_read == 0) {
        ...
        if (!ossl_quic_rstream_read(qs->rstream, buf, sizeof(buf), &bytes_read,
                                    &is_fin))
            return 0;
...
SSL_CTX *ossl_quic_tserver_get0_ssl_ctx(QUIC_TSERVER *srv) { return srv->ctx; }
...
void ossl_quic_tserver_set_psk_find_session_cb(
QUIC_TSERVER *srv, SSL_psk_find_session_cb_func cb) {
