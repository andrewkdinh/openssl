...
void ossl_qtx_set_mutator(OSSL_QTX             *qtx,
                          ossl_mutate_packet_cb mutatecb,
                          ossl_finish_mutate_cb finishmutatecb,
                          void                 *mutatearg)
{
...
int ossl_qtx_calculate_ciphertext_payload_len(OSSL_QTX *qtx,
                                              uint32_t  enc_level,
...
int ossl_qtx_calculate_plaintext_payload_len(OSSL_QTX *qtx,
                                             uint32_t  enc_level,
                                             size_t    ciphertext_len,
                                             size_t   *plaintext_len)
{
...
static int qtx_encrypt_into_txe(OSSL_QTX            *qtx,
                                ...
                                QUIC_PKT_HDR_PTRS   *ptrs)
{
    ...
    for (;;) {
        ...
        if (EVP_CipherUpdate(cctx, txe_data(txe) + txe->data_len, &l, src, (int)src_len) != 1) {
            ERR_raise(ERR_LIB_SSL, ERR_R_EVP_LIB);
            return 0;
        }
 ...
 * process.
 */
static int qtx_write(OSSL_QTX             *qtx,
