...
int ossl_qtx_provide_secret(OSSL_QTX *qtx, uint32_t enc_level,
  ...
  return ossl_qrl_enc_level_set_provide_secret(&qtx->el_set, qtx->libctx,
                                               qtx->propq, enc_level, suite_id,
                                               md, secret, secret_len, 0,
                                               /*is_tx=*/1);
}
...
static void iovec_cur_init(struct iovec_cur *cur, const OSSL_QTX_IOVEC *iovec,
                           size_t num_iovec) {
  cur->iovec = iovec;
...
static int qtx_write_hdr(OSSL_QTX *qtx, const QUIC_PKT_HDR *hdr, TXE *txe,
  ...
  if (!ossl_quic_wire_encode_pkt_hdr(&wpkt, hdr->dst_conn_id.id_len, hdr,
                                     ptrs) ||
...
static int qtx_encrypt_into_txe(OSSL_QTX *qtx, struct iovec_cur *cur, TXE *txe,
  ...
  for (;;) {
    ...
    if (EVP_CipherUpdate(cctx, txe_data(txe) + txe->data_len, &l, src,
                         src_len) != 1) {
      ERR_raise(ERR_LIB_SSL, ERR_R_EVP_LIB);
      return 0;
    }
  ...
  if (EVP_CIPHER_CTX_ctrl(cctx, EVP_CTRL_AEAD_GET_TAG, el->tag_len,
                          txe_data(txe) + txe->data_len) != 1) {
    ERR_raise(ERR_LIB_SSL, ERR_R_EVP_LIB);
    return 0;
  }
...
static int qtx_write(OSSL_QTX *qtx, const OSSL_QTX_PKT *pkt, TXE *txe,
  ...
  if (needs_encrypt)
    ...
    qtx, enc_level, cur.bytes_remaining, &payload_len);
  else
  ...
  hdr->len = payload_len;
  pred_hdr_len =
  ossl_quic_wire_get_encoded_pkt_hdr_len(hdr->dst_conn_id.id_len, hdr);
  if (pred_hdr_len == 0) {
    ret = QTX_FAIL_GENERIC;
    goto err;
  }
  ...
  if (!needs_encrypt) {
    ...
    /* Encrypt into TXE. */
    if (!qtx_encrypt_into_txe(qtx, &cur, txe, enc_level, pkt->pn, hdr_start,
                              hdr_len, &ptrs)) {
      ret = QTX_FAIL_GENERIC;
      goto err;
    }
...
static int qtx_mutate_write(OSSL_QTX *qtx, const OSSL_QTX_PKT *pkt, TXE *txe,
  ...
  if (qtx->mutatecb != NULL) {
    if (!qtx->mutatecb(pkt->hdr, pkt->iovec, pkt->num_iovec, &hdr, &iovec,
                       &num_iovec, qtx->mutatearg))
  ...
  if (ret == 1)
    ...
    qtx_get_qlog(qtx), hdr, pkt->pn, iovec, num_iovec, qtx->datagram_count);
...
int ossl_qtx_flush_net(OSSL_QTX *qtx) {
  ...
  for (;;) {
    for (txe = ossl_list_txe_head(&qtx->pending), i = 0;
         txe != NULL && i < OSSL_NELEM(msg); txe = ossl_list_txe_next(txe), ++i)
  ...
  return total_written > 0 ? QTX_FLUSH_NET_RES_OK
...
void ossl_qtx_set_bio(OSSL_QTX *qtx, BIO *bio) { qtx->bio = bio; }
...
size_t ossl_qtx_get_mdpl(OSSL_QTX *qtx) { return qtx->mdpl; }
