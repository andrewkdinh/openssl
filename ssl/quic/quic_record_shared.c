...
static int el_setup_keyslot(OSSL_QRL_ENC_LEVEL_SET *els, uint32_t enc_level,
  ...
  if (!ossl_assert(el != NULL && ossl_qrl_enc_level_set_has_keyslot(
  ...
  /* Derive "quic key" key. */
  if (!tls13_hkdf_expand_ex(el->libctx, el->propq, el->md, secret,
                            quic_v1_key_label, sizeof(quic_v1_key_label), NULL,
                            0, key, key_len, 1))
    goto err;
...
uint32_t enc_level, uint32_t suite_id, EVP_MD *md, const unsigned char *secret,
size_t secret_len, unsigned char init_key_phase_bit, int is_tx) {
  OSSL_QRL_ENC_LEVEL *el = ossl_qrl_enc_level_set_get(els, enc_level, 0);
  ...
  if (el == NULL || md_name == NULL || init_key_phase_bit > 1 || is_tx < 0 ||
      is_tx > 1 ||
  ...
  if (!el_setup_keyslot(els, enc_level, QRL_EL_STATE_PROV_NORMAL, init_keyslot,
                        secret, secret_len))
    goto err;
  ...
  if (enc_level == QUIC_ENC_LEVEL_1RTT) {
    ...
    if (!tls13_hkdf_expand_ex(libctx, propq, md, secret, quic_v1_ku_label,
    ...
    if (!is_tx) {
      ...
      if (!tls13_hkdf_expand_ex(libctx, propq, md, ku_key, quic_v1_ku_label,
  ...
  if (!ossl_quic_hdr_protector_init(
      &el->hpr, libctx, propq, ossl_qrl_get_suite_hdr_prot_cipher_id(suite_id),
      hpr_key, hpr_key_len))
    goto err;
...
int ossl_qrl_enc_level_set_key_update(OSSL_QRL_ENC_LEVEL_SET *els,
  ...
  if (!tls13_hkdf_expand_ex(el->libctx, el->propq, el->md, el->ku,
                            quic_v1_ku_label, sizeof(quic_v1_ku_label), NULL, 0,
                            new_ku, secret_len, 1))
    return 0;
  ...
  if (!el_setup_keyslot(els, enc_level, QRL_EL_STATE_PROV_NORMAL, 0, el->ku,
                        secret_len))
    return 0;
