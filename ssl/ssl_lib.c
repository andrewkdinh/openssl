...
static int ssl_undefined_function_7(SSL_CONNECTION      *sc,
                                    unsigned char       *r,
                                    size_t               s,
                                    const char          *t,
                                    size_t               u,
...
static int
dane_tlsa_add(SSL_DANE *dane, uint8_t usage, uint8_t selector, uint8_t mtype, const unsigned char *data, size_t dlen)
{
    danetls_record *t;
    ...
    if (ctx->ext.supportedgroups != NULL) {
        ...
        s->ext.supportedgroups =
            OPENSSL_memdup(ctx->ext.supportedgroups,
                           (ctx->ext.supportedgroups_len + add) * sizeof(*ctx->ext.supportedgroups));
...
long SSL_CTX_callback_ctrl(SSL_CTX *ctx, int cmd, void (*fp)(void))
{
    ...
    case SSL_CTRL_SET_MSG_CALLBACK:
        ctx->msg_callback =
            (void (*)(int write_p, int version, int content_type, const void *buf, size_t len, SSL *ssl, void *arg))(
                fp);
        return 1;
                               ...
                               int                  use_context)
{
    ...
    return sc->ssl.method->ssl3_enc
        ->export_keying_material(sc, out, olen, label, llen, context, contextlen, use_context);
}

int SSL_export_keying_material_early(SSL                 *s,
                                     unsigned char       *out,
                                     size_t               olen,
                                     const char          *label,
                                     size_t               llen,
 ...
 */
static long
check_keylog_bio_free(BIO *b, int oper, const char *argp, size_t len, int argi, long argl, int ret, size_t *processed)
{
    /*
...
int (*SSL_get_security_callback(
    const SSL *s))(const SSL *s, const SSL_CTX *ctx, int op, int bits, int nid, void *other, void *ex)
{
    const SSL_CONNECTION *sc = SSL_CONNECTION_FROM_CONST_SSL(s);
...
int (*SSL_CTX_get_security_callback(
    const SSL_CTX *ctx))(const SSL *s, const SSL_CTX *ctx, int op, int bits, int nid, void *other, void *ex)
{
...
int ossl_bytes_to_cipher_list(SSL_CONNECTION        *s,
