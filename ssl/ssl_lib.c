...
SSL *ossl_ssl_connection_new_int(SSL_CTX *ctx, SSL *user_ssl,
    ...
    if (ctx->ext.supportedgroups != NULL) {
        ...
        ctx->ext.supportedgroups, (ctx->ext.supportedgroups_len + add) *
...
int SSL_set1_host(SSL *s, const char *host) {
    ...
    /* clear hostname(s) and IP address in any case, also if host parses as an
     * IP address */
...
void SSL_set_read_ahead(SSL *s, int yes) {
    ...
    *opts++ = OSSL_PARAM_construct_int(
    OSSL_LIBSSL_RECORD_LAYER_PARAM_READ_AHEAD, &sc->rlayer.read_ahead);
...
ossl_ssize_t SSL_sendfile(SSL *s, int fd, off_t offset, size_t size,
    ...
    if (ret < 0) {
        ...
        if ((get_last_sys_error() == EAGAIN) ||
            (get_last_sys_error() == EINTR) || (get_last_sys_error() == EBUSY))
...
long SSL_CTX_ctrl(SSL_CTX *ctx, int cmd, long larg, void *parg) {
    ...
    if (cmd == SSL_CTRL_SET_GROUPS_LIST && larg == 1)
        return tls1_set_groups_list(ctx, NULL, NULL, NULL, NULL, NULL, NULL,
                                    parg);
    if (ctx == NULL) {
        switch (cmd) {
...
const char *SSL_get_servername(const SSL *s, const int type) {
    ...
    if (server) {
         ...
         *
         * During/after the handshake (TLSv1.2 or below resumption did not
         * occur):
...
int SSL_set0_tmp_dh_pkey(SSL *s, EVP_PKEY *dhpkey) {
    ...
    if (!ssl_security(sc, SSL_SECOP_TMP_DH, EVP_PKEY_get_security_bits(dhpkey),
                      0, dhpkey)) {
