                               ...
                               int (*func)(void *))
{
    ...
    if (sc->waitctx == NULL) {
        ...
        if (sc->waitctx == NULL)
            return -1;
        if (sc->async_cb != NULL
            && !ASYNC_WAIT_CTX_set_callback(sc->waitctx, ssl_async_wait_ctx_cb,
                                            s))
            return -1;
    }
...
long SSL_CTX_callback_ctrl(SSL_CTX *ctx, int cmd, void (*fp)(void))
{
    ...
    case SSL_CTRL_SET_MSG_CALLBACK:
        ctx->msg_callback =
            (void (*)(int write_p, int version, int content_type,
...
SSL *SSL_dup(SSL *s)
{
    ...
    if (sc->cipher_list_by_id != NULL)
        if ((retsc->cipher_list_by_id =
                 sk_SSL_CIPHER_dup(sc->cipher_list_by_id))
            == NULL)
            goto err;
...
int ossl_adjust_domain_flags(uint64_t domain_flags, uint64_t *p_domain_flags)
{
    ...
    if ((domain_flags
         & (SSL_DOMAIN_FLAG_MULTI_THREAD | SSL_DOMAIN_FLAG_SINGLE_THREAD))
        == 0)
        domain_flags |= SSL_DOMAIN_FLAG_MULTI_THREAD;
...
int SSL_add_expected_rpk(SSL *s, EVP_PKEY *rpk)
{
    ...
    ret = SSL_dane_tlsa_add(s, DANETLS_USAGE_DANE_EE, DANETLS_SELECTOR_SPKI,
                            DANETLS_MATCHING_FULL, data, (size_t)ret)
        > 0;
    OPENSSL_free(data);
    return ret;
}
