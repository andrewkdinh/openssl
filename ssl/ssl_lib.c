...
static int ssl_dane_dup(SSL_CONNECTION *to, SSL_CONNECTION *from) {
  ...
  for (i = 0; i < num; ++i) {
    ...
    if (SSL_dane_tlsa_add(SSL_CONNECTION_GET_SSL(to), t->usage, t->selector,
                          t->mtype, t->data, t->dlen) <= 0)
      return 0;
  }
  return 1;
}
...
int SSL_CTX_set_ssl_version(SSL_CTX *ctx, const SSL_METHOD *meth) {
  ...
  sk = ssl_create_cipher_list(ctx, ctx->tls13_ciphersuites, &(ctx->cipher_list),
...
SSL *ossl_ssl_connection_new_int(SSL_CTX *ctx, SSL *user_ssl,
  ...
  if (ctx->ext.ecpointformats != NULL) {
    ...
    OPENSSL_memdup(ctx->ext.ecpointformats, ctx->ext.ecpointformats_len);
  ...
  if (ctx->ext.keyshares != NULL) {
    ...
    if (ctx->ext.keyshares_len == 0)
      ...
      add = 1;
    s->ext.keyshares =
    OPENSSL_memdup(ctx->ext.keyshares, (ctx->ext.keyshares_len + add) *
  ...
  if (ctx->ext.tuples != NULL) {
    ...
    ctx->ext.tuples, (ctx->ext.tuples_len + add) * sizeof(*ctx->ext.tuples));
  ...
  if (ctx->client_cert_type != NULL) {
    s->client_cert_type =
    OPENSSL_memdup(ctx->client_cert_type, ctx->client_cert_type_len);
  ...
  if (ctx->server_cert_type != NULL) {
    s->server_cert_type =
    OPENSSL_memdup(ctx->server_cert_type, ctx->server_cert_type_len);
...
int SSL_is_quic(const SSL *s) { return IS_QUIC(s); }
...
int SSL_set1_host(SSL *s, const char *host) {
  ...
  /* clear hostname(s) and IP address in any case, also if host parses as an IP
   * address */
...
int SSL_CTX_dane_enable(SSL_CTX *ctx) { return dane_ctx_enable(&ctx->dane); }
...
ossl_ssize_t SSL_sendfile(SSL *s, int fd, off_t offset, size_t size,
                          int flags) {
  ossl_ssize_t ret;
  SSL_CONNECTION *sc = SSL_CONNECTION_FROM_SSL_ONLY(s);
  ...
  if (ret < 0) {
    ...
    if ((get_last_sys_error() == EAGAIN) || (get_last_sys_error() == EINTR) ||
...
int SSL_new_session_ticket(SSL *s) {
  ...
  if ((SSL_in_init(s) && sc->ext.extra_tickets_expected == 0) ||
      SSL_IS_FIRST_HANDSHAKE(sc) || !sc->server || !SSL_CONNECTION_IS_TLS13(sc))
    return 0;
...
long ossl_ctrl_internal(SSL *s, int cmd, long larg, void *parg, int no_quic) {
  ...
  case SSL_CTRL_MODE: {
    ...
    *opts++ =
    OSSL_PARAM_construct_uint32(OSSL_LIBSSL_RECORD_LAYER_PARAM_MODE, &sc->mode);
    *opts = OSSL_PARAM_construct_end();
...
int SSL_set_cipher_list(SSL *s, const char *str) {
  ...
  ctx = s->ctx;
  sk = ssl_create_cipher_list(ctx, sc->tls13_ciphersuites, &sc->cipher_list,
                              &sc->cipher_list_by_id, str, sc->cert);
...
int SSL_export_keying_material_early(SSL *s, unsigned char *out, size_t olen,
  ...
  return tls13_export_keying_material_early(sc, out, olen, label, llen, context,
                                            contextlen);
}
...
/**
 * @brief Static initialization for a one-time action to initialize the SSL key
 * log.
 */
...
SSL_CTX *SSL_CTX_new_ex(OSSL_LIB_CTX *libctx, const char *propq,
  ...
  if (!ssl_create_cipher_list(ret, ret->tls13_ciphersuites, &ret->cipher_list,
...
#define eng_str(x) eng_strx(x)
  ...
  if (IS_QUIC_METHOD(meth)) {
    ...
    if (meth == OSSL_QUIC_client_thread_method())
      ...
      ret->domain_flags =
      SSL_DOMAIN_FLAG_MULTI_THREAD | SSL_DOMAIN_FLAG_LEGACY_BLOCKING;
#else
    ret->domain_flags =
    SSL_DOMAIN_FLAG_SINGLE_THREAD | SSL_DOMAIN_FLAG_LEGACY_BLOCKING;
#endif
  }
#endif
...
const SSL_METHOD *SSL_get_ssl_method(const SSL *s) { return s->method; }
...
uint64_t SSL_CTX_get_options(const SSL_CTX *ctx) { return ctx->options; }
...
const EVP_CIPHER *ssl_evp_cipher_fetch(OSSL_LIB_CTX *libctx, int nid,
  ...
  if (ciph != NULL) {
    ...
    params[0] =
    OSSL_PARAM_construct_int(OSSL_CIPHER_PARAM_DECRYPT_ONLY, &decrypt_only);
...
int SSL_set0_tmp_dh_pkey(SSL *s, EVP_PKEY *dhpkey) {
  ...
  if (!ssl_security(sc, SSL_SECOP_TMP_DH, EVP_PKEY_get_security_bits(dhpkey), 0,
                    dhpkey)) {
...
int SSL_is_connection(SSL *s) { return SSL_get0_connection(s) == s; }
...
int SSL_is_listener(SSL *s) { return SSL_get0_listener(s) == s; }

int SSL_is_domain(SSL *s) { return SSL_get0_domain(s) == s; }
...
int SSL_stream_reset(SSL *s, const SSL_STREAM_RESET_ARGS *args,
                     size_t args_len) {
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC(s))
    return 0;
...
int ossl_adjust_domain_flags(uint64_t domain_flags, uint64_t *p_domain_flags) {
  ...
  if ((domain_flags &
       (SSL_DOMAIN_FLAG_MULTI_THREAD | SSL_DOMAIN_FLAG_SINGLE_THREAD)) == 0)
    domain_flags |= SSL_DOMAIN_FLAG_MULTI_THREAD;
...
int SSL_set1_client_cert_type(SSL *s, const unsigned char *val, size_t len) {
  ...
  return set_cert_type(&sc->client_cert_type, &sc->client_cert_type_len, val,
                       len);
}
...
int SSL_set1_server_cert_type(SSL *s, const unsigned char *val, size_t len) {
  ...
  return set_cert_type(&sc->server_cert_type, &sc->server_cert_type_len, val,
                       len);
}

int SSL_CTX_set1_client_cert_type(SSL_CTX *ctx, const unsigned char *val,
                                  size_t len) {
  return set_cert_type(&ctx->client_cert_type, &ctx->client_cert_type_len, val,
                       len);
}

int SSL_CTX_set1_server_cert_type(SSL_CTX *ctx, const unsigned char *val,
                                  size_t len) {
  return set_cert_type(&ctx->server_cert_type, &ctx->server_cert_type_len, val,
                       len);
}
