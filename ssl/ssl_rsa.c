...
int SSL_use_PrivateKey_file(SSL *ssl, const char *file, int type)
{
    ...
    if (type == SSL_FILETYPE_PEM) {
        ...
        j = ERR_R_PEM_LIB;
        pkey = PEM_read_bio_PrivateKey_ex(in, NULL, sc->default_passwd_callback,
                                          sc->default_passwd_callback_userdata,
                                          ssl->ctx->libctx, ssl->ctx->propq);
    } else if (type == SSL_FILETYPE_ASN1) {
        j = ERR_R_ASN1_LIB;
        pkey =
            d2i_PrivateKey_ex_bio(in, NULL, ssl->ctx->libctx, ssl->ctx->propq);
    } else {
        ERR_raise(ERR_LIB_SSL, SSL_R_BAD_SSL_FILETYPE);
        goto end;
    }
...
int SSL_CTX_use_PrivateKey_file(SSL_CTX *ctx, const char *file, int type)
{
    ...
    if (type == SSL_FILETYPE_PEM) {
        ...
        pkey = PEM_read_bio_PrivateKey_ex(
            ...
            ctx->default_passwd_callback_userdata, ctx->libctx, ctx->propq);
    } else if (type == SSL_FILETYPE_ASN1) {
        j = ERR_R_ASN1_LIB;
...
static int use_certificate_chain_file(SSL_CTX *ctx, SSL *ssl, const char *file)
{
    ...
    if (PEM_read_bio_X509_AUX(in, &x, passwd_callback, passwd_callback_userdata)
        == NULL) {
                                   ...
                                   X509 *x, size_t chainidx, int *al, void *arg)
{
    ...
    if ((ssl_get_server_cert_serverinfo(sc, &serverinfo, &serverinfo_length))
        != 0) {
                              ...
                              size_t serverinfo_length)
{
    ...
    if (version == SSL_SERVERINFOV1) {
         ...
         */
        const size_t sinfo_length =
            extension_append_length(SSL_SERVERINFOV1, serverinfo_length);
        unsigned char *sinfo;
        int ret;
        ...
        extension_append(SSL_SERVERINFOV1, serverinfo, serverinfo_length,
                         sinfo);
