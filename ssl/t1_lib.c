...
static int add_provider_groups(const OSSL_PARAM params[], void *data) {
  ...
  if (ctx->group_list_max_len == ctx->group_list_len) {
    ...
    if (ctx->group_list_max_len == 0)
      tmp =
      OPENSSL_malloc(sizeof(TLS_GROUP_INFO) * TLS_GROUP_LIST_MALLOC_BLOCK_SIZE);
    else
      tmp =
      OPENSSL_realloc(ctx->group_list, (ctx->group_list_max_len +
...
static int add_provider_sigalgs(const OSSL_PARAM params[], void *data) {
  ...
  if (ctx->sigalg_list_max_len == ctx->sigalg_list_len) {
    ...
    if (ctx->sigalg_list_max_len == 0)
      ...
      tmp =
      OPENSSL_realloc(ctx->sigalg_list, (ctx->sigalg_list_max_len +
                                         TLS_SIGALG_LIST_MALLOC_BLOCK_SIZE) *
  ...
  if (keymgmt != NULL) {
    ...
    if (EVP_KEYMGMT_get0_provider(keymgmt) == provider) {
      ...
      OBJ_add_sigid(
      OBJ_txt2nid(sinf->sigalg_name),
      (sinf->hash_name != NULL ? OBJ_txt2nid(sinf->hash_name) : NID_undef),
      OBJ_txt2nid(keytype));
      ctx->sigalg_list_len++;
      sinf = NULL;
    }
...
typedef struct {
  ...
  size_t tuple_mode; /* Keeps track whether tuple_cb called from 'the top' or
                        from gid_cb */
  int ignore_unknown_default; /* Flag such that unknown groups for DEFAULT[_XYZ]
                                 are ignored */
} gid_cb_st;
...
static int gid_cb(const char *elem, int len, void *arg) {
   ...
   * cleared) */
  while (
  continue_while_loop && len > 0 &&
  ((current_prefix = strchr(prefixes, elem[0])) != NULL ||
   OPENSSL_strncasecmp(
   current_prefix = (char *)DEFAULT_GROUPNAME_FIRST_CHARACTER, elem, 1) == 0)) {
    ...
    default:
      ...
      for (i = 0; i < OSSL_NELEM(default_group_strings); i++) {
        if ((size_t)len == (strlen(default_group_strings[i].list_name)) &&
            OPENSSL_strncasecmp(default_group_strings[i].list_name, elem,
                                len) == 0) {
          /*
           ...
           * rather then starting a new tuple. Variable tuple_mode is the flag
           * which controls append tuple vs start new tuple.
           */
          ...
          /*
           * First, we restore any keyshare prefix in a new zero-terminated
           * string (if not already present)
           */
          restored_default_group_string = OPENSSL_malloc(
          (1 /* max prefix length */ +
           strlen(default_group_strings[i].group_string) + 1 /* \0 */) *
          sizeof(char));
      ...
      /*
       * If we reached this point, a group name started with a 'd' or 'D', but
       * no request for a DEFAULT[_XYZ] 'pseudo group' was detected, hence
       * processing of the group name can continue as usual (= the while loop
       * checking prefixes can end)
       */
   ...
   * e.g. for an unknown group, which we'd ignore (only) if relevant prefix was
   * set
   */
  if (gid == 0) {
  ...
  /* Remove group (and keyshare) from anywhere in the list if present, ignore if
   * not present */
  if (remove_group) {
    ...
    if (found_group) {
      ...
      garg
      ->gidcnt--; /* ..and update the book keeping for the number of groups */

      /*
       * We also must update the number of groups either in a previous tuple
       * (which we must identify and check whether it becomes empty due to the
       * deletion) or in the current tuple, pending where the deleted group
       * resides
       */
      k = 0;
      for (j = 0; j < garg->tplcnt; j++) {
        ...
        if (k > i) { /* remove from one of the previous tuples */
          garg->tuplcnt_arr[j]--;
          break; /* We took care not to have group duplicates, hence we can stop
                    here */
        }
      }
      ...
      if (found_group) {
        /* Found, hence we remove that keyshare from its position (which is at
         * i)... */
...
static int tuple_cb(const char *tuple, int len, void *arg) {
  ...
  if (garg->tplcnt == garg->tplmax) {
    size_t *tmp =
    OPENSSL_realloc(garg->tuplcnt_arr, (garg->tplmax + GROUPLIST_INCREMENT) *
...
int tls1_set_groups_list(SSL_CTX *ctx, uint16_t **grpext, size_t *grpextlen,
  ...
  if (gcb.gidcnt > 0 && gcb.ksidcnt == 0) {
    /*
     * No key share group prefix character was used, hence we indicate that a
     * single key share should be sent and flag that it should come from the
...
empty_list:
   ...
   */
  if (grpext == NULL || ksext == NULL || tplext == NULL || grpextlen == NULL ||
      ksextlen == NULL || tplextlen == NULL) {
    ret = 1;
    goto end;
  }

  /*
   * tuple_cb and gid_cb combo ensures there are no duplicates or unknown groups
   * so we can just go ahead and set the results (after diposing the existing)
   */
  OPENSSL_free(*grpext);
  *grpext = gcb.gid_arr;
...
static const SIGALG_LOOKUP *tls1_get_legacy_sigalg(const SSL_CONNECTION *s,
  ...
  if (idx == -1) {
    if (s->server) {
       ...
       * with new (aGOST12-only) ciphersuites, we should find out which one is
       * available really.
       */
...
SSL_TICKET_STATUS tls_decrypt_ticket(SSL_CONNECTION *s,
...
#endif
  {
    ...
    if (tctx->ext.ticket_key_evp_cb != NULL)
      rv = tctx->ext.ticket_key_evp_cb(SSL_CONNECTION_GET_USER_SSL(s), nctick,
  ...
  if (s->session_ctx->decrypt_ticket_cb != NULL &&
    ...
    retcb = s->session_ctx->decrypt_ticket_cb(SSL_CONNECTION_GET_SSL(s), sess,
                                              etick, keyname_len, ret,
...
static int tls1_check_sig_alg(SSL_CONNECTION *s, X509 *x, int default_nid) {
  ...
  for (i = 0; i < sigalgslen; i++) {
    ...
    sigalg = use_pc_sigalgs ? tls1_lookup_sigalg(SSL_CONNECTION_GET_CTX(s),
...
static int check_cert_usable(SSL_CONNECTION *s, const SIGALG_LOOKUP *sig,
  ...
  supported =
  EVP_PKEY_digestsign_supports_digest(pkey, sctx->libctx, mdname, sctx->propq);
  if (supported <= 0)
    return 0;
...
#ifndef OPENSSL_NO_GOST
        /*
         * Some Windows-based implementations do not send GOST algorithms
         * indication in supported_algorithms extension, so when we have
         * GOST-based ciphersuite, we have to assume GOST support.
         */
        if (i == s->shared_sigalgslen && (s->s3.tmp.new_cipher->algorithm_auth &
