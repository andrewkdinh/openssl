...
static int add_provider_groups(const OSSL_PARAM params[], void *data) {
    ...
    p =
    OSSL_PARAM_locate_const(params, OSSL_CAPABILITY_TLS_GROUP_NAME_INTERNAL);
    if (p == NULL || p->data_type != OSSL_PARAM_UTF8_STRING) {
        ERR_raise(ERR_LIB_SSL, ERR_R_PASSED_INVALID_ARGUMENT);
        goto err;
    }
    ...
    p =
    OSSL_PARAM_locate_const(params, OSSL_CAPABILITY_TLS_GROUP_SECURITY_BITS);
...
static int add_provider_sigalgs(const OSSL_PARAM params[], void *data) {
    ...
    p =
    OSSL_PARAM_locate_const(params, OSSL_CAPABILITY_TLS_SIGALG_SECURITY_BITS);
    ...
    keytype = (sinf->keytype != NULL
               ? sinf->keytype
               : (sinf->sig_name != NULL ? sinf->sig_name : sinf->sigalg_name));
    ...
    if (keymgmt != NULL) {
         ...
         * same one back if you repeat the exact same fetch. Is this a
         * reasonable assumption to make (in which case perhaps we should
...
static int discover_provider_sigalgs(OSSL_PROVIDER *provider, void *vctx) {
    ...
    pgd.ctx = vctx;
    pgd.provider = provider;
    OSSL_PROVIDER_get_capabilities(provider, "TLS-SIGALG", add_provider_sigalgs,
                                   &pgd);
    /*
 ...
 * s->ext.supportedgroups == NULL means legacy syntax (no [*,/,-]) from built-in
 * group array. In this case, we need to send exactly one key share, which MUST
 * be the first (leftmost) eligible group from the legacy list. Therefore, we
 * provide the entire list of supported groups in this case.
 *
 * A 'flag' to indicate legacy syntax is created by setting the number of key
 * shares to 1, but the groupID to 0. The 'flag' is checked right at the
 * beginning in tls_construct_ctos_key_share and either the "list of requested
 * key share groups" is used, or the "list of supported groups" in combination
 * with setting add_only_one = 1 is applied.
 */
...
int tls_valid_group(SSL_CONNECTION *s, uint16_t group_id, int minversion,
    ...
    group_minversion =
    SSL_CONNECTION_IS_DTLS(s) ? ginfo->mindtls : ginfo->mintls;
    group_maxversion =
    SSL_CONNECTION_IS_DTLS(s) ? ginfo->maxdtls : ginfo->maxtls;
    ...
    if (!SSL_CONNECTION_IS_DTLS(s)) {
        if (ret && okfortls13 != NULL && maxversion == TLS1_3_VERSION)
            *okfortls13 =
            (group_maxversion == 0) || (group_maxversion >= TLS1_3_VERSION);
    }
...
static void free_wrapper(TLS_GROUP_IX *a) { OPENSSL_free(a); }
 ...
 * and or tuple delimiters. It can be hierarchically defined (for potential
 * future use). IMPORTANT REMARK: For ease of use, in the built-in lists of
 * groups, unknown groups or groups not backed by a provider will always
 * silently be ignored, even without '?' prefix
 */
typedef struct {
...
{"GC256A", OSSL_TLS_GROUP_ID_gc256A}, {"GC256B", OSSL_TLS_GROUP_ID_gc256B},
{"GC256C", OSSL_TLS_GROUP_ID_gc256C}, {"GC256D", OSSL_TLS_GROUP_ID_gc256D},
{"GC512A", OSSL_TLS_GROUP_ID_gc512A}, {"GC512B", OSSL_TLS_GROUP_ID_gc512B},
 ...
 * second callback function (gid_cb) is used to process the groups inside a
 * tuple. Those callback functions are (indirectly) called by CONF_parse_list
 * with different separators (nominally ':' or '/'), a variable based on
 * gid_cb_st is used to keep track of the parsing results between the various
 * calls
 */

typedef struct {
    SSL_CTX *ctx;
    /* Variables to hold the three lists (groups, requested keyshares, tuple
     * structure) */
     ...
     */
    size_t tuple_mode; /* Keeps track whether tuple_cb called from 'the top' or
                          from gid_cb */
...
static int gid_cb(const char *elem, int len, void *arg) {
    ...
    /* Check the possible prefixes (remark: Leading and trailing spaces already
     * cleared) */
    while (continue_while_loop && len > 0 &&
           ((current_prefix = strchr(prefixes, elem[0])) != NULL ||
            OPENSSL_strncasecmp(current_prefix =
        ...
        case GROUP_DELIMITER_CHARACTER:
            return -1; /* Not a valid prefix for a single group name-> syntax
                          error */
            break;
        case KEY_SHARE_INDICATOR_CHARACTER:
            if (add_keyshare)
                return -1; /* Only single key share prefix allowed -> syntax
                              error */
            add_keyshare = 1;
            ++elem;
            --len;
        ...
        case REMOVE_GROUP_INDICATOR_CHARACTER:
            if (remove_group)
                return -1; /* Only single remove group prefix allowed -> syntax
                              error */
            remove_group = 1;
            ++elem;
            --len;
        ...
        default:
            ...
            for (i = 0; i < OSSL_NELEM(default_group_strings); i++) {
                if ((size_t)len ==
                    (strlen(default_group_strings[i].list_name)) &&
                     ...
                     * DEFAULT group string like a tuple which is appended to
                     * the current tuple rather then starting a new tuple.
                     * Variable tuple_mode is the flag which controls append
                     * tuple vs start new tuple.
                     */
                    ...
                    /*
                     * First, we restore any keyshare prefix in a new
                     * zero-terminated string (if not already present)
                     */
                    ...
                    if (restored_default_group_string == NULL)
                        return 0;
                    if (add_keyshare
                        /* Remark: we tolerate a duplicated keyshare indicator
                           here */
                     ...
                     */
                    garg->tuple_mode = 0;
                    /* We use the tuple_cb callback to process the pseudo group
                     * tuple */
            ...
            /*
             * If we reached this point, a group name started with a 'd' or 'D',
             * but no request for a DEFAULT[_XYZ] 'pseudo group' was detected,
             * hence processing of the group name can continue as usual (= the
             * while loop checking prefixes can end)
             */
    ...
    if (garg->ksidcnt == garg->ksidmax) {
        uint16_t *tmp =
        OPENSSL_realloc(garg->ksid_arr, (garg->ksidmax + GROUPLIST_INCREMENT) *
     ...
     * e.g. for an unknown group, which we'd ignore (only) if relevant prefix
     * was set
     */
    if (gid == 0) {
    ...
    /* Remove group (and keyshare) from anywhere in the list if present, ignore
     * if not present */
    if (remove_group) {
        ...
        if (found_group) {
            ...
            for (j = i; j < (garg->gidcnt - 1); j++)
                garg->gid_arr[j] =
                garg->gid_arr[j + 1]; /* ...shift remaining groups left ... */
            garg->gidcnt--; /* ..and update the book keeping for the number of
                               groups */

            /*
             * We also must update the number of groups either in a previous
             * tuple (which we must identify and check whether it becomes empty
             * due to the deletion) or in the current tuple, pending where the
             * deleted group resides
             */
            k = 0;
            ...
            if (found_group) {
                /* Found, hence we remove that keyshare from its position (which
                 * is at i)... */
                for (j = i; j < (garg->ksidcnt - 1); j++)
                    garg->ksid_arr[j] =
                    garg->ksid_arr[j + 1]; /* shift remaining key shares */
        ...
        /* We memorize if needed that we want to add a key share for the current
         * group */
        if (add_keyshare)
...
static int tuple_cb(const char *tuple, int len, void *arg) {
    ...
    retval = CONF_parse_list(restored_tuple_string, GROUP_DELIMITER_CHARACTER,
                             1, gid_cb, arg);
...
/*
 * Set groups and prepare generation of keyshares based on a string of
 * groupnames, names separated by the group or the tuple delimiter, with
 * per-group prefixes to (1) add a key share for this group, (2) ignore the
 * group if unkown to the current context, (3) delete a previous occurrence of
 * the group in the current tuple.
 *
 * The list parsing is done in two hierachical steps: The top-level step
 * extracts the string of a tuple using tuple_cb, while the next lower step uses
 * gid_cb to parse and process the groups inside a tuple
 */
int tls1_set_groups_list(SSL_CTX *ctx, uint16_t **grpext, size_t *grpextlen,
    ...
    /* Prepare initial chunks of memory for groups, tuples and keyshares
     * groupIDs
     */
    ...
    /*
     * Start the (potentially recursive) tuple processing by calling
     * CONF_parse_list with the TUPLE_DELIMITER_CHARACTER (which will call
     * tuple_cb after cleaning spaces)
     */
    parse_ret =
    CONF_parse_list(str, TUPLE_DELIMITER_CHARACTER, 1, tuple_cb, &gcb);
    ...
    if (gcb.ksidcnt > OPENSSL_CLIENT_MAX_KEY_SHARES) {
        ERR_raise_data(ERR_LIB_SSL, ERR_R_PASSED_INVALID_ARGUMENT,
                       "To many keyshares requested in '%s' (max = %d)", str,
                       OPENSSL_CLIENT_MAX_KEY_SHARES);
        goto end;
    }

    /*
     * For backward compatibility we let the rest of the code know that a key
     * share for the first valid group should be added if no "*" prefix was used
     * anywhere
     */
    if (gcb.gidcnt > 0 && gcb.ksidcnt == 0) {
        /*
         * No key share group prefix character was used, hence we indicate that
         * a single key share should be sent and flag that it should come from
...
empty_list:
     ...
     * to NULL only does a syntax check, hence we're done here and report
     * success
     */
    ...
    /*
     * tuple_cb and gid_cb combo ensures there are no duplicates or unknown
     * groups so we can just go ahead and set the results (after diposing the
     * existing)
     */
    OPENSSL_free(*grpext);
    *grpext = gcb.gid_arr;
...
{TLSEXT_SIGALG_ecdsa_secp256r1_sha256_name, "ECDSA+SHA256",
 ...
 EVP_PKEY_EC, SSL_PKEY_ECC, NID_ecdsa_with_SHA256, NID_X9_62_prime256v1, 1, 0,
...
{TLSEXT_SIGALG_ed25519_name, NULL, TLSEXT_SIGALG_ed25519, NID_undef, -1,
 EVP_PKEY_ED25519, SSL_PKEY_ED25519, NID_undef, NID_undef, 1, 0, TLS1_2_VERSION,
 0, DTLS1_2_VERSION, 0},
{TLSEXT_SIGALG_ed448_name, NULL, TLSEXT_SIGALG_ed448, NID_undef, -1,
 EVP_PKEY_ED448, SSL_PKEY_ED448, NID_undef, NID_undef, 1, 0, TLS1_2_VERSION, 0,
 DTLS1_2_VERSION, 0},

{TLSEXT_SIGALG_ecdsa_sha224_name, "ECDSA+SHA224", TLSEXT_SIGALG_ecdsa_sha224,
 NID_sha224, SSL_MD_SHA224_IDX, EVP_PKEY_EC, SSL_PKEY_ECC,
 NID_ecdsa_with_SHA224, NID_undef, 1, 0, TLS1_2_VERSION, TLS1_2_VERSION,
 DTLS1_2_VERSION, DTLS1_2_VERSION},
{TLSEXT_SIGALG_ecdsa_sha1_name, "ECDSA+SHA1", TLSEXT_SIGALG_ecdsa_sha1,
 NID_sha1, SSL_MD_SHA1_IDX, EVP_PKEY_EC, SSL_PKEY_ECC, NID_ecdsa_with_SHA1,
...
{TLSEXT_SIGALG_ecdsa_brainpoolP256r1_sha256_name,
 ...
 TLSEXT_SIGALG_ecdsa_brainpoolP256r1_sha256, NID_sha256, SSL_MD_SHA256_IDX,
 EVP_PKEY_EC, SSL_PKEY_ECC, NID_ecdsa_with_SHA256, NID_brainpoolP256r1, 1, 0,
 TLS1_3_VERSION, 0, -1, -1},
{TLSEXT_SIGALG_ecdsa_brainpoolP384r1_sha384_name,
 TLSEXT_SIGALG_ecdsa_brainpoolP384r1_sha384_alias,
 TLSEXT_SIGALG_ecdsa_brainpoolP384r1_sha384, NID_sha384, SSL_MD_SHA384_IDX,
 EVP_PKEY_EC, SSL_PKEY_ECC, NID_ecdsa_with_SHA384, NID_brainpoolP384r1, 1, 0,
 TLS1_3_VERSION, 0, -1, -1},
{TLSEXT_SIGALG_ecdsa_brainpoolP512r1_sha512_name,
 TLSEXT_SIGALG_ecdsa_brainpoolP512r1_sha512_alias,
 TLSEXT_SIGALG_ecdsa_brainpoolP512r1_sha512, NID_sha512, SSL_MD_SHA512_IDX,
 EVP_PKEY_EC, SSL_PKEY_ECC, NID_ecdsa_with_SHA512, NID_brainpoolP512r1, 1, 0,
 TLS1_3_VERSION, 0, -1, -1},

{TLSEXT_SIGALG_rsa_pss_rsae_sha256_name, "PSS+SHA256",
 TLSEXT_SIGALG_rsa_pss_rsae_sha256, NID_sha256, SSL_MD_SHA256_IDX,
 EVP_PKEY_RSA_PSS, SSL_PKEY_RSA, NID_undef, NID_undef, 1, 0, TLS1_2_VERSION, 0,
 DTLS1_2_VERSION, 0},
...
{TLSEXT_SIGALG_rsa_pss_pss_sha256_name, NULL, TLSEXT_SIGALG_rsa_pss_pss_sha256,
 NID_sha256, SSL_MD_SHA256_IDX, EVP_PKEY_RSA_PSS, SSL_PKEY_RSA_PSS_SIGN,
 NID_undef, NID_undef, 1, 0, TLS1_2_VERSION, 0, DTLS1_2_VERSION, 0},
{TLSEXT_SIGALG_rsa_pss_pss_sha384_name, NULL, TLSEXT_SIGALG_rsa_pss_pss_sha384,
 NID_sha384, SSL_MD_SHA384_IDX, EVP_PKEY_RSA_PSS, SSL_PKEY_RSA_PSS_SIGN,
 NID_undef, NID_undef, 1, 0, TLS1_2_VERSION, 0, DTLS1_2_VERSION, 0},
{TLSEXT_SIGALG_rsa_pss_pss_sha512_name, NULL, TLSEXT_SIGALG_rsa_pss_pss_sha512,
 NID_sha512, SSL_MD_SHA512_IDX, EVP_PKEY_RSA_PSS, SSL_PKEY_RSA_PSS_SIGN,
 NID_undef, NID_undef, 1, 0, TLS1_2_VERSION, 0, DTLS1_2_VERSION, 0},

{TLSEXT_SIGALG_rsa_pkcs1_sha256_name, "RSA+SHA256",
 TLSEXT_SIGALG_rsa_pkcs1_sha256, NID_sha256, SSL_MD_SHA256_IDX, EVP_PKEY_RSA,
 SSL_PKEY_RSA, NID_sha256WithRSAEncryption, NID_undef, 1, 0, TLS1_2_VERSION, 0,
 DTLS1_2_VERSION, 0},
{TLSEXT_SIGALG_rsa_pkcs1_sha384_name, "RSA+SHA384",
 TLSEXT_SIGALG_rsa_pkcs1_sha384, NID_sha384, SSL_MD_SHA384_IDX, EVP_PKEY_RSA,
 SSL_PKEY_RSA, NID_sha384WithRSAEncryption, NID_undef, 1, 0, TLS1_2_VERSION, 0,
 DTLS1_2_VERSION, 0},
{TLSEXT_SIGALG_rsa_pkcs1_sha512_name, "RSA+SHA512",
 TLSEXT_SIGALG_rsa_pkcs1_sha512, NID_sha512, SSL_MD_SHA512_IDX, EVP_PKEY_RSA,
 SSL_PKEY_RSA, NID_sha512WithRSAEncryption, NID_undef, 1, 0, TLS1_2_VERSION, 0,
 DTLS1_2_VERSION, 0},

{TLSEXT_SIGALG_rsa_pkcs1_sha224_name, "RSA+SHA224",
 TLSEXT_SIGALG_rsa_pkcs1_sha224, NID_sha224, SSL_MD_SHA224_IDX, EVP_PKEY_RSA,
 SSL_PKEY_RSA, NID_sha224WithRSAEncryption, NID_undef, 1, 0, TLS1_2_VERSION,
 TLS1_2_VERSION, DTLS1_2_VERSION, DTLS1_2_VERSION},
{TLSEXT_SIGALG_rsa_pkcs1_sha1_name, "RSA+SHA1", TLSEXT_SIGALG_rsa_pkcs1_sha1,
 NID_sha1, SSL_MD_SHA1_IDX, EVP_PKEY_RSA, SSL_PKEY_RSA,
 NID_sha1WithRSAEncryption, NID_undef, 1, 0, TLS1_2_VERSION, TLS1_2_VERSION,
 DTLS1_2_VERSION, DTLS1_2_VERSION},
...
{TLSEXT_SIGALG_dsa_sha384_name, "DSA+SHA384", TLSEXT_SIGALG_dsa_sha384,
 NID_sha384, SSL_MD_SHA384_IDX, EVP_PKEY_DSA, SSL_PKEY_DSA_SIGN, NID_undef,
 NID_undef, 1, 0, TLS1_2_VERSION, TLS1_2_VERSION, DTLS1_2_VERSION,
 DTLS1_2_VERSION},
{TLSEXT_SIGALG_dsa_sha512_name, "DSA+SHA512", TLSEXT_SIGALG_dsa_sha512,
 NID_sha512, SSL_MD_SHA512_IDX, EVP_PKEY_DSA, SSL_PKEY_DSA_SIGN, NID_undef,
 NID_undef, 1, 0, TLS1_2_VERSION, TLS1_2_VERSION, DTLS1_2_VERSION,
 DTLS1_2_VERSION},
{TLSEXT_SIGALG_dsa_sha224_name, "DSA+SHA224", TLSEXT_SIGALG_dsa_sha224,
 NID_sha224, SSL_MD_SHA224_IDX, EVP_PKEY_DSA, SSL_PKEY_DSA_SIGN, NID_undef,
 NID_undef, 1, 0, TLS1_2_VERSION, TLS1_2_VERSION, DTLS1_2_VERSION,
 DTLS1_2_VERSION},
{TLSEXT_SIGALG_dsa_sha1_name, "DSA+SHA1", TLSEXT_SIGALG_dsa_sha1, NID_sha1,
 SSL_MD_SHA1_IDX, EVP_PKEY_DSA, SSL_PKEY_DSA_SIGN, NID_dsaWithSHA1, NID_undef,
 1, 0, TLS1_2_VERSION, TLS1_2_VERSION, DTLS1_2_VERSION, DTLS1_2_VERSION},
...
{TLSEXT_SIGALG_gostr34102012_256_intrinsic_alias, /* RFC9189 */
 ...
 NID_undef, NID_undef, 1, 0, TLS1_2_VERSION, TLS1_2_VERSION, DTLS1_2_VERSION,
 DTLS1_2_VERSION},
{TLSEXT_SIGALG_gostr34102012_256_intrinsic_alias, /* RFC9189 */
 TLSEXT_SIGALG_gostr34102012_256_intrinsic_name,
...
{TLSEXT_SIGALG_gostr34102012_256_gostr34112012_256_name, NULL,
 TLSEXT_SIGALG_gostr34102012_256_gostr34112012_256, NID_id_GostR3411_2012_256,
 SSL_MD_GOST12_256_IDX, NID_id_GostR3410_2012_256, SSL_PKEY_GOST12_256,
 NID_undef, NID_undef, 1, 0, TLS1_2_VERSION, TLS1_2_VERSION, DTLS1_2_VERSION,
 DTLS1_2_VERSION},
{TLSEXT_SIGALG_gostr34102012_512_gostr34112012_512_name, NULL,
 TLSEXT_SIGALG_gostr34102012_512_gostr34112012_512, NID_id_GostR3411_2012_512,
 SSL_MD_GOST12_512_IDX, NID_id_GostR3410_2012_512, SSL_PKEY_GOST12_512,
 NID_undef, NID_undef, 1, 0, TLS1_2_VERSION, TLS1_2_VERSION, DTLS1_2_VERSION,
 DTLS1_2_VERSION},
{TLSEXT_SIGALG_gostr34102001_gostr3411_name, NULL,
 TLSEXT_SIGALG_gostr34102001_gostr3411, NID_id_GostR3411_94, SSL_MD_GOST94_IDX,
 NID_id_GostR3410_2001, SSL_PKEY_GOST01, NID_undef, NID_undef, 1, 0,
...
static const SIGALG_LOOKUP *tls1_get_legacy_sigalg(const SSL_CONNECTION *s,
    ...
    if (idx == -1) {
        if (s->server) {
            ...
            /*
             * As both SSL_PKEY_GOST12_512 and SSL_PKEY_GOST12_256 indices can
             * be used with new (aGOST12-only) ciphersuites, we should find out
             * which one is available really.
             */
            else if (idx == SSL_PKEY_GOST12_256) {
                int real_idx;

                for (real_idx = SSL_PKEY_GOST12_512;
                     real_idx >= SSL_PKEY_GOST12_256; real_idx--) {
                    if (s->cert->pkeys[real_idx].privatekey != NULL) {
                        idx = real_idx;
                        break;
                    }
    ...
    if (SSL_USE_SIGALGS(s) || idx != SSL_PKEY_RSA) {
        const SIGALG_LOOKUP *lu =
        tls1_lookup_sigalg(SSL_CONNECTION_GET_CTX(s), tls_default_sigalg[idx]);
...
int tls1_set_server_sigalgs(SSL_CONNECTION *s) {
    ...
    if (s->s3.tmp.valid_flags)
        ...
        OPENSSL_zalloc(s->ssl_pkey_num * sizeof(uint32_t));
...
SSL_TICKET_STATUS tls_decrypt_ticket(SSL_CONNECTION *s,
...
#endif
    {
        ...
        if (tctx->ext.ticket_key_evp_cb != NULL)
            rv =
            tctx->ext.ticket_key_evp_cb(SSL_CONNECTION_GET_USER_SSL(s), nctick,
    ...
    if (s->session_ctx->decrypt_ticket_cb != NULL &&
        ...
        SSL_CONNECTION_GET_SSL(s), sess, etick, keyname_len, ret,
...
int SSL_get_shared_sigalgs(SSL *s, int idx, int *psign, int *phash,
    ...
    if (sc->shared_sigalgs == NULL || idx < 0 ||
        idx >= (int)sc->shared_sigalgslen || sc->shared_sigalgslen > INT_MAX)
        return 0;
...
int tls1_check_chain(SSL_CONNECTION *s, X509 *x, EVP_PKEY *pk,
    ...
    if (idx != -1) {
        ...
        if (ssl_cert_lookup_by_pkey(pk, &certidx, SSL_CONNECTION_GET_CTX(s)) ==
            NULL)
            return 0;
        idx = certidx;
        pvalid = s->s3.tmp.valid_flags + idx;
...
EVP_PKEY *ssl_get_auto_dh(SSL_CONNECTION *s) {
    ...
    sec_level_bits =
    ssl_get_security_level_bits(SSL_CONNECTION_GET_SSL(s), NULL, NULL);
...
static int check_cert_usable(SSL_CONNECTION *s, const SIGALG_LOOKUP *sig,
    ...
    supported = EVP_PKEY_digestsign_supports_digest(pkey, sctx->libctx, mdname,
                                                    sctx->propq);
    if (supported <= 0)
        return 0;
 ...
 */
static int has_usable_cert(SSL_CONNECTION *s, const SIGALG_LOOKUP *sig,
                           int idx) {
...
static const SIGALG_LOOKUP *find_sig_alg(SSL_CONNECTION *s, X509 *x,
    ...
    for (i = 0; i < s->shared_sigalgslen; i++) {
        ...
        tmppkey =
        (pkey != NULL) ? pkey : s->cert->pkeys[lu->sig_idx].privatekey;
    ...
    if (SSL_CONNECTION_IS_TLS13(s)) {
        ...
        if (SSL_USE_SIGALGS(s)) {
            ...
            if (s->s3.tmp.peer_sigalgs != NULL) {
                 ...
                 * algorithms indication in supported_algorithms extension, so
                 * when we have GOST-based ciphersuite, we have to assume GOST
                 * support.
                 */
                if (i == s->shared_sigalgslen &&
...
EVP_MAC_CTX *ssl_hmac_get0_EVP_MAC_CTX(SSL_HMAC *ctx) { return ctx->ctx; }
