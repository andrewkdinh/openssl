...
static int do_ssl_trace_list(BIO *bio, int indent, const unsigned char *msg, size_t msglen, size_t vlen,
                             const ssl_trace_tbl *tbl, size_t ntbl)
{
    int val;
...
static const ssl_trace_tbl ssl_version_tbl[] = {{SSL3_VERSION, "SSL 3.0"},     {TLS1_VERSION, "TLS 1.0"},
                                                {TLS1_1_VERSION, "TLS 1.1"},   {TLS1_2_VERSION, "TLS 1.2"},
                                                {TLS1_3_VERSION, "TLS 1.3"},   {DTLS1_VERSION, "DTLS 1.0"},
                                                {DTLS1_2_VERSION, "DTLS 1.2"}, {DTLS1_BAD_VER, "DTLS 1.0 (bad)"}};
...
static const ssl_trace_tbl ssl_handshake_tbl[] = {{SSL3_MT_HELLO_REQUEST, "HelloRequest"},
...
static const ssl_trace_tbl ssl_groups_tbl[] = {{1, "sect163k1 (K-163)"},
                                               {2, "sect163r1"},
                                               {3, "sect163r2 (B-163)"},
                                               {4, "sect193r1"},
                                               {5, "sect193r2"},
...
static const ssl_trace_tbl ssl_point_tbl[] = {
    {0, "uncompressed"}, {1, "ansiX962_compressed_prime"}, {2, "ansiX962_compressed_char2"}};

static const ssl_trace_tbl ssl_mfl_tbl[] = {{0, "disabled"},
...
static const ssl_trace_tbl ssl_ctype_tbl[] = {
    ...
    {5, "rsa_ephemeral_dh"}, {6, "dss_ephemeral_dh"},  {20, "fortezza_dms"}, {64, "ecdsa_sign"},
    {65, "rsa_fixed_ecdh"},  {66, "ecdsa_fixed_ecdh"}, {67, "gost_sign256"}, {68, "gost_sign512"},
};

static const ssl_trace_tbl ssl_psk_kex_modes_tbl[] = {{TLSEXT_KEX_MODE_KE, "psk_ke"},
                                                      {TLSEXT_KEX_MODE_KE_DHE, "psk_dhe_ke"}};

static const ssl_trace_tbl ssl_key_update_tbl[] = {{SSL_KEY_UPDATE_NOT_REQUESTED, "update_not_requested"},
                                                   {SSL_KEY_UPDATE_REQUESTED, "update_requested"}};

static const ssl_trace_tbl ssl_comp_cert_tbl[] = {{TLSEXT_comp_cert_none, "none"},
 ...
 */
static const ssl_trace_tbl ssl_cert_type_tbl[] = {{TLSEXT_cert_type_x509, "x509"},
...
static int ssl_print_hexbuf(BIO *bio, int indent, const char *name, size_t nlen, const unsigned char **pmsg,
                            size_t *pmsglen)
{
    size_t blen;
...
static int ssl_print_version(BIO *bio, int indent, const char *name, const unsigned char **pmsg, size_t *pmsglen,
...
static int ssl_print_signature(BIO *bio, int indent, const SSL_CONNECTION *sc, const unsigned char **pmsg,
                               size_t *pmsglen)
{
    if (*pmsglen < 2)
        return 0;
...
static int ssl_print_extension(BIO *bio, int indent, int server, unsigned char mt, int extype, const unsigned char *ext,
                               size_t extlen)
{
    ...
    case TLSEXT_TYPE_early_data:
        ...
        if (extlen != 4)
            return 0;
        max_early_data = ((unsigned int)ext[0] << 24) | ((unsigned int)ext[1] << 16) | ((unsigned int)ext[2] << 8)
            | (unsigned int)ext[3];
        BIO_indent(bio, indent + 2, 80);
...
static int ssl_print_extensions(BIO *bio, int indent, int server, unsigned char mt, const unsigned char **msgin,
                                size_t *msginlen)
{
...
static int ssl_print_client_hello(BIO *bio, const SSL_CONNECTION *sc, int indent, const unsigned char *msg,
                                  size_t msglen)
{
    size_t len;
    unsigned int cs;
...
static int ssl_print_client_keyex(BIO *bio, int indent, const SSL_CONNECTION *sc, const unsigned char *msg,
                                  size_t msglen)
{
    const char *algname;
...
static int ssl_print_server_keyex(BIO *bio, int indent, const SSL_CONNECTION *sc, const unsigned char *msg,
                                  size_t msglen)
{
    const char *algname;
    int id = ssl_get_keyex(&algname, sc);
...
static int ssl_print_certificate(BIO *bio, const SSL_CONNECTION *sc, int indent, const unsigned char **pmsg,
                                 size_t *pmsglen)
{
...
static int ssl_print_raw_public_key(BIO *bio, const SSL *ssl, int server, int indent, const unsigned char **pmsg,
                                    size_t *pmsglen)
{
    EVP_PKEY *pkey;
    size_t clen;
...
static int ssl_print_certificates(BIO *bio, const SSL_CONNECTION *sc, int server, int indent, const unsigned char *msg,
                                  size_t msglen)
{
    size_t clen;

    if (SSL_CONNECTION_IS_TLS13(sc) && !ssl_print_hexbuf(bio, indent, "context", 1, &msg, &msglen))
        return 0;
    ...
    if ((server && sc->ext.server_cert_type == TLSEXT_cert_type_rpk)
        ...
        if (SSL_CONNECTION_IS_TLS13(sc)
            && !ssl_print_extensions(bio, indent + 2, server, SSL3_MT_CERTIFICATE, &msg, &clen))
            return 0;
        return 1;
    }
    BIO_indent(bio, indent, 80);
...
static int ssl_print_compressed_certificates(BIO *bio, const SSL_CONNECTION *sc, int server, int indent,
...
static int ssl_print_cert_request(BIO *bio, int indent, const SSL_CONNECTION *sc, const unsigned char *msg,
                                  size_t msglen)
{
    size_t xlen;
    unsigned int sigalg;
...
static int ssl_print_ticket(BIO *bio, int indent, const SSL_CONNECTION *sc, const unsigned char *msg, size_t msglen)
{
    ...
    if (msglen < 4)
        return 0;
    tick_life = ((unsigned int)msg[0] << 24) | ((unsigned int)msg[1] << 16) | ((unsigned int)msg[2] << 8)
        | (unsigned int)msg[3];
    msglen -= 4;
    msg += 4;
    BIO_indent(bio, indent + 2, 80);
    ...
    if (SSL_CONNECTION_IS_TLS13(sc)) {
        ...
        if (msglen < 4)
            return 0;
        ticket_age_add = ((unsigned int)msg[0] << 24) | ((unsigned int)msg[1] << 16) | ((unsigned int)msg[2] << 8)
            | (unsigned int)msg[3];
        msglen -= 4;
        msg += 4;
        BIO_indent(bio, indent + 2, 80);
    ...
    if (!ssl_print_hexbuf(bio, indent + 2, "ticket", 2, &msg, &msglen))
        return 0;
    if (SSL_CONNECTION_IS_TLS13(sc)
        && !ssl_print_extensions(bio, indent + 2, 0, SSL3_MT_NEWSESSION_TICKET, &msg, &msglen))
        return 0;
    if (msglen)
        return 0;
    return 1;
...
static int ssl_print_handshake(BIO *bio, const SSL_CONNECTION *sc, int server, const unsigned char *msg, size_t msglen,
                               int indent)
{
    size_t hlen;
    unsigned char htype;
    ...
    if (SSL_CONNECTION_IS_DTLS(sc)) {
        ...
        BIO_printf(bio,
                   ...
                   "fragment_length=%d\n",
                   (msg[0] << 8) | msg[1], (msg[2] << 16) | (msg[3] << 8) | msg[4],
...
# ifndef OPENSSL_NO_QUIC
    ...
    if (qc != NULL) {
        if (ossl_quic_trace(write_p, version, content_type, buf, msglen, ssl, arg))
            return;
        /*
...
# endif
    ...
    case SSL3_RT_HEADER: {
        ...
        if (SSL_CONNECTION_IS_DTLS(sc)) {
            BIO_printf(bio, "  epoch=%d, sequence_number=%04x%04x%04x\n", (msg[3] << 8 | msg[4]),
                       (msg[5] << 8 | msg[6]), (msg[7] << 8 | msg[8]), (msg[9] << 8 | msg[10]));
        }

        BIO_printf(bio, "  Content Type = %s (%d)\n  Length = %d", ssl_trace_str(msg[0], ssl_content_tbl), msg[0],
    ...
    case SSL3_RT_HANDSHAKE:
        if (!ssl_print_handshake(bio, sc, sc->server ? write_p : !write_p, msg, msglen, 4))
    ...
    case SSL3_RT_ALERT:
        if (msglen != 2)
            ...
            BIO_printf(bio, "    Level=%s(%d), description=%s(%d)\n", SSL_alert_type_string_long(msg[0] << 8), msg[0],
                       SSL_alert_desc_string_long(msg[1]), msg[1]);
        }
    }
