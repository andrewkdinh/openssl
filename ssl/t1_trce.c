...
static int ssl_print_extension(BIO *bio, int indent, int server,
  ...
  case TLSEXT_TYPE_signature_algorithms:
    ...
    while (xlen > 0) {
      ...
      BIO_printf(bio, "%s (0x%04x)\n", ssl_trace_str(sigalg, ssl_sigalg_tbl),
                 sigalg);
      xlen -= 2;
      ext += 2;
    }
...
static int ssl_print_client_keyex(BIO *bio, int indent,
  ...
  case SSL_kRSAPSK:
    if (TLS1_get_version(SSL_CONNECTION_GET_SSL(sc)) == SSL3_VERSION) {
      ...
      if (!ssl_print_hexbuf(bio, indent + 2, "EncryptedPreMasterSecret", 2,
                            &msg, &msglen))
        return 0;
    }
    break;
...
static int ssl_print_cert_request(BIO *bio, int indent,
  ...
  if (SSL_CONNECTION_IS_TLS13(sc)) {
    if (!ssl_print_hexbuf(bio, indent, "request_context", 1, &msg, &msglen))
      return 0;
    if (!ssl_print_extensions(bio, indent, 1, SSL3_MT_CERTIFICATE_REQUEST, &msg,
                              &msglen))
      return 0;
    return 1;
  } else {
...
static int ssl_print_ticket(BIO *bio, int indent, const SSL_CONNECTION *sc,
  ...
  if (!ssl_print_hexbuf(bio, indent + 2, "ticket", 2, &msg, &msglen))
    return 0;
  if (SSL_CONNECTION_IS_TLS13(sc) &&
      !ssl_print_extensions(bio, indent + 2, 0, SSL3_MT_NEWSESSION_TICKET, &msg,
                            &msglen))
    return 0;
  if (msglen)
    return 0;
...
static int ssl_print_handshake(BIO *bio, const SSL_CONNECTION *sc, int server,
  ...
  case SSL3_MT_ENCRYPTED_EXTENSIONS:
    if (!ssl_print_extensions(bio, indent + 2, 1, SSL3_MT_ENCRYPTED_EXTENSIONS,
                              &msg, &msglen))
      return 0;
    break;
