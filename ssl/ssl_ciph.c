...
{0, SSL3_TXT_EDH_DSS_DES_192_CBC3_SHA, NULL, 0, SSL_kDHE, SSL_aDSS, SSL_3DES,
 SSL_SHA1, 0, 0, 0, 0, SSL_HIGH | SSL_FIPS},
{0, SSL3_TXT_EDH_RSA_DES_192_CBC3_SHA, NULL, 0, SSL_kDHE, SSL_aRSA, SSL_3DES,
 SSL_SHA1, 0, 0, 0, 0, SSL_HIGH | SSL_FIPS},
...
int ssl_cipher_get_evp(SSL_CTX *ctx, const SSL_SESSION *s,
  ...
  if ((*enc != NULL) &&
      ...
      (c->algorithm_mac == SSL_AEAD || mac_pkey_type == NULL ||
       *mac_pkey_type != NID_undef)) {
    const EVP_CIPHER *evp = NULL;

    if (use_etm || s->ssl_version >> 8 != TLS1_VERSION_MAJOR ||
    ...
    if (c->algorithm_enc == SSL_RC4 && c->algorithm_mac == SSL_MD5)
      ...
      evp =
      ssl_evp_cipher_fetch(ctx->libctx, NID_aes_128_cbc_hmac_sha1, ctx->propq);
    else if (c->algorithm_enc == SSL_AES256 && c->algorithm_mac == SSL_SHA1)
      evp =
      ssl_evp_cipher_fetch(ctx->libctx, NID_aes_256_cbc_hmac_sha1, ctx->propq);
    else if (c->algorithm_enc == SSL_AES128 && c->algorithm_mac == SSL_SHA256)
      evp = ssl_evp_cipher_fetch(ctx->libctx, NID_aes_128_cbc_hmac_sha256,
                                 ctx->propq);
    else if (c->algorithm_enc == SSL_AES256 && c->algorithm_mac == SSL_SHA256)
      evp = ssl_evp_cipher_fetch(ctx->libctx, NID_aes_256_cbc_hmac_sha256,
                                 ctx->propq);
...
ssl_cipher_collect_aliases(const SSL_CIPHER **ca_list, int num_of_group_aliases,
                           uint32_t disabled_mkey, uint32_t disabled_auth,
                           uint32_t disabled_enc, uint32_t disabled_mac,
                           CIPHER_ORDER *head) {
    ...
    if (strength_bits >= 0) {
      ...
      if (trc_out != NULL) {
        BIO_printf(trc_out,
                   ...
                   cp->name, (unsigned int)cp->algorithm_mkey,
                   (unsigned int)cp->algorithm_auth,
                   (unsigned int)cp->algorithm_enc,
                   (unsigned int)cp->algorithm_mac, cp->min_tls,
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      while (((ch >= 'A') && (ch <= 'Z')) || ((ch >= '0') && (ch <= '9')) ||
             ...
             (ch == '.') || (ch == '='))
#else
      while (isalnum((unsigned char)ch) || (ch == '-') || (ch == '_') ||
             (ch == '.') || (ch == '='))
#endif
      {
        ch = *(++l);
      ...
      if (ca_list[j]->algo_strength & SSL_STRONG_MASK) {
        if (algo_strength & SSL_STRONG_MASK) {
          algo_strength &=
          (ca_list[j]->algo_strength & SSL_STRONG_MASK) | ~SSL_STRONG_MASK;
          if (!(algo_strength & SSL_STRONG_MASK)) {
            found = 0;
            break;
          }
      ...
      if (ca_list[j]->algo_strength & SSL_DEFAULT_MASK) {
        if (algo_strength & SSL_DEFAULT_MASK) {
          algo_strength &=
          (ca_list[j]->algo_strength & SSL_DEFAULT_MASK) | ~SSL_DEFAULT_MASK;
          if (!(algo_strength & SSL_DEFAULT_MASK)) {
            found = 0;
            break;
          }
    ...
    if (rule == CIPHER_SPECIAL) { /* special command */
      ...
      ssl_cipher_apply_rule(cipher_id, alg_mkey, alg_auth, alg_enc, alg_mac,
                            min_tls, algo_strength, rule, -1, head_p, tail_p);
    } else {
      while ((*l != '\0') && !ITEM_SEP(*l))
        l++;
    }
...
static int update_cipher_list(SSL_CTX *ctx, STACK_OF(SSL_CIPHER) * *cipher_list,
  ...
  for (i = sk_SSL_CIPHER_num(tls13_ciphersuites) - 1; i >= 0; i--) {
    ...
    if ((sslc->algorithm_enc & ctx->disabled_enc_mask) == 0 &&
        (ssl_cipher_table_mac[sslc->algorithm2 & SSL_HANDSHAKE_MAC_MASK].mask &
         ctx->disabled_mac_mask) == 0) {
...
int SSL_set_ciphersuites(SSL *s, const char *str) {
  ...
  if (ret && sc->cipher_list != NULL)
    return update_cipher_list(s->ctx, &sc->cipher_list, &sc->cipher_list_by_id,
...
ssl_create_cipher_list(SSL_CTX *ctx, STACK_OF(SSL_CIPHER) * tls13_ciphersuites,
  ...
  ssl_cipher_collect_ciphers(ssl_method, num_of_ciphers, disabled_mkey,
                             disabled_auth, disabled_enc, disabled_mac, co_list,
                             &head, &tail);
   ...
   * strength.
   */
  ssl_cipher_apply_rule(0, 0, 0, SSL_AES ^ SSL_AESGCM, 0, 0, 0, CIPHER_ADD, -1,
                        &head, &tail);
  ...
  if (HAS_PREFIX(rule_str, "DEFAULT")) {
    ok = ssl_cipher_process_rulestr(OSSL_default_cipher_list(), &head, &tail,
                                    ca_list, c);
    rule_p += 7;
    if (*rule_p == ':')
      rule_p++;
  ...
  OSSL_TRACE_BEGIN(TLS_CIPHER) { BIO_printf(trc_out, "cipher selection:\n"); }
  /*
...
STACK_OF(SSL_COMP) * SSL_COMP_get_compression_methods(void) {
  ...
  rv =
  (STACK_OF(SSL_COMP) **)OSSL_LIB_CTX_get_data(NULL, OSSL_LIB_CTX_COMP_METHODS);
  if (rv != NULL)
    return *rv;
  else
    return NULL;
...
const SSL_CIPHER *ssl_get_cipher_by_char(SSL_CONNECTION *s,
                                         const unsigned char *ptr, int all) {
  const SSL_CIPHER *c =
  SSL_CONNECTION_GET_SSL(s)->method->get_cipher_by_char(ptr);
