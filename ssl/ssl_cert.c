...
CERT *ssl_cert_dup(CERT *cert) {
  ...
  if (cert->client_sigalgs) {
    ret->client_sigalgs =
    OPENSSL_malloc(cert->client_sigalgslen * sizeof(*cert->client_sigalgs));
...
int ssl_cert_set0_chain(SSL_CONNECTION *s, SSL_CTX *ctx,
                        STACK_OF(X509) * chain) {
  int i, r;
...
int ssl_cert_set1_chain(SSL_CONNECTION *s, SSL_CTX *ctx,
                        STACK_OF(X509) * chain) {
...
static int add_uris_recursive(STACK_OF(X509_NAME) * stack, const char *uri,
  ...
  while (!OSSL_STORE_eof(ctx) && !OSSL_STORE_error(ctx)) {
    ...
    if (infotype == OSSL_STORE_INFO_NAME) {
      ...
      if (depth > 0)
        ok =
        add_uris_recursive(stack, OSSL_STORE_INFO_get0_NAME(info), depth - 1);
