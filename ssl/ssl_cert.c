...
static int
ssl_security_default_callback(const SSL *s, const SSL_CTX *ctx, int op, int bits, int nid, void *other, void *ex);
...
static int
add_file_cert_subjects_to_stack(STACK_OF(X509_NAME) *stack, const char *file, LHASH_OF(X509_NAME) * name_hash)
{
    BIO       *in;
    X509      *x   = NULL;
    X509_NAME *xn  = NULL;
