    ...
    if (_hidden_sha256_md == NULL) {
        ...
        if ((md = EVP_MD_meth_new(NID_sha256, NID_sha256WithRSAEncryption))
            ...
            || !EVP_MD_meth_set_app_datasize(
                md, sizeof(EVP_MD *) + sizeof(SHA256_CTX))
            || !EVP_MD_meth_set_flags(md, EVP_MD_FLAG_DIGALGID_ABSENT)
...
static const EVP_MD *digest_sha384(void)
{
    if (_hidden_sha384_md == NULL) {
        ...
        if ((md = EVP_MD_meth_new(NID_sha384, NID_sha384WithRSAEncryption))
            ...
            || !EVP_MD_meth_set_app_datasize(
                md, sizeof(EVP_MD *) + sizeof(SHA512_CTX))
            || !EVP_MD_meth_set_flags(md, EVP_MD_FLAG_DIGALGID_ABSENT)
...
static const EVP_CIPHER *ossltest_aes_128_gcm(void)
{
    if (_hidden_aes_128_gcm == NULL
        && ((_hidden_aes_128_gcm = EVP_CIPHER_meth_new(
                 NID_aes_128_gcm, 1 /* block size */, 16 /* key len */))
                == NULL
...
static void fill_known_data(unsigned char *md, unsigned int len)
{
    ...
    for (i = 0; i < len; i++) { md[i] = (unsigned char)(i & 0xff); }
}
