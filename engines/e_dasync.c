...
static int dasync_digests(ENGINE *e, const EVP_MD **digest, const int **nids,
                          int nid);
...
static const EVP_MD *dasync_sha1(void) { return _hidden_sha1_md; }
...
/* RSA */
static int dasync_pkey(ENGINE *e, EVP_PKEY_METHOD **pmeth, const int **pnids,
                       int nid);
 ...
 * which is implemented only if the AES-NI instruction set extension is
 * available (see OPENSSL_IA32CAP(3)). If that's not the case, then this cipher
...
static int dasync_cipher_nids[] = {NID_aes_128_cbc, NID_aes_256_ctr,
...
static int bind_dasync(ENGINE *e) {
  ...
  if ((dasync_rsa_orig = EVP_PKEY_meth_find(EVP_PKEY_RSA)) == NULL ||
      (dasync_rsa =
       EVP_PKEY_meth_new(EVP_PKEY_RSA, EVP_PKEY_FLAG_AUTOARGLEN)) == NULL)
    return 0;
  ...
  _hidden_aes_128_cbc =
  EVP_CIPHER_meth_new(NID_aes_128_cbc, 16 /* block size */, 16 /* key len */);
  if (_hidden_aes_128_cbc == NULL ||
      !EVP_CIPHER_meth_set_iv_length(_hidden_aes_128_cbc, 16) ||
      !EVP_CIPHER_meth_set_flags(
      _hidden_aes_128_cbc, EVP_CIPH_FLAG_DEFAULT_ASN1 | EVP_CIPH_CBC_MODE |
  ...
  _hidden_aes_256_ctr =
  EVP_CIPHER_meth_new(NID_aes_256_ctr, 1 /* block size */, 32 /* key len */);
  if (_hidden_aes_256_ctr == NULL ||
      !EVP_CIPHER_meth_set_iv_length(_hidden_aes_256_ctr, 16) ||
      !EVP_CIPHER_meth_set_flags(
      _hidden_aes_256_ctr, EVP_CIPH_FLAG_DEFAULT_ASN1 | EVP_CIPH_CTR_MODE |
                           EVP_CIPH_FLAG_PIPELINE | EVP_CIPH_CUSTOM_COPY) ||
  ...
  NID_aes_128_cbc_hmac_sha1, 16 /* block size */, 16 /* key len */);
  if (_hidden_aes_128_cbc_hmac_sha1 == NULL ||
      !EVP_CIPHER_meth_set_iv_length(_hidden_aes_128_cbc_hmac_sha1, 16) ||
      !EVP_CIPHER_meth_set_flags(
      _hidden_aes_128_cbc_hmac_sha1,
      EVP_CIPH_CBC_MODE | EVP_CIPH_FLAG_DEFAULT_ASN1 |
      EVP_CIPH_FLAG_AEAD_CIPHER | EVP_CIPH_FLAG_PIPELINE |
...
static int dasync_init(ENGINE *e) { return 1; }

static int dasync_finish(ENGINE *e) { return 1; }
...
static void dummy_pause_job(void) {
  ...
  if (ASYNC_WAIT_CTX_get_callback(waitctx, &callback, &callback_arg) &&
      callback != NULL) {
    /*
     * In the Dummy async engine we are cheating. We call the callback that the
     * job is complete before the call to ASYNC_pause_job(). A real async engine
     * would only call the callback when the job was actually complete
     */
...
static int dasync_cipher_ctrl_helper(EVP_CIPHER_CTX *ctx, int type, int arg,
  ...
  case EVP_CTRL_AEAD_SET_MAC_KEY:
    ...
    ret =
    EVP_CIPHER_meth_get_ctrl(EVP_aes_128_cbc_hmac_sha1())(ctx, type, arg, ptr);
  ...
  case EVP_CTRL_AEAD_TLS1_AAD: {
    ...
    if (EVP_CIPHER_CTX_is_encrypting(ctx)) {
      ...
      return ((len + SHA_DIGEST_LENGTH + AES_BLOCK_SIZE) & -AES_BLOCK_SIZE) -
             len;
    } else {
...
static int dasync_cipher_init_key_helper(EVP_CIPHER_CTX *ctx,
  ...
  if (pipe_ctx->inner_cipher_data == NULL &&
    ...
    OPENSSL_zalloc(EVP_CIPHER_impl_ctx_size(cipher));
    if (pipe_ctx->inner_cipher_data == NULL)
      return 0;
  }
...
static int dasync_cipher_helper(EVP_CIPHER_CTX *ctx, unsigned char *out,
  ...
  if (pipes == 0) {
    if (pipe_ctx->aadctr != 0) {
      ...
      ctx, EVP_CTRL_AEAD_TLS1_AAD, EVP_AEAD_TLS1_AAD_LEN, pipe_ctx->tlsaad[0]);
    }
    ...
    for (i = 0; i < pipes; i++) {
      ...
      ret && EVP_CIPHER_meth_get_do_cipher(cipher)(
             ctx, pipe_ctx->outbufs[i], pipe_ctx->inbufs[i], pipe_ctx->lens[i]);
    }
    pipe_ctx->numpipes = 0;
  }
  pipe_ctx->aadctr = 0;
...
static int dasync_rsa_decrypt(EVP_PKEY_CTX *ctx, unsigned char *out,
                              size_t *outlen, const unsigned char *in,
                              size_t inlen) {
  static int (*pdecrypt)(EVP_PKEY_CTX *ctx, unsigned char *out, size_t *outlen,
