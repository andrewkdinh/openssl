...
static int bind_dasync(ENGINE *e) {
    ...
    if (_hidden_aes_128_cbc == NULL ||
        ...
        _hidden_aes_128_cbc, sizeof(struct dasync_pipeline_ctx))) {
        EVP_CIPHER_meth_free(_hidden_aes_128_cbc);
        _hidden_aes_128_cbc = NULL;
    }
    ...
    if (_hidden_aes_256_ctr == NULL ||
        ...
        !EVP_CIPHER_meth_set_impl_ctx_size(
        _hidden_aes_256_ctr, sizeof(struct dasync_pipeline_ctx))) {
        EVP_CIPHER_meth_free(_hidden_aes_256_ctr);
        _hidden_aes_256_ctr = NULL;
    }
    ...
    if (_hidden_aes_128_cbc_hmac_sha1 == NULL ||
        ...
        !EVP_CIPHER_meth_set_impl_ctx_size(
        _hidden_aes_128_cbc_hmac_sha1, sizeof(struct dasync_pipeline_ctx))) {
        EVP_CIPHER_meth_free(_hidden_aes_128_cbc_hmac_sha1);
        _hidden_aes_128_cbc_hmac_sha1 = NULL;
    }
...
static void dummy_pause_job(void) {
    ...
    if (ASYNC_WAIT_CTX_get_callback(waitctx, &callback, &callback_arg) &&
        callback != NULL) {
        /*
         * In the Dummy async engine we are cheating. We call the callback that
         * the job is complete before the call to ASYNC_pause_job(). A real
         * async engine would only call the callback when the job was actually
         * complete
         */
...
static int dasync_cipher_ctrl_helper(EVP_CIPHER_CTX *ctx, int type, int arg,
    ...
    case EVP_CTRL_AEAD_SET_MAC_KEY:
        ...
        ret = EVP_CIPHER_meth_get_ctrl(EVP_aes_128_cbc_hmac_sha1())(ctx, type,
                                                                    arg, ptr);
