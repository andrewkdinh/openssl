...
{CAPI_CMD_SET_CSP_NAME, "csp_name",
 "Set CSP name, (default CSP used if not specified)", ENGINE_CMD_FLAG_STRING},
...
{CAPI_CMD_LIST_OPTIONS, "list_options",
 "Set list options (1=summary,2=friendly name, 4=full printout, 8=PEM output, "
 "16=XXX, "
 "32=private key info)",
 ENGINE_CMD_FLAG_NUMERIC},
{CAPI_CMD_LOOKUP_METHOD, "lookup_method",
 ...
 ENGINE_CMD_FLAG_NUMERIC},
{CAPI_CMD_STORE_NAME, "store_name", "certificate store name, default \"MY\"",
 ENGINE_CMD_FLAG_STRING},
...
static int capi_init(ENGINE *e) {
...
#ifdef OPENSSL_CAPIENG_DIALOG
  {
    ...
    if (cryptui)
      ctx->certselectdlg =
      (CERTDLG)GetProcAddress(cryptui, "CryptUIDlgSelectCertificateFromStore");
    if (kernel)
      ctx->getconswindow =
...
static void capi_addlasterror(void) { capi_adderror(GetLastError()); }
...
static CRYPT_KEY_PROV_INFO *capi_get_prov_info(CAPI_CTX *ctx,
  ...
  if (!CertGetCertificateContextProperty(cert, CERT_KEY_PROV_INFO_PROP_ID, NULL,
                                         &len))
    return NULL;
...
static HCERTSTORE capi_open_store(CAPI_CTX *ctx, char *storename) {
  ...
  hstore =
  CertOpenStore(CERT_STORE_PROV_SYSTEM_A, 0, 0, ctx->store_flags, storename);
  if (!hstore) {
...
static int capi_ctx_set_provname(CAPI_CTX *ctx, LPSTR pname, DWORD type,
  ...
  if (check) {
    ...
    if (name == NULL ||
        !CryptAcquireContextW(&hprov, NULL, name, type, CRYPT_VERIFYCONTEXT)) {
...
static int cert_select_dialog(ENGINE *e, SSL *ssl, STACK_OF(X509) * certs) {
  ...
  cert =
  ctx->certselectdlg(dstore, hwnd, dlg_title, dlg_prompt, dlg_columns, 0, NULL);
