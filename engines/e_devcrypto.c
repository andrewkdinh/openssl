 ...
 */
static int cfd = -1;
#define DEVCRYPTO_REQUIRE_ACCELERATED                                          \
  0                              /* require confirmation of acceleration */
#define DEVCRYPTO_USE_SOFTWARE 1 /* allow software drivers */
#define DEVCRYPTO_REJECT_SOFTWARE                                              \
  2 /* only disallow confirmed software drivers */
...
static int ctr_do_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out,
  ...
  if (inl) {
    ...
    while (inl--) {
      out[cipher_ctx->num] =
      in[cipher_ctx->num] ^ cipher_ctx->partial[cipher_ctx->num];
      cipher_ctx->num++;
    }
  }
...
static int cipher_ctrl(EVP_CIPHER_CTX *ctx, int type, int p1, void *p2) {
  ...
  case EVP_CTRL_COPY:
    ...
    return cipher_init(to_ctx, (void *)cipher_ctx->sess.key,
                       EVP_CIPHER_CTX_iv(ctx), (cipher_ctx->op == COP_ENCRYPT));
...
static int devcrypto_test_cipher(size_t cipher_data_index) {
  return (
  cipher_driver_info[cipher_data_index].status == DEVCRYPTO_STATUS_USABLE &&
...
static void prepare_cipher_methods(void) {
  ...
  for (i = 0, known_cipher_nids_amount = 0; i < OSSL_NELEM(cipher_data); i++) {
    ...
    if ((known_cipher_methods[i] = EVP_CIPHER_meth_new(
         cipher_data[i].nid,
        ...
        !EVP_CIPHER_meth_set_flags(known_cipher_methods[i],
                                   cipher_data[i].flags | EVP_CIPH_CUSTOM_COPY |
                                   EVP_CIPH_CTRL_INIT |
                                   EVP_CIPH_FLAG_DEFAULT_ASN1) ||
        !EVP_CIPHER_meth_set_init(known_cipher_methods[i], cipher_init) ||
        !EVP_CIPHER_meth_set_do_cipher(
        known_cipher_methods[i],
      ...
      if (ioctl(cfd, CIOCGSESSINFO, &siop) < 0) {
        ...
        cipher_driver_info[i].driver_name = OPENSSL_strndup(
        siop.cipher_info.cra_driver_name, CRYPTODEV_MAX_ALG_NAME);
...
static int devcrypto_test_digest(size_t digest_data_index) {
  return (
  digest_driver_info[digest_data_index].status == DEVCRYPTO_STATUS_USABLE &&
...
static void prepare_digest_methods(void) {
  ...
  for (i = 0, known_digest_nids_amount = 0; i < OSSL_NELEM(digest_data); i++) {
    ...
    if (ioctl(cfd, CIOCGSESSINFO, &siop) < 0) {
      ...
      OPENSSL_strndup(siop.hash_info.cra_driver_name, CRYPTODEV_MAX_ALG_NAME);
      if (siop.flags & SIOP_FLAG_KERNEL_DRIVER_ONLY)
    ...
    if ((known_digest_methods[i] =
         EVP_MD_meth_new(digest_data[i].nid, NID_undef)) == NULL ||
...
static int devcrypto_digests(ENGINE *e, const EVP_MD **digest, const int **nids,
                             int nid) {
  if (digest == NULL)
...
static void dump_digest_info(void) {
  ...
  for (i = 0; i < OSSL_NELEM(digest_data); i++) {
    ...
    fprintf(
    stderr, "Digest %s, NID=%d, /dev/crypto info: id=%d, driver=%s",
    name ? name : "unknown", digest_data[i].nid, digest_data[i].devcryptoid,
    digest_driver_info[i].driver_name ? digest_driver_info[i].driver_name
                                      : "unknown");
...
{DEVCRYPTO_CMD_USE_SOFTDRIVERS, "USE_SOFTDRIVERS",
 "specifies whether to use software (not accelerated) drivers (" OPENSSL_MSTR(DEVCRYPTO_REQUIRE_ACCELERATED) "=use only accelerated drivers, " OPENSSL_MSTR(DEVCRYPTO_USE_SOFTWARE) "=allow all drivers, " OPENSSL_MSTR(
 DEVCRYPTO_REJECT_SOFTWARE) "=use if acceleration can't be determined) "
                            "[default=" OPENSSL_MSTR(DEVCRYPTO_DEFAULT_USE_SOFTDRIVERS) "]",
 ENGINE_CMD_FLAG_NUMERIC},
#endif

{DEVCRYPTO_CMD_CIPHERS, "CIPHERS",
 "either ALL, NONE, or a comma-separated list of ciphers to enable "
 "[default=ALL]",
 ENGINE_CMD_FLAG_STRING},

#ifdef IMPLEMENT_DIGEST
{DEVCRYPTO_CMD_DIGESTS, "DIGESTS",
 "either ALL, NONE, or a comma-separated list of digests to enable "
 "[default=ALL]",
 ENGINE_CMD_FLAG_STRING},
#endif

{DEVCRYPTO_CMD_DUMP_INFO, "DUMP_INFO",
 "dump info about each algorithm to stderr; use 'openssl engine -pre DUMP_INFO "
 "devcrypto'",
 ENGINE_CMD_FLAG_NO_INPUT},

{0, NULL, NULL, 0}};

static int devcrypto_ctrl(ENGINE *e, int cmd, long i, void *p,
                          void (*f)(void)) {
  int *new_list;
  switch (cmd) {
#if defined(CIOCGSESSINFO) || defined(CIOCGSESSION2)
