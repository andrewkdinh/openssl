...
static void prepare_cipher_methods(void)
{
    ...
    for (i = 0, known_cipher_nids_amount = 0; i < OSSL_NELEM(cipher_data); i++) {
        ...
        if ((known_cipher_methods[i] = EVP_CIPHER_meth_new(
                 cipher_data[i].nid, cipher_mode == EVP_CIPH_CTR_MODE ? 1 : cipher_data[i].blocksize,
...
static void dump_cipher_info(void)
{
    ...
    for (i = 0; i < OSSL_NELEM(cipher_data); i++) {
        ...
        fprintf(stderr, "Cipher %s, NID=%d, /dev/crypto info: id=%d, ", name ? name : "unknown", cipher_data[i].nid,
...
# endif
    ...
    for (i = 0; i < OSSL_NELEM(digest_data); i++) {
        ...
        fprintf(stderr, "Digest %s, NID=%d, /dev/crypto info: id=%d, driver=%s", name ? name : "unknown",
                digest_data[i].nid, digest_data[i].devcryptoid,
...
static const ENGINE_CMD_DEFN devcrypto_cmds[] = {
    ...
    {DEVCRYPTO_CMD_CIPHERS, "CIPHERS", "either ALL, NONE, or a comma-separated list of ciphers to enable [default=ALL]",
     ENGINE_CMD_FLAG_STRING},

#ifdef IMPLEMENT_DIGEST
    {DEVCRYPTO_CMD_DIGESTS, "DIGESTS", "either ALL, NONE, or a comma-separated list of digests to enable [default=ALL]",
     ENGINE_CMD_FLAG_STRING},
#endif
