...
static void prepare_cipher_methods(void) {
    ...
    for (i = 0, known_cipher_nids_amount = 0; i < OSSL_NELEM(cipher_data);
        ...
        if ((known_cipher_methods[i] = EVP_CIPHER_meth_new(
            ...
            known_cipher_methods[i], cipher_data[i].flags |
                                     EVP_CIPH_CUSTOM_COPY | EVP_CIPH_CTRL_INIT |
...
static void prepare_digest_methods(void) {
    ...
    for (i = 0, known_digest_nids_amount = 0; i < OSSL_NELEM(digest_data);
        ...
        if (ioctl(cfd, CIOCGSESSINFO, &siop) < 0) {
            ...
            digest_driver_info[i].driver_name = OPENSSL_strndup(
            siop.hash_info.cra_driver_name, CRYPTODEV_MAX_ALG_NAME);
            if (siop.flags & SIOP_FLAG_KERNEL_DRIVER_ONLY)
...
{DEVCRYPTO_CMD_USE_SOFTDRIVERS, "USE_SOFTDRIVERS",
 "specifies whether to use software (not accelerated) drivers (" OPENSSL_MSTR(DEVCRYPTO_REQUIRE_ACCELERATED) "=use only accelerated drivers, " OPENSSL_MSTR(
 DEVCRYPTO_USE_SOFTWARE) "=allow all drivers, " OPENSSL_MSTR(DEVCRYPTO_REJECT_SOFTWARE) "=use if acceleration can't be determined) "
                                                                                        "[default=" OPENSSL_MSTR(
