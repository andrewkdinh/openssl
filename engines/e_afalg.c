...
static cbc_handles cbc_handle[] = {
{AES_KEY_SIZE_128, NULL}, {AES_KEY_SIZE_192, NULL}, {AES_KEY_SIZE_256, NULL}};

static ossl_inline int io_setup(unsigned n, aio_context_t *ctx) {
  return syscall(__NR_io_setup, n, ctx);
}

static ossl_inline int eventfd(int n) { return syscall(__NR_eventfd2, n, 0); }
...
static ossl_inline int io_getevents(aio_context_t ctx, long min, long max,
  ...
  if (sizeof(*timeout) != sizeof(struct __timespec32)) {
    int ret =
    syscall(__NR_io_pgetevents_time64, ctx, min, max, events, timeout, NULL);
  ...
  if ((job = ASYNC_get_current_job()) != NULL) {
    ...
    if (ret == 0) {
      ...
      if (fcntl(aio->efd, F_SETFL, O_NONBLOCK) != 0) {
        ALG_WARN("%s(%d): Failed to set event fd as NONBLOCKING", __FILE__,
                 __LINE__);
      }
    }
    aio->mode = MODE_ASYNC;
      ...
      if (r > 0) {
        ...
        if (events[0].res < 0) {
          ...
          if (events[0].res == -EBUSY && retry++ < 3) {
            r = io_read(aio->aio_ctx, 1, &cb);
            if (r < 0) {
              ALG_PERR("%s(%d): retry %d for io_read failed : ", __FILE__,
                       __LINE__, retry);
              return 0;
            }
            continue;
...
static int afalg_do_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out,
  ...
  ret =
  afalg_start_cipher_sk(actx, (unsigned char *)in, inl, EVP_CIPHER_CTX_iv(ctx),
...
static int afalg_init(ENGINE *e) { return 1; }

static int afalg_finish(ENGINE *e) { return 1; }
