...
/* This file has quite some overlap with
...
static char *file_get_pass(const UI_METHOD *ui_method, char *pass,
  ...
  if ((prompt = UI_construct_prompt(ui, desc, info)) == NULL) {
    ATTICerr(0, ERR_R_UI_LIB);
    pass = NULL;
  } else if (UI_add_input_string(ui, prompt, UI_INPUT_FLAG_DEFAULT_PWD, pass, 0,
                                 maxsize - 1) <= 0) {
    ATTICerr(0, ERR_R_UI_LIB);
    pass = NULL;
  } else {
...
static int file_get_pem_pass(char *buf, int num, int w, void *data) {
  ...
  char *pass =
  file_get_pass(pass_data->ui_method, buf, num, pass_data->prompt_desc,
...
try_decode_PKCS12(const char *pem_name, const char *pem_header,
                  const unsigned char *blob, size_t len, void **pctx,
                  int *matchcount, const UI_METHOD *ui_method, void *ui_data,
  ...
  if (ctx == NULL) {
    ...
    if ((p12 = d2i_PKCS12(NULL, &blob, len)) != NULL) {
      ...
      *matchcount = 1;
      ...
      if (!PKCS12_mac_present(p12) || PKCS12_verify_mac(p12, "", 0) ||
          PKCS12_verify_mac(p12, NULL, 0)) {
        pass = "";
      } else {
        if ((pass = file_get_pass(ui_method, tpass, PEM_BUFSIZE,
                                  "PKCS12 import", uri, ui_data)) == NULL) {
...
void *ui_data, const char *uri, OSSL_LIB_CTX *libctx, const char *propq) {
  ...
  *matchcount = 1;
  ...
  if ((pass = file_get_pass(ui_method, kbuf, PEM_BUFSIZE,
                            "PKCS8 decrypt pass phrase", uri, ui_data)) ==
      NULL) {
...
void *ui_data, const char *uri, OSSL_LIB_CTX *libctx, const char *propq) {
  ...
  if (pem_name != NULL) {
      ...
      if (asn1meths != NULL) {
        ...
        for (i = 0; i < nids_n; i++) {
          ...
          if (!asn1meths(curengine, &ameth2, NULL, nids[i]) ||
              !EVP_PKEY_asn1_get0_info(&pkey_id, NULL, &pkey_flags, NULL, NULL,
                                       ameth2) ||
          ...
          tmp_pkey =
          d2i_PrivateKey_ex(pkey_id, NULL, &tmp_blob, len, libctx, propq);
          if (tmp_pkey != NULL) {
            if (pkey != NULL)
    ...
    for (i = 0; i < EVP_PKEY_asn1_get_count(); i++) {
      ...
      if (!EVP_PKEY_asn1_get0_info(&pkey_id, NULL, &pkey_flags, NULL, NULL,
                                   ameth) ||
          (pkey_flags & ASN1_PKEY_ALIAS) != 0)
        continue;
 ...
 */
static OSSL_STORE_INFO *
try_decode_PUBKEY(const char *pem_name, const char *pem_header,
                  const unsigned char *blob, size_t len, void **pctx,
                  int *matchcount, const UI_METHOD *ui_method, void *ui_data,
                  const char *uri, OSSL_LIB_CTX *libctx, const char *propq) {
  OSSL_STORE_INFO *store_info = NULL;
 ...
 */
static OSSL_STORE_INFO *
try_decode_params(const char *pem_name, const char *pem_header,
                  const unsigned char *blob, size_t len, void **pctx,
                  int *matchcount, const UI_METHOD *ui_method, void *ui_data,
                  const char *uri, OSSL_LIB_CTX *libctx, const char *propq) {
  OSSL_STORE_INFO *store_info = NULL;
 ...
 */
static OSSL_STORE_INFO *try_decode_X509Certificate(
const char *pem_name, const char *pem_header, const unsigned char *blob,
size_t len, void **pctx, int *matchcount, const UI_METHOD *ui_method,
void *ui_data, const char *uri, OSSL_LIB_CTX *libctx, const char *propq) {
  OSSL_STORE_INFO *store_info = NULL;
...
file_open_ex(const OSSL_STORE_LOADER *loader, const char *uri,
...
static OSSL_STORE_LOADER_CTX *
file_attach(const OSSL_STORE_LOADER *loader, BIO *bp, OSSL_LIB_CTX *libctx,
...
static int file_find(OSSL_STORE_LOADER_CTX *ctx,
  ...
  if (OSSL_STORE_SEARCH_get_type(search) == OSSL_STORE_SEARCH_BY_NAME) {
    ...
    hash =
    X509_NAME_hash_ex(OSSL_STORE_SEARCH_get0_name(search), NULL, NULL, NULL);
...
static int file_read_pem(BIO *bp, char **pem_name, char **pem_header,
  ...
  if (strlen(*pem_header) > 10) {
    ...
    if (!PEM_get_EVP_CIPHER_INFO(*pem_header, &cipher) ||
        !file_fill_pem_pass_data(&pass_data, "PEM pass phrase", uri, ui_method,
                                 ui_data) ||
...
static OSSL_STORE_INFO *file_try_read_msblob(BIO *bp, int *matchcount) {
...

  {
    ...
    if (BIO_buffer_peek(bp, peekbuf, sizeof(peekbuf)) <= 0)
      return 0;
    if (ossl_do_blob_header(&p, sizeof(peekbuf), &magic, &bitlen, &isdss,
                            &ispub) <= 0)
      return 0;
  }
...
static OSSL_STORE_INFO *file_try_read_PVK(BIO *bp, const UI_METHOD *ui_method,
...

  {
    ...
    if (!file_fill_pem_pass_data(&pass_data, "PVK pass phrase", uri, ui_method,
                                 ui_data) ||
...
static int file_name_to_uri(OSSL_STORE_LOADER_CTX *ctx, const char *name,
  ...
  assert(data != NULL);
  {
    ...
    strlen(ctx->uri) + strlen(pathsep) + strlen(name) + 1 /* \0 */;
...
static OSSL_STORE_INFO *file_load(OSSL_STORE_LOADER_CTX *ctx,
  ...
  again:
    ...
    do {
      ...
      matchcount = -1;
      if (ctx->type == is_pem) {
        if (!file_read_pem(ctx->_.file.file, &pem_name, &pem_header, &data,
                           &len, ui_method, ui_data, ctx->uri,
                           (ctx->flags & FILE_FLAG_SECMEM) != 0)) {
          ctx->errcnt++;
          goto endloop;
        }
      } else {
        if ((result = file_try_read_msblob(ctx->_.file.file, &matchcount)) !=
            NULL ||
            (result = file_try_read_PVK(ctx->_.file.file, ui_method, ui_data,
                                        ctx->uri, &matchcount)) != NULL)
          goto endloop;
...
static int file_error(OSSL_STORE_LOADER_CTX *ctx) { return ctx->errcnt > 0; }
...
static int loader_attic_init(ENGINE *e) { return 1; }

static int loader_attic_finish(ENGINE *e) { return 1; }
