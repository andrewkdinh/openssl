        ...
        if (strcmp(pem_name, PEM_STRING_PKCS8INF) == 0) {
            ...
            *matchcount = 1;
            ...
            if ((slen = check_suffix(pem_name, "PRIVATE KEY")) > 0 &&
                (ameth = EVP_PKEY_asn1_find_str(NULL, pem_name, slen)) !=
                NULL &&
                EVP_PKEY_asn1_get0_info(&pkey_id, NULL, NULL, NULL, NULL,
                                        ameth)) {
                *matchcount = 1;
                pkey =
                d2i_PrivateKey_ex(pkey_id, NULL, &blob, len, libctx, propq);
            }
        }
    } else {
        int i;
            ...
            if (asn1meths != NULL) {
                ...
                for (i = 0; i < nids_n; i++) {
                    ...
                    if (!asn1meths(curengine, &ameth2, NULL, nids[i]) ||
                        !EVP_PKEY_asn1_get0_info(&pkey_id, NULL, &pkey_flags,
                                                 NULL, NULL, ameth2) ||
...
file_open_ex(const OSSL_STORE_LOADER *loader, const char *uri,
    ...
    for (i = 0, path = NULL; path == NULL && i < path_data_n; i++) {
        if (stat(path_data[i], &st) < 0) {
            ERR_raise_data(ERR_LIB_SYS, errno, "calling stat(%s)",
                           path_data[i]);
        } else {
            path = path_data[i];
        }
...
static int file_find(OSSL_STORE_LOADER_CTX *ctx,
    ...
    if (OSSL_STORE_SEARCH_get_type(search) == OSSL_STORE_SEARCH_BY_NAME) {
        ...
        hash = X509_NAME_hash_ex(OSSL_STORE_SEARCH_get0_name(search), NULL,
                                 NULL, NULL);
...
again: {
    ...
    *matchcount = 0;
    ...
    if (result != NULL && *matchcount == 1 &&
        matching_handlers[0]->repeatable) {
...
static OSSL_STORE_INFO *file_try_read_PVK(BIO *bp, const UI_METHOD *ui_method,
...

    {
        ...
        if (BIO_buffer_peek(bp, peekbuf, sizeof(peekbuf)) <= 0)
            return 0;
        if (!ossl_do_PVK_header(&p, sizeof(peekbuf), 0, &isdss, &saltlen,
                                &keylen))
            return 0;
    }
        ...
        do {
            ...
            if (ctx->type == is_pem) {
                ...
                if ((result = file_try_read_msblob(ctx->_.file.file,
                                                   &matchcount)) != NULL ||
                    (result =
                     file_try_read_PVK(ctx->_.file.file, ui_method, ui_data,
