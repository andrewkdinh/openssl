...
static char *
file_get_pass(const UI_METHOD *ui_method, char *pass, size_t maxsize, const char *desc, const char *info, void *data)
{
    UI   *ui     = UI_new();
    char *prompt = NULL;
    ...
    if ((prompt = UI_construct_prompt(ui, desc, info)) == NULL) {
        ATTICerr(0, ERR_R_UI_LIB);
        pass = NULL;
    } else if (UI_add_input_string(ui, prompt, UI_INPUT_FLAG_DEFAULT_PWD, pass, 0, (int)(maxsize - 1)) <= 0) {
        ATTICerr(0, ERR_R_UI_LIB);
        pass = NULL;
    } else {
                                                  ...
                                                  const char          *propq)
{
    ...
    *matchcount = 1;
    ...
    if (!PKCS12_pbe_crypt(dalg, pass, (int)strlen(pass), doct->data, doct->length, &new_data, &new_data_len, 0))
        goto nop8;
                                              ...
                                              const char          *propq)
{
    ...
    if (pem_name != NULL) {
        if (strcmp(pem_name, PEM_STRING_PKCS8INF) == 0) {
            PKCS8_PRIV_KEY_INFO *p8inf = d2i_PKCS8_PRIV_KEY_INFO(NULL, &blob, (long)len);
            ...
            *matchcount                = 1;
            ...
            if ((slen = check_suffix(pem_name, "PRIVATE KEY")) > 0
                ...
                *matchcount = 1;
                pkey        = d2i_PrivateKey_ex(pkey_id, NULL, &blob, (long)len, libctx, propq);
            }
        }
    } else {
        int i;
        ...
        while (curengine != NULL) {
            ...
            if (asn1meths != NULL) {
                ...
                for (i = 0; i < nids_n; i++) {
                    ...
                    tmp_pkey = d2i_PrivateKey_ex(pkey_id, NULL, &tmp_blob, (long)len, libctx, propq);
                    if (tmp_pkey != NULL) {
                        if (pkey != NULL)
...
static OSSL_STORE_LOADER_CTX *file_open_ex(const OSSL_STORE_LOADER *loader,
                                           const char              *uri,
                                           OSSL_LIB_CTX            *libctx,
                                           const char              *propq,
                                           const UI_METHOD         *ui_method,
...
file_open(const OSSL_STORE_LOADER *loader, const char *uri, const UI_METHOD *ui_method, void *ui_data)
{
...
again: {
    ...
    *matchcount = 0;
    for (i = 0; i < OSSL_NELEM(file_handlers); i++) {
        ...
        ERR_set_mark();
        tmp_result = handler->try_decode(pem_name,
                                         pem_header,
                                         data,
                                         len,
                                         &tmp_handler_ctx,
...
static OSSL_STORE_INFO *file_load_try_repeat(OSSL_STORE_LOADER_CTX *ctx, const UI_METHOD *ui_method, void *ui_data)
{
    ...
    if (ctx->_.file.last_handler != NULL) {
        result = ctx->_.file.last_handler->try_decode(NULL,
                                                      NULL,
                                                      NULL,
                                                      0,
...
static int file_read_pem(BIO             *bp,
                         char           **pem_name,
                         char           **pem_header,
                         unsigned char  **data,
                         long            *len,
...
file_try_read_PVK(BIO *bp, const UI_METHOD *ui_method, void *ui_data, const char *uri, int *matchcount)
{
    OSSL_STORE_INFO *result = NULL;
...
static int file_name_to_uri(OSSL_STORE_LOADER_CTX *ctx, const char *name, char **data)
{
    ...
    assert(data != NULL);
    {
        ...
        long        calculated_length = (long)(strlen(ctx->uri) + strlen(pathsep) + strlen(name) + 1 /* \0 */);
...
again:
        ...
        do {
            ...
            matchcount                = -1;
            if (ctx->type == is_pem) {
                if (!file_read_pem(ctx->_.file.file,
                                   &pem_name,
                                   &pem_header,
                                   &data,
                                   &len,
