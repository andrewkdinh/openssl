...
const RSA_METHOD *RSA_null_method(void) { return NULL; }
...
static int rsa_ossl_public_encrypt(int flen, const unsigned char *from,
    ...
    case RSA_PKCS1_PADDING:
        i =
        ossl_rsa_padding_add_PKCS1_type_2_ex(rsa->libctx, buf, num, from, flen);
        break;
    case RSA_PKCS1_OAEP_PADDING:
        i = ossl_rsa_padding_add_PKCS1_OAEP_mgf1_ex(rsa->libctx, buf, num, from,
...
static int rsa_ossl_private_decrypt(int flen, const unsigned char *from,
    ...
    case RSA_PKCS1_NO_IMPLICIT_REJECT_PADDING:
        ...
        break;
    case RSA_PKCS1_PADDING:
        r = ossl_rsa_padding_check_PKCS1_type_2(rsa->libctx, to, num, buf, j,
                                                num, kdk);
        break;
    case RSA_PKCS1_OAEP_PADDING:
    ...
    if (smooth) {
        ...
        if (/* m1 = I moq q */
            ...
            || !BN_mod_exp_mont_consttime_x2(
               m1, m1, rsa->dmq1, rsa->q, rsa->_method_mod_q, r1, r1, rsa->dmp1,
               rsa->p, rsa->_method_mod_p, ctx)
            /* r1 = (r1 - m1) mod p */
            /*
...
static int rsa_ossl_s390x_mod_exp(BIGNUM *r0, const BIGNUM *i, RSA *rsa,
                                  BN_CTX *ctx) {
    if (rsa->version != RSA_ASN1_VERSION_MULTI) {
        if (s390x_crt(r0, i, rsa->p, rsa->q, rsa->dmp1, rsa->dmq1, rsa->iqmp) ==
            1)
            return 1;
    }
