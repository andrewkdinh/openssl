 ...
 *
 * The RSA modulus size n (which for X9.31 is 1024 + 256*s) is the size of the
 * data structure, which determines the padding size. i.e. len(padding) = n -
 * len(header) - len(hash) - len(trailer)
 *
 * Params:
...
int RSA_padding_add_X931(unsigned char *to, int tlen, const unsigned char *from,
     ...
     * And 2 trailer bytes (but we subtract 1 since flen includes 1 trailer
     * byte)
     */
    j = tlen - flen - 2;
