...
int RSA_padding_check_PKCS1_type_2(unsigned char *to, int tlen,
     ...
     * the left. Then if |good| move |mlen| bytes from
     * |em|+RSA_PKCS1_PADDING_SIZE to |to|. Otherwise leave |to| unchanged. Copy
     * the memory back in a way that does not reveal the size of the data being
     * copied via a timing side channel. This requires copying parts of the
     * buffer multiple times based on the bits set in the real length. Clear
     * bits do a non-copy with identical access pattern. The loop below has
     * overall complexity of O(N*log(N)).
     */
    tlen = constant_time_select_int(
    constant_time_lt(num - RSA_PKCS1_PADDING_SIZE, tlen),
...
#define MAX_LEN_GEN_TRIES 128
    ...
    for (i = 0; i < MAX_LEN_GEN_TRIES * (int)sizeof(len_candidate);
        ...
        constant_time_lt(len_candidate, max_sep_offset), len_candidate,
        synthetic_length);
    }
...
int ossl_rsa_padding_check_PKCS1_type_2_TLS(OSSL_LIB_CTX *libctx,
    ...
    if (alt_version > 0) {
        ...
        from[flen - SSL_MAX_MASTER_KEY_LENGTH], (alt_version >> 8) & 0xff);
        workaround_good &= constant_time_eq(
    ...
    for (i = 0; i < SSL_MAX_MASTER_KEY_LENGTH; i++) {
        to[i] =
        constant_time_select_8(good, from[flen - SSL_MAX_MASTER_KEY_LENGTH + i],
