...
int RSA_padding_check_PKCS1_type_2(unsigned char *to, int tlen,
  ...
  /*
   * Move the result in-place by |num|-RSA_PKCS1_PADDING_SIZE-|mlen| bytes to
   * the left. Then if |good| move |mlen| bytes from |em|+RSA_PKCS1_PADDING_SIZE
   * to |to|. Otherwise leave |to| unchanged. Copy the memory back in a way that
   * does not reveal the size of the data being copied via a timing side
   * channel. This requires copying parts of the buffer multiple times based on
   * the bits set in the real length. Clear bits do a non-copy with identical
   * access pattern. The loop below has overall complexity of O(N*log(N)).
   */
  tlen =
  constant_time_select_int(constant_time_lt(num - RSA_PKCS1_PADDING_SIZE, tlen),
                           num - RSA_PKCS1_PADDING_SIZE, tlen);
  for (msg_index = 1; msg_index < num - RSA_PKCS1_PADDING_SIZE;
       msg_index <<= 1) {
    mask =
    ~constant_time_eq(msg_index & (num - RSA_PKCS1_PADDING_SIZE - mlen), 0);
...
static int ossl_rsa_prf(OSSL_LIB_CTX *ctx, unsigned char *to, int tlen,
                        const char *label, int llen, const unsigned char *kdk,
                        uint16_t bitlen) {
  int pos;
  int ret = -1;
  uint16_t iter = 0;
...
#define MAX_LEN_GEN_TRIES 128
  ...
  if (ossl_rsa_prf(ctx, candidate_lengths, sizeof(candidate_lengths), "length",
                   6, kdk, MAX_LEN_GEN_TRIES * sizeof(len_candidate) * 8) < 0)
    goto err;
  ...
  for (i = 0; i < MAX_LEN_GEN_TRIES * (int)sizeof(len_candidate);
    ...
    synthetic_length =
    constant_time_select_int(constant_time_lt(len_candidate, max_sep_offset),
...
int ossl_rsa_padding_check_PKCS1_type_2_TLS(OSSL_LIB_CTX *libctx,
  ...
  if (alt_version > 0) {
    ...
    workaround_good = constant_time_eq(from[flen - SSL_MAX_MASTER_KEY_LENGTH],
                                       (alt_version >> 8) & 0xff);
    workaround_good &= constant_time_eq(
    from[flen - SSL_MAX_MASTER_KEY_LENGTH + 1], alt_version & 0xff);
  ...
  for (i = 0; i < SSL_MAX_MASTER_KEY_LENGTH; i++) {
    to[i] = constant_time_select_8(
    good, from[flen - SSL_MAX_MASTER_KEY_LENGTH + i], rand_premaster_secret[i]);
  }
