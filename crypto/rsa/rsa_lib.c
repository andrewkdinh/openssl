...
#ifndef FIPS_MODULE
RSA *RSA_new(void) { return rsa_new_intern(NULL, NULL); }

const RSA_METHOD *RSA_get_method(const RSA *rsa) { return rsa->meth; }
...
RSA *RSA_new_method(ENGINE *engine) { return rsa_new_intern(engine, NULL); }
#endif
...
OSSL_LIB_CTX *ossl_rsa_get0_libctx(RSA *r) { return r->libctx; }

void ossl_rsa_set0_libctx(RSA *r, OSSL_LIB_CTX *libctx) { r->libctx = libctx; }
...
uint16_t ossl_ifc_ffc_compute_security_bits(int n) {
    ...
    lx = ilog_e(x);
    y =
    (uint16_t)((mul2(c1_923, icbrt64(mul2(mul2(x, lx), lx))) - c4_690) / log_2);
    y = (y + 4) & ~7;
    if (y > cap)
        y = cap;
    return y;
...
#ifndef FIPS_MODULE
ENGINE *RSA_get0_engine(const RSA *r) { return r->engine; }
...
int ossl_rsa_set0_all_params(RSA *r, STACK_OF(BIGNUM) * primes,
    ...
    if (pnum > 2) {
        ...
        for (i = 2; i < pnum; i++) {
            ...
            /* Using ossl_rsa_multip_info_new() is wasteful, so allocate
             * directly */
...
int EVP_PKEY_CTX_set_rsa_padding(EVP_PKEY_CTX *ctx, int pad_mode) {
    return RSA_pkey_ctx_ctrl(ctx, -1, EVP_PKEY_CTRL_RSA_PADDING, pad_mode,
                             NULL);
}
...
int EVP_PKEY_CTX_get_rsa_padding(EVP_PKEY_CTX *ctx, int *pad_mode) {
    return RSA_pkey_ctx_ctrl(ctx, -1, EVP_PKEY_CTRL_GET_RSA_PADDING, 0,
                             pad_mode);
}
...
int EVP_PKEY_CTX_set_rsa_pss_keygen_mgf1_md_name(EVP_PKEY_CTX *ctx,
                                                 const char *mdname) {
    return int_set_rsa_md_name(ctx, EVP_PKEY_RSA_PSS, EVP_PKEY_OP_KEYGEN,
                               OSSL_PKEY_PARAM_MGF1_DIGEST, mdname, NULL, NULL);
}
