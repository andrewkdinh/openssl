...
static int rsa_item_sign(EVP_MD_CTX *ctx, const ASN1_ITEM *it, const void *asn,
  ...
  if (pad_mode == RSA_PKCS1_PSS_PADDING) {
    ...
    if (evp_pkey_ctx_is_legacy(pkctx)) {
      ...
      if (!X509_ALGOR_set0(alg1, OBJ_nid2obj(EVP_PKEY_RSA_PSS), V_ASN1_SEQUENCE,
                           os1)) {
        ASN1_STRING_free(os1);
        return 0;
      }
...
static int rsa_sig_info_set(X509_SIG_INFO *siginf, const X509_ALGOR *sigalg,
  ...
  if ((mdnid == NID_sha256 || mdnid == NID_sha384 || mdnid == NID_sha512) &&
      mdnid == EVP_MD_get_type(mgf1md) && saltlen == md_size)
...
static int rsa_int_export_to(const EVP_PKEY *from, int rsa_type,
  ...
  if (rsa->pss != NULL) {
    ...
    if (!ossl_rsa_pss_get_param_unverified(rsa->pss, &md, &mgf1md, &saltlen,
                                           &trailerfield))
      goto err;
...
static int rsa_pkey_export_to(const EVP_PKEY *from, void *to_keydata,
                              OSSL_FUNC_keymgmt_import_fn *importer,
                              OSSL_LIB_CTX *libctx, const char *propq) {
  return rsa_int_export_to(from, RSA_FLAG_TYPE_RSA, to_keydata, importer,
                           libctx, propq);
}
...
static int rsa_pss_pkey_export_to(const EVP_PKEY *from, void *to_keydata,
                                  OSSL_FUNC_keymgmt_import_fn *importer,
                                  OSSL_LIB_CTX *libctx, const char *propq) {
  return rsa_int_export_to(from, RSA_FLAG_TYPE_RSASSAPSS, to_keydata, importer,
                           libctx, propq);
}
