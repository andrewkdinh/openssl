...
static int pkey_rsa_sign(EVP_PKEY_CTX *ctx, unsigned char *sig, size_t *siglen,
  ...
  if (rctx->md) {
    ...
    if (EVP_MD_get_type(rctx->md) == NID_mdc2) {
      ...
      ret =
      RSA_private_encrypt(tbslen + 1, rctx->tbuf, sig, rsa, RSA_X931_PADDING);
      ...
      if (!RSA_padding_add_PKCS1_PSS_mgf1(rsa, rctx->tbuf, tbs, rctx->md,
        ...
        return -1;
      ret =
      RSA_private_encrypt(RSA_size(rsa), rctx->tbuf, sig, rsa, RSA_NO_PADDING);
    } else {
      return -1;
    }
  } else {
...
static int pkey_rsa_verifyrecover(EVP_PKEY_CTX *ctx, unsigned char *rout,
  ...
  if (rctx->md) {
    if (rctx->pad_mode == RSA_X931_PADDING) {
      ...
      size_t sltmp;
      ret = ossl_rsa_verify(EVP_MD_get_type(rctx->md), NULL, 0, rout, &sltmp,
                            sig, siglen, rsa);
      if (ret <= 0)
        return 0;
      ret = sltmp;
...
static int pkey_rsa_verify(EVP_PKEY_CTX *ctx, const unsigned char *sig,
  ...
  if (rctx->md) {
    if (rctx->pad_mode == RSA_PKCS1_PADDING)
      return RSA_verify(EVP_MD_get_type(rctx->md), tbs, tbslen, sig, siglen,
                        rsa);
    md_size = EVP_MD_get_size(rctx->md);
    if (md_size <= 0) {
      ERR_raise(ERR_LIB_RSA, RSA_R_INVALID_DIGEST_LENGTH);
