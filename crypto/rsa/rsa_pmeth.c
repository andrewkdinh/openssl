...
static int setup_tbuf(RSA_PKEY_CTX *ctx, EVP_PKEY_CTX *pk) {
    if (ctx->tbuf != NULL)
        return 1;
    if ((ctx->tbuf = OPENSSL_malloc(RSA_size(EVP_PKEY_get0_RSA(pk->pkey)))) ==
        NULL)
        return 0;
    return 1;
}
...
static int pkey_rsa_sign(EVP_PKEY_CTX *ctx, unsigned char *sig, size_t *siglen,
    ...
    if (rctx->md) {
        ...
        if (EVP_MD_get_type(rctx->md) == NID_mdc2) {
            ...
            unsigned int sltmp;
            ret =
            RSA_sign(EVP_MD_get_type(rctx->md), tbs, tbslen, sig, &sltmp, rsa);
            if (ret <= 0)
                return ret;
            ret = sltmp;
    ...
    if (rctx->md) {
        if (rctx->pad_mode == RSA_X931_PADDING) {
            if (!setup_tbuf(rctx, ctx))
                return -1;
            ret =
            RSA_public_decrypt(siglen, sig, rctx->tbuf, rsa, RSA_X931_PADDING);
            if (ret < 1)
                return 0;
            ret--;
            if (rctx->tbuf[ret] !=
            ...
            size_t sltmp;
            ret = ossl_rsa_verify(EVP_MD_get_type(rctx->md), NULL, 0, rout,
                                  &sltmp, sig, siglen, rsa);
            if (ret <= 0)
                return 0;
            ret = sltmp;
        } else {
...
static int pkey_rsa_verify(EVP_PKEY_CTX *ctx, const unsigned char *sig,
    ...
    if (rctx->md) {
        if (rctx->pad_mode == RSA_PKCS1_PADDING)
            return RSA_verify(EVP_MD_get_type(rctx->md), tbs, tbslen, sig,
                              siglen, rsa);
        md_size = EVP_MD_get_size(rctx->md);
        if (md_size <= 0) {
            ERR_raise(ERR_LIB_RSA, RSA_R_INVALID_DIGEST_LENGTH);
        ...
        if (rctx->pad_mode == RSA_X931_PADDING) {
            ...
            if (!setup_tbuf(rctx, ctx))
                return -1;
            ret =
            RSA_public_decrypt(siglen, sig, rctx->tbuf, rsa, RSA_NO_PADDING);
            if (ret <= 0)
                return 0;
...
static int pkey_rsa_encrypt(EVP_PKEY_CTX *ctx, unsigned char *out,
    ...
    if (rctx->pad_mode == RSA_PKCS1_OAEP_PADDING) {
        ...
        if (!RSA_padding_add_PKCS1_OAEP_mgf1(
            rctx->tbuf, klen, in, inlen, rctx->oaep_label, rctx->oaep_labellen,
            rctx->md, rctx->mgf1md))
            return -1;
...
static int pkey_rsa_decrypt(EVP_PKEY_CTX *ctx, unsigned char *out,
    ...
    if (rctx->pad_mode == RSA_PKCS1_OAEP_PADDING) {
        ...
        out, ret, rctx->tbuf, ret, ret, rctx->oaep_label, rctx->oaep_labellen,
        rctx->md, rctx->mgf1md);
    } else {
        if (rctx->pad_mode == RSA_PKCS1_PADDING &&
...
static int rsa_set_pss_param(RSA *rsa, EVP_PKEY_CTX *ctx) {
    ...
    rctx->md, rctx->mgf1md, rctx->saltlen == -2 ? 0 : rctx->saltlen);
    if (rsa->pss == NULL)
        return 0;
    return 1;
}
...
const EVP_PKEY_METHOD *ossl_rsa_pkey_method(void) { return &rsa_pkey_meth; }
