...
int ossl_rsa_fromdata(RSA *rsa, const OSSL_PARAM params[],
  ...
  if (include_private) {
    ...
    param_derive =
    OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_RSA_DERIVE_FROM_PQ);
  ...
  if (is_private) {
    ...
    if (derive_from_pq && sk_BIGNUM_num(exps) == 0 &&
      ...
      if (sk_BIGNUM_num(factors) == 2) {
         ...
         */
        if (!ossl_rsa_sp800_56b_derive_params_from_pq(rsa, RSA_bits(rsa), NULL,
                                                      ctx)) {
          ERR_raise(ERR_LIB_RSA, ERR_R_INTERNAL_ERROR);
          goto err;
        }
        ...
        if (!ossl_rsa_multiprime_derive(rsa, RSA_bits(rsa),
                                        sk_BIGNUM_num(factors), rsa->e, factors,
                                        exps, coeffs)) {
          ERR_raise(ERR_LIB_RSA, ERR_R_INTERNAL_ERROR);
          goto err;
        }
    ...
    if (sk_BIGNUM_num(factors) != 0 || sk_BIGNUM_num(exps) != 0 ||
      ...
      ERR_raise_data(ERR_LIB_RSA, ERR_R_INTERNAL_ERROR,
                     "There are %d, %d, %d elements left on our factors, exps, "
                     "coeffs stacks\n",
  ...
  if (include_private && rsa_d != NULL) {
    ...
    if (!ossl_param_build_set_bn(bld, params, OSSL_PKEY_PARAM_RSA_D, rsa_d) ||
        !ossl_param_build_set_multi_key_bn(bld, params,
                                           ossl_rsa_mp_factor_names, factors) ||
        !ossl_param_build_set_multi_key_bn(bld, params, ossl_rsa_mp_exp_names,
                                           exps) ||
        !ossl_param_build_set_multi_key_bn(bld, params, ossl_rsa_mp_coeff_names,
                                           coeffs))
      goto err;
  }
...
int ossl_rsa_pss_params_30_todata(const RSA_PSS_PARAMS_30 *pss,
  ...
  if (!ossl_rsa_pss_params_30_is_unrestricted(pss)) {
    ...
    if ((mdname != NULL &&
        ...
        (mgfname != NULL &&
         !ossl_param_build_set_utf8_string(bld, params, key_mgf, mgfname)) ||
        (mgf1mdname != NULL && !ossl_param_build_set_utf8_string(
                               bld, params, key_mgf1_md, mgf1mdname)) ||
...
RSA *ossl_rsa_dup(const RSA *rsa, int selection) {
  ...
  if (rsa->pss != NULL) {
    ...
    if (rsa->pss->maskGenAlgorithm != NULL &&
      ...
      dupkey->pss->maskHash =
      ossl_x509_algor_mgf1_decode(rsa->pss->maskGenAlgorithm);
...
RSA_PSS_PARAMS *ossl_rsa_pss_decode(const X509_ALGOR *alg) {
  ...
  pss =
  ASN1_TYPE_unpack_sequence(ASN1_ITEM_rptr(RSA_PSS_PARAMS), alg->parameter);
...
static int ossl_rsa_sync_to_pss_params_30(RSA *rsa) {
  ...
  if (rsa != NULL && (legacy_pss = RSA_get0_pss_params(rsa)) != NULL &&
     ...
     */
    if (!ossl_rsa_pss_get_param_unverified(legacy_pss, &md, &mgf1md, &saltlen,
                                           &trailerField))
      return 0;
