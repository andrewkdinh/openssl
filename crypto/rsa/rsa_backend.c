...
int ossl_rsa_fromdata(RSA *rsa, const OSSL_PARAM params[], int include_private)
{
    ...
    if (include_private) {
        ...
        param_derive = OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_RSA_DERIVE_FROM_PQ);
        if ((param_derive != NULL) && !OSSL_PARAM_get_int(param_derive, &derive_from_pq))
            goto err;
    ...
    if (is_private) {
        ...
        if (derive_from_pq && sk_BIGNUM_num(exps) == 0 && sk_BIGNUM_num(coeffs) == 0) {
            /*
             ...
             * been provided
             */
            if (sk_BIGNUM_num(factors) > 2 && (param_n == NULL || param_d == NULL)) {
                ERR_raise(ERR_LIB_RSA, ERR_R_PASSED_NULL_PARAMETER);
                goto err;
            }
            ...
            if (sk_BIGNUM_num(factors) == 2) {
                ...
                if (!RSA_set0_factors(rsa, sk_BIGNUM_value(factors, 0), sk_BIGNUM_value(factors, 1))) {
                 ...
                 */
                if (!ossl_rsa_sp800_56b_derive_params_from_pq(rsa, RSA_bits(rsa), NULL, ctx)) {
                    ERR_raise(ERR_LIB_RSA, ERR_R_INTERNAL_ERROR);
                    goto err;
                }
            } else {
                 ...
                 */
                if (!ossl_rsa_multiprime_derive(rsa, RSA_bits(rsa), sk_BIGNUM_num(factors), rsa->e, factors, exps,
                                                coeffs)) {
                    ERR_raise(ERR_LIB_RSA, ERR_R_INTERNAL_ERROR);
                    goto err;
                }
        ...
        if (sk_BIGNUM_num(factors) != 0 || sk_BIGNUM_num(exps) != 0 || sk_BIGNUM_num(coeffs) != 0) {
            ERR_raise_data(ERR_LIB_RSA, ERR_R_INTERNAL_ERROR,
                           "There are %d, %d, %d elements left on our factors, exps, coeffs stacks\n",
                           sk_BIGNUM_num(factors), sk_BIGNUM_num(exps), sk_BIGNUM_num(coeffs));
            goto err;
        }
    }

    if (!ossl_rsa_check_factors(rsa)) {
        ERR_raise_data(ERR_LIB_RSA, RSA_R_INVALID_KEYPAIR, "RSA factors/exponents are too big for for n-modulus\n");
        goto err;
    }
...
int ossl_rsa_pss_params_30_todata(const RSA_PSS_PARAMS_30 *pss, OSSL_PARAM_BLD *bld, OSSL_PARAM params[])
{
    if (!ossl_rsa_pss_params_30_is_unrestricted(pss)) {
        ...
        const char *mgfname =
            (maskgenalg_nid == default_maskgenalg_nid ? NULL : ossl_rsa_oaeppss_nid2name(maskgenalg_nid));
        const char *mgf1mdname =
            (maskgenhashalg_nid == default_maskgenhashalg_nid ? NULL : ossl_rsa_oaeppss_nid2name(maskgenhashalg_nid));
...
int ossl_rsa_pss_params_30_fromdata(RSA_PSS_PARAMS_30 *pss_params, int *defaults_set, const OSSL_PARAM params[],
                                    OSSL_LIB_CTX *libctx)
{
...
static int ossl_rsa_sync_to_pss_params_30(RSA *rsa)
{
    ...
    if (rsa != NULL && (legacy_pss = RSA_get0_pss_params(rsa)) != NULL
        ...
        if (!ossl_rsa_pss_params_30_set_defaults(&pss_params)
            ...
            || !ossl_rsa_pss_params_30_set_maskgenhashalg(&pss_params, mgf1md_nid)
...
int ossl_rsa_pss_get_param_unverified(const RSA_PSS_PARAMS *pss, const EVP_MD **pmd, const EVP_MD **pmgf1md,
