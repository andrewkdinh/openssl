  ...
  *reason++ = '\0';
  ...
  default:
    if (retcode < HTTP_STATUS_CODES_NONFATAL_ERROR) {
      ERR_raise_data(ERR_LIB_HTTP, HTTP_R_STATUS_CODE_UNSUPPORTED, "code=%s",
                     code);
      if (*reason != '\0')
...
next_io:
  ...
  if ((rctx->state & OHS_NOREAD) == 0) {
    if (rctx->expect_asn1 &&
        (rctx->state == OHS_ASN1_HEADER || rctx->state == OHS_ASN1_CONTENT)) {
  ...
  case OHS_FLUSH:
    ...
    /* State machine could be broken up at this point and bulky code sections
     * factorized out. */
  ...
  next_line:
    ...
    if (rctx->state == OHS_FIRSTLINE) {
      ...
      default:
        ...
        rctx->state =
        i < HTTP_STATUS_CODES_NONFATAL_ERROR ? OHS_HEADERS_ERROR : OHS_HEADERS;
    ...
    if (value != NULL && line_end != NULL) {
      ...
      if (OPENSSL_strcasecmp(key, "Connection") == 0) {
        ...
        if (line_end == value || *line_end != '\0') {
          ERR_raise_data(ERR_LIB_HTTP, HTTP_R_ERROR_PARSING_CONTENT_LENGTH,
                         "input=%s", value);
          return 0;
        }
    ...
    if (rctx->expected_ct != NULL && !found_expected_ct) {
      ERR_raise_data(ERR_LIB_HTTP, HTTP_R_MISSING_CONTENT_TYPE, "expected=%s",
                     rctx->expected_ct);
      return 0;
    }
    if (rctx->state == OHS_REDIRECT) {
...
BIO *OSSL_HTTP_transfer(OSSL_HTTP_REQ_CTX **prctx, const char *server,
  ...
  if (rctx != NULL) {
    if (OSSL_HTTP_set1_request(rctx, path, headers, content_type, req,
                               expected_ct, expect_asn1, max_resp_len, timeout,
                               keep_alive))
