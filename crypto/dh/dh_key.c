...
int ossl_dh_compute_key(unsigned char *key, const BIGNUM *pub_key, DH *dh) {
  ...
  if (dh->flags & DH_FLAG_CACHE_MONT_P) {
    mont =
    BN_MONT_CTX_set_locked(&dh->method_mont_p, dh->lock, dh->params.p, ctx);
  ...
  if (BN_copy(pminus1, dh->params.p) == NULL || !BN_sub_word(pminus1, 1) ||
      BN_cmp(z, BN_value_one()) <= 0 || BN_cmp(z, pminus1) == 0) {
...
"OpenSSL DH Method", generate_key, ossl_dh_compute_key,
...
int ossl_dh_generate_public_key(BN_CTX *ctx, const DH *dh,
  ...
  if (!dh->meth->bn_mod_exp(dh, pub_key, dh->params.g, prk, dh->params.p, ctx,
                            mont))
    goto err;
  ret = 1;
err:
  ...
  if (generate_new_key) {
    ...
    if (DH_get_nid(dh) != NID_undef) {
      ...
      if (dh->params.q == NULL) {
        ...
        if (!BN_priv_rand_ex(priv_key, l, BN_RAND_TOP_ONE, BN_RAND_BOTTOM_ANY,
                             0, ctx))
          goto err;
        /*
...
size_t ossl_dh_key2buf(const DH *dh, unsigned char **pbuf_out, size_t size,
  ...
  if (p == NULL || pubkey == NULL || (p_size = BN_num_bytes(p)) == 0 ||
