...
const DH_METHOD *ossl_dh_get_method(const DH *dh) { return dh->meth; }
#ifndef OPENSSL_NO_DEPRECATED_3_0
DH *DH_new(void) { return dh_new_intern(NULL, NULL); }
#endif

DH *DH_new_method(ENGINE *engine) { return dh_new_intern(engine, NULL); }
#endif /* !FIPS_MODULE */

DH *ossl_dh_new_ex(OSSL_LIB_CTX *libctx) { return dh_new_intern(NULL, libctx); }
...
void ossl_dh_set0_libctx(DH *d, OSSL_LIB_CTX *libctx) { d->libctx = libctx; }
...
long DH_get_length(const DH *dh) { return dh->length; }
...
const BIGNUM *DH_get0_p(const DH *dh) { return dh->params.p; }

const BIGNUM *DH_get0_q(const DH *dh) { return dh->params.q; }

const BIGNUM *DH_get0_g(const DH *dh) { return dh->params.g; }

const BIGNUM *DH_get0_priv_key(const DH *dh) { return dh->priv_key; }

const BIGNUM *DH_get0_pub_key(const DH *dh) { return dh->pub_key; }

void DH_clear_flags(DH *dh, int flags) { dh->flags &= ~flags; }

int DH_test_flags(const DH *dh, int flags) { return dh->flags & flags; }

void DH_set_flags(DH *dh, int flags) { dh->flags |= flags; }

#ifndef FIPS_MODULE
ENGINE *DH_get0_engine(DH *dh) { return dh->engine; }
#endif /*FIPS_MODULE */

FFC_PARAMS *ossl_dh_get0_params(DH *dh) { return &dh->params; }
int ossl_dh_get0_nid(const DH *dh) { return dh->params.nid; }
