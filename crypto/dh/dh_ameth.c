...
static void int_dh_free(EVP_PKEY *pkey) { DH_free(pkey->pkey.dh); }
...
static int dh_pub_encode(X509_PUBKEY *pk, const EVP_PKEY *pkey) {
  ...
  if (X509_PUBKEY_set0_param(pk, OBJ_nid2obj(pkey->ameth->pkey_id), ptype, str,
                             penc, penclen))
    return 1;
...
static int int_dh_size(const EVP_PKEY *pkey) { return DH_size(pkey->pkey.dh); }
...
int DHparams_print(BIO *bp, const DH *x) { return do_dh_print(bp, x, 4, 0); }
