...
static DH *ffc_params_generate(OSSL_LIB_CTX *libctx, DH_PKEY_CTX *dctx,
  ...
  if (dctx->paramgen_type == DH_PARAMGEN_TYPE_FIPS_186_2)
    rv =
    ossl_ffc_params_FIPS186_2_generate(libctx, &ret->params, FFC_PARAM_TYPE_DH,
    ...
    if (dctx->paramgen_type >= DH_PARAMGEN_TYPE_FIPS_186_2)
      rv = ossl_ffc_params_FIPS186_4_generate(libctx, &ret->params,
                                              FFC_PARAM_TYPE_DH, prime_len,
                                              subprime_len, &res, pcb);
  if (rv <= 0) {
    DH_free(ret);
    return NULL;
  }
...
const EVP_PKEY_METHOD *ossl_dh_pkey_method(void) { return &dh_pkey_meth; }
...
const EVP_PKEY_METHOD *ossl_dhx_pkey_method(void) { return &dhx_pkey_meth; }
