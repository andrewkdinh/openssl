...
static int provider_create_child_cb(const OSSL_CORE_HANDLE *prov,
                                    void *cbdata) {
    ...
    if ((cprov = ossl_provider_find(ctx, provname, 1)) != NULL) {
        /*
         * We free the newly created ref. We rely on the provider sticking
         * around in the provider store.
         */
...
static int provider_remove_child_cb(const OSSL_CORE_HANDLE *prov,
                                    void *cbdata) {
    OSSL_LIB_CTX *ctx = cbdata;
    struct child_prov_globals *gbl;
    const char *provname;
...
int ossl_provider_init_as_child(OSSL_LIB_CTX *ctx,
    ...
    if (gbl->c_get_libctx == NULL ||
        gbl->c_provider_register_child_cb == NULL || gbl->c_prov_name == NULL ||
