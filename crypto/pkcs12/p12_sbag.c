...
X509 *PKCS12_SAFEBAG_get1_cert_ex(const PKCS12_SAFEBAG *bag,
  ...
  if (OBJ_obj2nid(bag->value.bag->type) != NID_x509Certificate)
    return NULL;
  ret = ASN1_item_unpack_ex(bag->value.bag->value.octet, ASN1_ITEM_rptr(X509),
                            libctx, propq);
...
int pbe_nid, const char *pass, int passlen, unsigned char *salt, int saltlen,
int iter, PKCS8_PRIV_KEY_INFO *p8inf, OSSL_LIB_CTX *ctx, const char *propq) {
...
PKCS12_SAFEBAG_create_pkcs8_encrypt(int pbe_nid, const char *pass, int passlen,
  ...
  return PKCS12_SAFEBAG_create_pkcs8_encrypt_ex(
  pbe_nid, pass, passlen, salt, saltlen, iter, p8inf, NULL, NULL);
}
