...
int PKCS12_parse(PKCS12 *p12, const char *pass, EVP_PKEY **pkey, X509 **cert,
    ...
    if (PKCS12_mac_present(p12)) {
         ...
         * determine which password is correct. The reason for this is that
...
static int parse_pk12(PKCS12 *p12, const char *pass, int passlen,
    ...
    for (i = 0; i < sk_PKCS7_num(asafes); i++) {
        ...
        if (!parse_bags(bags, pass, passlen, pkey, ocerts, p7->ctx.libctx,
                        p7->ctx.propq)) {
...
static int parse_bag(PKCS12_SAFEBAG *bag, const char *pass, int passlen,
    ...
    case NID_keyBag:
        if (pkey == NULL || *pkey != NULL)
            return 1;
        *pkey =
        EVP_PKCS82PKEY_ex(PKCS12_SAFEBAG_get0_p8inf(bag), libctx, propq);
        if (*pkey == NULL)
            return 0;
        break;
    ...
    case NID_pkcs8ShroudedKeyBag:
        if (pkey == NULL || *pkey != NULL)
            return 1;
        if ((p8 = PKCS12_decrypt_skey_ex(bag, pass, passlen, libctx, propq)) ==
            NULL)
            return 0;
