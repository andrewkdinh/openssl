...
static int parse_bags(const STACK_OF(PKCS12_SAFEBAG) *bags, const char *pass, int passlen, EVP_PKEY **pkey,
...
static int parse_pk12(PKCS12 *p12, const char *pass, int passlen, EVP_PKEY **pkey, STACK_OF(X509) *ocerts)
{
    ...
    for (i = 0; i < sk_PKCS7_num(asafes); i++) {
        ...
        if (!parse_bags(bags, pass, passlen, pkey, ocerts, p7->ctx.libctx, p7->ctx.propq)) {
                      ...
                      STACK_OF(X509) *ocerts, OSSL_LIB_CTX *libctx, const char *propq)
{
    ...
    for (i = 0; i < sk_PKCS12_SAFEBAG_num(bags); i++) {
        if (!parse_bag(sk_PKCS12_SAFEBAG_value(bags, i), pass, passlen, pkey, ocerts, libctx, propq))
            return 0;
    }
    return 1;
}
                     ...
                     OSSL_LIB_CTX *libctx, const char *propq)
{
    ...
    case NID_keyBag:
        if (pkey == NULL || *pkey != NULL)
            return 1;
        *pkey = EVP_PKCS82PKEY_ex(PKCS12_SAFEBAG_get0_p8inf(bag), libctx, propq);
        if (*pkey == NULL)
            return 0;
        break;
    ...
    case NID_pkcs8ShroudedKeyBag:
        if (pkey == NULL || *pkey != NULL)
            return 1;
        if ((p8 = PKCS12_decrypt_skey_ex(bag, pass, passlen, libctx, propq)) == NULL)
            return 0;
    ...
    case NID_safeContentsBag:
        return parse_bags(PKCS12_SAFEBAG_get0_safes(bag), pass, passlen, pkey, ocerts, libctx, propq);
