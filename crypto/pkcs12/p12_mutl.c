                          ...
                          const EVP_MD *md_type))
{
    ...
    if (OBJ_obj2nid(macoid) == NID_pbmac1) {
        if (OBJ_obj2txt(md_name, sizeof(md_name), OBJ_nid2obj(pbmac1_md_nid), 0)
            < 0)
            return 0;
    } else {
    ...
    if (pbmac1_md_nid != NID_undef && pkcs12_key_gen == NULL) {
        ...
        if (pkcs12_key_gen != NULL) {
            int res =
                (*pkcs12_key_gen)(pass, passlen, salt, saltlen, PKCS12_MAC_ID,
            ...
            if (!PKCS12_key_gen_utf8_ex(pass, passlen, salt, saltlen,
                                        PKCS12_MAC_ID, iter, keylen, key, md,
                             ...
                             const EVP_MD *md_type, const char *prf_md_name)
{
    ...
    alg = PKCS5_pbkdf2_set(iter, salt ? salt : known_salt, saltlen, prf_nid,
                           keylen);
