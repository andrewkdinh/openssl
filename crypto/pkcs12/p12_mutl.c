...
int PKCS12_mac_present(const PKCS12 *p12) { return p12->mac ? 1 : 0; }
...
PBKDF2PARAM *PBMAC1_get1_pbkdf2_param(const X509_ALGOR *macalg) {
    ...
    param =
    ASN1_TYPE_unpack_sequence(ASN1_ITEM_rptr(PBMAC1PARAM), macalg->parameter);
    if (param == NULL) {
    ...
    pbkdf2_param = ASN1_TYPE_unpack_sequence(
    ASN1_ITEM_rptr(PBKDF2PARAM), param->keyDerivationFunc->parameter);
    PBMAC1PARAM_free(param);
...
static int PBMAC1_PBKDF2_HMAC(OSSL_LIB_CTX *ctx, const char *propq,
    ...
    kdf_md =
    EVP_MD_fetch(ctx, OBJ_nid2sn(ossl_hmac2mdnid(kdf_hmac_nid)), propq);
    if (kdf_md == NULL) {
    ...
    if (PKCS5_PBKDF2_HMAC(pass, passlen, pbkdf2_salt->data, pbkdf2_salt->length,
                          ASN1_INTEGER_get(pbkdf2_param->iter), kdf_md, keylen,
                          key) <= 0) {
...
static int pkcs12_gen_mac(
PKCS12 *p12, const char *pass, int passlen, unsigned char *mac,
unsigned int *maclen, int pbmac1_md_nid, int pbmac1_kdf_nid,
int (*pkcs12_key_gen)(const char *pass, int passlen, unsigned char *salt,
    ...
    if (OBJ_obj2nid(macoid) == NID_pbmac1) {
        if (OBJ_obj2txt(md_name, sizeof(md_name), OBJ_nid2obj(pbmac1_md_nid),
                        0) < 0)
            return 0;
    } else {
    ...
    /* For PBMAC1 we use a special keygen callback if not provided (e.g. on
     * verification) */
    if (pbmac1_md_nid != NID_undef && pkcs12_key_gen == NULL) {
        keylen = PBMAC1_PBKDF2_HMAC(p12->authsafes->ctx.libctx,
                                    p12->authsafes->ctx.propq, pass, passlen,
                                    macalg, key);
        if (keylen < 0)
            goto err;
        ...
        if (!pkcs12_gen_gost_mac_key(pass, passlen, salt, saltlen, iter, keylen,
                                     key, md)) {
        ...
        if (pbmac1_kdf_nid != NID_undef) {
            ...
            if (OBJ_obj2txt(hmac_md_name, sizeof(hmac_md_name),
                            OBJ_nid2obj(pbmac1_kdf_nid), 0) < 0)
                goto err;
        ...
        if (pkcs12_key_gen != NULL) {
            int res =
            (*pkcs12_key_gen)(pass, passlen, salt, saltlen, PKCS12_MAC_ID, iter,
                              keylen, key, hmac_md);
            ...
            if (!PKCS12_key_gen_utf8_ex(
                pass, passlen, salt, saltlen, PKCS12_MAC_ID, iter, keylen, key,
                md, p12->authsafes->ctx.libctx, p12->authsafes->ctx.propq)) {
                ERR_raise(ERR_LIB_PKCS12, PKCS12_R_KEY_GEN_ERROR);
                goto err;
            }
        }
...
int PKCS12_gen_mac(PKCS12 *p12, const char *pass, int passlen,
    ...
    return pkcs12_gen_mac(p12, pass, passlen, mac, maclen, NID_undef, NID_undef,
                          NULL);
}
...
int PKCS12_verify_mac(PKCS12 *p12, const char *pass, int passlen) {
    ...
    if (OBJ_obj2nid(macoid) == NID_pbmac1) {
        ...
        param = ASN1_TYPE_unpack_sequence(ASN1_ITEM_rptr(PBMAC1PARAM),
                                          macalg->parameter);
        if (param == NULL) {
            ERR_raise(ERR_LIB_PKCS12, ERR_R_UNSUPPORTED);
            return 0;
        ...
        if (!pkcs12_gen_mac(p12, pass, passlen, mac, &maclen, md_nid, NID_undef,
                            NULL)) {
        ...
        if (!pkcs12_gen_mac(p12, pass, passlen, mac, &maclen, NID_undef,
                            NID_undef, NULL)) {
            ERR_raise(ERR_LIB_PKCS12, PKCS12_R_MAC_GENERATION_ERROR);
            return 0;
        }
...
int PKCS12_set_mac(PKCS12 *p12, const char *pass, int passlen,
     ...
     */
    if (!pkcs12_gen_mac(p12, pass, passlen, mac, &maclen, NID_undef, NID_undef,
                        NULL)) {
        ERR_raise(ERR_LIB_PKCS12, PKCS12_R_MAC_GENERATION_ERROR);
        return 0;
    }
...
static int pkcs12_pbmac1_pbkdf2_key_gen(const char *pass, int passlen,
                                        ...
                                        unsigned char *out,
                                        const EVP_MD *md_type) {
    return PKCS5_PBKDF2_HMAC(pass, passlen, salt, saltlen, iter, md_type,
                             keylen, out);
}

static int pkcs12_setup_mac(PKCS12 *p12, int iter, unsigned char *salt,
                            int saltlen, int nid) {
    X509_ALGOR *macalg;
...
int PKCS12_set_pbmac1_pbkdf2(PKCS12 *p12, const char *pass, int passlen,
    ...
    alg =
    PKCS5_pbkdf2_set(iter, salt ? salt : known_salt, saltlen, prf_nid, keylen);
    ...
    if (!ASN1_TYPE_pack_sequence(ASN1_ITEM_rptr(PBMAC1PARAM), param,
                                 &macalg->parameter))
        goto err;
