...
static int newpass_bags(STACK_OF(PKCS12_SAFEBAG) * bags, const char *oldpass,
                        const char *newpass, OSSL_LIB_CTX *libctx,
                        const char *propq);
  ...
  for (i = 0; i < sk_PKCS7_num(asafes); i++) {
    ...
    if (bagnid == NID_pkcs7_data) {
      ...
      if (p7->d.encrypted == NULL ||
          !alg_get(p7->d.encrypted->enc_data->algorithm, &pbe_nid, &pbe_iter,
                   &pbe_saltlen, &cipherid))
        goto err;
    } else {
      continue;
    }
    ...
    if (bagnid == NID_pkcs7_data)
      ...
      p7new =
      PKCS12_pack_p7encdata_ex(pbe_nid, newpass, -1, NULL, pbe_saltlen,
                               pbe_iter, bags, p7->ctx.libctx, p7->ctx.propq);
...
static int newpass_bags(STACK_OF(PKCS12_SAFEBAG) * bags, const char *oldpass,
  ...
  for (i = 0; i < sk_PKCS12_SAFEBAG_num(bags); i++) {
    if (!newpass_bag(sk_PKCS12_SAFEBAG_value(bags, i), oldpass, newpass, libctx,
                     propq))
      return 0;
  }
  return 1;
...
static int newpass_bag(PKCS12_SAFEBAG *bag, const char *oldpass,
  ...
  if ((p8 = PKCS8_decrypt_ex(bag->value.shkeybag, oldpass, -1, libctx,
                             propq)) == NULL)
    return 0;
