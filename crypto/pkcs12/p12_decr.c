                                   ...
                                   OSSL_LIB_CTX *libctx, const char *propq)
{
    ...
    if ((EVP_CIPHER_get_flags(EVP_CIPHER_CTX_get0_cipher(ctx))
        ...
        if (EVP_CIPHER_CTX_is_encrypting(ctx)) {
            ...
            inlen -= mac_len;
            if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG, (int)mac_len,
    ...
    if ((EVP_CIPHER_get_flags(EVP_CIPHER_CTX_get0_cipher(ctx))
        ...
        != 0) {
        if (EVP_CIPHER_CTX_is_encrypting(ctx)) {
            if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, (int)mac_len,
                                    out + outlen)
                < 0) {
                OPENSSL_free(out);
                out = NULL;
                                 ...
                                 OSSL_LIB_CTX *libctx, const char *propq)
{
    ...
    if (!PKCS12_pbe_crypt_ex(algor, pass, passlen, oct->data, oct->length, &out,
                             &outlen, 0, libctx, propq))
        return NULL;
    p = out;
...
void *PKCS12_item_decrypt_d2i(const X509_ALGOR *algor, const ASN1_ITEM *it,
                              ...
                              const ASN1_OCTET_STRING *oct, int zbuf)
{
    return PKCS12_item_decrypt_d2i_ex(algor, it, pass, passlen, oct, zbuf, NULL,
                                      NULL);
}
...
ASN1_OCTET_STRING *PKCS12_item_i2d_encrypt(X509_ALGOR *algor,
                                           ...
                                           void *obj, int zbuf)
{
    return PKCS12_item_i2d_encrypt_ex(algor, it, pass, passlen, obj, zbuf, NULL,
                                      NULL);
}
