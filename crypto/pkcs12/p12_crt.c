...
static PKCS12_SAFEBAG *pkcs12_add_cert_bag(STACK_OF(PKCS12_SAFEBAG) * *pbags,
                                           X509 *cert, const char *name,
                                           int namelen, unsigned char *keyid,
                                           int keyidlen);
...
PKCS12 *PKCS12_create_ex2(const char *pass, const char *name, EVP_PKEY *pkey,
  ...
  if (bags &&
      !PKCS12_add_safe_ex(&safes, bags, nid_cert, iter, pass, ctx, propq))
    goto err;
  ...
  if (pkey) {
    bag =
    PKCS12_add_key_ex(&bags, pkey, keytype, iter, nid_key, pass, ctx, propq);
...
PKCS12 *PKCS12_create_ex(const char *pass, const char *name, EVP_PKEY *pkey,
                         ...
                         int nid_cert, int iter, int mac_iter, int keytype,
                         OSSL_LIB_CTX *ctx, const char *propq) {
  return PKCS12_create_ex2(pass, name, pkey, cert, ca, nid_key, nid_cert, iter,
                           mac_iter, keytype, ctx, propq, NULL, NULL);
}

PKCS12 *PKCS12_create(const char *pass, const char *name, EVP_PKEY *pkey,
                      X509 *cert, STACK_OF(X509) * ca, int nid_key,
                      int nid_cert, int iter, int mac_iter, int keytype) {
  return PKCS12_create_ex(pass, name, pkey, cert, ca, nid_key, nid_cert, iter,
                          mac_iter, keytype, NULL, NULL);
}
...
PKCS12_SAFEBAG *PKCS12_add_key(STACK_OF(PKCS12_SAFEBAG) * *pbags, EVP_PKEY *key,
                               ...
                               const char *pass) {
  return PKCS12_add_key_ex(pbags, key, key_usage, iter, nid_key, pass, NULL,
                           NULL);
}
...
PKCS12_SAFEBAG *PKCS12_add_secret(STACK_OF(PKCS12_SAFEBAG) * *pbags,
  ...
  if ((bag = PKCS12_SAFEBAG_create_secret(nid_type, V_ASN1_OCTET_STRING, value,
                                          len)) == NULL)
    goto err;
...
int PKCS12_add_safe_ex(STACK_OF(PKCS7) * *psafes,
                       STACK_OF(PKCS12_SAFEBAG) * bags, int nid_safe, int iter,
  ...
  if (nid_safe == -1)
    ...
    p7 = PKCS12_pack_p7encdata_ex(nid_safe, pass, -1, NULL, 0, iter, bags, ctx,
                                  propq);
  if (p7 == NULL)
    goto err;
