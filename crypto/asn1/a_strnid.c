...
void ASN1_STRING_set_default_mask(unsigned long mask) { global_mask = mask; }

unsigned long ASN1_STRING_get_default_mask(void) { return global_mask; }
...
ASN1_STRING *ASN1_STRING_set_by_NID(ASN1_STRING **out, const unsigned char *in,
  ...
  if (tbl != NULL) {
    ...
    ret =
    ASN1_mbstring_copy(out, in, inlen, inform, DIRSTRING_TYPE & global_mask);
  }
  if (ret <= 0)
    return NULL;
  return *out;
