...
static int asn1_item_ex_d2i_intern(ASN1_VALUE **pval, const unsigned char **in,
  ...
  rv = asn1_item_embed_d2i(pval, in, len, it, tag, aclass, opt, ctx, 0, libctx,
                           propq);
  if (rv <= 0)
...
int ASN1_item_ex_d2i(ASN1_VALUE **pval, const unsigned char **in, long len,
                     ...
                     ASN1_TLC *ctx) {
  return asn1_item_ex_d2i_intern(pval, in, len, it, tag, aclass, opt, ctx, NULL,
                                 NULL);
}
...
ASN1_VALUE *ASN1_item_d2i_ex(ASN1_VALUE **pval, const unsigned char **in,
  ...
  if (pval == NULL)
    pval = &ptmpval;
  asn1_tlc_clear_nc(&c);
  if (asn1_item_ex_d2i_intern(pval, in, len, it, -1, 0, 0, &c, libctx, propq) >
      0)
    return *pval;
  return NULL;
}
...
static int asn1_item_embed_d2i(ASN1_VALUE **pval, const unsigned char **in,
  ...
  case ASN1_ITYPE_PRIMITIVE:
    if (it->templates) {
      ...
      return asn1_template_ex_d2i(pval, in, len, it->templates, opt, ctx, depth,
                                  libctx, propq);
    }
  ...
  case ASN1_ITYPE_MSTRING:
    ...
    ret =
    asn1_check_tlen(NULL, &otag, &oclass, NULL, NULL, &p, len, -1, 0, 1, ctx);
    if (!ret) {
  ...
  case ASN1_ITYPE_CHOICE:
    ...
    for (i = 0, tt = it->templates; i < it->tcount; i++, tt++) {
       ...
       */
      ret =
      asn1_template_ex_d2i(pchptr, &p, len, tt, 1, ctx, depth, libctx, propq);
  ...
  case ASN1_ITYPE_SEQUENCE:
    ...
    ret = asn1_check_tlen(&len, NULL, NULL, &seq_eoc, &cst, &p, len, tag,
                          aclass, opt, ctx);
    if (!ret) {
      ERR_raise(ERR_LIB_ASN1, ERR_R_NESTED_ASN1_ERROR);
      goto err;
    ...
    for (i = 0, tt = it->templates; i < it->tcount; i++, tt++) {
      ...
      ret = asn1_template_ex_d2i(pseqval, &p, len, seqtt, isopt, ctx, depth,
                                 libctx, propq);
      if (!ret) {
        errtt = seqtt;
        goto err;
...
static int asn1_template_ex_d2i(ASN1_VALUE **val, const unsigned char **in,
  ...
  if (flags & ASN1_TFLG_EXPTAG) {
     ...
     */
    ret = asn1_check_tlen(&len, NULL, NULL, &exp_eoc, &cst, &p, inlen, tt->tag,
                          aclass, opt, ctx);
    q = p;
    if (!ret) {
      ERR_raise(ERR_LIB_ASN1, ERR_R_NESTED_ASN1_ERROR);
    ...
    ret =
    asn1_template_noexp_d2i(val, &p, len, tt, 0, ctx, depth, libctx, propq);
    if (!ret) {
      ERR_raise(ERR_LIB_ASN1, ERR_R_NESTED_ASN1_ERROR);
      return 0;
    }
    ...
    return asn1_template_noexp_d2i(val, in, inlen, tt, opt, ctx, depth, libctx,
                                   propq);
...
static int asn1_template_noexp_d2i(ASN1_VALUE **val, const unsigned char **in,
  ...
  if (flags & ASN1_TFLG_SK_MASK) {
    ...
    while (len > 0) {
      ...
      skfield = NULL;
      if (asn1_item_embed_d2i(&skfield, &p, len, ASN1_ITEM_ptr(tt->item), -1, 0,
                              0, ctx, depth, libctx, propq) <= 0) {
        ERR_raise(ERR_LIB_ASN1, ERR_R_NESTED_ASN1_ERROR);
    ...
    /* IMPLICIT tagging */
    ret = asn1_item_embed_d2i(val, &p, len, ASN1_ITEM_ptr(tt->item), tt->tag,
                              aclass, opt, ctx, depth, libctx, propq);
    if (!ret) {
      ERR_raise(ERR_LIB_ASN1, ERR_R_NESTED_ASN1_ERROR);
      goto err;
    } else if (ret == -1)
    ...
    /* Nothing special */
    ret = asn1_item_embed_d2i(val, &p, len, ASN1_ITEM_ptr(tt->item), -1, 0, opt,
...
static int asn1_d2i_ex_primitive(ASN1_VALUE **pval, const unsigned char **in,
  ...
  if (utype == V_ASN1_ANY) {
    ...
    p = *in;
    ret = asn1_check_tlen(NULL, &utype, &oclass, NULL, NULL, &p, inlen, -1, 0,
                          0, ctx);
    if (!ret) {
      ERR_raise(ERR_LIB_ASN1, ERR_R_NESTED_ASN1_ERROR);
      return 0;
...
static int asn1_collect(BUF_MEM *buf, const unsigned char **in, long len,
  ...
  while (len > 0) {
    ...
    if (!asn1_check_tlen(&plen, NULL, NULL, &ininf, &cst, &p, len, tag, aclass,
                         0, NULL)) {
      ERR_raise(ERR_LIB_ASN1, ERR_R_NESTED_ASN1_ERROR);
      return 0;
    }
