...
static int asn1_print_info(BIO *bp, long offset, int depth, int hl, long len,
  ...
  if (constructed != (V_ASN1_CONSTRUCTED | 1)) {
    if (BIO_snprintf(str, sizeof(str), "%5ld:d=%-2d hl=%ld l=%4ld %s", offset,
                     depth, (long)hl, len, p) <= 0)
      goto err;
  } else {
    if (BIO_snprintf(str, sizeof(str), "%5ld:d=%-2d hl=%ld l=inf  %s", offset,
                     depth, (long)hl, p) <= 0)
      goto err;
  }
  if (bp != NULL) {
...
static int asn1_parse2(BIO *bp, const unsigned char **pp, long length,
  ...
  while (length > 0) {
     ...
     */
    if (!asn1_print_info(bp, (long)offset + (long)(op - *pp), depth, hl, len,
                         tag, xclass, j, (indent) ? depth : 0))
      goto end;
    if (j & V_ASN1_CONSTRUCTED) {
      ...
      if ((j == 0x21) && (len == 0)) {
        ...
        while (p < ep) {
          sp = p;
          r =
          asn1_parse2(bp, &p, tmp, offset + (p - *pp), depth + 1, indent, dump);
          if (r == 0)
            goto end;
          tmp -= p - sp;
        }
