...
static int asn1_print_info(BIO *bp, long offset, int depth, int hl, long len, int tag, int xclass, int constructed,
                           int indent)
{
    char str[128];
    const char *p;
    ...
    while (length > 0) {
        ...
        if (j & V_ASN1_CONSTRUCTED) {
            ...
            if ((tag == V_ASN1_PRINTABLESTRING) || (tag == V_ASN1_T61STRING) || (tag == V_ASN1_IA5STRING)
                || (tag == V_ASN1_VISIBLESTRING) || (tag == V_ASN1_NUMERICSTRING) || (tag == V_ASN1_UTF8STRING)
                ...
                if (os != NULL && os->length > 0) {
                        ...
                        /* print the normal dump */
                    {
                        ...
                        if (BIO_dump_indent(bp, (const char *)opp,
                                            ((dump == -1 || dump > os->length) ? os->length : dump), dump_indent)
                            <= 0)
                            goto end;
                        nl = 1;
                    }
