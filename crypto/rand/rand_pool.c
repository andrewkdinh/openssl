 ...
 */
size_t ossl_rand_pool_entropy(RAND_POOL *pool) { return pool->entropy; }

/*
 * Return the |pool|'s buffer length to the caller.
 */
size_t ossl_rand_pool_length(RAND_POOL *pool) { return pool->len; }
...
size_t ossl_rand_pool_bytes_needed(RAND_POOL *pool,
  ...
  if (bytes_needed > pool->max_len - pool->len) {
    ...
    ERR_raise_data(ERR_LIB_RAND, RAND_R_RANDOM_POOL_OVERFLOW,
                   ...
                   entropy_factor, entropy_needed, bytes_needed, pool->max_len,
                   pool->len);
    return 0;
  }
 ...
 *
 * @return 1 if there is any existing entropy in the pool so the additional
 * input can be mixed in, 0 otherwise.
 */
