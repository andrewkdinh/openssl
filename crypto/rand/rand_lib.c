...
int RAND_poll(void) {
    ...
    if (!ret) {
        ...
        RAND_POOL *pool =
        ossl_rand_pool_new(RAND_DRBG_STRENGTH, 1, (RAND_DRBG_STRENGTH + 7) / 8,
        ...
        if (meth->add == NULL ||
            meth->add(ossl_rand_pool_buffer(pool), ossl_rand_pool_length(pool),
...
int RAND_priv_bytes_ex(OSSL_LIB_CTX *ctx, unsigned char *buf, size_t num,
    ...
    if (dgbl->random_provider != NULL)
        return ossl_provider_random_bytes(
        dgbl->random_provider, OSSL_PROV_RANDOM_PRIVATE, buf, num, strength);
#endif /* !FIPS_MODULE */
...
int RAND_bytes_ex(OSSL_LIB_CTX *ctx, unsigned char *buf, size_t num,
    ...
    if (dgbl->random_provider != NULL)
        return ossl_provider_random_bytes(
        dgbl->random_provider, OSSL_PROV_RANDOM_PUBLIC, buf, num, strength);
#endif /* !FIPS_MODULE */
...
static EVP_RAND_CTX *rand_new_drbg(OSSL_LIB_CTX *libctx, EVP_RAND_CTX *parent,
    ...
    if (OSSL_PARAM_locate_const(settables, OSSL_DRBG_PARAM_CIPHER)) {
        ...
        *p++ =
        OSSL_PARAM_construct_utf8_string(OSSL_DRBG_PARAM_CIPHER, cipher, 0);
    }
...
static EVP_RAND_CTX *rand_new_crngt(OSSL_LIB_CTX *libctx,
                                    EVP_RAND_CTX *parent) {
    EVP_RAND *rand;
    EVP_RAND_CTX *ctx;
    ...
    for (i = 0; i < sk_CONF_VALUE_num(elist); i++) {
        ...
        if (OPENSSL_strcasecmp(cval->name, "random") == 0) {
            ...
            if (prov != NULL) {
                 ...
                 * because we don't want to keep a reference counted handle to
                 * the provider.
                 *
                 * The provider unload code checks for the random provider and,
                 * if present, our reference will be NULLed when it is fully
                 * freed. The provider load code, conversely, checks the
                 * provider name and re-hooks our reference if required.  This
                 * means that a load, hook random provider, use, unload, reload,
                 * reuse sequence will work as expected.
                 */
                OSSL_PROVIDER_unload(prov);
