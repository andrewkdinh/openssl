...
int PKCS7_SIGNER_INFO_sign(PKCS7_SIGNER_INFO *si) {
  ...
  if (EVP_DigestSignInit_ex(
      mctx, &pctx, EVP_MD_get0_name(md), ossl_pkcs7_ctx_get0_libctx(ctx),
      ossl_pkcs7_ctx_get0_propq(ctx), si->pkey, NULL) <= 0)
    goto err;
...
int PKCS7_signatureVerify(BIO *bio, PKCS7 *p7, PKCS7_SIGNER_INFO *si,
  ...
  if ((sk != NULL) && (sk_X509_ATTRIBUTE_num(sk) != 0)) {
    ...
    alen =
    ASN1_item_i2d((ASN1_VALUE *)sk, &abuf, ASN1_ITEM_rptr(PKCS7_ATTR_VERIFY));
    if (alen <= 0 || abuf == NULL) {
      ERR_raise(ERR_LIB_PKCS7, ERR_R_ASN1_LIB);
      ret = -1;
      goto err;
...
int PKCS7_set_signed_attributes(PKCS7_SIGNER_INFO *p7si,
  ...
  p7si->auth_attr =
  sk_X509_ATTRIBUTE_deep_copy(sk, X509_ATTRIBUTE_dup, X509_ATTRIBUTE_free);
