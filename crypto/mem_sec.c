...
static int sh_init(size_t size, size_t minsize) {
...
#else
    {
        ...
        if ((fd = open("/dev/zero", O_RDWR)) >= 0) {
            sh.map_result =
            mmap(NULL, sh.map_size, PROT_READ | PROT_WRITE, MAP_PRIVATE, fd, 0);
            close(fd);
        }
    }
#endif
...
#else
    sh.map_result =
    VirtualAlloc(NULL, sh.map_size, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
    ...
    if (mprotect(sh.map_result, pgsize, PROT_NONE) < 0)
        ret = 2;
#else
    if (VirtualProtect(sh.map_result, pgsize, PAGE_NOACCESS, &flOldProtect) ==
        FALSE)
        ret = 2;
#endif
...
static int sh_allocated(const char *ptr) { return WITHIN_ARENA(ptr) ? 1 : 0; }
