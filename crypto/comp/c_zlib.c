...
static void zlib_zfree(void *opaque, void *address) { OPENSSL_free(address); }

static COMP_METHOD zlib_stateful_method = {
NID_zlib_compression, LN_zlib_compression,          zlib_stateful_init,
zlib_stateful_finish, zlib_stateful_compress_block, zlib_stateful_expand_block};
...
typedef int (*compress_ft)(Bytef *dest, uLongf *destLen, const Bytef *source,
                           uLong sourceLen);
typedef int (*uncompress_ft)(Bytef *dest, uLongf *destLen, const Bytef *source,
                             uLong sourceLen);
...
static int zlib_oneshot_init(COMP_CTX *ctx) { return 1; }
...
NID_zlib_compression, LN_zlib_compression,         zlib_oneshot_init,
...
#define LIBZ "z"
  ...
  if (zlib_dso != NULL) {
    ...
    if (p_compress == NULL || p_uncompress == NULL || p_inflateEnd == NULL ||
        p_inflate == NULL || p_inflateInit_ == NULL || p_deflateEnd == NULL ||
        p_deflate == NULL || p_deflateInit_ == NULL || p_zError == NULL) {
...
BIO_TYPE_COMP, "zlib",        bwrite_conv,   bio_zlib_write,
...
static int bio_zlib_read(BIO *b, char *out, int outl) {
  ...
  if (!ctx->ibuf) {
    ...
    if ((ret = inflateInit(zin)) != Z_OK) {
      ERR_raise_data(ERR_LIB_COMP, COMP_R_ZLIB_INFLATE_ERROR, "zlib error: %s",
                     zError(ret));
      return 0;
    }
...
static int bio_zlib_write(BIO *b, const char *in, int inl) {
  ...
  if (!ctx->obuf) {
    ...
    if ((ret = deflateInit(zout, ctx->comp_level)) != Z_OK) {
      ERR_raise_data(ERR_LIB_COMP, COMP_R_ZLIB_DEFLATE_ERROR, "zlib error: %s",
                     zError(ret));
      return 0;
    }
