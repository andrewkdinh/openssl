                                        ...
                                        unsigned char *buf, int idx, int window)
{
    ...
    if (top > b->top)
        top = b->top;           /* this works because 'buf' is explicitly
                                 * zeroed */
    for (i = 0, j = idx; i < top; i++, j += width) { table[j] = b->d[i]; }
                              ...
                              BN_MONT_CTX *in_mont)
{
    ...
    if (t4) {
         ...
         */
        for (np = am.d + top, i = 0; i < top; i++) np[i] = mont->N.d[i];
                                 ...
                                 BN_CTX *ctx)
{
    ...
    if ((ossl_rsaz_avx512ifma_eligible() || ossl_rsaz_avxifma_eligible())
        ...
        ret = ossl_rsaz_mod_exp_avx512_x2(
            rr1->d, a1->d, p1->d, m1->d, mont1->RR.d, mont1->n0[0], rr2->d,
            a2->d, p2->d, m2->d, mont2->RR.d, mont2->n0[0], mod_bits);
