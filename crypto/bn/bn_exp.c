...
static int MOD_EXP_CTIME_COPY_FROM_PREBUF(BIGNUM *b, int top,
    ...
    if (window <= 3) {
        for (i = 0; i < top; i++, table += width) {
            ...
            for (j = 0; j < width; j++) {
                acc |=
                table[j] & ((BN_ULONG)0 - (constant_time_eq_int(j, idx) & 1));
            }
        ...
        for (i = 0; i < top; i++, table += width) {
            ...
            for (j = 0; j < xstride; j++) {
                acc |=
                ((table[j + 0 * xstride] & y0) | (table[j + 1 * xstride] & y1) |
...
int BN_mod_exp_mont_consttime_x2(BIGNUM *rr1, const BIGNUM *a1,
    ...
    if ((ossl_rsaz_avx512ifma_eligible() || ossl_rsaz_avxifma_eligible()) &&
        ...
        rr1->d, a1->d, p1->d, m1->d, mont1->RR.d, mont1->n0[0], rr2->d, a2->d,
        p2->d, m2->d, mont2->RR.d, mont2->n0[0], mod_bits);
