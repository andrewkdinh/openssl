...
static int bnrand_range(BNRAND_FLAG flag, BIGNUM *r, const BIGNUM *range,
  ...
  if (n == 1)
     ...
     */
    do {
      if (!bnrand(flag, r, n + 1, BN_RAND_TOP_ANY, BN_RAND_BOTTOM_ANY, strength,
                  ctx))
        return 0;
    ...
    do {
      ...
      if (!bnrand(flag, r, n, BN_RAND_TOP_ANY, BN_RAND_BOTTOM_ANY, strength,
                  ctx))
        return 0;
...
int BN_generate_dsa_nonce(BIGNUM *out, const BIGNUM *range, const BIGNUM *priv,
  ...
  ret =
  ossl_bn_gen_dsa_nonce_fixed_top(out, range, priv, message, message_len, ctx);
  /*
