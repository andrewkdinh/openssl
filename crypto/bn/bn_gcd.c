 ...
 */
static ossl_inline BIGNUM *bn_mod_inverse_no_branch(BIGNUM *in, const BIGNUM *a,
                                                    const BIGNUM *n,
...
int BN_gcd(BIGNUM *r, const BIGNUM *in_a, const BIGNUM *in_b, BN_CTX *ctx) {
  ...
  for (i = 0; i < r->dmax && i < g->dmax; i++) {
    ...
    pow2_numbits = constant_time_select_bn(pow2_condition_mask, pow2_numbits,
                                           pow2_numbits_temp);
  }
  ...
  if (bn_wexpand(r, top) == NULL || bn_wexpand(g, top) == NULL ||
