...
int ossl_bn_rsa_fips186_4_gen_prob_primes(BIGNUM *p, BIGNUM *Xpout, BIGNUM *p1,
  ...
  if (Xp1 == NULL) {
    ...
    if (!BN_priv_rand_ex(Xp1i, bitlen, BN_RAND_TOP_ONE, BN_RAND_BOTTOM_ODD, 0,
                         ctx))
      goto err;
  }
  ...
  if (Xp2 == NULL) {
    /* Set the top and bottom bits to make it odd and the correct size */
    if (!BN_priv_rand_ex(Xp2i, bitlen, BN_RAND_TOP_ONE, BN_RAND_BOTTOM_ODD, 0,
                         ctx))
      goto err;
  }
  ...
  if (!ossl_bn_rsa_fips186_4_derive_prime(p, Xpout, Xp, p1i, p2i, nlen, e, ctx,
                                          cb))
    goto err;
  ret = 1;
err:
 ...
 * Primes". Used by FIPS 186-4 B.3.6 Section (4.3) for p and Section (5.3) for
 * q.
 *
 * Params:
