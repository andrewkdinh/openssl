 ...
 * radix. Exponent is not converted to redundant representation. [in]  m -
 * moduli (2x{20,30,40} qword values in 2^52 radix) [in]  rr       - Montgomery
 * parameter for 2 moduli: RR(1024) = 2^2080 mod m. RR(1536) = 2^3120 mod m.
...
#define DAMS(r, a, m, k0) damm((r), (a), (a), (m), (k0))
  ...
  extract =
  ossl_extract_multiplier_win5[(modulus_bitsize / 512 - 2) * 2 + avx512ifma];
  ...
  /* Exponentiation */
  {
    ...
    for (exp_bit_no -= exp_win_size; exp_bit_no >= 0;
      ...
      damm((BN_ULONG *)red_Y, (const BN_ULONG *)red_Y, (const BN_ULONG *)red_X,
           m, k0);
    }
  }
