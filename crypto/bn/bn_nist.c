...
const BIGNUM *BN_get0_nist_prime_192(void) { return &ossl_bignum_nist_p_192; }

const BIGNUM *BN_get0_nist_prime_224(void) { return &ossl_bignum_nist_p_224; }

const BIGNUM *BN_get0_nist_prime_256(void) { return &ossl_bignum_nist_p_256; }

const BIGNUM *BN_get0_nist_prime_384(void) { return &ossl_bignum_nist_p_384; }

const BIGNUM *BN_get0_nist_prime_521(void) { return &ossl_bignum_nist_p_521; }
...
int BN_nist_mod_192(BIGNUM *r, const BIGNUM *a, const BIGNUM *field,
  ...
  if (carry > 0)
    carry =
    (int)bn_sub_words(r_d, r_d, _nist_p_192[carry - 1], BN_NIST_192_TOP);
  else
    carry = 1;
...
int BN_nist_mod_224(BIGNUM *r, const BIGNUM *a, const BIGNUM *field,
  ...
  if (carry > 0) {
    carry =
    (int)bn_sub_words(r_d, r_d, _nist_p_224[carry - 1], BN_NIST_224_TOP);
#if BN_BITS2 == 64
  ...
  res =
  ((*adjust)(c_d, r_d, _nist_p_224[0], BN_NIST_224_TOP) && carry) ? r_d : c_d;
...
#define nist_set_256(to, from, a1, a2, a3, a4, a5, a6, a7, a8)                 \
  {bn_cp_32(to, 0, from, (a8) - 8) bn_cp_32(to, 1, from, (a7) - 8)             \
   ...
   bn_cp_32(to, 6, from, (a2) - 8) bn_cp_32(to, 7, from, (a1) - 8)}
...
int BN_nist_mod_256(BIGNUM *r, const BIGNUM *a, const BIGNUM *field,
  ...
  if (carry > 0)
    carry =
    (int)bn_sub_words(r_d, r_d, _nist_p_256[carry - 1], BN_NIST_256_TOP);
  else if (carry < 0) {
    carry =
    (int)bn_add_words(r_d, r_d, _nist_p_256[-carry - 1], BN_NIST_256_TOP);
    adjust = carry ? bn_sub_words : bn_add_words;
  } else
    carry = 1;

  res =
  ((*adjust)(c_d, r_d, _nist_p_256[0], BN_NIST_256_TOP) && carry) ? r_d : c_d;
...
#define nist_set_384(to, from, a1, a2, a3, a4, a5, a6, a7, a8, a9, a10, a11,   \
  ...
  {bn_cp_32(to, 0, from, (a12) - 12) bn_cp_32(to, 1, from, (a11) - 12)         \
   ...
   bn_cp_32(to, 6, from, (a6) - 12) bn_cp_32(to, 7, from, (a5) - 12)           \
   bn_cp_32(to, 8, from, (a4) - 12) bn_cp_32(to, 9, from, (a3) - 12)           \
   bn_cp_32(to, 10, from, (a2) - 12) bn_cp_32(to, 11, from, (a1) - 12)}
...
int BN_nist_mod_384(BIGNUM *r, const BIGNUM *a, const BIGNUM *field,
  ...
  if (carry > 0)
    carry =
    (int)bn_sub_words(r_d, r_d, _nist_p_384[carry - 1], BN_NIST_384_TOP);
  else if (carry < 0) {
    carry =
    (int)bn_add_words(r_d, r_d, _nist_p_384[-carry - 1], BN_NIST_384_TOP);
    adjust = carry ? bn_sub_words : bn_add_words;
  } else
    carry = 1;

  res =
  ((*adjust)(c_d, r_d, _nist_p_384[0], BN_NIST_384_TOP) && carry) ? r_d : c_d;
...
#else
    t_d[i] =
    (val >> BN_NIST_521_RSHIFT | (tmp = t_d[i + 1]) << BN_NIST_521_LSHIFT) &
    BN_MASK2;
    val = tmp;
#endif
  }
