 ...
 * (but since r0 is kept positive it effectively adds q and then reduces by q if
 * needed) Similarly for the range r = 8192..8192+4096 r1=1 and r0=0..4096 &
 ...
 */
void ossl_ml_dsa_key_compress_power2_round(uint32_t r, uint32_t *r1,
                                           uint32_t *r0) {
    unsigned int mask;
 ...
 * @param gamma2 Depending on the algorithm gamma2 is either (q-1)/32 or
 * (q-1)/88
 ...
 * But z + r is just w - cs2, so this takes three arguments and saves an
 addition.
 *
 * @params ct0 A polynomial c (with coefficients of (-1,0,1)) multiplied by the
 *             polynomial vector t0 (which encodes the least significant bits of
 each coefficient of the uncompressed public-key polynomial t)
 * @params cs2 A polynomial c (with coefficients of (-1,0,1)) multiplied by s2
 (a secret polynomial)
 * @params gamma2 Depending on the algorithm gamma2 is either (q-1)/32 or
 (q-1)/88
 * @params w  (A * y)
