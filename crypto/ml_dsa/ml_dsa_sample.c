 ...
 *
 * This is not constant time as it is used to generate the matrix A which is
 * public.
 *
...
static ossl_inline int coeff_from_three_bytes(const uint8_t *s, uint32_t *out) {
    /* Zero out the top bit of the 3rd byte to get a value in the range
     * 0..2^23-1)
     */
    *out =
    (uint32_t)s[0] | ((uint32_t)s[1] << 8) | (((uint32_t)s[2] & 0x7f) << 16);
    return *out < ML_DSA_Q;
}
 ...
 * This algorithm is used for matrix expansion and only operates on public
 * inputs.
 *
 ...
 *                         nibble and tries to see if it is in the correct
 * range.
 * @param seed The seed to use for sampling.
 * @param seed_len The size of |seed|
 * @param out The returned polynomial with coefficients in the range of
...
int ossl_ml_dsa_matrix_expand_A(EVP_MD_CTX *g_ctx, const EVP_MD *md,
    ...
    for (i = 0; i < out->k; i++) {
        for (j = 0; j < out->l; j++) {
            ...
            /* Generate the polynomial for each matrix element using a unique
             * seed */
            if (!rej_ntt_poly(g_ctx, md, derived_seed, sizeof(derived_seed),
                              poly++))
                goto err;
        }
    }
...
int ossl_ml_dsa_poly_expand_mask(POLY *out, const uint8_t *seed,
                                 size_t seed_len, uint32_t gamma1,
 ...
 *
 * @param tau is the number of +1 or -1's in the polynomial 'out_c' (39, 49 or
 * 60) that is less than or equal to 64
 */
int ossl_ml_dsa_poly_sample_in_ball(POLY *out_c, const uint8_t *seed,
                                    int seed_len, EVP_MD_CTX *h_ctx,
