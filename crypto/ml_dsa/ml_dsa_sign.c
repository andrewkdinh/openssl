 ...
 */
static void signature_init(ML_DSA_SIG *sig, POLY *hint, uint32_t k, POLY *z, uint32_t l, uint8_t *c_tilde,
                           size_t c_tilde_len)
{
...
EVP_MD_CTX *ossl_ml_dsa_mu_init(const ML_DSA_KEY *key, int encode, const uint8_t *ctx, size_t ctx_len)
{
    EVP_MD_CTX *md_ctx;
    uint8_t itb[2];
 ...
 * @returns 1 on success, 0 on error
 */
static int ml_dsa_sign_internal(const ML_DSA_KEY *priv, const uint8_t *mu, size_t mu_len, const uint8_t *rnd,
                                size_t rnd_len, uint8_t *out_sig)
{
    ...
    alloc_len = w1_encoded_len + sizeof(*p) * (1 + num_polys_k + num_polys_l + num_polys_k_by_l + num_polys_sig_k);
    ...
    if (!shake_xof_3(md_ctx, priv->shake256_md, priv->K, sizeof(priv->K), rnd, rnd_len, mu, mu_len, rho_prime,
                     sizeof(rho_prime)))
        goto err;
    ...
    for (kappa = 0;; kappa += l) {
        ...
        vector_expand_mask(&y, rho_prime, sizeof(rho_prime), kappa, gamma1, md_ctx, priv->shake256_md);
        vector_copy(y_ntt, &y);
        vector_ntt(y_ntt);
        ...
        if (!shake_xof_2(md_ctx, priv->shake256_md, mu, mu_len, w1_encoded, w1_encoded_len, c_tilde, c_tilde_len))
            break;

        if (!poly_sample_in_ball_ntt(c_ntt, c_tilde, c_tilde_len, md_ctx, priv->shake256_md, params->tau))
            break;
        ...
        if (value_barrier_32(constant_time_ge(ct0_max, gamma2) | constant_time_lt(params->omega, h_ones)))
            continue;
 ...
 * @returns 1 on success, 0 on error
 */
static int ml_dsa_verify_internal(const ML_DSA_KEY *pub, const uint8_t *mu, size_t mu_len, const uint8_t *sig_enc,
                                  size_t sig_enc_len)
{
    int ret = 0;
    uint8_t *alloc = NULL, *w1_encoded;
    ...
    if (!poly_sample_in_ball_ntt(c_ntt, c_tilde_sig, c_tilde_len, md_ctx, pub->shake256_md, params->tau))
        goto err;
    ...
    if (!shake_xof_3(md_ctx, pub->shake256_md, mu, mu_len, w1_encoded, w1_encoded_len, NULL, 0, c_tilde, c_tilde_len))
        goto err;

    ret = (z_max < (uint32_t)(params->gamma1 - params->beta)) && memcmp(c_tilde, sig.c_tilde, c_tilde_len) == 0;
err:
    OPENSSL_free(alloc);
    EVP_MD_CTX_free(md_ctx);
    return ret;
 ...
 */
int ossl_ml_dsa_sign(const ML_DSA_KEY *priv, int msg_is_mu, const uint8_t *msg, size_t msg_len, const uint8_t *context,
                     size_t context_len, const uint8_t *rand, size_t rand_len, int encode, unsigned char *sig,
 ...
 * @returns 1 on success, or 0 on error.
 */
int ossl_ml_dsa_verify(const ML_DSA_KEY *pub, int msg_is_mu, const uint8_t *msg, size_t msg_len, const uint8_t *context,
                       size_t context_len, int encode, const uint8_t *sig, size_t sig_len)
{
    EVP_MD_CTX *md_ctx = NULL;
    uint8_t mu[ML_DSA_MU_BYTES];
    const uint8_t *mu_ptr = mu;
