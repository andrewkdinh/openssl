...
static int ml_dsa_sign_internal(const ML_DSA_KEY *priv, const uint8_t *mu,
    ...
    alloc_len =
    w1_encoded_len + sizeof(*p) * (1 + num_polys_k + num_polys_l +
    ...
    for (kappa = 0;; kappa += l) {
        ...
        /*
         * Leaking that the signature is rejected is fine as the next attempt at
         * a signature will be (indistinguishable from) independent of this one.
         */
