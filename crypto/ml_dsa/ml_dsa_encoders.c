...
static int poly_decode_signed_4(POLY *p, PACKET *pkt) {
    ...
    for (i = 0; i < (ML_DSA_NUM_POLY_COEFFICIENTS / 8); i++) {
         ...
         */
        msbs = v & 0x88888888u;
        /* For each nibble where the MSB is set, form a mask of all the other
         * bits.
         */
...
static int poly_decode_signed_2(POLY *p, PACKET *pkt) {
    ...
    for (i = 0; i < (ML_DSA_NUM_POLY_COEFFICIENTS / 8); i++) {
         ...
         */
        msbs = v & 044444444;
        /* For each octal value where the MSB is set, form a mask of the 2 other
         * bits. */
        mask = (msbs >> 1) | (msbs >> 2);
        /*
         * A nibble is only out of range in the case of invalid input, in which
 ...
 * @param gamma2 either ML_DSA_GAMMA2_Q_MINUS1_DIV32 or
