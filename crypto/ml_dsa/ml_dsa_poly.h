 ...
 *            range 0..q-1
 */
static ossl_inline ossl_unused void poly_add(const POLY *lhs, const POLY *rhs, POLY *out)
{
    int i;
 ...
 *            range 0..q-1
 */
static ossl_inline ossl_unused void poly_sub(const POLY *lhs, const POLY *rhs, POLY *out)
{
    int i;
...
static ossl_inline ossl_unused int poly_sample_in_ball_ntt(POLY *out, const uint8_t *seed, int seed_len,
...
static ossl_inline ossl_unused int poly_expand_mask(POLY *out, const uint8_t *seed, size_t seed_len, uint32_t gamma1,
...
static ossl_inline ossl_unused void poly_high_bits(const POLY *in, uint32_t gamma2, POLY *out)
{
    int i;
...
static ossl_inline ossl_unused void poly_low_bits(const POLY *in, uint32_t gamma2, POLY *out)
{
    int i;
...
static ossl_inline ossl_unused void poly_make_hint(const POLY *ct0, const POLY *cs2, const POLY *w, uint32_t gamma2,
                                                   POLY *out)
{
    int i;

    for (i = 0; i < ML_DSA_NUM_POLY_COEFFICIENTS; i++)
        out->coeff[i] = ossl_ml_dsa_key_compress_make_hint(ct0->coeff[i], cs2->coeff[i], gamma2, w->coeff[i]);
}

static ossl_inline ossl_unused void poly_use_hint(const POLY *h, const POLY *r, uint32_t gamma2, POLY *out)
{
    int i;
