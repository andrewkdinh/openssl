...
/* Polynomial object with 256 coefficients. The coefficients are unsigned 32
 * bits */
struct poly_st {
 ...
 *           the top 10 MSB of each coefficient in t (i.e each ranging from
 * 0..1023)
 * @param t0 The remainder coefficients of t in the range (0..4096 or
 * q-4095..q-1) Each t0 coefficient has an effective range of 8192 (i.e. 13
 * bits).
 */
...
static ossl_inline ossl_unused void poly_scale_power2_round(POLY *in,
                                                            POLY *out) {
    int i;
...
static ossl_inline ossl_unused void poly_max_signed(const POLY *p,
                                                    uint32_t *mx) {
    int i;
