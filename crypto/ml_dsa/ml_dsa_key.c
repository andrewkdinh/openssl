  ...
  if (ret != NULL) {
    ...
    if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
      if (src->pub_encoding != NULL) {
        ...
        if ((ret->pub_encoding =
             OPENSSL_memdup(src->pub_encoding, src->params->pk_len)) == NULL)
          goto err;
      }
      if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0) {
        if (src->priv_encoding != NULL) {
          ...
          if ((ret->priv_encoding =
               OPENSSL_memdup(src->priv_encoding, src->params->sk_len)) == NULL)
            goto err;
        }
        if (src->seed != NULL &&
            (ret->seed = OPENSSL_memdup(src->seed, ML_DSA_SEED_BYTES)) == NULL)
          goto err;
      }
    }
...
static int keygen_internal(ML_DSA_KEY *out) {
  ...
  if (!shake_xof(md_ctx, out->shake256_md, augmented_seed,
                 sizeof(augmented_seed), expanded_seed, sizeof(expanded_seed)))
    goto err;
