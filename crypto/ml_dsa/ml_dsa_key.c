...
ML_DSA_KEY *ossl_ml_dsa_key_dup(const ML_DSA_KEY *src, int selection) {
    ...
    if (ret != NULL) {
        ...
        if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
            if (src->pub_encoding != NULL) {
                /* The public components are present if the private key is
                 * present */
            ...
            if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0) {
                if (src->priv_encoding != NULL) {
                    ...
                    if ((ret->priv_encoding = OPENSSL_memdup(
                         src->priv_encoding, src->params->sk_len)) == NULL)
                        goto err;
                }
                if (src->seed != NULL &&
                    (ret->seed =
                     OPENSSL_memdup(src->seed, ML_DSA_SEED_BYTES)) == NULL)
                    goto err;
            }
        }
 ...
 * (Note that in OpenSSL that the private key always has a public key
 * component).
 *
...
int ossl_ml_dsa_key_public_from_private(ML_DSA_KEY *key) {
    ...
    if (!vector_alloc(&t0,
                      key->params->k)) /* t0 is already in the private key */
        return 0;
    ret =
    ...
    shake_xof(md_ctx, key->shake256_md, key->pub_encoding, key->params->pk_len,
...
static int keygen_internal(ML_DSA_KEY *out) {
    ...
    uint8_t
    expanded_seed[ML_DSA_RHO_BYTES + ML_DSA_PRIV_SEED_BYTES + ML_DSA_K_BYTES];
