 ...
 */
static ossl_inline ossl_unused void vector_copy(VECTOR *dst,
                                                const VECTOR *src) {
...
vector_expand_mask(VECTOR *out, const uint8_t *rho_prime, size_t rho_prime_len,
  ...
  for (i = 0; i < out->num_poly; i++) {
    ...
    poly_expand_mask(out->poly + i, derived_seed, sizeof(derived_seed), gamma1,
                     h_ctx, md);
  }
}
...
static ossl_inline ossl_unused void vector_use_hint(const VECTOR *h,
                                                    const VECTOR *r,
                                                    uint32_t gamma2,
                                                    VECTOR *out) {
  size_t i;
