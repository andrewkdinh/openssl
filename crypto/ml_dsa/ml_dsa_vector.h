...
static ossl_inline ossl_unused int vector_equal(const VECTOR *a, const VECTOR *b)
{
    size_t i;
...
/* @brief add 2 vectors */
static ossl_inline ossl_unused void vector_add(const VECTOR *lhs, const VECTOR *rhs, VECTOR *out)
{
    size_t i;
...
static ossl_inline ossl_unused void vector_sub(const VECTOR *lhs, const VECTOR *rhs, VECTOR *out)
{
    size_t i;
...
static ossl_inline ossl_unused void vector_mult_scalar(const VECTOR *lhs, const POLY *rhs, VECTOR *out)
{
    size_t i;
...
static ossl_inline ossl_unused int vector_expand_S(EVP_MD_CTX *h_ctx, const EVP_MD *md, int eta, const uint8_t *seed,
                                                   VECTOR *s1, VECTOR *s2)
{
    return ossl_ml_dsa_vector_expand_S(h_ctx, md, eta, seed, s1, s2);
}

static ossl_inline ossl_unused void vector_expand_mask(VECTOR *out, const uint8_t *rho_prime, size_t rho_prime_len,
                                                       ...
                                                       const EVP_MD *md)
{
    ...
    for (i = 0; i < out->num_poly; i++) {
        ...
        poly_expand_mask(out->poly + i, derived_seed, sizeof(derived_seed), gamma1, h_ctx, md);
    }
}

/* Scale back previously rounded value */
static ossl_inline ossl_unused void vector_scale_power2_round_ntt(const VECTOR *in, VECTOR *out)
{
    size_t i;
 ...
 */
static ossl_inline ossl_unused void vector_power2_round(const VECTOR *t, VECTOR *t1, VECTOR *t0)
{
    size_t i;
...
static ossl_inline ossl_unused void vector_high_bits(const VECTOR *in, uint32_t gamma2, VECTOR *out)
{
    size_t i;
...
static ossl_inline ossl_unused void vector_low_bits(const VECTOR *in, uint32_t gamma2, VECTOR *out)
{
    size_t i;
...
static ossl_inline ossl_unused void vector_make_hint(const VECTOR *ct0, const VECTOR *cs2, const VECTOR *w,
...
static ossl_inline ossl_unused void vector_use_hint(const VECTOR *h, const VECTOR *r, uint32_t gamma2, VECTOR *out)
{
    size_t i;
