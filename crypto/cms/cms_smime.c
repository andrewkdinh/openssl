...
int CMS_verify(CMS_ContentInfo *cms, STACK_OF(X509) * certs, X509_STORE *store,
    ...
    if ((flags & CMS_NO_SIGNER_CERT_VERIFY) == 0 || cadesVerify) {
        ...
        if (sk_X509_num(certs) > 0 &&
            ...
            &untrusted, certs, X509_ADD_FLAG_UP_REF | X509_ADD_FLAG_NO_DUP))
            goto err;
...
CMS_ContentInfo *CMS_sign_receipt(CMS_SignerInfo *si, X509 *signcert,
    ...
    cms =
    CMS_sign_ex(NULL, NULL, certs, NULL, flags, ossl_cms_ctx_get0_libctx(ctx),
    ...
    for (i = 0; i < sk_CMS_RecipientInfo_num(ris); i++) {
        ...
        /* If we have a cert, try matching RecipientInfo, else try them all */
             ...
             */
                     ...
                     (debug || cms_pkey_ri_type != CMS_RECIPINFO_TRANS))
                return 1;
        }
    }
