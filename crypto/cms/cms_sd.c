...
CMS_SignerInfo *CMS_add1_signer(CMS_ContentInfo *cms, X509 *signer,
    ...
    if (!(flags & CMS_NOATTR)) {
        ...
        if (flags & CMS_CADES) {
            ...
            if (md == NULL || EVP_MD_is_a(md, SN_sha1)) {
                if ((sc = OSSL_ESS_signing_cert_new_init(signer, NULL, 1)) ==
                    NULL)
                    goto err;
                add_sc = ossl_cms_add1_signing_cert(si, sc);
                ESS_SIGNING_CERT_free(sc);
            } else {
                if ((sc2 = OSSL_ESS_signing_cert_v2_new_init(md, signer, NULL,
                                                             1)) == NULL)
                    goto err;
    ...
    if (flags & CMS_KEY_PARAM) {
        if (flags & CMS_NOATTR) {
            si->pctx =
            EVP_PKEY_CTX_new_from_pkey(ossl_cms_ctx_get0_libctx(ctx), si->pkey,
            ...
            if (EVP_PKEY_CTX_set_signature_md(si->pctx, md) <= 0)
                goto err;
                   ...
                   si->mctx, &si->pctx, EVP_MD_get0_name(md),
