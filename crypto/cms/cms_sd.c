    ...
    if (!(flags & CMS_NOATTR)) {
        ...
        if (flags & CMS_CADES) {
            ...
            if (md == NULL || EVP_MD_is_a(md, SN_sha1)) {
                ...
                if ((sc2 =
                         OSSL_ESS_signing_cert_v2_new_init(md, signer, NULL, 1))
                    == NULL)
                    goto err;
...
static int cms_add1_signingTime(CMS_SignerInfo *si, ASN1_TIME *t)
{
    ...
    if (CMS_signed_add1_attr_by_NID(si, NID_pkcs9_signingTime, tt->type, tt, -1)
        <= 0) {
        ERR_raise(ERR_LIB_CMS, ERR_R_CMS_LIB);
        goto err;
    }
...
int CMS_SignerInfo_sign(CMS_SignerInfo *si)
{
    ...
    if (si->pctx) {
        ...
        if (EVP_DigestSignInit_ex(mctx, &pctx, md_name,
                                  ossl_cms_ctx_get0_libctx(ctx),
                                  ossl_cms_ctx_get0_propq(ctx), si->pkey, NULL)
            <= 0)
            goto err;
    ...
    if (md_name == NULL) {
        if (ASN1_item_sign_ctx(ASN1_ITEM_rptr(CMS_Attributes_Sign), NULL, NULL,
                               si->signature, si->signedAttrs, mctx)
            <= 0)
            goto err;
        return 1;
    }
