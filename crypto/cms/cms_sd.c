...
static int cms_copy_messageDigest(CMS_ContentInfo *cms, CMS_SignerInfo *si)
{
    ...
    for (i = 0; i < sk_CMS_SignerInfo_num(sinfos); i++) {
        ...
        if (OBJ_cmp(si->digestAlgorithm->algorithm, sitmp->digestAlgorithm->algorithm))
            continue;
        messageDigest =
            CMS_signed_get0_data_by_OBJ(sitmp, OBJ_nid2obj(NID_pkcs9_messageDigest), -3, V_ASN1_OCTET_STRING);
        if (!messageDigest) {
...
int ossl_cms_SignerIdentifier_get0_signer_id(CMS_SignerIdentifier *sid, ASN1_OCTET_STRING **keyid, X509_NAME **issuer,
                                             ASN1_INTEGER **sno)
{
...
static int cms_signature_nomd(EVP_PKEY *pkey)
{
    ...
    return EVP_PKEY_get_default_digest_name(pkey, def_md, sizeof(def_md)) == 2 && strcmp(def_md, "UNDEF") == 0 ? 1 : 0;
}
...
CMS_SignerInfo *CMS_add1_signer(CMS_ContentInfo *cms, X509 *signer, EVP_PKEY *pk, const EVP_MD *md, unsigned int flags)
{
    ...
    if (flags & CMS_KEY_PARAM) {
        if (flags & CMS_NOATTR) {
            si->pctx =
                EVP_PKEY_CTX_new_from_pkey(ossl_cms_ctx_get0_libctx(ctx), si->pkey, ossl_cms_ctx_get0_propq(ctx));
            if (si->pctx == NULL)
                goto err;
            ...
            if (EVP_PKEY_CTX_set_signature_md(si->pctx, md) <= 0)
                goto err;
        } else if (EVP_DigestSignInit_ex(si->mctx, &si->pctx, EVP_MD_get0_name(md), ossl_cms_ctx_get0_libctx(ctx),
                                         ossl_cms_ctx_get0_propq(ctx), pk, NULL)
                   <= 0) {
            si->pctx = NULL;
            goto err;
        } else {
...
static int cms_SignerInfo_content_sign(CMS_ContentInfo *cms, CMS_SignerInfo *si, BIO *chain, const unsigned char *md,
                                       unsigned int mdlen)
{
    ...
    if (CMS_signed_get_attr_count(si) >= 0) {
        ...
        if (siglen == 0 || (sig = OPENSSL_malloc(siglen)) == NULL)
            goto err;
        if (!EVP_SignFinal_ex(mctx, sig, &siglen, si->pkey, ossl_cms_ctx_get0_libctx(ctx),
...
int CMS_SignerInfo_sign(CMS_SignerInfo *si)
{
    ...
    if (OBJ_obj2txt(md_name_buf, sizeof(md_name_buf), si->digestAlgorithm->algorithm, 0) <= 0)
        return 0;
    ...
    if (si->pctx) {
        ...
        if (EVP_DigestSignInit_ex(mctx, &pctx, md_name, ossl_cms_ctx_get0_libctx(ctx), ossl_cms_ctx_get0_propq(ctx),
                                  si->pkey, NULL)
            <= 0)
            goto err;
    ...
    if (md_name == NULL) {
        if (ASN1_item_sign_ctx(ASN1_ITEM_rptr(CMS_Attributes_Sign), NULL, NULL, si->signature, si->signedAttrs, mctx)
            <= 0)
            goto err;
        return 1;
    }
...
int CMS_SignerInfo_verify(CMS_SignerInfo *si)
{
    ...
    if (cms_signature_nomd(si->pkey)) {
        r = ASN1_item_verify_ex(ASN1_ITEM_rptr(CMS_Attributes_Sign), si->signatureAlgorithm, si->signature,
                                si->signedAttrs, NULL, si->pkey, libctx, propq);
        if (r <= 0)
...
BIO *CMS_SignedData_verify(CMS_SignedData *sd, BIO *detached_data, STACK_OF(X509) *scerts, X509_STORE *store,
                           STACK_OF(X509) *extra, STACK_OF(X509_CRL) *crls, unsigned int flags, OSSL_LIB_CTX *libctx,
                           const char *propq)
{
    CMS_ContentInfo *ci;
    BIO *bio = NULL;
...
int CMS_add_standard_smimecap(STACK_OF(X509_ALGOR) **smcap)
{
    if (!cms_add_cipher_smcap(smcap, NID_aes_256_cbc, -1) || !cms_add_digest_smcap(smcap, NID_id_GostR3411_2012_256, -1)
        ...
        || !cms_add_cipher_smcap(smcap, NID_rc2_cbc, 128) || !cms_add_cipher_smcap(smcap, NID_rc2_cbc, 64)
        || !cms_add_cipher_smcap(smcap, NID_des_cbc, -1) || !cms_add_cipher_smcap(smcap, NID_rc2_cbc, 40))
        return 0;
    return 1;
}
