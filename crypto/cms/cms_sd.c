 ...
 */
static int cms_set_si_contentType_attr(CMS_ContentInfo *cms,
                                       CMS_SignerInfo *si) {
  ASN1_OBJECT *ctype = cms->d.signedData->encapContentInfo->eContentType;

  /* Add the contentType attribute */
  return CMS_signed_add1_attr_by_NID(si, NID_pkcs9_contentType, V_ASN1_OBJECT,
                                     ctype, -1) > 0;
}
...
static int cms_copy_messageDigest(CMS_ContentInfo *cms, CMS_SignerInfo *si) {
  ...
  for (i = 0; i < sk_CMS_SignerInfo_num(sinfos); i++) {
    ...
    sitmp, OBJ_nid2obj(NID_pkcs9_messageDigest), -3, V_ASN1_OCTET_STRING);
    if (!messageDigest) {
...
CMS_SignerInfo *CMS_add1_signer(CMS_ContentInfo *cms, X509 *signer,
  ...
  if (md == NULL) {
    ...
    if (EVP_PKEY_get_default_digest_nid(pk, &def_nid) <= 0) {
      ERR_raise_data(ERR_LIB_CMS, CMS_R_NO_DEFAULT_DIGEST, "pkey nid=%d",
                     EVP_PKEY_get_id(pk));
      goto err;
    }
    md = EVP_get_digestbynid(def_nid);
    if (md == NULL) {
      ERR_raise_data(ERR_LIB_CMS, CMS_R_NO_DEFAULT_DIGEST, "default md nid=%d",
                     def_nid);
      goto err;
    }
  }
  ...
  if (!(flags & CMS_NOATTR)) {
    ...
    if (flags & CMS_CADES) {
      ...
      if (md == NULL || EVP_MD_is_a(md, SN_sha1)) {
        ...
        if ((sc2 = OSSL_ESS_signing_cert_v2_new_init(md, signer, NULL, 1)) ==
            NULL)
          goto err;
  ...
  if (flags & CMS_KEY_PARAM) {
    if (flags & CMS_NOATTR) {
      ...
      ossl_cms_ctx_get0_libctx(ctx), si->pkey, ossl_cms_ctx_get0_propq(ctx));
      if (si->pctx == NULL)
        goto err;
      ...
      if (EVP_PKEY_CTX_set_signature_md(si->pctx, md) <= 0)
        goto err;
    } else if (EVP_DigestSignInit_ex(si->mctx, &si->pctx, EVP_MD_get0_name(md),
                                     ossl_cms_ctx_get0_libctx(ctx),
                                     ossl_cms_ctx_get0_propq(ctx), pk,
                                     NULL) <= 0) {
      si->pctx = NULL;
      goto err;
    } else {
...
EVP_MD_CTX *CMS_SignerInfo_get0_md_ctx(CMS_SignerInfo *si) { return si->mctx; }
...
int CMS_SignerInfo_sign(CMS_SignerInfo *si) {
  ...
  if (md_name == NULL) {
    if (ASN1_item_sign_ctx(ASN1_ITEM_rptr(CMS_Attributes_Sign), NULL, NULL,
                           si->signature, si->signedAttrs, mctx) <= 0)
      goto err;
    return 1;
  }
...
int CMS_SignerInfo_verify_content(CMS_SignerInfo *si, BIO *chain) {
  ...
  if (CMS_signed_get_attr_count(si) >= 0) {
    os = CMS_signed_get0_data_by_OBJ(si, OBJ_nid2obj(NID_pkcs9_messageDigest),
  ...
  if (os != NULL) {
    ...
    pkctx = EVP_PKEY_CTX_new_from_pkey(ossl_cms_ctx_get0_libctx(ctx), si->pkey,
                                       ossl_cms_ctx_get0_propq(ctx));
    if (pkctx == NULL)
      goto err;
    ...
    si->pctx = NULL;
    r = EVP_PKEY_verify(pkctx, si->signature->data, si->signature->length, mval,
                        mlen);
    if (r <= 0) {
      ERR_raise(ERR_LIB_CMS, CMS_R_VERIFICATION_FAILURE);
      r = 0;
