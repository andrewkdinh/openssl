...
int CMS_RecipientInfo_kari_set0_pkey_and_peer(CMS_RecipientInfo *ri,
  ...
  pctx = EVP_PKEY_CTX_new_from_pkey(ossl_cms_ctx_get0_libctx(kari->cms_ctx), pk,
...
static int cms_kari_set_originator_private_key(CMS_KeyAgreeRecipientInfo *kari,
  ...
  pctx =
  EVP_PKEY_CTX_new_from_pkey(ossl_cms_ctx_get0_libctx(ctx), originatorPrivKey,
...
static int cms_wrap_init(CMS_KeyAgreeRecipientInfo *kari,
  ...
  if ((EVP_CIPHER_get_flags(cipher) & EVP_CIPH_FLAG_GET_WRAP_CIPHER) != 0) {
    ret = EVP_CIPHER_meth_get_ctrl(cipher)(NULL, EVP_CTRL_GET_WRAP_CIPHER, 0,
                                           &kekcipher);
    if (ret <= 0)
      return 0;
...
enc:
  fetched_kekcipher =
  EVP_CIPHER_fetch(ossl_cms_ctx_get0_libctx(cms_ctx), kekcipher_name,
