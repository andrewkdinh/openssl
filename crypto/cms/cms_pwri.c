...
CMS_RecipientInfo *CMS_add0_recipient_password(CMS_ContentInfo *cms, int iter,
  ...
  pwri->keyDerivationAlgorithm =
  PKCS5_pbkdf2_set_ex(iter, NULL, 0, -1, -1, cms_ctx->libctx);
...
int ossl_cms_RecipientInfo_pwri_crypt(const CMS_ContentInfo *cms,
  ...
  kekalg =
  ASN1_TYPE_unpack_sequence(ASN1_ITEM_rptr(X509_ALGOR), algtmp->parameter);
  ...
  if (EVP_PBE_CipherInit_ex(algtmp->algorithm, (char *)pwri->pass,
