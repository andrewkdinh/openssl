...
CMS_RecipientInfo *CMS_add0_recipient_password(CMS_ContentInfo *cms, int iter, int wrap_nid, int pbe_nid,
                                               unsigned char *pass, ossl_ssize_t passlen, const EVP_CIPHER *kekciph)
{
    ...
    pwri->keyDerivationAlgorithm = PKCS5_pbkdf2_set_ex(iter, NULL, 0, -1, -1, cms_ctx->libctx);
...
static int kek_unwrap_key(unsigned char *out, size_t *outlen, const unsigned char *in, size_t inlen,
                          EVP_CIPHER_CTX *ctx)
{
...
static int kek_wrap_key(unsigned char *out, size_t *outlen, const unsigned char *in, size_t inlen, EVP_CIPHER_CTX *ctx,
                        const CMS_CTX *cms_ctx)
{
    ...
    if (out) {
        ...
        if (olen > inlen + 4
            && RAND_bytes_ex(ossl_cms_ctx_get0_libctx(cms_ctx), out + 4 + inlen, olen - 4 - inlen, 0) <= 0)
            return 0;
        /* Encrypt twice */
...
int ossl_cms_RecipientInfo_pwri_crypt(const CMS_ContentInfo *cms, CMS_RecipientInfo *ri, int en_de)
{
    ...
    if (EVP_PBE_CipherInit_ex(algtmp->algorithm, (char *)pwri->pass, pwri->passlen, algtmp->parameter, kekctx, en_de,
