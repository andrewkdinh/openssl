...
CMS_ContentInfo *d2i_CMS_ContentInfo(CMS_ContentInfo **a,
  ...
  ci = (CMS_ContentInfo *)ASN1_item_d2i_ex(
  (ASN1_VALUE **)a, in, len, (CMS_ContentInfo_it()),
...
CMS_ContentInfo *CMS_ContentInfo_new_ex(OSSL_LIB_CTX *libctx,
                                        const char *propq) {
  CMS_ContentInfo *ci;
  ...
  if (EVP_MD_xof(digest)) {
    ...
    if (xof_len > 0) {
      ...
      if (BIO_get_md_ctx(mdbio, &mdctx) <= 0 || mdctx == NULL)
        goto err;
      params[0] =
      OSSL_PARAM_construct_size_t(OSSL_DIGEST_PARAM_XOFLEN, &xof_len);
...
CMS_EnvelopedData *ossl_cms_sign_encrypt(
BIO *data, X509 *sign_cert, STACK_OF(X509) * certs, EVP_PKEY *sign_key,
...
unsigned int enc_flags, OSSL_LIB_CTX *libctx, const char *propq) {
  ...
  if (data == NULL || sign_key == NULL || sign_cert == NULL ||
      enc_recip == NULL) {
    ERR_raise(ERR_LIB_CMS, ERR_R_PASSED_NULL_PARAMETER);
    return NULL;
  }
  signcms =
  CMS_sign_ex(sign_cert, sign_key, certs, data, sign_flags, libctx, propq);
  if (signcms == NULL)
    goto err;

  signbio = BIO_new(BIO_s_mem());
  if (signbio == NULL || ASN1_item_i2d_bio(ASN1_ITEM_rptr(CMS_SignedData),
                                           signbio, signcms->d.signedData) <= 0)
    goto err;
