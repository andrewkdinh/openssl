...
/* See RFC Section 11 */
{NID_pkcs9_contentType, CMS_ATTR_F_SIGNED | CMS_ATTR_F_ONLY_ONE |
                        CMS_ATTR_F_ONE_ATTR_VALUE | CMS_ATTR_F_REQUIRED_COND},
{NID_pkcs9_messageDigest, CMS_ATTR_F_SIGNED | CMS_ATTR_F_ONLY_ONE |
                          CMS_ATTR_F_ONE_ATTR_VALUE | CMS_ATTR_F_REQUIRED_COND},
...
void *CMS_signed_get0_data_by_OBJ(const CMS_SignerInfo *si,
                                  const ASN1_OBJECT *oid, int lastpos,
                                  int type) {
...
int CMS_unsigned_add1_attr_by_txt(CMS_SignerInfo *si, const char *attrname,
                                  int type, const void *bytes, int len) {
  if (ossl_x509at_add1_attr_by_txt(&si->unsignedAttrs, attrname, type, bytes,
                                   len))
    return 1;
  return 0;
}
...
cms_attrib_get(int nid, const STACK_OF(X509_ATTRIBUTE) * attrs, int *lastpos) {
  X509_ATTRIBUTE *at;
  int loc;
...
static int cms_check_attribute(int nid, int flags, int type,
  ...
  if (at != NULL) {
    ...
    if (have_attrs && ((flags & CMS_ATTR_F_REQUIRED_COND) != 0) &&
        (flags & type) != 0)
      return 0;
  }
  return 1;
...
int ossl_cms_si_check_attributes(const CMS_SignerInfo *si) {
  ...
  for (i = 0; i < (int)OSSL_NELEM(cms_attribute_properties); ++i) {
    ...
    if (!cms_check_attribute(nid, flags, CMS_ATTR_F_SIGNED, si->signedAttrs,
                             have_signed_attrs) ||
        !cms_check_attribute(nid, flags, CMS_ATTR_F_UNSIGNED, si->unsignedAttrs,
                             have_unsigned_attrs)) {
