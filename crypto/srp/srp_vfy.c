...
static int t_fromb64(unsigned char *a, size_t alen, const char *src)
{
    ...
    if (EVP_DecodeUpdate(ctx, a, &outl2, (const unsigned char *)src, size)
        < 0) {
        outl = -1;
        goto err;
    }
...
static int t_tob64(char *dst, const unsigned char *src, int size)
{
    ...
    evp_encode_ctx_set_flags(
        ctx, EVP_ENCODE_CTX_NO_NEWLINES | EVP_ENCODE_CTX_USE_SRP_ALPHABET);
    ...
    for (i = 0; i < sk_OPENSSL_PSTRING_num(tmpdb->data); i++) {
        ...
        if (pp[DB_srptype][0] == DB_SRP_INDEX) {
            ...
            if ((lgN = SRP_get_gN_by_id(pp[DB_srpgN], SRP_gN_tab)) != NULL) {
                ...
                if (!SRP_user_pwd_set1_ids(user_pwd, pp[DB_srpid],
                                           pp[DB_srpinfo]))
                    goto err;
