...
int X509_verify(X509 *a, EVP_PKEY *r) {
  ...
  return ASN1_item_verify_ex(ASN1_ITEM_rptr(X509_CINF), &a->sig_alg,
                             &a->signature, &a->cert_info, a->distinguishing_id,
                             r, a->libctx, a->propq);
}
...
int X509_CRL_sign(X509_CRL *x, EVP_PKEY *pkey, const EVP_MD *md) {
  ...
  return ASN1_item_sign_ex(ASN1_ITEM_rptr(X509_CRL_INFO), &x->crl.sig_alg,
                           &x->sig_alg, &x->signature, &x->crl, NULL, pkey, md,
                           x->libctx, x->propq);
}
...
RSA *d2i_RSA_PUBKEY_fp(FILE *fp, RSA **rsa) {
  return ASN1_d2i_fp((void *(*)(void))RSA_new, (D2I_OF(void))d2i_RSA_PUBKEY, fp,
                     (void **)rsa);
}
...
int X509_digest(const X509 *cert, const EVP_MD *md, unsigned char *data,
  ...
  return ossl_asn1_item_digest_ex(ASN1_ITEM_rptr(X509), md, (char *)cert, data,
