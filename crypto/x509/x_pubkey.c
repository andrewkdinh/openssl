...
ASN1_SEQUENCE(X509_PUBKEY_INTERNAL)
    = {ASN1_SIMPLE(X509_PUBKEY, algor, X509_ALGOR),
       ASN1_SIMPLE(
           ...
           ASN1_BIT_STRING)} static_ASN1_SEQUENCE_END_name(X509_PUBKEY,
            ...
            X509_PUBKEY
        * ossl_d2i_X509_PUBKEY_INTERNAL(const unsigned char **pp, long len,
    ...
    {
        ASN1_item_free((ASN1_VALUE *)xpub,
    ...
    static int x509_pubkey_ex_d2i_ex(
    ...
    /* This ensures that |*in| advances properly no matter what */
        if ((ret = ASN1_item_ex_d2i(pval, in, len,
                                    ASN1_ITEM_rptr(X509_PUBKEY_INTERNAL), tag,
                                    aclass, opt, ctx))
            <= 0)
            return ret;
    ...
    /* Try to decode it into an EVP_PKEY with OSSL_DECODER */
        ...
        */
            ...
            if ((dctx = OSSL_DECODER_CTX_new_for_pkey(
                     &pubkey->pkey, "DER", "SubjectPublicKeyInfo", txtoidname,
    ...
    static int x509_pubkey_ex_print(BIO * out, const ASN1_VALUE **pval,
    ...
    X509_PUBKEY *X509_PUBKEY_new_ex(OSSL_LIB_CTX * libctx, const char *propq)
    {
        ...
        pubkey =
            (X509_PUBKEY *)ASN1_item_new_ex(X509_PUBKEY_it(), libctx, propq);
 ...
 */
    X509_PUBKEY *X509_PUBKEY_dup(const X509_PUBKEY *a)
    {
        ...
        if ((pubkey->algor = X509_ALGOR_dup(a->algor)) == NULL
            || (pubkey->public_key = ASN1_BIT_STRING_new()) == NULL
            || !ASN1_BIT_STRING_set(pubkey->public_key, a->public_key->data,
                                    a->public_key->length)) {
            x509_pubkey_ex_free((ASN1_VALUE **)&pubkey,
                                ASN1_ITEM_rptr(X509_PUBKEY_INTERNAL));
    ...
    int X509_PUBKEY_set(X509_PUBKEY * *x, EVP_PKEY * pkey)
    {
        ...
        if (pkey->ameth != NULL) {
            ...
            OSSL_ENCODER_CTX *ectx = OSSL_ENCODER_CTX_new_for_pkey(
                pkey, EVP_PKEY_PUBLIC_KEY, "DER", "SubjectPublicKeyInfo", NULL);
 ...
 */
    static EVP_PKEY *d2i_PUBKEY_int(
        EVP_PKEY * *a, const unsigned char **pp, long length,
    ...
    EVP_PKEY *d2i_PUBKEY_ex(EVP_PKEY * *a, const unsigned char **pp,
    ...
    int i2d_PUBKEY(const EVP_PKEY *a, unsigned char **pp)
    {
        ...
        if (a->ameth != NULL) {
            ...
            OSSL_ENCODER_CTX *ctx = OSSL_ENCODER_CTX_new_for_pkey(
                a, EVP_PKEY_PUBLIC_KEY, "DER", "SubjectPublicKeyInfo", NULL);
            BIO *out = BIO_new(BIO_s_mem());
            BUF_MEM *buf = NULL;

            if (OSSL_ENCODER_CTX_get_num_encoders(ctx) != 0 && out != NULL
...
# endif /* OPENSSL_NO_ECX */
    ...
    void X509_PUBKEY_set0_public_key(X509_PUBKEY * pub, unsigned char *penc,
                                     int penclen)
    {
    ...
    int ossl_x509_PUBKEY_get0_libctx(
        OSSL_LIB_CTX * *plibctx, const char **ppropq, const X509_PUBKEY *key)
    {
        if (plibctx)
            *plibctx = key->libctx;
        if (ppropq)
