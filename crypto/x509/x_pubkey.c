...
static int x509_pubkey_ex_d2i_ex(ASN1_VALUE **pval, const unsigned char **in,
  ...
  if ((ret =
       ASN1_item_ex_d2i(pval, in, len, ASN1_ITEM_rptr(X509_PUBKEY_INTERNAL),
  ...
  if (ret <= 0 && !pubkey->flag_force_legacy) {
    ...
    if (OBJ_obj2txt(txtoidname, sizeof(txtoidname), pubkey->algor->algorithm,
                    0) <= 0) {
      ERR_clear_last_mark();
      goto end;
    }
    if ((dctx = OSSL_DECODER_CTX_new_for_pkey(
         ...
         EVP_PKEY_PUBLIC_KEY, pubkey->libctx, pubkey->propq)) != NULL)
      /*
...
static int x509_pubkey_ex_i2d(const ASN1_VALUE **pval, unsigned char **out,
  ...
  return ASN1_item_ex_i2d(pval, out, ASN1_ITEM_rptr(X509_PUBKEY_INTERNAL), tag,
                          aclass);
}
...
EVP_PKEY **a, const unsigned char **pp, long length, OSSL_LIB_CTX *libctx,
const char *propq, unsigned int force_legacy,
X509_PUBKEY *(*d2i_x509_pubkey)(X509_PUBKEY **a, const unsigned char **in,
                                long len)) {
...
int i2d_PUBKEY(const EVP_PKEY *a, unsigned char **pp) {
  ...
  if (a->ameth != NULL) {
    ...
    if (OSSL_ENCODER_CTX_get_num_encoders(ctx) != 0 && out != NULL &&
        OSSL_ENCODER_to_bio(ctx, out) && BIO_get_mem_ptr(out, &buf) > 0) {
      ret = buf->length;
