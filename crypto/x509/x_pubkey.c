...
ASN1_SEQUENCE(X509_PUBKEY_INTERNAL)
    ...
    static int x509_pubkey_ex_new_ex(ASN1_VALUE * *pval, const ASN1_ITEM *it, OSSL_LIB_CTX *libctx, const char *propq)
    {
        ...
        if ((ret = OPENSSL_zalloc(sizeof(*ret))) == NULL)
            return 0;
        if (!x509_pubkey_ex_populate((ASN1_VALUE **)&ret, NULL) || !x509_pubkey_set0_libctx(ret, libctx, propq)) {
    ...
    static int x509_pubkey_ex_d2i_ex(ASN1_VALUE * *pval, const unsigned char **in, long len, const ASN1_ITEM *it,
                                     int tag, int aclass, char opt, ASN1_TLC *ctx, OSSL_LIB_CTX *libctx,
                                     const char *propq)
    {
    ...
    /* Try to decode it into an EVP_PKEY with OSSL_DECODER */
        ...
        */
            ...
            if ((dctx = OSSL_DECODER_CTX_new_for_pkey(&pubkey->pkey, "DER", "SubjectPublicKeyInfo", txtoidname,
                                                      EVP_PKEY_PUBLIC_KEY, pubkey->libctx, pubkey->propq))
                != NULL)
            /*
    ...
    static int x509_pubkey_ex_i2d(const ASN1_VALUE **pval, unsigned char **out, const ASN1_ITEM *it, int tag,
                                  int aclass)
    {
        return ASN1_item_ex_i2d(pval, out, ASN1_ITEM_rptr(X509_PUBKEY_INTERNAL), tag, aclass);
    }

    static int x509_pubkey_ex_print(BIO * out, const ASN1_VALUE **pval, int indent, const char *fname,
 ...
 */
    X509_PUBKEY *X509_PUBKEY_dup(const X509_PUBKEY *a)
    {
        ...
        if ((pubkey->algor = X509_ALGOR_dup(a->algor)) == NULL || (pubkey->public_key = ASN1_BIT_STRING_new()) == NULL
    ...
    int X509_PUBKEY_set(X509_PUBKEY * *x, EVP_PKEY * pkey)
    {
        ...
        if (pkey->ameth != NULL) {
            ...
            OSSL_ENCODER_CTX *ectx =
                OSSL_ENCODER_CTX_new_for_pkey(pkey, EVP_PKEY_PUBLIC_KEY, "DER", "SubjectPublicKeyInfo", NULL);
 ...
 */
    static EVP_PKEY *d2i_PUBKEY_int(
        EVP_PKEY * *a, const unsigned char **pp, long length, OSSL_LIB_CTX *libctx, const char *propq,
        unsigned int force_legacy, X509_PUBKEY *(*d2i_x509_pubkey)(X509_PUBKEY **a, const unsigned char **in, long len))
    {
...
/* For the algorithm specific d2i functions further down */
    ...
    EVP_PKEY *d2i_PUBKEY_ex(EVP_PKEY * *a, const unsigned char **pp, long length, OSSL_LIB_CTX *libctx,
                            const char *propq)
    {
    ...
    int i2d_PUBKEY(const EVP_PKEY *a, unsigned char **pp)
    {
        ...
        /* pub_encode() only encode parameters, not the key itself */
            ...
            OSSL_ENCODER_CTX *ctx =
                OSSL_ENCODER_CTX_new_for_pkey(a, EVP_PKEY_PUBLIC_KEY, "DER", "SubjectPublicKeyInfo", NULL);
            BIO *out = BIO_new(BIO_s_mem());
            BUF_MEM *buf = NULL;

            if (OSSL_ENCODER_CTX_get_num_encoders(ctx) != 0 && out != NULL && OSSL_ENCODER_to_bio(ctx, out)
...
# endif /* OPENSSL_NO_ECX */
    ...
    int X509_PUBKEY_set0_param(X509_PUBKEY * pub, ASN1_OBJECT * aobj, int ptype, void *pval, unsigned char *penc,
                               int penclen)
    {
    ...
    int X509_PUBKEY_get0_param(ASN1_OBJECT * *ppkalg, const unsigned char **pk, int *ppklen, X509_ALGOR **pa,
