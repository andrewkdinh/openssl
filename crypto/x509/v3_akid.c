                                                 ...
                                                 STACK_OF(CONF_VALUE) *extlist)
{
    ...
    if (akeyid->keyid) {
        ...
        if (!X509V3_add_value((akeyid->issuer || akeyid->serial) ? "keyid"
                                                                 : NULL,
                              tmp, &extlist)) {
            OPENSSL_free(tmp);
                                            ...
                                            STACK_OF(CONF_VALUE) *values)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        if (strcmp(cnf->name, "keyid") == 0 && keyid == 0) {
            ...
            ERR_raise_data(ERR_LIB_X509V3, X509V3_R_BAD_VALUE, "name=%s",
                           cnf->name);
            goto err;
        } else {
            ERR_raise_data(ERR_LIB_X509V3, X509V3_R_UNKNOWN_VALUE, "name=%s",
                           cnf->name);
            goto err;
        }
    }
