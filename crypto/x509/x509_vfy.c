 ...
 * Yet if |sk| is a list of trusted certs, as with
 * X509_STORE_CTX_set0_trusted_stack(), better not set |check_signing_allowed|.
...
static X509 *get0_best_issuer_sk(X509_STORE_CTX *ctx, int check_signing_allowed,
  ...
  for (i = 0; i < sk_X509_num(sk); i++) {
    ...
    if (ctx->check_issued(ctx, x, candidate)) {
       ...
       */
      if (issuer == NULL || ASN1_TIME_compare(X509_get0_notAfter(candidate),
...
static int check_issued(ossl_unused X509_STORE_CTX *ctx, X509 *x,
                        X509 *issuer) {
 ...
 * Returns -1 on internal error.
 */
static int get1_best_issuer_other_sk(X509 **issuer, X509_STORE_CTX *ctx,
                                     X509 *x) {
...
lookup_certs_sk(X509_STORE_CTX *ctx, const X509_NAME *nm) {
...
static int check_extensions(X509_STORE_CTX *ctx) {
  ...
  for (i = 0; i < num; i++) {
    ...
    CB_FAIL_IF(!allow_proxy_certs && (x->ex_flags & EXFLAG_PROXY) != 0, ctx, x,
    ...
    default:
      ...
      CB_FAIL_IF(ret == 0 || ((i + 1 < num || (ctx->param->flags &
    ...
    if ((ctx->param->flags & X509_V_FLAG_X509_STRICT) != 0 &&
      ...
      CB_FAIL_IF(((x->ex_flags & EXFLAG_CA) != 0 ||
                  (x->ex_kusage & KU_CRL_SIGN) != 0 || x->altname == NULL) &&
      ...
      CB_FAIL_IF(X509_ALGOR_cmp(&x->sig_alg, &x->cert_info.signature) != 0, ctx,
                 x, i, X509_V_ERR_SIGNATURE_ALGORITHM_INCONSISTENCY);
      ...
      if (X509_get_version(x) >= X509_VERSION_3) {
        ...
        CB_FAIL_IF(sk_X509_EXTENSION_num(X509_get0_extensions(x)) > 0, ctx, x,
                   i, X509_V_ERR_EXTENSIONS_REQUIRE_VERSION_3);
      }
    }
...
static int internal_verify(X509_STORE_CTX *ctx) {
  ...
  while (n >= 0) {
     ...
     */
    if (xi != NULL && (xs != xi || ((ctx->param->flags &
                                     X509_V_FLAG_CHECK_SS_SIGNATURE) != 0 &&
...
int X509_STORE_CTX_init_rpk(X509_STORE_CTX *ctx, X509_STORE *store,
                            EVP_PKEY *rpk) {
...
static int dane_match_rpk(X509_STORE_CTX *ctx, EVP_PKEY *rpk) {
  ...
  for (i = 0; i < recnum; i++) {
    t = sk_danetls_record_value(dane->trecs, i);
    if (t->usage != DANETLS_USAGE_DANE_EE ||
        t->selector != DANETLS_SELECTOR_SPKI)
      continue;
...
#define S_DOALTERNATE (1 << 2) /* Retry with pruned alternate chain */
  ...
  while (search != 0) {
    ...
    if ((search & S_DOUNTRUSTED) != 0) {
      ...
      issuer =
      (X509_self_signed(curr, 0) > 0 || num > max_depth)
      ? NULL
  ...
  default:
    ...
    CB_FAIL_IF(num > max_depth, ctx, NULL, num - 1,
...
static int check_curve(X509 *cert) {
  ...
  ret = EVP_PKEY_get_int_param(
  pkey, OSSL_PKEY_PARAM_EC_DECODED_FROM_EXPLICIT_PARAMS, &val);
  return ret == 1 ? !val : -1;
}
