...
{ASN1_SIMPLE(IPAddressRange, min, ASN1_BIT_STRING),
 ASN1_SIMPLE(IPAddressRange, max,
...
ASN1_ITEM_TEMPLATE(IPAddrBlocks) = ASN1_EX_TEMPLATE_TYPE(ASN1_TFLG_SEQUENCE_OF,
                                                         0, IPAddrBlocks,
                                                         IPAddressFamily)
    ...
    for (i = 0; i < sk_IPAddressFamily_num(addr); i++) {
        ...
        case IPAddressChoice_addressesOrRanges:
            ...
            if (!i2r_IPAddressOrRanges(
                out, indent + 2, f->ipAddressChoice->u.addressesOrRanges, afi))
                return 0;
            break;
        }
    }
...
static void *v2i_IPAddrBlocks(const struct v3_ext_method *method,
    ...
    for (i = 0; i < sk_CONF_VALUE_num(values); i++) {
        ...
        if (!ossl_v3_name_cmp(val->name, "IPv4")) {
            ...
            ERR_raise_data(ERR_LIB_X509V3, X509V3_R_EXTENSION_NAME_ERROR, "%s",
                           val->name);
            goto err;
        }
...
int X509v3_addr_subset(IPAddrBlocks *a, IPAddrBlocks *b) {
    ...
    /* Could sort a here too and get O(|a|) running time instead of O(|a| ln
     * |b|)
     */
