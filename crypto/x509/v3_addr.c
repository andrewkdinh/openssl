 ...
 */
    static int i2r_address(BIO * out, const unsigned afi,
 ...
 */
    static int range_should_be_prefix(
        const unsigned char *min, const unsigned char *max, const int length)
    {
        unsigned char mask;
        int i, j;
 ...
 */
    static IPAddressFamily *make_IPAddressFamily(
        IPAddrBlocks * addr, const unsigned afi, const unsigned *safi)
    {
        IPAddressFamily *f;
        unsigned char key[3];
        int keylen;
 ...
 */
    static IPAddressOrRanges *make_prefix_or_range(
        IPAddrBlocks * addr, const unsigned afi, const unsigned *safi)
    {
        IPAddressFamily *f = make_IPAddressFamily(addr, afi, safi);
                                  ...
                                  STACK_OF(CONF_VALUE) *values)
    {
         ...
         */
            ...
            case '/':
                ...
                if (t == s + i2 || *t != '\0' || prefixlen > (length * 8)
                    || prefixlen < 0) {
 ...
 */
                             ...
                             IPAddressOrRanges * child, int length)
    {
        ...
        for (c = 0; c < sk_IPAddressOrRange_num(child); c++) {
            if (!extract_min_max(sk_IPAddressOrRange_value(child, c), c_min,
                                 c_max, length))
                return 0;
            for (;; p++) {
 ...
 */
    int X509v3_addr_subset(IPAddrBlocks * a, IPAddrBlocks * b)
    {
        ...
        for (i = 0; i < sk_IPAddressFamily_num(a); i++) {
            ...
            if (fb == NULL)
                return 0;
            if (!IPAddressFamily_check_len(fa)
                || !IPAddressFamily_check_len(fb))
                return 0;
 ...
 */
    static int addr_validate_path_internal(
        X509_STORE_CTX * ctx, STACK_OF(X509) *chain, IPAddrBlocks * ext)
    {
 ...
 */
    int X509v3_addr_validate_resource_set(
        STACK_OF(X509) *chain, IPAddrBlocks * ext, int allow_inheritance)
    {
        if (ext == NULL)
            return 1;
