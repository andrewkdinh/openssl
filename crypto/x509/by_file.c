...
static int by_file_ctrl_ex(X509_LOOKUP *ctx, int cmd, const char *argc, long argl, char **ret, OSSL_LIB_CTX *libctx,
                           const char *propq);
...
static int by_file_ctrl_ex(X509_LOOKUP *ctx, int cmd, const char *argp, long argl, char **ret, OSSL_LIB_CTX *libctx,
                           const char *propq)
{
    int ok = 0;
    const char *file;
    ...
    case X509_L_FILE_LOAD:
        if (argl == X509_FILETYPE_DEFAULT) {
            ...
            if (file)
                ...
                ok = (X509_load_cert_crl_file_ex(ctx, X509_get_default_cert_file(), X509_FILETYPE_PEM, libctx, propq)
                      != 0);
