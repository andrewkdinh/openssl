...
static int by_file_ctrl_ex(X509_LOOKUP *ctx, int cmd, const char *argp,
  ...
  case X509_L_FILE_LOAD:
    if (argl == X509_FILETYPE_DEFAULT) {
      ...
      if (file)
        ok = (X509_load_cert_crl_file_ex(ctx, file, X509_FILETYPE_PEM, libctx,
                                         propq) != 0);
      else
        ok =
        (X509_load_cert_crl_file_ex(ctx, X509_get_default_cert_file(),
      ...
      if (argl == X509_FILETYPE_PEM)
        ok = (X509_load_cert_crl_file_ex(ctx, argp, X509_FILETYPE_PEM, libctx,
                                         propq) != 0);
      else
