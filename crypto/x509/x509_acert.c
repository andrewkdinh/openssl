...
ASN1_IMP_OPT(X509_ACERT_ISSUER_V2FORM, baseCertificateId, OSSL_ISSUER_SERIAL,
             0),
ASN1_IMP_OPT(X509_ACERT_ISSUER_V2FORM, objectDigestInfo,
             OSSL_OBJECT_DIGEST_INFO, 1),
...
IMPLEMENT_ASN1_FUNCTIONS(X509_ACERT) IMPLEMENT_ASN1_DUP_FUNCTION(X509_ACERT)
...
static X509_NAME
* get_dirName(const GENERAL_NAMES *names) {
...
const X509_NAME *
OSSL_ISSUER_SERIAL_get0_issuer(const OSSL_ISSUER_SERIAL *isss) {
  return get_dirName(isss->issuer);
}

const ASN1_INTEGER *
OSSL_ISSUER_SERIAL_get0_serial(const OSSL_ISSUER_SERIAL *isss) {
  return &isss->serial;
}

const ASN1_BIT_STRING *
OSSL_ISSUER_SERIAL_get0_issuerUID(const OSSL_ISSUER_SERIAL *isss) {
...
const OSSL_ISSUER_SERIAL *
X509_ACERT_get0_holder_baseCertId(const X509_ACERT *x) {
  return x->acinfo->holder.baseCertificateID;
}

const OSSL_OBJECT_DIGEST_INFO *
X509_ACERT_get0_holder_digest(const X509_ACERT *x) {
...
int X509_ACERT_add_attr_nconf(CONF *conf, const char *section,
  ...
  for (i = 0; i < sk_CONF_VALUE_num(attr_sk); i++) {
    ...
    if (check_asn1_attribute(&value) == 1) {
      ...
      ret = X509_ACERT_add1_attr_by_txt(acert, v->name, V_ASN1_OCTET_STRING,
...
void *X509_ACERT_get_ext_d2i(const X509_ACERT *x, int nid, int *crit,
                             int *idx) {
