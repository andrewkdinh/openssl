...
ASN1_SEQUENCE(X509_ACERT_ISSUER_V2FORM)
    = {
        ASN1_SEQUENCE_OF_OPT(X509_ACERT_ISSUER_V2FORM, issuerName,
                             GENERAL_NAME),
        ASN1_IMP_OPT(X509_ACERT_ISSUER_V2FORM, baseCertificateId,
                     OSSL_ISSUER_SERIAL, 0),
                              ...
                              X509_ACERT *acert)
{
    ...
    for (i = 0; i < sk_CONF_VALUE_num(attr_sk); i++) {
        ...
        if (check_asn1_attribute(&value) == 1) {
            ...
            ret = X509_ACERT_add1_attr_by_txt(
                acert, v->name, V_ASN1_OCTET_STRING, (unsigned char *)value,
                strlen(value));
            if (!ret)
                goto err;
        }
