...
static int cache_objects(X509_LOOKUP *lctx, CACHED_STORE *store,
  ...
  if (ctx == NULL &&
      (ctx = OSSL_STORE_open_ex(store->uri, store->libctx, store->propq, NULL,
...
static int by_store_ctrl_ex(X509_LOOKUP *ctx, int cmd, const char *argp,
  ...
  case X509_L_ADD_STORE:
    if (argp != NULL) {
      ...
      store->ctx =
      OSSL_STORE_open_ex(argp, libctx, propq, NULL, NULL, NULL, NULL, NULL);
...
X509_LOOKUP_METHOD *X509_LOOKUP_store(void) { return &x509_store_lookup; }
