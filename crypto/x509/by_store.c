...
static int cache_objects(X509_LOOKUP *lctx, CACHED_STORE *store, const OSSL_STORE_SEARCH *criterion, int depth)
{
    int ok = 0;
    ...
    if (ctx == NULL
        && (ctx = OSSL_STORE_open_ex(store->uri, store->libctx, store->propq, NULL, NULL, NULL, NULL, NULL)) == NULL)
        return 0;
    store->ctx = ctx;
...
static int by_store_ctrl_ex(X509_LOOKUP *ctx, int cmd, const char *argp, long argl, char **retp, OSSL_LIB_CTX *libctx,
                            const char *propq)
{
    switch (cmd) {
    case X509_L_ADD_STORE:
        if (argp != NULL) {
            ...
            store->ctx = OSSL_STORE_open_ex(argp, libctx, propq, NULL, NULL, NULL, NULL, NULL);
            if (store->ctx == NULL || (propq != NULL && store->propq == NULL) || store->uri == NULL) {
                free_store(store);
                return 0;
            }
...
static int by_store(X509_LOOKUP *ctx, X509_LOOKUP_TYPE type, const OSSL_STORE_SEARCH *criterion, X509_OBJECT *ret)
{
    STACK_OF(CACHED_STORE) *stores = X509_LOOKUP_get_method_data(ctx);
    int i;
    int ok = 0;
    ...
    for (i = 0; i < sk_CACHED_STORE_num(stores); i++) {
        ok = cache_objects(ctx, sk_CACHED_STORE_value(stores, i), criterion, 1 /* depth */);
...
static X509_LOOKUP_METHOD x509_store_lookup = {"Load certs from STORE URIs",
                                               NULL,                        /* new_item */
                                               by_store_free,               /* free */
                                               NULL,                        /* init */
                                               NULL,                        /* shutdown */
