...
{NID_subject_alt_name, 0, ASN1_ITEM_ref(GENERAL_NAMES), 0, 0, 0, 0, 0, 0,
 (X509V3_EXT_I2V)i2v_GENERAL_NAMES, (X509V3_EXT_V2I)v2i_subject_alt, NULL, NULL,
 NULL},

{NID_issuer_alt_name, 0, ASN1_ITEM_ref(GENERAL_NAMES), 0, 0, 0, 0, 0, 0,
 (X509V3_EXT_I2V)i2v_GENERAL_NAMES, (X509V3_EXT_V2I)v2i_issuer_alt, NULL, NULL,
 NULL},
...
STACK_OF(CONF_VALUE) * i2v_GENERAL_NAME(X509V3_EXT_METHOD *method,
  ...
  case GEN_OTHERNAME:
    ...
    case NID_SRVName:
      if (gen->d.otherName->value->type != V_ASN1_IA5STRING ||
          !x509v3_add_len_value_uchar(
          "othername: SRVName", gen->d.otherName->value->value.ia5string->data,
    ...
    case NID_ms_upn:
      if (gen->d.otherName->value->type != V_ASN1_UTF8STRING ||
          !x509v3_add_len_value_uchar(
          "othername: UPN", gen->d.otherName->value->value.utf8string->data,
          gen->d.otherName->value->value.utf8string->length, &ret))
        return NULL;
      break;
    case NID_NAIRealm:
    ...
    default:
      ...
      if (gen->d.otherName->value->type == V_ASN1_IA5STRING) {
        if (x509v3_add_len_value_uchar(
            othername, gen->d.otherName->value->value.ia5string->data,
            gen->d.otherName->value->value.ia5string->length, &ret))
          return ret;
      }
      if (gen->d.otherName->value->type == V_ASN1_UTF8STRING) {
        if (x509v3_add_len_value_uchar(
            othername, gen->d.otherName->value->value.utf8string->data,
            gen->d.otherName->value->value.utf8string->length, &ret))
          return ret;
      }
  ...
  case GEN_DNS:
    if (!x509v3_add_len_value_uchar("DNS", gen->d.ia5->data, gen->d.ia5->length,
                                    &ret))
      return NULL;
    break;

  case GEN_URI:
    if (!x509v3_add_len_value_uchar("URI", gen->d.ia5->data, gen->d.ia5->length,
                                    &ret))
      return NULL;
    break;
  ...
  for (i = 0; i < num; i++) {
    ...
    if (ossl_v3_name_cmp(cnf->name, "email") == 0 && cnf->value &&
      ...
      if (!copy_email(ctx, gens, 0))
        goto err;
    } else if (ossl_v3_name_cmp(cnf->name, "email") == 0 && cnf->value &&
...
GENERAL_NAME *a2i_GENERAL_NAME(GENERAL_NAME *out,
  ...
  case GEN_IPADD:
    ...
    if (gen->d.ip == NULL) {
      ERR_raise_data(ERR_LIB_X509V3, X509V3_R_BAD_IP_ADDRESS, "value=%s",
                     value);
      goto err;
    }
    break;
...
GENERAL_NAME *v2i_GENERAL_NAME_ex(GENERAL_NAME *out,
  ...
  if (!ossl_v3_name_cmp(name, "email"))
    ...
    ERR_raise_data(ERR_LIB_X509V3, X509V3_R_UNSUPPORTED_OPTION, "name=%s",
                   name);
    return NULL;
  }
...
static int do_dirname(GENERAL_NAME *gen, const char *value, X509V3_CTX *ctx) {
  ...
  if (!sk) {
    ERR_raise_data(ERR_LIB_X509V3, X509V3_R_SECTION_NOT_FOUND, "section=%s",
                   value);
    goto err;
  }
