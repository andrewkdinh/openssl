...
ASN1_SEQUENCE(X509_REVOKED)
    = {ASN1_EMBED(X509_REVOKED, serialNumber, ASN1_INTEGER), ASN1_SIMPLE(X509_REVOKED, revocationDate, ASN1_TIME),
...
ASN1_SEQUENCE_enc(X509_CRL_INFO, enc, crl_inf_cb) = {ASN1_OPT(X509_CRL_INFO, version, ASN1_INTEGER),
    ...
    case ASN1_OP_D2I_POST:
        ...
        for (idx = 0; idx < sk_X509_EXTENSION_num(exts); idx++) {
            ...
            if (X509_EXTENSION_get_critical(ext)) {
                ...
                if ((nid == NID_issuing_distribution_point) || (nid == NID_authority_key_identifier)
                    || (nid == NID_delta_crl))
                    continue;
                crl->flags |= EXFLAG_CRITICAL;
                break;
...
ASN1_SEQUENCE_ref(X509_CRL,
                  crl_cb) = {ASN1_EMBED(X509_CRL, crl, X509_CRL_INFO), ASN1_EMBED(X509_CRL, sig_alg, X509_ALGOR),
...
static int def_crl_verify(X509_CRL *crl, EVP_PKEY *r)
{
    return ASN1_item_verify_ex(ASN1_ITEM_rptr(X509_CRL_INFO), &crl->sig_alg, &crl->signature, &crl->crl, NULL, r,
                               crl->libctx, crl->propq);
}
