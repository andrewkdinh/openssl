...
static int crl_cb(int operation, ASN1_VALUE **pval, const ASN1_ITEM *it,
    ...
    case ASN1_OP_D2I_POST:
        ...
        crl->base_crl_number =
        X509_CRL_get_ext_d2i(crl, NID_delta_crl, &i, NULL);
...
int X509_CRL_get0_by_cert(X509_CRL *crl, X509_REVOKED **ret, X509 *x) {
    if (crl->meth->crl_lookup)
        return crl->meth->crl_lookup(crl, ret, X509_get0_serialNumber(x),
                                     X509_get_issuer_name(x));
    return 0;
}

static int def_crl_verify(X509_CRL *crl, EVP_PKEY *r) {
    return ASN1_item_verify_ex(ASN1_ITEM_rptr(X509_CRL_INFO), &crl->sig_alg,
                               &crl->signature, &crl->crl, NULL, r, crl->libctx,
                               crl->propq);
}
...
void X509_CRL_set_meth_data(X509_CRL *crl, void *dat) { crl->meth_data = dat; }

void *X509_CRL_get_meth_data(X509_CRL *crl) { return crl->meth_data; }
