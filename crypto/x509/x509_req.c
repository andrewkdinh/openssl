...
void X509_REQ_set_extension_nids(int *nids) { ext_nids = nids; }

static STACK_OF(X509_EXTENSION) *
get_extensions_by_nid(const X509_REQ *req, int nid) {
  X509_ATTRIBUTE *attr;
  ASN1_TYPE *ext = NULL;
...
int X509_REQ_add_extensions_nid(X509_REQ *req,
  ...
  extlen =
  ASN1_item_i2d((const ASN1_VALUE *)(mod_exts == NULL ? exts : mod_exts), &ext,
...
int X509_REQ_add1_attr_by_txt(X509_REQ *req, const char *attrname, int type,
  ...
  if (!X509at_add1_attr_by_txt(&req->req_info.attributes, attrname, type, bytes,
                               len))
    return 0;
  req->req_info.enc.modified = 1;
  return 1;
