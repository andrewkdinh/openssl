...
i2v_TLS_FEATURE(const X509V3_EXT_METHOD *method, TLS_FEATURE *tls_feature, STACK_OF(CONF_VALUE) *ext_list);
...
ASN1_ITEM_TEMPLATE(TLS_FEATURE)
    ...
    static TLS_FEATURE *v2i_TLS_FEATURE(const X509V3_EXT_METHOD *method, X509V3_CTX *ctx, STACK_OF(CONF_VALUE) *nval)
    {
        ...
        for (i = 0; i < sk_CONF_VALUE_num(nval); i++) {
            ...
            if ((ai = ASN1_INTEGER_new()) == NULL || !ASN1_INTEGER_set(ai, tlsextid)
