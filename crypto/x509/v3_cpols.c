...
ASN1_SEQUENCE(POLICYINFO) = {ASN1_SIMPLE(POLICYINFO, policyid, ASN1_OBJECT),
                             ASN1_SEQUENCE_OF_OPT(
                             POLICYINFO, qualifiers,
...
ADB_ENTRY(NID_id_qt_unotice,
          ASN1_SIMPLE(POLICYQUALINFO, d.usernotice,
                      USERNOTICE))} ASN1_ADB_END(POLICYQUALINFO, 0, pqualid, 0,
...
{ASN1_OPT(USERNOTICE, noticeref, NOTICEREF),
 ASN1_OPT(USERNOTICE, exptext, DISPLAYTEXT)} ASN1_SEQUENCE_END(USERNOTICE)

IMPLEMENT_ASN1_FUNCTIONS(USERNOTICE)

ASN1_SEQUENCE(NOTICEREF) =
{ASN1_SIMPLE(NOTICEREF, organization, DISPLAYTEXT),
 ASN1_SEQUENCE_OF(NOTICEREF, noticenos,
...
r2i_certpol(X509V3_EXT_METHOD * method, X509V3_CTX *ctx, const char *value) {
  ...
  for (i = 0; i < num; i++) {
    ...
    if (strcmp(pstr, "ia5org") == 0) {
      ...
      if (polsect == NULL) {
        ERR_raise_data(ERR_LIB_X509V3, X509V3_R_INVALID_SECTION, "%s",
                       cnf->name);
        goto err;
      }
...
static void print_qualifiers(BIO *out, STACK_OF(POLICYQUALINFO) * quals,
  ...
  for (i = 0; i < sk_POLICYQUALINFO_num(quals); i++) {
    ...
    case NID_id_qt_cps:
      BIO_printf(out, "%*sCPS: %.*s", indent, "", qualinfo->d.cpsuri->length,
