...
static void tree_print(BIO *channel, char *str, X509_POLICY_TREE *tree,
...
static int tree_init(X509_POLICY_TREE **ptree, STACK_OF(X509) * certs,
  ...
  *ptree = NULL;
  ...
  tree->nlevel = n + 1;
  level = tree->levels;
  if ((data = ossl_policy_data_new(NULL, OBJ_nid2obj(NID_any_policy), 0)) ==
      NULL)
    goto bad_tree;
...
static int tree_link_any(X509_POLICY_LEVEL *curr,
  ...
  if (last->anyPolicy &&
      ossl_policy_level_add_node(curr, cache->anyPolicy, last->anyPolicy, tree,
                                 0) == NULL)
    return 0;
  return 1;
}
...
static int tree_calculate_user_set(X509_POLICY_TREE *tree,
  ...
  for (i = 0; i < sk_ASN1_OBJECT_num(policy_oids); i++) {
    ...
    if (!node) {
      ...
      node =
      ossl_policy_level_add_node(NULL, extra, anyPolicy->parent, tree, 1);
      if (node == NULL) {
