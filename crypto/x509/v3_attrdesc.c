...
IMPLEMENT_ASN1_FUNCTIONS(OSSL_HASH) IMPLEMENT_ASN1_FUNCTIONS(OSSL_INFO_SYNTAX)
...
static int i2r_HASH(X509V3_EXT_METHOD * method, OSSL_HASH *hash, BIO *out,
                    int indent) {
    ...
    if (hash->algorithmIdentifier->parameter) {
        ...
        if (ossl_print_attribute_value(
            out, 0, hash->algorithmIdentifier->parameter, indent + 4) <= 0)
            return 0;
        if (BIO_puts(out, "\n") <= 0)
            return 0;
    }
...
static int i2r_INFO_SYNTAX_POINTER(X509V3_EXT_METHOD *method,
                                   OSSL_INFO_SYNTAX_POINTER *pointer, BIO *out,
                                   int indent) {
...
static int i2r_OSSL_INFO_SYNTAX(X509V3_EXT_METHOD *method,
    ...
    case OSSL_INFO_SYNTAX_TYPE_POINTER:
        if (BIO_printf(out, "%*sPointer:\n", indent, "") <= 0)
            return 0;
        return i2r_INFO_SYNTAX_POINTER(method, info->choice.pointer, out,
                                       indent + 4);
    default:
        return 0;
    }
...
static int i2r_OSSL_ATTRIBUTE_DESCRIPTOR(X509V3_EXT_METHOD *method,
    ...
    if (BIO_printf(out, "%*sDomination Rule:\n", indent, "") <= 0)
        return 0;
    return i2r_OSSL_PRIVILEGE_POLICY_ID(method, ad->dominationRule, out,
                                        indent + 4);
}
