...
ASN1_SEQUENCE(ASRange)
    = {ASN1_SIMPLE(ASRange, min, ASN1_INTEGER), ASN1_SIMPLE(ASRange, max, ASN1_INTEGER)}
ASN1_SEQUENCE_END(ASRange)

ASN1_CHOICE(ASIdOrRange)
    = {ASN1_SIMPLE(ASIdOrRange, u.id, ASN1_INTEGER), ASN1_SIMPLE(ASIdOrRange, u.range, ASRange)}
...
static int ASIdentifierChoice_is_canonical(ASIdentifierChoice *choice)
{
    ...
    for (i = 0; i < sk_ASIdOrRange_num(choice->u.asIdsOrRanges) - 1; i++) {
         ...
         */
        if (ASN1_INTEGER_cmp(a_min, b_min) >= 0 || ASN1_INTEGER_cmp(a_min, a_max) > 0
...
{
    return (asid == NULL
            || (ASIdentifierChoice_is_canonical(asid->asnum) && ASIdentifierChoice_is_canonical(asid->rdi)));
}
...
{
    return (asid != NULL
            && ((asid->asnum != NULL && asid->asnum->type == ASIdentifierChoice_inherit)
...
int X509v3_asid_subset(ASIdentifiers *a, ASIdentifiers *b)
{
    ...
    subset =
        a->asnum == NULL || (b->asnum != NULL && asid_contains(b->asnum->u.asIdsOrRanges, a->asnum->u.asIdsOrRanges));
    if (!subset)
        return 0;
