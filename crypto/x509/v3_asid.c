...
ASN1_CHOICE(ASIdOrRange) = {ASN1_SIMPLE(ASIdOrRange, u.id, ASN1_INTEGER),
...
ASN1_SEQUENCE(
ASIdentifiers) = {ASN1_EXP_OPT(ASIdentifiers, asnum, ASIdentifierChoice, 0),
                  ASN1_EXP_OPT(ASIdentifiers, rdi, ASIdentifierChoice,
                               1)} ASN1_SEQUENCE_END(ASIdentifiers)

IMPLEMENT_ASN1_FUNCTIONS(ASRange) IMPLEMENT_ASN1_FUNCTIONS(ASIdOrRange)
...
static int ASIdentifierChoice_is_canonical(ASIdentifierChoice *choice) {
  ...
  for (i = 0; i < sk_ASIdOrRange_num(choice->u.asIdsOrRanges) - 1; i++) {
    ...
    if ((a_max_plus_one = BN_to_ASN1_INTEGER(bn, orig = a_max_plus_one)) ==
        NULL) {
      a_max_plus_one = orig;
...
int X509v3_asid_subset(ASIdentifiers *a, ASIdentifiers *b) {
  ...
  subset = a->asnum == NULL ||
           (b->asnum != NULL && asid_contains(b->asnum->u.asIdsOrRanges,
