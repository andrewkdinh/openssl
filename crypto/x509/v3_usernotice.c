...
ASN1_ITEM_TEMPLATE(OSSL_USER_NOTICE_SYNTAX) = ASN1_EX_TEMPLATE_TYPE(
ASN1_TFLG_SEQUENCE_OF, 0, OSSL_USER_NOTICE_SYNTAX, USERNOTICE)
...
static int print_notice(BIO * out, USERNOTICE *notice, int indent) {
  ...
  if (notice->noticeref) {
    ...
    if (BIO_printf(out, "%*sOrganization: %.*s\n", indent, "",
                   ref->organization->length, ref->organization->data) <= 0)
      return 0;
  ...
  return BIO_printf(out, "%*sExplicit Text: %.*s", indent, "",
                    notice->exptext->length, notice->exptext->data) >= 0;
}

static int i2r_USER_NOTICE_SYNTAX(X509V3_EXT_METHOD *method,
                                  OSSL_USER_NOTICE_SYNTAX *uns, BIO *out,
                                  int indent) {
  int i;
  USERNOTICE *unotice;
