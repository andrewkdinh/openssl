...
static int print_oid(BIO *out, const ASN1_OBJECT *oid) {
    ...
    rc = (ln != NULL) ? BIO_printf(out, "%s (%s)", objbuf, ln)
                      : BIO_printf(out, "%s", objbuf);
    return (rc >= 0);
}

int ossl_print_attribute_value(BIO *out, int obj_nid, const ASN1_TYPE *av,
                               int indent) {
    ASN1_STRING *str;
    unsigned char *value;
    X509_NAME *xn = NULL;
    ...
    case V_ASN1_SEQUENCE:
        ...
        case NID_owner:
            /*
             * d2i_ functions increment the ppin pointer. See
             * doc/man3/d2i_X509.pod. This preserves the original  pointer. We
             * don't want to corrupt this value.
             */
            value = av->value.sequence->data;
            xn = d2i_X509_NAME(NULL, (const unsigned char **)&value,
    ...
    case V_ASN1_SET:
        return ASN1_parse_dump(out, av->value.set->data, av->value.set->length,
                               indent, 1) > 0;
    ...
    default:
        return BIO_printf(out, "%*s<Unsupported tag %d>", indent, "",
                          av->type) >= 0;
    }
}
