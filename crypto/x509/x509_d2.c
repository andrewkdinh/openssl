...
int X509_STORE_load_path(X509_STORE *ctx, const char *path)
{
    ...
    if (path == NULL || (lookup = X509_STORE_add_lookup(ctx, X509_LOOKUP_hash_dir())) == NULL
...
int X509_STORE_load_store_ex(X509_STORE *ctx, const char *uri, OSSL_LIB_CTX *libctx, const char *propq)
{
    ...
    if (uri == NULL || (lookup = X509_STORE_add_lookup(ctx, X509_LOOKUP_store())) == NULL
...
int X509_STORE_load_locations_ex(X509_STORE *ctx, const char *file, const char *path, OSSL_LIB_CTX *libctx,
                                 const char *propq)
{
