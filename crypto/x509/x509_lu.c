...
int X509_LOOKUP_ctrl_ex(X509_LOOKUP *ctx, int cmd, const char *argc, long argl, char **ret, OSSL_LIB_CTX *libctx,
                        const char *propq)
{
    if (ctx->method == NULL)
        return -1;
...
int X509_LOOKUP_by_subject_ex(X509_LOOKUP *ctx, X509_LOOKUP_TYPE type, const X509_NAME *name, X509_OBJECT *ret,
                              OSSL_LIB_CTX *libctx, const char *propq)
{
    if (ctx->skip || ctx->method == NULL
        || (ctx->method->get_by_subject == NULL && ctx->method->get_by_subject_ex == NULL))
        return 0;
...
int X509_LOOKUP_by_fingerprint(X509_LOOKUP *ctx, X509_LOOKUP_TYPE type, const unsigned char *bytes, int len,
                               X509_OBJECT *ret)
{
...
X509_OBJECT *X509_STORE_CTX_get_obj_by_subject(X509_STORE_CTX *ctx, X509_LOOKUP_TYPE type, const X509_NAME *name)
{
 ...
 * -1 on failure
 */
int ossl_x509_store_ctx_get_by_subject(const X509_STORE_CTX *ctx, X509_LOOKUP_TYPE type, const X509_NAME *name,
                                       X509_OBJECT *ret)
{
    ...
    if (tmp == NULL || type == X509_LU_CRL) {
        for (i = 0; i < sk_X509_LOOKUP_num(store->get_cert_methods); i++) {
            ...
            if (lu->method == NULL)
                return -1;
            j = X509_LOOKUP_by_subject_ex(lu, type, name, &stmp, ctx->libctx, ctx->propq);
...
/* Also fill the cache |ctx->store->objs| with all matching certificates. */
int X509_STORE_CTX_get_by_subject(const X509_STORE_CTX *ctx, X509_LOOKUP_TYPE type, const X509_NAME *name,
                                  X509_OBJECT *ret)
{
...
STACK_OF(X509_OBJECT) *X509_STORE_get1_objects(X509_STORE *store)
{
    ...
    objs = sk_X509_OBJECT_deep_copy(store->objs, x509_object_dup, X509_OBJECT_free);
    X509_STORE_unlock(store);
    return objs;
}
