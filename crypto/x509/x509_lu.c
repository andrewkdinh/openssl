...
int X509_STORE_unlock(X509_STORE *xs) { return CRYPTO_THREAD_unlock(xs->lock); }
  ...
  if (tmp == NULL || type == X509_LU_CRL) {
    for (i = 0; i < sk_X509_LOOKUP_num(store->get_cert_methods); i++) {
      ...
      if (lu->method == NULL)
        return -1;
      j =
      X509_LOOKUP_by_subject_ex(lu, type, name, &stmp, ctx->libctx, ctx->propq);
...
X509_LOOKUP_TYPE X509_OBJECT_get_type(const X509_OBJECT *a) { return a->type; }
 ...
 */
STACK_OF(X509) *
X509_STORE_CTX_get1_certs(X509_STORE_CTX *ctx, const X509_NAME *nm) {
  int i, idx, cnt;
...
/* Returns NULL on internal/fatal error, empty stack if not found */
STACK_OF(X509_CRL) *
X509_STORE_CTX_get1_crls(const X509_STORE_CTX *ctx, const X509_NAME *nm) {
  int i = 1, idx, cnt;
...
X509_STORE_get_check_issued(const X509_STORE *xs) {
...
X509_STORE_get_check_policy(const X509_STORE *xs) {
...
X509_STORE_get_lookup_certs(const X509_STORE *xs) {
