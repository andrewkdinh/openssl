...
static void ossl_method_construct_this(OSSL_PROVIDER *provider,
  ...
  OSSL_TRACE2(QUERY,
              "ossl_method_construct_this: putting an algo to the store %p "
              "with no_store %d\n",
...
void *ossl_method_construct(OSSL_LIB_CTX *libctx, int operation_id,
  ...
  ossl_method_construct_unreserve_store, ossl_method_construct_postcondition,
  &cbdata);

  /* If there is a temporary store, try there first */
  if (cbdata.store != NULL)
    method =
    mcm->get(cbdata.store, (const OSSL_PROVIDER **)provider_rw, mcm_data);
