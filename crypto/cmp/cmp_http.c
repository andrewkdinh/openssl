...
static int keep_alive(int keep_alive, int body_type, BIO **bios)
{
    if (keep_alive != 0
         ...
         */
        && body_type != OSSL_CMP_PKIBODY_IR && body_type != OSSL_CMP_PKIBODY_CR && body_type != OSSL_CMP_PKIBODY_P10CR
        && body_type != OSSL_CMP_PKIBODY_KUR && body_type != OSSL_CMP_PKIBODY_POLLREQ)
        keep_alive = 0;
    return keep_alive;
}
...
OSSL_CMP_MSG *OSSL_CMP_MSG_http_perform(OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *req)
{
    ...
    tls_used =
        ctx->tls_used >= 0 ? ctx->tls_used != 0 : OSSL_CMP_CTX_get_http_cb_arg(ctx) != NULL; /* backward compat */
    if (ctx->http_ctx == NULL) { /* using existing connection or yet not set up own connection */
        ...
        if (*path == '/')
            path++;
        if (bios == NULL)
            ossl_cmp_log4(DEBUG, ctx, "connecting to CMP server via http%s://%s:%s/%s", tls_used ? "s" : "",
                          ctx->server, server_port, path);
        else
            ossl_cmp_log3(DEBUG, ctx, "using existing connection with CMP server %s:%s and HTTP path /%s", ctx->server,
                          server_port, path);
    }

    rsp = OSSL_HTTP_transfer(&ctx->http_ctx, ctx->server, server_port, ctx->serverPath, tls_used, ctx->proxy,
                             ctx->no_proxy, bios == NULL ? NULL : bios[0] /* bio */,
                             bios == NULL ? NULL : bios[1] /* rbio */, ctx->http_cb, OSSL_CMP_CTX_get_http_cb_arg(ctx),
                             0 /* buf_size */, headers, content_type_pkix, req_mem, content_type_pkix,
                             1 /* expect_asn1 */, OSSL_HTTP_DEFAULT_MAX_RESP_LEN, ctx->msg_timeout,
