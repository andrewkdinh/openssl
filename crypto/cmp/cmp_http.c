...
static int keep_alive(int keep_alive, int body_type, BIO **bios)
{
    if (keep_alive != 0
         ...
         */
        && body_type != OSSL_CMP_PKIBODY_IR && body_type != OSSL_CMP_PKIBODY_CR
                                        ...
                                        const OSSL_CMP_MSG *req)
{
    ...
    if (ctx->http_ctx
        == NULL) { /* using existing connection or yet not set up own connection */
    ...
    rsp = OSSL_HTTP_transfer(
        &ctx->http_ctx, ctx->server, server_port, ctx->serverPath, tls_used,
        ctx->proxy, ctx->no_proxy, bios == NULL ? NULL : bios[0] /* bio */,
        bios == NULL ? NULL : bios[1] /* rbio */, ctx->http_cb,
        OSSL_CMP_CTX_get_http_cb_arg(ctx), 0 /* buf_size */, headers,
        content_type_pkix, req_mem, content_type_pkix, 1 /* expect_asn1 */,
        OSSL_HTTP_DEFAULT_MAX_RESP_LEN, ctx->msg_timeout,
