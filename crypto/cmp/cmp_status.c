...
const char *ossl_cmp_PKIStatus_to_string(int status) {
  ...
  case OSSL_CMP_PKISTATUS_revocationNotification:
    return "PKIStatus: revocation notification - a revocation of the cert has "
           "occurred";
...
#define ADVANCE_BUFFER                                                         \
    ...
    for (failure = 0; failure <= OSSL_CMP_PKIFAILUREINFO_MAX; failure++) {
      if ((fail_info & (1 << failure)) != 0) {
        ...
        if (failure_string != NULL) {
          printed_chars =
          BIO_snprintf(write_ptr, bufsize, "%s%s", failinfo_found ? ", " : "",
                       failure_string);
          ADVANCE_BUFFER;
          failinfo_found = 1;
        }
  ...
  if (n_status_strings > 0) {
    ...
    write_ptr, bufsize, "; StatusString%s: ", n_status_strings > 1 ? "s" : "");
    ADVANCE_BUFFER;
