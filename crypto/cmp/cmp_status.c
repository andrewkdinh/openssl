...
const char *ossl_cmp_PKIStatus_to_string(int status) {
    ...
    case OSSL_CMP_PKISTATUS_revocationWarning:
        return "PKIStatus: revocation warning - a revocation of the cert is "
               "imminent";
    case OSSL_CMP_PKISTATUS_revocationNotification:
        return "PKIStatus: revocation notification - a revocation of the cert "
               "has "
               "occurred";
    case OSSL_CMP_PKISTATUS_keyUpdateWarning:
        return "PKIStatus: key update warning - update already done for the "
               "cert";
    default:
...
ossl_cmp_pkisi_get0_statusString(const OSSL_CMP_PKISI *si) {
...
int ossl_cmp_pkisi_check_pkifailureinfo(const OSSL_CMP_PKISI *si,
                                        int bit_index) {
 ...
 */
static char *
snprint_PKIStatusInfo_parts(int status, int fail_info,
...
#define ADVANCE_BUFFER                                                         \
        ...
        for (failure = 0; failure <= OSSL_CMP_PKIFAILUREINFO_MAX; failure++) {
            if ((fail_info & (1 << failure)) != 0) {
                ...
                if (failure_string != NULL) {
                    printed_chars =
                    BIO_snprintf(write_ptr, bufsize, "%s%s",
                                 failinfo_found ? ", " : "", failure_string);
                    ADVANCE_BUFFER;
                    failinfo_found = 1;
                }
            }
    ...
    if (n_status_strings > 0) {
        printed_chars =
        BIO_snprintf(write_ptr, bufsize,
                     "; StatusString%s: ", n_status_strings > 1 ? "s" : "");
        ADVANCE_BUFFER;
        for (i = 0; i < n_status_strings; i++) {
            text = sk_ASN1_UTF8STRING_value(status_strings, i);
            printed_chars = BIO_snprintf(
            write_ptr, bufsize, "\"%.*s\"%s", ASN1_STRING_length(text),
            ASN1_STRING_get0_data(text), i < n_status_strings - 1 ? ", " : "");
            ADVANCE_BUFFER;
        }
    }
...
char *OSSL_CMP_snprint_PKIStatusInfo(const OSSL_CMP_PKISI *statusInfo,
    ...
    return snprint_PKIStatusInfo_parts(ASN1_INTEGER_get(statusInfo->status),
                                       failure_info, statusInfo->statusString,
                                       buf, bufsize);
}
...
char *OSSL_CMP_CTX_snprint_PKIStatus(const OSSL_CMP_CTX *ctx, char *buf,
    ...
    return snprint_PKIStatusInfo_parts(
    OSSL_CMP_CTX_get_status(ctx), OSSL_CMP_CTX_get_failInfoCode(ctx),
    OSSL_CMP_CTX_get0_statusString(ctx), buf, bufsize);
}
