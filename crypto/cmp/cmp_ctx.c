...
static size_t ossl_cmp_log_trace_cb(const char *buf, size_t cnt, int category,
                                    int cmd, void *vdata) {
    ...
    if (!ctx->log_cb(func != NULL ? func : "(no func)",
                     file != NULL ? file : "(no file)", line, level, msg))
        cnt = 0;
    ...
    if (OSSL_TRACE_ENABLED(CMP)) {
        OSSL_TRACE_BEGIN(CMP) {
            ...
            BIO_snprintf(hugebuf, sizeof(hugebuf),
                         "%s:%s:%d:" OSSL_CMP_LOG_PREFIX "%s: ", func, file,
                         line, level_str);
            if (printed > 0 && (size_t)printed < sizeof(hugebuf)) {
                if (BIO_vsnprintf(hugebuf + printed, sizeof(hugebuf) - printed,
                                  format, args) > 0)
...
int OSSL_CMP_CTX_set_log_cb(OSSL_CMP_CTX *ctx, OSSL_CMP_log_cb_t cb) {
    ...
    if (!OSSL_trace_set_callback(OSSL_TRACE_CATEGORY_CMP, ossl_cmp_log_trace_cb,
                                 ctx))
        return 0;
#endif
...
#define DEFINE_OSSL_set1_certs(PREFIX, FIELD)                                  \
    int PREFIX##_set1_##FIELD(OSSL_CMP_CTX *ctx, STACK_OF(X509) * certs) {     \
        ...
        ctx->FIELD = NULL;                                                     \
        return certs == NULL ||                                                \
               (ctx->FIELD = X509_chain_up_ref(certs)) != NULL;                \
    }
...
int OSSL_CMP_CTX_reqExtensions_have_SAN(OSSL_CMP_CTX *ctx) {
    ...
    return ctx->reqExtensions != NULL &&
           X509v3_get_ext_by_NID(ctx->reqExtensions, NID_subject_alt_name,
                                 -1) >= 0;
}
