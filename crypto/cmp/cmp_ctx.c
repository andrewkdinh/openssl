  ...
  if (OSSL_TRACE_ENABLED(CMP)) {
    OSSL_TRACE_BEGIN(CMP) {
      int printed = BIO_snprintf(hugebuf, sizeof(hugebuf),
                                 "%s:%s:%d:" OSSL_CMP_LOG_PREFIX "%s: ", func,
                                 file, line, level_str);
      if (printed > 0 && (size_t)printed < sizeof(hugebuf)) {
        if (BIO_vsnprintf(hugebuf + printed, sizeof(hugebuf) - printed, format,
                          args) > 0)
...
int OSSL_CMP_CTX_reqExtensions_have_SAN(OSSL_CMP_CTX *ctx) {
  ...
  return ctx->reqExtensions != NULL &&
         X509v3_get_ext_by_NID(ctx->reqExtensions, NID_subject_alt_name, -1) >=
         0;
}
