...
static OSSL_CMP_MSG *process_cert_request(OSSL_CMP_SRV_CTX *srv_ctx,
  ...
  if (central_keygen == 0 &&
    ...
    if (!OSSL_CMP_CTX_set_option(srv_ctx->ctx, OSSL_CMP_OPT_IMPLICIT_CONFIRM,
                                 ...
                                 && certOut != NULL))
      goto err;
    if (central_keygen == 1 && srv_ctx->ctx->newPkey_priv &&
        srv_ctx->ctx->newPkey != NULL)
      keyOut = srv_ctx->ctx->newPkey;
  }

  msg = ossl_cmp_certrep_new(srv_ctx->ctx, bodytype, certReqId, si, certOut,
                             keyOut, NULL /* enc */, chainOut, caPubs,
                             srv_ctx->sendUnprotectedErrors);
...
static OSSL_CMP_MSG *process_pollReq(OSSL_CMP_SRV_CTX *srv_ctx,
  ...
  if (!srv_ctx->process_pollReq(srv_ctx, req, certReqId, &orig_req,
                                &check_after))
    return NULL;
  ...
  if (orig_req != NULL) {
    ...
    if ((msg = ossl_cmp_pollRep_new(srv_ctx->ctx, certReqId, check_after)) ==
        NULL)
...
OSSL_CMP_MSG *OSSL_CMP_SRV_process_request(OSSL_CMP_SRV_CTX *srv_ctx,
  ...
  if (srv_ctx == NULL || srv_ctx->ctx == NULL || req == NULL ||
      req->body == NULL || (hdr = OSSL_CMP_MSG_get0_header(req)) == NULL) {
    ERR_raise(ERR_LIB_CMP, CMP_R_NULL_ARGUMENT);
    return 0;
  }
  ctx = srv_ctx->ctx;
  ...
  ossl_cmp_log1(DEBUG, ctx, "received %s",
                ossl_cmp_bodytype_to_string(req_type));
...
err:
  if (rsp == NULL) {
    ...
    if ((si = OSSL_CMP_STATUSINFO_new(OSSL_CMP_PKISTATUS_rejection, fail_info,
                                      reason)) != NULL) {
      rsp = ossl_cmp_error_new(srv_ctx->ctx, si, err, data,
                               srv_ctx->sendUnprotectedErrors);
      OSSL_CMP_PKISI_free(si);
    }
  }
  ...
  if (rsp != NULL)
    ossl_cmp_log1(DEBUG, ctx, "sending %s",
                  ossl_cmp_bodytype_to_string(rsp_type));
  else
