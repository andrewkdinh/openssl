...
static void cert_msg(const char *func, const char *file, int lineno,
  ...
  ossl_cmp_print_log(level, ctx, func, file, lineno,
                     ...
                     "certificate from '%s' with subject '%s' %s", source, subj,
                     msg);
  OPENSSL_free(subj);
}
  ...
  if (type_CA >= 0 && (ex_flags & EXFLAG_V1) == 0) {
    ...
    if ((type_CA != 0) != is_CA) {
      cert_msg(OPENSSL_FUNC, OPENSSL_FILE, OPENSSL_LINE, level, ctx, source,
               cert, is_CA ? "is not an EE cert" : "is not a CA cert");
      ret = 0;
    }
  }
  return ret;
...
static OSSL_CMP_ITAV *get_genm_itav(OSSL_CMP_CTX *ctx,
  ...
  if (OSSL_CMP_CTX_get_status(ctx) != OSSL_CMP_PKISTATUS_unspecified) {
    ERR_raise_data(ERR_LIB_CMP, CMP_R_UNCLEAN_CTX,
                   "client context in unsuitable state; should call "
                   "CMPclient_reinit() before");
    goto err;
  }
  ...
  if (n > 1)
    ossl_cmp_log2(WARN, ctx,
                  "response on genm contains %d ITAVs; will use the first ITAV "
                  "with infoType id-it-%s",
                  n, desc);
  for (i = 0; i < n; i++) {
...
int OSSL_CMP_get1_rootCaKeyUpdate(OSSL_CMP_CTX *ctx, const X509 *oldWithOld,
  ...
  *newWithNew = NULL;
  ...
  if (oldWithOld != NULL && my_oldWithNew != NULL &&
      !verify_ss_cert_trans(ctx, *newWithNew, my_oldWithNew, oldWithOld_copy,
                            "oldWithOld")) {
    ERR_raise(ERR_LIB_CMP, CMP_R_INVALID_ROOTCAKEYUPDATE);
    goto end;
  }
