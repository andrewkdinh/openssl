...
ASN1_BIT_STRING *ossl_cmp_calc_protection(const OSSL_CMP_CTX *ctx,
  ...
  if (OBJ_obj2nid(algorOID) == NID_id_PasswordBasedMAC) {
    ...
    if (!OSSL_CRMF_pbm_new(ctx->libctx, ctx->propq, pbm, prot_part_der,
                           prot_part_der_len, ctx->secretValue->data,
                           ctx->secretValue->length, &protection, &sig_len))
      goto end;
...
static X509_ALGOR *pbmac_algor(const OSSL_CMP_CTX *ctx) {
  ...
  pbm =
  OSSL_CRMF_pbmp_new(ctx->libctx, ctx->pbm_slen, EVP_MD_get_type(ctx->pbm_owf),
  ...
  if (!ASN1_STRING_set(pbm_str, pbm_der, pbm_der_len))
    goto err;
  alg =
  ossl_X509_ALGOR_from_nid(NID_id_PasswordBasedMAC, V_ASN1_SEQUENCE, pbm_str);
err:
  if (alg == NULL)
