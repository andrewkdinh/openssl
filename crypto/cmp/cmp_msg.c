...
const char *ossl_cmp_bodytype_to_string(int type)
{
    static const char *type_names[] = {
        ...
        "CRLANN", "PKICONF", "NESTED", "GENM", "GENP",  "ERROR",   "CERTCONF", "POLLREQ", "POLLREP",
    };
...
OSSL_CRMF_MSG *OSSL_CMP_CTX_setup_CRM(OSSL_CMP_CTX *ctx, int for_KUR, int rid)
{
    ...
    int central_keygen = OSSL_CMP_CTX_get_option(ctx, OSSL_CMP_OPT_POPO_METHOD) == OSSL_CRMF_POPO_NONE;
    ...
    const X509_NAME *issuer = ctx->issuer != NULL || refcert == NULL ? (IS_NULL_DN(ctx->issuer) ? NULL : ctx->issuer)
    ...
    if (!OSSL_CRMF_MSG_set_certReqId(crm, rid)
        ...
        goto err;
    if (rkey != NULL && central_keygen)
        X509_PUBKEY_set0_public_key(OSSL_CRMF_CERTTEMPLATE_get0_publicKey(tmpl), NULL, 0);
    ...
    if (sk_GENERAL_NAME_num(ctx->subjectAltNames) > 0
        && !add1_extension(&exts, NID_subject_alt_name, crit, ctx->subjectAltNames))
        goto err;
    if (ctx->policies != NULL
        && !add1_extension(&exts, NID_certificate_policies, ctx->setPoliciesCritical, ctx->policies))
        goto err;
...
OSSL_CMP_MSG *ossl_cmp_certreq_new(OSSL_CMP_CTX *ctx, int type, const OSSL_CRMF_MSG *crm)
{
    ...
    if (type != OSSL_CMP_PKIBODY_IR && type != OSSL_CMP_PKIBODY_CR && type != OSSL_CMP_PKIBODY_KUR
    ...
    if (type != OSSL_CMP_PKIBODY_P10CR) {
        ...
        if (crm == NULL) {
            ...
            if (local_crm == NULL
                || !OSSL_CRMF_MSG_create_popo(ctx->popoMethod, local_crm, privkey, ctx->digest, ctx->libctx,
                                              ctx->propq))
                goto err;
        } else {
...
static OSSL_CRMF_ENCRYPTEDKEY *enc_privkey(OSSL_CMP_CTX *ctx, const EVP_PKEY *pkey)
{
    ...
    if (encryption_recips == NULL || !X509_add_cert(encryption_recips, recip, X509_ADD_FLAG_UP_REF))
        goto err;
    ...
    envData = ossl_cms_sign_encrypt(privbio, ctx->cert, ctx->chain, ctx->pkey, CMS_BINARY, encryption_recips, cipher,
...
OSSL_CMP_MSG *ossl_cmp_certrep_new(OSSL_CMP_CTX *ctx, int bodytype, int certReqId, const OSSL_CMP_PKISI *si, X509 *cert,
                                   const EVP_PKEY *pkey, const X509 *encryption_recip, STACK_OF(X509) *chain,
                                   STACK_OF(X509) *caPubs, int unprotectedErrors)
{
    ...
    if (sk_X509_num(chain) > 0
        && !ossl_x509_add_certs_new(&msg->extraCerts, chain, X509_ADD_FLAG_UP_REF | X509_ADD_FLAG_NO_DUP))
        goto err;
...
OSSL_CMP_MSG *ossl_cmp_rr_new(OSSL_CMP_CTX *ctx)
{
    ...
    if (!ossl_assert(
            ctx != NULL
            && (ctx->oldCert != NULL || ctx->p10CSR != NULL || (ctx->serialNumber != NULL && ctx->issuer != NULL))))
        return NULL;
    ...
    ret = OSSL_CRMF_CERTTEMPLATE_fill(rd->certDetails, pubkey, subject, issuer, serialNumber);
    if (!ret)
        goto err;
...
OSSL_CMP_MSG *ossl_cmp_rp_new(OSSL_CMP_CTX *ctx, const OSSL_CMP_PKISI *si, const OSSL_CRMF_CERTID *cid,
                              int unprotectedErrors)
{
    ...
    if ((si1 = OSSL_CMP_PKISI_dup(si)) == NULL || !sk_OSSL_CMP_PKISI_push(rep->status, si1))
        goto err;
    ...
    if ((rep->revCerts = sk_OSSL_CRMF_CERTID_new_null()) == NULL)
        goto err;
    if (cid != NULL) {
        if ((cid_copy = OSSL_CRMF_CERTID_dup(cid)) == NULL || !sk_OSSL_CRMF_CERTID_push(rep->revCerts, cid_copy))
            goto err;
...
OSSL_CMP_MSG *ossl_cmp_error_new(OSSL_CMP_CTX *ctx, const OSSL_CMP_PKISI *si, int64_t errorCode, const char *details,
                                 int unprotected)
{
    OSSL_CMP_MSG *msg = NULL;
...
OSSL_CMP_MSG *ossl_cmp_certConf_new(OSSL_CMP_CTX *ctx, int certReqId, int fail_info, const char *text)
{
    ...
    if (!ossl_assert(ctx != NULL && ctx->newCert != NULL
                     && (certReqId == OSSL_CMP_CERTREQID || certReqId == OSSL_CMP_CERTREQID_NONE)))
        return NULL;
...
OSSL_CMP_MSG *ossl_cmp_pollReq_new(OSSL_CMP_CTX *ctx, int crid)
{
    ...
    if ((preq = OSSL_CMP_POLLREQ_new()) == NULL || !ASN1_INTEGER_set(preq->certReqId, crid)
...
X509 *ossl_cmp_certresponse_get1_cert(const OSSL_CMP_CTX *ctx, const OSSL_CMP_CERTRESPONSE *crep)
{
    ...
    if (encr_key != NULL) {
        ...
        pkey = OSSL_CRMF_ENCRYPTEDKEY_get1_pkey(encr_key, ctx->trusted, ctx->untrusted, ctx->pkey, ctx->cert,
                                                ctx->secretValue, ctx->libctx, ctx->propq);
        if (pkey == NULL) {
    ...
    if ((coec = crep->certifiedKeyPair->certOrEncCert) != NULL) {
        ...
        case OSSL_CMP_CERTORENCCERT_ENCRYPTEDCERT:
            ...
            crt = OSSL_CRMF_ENCRYPTEDKEY_get1_encCert(coec->value.encryptedCert, ctx->libctx, ctx->propq, pkey, 0);
            break;
        default:
...
int ossl_cmp_is_error_with_waiting(const OSSL_CMP_MSG *msg)
{
    ...
    return (OSSL_CMP_MSG_get_bodytype(msg) == OSSL_CMP_PKIBODY_ERROR
            && ossl_cmp_pkisi_get_status(msg->body->value.error->pKIStatusInfo) == OSSL_CMP_PKISTATUS_waiting);
}
