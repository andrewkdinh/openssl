...
OSSL_CMP_MSG *OSSL_CMP_MSG_new(OSSL_LIB_CTX *libctx, const char *propq) {
  ...
  msg =
  (OSSL_CMP_MSG *)ASN1_item_new_ex(ASN1_ITEM_rptr(OSSL_CMP_MSG), libctx, propq);
...
OSSL_CRMF_MSG *OSSL_CMP_CTX_setup_CRM(OSSL_CMP_CTX *ctx, int for_KUR, int rid) {
  ...
  int central_keygen =
  OSSL_CMP_CTX_get_option(ctx, OSSL_CMP_OPT_POPO_METHOD) == OSSL_CRMF_POPO_NONE;
  ...
  if (ctx->policies != NULL &&
      !add1_extension(&exts, NID_certificate_policies, ctx->setPoliciesCritical,
                      ctx->policies))
    goto err;
...
OSSL_CMP_MSG *ossl_cmp_certreq_new(OSSL_CMP_CTX *ctx, int type,
  ...
  if (type != OSSL_CMP_PKIBODY_P10CR) {
    ...
    if (crm == NULL) {
      local_crm = OSSL_CMP_CTX_setup_CRM(ctx, type == OSSL_CMP_PKIBODY_KUR,
                                         OSSL_CMP_CERTREQID);
      if (local_crm == NULL ||
          !OSSL_CRMF_MSG_create_popo(ctx->popoMethod, local_crm, privkey,
