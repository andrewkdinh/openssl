...
OSSL_CMP_MSG *OSSL_CMP_MSG_new(OSSL_LIB_CTX *libctx, const char *propq) {
    ...
    msg = (OSSL_CMP_MSG *)ASN1_item_new_ex(ASN1_ITEM_rptr(OSSL_CMP_MSG), libctx,
                                           propq);
...
const char *ossl_cmp_bodytype_to_string(int type) {
    ...
    "IR",   "IP",     "CR",    "CP",       "P10CR",   "POPDECC", "POPDECR",
    "KUR",  "KUP",    "KRR",   "KRP",      "RR",      "RP",      "CCR",
...
static int add1_extension(X509_EXTENSIONS **pexts, int nid, int crit,
                          void *ex) {
    X509_EXTENSION *ext;
    int res;
...
OSSL_CMP_MSG *ossl_cmp_msg_create(OSSL_CMP_CTX *ctx, int bodytype) {
    ...
    if (ctx->geninfo_ITAVs != NULL &&
        !ossl_cmp_hdr_generalInfo_push1_items(msg->header, ctx->geninfo_ITAVs))
        goto err;
    ...
    case OSSL_CMP_PKIBODY_CERTCONF:
        if ((msg->body->value.certConf = sk_OSSL_CMP_CERTSTATUS_new_null()) ==
            NULL)
            goto err;
        return msg;
...
OSSL_CRMF_MSG *OSSL_CMP_CTX_setup_CRM(OSSL_CMP_CTX *ctx, int for_KUR, int rid) {
    ...
    int central_keygen = OSSL_CMP_CTX_get_option(
                         ctx, OSSL_CMP_OPT_POPO_METHOD) == OSSL_CRMF_POPO_NONE;
    ...
    if (!ctx->SubjectAltName_nodefault && !HAS_SAN(ctx) && refcert != NULL &&
        ...
        goto err;
    if (sk_X509_EXTENSION_num(ctx->reqExtensions) >
        0 /* augment/override existing ones */
        && X509v3_add_extensions(&exts, ctx->reqExtensions) == NULL)
        goto err;
    if (sk_GENERAL_NAME_num(ctx->subjectAltNames) > 0 &&
        !add1_extension(&exts, NID_subject_alt_name, crit,
                        ctx->subjectAltNames))
        goto err;
    ...
    if (for_KUR && refcert != NULL) {
        OSSL_CRMF_CERTID *cid = OSSL_CRMF_CERTID_gen(
        X509_get_issuer_name(refcert), X509_get0_serialNumber(refcert));
        int ret;
...
OSSL_CMP_MSG *ossl_cmp_certreq_new(OSSL_CMP_CTX *ctx, int type,
    ...
    if (type != OSSL_CMP_PKIBODY_P10CR) {
        ...
        if (crm == NULL) {
            local_crm = OSSL_CMP_CTX_setup_CRM(
            ctx, type == OSSL_CMP_PKIBODY_KUR, OSSL_CMP_CERTREQID);
            if (local_crm == NULL || !OSSL_CRMF_MSG_create_popo(
                                     ctx->popoMethod, local_crm, privkey,
...
static OSSL_CRMF_ENCRYPTEDKEY *enc_privkey(OSSL_CMP_CTX *ctx,
                                           const EVP_PKEY *pkey) {
    ...
    envData = ossl_cms_sign_encrypt(privbio, ctx->cert, ctx->chain, ctx->pkey,
                                    CMS_BINARY, encryption_recips, cipher,
...
ossl_cmp_certrep_new(OSSL_CMP_CTX *ctx, int bodytype, int certReqId,
    ...
    if (status != OSSL_CMP_PKISTATUS_rejection &&
        ...
        if (pkey != NULL) {
#ifndef OPENSSL_NO_CMS
            if ((resp->certifiedKeyPair->privateKey = enc_privkey(ctx, pkey)) ==
                NULL)
                goto err;
#else
            ERR_raise(ERR_LIB_CMP, ERR_R_UNSUPPORTED);
...
OSSL_CMP_MSG *ossl_cmp_rr_new(OSSL_CMP_CTX *ctx) {
    ...
    ret = OSSL_CRMF_CERTTEMPLATE_fill(rd->certDetails, pubkey, subject, issuer,
                                      serialNumber);
    if (!ret)
        goto err;
...
OSSL_CMP_MSG *ossl_cmp_genm_new(OSSL_CMP_CTX *ctx) {
    return gen_new(ctx, ctx->genm_ITAVs, OSSL_CMP_PKIBODY_GENM,
                   CMP_R_ERROR_CREATING_GENM);
}

OSSL_CMP_MSG *ossl_cmp_genp_new(OSSL_CMP_CTX *ctx,
                                const STACK_OF(OSSL_CMP_ITAV) * itavs) {
    return gen_new(ctx, itavs, OSSL_CMP_PKIBODY_GENP,
...
X509 *ossl_cmp_certresponse_get1_cert(const OSSL_CMP_CTX *ctx,
    ...
    if (encr_key != NULL) {
        ...
        pkey = OSSL_CRMF_ENCRYPTEDKEY_get1_pkey(
        encr_key, ctx->trusted, ctx->untrusted, ctx->pkey, ctx->cert,
    ...
    if ((coec = crep->certifiedKeyPair->certOrEncCert) != NULL) {
        ...
        case OSSL_CMP_CERTORENCCERT_ENCRYPTEDCERT:
            ...
            crt = OSSL_CRMF_ENCRYPTEDKEY_get1_encCert(
            coec->value.encryptedCert, ctx->libctx, ctx->propq, pkey, 0);
            break;
        default:
...
OSSL_CMP_MSG *d2i_OSSL_CMP_MSG(OSSL_CMP_MSG **msg, const unsigned char **in,
    ...
    return (OSSL_CMP_MSG *)ASN1_item_d2i_ex(
    (ASN1_VALUE **)msg, in, len, ASN1_ITEM_rptr(OSSL_CMP_MSG), libctx, propq);
}
