...
OSSL_CMP_MSG *ossl_cmp_certreq_new(OSSL_CMP_CTX *ctx, int type, const OSSL_CRMF_MSG *crm)
{
    ...
    if (type != OSSL_CMP_PKIBODY_P10CR) {
        ...
        if (crm == NULL) {
            ...
            if (local_crm == NULL
                || !OSSL_CRMF_MSG_create_popo(ctx->popoMethod,
                                              local_crm,
                                              privkey,
                                              ctx->digest,
                                              ctx->libctx,
...
static OSSL_CRMF_ENCRYPTEDKEY *enc_privkey(OSSL_CMP_CTX *ctx, const EVP_PKEY *pkey)
{
    ...
    envData = ossl_cms_sign_encrypt(privbio,
                                    ctx->cert,
                                    ctx->chain,
                                    ctx->pkey,
                                    CMS_BINARY,
...
OSSL_CMP_MSG *ossl_cmp_certrep_new(OSSL_CMP_CTX         *ctx,
                                   int                   bodytype,
                                   int                   certReqId,
...
OSSL_CMP_MSG *
ossl_cmp_rp_new(OSSL_CMP_CTX *ctx, const OSSL_CMP_PKISI *si, const OSSL_CRMF_CERTID *cid, int unprotectedErrors)
{
...
OSSL_CMP_MSG *
ossl_cmp_error_new(OSSL_CMP_CTX *ctx, const OSSL_CMP_PKISI *si, int64_t errorCode, const char *details, int unprotected)
{
    OSSL_CMP_MSG         *msg = NULL;
...
X509 *ossl_cmp_certresponse_get1_cert(const OSSL_CMP_CTX *ctx, const OSSL_CMP_CERTRESPONSE *crep)
{
    ...
    if (encr_key != NULL) {
        ...
        pkey = OSSL_CRMF_ENCRYPTEDKEY_get1_pkey(encr_key,
                                                ctx->trusted,
                                                ctx->untrusted,
                                                ctx->pkey,
                                                ctx->cert,
