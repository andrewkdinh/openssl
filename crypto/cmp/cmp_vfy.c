...
static int check_name(const OSSL_CMP_CTX *ctx, int log_success,
    ...
    if (X509_NAME_cmp(actual_name, expect_name) == 0) {
        if (log_success && str != NULL)
            ossl_cmp_log3(INFO, ctx, " %s matches %s: %s", actual_desc,
                          expect_desc, str);
        OPENSSL_free(str);
        return 1;
    }
...
static int check_msg_find_cert(OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *msg) {
    ...
    if (ctx->log_cb != NULL) {
        ossl_cmp_info(
        ctx, "trying to verify msg signature with a valid cert that..");
        if (sname != NULL)
...
int OSSL_CMP_validate_msg(OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *msg) {
    ...
    case NID_id_PasswordBasedMAC:
        if (ctx->secretValue == NULL) {
            ossl_cmp_info(ctx, "no secret available for verifying PBM-based "
        ...
        if (verify_PBMAC(ctx, msg)) {
            ...
            ossl_cmp_debug(
            ctx, "successfully validated PBM-based CMP message protection");
            return 1;
        }
    ...
    default:
        ...
        if (scrt == NULL) {
            if (ctx->trusted == NULL && ctx->secretValue != NULL) {
                ossl_cmp_info(
                ctx, "no trust store nor pinned server cert available for "
            ...
            if (check_msg_find_cert(ctx, msg)) {
                ossl_cmp_debug(ctx,
                               "successfully validated signature-based CMP "
...
static int check_transactionID_or_nonce(ASN1_OCTET_STRING *expected,
    ...
    if (expected != NULL &&
        ...
        actual_str =
        actual == NULL ? NULL : i2s_ASN1_OCTET_STRING(NULL, actual);
