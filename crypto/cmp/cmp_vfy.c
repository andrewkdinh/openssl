...
static int check_name(const OSSL_CMP_CTX *ctx, int log_success,
  ...
  if (X509_NAME_cmp(actual_name, expect_name) == 0) {
    if (log_success && str != NULL)
      ossl_cmp_log3(INFO, ctx, " %s matches %s: %s", actual_desc, expect_desc,
                    str);
    OPENSSL_free(str);
    return 1;
  }
...
static int cert_acceptable(const OSSL_CMP_CTX *ctx, const char *desc1,
  ...
  if (time_cmp != 0) {
    int err =
    time_cmp > 0 ? X509_V_ERR_CERT_HAS_EXPIRED : X509_V_ERR_CERT_NOT_YET_VALID;

    ossl_cmp_warn(ctx,
                  time_cmp > 0 ? "cert has expired" : "cert is not yet valid");
...
static int check_cert_path_3gpp(const OSSL_CMP_CTX *ctx,
  ...
  if (!valid) {
     ...
     */
    OSSL_CMP_CERTRESPONSE *crep = ossl_cmp_certrepmessage_get0_certresponse(
...
static int check_msg_given_cert(const OSSL_CMP_CTX *ctx, X509 *cert,
                                const OSSL_CMP_MSG *msg) {
  return cert_acceptable(ctx, "previously validated", "sender cert", cert, NULL,
                         NULL, msg) &&
...
static int check_msg_with_certs(OSSL_CMP_CTX *ctx, const STACK_OF(X509) * certs,
  ...
  for (i = 0; i < sk_X509_num(certs); i++) { /* certs may be NULL */
    ...
    if (!ossl_assert(cert != NULL))
      return 0;
    if (!cert_acceptable(ctx, "cert from", desc, cert, already_checked1,
                         already_checked2, msg))
      continue;
    n_acceptable_certs++;
...
static int check_msg_all_certs(OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *msg,
  ...
  if (check_msg_with_certs(ctx, msg->extraCerts, "extraCerts", NULL, NULL, msg,
                           mode_3gpp))
    return 1;
  ...
  if (ctx->trusted == NULL) {
    ossl_cmp_warn(ctx,
                  mode_3gpp ? "no self-issued extraCerts" : "no trusted store");
  } else {
...
static int check_msg_find_cert(OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *msg) {
  ...
  if (scrt != NULL) {
    ...
    ossl_cmp_info(
    ctx, "trying to verify msg signature with previously validated cert");
  ...
  default:
    ...
    if (scrt == NULL) {
      if (ctx->trusted == NULL && ctx->secretValue != NULL) {
        ossl_cmp_info(ctx,
                      "no trust store nor pinned server cert available for "
                      "verifying signature-based CMP message protection");
        ERR_raise(ERR_LIB_CMP, CMP_R_MISSING_TRUST_ANCHOR);
        return 0;
      }
      if (check_msg_find_cert(ctx, msg)) {
        ossl_cmp_debug(ctx, "successfully validated signature-based CMP "
                            "message protection using trust store");
        return 1;
      }
      ...
      if (verify_signature(ctx, msg, scrt)) {
        ossl_cmp_debug(ctx, "successfully validated signature-based CMP "
                            "message protection using pinned server cert");
...
int ossl_cmp_msg_check_update(OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *msg,
  ...
  if (expected_sender != NULL) {
     ...
     */
    if (!check_name(ctx, 0, "sender DN field", actual_sender, "expected sender",
                    expected_sender)) {
  ...
  num_added =
  (ctx->untrusted == NULL ? 0 : sk_X509_num(ctx->untrusted)) - num_untrusted;
  if (!res) {
    while (num_added-- > 0)
  ...
  if (!check_transactionID_or_nonce(ctx->senderNonce, hdr->recipNonce,
    ...
    if (ctx->first_senderNonce == NULL ||
        ...
        ||
        !check_transactionID_or_nonce(ctx->first_senderNonce, hdr->recipNonce,
                                      CMP_R_RECIPNONCE_UNMATCHED)) {
      (void)ERR_clear_last_mark();
      return 0;
    }
