...
static int verify_signature(const OSSL_CMP_CTX *cmp_ctx, const OSSL_CMP_MSG *msg, X509 *cert)
{
    ...
    if (ASN1_item_verify_ex(ASN1_ITEM_rptr(OSSL_CMP_PROTECTEDPART), msg->header->protectionAlg, msg->protection,
                            &prot_part, NULL, pubkey, cmp_ctx->libctx, cmp_ctx->propq)
        > 0) {
        res = 1;
        goto end;
    }
...
static int verify_PBMAC(OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *msg)
{
    ...
    valid = msg->protection != NULL && msg->protection->length >= 0 && msg->protection->type == protection->type
        ...
        && CRYPTO_memcmp(msg->protection->data, protection->data, protection->length) == 0;
...
static int verify_cb_cert(X509_STORE *ts, X509 *cert, int err)
{
    ...
    if (ts == NULL || (verify_cb = X509_STORE_get_verify_cb(ts)) == NULL)
        return ok;
    if ((csc = X509_STORE_CTX_new()) != NULL && X509_STORE_CTX_init(csc, ts, cert, NULL)) {
...
static int check_name(const OSSL_CMP_CTX *ctx, int log_success, const char *actual_desc, const X509_NAME *actual_name,
                      const char *expect_desc, const X509_NAME *expect_name)
{
    ...
    if (X509_NAME_cmp(actual_name, expect_name) == 0) {
        if (log_success && str != NULL)
            ossl_cmp_log3(INFO, ctx, " %s matches %s: %s", actual_desc, expect_desc, str);
        OPENSSL_free(str);
        return 1;
    }
 ...
 */
static int cert_acceptable(const OSSL_CMP_CTX *ctx, const char *desc1, const char *desc2, X509 *cert,
                           const STACK_OF(X509) *already_checked1, const STACK_OF(X509) *already_checked2,
                           const OSSL_CMP_MSG *msg)
{
    ...
    if (time_cmp != 0) {
        int err = time_cmp > 0 ? X509_V_ERR_CERT_HAS_EXPIRED : X509_V_ERR_CERT_NOT_YET_VALID;
    ...
    if (!check_name(ctx, 1, "cert subject", X509_get_subject_name(cert), "sender field",
...
static int check_cert_path_3gpp(const OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *msg, X509 *scrt)
{
    ...
    if ((store = X509_STORE_new()) == NULL
        || !ossl_cmp_X509_STORE_add1_certs(store, msg->extraCerts, 1 /* self-issued only */))
        goto err;
...
static int check_msg_all_certs(OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *msg, int mode_3gpp)
{
    ...
    if (ctx->trusted == NULL) {
        ...
        ret = check_msg_with_certs(ctx, trusted, mode_3gpp ? "self-issued extraCerts" : "certs in trusted store",
                                   msg->extraCerts, ctx->untrusted, msg, mode_3gpp);
...
int OSSL_CMP_validate_msg(OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *msg)
{
    ...
    case NID_id_PasswordBasedMAC:
        ...
        if (verify_PBMAC(ctx, msg)) {
            ...
            ossl_cmp_debug(ctx, "successfully validated PBM-based CMP message protection");
            return 1;
        }
    ...
    default:
        ...
        if (scrt == NULL) {
            ...
            if (check_msg_find_cert(ctx, msg)) {
                ossl_cmp_debug(ctx, "successfully validated signature-based CMP message protection using trust store");
                return 1;
            }
            ...
            if (verify_signature(ctx, msg, scrt)) {
                ossl_cmp_debug(
                    ctx, "successfully validated signature-based CMP message protection using pinned server cert");
...
static int check_transactionID_or_nonce(ASN1_OCTET_STRING *expected, ASN1_OCTET_STRING *actual, int reason)
{
    if (expected != NULL && (actual == NULL || ASN1_OCTET_STRING_cmp(expected, actual) != 0)) {
        ...
        ERR_raise_data(ERR_LIB_CMP, reason, "expected = %s, actual = %s", expected_str == NULL ? "?" : expected_str,
                       actual == NULL           ? "(none)"
                           : actual_str == NULL ? "?"
                                                : actual_str);
 ...
 */
int ossl_cmp_msg_check_update(OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *msg, ossl_cmp_allow_unprotected_cb_t cb,
                              int cb_arg)
{
    ...
    if (expected_sender != NULL) {
         ...
         */
        if (!check_name(ctx, 0, "sender DN field", actual_sender, "expected sender", expected_sender)) {
            str = X509_NAME_oneline(actual_sender, NULL, 0);
            ERR_raise_data(ERR_LIB_CMP, CMP_R_UNEXPECTED_SENDER, str != NULL ? str : "<unknown>");
            OPENSSL_free(str);
            return 0;
        }
    }
    ...
    if (num_added > 10)
        ossl_cmp_log1(WARN, ctx, "received CMP message contains %d extraCerts", num_added);
    /*
    ...
    num_added = (ctx->untrusted == NULL ? 0 : sk_X509_num(ctx->untrusted)) - num_untrusted;
    if (!res) {
        while (num_added-- > 0)
    ...
    if (ossl_cmp_hdr_get_pvno(hdr) != OSSL_CMP_PVNO_2 && ossl_cmp_hdr_get_pvno(hdr) != OSSL_CMP_PVNO_3) {
#ifndef FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION
    ...
    if (!check_transactionID_or_nonce(ctx->senderNonce, hdr->recipNonce, CMP_R_RECIPNONCE_UNMATCHED)) {
        ...
        if (ctx->first_senderNonce == NULL
            ...
            || !check_transactionID_or_nonce(ctx->first_senderNonce, hdr->recipNonce, CMP_R_RECIPNONCE_UNMATCHED)) {
            (void)ERR_clear_last_mark();
            return 0;
        }
    }
...
int ossl_cmp_verify_popo(const OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *msg, int acceptRAVerified)
{
    ...
    case OSSL_CMP_PKIBODY_KUR:
        if (!OSSL_CRMF_MSGS_verify_popo(msg->body->value.ir, OSSL_CMP_CERTREQID, acceptRAVerified, ctx->libctx,
                                        ctx->propq)) {
#ifndef FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION
            return 0;
#endif
