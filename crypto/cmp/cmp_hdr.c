...
int ossl_cmp_hdr_push1_freeText(OSSL_CMP_PKIHEADER *hdr,
    ...
    return ossl_cmp_sk_ASN1_UTF8STRING_push_str(
    hdr->freeText, (char *)text->data, text->length);
}
...
int ossl_cmp_hdr_set_transactionID(OSSL_CMP_CTX *ctx, OSSL_CMP_PKIHEADER *hdr) {
    if (ctx->transactionID == NULL) {
        ...
        if (tid != NULL)
            ossl_cmp_log1(DEBUG, ctx, "Starting new transaction with ID=%s",
                          tid);
        OPENSSL_free(tid);
    }
