  ...
  *rep = (*transfer_cb)(ctx, req);
  ...
  if (*rep == NULL) {
    ERR_raise_data(ERR_LIB_CMP,
                   ...
                   "request sent: %s, expected response: %s", req_type_str,
                   expected_type_str);
    return 0;
  }
   ...
   */
  if (bt == expected_type || (expected_type == OSSL_CMP_PKIBODY_POLLREP
  ...
  *rep = NULL;
  for (;;) {
    ...
    if (OSSL_CMP_MSG_get_bodytype(prep) == OSSL_CMP_PKIBODY_POLLREP) {
      ...
      if (check_after < 0 ||
          (uint64_t)check_after > (sleep ? ULONG_MAX / 1000 : INT_MAX)) {
      ...
      if (pollRep->reason == NULL ||
          (len = BIO_snprintf(str, OSSL_CMP_PKISI_BUFLEN, " with reason = '")) <
          0) {
        *str = '\0';
      } else {
        ...
        if (text == NULL ||
            BIO_snprintf(str + len, sizeof(str) - len, "%s'", text) < 0)
          *str = '\0';
        OPENSSL_free(text);
      }
      ...
      ctx, OSSL_CMP_PKISTATUS_rejection, OSSL_CMP_CTX_FAILINFO_badRequest,
...
static int send_receive_also_delayed(OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *req,
  ...
  if (ossl_cmp_is_error_with_waiting(*rep)) {
    ...
    *rep = NULL;
    ...
    if (poll_for_response(ctx, 1 /* can sleep */, OSSL_CMP_CERTREQID_NONE, rep,
                          NULL /* checkAfter */) <= 0) {
...
int ossl_cmp_exchange_certConf(OSSL_CMP_CTX *ctx, int certReqId, int fail_info,
  ...
  res =
  send_receive_also_delayed(ctx, certConf, &PKIconf, OSSL_CMP_PKIBODY_PKICONF);
...
int ossl_cmp_exchange_error(OSSL_CMP_CTX *ctx, int status, int fail_info,
  ...
  res =
  send_receive_also_delayed(ctx, error, &PKIconf, OSSL_CMP_PKIBODY_PKICONF);
...
static X509 *get1_cert_status(OSSL_CMP_CTX *ctx, int bodytype,
  ...
  case OSSL_CMP_PKISTATUS_waiting:
    ossl_cmp_err(ctx, "received \"waiting\" status for cert when actually "
                      "aiming to extract cert");
  ...
  case OSSL_CMP_PKISTATUS_revocationNotification:
    ossl_cmp_warn(ctx, "received \"revocationNotification\" - a revocation of "
                       "the cert has occurred");
    break;
...
int OSSL_CMP_certConf_cb(OSSL_CMP_CTX *ctx, X509 *cert, int fail_info,
  ...
  if (out_trusted == NULL) {
    ...
    chain = X509_build_chain(cert, ctx->untrusted, out_trusted, 0, ctx->libctx,
                             ctx->propq);
  } else {
  ...
  if (out_trusted != NULL) {
    ...
    ossl_cmp_warn(ctx, "could not build approximate chain for newly enrolled "
                       "cert, resorting to received extraCerts");
    chain = OSSL_CMP_CTX_get1_extraCertsIn(ctx);
  } else {
    ossl_cmp_debug(
    ctx, "success building approximate chain for newly enrolled cert");
  }
...
retry:
  ...
  if (fail_info != 0) /* immediately log error before any certConf exchange */
    ossl_cmp_log1(ERROR, ctx, "rejecting newly enrolled cert with subject: %s",
                  subj);
  /*
  ...
  if (fail_info != 0) {
    ERR_raise_data(ERR_LIB_CMP, CMP_R_CERTIFICATE_NOT_ACCEPTED,
                   "rejecting newly enrolled cert with subject: %s; %s", subj,
                   txt);
    ctx->status = OSSL_CMP_PKISTATUS_rejection;
    ret = 0;
  }
...
int OSSL_CMP_try_certreq(OSSL_CMP_CTX *ctx, int req_type,
  ...
  if (ctx->status != OSSL_CMP_PKISTATUS_waiting) { /* not polling already */
    ...
    if (req_type < 0)
      return ossl_cmp_exchange_error(ctx, OSSL_CMP_PKISTATUS_rejection, 0,
  ...
  res = cert_response(ctx, 0 /* no sleep */, rid, &rep, checkAfter, req_type,
                      rep_type);
...
int OSSL_CMP_exec_RR_ses(OSSL_CMP_CTX *ctx) {
  ...
  if (sk_OSSL_CRMF_CERTID_num(rrep->revCerts) >= 1) {
    ...
    if (ASN1_INTEGER_cmp(serial, OSSL_CRMF_CERTID_get0_serialNumber(cid)) !=
        0) {
#ifndef FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION
