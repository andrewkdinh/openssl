    ...
    *rep = (*transfer_cb)(ctx, req);
    ...
    if (bt != OSSL_CMP_PKIBODY_ERROR) {
        ...
        if (emc->errorDetails != NULL) {
            ...
            emc->errorDetails, ", ", OSSL_CMP_PKISI_BUFLEN - 1);
...
static int poll_for_response(OSSL_CMP_CTX *ctx, int sleep, int rid,
    ...
    ossl_cmp_info(
    ctx, "received 'waiting' PKIStatus, starting to poll for response");
    *rep = NULL;
    for (;;) {
        ...
        if (OSSL_CMP_MSG_get_bodytype(prep) == OSSL_CMP_PKIBODY_POLLREP) {
            ...
            if (pollRep->reason == NULL ||
                ...
                *str = '\0';
                ...
                char *text = ossl_sk_ASN1_UTF8STRING2text(
                pollRep->reason, ", ", sizeof(str) - len - 2);
            ...
            ossl_cmp_log2(
            INFO, ctx, "received polling response%s; checkAfter = %ld seconds",
            str, check_after);
...
static X509 *get1_cert_status(OSSL_CMP_CTX *ctx, int bodytype,
    ...
    case OSSL_CMP_PKISTATUS_revocationWarning:
        ossl_cmp_warn(ctx, "received \"revocationWarning\" - a revocation of "
                           "the cert is imminent");
        break;
    case OSSL_CMP_PKISTATUS_revocationNotification:
        ossl_cmp_warn(ctx,
                      "received \"revocationNotification\" - a revocation of "
...
int OSSL_CMP_certConf_cb(OSSL_CMP_CTX *ctx, X509 *cert, int fail_info,
    ...
    if (out_trusted != NULL) {
        ...
        ossl_cmp_warn(ctx,
                      "could not build approximate chain for newly enrolled "
