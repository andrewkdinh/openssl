 ...
 */
static int unprotected_exception(const OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *rep, int invalid_protection,
...
static int is_crep_with_waiting(const OSSL_CMP_MSG *resp, int rid)
{
    ...
    return (crep != NULL && ossl_cmp_pkisi_get_status(crep->status) == OSSL_CMP_PKISTATUS_waiting);
}
    ...
    *rep = (*transfer_cb)(ctx, req);
    ...
    if (*rep == NULL) {
        ERR_raise_data(ERR_LIB_CMP,
                       ctx->total_timeout != 0 && time(NULL) >= ctx->end_time ? CMP_R_TOTAL_TIMEOUT
     ...
     */
    if (bt == expected_type
        || (expected_type == OSSL_CMP_PKIBODY_POLLREP ? bt != OSSL_CMP_PKIBODY_ERROR
    ...
    if (bt != OSSL_CMP_PKIBODY_ERROR) {
        ...
        if (emc->errorCode != NULL
            && BIO_snprintf(buf, sizeof(buf), "; errorCode: %08lX", ASN1_INTEGER_get(emc->errorCode)) > 0)
            ERR_add_error_data(1, buf);
...
static int poll_for_response(OSSL_CMP_CTX *ctx, int sleep, int rid, OSSL_CMP_MSG **rep, int *checkAfter)
{
    ...
    *rep = NULL;
    for (;;) {
        ...
        } else if (is_crep_with_waiting(prep, rid) || ossl_cmp_is_error_with_waiting(prep)) {
            /* received status must not be 'waiting' */
            (void)ossl_cmp_exchange_error(ctx, OSSL_CMP_PKISTATUS_rejection, OSSL_CMP_CTX_FAILINFO_badRequest,
                                          "polling already started", 0 /* errorCode */, NULL);
            ERR_raise(ERR_LIB_CMP, CMP_R_UNEXPECTED_PKISTATUS);
            goto err;
        } else {
...
static int save_senderNonce_if_waiting(OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *rep, int rid)
{
    /*
     ...
     */
    if ((is_crep_with_waiting(rep, rid) || ossl_cmp_is_error_with_waiting(rep))
 ...
 */
static int send_receive_also_delayed(OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *req, OSSL_CMP_MSG **rep, int expected_type)
{
    ...
    if (ossl_cmp_is_error_with_waiting(*rep)) {
        ...
        *rep = NULL;
        ...
        if (poll_for_response(ctx, 1 /* can sleep */, OSSL_CMP_CERTREQID_NONE, rep, NULL /* checkAfter */) <= 0) {
...
int ossl_cmp_exchange_certConf(OSSL_CMP_CTX *ctx, int certReqId, int fail_info, const char *txt)
{
    ...
    res = send_receive_also_delayed(ctx, certConf, &PKIconf, OSSL_CMP_PKIBODY_PKICONF);
...
int ossl_cmp_exchange_error(OSSL_CMP_CTX *ctx, int status, int fail_info, const char *txt, int errorCode,
                            const char *details)
{
    ...
    res = send_receive_also_delayed(ctx, error, &PKIconf, OSSL_CMP_PKIBODY_PKICONF);
 ...
 */
static int cert_response(OSSL_CMP_CTX *ctx, int sleep, int rid, OSSL_CMP_MSG **resp, int *checkAfter,
                         ossl_unused int req_type, ossl_unused int expected_type)
{
    EVP_PKEY *rkey = NULL;
...
retry:
     ...
     */
    if (crepmsg != NULL && crepmsg->caPubs != NULL && !ossl_cmp_ctx_set1_caPubs(ctx, crepmsg->caPubs))
        return 0;
    ...
    if (fail_info != 0) {
        ERR_raise_data(ERR_LIB_CMP, CMP_R_CERTIFICATE_NOT_ACCEPTED,
                       "rejecting newly enrolled cert with subject: %s; %s", subj, txt);
        ctx->status = OSSL_CMP_PKISTATUS_rejection;
        ret = 0;
    }
    OPENSSL_free(subj);
...
static int initial_certreq(OSSL_CMP_CTX *ctx, int req_type, const OSSL_CRMF_MSG *crm, OSSL_CMP_MSG **p_rep,
                           int rep_type)
{
    OSSL_CMP_MSG *req;
    int res;
...
int OSSL_CMP_try_certreq(OSSL_CMP_CTX *ctx, int req_type, const OSSL_CRMF_MSG *crm, int *checkAfter)
{
    ...
    if (ctx->status != OSSL_CMP_PKISTATUS_waiting) { /* not polling already */
        ...
        if (req_type < 0)
            return ossl_cmp_exchange_error(ctx, OSSL_CMP_PKISTATUS_rejection, 0, "polling aborted", 0 /* errorCode */,
                                           "by application");
...
int OSSL_CMP_exec_RR_ses(OSSL_CMP_CTX *ctx)
{
    ...
    ctx->status = OSSL_CMP_PKISTATUS_request;
    if (ctx->oldCert == NULL && ctx->p10CSR == NULL && (ctx->serialNumber == NULL || ctx->issuer == NULL)) {
