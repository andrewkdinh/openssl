    ...
    *rep = NULL;
    for (;;) {
        ...
        if (OSSL_CMP_MSG_get_bodytype(prep) == OSSL_CMP_PKIBODY_POLLREP) {
            ...
            (void)ossl_cmp_exchange_error(ctx,
...
int ossl_cmp_exchange_error(OSSL_CMP_CTX *ctx,
                            int           status,
                            int           fail_info,
                            const char   *txt,
                            int           errorCode,
...
int OSSL_CMP_certConf_cb(OSSL_CMP_CTX *ctx, X509 *cert, int fail_info, const char **text)
{
    ...
    if (out_trusted == NULL) {
        ...
        if (!ossl_x509_add_certs_new(&chain,
 ...
 */
static int cert_response(OSSL_CMP_CTX   *ctx,
                         int             sleep,
                         int             rid,
                         OSSL_CMP_MSG  **resp,
                         int            *checkAfter,
...
static int
initial_certreq(OSSL_CMP_CTX *ctx, int req_type, const OSSL_CRMF_MSG *crm, OSSL_CMP_MSG **p_rep, int rep_type)
{
    OSSL_CMP_MSG *req;
    int           res;
...
int OSSL_CMP_try_certreq(OSSL_CMP_CTX *ctx, int req_type, const OSSL_CRMF_MSG *crm, int *checkAfter)
{
    ...
    if (ctx->status != OSSL_CMP_PKISTATUS_waiting) { /* not polling already */
        ...
        if (req_type < 0)
            return ossl_cmp_exchange_error(ctx,
                                           OSSL_CMP_PKISTATUS_rejection,
                                           0,
                                           "polling aborted",
                                           0 /* errorCode */,
