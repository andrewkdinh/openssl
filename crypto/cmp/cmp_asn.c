...
ASN1_SEQUENCE(OSSL_CMP_CHALLENGE)
    = {ASN1_OPT(OSSL_CMP_CHALLENGE, owf, X509_ALGOR), ASN1_SIMPLE(OSSL_CMP_CHALLENGE, witness, ASN1_OCTET_STRING),
...
ASN1_ITEM_TEMPLATE(OSSL_CMP_POPODECKEYCHALLCONTENT)
    = ASN1_EX_TEMPLATE_TYPE(ASN1_TFLG_SEQUENCE_OF, 0, OSSL_CMP_POPODECKEYCHALLCONTENT, OSSL_CMP_CHALLENGE)
ASN1_ITEM_TEMPLATE_END(OSSL_CMP_POPODECKEYCHALLCONTENT)

ASN1_ITEM_TEMPLATE(OSSL_CMP_POPODECKEYRESPCONTENT)
    = ASN1_EX_TEMPLATE_TYPE(ASN1_TFLG_SEQUENCE_OF, 0, OSSL_CMP_POPODECKEYRESPCONTENT, ASN1_INTEGER)
...
ASN1_ADB_TEMPLATE(infotypeandvalue_default) = ASN1_OPT(OSSL_CMP_ITAV, infoValue.other, ASN1_ANY);
...
ASN1_ADB(OSSL_CMP_ITAV) = {
    ...
    ADB_ENTRY(NID_id_it_caKeyUpdateInfo,
              ASN1_OPT(OSSL_CMP_ITAV, infoValue.caKeyUpdateInfo, OSSL_CMP_CAKEYUPDANNCONTENT)),
    ...
    ADB_ENTRY(NID_id_it_suppLangTags,
              ASN1_SEQUENCE_OF_OPT(OSSL_CMP_ITAV, infoValue.suppLangTagsValue, ASN1_UTF8STRING)),
    ADB_ENTRY(NID_id_it_caCerts, ASN1_SEQUENCE_OF_OPT(OSSL_CMP_ITAV, infoValue.caCerts, X509)),
    ADB_ENTRY(NID_id_it_rootCaCert, ASN1_OPT(OSSL_CMP_ITAV, infoValue.rootCaCert, X509)),
    ADB_ENTRY(NID_id_it_rootCaKeyUpdate, ASN1_OPT(OSSL_CMP_ITAV, infoValue.rootCaKeyUpdate, OSSL_CMP_ROOTCAKEYUPDATE)),
    ADB_ENTRY(NID_id_it_certReqTemplate, ASN1_OPT(OSSL_CMP_ITAV, infoValue.certReqTemplate, OSSL_CMP_CERTREQTEMPLATE)),
    ...
    ADB_ENTRY(NID_id_it_crlStatusList,
              ASN1_SEQUENCE_OF_OPT(OSSL_CMP_ITAV, infoValue.crlStatusList, OSSL_CMP_CRLSTATUS)),
    ADB_ENTRY(NID_id_it_crls, ASN1_SEQUENCE_OF_OPT(OSSL_CMP_ITAV, infoValue.crls, X509_CRL))}
ASN1_ADB_END
(OSSL_CMP_ITAV, 0, infoType, 0, &infotypeandvalue_default_tt, NULL);

ASN1_SEQUENCE(OSSL_CMP_ITAV)
    = {ASN1_SIMPLE(OSSL_CMP_ITAV, infoType, ASN1_OBJECT), ASN1_ADB_OBJECT(OSSL_CMP_ITAV)}
...
ASN1_ITEM_TEMPLATE(OSSL_CMP_ATAVS)
    = ASN1_EX_TEMPLATE_TYPE(ASN1_TFLG_SEQUENCE_OF, 0, OSSL_CMP_ATAVS, OSSL_CRMF_ATTRIBUTETYPEANDVALUE)
...
ASN1_SEQUENCE(OSSL_CMP_CERTREQTEMPLATE)
    = {ASN1_SIMPLE(OSSL_CMP_CERTREQTEMPLATE, certTemplate, OSSL_CRMF_CERTTEMPLATE),
       ASN1_SEQUENCE_OF_OPT(OSSL_CMP_CERTREQTEMPLATE, keySpec, OSSL_CRMF_ATTRIBUTETYPEANDVALUE)}
...
ASN1_SEQUENCE(OSSL_CMP_CRLSTATUS)
    = {ASN1_SIMPLE(OSSL_CMP_CRLSTATUS, source, OSSL_CMP_CRLSOURCE), ASN1_OPT(OSSL_CMP_CRLSTATUS, thisUpdate, ASN1_TIME)}
...
int OSSL_CMP_ITAV_get0_caCerts(const OSSL_CMP_ITAV *itav, STACK_OF(X509) **out)
{
    ...
    *out = sk_X509_num(itav->infoValue.caCerts) > 0 ? itav->infoValue.caCerts : NULL;
    return 1;
}
...
OSSL_CMP_ITAV *OSSL_CMP_ITAV_new_rootCaCert(const X509 *rootCaCert)
{
    ...
    if (itav == NULL)
        return NULL;
    if (rootCaCert != NULL && (itav->infoValue.rootCaCert = X509_dup(rootCaCert)) == NULL) {
        OSSL_CMP_ITAV_free(itav);
        return NULL;
    }
...
int OSSL_CMP_ITAV_get0_rootCaKeyUpdate(const OSSL_CMP_ITAV *itav, X509 **newWithNew, X509 **newWithOld,
                                       X509 **oldWithNew)
{
...
OSSL_CMP_ITAV
*OSSL_CMP_ITAV_new0_certReqTemplate(OSSL_CRMF_CERTTEMPLATE *certTemplate, OSSL_CMP_ATAVS *keySpec)
{
    OSSL_CMP_ITAV *itav;
...
int OSSL_CMP_ITAV_get1_certReqTemplate(const OSSL_CMP_ITAV *itav, OSSL_CRMF_CERTTEMPLATE **certTemplate,
        ...
        for (i = 0; i < n; i++) {
            ...
            if (nid != NID_id_regCtrl_algId && nid != NID_id_regCtrl_rsaKeyLen) {
                ...
                if (name == NULL)
                    name = "<undef>";
                ERR_raise_data(ERR_LIB_CMP, CMP_R_INVALID_KEYSPEC,
                               "keySpec with index %d in certReqTemplate has invalid type %s", i, name);
                goto err;
            }
...
OSSL_CMP_ATAV *OSSL_CMP_ATAV_new_algId(const X509_ALGOR *alg)
{
    ...
    if ((dup = X509_ALGOR_dup(alg)) == NULL)
        return NULL;
    res = OSSL_CMP_ATAV_create(OBJ_nid2obj(NID_id_regCtrl_algId), (ASN1_TYPE *)dup);
    if (res == NULL)
        X509_ALGOR_free(dup);
    return res;
}
...
OSSL_CMP_ATAV *OSSL_CMP_ATAV_new_rsaKeyLen(int len)
{
    ...
    if (!ASN1_INTEGER_set(aint, len)
        || (res = OSSL_CMP_ATAV_create(OBJ_nid2obj(NID_id_regCtrl_rsaKeyLen), (ASN1_TYPE *)aint)) == NULL)
...
int OSSL_CMP_ITAV_get0_crlStatusList(const OSSL_CMP_ITAV *itav, STACK_OF(OSSL_CMP_CRLSTATUS) **out)
{
    if (itav == NULL || out == NULL) {
        ERR_raise(ERR_LIB_CMP, ERR_R_PASSED_NULL_PARAMETER);
        return 0;
...
OSSL_CMP_CRLSTATUS *OSSL_CMP_CRLSTATUS_new1(const DIST_POINT_NAME *dpn, const GENERAL_NAMES *issuer,
                                            const ASN1_TIME *thisUpdate)
{
    ...
    if (dpn != NULL) {
        ...
        if ((crlsource->value.issuer = sk_GENERAL_NAME_deep_copy(issuer, GENERAL_NAME_dup, GENERAL_NAME_free)) == NULL)
            goto err;
    }

    if (thisUpdate != NULL && (crlstatus->thisUpdate = ASN1_TIME_dup(thisUpdate)) == NULL)
        goto err;
    return crlstatus;
...
static int gennames_allowed(GENERAL_NAMES *names, int only_DN)
{
    ...
    if (!only_DN)
        return 1;
    return sk_GENERAL_NAME_num(names) == 1 && sk_GENERAL_NAME_value(names, 0)->type == GEN_DIRNAME;
}
...
int OSSL_CMP_CRLSTATUS_get0(const OSSL_CMP_CRLSTATUS *crlstatus, DIST_POINT_NAME **dpn, GENERAL_NAMES **issuer,
...
OSSL_CMP_ITAV *OSSL_CMP_ITAV_new_crls(const X509_CRL *crl)
{
    ...
    if (crl != NULL) {
        if ((crls = sk_X509_CRL_new_reserve(NULL, 1)) == NULL || (crl_copy = X509_CRL_dup(crl)) == NULL
...
static int ossl_cmp_msg_cb(int operation, ASN1_VALUE **pval, ossl_unused const ASN1_ITEM *it, void *exarg)
{
...
ASN1_CHOICE(OSSL_CMP_CERTORENCCERT)
    ...
    /* OSSL_CMP_CMPCERTIFICATE is effectively X509 so it is used directly */
        ...
        ASN1_EXP(OSSL_CMP_CERTORENCCERT, value.encryptedCert, OSSL_CRMF_ENCRYPTEDKEY, 1),
    }
...
ASN1_SEQUENCE(OSSL_CMP_CERTIFIEDKEYPAIR)
    = {ASN1_SIMPLE(OSSL_CMP_CERTIFIEDKEYPAIR, certOrEncCert, OSSL_CMP_CERTORENCCERT),
       ...
       ASN1_EXP_OPT(OSSL_CMP_CERTIFIEDKEYPAIR, publicationInfo, OSSL_CRMF_PKIPUBLICATIONINFO, 1)}
...
ASN1_ITEM_TEMPLATE(OSSL_CMP_REVREQCONTENT)
    = ASN1_EX_TEMPLATE_TYPE(ASN1_TFLG_SEQUENCE_OF, 0, OSSL_CMP_REVREQCONTENT, OSSL_CMP_REVDETAILS)
...
ASN1_SEQUENCE(OSSL_CMP_KEYRECREPCONTENT)
    = {ASN1_SIMPLE(OSSL_CMP_KEYRECREPCONTENT, status, OSSL_CMP_PKISI),
       ...
       ASN1_EXP_SEQUENCE_OF_OPT(OSSL_CMP_KEYRECREPCONTENT, keyPairHist, OSSL_CMP_CERTIFIEDKEYPAIR, 2)}
...
ASN1_ITEM_TEMPLATE(OSSL_CMP_CERTCONFIRMCONTENT)
    = ASN1_EX_TEMPLATE_TYPE(ASN1_TFLG_SEQUENCE_OF, 0, OSSL_CMP_CERTCONFIRMCONTENT, OSSL_CMP_CERTSTATUS)
...
ASN1_ITEM_TEMPLATE(OSSL_CMP_POLLREQCONTENT)
    = ASN1_EX_TEMPLATE_TYPE(ASN1_TFLG_SEQUENCE_OF, 0, OSSL_CMP_POLLREQCONTENT, OSSL_CMP_POLLREQ)
...
ASN1_ITEM_TEMPLATE(OSSL_CMP_POLLREPCONTENT)
    = ASN1_EX_TEMPLATE_TYPE(ASN1_TFLG_SEQUENCE_OF, 0, OSSL_CMP_POLLREPCONTENT, OSSL_CMP_POLLREP)
...
ASN1_ITEM_TEMPLATE(OSSL_CMP_GENMSGCONTENT)
    = ASN1_EX_TEMPLATE_TYPE(ASN1_TFLG_SEQUENCE_OF, 0, OSSL_CMP_GENMSGCONTENT, OSSL_CMP_ITAV)
ASN1_ITEM_TEMPLATE_END(OSSL_CMP_GENMSGCONTENT)

ASN1_ITEM_TEMPLATE(OSSL_CMP_GENREPCONTENT)
    = ASN1_EX_TEMPLATE_TYPE(ASN1_TFLG_SEQUENCE_OF, 0, OSSL_CMP_GENREPCONTENT, OSSL_CMP_ITAV)
ASN1_ITEM_TEMPLATE_END(OSSL_CMP_GENREPCONTENT)

ASN1_ITEM_TEMPLATE(OSSL_CMP_CRLANNCONTENT)
    = ASN1_EX_TEMPLATE_TYPE(ASN1_TFLG_SEQUENCE_OF, 0, OSSL_CMP_CRLANNCONTENT, X509_CRL)
...
ASN1_SEQUENCE(OSSL_CMP_PKIHEADER)
    = {ASN1_SIMPLE(OSSL_CMP_PKIHEADER, pvno, ASN1_INTEGER), ASN1_SIMPLE(OSSL_CMP_PKIHEADER, sender, GENERAL_NAME),
    ...
    /* OSSL_CMP_PKIFREETEXT is a ASN1_UTF8STRING sequence, so used directly */
       ...
       ASN1_EXP_SEQUENCE_OF_OPT(OSSL_CMP_PKIHEADER, generalInfo, OSSL_CMP_ITAV, 8)}
ASN1_SEQUENCE_END(OSSL_CMP_PKIHEADER)
IMPLEMENT_ASN1_FUNCTIONS(OSSL_CMP_PKIHEADER)

ASN1_SEQUENCE(OSSL_CMP_PROTECTEDPART)
    = {ASN1_SIMPLE(OSSL_CMP_MSG, header, OSSL_CMP_PKIHEADER), ASN1_SIMPLE(OSSL_CMP_MSG, body, OSSL_CMP_PKIBODY)}
ASN1_SEQUENCE_END(OSSL_CMP_PROTECTEDPART)
IMPLEMENT_ASN1_FUNCTIONS(OSSL_CMP_PROTECTEDPART)

ASN1_SEQUENCE_cb(OSSL_CMP_MSG,
                 ossl_cmp_msg_cb) = {ASN1_SIMPLE(OSSL_CMP_MSG, header, OSSL_CMP_PKIHEADER),
    ...
    IMPLEMENT_ASN1_DUP_FUNCTION(OSSL_CMP_MSG)
        ...
        ASN1_ITEM_TEMPLATE(OSSL_CMP_MSGS) = ASN1_EX_TEMPLATE_TYPE(ASN1_TFLG_SEQUENCE_OF, 0, OSSL_CMP_MSGS, OSSL_CMP_MSG)
