  ...
  while (keymgmt != NULL && keydata == NULL && try_fallback-- > 0) {
    ...
    if (EVP_KEYMGMT_get0_provider(keymgmt) == provider) {
      ...
      if (export_object != NULL) {
         ...
         */
        (void)export_object(ctx->loader_ctx, data->ref, data->ref_size,
...
static EVP_PKEY *try_key_value(struct extracted_param_data_st *data,
  ...
  OSSL_DECODER_CTX_new_for_pkey(&pk, data->input_type, data->data_structure,
                                data->data_type, selection, libctx, propq);
...
static EVP_PKEY *try_key_value_legacy(struct extracted_param_data_st *data,
  ...
  if (pk == NULL &&
      (ctx->expected_type == 0 || ctx->expected_type == OSSL_STORE_INFO_PKEY)) {
    ...
    if (p8 != NULL) {
      ...
      if (!cb(pbuf, sizeof(pbuf), &plen, NULL, cbarg)) {
         ...
         */
        PKCS12_pbe_crypt(alg, pbuf, plen, oct->data, oct->length, &new_der,
                         &len, 0);
        der_len = len;
        der = new_der;
      }
  ...
  if (data->object_type == OSSL_OBJECT_UNKNOWN ||
    ...
    if (data->object_type == OSSL_OBJECT_PKEY && data->ref != NULL) {
      ...
      if (pk == NULL && harderr == 0)
        pk = try_key_value_legacy(data, &store_info_new, ctx, cb, cbarg, libctx,
                                  propq);
    }
  ...
  if (data->object_type == OSSL_OBJECT_UNKNOWN) {
    ...
    if (p12 != NULL) {
      ...
      if (!PKCS12_mac_present(p12) || PKCS12_verify_mac(p12, NULL, 0)) {
        ...
        if (!ossl_pw_get_passphrase(tpass, sizeof(tpass) - 1, &tpass_len,
        ...
        if (!PKCS12_verify_mac(p12, pass, tpass_len)) {
          ERR_raise_data(
          ERR_LIB_OSSL_STORE, OSSL_STORE_R_ERROR_VERIFYING_PKCS12_MAC,
