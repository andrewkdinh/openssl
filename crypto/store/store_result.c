...
int ossl_store_handle_load_result(const OSSL_PARAM params[], void *arg)
{
    ...
    if (p != NULL
        && !OSSL_PARAM_get_octet_string_ptr(p, &helper_data.ref,
    ...
    if (*v == NULL) {
        ...
        if (provider != NULL)
            ERR_raise_data(ERR_LIB_OSSL_STORE, ERR_R_UNSUPPORTED,
                           "provider=%s%s", OSSL_PROVIDER_get0_name(provider),
                           hint);
        else if (hint[0] != '\0')
                               ...
                               int *harderr)
{
    ...
    decoderctx = OSSL_DECODER_CTX_new_for_pkey(
        &pk, data->input_type, data->data_structure, data->data_type, selection,
        libctx, propq);
    ...
    if (pk == NULL
        ...
        if (p8 != NULL) {
            ...
            if (!cb(pbuf, sizeof(pbuf), &plen, NULL, cbarg)) {
                 ...
                 */
                PKCS12_pbe_crypt(alg, pbuf, plen, oct->data, oct->length,
                                 &new_der, &len, 0);
                der_len = len;
                der = new_der;
            }
    ...
    if (data->object_type == OSSL_OBJECT_UNKNOWN
        ...
        if (data->object_type == OSSL_OBJECT_PKEY && data->ref != NULL) {
            ...
            if (pk == NULL && harderr == 0)
                pk = try_key_value_legacy(data, &store_info_new, ctx, cb, cbarg,
                                          libctx, propq);
        }
                    ...
                    OSSL_LIB_CTX *libctx, const char *propq)
{
    if (data->object_type == OSSL_OBJECT_UNKNOWN
        ...
        if (data->data_type != NULL
            && (OPENSSL_strcasecmp(data->data_type, PEM_STRING_X509_TRUSTED)
                == 0))
            ignore_trusted = 0;
