    ...
    for (i = 0; loader_ctx == NULL && i < schemes_n; i++) {
        ...
        if ((loader = ossl_store_get0_loader_int(scheme)) != NULL) {
            ...
            if (loader->open_ex != NULL)
                loader_ctx =
                loader->open_ex(loader, uri, libctx, propq, ui_method, ui_data);
            else
...
#endif
        if (loader == NULL && (fetched_loader = OSSL_STORE_LOADER_fetch(
                               libctx, scheme, propq)) != NULL) {
            ...
            if (fetched_loader->p_open_ex != NULL) {
                loader_ctx = fetched_loader->p_open_ex(
                provctx, uri, params, ossl_pw_passphrase_callback_dec, &pwdata);
            } else {
                if (fetched_loader->p_open != NULL &&
                    (loader_ctx = fetched_loader->p_open(provctx, uri)) !=
                    NULL &&
                    !loader_set_params(fetched_loader, loader_ctx, params,
                                       propq)) {
...
int OSSL_STORE_find(OSSL_STORE_CTX *ctx, const OSSL_STORE_SEARCH *search) {
    ...
    if (ctx->fetched_loader != NULL) {
        ...
        case OSSL_STORE_SEARCH_BY_NAME:
            if ((name_der_sz = i2d_X509_NAME(
                 search->name, (unsigned char **)&name_der)) > 0 &&
                OSSL_PARAM_BLD_push_octet_string(bld, OSSL_STORE_PARAM_SUBJECT,
        ...
        case OSSL_STORE_SEARCH_BY_ISSUER_SERIAL:
            if ((name_der_sz = i2d_X509_NAME(
                ...
                OSSL_PARAM_BLD_push_octet_string(bld, OSSL_STORE_PARAM_ISSUER,
                                                 name_der, name_der_sz) &&
        ...
        case OSSL_STORE_SEARCH_BY_KEY_FINGERPRINT:
            if (OSSL_PARAM_BLD_push_utf8_string(
                bld, OSSL_STORE_PARAM_DIGEST, EVP_MD_get0_name(search->digest),
                0) &&
                OSSL_PARAM_BLD_push_octet_string(
                bld, OSSL_STORE_PARAM_FINGERPRINT, search->string,
                search->stringlength))
                ret = 1;
            break;
        ...
        if (ret) {
            ...
            ret =
            ctx->fetched_loader->p_set_ctx_params(ctx->loader_ctx, params);
...
again:
    ...
    if (ctx->cached_info != NULL) {
        ...
        if (ctx->fetched_loader != NULL) {
            ...
            if (!ctx->fetched_loader->p_load(
                ctx->loader_ctx, ossl_store_handle_load_result, &load_data,
 ...
 */
int OSSL_STORE_INFO_get_type(const OSSL_STORE_INFO *info) { return info->type; }
...
void OSSL_STORE_SEARCH_free(OSSL_STORE_SEARCH *search) { OPENSSL_free(search); }
...
const EVP_MD *
OSSL_STORE_SEARCH_get0_digest(const OSSL_STORE_SEARCH *criterion) {
...
OSSL_STORE_CTX *OSSL_STORE_attach(BIO *bp, const char *scheme,
    ...
    if ((loader = ossl_store_get0_loader_int(scheme)) != NULL)
        loader_ctx =
        loader->attach(loader, bp, libctx, propq, ui_method, ui_data);
#endif
    if (loader == NULL && (fetched_loader = OSSL_STORE_LOADER_fetch(
                           libctx, scheme, propq)) != NULL) {
        const OSSL_PROVIDER *provider =
        ...
        if (cbio == NULL || fetched_loader->p_attach == NULL ||
            ...
            OSSL_STORE_LOADER_free(fetched_loader);
            fetched_loader = NULL;
        } else if (!loader_set_params(fetched_loader, loader_ctx, params,
                                      propq)) {
            (void)fetched_loader->p_close(loader_ctx);
            OSSL_STORE_LOADER_free(fetched_loader);
            fetched_loader = NULL;
