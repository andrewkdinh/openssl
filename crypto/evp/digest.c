...
int EVP_MD_CTX_reset(EVP_MD_CTX *ctx) { return evp_md_ctx_reset_ex(ctx, 0); }
...
static int evp_md_init_internal(EVP_MD_CTX *ctx, const EVP_MD *type,
  ...
  EVP_MD_CTX_clear_flags(ctx,
                         EVP_MD_CTX_FLAG_CLEANED | EVP_MD_CTX_FLAG_FINALISED);
   ...
   */
  if (ctx->engine != NULL && ctx->digest != NULL &&
  ...
  if (type->prov == NULL) {
    ...
    EVP_MD *provmd = EVP_MD_fetch(
    NULL, type->type != NID_undef ? OBJ_nid2sn(type->type) : "NULL", "");
...
legacy:
  if (EVP_MD_xof(ctx->digest) && size <= INT_MAX &&
...
int EVP_Digest(const void *data, size_t count, unsigned char *md,
  ...
  ret = EVP_DigestInit_ex(ctx, type, impl) &&
        EVP_DigestUpdate(ctx, data, count) && EVP_DigestFinal_ex(ctx, md, size);
  EVP_MD_CTX_free(ctx);
...
int EVP_MD_CTX_ctrl(EVP_MD_CTX *ctx, int cmd, int p1, void *p2) {
  ...
  case EVP_MD_CTRL_MICALG:
    set_params = 0;
    params[0] = OSSL_PARAM_construct_utf8_string(OSSL_DIGEST_PARAM_MICALG, p2,
                                                 p1 ? p1 : 9999);
    break;
  case EVP_CTRL_SSL3_MASTER_SECRET:
    params[0] =
    OSSL_PARAM_construct_octet_string(OSSL_DIGEST_PARAM_SSL3_MS, p2, p1);
    break;
  default:
    goto conclude;
  }
...
static int evp_md_up_ref(void *md) { return EVP_MD_up_ref(md); }

static void evp_md_free(void *md) { EVP_MD_free(md); }
...
void EVP_MD_do_all_provided(OSSL_LIB_CTX *libctx,
                            void (*fn)(EVP_MD *mac, void *arg), void *arg) {
  evp_generic_do_all(libctx, OSSL_OP_DIGEST, (void (*)(void *, void *))fn, arg,
                     evp_md_from_algorithm, evp_md_up_ref, evp_md_free);
}

EVP_MD *evp_digest_fetch_from_prov(OSSL_PROVIDER *prov, const char *algorithm,
                                   const char *properties) {
  return evp_generic_fetch_from_prov(prov, OSSL_OP_DIGEST, algorithm,
                                     properties, evp_md_from_algorithm,
