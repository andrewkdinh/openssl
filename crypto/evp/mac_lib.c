...
EVP_MAC *EVP_MAC_CTX_get0_mac(EVP_MAC_CTX *ctx) { return ctx->meth; }
...
int EVP_MAC_init_SKEY(EVP_MAC_CTX *ctx, EVP_SKEY *skey,
                      const OSSL_PARAM params[]) {
  if (ctx->meth->init_skey == NULL || skey->skeymgmt->prov != ctx->meth->prov ||
...
int EVP_MAC_update(EVP_MAC_CTX *ctx, const unsigned char *data,
                   size_t datalen) {
  return ctx->meth->update(ctx->algctx, data, datalen);
}

static int evp_mac_final(EVP_MAC_CTX *ctx, int xof, unsigned char *out,
...
int evp_mac_get_number(const EVP_MAC *mac) { return mac->name_id; }

const char *EVP_MAC_get0_name(const EVP_MAC *mac) { return mac->type_name; }
