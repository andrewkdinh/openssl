...
int evp_cipher_set_asn1_aead_params(EVP_CIPHER_CTX *c, ASN1_TYPE *type,
  ...
  return ossl_asn1_type_set_octetstring_int(
  type, asn1_params->tag_len, asn1_params->iv, asn1_params->iv_len);
}
...
int evp_cipher_cache_constants(EVP_CIPHER *cipher) {
  ...
  params[7] =
  OSSL_PARAM_construct_int(OSSL_CIPHER_PARAM_TLS1_MULTIBLOCK, &multiblock);
  params[8] =
  OSSL_PARAM_construct_int(OSSL_CIPHER_PARAM_HAS_RAND_KEY, &randkey);
...
int EVP_CIPHER_CTX_get_iv_length(const EVP_CIPHER_CTX *ctx) {
  ...
  if (ctx->iv_len < 0) {
    ...
    else if ((EVP_CIPHER_get_flags(ctx->cipher) & EVP_CIPH_CUSTOM_IV_LENGTH) !=
             0) {
      rv =
      EVP_CIPHER_CTX_ctrl((EVP_CIPHER_CTX *)ctx, EVP_CTRL_GET_IVLEN, 0, &len);
      if (rv <= 0)
        return -1;
    }
    /*-
...
const unsigned char *EVP_CIPHER_CTX_original_iv(const EVP_CIPHER_CTX *ctx) {
  ...
  params[0] = OSSL_PARAM_construct_octet_ptr(OSSL_CIPHER_PARAM_IV, (void **)&v,
                                             sizeof(ctx->oiv));
...
int evp_cipher_get_number(const EVP_CIPHER *cipher) { return cipher->name_id; }
...
int evp_md_get_number(const EVP_MD *md) { return md->name_id; }
...
const OSSL_PROVIDER *EVP_MD_get0_provider(const EVP_MD *md) { return md->prov; }

int EVP_MD_get_type(const EVP_MD *md) { return md->type; }

int EVP_MD_get_pkey_type(const EVP_MD *md) { return md->pkey_type; }
...
unsigned long EVP_MD_get_flags(const EVP_MD *md) { return md->flags; }
...
int EVP_MD_meth_get_input_blocksize(const EVP_MD *md) { return md->block_size; }
int EVP_MD_meth_get_result_size(const EVP_MD *md) { return md->md_size; }
int EVP_MD_meth_get_app_datasize(const EVP_MD *md) { return md->ctx_size; }
unsigned long EVP_MD_meth_get_flags(const EVP_MD *md) { return md->flags; }
...
int EVP_MD_CTX_get_size_ex(const EVP_MD_CTX *ctx) {
  ...
  if (gettables != NULL &&
      OSSL_PARAM_locate_const(gettables, OSSL_DIGEST_PARAM_SIZE) != NULL) {
    OSSL_PARAM params[2] = {OSSL_PARAM_END, OSSL_PARAM_END};
    size_t sz = 0;
    ...
    if (EVP_MD_CTX_get_params(c, params) != 1 || sz == SIZE_MAX || sz == 0)
      return -1;
    return sz;
  }
...
void *EVP_MD_CTX_get0_md_data(const EVP_MD_CTX *ctx) { return ctx->md_data; }
...
void EVP_MD_CTX_set_flags(EVP_MD_CTX *ctx, int flags) { ctx->flags |= flags; }
...
int EVP_PKEY_CTX_set_group_name(EVP_PKEY_CTX *ctx, const char *name) {
  ...
  params[0] =
  OSSL_PARAM_construct_utf8_string(OSSL_PKEY_PARAM_GROUP_NAME, (char *)name, 0);
...
int EVP_PKEY_CTX_get_group_name(EVP_PKEY_CTX *ctx, char *name, size_t namelen) {
  ...
  *p++ =
  OSSL_PARAM_construct_utf8_string(OSSL_PKEY_PARAM_GROUP_NAME, name, namelen);
...
#if !defined(FIPS_MODULE)
int EVP_CIPHER_CTX_set_algor_params(EVP_CIPHER_CTX *ctx,
...
int EVP_CIPHER_CTX_get_algor_params(EVP_CIPHER_CTX *ctx, X509_ALGOR *alg) {
  ...
  if ((der = OPENSSL_malloc(derl)) != NULL) {
    ...
    if (EVP_CIPHER_CTX_get_params(ctx, params) &&
        ...
        d2i_ASN1_TYPE(&type, (const unsigned char **)&derp, (int)derl) !=
        NULL) {
      /*
...
int EVP_PKEY_CTX_get_algor_params(EVP_PKEY_CTX *ctx, X509_ALGOR *alg) {
  ...
  if (OSSL_PARAM_modified(&params[0])
      /* ... but, we should get a return size too! */
      && derl != 0 && (der = OPENSSL_malloc(derl)) != NULL) {
    unsigned char *derp = der;
