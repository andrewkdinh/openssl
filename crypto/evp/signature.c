...
static void evp_signature_free(void *data) { EVP_SIGNATURE_free(data); }
...
static void *evp_signature_from_algorithm(int name_id,
    ...
    if (valid &&
        ...
        valid = 0;
    if (valid &&
        (signature->sign != NULL || signature->sign_message_update != NULL ||
         signature->sign_message_final != NULL) &&
    ...
    if (valid && ((signature->digest_verify_init != NULL &&
        ...
        /*
         * You can't have a digest_verify_init without *some* performing
         * functions
         */
        valid = 0;
...
EVP_SIGNATURE *evp_signature_fetch_from_prov(OSSL_PROVIDER *prov,
    ...
    prov, OSSL_OP_SIGNATURE, algorithm, properties,
    evp_signature_from_algorithm, evp_signature_up_ref, evp_signature_free);
}
...
void EVP_SIGNATURE_do_all_provided(OSSL_LIB_CTX *libctx,
                                   void (*fn)(EVP_SIGNATURE *signature,
                                              void *arg),
                                   void *arg) {
    evp_generic_do_all(libctx, OSSL_OP_SIGNATURE, (void (*)(void *, void *))fn,
                       arg, evp_signature_from_algorithm, evp_signature_up_ref,
                       evp_signature_free);
}
...
static int evp_pkey_signature_init(EVP_PKEY_CTX *ctx, EVP_SIGNATURE *signature,
    ...
    if (signature != NULL) {
        ...
        (OSSL_PROVIDER *)tmp_prov, EVP_KEYMGMT_get0_name(ctx->keymgmt),
        ctx->propquery);
        if (tmp_keymgmt != NULL)
        ...
        if (signature->query_key_types != NULL) {
             ...
             */
            if (!ok) {
                const char *signame = evp_keymgmt_util_query_operation_name(
         ...
         * more).
         * If the export call succeeds (returns a non-NULL provider key
         * pointer), we're done and can perform the operation itself.  If not,
         * we perform the second iteration, or jump to legacy.
         */
        for (iter = 1; iter < 3 && provkey == NULL; iter++) {
            EVP_KEYMGMT *tmp_keymgmt_tofree = NULL;

            /*
             * If we're on the second iteration, free the results from the
             * first. They are NULL on the first iteration, so no need to check
            ...
            case 2:
                ...
                (OSSL_PROVIDER *)tmp_prov, supported_sig, ctx->propquery);
                if (signature == NULL)
                    goto legacy;
                break;
            }
    ...
    case EVP_PKEY_OP_VERIFYMSG:
        ...
        ret =
        signature->verify_message_init(ctx->op.sig.algctx, provkey, params);
        break;
...
int EVP_PKEY_sign_message_update(EVP_PKEY_CTX *ctx, const unsigned char *in,
    ...
    return ctx->op.sig.signature->sign_message_update(ctx->op.sig.algctx, in,
                                                      inlen);
}
...
int EVP_PKEY_sign_message_final(EVP_PKEY_CTX *ctx, unsigned char *sig,
    ...
    return ctx->op.sig.signature->sign_message_final(
    ctx->op.sig.algctx, sig, siglen, (sig == NULL) ? 0 : *siglen);
}
...
legacy:
    ...
    M_check_autoarg(
    ctx, sig, siglen,
    EVP_F_EVP_PKEY_SIGN) return ctx->pmeth->sign(ctx, sig, siglen, tbs, tbslen);
}
...
int EVP_PKEY_verify_message_update(EVP_PKEY_CTX *ctx, const unsigned char *in,
    ...
    return ctx->op.sig.signature->verify_message_update(ctx->op.sig.algctx, in,
                                                        inlen);
}
...
int EVP_PKEY_verify(EVP_PKEY_CTX *ctx, const unsigned char *sig, size_t siglen,
    ...
    ret =
    ctx->op.sig.signature->verify(ctx->op.sig.algctx, sig, siglen, tbs, tbslen);
...
int EVP_PKEY_verify_recover(EVP_PKEY_CTX *ctx, unsigned char *rout,
    ...
    ret = ctx->op.sig.signature->verify_recover(
    ctx->op.sig.algctx, rout, routlen, (rout == NULL ? 0 : *routlen), sig,
    siglen);
    return ret;
legacy:
