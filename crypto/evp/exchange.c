...
static EVP_KEYEXCH *evp_keyexch_new(OSSL_PROVIDER *prov)
{
    ...
    if (!CRYPTO_NEW_REF(&exchange->refcnt, 1) || !ossl_provider_up_ref(prov)) {
...
{
    return evp_generic_fetch(ctx, OSSL_OP_KEYEXCH, algorithm, properties, evp_keyexch_from_algorithm,
                             evp_keyexch_up_ref, evp_keyexch_free);
}

EVP_KEYEXCH *evp_keyexch_fetch_from_prov(OSSL_PROVIDER *prov, const char *algorithm, const char *properties)
{
    return evp_generic_fetch_from_prov(prov, OSSL_OP_KEYEXCH, algorithm, properties, evp_keyexch_from_algorithm,
                                       evp_keyexch_up_ref, evp_keyexch_free);
}
...
int EVP_PKEY_derive_init_ex(EVP_PKEY_CTX *ctx, const OSSL_PARAM params[])
{
    ...
    if (ctx->pkey == NULL) {
        ...
        if (pkey == NULL || !EVP_PKEY_set_type_by_keymgmt(pkey, ctx->keymgmt)
...
int EVP_PKEY_derive_set_peer_ex(EVP_PKEY_CTX *ctx, EVP_PKEY *peer, int validate_peer)
{
     ...
     */
    tmp_keymgmt_tofree = tmp_keymgmt =
        evp_keymgmt_fetch_from_prov((OSSL_PROVIDER *)EVP_KEYEXCH_get0_provider(ctx->op.kex.exchange),
...
#else
    if (ctx->pmeth == NULL
        || !(ctx->pmeth->derive != NULL || ctx->pmeth->encrypt != NULL || ctx->pmeth->decrypt != NULL)
        || ctx->pmeth->ctrl == NULL) {
        ERR_raise(ERR_LIB_EVP, EVP_R_OPERATION_NOT_SUPPORTED_FOR_THIS_KEYTYPE);
        return -2;
    }
...
{
    evp_generic_do_all(libctx, OSSL_OP_KEYEXCH, (void (*)(void *, void *))fn, arg, evp_keyexch_from_algorithm,
                       evp_keyexch_up_ref, evp_keyexch_free);
}
