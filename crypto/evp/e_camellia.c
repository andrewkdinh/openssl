...
static int cmll_t4_init_key(EVP_CIPHER_CTX *ctx, const unsigned char *key,
    ...
    if ((mode == EVP_CIPH_ECB_MODE || mode == EVP_CIPH_CBC_MODE) && !enc) {
        ...
        case 128:
            dat->stream.cbc =
            mode == EVP_CIPH_CBC_MODE ? (cbc128_f)cmll128_t4_cbc_decrypt : NULL;
            break;
        case 192:
        case 256:
            dat->stream.cbc =
            mode == EVP_CIPH_CBC_MODE ? (cbc128_f)cmll256_t4_cbc_decrypt : NULL;
            break;
        default:
            ret = -1;
        }
...
static int camellia_init_key(EVP_CIPHER_CTX *ctx, const unsigned char *key,
    ...
    ret =
    Camellia_set_key(key, EVP_CIPHER_CTX_get_key_length(ctx) * 8, &dat->ks);
    if (ret < 0) {
        ERR_raise(ERR_LIB_EVP, EVP_R_CAMELLIA_KEY_SETUP_FAILED);
        return 0;
    }
