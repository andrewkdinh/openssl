...
static int rc4_hmac_md5_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out,
  ...
  if (EVP_CIPHER_CTX_is_encrypting(ctx)) {
    ...
    if (plen > md5_off && (blocks = (plen - md5_off) / MD5_CBLOCK) &&
      ...
      rc4_md5_enc(&key->ks, in + rc4_off, out + rc4_off, &key->md, in + md5_off,
                  blocks);
      blocks *= MD5_CBLOCK;
      rc4_off += blocks;
      md5_off += blocks;
    ...
    if (len > rc4_off && (blocks = (len - rc4_off) / MD5_CBLOCK) &&
      ...
      rc4_md5_enc(&key->ks, in + rc4_off, out + rc4_off, &key->md,
                  out + md5_off, blocks);
      blocks *= MD5_CBLOCK;
      rc4_off += blocks;
      md5_off += blocks;
...
const EVP_CIPHER *EVP_rc4_hmac_md5(void) { return &r4_hmac_md5_cipher; }
#endif
