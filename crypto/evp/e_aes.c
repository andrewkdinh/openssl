...
typedef struct {
    ...
    XTS128_CONTEXT xts;
    void (*stream)(const unsigned char *in, unsigned char *out, size_t length, const AES_KEY *key1, const AES_KEY *key2,
...
static int aes_t4_ocb_init_key(EVP_CIPHER_CTX *ctx, const unsigned char *key, const unsigned char *iv, int enc)
{
    ...
    if (key != NULL) {
        ...
        do {
            ...
            if (!CRYPTO_ocb128_init(&octx->ocb, &octx->ksenc.ks, &octx->ksdec.ks, (block128_f)aes_t4_encrypt,
...
static int s390x_aes_gcm_tls_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t len)
{
    ...
    if (EVP_CIPHER_CTX_ctrl(ctx, enc ? EVP_CTRL_GCM_IV_GEN : EVP_CTRL_GCM_SET_IV_INV, EVP_GCM_TLS_EXPLICIT_IV_LEN, out)
        <= 0)
        goto err;
...
static int s390x_aes_gcm_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t len)
{
    ...
    if (in != NULL) {
        ...
        s390x_kma(gctx->ares, gctx->areslen, gctx->mres, gctx->mreslen, tmp, gctx->fc | S390X_KMA_LAAD | S390X_KMA_LPC,
                  &gctx->kma.param);
...
static void s390x_aes_ccm_aad(S390X_AES_CCM_CTX *ctx, const unsigned char *aad, size_t alen)
{
    ...
    if (rem) {
        ...
        s390x_km(ctx->aes.ccm.kmac_param.icv.b, 16, ctx->aes.ccm.kmac_param.icv.b, ctx->aes.ccm.fc,
...
static int s390x_aes_ccm(S390X_AES_CCM_CTX *ctx, const unsigned char *in, unsigned char *out, size_t len, int enc)
{
    ...
    if (enc) {
        ...
        CRYPTO_ctr128_encrypt_ctr32(in, out, len + rem, &ctx->aes.key.k, ctx->aes.ccm.nonce.b, ctx->aes.ccm.buf.b, &num,
...
static int aes_cfb1_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t len)
{
    ...
    while (len >= MAXBITCHUNK) {
        int num = EVP_CIPHER_CTX_get_num(ctx);
        CRYPTO_cfb128_1_encrypt(in, out, MAXBITCHUNK * 8, &dat->ks, ctx->iv, &num, EVP_CIPHER_CTX_is_encrypting(ctx),
                                dat->block);
        EVP_CIPHER_CTX_set_num(ctx, num);
        len -= MAXBITCHUNK;
        out += MAXBITCHUNK;
    ...
    if (len) {
        int num = EVP_CIPHER_CTX_get_num(ctx);
        CRYPTO_cfb128_1_encrypt(in, out, len * 8, &dat->ks, ctx->iv, &num, EVP_CIPHER_CTX_is_encrypting(ctx),
                                dat->block);
        EVP_CIPHER_CTX_set_num(ctx, num);
    }
...
static int aes_ctr_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in, size_t len)
{
    ...
    if (dat->stream.ctr)
        CRYPTO_ctr128_encrypt_ctr32(in, out, len, &dat->ks, ctx->iv, EVP_CIPHER_CTX_buf_noconst(ctx), &num,
                                    dat->stream.ctr);
    else
        ...
        if (out == NULL) {
            ...
            if (gctx->ctr) {
                ...
                if (len >= 32 && AES_GCM_ASM(gctx)) {
                    ...
                    bulk =
                        AES_gcm_encrypt(in + res, out + res, len - res, gctx->gcm.key, gctx->gcm.Yi.c, gctx->gcm.Xi.u);
                    gctx->gcm.len.u[1] += bulk;
                    bulk += res;
                }
#endif
            ...
            if (gctx->ctr) {
                ...
                if (len >= 16 && AES_GCM_ASM(gctx)) {
                    ...
                    bulk =
                        AES_gcm_decrypt(in + res, out + res, len - res, gctx->gcm.key, gctx->gcm.Yi.c, gctx->gcm.Xi.u);
                    gctx->gcm.len.u[1] += bulk;
                    bulk += res;
                }
#endif
...
static const EVP_CIPHER aes_128_wrap = {
    NID_id_aes128_wrap,       8,    16,   8,    WRAP_FLAGS, EVP_ORIG_GLOBAL, aes_wrap_init_key, aes_wrap_cipher, NULL,
...
static const EVP_CIPHER aes_192_wrap = {
    NID_id_aes192_wrap,       8,    24,   8,    WRAP_FLAGS, EVP_ORIG_GLOBAL, aes_wrap_init_key, aes_wrap_cipher, NULL,
    sizeof(EVP_AES_WRAP_CTX), NULL, NULL, NULL, NULL};
...
static const EVP_CIPHER aes_256_wrap = {
    NID_id_aes256_wrap,       8,    32,   8,    WRAP_FLAGS, EVP_ORIG_GLOBAL, aes_wrap_init_key, aes_wrap_cipher, NULL,
    sizeof(EVP_AES_WRAP_CTX), NULL, NULL, NULL, NULL};
...
static const EVP_CIPHER aes_128_wrap_pad = {
    NID_id_aes128_wrap_pad,   8,    16,   4,    WRAP_FLAGS, EVP_ORIG_GLOBAL, aes_wrap_init_key, aes_wrap_cipher, NULL,
    sizeof(EVP_AES_WRAP_CTX), NULL, NULL, NULL, NULL};
...
static const EVP_CIPHER aes_192_wrap_pad = {
    NID_id_aes192_wrap_pad,   8,    24,   4,    WRAP_FLAGS, EVP_ORIG_GLOBAL, aes_wrap_init_key, aes_wrap_cipher, NULL,
    sizeof(EVP_AES_WRAP_CTX), NULL, NULL, NULL, NULL};
...
static const EVP_CIPHER aes_256_wrap_pad = {
    NID_id_aes256_wrap_pad,   8,    32,   4,    WRAP_FLAGS, EVP_ORIG_GLOBAL, aes_wrap_init_key, aes_wrap_cipher, NULL,
    sizeof(EVP_AES_WRAP_CTX), NULL, NULL, NULL, NULL};
...
# ifdef HWAES_CAPABLE
            if (HWAES_CAPABLE) {
                ...
                if (!CRYPTO_ocb128_init(&octx->ocb, &octx->ksenc.ks, &octx->ksdec.ks, (block128_f)HWAES_encrypt,
                                        (block128_f)HWAES_decrypt, enc ? HWAES_ocb_encrypt : HWAES_ocb_decrypt))
                    return 0;
                break;
            }
# endif
# ifdef VPAES_CAPABLE
            if (VPAES_CAPABLE) {
                ...
                if (!CRYPTO_ocb128_init(&octx->ocb, &octx->ksenc.ks, &octx->ksdec.ks, (block128_f)vpaes_encrypt,
...
# endif
            ...
            if (!CRYPTO_ocb128_init(&octx->ocb, &octx->ksenc.ks, &octx->ksdec.ks, (block128_f)AES_encrypt,
