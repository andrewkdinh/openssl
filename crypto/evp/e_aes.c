...
static int aesni_cbc_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out,
  ...
  aesni_cbc_encrypt(in, out, len, &EVP_C_DATA(EVP_AES_KEY, ctx)->ks.ks, ctx->iv,
...
static int aesni_ocb_init_key(EVP_CIPHER_CTX *ctx, const unsigned char *key,
  ...
  if (key != NULL) {
    ...
    do {
      ...
      if (!CRYPTO_ocb128_init(&octx->ocb, &octx->ksenc.ks, &octx->ksdec.ks,
                              (block128_f)aesni_encrypt,
...
#define BLOCK_CIPHER_generic(nid, keylen, blocksize, ivlen, nmode, mode, MODE, \
                             flags)                                            \
  static const EVP_CIPHER aesni_##keylen##_##mode = {nid##_##keylen##_##nmode, \
                                                     blocksize,                \
                                                     keylen / 8,               \
                                                     ivlen,                    \
                                                     flags |                   \
                                                     ...
                                                     NULL,                     \
                                                     NULL};                    \
  static const EVP_CIPHER aes_##keylen##_##mode = {nid##_##keylen##_##nmode,   \
                                                   blocksize,                  \
                                                   keylen / 8,                 \
                                                   ivlen,                      \
                                                   flags |                     \
...
static int aes_t4_init_key(EVP_CIPHER_CTX *ctx, const unsigned char *key,
  ...
  if ((mode == EVP_CIPH_ECB_MODE || mode == EVP_CIPH_CBC_MODE) && !enc) {
    ...
    case 128:
      dat->stream.cbc =
      mode == EVP_CIPH_CBC_MODE ? (cbc128_f)aes128_t4_cbc_decrypt : NULL;
      break;
    case 192:
      dat->stream.cbc =
      mode == EVP_CIPH_CBC_MODE ? (cbc128_f)aes192_t4_cbc_decrypt : NULL;
      break;
    case 256:
      dat->stream.cbc =
      mode == EVP_CIPH_CBC_MODE ? (cbc128_f)aes256_t4_cbc_decrypt : NULL;
      break;
    default:
      ret = -1;
    }
...
static int s390x_aes_cfb_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out,
  ...
  if (rem) {
    s390x_km(cctx->kmf.param.cv, 16, cctx->kmf.param.cv, S390X_AES_FC(keylen),
             cctx->kmf.param.k);
...
static int s390x_aes_gcm(S390X_AES_GCM_CTX *ctx, const unsigned char *in,
  ...
  len &= ~(size_t)0xf;
  if (len) {
    s390x_kma(ctx->ares, ctx->areslen, in, len, out, ctx->fc | S390X_KMA_LAAD,
              &ctx->kma.param);
    in += len;
    out += len;
    ctx->fc |= S390X_KMA_HS;
...
static void s390x_aes_gcm_setiv(S390X_AES_GCM_CTX *ctx,
  ...
  if (ctx->ivlen == 12) {
    ...
    s390x_kma(ctx->iv, S390X_gcm_ivpadlen(ctx->ivlen), NULL, 0, NULL, ctx->fc,
              &ctx->kma.param);
    ctx->fc |= S390X_KMA_HS;
...
static void s390x_aes_ccm_aad(S390X_AES_CCM_CTX *ctx, const unsigned char *aad,
  ...
  if (rem) {
    ...
    s390x_km(ctx->aes.ccm.kmac_param.icv.b, 16, ctx->aes.ccm.kmac_param.icv.b,
...
static int s390x_aes_ccm(S390X_AES_CCM_CTX *ctx, const unsigned char *in,
  ...
  if (enc) {
    ...
    CRYPTO_ctr128_encrypt_ctr32(in, out, len + rem, &ctx->aes.key.k,
                                ctx->aes.ccm.nonce.b, ctx->aes.ccm.buf.b, &num,
...
static int s390x_aes_ccm_init_key(EVP_CIPHER_CTX *ctx, const unsigned char *key,
  ...
  if (key != NULL) {
    ...
    cctx->aes.ccm.nonce.b[0] =
    ((cctx->aes.ccm.l - 1) & 0x7) | (((cctx->aes.ccm.m - 2) >> 1) & 0x7) << 3;
...
static int aes_cfb1_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out,
  ...
  while (len >= MAXBITCHUNK) {
    int num = EVP_CIPHER_CTX_get_num(ctx);
    CRYPTO_cfb128_1_encrypt(in, out, MAXBITCHUNK * 8, &dat->ks, ctx->iv, &num,
                            EVP_CIPHER_CTX_is_encrypting(ctx), dat->block);
    EVP_CIPHER_CTX_set_num(ctx, num);
    len -= MAXBITCHUNK;
    out += MAXBITCHUNK;
  ...
  if (len) {
    int num = EVP_CIPHER_CTX_get_num(ctx);
    CRYPTO_cfb128_1_encrypt(in, out, len * 8, &dat->ks, ctx->iv, &num,
                            EVP_CIPHER_CTX_is_encrypting(ctx), dat->block);
    EVP_CIPHER_CTX_set_num(ctx, num);
  }
...
#define CUSTOM_FLAGS                                                           \
  (EVP_CIPH_FLAG_DEFAULT_ASN1 | EVP_CIPH_CUSTOM_IV |                           \
   ...
   EVP_CIPH_CTRL_INIT | EVP_CIPH_CUSTOM_COPY | EVP_CIPH_CUSTOM_IV_LENGTH)
...
const EVP_CIPHER *EVP_aes_192_wrap(void) { return &aes_192_wrap; }
...
const EVP_CIPHER *EVP_aes_256_wrap(void) { return &aes_256_wrap; }
...
const EVP_CIPHER *EVP_aes_192_wrap_pad(void) { return &aes_192_wrap_pad; }
...
const EVP_CIPHER *EVP_aes_256_wrap_pad(void) { return &aes_256_wrap_pad; }
...
static int aes_ocb_ctrl(EVP_CIPHER_CTX *c, int type, int arg, void *ptr) {
  ...
  case EVP_CTRL_COPY:
    ...
    return CRYPTO_ocb128_copy_ctx(&new_octx->ocb, &octx->ocb,
                                  &new_octx->ksenc.ks, &new_octx->ksdec.ks);
