...
static int aes_cfb1_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out,
    ...
    while (len >= MAXBITCHUNK) {
        int num = EVP_CIPHER_CTX_get_num(ctx);
        CRYPTO_cfb128_1_encrypt(in, out, MAXBITCHUNK * 8, &dat->ks, ctx->iv,
    ...
    if (in) {
        if (out == NULL) {
            ...
            if (gctx->ctr) {
                ...
                if (CRYPTO_gcm128_encrypt_ctr32(
                    &gctx->gcm, in + bulk, out + bulk, len - bulk, gctx->ctr))
                    return -1;
            } else {
                size_t bulk = 0;
#if defined(AES_GCM_ASM2)
        ...
        if (!EVP_CIPHER_CTX_is_encrypting(ctx)) {
            ...
            if (CRYPTO_gcm128_finish(
                &gctx->gcm, EVP_CIPHER_CTX_buf_noconst(ctx), gctx->taglen) != 0)
                return -1;
            gctx->iv_set = 0;
            return 0;
        }
...
static int aes_xts_init_key(EVP_CIPHER_CTX *ctx, const unsigned char *key,
    ...
    if (key != NULL) {
        do {
            ...
            if (BSAES_CAPABLE)
                xctx->stream =
                enc ? ossl_bsaes_xts_encrypt : ossl_bsaes_xts_decrypt;
            else
#endif
#ifdef VPAES_CAPABLE
            if (VPAES_CAPABLE) {
    ...
    if (key != NULL) {
        ...
        do {
            ...
            if (VPAES_CAPABLE) {
                ...
                if (!CRYPTO_ocb128_init(
                    &octx->ocb, &octx->ksenc.ks, &octx->ksdec.ks,
                    (block128_f)vpaes_encrypt, (block128_f)vpaes_decrypt, NULL))
                    return 0;
                break;
            }
#endif
