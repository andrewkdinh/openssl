...
static void evp_asym_cipher_free(void *data) { EVP_ASYM_CIPHER_free(data); }
...
static int evp_pkey_asym_cipher_init(EVP_PKEY_CTX *ctx, int operation,
  ...
  supported_ciph =
  evp_keymgmt_util_query_operation_name(ctx->keymgmt, OSSL_OP_ASYM_CIPHER);
...
legacy:
  ...
  M_check_autoarg(ctx, out, outlen,
                  EVP_F_EVP_PKEY_ENCRYPT) return ctx->pmeth->encrypt(ctx, out,
                                                                     outlen, in,
                                                                     inlen);
}
...
legacy:
  ...
  M_check_autoarg(ctx, out, outlen,
                  EVP_F_EVP_PKEY_DECRYPT) return ctx->pmeth->decrypt(ctx, out,
                                                                     outlen, in,
                                                                     inlen);
}
...
static void *evp_asym_cipher_from_algorithm(int name_id,
  ...
  if (ctxfncnt != 2 || (encfncnt != 0 && encfncnt != 2) ||
      (decfncnt != 0 && decfncnt != 2) || (encfncnt != 2 && decfncnt != 2) ||
...
EVP_ASYM_CIPHER *evp_asym_cipher_fetch_from_prov(OSSL_PROVIDER *prov,
  ...
  prov, OSSL_OP_ASYM_CIPHER, algorithm, properties,
  evp_asym_cipher_from_algorithm, evp_asym_cipher_up_ref, evp_asym_cipher_free);
}
...
void EVP_ASYM_CIPHER_do_all_provided(OSSL_LIB_CTX *libctx,
                                     void (*fn)(EVP_ASYM_CIPHER *cipher,
                                                void *arg),
                                     void *arg) {
  evp_generic_do_all(libctx, OSSL_OP_ASYM_CIPHER, (void (*)(void *, void *))fn,
                     arg, evp_asym_cipher_from_algorithm,
                     evp_asym_cipher_up_ref, evp_asym_cipher_free);
}
...
const OSSL_PARAM *
EVP_ASYM_CIPHER_gettable_ctx_params(const EVP_ASYM_CIPHER *cip) {
  void *provctx;
...
const OSSL_PARAM *
EVP_ASYM_CIPHER_settable_ctx_params(const EVP_ASYM_CIPHER *cip) {
  void *provctx;
