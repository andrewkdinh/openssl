 ...
 */
static int match_type(const EVP_KEYMGMT *keymgmt1,
                      const EVP_KEYMGMT *keymgmt2) {
...
void *evp_keymgmt_util_export_to_provider(EVP_PKEY *pk, EVP_KEYMGMT *keymgmt,
   ...
   * created.
   */
  if (pk->keymgmt == keymgmt || (pk->keymgmt->name_id == keymgmt->name_id &&
...
OP_CACHE_ELEM *evp_keymgmt_util_find_operation_cache(EVP_PKEY *pk,
  ...
  for (i = 0; i < end; i++) {
    ...
    if ((p->selection & selection) == selection &&
        (keymgmt == p->keymgmt || (keymgmt->name_id == p->keymgmt->name_id &&
...
void evp_keymgmt_util_cache_keyinfo(EVP_PKEY *pk) {
  ...
  if (pk->keydata != NULL) {
    ...
    params[1] =
    OSSL_PARAM_construct_int(OSSL_PKEY_PARAM_SECURITY_BITS, &security_bits);
...
int evp_keymgmt_util_match(EVP_PKEY *pk1, EVP_PKEY *pk2, int selection) {
  ...
  if (keymgmt1 != keymgmt2) {
    ...
    if (keymgmt2 != NULL && keymgmt2->match != NULL) {
      ...
      if (keydata1 != NULL) {
        tmp_keydata =
        evp_keymgmt_util_export_to_provider(pk1, keymgmt2, selection);
        ok = (tmp_keydata != NULL);
      }
      if (ok) {
        keymgmt1 = keymgmt2;
    ...
    if (!ok && keymgmt1 != NULL && keymgmt1->match != NULL) {
      ...
      if (keydata2 != NULL) {
        tmp_keydata =
        evp_keymgmt_util_export_to_provider(pk2, keymgmt1, selection);
        ok = (tmp_keydata != NULL);
      }
      if (ok) {
        keymgmt2 = keymgmt1;
...
int evp_keymgmt_util_copy(EVP_PKEY *to, EVP_PKEY *from, int selection) {
  ...
  if (to_keymgmt == from->keymgmt && to_keymgmt->dup != NULL &&
    ...
    if (!evp_keymgmt_util_export(from, selection, &evp_keymgmt_util_try_import,
                                 &import_data))
      return 0;
