...
const EVP_PKEY_METHOD *EVP_PKEY_meth_find(int type) {
  ...
  t = &tmp;
  ret =
  OBJ_bsearch_pmeth_func(&t, standard_methods, OSSL_NELEM(standard_methods));
...
const OSSL_PARAM *EVP_PKEY_CTX_gettable_params(const EVP_PKEY_CTX *ctx) {
  ...
  if (EVP_PKEY_CTX_IS_DERIVE_OP(ctx) && ctx->op.kex.exchange != NULL &&
    ...
    provctx =
    ossl_provider_ctx(EVP_KEYEXCH_get0_provider(ctx->op.kex.exchange));
  ...
  if (EVP_PKEY_CTX_IS_SIGNATURE_OP(ctx) && ctx->op.sig.signature != NULL &&
    ...
    provctx =
    ossl_provider_ctx(EVP_SIGNATURE_get0_provider(ctx->op.sig.signature));
  ...
  if (EVP_PKEY_CTX_IS_ASYM_CIPHER_OP(ctx) && ctx->op.ciph.cipher != NULL &&
    ...
    provctx =
    ossl_provider_ctx(EVP_ASYM_CIPHER_get0_provider(ctx->op.ciph.cipher));
...
int EVP_PKEY_CTX_get_signature_md(EVP_PKEY_CTX *ctx, const EVP_MD **md) {
  ...
  *p++ = OSSL_PARAM_construct_utf8_string(OSSL_SIGNATURE_PARAM_DIGEST, name,
                                          sizeof(name));
...
int EVP_PKEY_CTX_set_signature_md(EVP_PKEY_CTX *ctx, const EVP_MD *md) {
  return evp_pkey_ctx_set_md(ctx, md, ctx->op.sig.algctx == NULL,
                             OSSL_SIGNATURE_PARAM_DIGEST, EVP_PKEY_OP_TYPE_SIG,
                             EVP_PKEY_CTRL_MD);
}
...
static int evp_pkey_ctx_add1_octet_string(EVP_PKEY_CTX *ctx, int fallback,
  ...
  if (gettables == NULL || OSSL_PARAM_locate_const(gettables, param) == NULL)
    return evp_pkey_ctx_set1_octet_string(ctx, fallback, param, op, ctrl, data,
                                          datalen);
...
int EVP_PKEY_CTX_set1_tls1_prf_secret(EVP_PKEY_CTX *ctx,
  ...
  ctx, ctx->op.kex.algctx == NULL, OSSL_KDF_PARAM_SECRET, EVP_PKEY_OP_DERIVE,
...
int EVP_PKEY_CTX_add1_tls1_prf_seed(EVP_PKEY_CTX *ctx,
  ...
  return evp_pkey_ctx_set1_octet_string(ctx, ctx->op.kex.algctx == NULL,
                                        OSSL_KDF_PARAM_SEED, EVP_PKEY_OP_DERIVE,
...
int EVP_PKEY_CTX_set1_hkdf_salt(EVP_PKEY_CTX *ctx, const unsigned char *salt,
                                int saltlen) {
  return evp_pkey_ctx_set1_octet_string(ctx, ctx->op.kex.algctx == NULL,
                                        OSSL_KDF_PARAM_SALT, EVP_PKEY_OP_DERIVE,
...
int EVP_PKEY_CTX_set1_hkdf_key(EVP_PKEY_CTX *ctx, const unsigned char *key,
                               int keylen) {
  return evp_pkey_ctx_set1_octet_string(ctx, ctx->op.kex.algctx == NULL,
                                        OSSL_KDF_PARAM_KEY, EVP_PKEY_OP_DERIVE,
...
int EVP_PKEY_CTX_add1_hkdf_info(EVP_PKEY_CTX *ctx, const unsigned char *info,
  ...
  return evp_pkey_ctx_add1_octet_string(ctx, ctx->op.kex.algctx == NULL,
                                        OSSL_KDF_PARAM_INFO, EVP_PKEY_OP_DERIVE,
...
int EVP_PKEY_CTX_set1_pbe_pass(EVP_PKEY_CTX *ctx, const char *pass,
                               int passlen) {
  return evp_pkey_ctx_set1_octet_string(
  ctx, ctx->op.kex.algctx == NULL, OSSL_KDF_PARAM_PASSWORD, EVP_PKEY_OP_DERIVE,
...
int EVP_PKEY_CTX_set1_scrypt_salt(EVP_PKEY_CTX *ctx, const unsigned char *salt,
                                  int saltlen) {
  return evp_pkey_ctx_set1_octet_string(
  ctx, ctx->op.kex.algctx == NULL, OSSL_KDF_PARAM_SALT, EVP_PKEY_OP_DERIVE,
...
int EVP_PKEY_CTX_set_scrypt_N(EVP_PKEY_CTX *ctx, uint64_t n) {
  return evp_pkey_ctx_set_uint64(ctx, OSSL_KDF_PARAM_SCRYPT_N,
                                 EVP_PKEY_OP_DERIVE, EVP_PKEY_CTRL_SCRYPT_N, n);
}

int EVP_PKEY_CTX_set_scrypt_r(EVP_PKEY_CTX *ctx, uint64_t r) {
  return evp_pkey_ctx_set_uint64(ctx, OSSL_KDF_PARAM_SCRYPT_R,
                                 EVP_PKEY_OP_DERIVE, EVP_PKEY_CTRL_SCRYPT_R, r);
}

int EVP_PKEY_CTX_set_scrypt_p(EVP_PKEY_CTX *ctx, uint64_t p) {
  return evp_pkey_ctx_set_uint64(ctx, OSSL_KDF_PARAM_SCRYPT_P,
                                 EVP_PKEY_OP_DERIVE, EVP_PKEY_CTRL_SCRYPT_P, p);
}

int EVP_PKEY_CTX_set_scrypt_maxmem_bytes(EVP_PKEY_CTX *ctx,
                                         uint64_t maxmem_bytes) {
  return evp_pkey_ctx_set_uint64(
  ctx, OSSL_KDF_PARAM_SCRYPT_MAXMEM, EVP_PKEY_OP_DERIVE,
...
int EVP_PKEY_CTX_set_kem_op(EVP_PKEY_CTX *ctx, const char *op) {
  ...
  *p++ =
  OSSL_PARAM_construct_utf8_string(OSSL_KEM_PARAM_OPERATION, (char *)op, 0);
  *p = OSSL_PARAM_construct_end();
  return EVP_PKEY_CTX_set_params(ctx, params);
}
...
static int evp_pkey_ctx_ctrl_str_int(EVP_PKEY_CTX *ctx, const char *name,
  ...
  case EVP_PKEY_STATE_LEGACY:
    ...
    if (strcmp(name, "digest") == 0)
      ret = EVP_PKEY_CTX_md(ctx, EVP_PKEY_OP_TYPE_SIG | EVP_PKEY_OP_TYPE_CRYPT,
  ...
  if (ret && ctx->cached_parameters.dist_id_set) {
    ...
    if (name != NULL)
      ...
      ret = evp_pkey_ctx_ctrl_int(ctx, -1, ctx->operation,
                                  EVP_PKEY_CTRL_SET1_ID, (int)len, (void *)val);
  }
...
int EVP_PKEY_CTX_get_operation(EVP_PKEY_CTX *ctx) { return ctx->operation; }
...
void EVP_PKEY_CTX_set_data(EVP_PKEY_CTX *ctx, void *data) { ctx->data = data; }

void *EVP_PKEY_CTX_get_data(const EVP_PKEY_CTX *ctx) { return ctx->data; }

EVP_PKEY *EVP_PKEY_CTX_get0_pkey(EVP_PKEY_CTX *ctx) { return ctx->pkey; }

EVP_PKEY *EVP_PKEY_CTX_get0_peerkey(EVP_PKEY_CTX *ctx) { return ctx->peerkey; }
