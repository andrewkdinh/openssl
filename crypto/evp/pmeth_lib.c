...
EVP_PKEY_CTX *EVP_PKEY_CTX_dup(const EVP_PKEY_CTX *pctx) {
    ...
    if (pctx->pmeth == NULL) {
        if (rctx->operation == EVP_PKEY_OP_UNDEFINED) {
            ...
            pctx->pkey, pctx->libctx, &tmp_keymgmt, pctx->propquery);
            if (provkey == NULL)
                goto err;
...
int EVP_PKEY_CTX_set_params(EVP_PKEY_CTX *ctx, const OSSL_PARAM *params) {
    ...
    case EVP_PKEY_STATE_PROVIDER:
        ...
        if (EVP_PKEY_CTX_IS_GEN_OP(ctx) && ctx->keymgmt != NULL &&
            ...
            return evp_keymgmt_gen_set_params(ctx->keymgmt,
                                              ctx->op.keymgmt.genctx, params);
...
int EVP_PKEY_CTX_get_params(EVP_PKEY_CTX *ctx, OSSL_PARAM *params) {
    ...
    case EVP_PKEY_STATE_PROVIDER:
        ...
        if (EVP_PKEY_CTX_IS_GEN_OP(ctx) && ctx->keymgmt != NULL &&
            ...
            return evp_keymgmt_gen_get_params(ctx->keymgmt,
                                              ctx->op.keymgmt.genctx, params);
        break;
    case EVP_PKEY_STATE_UNKNOWN:
        break;
...
int EVP_PKEY_CTX_add1_tls1_prf_seed(EVP_PKEY_CTX *ctx,
    ...
    return evp_pkey_ctx_set1_octet_string(
    ctx, ctx->op.kex.algctx == NULL, OSSL_KDF_PARAM_SEED, EVP_PKEY_OP_DERIVE,
...
int EVP_PKEY_CTX_set1_hkdf_key(EVP_PKEY_CTX *ctx, const unsigned char *key,
                               int keylen) {
    return evp_pkey_ctx_set1_octet_string(
    ctx, ctx->op.kex.algctx == NULL, OSSL_KDF_PARAM_KEY, EVP_PKEY_OP_DERIVE,
...
int EVP_PKEY_CTX_add1_hkdf_info(EVP_PKEY_CTX *ctx, const unsigned char *info,
    ...
    ctx, ctx->op.kex.algctx == NULL, OSSL_KDF_PARAM_INFO, EVP_PKEY_OP_DERIVE,
...
int EVP_PKEY_CTX_set1_pbe_pass(EVP_PKEY_CTX *ctx, const char *pass,
    ...
    EVP_PKEY_OP_DERIVE, EVP_PKEY_CTRL_PASS, (const unsigned char *)pass,
    passlen);
}
...
int EVP_PKEY_CTX_ctrl(EVP_PKEY_CTX *ctx, int keytype, int optype, int cmd,
    ...
    ERR_set_mark();
    ret =
    evp_pkey_ctx_store_cached_data(ctx, keytype, optype, cmd, NULL, p2, p1);
    if (ret == -2) {
        ERR_pop_to_mark();
    } else {
        ERR_clear_last_mark();
...
static int evp_pkey_ctx_ctrl_str_int(EVP_PKEY_CTX *ctx, const char *name,
    ...
    case EVP_PKEY_STATE_LEGACY:
        ...
        if (strcmp(name, "digest") == 0)
            ret =
            EVP_PKEY_CTX_md(ctx, EVP_PKEY_OP_TYPE_SIG | EVP_PKEY_OP_TYPE_CRYPT,
...
int evp_pkey_ctx_use_cached_data(EVP_PKEY_CTX *ctx) {
    ...
    if (ret && ctx->cached_parameters.dist_id_set) {
        ...
        if (name != NULL)
            ...
            ret =
            evp_pkey_ctx_ctrl_int(ctx, -1, ctx->operation,
