...
nonlegacy:
    ...
    if (enc) {
        ...
        return ctx->cipher->einit(
            ctx->algctx, key,
            key == NULL ? 0 : EVP_CIPHER_CTX_get_key_length(ctx), iv,
            iv == NULL ? 0 : EVP_CIPHER_CTX_get_iv_length(ctx), params);
    }
                             ...
                             const unsigned char **in, const size_t *inl)
{
    ...
    for (i = 0; i < ctx->numpipes; i++) outl[i] = 0;
...
legacy:
    ...
    *outl = 0;
    ...
    if (b > 1) {
        ...
        for (i = 0; i < n; i++) out[i] = ctx->final[i];
        *outl = n;
    }
    return 1;
}
...
int EVP_CIPHER_CTX_ctrl(EVP_CIPHER_CTX *ctx, int type, int arg, void *ptr)
{
    ...
    case EVP_CTRL_AEAD_SET_TAG:
        params[0] = OSSL_PARAM_construct_octet_string(
            OSSL_CIPHER_PARAM_AEAD_TAG, ptr, sz);
        break;
    case EVP_CTRL_AEAD_TLS1_AAD:
        ...
        params[0] = OSSL_PARAM_construct_size_t(
            OSSL_CIPHER_PARAM_AEAD_TLS1_AAD_PAD, &sz);
                                       ...
                                       OSSL_PROVIDER *prov)
{
    ...
    if ((fnciphcnt != 0 && fnciphcnt != 3 && fnciphcnt != 4)
        ...
        || (fnpipecnt != 0
            && (fnpipecnt < 3 || cipher->p_cupdate == NULL
