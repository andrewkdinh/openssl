...
nonlegacy:
    ...
    if (enc) {
        ...
        ctx->algctx, key, key == NULL ? 0 : EVP_CIPHER_CTX_get_key_length(ctx),
        iv, iv == NULL ? 0 : EVP_CIPHER_CTX_get_iv_length(ctx), params);
    }
...
static int evp_cipher_init_skey_internal(EVP_CIPHER_CTX *ctx,
    ...
    if (enc) {
        if (ctx->cipher->einit_skey == NULL) {
            /*
             *  When skey is NULL, it's a multiple-step init as the current API
             * does. Otherwise we try to fallback for providers that do not
             * support SKEYs.
             */
            ...
            if (skey != NULL &&
                !EVP_SKEY_get0_raw_key(skey, &keydata, &keylen)) {
                ERR_raise(ERR_LIB_EVP, EVP_R_INITIALIZATION_ERROR);
                return 0;
            }

            ret = ctx->cipher->einit(ctx->algctx, keydata, keylen, iv, iv_len,
                                     params);
        } else {
        ...
        if (ctx->cipher->dinit_skey == NULL) {
            ...
            ret = ctx->cipher->dinit(ctx->algctx, keydata, keylen, iv, iv_len,
                                     params);
        } else {
...
int EVP_CipherPipelineUpdate(EVP_CIPHER_CTX *ctx, unsigned char **out,
    ...
    return ctx->cipher->p_cupdate(ctx->algctx, ctx->numpipes, out, outl,
                                  outsize, in, inl);
}
...
int EVP_CipherPipelineFinal(EVP_CIPHER_CTX *ctx, unsigned char **out,
    ...
    return ctx->cipher->p_cfinal(ctx->algctx, ctx->numpipes, out, outl,
                                 outsize);
}
...
int EVP_CIPHER_CTX_ctrl(EVP_CIPHER_CTX *ctx, int type, int arg, void *ptr) {
    ...
    case EVP_CTRL_RAND_KEY: /* Used by DES */
        ...
        OSSL_CIPHER_PARAM_RANDOM_KEY, ptr, sz);
        break;
    ...
    case EVP_CTRL_AEAD_TLS1_AAD:
        ...
        params[0] = OSSL_PARAM_construct_octet_string(
        OSSL_CIPHER_PARAM_AEAD_TLS1_AAD, ptr, sz);
    ...
    case EVP_CTRL_SET_RC2_KEY_BITS:
        params[0] =
        OSSL_PARAM_construct_size_t(OSSL_CIPHER_PARAM_RC2_KEYBITS, &sz);
        break;
...
EVP_CIPHER *EVP_CIPHER_fetch(OSSL_LIB_CTX *ctx, const char *algorithm,
    ...
    EVP_CIPHER *cipher = evp_generic_fetch(
    ctx, OSSL_OP_CIPHER, algorithm, properties, evp_cipher_from_algorithm,
...
int EVP_CIPHER_can_pipeline(const EVP_CIPHER *cipher, int enc) {
    if (((enc && cipher->p_einit != NULL) ||
         (!enc && cipher->p_dinit != NULL)) &&
...
void EVP_CIPHER_do_all_provided(OSSL_LIB_CTX *libctx,
                                ...
                                void *arg) {
    evp_generic_do_all(libctx, OSSL_OP_CIPHER, (void (*)(void *, void *))fn,
                       arg, evp_cipher_from_algorithm, evp_cipher_up_ref,
                       evp_cipher_free);
}
