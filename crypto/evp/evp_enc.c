...
nonlegacy:
  ...
  if (cipher->prov == NULL) {
    ...
    return 0;
#else
    EVP_CIPHER *provciph = EVP_CIPHER_fetch(
    NULL, cipher->nid == NID_undef ? "NULL" : OBJ_nid2sn(cipher->nid), "");
  ...
  if (enc) {
    ...
    ctx->algctx, key, key == NULL ? 0 : EVP_CIPHER_CTX_get_key_length(ctx), iv,
    iv == NULL ? 0 : EVP_CIPHER_CTX_get_iv_length(ctx), params);
  }
  ...
  if (ctx->cipher->dinit == NULL) {
    ...
    if (key == NULL && ctx->cipher->dinit_skey != NULL) {
      return ctx->cipher->dinit_skey(
      ctx->algctx, NULL, iv, iv == NULL ? 0 : EVP_CIPHER_CTX_get_iv_length(ctx),
      params);
    }
    ERR_raise(ERR_LIB_EVP, EVP_R_INITIALIZATION_ERROR);
    return 0;
...
skip_to_init:
  ...
  OPENSSL_assert(ctx->cipher->block_size == 1 || ctx->cipher->block_size == 8 ||
...
static int evp_cipher_init_skey_internal(EVP_CIPHER_CTX *ctx,
  ...
  if (enc) {
    if (ctx->cipher->einit_skey == NULL) {
      ...
      ret =
      ctx->cipher->einit(ctx->algctx, keydata, keylen, iv, iv_len, params);
    } else {
      ret = ctx->cipher->einit_skey(
      ctx->algctx, skey == NULL ? NULL : skey->keydata, iv, iv_len, params);
    }
  } else {
    if (ctx->cipher->dinit_skey == NULL) {
      ...
      ret =
      ctx->cipher->dinit(ctx->algctx, keydata, keylen, iv, iv_len, params);
    } else {
      ret = ctx->cipher->dinit_skey(
      ctx->algctx, skey == NULL ? NULL : skey->keydata, iv, iv_len, params);
    }
  }
...
int EVP_CipherInit_SKEY(EVP_CIPHER_CTX *ctx, const EVP_CIPHER *cipher,
                        ...
                        int enc, const OSSL_PARAM params[]) {
  return evp_cipher_init_skey_internal(ctx, cipher, skey, iv, iv_len, enc,
                                       params);
}
...
int EVP_CipherPipelineEncryptInit(EVP_CIPHER_CTX *ctx, const EVP_CIPHER *cipher,
  ...
  return ctx->cipher->p_einit(ctx->algctx, key, keylen, numpipes, iv, ivlen,
                              NULL);
}
...
int EVP_CipherPipelineDecryptInit(EVP_CIPHER_CTX *ctx, const EVP_CIPHER *cipher,
  ...
  return ctx->cipher->p_dinit(ctx->algctx, key, keylen, numpipes, iv, ivlen,
                              NULL);
}
...
int EVP_CipherPipelineUpdate(EVP_CIPHER_CTX *ctx, unsigned char **out,
  ...
  return ctx->cipher->p_cupdate(ctx->algctx, ctx->numpipes, out, outl, outsize,
                                in, inl);
}
...
int EVP_CipherPipelineFinal(EVP_CIPHER_CTX *ctx, unsigned char **out,
                            size_t *outl, const size_t *outsize) {
  size_t i;
...
int EVP_EncryptFinal_ex(EVP_CIPHER_CTX *ctx, unsigned char *out, int *outl) {
  ...
  ret =
  ctx->cipher->cfinal(ctx->algctx, out, &soutl, blocksize == 1 ? 0 : blocksize);
...
int EVP_CIPHER_CTX_ctrl(EVP_CIPHER_CTX *ctx, int type, int arg, void *ptr) {
  ...
  size_t sz = arg;
  unsigned int i;
  OSSL_PARAM params[4] = {OSSL_PARAM_END, OSSL_PARAM_END, OSSL_PARAM_END,
                          OSSL_PARAM_END};
  ...
  case EVP_CTRL_RAND_KEY: /* Used by DES */
    set_params = 0;
    params[0] =
    OSSL_PARAM_construct_octet_string(OSSL_CIPHER_PARAM_RANDOM_KEY, ptr, sz);
    break;
  ...
  case EVP_CTRL_AEAD_SET_TAG:
    params[0] =
    OSSL_PARAM_construct_octet_string(OSSL_CIPHER_PARAM_AEAD_TAG, ptr, sz);
    break;
  case EVP_CTRL_AEAD_TLS1_AAD:
    /* This one does a set and a get - since it returns a size */
    params[0] =
    OSSL_PARAM_construct_octet_string(OSSL_CIPHER_PARAM_AEAD_TLS1_AAD, ptr, sz);
  ...
  case EVP_CTRL_TLS1_1_MULTIBLOCK_ENCRYPT: {
    ...
    OSSL_CIPHER_PARAM_TLS1_MULTIBLOCK_ENC_IN, (void *)p->inp, p->len);
    ...
    if (ret <= 0)
      return ret;
    params[0] =
    OSSL_PARAM_construct_size_t(OSSL_CIPHER_PARAM_TLS1_MULTIBLOCK_ENC_LEN, &sz);
    params[1] = OSSL_PARAM_construct_end();
    ret = evp_do_ciph_ctx_getparams(ctx->cipher, ctx->algctx, params);
    if (ret <= 0)
      return 0;
  ...
  case EVP_CTRL_AEAD_SET_MAC_KEY:
    if (arg < 0)
      return -1;
    params[0] =
    OSSL_PARAM_construct_octet_string(OSSL_CIPHER_PARAM_AEAD_MAC_KEY, ptr, sz);
    break;
  }
...
static int evp_cipher_up_ref(void *cipher) { return EVP_CIPHER_up_ref(cipher); }

static void evp_cipher_free(void *cipher) { EVP_CIPHER_free(cipher); }

EVP_CIPHER *EVP_CIPHER_fetch(OSSL_LIB_CTX *ctx, const char *algorithm,
                             const char *properties) {
  EVP_CIPHER *cipher = evp_generic_fetch(ctx, OSSL_OP_CIPHER, algorithm,
                                         properties, evp_cipher_from_algorithm,
...
EVP_CIPHER *evp_cipher_fetch_from_prov(OSSL_PROVIDER *prov,
                                       ...
                                       const char *properties) {
  return evp_generic_fetch_from_prov(prov, OSSL_OP_CIPHER, algorithm,
                                     properties, evp_cipher_from_algorithm,
                                     evp_cipher_up_ref, evp_cipher_free);
}
...
void EVP_CIPHER_do_all_provided(OSSL_LIB_CTX *libctx,
                                ...
                                void *arg) {
  evp_generic_do_all(libctx, OSSL_OP_CIPHER, (void (*)(void *, void *))fn, arg,
