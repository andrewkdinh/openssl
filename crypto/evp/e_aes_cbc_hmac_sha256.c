...
static int aesni_cbc_hmac_sha256_init_key(EVP_CIPHER_CTX *ctx,
  ...
  if (enc)
    ret = aesni_set_encrypt_key(inkey, EVP_CIPHER_CTX_get_key_length(ctx) * 8,
                                &key->ks);
  else
    ret = aesni_set_decrypt_key(inkey, EVP_CIPHER_CTX_get_key_length(ctx) * 8,
                                &key->ks);
...
static int aesni_cbc_hmac_sha256_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out,
  ...
  if (EVP_CIPHER_CTX_is_encrypting(ctx)) {
    ...
    if (((OPENSSL_ia32cap_P[2] & (1 << 29)) ||         /* SHAEXT? */
      ...
      (void)aesni_cbc_sha256_enc(in, out, blocks, &key->ks, ctx->iv, &key->md,
                                 in + iv + sha_off);
...
static int aesni_cbc_hmac_sha256_ctrl(EVP_CIPHER_CTX *ctx, int type, int arg,
  ...
  case EVP_CTRL_AEAD_TLS1_AAD: {
    ...
    if (EVP_CIPHER_CTX_is_encrypting(ctx)) {
      ...
      return (
      int)(((len + SHA256_DIGEST_LENGTH + AES_BLOCK_SIZE) & -AES_BLOCK_SIZE) -
           len);
    } else {
