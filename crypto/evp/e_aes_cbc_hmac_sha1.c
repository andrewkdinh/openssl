...
static int aesni_cbc_hmac_sha1_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out,
  ...
  if (EVP_CIPHER_CTX_is_encrypting(ctx)) {
    ...
    if (plen != len) { /* "TLS" mode of operation */
      ...
      aesni_cbc_encrypt(out + aes_off, out + aes_off, len - aes_off, &key->ks,
                        ctx->iv, 1);
    } else {
      aesni_cbc_encrypt(in + aes_off, out + aes_off, len - aes_off, &key->ks,
                        ctx->iv, 1);
    }
  } else {
    union {
    ...
    if (plen != NO_PAYLOAD_LENGTH) { /* "TLS" mode of operation */
      ...
      if (len >= 1024 && keylen == 32) {
        ...
        aesni_cbc_encrypt(in + len - AES_BLOCK_SIZE, out + len - AES_BLOCK_SIZE,
                          AES_BLOCK_SIZE, &key->ks, tail_iv, 0);
        stitch = 1;
      } else
#endif
      ...
      if (stitch) {
        ...
        aesni256_cbc_sha1_dec(in + aes_off, out + aes_off, blocks, &key->ks,
...
const EVP_CIPHER *EVP_aes_128_cbc_hmac_sha1(void) {
  return (OPENSSL_ia32cap_P[1] & AESNI_CAPABLE ? &aesni_128_cbc_hmac_sha1_cipher
                                               : NULL);
}

const EVP_CIPHER *EVP_aes_256_cbc_hmac_sha1(void) {
  return (OPENSSL_ia32cap_P[1] & AESNI_CAPABLE ? &aesni_256_cbc_hmac_sha1_cipher
                                               : NULL);
}
#else
