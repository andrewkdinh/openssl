...
static int des_ede_ofb_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out,
  ...
  while (inl >= EVP_MAXCHUNK) {
    ...
    DES_ede3_ofb64_encrypt(in, out, (long)EVP_MAXCHUNK, &data(ctx)->ks1,
  ...
  if (inl) {
    int num = EVP_CIPHER_CTX_get_num(ctx);
    DES_ede3_ofb64_encrypt(in, out, (long)inl, &data(ctx)->ks1, &data(ctx)->ks2,
                           &data(ctx)->ks3, (DES_cblock *)ctx->iv, &num);
    EVP_CIPHER_CTX_set_num(ctx, num);
  }
  return 1;
}
...
static int des_ede_cbc_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out,
  ...
  if (inl)
    DES_ede3_cbc_encrypt(in, out, (long)inl, &dat->ks1, &dat->ks2, &dat->ks3,
                         (DES_cblock *)ctx->iv,
                         EVP_CIPHER_CTX_is_encrypting(ctx));
  return 1;
}
...
static int des_ede_cfb64_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out,
  ...
  if (inl) {
    int num = EVP_CIPHER_CTX_get_num(ctx);
    DES_ede3_cfb64_encrypt(in, out, (long)inl, &data(ctx)->ks1, &data(ctx)->ks2,
                           &data(ctx)->ks3, (DES_cblock *)ctx->iv, &num,
                           EVP_CIPHER_CTX_is_encrypting(ctx));
    EVP_CIPHER_CTX_set_num(ctx, num);
  }
...
static int des_ede3_cfb8_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out,
  ...
  while (inl >= EVP_MAXCHUNK) {
    ...
    in, out, 8, (long)EVP_MAXCHUNK, &data(ctx)->ks1, &data(ctx)->ks2,
    &data(ctx)->ks3, (DES_cblock *)ctx->iv, EVP_CIPHER_CTX_is_encrypting(ctx));
    inl -= EVP_MAXCHUNK;
    in += EVP_MAXCHUNK;
    out += EVP_MAXCHUNK;
  }
  if (inl)
    DES_ede3_cfb_encrypt(
    in, out, 8, (long)inl, &data(ctx)->ks1, &data(ctx)->ks2, &data(ctx)->ks3,
...
const EVP_CIPHER *EVP_des_ede3_wrap(void) { return &des3_wrap; }
