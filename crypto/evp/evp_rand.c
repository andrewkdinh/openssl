...
int EVP_RAND_up_ref(EVP_RAND *rand) { return evp_rand_up_ref(rand); }

void EVP_RAND_free(EVP_RAND *rand) { evp_rand_free(rand); }

int evp_rand_get_number(const EVP_RAND *rand) { return rand->name_id; }

const char *EVP_RAND_get0_name(const EVP_RAND *rand) { return rand->type_name; }
...
EVP_RAND *EVP_RAND_CTX_get0_rand(EVP_RAND_CTX *ctx) { return ctx->meth; }
...
void EVP_RAND_do_all_provided(OSSL_LIB_CTX *libctx,
                              ...
                              void *arg) {
  evp_generic_do_all(libctx, OSSL_OP_RAND, (void (*)(void *, void *))fn, arg,
...
int EVP_RAND_instantiate(EVP_RAND_CTX *ctx, unsigned int strength,
  ...
  if (!evp_rand_lock(ctx))
    return 0;
  res = evp_rand_instantiate_locked(ctx, strength, prediction_resistance, pstr,
                                    pstr_len, params);
  evp_rand_unlock(ctx);
  return res;
}
...
static int evp_rand_reseed_locked(EVP_RAND_CTX *ctx, int prediction_resistance,
  ...
  if (ctx->meth->reseed != NULL)
    return ctx->meth->reseed(ctx->algctx, prediction_resistance, ent, ent_len,
                             addin, addin_len);
  return 1;
}
...
int EVP_RAND_reseed(EVP_RAND_CTX *ctx, int prediction_resistance,
  ...
  if (!evp_rand_lock(ctx))
    return 0;
  res = evp_rand_reseed_locked(ctx, prediction_resistance, ent, ent_len, addin,
                               addin_len);
  evp_rand_unlock(ctx);
  return res;
}
...
int evp_rand_can_seed(EVP_RAND_CTX *ctx) { return ctx->meth->get_seed != NULL; }

static size_t evp_rand_get_seed_locked(EVP_RAND_CTX *ctx,
                                       unsigned char **buffer, int entropy,
                                       size_t min_len, size_t max_len,
                                       int prediction_resistance,
                                       const unsigned char *adin,
                                       size_t adin_len) {
  if (ctx->meth->get_seed != NULL)
    return ctx->meth->get_seed(ctx->algctx, buffer, entropy, min_len, max_len,
...
size_t evp_rand_get_seed(EVP_RAND_CTX *ctx, unsigned char **buffer, int entropy,
  ...
  if (!evp_rand_lock(ctx))
    return 0;
  res = evp_rand_get_seed_locked(ctx, buffer, entropy, min_len, max_len,
                                 prediction_resistance, adin, adin_len);
  evp_rand_unlock(ctx);
  return res;
}

static void evp_rand_clear_seed_locked(EVP_RAND_CTX *ctx, unsigned char *buffer,
                                       size_t b_len) {
