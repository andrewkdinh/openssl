...
static void detect_foreign_key(EVP_PKEY *pkey) {
    ...
    case EVP_PKEY_DH:
        pkey->foreign =
        pkey->pkey.dh != NULL && ossl_dh_is_foreign(pkey->pkey.dh);
        break;
#endif
    default:
        pkey->foreign = 0;
...
static int print_pkey(const EVP_PKEY *pkey, BIO *out, int indent,
    ...
    ctx =
    OSSL_ENCODER_CTX_new_for_pkey(pkey, selection, "TEXT", NULL, propquery);
...
size_t EVP_PKEY_get1_encoded_public_key(EVP_PKEY *pkey, unsigned char **ppub) {
...
#endif
    {
        ...
        pkey, OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY, NULL, 0, &return_size);
    ...
    {
        int rv =
        evp_pkey_asn1_ctrl(pkey, ASN1_PKEY_CTRL_GET1_TLS_ENCPT, 0, ppub);
        if (rv <= 0)
            return 0;
        return rv;
    }
    ...
    if (pk->pkey.ptr != NULL) {
        ...
        if (pk->ameth->dirty_cnt(pk) == pk->dirty_cnt_copy) {
            if (!CRYPTO_THREAD_read_lock(pk->lock))
                goto end;
            op =
            evp_keymgmt_util_find_operation_cache(pk, tmp_keymgmt, selection);
...
int evp_pkey_copy_downgraded(EVP_PKEY **dest, const EVP_PKEY *src) {
    ...
    if (evp_pkey_is_assigned(src) && evp_pkey_is_provided(src)) {
        ...
        if (!ossl_assert(type != EVP_PKEY_NONE)) {
            ...
            "keymgmt key type = %s but legacy type = EVP_PKEY_NONE", keytype);
            return 0;
        }
        ...
        if (EVP_PKEY_set_type(*dest, type)) {
            ...
            if ((*dest)->ameth->import_from == NULL) {
                ...
                if (pctx != NULL && evp_keymgmt_export(
...
int EVP_PKEY_get_params(const EVP_PKEY *pkey, OSSL_PARAM params[]) {
    if (pkey != NULL) {
        if (evp_pkey_is_provided(pkey))
            return evp_keymgmt_get_params(pkey->keymgmt, pkey->keydata,
                                          params) > 0;
#ifndef FIPS_MODULE
        else if (evp_pkey_is_legacy(pkey))
