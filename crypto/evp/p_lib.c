...
# endif
    ...
    if (e == NULL && ameth == NULL) {
        ...
        if (EVP_PKEY_fromdata_init(ctx) == 1) {
            ...
            ERR_clear_last_mark();
            params[0] =
                OSSL_PARAM_construct_octet_string(key_is_priv ? OSSL_PKEY_PARAM_PRIV_KEY : OSSL_PKEY_PARAM_PUB_KEY,
                                                  (void *)key,
                                                  len);
...
static int           get_raw_key_details(const OSSL_PARAM params[], void *arg)
{
    ...
    if (raw_key->selection == OSSL_KEYMGMT_SELECT_PRIVATE_KEY) {
        if ((p = OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_PRIV_KEY)) != NULL)
            return OSSL_PARAM_get_octet_string(p,
                                               (void **)raw_key->key,
...
static EVP_PKEY *new_cmac_key_int(const unsigned char *priv,
                                  size_t               len,
...
static int print_pkey(const EVP_PKEY *pkey,
                      BIO            *out,
                      int             indent,
...
int EVP_PKEY_set1_encoded_public_key(EVP_PKEY *pkey, const unsigned char *pub, size_t publen)
{
    ...
    if (evp_pkey_asn1_ctrl(pkey, ASN1_PKEY_CTRL_SET1_TLS_ENCPT, (int)publen, (void *)pub) <= 0)
        return 0;
    return 1;
#endif /* !FIPS_MODULE */
}
...
static void find_ameth(const char *name, void *data)
{
    ...
    if (pkey_set_type(NULL, NULL, EVP_PKEY_NONE, name, (int)strlen(name), NULL)) {
        if (str[0] == NULL)
            str[0] = name;
        else if (str[1] == NULL)
            str[1] = name;
