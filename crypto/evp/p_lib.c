...
int EVP_PKEY_copy_parameters(EVP_PKEY *to, const EVP_PKEY *from) {
  ...
  if (to->keymgmt != NULL && to->keydata == NULL) {
    ...
    void *from_keydata =
    evp_pkey_export_to_provider((EVP_PKEY *)from, NULL, &to_keymgmt, NULL);
    ...
    if (from_keydata == NULL)
      ...
      ok = (to->keydata = evp_keymgmt_dup(to->keymgmt, from_keydata,
...
static EVP_PKEY *new_raw_key_int(OSSL_LIB_CTX *libctx, const char *strtype,
  ...
  if (e == NULL && ameth == NULL) {
    ...
    libctx, strtype != NULL ? strtype : OBJ_nid2sn(nidtype), propq);
    if (ctx == NULL)
      goto err;
    ...
    if (EVP_PKEY_fromdata_init(ctx) == 1) {
      ...
      key_is_priv ? OSSL_PKEY_PARAM_PRIV_KEY : OSSL_PKEY_PARAM_PUB_KEY,
      (void *)key, len);
...
EVP_PKEY *EVP_PKEY_new_raw_private_key_ex(OSSL_LIB_CTX *libctx,
                                          ...
                                          size_t len) {
  return new_raw_key_int(libctx, keytype, propq, EVP_PKEY_NONE, NULL, priv, len,
                         1);
}
...
EVP_PKEY *EVP_PKEY_new_raw_public_key_ex(OSSL_LIB_CTX *libctx,
  ...
  return new_raw_key_int(libctx, keytype, propq, EVP_PKEY_NONE, NULL, pub, len,
                         0);
}
...
static int get_raw_key_details(const OSSL_PARAM params[], void *arg) {
  ...
  if (raw_key->selection == OSSL_KEYMGMT_SELECT_PRIVATE_KEY) {
    if ((p = OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_PRIV_KEY)) != NULL)
      ...
      p, (void **)raw_key->key, raw_key->key == NULL ? 0 : *raw_key->len,
      raw_key->len);
...
ENGINE *EVP_PKEY_get0_engine(const EVP_PKEY *pkey) { return pkey->engine; }
#endif
...
static void detect_foreign_key(EVP_PKEY *pkey) {
  ...
  case EVP_PKEY_RSA_PSS:
    pkey->foreign =
    pkey->pkey.rsa != NULL && ossl_rsa_is_foreign(pkey->pkey.rsa);
    break;
#ifndef OPENSSL_NO_EC
  case EVP_PKEY_SM2:
    break;
  case EVP_PKEY_EC:
    pkey->foreign =
    pkey->pkey.ec != NULL && ossl_ec_key_is_foreign(pkey->pkey.ec);
    break;
#endif
#ifndef OPENSSL_NO_DSA
  case EVP_PKEY_DSA:
    pkey->foreign =
    pkey->pkey.dsa != NULL && ossl_dsa_is_foreign(pkey->pkey.dsa);
    break;
#endif
...
int EVP_PKEY_get_id(const EVP_PKEY *pkey) { return pkey->type; }
...
int EVP_PKEY_get_default_digest_name(EVP_PKEY *pkey, char *mdname,
                                     size_t mdname_sz) {
  if (pkey->ameth == NULL)
    return evp_keymgmt_util_get_deflt_digest_name(pkey->keymgmt, pkey->keydata,
                                                  mdname, mdname_sz);
...
int EVP_PKEY_get_group_name(const EVP_PKEY *pkey, char *gname, size_t gname_sz,
                            size_t *gname_len) {
  return EVP_PKEY_get_utf8_string_param(pkey, OSSL_PKEY_PARAM_GROUP_NAME, gname,
                                        gname_sz, gname_len);
}
...
int EVP_PKEY_set1_encoded_public_key(EVP_PKEY *pkey, const unsigned char *pub,
  ...
  if (evp_pkey_is_provided(pkey))
    ...
    pkey, OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY, (unsigned char *)pub, publen);
...
#endif
  {
    ...
    *ppub = NULL;
    ...
    if (!EVP_PKEY_get_octet_string_param(
        pkey, OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY, buf, return_size, NULL)) {
      OPENSSL_free(buf);
      return 0;
    }
    *ppub = buf;
...
void *evp_pkey_export_to_provider(EVP_PKEY *pk, OSSL_LIB_CTX *libctx,
  ...
  if (pk->pkey.ptr != NULL) {
    ...
    if (!pk->ameth->export_to(pk, keydata, tmp_keymgmt->import, libctx,
                              propquery)) {
  ...
  if (evp_pkey_is_assigned(src) && evp_pkey_is_provided(src)) {
    ...
    if (EVP_PKEY_set_type(*dest, type)) {
      ...
      if ((*dest)->ameth->import_from == NULL) {
        ERR_raise_data(ERR_LIB_EVP, EVP_R_NO_IMPORT_FUNCTION, "key type = %s",
                       keytype);
      } else {
        /*
        ...
        if (pctx != NULL &&
            evp_keymgmt_export(keymgmt, keydata, OSSL_KEYMGMT_SELECT_ALL,
                               (*dest)->ameth->import_from, pctx)) {
          /* Synchronize the dirty count */
      ...
      ERR_raise_data(ERR_LIB_EVP, EVP_R_KEYMGMT_EXPORT_FAILURE, "key type = %s",
                     keytype);
    }
  }
...
int EVP_PKEY_set_octet_string_param(EVP_PKEY *pkey, const char *key_name,
  ...
  params[0] =
  OSSL_PARAM_construct_octet_string(key_name, (unsigned char *)buf, bsize);
  params[1] = OSSL_PARAM_construct_end();
  return EVP_PKEY_set_params(pkey, params);
}
...
int EVP_PKEY_get_ec_point_conv_form(const EVP_PKEY *pkey) {
  ...
  if (!EVP_PKEY_get_utf8_string_param(
      pkey, OSSL_PKEY_PARAM_EC_POINT_CONVERSION_FORMAT, name, sizeof(name),
      &name_len))
    return 0;
...
int EVP_PKEY_get_field_type(const EVP_PKEY *pkey) {
  ...
  if (!EVP_PKEY_get_utf8_string_param(pkey, OSSL_PKEY_PARAM_EC_FIELD_TYPE, fstr,
                                      sizeof(fstr), &fstrlen))
    return 0;
