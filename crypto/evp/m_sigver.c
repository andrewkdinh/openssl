...
static int do_sigver_init(EVP_MD_CTX *ctx, EVP_PKEY_CTX **pctx,
  ...
  if (reinit && (pkey != NULL ||
                 locpctx->operation !=
                 (ver ? EVP_PKEY_OP_VERIFYCTX : EVP_PKEY_OP_SIGNCTX) ||
  ...
  supported_sig =
  evp_keymgmt_util_query_operation_name(locpctx->keymgmt, OSSL_OP_SIGNATURE);
  ...
  for (iter = 1, provkey = NULL; iter < 3 && provkey == NULL; iter++) {
    ...
    case 2:
      ...
      (OSSL_PROVIDER *)tmp_prov, supported_sig, locpctx->propquery);
      if (signature == NULL)
        goto legacy;
      break;
    }
    ...
    (OSSL_PROVIDER *)tmp_prov, EVP_KEYMGMT_get0_name(locpctx->keymgmt),
    locpctx->propquery);
    if (tmp_keymgmt != NULL)
...
reinitialize:
  ...
  if (ver) {
    ...
    ret = signature->digest_verify_init(locpctx->op.sig.algctx, mdname, provkey,
                                        params);
  } else {
    ...
    ret = signature->digest_sign_init(locpctx->op.sig.algctx, mdname, provkey,
                                      params);
  }
...
int EVP_DigestSignUpdate(EVP_MD_CTX *ctx, const void *data, size_t dsize) {
  ...
  return pctx->op.sig.signature->digest_sign_update(pctx->op.sig.algctx, data,
                                                    dsize);
...
int EVP_DigestVerifyUpdate(EVP_MD_CTX *ctx, const void *data, size_t dsize) {
  ...
  return pctx->op.sig.signature->digest_verify_update(pctx->op.sig.algctx, data,
                                                      dsize);
...
int EVP_DigestSignFinal(EVP_MD_CTX *ctx, unsigned char *sigret,
  ...
  r = pctx->op.sig.signature->digest_sign_final(
  pctx->op.sig.algctx, sigret, siglen, sigret == NULL ? 0 : *siglen);
...
legacy:
  ...
  if (sigret != NULL) {
    ...
    if (ctx->flags & EVP_MD_CTX_FLAG_FINALISE) {
      ...
      if (sctx)
        r =
        tmp_ctx->pctx->pmeth->signctx(tmp_ctx->pctx, sigret, siglen, tmp_ctx);
      else
...
int EVP_DigestSign(EVP_MD_CTX *ctx, unsigned char *sigret, size_t *siglen,
  ...
  if (pctx->operation == EVP_PKEY_OP_SIGNCTX && pctx->op.sig.algctx != NULL &&
      pctx->op.sig.signature != NULL) {
    if (pctx->op.sig.signature->digest_sign != NULL) {
      if (sigret != NULL)
        ctx->flags |= EVP_MD_CTX_FLAG_FINALISED;
      return pctx->op.sig.signature->digest_sign(
      pctx->op.sig.algctx, sigret, siglen, sigret == NULL ? 0 : *siglen, tbs,
      tbslen);
    }
  } else {
    /* legacy */
...
int EVP_DigestVerifyFinal(EVP_MD_CTX *ctx, const unsigned char *sig,
  ...
  r =
  pctx->op.sig.signature->digest_verify_final(pctx->op.sig.algctx, sig, siglen);
  if (dctx == NULL)
    ctx->flags |= EVP_MD_CTX_FLAG_FINALISED;
  else
    EVP_PKEY_CTX_free(dctx);
...
int EVP_DigestVerify(EVP_MD_CTX *ctx, const unsigned char *sigret,
  ...
  if (pctx->operation == EVP_PKEY_OP_VERIFYCTX && pctx->op.sig.algctx != NULL &&
      pctx->op.sig.signature != NULL) {
    if (pctx->op.sig.signature->digest_verify != NULL) {
      ctx->flags |= EVP_MD_CTX_FLAG_FINALISED;
      return pctx->op.sig.signature->digest_verify(pctx->op.sig.algctx, sigret,
                                                   siglen, tbs, tbslen);
    }
  } else {
    /* legacy */
