...
static int do_sigver_init(EVP_MD_CTX *ctx, EVP_PKEY_CTX **pctx,
    ...
    for (iter = 1, provkey = NULL; iter < 3 && provkey == NULL; iter++) {
         ...
         * With the keymgmt we found (if we did), we try to export
         |locpctx->pkey|
         * to it (evp_pkey_export_to_provider() is smart enough to only actually

         * export it if |tmp_keymgmt| is different from |locpctx->pkey|'s
         keymgmt)
         */
        ...
        if (tmp_keymgmt != NULL)
            ...
            locpctx->pkey, locpctx->libctx, &tmp_keymgmt, locpctx->propquery);
        if (tmp_keymgmt == NULL)
...
reinitialize:
    ...
    if (ver) {
        ...
        ret = signature->digest_verify_init(locpctx->op.sig.algctx, mdname,
                                            provkey, params);
    } else {
        ...
        ret = signature->digest_sign_init(locpctx->op.sig.algctx, mdname,
                                          provkey, params);
    }
...
int EVP_DigestVerifyFinal(EVP_MD_CTX *ctx, const unsigned char *sig,
    ...
    r = pctx->op.sig.signature->digest_verify_final(pctx->op.sig.algctx, sig,
                                                    siglen);
    if (dctx == NULL)
        ctx->flags |= EVP_MD_CTX_FLAG_FINALISED;
    else
...
int EVP_DigestVerify(EVP_MD_CTX *ctx, const unsigned char *sigret,
    ...
    if (pctx->operation == EVP_PKEY_OP_VERIFYCTX &&
        ...
        if (pctx->op.sig.signature->digest_verify != NULL) {
            ctx->flags |= EVP_MD_CTX_FLAG_FINALISED;
            return pctx->op.sig.signature->digest_verify(
            pctx->op.sig.algctx, sigret, siglen, tbs, tbslen);
        }
    } else {
        /* legacy */
