                          ...
                          EVP_PKEY *pkey, int ver, const OSSL_PARAM params[])
{
    ...
    for (iter = 1, provkey = NULL; iter < 3 && provkey == NULL; iter++) {
        ...
        if (tmp_keymgmt != NULL)
            provkey =
                evp_pkey_export_to_provider(locpctx->pkey, locpctx->libctx,
    ...
    if (type != NULL) {
        ...
        if (mdname == NULL && !reinit) {
            if (evp_keymgmt_util_get_deflt_digest_name(
                    tmp_keymgmt, provkey, locmdname, sizeof(locmdname))
                > 0) {
    ...
    if (ver) {
        if (signature->digest_verify_init == NULL) {
            ERR_raise_data(ERR_LIB_EVP, EVP_R_PROVIDER_SIGNATURE_NOT_SUPPORTED,
                           "%s digest_verify_init:%s", signature->type_name,
                           desc);
            goto err;
        }
        ...
        if (signature->digest_sign_init == NULL) {
            ERR_raise_data(ERR_LIB_EVP, EVP_R_PROVIDER_SIGNATURE_NOT_SUPPORTED,
                           "%s digest_sign_init:%s", signature->type_name,
                           desc);
            goto err;
        }
...
int EVP_DigestVerifyUpdate(EVP_MD_CTX *ctx, const void *data, size_t dsize)
{
    ...
    if (signature->digest_verify_update == NULL) {
        ERR_raise_data(ERR_LIB_EVP, EVP_R_PROVIDER_SIGNATURE_NOT_SUPPORTED,
                       "%s digest_verify_update:%s", signature->type_name,
                       desc);
        return 0;
    }
                   ...
                   const unsigned char *tbs, size_t tbslen)
{
    ...
    if (pctx->operation == EVP_PKEY_OP_SIGNCTX && pctx->op.sig.algctx != NULL
        ...
        if (signature->digest_sign != NULL) {
            const char *desc =
                signature->description != NULL ? signature->description : "";

            if (sigret != NULL)
                ctx->flags |= EVP_MD_CTX_FLAG_FINALISED;
            ret = signature->digest_sign(pctx->op.sig.algctx, sigret, siglen,
                                         sigret == NULL ? 0 : *siglen, tbs,
                                         tbslen);
            if (ret <= 0)
                ERR_raise_data(ERR_LIB_EVP, EVP_R_PROVIDER_SIGNATURE_FAILURE,
                     ...
                     size_t siglen, const unsigned char *tbs, size_t tbslen)
{
    ...
    if (pctx->operation == EVP_PKEY_OP_VERIFYCTX && pctx->op.sig.algctx != NULL
        ...
        if (pctx->op.sig.signature->digest_verify != NULL) {
            ...
            ctx->flags |= EVP_MD_CTX_FLAG_FINALISED;
            ret = signature->digest_verify(pctx->op.sig.algctx, sigret, siglen,
                                           tbs, tbslen);
            if (ret <= 0)
                ERR_raise_data(ERR_LIB_EVP, EVP_R_PROVIDER_SIGNATURE_FAILURE,
                               "%s digest_verify:%s", signature->type_name,
                               desc);
            return ret;
        }
    } else {
