...
const EVP_CIPHER *EVP_chacha20(void) { return &chacha20; }
...
static int chacha20_poly1305_tls_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out,
  ...
  if (plen <= CHACHA_BLK_SIZE) {
    ...
    actx->key.counter[0] = 0;
    ChaCha20_ctr32(buf, zero, (buf_len = 2 * CHACHA_BLK_SIZE), actx->key.key.d,
                   actx->key.counter);
    Poly1305_Init(POLY1305_ctx(actx), buf);
    actx->key.partial_len = 0;
    memcpy(tohash, actx->tls_aad, POLY1305_BLOCK_SIZE);
...
static int chacha20_poly1305_ctrl(EVP_CIPHER_CTX *ctx, int type, int arg,
  ...
  case EVP_CTRL_AEAD_TLS1_AAD:
      ...
      return 0;
    {
      ...
      len =
      aad[EVP_AEAD_TLS1_AAD_LEN - 2] << 8 | aad[EVP_AEAD_TLS1_AAD_LEN - 1];
      aad = actx->tls_aad;
      if (!EVP_CIPHER_CTX_is_encrypting(ctx)) {
...
const EVP_CIPHER *EVP_chacha20_poly1305(void) { return (&chacha20_poly1305); }
#endif
#endif
