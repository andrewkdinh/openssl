...
static void *keymgmt_from_algorithm(int name_id, const OSSL_ALGORITHM *algodef, OSSL_PROVIDER *prov)
{
     ...
     */
    if (keymgmt->free == NULL || (keymgmt->new == NULL && keymgmt->gen == NULL && keymgmt->load == NULL)
        || keymgmt->has == NULL || (getparamfncnt != 0 && getparamfncnt != 2)
        || (setparamfncnt != 0 && setparamfncnt != 2) || (setgenparamfncnt != 0 && setgenparamfncnt != 2)
        || (getgenparamfncnt != 0 && getgenparamfncnt != 2) || (importfncnt != 0 && importfncnt != 2)
...
{
    return evp_generic_fetch_from_prov(prov, OSSL_OP_KEYMGMT, name, properties, keymgmt_from_algorithm,
                                       evp_keymgmt_up_ref, evp_keymgmt_free);
}

EVP_KEYMGMT *EVP_KEYMGMT_fetch(OSSL_LIB_CTX *ctx, const char *algorithm, const char *properties)
{
    return evp_generic_fetch(ctx, OSSL_OP_KEYMGMT, algorithm, properties, keymgmt_from_algorithm, evp_keymgmt_up_ref,
                             evp_keymgmt_free);
}
...
void *evp_keymgmt_gen(const EVP_KEYMGMT *keymgmt, void *genctx, OSSL_CALLBACK *cb, void *cbarg)
{
    ...
    if (keymgmt->gen == NULL) {
        ERR_raise_data(ERR_LIB_EVP, EVP_R_PROVIDER_KEYMGMT_NOT_SUPPORTED, "%s key generation:%s", keymgmt->type_name,
                       desc);
        return NULL;
    }
    ...
    if (ret == NULL)
        ERR_raise_data(ERR_LIB_EVP, EVP_R_PROVIDER_KEYMGMT_FAILURE, "%s key generation:%s", keymgmt->type_name, desc);
    return ret;
}
