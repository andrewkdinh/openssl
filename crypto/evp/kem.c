...
static void evp_kem_free(void *data) { EVP_KEM_free(data); }

static int evp_kem_up_ref(void *data) { return EVP_KEM_up_ref(data); }
...
static int evp_kem_init(EVP_PKEY_CTX *ctx, int operation,
  ...
  for (iter = 1, provkey = NULL; iter < 3 && provkey == NULL; iter++) {
    ...
    case 2:
      ...
      kem = evp_kem_fetch_from_prov((OSSL_PROVIDER *)tmp_prov, supported_kem,
                                    ctx->propquery);
...
int EVP_PKEY_decapsulate(EVP_PKEY_CTX *ctx, unsigned char *secret,
  ...
  return ctx->op.encap.kem->decapsulate(ctx->op.encap.algctx, secret, secretlen,
                                        in, inlen);
}
...
OSSL_PROVIDER *EVP_KEM_get0_provider(const EVP_KEM *kem) { return kem->prov; }
...
int evp_kem_get_number(const EVP_KEM *kem) { return kem->name_id; }

const char *EVP_KEM_get0_name(const EVP_KEM *kem) { return kem->type_name; }
...
void EVP_KEM_do_all_provided(OSSL_LIB_CTX *libctx,
  ...
  evp_generic_do_all(libctx, OSSL_OP_KEM, (void (*)(void *, void *))fn, arg,
                     evp_kem_from_algorithm, evp_kem_up_ref, evp_kem_free);
}
