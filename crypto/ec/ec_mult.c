...
int ossl_ec_wNAF_mul(const EC_GROUP *group, EC_POINT *r, const BIGNUM *scalar,
  ...
  for (i = 0; i < num + num_scalar; i++) {
    ...
    wNAF[i] =
    bn_compute_wNAF((i < num ? scalars[i] : scalar), wsize[i], &wNAF_len[i]);
    if (wNAF[i] == NULL)
      goto err;
      ...
      if (wNAF_len[i] > (size_t)k) {
        ...
        if (digit) {
          ...
          if (r_is_at_infinity) {
             ...
             * ossl_ec_point_blind_coordinates() returns 0 in case of errors or
             * 1 on success or if coordinate blinding is not implemented for
             * this group.
             */
