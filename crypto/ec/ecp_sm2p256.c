...
ALIGN32 static const BN_ULONG def_xG[P256_LIMBS] = {0x715a4589334c74c7, 0x8fe30bbff2660be1, 0x5f9904466a39c994,
                                                    0x32c4ae2c1f198119};
...
ALIGN32 static const BN_ULONG def_p[P256_LIMBS] = {0xffffffffffffffff, 0xffffffff00000000, 0xffffffffffffffff,
                                                   0xfffffffeffffffff};
ALIGN32 static const BN_ULONG def_ord[P256_LIMBS] = {0x53bbf40939d54123, 0x7203df6b21c6052b, 0xffffffffffffffff,
                                                     0xfffffffeffffffff};
...
static ossl_inline void ecp_sm2p256_mod_inverse(BN_ULONG *out, const BN_ULONG *in)
{
    BN_MOD_INV(out, in, ecp_sm2p256_div_by_2, ecp_sm2p256_sub, def_p);
}

/* Modular inverse mod order |out| = |in|^(-1) % |ord|. */
static ossl_inline void ecp_sm2p256_mod_ord_inverse(BN_ULONG *out, const BN_ULONG *in)
{
    BN_MOD_INV(out, in, ecp_sm2p256_div_by_2_mod_ord, ecp_sm2p256_sub_mod_ord, def_ord);
}
...
static void ecp_sm2p256_point_add_affine(P256_POINT *R, const P256_POINT *P, const P256_POINT_AFFINE *Q)
{
    unsigned int i;
...
/* Point add: R <- P + Q */
static void ecp_sm2p256_point_add(P256_POINT *R, const P256_POINT *P, const P256_POINT *Q)
{
    unsigned int i;
    ALIGN32 BN_ULONG tmp0[P256_LIMBS] = {0};
    ALIGN32 BN_ULONG tmp1[P256_LIMBS] = {0};
 ...
 */
static void ecp_sm2p256_point_P_mul_by_scalar(P256_POINT *R, const BN_ULONG *k, P256_POINT_AFFINE P)
{
    int i, init = 0;
...
/* Get affine point */
static void ecp_sm2p256_point_get_affine(P256_POINT_AFFINE *R, const P256_POINT *P)
{
...
static int ecp_sm2p256_is_affine_G(const EC_POINT *generator)
{
    return (bn_get_top(generator->X) == P256_LIMBS) && (bn_get_top(generator->Y) == P256_LIMBS)
        && is_equal(bn_get_words(generator->X), def_xG) && is_equal(bn_get_words(generator->Y), def_yG)
 ...
 */
static int ecp_sm2p256_get_affine(const EC_GROUP *group, const EC_POINT *point, BIGNUM *x, BIGNUM *y, BN_CTX *ctx)
{
    ALIGN32 BN_ULONG z_inv2[P256_LIMBS] = {0};
    ALIGN32 BN_ULONG z_inv3[P256_LIMBS] = {0};
    ...
    if (ecp_sm2p256_bignum_field_elem(point_x, point->X) <= 0 || ecp_sm2p256_bignum_field_elem(point_y, point->Y) <= 0
...
static int ecp_sm2p256_windowed_mul(const EC_GROUP *group, P256_POINT *r, const BIGNUM **scalar, const EC_POINT **point,
                                    size_t num, BN_CTX *ctx)
{
    ...
    if (num > OPENSSL_MALLOC_MAX_NELEMS(P256_POINT) || (scalars = OPENSSL_malloc(num * sizeof(BIGNUM *))) == NULL) {
    ...
    for (i = 0; i < num; i++) {
        ...
        if (ecp_sm2p256_bignum_field_elem(k, scalars[i]) <= 0 || ecp_sm2p256_bignum_field_elem(p.p.X, point[i]->X) <= 0
...
static int ecp_sm2p256_points_mul(const EC_GROUP *group, EC_POINT *r, const BIGNUM *scalar, size_t num,
                                  const EC_POINT *points[], const BIGNUM *scalars[], BN_CTX *ctx)
{
    ...
    if (scalar) {
...
#endif
        {
            ...
            if (!ecp_sm2p256_windowed_mul(group, &p.p, g_scalars, new_generator,
                                          (new_generator[0] != NULL && g_scalars[0] != NULL), ctx))
                goto err;
        }
    } else {
        p_is_infinity = 1;
    ...
    if (!bn_set_words(r->X, p.p.X, P256_LIMBS) || !bn_set_words(r->Y, p.p.Y, P256_LIMBS)
...
static int ecp_sm2p256_field_mul(const EC_GROUP *group, BIGNUM *r, const BIGNUM *a, const BIGNUM *b, BN_CTX *ctx)
{
    ...
    if (!ecp_sm2p256_bignum_field_elem(a_fe, a) || !ecp_sm2p256_bignum_field_elem(b_fe, b)) {
        ECerr(ERR_LIB_EC, EC_R_COORDINATES_OUT_OF_RANGE);
        return 0;
    }
...
static int ecp_sm2p256_field_sqr(const EC_GROUP *group, BIGNUM *r, const BIGNUM *a, BN_CTX *ctx)
{
    ALIGN32 BN_ULONG a_fe[P256_LIMBS] = {0};
    ALIGN32 BN_ULONG r_fe[P256_LIMBS] = {0};
...
static int ecp_sm2p256_inv_mod_ord(const EC_GROUP *group, BIGNUM *r, const BIGNUM *x, BN_CTX *ctx)
{
    int ret = 0;
