...
static int ec_GFp_s390x_nistp_mul(const EC_GROUP *group, EC_POINT *r,
  ...
  if ((scalar != NULL && num == 0 && BN_is_negative(scalar) == 0) ||
    ...
    if (group->meth->point_get_affine_coordinates(group, point_ptr, x, y,
        ...
        BN_bn2binpad(scalar_ptr, param + S390X_OFF_SCALAR(len), len) == -1 ||
...
static int ecdsa_s390x_nistp_verify_sig(const unsigned char *dgst, int dgstlen,
  ...
  if (group->meth->point_get_affine_coordinates(group, pubkey, x, y, ctx) !=
      1 ||
...
#define EC_GFP_S390X_NISTP_METHOD(bits)                                        \
  ...
  const EC_POINT *points[], const BIGNUM *scalars[], BN_CTX *ctx) {            \
    return ec_GFp_s390x_nistp_mul(group, r, scalar, num, points, scalars, ctx, \
                                  S390X_SCALAR_MULTIPLY_P##bits,               \
                                  S390X_SIZE_P##bits);                         \
  }                                                                            \
                                                                               \
  static ECDSA_SIG *ecdsa_s390x_nistp##bits##_sign_sig(                        \
  const unsigned char *dgst, int dgstlen, const BIGNUM *kinv, const BIGNUM *r, \
  EC_KEY *eckey) {                                                             \
    return ecdsa_s390x_nistp_sign_sig(dgst, dgstlen, kinv, r, eckey,           \
                                      ...
                                      S390X_SIZE_P##bits);                     \
                                                                               ...
                                                                               \
  static int ecdsa_s390x_nistp##bits##_verify_sig(                             \
  const unsigned char *dgst, int dgstlen, const ECDSA_SIG *sig,                \
  EC_KEY *eckey) {                                                             \
