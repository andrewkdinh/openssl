...
static int ecdsa_sign_setup(EC_KEY *eckey, BN_CTX *ctx_in, BIGNUM **kinvp,
  ...
  if (order_bits < MIN_ECDSA_SIGN_ORDERBITS || !BN_set_bit(k, order_bits) ||
      !BN_set_bit(r, order_bits) || !BN_set_bit(X, order_bits))
    goto err;
    ...
    do {
      ...
      if (dgst != NULL) {
        if (nonce_type == 1) {
          ...
          k, order, priv_key, dgst, dlen, digestname, libctx, propq);
#endif
        } else {
          res =
          ossl_bn_gen_dsa_nonce_fixed_top(k, order, priv_key, dgst, dlen, ctx);
        }
      } else {
...
int ossl_ecdsa_simple_sign_setup(EC_KEY *eckey, BN_CTX *ctx_in, BIGNUM **kinvp,
                                 BIGNUM **rp) {
  return ecdsa_sign_setup(eckey, ctx_in, kinvp, rp, NULL, 0, 0, NULL, NULL,
                          NULL);
}
...
ECDSA_SIG *ossl_ecdsa_simple_sign_sig(const unsigned char *dgst, int dgst_len,
  ...
  do {
    if (in_kinv == NULL || in_r == NULL) {
      if (!ecdsa_sign_setup(eckey, ctx, &kinv, &ret->r, dgst, dgst_len, 0, NULL,
                            NULL, NULL)) {
        ERR_raise(ERR_LIB_EC, ERR_R_ECDSA_LIB);
        goto err;
      }
