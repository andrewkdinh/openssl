...
__owur static int ecp_nistz256_points_mul(const EC_GROUP *group, EC_POINT *r,
  ...
  if (scalar) {
    ...
    if (preComputedTable) {
       ...
       */
      infty = (p.p.X[0] | p.p.X[1] | p.p.X[2] | p.p.X[3] | p.p.Y[0] | p.p.Y[1] |
               p.p.Y[2] | p.p.Y[3]);
      if (P256_LIMBS == 8)
...
static int ecp_nistz256_inv_mod_ord(const EC_GROUP *group, BIGNUM *r,
  ...
  for (i = 0; i < 27; i++) {
    static const struct {
      unsigned char p, i;
    } chain[27] = {{32, i_x32},    {6, i_101111}, {5, i_111},    {4, i_11},
...
static int ecp_nistz256group_full_init(EC_GROUP *group,
  ...
  if (!ossl_bn_mont_ctx_set(mont, p, 256, params + 6 * param_len, param_len, 1,
                            0))
    goto err;
