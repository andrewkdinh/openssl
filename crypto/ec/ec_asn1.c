...
ASN1_ADB_TEMPLATE(char_two_def) = ASN1_SIMPLE(X9_62_CHARACTERISTIC_TWO, p.other,
                                              ASN1_ANY);
...
{ADB_ENTRY(NID_X9_62_onBasis,
 ...
 ASN1_SIMPLE(X9_62_CHARACTERISTIC_TWO, p.ppBasis,
             X9_62_PENTANOMIAL))} ASN1_ADB_END(X9_62_CHARACTERISTIC_TWO, 0,
...
{ADB_ENTRY(NID_X9_62_prime_field,
 ...
 ADB_ENTRY(NID_X9_62_characteristic_two_field,
           ASN1_SIMPLE(X9_62_FIELDID, p.char_two,
                       X9_62_CHARACTERISTIC_TWO))} ASN1_ADB_END(X9_62_FIELDID,
                                                                0, fieldType, 0,
                                                                &fieldID_def_tt,
                                                                NULL);
...
{ASN1_SIMPLE(X9_62_FIELDID, fieldType, ASN1_OBJECT),
 ASN1_ADB_OBJECT(X9_62_FIELDID)} static_ASN1_SEQUENCE_END(X9_62_FIELDID)

ASN1_SEQUENCE(
X9_62_CURVE) = {ASN1_SIMPLE(X9_62_CURVE, a, ASN1_OCTET_STRING),
                ASN1_SIMPLE(X9_62_CURVE, b, ASN1_OCTET_STRING),
                ASN1_OPT(X9_62_CURVE, seed,
...
ASN1_CHOICE(
ECPKPARAMETERS) = {ASN1_SIMPLE(ECPKPARAMETERS, value.named_curve, ASN1_OBJECT),
                   ASN1_SIMPLE(ECPKPARAMETERS, value.parameters, ECPARAMETERS),
                   ASN1_SIMPLE(ECPKPARAMETERS, value.implicitlyCA,
...
ASN1_SEQUENCE(
EC_PRIVATEKEY) = {ASN1_EMBED(EC_PRIVATEKEY, version, INT32),
                  ASN1_SIMPLE(EC_PRIVATEKEY, privateKey, ASN1_OCTET_STRING),
                  ASN1_EXP_OPT(EC_PRIVATEKEY, parameters, ECPKPARAMETERS, 0),
                  ASN1_EXP_OPT(EC_PRIVATEKEY, publicKey, ASN1_BIT_STRING,
...
EC_GROUP *EC_GROUP_new_from_ecparameters(const ECPARAMETERS *params) {
  ...
  if (params->fieldID == NULL || params->fieldID->fieldType == NULL ||
   ...
   * compatibility.
   */
  if (params->curve == NULL || params->curve->a == NULL ||
      params->curve->a->data == NULL || params->curve->b == NULL ||
      params->curve->b->data == NULL) {
    ERR_raise(ERR_LIB_EC, EC_R_ASN1_ERROR);
    goto err;
  }
  ...
  if (params->order == NULL || params->base == NULL ||
      params->base->data == NULL || params->base->length == 0) {
    ERR_raise(ERR_LIB_EC, EC_R_ASN1_ERROR);
    goto err;
  }
  ...
  if (!EC_POINT_oct2point(ret, point, params->base->data, params->base->length,
                          NULL)) {
    ERR_raise(ERR_LIB_EC, ERR_R_EC_LIB);
    goto err;
  }
...
int i2d_ECPrivateKey(const EC_KEY *a, unsigned char **out) {
  ...
  if (!(a->enc_flag & EC_PKEY_NO_PARAMETERS)) {
    if ((priv_key->parameters =
         EC_GROUP_get_ecpkparameters(a->group, priv_key->parameters)) == NULL) {
      ERR_raise(ERR_LIB_EC, ERR_R_EC_LIB);
      goto err;
    }
  }
...
ECDSA_SIG *d2i_ECDSA_SIG(ECDSA_SIG **psig, const unsigned char **ppin,
                         long len) {
  ECDSA_SIG *sig;
...
int i2d_ECDSA_SIG(const ECDSA_SIG *sig, unsigned char **ppout) {
  ...
  if (!ossl_encode_der_dsa_sig(&pkt, sig->r, sig->s) ||
      !WPACKET_get_total_written(&pkt, &encoded_len) || !WPACKET_finish(&pkt)) {
    BUF_MEM_free(buf);
    WPACKET_cleanup(&pkt);
    return -1;
  }
