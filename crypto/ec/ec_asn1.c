...
ASN1_SEQUENCE(X9_62_CHARACTERISTIC_TWO)
    ...
    ASN1_ADB(X9_62_FIELDID) = {
        ...
        ADB_ENTRY(
            NID_X9_62_characteristic_two_field,
            ASN1_SIMPLE(X9_62_FIELDID, p.char_two, X9_62_CHARACTERISTIC_TWO))}
...
/* the function definitions */
    ...
    static int ec_asn1_group2fieldid(const EC_GROUP *group,
                                     X9_62_FIELDID *field)
    {
        int ok = 0, nid;
        BIGNUM *tmp = NULL;
...
err:
    ...
    /* set the base point */
        ...
        if (ret->base == NULL
            && (ret->base = ASN1_OCTET_STRING_new()) == NULL) {
            OPENSSL_free(buffer);
            ERR_raise(ERR_LIB_EC, ERR_R_ASN1_LIB);
            goto err;
        }
...
# endif
     ...
     */
        ...
        if ((curve_name = ossl_ec_curve_nid_from_params(dup, ctx))
            != NID_undef) {
        /*
...
/* EC_GROUP <-> DER encoding of ECPKPARAMETERS */
    ...
    EC_GROUP *d2i_ECPKParameters(EC_GROUP * *a, const unsigned char **in,
                                 long len)
    {
        EC_GROUP *group = NULL;
...
err:
    ...
    int i2o_ECPublicKey(const EC_KEY *a, unsigned char **out)
    {
        ...
        buf_len = EC_POINT_point2oct(a->group, a->pub_key, a->conv_form, NULL,
                                     0, NULL);
        ...
        if (!EC_POINT_point2oct(a->group, a->pub_key, a->conv_form, *out,
                                buf_len, NULL)) {
            ERR_raise(ERR_LIB_EC, ERR_R_EC_LIB);
            if (new_buffer) {
                OPENSSL_free(*out);
                             ...
                             long len)
    {
        ...
        if (sig->r == NULL || sig->s == NULL
            || ossl_decode_der_dsa_sig(sig->r, sig->s, ppin, (size_t)len)
                == 0) {
            if (psig == NULL || *psig == NULL)
                ECDSA_SIG_free(sig);
            return NULL;
