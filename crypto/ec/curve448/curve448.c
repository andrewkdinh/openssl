...
#define WBITS                                                                  \
    C448_WORD_BITS /* NB this may be different from ARCH_WORD_BITS             \
                    */
...
static int recode_wnaf(struct smvt_control *control,
    ...
    for (w = 1; w < (C448_SCALAR_BITS - 1) / 16 + 3; w++) {
        if (w < (C448_SCALAR_BITS - 1) / 16 + 1) {
            ...
            current +=
            (uint32_t)((scalar->limb[w / B_OVER_16] >> (16 * (w % B_OVER_16)))
                       << 16);
        }
    ...
    for (i--; i >= 0; i--) {
        ...
        if (cv) {
            ...
            if (control_var[contv].addend > 0)
                add_pniels_to_pt(
                combo, precmp_var[control_var[contv].addend >> 1], i && !cp);
            else
                sub_pniels_from_pt(
                combo, precmp_var[(-control_var[contv].addend) >> 1], i && !cp);
            contv++;
        ...
        if (cp) {
            ...
            if (control_pre[contp].addend > 0)
                add_niels_to_pt(
                combo, ossl_curve448_wnaf_base[control_pre[contp].addend >> 1],
                i);
            else
                sub_niels_from_pt(
                combo,
                ossl_curve448_wnaf_base[(-control_pre[contp].addend) >> 1], i);
            contp++;
        }
    }
