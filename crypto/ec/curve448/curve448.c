...
void ossl_curve448_precomputed_scalarmul(curve448_point_t out,
  ...
  for (i = s; i > 0; i--) {
    ...
    for (j = 0; j < n; j++) {
      ...
      constant_time_lookup_niels(ni, &table->table[j << (t - 1)], 1 << (t - 1),
                                 tab);
...
static int recode_wnaf(struct smvt_control *control,
  ...
  for (w = 1; w < (C448_SCALAR_BITS - 1) / 16 + 3; w++) {
    if (w < (C448_SCALAR_BITS - 1) / 16 + 1) {
      ...
      current +=
      (uint32_t)((scalar->limb[w / B_OVER_16] >> (16 * (w % B_OVER_16))) << 16);
    }
...
const curve448_point_t base2, const curve448_scalar_t scalar2) {
  ...
  if (i > control_pre[0].power) {
    ...
    pniels_to_pt(combo, precmp_var[control_var[0].addend >> 1]);
    add_niels_to_pt(combo, ossl_curve448_wnaf_base[control_pre[0].addend >> 1],
                    i);
    contv++;
    contp++;
  } else {
  ...
  for (i--; i >= 0; i--) {
    ...
    if (cv) {
      ...
      if (control_var[contv].addend > 0)
        add_pniels_to_pt(combo, precmp_var[control_var[contv].addend >> 1],
                         i && !cp);
      else
        sub_pniels_from_pt(combo, precmp_var[(-control_var[contv].addend) >> 1],
                           i && !cp);
      contv++;
    }
    ...
    if (cp) {
      ...
      if (control_pre[contp].addend > 0)
        add_niels_to_pt(
        combo, ossl_curve448_wnaf_base[control_pre[contp].addend >> 1], i);
      else
        sub_niels_from_pt(
        combo, ossl_curve448_wnaf_base[(-control_pre[contp].addend) >> 1], i);
      contp++;
