...
const uint8_t ed[EDDSA_448_PRIVATE_BYTES], const char *propq) {
  ...
  return oneshot_hash(ctx, x, X448_PRIVATE_BYTES, ed, EDDSA_448_PRIVATE_BYTES,
                      propq);
}
...
const uint8_t privkey[EDDSA_448_PRIVATE_BYTES], const char *propq) {
  ...
  if (!oneshot_hash(ctx, secret_scalar_ser, sizeof(secret_scalar_ser), privkey,
...
uint8_t context_len, const char *propq) {
...

  {
    ...
    if (hashctx == NULL ||
        !hash_init_with_dom(ctx, hashctx, prehashed, 0, context, context_len,
                            propq) ||
  ...
  response_scalar, &signature[EDDSA_448_PUBLIC_BYTES], EDDSA_448_PRIVATE_BYTES);

  /* pk_point = -c(x(P)) + (cx + k)G = kG */
  ossl_curve448_base_double_scalarmul_non_secret(pk_point, response_scalar,
