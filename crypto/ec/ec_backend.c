...
static int ec_group_explicit_todata(const EC_GROUP *group, OSSL_PARAM_BLD *tmpl,
  ...
  if (tmpl != NULL || param != NULL) {
    if (!ossl_param_build_set_utf8_string(
        tmpl, params, OSSL_PKEY_PARAM_EC_FIELD_TYPE, field_type)) {
      ERR_raise(ERR_LIB_EC, ERR_R_CRYPTO_LIB);
      goto err;
    }
  }
  ...
  if (tmpl != NULL || param != NULL) {
    ...
    if (!ossl_param_build_set_octet_string(
        tmpl, params, OSSL_PKEY_PARAM_EC_GENERATOR, *genbuf, genbuf_len)) {
      ERR_raise(ERR_LIB_EC, ERR_R_CRYPTO_LIB);
      goto err;
    }
  }
  ...
  if (tmpl != NULL || param != NULL) {
    ...
    if (cofactor != NULL &&
        !ossl_param_build_set_bn(tmpl, params, OSSL_PKEY_PARAM_EC_COFACTOR,
                                 cofactor)) {
      ERR_raise(ERR_LIB_EC, ERR_R_CRYPTO_LIB);
      goto err;
    }
  ...
  if (tmpl != NULL || param != NULL) {
    ...
    if (seed != NULL && seed_len > 0 &&
        ...
        tmpl, params, OSSL_PKEY_PARAM_EC_SEED, seed, seed_len)) {
      ERR_raise(ERR_LIB_EC, ERR_R_CRYPTO_LIB);
      goto err;
    }
  }
...
int ossl_ec_group_todata(const EC_GROUP *group, OSSL_PARAM_BLD *tmpl,
  ...
  if (pt_form_name == NULL ||
      ...
      tmpl, params, OSSL_PKEY_PARAM_EC_POINT_CONVERSION_FORMAT, pt_form_name)) {
  ...
  if (curve_nid != NID_undef) {
    ...
    if (curve_name == NULL ||
        !ossl_param_build_set_utf8_string(
        tmpl, params, OSSL_PKEY_PARAM_GROUP_NAME, curve_name)) {
      ERR_raise(ERR_LIB_EC, EC_R_INVALID_CURVE);
      goto err;
    }
  }
...
/*
 * Callers of ossl_ec_key_fromdata MUST make sure that ec_key_params_fromdata
...
int ossl_ec_key_fromdata(EC_KEY *ec, const OSSL_PARAM params[],
  ...
  if (param_pub_key != NULL)
    if (!OSSL_PARAM_get_octet_string(param_pub_key, (void **)&pub_key, 0,
                                     &pub_key_len) ||
