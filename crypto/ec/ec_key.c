...
ENGINE *EC_KEY_get0_engine(const EC_KEY *eckey) { return eckey->engine; }
...
int ossl_ec_key_public_check(const EC_KEY *eckey, BN_CTX *ctx) {
    ...
    if (cofactor != NULL && BN_is_one(cofactor)) {
        /* Skip the unnecessary expensive computation for curves with cofactor
         * of 1.
         */
        return 1;
    }
...
OSSL_LIB_CTX *ossl_ec_key_get_libctx(const EC_KEY *key) { return key->libctx; }

const char *ossl_ec_key_get0_propq(const EC_KEY *key) { return key->propq; }
