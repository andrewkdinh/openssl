...
static int ecx_pkey_export_to(const EVP_PKEY *from, void *to_keydata, OSSL_FUNC_keymgmt_import_fn *importer,
...
const EVP_PKEY_ASN1_METHOD ossl_ecx25519_asn1_meth = {EVP_PKEY_X25519,
                                                      EVP_PKEY_X25519,
                                                      0,
                                                      "X25519",
...
const EVP_PKEY_ASN1_METHOD ossl_ecx448_asn1_meth = {EVP_PKEY_X448,
                                                    EVP_PKEY_X448,
                                                    0,
                                                    "X448",
...
static int ecd_item_verify(EVP_MD_CTX *ctx, const ASN1_ITEM *it, const void *asn, const X509_ALGOR *sigalg,
...
static int ecd_item_sign25519(EVP_MD_CTX *ctx, const ASN1_ITEM *it, const void *asn, X509_ALGOR *alg1, X509_ALGOR *alg2,
                              ASN1_BIT_STRING *str)
{
...
static int ecd_item_sign448(EVP_MD_CTX *ctx, const ASN1_ITEM *it, const void *asn, X509_ALGOR *alg1, X509_ALGOR *alg2,
                            ASN1_BIT_STRING *str)
{
...
const EVP_PKEY_ASN1_METHOD ossl_ed25519_asn1_meth = {EVP_PKEY_ED25519,
                                                     EVP_PKEY_ED25519,
                                                     0,
                                                     "ED25519",
...
const EVP_PKEY_ASN1_METHOD ossl_ed448_asn1_meth = {EVP_PKEY_ED448,
                                                   EVP_PKEY_ED448,
                                                   0,
                                                   "ED448",
...
static int validate_ecx_derive(EVP_PKEY_CTX *ctx, unsigned char *key, size_t *keylen, const unsigned char **privkey,
...
static int pkey_ecx_derive25519(EVP_PKEY_CTX *ctx, unsigned char *key, size_t *keylen)
{
    ...
    if (!validate_ecx_derive(ctx, key, keylen, &privkey, &pubkey)
        || (key != NULL && ossl_x25519(key, privkey, pubkey) == 0))
        return 0;
...
static int pkey_ecx_derive448(EVP_PKEY_CTX *ctx, unsigned char *key, size_t *keylen)
{
    ...
    if (!validate_ecx_derive(ctx, key, keylen, &privkey, &pubkey)
        || (key != NULL && ossl_x448(key, privkey, pubkey) == 0))
        return 0;
    *keylen = X448_KEYLEN;
    return 1;
}
...
static const EVP_PKEY_METHOD ecx25519_pkey_meth = {EVP_PKEY_X25519,
                                                   0,
                                                   0,
                                                   0,
                                                   0,
...
static const EVP_PKEY_METHOD ecx448_pkey_meth = {EVP_PKEY_X448,
                                                 0,
                                                 0,
                                                 0,
                                                 0,
...
static int pkey_ecd_digestsign25519(EVP_MD_CTX *ctx, unsigned char *sig, size_t *siglen, const unsigned char *tbs,
                                    size_t tbslen)
{
    ...
    if (ossl_ed25519_sign(sig, tbs, tbslen, edkey->pubkey, edkey->privkey, 0, 0, 0, NULL, 0, NULL, NULL) == 0)
        return 0;
    *siglen = ED25519_SIGSIZE;
    return 1;
}

static int pkey_ecd_digestsign448(EVP_MD_CTX *ctx, unsigned char *sig, size_t *siglen, const unsigned char *tbs,
                                  size_t tbslen)
{
    const ECX_KEY *edkey = evp_pkey_get_legacy(EVP_MD_CTX_get_pkey_ctx(ctx)->pkey);
    ...
    if (ossl_ed448_sign(edkey->libctx, sig, tbs, tbslen, edkey->pubkey, edkey->privkey, NULL, 0, 0, edkey->propq) == 0)
        return 0;
    *siglen = ED448_SIGSIZE;
    return 1;
}

static int pkey_ecd_digestverify25519(EVP_MD_CTX *ctx, const unsigned char *sig, size_t siglen,
                                      const unsigned char *tbs, size_t tbslen)
{
    ...
    return ossl_ed25519_verify(tbs, tbslen, sig, edkey->pubkey, 0, 0, 0, NULL, 0, edkey->libctx, edkey->propq);
}

static int pkey_ecd_digestverify448(EVP_MD_CTX *ctx, const unsigned char *sig, size_t siglen, const unsigned char *tbs,
                                    size_t tbslen)
{
    const ECX_KEY *edkey = evp_pkey_get_legacy(EVP_MD_CTX_get_pkey_ctx(ctx)->pkey);
    ...
    return ossl_ed448_verify(edkey->libctx, tbs, tbslen, sig, edkey->pubkey, NULL, 0, 0, edkey->propq);
}
...
static const EVP_PKEY_METHOD ed25519_pkey_meth = {EVP_PKEY_ED25519,
...
static const EVP_PKEY_METHOD ed448_pkey_meth = {EVP_PKEY_ED448,
                                                EVP_PKEY_FLAG_SIGCTX_CUSTOM,
                                                0,
                                                0,
                                                0,
...
static int s390x_pkey_ecx_derive25519(EVP_PKEY_CTX *ctx, unsigned char *key, size_t *keylen)
{
    ...
    if (!validate_ecx_derive(ctx, key, keylen, &privkey, &pubkey)
        || (key != NULL && s390x_x25519_mul(key, privkey, pubkey) == 0))
        return 0;
    *keylen = X25519_KEYLEN;
    return 1;
}
...
static int s390x_pkey_ecx_derive448(EVP_PKEY_CTX *ctx, unsigned char *key, size_t *keylen)
{
    ...
    if (!validate_ecx_derive(ctx, key, keylen, &privkey, &pubkey)
        || (key != NULL && s390x_x448_mul(key, pubkey, privkey) == 0))
        return 0;
    *keylen = X448_KEYLEN;
    return 1;
}

static int s390x_pkey_ecd_digestsign25519(EVP_MD_CTX *ctx, unsigned char *sig, size_t *siglen, const unsigned char *tbs,
                                          size_t tbslen)
{
    union {
        struct {
...
static int s390x_pkey_ecd_digestsign448(EVP_MD_CTX *ctx, unsigned char *sig, size_t *siglen, const unsigned char *tbs,
                                        size_t tbslen)
{
    union {
        struct {
...
static int s390x_pkey_ecd_digestverify25519(EVP_MD_CTX *ctx, const unsigned char *sig, size_t siglen,
                                            const unsigned char *tbs, size_t tbslen)
{
    union {
        struct {
...
static int s390x_pkey_ecd_digestverify448(EVP_MD_CTX *ctx, const unsigned char *sig, size_t siglen,
                                          const unsigned char *tbs, size_t tbslen)
{
    union {
        struct {
...
static const EVP_PKEY_METHOD ecx25519_s390x_pkey_meth = {EVP_PKEY_X25519,
                                                         0,
                                                         0,
                                                         0,
                                                         0,
...
static const EVP_PKEY_METHOD ecx448_s390x_pkey_meth = {EVP_PKEY_X448,
                                                       0,
                                                       0,
                                                       0,
                                                       0,
                                                       ...
                                                       pkey_ecx_ctrl,
                                                       0};
static const EVP_PKEY_METHOD ed25519_s390x_pkey_meth = {EVP_PKEY_ED25519,
                                                        EVP_PKEY_FLAG_SIGCTX_CUSTOM,
                                                        0,
                                                        0,
                                                        0,
...
static const EVP_PKEY_METHOD ed448_s390x_pkey_meth = {EVP_PKEY_ED448,
                                                      EVP_PKEY_FLAG_SIGCTX_CUSTOM,
                                                      0,
                                                      0,
                                                      0,
