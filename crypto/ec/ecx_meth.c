...
static int ecx_set_priv_key(EVP_PKEY *pkey, const unsigned char *priv, size_t len)
{
    ...
    ecx = ossl_ecx_key_op(NULL, priv, (int)len, pkey->ameth->pkey_id, KEY_OP_PRIVATE, libctx, NULL);
...
static int ecx_set_pub_key(EVP_PKEY *pkey, const unsigned char *pub, size_t len)
{
    ...
    ecx = ossl_ecx_key_op(NULL, pub, (int)len, pkey->ameth->pkey_id, KEY_OP_PUBLIC, libctx, NULL);
...
static int ecx_pkey_export_to(const EVP_PKEY              *from,
                              void                        *to_keydata,
...
static int ecd_item_sign25519(EVP_MD_CTX      *ctx,
                              const ASN1_ITEM *it,
                              const void      *asn,
                              X509_ALGOR      *alg1,
                              X509_ALGOR      *alg2,
...
static int ecd_item_sign448(EVP_MD_CTX      *ctx,
                            const ASN1_ITEM *it,
                            const void      *asn,
                            X509_ALGOR      *alg1,
                            X509_ALGOR      *alg2,
...
static int
pkey_ecd_digestsign25519(EVP_MD_CTX *ctx, unsigned char *sig, size_t *siglen, const unsigned char *tbs, size_t tbslen)
{
...
static int
pkey_ecd_digestsign448(EVP_MD_CTX *ctx, unsigned char *sig, size_t *siglen, const unsigned char *tbs, size_t tbslen)
{
    const ECX_KEY *edkey = evp_pkey_get_legacy(EVP_MD_CTX_get_pkey_ctx(ctx)->pkey);
...
static int pkey_ecd_digestverify25519(EVP_MD_CTX          *ctx,
...
static int pkey_ecd_digestverify448(EVP_MD_CTX          *ctx,
                                    const unsigned char *sig,
                                    size_t               siglen,
                                    const unsigned char *tbs,
                                    size_t               tbslen)
