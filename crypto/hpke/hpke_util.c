...
{OSSL_HPKE_KEM_ID_P256, "EC", OSSL_HPKE_KEMSTR_P256, LN_sha256,
 SHA256_DIGEST_LENGTH, 65, 65, 32, 0xFF},
{OSSL_HPKE_KEM_ID_P384, "EC", OSSL_HPKE_KEMSTR_P384, LN_sha384,
 SHA384_DIGEST_LENGTH, 97, 97, 48, 0xFF},
{OSSL_HPKE_KEM_ID_P521, "EC", OSSL_HPKE_KEMSTR_P521, LN_sha512,
 SHA512_DIGEST_LENGTH, 133, 133, 66, 0x01},
#ifndef OPENSSL_NO_ECX
{OSSL_HPKE_KEM_ID_X25519, OSSL_HPKE_KEMSTR_X25519, NULL, LN_sha256,
 SHA256_DIGEST_LENGTH, X25519_KEYLEN, X25519_KEYLEN, X25519_KEYLEN, 0x00},
{OSSL_HPKE_KEM_ID_X448, OSSL_HPKE_KEMSTR_X448, NULL, LN_sha512,
 SHA512_DIGEST_LENGTH, X448_KEYLEN, X448_KEYLEN, X448_KEYLEN, 0x00}
#endif
#else
...
{OSSL_HPKE_KDF_ID_HKDF_SHA256, {OSSL_HPKE_KDFSTR_256, "0x1", "0x01", "1"}},
...
static int kdf_derive(EVP_KDF_CTX *kctx, unsigned char *out, size_t outlen,
                      int mode, const unsigned char *salt, size_t saltlen,
  ...
  *p++ = OSSL_PARAM_construct_int(OSSL_KDF_PARAM_MODE, &mode);
  if (salt != NULL)
    *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SALT, (char *)salt,
                                             saltlen);
  if (ikm != NULL)
    *p++ =
    OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_KEY, (char *)ikm, ikmlen);
  if (info != NULL)
    *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_INFO, (char *)info,
                                             infolen);
...
int ossl_hpke_kdf_extract(EVP_KDF_CTX *kctx, unsigned char *prk, size_t prklen,
                          ...
                          const unsigned char *ikm, size_t ikmlen) {
  return kdf_derive(kctx, prk, prklen, EVP_KDF_HKDF_MODE_EXTRACT_ONLY, salt,
                    saltlen, ikm, ikmlen, NULL, 0);
}
...
int ossl_hpke_kdf_expand(EVP_KDF_CTX *kctx, unsigned char *okm, size_t okmlen,
                         ...
                         const unsigned char *info, size_t infolen) {
  return kdf_derive(kctx, okm, okmlen, EVP_KDF_HKDF_MODE_EXPAND_ONLY, NULL, 0,
                    prk, prklen, info, infolen);
}
...
int ossl_hpke_labeled_extract(EVP_KDF_CTX *kctx, unsigned char *prk,
  ...
  labeled_ikmlen =
  label_hpkev1len + protocol_labellen + suiteidlen + labellen + ikmlen;
  ...
  ret = ossl_hpke_kdf_extract(kctx, prk, prklen, salt, saltlen, labeled_ikm,
                              labeled_ikmlen);
end:
  WPACKET_cleanup(&pkt);
...
int ossl_hpke_labeled_expand(EVP_KDF_CTX *kctx, unsigned char *okm,
  ...
  protocol_labellen = strlen(protocol_label);
  labellen = strlen(label);
  labeled_infolen = 2 + okmlen + prklen + label_hpkev1len + protocol_labellen +
                    suiteidlen + labellen + infolen;
  ...
  ret = ossl_hpke_kdf_expand(kctx, okm, okmlen, prk, prklen, labeled_info,
                             labeled_infolen);
end:
  WPACKET_cleanup(&pkt);
...
int ossl_hpke_str2suite(const char *suitestr, OSSL_HPKE_SUITE *suite) {
  ...
  while (st != NULL && labels < 3) {
    ...
    if (labels == 0 &&
        (kem = synonyms_name2id(st, kemstrtab, OSSL_NELEM(kemstrtab))) == 0)
      goto fail;
    else if (labels == 1 && (kdf = synonyms_name2id(
                             st, kdfstrtab, OSSL_NELEM(kdfstrtab))) == 0)
      goto fail;
    else if (labels == 2 && (aead = synonyms_name2id(
                             st, aeadstrtab, OSSL_NELEM(aeadstrtab))) == 0)
      goto fail;
