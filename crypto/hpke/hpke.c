...
static int hpke_aead_dec(OSSL_HPKE_CTX *hctx, const unsigned char *iv,
  ...
  if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_IVLEN, hctx->noncelen, NULL) !=
      1) {
    ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
    goto err;
  }
...
static int hpke_encap(OSSL_HPKE_CTX *ctx, unsigned char *enc, size_t *enclen,
  ...
  if (hpke_kem_id_nist_curve(ctx->suite.kem_id) == 1) {
    pkR = evp_pkey_new_raw_nist_public_key(ctx->libctx, ctx->propq,
                                           kem_info->groupname, pub, publen);
  } else {
    pkR = EVP_PKEY_new_raw_public_key_ex(ctx->libctx, kem_info->keytype,
  ...
  *p++ = OSSL_PARAM_construct_utf8_string(OSSL_KEM_PARAM_OPERATION,
                                          OSSL_KEM_PARAM_OPERATION_DHKEM, 0);
  if (ctx->ikme != NULL) {
    *p++ = OSSL_PARAM_construct_octet_string(OSSL_KEM_PARAM_IKME, ctx->ikme,
                                             ctx->ikmelen);
  }
...
static int hpke_decap(OSSL_HPKE_CTX *ctx, const unsigned char *enc,
  ...
  *p = OSSL_PARAM_construct_end();
  if (ctx->mode == OSSL_HPKE_MODE_AUTH || ctx->mode == OSSL_HPKE_MODE_PSKAUTH) {
    ...
    if (hpke_kem_id_nist_curve(ctx->suite.kem_id) == 1) {
      spub = evp_pkey_new_raw_nist_public_key(ctx->libctx, ctx->propq,
                                              kem_info->groupname, ctx->authpub,
                                              ctx->authpublen);
    } else {
      spub =
      EVP_PKEY_new_raw_public_key_ex(ctx->libctx, kem_info->keytype, ctx->propq,
  ...
  if (ctx->shared_secret == NULL)
    goto err;
  if (EVP_PKEY_decapsulate(pctx, ctx->shared_secret, &lsslen, enc, enclen) !=
      1) {
    ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
    goto err;
  }
...
static int hpke_do_middle(OSSL_HPKE_CTX *ctx, const unsigned char *info,
  ...
  if (ossl_hpke_labeled_extract(kctx, ks_context + 1, halflen, NULL, 0,
                                OSSL_HPKE_SEC51LABEL, suitebuf,
                                sizeof(suitebuf), OSSL_HPKE_PSKIDHASH_LABEL,
                                (unsigned char *)ctx->pskid, pskidlen) != 1) {
    ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
    goto err;
  }
  if (ossl_hpke_labeled_extract(kctx, ks_context + 1 + halflen, halflen, NULL,
                                0, OSSL_HPKE_SEC51LABEL, suitebuf,
                                sizeof(suitebuf), OSSL_HPKE_INFOHASH_LABEL,
  ...
  if (ossl_hpke_labeled_extract(
      kctx, secret, secretlen, ctx->shared_secret, ctx->shared_secretlen,
      OSSL_HPKE_SEC51LABEL, suitebuf, sizeof(suitebuf), OSSL_HPKE_SECRET_LABEL,
  ...
  if (ctx->suite.aead_id != OSSL_HPKE_AEAD_ID_EXPORTONLY) {
    ...
    if (ctx->nonce == NULL)
      goto err;
    if (ossl_hpke_labeled_expand(kctx, ctx->nonce, ctx->noncelen, secret,
                                 secretlen, OSSL_HPKE_SEC51LABEL, suitebuf,
                                 sizeof(suitebuf), OSSL_HPKE_NONCE_LABEL,
    ...
    if (ctx->key == NULL)
      goto err;
    if (ossl_hpke_labeled_expand(kctx, ctx->key, ctx->keylen, secret, secretlen,
                                 OSSL_HPKE_SEC51LABEL, suitebuf,
                                 sizeof(suitebuf), OSSL_HPKE_KEY_LABEL,
                                 ks_context, ks_contextlen) != 1) {
      ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
      goto err;
    }
  ...
  if (ossl_hpke_labeled_expand(kctx, ctx->exportersec, ctx->exporterseclen,
                               ...
                               suitebuf, sizeof(suitebuf), OSSL_HPKE_EXP_LABEL,
                               ks_context, ks_contextlen) != 1) {
    ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
    goto err;
  }
...
int OSSL_HPKE_CTX_set1_authpub(OSSL_HPKE_CTX *ctx, const unsigned char *pub,
  ...
  if (hpke_kem_id_nist_curve(ctx->suite.kem_id) == 1) {
    ...
    pubp = EVP_PKEY_new_raw_public_key_ex(ctx->libctx, kem_info->keytype,
                                          ctx->propq, pub, publen);
  }
  if (pubp == NULL) {
  ...
  if (lpub == NULL)
    goto err;
  if (EVP_PKEY_get_octet_string_param(pubp, OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY,
                                      lpub, OSSL_HPKE_MAXSIZE, &lpublen) != 1) {
    OPENSSL_free(lpub);
    ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
    goto err;
  }
...
int OSSL_HPKE_seal(OSSL_HPKE_CTX *ctx, unsigned char *ct, size_t *ctlen,
  ...
  if (ctx == NULL || ct == NULL || ctlen == NULL || *ctlen == 0 || pt == NULL ||
      ptlen == 0) {
    ERR_raise(ERR_LIB_CRYPTO, ERR_R_PASSED_INVALID_ARGUMENT);
    return 0;
  }
...
int OSSL_HPKE_open(OSSL_HPKE_CTX *ctx, unsigned char *pt, size_t *ptlen,
  ...
  if (ctx == NULL || pt == NULL || ptlen == NULL || *ptlen == 0 || ct == NULL ||
      ctlen == 0) {
    ERR_raise(ERR_LIB_CRYPTO, ERR_R_PASSED_INVALID_ARGUMENT);
    return 0;
  }
...
int OSSL_HPKE_export(OSSL_HPKE_CTX *ctx, unsigned char *secret,
  ...
  suitebuf[4] = ctx->suite.aead_id / 256;
  suitebuf[5] = ctx->suite.aead_id % 256;
  erv = ossl_hpke_labeled_expand(kctx, secret, secretlen, ctx->exportersec,
                                 ctx->exporterseclen, OSSL_HPKE_SEC51LABEL,
