                                ...
                                size_t blen)
{
    ...
    for (i = 0; i < blen; i++) buf[i] ^= ctx->nonce[i];
    return blen;
}
                      ...
                      const unsigned char *pub, size_t publen)
{
    ...
    if (hpke_kem_id_nist_curve(ctx->suite.kem_id) == 1) {
        pkR = evp_pkey_new_raw_nist_public_key(
            ctx->libctx, ctx->propq, kem_info->groupname, pub, publen);
    } else {
    ...
    if (ctx->mode == OSSL_HPKE_MODE_AUTH
        ...
        if (hpke_kem_id_nist_curve(ctx->suite.kem_id) == 1) {
            spub = evp_pkey_new_raw_nist_public_key(
                ctx->libctx, ctx->propq, kem_info->groupname, ctx->authpub,
                ctx->authpublen);
        } else {
            spub = EVP_PKEY_new_raw_public_key_ex(
                ctx->libctx, kem_info->keytype, ctx->propq, ctx->authpub,
                ctx->authpublen);
        }
        if (spub == NULL) {
            ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
                          ...
                          size_t infolen)
{
    ...
    if (ossl_hpke_labeled_extract(kctx, secret, secretlen, ctx->shared_secret,
                                  ctx->shared_secretlen, OSSL_HPKE_SEC51LABEL,
                                  suitebuf, sizeof(suitebuf),
                                  OSSL_HPKE_SECRET_LABEL, ctx->psk, ctx->psklen)
        != 1) {
        ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
        goto err;
    }
    if (ctx->suite.aead_id != OSSL_HPKE_AEAD_ID_EXPORTONLY) {
        ...
        if (ctx->key == NULL)
            goto err;
        if (ossl_hpke_labeled_expand(kctx, ctx->key, ctx->keylen, secret,
                                     secretlen, OSSL_HPKE_SEC51LABEL, suitebuf,
    ...
    if (ossl_hpke_labeled_expand(kctx, ctx->exportersec, ctx->exporterseclen,
                                 ...
                                 suitebuf, sizeof(suitebuf),
                                 OSSL_HPKE_EXP_LABEL, ks_context, ks_contextlen)
        != 1) {
        ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
        goto err;
    }
                               ...
                               size_t publen)
{
    ...
    if (kem_info == NULL)
        return 0;
    if (hpke_kem_id_nist_curve(ctx->suite.kem_id) == 1) {
        pubp = evp_pkey_new_raw_nist_public_key(
            ctx->libctx, ctx->propq, kem_info->groupname, pub, publen);
    } else {
                               ...
                               OSSL_LIB_CTX *libctx, const char *propq)
{
    ...
    *suite = chosen;
     ...
     * the relevant curve.
     */
    if (OSSL_HPKE_keygen(chosen, enc, enclen, &fakepriv, NULL, 0, libctx, propq)
        != 1) {
        ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
        goto err;
    }
