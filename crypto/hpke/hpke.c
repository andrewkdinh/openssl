...
static int hpke_aead_dec(OSSL_HPKE_CTX *hctx, const unsigned char *iv,
    ...
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_IVLEN, hctx->noncelen,
                            NULL) != 1) {
        ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
        goto err;
    }
...
static int hpke_do_middle(OSSL_HPKE_CTX *ctx, const unsigned char *info,
    ...
    if (ossl_hpke_labeled_extract(
        ...
        OSSL_HPKE_SECRET_LABEL, ctx->psk, ctx->psklen) != 1) {
        ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
        goto err;
    }
    ...
    if (ossl_hpke_labeled_expand(
        kctx, ctx->exportersec, ctx->exporterseclen, secret, secretlen,
        OSSL_HPKE_SEC51LABEL, suitebuf, sizeof(suitebuf), OSSL_HPKE_EXP_LABEL,
        ks_context, ks_contextlen) != 1) {
        ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
        goto err;
    }
...
int OSSL_HPKE_CTX_set1_authpub(OSSL_HPKE_CTX *ctx, const unsigned char *pub,
    ...
    if (EVP_PKEY_get_octet_string_param(
        pubp, OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY, lpub, OSSL_HPKE_MAXSIZE,
        &lpublen) != 1) {
        OPENSSL_free(lpub);
        ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
        goto err;
