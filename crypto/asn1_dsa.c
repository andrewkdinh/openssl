...
int ossl_encode_der_integer(WPACKET *pkt, const BIGNUM *n)
{
    ...
    if (!WPACKET_start_sub_packet(pkt) || !WPACKET_put_bytes_u8(pkt, ID_INTEGER)
        || !ossl_encode_der_length(pkt, cont_len) || !WPACKET_allocate_bytes(pkt, cont_len, &bnbytes)
        || !WPACKET_close(pkt))
        return 0;
...
int ossl_encode_der_dsa_sig(WPACKET *pkt, const BIGNUM *r, const BIGNUM *s)
{
    ...
    if (!ossl_encode_der_integer(dummypkt, r) || !ossl_encode_der_integer(dummypkt, s)
        || !WPACKET_get_length(dummypkt, &cont_len) || (!isnull && !WPACKET_finish(dummypkt))) {
        if (!isnull)
    ...
    if (!WPACKET_put_bytes_u8(pkt, ID_SEQUENCE)
        || !ossl_encode_der_length(pkt, cont_len)
                ...
                */
        || (!isnull && !ossl_encode_der_integer(pkt, r)) || (!isnull && !ossl_encode_der_integer(pkt, s))
        || !WPACKET_close(pkt))
        return 0;
...
size_t ossl_decode_der_dsa_sig(BIGNUM *r, BIGNUM *s, const unsigned char **ppin, size_t len)
{
    ...
    if (!PACKET_buf_init(&pkt, *ppin, len) || !PACKET_get_1(&pkt, &tag) || tag != ID_SEQUENCE
        || !ossl_decode_der_length(&pkt, &contpkt) || !ossl_decode_der_integer(&contpkt, r)
        || !ossl_decode_der_integer(&contpkt, s) || PACKET_remaining(&contpkt) != 0)
        return 0;
