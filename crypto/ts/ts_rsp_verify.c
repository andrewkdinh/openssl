...
int TS_RESP_verify_signature(PKCS7 *token, STACK_OF(X509) * certs,
  ...
  if (untrusted == NULL || !X509_add_certs(untrusted, certs, 0) ||
...
static int ts_verify_cert(X509_STORE *store, STACK_OF(X509) * untrusted,
  ...
  *chain = NULL;
  ...
  if (i <= 0) {
    ...
    ERR_raise_data(ERR_LIB_TS, TS_R_CERTIFICATE_VERIFY_ERROR, "Verify error:%s",
...
ossl_ess_get_signing_cert(const PKCS7_SIGNER_INFO *si) {
  ASN1_TYPE *attr;
...
ossl_ess_get_signing_cert_v2(const PKCS7_SIGNER_INFO *si) {
  ASN1_TYPE *attr;
  const unsigned char *p;
...
static int int_ts_RESP_verify_token(TS_VERIFY_CTX *ctx, PKCS7 *token,
  ...
  if ((flags & TS_VFY_IMPRINT) &&
      !ts_check_imprints(ctx->md_alg, ctx->imprint, ctx->imprint_len, tst_info))
    goto err;
...
static int ts_check_status_info(TS_RESP *response) {
  ...
  ERR_raise_data(
  ERR_LIB_TS, TS_R_NO_TIME_STAMP_TOKEN,
  "status code: %s, status text: %s, failure codes: %s", status_text,
