...
TS_RESP_CTX *TS_RESP_CTX_new(void) { return TS_RESP_CTX_new_ex(NULL, NULL); }
...
void TS_RESP_CTX_add_flags(TS_RESP_CTX *ctx, int flags) { ctx->flags |= flags; }
...
int TS_RESP_CTX_set_status_info(TS_RESP_CTX *ctx, int status,
                                const char *text) {
...
TS_REQ *TS_RESP_CTX_get_request(TS_RESP_CTX *ctx) { return ctx->request; }
...
static int ts_RESP_sign(TS_RESP_CTX *ctx) {
    ...
    if (ctx->signer_md == NULL)
        ...
        signer_md =
        EVP_MD_fetch(ctx->libctx, EVP_MD_get0_name(ctx->signer_md), ctx->propq);
    else
