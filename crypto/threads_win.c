...
CRYPTO_RCU_LOCK *ossl_rcu_lock_new(int num_writers, OSSL_LIB_CTX *ctx) {
  ...
  if (new->qp_group == NULL || new->alloc_signal == NULL ||
      new->prior_signal == NULL || new->write_lock == NULL ||
      new->alloc_lock == NULL || new->prior_lock == NULL ||
      new->rw_lock == NULL) {
  ...
  for (i = 0; i < MAX_QPS; i++) {
    if (data->thread_qps[i].lock == lock) {
      ...
      if (data->thread_qps[i].depth == 0) {
        CRYPTO_atomic_add64(&data->thread_qps[i].qp->users, (uint64_t)-1,
                            (uint64_t *)&ret, lock->rw_lock);
...
#else
  *ret =
  (int)InterlockedExchangeAdd((LONG volatile *)val, (LONG)amount) + amount;
  return 1;
#endif
}
