...
static void name_string_free(char *name) { OPENSSL_free(name); }
...
int ossl_namemap_add_names(OSSL_NAMEMAP *namemap, int number, const char *names,
  ...
  for (p = tmp; *p != '\0'; p = q) {
    ...
    if (number == 0) {
      ...
      ERR_raise_data(
      ERR_LIB_CRYPTO, CRYPTO_R_CONFLICTING_NAMES,
      "\"%s\" has an existing different identity %d (from \"%s\")", p,
      this_number, names);
      number = 0;
      goto end;
    }
  ...
  for (p = tmp; p < endp; p = q) {
    ...
    if (number == 0) {
      ...
      ERR_raise_data(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR,
                     "Got number %d when expecting %d", this_number, number);
      number = 0;
      goto end;
    }
  }
...
OSSL_NAMEMAP *ossl_namemap_stored(OSSL_LIB_CTX *libctx) {
  ...
  if (nms == 1) {
    ...
    OBJ_NAME_do_all(OBJ_NAME_TYPE_CIPHER_METH, get_legacy_cipher_names,
                    namemap);
