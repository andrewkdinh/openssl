...
static STACK_OF(CONF_IMODULE) *initialized_modules =
NULL; /* protected by lock */
...
static int module_run(const CONF *cnf, const char *name, const char *value,
  ...
  if (!md) {
    if (!(flags & CONF_MFLAGS_SILENT)) {
      ERR_raise_data(ERR_LIB_CONF, CONF_R_UNKNOWN_MODULE_NAME, "module=%s",
                     name);
    }
    return -1;
  }
  ...
  if (ret <= 0) {
    if (!(flags & CONF_MFLAGS_SILENT))
      ERR_raise_data(ERR_LIB_CONF, CONF_R_MODULE_INITIALIZATION_ERROR,
                     "module=%s, value=%s retcode=%-8d", name, value, ret);
  }
...
void CONF_modules_finish(void) { conf_modules_finish_int(); }
