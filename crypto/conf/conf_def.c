...
CONF_METHOD *NCONF_WIN32(void) { return &WIN32_method; }
#endif
  ...
  for (;;) {
    if (IS_QUOTE(conf, *from)) {
      ...
      break;
             ...
             (!conf->flag_dollarid || from[1] == '{' || from[1] == '(')) {
      size_t newsize;
...
static BIO *process_include(char *include, OPENSSL_DIR_CTX **dirctx,
  ...
  if (S_ISDIR(st.st_mode)) {
    if (*dirctx != NULL) {
      ERR_raise_data(ERR_LIB_CONF, CONF_R_RECURSIVE_DIRECTORY_INCLUDE, "%s",
                     include);
      return NULL;
    }
...
static int def_to_int(const CONF *conf, char c) { return c - '0'; }
