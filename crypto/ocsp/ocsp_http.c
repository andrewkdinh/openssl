...
OSSL_HTTP_REQ_CTX *OCSP_sendreq_new(BIO *io, const char *path,
  ...
  if (!OSSL_HTTP_REQ_CTX_set_expected(rctx, NULL /* content_type */,
                                      ...
                                      0 /* keep_alive */))
    goto err;
  if (req != NULL && !OSSL_HTTP_REQ_CTX_set1_req(
                     rctx, "application/ocsp-request",
                     ASN1_ITEM_rptr(OCSP_REQUEST), (const ASN1_VALUE *)req))
    goto err;
  return rctx;
...
OCSP_RESPONSE *OCSP_sendreq_bio(BIO *b, const char *path, OCSP_REQUEST *req) {
  ...
  resp =
  (OCSP_RESPONSE *)ASN1_item_d2i_bio(ASN1_ITEM_rptr(OCSP_RESPONSE), mem, NULL);
