...
static int ocsp_req_find_signer(X509 **psigner, OCSP_REQUEST *req, const X509_NAME *nm, STACK_OF(X509) *certs,
                                unsigned long flags);
                              ...
                              STACK_OF(X509) *untrusted, STACK_OF(X509) **chain)
{
    ...
    if (ret <= 0) {
        ...
        ERR_raise_data(ERR_LIB_OCSP, OCSP_R_CERTIFICATE_VERIFY_ERROR, "Verify error: %s",
