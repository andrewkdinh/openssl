 ...
 *    provider store lock, the provider activatecnt_lock and the provider
 * flag_lock.
 *  - When holding multiple locks they must be acquired in the following order
 * of precedence: 1) provider store lock 2) provider flag_lock 3) provider
 * activatecnt_lock
...
int ossl_provider_info_add_to_store(OSSL_LIB_CTX *libctx,
    ...
    if (store->provinfosz == 0) {
        store->provinfo =
        OPENSSL_zalloc(sizeof(*store->provinfo) * BUILTINS_BLOCK_SIZE);
        ...
        tmpbuiltins =
        OPENSSL_realloc(store->provinfo, sizeof(*store->provinfo) * newsz);
        if (tmpbuiltins == NULL)
            goto err;
...
/* Setters */
int ossl_provider_set_module_path(OSSL_PROVIDER *prov,
...
int ossl_provider_doall_activated(OSSL_LIB_CTX *ctx,
    ...
    for (curr = max - 1; curr >= 0; curr--) {
        ...
        if (prov->flag_activated) {
             ...
             * whilst still activated in the child for a short period. That's
             * ok.
             */
...
const char *ossl_provider_name(const OSSL_PROVIDER *prov) { return prov->name; }

const DSO *ossl_provider_dso(const OSSL_PROVIDER *prov) { return prov->module; }
 ...
 *
 * @param prov Pointer to the OSSL_PROVIDER structure representing the provider.
 * @param params Array of OSSL_PARAM structures to store the retrieved
 * parameters.
 *
 ...
 * @param prov Pointer to the OSSL_PROVIDER structure representing the provider.
 *
 * @return 1 if the self-test passes or the callback is not defined, 0 on
 * failure.
 *
 ...
 * @param no_cache Pointer to an integer flag to indicate whether caching is
 * allowed.
 *
...
int ossl_provider_is_child(const OSSL_PROVIDER *prov) { return prov->ischild; }
...
#ifndef FIPS_MODULE
OSSL_PARAM_DEFN(OSSL_PROV_PARAM_CORE_MODULE_FILENAME, OSSL_PARAM_UTF8_PTR, NULL,
                0),
#endif
OSSL_PARAM_END};
...
static int core_get_params(const OSSL_CORE_HANDLE *handle,
                           OSSL_PARAM params[]) {
    OSSL_PARAM *p;
    /*
...
#ifndef FIPS_MODULE
    if ((p = OSSL_PARAM_locate(params, OSSL_PROV_PARAM_CORE_MODULE_FILENAME)) !=
        NULL)
 ...
 * ERR subsystem.
 */
static void core_new_error(const OSSL_CORE_HANDLE *handle) { ERR_new(); }
...
static size_t rand_get_entropy(const OSSL_CORE_HANDLE *handle,
    ...
    return ossl_rand_get_entropy((OSSL_LIB_CTX *)core_get_libctx(handle), pout,
...
static void rand_cleanup_user_entropy(const OSSL_CORE_HANDLE *handle,
                                      unsigned char *buf, size_t len) {
    ossl_rand_cleanup_user_entropy((OSSL_LIB_CTX *)core_get_libctx(handle), buf,
                                   len);
}
...
static size_t rand_get_nonce(const OSSL_CORE_HANDLE *handle,
                             ...
                             size_t salt_len) {
    return ossl_rand_get_nonce((OSSL_LIB_CTX *)core_get_libctx(handle), pout,
...
static void rand_cleanup_user_nonce(const OSSL_CORE_HANDLE *handle,
                                    unsigned char *buf, size_t len) {
    ossl_rand_cleanup_user_nonce((OSSL_LIB_CTX *)core_get_libctx(handle), buf,
                                 len);
}
