...
static ossl_inline void err_set_error(ERR_STATE *es, size_t i, int lib,
                                      int reason) {
...
static ossl_inline void err_set_debug(ERR_STATE *es, size_t i, const char *file,
  ...
  if (file == NULL || file[0] == '\0')
    es->err_file[i] = NULL;
  else if ((es->err_file[i] = CRYPTO_malloc(strlen(file) + 1, NULL, 0)) != NULL)
  ...
  if (fn == NULL || fn[0] == '\0')
    es->err_func[i] = NULL;
  else if ((es->err_func[i] = CRYPTO_malloc(strlen(fn) + 1, NULL, 0)) != NULL)
    strcpy(es->err_func[i], fn);
}

static ossl_inline void err_set_data(ERR_STATE *es, size_t i, void *data,
                                     size_t datasz, int flags) {
