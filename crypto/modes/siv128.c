...
__owur static ossl_inline uint64_t siv128_getword(SIV_BLOCK const *b,
                                                  size_t i) {
  DECLARE_IS_ENDIAN;
...
int ossl_siv128_init(SIV128_CONTEXT *ctx, const unsigned char *key, int klen,
  ...
  params[0] =
  OSSL_PARAM_construct_utf8_string(OSSL_MAC_PARAM_CIPHER, (char *)cbc_name, 0);
  params[1] =
  OSSL_PARAM_construct_octet_string(OSSL_MAC_PARAM_KEY, (void *)key, klen);
 ...
 */
int ossl_siv128_finish(SIV128_CONTEXT *ctx) { return ctx->final_ret; }
