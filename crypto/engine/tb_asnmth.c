  ...
  if (e->pkey_asn1_meths) {
    ...
    if (num_nids > 0)
      return engine_table_register(&pkey_asn1_meth_table,
                                   engine_unregister_all_pkey_asn1_meths, e,
                                   nids, num_nids, 0);
  }
  return 1;
}
...
ENGINE *ENGINE_get_pkey_asn1_meth_engine(int nid) {
  return ossl_engine_table_select(&pkey_asn1_meth_table, nid, OPENSSL_FILE,
                                  OPENSSL_LINE);
}
...
ENGINE_get_pkey_asn1_meth_str(ENGINE *e, const char *str, int len) {
  ...
  for (i = 0; i < nidcount; i++) {
    ...
    if (ameth != NULL && ((int)strlen(ameth->pem_str) == len) &&
...
static void look_str_cb(int nid, STACK_OF(ENGINE) * sk, ENGINE *def,
                        void *arg) {
