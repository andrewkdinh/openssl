...
{DYNAMIC_CMD_SO_PATH, "SO_PATH",
 "Specifies the path to the new ENGINE shared library", ENGINE_CMD_FLAG_STRING},
...
{DYNAMIC_CMD_DIR_ADD, "DIR_ADD",
 "Adds a directory from which ENGINEs can be loaded", ENGINE_CMD_FLAG_STRING},
...
static int dynamic_set_data_ctx(ENGINE *e, dynamic_data_ctx **ctx) {
  ...
  if (!CRYPTO_THREAD_write_lock(global_engine_lock))
    goto end;
  if ((*ctx = (dynamic_data_ctx *)ENGINE_get_ex_data(e, dynamic_ex_data_idx)) ==
      NULL) {
...
static dynamic_data_ctx *dynamic_get_data_ctx(ENGINE *e) {
  ...
  if (dynamic_ex_data_idx < 0) {
     ...
     */
    int new_idx =
    ENGINE_get_ex_new_index(0, NULL, NULL, NULL, dynamic_data_ctx_free_func);
    if (new_idx == -1) {
...
static int dynamic_load(ENGINE *e, dynamic_data_ctx *ctx) {
  ...
  if (!ctx->no_vcheck) {
     ...
     */
    ctx->v_check =
    (dynamic_v_check_fn)DSO_bind_func(ctx->dynamic_dso, ctx->DYNAMIC_F1);
    if (ctx->v_check)
