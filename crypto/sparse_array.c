...
static void sa_free_node(void **p) { OPENSSL_free(p); }
...
size_t ossl_sa_num(const OPENSSL_SA *sa) { return sa == NULL ? 0 : sa->nelem; }
...
void *ossl_sa_get(const OPENSSL_SA *sa, ossl_uintmax_t n) {
    ...
    if (n <= sa->top) {
        ...
        for (level = sa->levels - 1; p != NULL && level > 0; level--)
            p =
            (void **)p[(n >> (OPENSSL_SA_BLOCK_BITS * level)) & SA_BLOCK_MASK];
