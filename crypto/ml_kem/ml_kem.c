...
static const ML_KEM_VINFO vinfo_map[3] = {
    ...
    {"ML-KEM-1024", PRVKEY_BYTES(1024), sizeof(struct prvkey_1024_alloc),
     ...
     ML_KEM_1024_SECBITS, ML_KEM_1024_SECURITY_CATEGORY}};
...
static void scalar_add(scalar *lhs, const scalar *rhs)
{
    ...
    for (i = 0; i < DEGREE; i++) lhs->c[i] = reduce_once(lhs->c[i] + rhs->c[i]);
}
...
static void scalar_encode_1(uint8_t out[DEGREE / 8], const scalar *s)
{
    ...
    for (i = 0; i < DEGREE; i += 8) {
        out_byte = 0;
        for (j = 0; j < 8; j++) out_byte |= bit0(s->c[i + j]) << j;
        *out = out_byte;
        out++;
    }
}
...
static void scalar_compress(scalar *s, int bits)
{
    ...
    for (i = 0; i < DEGREE; i++) s->c[i] = compress(s->c[i], bits);
}
...
static void scalar_decompress(scalar *s, int bits)
{
    ...
    for (i = 0; i < DEGREE; i++) s->c[i] = decompress(s->c[i], bits);
}
...
static void vector_encode(uint8_t *out, const scalar *a, int bits, int rank)
{
    ...
    for (; rank-- > 0; out += stride) scalar_encode(out, a++, bits);
}
                          ...
                          int rank)
{
    ...
    while (--rank > 0) scalar_mult_add(out, ++lhs, ++rhs);
}
                             ...
                             int rank)
{
    ...
    for (i = rank; i-- > 0; ++out) {
        ...
        for (j = rank - 1; j > 0; --j) scalar_mult_add(out, m++, ++ar);
                                      ...
                                      const scalar *a, int rank)
{
    ...
    for (i = rank; i-- > 0; ++out) {
        ...
        for (j = rank; --j > 0;) scalar_mult_add(out, (mr += rank), ++ar);
    }
}
...
static int parse_pubkey(const uint8_t *in, EVP_MD_CTX *mdctx, ML_KEM_KEY *key)
{
    ...
    if (!vector_decode_12(key->t, in, vinfo->rank)) {
        ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_KEY,
                       "%s invalid public 't' vector", vinfo->algorithm_name);
        return 0;
    }
...
static int parse_prvkey(const uint8_t *in, EVP_MD_CTX *mdctx, ML_KEM_KEY *key)
{
    ...
    if (!vector_decode_12(key->s, in, vinfo->rank)) {
        ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_KEY,
                       "%s invalid private 's' vector", vinfo->algorithm_name);
        return 0;
    }
    ...
    if (memcmp(key->pkhash, in, ML_KEM_PKHASH_BYTES) != 0) {
        ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_KEY,
                       "%s public key hash mismatch", vinfo->algorithm_name);
        return 0;
    }
