...
{"ML-KEM-512", PRVKEY_BYTES(512), sizeof(struct prvkey_512_alloc),
 ...
 VECTOR_BYTES(512), U_VECTOR_BYTES(512), EVP_PKEY_ML_KEM_512, ML_KEM_512_BITS,
 ML_KEM_512_RANK, ML_KEM_512_DU, ML_KEM_512_DV, ML_KEM_512_SECBITS},
{"ML-KEM-768", PRVKEY_BYTES(768), sizeof(struct prvkey_768_alloc),
 ...
 VECTOR_BYTES(768), U_VECTOR_BYTES(768), EVP_PKEY_ML_KEM_768, ML_KEM_768_BITS,
 ML_KEM_768_RANK, ML_KEM_768_DU, ML_KEM_768_DV, ML_KEM_768_SECBITS},
{"ML-KEM-1024", PRVKEY_BYTES(1024), sizeof(struct prvkey_1024_alloc),
 PUBKEY_BYTES(1024), sizeof(struct pubkey_1024_alloc), CTEXT_BYTES(1024),
 VECTOR_BYTES(1024), U_VECTOR_BYTES(1024), EVP_PKEY_ML_KEM_1024,
 ML_KEM_1024_BITS, ML_KEM_1024_RANK, ML_KEM_1024_DU, ML_KEM_1024_DV,
 ML_KEM_1024_SECBITS}};
...
1,    1175, 2444, 394,  1219, 2300, 1455, 2117, 1607, 2443, 554,  1179, 2186,
2303, 2926, 2237, 525,  735,  863,  2768, 1230, 2572, 556,  3010, 2266, 1684,
1239, 780,  2954, 109,  1292, 1031, 1745, 2688, 3061, 992,  2596, 941,  892,
1021, 2390, 642,  1868, 2377, 1482, 1540, 540,  1678, 1626, 279,  314,  1173,
2573, 3096, 48,   667,  1920, 2229, 1041, 2606, 1692, 680,  2746, 568,  3312,
2419, 2102, 219,  855,  2681, 1848, 712,  682,  927,  1795, 461,  1891, 2877,
2522, 1894, 1010, 1414, 2009, 3296, 464,  2697, 816,  1352, 2679, 1274, 1052,
1025, 2132, 1573, 76,   2998, 3040, 2508, 1355, 450,  936,  447,  2794, 1235,
1903, 1996, 1089, 3273, 283,  1853, 1990, 882,  3033, 1583, 2760, 69,   543,
2532, 3136, 1410, 2267, 2481, 1432, 2699, 687,  40,   749,  1600,
};
...
static int decap(uint8_t secret[ML_KEM_SHARED_SECRET_BYTES],
  ...
  mask = constant_time_eq_int_8(
  0, CRYPTO_memcmp(ctext, tmp_ctext, vinfo->ctext_bytes));
...
void ossl_ml_kem_key_reset(ML_KEM_KEY *key) {
  ...
  if (ossl_ml_kem_have_prvkey(key))
    OPENSSL_cleanse(key->s, key->vinfo->rank * sizeof(scalar) +
                            2 * ML_KEM_RANDOM_BYTES);
  OPENSSL_free(key->t);
...
int ossl_ml_kem_encap_seed(uint8_t *ctext, size_t clen, uint8_t *shared_secret,
  ...
  if (ctext == NULL || clen != vinfo->ctext_bytes || shared_secret == NULL ||
      slen != ML_KEM_SHARED_SECRET_BYTES || entropy == NULL ||
...
int ossl_ml_kem_encap_rand(uint8_t *ctext, size_t clen, uint8_t *shared_secret,
  ...
  if (RAND_bytes_ex(key->libctx, r, ML_KEM_RANDOM_BYTES, key->vinfo->secbits) <
      1)
    return 0;

  return ossl_ml_kem_encap_seed(ctext, clen, shared_secret, slen, r, sizeof(r),
                                key);
}
