...
static const ML_KEM_VINFO vinfo_map[3] = {
    {"ML-KEM-512", PRVKEY_BYTES(512), sizeof(struct prvkey_512_alloc), PUBKEY_BYTES(512),
     sizeof(struct pubkey_512_alloc), CTEXT_BYTES(512), VECTOR_BYTES(512), U_VECTOR_BYTES(512), EVP_PKEY_ML_KEM_512,
     ML_KEM_512_BITS, ML_KEM_512_RANK, ML_KEM_512_DU, ML_KEM_512_DV, ML_KEM_512_SECBITS, ML_KEM_512_SECURITY_CATEGORY},
    {"ML-KEM-768", PRVKEY_BYTES(768), sizeof(struct prvkey_768_alloc), PUBKEY_BYTES(768),
     sizeof(struct pubkey_768_alloc), CTEXT_BYTES(768), VECTOR_BYTES(768), U_VECTOR_BYTES(768), EVP_PKEY_ML_KEM_768,
     ML_KEM_768_BITS, ML_KEM_768_RANK, ML_KEM_768_DU, ML_KEM_768_DV, ML_KEM_768_SECBITS, ML_KEM_768_SECURITY_CATEGORY},
    {"ML-KEM-1024", PRVKEY_BYTES(1024), sizeof(struct prvkey_1024_alloc), PUBKEY_BYTES(1024),
     sizeof(struct pubkey_1024_alloc), CTEXT_BYTES(1024), VECTOR_BYTES(1024), U_VECTOR_BYTES(1024),
     EVP_PKEY_ML_KEM_1024, ML_KEM_1024_BITS, ML_KEM_1024_RANK, ML_KEM_1024_DU, ML_KEM_1024_DV, ML_KEM_1024_SECBITS,
...
static const uint16_t kInverseNTTRoots[128] = {
    1,    1175, 2444, 394,  1219, 2300, 1455, 2117, 1607, 2443, 554,  1179, 2186, 2303, 2926, 2237, 525,  735,  863,
    2768, 1230, 2572, 556,  3010, 2266, 1684, 1239, 780,  2954, 109,  1292, 1031, 1745, 2688, 3061, 992,  2596, 941,
    892,  1021, 2390, 642,  1868, 2377, 1482, 1540, 540,  1678, 1626, 279,  314,  1173, 2573, 3096, 48,   667,  1920,
    2229, 1041, 2606, 1692, 680,  2746, 568,  3312, 2419, 2102, 219,  855,  2681, 1848, 712,  682,  927,  1795, 461,
    1891, 2877, 2522, 1894, 1010, 1414, 2009, 3296, 464,  2697, 816,  1352, 2679, 1274, 1052, 1025, 2132, 1573, 76,
    2998, 3040, 2508, 1355, 450,  936,  447,  2794, 1235, 1903, 1996, 1089, 3273, 283,  1853, 1990, 882,  3033, 1583,
    2760, 69,   543,  2532, 3136, 1410, 2267, 2481, 1432, 2699, 687,  40,   749,  1600,
};
 ...
 */
static __owur int single_keccak(uint8_t *out, size_t outlen, const uint8_t *in, size_t inlen, EVP_MD_CTX *mdctx)
{
 ...
 */
static __owur int prf(uint8_t *out, size_t len, const uint8_t in[ML_KEM_RANDOM_BYTES + 1], EVP_MD_CTX *mdctx,
                      const ML_KEM_KEY *key)
{
 ...
 */
static __owur int hash_h(uint8_t out[ML_KEM_PKHASH_BYTES], const uint8_t *in, size_t len, EVP_MD_CTX *mdctx,
                         const ML_KEM_KEY *key)
{
    return EVP_DigestInit_ex(mdctx, key->sha3_256_md, NULL) && single_keccak(out, ML_KEM_PKHASH_BYTES, in, len, mdctx);
}

/* Incremental hash_h of expanded public key */
static int hash_h_pubkey(uint8_t pkhash[ML_KEM_PKHASH_BYTES], EVP_MD_CTX *mdctx, ML_KEM_KEY *key)
{
 ...
 */
static __owur int hash_g(uint8_t out[ML_KEM_SEED_BYTES], const uint8_t *in, size_t len, EVP_MD_CTX *mdctx,
                         const ML_KEM_KEY *key)
{
    return EVP_DigestInit_ex(mdctx, key->sha3_512_md, NULL) && single_keccak(out, ML_KEM_SEED_BYTES, in, len, mdctx);
}
 ...
 * side-channel leaks).
 */
static __owur int kdf(uint8_t out[ML_KEM_SHARED_SECRET_BYTES], const uint8_t z[ML_KEM_RANDOM_BYTES],
                      const uint8_t *ctext, size_t len, EVP_MD_CTX *mdctx, const ML_KEM_KEY *key)
{
    return EVP_DigestInit_ex(mdctx, key->shake256_md, NULL) && EVP_DigestUpdate(mdctx, z, ML_KEM_RANDOM_BYTES)
        && EVP_DigestUpdate(mdctx, ctext, len) && EVP_DigestFinalXOF(mdctx, out, ML_KEM_SHARED_SECRET_BYTES);
}
...
static ossl_inline void scalar_mult_add(scalar *out, const scalar *lhs, const scalar *rhs)
{
    uint16_t *curr = out->c, *end = curr + DEGREE;
    const uint16_t *lc = lhs->c, *rc = rhs->c;
    const uint16_t *roots = kModRoots;
 ...
 */
static void scalar_decode_decompress_add(scalar *out, const uint8_t in[DEGREE / 8])
{
 ...
 */
static void vector_decode_decompress_ntt(scalar *out, const uint8_t *in, int bits, int rank)
{
    int stride = bits * DEGREE / 8;
...
static void inner_product(scalar *out, const scalar *lhs, const scalar *rhs, int rank)
{
 ...
 */
static void matrix_mult_intt(scalar *out, const scalar *m, const scalar *a, int rank)
{
    const scalar *ar;
    int i, j;
...
static void matrix_mult_transpose_add(scalar *out, const scalar *m, const scalar *a, int rank)
{
...
static __owur int matrix_expand(EVP_MD_CTX *mdctx, ML_KEM_KEY *key)
{
    ...
    for (i = 0; i < rank; i++) {
        for (j = 0; j < rank; j++) {
            ...
            if (!EVP_DigestInit_ex(mdctx, key->shake128_md, NULL) || !EVP_DigestUpdate(mdctx, input, sizeof(input))
 ...
 */
static __owur int cbd_2(scalar *out, uint8_t in[ML_KEM_RANDOM_BYTES + 1], EVP_MD_CTX *mdctx, const ML_KEM_KEY *key)
{
    uint16_t *curr = out->c, *end = curr + DEGREE;
 ...
 */
static __owur int cbd_3(scalar *out, uint8_t in[ML_KEM_RANDOM_BYTES + 1], EVP_MD_CTX *mdctx, const ML_KEM_KEY *key)
{
    uint16_t *curr = out->c, *end = curr + DEGREE;
 ...
 */
static __owur int gencbd_vector(scalar *out, CBD_FUNC cbd, uint8_t *counter, const uint8_t seed[ML_KEM_RANDOM_BYTES],
 ...
 */
static __owur int encrypt_cpa(uint8_t out[ML_KEM_SHARED_SECRET_BYTES], const uint8_t message[DEGREE / 8],
 ...
 */
static void decrypt_cpa(uint8_t out[ML_KEM_SHARED_SECRET_BYTES], const uint8_t *ctext, scalar *u, const ML_KEM_KEY *key)
{
    const ML_KEM_VINFO *vinfo = key->vinfo;
    scalar v, mask;
    int rank = vinfo->rank;
...
static int parse_pubkey(const uint8_t *in, EVP_MD_CTX *mdctx, ML_KEM_KEY *key)
{
    ...
    if (!vector_decode_12(key->t, in, vinfo->rank)) {
        ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_KEY, "%s invalid public 't' vector", vinfo->algorithm_name);
        return 0;
    }
     ...
     */
    if (!hash_h(key->pkhash, in, vinfo->pubkey_bytes, mdctx, key) || !matrix_expand(mdctx, key)) {
        ERR_raise_data(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR, "internal error while parsing %s public key",
...
static int parse_prvkey(const uint8_t *in, EVP_MD_CTX *mdctx, ML_KEM_KEY *key)
{
    ...
    if (!vector_decode_12(key->s, in, vinfo->rank)) {
        ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_KEY, "%s invalid private 's' vector", vinfo->algorithm_name);
        return 0;
    }
    ...
    if (memcmp(key->pkhash, in, ML_KEM_PKHASH_BYTES) != 0) {
        ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_KEY, "%s public key hash mismatch", vinfo->algorithm_name);
        return 0;
    }
 ...
 */
static __owur int genkey(const uint8_t seed[ML_KEM_SEED_BYTES], EVP_MD_CTX *mdctx, uint8_t *pubenc, ML_KEM_KEY *key)
{
    ...
    if (!matrix_expand(mdctx, key) || !gencbd_vector_ntt(key->s, cbd_1, &counter, sigma, rank, mdctx, key)
    ...
    if (ret == 0) {
        ERR_raise_data(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR, "internal error while generating %s private key",
                       vinfo->algorithm_name);
    }
    return ret;
}
 ...
 * of the provided key.
 */
static int encap(uint8_t *ctext, uint8_t secret[ML_KEM_SHARED_SECRET_BYTES], const uint8_t entropy[ML_KEM_RANDOM_BYTES],
                 scalar *tmp, EVP_MD_CTX *mdctx, const ML_KEM_KEY *key)
{
    ...
    ret = hash_g(Kr, input, sizeof(input), mdctx, key) && encrypt_cpa(ctext, entropy, r, tmp, mdctx, key);
    OPENSSL_cleanse((void *)input, sizeof(input));

    if (ret)
        memcpy(secret, Kr, ML_KEM_SHARED_SECRET_BYTES);
    else
        ERR_raise_data(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR, "internal error while performing %s encapsulation",
                 ...
                 EVP_MD_CTX *mdctx, const ML_KEM_KEY *key)
{
    ...
    if (!kdf(failure_key, key->z, ctext, vinfo->ctext_bytes, mdctx, key)) {
        ERR_raise_data(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR, "internal error while performing %s decapsulation",
                       vinfo->algorithm_name);
        return 0;
    }
...
ML_KEM_KEY *ossl_ml_kem_key_new(OSSL_LIB_CTX *libctx, const char *properties, int evp_type)
{
    const ML_KEM_VINFO *vinfo = ossl_ml_kem_get_vinfo(evp_type);
    ML_KEM_KEY *key;

    if (vinfo == NULL) {
        ERR_raise_data(ERR_LIB_CRYPTO, ERR_R_PASSED_INVALID_ARGUMENT, "unsupported ML-KEM key type: %d", evp_type);
        return NULL;
    }
    ...
    if (key->shake128_md != NULL && key->shake256_md != NULL && key->sha3_256_md != NULL && key->sha3_512_md != NULL)
        return key;

    ossl_ml_kem_key_free(key);
    ERR_raise_data(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR, "missing SHA3 digest algorithms while creating %s key",
                   vinfo->algorithm_name);
    return NULL;
}
...
ML_KEM_KEY *ossl_ml_kem_key_dup(const ML_KEM_KEY *key, int selection)
{
    ...
    if (key == NULL || (ret = OPENSSL_memdup(key, sizeof(*key))) == NULL)
        return NULL;
...
int ossl_ml_kem_encode_private_key(uint8_t *out, size_t len, const ML_KEM_KEY *key)
{
...
{
    if (key == NULL || ossl_ml_kem_have_pubkey(key) || ossl_ml_kem_have_seed(key) || seedlen != ML_KEM_SEED_BYTES)
        return NULL;
    /*
...
int ossl_ml_kem_parse_private_key(const uint8_t *in, size_t len, ML_KEM_KEY *key)
{
    EVP_MD_CTX *mdctx = NULL;
    const ML_KEM_VINFO *vinfo;
    int ret = 0;
 ...
 * This is the deterministic version with randomness supplied externally.
 */
int ossl_ml_kem_encap_seed(uint8_t *ctext, size_t clen, uint8_t *shared_secret, size_t slen, const uint8_t *entropy,
                           size_t elen, const ML_KEM_KEY *key)
{
    const ML_KEM_VINFO *vinfo;
    EVP_MD_CTX *mdctx;
    int ret = 0;
    ...
    if (ctext == NULL || clen != vinfo->ctext_bytes || shared_secret == NULL || slen != ML_KEM_SHARED_SECRET_BYTES
        || entropy == NULL || elen != ML_KEM_RANDOM_BYTES || (mdctx = EVP_MD_CTX_new()) == NULL)
        return 0;
    /*
...
int ossl_ml_kem_encap_rand(uint8_t *ctext, size_t clen, uint8_t *shared_secret, size_t slen, const ML_KEM_KEY *key)
{
...
int ossl_ml_kem_decap(uint8_t *shared_secret, size_t slen, const uint8_t *ctext, size_t clen, const ML_KEM_KEY *key)
{
    const ML_KEM_VINFO *vinfo;
    EVP_MD_CTX *mdctx;
    int ret = 0;
    ...
    if (shared_secret == NULL || slen != ML_KEM_SHARED_SECRET_BYTES || ctext == NULL || clen != vinfo->ctext_bytes
        || (mdctx = EVP_MD_CTX_new()) == NULL) {
        (void)RAND_bytes_ex(key->libctx, shared_secret, ML_KEM_SHARED_SECRET_BYTES, vinfo->secbits);
        return 0;
    }
#if defined(OPENSSL_CONSTANT_TIME_VALIDATION)
    /*
