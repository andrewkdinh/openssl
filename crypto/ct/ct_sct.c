...
void SCT_LIST_free(STACK_OF(SCT) * a) { sk_SCT_pop_free(a, SCT_free); }
...
sct_version_t SCT_get_version(const SCT *sct) { return sct->version; }
...
uint64_t SCT_get_timestamp(const SCT *sct) { return sct->timestamp; }
...
sct_source_t SCT_get_source(const SCT *sct) { return sct->source; }
...
int SCT_validate(SCT *sct, const CT_POLICY_EVAL_CTX *ctx) {
  ...
  log =
  CTLOG_STORE_get0_log_by_id(ctx->log_store, sct->log_id, sct->log_id_len);
