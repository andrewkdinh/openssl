...
int OSSL_CRMF_MSGS_verify_popo(const OSSL_CRMF_MSGS *reqs, int rid,
  ...
  case OSSL_CRMF_POPO_SIGNATURE:
    ...
    if (ASN1_item_verify_ex(it, sig->algorithmIdentifier, sig->signature, asn,
...
OSSL_LIB_CTX *libctx, const char *propq) {
  ...
  if ((bio = CMS_EnvelopedData_decrypt(encryptedKey->value.envelopedData, NULL,
                                       pkey, cert, secret, 0, libctx, propq)) ==
      NULL) {
  ...
  pkey_bio = CMS_SignedData_verify(sd, NULL, NULL /* scerts */, ts, extra, NULL,
                                   0, libctx, propq);
