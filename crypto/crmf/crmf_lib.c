...
#define IMPLEMENT_CRMF_CTRL_FUNC(atyp, valt, ctrlinf)                          \
    valt *OSSL_CRMF_MSG_get0_##ctrlinf##_##atyp(const OSSL_CRMF_MSG *msg) {    \
        ...
        for (i = 0; i < sk_OSSL_CRMF_ATTRIBUTETYPEANDVALUE_num(controls);      \
             i++) {                                                            \
...
int OSSL_CRMF_MSG_create_popo(int meth, OSSL_CRMF_MSG *crm, EVP_PKEY *pkey,
    ...
    case OSSL_CRMF_POPO_SIGNATURE: {
        ...
        if (ps == NULL)
            goto err;
        if (!create_popo_signature(ps, crm->certReq, pkey, digest, libctx,
                                   propq)) {
...
int OSSL_CRMF_MSG_centralkeygen_requested(const OSSL_CRMF_MSG *crm,
    ...
    if (crm != NULL)
        pubkey =
        OSSL_CRMF_CERTTEMPLATE_get0_publicKey(OSSL_CRMF_MSG_get0_tmpl(crm));
    else
        pubkey = p10cr->req_info.pubkey;

    if (pubkey == NULL ||
        (X509_PUBKEY_get0_param(NULL, &pk, &pklen, NULL, pubkey) && pklen == 0))
        ret = 1;
...
OSSL_LIB_CTX *libctx, const char *propq) {
    ...
    if ((bio = CMS_EnvelopedData_decrypt(encryptedKey->value.envelopedData,
                                         NULL, pkey, cert, secret, 0, libctx,
                                         propq)) == NULL) {
    ...
    *outlen = 0;
    ...
    if (pkctx != NULL && EVP_PKEY_decrypt_init(pkctx) > 0) {
        ...
        if (EVP_PKEY_decrypt(pkctx, NULL, &eksize, encKey->data,
            ...
            goto end;
        retval =
        EVP_PKEY_decrypt(pkctx, ek, &eksize, encKey->data, encKey->length);
...
#ifndef OPENSSL_NO_CMS
    bio =
    CMS_EnvelopedData_decrypt(ecert->value.envelopedData, NULL, pkey,
                              NULL /* cert */, NULL, flags, libctx, propq);
    if (bio == NULL)
        return NULL;
