...
DSA_SIG *ossl_dsa_do_sign_int(const unsigned char *dgst, int dlen, DSA *dsa, unsigned int nonce_type,
...
redo:
    if (!dsa_sign_setup(dsa, ctx, &kinv, &ret->r, dgst, dlen, nonce_type, digestname, libctx, propq))
        goto err;
                          ...
                          unsigned int nonce_type, const char *digestname, OSSL_LIB_CTX *libctx, const char *propq)
{
    ...
    if (BN_is_zero(dsa->params.p) || BN_is_zero(dsa->params.q) || BN_is_zero(dsa->params.g)
        || BN_is_negative(dsa->params.p) || BN_is_negative(dsa->params.q) || BN_is_negative(dsa->params.g)) {
    ...
    do {
        if (dgst != NULL) {
            if (nonce_type == 1) {
#ifndef FIPS_MODULE
                if (!ossl_gen_deterministic_nonce_rfc6979(k, dsa->params.q, dsa->priv_key, dgst, dlen, digestname,
                                                          libctx, propq))
#endif
                    goto err;
            } else {
                 ...
                 */
                if (!ossl_bn_gen_dsa_nonce_fixed_top(k, dsa->params.q, dsa->priv_key, dgst, dlen, ctx))
                    goto err;
            }
