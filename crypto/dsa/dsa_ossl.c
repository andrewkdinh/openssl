...
redo:
   ...
   * The size of q is tested in dsa_sign_setup() so there should not be an
   * infinite loop here.
   */
  do {
    if (!BN_priv_rand_ex(blind, BN_num_bits(dsa->params.q) - 1, BN_RAND_TOP_ANY,
...
static int dsa_sign_setup(DSA *dsa, BN_CTX *ctx_in, BIGNUM **kinvp, BIGNUM **rp,
  ...
  do {
    if (dgst != NULL) {
      if (nonce_type == 1) {
         ...
         */
        if (!ossl_bn_gen_dsa_nonce_fixed_top(k, dsa->params.q, dsa->priv_key,
                                             dgst, dlen, ctx))
          goto err;
      }
  ...
  if (dsa->flags & DSA_FLAG_CACHE_MONT_P) {
    if (!BN_MONT_CTX_set_locked(&dsa->method_mont_p, dsa->lock, dsa->params.p,
                                ctx))
      goto err;
  }
...
static int dsa_do_verify(const unsigned char *dgst, int dgst_len, DSA_SIG *sig,
  ...
  if (dsa->flags & DSA_FLAG_CACHE_MONT_P) {
    mont =
    BN_MONT_CTX_set_locked(&dsa->method_mont_p, dsa->lock, dsa->params.p, ctx);
    if (!mont)
      goto err;
  }
