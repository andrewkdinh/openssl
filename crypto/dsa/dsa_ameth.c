...
static int dsa_pub_encode(X509_PUBKEY *pk, const EVP_PKEY *pkey) {
  ...
  dsa = pkey->pkey.dsa;
  if (pkey->save_parameters && dsa->params.p != NULL && dsa->params.q != NULL &&
      dsa->params.g != NULL) {
    str = ASN1_STRING_new();
    if (str == NULL) {
...
static int dsa_priv_encode(PKCS8_PRIV_KEY_INFO *p8, const EVP_PKEY *pkey) {
  ...
  if (!PKCS8_pkey_set0(p8, OBJ_nid2obj(NID_dsa), 0, V_ASN1_SEQUENCE, params, dp,
                       dplen)) {
...
static int dsa_bits(const EVP_PKEY *pkey) { return DSA_bits(pkey->pkey.dsa); }
...
static int dsa_missing_parameters(const EVP_PKEY *pkey) {
  DSA *dsa;
  dsa = pkey->pkey.dsa;
  return dsa == NULL || dsa->params.p == NULL || dsa->params.q == NULL ||
         dsa->params.g == NULL;
}
