...
int ossl_dsa_generate_ffc_parameters(DSA *dsa, int type, int pbits, int qbits,
  ...
  if (type == DSA_PARAMGEN_TYPE_FIPS_186_2)
    ...
    dsa->libctx, &dsa->params, FFC_PARAM_TYPE_DSA, pbits, qbits, &res, cb);
  else
#endif
...
int DSA_generate_parameters_ex(DSA *dsa, int bits, const unsigned char *seed_in,
  ...
  if (dsa->meth->dsa_paramgen)
    return dsa->meth->dsa_paramgen(dsa, bits, seed_in, seed_len, counter_ret,
                                   h_ret, cb);
  if (seed_in != NULL &&
      !ossl_ffc_params_set_validate_params(&dsa->params, seed_in, seed_len, -1))
    return 0;
