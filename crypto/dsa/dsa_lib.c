...
void DSA_clear_flags(DSA *d, int flags) { d->flags &= ~flags; }
...
const DSA_METHOD *DSA_get_method(DSA *d) { return d->meth; }
...
DSA *DSA_new_method(ENGINE *engine) { return dsa_new_intern(engine, NULL); }

DSA *ossl_dsa_new(OSSL_LIB_CTX *libctx) { return dsa_new_intern(NULL, libctx); }

#ifndef FIPS_MODULE
DSA *DSA_new(void) { return dsa_new_intern(NULL, NULL); }
#endif
...
void ossl_dsa_set0_libctx(DSA *d, OSSL_LIB_CTX *libctx) { d->libctx = libctx; }
...
int DSA_set0_pqg(DSA *d, BIGNUM *p, BIGNUM *q, BIGNUM *g) {
  ...
  if ((d->params.p == NULL && p == NULL) ||
      (d->params.q == NULL && q == NULL) || (d->params.g == NULL && g == NULL))
    return 0;
...
const BIGNUM *DSA_get0_p(const DSA *d) { return d->params.p; }

const BIGNUM *DSA_get0_q(const DSA *d) { return d->params.q; }

const BIGNUM *DSA_get0_g(const DSA *d) { return d->params.g; }

const BIGNUM *DSA_get0_pub_key(const DSA *d) { return d->pub_key; }

const BIGNUM *DSA_get0_priv_key(const DSA *d) { return d->priv_key; }
...
FFC_PARAMS *ossl_dsa_get0_params(DSA *dsa) { return &dsa->params; }
