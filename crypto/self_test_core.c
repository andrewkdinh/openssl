...
void ossl_self_test_set_callback_free(void *stcb) { OPENSSL_free(stcb); }
...
void OSSL_SELF_TEST_free(OSSL_SELF_TEST *st) { OPENSSL_free(st); }
 ...
 * If the application callback decides to return 0 then the first byte of
 * 'bytes' is modified (corrupted). This is used to modify output signatures or
