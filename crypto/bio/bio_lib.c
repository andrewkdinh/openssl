...
BIO *BIO_new(const BIO_METHOD *method) { return BIO_new_ex(NULL, method); }
...
static int bio_read_intern(BIO *b, void *data, size_t dlen, size_t *readbytes) {
  ...
  if (HAS_CALLBACK(b) && ((ret = (int)bio_call_callback(
                           b, BIO_CB_READ, data, dlen, 0, 0L, 1L, NULL)) <= 0))
    return ret;
  ...
  if (HAS_CALLBACK(b))
    ret = (int)bio_call_callback(b, BIO_CB_READ | BIO_CB_RETURN, data, dlen, 0,
                                 0L, ret, readbytes);
...
static int bio_write_intern(BIO *b, const void *data, size_t dlen,
  ...
  if (HAS_CALLBACK(b) && ((ret = (int)bio_call_callback(
                           b, BIO_CB_WRITE, data, dlen, 0, 0L, 1L, NULL)) <= 0))
    return ret;
  ...
  if (HAS_CALLBACK(b))
    ret = (int)bio_call_callback(b, BIO_CB_WRITE | BIO_CB_RETURN, data, dlen, 0,
                                 0L, ret, &local_written);
...
int BIO_sendmmsg(BIO *b, BIO_MSG *msg, size_t stride, size_t num_msg,
  ...
  if (HAS_CALLBACK(b)) {
    ...
    ret = (size_t)bio_call_callback(b, BIO_CB_SENDMMSG, (void *)&args, 0, 0, 0,
                                    1, NULL);
    if (ret <= 0)
      return 0;
  }
...
int BIO_recvmmsg(BIO *b, BIO_MSG *msg, size_t stride, size_t num_msg,
  ...
  if (HAS_CALLBACK(b)) {
    ...
    ret =
    bio_call_callback(b, BIO_CB_RECVMMSG, (void *)&args, 0, 0, 0, 1, NULL);
    if (ret <= 0)
      return 0;
  }
...
int BIO_puts(BIO *b, const char *buf) {
  ...
  if (HAS_CALLBACK(b))
    ret = (int)bio_call_callback(b, BIO_CB_PUTS | BIO_CB_RETURN, buf, 0, 0, 0L,
                                 ret, &written);
...
int BIO_gets(BIO *b, char *buf, int size) {
  ...
  if (HAS_CALLBACK(b))
    ret = (int)bio_call_callback(b, BIO_CB_GETS | BIO_CB_RETURN, buf, size, 0,
                                 0L, ret, &readbytes);
...
long BIO_ctrl(BIO *b, int cmd, long larg, void *parg) {
  ...
  if (HAS_CALLBACK(b))
    ret = bio_call_callback(b, BIO_CB_CTRL | BIO_CB_RETURN, parg, 0, cmd, larg,
                            ret, NULL);
...
long BIO_callback_ctrl(BIO *b, int cmd, BIO_info_cb *fp) {
  ...
  if (HAS_CALLBACK(b))
    ret = bio_call_callback(b, BIO_CB_CTRL | BIO_CB_RETURN, (void *)&fp, 0, cmd,
                            0, ret, NULL);
...
void BIO_set_next(BIO *b, BIO *next) { b->next_bio = next; }
