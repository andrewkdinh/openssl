...
BIO_TYPE_CONNECT, "socket connect",   bwrite_conv,   conn_write,    bread_conv,
conn_read,        conn_puts,          conn_gets,     conn_ctrl,     conn_new,
conn_free,        conn_callback_ctrl, conn_sendmmsg, conn_recvmmsg,
};
    ...
    for (;;) {
        ...
        case BIO_CONN_S_BEFORE:
            if (c->param_hostname == NULL && c->param_service == NULL) {
                ...
                "hostname=%s service=%s", c->param_hostname, c->param_service);
                goto exit_loop;
            }
...
#ifndef OPENSSL_NO_KTLS
                /*
                 * The new socket is created successfully regardless of
                 * ktls_enable. ktls_enable doesn't change any functionality of
                 * the socket, except changing the setsockopt to enable the
                 * processing of ktls_start. Thus, it is not a problem to call
                 * it for non-TLS sockets.
                 */
                ktls_enable(b->num);
#endif
            }
    ...
    case BIO_C_SET_CONNECT:
        if (ptr != NULL) {
            ...
            if (num == 0) { /* BIO_set_conn_hostname */
                ...
                ret =
                BIO_parse_hostserv(ptr, &data->param_hostname,
                                   &data->param_service, BIO_PARSE_PRIO_HOST);
