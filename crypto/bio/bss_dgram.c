...
static void dgram_adjust_rcv_timeout(BIO *b) {
    ...
    if (!ossl_time_is_zero(data->next_timeout)) {
        ...
        if (getsockopt(b->num, SOL_SOCKET, SO_RCVTIMEO, (void *)&timeout, &sz) <
            0)
        ...
        if (ossl_time_is_zero(data->socket_timeout) ||
            ...
            if (setsockopt(b->num, SOL_SOCKET, SO_RCVTIMEO, &tv, sizeof(tv)) <
                0)
                ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(),
                               "calling setsockopt()");
#endif
...
#define BIO_MAX_MSGS_PER_CALL 64
    ...
    for (i = 0; i < num_msg; ++i) {
        ...
        if (BIO_MSG_N(msg, stride, i).local != NULL) {
            ...
            if (pack_local(b, &mh[i].msg_hdr, BIO_MSG_N(msg, stride, i).local) <
                1) {
                ERR_raise(ERR_LIB_BIO, BIO_R_LOCAL_ADDR_NOT_AVAILABLE);
                *num_processed = 0;
                return 0;
...
static long dgram_sctp_ctrl(BIO *b, int cmd, long num, void *ptr) {
    ...
    case BIO_CTRL_DGRAM_SCTP_ADD_AUTH_KEY:
        ...
        ret =
        setsockopt(b->num, IPPROTO_SCTP, SCTP_AUTH_KEY, authkey, sockopt_len);
...
static int dgram_sctp_wait_for_dry(BIO *b) {
    ...
    while (msg.msg_flags & MSG_NOTIFICATION) {
        ...
        if (snp.sn_header.sn_type == SCTP_SENDER_DRY_EVENT) {
            ...
            ret =
            getsockopt(b->num, IPPROTO_SCTP, SCTP_EVENTS, &event, &eventsize);
            if (ret < 0)
                return -1;
