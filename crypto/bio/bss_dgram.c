...
static int dgram_sendmmsg(BIO *b, BIO_MSG *msg, size_t stride, size_t num_msg, uint64_t flags, size_t *num_processed);
static int dgram_recvmmsg(BIO *b, BIO_MSG *msg, size_t stride, size_t num_msg, uint64_t flags, size_t *num_processed);
...
# endif
         ...
         * earlier.
         */
        if (ossl_time_is_zero(data->socket_timeout) || ossl_time_compare(data->socket_timeout, timeleft) >= 0) {
# ifdef OPENSSL_SYS_WINDOWS
...
# endif
    ...
    case BIO_CTRL_DGRAM_QUERY_MTU:
        ...
        switch (addr.sa.sa_family) {
        case AF_INET:
            if ((ret = getsockopt(b->num, IPPROTO_IP, IP_MTU, (void *)&sockopt_val, &sockopt_len)) < 0
                || sockopt_val < 0) {
                ret = 0;
            } else {
                /*
...
# if OPENSSL_USE_IPV6 && defined(IPV6_MTU)
        case AF_INET6:
            if ((ret = getsockopt(b->num, IPPROTO_IPV6, IPV6_MTU, (void *)&sockopt_val, &sockopt_len)) < 0
                || sockopt_val < 0) {
                ret = 0;
            } else {
                /*
...
# endif
    ...
    case BIO_CTRL_DGRAM_DETECT_PEER_ADDR: {
        ...
        if (BIO_ADDR_family(p) == AF_UNSPEC) {
            if (getpeername(b->num, (void *)&xaddr.sa, &xaddr_len) == 0 && BIO_ADDR_family(&xaddr) != AF_UNSPEC) {
                p = &xaddr;
            } else {
                ret = 0;
                break;
...
# ifdef OPENSSL_SYS_WINDOWS
    {
        ...
        if ((ret = setsockopt(b->num, SOL_SOCKET, SO_RCVTIMEO, (void *)&timeout, sizeof(timeout))) < 0)
            ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(), "calling setsockopt()");
    }
# else
        if ((ret = setsockopt(b->num, SOL_SOCKET, SO_RCVTIMEO, ptr, sizeof(struct timeval))) < 0)
            ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(), "calling setsockopt()");
# endif
    break;
...
# ifdef OPENSSL_SYS_WINDOWS
        ...
        sz = sizeof(timeout);
        if ((ret = getsockopt(b->num, SOL_SOCKET, SO_RCVTIMEO, (void *)&timeout, &sz)) < 0) {
            ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(), "calling getsockopt()");
        } else {
...
# else
        ...
        if ((ret = getsockopt(b->num, SOL_SOCKET, SO_RCVTIMEO, ptr, &sz)) < 0) {
            ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(), "calling getsockopt()");
        } else {
...
# ifdef OPENSSL_SYS_WINDOWS
    {
        ...
        if ((ret = setsockopt(b->num, SOL_SOCKET, SO_SNDTIMEO, (void *)&timeout, sizeof(timeout))) < 0)
            ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(), "calling setsockopt()");
    }
# else
        if ((ret = setsockopt(b->num, SOL_SOCKET, SO_SNDTIMEO, ptr, sizeof(struct timeval))) < 0)
            ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(), "calling setsockopt()");
# endif
    break;
...
# ifdef OPENSSL_SYS_WINDOWS
        ...
        sz = sizeof(timeout);
        if ((ret = getsockopt(b->num, SOL_SOCKET, SO_SNDTIMEO, (void *)&timeout, &sz)) < 0) {
            ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(), "calling getsockopt()");
        } else {
            tv->tv_sec = timeout / 1000;
            tv->tv_usec = (timeout % 1000) * 1000;
...
# else
        ...
        if ((ret = getsockopt(b->num, SOL_SOCKET, SO_SNDTIMEO, ptr, &sz)) < 0) {
            ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(), "calling getsockopt()");
        } else {
            OPENSSL_assert((size_t)sz <= sizeof(struct timeval));
            ret = (int)sz;
...
# endif
    ...
    case BIO_CTRL_DGRAM_SET_DONT_FRAG:
        ...
        case AF_INET:
            ...
            sockopt_val = num ? 1 : 0;
            if ((ret = setsockopt(b->num, IPPROTO_IP, IP_DONTFRAGMENT, (const char *)&sockopt_val, sizeof(sockopt_val)))
                < 0)
                ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(), "calling setsockopt()");
#else
            ret = -1;
...
# if defined(IPV6_DONTFRAG)
            sockopt_val = num ? 1 : 0;
            if ((ret = setsockopt(b->num, IPPROTO_IPV6, IPV6_DONTFRAG, (const void *)&sockopt_val, sizeof(sockopt_val)))
                < 0)
                ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(), "calling setsockopt()");
...
# endif
    ...
    case BIO_CTRL_DGRAM_GET_EFFECTIVE_CAPS:
        ret = (long)(BIO_DGRAM_CAP_HANDLES_DST_ADDR | BIO_DGRAM_CAP_HANDLES_SRC_ADDR | BIO_DGRAM_CAP_PROVIDES_DST_ADDR
...
static void translate_msg_win(BIO *b, WSAMSG *mh, WSABUF *iov, unsigned char *control, BIO_MSG *msg)
{
...
static void translate_msg(BIO *b, struct msghdr *mh, struct iovec *iov, unsigned char *control, BIO_MSG *msg)
{
    bio_dgram_data *data;
...
#  endif
         ...
         * ignore this.)
         */
        if (local->s_in.sin_port != 0 && data->local_addr.s_in.sin_port != local->s_in.sin_port) {
...
#   endif
         ...
         * in sockaddr_in6.
         */
        if (local->s_in6.sin6_port != 0 && data->local_addr.s_in6.sin6_port != local->s_in6.sin6_port) {
            ERR_raise(ERR_LIB_BIO, BIO_R_PORT_MISMATCH);
            return 0;
        }

        if (local->s_in6.sin6_scope_id != 0 && data->local_addr.s_in6.sin6_scope_id != local->s_in6.sin6_scope_id) {
            ERR_raise(ERR_LIB_BIO, BIO_R_PORT_MISMATCH);
            return 0;
        }
...
# define BIO_MAX_MSGS_PER_CALL   64
    ...
    for (i = 0; i < num_msg; ++i) {
        translate_msg(b, &mh[i].msg_hdr, &iov[i], control[i], &BIO_MSG_N(msg, stride, i));
        ...
        if (BIO_MSG_N(msg, stride, i).local != NULL) {
            ...
            if (pack_local(b, &mh[i].msg_hdr, BIO_MSG_N(msg, stride, i).local) < 1) {
                ERR_raise(ERR_LIB_BIO, BIO_R_LOCAL_ADDR_NOT_AVAILABLE);
                *num_processed = 0;
                return 0;
            }
...
# else
                 msg[0].data_len,
# endif
                 sysflags, msg[0].peer != NULL ? BIO_ADDR_sockaddr(msg[0].peer) : NULL,
...
static int dgram_recvmmsg(BIO *b, BIO_MSG *msg, size_t stride, size_t num_msg, uint64_t flags, size_t *num_processed)
{
    ...
    for (i = 0; i < (size_t)ret; ++i) {
         ...
         */
        if (BIO_MSG_N(msg, stride, i).local != NULL)
            if (extract_local(b, &mh[i].msg_hdr, BIO_MSG_N(msg, stride, i).local) < 1)
                /*
...
# else
                   msg[0].data_len,
# endif
                   sysflags, msg[0].peer != NULL ? &msg[0].peer->sa : NULL, msg[0].peer != NULL ? &slen : NULL);
    if (ret <= 0) {
        ERR_raise(ERR_LIB_SYS, get_last_socket_error());
        return 0;
    }
...
# endif
    ...
    for (p = (unsigned char *)authchunks->gauth_chunks; p < (unsigned char *)authchunks + sockopt_len;
         p += sizeof(uint8_t)) {
...
# endif
        ...
        if (!data->peer_auth_tested) {
            ...
            if (ii >= 0)
                for (p = (unsigned char *)authchunks->gauth_chunks; p < (unsigned char *)authchunks + optlen;
                     p += sizeof(uint8_t)) {
                    if (*p == OPENSSL_SCTP_DATA_CHUNK_TYPE)
                        auth_data = 1;
                    if (*p == OPENSSL_SCTP_FORWARD_CUM_TSN_CHUNK_TYPE)
