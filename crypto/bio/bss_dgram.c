...
static int dgram_read(BIO *b, char *out, int outl) {
  ...
  if (out != NULL) {
    ...
    if (data->peekmode)
      flags = MSG_PEEK;
    ret =
    recvfrom(b->num, out, outl, flags, BIO_ADDR_sockaddr_noconst(&peer), &len);
...
static int enable_local_addr(BIO *b, int enable) {
  ...
  if (af == AF_INET) {
    ...
    if (setsockopt(b->num, IPPROTO_IP, IP_PKTINFO, (void *)&enable,
                   sizeof(enable)) < 0)
      return 0;
    ...
    if (setsockopt(b->num, IPPROTO_IP, IP_RECVDSTADDR, &enable,
                   sizeof(enable)) < 0)
      return 0;
  ...
  if (af == AF_INET6) {
    ...
    if (setsockopt(b->num, IPPROTO_IPV6, IPV6_RECVPKTINFO, &enable,
                   sizeof(enable)) < 0)
      return 0;
...
defined(IP_PMTUDISC_DO)
    ...
    case AF_INET:
      ...
      if ((ret = setsockopt(b->num, IPPROTO_IP, IP_MTU_DISCOVER, &sockopt_val,
...
#ifdef OPENSSL_SYS_WINDOWS
  {
    ...
    if ((ret = setsockopt(b->num, SOL_SOCKET, SO_RCVTIMEO, (void *)&timeout,
                          sizeof(timeout))) < 0)
      ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(),
                     "calling setsockopt()");
  }
  ...
  case BIO_CTRL_DGRAM_GET_RECV_TIMEOUT: {
    ...
    sz = sizeof(timeout);
    if ((ret = getsockopt(b->num, SOL_SOCKET, SO_RCVTIMEO, (void *)&timeout,
                          &sz)) < 0) {
      ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(),
                     "calling getsockopt()");
    } else {
...
#ifdef OPENSSL_SYS_WINDOWS
  {
    ...
    if ((ret = setsockopt(b->num, SOL_SOCKET, SO_SNDTIMEO, (void *)&timeout,
                          sizeof(timeout))) < 0)
      ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(),
                     "calling setsockopt()");
  }
  ...
  case BIO_CTRL_DGRAM_GET_SEND_TIMEOUT: {
    ...
    sz = sizeof(timeout);
    if ((ret = getsockopt(b->num, SOL_SOCKET, SO_SNDTIMEO, (void *)&timeout,
                          &sz)) < 0) {
      ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(),
                     "calling getsockopt()");
    } else {
  ...
  case BIO_CTRL_DGRAM_SET_DONT_FRAG:
    ...
    case AF_INET:
      ...
      sockopt_val = num ? 1 : 0;
      if ((ret = setsockopt(b->num, IPPROTO_IP, IP_DONTFRAG, &sockopt_val,
                            sizeof(sockopt_val))) < 0)
        ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(),
                       "calling setsockopt()");
...
defined(IP_PMTUDISC_PROBE)
      ...
      if ((ret = setsockopt(b->num, IPPROTO_IP, IP_DONTFRAGMENT,
                            (const char *)&sockopt_val, sizeof(sockopt_val))) <
          0)
        ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(),
                       "calling setsockopt()");
#else
    ...
    case AF_INET6:
      ...
      if ((ret = setsockopt(b->num, IPPROTO_IPV6, IPV6_DONTFRAG,
                            (const void *)&sockopt_val, sizeof(sockopt_val))) <
          0)
        ERR_raise_data(ERR_LIB_SYS, get_last_socket_error(),
                       "calling setsockopt()");
  ...
  case BIO_CTRL_DGRAM_GET_EFFECTIVE_CAPS:
    ret =
    (long)(BIO_DGRAM_CAP_HANDLES_DST_ADDR | BIO_DGRAM_CAP_HANDLES_SRC_ADDR |
           BIO_DGRAM_CAP_PROVIDES_DST_ADDR | BIO_DGRAM_CAP_PROVIDES_SRC_ADDR);
    break;
...
static int translate_flags(uint64_t flags) { return 0; }
#endif
...
#define BIO_MAX_MSGS_PER_CALL 64
  ...
  for (i = 0; i < num_msg; ++i) {
    translate_msg(b, &mh[i].msg_hdr, &iov[i], control[i],
                  &BIO_MSG_N(msg, stride, i));
...
static int dgram_recvmmsg(BIO *b, BIO_MSG *msg, size_t stride, size_t num_msg,
  ...
  *num_processed = 1;
  ...
  ret = recvfrom(b->num, msg[0].data,
                 ...
                 msg[0].data_len,
#endif
                 sysflags, msg[0].peer != NULL ? &msg[0].peer->sa : NULL,
...
#ifndef OPENSSL_NO_SCTP
const BIO_METHOD *BIO_s_datagram_sctp(void) { return &methods_dgramp_sctp; }
...
BIO *BIO_new_dgram_sctp(int fd, int close_flag) {
  ...
  event.se_on = 1;
  ret =
  setsockopt(fd, IPPROTO_SCTP, SCTP_EVENT, &event, sizeof(struct sctp_event));
  if (ret < 0) {
    BIO_vfree(bio);
    return NULL;
  }
...
static int dgram_sctp_read(BIO *b, char *out, int outl) {
  ...
  if (out != NULL) {
    ...
    if (!data->peer_auth_tested) {
      ...
      ii = getsockopt(b->num, IPPROTO_SCTP, SCTP_PEER_AUTH_CHUNKS, authchunks,
                      &optlen);

      if (ii >= 0)
        for (p = (unsigned char *)authchunks->gauth_chunks;
             p < (unsigned char *)authchunks + optlen; p += sizeof(uint8_t)) {
          if (*p == OPENSSL_SCTP_DATA_CHUNK_TYPE)
            auth_data = 1;
          if (*p == OPENSSL_SCTP_FORWARD_CUM_TSN_CHUNK_TYPE)
            auth_forward = 1;
...
static long dgram_sctp_ctrl(BIO *b, int cmd, long num, void *ptr) {
  ...
  case BIO_CTRL_DGRAM_SCTP_ADD_AUTH_KEY:
    ...
    /* Reset active key */
    ret = setsockopt(b->num, IPPROTO_SCTP, SCTP_AUTH_ACTIVE_KEY, &authkeyid,
                     sizeof(struct sctp_authkeyid));
    if (ret < 0)
      break;
...
BIO *b, BIO_dgram_sctp_notification_handler_fn handle_notifications,
void *context) {
  bio_dgram_sctp_data *data = (bio_dgram_sctp_data *)b->ptr;
