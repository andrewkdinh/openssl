...
const BIO_METHOD *BIO_s_dgram_pair(void) { return &dgram_pair_method; }

const BIO_METHOD *BIO_s_dgram_mem(void) { return &dgram_mem_method; }
...
static int dgram_pair_sendmmsg(BIO *bio, BIO_MSG *msg, size_t stride,
  ...
  for (i = 0; i < num_msg; ++i) {
    m = &BIO_MSG_N(msg, i);
    l =
    dgram_pair_write_actual(bio, m->data, m->data_len, m->local, m->peer, 1);
    if (l < 0) {
      *num_processed = i;
      if (i > 0) {
        ret = 1;
