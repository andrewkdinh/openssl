...
static void slh_base_2b(const uint8_t *in, uint32_t b, uint32_t *out,
                        size_t out_len);
...
static int slh_fors_sk_gen(SLH_DSA_HASH_CTX *ctx, const uint8_t *sk_seed,
    ...
    return key->hash_func->PRF(ctx, pk_seed, sk_seed, sk_adrs, pk_out,
                               pk_out_len);
}
...
static int slh_fors_node(SLH_DSA_HASH_CTX *ctx, const uint8_t *sk_seed,
                         ...
                         uint32_t node_id, uint32_t height, uint8_t *node,
                         size_t node_len) {
    int ret = 0;
    const SLH_DSA_KEY *key = ctx->key;
    ...
    if (height == 0) {
        ...
        adrsf->set_tree_index(adrs, node_id);
        if (!key->hash_func->H(ctx, pk_seed, adrs, lnode, rnode, node,
                               node_len))
            return 0;
    }
    return 1;
 ...
 * There are k trees, each of which have a private key value of size |n|
 * followed by an authentication path of size |a| (where each path is size |n|)
 *
 * @param ctx Contains SLH_DSA algorithm functions and constants.
...
int ossl_slh_fors_sign(SLH_DSA_HASH_CTX *ctx, const uint8_t *md,
    ...
    for (tree_id = 0; tree_id < k; ++tree_id) {
         ...
         * level. For the next level up from the bottom there would be 2048
         * nodes (so tree i uses indexes 2048 * i + (0...2047) for this level)
         */
        tree_offset = tree_id_times_two_power_a;

        if (!slh_fors_sk_gen(ctx, sk_seed, pk_seed, adrs,
                             node_id + tree_id_times_two_power_a, out,
                             sizeof(out)) ||
         ...
         * well as all the leaf nodes. So it is calculating nodes multiple
         * times.
         */
        for (layer = 0; layer < a; ++layer) {
            s = node_id ^
                1; /* XOR gets the index of the other child in a binary tree */
            if (!slh_fors_node(ctx, sk_seed, pk_seed, adrs, s + tree_offset,
                               layer, out, sizeof(out)))
                return 0;
...
int ossl_slh_fors_pk_from_sig(SLH_DSA_HASH_CTX *ctx, PACKET *fors_sig_rpkt,
    ...
    const uint8_t *sk,
    *authj; /* Pointers to |sig| buffer inside fors_sig_rpkt */
