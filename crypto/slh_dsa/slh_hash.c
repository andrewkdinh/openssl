...
static int slh_hmsg_shake(SLH_DSA_HASH_CTX *ctx, const uint8_t *r,
  ...
  return xof_digest_4(ctx->md_ctx, r, n, pk_seed, n, pk_root, n, msg, msg_len,
                      out, m);
}
...
static int slh_prf_shake(SLH_DSA_HASH_CTX *ctx, const uint8_t *pk_seed,
  ...
  return xof_digest_3(ctx->md_ctx, pk_seed, n, adrs, SLH_ADRS_SIZE, sk_seed, n,
                      out, n);
}
...
static int slh_prf_msg_shake(SLH_DSA_HASH_CTX *ctx, const uint8_t *sk_prf,
  ...
  return xof_digest_3(ctx->md_ctx, sk_prf, n, opt_rand, n, msg, msg_len, out,
                      n) &&
...
static int slh_f_shake(SLH_DSA_HASH_CTX *ctx, const uint8_t *pk_seed,
  ...
  return xof_digest_3(ctx->md_ctx, pk_seed, n, adrs, SLH_ADRS_SIZE, m1, m1_len,
                      out, n);
}
...
static int slh_h_shake(SLH_DSA_HASH_CTX *ctx, const uint8_t *pk_seed,
  ...
  return xof_digest_4(ctx->md_ctx, pk_seed, n, adrs, SLH_ADRS_SIZE, m1, n, m2,
                      n, out, n);
}
...
static int slh_t_shake(SLH_DSA_HASH_CTX *ctx, const uint8_t *pk_seed,
  ...
  return xof_digest_3(ctx->md_ctx, pk_seed, n, adrs, SLH_ADRS_SIZE, ml, ml_len,
                      out, n);
}
...
static int slh_prf_msg_sha2(SLH_DSA_HASH_CTX *hctx, const uint8_t *sk_prf,
  ...
  if (hctx->hmac_digest_used == 0) {
    ...
    *p++ = OSSL_PARAM_construct_utf8_string(
    OSSL_MAC_PARAM_DIGEST, (char *)EVP_MD_get0_name(key->md_big), 0);
    if (key->propq != NULL)
...
static ossl_inline int do_hash(EVP_MD_CTX *ctx, size_t n,
  ...
  ret = digest_4(ctx, pk_seed, n, zeros, b - n, adrs, SLH_ADRSC_SIZE, m, m_len,
                 digest);
...
const SLH_HASH_FUNC *ossl_slh_get_hash_fn(int is_shake) {
  ...
  {slh_hmsg_shake, slh_prf_shake, slh_prf_msg_shake, slh_f_shake, slh_h_shake,
   slh_t_shake},
  {slh_hmsg_sha2, slh_prf_sha2, slh_prf_msg_sha2, slh_f_sha2, slh_h_sha2,
   slh_t_sha2}};
