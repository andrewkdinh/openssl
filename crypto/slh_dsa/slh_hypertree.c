 ...
 * @param leaf_id Index of the WOTS+ key within the XMSS tree that will sign the
 * message
...
int ossl_slh_ht_sign(SLH_DSA_HASH_CTX *ctx, const uint8_t *msg,
     ...
     * tree. Subsequent passes use the parent trees leaf node to sign the
     * current trees public key. Each node in an XMSS tree has a sibling (except
     * for the root node), so starting at the leaf node it traverses up the tree
     * calculating hashes for all the siblings in the path to the root node,
    ...
    for (layer = 0; layer < d; ++layer) {
        ...
        if (!ossl_slh_xmss_sign(ctx, root, sk_seed, leaf_id, pk_seed, adrs,
            ...
            return 0;
        /*
         * On the last loop it skips getting the public key since it is not
         * needed to calculate another signature. If this was called it should
         * equal the PK_ROOT (i.e. the public key of the top level tree).
         */
        if (layer < d - 1) {
 ...
 * @param leaf_id Index of the WOTS+ key within the XMSS tree that signed the
 * message
...
int ossl_slh_ht_verify(SLH_DSA_HASH_CTX *ctx, const uint8_t *msg,
    ...
    for (layer = 0; layer < d; ++layer) {
        adrsf->set_layer_address(adrs, layer);
        adrsf->set_tree_address(adrs, tree_id);
        if (!ossl_slh_xmss_pk_from_sig(ctx, leaf_id, sig_pkt, node, pk_seed,
                                       adrs, node, sizeof(node)))
            return 0;
        leaf_id = tree_id & mask;
        tree_id >>= tree_height;
    }
