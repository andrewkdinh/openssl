...
SLH_DSA_KEY *ossl_slh_dsa_key_dup(const SLH_DSA_KEY *src, int selection) {
    ...
    if (ret != NULL) {
        *ret =
        *src; /* this copies everything including the keydata in priv[] */
        ret->propq = NULL;
        ret->pub = NULL;
        ret->has_priv = 0;
    ...
    if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
        ...
        if (!key_checked &&
            ...
            if (key1->has_priv && key2->has_priv) {
                if (memcmp(key1->priv, key2->priv, key1->params->pk_len) != 0)
                    return 0;
                key_checked = 1;
            }
        }
...
int ossl_slh_dsa_key_fromdata(SLH_DSA_KEY *key, const OSSL_PARAM params[],
    ...
    /* The private key consists of 4 elements SK_SEED, SK_PRF, PK_SEED and
     * PK_ROOT
     */
    ...
    if (include_private) {
        ...
        if (param_priv != NULL) {
            p = key->priv;
            if (!OSSL_PARAM_get_octet_string(param_priv, &p, priv_len,
                                             &data_len))
                return 0;
    ...
    /*
     * In the case where the passed in private key does not contain the public
     * key there MUST be a separate public key, since the private key cannot
     * exist without the public key elements. NOTE that this does not accept
     * half of the public key, (Keygen must be used for this case currently).
     */
    p = SLH_DSA_PUB(key);
...
/**
 * Generate the public key root from private key (seed and prf) and public key
 * seed. See FIPS 205 Section 9.1 Algorithm 18
 *
 * @param ctx Contains SLH_DSA algorithm functions and constants.
 * @param out An SLH_DSA key containing the private key (seed and prf) and
 * public key seed. The public root key is written to this key.
 * @param validate If set to 1 the computed public key is not written to the
 * key, but will be compared to the existing value.
 * @returns 1 if the root key is generated or compared successfully, or 0 on
 * error.
 */
static int slh_dsa_compute_pk_root(SLH_DSA_HASH_CTX *ctx, SLH_DSA_KEY *out,
                                   int validate) {
...
/**
 * @brief Generate a SLH_DSA keypair. The private key seed and prf as well as
 * the public key seed are generated using an approved DRBG's. The public key
 * root is calculated using these generated values. See FIPS 205 Section 10.1
 * Algorithm 21
 *
 * @param ctx Contains SLH_DSA algorithm functions and constants
...
size_t ossl_slh_dsa_key_get_n(const SLH_DSA_KEY *key) { return key->params->n; }
