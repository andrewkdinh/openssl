...
void ossl_ffc_params_set_h(FFC_PARAMS *params, int index) { params->h = index; }
...
int ossl_ffc_params_cmp(const FFC_PARAMS *a, const FFC_PARAMS *b,
                        int ignore_q) {
...
int ossl_ffc_params_todata(const FFC_PARAMS *ffc, OSSL_PARAM_BLD *bld,
  ...
  if (ffc->j != NULL && !ossl_param_build_set_bn(
                        bld, params, OSSL_PKEY_PARAM_FFC_COFACTOR, ffc->j))
    return 0;
  ...
  if (!ossl_param_build_set_int(bld, params, OSSL_PKEY_PARAM_FFC_H, ffc->h))
    return 0;
  if (ffc->seed != NULL &&
      !ossl_param_build_set_octet_string(bld, params, OSSL_PKEY_PARAM_FFC_SEED,
  ...
  if (ffc->mdname != NULL &&
      !ossl_param_build_set_utf8_string(bld, params, OSSL_PKEY_PARAM_FFC_DIGEST,
                                        ffc->mdname))
    return 0;
  if (ffc->mdprops != NULL &&
