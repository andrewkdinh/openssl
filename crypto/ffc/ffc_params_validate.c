...
int ossl_ffc_params_FIPS186_4_validate(OSSL_LIB_CTX *libctx,
  ...
  libctx, (FFC_PARAMS *)params, FFC_PARAM_MODE_VERIFY, type, L, N, res, cb);
}
...
int ossl_ffc_params_FIPS186_2_validate(OSSL_LIB_CTX *libctx,
  ...
  L = BN_num_bits(params->p);
  N = BN_num_bits(params->q);
  return ossl_ffc_params_FIPS186_2_gen_verify(
  libctx, (FFC_PARAMS *)params, FFC_PARAM_MODE_VERIFY, type, L, N, res, cb);
}
...
int ossl_ffc_params_full_validate(OSSL_LIB_CTX *libctx,
  ...
  if (params->seed != NULL) {
    if (params->flags & FFC_PARAM_FLAG_VALIDATE_LEGACY)
      return ossl_ffc_params_FIPS186_2_validate(libctx, params, paramstype, res,
                                                NULL);
    else
      return ossl_ffc_params_FIPS186_4_validate(libctx, params, paramstype, res,
                                                NULL);
