...
static int generate_canonical_g(BN_CTX *ctx, BN_MONT_CTX *mont,
  ...
  for (counter = 1; counter <= 0xFFFF; ++counter) {
    ...
    if (!EVP_DigestInit_ex(mctx, evpmd, NULL) ||
        ...
        !EVP_DigestUpdate(mctx, md, 3) || !EVP_DigestFinal_ex(mctx, md, NULL) ||
...
g_only:
  ...
  if (((flags & FFC_PARAM_FLAG_VALIDATE_G) != 0) &&
      !ossl_ffc_params_validate_unverifiable_g(ctx, mont, p, q, params->g, tmp,
                                               res))
    goto err;
  ...
  if ((seed != NULL) && (params->gindex != FFC_UNVERIFIABLE_GINDEX)) {
    canonical_g = 1;
    if (!generate_canonical_g(ctx, mont, md, g, tmp, p, e, params->gindex, seed,
                              seedlen)) {
      *res = FFC_CHECK_INVALID_G;
      goto err;
    }
...
int ossl_ffc_params_FIPS186_2_gen_verify(OSSL_LIB_CTX *libctx,
  ...
  *res = 0;
  ...
  for (;;) {
    if (!generate_q_fips186_2(ctx, q, md, buf, seed, qsize, use_random_seed, &m,
                              res, cb))
      goto err;
    ...
    rv =
    generate_p(ctx, md, counter, n, buf, qsize, q, p, L, cb, &pcounter, res);
    if (rv > 0)
      break; /* found it */
    if (rv == -1)
      goto err;
