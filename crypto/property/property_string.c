...
static void property_free(PROPERTY_STRING *ps) { OPENSSL_free(ps); }
...
static const char *ossl_property_str(int name, OSSL_LIB_CTX *ctx,
...
#ifdef OPENSSL_SMALL_FOOTPRINT
  {
    ...
    lh_PROPERTY_STRING_doall_arg(
    name ? propdata->prop_names : propdata->prop_values, find_str_fn, &findstr);
    r = findstr.str;
  }
#else
  r = sk_OPENSSL_CSTRING_value(
  name ? propdata->prop_namelist : propdata->prop_valuelist, idx - 1);
#endif
  CRYPTO_THREAD_unlock(propdata->lock);
