...
static ALGORITHM *ossl_method_store_retrieve(OSSL_METHOD_STORE *store,
                                             int nid) {
...
int ossl_method_store_add(OSSL_METHOD_STORE *store, const OSSL_PROVIDER *prov,
     ...
     */
    if ((impl->properties = ossl_prop_defn_get(store->ctx, properties)) ==
        NULL) {
 ...
 */
static void alg_cleanup_by_provider(ossl_uintmax_t idx, ALGORITHM *alg,
                                    void *arg) {
        ...
        if (impl->provider == data->prov) {
            ...
            OSSL_TRACE_BEGIN(QUERY) {
                ...
                BIO_printf(trc_out,
                           ...
                           "query cache\nproperties %s\nprovider %s\n",
                           size == 0 ? "none" : buf,
                           ossl_provider_name(impl->provider) == NULL
                           ? "none"
...
void ossl_method_store_do_all(OSSL_METHOD_STORE *store,
    ...
    if (store != NULL) {
        ...
        tmpalgs =
        sk_ALGORITHM_new_reserve(NULL, ossl_sa_ALGORITHM_num(store->algs));
        if (tmpalgs == NULL) {
            ossl_property_unlock(store);
            return;
        }
        ...
        for (i = 0; i < numalgs; i++) {
            ...
            for (j = 0; j < numimps; j++)
                alg_do_one(alg, sk_IMPLEMENTATION_value(alg->impls, j), fn,
                           fnarg);
        }
