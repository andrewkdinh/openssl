...
static int parse_number(const char *t[], OSSL_PROPERTY_DEFINITION *res) {
  ...
  do {
    ...
    if (v > ((INT64_MAX - (*s - '0')) / 10)) {
      ERR_raise_data(ERR_LIB_PROP, PROP_R_PARSE_FAILED, "Property %s overflows",
                     *t);
      return 0;
    }
    v = v * 10 + (*s++ - '0');
...
static int parse_hex(const char *t[], OSSL_PROPERTY_DEFINITION *res) {
  ...
  if (!ossl_isspace(*s) && *s != '\0' && *s != ',') {
    ERR_raise_data(ERR_LIB_PROP, PROP_R_NOT_AN_HEXADECIMAL_DIGIT, "HERE-->%s",
                   *t);
    return 0;
  }
  *t = skip_space(s);
...
static void pd_free(OSSL_PROPERTY_DEFINITION *pd) { OPENSSL_free(pd); }
...
stack_to_property_list(OSSL_LIB_CTX *ctx,
  ...
  r =
  OPENSSL_malloc(sizeof(*r) + (n <= 0 ? 0 : n - 1) * sizeof(r->properties[0]));
  if (r != NULL) {
  ...
  while (i < query->num_properties) {
    ...
    if (j < defn->num_properties) {
      ...
      if (q[i].name_idx == d[j].name_idx) { /* both in defn and query */
        const int eq =
        q[i].type == d[j].type && memcmp(&q[i].v, &d[j].v, sizeof(q[i].v)) == 0;
...
void ossl_property_free(OSSL_PROPERTY_LIST *p) { OPENSSL_free(p); }
...
OSSL_PROPERTY_LIST *ossl_property_merge(const OSSL_PROPERTY_LIST *a,
  ...
  r =
  OPENSSL_malloc(sizeof(*r) + (t == 0 ? 0 : t - 1) * sizeof(r->properties[0]));
  if (r == NULL)
    return NULL;
...
static void put_str(const char *str, char **buf, size_t *remain,
                    size_t *needed) {
  size_t olen, len, i;
  char quote = '\0';
  int quotes;
