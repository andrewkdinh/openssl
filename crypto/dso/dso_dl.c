...
DSO_METHOD *DSO_METHOD_openssl(void) { return &dso_meth_dl; }
...
static int dl_load(DSO *dso) {
  ...
  if (ptr == NULL) {
    ...
    if (openssl_strerror_r(errno, errbuf, sizeof(errbuf)))
      ...
      ERR_raise_data(ERR_LIB_DSO, DSO_R_LOAD_FAILED, "filename(%s): errno %d",
                     filename, errno);
    goto err;
  }
...
static DSO_FUNC_TYPE dl_bind_func(DSO *dso, const char *symname) {
  ...
  if (shl_findsym(&ptr, symname, TYPE_UNDEFINED, &sym) < 0) {
    ...
    if (openssl_strerror_r(errno, errbuf, sizeof(errbuf)))
      ERR_raise_data(ERR_LIB_DSO, DSO_R_SYM_FAILURE, "symname(%s): %s", symname,
                     errbuf);
    else
      ERR_raise_data(ERR_LIB_DSO, DSO_R_SYM_FAILURE, "symname(%s): errno %d",
                     symname, errno);
    return NULL;
  }
