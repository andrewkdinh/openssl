...
DSO_METHOD *DSO_METHOD_openssl(void) { return &dso_meth_win32; }
...
static char *win32_joiner(DSO *dso, const struct file_st *file_split) {
  ...
  while (file_split->predirlen > (start - file_split->predir)) {
    ...
    start, '/', file_split->predirlen - (start - file_split->predir));
    if (!end)
  ...
  while (file_split->dirlen > (start - file_split->dir)) {
    const char *end =
    openssl_strnchr(start, '/', file_split->dirlen - (start - file_split->dir));
    if (!end)
...
static int win32_pathbyaddr(void *addr, char *path, int sz) {
  ...
  create_snap =
  (CREATETOOLHELP32SNAPSHOT)GetProcAddress(dll, "CreateToolhelp32Snapshot");
  if (create_snap == NULL) {
    FreeLibrary(dll);
    ERR_raise(ERR_LIB_DSO, DSO_R_UNSUPPORTED);
    return -1;
...
#ifdef _WIN32_WCE
  close_snap =
  (CLOSETOOLHELP32SNAPSHOT)GetProcAddress(dll, "CloseToolhelp32Snapshot");
#else
...
#ifdef _WIN32_WCE
#if _WIN32_WCE >= 101
      return WideCharToMultiByte(CP_ACP, 0, me32.szExePath, -1, path, sz, NULL,
                                 NULL);
#else
      {
