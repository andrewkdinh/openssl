...
EVP_PKEY *PEM_read_PUBKEY(FILE *fp, EVP_PKEY **x, pem_password_cb *cb,
                          void *u) {
...
legacy:
  if (x != NULL && (x->ameth == NULL || x->ameth->priv_encode != NULL))
    return PEM_write_bio_PKCS8PrivateKey(out, x, enc, (const char *)kstr, klen,
                                         cb, u);
...
legacy:
  ...
  return PEM_ASN1_write_bio((i2d_of_void *)x->ameth->param_encode, pem_str, out,
...
PEM_write_cb_ex_fnsig(PrivateKey, EVP_PKEY, FILE, write) {
  ...
  ret =
  PEM_write_bio_PrivateKey_ex(b, x, enc, kstr, klen, cb, u, libctx, propq);
  BIO_free(b);
  return ret;
}
