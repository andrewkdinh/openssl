...
static int do_i2b(unsigned char **out, const EVP_PKEY *pk, int ispub) {
  ...
  outlen =
  16 + ossl_blob_length(bitlen, keyalg == MS_KEYALG_DSS_SIGN ? 1 : 0, ispub);
  if (out == NULL)
    goto end;
  if (*out)
    p = *out;
...
int ossl_do_PVK_header(const unsigned char **in, unsigned int length,
                       int skip_magic, int *isdss, unsigned int *psaltlen,
...
static int derive_pvk_key(unsigned char *key, size_t keylen,
  ...
  *p++ =
  OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SALT, (void *)salt, saltlen);
  *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_PASSWORD,
                                           (void *)pass, passlen);
  *p++ = OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_DIGEST, SN_sha1, 0);
  *p++ =
  OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_PROPERTIES, (char *)propq, 0);
...
static void *do_PVK_body_key(const unsigned char **in, unsigned int saltlen,
                             ...
                             int *isdss, int *ispub, OSSL_LIB_CTX *libctx,
                             const char *propq) {
  const unsigned char *p = *in;
...
static void *do_PVK_key_bio(BIO *in, pem_password_cb *cb, void *u, int *isdss,
                            int *ispub, OSSL_LIB_CTX *libctx,
                            const char *propq) {
  ...
  key =
  do_PVK_body_key(&p, saltlen, keylen, cb, u, isdss, ispub, libctx, propq);
