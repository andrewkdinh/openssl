...
#define ROTATE(x, n)                                                           \
  ({                                                                           \
    u32 ret;                                                                   \
    asm("roriw %0, %1, %2" : "=r"(ret) : "r"(x), "i"(32 - (n)));               \
    ret;                                                                       \
  })
#endif
#if __riscv_xlen == 32
...
#define ROTATE(x, n)                                                           \
  ({                                                                           \
    u32 ret;                                                                   \
    asm("rori %0, %1, %2" : "=r"(ret) : "r"(x), "i"(32 - (n)));                \
    ret;                                                                       \
  })
#endif
#endif
...
#define QUARTERROUND(a, b, c, d)                                               \
  (x[a] += x[b], x[d] = ROTATE((x[d] ^ x[a]), 16), x[c] += x[d],               \
   x[b] = ROTATE((x[b] ^ x[c]), 12), x[a] += x[b],                             \
   x[d] = ROTATE((x[d] ^ x[a]), 8), x[c] += x[d],                              \
...
#endif
{
  ...
  input[0] = ((u32)ossl_toascii('e')) | ((u32)ossl_toascii('x') << 8) |
             ((u32)ossl_toascii('p') << 16) | ((u32)ossl_toascii('a') << 24);
  input[1] = ((u32)ossl_toascii('n')) | ((u32)ossl_toascii('d') << 8) |
             ((u32)ossl_toascii(' ') << 16) | ((u32)ossl_toascii('3') << 24);
  input[2] = ((u32)ossl_toascii('2')) | ((u32)ossl_toascii('-') << 8) |
             ((u32)ossl_toascii('b') << 16) | ((u32)ossl_toascii('y') << 24);
  input[3] = ((u32)ossl_toascii('t')) | ((u32)ossl_toascii('e') << 8) |
             ((u32)ossl_toascii(' ') << 16) | ((u32)ossl_toascii('k') << 24);
