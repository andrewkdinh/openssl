...
static struct ht_neighborhood_st *alloc_new_neighborhood_list(size_t len,
    ...
    if (ret == NULL) {
        ret = *freeptr =
        OPENSSL_malloc(sizeof(struct ht_neighborhood_st) * len);
        if (ret == NULL)
            return NULL;
    }
...
static void internal_free_nop(HT_VALUE *v) { return; }
...
void ossl_ht_read_lock(HT *htable) { ossl_rcu_read_lock(htable->lock); }

void ossl_ht_read_unlock(HT *htable) { ossl_rcu_read_unlock(htable->lock); }
...
int ossl_ht_flush(HT *h) { return ossl_ht_flush_internal(h); }
...
HT_VALUE_LIST *ossl_ht_filter(HT *h, size_t max_len,
    ...
    struct ht_mutable_data_st *md;
    HT_VALUE_LIST *list =
    OPENSSL_zalloc(sizeof(HT_VALUE_LIST) + (sizeof(HT_VALUE *) * max_len));
    size_t i, j;
    struct ht_internal_value_st *v;
...
void ossl_ht_value_list_free(HT_VALUE_LIST *list) { OPENSSL_free(list); }
...
static int grow_hashtable(HT *h, size_t oldsize) {
    ...
    newmd->neighborhoods =
    alloc_new_neighborhood_list(oldsize * 2, &newmd->neighborhood_ptr_to_free);
    if (newmd->neighborhoods == NULL)
        goto out_free;
...
static int ossl_ht_insert_locked(HT *h, uint64_t hash,
    ...
    do {
        ...
        for (j = 0; j < NEIGHBORHOOD_LEN; j++) {
            ival =
            ossl_rcu_deref(&md->neighborhoods[neigh_idx].entries[j].value);
            if (ival == NULL) {
                empty_idx = j;
            ...
            if (!CRYPTO_atomic_load(
                &md->neighborhoods[neigh_idx].entries[j].hash, &ihash,
                h->atomic_lock))
                return 0;
            if (compare_hash(hash, ihash) &&
                ...
                if (!CRYPTO_atomic_store(
                    ...
                    return 0;
                *olddata =
                (HT_VALUE *)md->neighborhoods[neigh_idx].entries[j].value;
                ossl_rcu_assign_ptr(
...
static struct ht_internal_value_st *
alloc_new_value(HT *h, HT_KEY *key, void *data, uintptr_t *type) {
...
static void free_value(struct ht_internal_value_st *v) { OPENSSL_free(v); }
...
int ossl_ht_insert(HT *h, HT_KEY *key, HT_VALUE *data, HT_VALUE **olddata) {
    ...
    for (i = 0;
         (rc = ossl_ht_insert_locked(h, hash, newval, olddata)) == -1 && i < 4;
         ++i)
...
HT_VALUE *ossl_ht_get(HT *h, HT_KEY *key) {
    ...
    do {
        ...
        for (j = 0; j < NEIGHBORHOOD_LEN; j++) {
            ...
            if (!CRYPTO_atomic_load(
                &md->neighborhoods[neigh_idx].entries[j].hash, &ehash,
                h->atomic_lock))
                return NULL;
...
int ossl_ht_delete(HT *h, HT_KEY *key) {
    ...
    for (j = 0; j < NEIGHBORHOOD_LEN; j++) {
        v = (struct ht_internal_value_st *)h->md->neighborhoods[neigh_idx]
            .entries[j]
            .value;
        if (v == NULL)
            continue;
        if (compare_hash(hash,
            ...
            h->wpd.value_count--;
            ossl_rcu_assign_ptr(
            &h->md->neighborhoods[neigh_idx].entries[j].value, &nv);
            rc = 1;
            break;
        }
    }
