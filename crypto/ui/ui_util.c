...
int UI_UTIL_read_pw_string(char *buf, int length, const char *prompt,
  ...
  ret = UI_UTIL_read_pw(buf, buff, (length > BUFSIZ) ? BUFSIZ : length, prompt,
                        verify);
...
DEFINE_RUN_ONCE_STATIC(ui_method_data_index_init) {
  ...
  CRYPTO_EX_INDEX_UI_METHOD, 0, NULL, ui_new_method_data, ui_dup_method_data,
  ui_free_method_data);
  return 1;
}

static int ui_open(UI *ui) { return 1; }
...
static int ui_close(UI *ui) { return 1; }
