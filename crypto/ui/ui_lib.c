...
UI *UI_new(void) { return UI_new_method(NULL); }
...
general_allocate_boolean(UI *ui, const char *prompt, const char *action_desc,
  ...
  if (ok_chars == NULL) {
    ...
    s = general_allocate_prompt(ui, prompt, prompt_freeable, type, input_flags,
                                result_buf);
...
int UI_dup_info_string(UI *ui, const char *text) {
  ...
  ret =
  general_allocate_string(ui, text_copy, 1, UIT_INFO, 0, NULL, 0, 0, NULL);
  if (ret <= 0)
...
int UI_dup_error_string(UI *ui, const char *text) {
  ...
  ret =
  general_allocate_string(ui, text_copy, 1, UIT_ERROR, 0, NULL, 0, 0, NULL);
  if (ret <= 0)
    OPENSSL_free(text_copy);
  return ret;
}
...
void *UI_get0_user_data(UI *ui) { return ui->user_data; }
...
int UI_process(UI *ui) {
  ...
  for (i = 0; i < sk_UI_STRING_num(ui->strings); i++) {
    if (ui->meth->ui_write_string != NULL &&
        (ui->meth->ui_write_string(ui, sk_UI_STRING_value(ui->strings, i)) <=
         0)) {
  ...
  for (i = 0; i < sk_UI_STRING_num(ui->strings); i++) {
    if (ui->meth->ui_read_string != NULL) {
      switch (
      ui->meth->ui_read_string(ui, sk_UI_STRING_value(ui->strings, i))) {
      case -1: /* Interrupt/Cancel/something... */
        ui->flags &= ~UI_FLAG_REDOABLE;
        ok = -2;
        goto err;
...
const UI_METHOD *UI_get_method(UI *ui) { return ui->meth; }
...
UI_METHOD *method,
char *(*prompt_constructor)(UI *ui, const char *, const char *)) {
  if (method != NULL) {
...
int UI_set_result_ex(UI *ui, UI_STRING *uis, const char *result, int len) {
  ...
  case UIT_VERIFY:
    if (len < uis->_.string_data.result_minsize) {
      ui->flags |= UI_FLAG_REDOABLE;
      ERR_raise_data(
      ERR_LIB_UI, UI_R_RESULT_TOO_SMALL, "You must type in %d to %d characters",
    ...
    if (len > uis->_.string_data.result_maxsize) {
      ui->flags |= UI_FLAG_REDOABLE;
      ERR_raise_data(
      ERR_LIB_UI, UI_R_RESULT_TOO_LARGE, "You must type in %d to %d characters",
      uis->_.string_data.result_minsize, uis->_.string_data.result_maxsize);
      return -1;
    }
