...
static UI_STRING *general_allocate_prompt(UI *ui, const char *prompt, int prompt_freeable, enum UI_string_types type,
...
static int general_allocate_string(UI *ui, const char *prompt, int prompt_freeable, enum UI_string_types type,
                                   int input_flags, char *result_buf, int minsize, int maxsize, const char *test_buf)
{
    int ret = -1;
...
static int general_allocate_boolean(UI *ui, const char *prompt, const char *action_desc, const char *ok_chars,
...
int UI_dup_input_string(UI *ui, const char *prompt, int flags, char *result_buf, int minsize, int maxsize)
{
    ...
    ret = general_allocate_string(ui, prompt_copy, 1, UIT_PROMPT, flags, result_buf, minsize, maxsize, NULL);
    if (ret <= 0)
...
int UI_add_verify_string(UI *ui, const char *prompt, int flags, char *result_buf, int minsize, int maxsize,
                         const char *test_buf)
{
    return general_allocate_string(ui, prompt, 0, UIT_VERIFY, flags, result_buf, minsize, maxsize, test_buf);
}

int UI_dup_verify_string(UI *ui, const char *prompt, int flags, char *result_buf, int minsize, int maxsize,
                         const char *test_buf)
{
    char *prompt_copy = NULL;
    int ret;
    ...
    ret = general_allocate_string(ui, prompt_copy, 1, UIT_VERIFY, flags, result_buf, minsize, maxsize, test_buf);
    if (ret <= 0)
        OPENSSL_free(prompt_copy);
    return ret;
}
                         ...
                         const char *cancel_chars, int flags, char *result_buf)
{
    ...
    ret = general_allocate_boolean(ui, prompt_copy, action_desc_copy, ok_chars_copy, cancel_chars_copy, 1, UIT_BOOLEAN,
                                   flags, result_buf);
    if (ret <= 0)
        goto err;
...
int UI_dup_info_string(UI *ui, const char *text)
{
    ...
    ret = general_allocate_string(ui, text_copy, 1, UIT_INFO, 0, NULL, 0, 0, NULL);
    if (ret <= 0)
...
int UI_dup_error_string(UI *ui, const char *text)
{
    ...
    ret = general_allocate_string(ui, text_copy, 1, UIT_ERROR, 0, NULL, 0, 0, NULL);
    if (ret <= 0)
        OPENSSL_free(text_copy);
    return ret;
}
...
int UI_process(UI *ui)
{
    ...
    for (i = 0; i < sk_UI_STRING_num(ui->strings); i++) {
        if (ui->meth->ui_write_string != NULL
            && (ui->meth->ui_write_string(ui, sk_UI_STRING_value(ui->strings, i)) <= 0)) {
...
int UI_set_result_ex(UI *ui, UI_STRING *uis, const char *result, int len)
{
    ...
    case UIT_VERIFY:
        if (len < uis->_.string_data.result_minsize) {
            ui->flags |= UI_FLAG_REDOABLE;
            ERR_raise_data(ERR_LIB_UI, UI_R_RESULT_TOO_SMALL, "You must type in %d to %d characters",
                           uis->_.string_data.result_minsize, uis->_.string_data.result_maxsize);
            return -1;
        }
        if (len > uis->_.string_data.result_maxsize) {
            ui->flags |= UI_FLAG_REDOABLE;
            ERR_raise_data(ERR_LIB_UI, UI_R_RESULT_TOO_LARGE, "You must type in %d to %d characters",
                           uis->_.string_data.result_minsize, uis->_.string_data.result_maxsize);
            return -1;
        }
