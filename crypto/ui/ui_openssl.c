...
!(defined(OPENSSL_SYS_TANDEM) && defined(_SPT_MODEL_))
#include <sys/ioctl.h>
#endif
...
static int read_string(UI *ui, UI_STRING *uis) {
    ...
    case UIT_VERIFY:
        ...
        if ((ok = read_string_inner(
             ui, uis, UI_get_input_flags(uis) & UI_INPUT_FLAG_ECHO, 1)) <= 0)
            return ok;
...
#define DEV_TTY "/dev/tty"
...
#ifdef ENODEV
                            /*
                             * MacOS X returns ENODEV (Operation not supported
                             * by device), which seems appropriate.
                             */
                            if (errno == ENODEV)
                                is_a_tty = 0;
                            else
    ...
    if (status != SS$_NORMAL) {
        ERR_raise_data(ERR_LIB_UI, UI_R_SYSASSIGN_ERROR, "status=%%X%08X",
                       status);
        return 0;
    }

    status =
    sys$qiow(0, channel, IO$_SENSEMODE, &iosb, 0, 0, tty_orig, 12, 0, 0, 0, 0);
    ...
    if (is_a_tty) {
        ...
        if ((status != SS$_NORMAL) || (iosb.iosb$w_value != SS$_NORMAL)) {
            ERR_raise_data(ERR_LIB_UI, UI_R_SYSQIOW_ERROR,
                           "status=%%X%08X, iosb.iosb$w_value=%%X%08X", status,
                           iosb.iosb$w_value);
            return 0;
        }
    }
...
static int close_console(UI *ui) {
    ...
    if (status != SS$_NORMAL) {
        ERR_raise_data(ERR_LIB_UI, UI_R_SYSDASSGN_ERROR, "status=%%X%08X",
                       status);
        ret = 0;
    }
#endif
...
UI_METHOD *UI_OpenSSL(void) { return &ui_openssl; }
...
void UI_set_default_method(const UI_METHOD *meth) { default_UI_meth = meth; }

const UI_METHOD *UI_get_default_method(void) { return default_UI_meth; }
