...
/*
 * OS/X 10.7 and 10.8 had a weird version of clang which has __ATOMIC_ACQUIRE
 * and
 * __ATOMIC_ACQ_REL but which expects only one parameter for
 * __atomic_is_lock_free() rather than two which has signature
 * __atomic_is_lock_free(sizeof(_Atomic(T))). All of this makes impossible to
 * use __atomic_is_lock_free here.
 *
 * See:
 * https://github.com/llvm/llvm-project/commit/a4c2602b714e6c6edb98164550a5ae829b2de760
 */
...
static struct rcu_qp *get_hold_current_qp(struct rcu_lock_st *lock) {
    ...
    for (;;) {
         ...
         * 2) We want to ensure that any updating of reader_idx on the write
         * side of the lock is flushed from a local cpu cache so that we see any
...
#ifndef FIPS_MODULE
int openssl_init_fork_handlers(void) { return 1; }
#endif /* FIPS_MODULE */
