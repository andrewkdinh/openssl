...
static void xor128(ARIA_c128 o, const ARIA_c128 x, const ARIA_u128 *y)
{
    ...
    for (i = 0; i < ARIA_BLOCK_SIZE; i++) o[i] = x[i] ^ y->c[i];
}
                              ...
                              ARIA_KEY *key)
{
    ...
    if (r == 0) {
        ...
        for (i = 1; i < rounds; i++) a(&key->rd_key[i], &ek.rd_key[rounds - i]);
