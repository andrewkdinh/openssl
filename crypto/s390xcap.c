...
#define TOK_CPU_ALIAS(NAME, STRUCT_NAME)                                       \
  (sscanf(tok_begin, " %" STR(LEN) "s %" STR(LEN) "s ", tok[0], tok[1]) ==     \
   1 &&                                                                        \
   !strcmp(tok[0], #NAME)) {                                                   \
...
static int parse_env(struct OPENSSL_s390xcap_st *cap, int *cex) {
  ...
  /*.stfle  = */ {S390X_CAPBIT(S390X_MSA) | S390X_CAPBIT(S390X_STCKF), 0ULL,
                  0ULL, 0ULL},
  /*.kimd   = */
  ...
  /*.stfle  = */ {S390X_CAPBIT(S390X_MSA) | S390X_CAPBIT(S390X_STCKF),
                  S390X_CAPBIT(S390X_MSA3) | S390X_CAPBIT(S390X_MSA4), 0ULL,
                  0ULL},
  /*.kimd   = */
  {S390X_CAPBIT(S390X_QUERY) | S390X_CAPBIT(S390X_SHA_1) |
   S390X_CAPBIT(S390X_SHA_256) | S390X_CAPBIT(S390X_SHA_512),
  ...
  {S390X_CAPBIT(S390X_QUERY) | S390X_CAPBIT(S390X_AES_128) |
   S390X_CAPBIT(S390X_AES_192) | S390X_CAPBIT(S390X_AES_256) |
   S390X_CAPBIT(S390X_XTS_AES_128) | S390X_CAPBIT(S390X_XTS_AES_256),
   0ULL},
  /*.kmc    = */
  {S390X_CAPBIT(S390X_QUERY) | S390X_CAPBIT(S390X_AES_128) |
   S390X_CAPBIT(S390X_AES_192) | S390X_CAPBIT(S390X_AES_256),
  ...
  {S390X_CAPBIT(S390X_QUERY) | S390X_CAPBIT(S390X_AES_128) |
   ...
   0ULL},
  /*.pcc    = */
  {S390X_CAPBIT(S390X_QUERY), S390X_CAPBIT(S390X_SCALAR_MULTIPLY_P256) |
  ...
  {S390X_CAPBIT(S390X_QUERY) | S390X_CAPBIT(S390X_AES_128) |
   ...
   S390X_CAPBIT(S390X_HMAC_SHA_384) | S390X_CAPBIT(S390X_HMAC_SHA_512)},
  /*.kmctr  = */
  {S390X_CAPBIT(S390X_QUERY) | S390X_CAPBIT(S390X_AES_128) |
   S390X_CAPBIT(S390X_AES_192) | S390X_CAPBIT(S390X_AES_256),
   0ULL},
  ...
  while (tok_begin != NULL) {
    ...
    /* query function tokens */
      ...
      else if (sscanf(tok_begin, " %" STR(LEN) "s %" STR(LEN) "s ", tok[0],
                      tok[1]) == 1 &&
               !strcmp(tok[0], "nocex")) {
        *cex = 0;
      }
