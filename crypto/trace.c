...
static int trace_write(BIO *channel, const char *buf, size_t num,
                       size_t *written) {
  struct trace_data_st *ctx = BIO_get_data(channel);
  size_t cnt =
  ctx->callback(buf, num, ctx->category, OSSL_TRACE_CTRL_WRITE, ctx->data);
...
DEFINE_RUN_ONCE_STATIC(ossl_trace_init) { return do_ossl_trace_init(); }
...
void ossl_trace_cleanup(void) {
  ...
  for (category = 0; category < OSSL_TRACE_CATEGORY_NUM; category++) {
    ...
    if (category == OSSL_TRACE_CATEGORY_TRACE)
      continue;
    set_trace_data(category, 0, &channel, &prefix, &suffix, trace_attach_cb,
                   trace_detach_cb);
  }
  set_trace_data(OSSL_TRACE_CATEGORY_TRACE, 0, &channel, &prefix, &suffix,
...
#ifndef OPENSSL_NO_TRACE
static int trace_attach_w_callback_cb(int category, int type,
                                      const void *data) {
  switch (type) {
  case CHANNEL:
...
#ifndef OPENSSL_NO_TRACE
  if (category >= 0 && category < OSSL_TRACE_CATEGORY_NUM)
    return set_trace_data(category, 0, NULL, &prefix, NULL, trace_attach_cb,
                          trace_detach_cb);
#endif
  return 0;
}
...
#ifndef OPENSSL_NO_TRACE
  if (category >= 0 && category < OSSL_TRACE_CATEGORY_NUM)
    return set_trace_data(category, 0, NULL, NULL, &suffix, trace_attach_cb,
                          trace_detach_cb);
#endif
  return 0;
}
...
int OSSL_trace_string(BIO *out, int text, int full, const unsigned char *data,
  ...
  if (!full && size > OSSL_TRACE_STRING_MAX) {
    BIO_printf(out, "[len %zu limited to %d]: ", size, OSSL_TRACE_STRING_MAX);
