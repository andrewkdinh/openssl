...
ESS_SIGNING_CERT_V2 *
OSSL_ESS_signing_cert_v2_new_init(const EVP_MD *hash_alg, const X509 *signcert,
                                  const STACK_OF(X509) * certs,
                                  int set_issuer_serial) {
...
static int ess_issuer_serial_cmp(const ESS_ISSUER_SERIAL *is,
                                 const X509 *cert) {
  GENERAL_NAME *issuer;
 ...
 */
static int find(const ESS_CERT_ID *cid, const ESS_CERT_ID_V2 *cid_v2, int index,
                const STACK_OF(X509) * certs) {
  const X509 *cert;
  EVP_MD *md = NULL;
  ...
  for (i = 0; i < sk_X509_num(certs); ++i) {
    ...
    if (memcmp(cid != NULL ? cid->hash->data : cid_v2->hash->data, cert_digest,
               len) == 0) {
