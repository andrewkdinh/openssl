...
static void ossl_decoder_free(void *data) { OSSL_DECODER_free(data); }

static int ossl_decoder_up_ref(void *data) { return OSSL_DECODER_up_ref(data); }
...
static int up_ref_decoder(void *method) { return OSSL_DECODER_up_ref(method); }

static void free_decoder(void *method) { OSSL_DECODER_free(method); }
...
static OSSL_DECODER *inner_ossl_decoder_fetch(struct decoder_data_st *methdata,
  ...
  if (id == 0 ||
    ...
    get_tmp_decoder_store,  reserve_decoder_store, unreserve_decoder_store,
    ...
    if ((method =
         ossl_method_construct(methdata->libctx, OSSL_OP_DECODER, &prov,
