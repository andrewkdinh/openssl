    ...
    if (!data.flag_construct_called) {
        ...
        if (ERR_peek_last_error() == lasterr || ERR_peek_error() == 0)
            ...
            ERR_raise_data(ERR_LIB_OSSL_DECODER, ERR_R_UNSUPPORTED,
                           ...
                           input_type_label, input_type, comma,
    ...
    if (ossl_decoder_fast_is_a(decoder, data->output_type,
        ...
        case IS_SAME:
            ...
            if (!ossl_decoder_fast_is_a(
                decoder, OSSL_DECODER_INSTANCE_get_input_type(di),
                &di->input_type_id)) {
        ...
        case IS_DIFFERENT:
            ...
            if (ossl_decoder_fast_is_a(decoder,
                ...
                OSSL_TRACE_BEGIN(DECODER) {
                    BIO_printf(
                    trc_out, "    REJECTED: input type matches output type\n");
                }
                OSSL_TRACE_END(DECODER);
                return;
            }
...
#define LEVEL                                                                  \
    ...
    if (params == NULL) {
        ...
        if (p != NULL) {
            if (!OSSL_PARAM_get_utf8_string_ptr(p, &ctx->start_input_type))
                goto end;
            /*
             * When switching PKCS8 from PEM to DER we decrypt the data if
             * needed and then determine the algorithm OID.  Likewise, with
             * SPKI, only this time sans decryption.
             */
            if (ctx->input_structure != NULL &&
                (OPENSSL_strcasecmp(ctx->input_structure,
                                    "SubjectPublicKeyInfo") == 0 ||
                 OPENSSL_strcasecmp(data_structure, "PrivateKeyInfo") == 0 ||
                 OPENSSL_strcasecmp(ctx->input_structure, "PrivateKeyInfo") ==
                 0))
    ...
    for (i = data->current_decoder_inst_index; i-- > 0;) {
        ...
        OSSL_TRACE_BEGIN(DECODER) {
            ...
            (void *)new_decoder_inst, (void *)new_decoder, new_decoder_name,
        ...
        if (decoder != NULL &&
            ...
            OSSL_TRACE_BEGIN(DECODER) {
                ...
                (void *)new_data.ctx, LEVEL, (unsigned int)i, (void *)decoder);
            }
            OSSL_TRACE_END(DECODER);
            continue;
        }
        ...
        if (data_structure != NULL &&
            ...
            OSSL_TRACE_BEGIN(DECODER) {
                BIO_printf(
                trc_out,
                "(ctx %p) %s [%u] the previous decoder's data structure "
                "doesn't match "
        ...
        if (!data->flag_input_structure_checked &&
            ctx->input_structure != NULL && new_input_structure != NULL) {
            data->flag_input_structure_checked = 1;
            if (OPENSSL_strcasecmp(new_input_structure, ctx->input_structure) !=
                0) {
                OSSL_TRACE_BEGIN(DECODER) {
                    BIO_printf(trc_out,
                               "(ctx %p) %s [%u] the previous decoder's data "
                               "structure doesn't "
                               "match the input structure given by the user, "
                               "skipping...\n",
                               (void *)new_data.ctx, LEVEL, (unsigned int)i);
                }
                OSSL_TRACE_END(DECODER);
        ...
        new_decoderctx, cbio, new_data.ctx->selection, decoder_process,
        &new_data, ossl_pw_passphrase_callback_dec, &new_data.ctx->pwdata);
