...
static int put_encoder_in_store(void *store, void *method, const OSSL_PROVIDER *prov, const char *names,
                                const char *propdef, void *data)
{
    ...
    return ossl_method_store_add(store, prov, id, propdef, method, ossl_encoder_up_ref, ossl_encoder_free);
}
...
static OSSL_ENCODER *inner_ossl_encoder_fetch(struct encoder_data_st *methdata, const char *name,
                                              const char *properties)
{
    ...
    if (id == 0 || !ossl_method_store_cache_get(store, NULL, id, propq, &method)) {
        OSSL_METHOD_CONSTRUCT_METHOD mcm = {get_tmp_encoder_store,  reserve_encoder_store, unreserve_encoder_store,
                                            get_encoder_from_store, put_encoder_in_store,  construct_encoder,
                                            destruct_encoder};
        ...
        if ((method =
                 ossl_method_construct(methdata->libctx, OSSL_OP_ENCODER, &prov, 0 /* !force_cache */, &mcm, methdata))
            != NULL) {
            /*
    ...
    if ((id != 0 || name != NULL) && method == NULL) {
        ...
        ERR_raise_data(ERR_LIB_OSSL_ENCODER, code, "%s, Name (%s : %d), Properties (%s)",
                       ossl_lib_ctx_get_descriptor(methdata->libctx), name == NULL ? "<null>" : name, id,
...
void OSSL_ENCODER_do_all_provided(OSSL_LIB_CTX *libctx, void (*user_fn)(OSSL_ENCODER *encoder, void *arg),
                                  void *user_arg)
{
    struct encoder_data_st methdata;
