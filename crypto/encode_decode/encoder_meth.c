...
static void ossl_encoder_free(void *data) { OSSL_ENCODER_free(data); }

static int ossl_encoder_up_ref(void *data) { return OSSL_ENCODER_up_ref(data); }
...
static int up_ref_encoder(void *method) { return OSSL_ENCODER_up_ref(method); }

static void free_encoder(void *method) { OSSL_ENCODER_free(method); }
...
static OSSL_ENCODER *inner_ossl_encoder_fetch(struct encoder_data_st *methdata,
    ...
    if (id == 0 ||
        ...
        get_tmp_encoder_store,  reserve_encoder_store, unreserve_encoder_store,
