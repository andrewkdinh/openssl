    ...
    if ((data->keymgmt_prov == prov) == data->flag_find_same_provider) {
        ...
        for (i = 0; i < end_i; i++) {
            if (data->flag_find_same_provider)
                ...
                encoder, sk_OPENSSL_CSTRING_value(data->names, i));
            if (!match ||
...
static int ossl_encoder_ctx_setup_for_pkey(OSSL_ENCODER_CTX *ctx,
    ...
    if (pkey->keymgmt != NULL) {
        ...
        /*
         * collect_encoder() is called many times, and for every call it
         * converts all encoder_data.names into namemap ids if it calls
         * OSSL_ENCODER_is_a(). We cache the ids here instead, and can use them
         * for encoders with the same provider as the keymgmt.
         */
        ...
        if (end > 0) {
            ...
            for (i = 0; i < end; ++i) {
                const char *name =
                sk_OPENSSL_CSTRING_value(keymgmt_data.names, i);
