...
int OSSL_ENCODER_to_data(OSSL_ENCODER_CTX *ctx, unsigned char **pdata,
  ...
  if (out != NULL && OSSL_ENCODER_to_bio(ctx, out) &&
...
const char *OSSL_ENCODER_INSTANCE_get_output_structure(
...
static int encoder_process(struct encoder_process_data_st *data) {
  ...
  for (i = data->current_encoder_inst_index; i-- > 0;) {
    ...
    if (top) {
      if (data->ctx->output_type != NULL &&
          OPENSSL_strcasecmp(current_output_type, data->ctx->output_type) !=
          0) {
        OSSL_TRACE_BEGIN(ENCODER) {
          BIO_printf(trc_out,
                     "[%d]    Skipping because current encoder output type "
                     "(%s) != desired output type (%s)\n",
    ...
    if (data->ctx->output_structure != NULL &&
      ...
      if (OPENSSL_strcasecmp(data->ctx->output_structure,
        ...
        OSSL_TRACE_BEGIN(ENCODER) {
          BIO_printf(trc_out,
                     "[%d]    Skipping because current encoder output "
                     "structure (%s) != ctx output structure (%s)\n",
                     data->level, current_output_structure,
  ...
  if (i < 0) {
    ...
    case -1:
      ...
      original_data =
      data->ctx->construct(current_encoder_inst, data->ctx->construct_data);
    ...
    case 1:
...

      {
        ...
        *abstract_p++ = OSSL_PARAM_construct_utf8_string(
        OSSL_OBJECT_PARAM_DATA_TYPE, (char *)data->data_type, 0);
        if (prev_output_structure != NULL)
          *abstract_p++ = OSSL_PARAM_construct_utf8_string(
          OSSL_OBJECT_PARAM_DATA_STRUCTURE, (char *)prev_output_structure, 0);
        *abstract_p++ = OSSL_PARAM_construct_octet_string(
        OSSL_OBJECT_PARAM_DATA, data->running_output,
    ...
    if (ok) {
      ...
      if (ok) {
        ok = current_encoder->encode(current_encoder_ctx, cbio, original_data,
                                     current_abstract, data->ctx->selection,
                                     ossl_pw_passphrase_callback_enc,
                                     &data->ctx->pwdata);
        OSSL_TRACE_BEGIN(ENCODER) {
          BIO_printf(
          trc_out, "[%d] (ctx %p) Running encoder instance %p => %d\n",
...
int ossl_bio_print_labeled_bignum(BIO *out, const char *label,
  ...
  if (BN_num_bytes(bn) <= BN_BYTES) {
    ...
    return BIO_printf(out, "%s%s%s" BN_FMTu " (%s0x" BN_FMTx ")\n", label,
                      post_label_spc, neg, words[0], neg, words[0]);
  }
