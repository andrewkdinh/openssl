...
#define RPK_DIMS (2 * 4 * 2 * 2 * 2 * 2)
  ...
  if (idx == 1) {
    ...
    /* Add certificate */
    if (!TEST_int_eq(
        SSL_use_certificate_file(serverssl, cert_file, SSL_FILETYPE_PEM), 1))
      goto end;
  ...
  case 2:
    ...
    if (!TEST_int_eq(
        SSL_use_PrivateKey_file(clientssl, privkey_file, SSL_FILETYPE_PEM), 1))
      goto end;
    if (!TEST_int_eq(
        SSL_use_certificate_file(clientssl, cert_file, SSL_FILETYPE_PEM), 1))
      goto end;
  ...
  if (testresult == 0) {
    TEST_info("idx_ss_rpk=%d, idx_sc_rpk=%d, idx_cs_rpk=%d, idx_cc_rpk=%d, "
              ...
              idx_client_server_rpk, idx_client_client_rpk, idx_cert, idx_prot,
              idx);
  }
  return testresult;
}
