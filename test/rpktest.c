...
# endif
    ...
    if (!TEST_true(create_ssl_ctx_pair(test_libctx, TLS_server_method(), TLS_client_method(), tls_version, tls_version,
                                       &sctx, &cctx, NULL, NULL)))
        goto end;
    ...
    case 9:
        ...
        if (!TEST_int_eq(SSL_check_private_key(clientssl), 1))
            goto end;
        SSL_set_verify(serverssl, SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT | SSL_VERIFY_POST_HANDSHAKE,
    ...
    if (testresult == 0) {
        TEST_info("idx_ss_rpk=%d, idx_sc_rpk=%d, idx_cs_rpk=%d, idx_cc_rpk=%d, idx_cert=%d, idx_prot=%d, idx=%d",
                  idx_server_server_rpk, idx_server_client_rpk, idx_client_server_rpk, idx_client_client_rpk, idx_cert,
                  idx_prot, idx);
    }
    return testresult;
}
...
static int test_rpk_api(void)
{
    int ret = 0;
    SSL_CTX *cctx = NULL, *sctx = NULL;
    unsigned char cert_type_dups[] = {TLSEXT_cert_type_rpk, TLSEXT_cert_type_x509, TLSEXT_cert_type_x509};
    ...
    if (!TEST_true(create_ssl_ctx_pair(NULL, TLS_server_method(), TLS_client_method(), TLS1_2_VERSION, TLS1_2_VERSION,
                                       &sctx, &cctx, NULL, NULL)))
        goto end;
