...
static int setup_selftest_pairwise_failure(const char *type) {
    ...
    /* Setup a callback that corrupts the pairwise self tests and causes
     * failures
     */
...
static int test_keygen_pairwise_failure(void) {
    ...
    if (strcmp(pairwise_name, "rsa") == 0) {
        ...
        if (!TEST_true(setup_selftest_pairwise_failure(type)))
            goto err;
        if (!TEST_ptr(ctx =
                      EVP_PKEY_CTX_new_from_name(libctx, "ED25519", NULL)))
            goto err;
        if (!TEST_int_eq(EVP_PKEY_keygen_init(ctx), 1))
            goto err;
        if (!TEST_int_le(EVP_PKEY_keygen(ctx, &pkey), 0))
        ...
        if (!TEST_true(setup_selftest_pairwise_failure(type)))
            goto err;
        if (!TEST_ptr(ctx =
                      EVP_PKEY_CTX_new_from_name(libctx, "ML-DSA-87", NULL)))
            goto err;
        if (!TEST_int_eq(EVP_PKEY_keygen_init(ctx), 1))
            goto err;
        if (!TEST_int_le(EVP_PKEY_keygen(ctx, &pkey), 0))
        ...
        if (!TEST_true(setup_selftest_pairwise_failure(type)))
            goto err;
        if (!TEST_ptr(ctx =
                      EVP_PKEY_CTX_new_from_name(libctx, "ML-KEM-1024", NULL)))
            goto err;
        if (!TEST_int_eq(EVP_PKEY_keygen_init(ctx), 1))
            goto err;
        if (!TEST_int_le(EVP_PKEY_keygen(ctx, &pkey), 0))
