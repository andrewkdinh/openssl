...
static void save_errno(void) { saved_errno = errno; }
...
static STACK_OF(X509) * load_chain(BIO *fp, int nelem) {
  ...
  for (count = 0; count < nelem && errtype == 0 &&
    ...
    if (strcmp(name, PEM_STRING_X509) == 0 ||
      ...
      d2i_X509_t d =
      strcmp(name, PEM_STRING_X509_TRUSTED) != 0 ? d2i_X509_AUX : d2i_X509;
      X509 *cert;
      const unsigned char *p = data;

      if (!TEST_ptr(cert = d(0, &p, len)) || !TEST_long_eq(p - data, len)) {
...
static int test_tlsafile(SSL_CTX *ctx, const char *base_name, BIO *f,
  ...
  while (ret > 0 && (line = read_to_eol(f)) != NULL) {
    ...
    ++testno;
    if (sscanf(line, "%d %d %d %d %d%n", &ntlsa, &ncert, &noncheck, &want,
               &want_depth, &off) != 5 ||
        !allws(line + off)) {
    ...
    if (!TEST_int_eq(err, want)) {
      if (want == X509_V_OK)
        TEST_info("Verification failure in test %d: %d=%s", testno, err,
