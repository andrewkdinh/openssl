...
static int do_testhpke(const TEST_BASEDATA *base, const TEST_AEADDATA *aead,
    ...
    if (!TEST_ptr(sealctx =
                  OSSL_HPKE_CTX_new(base->mode, base->suite,
                                    OSSL_HPKE_ROLE_SENDER, libctx, propq)))
        goto end;
        ...
        for (kemind = 0; /* iterate over the kems, kdfs and aeads */
            ...
            for (kdfind = 0;
                ...
                for (aeadind = 0;
                    ...
                    if (!TEST_true(OSSL_HPKE_keygen(hpke_suite, pub, &publen,
                    ...
                    if (hpke_mode == OSSL_HPKE_MODE_PSK ||
                        hpke_mode == OSSL_HPKE_MODE_PSKAUTH) {
                        if (!TEST_true(
                            OSSL_HPKE_CTX_set1_psk(ctx, pskidp, pskp, psklen)))
                            overallresult = 0;
                    }
                    if (hpke_mode == OSSL_HPKE_MODE_AUTH ||
                        hpke_mode == OSSL_HPKE_MODE_PSKAUTH) {
                        if (!TEST_true(
                            OSSL_HPKE_CTX_set1_authpriv(ctx, authpriv)))
                            overallresult = 0;
                    }
                    ...
                    if (!TEST_true(OSSL_HPKE_seal(ctx, cipher, &cipherlen, aadp,
                                                  aadlen, plain, plainlen)))
                        overallresult = 0;
                    OSSL_HPKE_CTX_free(ctx);
                    memset(clear, 0, clearlen);
                    rctx =
                    OSSL_HPKE_CTX_new(hpke_mode, hpke_suite,
                    ...
                    if (hpke_mode == OSSL_HPKE_MODE_PSK ||
                        hpke_mode == OSSL_HPKE_MODE_PSKAUTH) {
                        if (!TEST_true(
                            OSSL_HPKE_CTX_set1_psk(rctx, pskidp, pskp, psklen)))
                            overallresult = 0;
                    }
                    if (hpke_mode == OSSL_HPKE_MODE_AUTH ||
                        hpke_mode == OSSL_HPKE_MODE_PSKAUTH) {
                        ...
                        if (hpke_suite.kem_id == OSSL_HPKE_KEM_ID_P256) {
                            ...
                            if (!TEST_false(
                                OSSL_HPKE_CTX_set1_authpub(rctx, authpub, 10)))
                                overallresult = 0;
                        }
                        if (!TEST_true(OSSL_HPKE_CTX_set1_authpub(
                            rctx, authpubp, authpublen)))
                            overallresult = 0;
                    }
                    if (!TEST_true(OSSL_HPKE_decap(
                        rctx, senderpub, senderpublen, privp, infop, infolen)))
                        overallresult = 0;
                    /* throw in a call with a too-short clearlen */
                    clearlen = 15;
                    if (!TEST_false(OSSL_HPKE_open(rctx, clear, &clearlen, aadp,
                                                   aadlen, cipher, cipherlen)))
                        overallresult = 0;
                    /* fix up real clearlen again */
                    clearlen = OSSL_HPKE_TSTSIZE;
                    if (!TEST_true(OSSL_HPKE_open(rctx, clear, &clearlen, aadp,
                                                  aadlen, cipher, cipherlen)))
                        overallresult = 0;
...
static int test_hpke_export(void) {
    ...
    if (!TEST_true(OSSL_HPKE_keygen(hpke_suite, pub, &publen, &privp, NULL, 0,
                                    testctx, NULL)))
        goto end;
    if (!TEST_ptr(ctx = OSSL_HPKE_CTX_new(
                  hpke_mode, hpke_suite, OSSL_HPKE_ROLE_SENDER, testctx, NULL)))
        goto end;
...
static int test_hpke_random_suites(void) {
    ...
    if (!TEST_false(OSSL_HPKE_get_grease_value(
        &def_suite, &suite2, enc, &enclen, ct, ctlen, testctx, NULL)))
        return 0;
    ...
    if (!TEST_false(OSSL_HPKE_get_grease_value(&suite, &suite2, enc, &enclen,
                                               ct, ctlen, testctx, NULL)))
        return 0;
    enclen = sizeof(enc);
