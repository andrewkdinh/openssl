...
static int test_quic_write_read(int idx) {
  ...
  for (k = 0; k < 2; k++) {
    if (!TEST_ptr(cctx) ||
        ...
        libctx, cctx, sctx, cert, privkey, idx >= 1 ? QTEST_FLAG_BLOCK : 0,
    ...
    for (j = 0; j < 2; j++) {
      ...
      if (!TEST_true(SSL_read_ex(clientquic, buf, 1, &numbytes)) ||
          ...
          !TEST_true(
          SSL_read_ex(clientquic, buf + 1, sizeof(buf) - 1, &numbytes)) ||
...
static int test_fin_only_blocking(void) {
  ...
  if (!TEST_ptr(cctx) ||
      !TEST_true(qtest_create_quic_objects(libctx, cctx, sctx, cert, privkey,
                                           QTEST_FLAG_BLOCK, &qtserv,
  ...
  if (!TEST_true(ossl_quic_tserver_stream_new(qtserv, 0, &sid)) ||
      !TEST_true(ossl_quic_tserver_write(qtserv, sid, (unsigned char *)msg,
                                         strlen(msg), &numbytes)) ||
...
static int test_cipher_find(void) {
  ...
  for (i = 0; i < OSSL_NELEM(testciphers); i++)
    if (testciphers[i].ok) {
      ...
      if (!TEST_ptr_null(
          SSL_CIPHER_find(clientquic, testciphers[i].cipherbytes)))
        goto err;
    }
...
static int test_version(void) {
  ...
  if (!TEST_ptr(cctx) ||
      !TEST_true(qtest_create_quic_objects(libctx, cctx, NULL, cert, privkey, 0,
                                           &qtserv, &clientquic, NULL, NULL)) ||
...
static int test_ssl_trace(void) {
  ...
  if (!TEST_ptr(cctx =
      ...
      !TEST_true(qtest_create_quic_objects(libctx, cctx, NULL, cert, privkey,
...
static int test_bio_ssl(void) {
  ...
  for (i = 0, thisbio = cbio; i < 2; i++) {
    if (!TEST_true(
        ossl_quic_tserver_read(qtserv, sid, buf, sizeof(buf), &readbytes)) ||
...
static int test_quic_psk(void) {
  ...
  if (!TEST_ptr(cctx)
      ...
      || !TEST_true(qtest_create_quic_objects(
         libctx, cctx, NULL, NULL, NULL, 0, &qtserv, &clientquic, NULL, NULL)))
    goto end;
...
static int test_client_auth(int idx) {
  ...
  if (idx > 0 && (!TEST_true(SSL_CTX_use_certificate_chain_file(cctx, ccert)) ||
                  !TEST_true(SSL_CTX_use_PrivateKey_file(cctx, cprivkey,
                                                         SSL_FILETYPE_PEM))))
    goto err;

  if (!TEST_true(qtest_create_quic_objects(libctx, cctx, sctx, cert, privkey, 0,
                                           &qtserv, &clientquic, NULL, NULL)))
    goto err;
...
static int test_noisy_dgram(int idx) {
  ...
  if (!TEST_ptr(cctx) || !TEST_true(qtest_create_quic_objects(
                         libctx, cctx, NULL, cert, privkey, flags, &qtserv,
...
/*
 * Create a connection and send some big data using a transport with limited
 * bandwidth.
 */
...
#define TPARAM_CHECK_DROP_INJECT(name, buf, buf_len, reason)                   \
  {QUIC_TPARAM_##name, TPARAM_OP_DROP_INJECT, (reason), (buf), (buf_len)},
...
#define TPARAM_CHECK_INJECT_TWICE(name, buf, buf_len, reason)                  \
  {QUIC_TPARAM_##name, TPARAM_OP_INJECT_TWICE, (reason), (buf), (buf_len)},
...
TPARAM_CHECK_DUP(ORIG_DCID, "ORIG_DCID appears multiple times")
TPARAM_CHECK_DUP(INITIAL_SCID, "INITIAL_SCID appears multiple times")
TPARAM_CHECK_DUP(INITIAL_MAX_DATA, "INITIAL_MAX_DATA appears multiple times")
...
TPARAM_CHECK_DUP(MAX_IDLE_TIMEOUT, "MAX_IDLE_TIMEOUT appears multiple times")
...
TPARAM_CHECK_DROP(INITIAL_SCID, "INITIAL_SCID was not sent but is required")
TPARAM_CHECK_DROP(ORIG_DCID, "ORIG_DCID was not sent but is required")

TPARAM_CHECK_DROP_INJECT_A(DISABLE_ACTIVE_MIGRATION, disable_active_migration_1,
                           "DISABLE_ACTIVE_MIGRATION is malformed")
TPARAM_CHECK_INJECT(UNKNOWN_1, NULL, 0, NULL) TPARAM_CHECK_INJECT_RAW_A(
malformed_stateless_reset_token_1, "STATELESS_RESET_TOKEN is malformed")
TPARAM_CHECK_INJECT_A(STATELESS_RESET_TOKEN, malformed_stateless_reset_token_2,
                      "STATELESS_RESET_TOKEN is malformed")
TPARAM_CHECK_INJECT_A(STATELESS_RESET_TOKEN, malformed_stateless_reset_token_3,
                      "STATELESS_RESET_TOKEN is malformed")
TPARAM_CHECK_INJECT_A(STATELESS_RESET_TOKEN, malformed_stateless_reset_token_4,
                      "STATELESS_RESET_TOKEN is malformed")
TPARAM_CHECK_INJECT(STATELESS_RESET_TOKEN, NULL, 0,
                    "STATELESS_RESET_TOKEN is malformed")
...
TPARAM_CHECK_INJECT_RAW_A(malformed_preferred_addr_4,
                          "PREFERRED_ADDR is malformed")
TPARAM_CHECK_INJECT_RAW_A(malformed_unknown_1, "bad transport parameter")
TPARAM_CHECK_INJECT_RAW_A(malformed_unknown_2, "bad transport parameter")
TPARAM_CHECK_INJECT_RAW_A(malformed_unknown_3, "bad transport parameter")
...
TPARAM_CHECK_MUTATE(ORIG_DCID, "ORIG_DCID does not match expected value")
TPARAM_CHECK_MUTATE(INITIAL_SCID, "INITIAL_SCID does not match expected value")
...
TPARAM_CHECK_INT(INITIAL_MAX_DATA, "INITIAL_MAX_DATA is malformed")
...
TPARAM_CHECK_INT(ACK_DELAY_EXP, "ACK_DELAY_EXP is malformed")
TPARAM_CHECK_INT(MAX_ACK_DELAY, "MAX_ACK_DELAY is malformed") TPARAM_CHECK_INT(
INITIAL_MAX_STREAMS_BIDI, "INITIAL_MAX_STREAMS_BIDI is malformed")
TPARAM_CHECK_INT(INITIAL_MAX_STREAMS_UNI,
                 "INITIAL_MAX_STREAMS_UNI is malformed")
TPARAM_CHECK_INT(MAX_IDLE_TIMEOUT, "MAX_IDLE_TIMEOUT is malformed")
TPARAM_CHECK_INT(MAX_UDP_PAYLOAD_SIZE, "MAX_UDP_PAYLOAD_SIZE is malformed")
TPARAM_CHECK_INT(ACTIVE_CONN_ID_LIMIT, "ACTIVE_CONN_ID_LIMIT is malformed")};
...
static int tparam_handle(struct tparam_ctx *ctx, uint64_t id,
  ...
  case TPARAM_OP_DUP:
     ...
     */
    if (id == t->id && !TEST_ptr(ossl_quic_wire_encode_transport_param_bytes(
...
static int test_tparam(int idx) {
  ...
  if (!TEST_true(qtest_create_quic_objects(libctx, c_ctx, NULL, cert, privkey,
                                           0, &s, &c_ssl, &qtf, NULL)))
    goto err;
  ...
  if (ctx.t->expect_fail != NULL) {
    ...
    if (!TEST_true((info.flags & SSL_CONN_CLOSE_FLAG_TRANSPORT) != 0) ||
      ...
      TEST_error("expected connection closure information mismatch"
                 ...
                 (unsigned long long)info.error_code, info.reason);
      goto err;
    }
  }
...
static SSL_CTX *create_server_ctx(void) {
  ...
  if (!TEST_ptr(ssl_ctx =
                SSL_CTX_new_ex(libctx, NULL, OSSL_QUIC_server_method())) ||
      !TEST_true(
      SSL_CTX_use_certificate_file(ssl_ctx, cert, SSL_FILETYPE_PEM)) ||
      !TEST_true(
      SSL_CTX_use_PrivateKey_file(ssl_ctx, privkey, SSL_FILETYPE_PEM))) {
    SSL_CTX_free(ssl_ctx);
    ssl_ctx = NULL;
  } else {
...
static int bio_addr_bind(BIO *bio, BIO_ADDR *addr) {
  int bio_caps =
  BIO_DGRAM_CAP_HANDLES_DST_ADDR | BIO_DGRAM_CAP_HANDLES_SRC_ADDR;
...
static int test_ssl_accept_connection(void) {
  ...
  if (!TEST_true(
      create_bare_ssl_connection(serverssl, clientssl, SSL_ERROR_NONE, 0, 0)))
    goto err;
