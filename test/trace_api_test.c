...
static int put_trace_output(void) {
  ...
  OSSL_TRACE_BEGIN(HTTP) {
    ...
    res += TEST_int_eq(trace_string(0, 1, OSSL_MASKED),
                       strlen(OSSL_MASKED) + 1); /* newline added */
...
static int test_trace_channel(void) {
  ...
  if (!TEST_int_eq(OSSL_trace_set_prefix(OSSL_TRACE_CATEGORY_HTTP, OSSL_START),
                   1))
    goto end;
  if (!TEST_int_eq(OSSL_trace_set_suffix(OSSL_TRACE_CATEGORY_HTTP, OSSL_END),
                   1))
    goto end;
  ...
  if (!TEST_strn2_eq(p_buf, len, expected, expected_len))
    ret = 0;
  ret =
  TEST_int_eq(OSSL_trace_set_channel(OSSL_TRACE_CATEGORY_HTTP, NULL), 1) && ret;
...
static int test_trace_callback(void) {
  ...
  if (!TEST_true(
      OSSL_trace_set_callback(OSSL_TRACE_CATEGORY_TRACE, trace_cb, NULL)))
    goto end;
