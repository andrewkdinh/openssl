...
static char buf[256];
static OSSL_PARAM greeting_request[] = {{"greeting", OSSL_PARAM_UTF8_STRING, buf, sizeof(buf), 0},
                                        {NULL, 0, NULL, 0, 0}};
...
static int test_provider(OSSL_PROVIDER *prov, const char *expected_greeting)
{
    ...
    ret = TEST_true(ossl_provider_activate(prov, 1, 0)) && TEST_true(ossl_provider_get_params(prov, greeting_request))
        && TEST_ptr(greeting = greeting_request[0].data) && TEST_size_t_gt(greeting_request[0].data_size, 0)
        && TEST_str_eq(greeting, expected_greeting) && TEST_true(ossl_provider_deactivate(prov, 1));
...
static int test_cache_flushes(void)
{
    ...
    if (!TEST_ptr(ctx = OSSL_LIB_CTX_new()) || !TEST_ptr(prov = OSSL_PROVIDER_load(ctx, "default"))
        || !TEST_true(OSSL_PROVIDER_available(ctx, "default")) || !TEST_ptr(md = EVP_MD_fetch(ctx, "SHA256", NULL)))
        goto err;
    EVP_MD_free(md);
    md = NULL;
