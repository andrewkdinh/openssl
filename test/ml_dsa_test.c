...
static EVP_PKEY *do_gen_key(const char *alg, const uint8_t *seed,
  ...
  if (seed_len != 0)
    *p++ = OSSL_PARAM_construct_octet_string(OSSL_PKEY_PARAM_ML_DSA_SEED,
                                             (char *)seed, seed_len);
  *p++ =
  OSSL_PARAM_construct_utf8_string(OSSL_PKEY_PARAM_PROPERTIES, "?fips=yes", 0);
  ...
  *p = OSSL_PARAM_construct_end();
  ...
  if (!TEST_true(ml_dsa_create_keypair(&pkey, td->alg, td->priv, td->priv_len,
      ...
      !TEST_int_eq(EVP_PKEY_sign(sctx, psig, &psig_len, td->msg, td->msg_len),
                   1) ||
      !TEST_int_eq(EVP_Q_digest(lib_ctx, "SHA256", NULL, psig, psig_len, digest,
                                &digest_len),
                   1) ||
...
static int from_data_invalid_public_test(void) {
  ...
  if (!TEST_true(ml_dsa_create_keypair(
      &pkey, tst->name, tst->priv, tst->priv_len, tst->pub, tst->pub_len, 1)) ||
      !TEST_true(ml_dsa_create_keypair(&pkey, tst->name, tst->priv,
                                       tst->priv_len, pub, tst->pub_len, 0)) ||
      !TEST_true(ml_dsa_create_keypair(&pkey, tst->name, tst->priv,
...
static int from_data_bad_input_test(void) {
  ...
  params[1] = OSSL_PARAM_construct_end();
  if (!TEST_int_ne(
      EVP_PKEY_fromdata(ctx, &pkey, OSSL_KEYMGMT_SELECT_PUBLIC_KEY, params), 1))
    goto err;
  ret = 1;
err:
  EVP_PKEY_free(pkey);
