...
static int ml_dsa_keygen_test(int tst_id) {
    ...
    if (!TEST_ptr(pkey = do_gen_key(tst->name, tst->seed, tst->seed_len)) ||
        ...
        pkey, OSSL_PKEY_PARAM_ML_DSA_SEED, seed, sizeof(seed), &seed_len)) ||
        !TEST_true(EVP_PKEY_get_octet_string_param(
        pkey, OSSL_PKEY_PARAM_PRIV_KEY, priv, sizeof(priv), &priv_len)) ||
        !TEST_true(EVP_PKEY_get_octet_string_param(
    ...
    *p++ =
    ...
    if (td->add_random != NULL)
        *p++ = OSSL_PARAM_construct_octet_string(
        OSSL_SIGNATURE_PARAM_TEST_ENTROPY, (char *)td->add_random,
        td->add_random_len);
    *p = OSSL_PARAM_construct_end();
    ...
    if (!TEST_true(ml_dsa_create_keypair(&pkey, td->alg, td->priv, td->priv_len,
        ...
        !TEST_int_eq(EVP_PKEY_sign(sctx, NULL, &psig_len, td->msg, td->msg_len),
                     1) ||
        !TEST_true(
        ...
        !TEST_int_eq(EVP_PKEY_sign(sctx, psig, &psig_len, td->msg, td->msg_len),
                     1) ||
...
static int ml_dsa_sigver_test(int tst_id) {
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    if (!TEST_true(ml_dsa_create_keypair(&pkey, td->alg, NULL, 0, td->pub,
                                         td->pub_len, 1)))
        goto err;
...
static int ml_dsa_siggen_upd_test(int tst_id) {
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    if (!TEST_true(ml_dsa_create_keypair(&pkey, td->alg, td->priv, td->priv_len,
        ...
        !TEST_int_eq(
        EVP_PKEY_sign_message_update(sctx, td->msg + 1, td->msg_len - 1), 1) ||
...
static int ml_dsa_sigver_upd_test(int tst_id) {
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    if (!TEST_ptr(sig_alg = EVP_SIGNATURE_fetch(lib_ctx, td->alg, NULL)))
        goto err;
    if (!TEST_int_eq(EVP_PKEY_verify_message_init(vctx, sig_alg, params), 1) ||
        !TEST_int_eq(EVP_PKEY_CTX_set_signature(vctx, td->sig, td->sig_len),
                     1) ||
        !TEST_int_eq(EVP_PKEY_verify_message_update(vctx, td->msg, td->msg_len),
                     1) ||
...
static int ml_dsa_key_internal_test(void) {
    ...
    if (!TEST_ptr_null(key =
                       ossl_ml_dsa_key_new(NULL, NULL, EVP_PKEY_ML_DSA_44))
        ...
        || !TEST_ptr_null(
           key1 = ossl_ml_dsa_key_dup(NULL, OSSL_KEYMGMT_SELECT_KEYPAIR)) ||
        !TEST_ptr(
        key = ossl_ml_dsa_key_new(lib_ctx, "?fips=yes", EVP_PKEY_ML_DSA_44)) ||
        !TEST_ptr(key1 =
                  ossl_ml_dsa_key_dup(key, OSSL_KEYMGMT_SELECT_KEYPAIR)) ||
...
static int from_data_invalid_public_test(void) {
    ...
    if (!TEST_true(ml_dsa_create_keypair(&pkey, tst->name, tst->priv,
                                         tst->priv_len, tst->pub, tst->pub_len,
                                         1)) ||
        !TEST_true(ml_dsa_create_keypair(
        &pkey, tst->name, tst->priv, tst->priv_len, pub, tst->pub_len, 0)) ||
        !TEST_true(ml_dsa_create_keypair(&pkey, tst->name, tst->priv,
                                         tst->priv_len, tst->pub,
                                         tst->pub_len - 1, 0)) ||
        !TEST_true(ml_dsa_create_keypair(&pkey, tst->name, tst->priv,
                                         tst->priv_len, tst->pub,
                                         tst->pub_len + 1, 0)) ||
        !TEST_true(ml_dsa_create_keypair(&pkey, tst->name, tst->priv,
                                         tst->priv_len - 1, tst->pub,
                                         tst->pub_len, 0)) ||
        !TEST_true(ml_dsa_create_keypair(&pkey, tst->name, tst->priv,
                                         tst->priv_len + 1, tst->pub,
                                         tst->pub_len, 0)))
...
static int from_data_bad_input_test(void) {
    ...
    if (!TEST_ptr(ctx =
                  EVP_PKEY_CTX_new_from_name(lib_ctx, "ML-DSA-44", NULL)) ||
        !TEST_int_eq(EVP_PKEY_fromdata_init(ctx), 1) ||
        !TEST_ptr(
        EVP_PKEY_fromdata_settable(ctx, OSSL_KEYMGMT_SELECT_KEYPAIR)) ||
...
static int ml_dsa_keygen_drbg_test(void) {
    ...
    if (!TEST_ptr(pkey = do_gen_key("ML-DSA-44", NULL, 0)) ||
        ...
        !TEST_int_gt(len, 0) || !TEST_ptr(pub = OPENSSL_malloc(len)) ||
...
static int do_ml_dsa_sign_verify(const char *alg, int tstid) {
    ...
    *p++ = OSSL_PARAM_construct_end();
    ...
    if (!TEST_int_eq(EVP_PKEY_sign(sctx, NULL, &sig_len, sp->msg, sp->msg_len),
                     1) ||
        !TEST_ptr(sig = OPENSSL_zalloc(sig_len)))
        goto err;
    sig_len--;
    if (!TEST_int_eq(EVP_PKEY_sign(sctx, sig, &sig_len, sp->msg, sp->msg_len),
                     0))
        goto err;
    sig_len++;
    if (!TEST_int_eq(EVP_PKEY_sign(sctx, sig, &sig_len, sp->msg, sp->msg_len),
        ...
        !TEST_int_eq(EVP_PKEY_verify(vctx, sig, sig_len, sp->msg, sp->msg_len),
                     1))
        goto err;
    ret = 1;
err:
...
static int ml_dsa_digest_sign_verify_test(void) {
    ...
    *p++ = OSSL_PARAM_construct_end();
    ...
    if (!TEST_int_eq(EVP_DigestSign(mctx, NULL, &sig_len, sp->msg, sp->msg_len),
                     1) ||
        !TEST_ptr(sig = OPENSSL_zalloc(sig_len)))
        goto err;
    sig_len--;
    if (!TEST_int_eq(EVP_DigestSign(mctx, sig, &sig_len, sp->msg, sp->msg_len),
                     0))
        goto err;
    sig_len++;
    if (!TEST_int_eq(EVP_DigestSignInit_ex(mctx, NULL, NULL, lib_ctx,
                                           "?fips=true", key, params),
                     1) ||
        !TEST_int_eq(EVP_DigestSign(mctx, sig, &sig_len, sp->msg, sp->msg_len),
                     1) ||
        ...
        !TEST_int_eq(EVP_DigestVerifyInit_ex(mctx, NULL, NULL, lib_ctx,
                                             "?fips=true", key, params),
                     1) ||
        !TEST_int_eq(EVP_DigestVerify(mctx, sig, sig_len, sp->msg, sp->msg_len),
                     1))
        goto err;
    ret = 1;
err:
...
int setup_tests(void) {
    ...
    if (fips_provider_version_ge(lib_ctx, 3, 6, 0)) {
        ADD_ALL_TESTS(ml_dsa_siggen_upd_test,
                      OSSL_NELEM(ml_dsa_siggen_testdata));
        ADD_ALL_TESTS(ml_dsa_sigver_upd_test,
                      OSSL_NELEM(ml_dsa_sigver_testdata));
    }
