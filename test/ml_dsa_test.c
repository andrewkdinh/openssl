                                 ...
                                 int expected)
{
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    if (!TEST_ptr(ctx = EVP_PKEY_CTX_new_from_name(lib_ctx, name, NULL))
        ...
        || !TEST_int_eq(EVP_PKEY_fromdata(ctx, pkey, selection, params),
                        expected))
        goto err;
...
static int ml_dsa_siggen_test(int tst_id)
{
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    if (!TEST_true(ml_dsa_create_keypair(&pkey, td->alg, td->priv, td->priv_len,
        ...
        || !TEST_int_eq(
            EVP_PKEY_sign(sctx, NULL, &psig_len, td->msg, td->msg_len), 1)
        ...
        || !TEST_int_eq(
            EVP_PKEY_sign(sctx, psig, &psig_len, td->msg, td->msg_len), 1)
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    if (!TEST_int_eq(EVP_PKEY_verify_message_init(vctx, sig_alg, params), 1)
        || !TEST_int_eq(EVP_PKEY_CTX_set_signature(vctx, td->sig, td->sig_len),
                        1)
        || !TEST_int_eq(
            EVP_PKEY_verify_message_update(vctx, td->msg, td->msg_len), 1)
...
static int from_data_bad_input_test(void)
{
    ...
    if (!TEST_int_ne(EVP_PKEY_fromdata(ctx, &pkey,
...
static int do_ml_dsa_sign_verify(const char *alg, int tstid)
{
    ...
    *p++ = OSSL_PARAM_construct_end();
    ...
    if (!TEST_int_eq(EVP_PKEY_sign(sctx, sig, &sig_len, sp->msg, sp->msg_len),
        ...
        || !TEST_int_eq(
            EVP_PKEY_verify(vctx, sig, sig_len, sp->msg, sp->msg_len), 1))
        goto err;
    ret = 1;
err:
    EVP_PKEY_free(key);
...
static int ml_dsa_digest_sign_verify_test(void)
{
    ...
    *p++ = OSSL_PARAM_construct_end();
    ...
    if (!TEST_int_eq(EVP_DigestSignInit_ex(mctx, NULL, NULL, lib_ctx,
                     ...
                     1)
        || !TEST_int_eq(
            EVP_DigestSign(mctx, sig, &sig_len, sp->msg, sp->msg_len), 1)
        ...
        || !TEST_int_eq(EVP_DigestVerifyInit_ex(mctx, NULL, NULL, lib_ctx,
                        ...
                        1)
        || !TEST_int_eq(
            EVP_DigestVerify(mctx, sig, sig_len, sp->msg, sp->msg_len), 1))
        goto err;
    ret = 1;
err:
    EVP_PKEY_free(key);
...
static int ml_dsa_priv_pub_bad_t0_test(void)
{
    ...
    if (!TEST_true(ml_dsa_create_keypair(&key, td->alg, priv, td->priv_len,
                                         NULL, 0, 0)))
        goto err;
