...
static int test_hexstr_sep_to_from(int test_index)
{
    ...
    if (!TEST_ptr(buf = ossl_hexstr2buf_sep(test->in, &len, test->sep))
        ...
        || !TEST_ptr(out = ossl_buf2hexstr_sep(buf, len, test->sep)) || !TEST_str_eq(out, test->in))
        goto err;
...
static int test_hexstr_ex_to_from(int test_index)
{
    ...
    return TEST_true(OPENSSL_hexstr2buf_ex(buf, sizeof(buf), &len, test->in, ':'))
        ...
        && TEST_true(OPENSSL_buf2hexstr_ex(out, sizeof(out), NULL, buf, len, ':')) && TEST_str_eq(out, test->in)
        && TEST_true(OPENSSL_buf2hexstr_ex(out, sizeof(out), NULL, buf, 0, ':')) && TEST_size_t_eq(strlen(out), 0);
}
