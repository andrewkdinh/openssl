...
static char *change_path(const char *file)
{
    ...
    while ((p = strpbrk(p, DIRSEP)) != NULL) { last = p++; }
    if (last == NULL)
        goto err;
...
static int test_load_config(void)
{
    ...
    val = 0;
    if (!TEST_int_eq(NCONF_get_number(conf, "CA_default", "default_days", &val),
                     1)
        || !TEST_int_eq(val, 365)) {
    ...
    str =
        NCONF_get_string(conf, "req_distinguished_name", "countryName_default");
    ...
    if (test_providers != 0) {
        ...
        val = 0;
        if (!TEST_int_eq(NCONF_get_number(conf, "null_sect", "activate", &val),
                         1)
            || !TEST_int_eq(val, 1)) {
        ...
        val = 0;
        if (!TEST_int_eq(
                NCONF_get_number(conf, "default_sect", "activate", &val), 1)
            || !TEST_int_eq(val, 1)) {
        ...
        val = 0;
        if (!TEST_int_eq(
                NCONF_get_number(conf, "legacy_sect", "activate", &val), 1)
            || !TEST_int_eq(val, 1)) {
