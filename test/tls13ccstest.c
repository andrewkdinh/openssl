...
static const BIO_METHOD *bio_f_watchccs_filter(void) {
  if (method_watchccs == NULL) {
    method_watchccs =
    BIO_meth_new(BIO_TYPE_WATCHCCS_FILTER, "Watch CCS filter");
    if (method_watchccs == NULL ||
      ...
      if (msgtype == SSL3_MT_CLIENT_HELLO) {
        ...
        if (chseen == 1) {
          ...
          if (PACKET_remaining(&sessionid) != chsessidlen ||
              (chsessidlen > 0 &&
               memcmp(chsessid, PACKET_data(&sessionid), chsessidlen) != 0))
            badsessid = 1;
        }
...
static int test_tls13ccs(int tst) {
  ...
  if (tst >= 6) {
    ...
    if (!TEST_true(SSL_set_session(cssl, sess)) ||
        ...
        (tst <= 8 &&
         !TEST_int_eq(SSL_read_early_data(sssl, buf, sizeof(buf), &readbytes),
    ...
    if (!TEST_int_eq(SSL_read_early_data(sssl, buf, sizeof(buf), &readbytes),
  ...
  switch (tst) {
  case 0:
    if (!TEST_true(sccsseen) || !TEST_true(ccsaftersh) ||
        !TEST_false(ccsbeforesh) || !TEST_size_t_gt(chsessidlen, 0))
      goto err;
    break;

  case 1:
    if (!TEST_true(sccsseen) || !TEST_false(ccsaftersh) ||
        !TEST_false(ccsbeforesh) || !TEST_size_t_eq(chsessidlen, 0))
      goto err;
    break;

  case 2:
    if (!TEST_false(sccsseen) || !TEST_true(ccsaftersh) ||
        !TEST_false(ccsbeforesh) || !TEST_size_t_gt(chsessidlen, 0))
      goto err;
    break;
  ...
  case 6:
    if (!TEST_true(sccsseen) || !TEST_false(ccsaftersh) ||
        !TEST_true(ccsbeforesh) || !TEST_size_t_gt(chsessidlen, 0))
      goto err;
    break;
  ...
  case 8:
    if (!TEST_false(sccsseen) || !TEST_false(ccsaftersh) ||
        !TEST_true(ccsbeforesh) || !TEST_size_t_gt(chsessidlen, 0))
      goto err;
    break;
...
void cleanup_tests(void) { BIO_meth_free(method_watchccs); }
