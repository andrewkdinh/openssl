    ...
    while (PACKET_remaining(&pkt)) {
        ...
        if (rectype == SSL3_RT_HANDSHAKE) {
            ...
            if (msgtype == SSL3_MT_CLIENT_HELLO) {
                ...
                if (PACKET_remaining(&sessionid) != chsessidlen ||
                    (chsessidlen > 0 &&
                     memcmp(chsessid, PACKET_data(&sessionid), chsessidlen) !=
                     0))
                    badsessid = 1;
            }
...
static int test_tls13ccs(int tst) {
    ...
    if (tst >= 6) {
        ...
        if (!TEST_true(SSL_set_session(cssl, sess)) ||
            !TEST_true(
            SSL_write_early_data(cssl, msg, strlen(msg), &written)) ||
            (tst <= 8 && !TEST_int_eq(SSL_read_early_data(
                                      sssl, buf, sizeof(buf), &readbytes),
