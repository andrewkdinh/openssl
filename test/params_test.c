...
static int raw_set_params(void *vobj, const OSSL_PARAM *params)
{
    ...
    for (; params->key != NULL; params++)
        if (strcmp(params->key, "p1") == 0) {
            ...
            BN_free(obj->p3);
            if (!TEST_ptr(obj->p3 = BN_native2bn(params->data, params->data_size, NULL)))
                return 0;
        } else if (strcmp(params->key, "p4") == 0) {
            OPENSSL_free(obj->p4);
            if (!TEST_ptr(obj->p4 = OPENSSL_strndup(params->data, params->data_size)))
                return 0;
...
/* "raw" provider */
static const struct provider_dispatch_st provider_raw = {raw_set_params, raw_get_params};

/* "api" provider */
static const struct provider_dispatch_st provider_api = {api_set_params, api_get_params};
...
static OSSL_PARAM static_raw_params[] = {{"p1", OSSL_PARAM_INTEGER, &app_p1, sizeof(app_p1), 0},
...
static OSSL_PARAM static_api_params[] = {
    OSSL_PARAM_int("p1", &app_p1), OSSL_PARAM_BN("p3", &bignumbin, sizeof(bignumbin)),
    ...
    OSSL_PARAM_DEFN("foo", OSSL_PARAM_OCTET_STRING, &foo, sizeof(foo)), OSSL_PARAM_END};
...
static const struct param_owner_st api_params = {static_api_params, construct_api_params};
...
static int test_case_variant(OSSL_PARAM *params, const struct provider_dispatch_st *prov)
{
    ...
    if (!TEST_true(prov->get_params(obj, params)) || !TEST_int_eq(app_p1, p1_init)        /* "provider" value */
        || !TEST_double_eq(app_p2, app_p2_init) /* Should remain untouched */
        || !TEST_ptr(p = OSSL_PARAM_locate(params, "p3")) || !TEST_ptr(BN_native2bn(bignumbin, p->return_size, app_p3))
...
static int check_int_from_text(const struct int_from_text_test_st a)
{
    ...
    if (param.data_size != a.expected_bufsize) {
        TEST_error("unexpected size for %s \"%s\":  %d != %d", a.argname, a.strval, (int)a.expected_bufsize,
                   (int)param.data_size);
        return 0;
    }
...
static int check_octetstr_from_hexstr(void)
{
    ...
    for (i = 0; values[i] != NULL; i++) {
        ...
        if (expected)
            result =
                TEST_true(OSSL_PARAM_allocate_from_text(&param, params_from_text, "hexoctets", values[i], 0, NULL));
        else
            result =
                TEST_false(OSSL_PARAM_allocate_from_text(&param, params_from_text, "hexoctets", values[i], 0, NULL));
        if (!result) {
