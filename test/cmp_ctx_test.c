...
static int execute_CTX_reinit_test(OSSL_CMP_CTX_TEST_FIXTURE *fixture) {
    ...
    if (!TEST_true(ctx->status == -1 && ctx->failInfoCode == -1 &&
                   ...
                   ctx->extraCertsIn == NULL && ctx->validatedSrvCert == NULL &&
    ...
    OSSL_CMP_CTX_TEST_FIXTURE *fixture) {                                      \
        ...
        if (SETN == 0 && GETN == 0) {                                          \
            ...
            if (GETN == 1 && val2_read == val1_read) {                         \
                 ...
                 * because set1 may allocate the new copy at the same          \
                 * location.                                                   \
                 */                                                            \
...
static char *char_new(void) { return OPENSSL_strdup("test"); }

static void char_free(char *val) { OPENSSL_free(val); }
...
#define DROP(x)                                                                \
    (void)(x) /* dummy free() for non-pointer and function types               \
               */
...
#define DEFINE_SET_CB_TEST(FIELD)                                              \
    ...
    DEFINE_SET_GET_BASE_TEST(OSSL_CMP_CTX, set, get, 0, FIELD,                 \
                             OSSL_CMP_##FIELD##_t, NULL, IS_0, test_##FIELD,   \
                             DROP)
#define DEFINE_SET_GET_P_VOID_TEST(FIELD)                                      \
    DEFINE_SET_GET_BASE_TEST(OSSL_CMP_CTX, set, get, 0, FIELD, void *, NULL,   \
                             IS_0, ((void *)1), DROP)
...
#define DEFINE_PUSH_BASE_TEST(PUSHN, DUP, FIELD, ELEM, TYPE, T, DEFAULT, NEW,  \
    ...
    OSSL_CMP_CTX_TEST_FIXTURE *fixture) {                                      \
        ...
        /*                                                                     \
         * need type cast in above assignment because TYPE arg sometimes is    \
         * const                                                               \
         */                                                                    \
...
void cleanup_tests(void) { return; }
