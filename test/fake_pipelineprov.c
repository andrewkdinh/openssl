...
static int fake_pipeline_init(void *vctx, const unsigned char *key, size_t keylen, size_t numpipes,
...
int fake_pipeline_einit(void *vctx, const unsigned char *key, size_t keylen, size_t numpipes, const unsigned char **iv,
                        size_t ivlen, const OSSL_PARAM params[])
{
    return fake_pipeline_init(vctx, key, keylen, numpipes, iv, ivlen, 1);
}

int fake_pipeline_dinit(void *vctx, const unsigned char *key, size_t keylen, size_t numpipes, const unsigned char **iv,
                        size_t ivlen, const OSSL_PARAM params[])
{
    return fake_pipeline_init(vctx, key, keylen, numpipes, iv, ivlen, 0);
}

int fake_pipeline_update(void *vctx, size_t numpipes, unsigned char **out, size_t *outl, const size_t *outsize,
...
static const OSSL_PARAM fake_pipeline_aead_known_gettable_ctx_params[] = {
    ...
    OSSL_PARAM_octet_ptr(OSSL_CIPHER_PARAM_PIPELINE_AEAD_TAG, NULL, 0), OSSL_PARAM_END};
...
static const OSSL_ALGORITHM fake_ciphers[] = {
    {"AES-256-GCM", "provider=fake-pipeline", fake_pipeline_aes256gcm_functions}, {NULL, NULL, NULL}};
...
static int fake_pipeline_provider_init(const OSSL_CORE_HANDLE *handle, const OSSL_DISPATCH *in,
