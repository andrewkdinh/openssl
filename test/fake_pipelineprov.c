...
static void *fake_pipeline_newctx(void *provctx, char *ciphername, size_t kbits,
                                  size_t ivbits) {
...
static int fake_pipeline_init(void *vctx, const unsigned char *key,
...
int fake_pipeline_einit(void *vctx, const unsigned char *key, size_t keylen,
                        size_t numpipes, const unsigned char **iv, size_t ivlen,
                        const OSSL_PARAM params[]) {
    return fake_pipeline_init(vctx, key, keylen, numpipes, iv, ivlen, 1);
}

int fake_pipeline_dinit(void *vctx, const unsigned char *key, size_t keylen,
                        size_t numpipes, const unsigned char **iv, size_t ivlen,
                        const OSSL_PARAM params[]) {
    return fake_pipeline_init(vctx, key, keylen, numpipes, iv, ivlen, 0);
}

int fake_pipeline_update(void *vctx, size_t numpipes, unsigned char **out,
    ...
    for (i = 0; i < numpipes; i++) {
        ...
        if (!EVP_CipherUpdate(ctx->cipher_ctxs[i],
                              (out != NULL) ? out[i] : NULL, &ioutl, in[i],
                              inl_))
            return 0;
        outl[i] = (size_t)ioutl;
    }
...
int fake_pipeline_final(void *vctx, size_t numpipes, unsigned char **out,
                        size_t *outl, const size_t *outsize) {
    CIPHER_PIPELINE_CTX *ctx = (CIPHER_PIPELINE_CTX *)vctx;
    int ioutl;
    size_t i = 0;
...
OSSL_PARAM_END};
const OSSL_PARAM *
fake_pipeline_aead_gettable_ctx_params(ossl_unused void *cctx,
...
OSSL_PARAM_END};
const OSSL_PARAM *
fake_pipeline_aead_settable_ctx_params(ossl_unused void *cctx,
                                       ossl_unused void *provctx) {
...
int fake_pipeline_aead_get_ctx_params(void *vctx, OSSL_PARAM params[]) {
    ...
    if (p != NULL) {
        ...
        for (i = 0; i < ctx->numpipes; i++) {
            aead_params[0] = OSSL_PARAM_construct_octet_string(
            OSSL_CIPHER_PARAM_AEAD_TAG, (void *)aead_tags[i], taglen);
...
#define IMPLEMENT_aead_cipher_pipeline(alg, lc, UCMODE, flags, kbits, blkbits, \
    ...
    fake_pipeline_##alg##_##kbits##_##lc##_newctx;                             \
    static void *fake_pipeline_##alg##_##kbits##_##lc##_newctx(                \
    void *provctx) {                                                           \
    ...
    {{OSSL_FUNC_CIPHER_NEWCTX,                                                 \
     ...
     {OSSL_FUNC_CIPHER_FREECTX, (void (*)(void))fake_pipeline_freectx},        \
     ...
     {OSSL_FUNC_CIPHER_PIPELINE_UPDATE, (void (*)(void))fake_pipeline_update}, \
     {OSSL_FUNC_CIPHER_PIPELINE_FINAL, (void (*)(void))fake_pipeline_final},   \
...
fake_pipeline_query(OSSL_PROVIDER *prov, int operation_id, int *no_cache) {
    *no_cache = 0;
    switch (operation_id) {
    case OSSL_OP_CIPHER:
        return fake_ciphers;
...
void fake_pipeline_finish(OSSL_PROVIDER *p) { OSSL_PROVIDER_unload(p); }
