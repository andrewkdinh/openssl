...
static int test_init_multiple(int tstid)
{
    ...
    return TEST_int_eq(EVP_PKEY_encapsulate_init(ctx, NULL), 1) && TEST_int_eq(EVP_PKEY_encapsulate_init(ctx, NULL), 1)
...
static int test_ecx_dhkem_derivekey(int tstid)
{
    ...
    ret = TEST_ptr(genctx = EVP_PKEY_CTX_new_from_name(libctx, t->curvename, NULL))
        ...
        && TEST_int_eq(EVP_PKEY_get_octet_string_param(pkey, OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY, pubkey, sizeof(pubkey),
                       ...
                       1)
        && TEST_int_eq(
              EVP_PKEY_get_octet_string_param(pkey, OSSL_PKEY_PARAM_PRIV_KEY, privkey, sizeof(privkey), &privkeylen), 1)
