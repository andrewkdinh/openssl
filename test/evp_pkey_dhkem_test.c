...
static int test_dhkem_encapsulate(int tstid)
{
    ...
    if (t->spriv != NULL) {
        if (!TEST_ptr(spriv =
                          new_raw_private_key(t->curve, t->spriv, t->sprivlen,
                                              t->spub, t->spublen)))
            goto err;
    }
...
static int test_ikm_small(int tstid)
{
    ...
    params[1] = OSSL_PARAM_construct_octet_string(OSSL_KEM_PARAM_IKME, tmp,
                                                  sizeof(tmp));
    params[2] = OSSL_PARAM_construct_end();
...
static int test_ec_dhkem_derivekey(int tstid)
{
    ...
    ret = TEST_ptr(genctx = EVP_PKEY_CTX_new_from_name(libctx, "EC", NULL))
        ...
        && TEST_true(EVP_PKEY_get_octet_string_param(
            pkey, OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY, pubkey, sizeof(pubkey),
            &pubkeylen))
        && TEST_true(
...
static int test_ec_noikme(int tstid)
{
    ...
    if (curve[0] == 'X') {
        if (!TEST_ptr(recip = EVP_PKEY_Q_keygen(libctx, NULL, curve))
            || (auth
                && !TEST_ptr(sender_auth =
                                 EVP_PKEY_Q_keygen(libctx, NULL, curve))))
            goto err;
    } else {
    ...
    ret = TEST_ptr(ctx = EVP_PKEY_CTX_new_from_pkey(libctx, recip, NULL))
        && (sender_auth == NULL
            || TEST_int_eq(
                EVP_PKEY_auth_encapsulate_init(ctx, sender_auth, NULL), 1))
        && (sender_auth != NULL
            || TEST_int_eq(EVP_PKEY_encapsulate_init(ctx, NULL), 1))
        ...
        && (sender_auth == NULL
            || TEST_int_eq(
                EVP_PKEY_auth_decapsulate_init(ctx, sender_auth, NULL), 1))
        && (sender_auth != NULL
            || TEST_int_eq(EVP_PKEY_decapsulate_init(ctx, NULL), 1))
        && TEST_int_eq(EVP_PKEY_CTX_set_kem_op(ctx, op), 1)
        && TEST_int_eq(EVP_PKEY_decapsulate(ctx, recip_secret, &recip_secretlen,
                       ...
                       1)
        && TEST_mem_eq(recip_secret, recip_secretlen, sender_secret,
                       sender_secretlen);
err:
    EVP_PKEY_CTX_free(ctx);
...
static int test_ec_badpublic(int tstid)
{
    ...
    pub[0] = 0;
    if (!TEST_ptr(recippriv = new_raw_private_key(t->curve, t->rpriv,
                                                  t->rprivlen, pub, 1)))
        goto err;
...
static int test_ecx_dhkem_derivekey(int tstid)
{
    ...
    ret = TEST_ptr(genctx =
                       EVP_PKEY_CTX_new_from_name(libctx, t->curvename, NULL))
        && TEST_int_eq(EVP_PKEY_keygen_init(genctx), 1)
        && TEST_int_eq(EVP_PKEY_CTX_set_params(genctx, params), 1)
        && TEST_int_eq(EVP_PKEY_keygen(genctx, &pkey), 1)
        && TEST_int_eq(EVP_PKEY_get_octet_string_param(
                           pkey, OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY, pubkey,
                           sizeof(pubkey), &pubkeylen),
                       1)
        && TEST_int_eq(EVP_PKEY_get_octet_string_param(
                           pkey, OSSL_PKEY_PARAM_PRIV_KEY, privkey,
                           sizeof(privkey), &privkeylen),
                       1)
