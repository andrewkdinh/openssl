...
static int test_dhkem_encapsulate(int tstid) {
  ...
  if (t->spriv != NULL) {
    if (!TEST_ptr(spriv = new_raw_private_key(t->curve, t->spriv, t->sprivlen,
                                              t->spub, t->spublen)))
      goto err;
  }
...
static int test_ec_dhkem_derivekey_fail(void) {
  ...
  params[0] =
  OSSL_PARAM_construct_utf8_string(OSSL_PKEY_PARAM_GROUP_NAME, "secp256k1", 0);
...
static int test_ec_public_key_infinity(void) {
  ...
  ret = TEST_ptr(key = new_raw_private_key(t->curve, t->rpriv, t->rprivlen, tmp,
                                           sizeof(tmp))) &&
...
static int test_nopublic(int tstid) {
  ...
  if (!TEST_ptr(priv =
                new_raw_private_key(t->curve, t->rpriv, t->rprivlen, NULL, 0)))
    goto err;
...
static int test_ec_noikme(int tstid) {
  ...
  if (curve[0] == 'X') {
    ...
    if (!TEST_ptr(recip = EVP_PKEY_Q_keygen(libctx, NULL, "EC", curve)) ||
        (auth &&
         !TEST_ptr(sender_auth = EVP_PKEY_Q_keygen(libctx, NULL, "EC", curve))))
      goto err;
  }
...
static int test_ec_badpublic(int tstid) {
  ...
  if (encap) {
    ...
    if (!TEST_int_eq(EVP_PKEY_decapsulate(ctx, secret, &secretlen,
                                          t->expected_enc, t->expected_enclen),
                     0))
      goto err;
  }
...
static int test_ecx_auth_key_curve_mismatch(void) {
  ...
  ret = TEST_int_eq(
  EVP_PKEY_auth_encapsulate_init(rctx[TEST_KEYTYPE_X25519], auth, opparam), 0);
  EVP_PKEY_free(auth);
  return ret;
}
