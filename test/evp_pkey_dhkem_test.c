...
static int test_dhkem_encapsulate(int tstid) {
    ...
    if (t->spriv != NULL) {
        if (!TEST_ptr(spriv = new_raw_private_key(
                      t->curve, t->spriv, t->sprivlen, t->spub, t->spublen)))
            goto err;
    }
...
static int test_settables(int tstid) {
    ...
    return TEST_int_eq(EVP_PKEY_encapsulate_init(ctx, NULL), 1) &&
           ...
           TEST_ptr(
           p = OSSL_PARAM_locate_const(settableparams, OSSL_KEM_PARAM_IKME)) &&
...
static int test_ec_dhkem_derivekey_fail(void) {
    ...
    params[0] =
    OSSL_PARAM_construct_utf8_string(OSSL_PKEY_PARAM_GROUP_NAME, "P-224", 0);
    params[1] = OSSL_PARAM_construct_octet_string(OSSL_PKEY_PARAM_DHKEM_IKM,
                                                  (char *)t->ikm, t->ikmlen);
    params[2] = OSSL_PARAM_construct_end();
    ...
    OSSL_PKEY_PARAM_DHKEM_IKM, (char *)t->ikm, t->ikmlen - 1);
    params[2] = OSSL_PARAM_construct_end();
...
static int test_no_operation_set(int tstid) {
    ...
    return TEST_int_eq(EVP_PKEY_encapsulate_init(ctx, NULL), 1) &&
           ...
           TEST_int_eq(EVP_PKEY_decapsulate_init(ctx, NULL), 1) &&
           TEST_int_eq(EVP_PKEY_decapsulate(ctx, NULL, &len, t->expected_enc,
                                            t->expected_enclen),
                       1);
}
...
static int test_ikm_small(int tstid) {
    ...
    params[1] =
    OSSL_PARAM_construct_octet_string(OSSL_KEM_PARAM_IKME, tmp, sizeof(tmp));
    params[2] = OSSL_PARAM_construct_end();

    return TEST_int_eq(EVP_PKEY_encapsulate_init(ctx, params), 1) &&
           TEST_int_eq(
           EVP_PKEY_encapsulate(ctx, enc, &enclen, secret, &secretlen), 0);
}
...
static int test_input_size_small(int tstid) {
    ...
    if (!TEST_int_eq(EVP_PKEY_encapsulate_init(ctx, opparam), 1) ||
        !TEST_int_eq(EVP_PKEY_encapsulate(ctx, NULL, &enclen, NULL, &seclen),
                     1))
        goto err;
...
static int test_ec_auth_key_curve_mismatch(void) {
    ...
    ret =
    TEST_int_eq(EVP_PKEY_auth_encapsulate_init(rctx[0], auth, opparam), 0);
    EVP_PKEY_free(auth);
    return ret;
}
...
static int test_auth_key_type_mismatch(int tstid) {
    ...
    return TEST_int_eq(
    EVP_PKEY_auth_encapsulate_init(rctx[id1], rkey[id2], opparam), 0);
}
...
static int test_ec_public_key_infinity(void) {
    ...
    ret = TEST_ptr(key = new_raw_private_key(t->curve, t->rpriv, t->rprivlen,
          ...
          TEST_int_eq(EVP_PKEY_decapsulate(keyctx, s, &slen, t->expected_enc,
                                           t->expected_enclen),
                      0)
...
static int test_null_params(int tstid) {
    ...
    return TEST_int_eq(EVP_PKEY_auth_encapsulate_init(ctx, NULL, opparam), 0) &&
           ...
           TEST_int_eq(EVP_PKEY_decapsulate(ctx, NULL, NULL, t->expected_enc,
                                            t->expected_enclen),
                       0)
...
static int test_set_params(int tstid) {
    ...
    badparams[0] =
    OSSL_PARAM_construct_utf8_string(OSSL_KEM_PARAM_OPERATION, "unknown_op", 0);
    badparams[1] = OSSL_PARAM_construct_end();
    if (!TEST_int_eq(EVP_PKEY_encapsulate_init(ctx, badparams), 0))
        goto err;
    ...
    badparams[0] =
    OSSL_PARAM_construct_utf8_string(OSSL_KEM_PARAM_OPERATION, NULL, 0);
    badparams[1] = OSSL_PARAM_construct_end();
    if (!TEST_int_eq(EVP_PKEY_encapsulate_init(ctx, badparams), 0))
        goto err;
    ...
    badparams[0] =
    OSSL_PARAM_construct_octet_string(OSSL_KEM_PARAM_IKME, NULL, 0);
    badparams[1] = OSSL_PARAM_construct_end();
...
static int test_nopublic(int tstid) {
    ...
    if (!TEST_ptr(
        priv = new_raw_private_key(t->curve, t->rpriv, t->rprivlen, NULL, 0)))
        goto err;
...
static int test_noauthpublic(int tstid) {
    ...
    if (encap) {
        if (!TEST_int_eq(EVP_PKEY_auth_encapsulate_init(ctx, auth, opparam),
                         expected))
            goto err;
    } else {
        if (!TEST_int_eq(EVP_PKEY_auth_decapsulate_init(ctx, auth, opparam),
                         expected))
            goto err;
    }
    if (expected == 0 &&
...
static int test_ec_dhkem_derivekey(int tstid) {
    ...
    ret =
    TEST_ptr(genctx = EVP_PKEY_CTX_new_from_name(libctx, "EC", NULL)) &&
    TEST_int_eq(EVP_PKEY_keygen_init(genctx), 1) &&
    TEST_int_eq(EVP_PKEY_CTX_set_params(genctx, params), 1) &&
...
static int test_ec_noikme(int tstid) {
    ...
    if (curve[0] == 'X') {
        if (!TEST_ptr(recip = EVP_PKEY_Q_keygen(libctx, NULL, curve)) ||
            (auth &&
             !TEST_ptr(sender_auth = EVP_PKEY_Q_keygen(libctx, NULL, curve))))
            goto err;
    } else {
        if (!TEST_ptr(recip = EVP_PKEY_Q_keygen(libctx, NULL, "EC", curve)) ||
            (auth && !TEST_ptr(sender_auth =
                               EVP_PKEY_Q_keygen(libctx, NULL, "EC", curve))))
            goto err;
    }

    ret =
    TEST_ptr(ctx = EVP_PKEY_CTX_new_from_pkey(libctx, recip, NULL)) &&
    (sender_auth == NULL ||
     TEST_int_eq(EVP_PKEY_auth_encapsulate_init(ctx, sender_auth, NULL), 1)) &&
    (sender_auth != NULL ||
     TEST_int_eq(EVP_PKEY_encapsulate_init(ctx, NULL), 1)) &&
    ...
    TEST_int_eq(EVP_PKEY_encapsulate(ctx, sender_pub, &sender_publen,
                ...
                1) &&
    (sender_auth == NULL ||
     TEST_int_eq(EVP_PKEY_auth_decapsulate_init(ctx, sender_auth, NULL), 1)) &&
    (sender_auth != NULL ||
     TEST_int_eq(EVP_PKEY_decapsulate_init(ctx, NULL), 1)) &&
    TEST_int_eq(EVP_PKEY_CTX_set_kem_op(ctx, op), 1) &&
...
static int test_ec_badpublic(int tstid) {
    ...
    pub[0] = 0;
    if (!TEST_ptr(recippriv =
                  new_raw_private_key(t->curve, t->rpriv, t->rprivlen, pub, 1)))
        goto err;
...
static int test_ec_invalid_decap_enc_buffer(void) {
    ...
    return TEST_int_eq(EVP_PKEY_decapsulate_init(ctx, opparam), 1) &&
           TEST_int_eq(EVP_PKEY_decapsulate(ctx, secret, &secretlen, enc,
                                            t->expected_enclen),
                       0);
}
...
static int test_ecx_dhkem_derivekey(int tstid) {
    ...
    if (OPENSSL_strcasecmp(t->curvename, "X25519") == 0) {
        /*
         * The RFC test vector seems incorrect since it is not in serialized
         * form, So manually do the conversion here for now.
         */
        masked_priv[0] &= 248;
    ...
    ret =
    TEST_ptr(genctx = EVP_PKEY_CTX_new_from_name(libctx, t->curvename, NULL)) &&
    TEST_int_eq(EVP_PKEY_keygen_init(genctx), 1) &&
    TEST_int_eq(EVP_PKEY_CTX_set_params(genctx, params), 1) &&
    ...
    EVP_PKEY_get_octet_string_param(pkey, OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY,
                                    pubkey, sizeof(pubkey), &pubkeylen),
    1) &&
    TEST_int_eq(EVP_PKEY_get_octet_string_param(pkey, OSSL_PKEY_PARAM_PRIV_KEY,
...
int setup_tests(void) {
    ...
    if (!test_get_libctx(&libctx, &nullprov, config_file, &libprov, prov_name))
        return 0;
    opparam[0] =
    OSSL_PARAM_construct_utf8_string(OSSL_KEM_PARAM_OPERATION, op, 0);
    opparam[1] = OSSL_PARAM_construct_end();

    /* Create P256 and X25519 keys and ctxs */
    if (!TEST_ptr(rkey[TEST_KEYTYPE_P256] =
                  EVP_PKEY_Q_keygen(libctx, NULL, "EC", "P-256")))
        goto err;
#ifndef OPENSSL_NO_ECX
    if (!TEST_ptr(rkey[TEST_KEYTYPE_X25519] =
                  EVP_PKEY_Q_keygen(libctx, NULL, "X25519")))
        goto err;
#endif
    if (!TEST_ptr(rctx[TEST_KEYTYPE_P256] = EVP_PKEY_CTX_new_from_pkey(
                  libctx, rkey[TEST_KEYTYPE_P256], NULL)))
        goto err;
#ifndef OPENSSL_NO_ECX
    if (!TEST_ptr(rctx[TEST_KEYTYPE_X25519] = EVP_PKEY_CTX_new_from_pkey(
                  libctx, rkey[TEST_KEYTYPE_X25519], NULL)))
        goto err;
#endif
