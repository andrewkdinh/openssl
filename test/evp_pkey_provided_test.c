...
static int test_fromdata_dh_named_group(void)
{
    ...
    if (!TEST_true(EVP_PKEY_get_utf8_string_param(
        ...
        || !TEST_false(EVP_PKEY_get_utf8_string_param(
            pk, OSSL_PKEY_PARAM_GROUP_NAME, name_out, sizeof(group_name) - 1,
            &len))
    ...
    for (;;) {
        ...
        if (!TEST_true(EVP_PKEY_get_utf8_string_param(
                pk, OSSL_PKEY_PARAM_GROUP_NAME, name_out, sizeof(name_out),
                &len))
        ...
        if (tst <= 7) {
            ...
            if (!TEST_true(EVP_PKEY_get_octet_string_param(
                ...
                || !TEST_true(EVP_PKEY_get_octet_string_param(
                    pk, orig_fromdata_params[PUB_KEY].key, out_pub,
                    sizeof(out_pub), &len))
...
static int test_fromdata_dsa_fips186_4(void)
{
    ...
    if (!TEST_ptr(bld = OSSL_PARAM_BLD_new())
        ...
        || !TEST_true(OSSL_PARAM_BLD_push_int(bld, OSSL_PKEY_PARAM_FFC_PCOUNTER,
                                              pcounter))
        || !TEST_true(OSSL_PARAM_BLD_push_BN(bld, OSSL_PKEY_PARAM_PUB_KEY, pub))
        || !TEST_true(
            OSSL_PARAM_BLD_push_BN(bld, OSSL_PKEY_PARAM_PRIV_KEY, priv))
                                            ...
                                            size_t keylen, char *salt)
{
    ...
    *p++ = OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_MODE, "EXTRACT_ONLY",
                                            0);
