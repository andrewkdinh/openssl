...
static const OSSL_DISPATCH obj_dispatch_table[] = {{OSSL_FUNC_PROVIDER_QUERY_OPERATION, (void (*)(void))obj_query},
                                                   OSSL_DISPATCH_END};
...
static int obj_provider_init(const OSSL_CORE_HANDLE *handle, const OSSL_DISPATCH *in, const OSSL_DISPATCH **out,
                             void **provctx)
{
    *provctx = (void *)handle;
    *out = obj_dispatch_table;
    ...
    if (!c_obj_create(handle, DIGEST_OID, DIGEST_SN, DIGEST_LN) || !c_obj_create(handle, SIG_OID, SIG_SN, SIG_LN)
...
static int obj_create_test(void)
{
    ...
    if (!TEST_true(OSSL_PROVIDER_add_builtin(libctx, "obj-prov", obj_provider_init))
    ...
    if (!TEST_int_ne(sigalgnid, NID_undef) || !TEST_true(OBJ_find_sigid_algs(sigalgnid, &digestnid, &signid))
        || !TEST_int_ne(digestnid, NID_undef) || !TEST_int_ne(signid, NID_undef)
        || !TEST_int_eq(digestnid, OBJ_sn2nid(DIGEST_SN)) || !TEST_int_eq(signid, OBJ_ln2nid(SIG_LN)))
        goto err;

    /* Check empty digest alg storage capability */
    sigalgnid = OBJ_txt2nid(NODIG_SIGALG_OID);
    if (!TEST_int_ne(sigalgnid, NID_undef) || !TEST_true(OBJ_find_sigid_algs(sigalgnid, &digestnid, &signid))
        || !TEST_int_eq(digestnid, NID_undef) || !TEST_int_ne(signid, NID_undef))
        goto err;
