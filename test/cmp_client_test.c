...
static CMP_SES_TEST_FIXTURE *set_up(const char *const test_case_name)
{
    ...
    if (!TEST_ptr(fixture->srv_ctx = ossl_cmp_mock_srv_new(libctx, NULL))
        ...
        || !OSSL_CMP_CTX_set1_cert(srv_cmp_ctx, server_cert) || !OSSL_CMP_CTX_set1_pkey(srv_cmp_ctx, server_key))
        goto err;
    if (!TEST_ptr(fixture->cmp_ctx = ctx = OSSL_CMP_CTX_new(libctx, NULL))
        ...
        || !OSSL_CMP_CTX_set1_srvCert(ctx, server_cert) || !OSSL_CMP_CTX_set1_referenceValue(ctx, ref, sizeof(ref)))
        goto err;
    fixture->req_type = -1;
    return fixture;
...
{
    return TEST_int_eq(OSSL_CMP_CTX_get_status(fixt->cmp_ctx), OSSL_CMP_PKISTATUS_unspecified)
        && TEST_int_eq(OSSL_CMP_exec_RR_ses(fixt->cmp_ctx), fixt->expected == OSSL_CMP_PKISTATUS_accepted)
...
{
    return execute_exec_GENM_ses_test_single(fixture) && OSSL_CMP_CTX_reinit(fixture->cmp_ctx)
...
static int execute_exec_certrequest_ses_test(CMP_SES_TEST_FIXTURE *fixture)
{
    ...
    if (!TEST_int_eq(status, fixture->expected)
        && !(fixture->expected == OSSL_CMP_PKISTATUS_waiting && TEST_int_eq(status, OSSL_CMP_PKISTATUS_trans)))
        return 0;
...
static int test_exec_RR_ses(int request_error)
{
    ...
    fixture->expected = request_error ? OSSL_CMP_PKISTATUS_request : OSSL_CMP_PKISTATUS_accepted;
...
{
    SETUP_TEST_FIXTURE(CMP_SES_TEST_FIXTURE, set_up);
    ossl_cmp_mock_srv_set_statusInfo(fixture->srv_ctx, OSSL_CMP_PKISTATUS_rejection,
                                     OSSL_CMP_CTX_FAILINFO_signerNotTrusted, "test string");
...
static int test_exec_IR_ses(void)
{
    ...
    if (!sk_X509_push(fixture->caPubs, server_cert) || !sk_X509_push(fixture->caPubs, server_cert)) {
        tear_down(fixture);
        return 0;
    }
...
static int test_exec_REQ_ses_poll(int req_type, int check_after, int poll_count, int total_timeout, int expect)
{
    SETUP_TEST_FIXTURE(CMP_SES_TEST_FIXTURE, set_up);
    ...
    OSSL_CMP_CTX_set_option(fixture->cmp_ctx, OSSL_CMP_OPT_TOTAL_TIMEOUT, total_timeout);
...
{
    return test_exec_REQ_ses_poll(OSSL_CMP_PKIBODY_IR, checkAfter, 2, 0, OSSL_CMP_PKISTATUS_accepted);
}

static int test_exec_IR_ses_poll_no_timeout(void)
{
    return test_exec_REQ_ses_poll(OSSL_CMP_PKIBODY_IR, checkAfter, 2 /* pollCount */,
...
{
    return !test_exec_REQ_ses_poll(OSSL_CMP_PKIBODY_IR, checkAfter + 1, 3 /* pollCount */, checkAfter + 6,
...
static int test_exec_CR_ses(int implicit_confirm, int granted, int reject)
{
    ...
    OSSL_CMP_CTX_set_option(fixture->cmp_ctx, OSSL_CMP_OPT_IMPLICIT_CONFIRM, implicit_confirm);
    OSSL_CMP_SRV_CTX_set_grant_implicit_confirm(fixture->srv_ctx, granted);
    ossl_cmp_mock_srv_set_sendError(fixture->srv_ctx, reject ? OSSL_CMP_PKIBODY_CERTCONF : -1);
    fixture->expected = reject ? OSSL_CMP_PKISTATUS_rejection : OSSL_CMP_PKISTATUS_accepted;
    EXECUTE_TEST(execute_exec_certrequest_ses_test, tear_down);
    return result;
}
...
{
    return test_exec_CR_ses(0, 0, 0) && test_exec_CR_ses(0, 0, 1 /* reject */);
}

static int test_exec_CR_ses_implicit_confirm(void)
{
    return test_exec_CR_ses(1, 0, 0) && test_exec_CR_ses(1, 1 /* granted */, 0);
}
...
static int test_exec_KUR_ses(int transfer_error, int pubkey, int raverified)
{
    ...
    if (pubkey || raverified)
        OSSL_CMP_CTX_set_option(fixture->cmp_ctx, OSSL_CMP_OPT_POPO_METHOD, OSSL_CRMF_POPO_RAVERIFIED);
...
static int test_certConf_cb(OSSL_CMP_CTX *ctx, X509 *cert, int fail_info, const char **txt)
{
...
static int test_exec_P10CR_ses(int reject)
{
    ...
    fixture->expected = reject ? OSSL_CMP_PKISTATUS_rejection : OSSL_CMP_PKISTATUS_accepted;
    ctx = fixture->cmp_ctx;
    if (!TEST_ptr(csr = load_csr_der(pkcs10_f, libctx)) || !TEST_true(OSSL_CMP_CTX_set1_p10CSR(ctx, csr))
...
static int execute_try_certreq_poll_test(CMP_SES_TEST_FIXTURE *fixture)
{
    ...
    ossl_cmp_mock_srv_set_checkAfterTime(fixture->srv_ctx, CHECK_AFTER);
    return TEST_int_eq(-1, OSSL_CMP_try_certreq(ctx, TYPE, NULL, &check_after)) && check_after == CHECK_AFTER
        && TEST_ptr_eq(OSSL_CMP_CTX_get0_newCert(ctx), NULL)
        && TEST_int_eq(-1, OSSL_CMP_try_certreq(ctx, TYPE, NULL, &check_after)) && check_after == CHECK_AFTER
        && TEST_ptr_eq(OSSL_CMP_CTX_get0_newCert(ctx), NULL)
        && TEST_int_eq(fixture->expected, OSSL_CMP_try_certreq(ctx, TYPE, NULL, NULL))
...
static int execute_try_certreq_poll_abort_test(CMP_SES_TEST_FIXTURE *fixture)
{
    ...
    return TEST_int_eq(-1, OSSL_CMP_try_certreq(ctx, TYPE, NULL, &check_after)) && check_after == CHECK_AFTER
        && TEST_ptr_eq(OSSL_CMP_CTX_get0_newCert(ctx), NULL)
        && TEST_int_eq(fixture->expected, OSSL_CMP_try_certreq(ctx, -1 /* abort */, NULL, NULL))
...
{
    return test_exec_REQ_ses_poll(OSSL_CMP_PKIBODY_GENM, checkAfter, 2, 0, OSSL_CMP_PKISTATUS_accepted);
}

static int test_exec_GENM_ses_poll_no_timeout(void)
{
    return test_exec_REQ_ses_poll(OSSL_CMP_PKIBODY_GENM, checkAfter, 1 /* pollCount */, checkAfter + 1,
                                  OSSL_CMP_PKISTATUS_accepted);
}

static int test_exec_GENM_ses_poll_total_timeout(void)
{
    return test_exec_REQ_ses_poll(OSSL_CMP_PKIBODY_GENM, checkAfter + 1, 3 /* pollCount */, checkAfter + 2,
                                  OSSL_CMP_PKISTATUS_waiting);
}
...
static int execute_exchange_error_test(CMP_SES_TEST_FIXTURE *fixture)
{
    int res =
        ossl_cmp_exchange_error(fixture->cmp_ctx, OSSL_CMP_PKISTATUS_rejection,
                                1 << OSSL_CMP_PKIFAILUREINFO_unsupportedVersion, "foo_status", 999, "foo_details");
...
int setup_tests(void)
{
    ...
    if (!TEST_ptr(server_key_f = test_get_argument(0)) || !TEST_ptr(server_cert_f = test_get_argument(1))
        || !TEST_ptr(client_key_f = test_get_argument(2)) || !TEST_ptr(client_cert_f = test_get_argument(3))
