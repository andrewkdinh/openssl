...
static int test_exec_RR_ses(int request_error) {
    ...
    fixture->expected =
    request_error ? OSSL_CMP_PKISTATUS_request : OSSL_CMP_PKISTATUS_accepted;
    EXECUTE_TEST(execute_exec_RR_ses_test, tear_down);
    return result;
}

static int test_exec_RR_ses_ok(void) { return test_exec_RR_ses(0); }

static int test_exec_RR_ses_request_error(void) { return test_exec_RR_ses(1); }

static int test_exec_RR_ses_receive_error(void) {
    SETUP_TEST_FIXTURE(CMP_SES_TEST_FIXTURE, set_up);
    ossl_cmp_mock_srv_set_statusInfo(
    fixture->srv_ctx, OSSL_CMP_PKISTATUS_rejection,
...
static int test_exec_CR_ses(int implicit_confirm, int granted, int reject) {
    ...
    fixture->expected =
    reject ? OSSL_CMP_PKISTATUS_rejection : OSSL_CMP_PKISTATUS_accepted;
    EXECUTE_TEST(execute_exec_certrequest_ses_test, tear_down);
    return result;
}
...
static int test_exec_KUR_ses_ok(void) { return test_exec_KUR_ses(0, 0, 0); }
...
static int test_exec_KUR_ses_pub(void) { return test_exec_KUR_ses(0, 1, 0); }
...
static int test_exec_P10CR_ses_ok(void) { return test_exec_P10CR_ses(0); }

static int test_exec_P10CR_ses_reject(void) { return test_exec_P10CR_ses(1); }
...
static int execute_try_certreq_poll_test(CMP_SES_TEST_FIXTURE *fixture) {
    ...
    ossl_cmp_mock_srv_set_checkAfterTime(fixture->srv_ctx, CHECK_AFTER);
    return TEST_int_eq(-1,
                       OSSL_CMP_try_certreq(ctx, TYPE, NULL, &check_after)) &&
...
static int test_exec_GENM_ses(int transfer_error, int total_timeout,
                              int expect) {
    SETUP_TEST_FIXTURE(CMP_SES_TEST_FIXTURE, set_up);
    if (transfer_error)
        OSSL_CMP_CTX_set_transfer_cb_arg(fixture->cmp_ctx, NULL);
...
#define USAGE                                                                  \
    "server.key server.crt client.key client.crt client.csr module_name "      \
    "[module_conf_file]\n"
