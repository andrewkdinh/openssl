...
static int test_bio_callback_ex(void) {
  ...
  if (!TEST_mem_eq(buf, i, test1, test1len) ||
      !TEST_int_eq(my_param_count, 2) || !TEST_ptr_eq(my_param_b[0], bio) ||
...
static int test_bio_callback(void) {
  ...
  if (!TEST_int_eq(i, 0) || !TEST_int_eq(my_param_count, 2) ||
      ...
      !TEST_ptr_eq(my_param_argp[1], buf) ||
      !TEST_int_eq(my_param_argi[1], sizeof(buf)) ||
      !TEST_long_eq(my_param_argl[1], 0L) || !TEST_long_eq(my_param_ret[1], 0L))
    goto err;
  ...
  if (!TEST_int_eq(i, 1) || !TEST_int_eq(my_param_count, 1) ||
      ...
      !TEST_ptr_eq(my_param_argp[0], NULL) ||
      !TEST_int_eq(my_param_argi[0], 0) ||
      !TEST_long_eq(my_param_argl[0], 0L) || !TEST_long_eq(my_param_ret[0], 1L))
    goto finish;
