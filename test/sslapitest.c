...
int tls_provider_init(const OSSL_CORE_HANDLE *handle, const OSSL_DISPATCH *in,
...
static int compare_hex_encoded_buffer(const char *hex_encoded,
                                      size_t hex_length, const uint8_t *raw,
                                      size_t raw_length) {
  size_t i, j;
  char hexed[3];
...
static int test_keylog_output(char *buffer, const SSL *ssl,
  ...
  for (token = strtok(buffer, " \n"); token != NULL;
    ...
    if (strcmp(token, "RSA") == 0) {
       ...
       */
      client_random_size =
      SSL_get_client_random(ssl, actual_client_random, SSL3_RANDOM_SIZE);
      ...
      if (!TEST_false(compare_hex_encoded_buffer(
          token, 64, actual_client_random, client_random_size)))
        return 0;

      if (!TEST_ptr(token = strtok(NULL, " \n")))
        return 0;
      master_key_size =
      SSL_SESSION_get_master_key(session, actual_master_key, master_key_size);
...
static int test_keylog_no_master_key(void) {
  ...
  if (!TEST_true(
      ...
      !TEST_int_eq(SSL_read_early_data(serverssl, buf, sizeof(buf), &readbytes),
...
static int test_client_cert_verify_cb(void) {
  ...
  if (!TEST_int_eq(SSL_CTX_use_certificate_chain_file(sctx, leaf), 1) ||
      !TEST_int_eq(SSL_CTX_use_PrivateKey_file(sctx, skey, SSL_FILETYPE_PEM),
                   1) ||
  ...
  if (!TEST_false(
      create_ssl_connection(serverssl, clientssl, SSL_ERROR_WANT_RETRY_VERIFY)))
    goto end;
...
static int test_ssl_ctx_build_cert_chain(void) {
  ...
  if (!TEST_int_eq(SSL_CTX_use_certificate_chain_file(ctx, leaf_chain), 1) ||
      !TEST_int_eq(SSL_CTX_use_PrivateKey_file(ctx, skey, SSL_FILETYPE_PEM),
                   1) ||
...
static int full_client_hello_callback(SSL *s, int *al, void *arg) {
  ...
  if (!TEST_mem_eq(p, len, expected_ciphers, sizeof(expected_ciphers)) ||
      !TEST_size_t_eq(SSL_client_hello_get0_compression_methods(s, &p), 1) ||
      !TEST_int_eq(*p, 0))
...
static int test_client_hello_cb(void) {
  ...
  if (!TEST_true(SSL_CTX_set_cipher_list(
      cctx, "AES256-GCM-SHA384:ECDHE-ECDSA-AES256-GCM-SHA384")) ||
      !TEST_true(
      create_ssl_objects(sctx, cctx, &serverssl, &clientssl, NULL, NULL)) ||
...
static int test_no_ems(void) {
  ...
  if (!create_ssl_ctx_pair(libctx, TLS_server_method(), TLS_client_method(),
                           TLS1_VERSION, TLS1_2_VERSION, &sctx, &cctx, cert,
                           privkey)) {
  ...
  if (fips_ems_check) {
    if (status == 1) {
      printf("When FIPS uses the EMS check a connection that doesn't use EMS "
             "should fail\n");
      goto end;
    }
  } else {
...
static int test_ccs_change_cipher(void) {
  ...
  if (!TEST_true(create_ssl_ctx_pair(
      libctx, TLS_server_method(), TLS_client_method(), TLS1_VERSION,
  ...
  if (!TEST_true(
      ...
      !TEST_true(SSL_set_session(clientssl, sess)) ||
      !TEST_true(
      SSL_set_cipher_list(clientssl, "AES256-GCM-SHA384:AES128-GCM-SHA256")) ||
      !TEST_true(create_ssl_connection(serverssl, clientssl, SSL_ERROR_NONE)) ||
      !TEST_true(SSL_session_reused(clientssl)) ||
...
static int execute_test_large_message(const SSL_METHOD *smeth,
                                      const SSL_METHOD *cmeth, int min_version,
...
static int ping_pong_query(SSL *clientssl, SSL *serverssl) {
  ...
  if (!BIO_get_ktls_send(clientsc->wbio)) {
    if (!TEST_mem_ne(crec_wseq_before, SEQ_NUM_SIZE, crec_wseq_after,
                     SEQ_NUM_SIZE))
      goto end;
  } else {
    if (!TEST_mem_eq(crec_wseq_before, SEQ_NUM_SIZE, crec_wseq_after,
                     SEQ_NUM_SIZE))
      goto end;
  }

  if (!BIO_get_ktls_send(serversc->wbio)) {
    if (!TEST_mem_ne(srec_wseq_before, SEQ_NUM_SIZE, srec_wseq_after,
                     SEQ_NUM_SIZE))
      goto end;
  } else {
    if (!TEST_mem_eq(srec_wseq_before, SEQ_NUM_SIZE, srec_wseq_after,
                     SEQ_NUM_SIZE))
      goto end;
  }

  if (!BIO_get_ktls_recv(clientsc->wbio)) {
    if (!TEST_mem_ne(crec_rseq_before, SEQ_NUM_SIZE, crec_rseq_after,
                     SEQ_NUM_SIZE))
      goto end;
  } else {
    if (!TEST_mem_eq(crec_rseq_before, SEQ_NUM_SIZE, crec_rseq_after,
                     SEQ_NUM_SIZE))
      goto end;
  }

  if (!BIO_get_ktls_recv(serversc->wbio)) {
    if (!TEST_mem_ne(srec_rseq_before, SEQ_NUM_SIZE, srec_rseq_after,
                     SEQ_NUM_SIZE))
      goto end;
  } else {
    if (!TEST_mem_eq(srec_rseq_before, SEQ_NUM_SIZE, srec_rseq_after,
                     SEQ_NUM_SIZE))
      goto end;
  }
...
static int execute_test_ktls(int cis_ktls, int sis_ktls, int tls_version,
                             const char *cipher) {
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
  ...
  if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(),
                                     TLS_client_method(), tls_version,
                                     tls_version, &sctx, &cctx, cert, privkey)))
    goto end;
  ...
  if (!TEST_true(
      create_ssl_objects2(sctx, cctx, &serverssl, &clientssl, sfd, cfd)))
    goto end;
...
static int execute_test_ktls_sendfile(int tls_version, const char *cipher,
  ...
  if (zerocopy) {
    if (!TEST_true(
        SSL_set_options(serverssl, SSL_OP_ENABLE_KTLS_TX_ZEROCOPY_SENDFILE)))
      goto end;
  }
  ...
  while (chunk_off < SENDFILE_SZ) {
    ...
    while ((err = SSL_sendfile(serverssl, ffd, chunk_off, chunk_size, 0)) !=
           chunk_size) {
      if (SSL_get_error(serverssl, err) != SSL_ERROR_WANT_WRITE)
        goto end;
    }
    while ((err = SSL_read(clientssl, buf_dst + chunk_off, chunk_size)) !=
           chunk_size) {
      if (SSL_get_error(clientssl, err) != SSL_ERROR_WANT_READ)
        goto end;
    }
    ...
    /* verify the payload */
    if (!TEST_mem_eq(buf_dst + chunk_off, chunk_size, buf + chunk_off,
                     chunk_size))
      goto end;
...
static int test_large_message_dtls(void) {
   ...
   * read_ahead is set.
   */
  return execute_test_large_message(DTLS_server_method(), DTLS_client_method(),
                                    DTLS1_VERSION, 0, 0);
}
#endif
...
static int test_large_app_data(int tst) {
  ...
  if (!TEST_true(create_ssl_ctx_pair(libctx, smeth, cmeth, prot, prot, &sctx,
                                     &cctx, cert, privkey)))
    goto end;
  ...
  if ((tst & 1) != 0) {
    ...
    if (!TEST_true(
        SSL_set_options(serverssl, SSL_OP_DONT_INSERT_EMPTY_FRAGMENTS)) ||
        !TEST_true(
        SSL_set_options(clientssl, SSL_OP_DONT_INSERT_EMPTY_FRAGMENTS)))
      goto end;
  }
  ...
  if (!TEST_true(
      SSL_write_ex(clientssl, msg, SSL3_RT_MAX_PLAIN_LENGTH, &written)) ||
      !TEST_size_t_eq(written, SSL3_RT_MAX_PLAIN_LENGTH))
    goto end;

  /* We provide a buffer slightly larger than what we are actually expecting */
  if (!TEST_true(
      SSL_read_ex(serverssl, buf, SSL3_RT_MAX_PLAIN_LENGTH + 1, &readbytes)))
    goto end;
...
static int test_cleanse_plaintext(void) {
  ...
  if (!TEST_true(execute_cleanse_plaintext(TLS_server_method(),
                                           TLS_client_method(), TLS1_2_VERSION,
                                           TLS1_2_VERSION)))
    return 0;
  ...
  if (!TEST_true(execute_cleanse_plaintext(
      DTLS_server_method(), DTLS_client_method(), DTLS1_VERSION, 0)))
    return 0;
#endif
  return 1;
}
...
static int test_tlsext_status_type(void) {
  ...
  if (!TEST_true(
      ...
      !TEST_false(ocsp_client_called) || !TEST_false(ocsp_server_called))
    goto end;
  SSL_free(serverssl);
  SSL_free(clientssl);
  serverssl = NULL;
...
static int execute_test_session(int maxprot, int use_int_cache,
  ...
  if (!TEST_true(
      create_ssl_objects(sctx, cctx, &serverssl1, &clientssl1, NULL, NULL)) ||
      !TEST_true(
      create_ssl_connection(serverssl1, clientssl1, SSL_ERROR_NONE)) ||
  ...
  if (use_int_cache && !TEST_false(SSL_CTX_add_session(cctx, sess1)))
    goto end;
  if (use_ext_cache && (!TEST_int_eq(new_called, numnewsesstick)
  ...
  new_called = remove_called = 0;
  if (!TEST_true(
      create_ssl_objects(sctx, cctx, &serverssl2, &clientssl2, NULL, NULL)) ||
      !TEST_true(SSL_set_session(clientssl2, sess1)) ||
      !TEST_true(
      create_ssl_connection(serverssl2, clientssl2, SSL_ERROR_NONE)) ||
  ...
  if (maxprot == TLS1_3_VERSION) {
     ...
     */
    if (use_ext_cache &&
        (!TEST_int_eq(new_called, 1) || !TEST_int_eq(remove_called, 1)))
      goto end;
  } else {
    /*
     ...
     */
    if (use_ext_cache &&
        (!TEST_int_eq(new_called, 0) || !TEST_int_eq(remove_called, 0)))
      goto end;
  }
  ...
  if (!TEST_true(
      create_ssl_objects(sctx, cctx, &serverssl3, &clientssl3, NULL, NULL)) ||
      !TEST_true(SSL_set_session(clientssl3, sess1))
      /* This should fail because of the mismatched protocol versions */
      || !TEST_false(
         create_ssl_connection(serverssl3, clientssl3, SSL_ERROR_NONE)))
    goto end;
  ...
  if (use_ext_cache) {
    ...
    if (!TEST_int_eq(new_called, numnewsesstick) ||
        !TEST_int_eq(remove_called, 0) || !TEST_int_eq(get_called, 0))
      goto end;
    /*
    ...
    if (use_int_cache && maxprot != TLS1_3_VERSION) {
      if (!TEST_ptr(tmp = SSL_SESSION_dup(sess2)) ||
          !TEST_true(sess2->owner != NULL) || !TEST_true(tmp->owner == NULL) ||
  ...
  if (!TEST_true(SSL_CTX_add_session(sctx, sess1)) || !TEST_ptr(sess1->owner) ||
...
static int setup_ticket_test(int stateful, int idx, SSL_CTX **sctx,
  ...
  if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(),
                                     TLS_client_method(), TLS1_VERSION, 0, sctx,
                                     cctx, cert, privkey)) ||
      !TEST_true(SSL_CTX_set_num_tickets(*sctx, idx)) ||
      !TEST_true(SSL_CTX_set_session_id_context(*sctx, (void *)&sess_id_ctx,
                                                sizeof(sess_id_ctx))))
    return 0;
...
static int check_resumption(int idx, SSL_CTX *sctx, SSL_CTX *cctx, int succ) {
  ...
  for (i = 0; i < idx * 2; i++) {
    ...
    if (succ && (!post_handshake_verify(serverssl, clientssl) ||
                 !TEST_int_eq(new_called, 1)))
      goto end;
...
static int test_stateless_tickets(int idx) { return test_tickets(0, idx); }

static int test_stateful_tickets(int idx) { return test_tickets(1, idx); }
...
static int test_extra_tickets(int idx) {
  ...
  if (!TEST_true(SSL_write_ex(serverssl, &c, 1, &nbytes)) ||
      !TEST_size_t_eq(1, nbytes) || !TEST_int_eq(idx * 2 + 2, new_called) ||
  ...
  if (!TEST_true(SSL_new_session_ticket(serverssl)) ||
      ...
      !TEST_true(SSL_read_ex(clientssl, buf, sizeof(buf), &nbytes)) ||
      !TEST_int_eq(2, new_called) || !TEST_size_t_eq(sizeof(buf), nbytes) ||
      !TEST_int_eq(c, buf[0]))
    goto end;
  ...
  if (!TEST_true(SSL_new_session_ticket(serverssl)) ||
      ...
      !TEST_size_t_eq(0, nbytes) || !TEST_int_eq(2, new_called) ||
      !TEST_false(SSL_read_ex(clientssl, buf, sizeof(buf), &nbytes)) ||
  ...
  if (!TEST_true(SSL_new_session_ticket(serverssl)) ||
      !TEST_true(SSL_new_session_ticket(serverssl)) ||
      !TEST_int_eq(0, new_called) || !TEST_true(SSL_do_handshake(serverssl)) ||
      !TEST_int_eq(0, new_called))
    goto end;
  /* Re-do the write; still no tickets sent */
  if (!TEST_true(SSL_write_ex(serverssl, &c, 1, &nbytes)) ||
      !TEST_size_t_eq(1, nbytes) || !TEST_int_eq(0, new_called) ||
      !TEST_true(SSL_read_ex(clientssl, buf, sizeof(buf), &nbytes)) ||
      !TEST_int_eq(0, new_called) || !TEST_int_eq(sizeof(buf), nbytes) ||
      !TEST_int_eq(c, buf[0]) ||
      !TEST_false(SSL_read_ex(clientssl, buf, sizeof(buf), &nbytes)))
    goto end;
  ...
  c = '6';
  if (!TEST_true(SSL_write_ex(serverssl, &c, 1, &nbytes)) ||
      !TEST_size_t_eq(1, nbytes) || !TEST_int_eq(2, new_called) ||
      !TEST_true(SSL_read_ex(clientssl, buf, sizeof(buf), &nbytes)) ||
      !TEST_int_eq(4, new_called) || !TEST_int_eq(sizeof(buf), nbytes) ||
      !TEST_int_eq(c, buf[0]) ||
      !TEST_false(SSL_read_ex(clientssl, buf, sizeof(buf), &nbytes)))
    goto end;
...
#define TOTAL_SSL_SET_BIO_TESTS                                                \
  ...
  +TOTAL_CONN_SUCCESS_SSL_SET_BIO_TESTS + TOTAL_CONN_FAIL_SSL_SET_BIO_TESTS
...
#ifndef OPENSSL_NO_EC
static const int validlist2[] = {NID_sha256, EVP_PKEY_RSA, NID_sha512,
                                 EVP_PKEY_EC};
...
static int test_set_sigalgs(int idx) {
  ...
  curr =
  testctx ? &testsigalgs[idx] : &testsigalgs[idx - OSSL_NELEM(testsigalgs)];
  ...
  if (!TEST_int_eq(create_ssl_connection(serverssl, clientssl, SSL_ERROR_NONE),
                   curr->connsuccess))
    goto end;
...
static int test_early_data_read_write(int idx) {
  ...
  timer = ossl_time_now();
  if (!TEST_true(
      SSL_write_early_data(clientssl, MSG1, strlen(MSG1), &written)) ||
      !TEST_size_t_eq(written, strlen(MSG1)))
    goto end;

  if (!TEST_int_eq(SSL_read_early_data(serverssl, buf, sizeof(buf), &readbytes),
   ...
   */
  if (!TEST_true(
      SSL_write_early_data(serverssl, MSG2, strlen(MSG2), &written)) ||
  ...
  if (!TEST_true(
      SSL_write_early_data(clientssl, MSG3, strlen(MSG3), &written)) ||
  ...
  if (!TEST_true(
      SSL_write_early_data(serverssl, MSG4, strlen(MSG4), &written)) ||
   ...
   */
  if (!TEST_true(
      SSL_write_early_data(serverssl, MSG6, strlen(MSG6), &written)) ||
      !TEST_size_t_eq(written, strlen(MSG6)))
    goto end;

  /* Push the ClientFinished and the normal data back into the server rbio */
  if (!TEST_true(
      BIO_write_ex(rbio, data + eoedlen, rawread - eoedlen, &rawwritten)) ||
  ...
  if (!TEST_false(
      SSL_write_early_data(clientssl, MSG6, strlen(MSG6), &written)))
    goto end;
  ERR_clear_error();
  if (!TEST_int_eq(SSL_read_early_data(serverssl, buf, sizeof(buf), &readbytes),
                   SSL_READ_EARLY_DATA_ERROR))
...
static int early_data_skip_helper(int testtype, int cipher, int idx) {
  ...
  if (!TEST_true(setupearly_data_test(
      &cctx, &sctx, &clientssl, &serverssl, &sess, idx,
      (cipher == 2 || cipher == 6) ? SHA384_DIGEST_LENGTH
  ...
  case 2: {
     ...
     */
    if (!TEST_true(
        BIO_write_ex(wbio, bad_early_data, sizeof(bad_early_data), &written)))
      goto end;
  }
    /* FALLTHROUGH */
...
static int test_early_data_skip(int idx) {
  return early_data_skip_helper(0, idx % OSSL_NELEM(ciphersuites),
...
static int test_early_data_skip_hrr(int idx) {
  return early_data_skip_helper(1, idx % OSSL_NELEM(ciphersuites),
                                idx / OSSL_NELEM(ciphersuites));
}
...
static int test_early_data_skip_hrr_fail(int idx) {
  return early_data_skip_helper(2, idx % OSSL_NELEM(ciphersuites),
                                idx / OSSL_NELEM(ciphersuites));
}
...
static int test_early_data_skip_abort(int idx) {
  return early_data_skip_helper(3, idx % OSSL_NELEM(ciphersuites),
                                idx / OSSL_NELEM(ciphersuites));
}
...
static int test_early_data_psk(int idx) {
  ...
  int testresult = 0;
  SSL_SESSION *sess = NULL;
  unsigned char alpnlist[] = {0x08, 'g', 'o', 'o', 'd', 'a', 'l', 'p', 'n',
                              0x07, 'b', 'a', 'd', 'a', 'l', 'p', 'n'};
...
#define BADALPN (alpnlist + GOODALPNLEN)
  ...
  case 1:
    ...
    if (!TEST_true(
        SSL_SESSION_set1_alpn_selected(sess, GOODALPN, GOODALPNLEN)) ||
  ...
  case 7:
    ...
    serverpsk = SSL_SESSION_dup(clientpsk);
    if (!TEST_ptr(serverpsk) ||
        !TEST_true(SSL_SESSION_set1_alpn_selected(clientpsk, BADALPN + 1,
                                                  BADALPNLEN - 1)) ||
        !TEST_true(SSL_SESSION_set1_alpn_selected(serverpsk, GOODALPN + 1,
                                                  GOODALPNLEN - 1)) ||
 ...
 * Test 1: Set a non-default ciphersuite in the SSL_CTX (no explicit
 * cipher_list) Test 2: Set a default ciphersuite in the SSL (no explicit
 * cipher_list) Test 3: Set a non-default ciphersuite in the SSL (no explicit
 * cipher_list) Test 4: Set a default ciphersuite in the SSL_CTX (SSL_CTX
 * cipher_list) Test 5: Set a non-default ciphersuite in the SSL_CTX (SSL_CTX
 * cipher_list) Test 6: Set a default ciphersuite in the SSL (SSL_CTX
 * cipher_list) Test 7: Set a non-default ciphersuite in the SSL (SSL_CTX
 * cipher_list) Test 8: Set a default ciphersuite in the SSL (SSL cipher_list)
...
static int test_set_ciphersuite(int idx) {
  ...
  if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(),
      ...
      sctx, "TLS_AES_128_GCM_SHA256:TLS_AES_128_CCM_SHA256")))
    goto end;
...
static int test_ciphersuite_change(void) {
  ...
  if (!TEST_true(SSL_CTX_set_ciphersuites(
      cctx, "TLS_AES_128_GCM_SHA256:TLS_AES_256_GCM_SHA384")) ||
       ...
       */
      || !TEST_false(
         create_ssl_connection(serverssl, clientssl, SSL_ERROR_WANT_READ)))
    goto end;
...
static int test_key_exchange(int idx) {
  ...
  if (is_fips && fips_provider_version_lt(libctx, 3, 5, 0) && idx >= 12 &&
      idx <= 19)
  ...
  if (!TEST_true(
      SSL_CTX_set_cipher_list(sctx, TLS1_TXT_ECDHE_RSA_WITH_AES_128_GCM_SHA256
...
#ifndef OPENSSL_NO_TLS1_2
  if (!TEST_true(
      SSL_CTX_set_cipher_list(cctx, TLS1_TXT_ECDHE_RSA_WITH_AES_128_GCM_SHA256
                              ":" TLS1_TXT_DHE_RSA_WITH_AES_128_GCM_SHA256)))
    goto end;
#endif
  ...
  if (kexch_groups != NULL) {
    if (!TEST_true(
        SSL_set1_groups(serverssl, kexch_groups, kexch_groups_size)) ||
...
static int test_negotiated_group(int idx) {
  ...
  if (is_fips && (kexch_alg == NID_X25519 || kexch_alg == NID_X448))
    return TEST_skip(
    "X25519 and X448 might not be available in fips provider.");
  ...
  if (!TEST_true(
      set_ssl_groups(serverssl, clientssl, clientmulti, isecdhe, idx)))
    goto end;
...
static int test_tls13_ciphersuite(int idx) {
  ...
  for (max_ver = TLS1_2_VERSION; max_ver <= TLS1_3_VERSION; max_ver++) {
    ...
    for (i = 0; i < OSSL_NELEM(t13_ciphers); i++) {
      ...
      if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(),
                                         TLS_client_method(), TLS1_VERSION,
                                         max_ver, &sctx, &cctx, cert, privkey)))
        goto end;
      ...
      negotiated_scipher =
      SSL_CIPHER_get_name(SSL_get_current_cipher(serverssl));
      negotiated_ccipher =
...
static int test_tls13_psk(int idx) {
  ...
  if (!TEST_true(create_ssl_ctx_pair(
      libctx, TLS_server_method(), TLS_client_method(), TLS1_VERSION, 0, &sctx,
      &cctx, idx == 3 ? NULL : cert, idx == 3 ? NULL : privkey)))
    goto end;
...
static int test_tls13_no_dhe_kex(const int idx) {
  ...
  0 | (server_allow_no_dhe ? SSL_OP_ALLOW_NO_DHE_KEX : 0) |
  (server_prefer_no_dhe ? SSL_OP_PREFER_NO_DHE_KEX : 0);

  uint64_t client_options =
  0 | (client_allow_no_dhe ? SSL_OP_ALLOW_NO_DHE_KEX : 0);
...
static int verify_stateless_cookie_callback(SSL *ssl,
...
static int verify_cb(int preverify_ok, X509_STORE_CTX *x509_ctx) { return 1; }
...
static int test_custom_exts(int tst) {
  ...
  if (tst == 2 && !TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(),
                                                 NULL, TLS1_VERSION, 0, &sctx2,
                                                 NULL, cert, privkey)))
    goto end;
  ...
  if (tst == 5) {
    ...
    SSL_CTX_set_verify(sctx, SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT,
                       verify_cb);
    if (!TEST_int_eq(SSL_CTX_use_certificate_file(cctx, cert, SSL_FILETYPE_PEM),
                     1) ||
        !TEST_int_eq(
        SSL_CTX_use_PrivateKey_file(cctx, privkey, SSL_FILETYPE_PEM), 1) ||
  ...
  if (tst == 0) {
    if (!TEST_true(SSL_CTX_add_client_custom_ext(
        cctx, TEST_EXT_TYPE1, old_add_cb, old_free_cb, &client, old_parse_cb,
        &client)))
      goto end;
  } else {
    if (!TEST_true(SSL_CTX_add_custom_ext(cctx, TEST_EXT_TYPE1, context,
                                          new_add_cb, new_free_cb, &client,
                                          new_parse_cb, &client)))
      goto end;
  }

  /* Should not be able to add duplicates */
  if (!TEST_false(SSL_CTX_add_client_custom_ext(
      cctx, TEST_EXT_TYPE1, old_add_cb, old_free_cb, &client, old_parse_cb,
      &client)) ||
      !TEST_false(SSL_CTX_add_custom_ext(cctx, TEST_EXT_TYPE1, context,
                                         new_add_cb, new_free_cb, &client,
                                         new_parse_cb, &client)))
    goto end;
  ...
  if (tst == 0) {
    if (!TEST_true(SSL_CTX_add_server_custom_ext(
        sctx, TEST_EXT_TYPE1, old_add_cb, old_free_cb, &server, old_parse_cb,
        &server)))
      goto end;
  } else {
    if (!TEST_true(SSL_CTX_add_custom_ext(sctx, TEST_EXT_TYPE1, context,
                                          new_add_cb, new_free_cb, &server,
                                          new_parse_cb, &server)))
      goto end;
    if (sctx2 != NULL && !TEST_true(SSL_CTX_add_custom_ext(
                         sctx2, TEST_EXT_TYPE1, context, new_add_cb,
                         new_free_cb, &server, new_parse_cb, &server)))
      goto end;
  }

  /* Should not be able to add duplicates */
  if (!TEST_false(SSL_CTX_add_server_custom_ext(
      sctx, TEST_EXT_TYPE1, old_add_cb, old_free_cb, &server, old_parse_cb,
      &server)) ||
      !TEST_false(SSL_CTX_add_custom_ext(sctx, TEST_EXT_TYPE1, context,
                                         new_add_cb, new_free_cb, &server,
                                         new_parse_cb, &server)))
    goto end;
  ...
  if (tst == 0) {
    ...
    if (clntaddnewcb != 1 || clntparsenewcb != 5 || srvaddnewcb != 5 ||
        srvparsenewcb != 1)
      goto end;
  }
  ...
  if (tst == 0) {
     ...
     */
    if (clntaddnewcb != 2 || clntparsenewcb != 8 || srvaddnewcb != 8 ||
        srvparsenewcb != 2)
      goto end;
  }
...
#define TLS13CONTEXT                                                           \
  (SSL_EXT_TLS1_3_CERTIFICATE | SSL_EXT_TLS1_2_SERVER_HELLO |                  \
   SSL_EXT_CLIENT_HELLO)

#define SERVERINFO_CUSTOM                                                      \
  0x00, (char)TLSEXT_TYPE_signed_certificate_timestamp, 0x00, 0x03, 0x04,      \
  0x05, 0x06

static const unsigned char serverinfo_custom_tls13[] = {
0x00, 0x00, (TLS13CONTEXT >> 8) & 0xff, TLS13CONTEXT & 0xff, SERVERINFO_CUSTOM};
...
static const size_t serverinfo_custom_tls13_len =
...
static int test_serverinfo_custom(const int idx) {
  ...
  if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_method(), TLS_method(),
                                     protocol_version, protocol_version, &sctx,
                                     &cctx, cert, privkey)))
    goto end;

  if (call_use_serverinfo_ex) {
    if (!TEST_true(
        SSL_CTX_use_serverinfo_ex(sctx, serverinfo_version, si, si_len)))
      goto end;
  } else {
  ...
  if (!TEST_true(SSL_CTX_add_custom_ext(
      cctx, TLSEXT_TYPE_signed_certificate_timestamp, extension_context, NULL,
      NULL, NULL, serverinfo_custom_parse_cb, &cb_result)) ||
      !TEST_true(
      create_ssl_objects(sctx, cctx, &serverssl, &clientssl, NULL, NULL)) ||
      !TEST_true(create_ssl_connection(serverssl, clientssl, SSL_ERROR_NONE)) ||
...
static int test_export_key_mat(int tst) {
   ...
   */
  if (!TEST_int_le(SSL_export_keying_material(
                   clientssl, ckeymat1, sizeof(ckeymat1), label,
                   SMALL_LABEL_LEN + 1, context, sizeof(context) - 1, 1),
                   0))
    goto end;
  ...
  if (tst == 5) {
    ...
    if (!TEST_int_le(SSL_export_keying_material(
                     clientssl, ckeymat1, sizeof(ckeymat1), label,
                     LONG_LABEL_LEN + 1, context, sizeof(context) - 1, 1),
                     0))
      goto end;
  ...
  if (!TEST_int_eq(SSL_export_keying_material(clientssl, ckeymat1,
                                              sizeof(ckeymat1), label, labellen,
                                              context, sizeof(context) - 1, 1),
                   1) ||
      !TEST_int_eq(SSL_export_keying_material(clientssl, ckeymat2,
                                              sizeof(ckeymat2), label, labellen,
                                              emptycontext, 0, 1),
                   1) ||
      !TEST_int_eq(SSL_export_keying_material(clientssl, ckeymat3,
                                              sizeof(ckeymat3), label, labellen,
                                              NULL, 0, 0),
                   1) ||
      (test_longcontext &&
       !TEST_int_eq(SSL_export_keying_material(
                    clientssl, ckeymat4, sizeof(ckeymat4), label, labellen,
                    longcontext, sizeof(longcontext), 1),
                    1)) ||
      !TEST_int_eq(SSL_export_keying_material(serverssl, skeymat1,
                                              sizeof(skeymat1), label, labellen,
                                              context, sizeof(context) - 1, 1),
                   1) ||
      !TEST_int_eq(SSL_export_keying_material(serverssl, skeymat2,
                                              sizeof(skeymat2), label, labellen,
                                              emptycontext, 0, 1),
                   1) ||
      !TEST_int_eq(SSL_export_keying_material(serverssl, skeymat3,
                                              sizeof(skeymat3), label, labellen,
                                              NULL, 0, 0),
                   1) ||
      (test_longcontext &&
       !TEST_int_eq(SSL_export_keying_material(
                    serverssl, skeymat4, sizeof(skeymat4), label, labellen,
                    longcontext, sizeof(longcontext), 1),
                    1))
      /*
       ...
       */
      || (test_longcontext &&
          !TEST_mem_eq(ckeymat4, sizeof(ckeymat4), skeymat4, sizeof(skeymat4)))
   ...
   */
  if ((tst < 3 &&
       !TEST_mem_ne(ckeymat2, sizeof(ckeymat2), ckeymat3, sizeof(ckeymat3))) ||
      (tst >= 3 &&
       !TEST_mem_eq(ckeymat2, sizeof(ckeymat2), ckeymat3, sizeof(ckeymat3))))
    goto end;
...
static int test_key_update(void) {
  ...
  for (j = 0; j < 2; j++) {
    ...
    for (i = 0; i < NUM_KEY_UPDATE_MESSAGES; i++) {
      if (!TEST_true(SSL_key_update(clientssl, (j == 0)
...
TLSEXT_max_fragment_length_512, TLSEXT_max_fragment_length_1024,
...
static int test_max_fragment_len_ext(int idx_tst) {
  ...
  if (!TEST_true(create_ssl_ctx_pair(libctx, NULL, TLS_client_method(),
                                     TLS1_VERSION, 0, NULL, &ctx, NULL, NULL)))
    return 0;
...
static int create_new_vfile(char *userid, char *password,
                            const char *filename) {
  char *gNid = NULL;
  ...
  if (!TEST_ptr(row[DB_srpid]) || !TEST_ptr(row[DB_srptype]) ||
      !TEST_ptr(row[DB_srpgN]) || !TEST_true(TXT_DB_insert(db, row)))
    goto end;
...
static void sslapi_info_callback(const SSL *s, int where, int ret) {
  ...
  if ((where & SSL_CB_LOOP) != 0 &&
      !TEST_int_eq(
      strcmp(SSL_state_string(s), state[info_cb_this_state].statestr), 0)) {
    info_cb_failed = 1;
    return;
  }
...
(!defined(OPENSSL_NO_EC) || !defined(OPENSSL_NO_DH))
  ...
  if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(),
                                     TLS_client_method(), tlsvers, tlsvers,
                                     &sctx, &cctx, cert, privkey)))
    goto end;
 ...
 */
#if defined(OSSL_NO_USABLE_TLS1_3) || !defined(OPENSSL_NO_TLS1_2)
{TLS1_2_VERSION, "AES128-SHA:AES256-SHA", NULL, "AES256-SHA:DHE-RSA-AES128-SHA",
 NULL, "AES256-SHA", "AES256-SHA"},
#if !defined(OPENSSL_NO_CHACHA) && !defined(OPENSSL_NO_POLY1305) &&            \
!defined(OPENSSL_NO_EC)
{TLS1_2_VERSION, "AES128-SHA:ECDHE-RSA-CHACHA20-POLY1305", NULL,
 "AES128-SHA:ECDHE-RSA-CHACHA20-POLY1305", NULL,
 "AES128-SHA:ECDHE-RSA-CHACHA20-POLY1305", "AES128-SHA"},
#endif
{TLS1_2_VERSION, "AES128-SHA:DHE-RSA-AES128-SHA:AES256-SHA", NULL,
 "AES128-SHA:DHE-RSA-AES256-SHA:AES256-SHA", NULL, "AES128-SHA:AES256-SHA",
 "AES128-SHA:AES256-SHA"},
{TLS1_2_VERSION, "AES128-SHA:AES256-SHA", NULL, "AES128-SHA:DHE-RSA-AES128-SHA",
 NULL, "AES128-SHA", "AES128-SHA"},
#endif
/*
...
{TLS1_3_VERSION, "AES128-SHA:AES256-SHA", NULL, "AES256-SHA:AES128-SHA256",
 NULL,
...
#endif
#ifndef OSSL_NO_USABLE_TLS1_3
{TLS1_3_VERSION, "AES128-SHA", "TLS_AES_256_GCM_SHA384", "AES256-SHA",
 "TLS_AES_256_GCM_SHA384", "TLS_AES_256_GCM_SHA384", "TLS_AES_256_GCM_SHA384"},
#endif
};
...
static int int_test_ssl_get_shared_ciphers(int tst, int clnt) {
  ...
  if (!TEST_true(
      SSL_CTX_set_cipher_list(cctx, shared_ciphers_data[tst].clntciphers)) ||
      (shared_ciphers_data[tst].clnttls13ciphers != NULL &&
       !TEST_true(SSL_CTX_set_ciphersuites(
       cctx, shared_ciphers_data[tst].clnttls13ciphers))) ||
      !TEST_true(
      SSL_CTX_set_cipher_list(sctx, shared_ciphers_data[tst].srvrciphers)) ||
      (shared_ciphers_data[tst].srvrtls13ciphers != NULL &&
       !TEST_true(SSL_CTX_set_ciphersuites(
       sctx, shared_ciphers_data[tst].srvrtls13ciphers))))
    goto end;
  ...
  if (!TEST_ptr(SSL_get_shared_ciphers(serverssl, buf, sizeof(buf))) ||
      !TEST_int_eq(strcmp(buf, is_fips ? shared_ciphers_data[tst].fipsshared
...
static int test_ticket_callbacks(int tst) {
  ...
  if (!TEST_true(
      SSL_CTX_set_session_ticket_cb(sctx, gen_tick_cb, dec_tick_cb, NULL)))
    goto end;
  ...
  if (tick_dec_ret == SSL_TICKET_RETURN_IGNORE ||
      tick_dec_ret == SSL_TICKET_RETURN_IGNORE_RENEW || tick_key_renew == -1) {
    if (!TEST_false(SSL_session_reused(clientssl)))
      goto end;
  } else {
    if (!TEST_true(SSL_session_reused(clientssl)))
...
static int test_shutdown(int tst) {
  ...
  if (!TEST_true(create_ssl_ctx_pair(
      libctx, TLS_server_method(), TLS_client_method(), TLS1_VERSION,
      (tst <= 1) ? TLS1_2_VERSION : TLS1_3_VERSION, &sctx, &cctx, cert,
      privkey)))
    goto end;
  ...
  if (pkey == NULL) {
    ...
    if (chain == NULL)
      *x509 = x;
      ...
      goto out;
  } else if (PEM_read_bio_PrivateKey_ex(in, pkey, NULL, NULL, libctx, NULL) ==
             NULL) {
    goto out;
  }
...
static int test_cert_cb_int(int prot, int tst) {
  ...
  if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(),
                                     TLS_client_method(), prot, prot, &sctx,
                                     &cctx, NULL, NULL)))
    goto end;
  ...
  if (tst == 4) {
     ...
     */
    if (!TEST_true(SSL_set1_sigalgs_list(
        clientssl, "rsa_pss_rsae_sha256:rsa_pkcs1_sha256")))
      goto end;
  }
...
static int client_cert_cb(SSL *ssl, X509 **x509, EVP_PKEY **pkey) {
  ...
  if (!TEST_ptr(xcert = X509_new_ex(libctx, NULL)) ||
      ...
      !TEST_ptr(privpkey = PEM_read_bio_PrivateKey_ex(priv_in, NULL, NULL, NULL,
                                                      libctx, NULL)))
    goto err;
...
static int test_ca_names_int(int prot, int tst) {
  ...
  for (i = 0; i < OSSL_NELEM(name); i++) {
    ...
    if (!TEST_ptr(name[i]) ||
        !TEST_true(X509_NAME_add_entry_by_txt(
        name[i], "CN", MBSTRING_ASC, (unsigned char *)strnames[i], -1, -1, 0)))
      goto end;
  }

  if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(),
                                     TLS_client_method(), TLS1_VERSION, prot,
                                     &sctx, &cctx, cert, privkey)))
    goto end;
  ...
  if (!TEST_ptr(sktmp) || !TEST_int_eq(sk_X509_NAME_num(sktmp), 2) ||
      !TEST_int_eq(
      X509_NAME_cmp(sk_X509_NAME_value(sktmp, 0), name[tst == 0 ? 0 : 2]), 0) ||
      !TEST_int_eq(
      X509_NAME_cmp(sk_X509_NAME_value(sktmp, 1), name[tst == 0 ? 1 : 3]), 0))
    goto end;
...
static int test_multiblock_write(int test_index) {
  ...
  "AES-128-CBC-HMAC-SHA1", "AES-128-CBC-HMAC-SHA256", "AES-256-CBC-HMAC-SHA1",
...
#define TIMEOUT 10
  ...
  if (!TEST_ptr(early->prev) || !TEST_ptr(middle->prev) ||
      !TEST_ptr(late->prev))
    goto end;
  ...
  if (!TEST_ptr_eq(late->next, middle) || !TEST_ptr_eq(middle->next, early) ||
      !TEST_ptr_eq(early->prev, middle) || !TEST_ptr_eq(middle->prev, late))
    goto end;

  /* This should remove "early" */
  SSL_CTX_flush_sessions_ex(ctx, now + TIMEOUT - 1);
  if (!TEST_ptr_null(early->prev) || !TEST_ptr(middle->prev) ||
      !TEST_ptr(late->prev))
    goto end;

  /* This should remove "middle" */
  SSL_CTX_flush_sessions_ex(ctx, now + TIMEOUT + 1);
  if (!TEST_ptr_null(early->prev) || !TEST_ptr_null(middle->prev) ||
      !TEST_ptr(late->prev))
    goto end;
  ...
  ctx, SSL_SESS_CACHE_UPDATE_TIME | SSL_CTX_get_session_cache_mode(ctx));
...
static int test_session_cache_overflow(int idx) {
  ...
  if (!TEST_true(create_ssl_ctx_pair(
      libctx, TLS_server_method(), TLS_client_method(), TLS1_VERSION,
      (idx % 2 == 0) ? TLS1_3_VERSION : TLS1_2_VERSION, &sctx, &cctx, cert,
      privkey)) ||
      !TEST_true(SSL_CTX_set_options(sctx, SSL_OP_NO_TICKET)))
    goto end;
...
static int test_servername(int tst) {
  ...
  if (!TEST_true(create_ssl_ctx_pair(
      libctx, TLS_server_method(), TLS_client_method(), TLS1_VERSION,
      (tst <= 4) ? TLS1_2_VERSION : TLS1_3_VERSION, &sctx, &cctx, cert,
      privkey)) ||
      !TEST_true(
      create_ssl_objects(sctx, cctx, &serverssl, &clientssl, NULL, NULL)))
    goto end;
  ...
  if (!TEST_str_eq(SSL_get_servername(clientssl, TLSEXT_NAMETYPE_host_name),
                   cexpectedhost) ||
      !TEST_str_eq(SSL_get_servername(serverssl, TLSEXT_NAMETYPE_host_name),
                   sexpectedhost))
    goto end;
  ...
  if (!TEST_true(SSL_session_reused(clientssl)) ||
      !TEST_true(SSL_session_reused(serverssl)) ||
      !TEST_str_eq(SSL_get_servername(clientssl, TLSEXT_NAMETYPE_host_name),
                   cexpectedhost) ||
      !TEST_str_eq(SSL_get_servername(serverssl, TLSEXT_NAMETYPE_host_name),
                   sexpectedhost))
    goto end;
...
static int test_unknown_sigalgs_groups(void) {
  ...
  if (!TEST_int_gt(
      SSL_CTX_set1_sigalgs_list(ctx, "RSA+SHA256:?nonexistent:?RSA+SHA512"), 0))
    goto end;
  if (!TEST_size_t_eq(ctx->cert->conf_sigalgslen, 2) ||
      !TEST_int_eq(ctx->cert->conf_sigalgs[0],
                   TLSEXT_SIGALG_rsa_pkcs1_sha256) ||
      !TEST_int_eq(ctx->cert->conf_sigalgs[1], TLSEXT_SIGALG_rsa_pkcs1_sha512))
    goto end;

  if (!TEST_int_gt(SSL_CTX_set1_client_sigalgs_list(
                   ctx, "RSA+SHA256:?nonexistent:?RSA+SHA512"),
                   0))
    goto end;
  if (!TEST_size_t_eq(ctx->cert->client_sigalgslen, 2) ||
      !TEST_int_eq(ctx->cert->client_sigalgs[0],
                   TLSEXT_SIGALG_rsa_pkcs1_sha256) ||
      !TEST_int_eq(ctx->cert->client_sigalgs[1],
  ...
  if (!TEST_int_gt(SSL_CTX_set1_groups_list(
                   ctx, "?nonexistent1:?nonexistent2:?nonexistent3"),
                   0))
    goto end;
  ...
  if (!TEST_int_gt(SSL_CTX_set1_groups_list(ctx, "P-384:?nonexistent:?P-521"),
                   0))
    goto end;
...
static int test_sigalgs_available(int idx) {
  ...
  if (idx != 0 && idx != 3) {
    if (!TEST_true(
        OSSL_PROVIDER_add_builtin(tmpctx, "filter", filter_provider_init)))
      goto end;
    ...
    if (idx < 3) {
       ...
       */
      if (!TEST_true(
          filter_provider_set_filter(OSSL_OP_DIGEST, "SHA2-256:SHA1")))
        goto end;
    } else {
...
#else
          || !TEST_true(
             filter_provider_set_filter(OSSL_OP_KEYMGMT, "EC:X25519:X448"))
#endif
      )
        goto end;
    }
  ...
  if (!TEST_true(SSL_CTX_set1_groups_list(
      cctx, "?X25519:?secp256r1:?ffdhe2048:?ffdhe3072")) ||
      !TEST_true(SSL_CTX_set1_groups_list(
      sctx, "?X25519:?secp256r1:?ffdhe2048:?ffdhe3072")))
    goto end;
  ...
  if (idx < 4) {
    ...
    if (!TEST_true(
        SSL_CTX_set_cipher_list(cctx, "ECDHE-ECDSA-AES128-GCM-SHA256")))
      goto end;
  }

  if (idx < 3) {
    if (!SSL_CTX_set1_sigalgs_list(cctx, "rsa_pss_rsae_sha384"
                                         ":rsa_pss_rsae_sha256") ||
        !SSL_CTX_set1_sigalgs_list(sctx, "rsa_pss_rsae_sha384"
                                         ":rsa_pss_rsae_sha256"))
      goto end;
  } else {
  ...
  if (idx != 5 &&
      (!TEST_int_eq(SSL_CTX_use_certificate_file(sctx, cert2, SSL_FILETYPE_PEM),
                    1) ||
       !TEST_int_eq(
       SSL_CTX_use_PrivateKey_file(sctx, privkey2, SSL_FILETYPE_PEM), 1) ||
       !TEST_int_eq(SSL_CTX_check_private_key(sctx), 1)))
    goto end;
  ...
  numshared =
  SSL_get_shared_sigalgs(serverssl, 0, &sig, &hash, NULL, NULL, NULL);
  ...
  if (!TEST_int_eq(numshared, numshared_expected) ||
      !TEST_int_eq(hash, hash_expected) || !TEST_int_eq(sig, sig_expected) ||
...
static int test_pluggable_group(int idx) {
  ...
  if (!TEST_true(create_ssl_ctx_pair(
      libctx, TLS_server_method(), TLS_client_method(), TLS1_3_VERSION,
      TLS1_3_VERSION, &sctx, &cctx, cert, privkey)) ||
      !TEST_true(
      create_ssl_objects(sctx, cctx, &serverssl, &clientssl, NULL, NULL)))
    goto end;
  ...
  if (!TEST_true(SSL_set1_groups_list(
      serverssl,
      "xorgroup:xorkemgroup:dummy1:dummy2:dummy3:dummy4:dummy5:dummy6:dummy7:"
      "dummy8:dummy9:dummy10:dummy11:dummy12:dummy13:dummy14:dummy15:dummy16:"
      "dummy17:dummy18:dummy19:dummy20:dummy21:dummy22:dummy23:dummy24:dummy25:"
      "dummy26:dummy27:dummy28:dummy29:dummy30:dummy31:dummy32:dummy33:dummy34:"
      "dummy35:dummy36:dummy37:dummy38:dummy39:dummy40:dummy41:dummy42:"
      "dummy43"))
  ...
  if (!TEST_str_eq(group_name, SSL_group_to_name(
                               serverssl, SSL_get_shared_group(serverssl, 0))))
    goto end;
...
static int create_cert_key(int idx, char *certfilename, char *privkeyfilename) {
  EVP_PKEY_CTX *evpctx = EVP_PKEY_CTX_new_from_name(
  libctx, (idx == 0) ? "xorhmacsig" : "xorhmacsha2sig", NULL);
  EVP_PKEY *pkey = NULL;
  X509 *x509 = X509_new();
  X509_NAME *name = NULL;
  ...
  if (!TEST_ptr(evpctx) || !TEST_int_gt(EVP_PKEY_keygen_init(evpctx), 0) ||
      !TEST_true(EVP_PKEY_generate(evpctx, &pkey)) || !TEST_ptr(pkey) ||
      !TEST_ptr(x509) ||
      ...
      !TEST_true(X509_NAME_add_entry_by_txt(
      name, "C", MBSTRING_ASC, (unsigned char *)"CH", -1, -1, 0)) ||
      !TEST_true(X509_NAME_add_entry_by_txt(
      name, "O", MBSTRING_ASC, (unsigned char *)"test.org", -1, -1, 0)) ||
      !TEST_true(X509_NAME_add_entry_by_txt(
...
static int test_pluggable_signature(int idx) {
  ...
  if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(),
                                     TLS_client_method(), TLS1_3_VERSION,
                                     TLS1_3_VERSION, &sctx, &cctx, NULL, NULL)))
    goto end;
  ...
  if (do_conf_cmd) {
    ...
    if (!TEST_int_eq(
        SSL_CTX_use_certificate_file(sctx, certfilename, SSL_FILETYPE_PEM),
        1) ||
        !TEST_int_eq(
        SSL_CTX_use_PrivateKey_file(sctx, privkeyfilename, SSL_FILETYPE_PEM),
        1))
      goto end;
  }
  ...
  if (rpkidx) {
    if (!TEST_true(SSL_set1_server_cert_type(serverssl, cert_type_rpk,
                                             sizeof(cert_type_rpk))) ||
        !TEST_true(SSL_set1_server_cert_type(clientssl, cert_type_rpk,
                                             sizeof(cert_type_rpk))))
      goto end;
  }
  ...
  if (rpkidx &&
      !TEST_long_eq(SSL_get_verify_result(clientssl), X509_V_ERR_RPK_UNTRUSTED))
    goto end;
...
static EVP_PKEY *get_tmp_dh_params(void) {
  if (tmp_dh_params == NULL) {
    ...
    if (!TEST_ptr(tmpl) ||
        !TEST_true(OSSL_PARAM_BLD_push_BN(tmpl, OSSL_PKEY_PARAM_FFC_P, p)) ||
    ...
    if (!TEST_ptr(params) ||
        !TEST_int_eq(
        EVP_PKEY_fromdata(pctx, &dhpkey, EVP_PKEY_KEY_PARAMETERS, params), 1))
      goto end;
...
static int test_dh_auto(int idx) {
  ...
  if (!TEST_true(create_ssl_ctx_pair(libctx, NULL, NULL, 0, 0, &sctx, &cctx,
                                     thiscert, thiskey)))
    goto end;
...
#define TWO_WEEK_SEC (2 * ONE_WEEK_SEC)
  ...
  if (idx == 0) {
    ...
    if (!TEST_ulong_eq(
        SSL_SESSION_get_ticket_lifetime_hint(SSL_get_session(clientssl)),
        TWO_WEEK_SEC))
      goto end;
  } else {
...
static int test_set_alpn(void) {
  ...
  /* the set_alpn functions return 0 (false) on success, non-zero (true) on
   * failure */
...
static int test_legacy_ec_point_formats(void) {
  ...
  if (!TEST_true(create_ssl_ctx_pair(
      libctx, TLS_server_method(), TLS_client_method(), TLS1_2_VERSION,
      TLS1_2_VERSION, &sctx, &cctx, cert, privkey)))
    goto end;
...
/*
 * Test SSL_CTX_set1_verify/chain_cert_store and
 * SSL_CTX_get_verify/chain_cert_store.
 */
static int test_set_verify_cert_store_ssl_ctx(void) {
  SSL_CTX *ctx = NULL;
  int testresult = 0;
  X509_STORE *store = NULL, *new_store = NULL, *cstore = NULL,
             *new_cstore = NULL;
...
static int test_tls13_record_padding(int idx) {
  ...
  if (idx == 0) {
    ...
    if (!TEST_false(
        SSL_CTX_set_block_padding(cctx, SSL3_RT_MAX_PLAIN_LENGTH + 1)))
      goto end;
  ...
  if (idx == 1) {
    ...
    /* Exceeding the max plain length should fail */
    if (!TEST_false(
        SSL_set_block_padding(clientssl, SSL3_RT_MAX_PLAIN_LENGTH + 1)))
      goto end;
    ...
    /* Exceeding the max plain length should fail */
    if (!TEST_false(
        SSL_set_block_padding_ex(clientssl, 0, SSL3_RT_MAX_PLAIN_LENGTH + 1)))
      goto end;
...
static int test_pipelining(int idx) {
  ...
  /* A 55 byte message */
  unsigned char *msg =
  (unsigned char *)"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz123";
   ...
   */
  for (offset = 0, numreads = 0; offset < msglen;
       offset += readbytes, numreads++) {
    if (!TEST_true(
        SSL_read_ex(peerb, buf + offset, msglen - offset, &readbytes)))
      goto end;
  }

  expectedreads =
  idx == 4 ? numpipes + 1 : (idx == 3 ? numpipes - 1 : numpipes);
  ...
  if (idx == 4) {
    ...
    if (!TEST_true(
        SSL_read_ex(peera, buf + readbytes, msglen - readbytes, &readbytes2)))
      goto end;
...
static int test_version(int idx) {
  ...
  if (is_fips && (version == SSL3_VERSION || version == TLS1_VERSION ||
                  version == DTLS1_VERSION)) {
...
static int test_rstate_string(void) {
  ...
  SSL3_RT_APPLICATION_DATA, TLS1_2_VERSION_MAJOR, 0, /* To be filled in later */
  0, 1};

  if (!TEST_true(create_ssl_ctx_pair(libctx, servmeth, clientmeth, 0, 0, &sctx,
                                     &cctx, cert, privkey)))
    goto end;
...
static int test_handshake_retry(int idx) {
  ...
  if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(),
                                     TLS_client_method(), 0, maxversion, &sctx,
                                     &cctx, cert, privkey)))
    goto end;
...
static int test_data_retry(void) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (!TEST_true(
        BIO_ctrl(bretry, MAYBE_RETRY_CTRL_SET_RETRY_AFTER_CNT, 1, NULL)))
      goto end;
...
static int test_quic_tls(int idx) {
  ...
  if (!TEST_true(SSL_set_quic_tls_cbs(clientssl, qtdis, &cdata)) ||
      ...
      !TEST_true(
      SSL_set_quic_tls_transport_params(clientssl, cparams, sizeof(cparams))) ||
      !TEST_true(
      SSL_set_quic_tls_transport_params(serverssl, sparams, sizeof(sparams))))
    goto end;
  ...
  if (!TEST_false(sdata.alert) || !TEST_false(cdata.alert) ||
      !TEST_false(sdata.err) || !TEST_false(cdata.err))
    goto end;

  /* Check the secrets all match */
  for (i = OSSL_RECORD_PROTECTION_LEVEL_EARLY - 1;
       i < OSSL_RECORD_PROTECTION_LEVEL_APPLICATION; i++) {
    if (!TEST_mem_eq(sdata.wsecret[i], sdata.wsecret_len[i], cdata.rsecret[i],
                     cdata.rsecret_len[i]))
      goto end;
  }
  ...
  if (!TEST_true(sdata.renc_level ==
                 OSSL_RECORD_PROTECTION_LEVEL_APPLICATION) ||
      !TEST_true(sdata.wenc_level ==
                 OSSL_RECORD_PROTECTION_LEVEL_APPLICATION) ||
      !TEST_true(cdata.renc_level ==
                 OSSL_RECORD_PROTECTION_LEVEL_APPLICATION) ||
...
static void assert_no_end_of_early_data(int write_p, int version,
...
int setup_tests(void) {
   ...
   */
  if (!TEST_true(
      OSSL_PROVIDER_add_builtin(libctx, "tls-provider", tls_provider_init)))
    return 0;
...
#if !defined(OPENSSL_NO_EC) && !defined(OPENSSL_NO_DH)
  ADD_ALL_TESTS(test_negotiated_group, 4 * (OSSL_NELEM(ecdhe_kexch_groups) +
