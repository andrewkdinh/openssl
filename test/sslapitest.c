...
static int
test_keylog_output(char *buffer, const SSL *ssl, const SSL_SESSION *session, struct sslapitest_log_counts *expected)
{
    char         *token                                        = NULL;
...
static int test_keylog(void)
{
    ...
    if (!TEST_true(create_ssl_objects(sctx, cctx, &serverssl, &clientssl, NULL, NULL))
        ...
        || !TEST_size_t_gt(client_log_buffer_index, 0) || !TEST_size_t_gt(server_log_buffer_index, 0))
        goto end;
...
# endif
    ...
    if ((cis_ktls || sis_ktls) && !ktls_used) {
        testresult = TEST_skip("KTLS not supported for %s cipher %s",
                               tls_version == TLS1_3_VERSION ? "TLS 1.3" : "TLS 1.2",
                               cipher);
        goto end;
    }
...
static int test_extra_tickets(int idx)
{
    ...
    if (!TEST_true(SSL_write_ex(serverssl, &c, 1, &nbytes)) || !TEST_size_t_eq(1, nbytes)
        ...
        || !TEST_int_eq(idx * 2 + 4, new_called) || !TEST_size_t_eq(sizeof(buf), nbytes) || !TEST_int_eq(c, buf[0])
 ...
 * error.
 */
static int setupearly_data_test(SSL_CTX     **cctx,
                                SSL_CTX     **sctx,
                                SSL         **clientssl,
                                SSL         **serverssl,
                                SSL_SESSION **sess,
                                ...
                                size_t        mdsize)
{
    ...
    if (*sctx == NULL
        && !TEST_true(create_ssl_ctx_pair(libctx,
                                          TLS_server_method(),
                                          TLS_client_method(),
                                          TLS1_VERSION,
                                          0,
...
static int         early_data_skip_helper(int testtype, int cipher, int idx)
{
    ...
    if (!TEST_true(setupearly_data_test(&cctx,
                                        &sctx,
                                        &clientssl,
                                        &serverssl,
                                        &sess,
 ...
 */
# ifndef OPENSSL_NO_EC
static int ecdhe_kexch_groups[] = {NID_X9_62_prime256v1,
                                   NID_secp384r1,
                                   NID_secp521r1,
...
static int test_custom_exts(int tst)
{
    ...
    if (!TEST_false(SSL_CTX_add_client_custom_ext(cctx,
                                                  TEST_EXT_TYPE1,
                                                  old_add_cb,
                                                  old_free_cb,
                                                  &client,
                                                  old_parse_cb,
                                                  &client))
        || !TEST_false(SSL_CTX_add_custom_ext(cctx,
                                              TEST_EXT_TYPE1,
                                              context,
                                              new_add_cb,
                                              new_free_cb,
    ...
    if (tst == 0) {
        ...
        if (!TEST_true(SSL_CTX_add_custom_ext(sctx,
                                              TEST_EXT_TYPE1,
                                              context,
                                              new_add_cb,
                                              new_free_cb,
            ...
            goto end;
        if (sctx2 != NULL
            && !TEST_true(SSL_CTX_add_custom_ext(sctx2,
                                                 TEST_EXT_TYPE1,
                                                 context,
                                                 new_add_cb,
                                                 new_free_cb,
    ...
    /* Should not be able to add duplicates */
    if (!TEST_false(SSL_CTX_add_server_custom_ext(sctx,
                                                  TEST_EXT_TYPE1,
                                                  old_add_cb,
                                                  old_free_cb,
                                                  &server,
                                                  old_parse_cb,
                                                  &server))
        || !TEST_false(SSL_CTX_add_custom_ext(sctx,
                                              TEST_EXT_TYPE1,
                                              context,
                                              new_add_cb,
                                              new_free_cb,
...
static const unsigned char serverinfo_custom_tls13[]   = {0x00,
                                                          0x00,
                                                          (TLS13CONTEXT >> 8) & 0xff,
                                                          TLS13CONTEXT & 0xff,
                                                          SERVERINFO_CUSTOM};
static const unsigned char serverinfo_custom_v2[]      = {0x00,
                                                          0x00,
                                                          (SYNTHV1CONTEXT >> 8) & 0xff,
                                                          SYNTHV1CONTEXT & 0xff,
                                                          SERVERINFO_CUSTOM};
...
static int                 serverinfo_custom_parse_cb(SSL                 *s,
                                                      unsigned int         ext_type,
                                                      unsigned int         context,
                                                      const unsigned char *in,
                                                      size_t               inlen,
...
static int test_export_key_mat(int tst)
{
    ...
    size_t               labellen;
    const int            protocols[] =
        {TLS1_VERSION, TLS1_1_VERSION, TLS1_2_VERSION, TLS1_3_VERSION, TLS1_3_VERSION, TLS1_3_VERSION};
     ...
     */
    if (!TEST_int_le(SSL_export_keying_material(clientssl,
                                                ckeymat1,
                                                sizeof(ckeymat1),
                                                label,
                                                SMALL_LABEL_LEN + 1,
    ...
    if (!TEST_int_eq(SSL_export_keying_material(clientssl,
                                                ckeymat1,
                                                sizeof(ckeymat1),
                                                label,
                                                labellen,
                     ...
                     1)
        || !TEST_int_eq(
            SSL_export_keying_material(clientssl, ckeymat2, sizeof(ckeymat2), label, labellen, emptycontext, 0, 1),
            1)
        ...
        || (test_longcontext
            && !TEST_int_eq(SSL_export_keying_material(clientssl,
                                                       ...
                                                       1),
                            1))
        || !TEST_int_eq(SSL_export_keying_material(serverssl,
                                                   skeymat1,
                                                   sizeof(skeymat1),
                                                   label,
                                                   labellen,
                        ...
                        1)
        || !TEST_int_eq(
            SSL_export_keying_material(serverssl, skeymat2, sizeof(skeymat2), label, labellen, emptycontext, 0, 1),
            1)
...
static int test_export_key_mat_early(int idx)
{
    ...
    if (!TEST_int_eq(SSL_export_keying_material_early(clientssl,
                                                      ckeymat1,
                                                      sizeof(ckeymat1),
                                                      label,
                                                      sizeof(label) - 1,
...
static int test_key_update(void)
{
    ...
    for (j = 0; j < 2; j++) {
        ...
        if (!TEST_int_eq(SSL_write(clientssl, mess, (int)strlen(mess)), (int)strlen(mess))
            || !TEST_int_eq(SSL_read(serverssl, buf, sizeof(buf)), (int)strlen(mess)))
            goto end;

        if (!TEST_int_eq(SSL_write(serverssl, mess, (int)strlen(mess)), (int)strlen(mess))
            || !TEST_int_eq(SSL_read(clientssl, buf, sizeof(buf)), (int)strlen(mess)))
            goto end;
    }
...
static int get_MFL_from_client_hello(BIO *bio, int *mfl_codemfl_code)
{
    ...
    while (PACKET_remaining(&pkt2)) {
        ...
        if (type == TLSEXT_TYPE_max_fragment_length) {
            if (!TEST_size_t_ne(PACKET_remaining(&pkt3), 0) || !TEST_true(PACKET_get_1(&pkt3, &MFL_code)))
                goto end;
...
static const unsigned char max_fragment_len_test[] = {TLSEXT_max_fragment_length_512,
...
# if !defined(OPENSSL_NO_CHACHA) && !defined(OPENSSL_NO_POLY1305) && !defined(OPENSSL_NO_EC)
    {TLS1_2_VERSION,
                                                                     "AES128-SHA:ECDHE-RSA-CHACHA20-POLY1305",   NULL,
                                                                     "AES128-SHA:ECDHE-RSA-CHACHA20-POLY1305",                                                              NULL,
                                                                     "AES128-SHA:ECDHE-RSA-CHACHA20-POLY1305",                                                                                                      "AES128-SHA"                                              },
# endif
    {TLS1_2_VERSION,
                                                                     "AES128-SHA:DHE-RSA-AES128-SHA:AES256-SHA", NULL,
                                                                     "AES128-SHA:DHE-RSA-AES256-SHA:AES256-SHA",                                                            NULL,
 ...
 */
    ...
    {TLS1_3_VERSION,
                                                                     "AES128-SHA:AES256-SHA",                    NULL,
                                                                     "AES256-SHA:AES128-SHA256",                                                                            NULL,
                                                                     "TLS_AES_256_GCM_SHA384:TLS_CHACHA20_POLY1305_SHA256:"
     "TLS_AES_128_GCM_SHA256:AES256-SHA",                                                                                                                                                                           "TLS_AES_256_GCM_SHA384:TLS_AES_128_GCM_SHA256:AES256-SHA"},
#endif
#ifndef OSSL_NO_USABLE_TLS1_3
    {TLS1_3_VERSION,
                                                                     "AES128-SHA",                               "TLS_AES_256_GCM_SHA384",
                                                                     "AES256-SHA",                                                                                          "TLS_AES_256_GCM_SHA384",
...
static SSL_TICKET_RETURN dec_tick_cb(SSL                 *s,
                                     SSL_SESSION         *ss,
...
static int tick_key_evp_cb(SSL            *s,
                           unsigned char   key_name[16],
                           unsigned char   iv[EVP_MAX_IV_LENGTH],
                           EVP_CIPHER_CTX *ctx,
                           EVP_MAC_CTX    *hctx,
...
static int test_ticket_callbacks(int tst)
{
    ...
    if (!TEST_true(create_ssl_ctx_pair(libctx,
                                       ...
                                       TLS_client_method(),
                                       TLS1_VERSION,
                                       ((tst % 2) == 0) ? TLS1_2_VERSION : TLS1_3_VERSION,
                                       &sctx,
                                       &cctx,
                                       cert,
                                       privkey)))
...
static int test_client_cert_cb(int tst)
{
    ...
    if (!TEST_true(create_ssl_ctx_pair(libctx,
                                       ...
                                       TLS_client_method(),
                                       TLS1_VERSION,
                                       tst == 0 ? TLS1_2_VERSION : TLS1_3_VERSION,
                                       &sctx,
                                       &cctx,
                                       cert,
                                       privkey)))
...
{
    static const char *fetchable_ciphers[] = {"AES-128-CBC-HMAC-SHA1",
...
# endif
    ...
    if (!TEST_true(create_ssl_ctx_pair(libctx,
                                       ...
                                       TLS_client_method(),
                                       TLS1_VERSION,
                                       (idx % 2 == 0) ? TLS1_3_VERSION : TLS1_2_VERSION,
                                       &sctx,
                                       &cctx,
                                       cert,
                                       privkey))
...
static int test_servername(int tst)
{
    ...
    if (!TEST_true(create_ssl_ctx_pair(libctx,
                                       ...
                                       TLS_client_method(),
                                       TLS1_VERSION,
                                       (tst <= 4) ? TLS1_2_VERSION : TLS1_3_VERSION,
                                       &sctx,
                                       &cctx,
                                       cert,
                                       privkey))
...
static int test_rstate_string(void)
{
    ...
    size_t            written, readbytes;
    unsigned char     buf[2];
    unsigned char     dummyheader[SSL3_RT_HEADER_LENGTH] = {SSL3_RT_APPLICATION_DATA,
...
static int npn_select_cb(SSL                 *s,
                         unsigned char      **out,
                         unsigned char       *outlen,
                         const unsigned char *in,
                         unsigned int         inlen,
...
static int alpn_select_cb2(SSL                  *ssl,
                           const unsigned char **out,
                           unsigned char        *outlen,
                           const unsigned char  *in,
                           unsigned int          inlen,
 ...
 * to yield_secret_cb
 */
typedef enum { LAST_DIR_READ = 0, LAST_DIR_WRITE = 1, LAST_DIR_UNSET = 2 } last_dir_history_state;
...
static int check_secret_history(SSL *s)
{
    ...
    for (i = 0; secret_history[i].recorded == 1; i++) {
        ...
        TEST_info("Got %s(%d) secret for level %d, last level %d, last state %d, gen %d\n",
                  secret_history[i].direction == 1 ? "Write" : "Read",
                  secret_history[i].direction,
                  secret_history[i].prot_level,
                  last_prot_level,
                  last_state,
...
static int
yield_secret_cb(SSL *s, uint32_t prot_level, int direction, const unsigned char *secret, size_t secret_len, void *arg)
{
    struct quic_tls_test_data *data = (struct quic_tls_test_data *)arg;
...
static int yield_secret_cb_fail(SSL                 *s,
                                uint32_t             prot_level,
                                int                  direction,
...
static void assert_no_end_of_early_data(int         write_p,
                                        int         version,
                                        int         content_type,
                                        const void *buf,
                                        size_t      msglen,
