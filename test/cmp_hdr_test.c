...
static int execute_HDR_set1_senderKID_test(CMP_HDR_TEST_FIXTURE *fixture)
{
    ...
    if (!TEST_int_eq(
            ASN1_OCTET_STRING_set(senderKID, rand_data, sizeof(rand_data)), 1))
        goto err;
    if (!TEST_int_eq(ossl_cmp_hdr_set1_senderKID(fixture->hdr, senderKID), 1))
        goto err;
    if (!TEST_int_eq(ASN1_OCTET_STRING_cmp(fixture->hdr->senderKID, senderKID),
                     0))
        goto err;
    res = 1;
err:
...
execute_HDR_generalInfo_push0_item_test(CMP_HDR_TEST_FIXTURE *fixture)
{
    ...
    if (!TEST_true(itav
                   == sk_OSSL_CMP_ITAV_value(fixture->hdr->generalInfo, 0)))
        return 0;
...
execute_HDR_generalInfo_push1_items_test(CMP_HDR_TEST_FIXTURE *fixture)
{
    ...
    if (!TEST_ptr(val) || !TEST_true(ASN1_INTEGER_set(asn1int, 88))) {
...
static int
execute_HDR_set_and_check_implicitConfirm_test(CMP_HDR_TEST_FIXTURE *fixture)
{
...
static int execute_HDR_init_test(CMP_HDR_TEST_FIXTURE *fixture)
{
    ...
    if (!TEST_int_eq(
            0,
            ASN1_OCTET_STRING_cmp(header_nonce, fixture->cmp_ctx->senderNonce)))
        return 0;
...
static int test_HDR_init_with_subject(void)
{
    ...
    if (!TEST_ptr(subject = X509_NAME_new())
        ...
        || !TEST_true(
            OSSL_CMP_CTX_set1_subjectName(fixture->cmp_ctx, subject))) {
        tear_down(fixture);
        fixture = NULL;
    }
