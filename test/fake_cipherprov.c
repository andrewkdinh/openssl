...
static int fake_skeymgmt_export(void *keydata, int selection, OSSL_CALLBACK *param_callback, void *cbarg)
{
    OSSL_PARAM params[3];
    ...
    if (selection & OSSL_SKEYMGMT_SELECT_PARAMETERS) {
        *p = OSSL_PARAM_construct_utf8_string(FAKE_CIPHER_PARAM_KEY_NAME, ctx->key_name, strlen(ctx->key_name));
        p++;
    }

    if (selection & OSSL_SKEYMGMT_SELECT_SECRET_KEY) {
        *p = OSSL_PARAM_construct_octet_string(OSSL_SKEY_PARAM_RAW_BYTES, ctx->key, sizeof(ctx->key));
        p++;
    }
...
static const OSSL_DISPATCH fake_skeymgmt_funcs[] = {{OSSL_FUNC_SKEYMGMT_FREE, (void (*)(void))fake_skeymgmt_free},
...
static int fake_skey_init(PROV_CIPHER_FAKE_CTX *ctx, void *pkeyparam, const unsigned char *iv, size_t ivlen,
...
static int fake_skey_einit(void *vctx, void *pkeyparam, const unsigned char *iv, size_t ivlen,
                           const OSSL_PARAM params[])
{
...
static int fake_skey_dinit(void *vctx, void *pkeyparam, const unsigned char *iv, size_t ivlen,
                           const OSSL_PARAM params[])
{
    PROV_CIPHER_FAKE_CTX *ctx = (PROV_CIPHER_FAKE_CTX *)vctx;
...
static int fake_cipher(void *vctx, unsigned char *out, size_t *outl, size_t outsize, const unsigned char *in,
                       size_t inl)
{
    PROV_CIPHER_FAKE_CTX *ctx = (PROV_CIPHER_FAKE_CTX *)vctx;
    size_t i;
...
static int fake_final(void *vctx, unsigned char *out, size_t *outl, size_t outsize)
{
    *outl = 0;
    return 1;
}
...
static const OSSL_PARAM fake_known_gettable_ctx_params[] = {OSSL_PARAM_size_t(OSSL_CIPHER_PARAM_KEYLEN, NULL),
                                                            OSSL_PARAM_size_t(OSSL_CIPHER_PARAM_BLOCK_SIZE, NULL),
                                                            OSSL_PARAM_END};

static OSSL_FUNC_cipher_gettable_ctx_params_fn fake_gettable_ctx_params;
static const OSSL_PARAM *fake_gettable_ctx_params(ossl_unused void *cctx, ossl_unused void *provctx)
{
...
static const OSSL_PARAM fake_known_settable_ctx_params[] = {OSSL_PARAM_utf8_string(FAKE_CIPHER_PARAM_KEY_NAME, NULL, 0),
                                                            OSSL_PARAM_END};

static OSSL_FUNC_cipher_settable_ctx_params_fn fake_settable_ctx_params;
static const OSSL_PARAM *fake_settable_ctx_params(ossl_unused void *cctx, ossl_unused void *provctx)
{
    return fake_known_settable_ctx_params;
}

static const OSSL_DISPATCH ossl_fake_functions[] = {
    {OSSL_FUNC_CIPHER_NEWCTX, (void (*)(void))fake_newctx},
    ...
    {OSSL_FUNC_CIPHER_GETTABLE_CTX_PARAMS, (void (*)(void))fake_gettable_ctx_params},
    {OSSL_FUNC_CIPHER_SETTABLE_CTX_PARAMS, (void (*)(void))fake_settable_ctx_params},
    {OSSL_FUNC_CIPHER_ENCRYPT_SKEY_INIT, (void (*)(void))fake_skey_einit},
    {OSSL_FUNC_CIPHER_DECRYPT_SKEY_INIT, (void (*)(void))fake_skey_dinit},
    OSSL_DISPATCH_END};

static const OSSL_ALGORITHM fake_cipher_algs[] = {{"fake_cipher", FAKE_CIPHER_FETCH_PROPS, ossl_fake_functions},
                                                  {NULL, NULL, NULL}};

static const OSSL_ALGORITHM *fake_cipher_query(void *provctx, int operation_id, int *no_cache)
{
    *no_cache = 0;
    switch (operation_id) {
    case OSSL_OP_CIPHER:
...
static int fake_cipher_provider_init(const OSSL_CORE_HANDLE *handle, const OSSL_DISPATCH *in, const OSSL_DISPATCH **out,
                                     void **provctx)
{
...
OSSL_PROVIDER *fake_cipher_start(OSSL_LIB_CTX *libctx)
{
    ...
    if (!TEST_true(OSSL_PROVIDER_add_builtin(libctx, FAKE_PROV_NAME, fake_cipher_provider_init))
