...
static int ctx_from_key_params(PROV_CIPHER_FAKE_CTX *pctx,
...
static void fake_skeymgmt_free(void *keydata) { OPENSSL_free(keydata); }

static void *fake_skeymgmt_import(void *provctx, int selection,
                                  const OSSL_PARAM *p) {
...
static int fake_skeymgmt_export(void *keydata, int selection,
  ...
  if (selection & OSSL_SKEYMGMT_SELECT_PARAMETERS) {
    *p = OSSL_PARAM_construct_utf8_string(FAKE_CIPHER_PARAM_KEY_NAME,
                                          ctx->key_name, strlen(ctx->key_name));
    p++;
  }

  if (selection & OSSL_SKEYMGMT_SELECT_SECRET_KEY) {
    *p = OSSL_PARAM_construct_octet_string(OSSL_SKEY_PARAM_RAW_BYTES, ctx->key,
                                           sizeof(ctx->key));
    p++;
  }
...
{"fake_cipher", FAKE_CIPHER_FETCH_PROPS, fake_skeymgmt_funcs,
 "Fake Cipher Key Management"},
{NULL, NULL, NULL, NULL}};
...
static void fake_freectx(void *vctx) { OPENSSL_free(vctx); }
...
static int fake_skey_einit(void *vctx, void *pkeyparam, const unsigned char *iv,
...
static int fake_skey_dinit(void *vctx, void *pkeyparam, const unsigned char *iv,
                           size_t ivlen, const OSSL_PARAM params[]) {
  PROV_CIPHER_FAKE_CTX *ctx = (PROV_CIPHER_FAKE_CTX *)vctx;
...
static int fake_get_ctx_params(void *vctx, OSSL_PARAM params[]) {
  ...
  p = OSSL_PARAM_locate(params, OSSL_CIPHER_PARAM_KEYLEN);
  if (p != NULL &&
      !OSSL_PARAM_set_size_t(p, sizeof((PROV_CIPHER_FAKE_CTX *)vctx)->key)) {
    ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_SET_PARAMETER);
    return 0;
  }
  return 1;
...
OSSL_PARAM_utf8_string(FAKE_CIPHER_PARAM_KEY_NAME, NULL, 0), OSSL_PARAM_END};
...
static const OSSL_ALGORITHM *fake_cipher_query(void *provctx, int operation_id,
                                               int *no_cache) {
  *no_cache = 0;
  switch (operation_id) {
  case OSSL_OP_CIPHER:
...
void fake_cipher_finish(OSSL_PROVIDER *p) { OSSL_PROVIDER_unload(p); }
