...
static int test_property_string(void)
{
     ...
     * pristine state.
     */
    if (TEST_ptr(ctx = OSSL_LIB_CTX_new()) && TEST_ptr(store = ossl_method_store_new(ctx))
        && TEST_int_eq(ossl_property_name(ctx, "fnord", 0), 0) && TEST_int_ne(ossl_property_name(ctx, "fnord", 1), 0)
        && TEST_int_ne(ossl_property_name(ctx, "name", 1), 0)
        /* Pre loaded names */
        && TEST_str_eq(ossl_property_name_str(ctx, 1), "provider")
        && TEST_str_eq(ossl_property_name_str(ctx, 2), "version") && TEST_str_eq(ossl_property_name_str(ctx, 3), "fips")
        && TEST_str_eq(ossl_property_name_str(ctx, 4), "output") && TEST_str_eq(ossl_property_name_str(ctx, 5), "input")
        ...
        && TEST_int_eq(ossl_property_value(ctx, "fnord", 0), 0) && TEST_int_ne(i = ossl_property_value(ctx, "no", 0), 0)
        && TEST_int_ne(j = ossl_property_value(ctx, "yes", 0), 0) && TEST_int_ne(i, j)
        && TEST_int_eq(ossl_property_value(ctx, "yes", 1), j) && TEST_int_eq(ossl_property_value(ctx, "no", 1), i)
        ...
        && TEST_int_ne(ossl_property_name(ctx, "fnord", 0), ossl_property_value(ctx, "fnord", 0)))
        res = 1;
...
static int test_property_query_value_create(void)
{
    ...
    if (TEST_ptr(store = ossl_method_store_new(NULL)) && add_property_names("wood", NULL)
        ...
        && TEST_int_eq(ossl_property_match_count(q, p), -1) && TEST_int_eq(ossl_property_match_count(q, o), 1))
        r = 1;
...
static const struct {
    ...
    {"clouds=pink, urn=red", "urn=blue, colour=green", "urn=blue, colour=green, clouds=pink"},
...
static int test_property_merge(int n)
{
    ...
    if (TEST_ptr(store = ossl_method_store_new(NULL))
        ...
        && TEST_ptr(q_global = ossl_parse_query(NULL, merge_tests[n].q_global, 0))
...
static int test_property_defn_cache(void)
{
    ...
    r = TEST_ptr(store = ossl_method_store_new(NULL)) && add_property_names("red", "blue", NULL)
        && TEST_ptr(red = ossl_parse_property(NULL, "red")) && TEST_ptr(blue = ossl_parse_property(NULL, "blue"))
        && TEST_ptr_ne(red, blue) && TEST_true(ossl_prop_defn_set(NULL, "red", &red));
    ...
    r = r && TEST_ptr_eq(ossl_prop_defn_get(NULL, "red"), red) && TEST_ptr_eq(ossl_prop_defn_get(NULL, "blue"), blue)
        && TEST_ptr(blue2 = ossl_parse_property(NULL, "blue")) && TEST_ptr_ne(blue2, blue)
    ...
    r = r && TEST_ptr_eq(blue2, blue) && TEST_ptr_eq(ossl_prop_defn_get(NULL, "blue"), blue);
...
static const struct {
    ...
    const char *query;
    int e;
} definition_tests[] = {{"alpha", "alpha=yes", 1},         {"alpha=no", "alpha", -1},       {"alpha=1", "alpha=1", 1},
                        {"alpha=2", "alpha=1", -1},        {"alpha", "omega", -1},          {"alpha", "?omega", 0},
                        {"alpha", "?omega=1", 0},          {"alpha", "?omega=no", 1},       {"alpha", "?omega=yes", 0},
                        {"alpha, omega", "?omega=yes", 1}, {"alpha, omega", "?omega=no", 0}};
...
static int test_definition_compares(int n)
{
    ...
    r = TEST_ptr(store = ossl_method_store_new(NULL)) && add_property_names("alpha", "omega", NULL)
...
static int test_property(void)
{
    ...
    for (i = 0; i < OSSL_NELEM(impls); i++)
        if (!TEST_true(ossl_method_store_add(store, *impls[i].prov, impls[i].nid, impls[i].prop, impls[i].impl, &up_ref,
                                             &down_ref))) {
            TEST_note("iteration %zd", i + 1);
            goto err;
        }
    ...
    for (i = 0; i < OSSL_NELEM(queries); i++) {
        ...
        if (queries[i].prov == &fake_prov1) {
            if (!TEST_true(ossl_method_store_fetch(store, queries[i].nid, queries[i].prop, &fake_prov1, &result))
                || !TEST_ptr_eq(fake_prov1, &fake_provider1) || !TEST_str_eq((char *)result, queries[i].expected)) {
                TEST_note("iteration %zd", i + 1);
                ossl_property_free(pq);
                goto err;
            }
            ...
            if (!TEST_false(ossl_method_store_fetch(store, queries[i].nid, queries[i].prop, &fake_prov1, &result))
                || !TEST_ptr_eq(fake_prov1, &fake_provider1) || !TEST_ptr_null(result)) {
                TEST_note("iteration %zd", i + 1);
                ossl_property_free(pq);
                goto err;
            }
    ...
    for (i = 0; i < OSSL_NELEM(queries); i++) {
        ...
        if (queries[i].prov == &fake_prov2) {
            if (!TEST_true(ossl_method_store_fetch(store, queries[i].nid, queries[i].prop, &fake_prov2, &result))
                || !TEST_ptr_eq(fake_prov2, &fake_provider2) || !TEST_str_eq((char *)result, queries[i].expected)) {
                TEST_note("iteration %zd", i + 1);
                ossl_property_free(pq);
                goto err;
            }
            ...
            if (!TEST_false(ossl_method_store_fetch(store, queries[i].nid, queries[i].prop, &fake_prov2, &result))
                || !TEST_ptr_eq(fake_prov2, &fake_provider2) || !TEST_ptr_null(result)) {
                TEST_note("iteration %zd", i + 1);
                ossl_property_free(pq);
                goto err;
            }
...
static int test_query_cache_stochastic(void)
{
    ...
    for (i = 1; i <= max; i++) {
        ...
        if (!TEST_true(ossl_method_store_add(store, &prov, i, buf, "abc", &up_ref, &down_ref))
            || !TEST_true(ossl_method_store_cache_set(store, &prov, i, buf, v + i, &up_ref, &down_ref))
            || !TEST_true(ossl_method_store_cache_set(store, &prov, i, "n=1234", "miss", &up_ref, &down_ref))) {
    ...
    for (i = 1; i <= max; i++) {
        BIO_snprintf(buf, sizeof(buf), "n=%d\n", i);
        if (!ossl_method_store_cache_get(store, NULL, i, buf, &result) || result != v + i)
            errors++;
    }
...
static struct {
    const char *in;
    const char *out;
} to_string_tests[] = {{"fips=yes", "fips=yes"},
                       {"fips!=yes", "fips!=yes"},
                       {"fips = yes", "fips=yes"},
                       {"fips", "fips=yes"},
                       {"fips=no", "fips=no"},
...
static int test_property_list_to_string(int i)
{
    ...
    if (to_string_tests[i].in != NULL && !TEST_ptr(pl = ossl_parse_query(NULL, to_string_tests[i].in, 1)))
        goto err;
    ...
    if (!TEST_ptr(buf) || !TEST_size_t_eq(ossl_property_list_to_string(NULL, pl, buf, bufsize), bufsize)
        || !TEST_str_eq(to_string_tests[i].out, buf) || !TEST_size_t_eq(bufsize, strlen(to_string_tests[i].out) + 1))
        goto err;
