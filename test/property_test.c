...
static int up_ref(void *p) { return 1; }
...
static int test_property_string(void) {
  ...
  if (TEST_ptr(ctx = OSSL_LIB_CTX_new()) &&
      ...
      TEST_int_ne(i, j) && TEST_int_eq(ossl_property_value(ctx, "yes", 1), j) &&
...
static int test_register_deregister(void) {
  ...
  for (i = 0; i < OSSL_NELEM(impls); i++)
    if (!TEST_true(ossl_method_store_add(store, &prov, impls[i].nid,
                                         impls[i].prop, impls[i].impl, &up_ref,
                                         &down_ref))) {
...
static int test_property(void) {
  ...
  for (i = 0; i < OSSL_NELEM(impls); i++)
    if (!TEST_true(ossl_method_store_add(store, *impls[i].prov, impls[i].nid,
                                         impls[i].prop, impls[i].impl, &up_ref,
                                         &down_ref))) {
      TEST_note("iteration %zd", i + 1);
      goto err;
  ...
  for (i = 0; i < OSSL_NELEM(queries); i++) {
    ...
    if (!TEST_true(ossl_method_store_fetch(
        store, queries[i].nid, queries[i].prop, &nullprov, &result)) ||
  ...
  for (i = 0; i < OSSL_NELEM(queries); i++) {
    ...
    if (queries[i].prov == &fake_prov1) {
      if (!TEST_true(ossl_method_store_fetch(
          store, queries[i].nid, queries[i].prop, &fake_prov1, &result)) ||
  ...
  for (i = 0; i < OSSL_NELEM(queries); i++) {
    ...
    if (queries[i].prov == &fake_prov2) {
      if (!TEST_true(ossl_method_store_fetch(
          store, queries[i].nid, queries[i].prop, &fake_prov2, &result)) ||
...
static int test_query_cache_stochastic(void) {
  ...
  for (i = 1; i <= max; i++) {
    ...
    if (!TEST_true(ossl_method_store_add(store, &prov, i, buf, "abc", &up_ref,
                                         &down_ref)) ||
        !TEST_true(ossl_method_store_cache_set(store, &prov, i, buf, v + i,
                                               &up_ref, &down_ref)) ||
        !TEST_true(ossl_method_store_cache_set(store, &prov, i, "n=1234",
                                               "miss", &up_ref, &down_ref))) {
...
static int test_property_list_to_string(int i) {
  ...
  if (!TEST_ptr(buf) ||
      !TEST_size_t_eq(ossl_property_list_to_string(NULL, pl, buf, bufsize),
                      bufsize) ||
