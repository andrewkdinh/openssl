...
static OSSL_PARAM *construct_tls1_prf_params(const char *digest,
    ...
    *p++ = OSSL_PARAM_construct_octet_string(
    OSSL_KDF_PARAM_SEED, (unsigned char *)seed, strlen(seed));
...
static int do_kdf_hkdf_gettables(int extract_only, int has_digest) {
    ...
    if (!TEST_ptr(params = construct_hkdf_params(
                  has_digest ? "sha256" : NULL, "secret", 6, "salt",
    ...
    if (!TEST_ptr(gettables = EVP_KDF_CTX_gettable_params(kctx)) ||
        !TEST_ptr(p =
                  OSSL_PARAM_locate_const(gettables, OSSL_KDF_PARAM_SIZE)) ||
        !TEST_ptr(p =
                  OSSL_PARAM_locate_const(gettables, OSSL_KDF_PARAM_MODE)) ||
        !TEST_ptr(p =
                  OSSL_PARAM_locate_const(gettables, OSSL_KDF_PARAM_DIGEST)) ||
        !TEST_ptr(p =
                  OSSL_PARAM_locate_const(gettables, OSSL_KDF_PARAM_SALT)) ||
...
static OSSL_PARAM *construct_pbkdf1_params(char *pass, char *digest, char *salt,
    ...
    *p++ = OSSL_PARAM_construct_octet_string(
    OSSL_KDF_PARAM_PASSWORD, (unsigned char *)pass, strlen(pass));
    *p++ = OSSL_PARAM_construct_octet_string(
    OSSL_KDF_PARAM_SALT, (unsigned char *)salt, strlen(salt));
...
static int test_kdf_pbkdf1(void) {
    ...
    0xfb, 0x83, 0x4d, 0x36, 0x6d, 0xbc, 0x53, 0x87, 0x35,
...
static int test_kdf_kbkdf_6803_128(void) {
    ...
    {0xD1, 0x55, 0x77, 0x5A, 0x20, 0x9D, 0x05, 0xF0, 0x2B, 0x38, 0xD4, 0x2A,
     0x38, 0x9E, 0x5A, 0x56},
    {0x64, 0xDF, 0x83, 0xF8, 0x5A, 0x53, 0x2F, 0x17, 0x57, 0x7D, 0x8C, 0x37,
     0x03, 0x57, 0x96, 0xAB},
    {0x3E, 0x4F, 0xBD, 0xF3, 0x0F, 0xB8, 0x25, 0x9C, 0x42, 0x5C, 0xB6, 0xC9,
     0x6F, 0x1F, 0x46, 0x35}};
...
static int test_kdf_kbkdf_invalid_mac(void) {
    ...
    params =
    construct_kbkdf_params("sha256", "blah", key, 1, "prf", "test", &r);
    if (!TEST_ptr(params))
        return 0;
...
static int test_kdf_kbkdf_invalid_r(void) {
    ...
    params =
    construct_kbkdf_params("sha256", "HMAC", key, 1, "prf", "test", &r);
    if (!TEST_ptr(params))
        return 0;
...
static int test_kdf_kbkdf_empty_key(void) {
    ...
    params =
    construct_kbkdf_params("sha256", "HMAC", key, 0, "prf", "test", &r);
    if (!TEST_ptr(params))
        return 0;
...
static int test_kdf_kbkdf_8009_prf1(void) {
    ...
    OSSL_KDF_PARAM_KEY, input_key, sizeof(input_key));
    params[i++] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SALT, label,
                                                    strlen(label));
    params[i++] = OSSL_PARAM_construct_octet_string(
...
static int test_kdf_kbkdf_kmac(void) {
    ...
    ,
    ...
    static unsigned char output[] = {
    };
    unsigned char result[sizeof(output)] = {0};
...
static int test_kdf_ss_kmac(void) {
    ...
    static unsigned char salt[] = {0x36, 0x38, 0x27, 0x1c, 0xcd, 0x68,
                                   ...
                                   0x39, 0xef, 0x3f, 0x89};
    static const unsigned char expected[sizeof(out)] = {
    0xe9, 0xc1, 0x84, 0x53, 0xa0, 0x62, 0xb5, 0x3b, 0xdb, 0xfc, 0xbb,
    0x5a, 0x34, 0xbd, 0xb8, 0xe5, 0xe7, 0x07, 0xee, 0xbb, 0x5d, 0xd1,
    0x34, 0x42, 0x43, 0xd8, 0xcf, 0xc2, 0xc2, 0xe6, 0x33, 0x2f, 0x91,
    0xbd, 0xa5, 0x86, 0xf3, 0x7d, 0xe4, 0x8a, 0x65, 0xd4, 0xc5, 0x14,
    0xfd, 0xef, 0xaa, 0x1e, 0x67, 0x54, 0xf3, 0x73, 0xd2, 0x38, 0xe1,
    0x95, 0xae, 0x15, 0x7e, 0x1d, 0xe8, 0x14, 0x98, 0x03};
...
static int test_kdf_krb5kdf(void) {
    ...
    OSSL_PARAM params[4], *p = params;
    unsigned char out[16];
    static unsigned char key[] = {0x42, 0x26, 0x3C, 0x6E, 0x89, 0xF4,
                                  0xFC, 0x28, 0xB8, 0xDF, 0x68, 0xEE,
                                  0x09, 0x79, 0x9F, 0x15};
...
static int test_kdf_hmac_drbg_settables(void) {
    ...
    for (i = 0; settableparams[i].key != NULL; ++i) {
        /* Skip "properties" key since it returns 1 unless the digest is also
         * set */
    ...
    OSSL_KDF_PARAM_HMACDRBG_ENTROPY, (char *)ent, sizeof(ent));
    ...
    OSSL_ALG_PARAM_DIGEST, digestname, sizeof(digestname));
...
static int test_kdf_hmac_drbg_gettables(void) {
    ...
    for (i = 0; gettableparams[i].key != NULL; ++i) {
        params[0] = OSSL_PARAM_construct_utf8_string(gettableparams[i].key, buf,
                                                     sizeof(buf));
        if (!TEST_int_le(EVP_KDF_CTX_get_params(kctx, params), 0))
            goto err;
    }
