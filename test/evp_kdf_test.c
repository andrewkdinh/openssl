...
static EVP_KDF_CTX *get_kdfbyname_libctx(OSSL_LIB_CTX *libctx,
                                         const char *name) {
...
static OSSL_PARAM *construct_tls1_prf_params(const char *digest,
                                             const char *secret,
                                             const char *seed) {
  ...
  *p++ =
  OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_DIGEST, (char *)digest, 0);
  *p++ = OSSL_PARAM_construct_octet_string(
  OSSL_KDF_PARAM_SECRET, (unsigned char *)secret, strlen(secret));
...
static OSSL_PARAM *construct_hkdf_params(char *digest, char *key, size_t keylen,
                                         char *salt, char *info) {
  ...
  *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_KEY,
  ...
  if (info != NULL)
    *p++ =
    OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_INFO, info, strlen(info));
  else
    *p++ =
...
static int do_kdf_hkdf_gettables(int extract_only, int has_digest) {
  ...
  if (!TEST_ptr(params =
                construct_hkdf_params(has_digest ? "sha256" : NULL, "secret", 6,
                                      "salt", extract_only ? NULL : "label")) ||
      !TEST_ptr(kctx = get_kdfbyname(OSSL_KDF_NAME_HKDF)) ||
      !TEST_true(EVP_KDF_CTX_set_params(kctx, params)))
    goto err;
  ...
  if (!TEST_ptr(gettables = EVP_KDF_CTX_gettable_params(kctx)) ||
      ...
      !TEST_ptr(p =
                OSSL_PARAM_locate_const(gettables, OSSL_KDF_PARAM_DIGEST)) ||
  ...
  params_get[0] = OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_DIGEST,
  ...
  params_get[0] = OSSL_PARAM_construct_utf8_string(
  OSSL_KDF_PARAM_MODE, mode_utf8, sizeof(mode_utf8));
  ...
  if (!TEST_int_eq(EVP_KDF_CTX_get_params(kctx, params_get), 1) ||
      !TEST_str_eq(mode_utf8,
                   extract_only ? "EXTRACT_ONLY" : "EXTRACT_AND_EXPAND") ||
      !TEST_int_eq(mode_int, extract_only
...
static int test_kdf_hkdf_gettables(void) { return do_kdf_hkdf_gettables(0, 1); }
...
static int test_kdf_hkdf_set_invalid_mode(void) {
  ...
  if (!TEST_ptr(kctx = get_kdfbyname(OSSL_KDF_NAME_HKDF)))
    goto end;
  params[0] =
  OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_MODE, "BADMODE", 0);
  params[1] = OSSL_PARAM_construct_end();
...
static int test_kdf_pbkdf1(void) {
  ...
  0xfb, 0x83, 0x4d, 0x36, 0x6d, 0xbc, 0x53, 0x87, 0x35, 0x1b, 0x34, 0x75, 0x95,
  0x88, 0x32, 0x4f, 0x3e, 0x82, 0x81, 0x01, 0x21, 0x93, 0x64, 0x00, 0xcc};
...
static int test_kdf_scrypt(void) {
  ...
  *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_PASSWORD,
                                           (char *)"password", 8);
  *p++ =
  OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SALT, (char *)"NaCl", 4);
...
static int test_kdf_ss_hash(void) {
  ...
  0x6d, 0xbd, 0xc2, 0x3f, 0x04, 0x54, 0x88, 0xe4, 0x06, 0x27, 0x57, 0xb0,
  0x6b, 0x9e, 0xba, 0xe1, 0x83, 0xfc, 0x5a, 0x59, 0x46, 0xd8, 0x0d, 0xb9,
  0x3f, 0xec, 0x6f, 0x62, 0xec, 0x07, 0xe3, 0x72, 0x7f, 0x01, 0x26, 0xae,
  0xd1, 0x2c, 0xe4, 0xb2, 0x62, 0xf4, 0x7d, 0x48, 0xd5, 0x42, 0x87, 0xf8,
  0x1d, 0x47, 0x4c, 0x7c, 0x3b, 0x18, 0x50, 0xe9};
  static unsigned char other[] = {
  0xa1, 0xb2, 0xc3, 0xd4, 0xe5, 0x43, 0x41, 0x56, 0x53, 0x69, 0x64, 0x3c,
  ...
  static const unsigned char expected[sizeof(out)] = {
  0xa4, 0x62, 0xde, 0x16, 0xa8, 0x9d, 0xe8,
  0x46, 0x6e, 0xf5, 0x46, 0x0b, 0x47, 0xb8};

  *p++ =
  OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_DIGEST, (char *)"sha224", 0);
  *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_KEY, z, sizeof(z));
  *p++ =
  OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_INFO, other, sizeof(other));
  *p = OSSL_PARAM_construct_end();
...
static int test_kdf_x963(void) {
   ...
   */
  static unsigned char z[] = {
  0x00, 0xaa, 0x5b, 0xb7, 0x9b, 0x33, 0xe3, 0x89, 0xfa, 0x58, 0xce,
  0xad, 0xc0, 0x47, 0x19, 0x7f, 0x14, 0xe7, 0x37, 0x12, 0xf4, 0x52,
  0xca, 0xa9, 0xfc, 0x4c, 0x9a, 0xdb, 0x36, 0x93, 0x48, 0xb8, 0x15,
  0x07, 0x39, 0x2f, 0x1a, 0x86, 0xdd, 0xfd, 0xb7, 0xc4, 0xff, 0x82,
  0x31, 0xc4, 0xbd, 0x0f, 0x44, 0xe4, 0x4a, 0x1b, 0x55, 0xb1, 0x40,
  0x47, 0x47, 0xa9, 0xe2, 0xe7, 0x53, 0xf5, 0x5e, 0xf0, 0x5a, 0x2d};
  static unsigned char shared[] = {0xe3, 0xb5, 0xb4, 0xc1, 0xb0, 0xd5,
                                   0xcf, 0x1d, 0x2b, 0x3a, 0x2f, 0x99,
                                   0x37, 0x89, 0x5d, 0x31};
  static const unsigned char expected[sizeof(out)] = {
  };
...
static int test_kdf_kbkdf_6803_128(void) {
  ...
  for (i = 0; i < 3; i++) {
    p = 0;
    params[p++] = OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_CIPHER,
                                                   "CAMELLIA-128-CBC", 0);
    params[p++] =
    OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_MAC, "CMAC", 0);
    params[p++] =
    OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_MODE, "FEEDBACK", 0);
    ...
    params[p++] =
    OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SEED, iv, sizeof(iv));
...
static int test_kdf_kbkdf_6803_256(void) {
  ...
  OSSL_PARAM params[7];
  static unsigned char input_key[] = {
  0xB9, 0xD6, 0x82, 0x8B, 0x20, 0x56, 0xB7, 0xBE, 0x65, 0x6D, 0x88,
  0xA1, 0x23, 0xB1, 0xFA, 0xC6, 0x82, 0x14, 0xAC, 0x2B, 0x72, 0x7E,
  0xCF, 0x5F, 0x69, 0xAF, 0xE0, 0xC4, 0xDF, 0x2A, 0x6D, 0x2C,
  };
  static unsigned char constants[][5] = {
  {0x00, 0x00, 0x00, 0x02, 0x99},
  {0x00, 0x00, 0x00, 0x02, 0xaa},
  ...
  {
  0xE4, 0x67, 0xF9, 0xA9, 0x55, 0x2B, 0xC7, 0xD3, 0x15, 0x5A, 0x62,
  0x20, 0xAF, 0x9C, 0x19, 0x22, 0x0E, 0xEE, 0xD4, 0xFF, 0x78, 0xB0,
  0xD1, 0xE6, 0xA1, 0x54, 0x49, 0x91, 0x46, 0x1A, 0x9E, 0x50,
  },
  {
  0x41, 0x2A, 0xEF, 0xC3, 0x62, 0xA7, 0x28, 0x5F, 0xC3, 0x96, 0x6C,
  0x6A, 0x51, 0x81, 0xE7, 0x60, 0x5A, 0xE6, 0x75, 0x23, 0x5B, 0x6D,
  0x54, 0x9F, 0xBF, 0xC9, 0xAB, 0x66, 0x30, 0xA4, 0xC6, 0x04,
  },
  {
  0xFA, 0x62, 0x4F, 0xA0, 0xE5, 0x23, 0x99, 0x3F, 0xA3, 0x88, 0xAE,
  0xFD, 0xC6, 0x7E, 0x67, 0xEB, 0xCD, 0x8C, 0x08, 0xE8, 0xA0, 0x24,
  0x6B, 0x1D, 0x73, 0xB0, 0xD1, 0xDD, 0x9F, 0xC5, 0x82, 0xB0,
  },
  };
  static unsigned char iv[16] = {0};
...
static OSSL_PARAM *construct_kbkdf_params(char *digest, char *mac,
                                          unsigned char *key, size_t keylen,
                                          char *salt, char *info, int *r) {
...
static int test_kdf_kbkdf_8009_prf1(void) {
  ...
  0x9D, 0x18, 0x86, 0x16, 0xF6, 0x38, 0x52, 0xFE, 0x86, 0x91, 0x5B,
  0xB8, 0x40, 0xB4, 0xA8, 0x86, 0xFF, 0x3E, 0x6B, 0xB0, 0xF8, 0x19,
  0xB4, 0x9B, 0x89, 0x33, 0x93, 0xD3, 0x93, 0x85, 0x42, 0x95,
  };
  ...
  params[i++] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_KEY, input_key,
                                                  sizeof(input_key));
  params[i++] =
  OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SALT, label, strlen(label));
  params[i++] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_INFO,
                                                  prf_input, strlen(prf_input));
...
static int test_kdf_kbkdf_8009_prf2(void) {
  ...
  static unsigned char input_key[] = {
  0x6D, 0x40, 0x4D, 0x37, 0xFA, 0xF7, 0x9F, 0x9D, 0xF0, 0xD3, 0x35,
  0x68, 0xD3, 0x20, 0x66, 0x98, 0x00, 0xEB, 0x48, 0x36, 0x47, 0x2E,
  0xA8, 0xA0, 0x26, 0xD1, 0x6B, 0x71, 0x82, 0x46, 0x0C, 0x52,
  };
  static unsigned char output[] = {
  0x98, 0x01, 0xF6, 0x9A, 0x36, 0x8C, 0x2B, 0xF6, 0x75, 0xE5, 0x95, 0x21,
  0xE1, 0x77, 0xD9, 0xA0, 0x7F, 0x67, 0xEF, 0xE1, 0xCF, 0xDE, 0x8D, 0x3C,
  0x8D, 0x6F, 0x6A, 0x02, 0x56, 0xE3, 0xB1, 0x7D, 0xB3, 0xC1, 0xB6, 0x2A,
  0xD1, 0xB8, 0x55, 0x33, 0x60, 0xD1, 0x73, 0x67, 0xEB, 0x15, 0x14, 0xD2,
  };
  unsigned char result[sizeof(output)] = {0};
...
static int test_kdf_kbkdf_fixedinfo(void) {
  ...
  *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_KEY, input_key,
                                           sizeof(input_key));
  *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_INFO, fixed_input,
                                           sizeof(fixed_input));
  *p++ = OSSL_PARAM_construct_int(OSSL_KDF_PARAM_KBKDF_USE_L, &use_l);
  *p++ =
  OSSL_PARAM_construct_int(OSSL_KDF_PARAM_KBKDF_USE_SEPARATOR, &use_separator);
  *p = OSSL_PARAM_construct_end();
...
static int test_kdf_kbkdf_kmac(void) {
  ...
  static unsigned char input_key[] = {
  0xDD, 0x81, 0xEF, 0xC8, 0x2C, 0xDD, 0xEC, 0x51, 0xC4, 0x09, 0xBD,
  ...
  };
  static unsigned char label[] = {
  0xA5, 0xDE, 0x2A, 0x0A, 0xF0, 0xDA, 0x59, 0x04, 0xCC, 0xFF, 0x50,
  0xD3, 0xA5, 0xD2, 0xDE, 0xA3, 0x33, 0xC0, 0x27, 0xED, 0xDC, 0x6A,
  0x54, 0x54, 0x95, 0x78, 0x74, 0x0D, 0xE7, 0xB7, 0x92, 0xD6, 0x64,
  0xD5, 0xFB, 0x1F, 0x0F, 0x87, 0xFD, 0x65, 0x79, 0x8B, 0x81, 0x83,
  0x95, 0x40, 0x7A, 0x19, 0x8D, 0xCA, 0xE0, 0x4A, 0x93, 0xA8};
  static unsigned char output[] = {
  };
  unsigned char result[sizeof(output)] = {0};
...
static int test_kdf_ss_hmac(void) {
  ...
  unsigned char out[16];
  static unsigned char z[] = {0xb7, 0x4a, 0x14, 0x9a, 0x16, 0x15, 0x46,
                              0xf8, 0xc2, 0x0b, 0x06, 0xac, 0x4e, 0xd4};
  static unsigned char other[] = {0x34, 0x8a, 0x37, 0xa2, 0x7e, 0xf1,
                                  0x28, 0x2f, 0x5f, 0x02, 0x0d, 0xcc};
  static unsigned char salt[] = {0x36, 0x38, 0x27, 0x1c, 0xcd, 0x68,
                                 0xa2, 0x5d, 0xc2, 0x4e, 0xcd, 0xdd,
                                 0x39, 0xef, 0x3f, 0x89};
  static const unsigned char expected[sizeof(out)] = {
  0x44, 0xf6, 0x76, 0xe8, 0x5c, 0x1b, 0x1a, 0x8b,
...
static int test_kdf_ss_kmac(void) {
  ...
  static unsigned char salt[] = {0x36, 0x38, 0x27, 0x1c, 0xcd, 0x68,
                                 ...
                                 0x39, 0xef, 0x3f, 0x89};
  static const unsigned char expected[sizeof(out)] = {
  0xe9, 0xc1, 0x84, 0x53, 0xa0, 0x62, 0xb5, 0x3b, 0xdb, 0xfc, 0xbb, 0x5a, 0x34,
  0xbd, 0xb8, 0xe5, 0xe7, 0x07, 0xee, 0xbb, 0x5d, 0xd1, 0x34, 0x42, 0x43, 0xd8,
  0xcf, 0xc2, 0xc2, 0xe6, 0x33, 0x2f, 0x91, 0xbd, 0xa5, 0x86, 0xf3, 0x7d, 0xe4,
  0x8a, 0x65, 0xd4, 0xc5, 0x14, 0xfd, 0xef, 0xaa, 0x1e, 0x67, 0x54, 0xf3, 0x73,
  0xd2, 0x38, 0xe1, 0x95, 0xae, 0x15, 0x7e, 0x1d, 0xe8, 0x14, 0x98, 0x03};
...
static int test_kdf_sshkdf(void) {
  ...
  };
  ...
  0x23, 0xb2, 0x74, 0x5e, 0xbc, 0x99, 0xb2, 0x67, 0xda, 0xa8, 0x6a,
  0x4a, 0xa7, 0x69, 0x72, 0x39, 0x08, 0x82, 0x53, 0xf6, 0x42};
  static const unsigned char expected[sizeof(out)] = {0x41, 0xff, 0x2e, 0xad,
                                                      0x16, 0x83, 0xf1, 0xe6};
  ...
  *p++ = OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_SSHKDF_TYPE, &kdftype,
                                          sizeof(kdftype));
  *p = OSSL_PARAM_construct_end();
...
static int test_kdfs_same(EVP_KDF *kdf1, EVP_KDF *kdf2) {
   ...
   */
  return TEST_ptr_eq(EVP_KDF_get0_provider(kdf1),
...
static int test_kdf_get_kdf(void) {
  ...
  if (!TEST_ptr(obj = OBJ_nid2obj(NID_id_pbkdf2)) ||
      ...
      !TEST_ptr(kdf2 =
                EVP_KDF_fetch(NULL, OBJ_nid2sn(OBJ_obj2nid(obj)), NULL)) ||
...
static int test_kdf_x942_asn1(void) {
  ...
  static unsigned char z[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06,
                              0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d,
                              0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13};
  static const unsigned char expected[sizeof(out)] = {
  0xa0, 0x96, 0x61, 0x39, 0x23, 0x76, 0xf7, 0x04, 0x4d, 0x90, 0x52, 0xa3,
  0x97, 0x88, 0x32, 0x46, 0xb6, 0x7f, 0x5f, 0x1e, 0xf6, 0x3e, 0xb5, 0xfb};

  *p++ =
  OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_DIGEST, (char *)"sha1", 0);
  *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_KEY, z, sizeof(z));
  *p++ =
  OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_CEK_ALG, (char *)cek_alg, 0);
  *p = OSSL_PARAM_construct_end();
...
static int test_kdf_krb5kdf(void) {
  ...
  *p++ =
  OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_KEY, key, sizeof(key));
  *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_CONSTANT, constant,
                                           sizeof(constant));
  *p = OSSL_PARAM_construct_end();
...
static int test_kdf_hmac_drbg_settables(void) {
  ...
  for (i = 0; settableparams[i].key != NULL; ++i) {
    ...
    if (OPENSSL_strcasecmp(settableparams[i].key, OSSL_KDF_PARAM_PROPERTIES) !=
        0) {
  ...
  params[1] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_HMACDRBG_NONCE,
                                                (char *)ent, sizeof(ent));
  params[2] =
  OSSL_PARAM_construct_utf8_string(OSSL_ALG_PARAM_DIGEST, "SHA256", 0);
  params[3] =
  OSSL_PARAM_construct_utf8_string(OSSL_ALG_PARAM_PROPERTIES, "", 0);
  ...
  params[1] = OSSL_PARAM_construct_utf8_string(OSSL_ALG_PARAM_MAC, macname,
                                               sizeof(macname));
  ...
  params[0] =
  OSSL_PARAM_construct_utf8_string(OSSL_ALG_PARAM_DIGEST, "shake256", 0);
  params[1] = OSSL_PARAM_construct_end();
  if (!TEST_int_le(EVP_KDF_CTX_set_params(kctx, params), 0))
    goto err;
...
static int test_kdf_hmac_drbg_gettables(void) {
  ...
  for (i = 0; gettableparams[i].key != NULL; ++i) {
    params[0] =
    OSSL_PARAM_construct_utf8_string(gettableparams[i].key, buf, sizeof(buf));
    if (!TEST_int_le(EVP_KDF_CTX_get_params(kctx, params), 0))
      goto err;
  }
  ret = 1;
...
static int test_kbkdf_mac_change(void) {
  ...
  0x3e, 0xdc, 0x6b, 0x5b, 0x8f, 0x7a, 0xad, 0xbd, 0x71, 0x37, 0x32,
  0xb4, 0x82, 0xb8, 0xf9, 0x79, 0x28, 0x6e, 0x1e, 0xa3, 0xb8, 0xf8,
  0xf9, 0x9c, 0x30, 0xc8, 0x84, 0xcf, 0xe3, 0x34, 0x9b, 0x83};
  static /* const */ unsigned char info[] = {
  0x98, 0xe9, 0x98, 0x8b, 0xb4, 0xcc, 0x8b, 0x34, 0xd7, 0x92, 0x2e, 0x1c,
  ...
  static const unsigned char output[] = {0x6c, 0x03, 0x76, 0x52, 0x99, 0x06,
                                         0x74, 0xa0, 0x78, 0x44, 0x73, 0x2d,
                                         0x0a, 0xd9, 0x85, 0xf9};
