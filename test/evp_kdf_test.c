...
static int do_kdf_hkdf_gettables(int extract_only, int has_digest)
{
    ...
    if (!TEST_ptr(params = construct_hkdf_params(has_digest ? "sha256" : NULL,
                                                 "secret", 6, "salt",
    ...
    if (!TEST_ptr(gettables = EVP_KDF_CTX_gettable_params(kctx))
        ...
        || !TEST_ptr(
            p = OSSL_PARAM_locate_const(gettables, OSSL_KDF_PARAM_MODE))
        || !TEST_ptr(
            p = OSSL_PARAM_locate_const(gettables, OSSL_KDF_PARAM_DIGEST))
        || !TEST_ptr(
            p = OSSL_PARAM_locate_const(gettables, OSSL_KDF_PARAM_SALT))
        || !TEST_ptr(
            p = OSSL_PARAM_locate_const(gettables, OSSL_KDF_PARAM_INFO)))
        goto err;
    ...
    if (has_digest) {
        if (!TEST_int_eq(EVP_KDF_CTX_get_params(kctx, params_get), 1)
            || !TEST_size_t_eq(sz,
                               extract_only ? SHA256_DIGEST_LENGTH : SIZE_MAX))
            goto err;
    } else {
    ...
    params_get[2] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SALT, salt,
                                                      sizeof(salt));
    params_get[3] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_INFO, info,
                                                      sizeof(info));
    params_get[4] = OSSL_PARAM_construct_end();
    if (!TEST_int_eq(EVP_KDF_CTX_get_params(kctx, params_get), 1)
        || !TEST_str_eq(mode_utf8,
                        extract_only ? "EXTRACT_ONLY" : "EXTRACT_AND_EXPAND")
        || !TEST_int_eq(mode_int,
                        extract_only ? EVP_KDF_HKDF_MODE_EXTRACT_ONLY
...
static int test_kdf_kbkdf_6803_128(void)
{
    ...
    for (i = 0; i < 3; i++) {
        ...
        params[p++] = OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_MODE,
                                                       "FEEDBACK", 0);
        ...
        params[p++] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SEED, iv,
                                                        sizeof(iv));
        params[p] = OSSL_PARAM_construct_end();

        kctx = get_kdfbyname("KBKDF");
        ret = TEST_ptr(kctx)
            && TEST_int_gt(EVP_KDF_derive(kctx, result, sizeof(result), params),
                           0)
...
static int test_kdf_get_kdf(void)
{
    ...
    if (!TEST_ptr(obj = OBJ_nid2obj(NID_id_pbkdf2))
        ...
        || !TEST_ptr(
            kdf2 = EVP_KDF_fetch(NULL, OBJ_nid2sn(OBJ_obj2nid(obj)), NULL))
