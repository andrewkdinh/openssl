...
static OSSL_PARAM *construct_tls1_prf_params(const char *digest, const char *secret, const char *seed)
{
    ...
    *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SECRET, (unsigned char *)secret, strlen(secret));
    *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SEED, (unsigned char *)seed, strlen(seed));
...
static int test_kdf_tls1_prf(void)
{
    ...
    OSSL_PARAM *params;
    static const unsigned char expected[sizeof(out)] = {0x8e, 0x4d, 0x93, 0x25, 0x30, 0xd7, 0x65, 0xa0,
...
static int test_kdf_hkdf(void)
{
    ...
    OSSL_PARAM *params;
    static const unsigned char expected[sizeof(out)] = {0x2a, 0xc4, 0x36, 0x9f, 0x52, 0x59, 0x96, 0xf8, 0xde, 0x13};
...
static int do_kdf_hkdf_gettables(int extract_only, int has_digest)
{
    ...
    if (!TEST_ptr(params = construct_hkdf_params(has_digest ? "sha256" : NULL, "secret", 6, "salt",
                                                 extract_only ? NULL : "label"))
        || !TEST_ptr(kctx = get_kdfbyname(OSSL_KDF_NAME_HKDF)) || !TEST_true(EVP_KDF_CTX_set_params(kctx, params)))
        goto err;
    ...
    params_get[0] = OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_DIGEST, digest, sizeof(digest));
    params_get[1] = OSSL_PARAM_construct_end();
    if (has_digest) {
        if (!TEST_int_eq(EVP_KDF_CTX_get_params(kctx, params_get), 1) || !TEST_str_eq(digest, "SHA2-256"))
            goto err;
    } else {
        if (!TEST_int_eq(EVP_KDF_CTX_get_params(kctx, params_get), 0))
            goto err;
    ...
    params_get[0] = OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_MODE, mode_utf8, sizeof(mode_utf8));
    ...
    if (!TEST_int_eq(EVP_KDF_CTX_get_params(kctx, params_get), 1)
        ...
        || !TEST_str_eq(info, extract_only ? "" : "label") || !TEST_str_eq(salt, "salt"))
        goto err;
...
static OSSL_PARAM *construct_pbkdf1_params(char *pass, char *digest, char *salt, unsigned int *iter)
{
    OSSL_PARAM *params = OPENSSL_malloc(sizeof(OSSL_PARAM) * 5);
    OSSL_PARAM *p = params;
    ...
    *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_PASSWORD, (unsigned char *)pass, strlen(pass));
    *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SALT, (unsigned char *)salt, strlen(salt));
...
static int test_kdf_pbkdf1(void)
{
    ...
    const unsigned char expected[sizeof(out)] = {0xfb, 0x83, 0x4d, 0x36, 0x6d, 0xbc, 0x53, 0x87, 0x35,
                                                 0x1b, 0x34, 0x75, 0x95, 0x88, 0x32, 0x4f, 0x3e, 0x82,
                                                 0x81, 0x01, 0x21, 0x93, 0x64, 0x00, 0xcc};
    ...
    params = construct_pbkdf1_params("passwordPASSWORDpassword", "sha256", "saltSALTsaltSALTsaltSALTsaltSALTsalt",
                                     &iterations);

    if (!TEST_ptr(params) || !TEST_ptr(kctx = get_kdfbyname_libctx(libctx, OSSL_KDF_NAME_PBKDF1))
        || !TEST_true(EVP_KDF_CTX_set_params(kctx, params))
...
static OSSL_PARAM *construct_pbkdf2_params(char *pass, char *digest, char *salt, unsigned int *iter, int *mode)
{
...
static int test_kdf_pbkdf2(void)
{
    ...
    int mode = 0;
    OSSL_PARAM *params;
    const unsigned char expected[sizeof(out)] = {0x34, 0x8c, 0x89, 0xdb, 0xcb, 0xd3, 0x2b, 0x2f, 0x32,
                                                 0xd8, 0x14, 0xb8, 0x11, 0x6e, 0x84, 0xcf, 0x2b, 0x17,
                                                 0x34, 0x7e, 0xbc, 0x18, 0x00, 0x18, 0x1c};

    params = construct_pbkdf2_params("passwordPASSWORDpassword", "sha256", "saltSALTsaltSALTsaltSALTsaltSALTsalt",
                                     &iterations, &mode);

    if (!TEST_ptr(params) || !TEST_ptr(kctx = get_kdfbyname(OSSL_KDF_NAME_PBKDF2))
        || !TEST_int_gt(EVP_KDF_derive(kctx, out, sizeof(out), params), 0)
        || !TEST_mem_eq(out, sizeof(out), expected, sizeof(expected)))
        goto err;
...
static int test_kdf_pbkdf2_small_salt(void)
{
    ...
    params = construct_pbkdf2_params("passwordPASSWORDpassword", "sha256", "saltSALT", &iterations, &mode);
...
static int test_kdf_pbkdf2_invalid_digest(void)
{
    ...
    params = construct_pbkdf2_params("passwordPASSWORDpassword", "blah", "saltSALTsaltSALTsaltSALTsaltSALTsalt",
                                     &iterations, &mode);
...
static int test_kdf_scrypt(void)
{
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    for (i = 0; ret && i < 2; ++i) {
        ret = ret && TEST_true(EVP_KDF_CTX_set_params(kctx, params));
        if (i == 0)
            ret = ret && TEST_int_le(EVP_KDF_derive(kctx, out, sizeof(out), NULL), 0)
                && TEST_true(OSSL_PARAM_set_uint(p - 1, 10 * 1024 * 1024))
                && TEST_true(EVP_KDF_CTX_set_params(kctx, p - 1));
        ret = ret && TEST_int_gt(EVP_KDF_derive(kctx, out, sizeof(out), NULL), 0)
            && TEST_mem_eq(out, sizeof(out), expected, sizeof(expected));
        if (i == 0)
...
static int test_kdf_ss_hash(void)
{
    ...
    static unsigned char z[] = {0x6d, 0xbd, 0xc2, 0x3f, 0x04, 0x54, 0x88, 0xe4, 0x06, 0x27, 0x57, 0xb0, 0x6b, 0x9e,
    ...
    static unsigned char other[] = {0xa1, 0xb2, 0xc3, 0xd4, 0xe5, 0x43, 0x41, 0x56, 0x53, 0x69, 0x64, 0x3c,
                                    0x83, 0x2e, 0x98, 0x49, 0xdc, 0xdb, 0xa7, 0x1e, 0x9a, 0x31, 0x39, 0xe6,
                                    0x06, 0xe0, 0x95, 0xde, 0x3c, 0x26, 0x4a, 0x66, 0xe9, 0x8a, 0x16, 0x58,
                                    0x54, 0xcd, 0x07, 0x98, 0x9b, 0x1e, 0xe0, 0xec, 0x3f, 0x8d, 0xbe};
    static const unsigned char expected[sizeof(out)] = {0xa4, 0x62, 0xde, 0x16, 0xa8, 0x9d, 0xe8,
                                                        0x46, 0x6e, 0xf5, 0x46, 0x0b, 0x47, 0xb8};
...
static int test_kdf_x963(void)
{
     ...
     */
    static unsigned char z[] = {0x00, 0xaa, 0x5b, 0xb7, 0x9b, 0x33, 0xe3, 0x89, 0xfa, 0x58, 0xce, 0xad, 0xc0, 0x47,
                                0x19, 0x7f, 0x14, 0xe7, 0x37, 0x12, 0xf4, 0x52, 0xca, 0xa9, 0xfc, 0x4c, 0x9a, 0xdb,
                                0x36, 0x93, 0x48, 0xb8, 0x15, 0x07, 0x39, 0x2f, 0x1a, 0x86, 0xdd, 0xfd, 0xb7, 0xc4,
                                0xff, 0x82, 0x31, 0xc4, 0xbd, 0x0f, 0x44, 0xe4, 0x4a, 0x1b, 0x55, 0xb1, 0x40, 0x47,
                                0x47, 0xa9, 0xe2, 0xe7, 0x53, 0xf5, 0x5e, 0xf0, 0x5a, 0x2d};
    static unsigned char shared[] = {0xe3, 0xb5, 0xb4, 0xc1, 0xb0, 0xd5, 0xcf, 0x1d,
                                     0x2b, 0x3a, 0x2f, 0x99, 0x37, 0x89, 0x5d, 0x31};
    static const unsigned char expected[sizeof(out)] = {
        };
...
static int test_kdf_kbkdf_6803_128(void)
{
    ...
    static unsigned char input_key[] = {
        0x57, 0xD0, 0x29, 0x72, 0x98, 0xFF, 0xD9, 0xD3, 0x5D, 0xE5, 0xA4, 0x7F, 0xB4, 0xBD, 0xE2, 0x4B,
    };
    ...
    static unsigned char outputs[][16] = {
        {0xD1, 0x55, 0x77, 0x5A, 0x20, 0x9D, 0x05, 0xF0, 0x2B, 0x38, 0xD4, 0x2A, 0x38, 0x9E, 0x5A, 0x56},
        {0x64, 0xDF, 0x83, 0xF8, 0x5A, 0x53, 0x2F, 0x17, 0x57, 0x7D, 0x8C, 0x37, 0x03, 0x57, 0x96, 0xAB},
        {0x3E, 0x4F, 0xBD, 0xF3, 0x0F, 0xB8, 0x25, 0x9C, 0x42, 0x5C, 0xB6, 0xC9, 0x6F, 0x1F, 0x46, 0x35}};
...
static int test_kdf_kbkdf_6803_256(void)
{
    ...
    OSSL_PARAM params[7];
    static unsigned char input_key[] = {
        0xB9, 0xD6, 0x82, 0x8B, 0x20, 0x56, 0xB7, 0xBE, 0x65, 0x6D, 0x88, 0xA1, 0x23, 0xB1, 0xFA, 0xC6,
        0x82, 0x14, 0xAC, 0x2B, 0x72, 0x7E, 0xCF, 0x5F, 0x69, 0xAF, 0xE0, 0xC4, 0xDF, 0x2A, 0x6D, 0x2C,
    };
    static unsigned char constants[][5] = {
        {0x00, 0x00, 0x00, 0x02, 0x99},
        {0x00, 0x00, 0x00, 0x02, 0xaa},
    ...
    static unsigned char outputs[][32] = {
        {
            0xE4, 0x67, 0xF9, 0xA9, 0x55, 0x2B, 0xC7, 0xD3, 0x15, 0x5A, 0x62, 0x20, 0xAF, 0x9C, 0x19, 0x22,
            0x0E, 0xEE, 0xD4, 0xFF, 0x78, 0xB0, 0xD1, 0xE6, 0xA1, 0x54, 0x49, 0x91, 0x46, 0x1A, 0x9E, 0x50,
        },
        {
            0x41, 0x2A, 0xEF, 0xC3, 0x62, 0xA7, 0x28, 0x5F, 0xC3, 0x96, 0x6C, 0x6A, 0x51, 0x81, 0xE7, 0x60,
            0x5A, 0xE6, 0x75, 0x23, 0x5B, 0x6D, 0x54, 0x9F, 0xBF, 0xC9, 0xAB, 0x66, 0x30, 0xA4, 0xC6, 0x04,
        },
        {
            0xFA, 0x62, 0x4F, 0xA0, 0xE5, 0x23, 0x99, 0x3F, 0xA3, 0x88, 0xAE, 0xFD, 0xC6, 0x7E, 0x67, 0xEB,
            0xCD, 0x8C, 0x08, 0xE8, 0xA0, 0x24, 0x6B, 0x1D, 0x73, 0xB0, 0xD1, 0xDD, 0x9F, 0xC5, 0x82, 0xB0,
        },
    };
    static unsigned char iv[16] = {0};
...
static OSSL_PARAM *construct_kbkdf_params(char *digest, char *mac, unsigned char *key, size_t keylen, char *salt,
                                          char *info, int *r)
{
...
static int test_kdf_kbkdf_empty_key(void)
{
    ...
    /* Negative test case - derive should fail */
    kctx = get_kdfbyname("KBKDF");
    ret = TEST_ptr(kctx) && TEST_true(EVP_KDF_CTX_set_params(kctx, params))
...
static int test_kdf_kbkdf_8009_prf2(void)
{
    ...
    EVP_KDF_CTX *kctx;
    OSSL_PARAM params[6];
    char *label = "prf", *digest = "sha384", *prf_input = "test", *mac = "HMAC";
    static unsigned char input_key[] = {
...
static int test_kdf_kbkdf_fixedinfo(void)
{
    ...
    static unsigned char input_key[] = {
        0xc1, 0x0b, 0x15, 0x2e, 0x8c, 0x97, 0xb7, 0x7e, 0x18, 0x70, 0x4e, 0x0f, 0x0b, 0xd3, 0x83, 0x05,
    };
    static unsigned char fixed_input[] = {
        0x98, 0xcd, 0x4c, 0xbb, 0xbe, 0xbe, 0x15, 0xd1, 0x7d, 0xc8, 0x6e, 0x6d, 0xba, 0xd8, 0x00,
        0xa2, 0xdc, 0xbd, 0x64, 0xf7, 0xc7, 0xad, 0x0e, 0x78, 0xe9, 0xcf, 0x94, 0xff, 0xdb, 0xa8,
        0x9d, 0x03, 0xe9, 0x7e, 0xad, 0xf6, 0xc4, 0xf7, 0xb8, 0x06, 0xca, 0xf5, 0x2a, 0xa3, 0x8f,
        0x09, 0xd0, 0xeb, 0x71, 0xd7, 0x1f, 0x49, 0x7b, 0xcc, 0x69, 0x06, 0xb4, 0x8d, 0x36, 0xc4,
...
static int test_kdf_kbkdf_kmac(void)
{
    ...
    static unsigned char input_key[] = {0xDD, 0x81, 0xEF, 0xC8, 0x2C, 0xDD, 0xEC, 0x51, 0xC4, 0x09, 0xBD,
                                        0x8C, 0xCB, 0xAF, 0x94, 0xF6, 0x5F, 0xFA, 0x7B, 0x92, 0xF1, 0x11,
                                        0xF9, 0x40, 0x2B, 0x0D, 0x6A, 0xE0, 0x5E, 0x44, 0x92, 0x34, 0xF0,
                                        0x3B, 0xBA, 0xF5, 0x4F, 0xEF, 0x19, 0x45, 0xDA};
    static unsigned char context[] = {
        };
    static unsigned char label[] = {0xA5, 0xDE, 0x2A, 0x0A, 0xF0, 0xDA, 0x59, 0x04, 0xCC, 0xFF, 0x50, 0xD3, 0xA5, 0xD2,
                                    0xDE, 0xA3, 0x33, 0xC0, 0x27, 0xED, 0xDC, 0x6A, 0x54, 0x54, 0x95, 0x78, 0x74, 0x0D,
                                    0xE7, 0xB7, 0x92, 0xD6, 0x64, 0xD5, 0xFB, 0x1F, 0x0F, 0x87, 0xFD, 0x65, 0x79, 0x8B,
                                    0x81, 0x83, 0x95, 0x40, 0x7A, 0x19, 0x8D, 0xCA, 0xE0, 0x4A, 0x93, 0xA8};
    static unsigned char output[] = {
        };
    unsigned char result[sizeof(output)] = {0};

    *p++ = OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_MAC, mac, 0);
    *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_KEY, input_key, sizeof(input_key));
    *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_INFO, context, sizeof(context));
    *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SALT, label, sizeof(label));
    *p = OSSL_PARAM_construct_end();

    kctx = get_kdfbyname("KBKDF");
    ret = TEST_ptr(kctx) && TEST_size_t_eq(EVP_KDF_CTX_get_kdf_size(kctx), SIZE_MAX)
...
static int test_kdf_ss_hmac(void)
{
    ...
    unsigned char out[16];
    static unsigned char z[] = {0xb7, 0x4a, 0x14, 0x9a, 0x16, 0x15, 0x46, 0xf8, 0xc2, 0x0b, 0x06, 0xac, 0x4e, 0xd4};
    static unsigned char other[] = {0x34, 0x8a, 0x37, 0xa2, 0x7e, 0xf1, 0x28, 0x2f, 0x5f, 0x02, 0x0d, 0xcc};
    static unsigned char salt[] = {0x36, 0x38, 0x27, 0x1c, 0xcd, 0x68, 0xa2, 0x5d,
                                   0xc2, 0x4e, 0xcd, 0xdd, 0x39, 0xef, 0x3f, 0x89};
    static const unsigned char expected[sizeof(out)] = {0x44, 0xf6, 0x76, 0xe8, 0x5c, 0x1b, 0x1a, 0x8b,
                                                        0xbc, 0x3d, 0x31, 0x92, 0x18, 0x63, 0x1c, 0xa3};
...
static int test_kdf_ss_kmac(void)
{
    ...
    static unsigned char salt[] = {0x36, 0x38, 0x27, 0x1c, 0xcd, 0x68, 0xa2, 0x5d,
                                   0xc2, 0x4e, 0xcd, 0xdd, 0x39, 0xef, 0x3f, 0x89};
    static const unsigned char expected[sizeof(out)] = {
        0xe9, 0xc1, 0x84, 0x53, 0xa0, 0x62, 0xb5, 0x3b, 0xdb, 0xfc, 0xbb, 0x5a, 0x34, 0xbd, 0xb8, 0xe5,
        0xe7, 0x07, 0xee, 0xbb, 0x5d, 0xd1, 0x34, 0x42, 0x43, 0xd8, 0xcf, 0xc2, 0xc2, 0xe6, 0x33, 0x2f,
        0x91, 0xbd, 0xa5, 0x86, 0xf3, 0x7d, 0xe4, 0x8a, 0x65, 0xd4, 0xc5, 0x14, 0xfd, 0xef, 0xaa, 0x1e,
        0x67, 0x54, 0xf3, 0x73, 0xd2, 0x38, 0xe1, 0x95, 0xae, 0x15, 0x7e, 0x1d, 0xe8, 0x14, 0x98, 0x03};
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    ret = TEST_ptr(kctx = get_kdfbyname(OSSL_KDF_NAME_SSKDF)) && TEST_size_t_eq(EVP_KDF_CTX_get_kdf_size(kctx), 0)
        && TEST_int_eq(EVP_KDF_CTX_set_params(kctx, params), 1)
        /* The bug fix for KMAC returning SIZE_MAX was added in 3.0.8 */
        && (fips_provider_version_lt(NULL, 3, 0, 8) || TEST_size_t_eq(EVP_KDF_CTX_get_kdf_size(kctx), SIZE_MAX))
        && TEST_int_gt(EVP_KDF_derive(kctx, out, sizeof(out), NULL), 0)
        && TEST_mem_eq(out, sizeof(out), expected, sizeof(expected));
...
static int test_kdf_sshkdf(void)
{
    ...
    static unsigned char key[] = {
        };
    static unsigned char xcghash[] = {0x0e, 0x68, 0x3f, 0xc8, 0xa9, 0xed, 0x7c, 0x2f, 0xf0, 0x2d, 0xef,
                                      0x23, 0xb2, 0x74, 0x5e, 0xbc, 0x99, 0xb2, 0x67, 0xda, 0xa8, 0x6a,
                                      0x4a, 0xa7, 0x69, 0x72, 0x39, 0x08, 0x82, 0x53, 0xf6, 0x42};
    static unsigned char sessid[] = {0x0e, 0x68, 0x3f, 0xc8, 0xa9, 0xed, 0x7c, 0x2f, 0xf0, 0x2d, 0xef,
                                     0x23, 0xb2, 0x74, 0x5e, 0xbc, 0x99, 0xb2, 0x67, 0xda, 0xa8, 0x6a,
                                     0x4a, 0xa7, 0x69, 0x72, 0x39, 0x08, 0x82, 0x53, 0xf6, 0x42};
    static const unsigned char expected[sizeof(out)] = {0x41, 0xff, 0x2e, 0xad, 0x16, 0x83, 0xf1, 0xe6};
...
static int test_kdf_get_kdf(void)
{
    ...
    if (!TEST_ptr(obj = OBJ_nid2obj(NID_id_pbkdf2)) || !TEST_ptr(kdf1 = EVP_KDF_fetch(NULL, OSSL_KDF_NAME_PBKDF2, NULL))
        || !TEST_ptr(kdf2 = EVP_KDF_fetch(NULL, OBJ_nid2sn(OBJ_obj2nid(obj)), NULL)) || !test_kdfs_same(kdf1, kdf2))
        ok = 0;
    EVP_KDF_free(kdf1);
    kdf1 = NULL;
    EVP_KDF_free(kdf2);
    ...
    if (!TEST_ptr(kdf1 = EVP_KDF_fetch(NULL, SN_tls1_prf, NULL))
        || !TEST_ptr(kdf2 = EVP_KDF_fetch(NULL, LN_tls1_prf, NULL)) || !test_kdfs_same(kdf1, kdf2))
        ok = 0;
...
static int test_kdf_x942_asn1(void)
{
    ...
    static unsigned char z[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09,
                                0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12, 0x13};
    static const unsigned char expected[sizeof(out)] = {0xa0, 0x96, 0x61, 0x39, 0x23, 0x76, 0xf7, 0x04,
...
static int test_kdf_krb5kdf(void)
{
    ...
    OSSL_PARAM params[4], *p = params;
    unsigned char out[16];
    static unsigned char key[] = {0x42, 0x26, 0x3C, 0x6E, 0x89, 0xF4, 0xFC, 0x28,
                                  0xB8, 0xDF, 0x68, 0xEE, 0x09, 0x79, 0x9F, 0x15};
    static unsigned char constant[] = {0x00, 0x00, 0x00, 0x02, 0x99};
    static const unsigned char expected[sizeof(out)] = {0x34, 0x28, 0x0A, 0x38, 0x2B, 0xC9, 0x27, 0x69,
...
static int test_kdf_hmac_drbg_settables(void)
{
    ...
    for (i = 0; settableparams[i].key != NULL; ++i) {
        ...
        if (OPENSSL_strcasecmp(settableparams[i].key, OSSL_KDF_PARAM_PROPERTIES) != 0) {
    ...
    params[0] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_HMACDRBG_ENTROPY, (char *)ent, sizeof(ent));
    params[1] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_HMACDRBG_NONCE, (char *)ent, sizeof(ent));
    params[2] = OSSL_PARAM_construct_utf8_string(OSSL_ALG_PARAM_DIGEST, "SHA256", 0);
    params[3] = OSSL_PARAM_construct_utf8_string(OSSL_ALG_PARAM_PROPERTIES, "", 0);
    ...
    params[0] = OSSL_PARAM_construct_utf8_string(OSSL_ALG_PARAM_DIGEST, digestname, sizeof(digestname));
    params[1] = OSSL_PARAM_construct_utf8_string(OSSL_ALG_PARAM_MAC, macname, sizeof(macname));
    ...
    params[0] = OSSL_PARAM_construct_utf8_string(OSSL_ALG_PARAM_DIGEST, "shake256", 0);
    params[1] = OSSL_PARAM_construct_end();
    if (!TEST_int_le(EVP_KDF_CTX_set_params(kctx, params), 0))
        goto err;
...
static int test_kdf_hmac_drbg_gettables(void)
{
    ...
    for (i = 0; gettableparams[i].key != NULL; ++i) {
        params[0] = OSSL_PARAM_construct_utf8_string(gettableparams[i].key, buf, sizeof(buf));
        if (!TEST_int_le(EVP_KDF_CTX_get_params(kctx, params), 0))
            goto err;
    }
    ret = 1;
...
static int test_kbkdf_mac_change(void)
{
    ...
    int l = 0, sep = 0, r = 8;
    static /* const */ unsigned char key[] = {0x3e, 0xdc, 0x6b, 0x5b, 0x8f, 0x7a, 0xad, 0xbd, 0x71, 0x37, 0x32,
                                              0xb4, 0x82, 0xb8, 0xf9, 0x79, 0x28, 0x6e, 0x1e, 0xa3, 0xb8, 0xf8,
                                              0xf9, 0x9c, 0x30, 0xc8, 0x84, 0xcf, 0xe3, 0x34, 0x9b, 0x83};
    static /* const */ unsigned char info[] = {0x98, 0xe9, 0x98, 0x8b, 0xb4, 0xcc, 0x8b, 0x34, 0xd7, 0x92, 0x2e, 0x1c,
                                               0x68, 0xad, 0x69, 0x2b, 0xa2, 0xa1, 0xd9, 0xae, 0x15, 0x14, 0x95, 0x71,
                                               0x67, 0x5f, 0x17, 0xa7, 0x7a, 0xd4, 0x9e, 0x80, 0xc8, 0xd2, 0xa8, 0x5e,
                                               0x83, 0x1a, 0x26, 0x44, 0x5b, 0x1f, 0x0f, 0xf4, 0x4d, 0x70, 0x84, 0xa1,
                                               0x72, 0x06, 0xb4, 0x89, 0x6c, 0x81, 0x12, 0xda, 0xad, 0x18, 0x60, 0x5a};
    static const unsigned char output[] = {0x6c, 0x03, 0x76, 0x52, 0x99, 0x06, 0x74, 0xa0,
                                           0x78, 0x44, 0x73, 0x2d, 0x0a, 0xd9, 0x85, 0xf9};
    unsigned char out[sizeof(output)];

    params[0] = OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_MAC, OSSL_MAC_NAME_KMAC128, 0);
    params[1] = OSSL_PARAM_construct_end();
    if (!TEST_ptr(kctx = get_kdfbyname(OSSL_KDF_NAME_KBKDF)) || !TEST_true(EVP_KDF_CTX_set_params(kctx, params)))
        goto err;
    ...
    *p++ = OSSL_PARAM_construct_int(OSSL_KDF_PARAM_KBKDF_R, &r);
    *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_KEY, key, sizeof(key));
    *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_INFO, info, sizeof(info));
    *p = OSSL_PARAM_construct_end();
