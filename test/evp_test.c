...
static int ctrl2params(EVP_TEST *t, STACK_OF(OPENSSL_STRING) * controls,
    ...
    for (i = 0; i < sk_OPENSSL_STRING_num(controls); i++) {
        ...
        if (tmpval == NULL || !OSSL_PARAM_allocate_from_text(
                              &params[*params_n], settables, tmpkey, tmpval,
                              strlen(tmpval), NULL)) {
            OPENSSL_free(tmpkey);
            t->err = "ERR_PARAM_ERROR";
            goto err;
...
static int parse_bin_chunk(const char *value, size_t offset, size_t max,
    ...
    value[0] == '"' ? value + offset + 1 : value + offset;
...
static int digest_test_run(EVP_TEST *t) {
    ...
    *p++ = OSSL_PARAM_construct_end();
    ...
    if (sk_EVP_TEST_BUFFER_num(expected->input) == 1 &&
        ...
        if (!TEST_true(
            EVP_Q_digest(libctx, EVP_MD_get0_name(expected->fetched_digest),
...
static int cipher_test_enc(EVP_TEST *t, int enc, size_t out_misalign,
    ...
    if (expected->cts_mode != NULL) {
        ...
        OSSL_CIPHER_PARAM_CTS_MODE, (char *)expected->cts_mode, 0);
    ...
    if (expected->rounds > 0) {
        ...
        if (EVP_CIPHER_CTX_ctrl(ctx_base, EVP_CTRL_SET_RC5_ROUNDS, rounds,
                                NULL) <= 0) {
    ...
    if (expected->key_bits > 0) {
        ...
        if (EVP_CIPHER_CTX_ctrl(ctx_base, EVP_CTRL_SET_RC2_KEY_BITS, bits,
                                NULL) <= 0) {
    ...
    if (expected->iv != NULL) {
        ...
        if (!TEST_true(
            EVP_CIPHER_CTX_get_updated_iv(ctx_base, iv, sizeof(iv))) ||
            ((EVP_CIPHER_get_flags(expected->cipher) & EVP_CIPH_CUSTOM_IV) ==
             0 &&
            ...
            for (i = 0; expected->aad[i] != NULL; i++) {
                ...
                do {
                    ...
                    if (data_chunk_size == 0 ||
                        (size_t)data_chunk_size > aad_len)
    ...
    if (expected->tls_aad) {
        ...
        if ((tls_aad =
             OPENSSL_memdup(expected->aad[0], expected->aad_len[0])) == NULL)
            goto err;
...
static int mac_test_run_pkey(EVP_TEST *t) {
    ...
    if (expected->type == EVP_PKEY_CMAC) {
        ...
        key =
        EVP_PKEY_new_CMAC_key(NULL, expected->key, expected->key_len, cipher);
        OSSL_LIB_CTX_set0_default(tmpctx);
#endif
    } else {
        key =
        EVP_PKEY_new_raw_private_key_ex(libctx, OBJ_nid2sn(expected->type),
                                        NULL, expected->key, expected->key_len);
    }
    if (key == NULL) {
        t->err = "MAC_KEY_CREATE_ERROR";
        goto err;
    ...
    for (i = 0; i < sk_OPENSSL_STRING_num(expected->controls); i++)
        if (!mac_test_ctrl_pkey(
            t, pctx, sk_OPENSSL_STRING_value(expected->controls, i))) {
...
static int kem_test_run(EVP_TEST *t) {
    ...
    if (kdata->input == NULL) {
        ...
        ret = decapsulate(t, kdata->ctx, kdata->op, kdata->input,
                          kdata->inputlen, kdata->output, kdata->outputlen);
    }
...
static int pbe_test_run(EVP_TEST *t) {
    ...
    if (expected->pbe_type == PBE_TYPE_PBKDF2) {
        if (PKCS5_PBKDF2_HMAC((char *)expected->pass, expected->pass_len,
                              ...
                              expected->iter, expected->md, expected->key_len,
                              key) == 0) {
            t->err = "PBKDF2_ERROR";
            goto err;
        }
        ...
        if (PKCS12_key_gen_uni(expected->pass, expected->pass_len,
                               expected->salt, expected->salt_len, expected->id,
                               expected->iter, expected->key_len, key,
                               fetched_digest) == 0) {
            t->err = "PKCS12_ERROR";
            goto err;
        }
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    for (i = 0; i <= expected->n; i++) {
        ...
        *p++ = OSSL_PARAM_construct_octet_string(OSSL_RAND_PARAM_TEST_NONCE, z,
                                                 item->nonce_len);
        *p = OSSL_PARAM_construct_end();
        if (!TEST_true(EVP_RAND_instantiate(expected->parent, strength, 0, NULL,
                                            0, params)))
            goto err;
        ...
        if (item->reseed_entropy != NULL) {
            ...
            if (!TEST_true(EVP_RAND_reseed(
                expected->ctx, expected->prediction_resistance, NULL, 0,
...
static int kdf_test_ctrl(EVP_TEST *t, EVP_KDF_CTX *kctx, const char *value) {
    ...
    rv =
    OSSL_PARAM_allocate_from_text(kdata->p, defs, name, p, strlen(p), NULL);
    *++kdata->p = OSSL_PARAM_construct_end();
    if (!rv) {
...
static int digestverify_test_run(EVP_TEST *t) {
    ...
    if (EVP_DigestVerifyFinal(mdata->ctx, mdata->output, mdata->output_len) <=
        0)
        t->err = "VERIFY_ERROR";
    return 1;
}
...
static int securitycheck_enabled(void) {
    ...
    if (enabled == -1) {
        if (OSSL_PROVIDER_available(libctx, "fips")) {
            ...
            if (prov != NULL) {
                params[0] = OSSL_PARAM_construct_int(
                OSSL_PROV_PARAM_SECURITY_CHECKS, &check);
                params[1] = OSSL_PARAM_construct_end();
...
start:
    if (strcmp(pp->key, "PrivateKey") == 0) {
        pkey =
        PEM_read_bio_PrivateKey_ex(t->s.key, NULL, 0, NULL, libctx, NULL);
    ...
    for (pp++, i = 1; i < (t->s.numpairs - skipped); pp++, i++) {
        if (strcmp(pp->key, "Securitycheck") == 0) {
            ...
            if (!extended_tests) {
                TEST_info("skipping extended test: %s:%d", t->s.test_file,
                          t->s.start);
                t->skip = 1;
            }
        } else {
...
const OPTIONS *test_get_options(void) {
    ...
    {"process", OPT_IN_PLACE, 's',
     "Mode for data processing by cipher tests [in_place/both], both by "
     "default"},
...
int setup_tests(void) {
    ...
    while ((o = opt_next()) != OPT_EOF) {
        ...
        case OPT_CONFIG_FILE:
            ...
            break;
        case OPT_IN_PLACE:
            if ((process_mode_in_place = evp_test_process_mode(opt_arg())) ==
                -1)
            case OPT_DATA_CHUNK:
