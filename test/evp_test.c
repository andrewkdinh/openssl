...
static int mac_check_fips_approved(EVP_MAC_CTX *ctx, EVP_TEST *t) {
  ...
  params[0] =
  OSSL_PARAM_construct_int(OSSL_MAC_PARAM_FIPS_APPROVED_INDICATOR, &approved);
...
static int pkey_check_fips_approved(EVP_PKEY_CTX *ctx, EVP_TEST *t) {
  ...
  if (gettables == NULL ||
      OSSL_PARAM_locate_const(gettables,
                              OSSL_ALG_PARAM_FIPS_APPROVED_INDICATOR) == NULL)
    return 1;
  ...
  params[0] =
  OSSL_PARAM_construct_int(OSSL_ALG_PARAM_FIPS_APPROVED_INDICATOR, &approved);
...
static int rand_check_fips_approved(EVP_RAND_CTX *ctx, EVP_TEST *t) {
  ...
  params[0] =
  OSSL_PARAM_construct_int(OSSL_DRBG_PARAM_FIPS_APPROVED_INDICATOR, &approved);
...
static void openssl_free(char *m) { OPENSSL_free(m); }
...
static int ctrl2params(EVP_TEST *t, STACK_OF(OPENSSL_STRING) * controls,
  ...
  for (i = 0; i < sk_OPENSSL_STRING_num(controls); i++) {
    ...
    if (tmpval == NULL ||
        !OSSL_PARAM_allocate_from_text(&params[*params_n], settables, tmpkey,
...
static int kdf_check_fips_approved(EVP_KDF_CTX *ctx, EVP_TEST *t) {
  ...
  params[0] =
  OSSL_PARAM_construct_int(OSSL_KDF_PARAM_FIPS_APPROVED_INDICATOR, &approved);
...
static int digest_update_fn(void *ctx, const unsigned char *buf,
                            size_t buflen) {
...
static int digest_test_run(EVP_TEST *t) {
  ...
  got =
  OPENSSL_malloc(expected->output_len > EVP_MAX_MD_SIZE ? expected->output_len
                                                        : EVP_MAX_MD_SIZE);
  if (!TEST_ptr(got))
    goto err;
  ...
  if (expected->digest_size > 0) {
    *p++ =
    OSSL_PARAM_construct_size_t(OSSL_DIGEST_PARAM_SIZE, &expected->digest_size);
  }
  if (expected->pad_type > 0)
    *p++ =
    OSSL_PARAM_construct_int(OSSL_DIGEST_PARAM_PAD_TYPE, &expected->pad_type);
...
"Digest", digest_test_init, digest_test_cleanup, digest_test_parse,
digest_test_run};
...
static int cipher_test_init(EVP_TEST *t, const char *alg) {
  ...
  ERR_set_mark();
  if ((cipher = fetched_cipher = EVP_CIPHER_fetch(libctx, alg, propquery)) ==
      NULL &&
...
static int cipher_test_enc(EVP_TEST *t, int enc, size_t out_misalign,
  ...
  if (!EVP_CipherInit_ex(ctx_base, NULL, NULL, expected->key, expected->iv,
                         -1)) {
    t->err = "KEY_SET_ERROR";
    goto err;
  }
    ...
    if (!frag) {
      ...
      for (i = 0; expected->aad[i] != NULL; i++) {
        ...
        do {
          ...
          if (!EVP_CipherUpdate(ctx, NULL, &chunklen,
                                expected->aad[i] + donelen, current_aad_len))
            goto err;
      ...
      for (i = 0; expected->aad[i] != NULL; i++) {
        ...
        if (expected->aad_len[i] > 1 &&
            !EVP_CipherUpdate(ctx, NULL, &chunklen, expected->aad[i] + donelen,
                              1))
          goto err;
      }
    }
  ...
  if (expected->tls_aad) {
    ...
    if ((tls_aad = OPENSSL_memdup(expected->aad[0], expected->aad_len[0])) ==
        NULL)
      goto err;
    params[0] = OSSL_PARAM_construct_octet_string(
    OSSL_CIPHER_PARAM_AEAD_TLS1_AAD, tls_aad, expected->aad_len[0]);
    params[1] = OSSL_PARAM_construct_end();
    if (!EVP_CIPHER_CTX_set_params(ctx, params)) {
    ...
    OPENSSL_free(tls_aad);
             ...
             (expected->aead == EVP_CIPH_OCB_MODE || expected->tag_late)) {
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG, expected->tag_len,
                            expected->tag) <= 0) {
      t->err = "TAG_SET_ERROR";
      goto err;
    }
  ...
  if (expected->xts_standard != NULL) {
    ...
    OSSL_CIPHER_PARAM_XTS_STANDARD, (char *)expected->xts_standard, 0);
    params[1] = OSSL_PARAM_construct_end();
    if (!EVP_CIPHER_CTX_set_params(ctx, params)) {
  ...
  if (!frag) {
    do {
      ...
      if (!EVP_CipherUpdate(ctx, tmp + out_misalign + tmplen, &chunklen, in,
                            current_in_len))
        goto err;
      tmplen += chunklen;
      in += current_in_len;
  ...
  if (enc && expected->aead && !expected->tls_aad) {
    ...
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, expected->tag_len,
                            rtag) <= 0) {
  ...
  if (enc && expected->tag) {
    if (EVP_CIPHER_is_a(expected->cipher, "AES-128-CBC-HMAC-SHA1-ETM") ||
      ...
      if (EVP_CIPHER_is_a(expected->cipher, "AES-128-CBC-HMAC-SHA1-ETM") ||
          ...
          EVP_CIPHER_is_a(expected->cipher, "AES-256-CBC-HMAC-SHA1-ETM"))
        tag_len = 20;
      else if (EVP_CIPHER_is_a(expected->cipher,
      ...
      for (out_misalign = 0; out_misalign <= 1; out_misalign++) {
        for (inp_misalign = 0; inp_misalign <= 1; inp_misalign++) {
          ...
          if (in_place == 1) {
            ...
            BIO_snprintf(
            aux_err, sizeof(aux_err), "%s output and %s input, %sfragmented",
            out_misalign ? "misaligned" : "aligned",
...
"Cipher", cipher_test_init, cipher_test_cleanup, cipher_test_parse,
cipher_test_run};
...
static int mac_test_run_pkey(EVP_TEST *t) {
  ...
  if (expected->alg == NULL)
    ...
    TEST_info("Trying the EVP_PKEY %s test with %s", OBJ_nid2sn(expected->type),
              expected->alg);
  ...
  if (expected->type == EVP_PKEY_CMAC) {
    ...
    if (!TEST_ptr(cipher =
                  EVP_CIPHER_fetch(libctx, expected->alg, propquery))) {
    ...
    libctx, OBJ_nid2sn(expected->type), NULL, expected->key, expected->key_len);
  }
  if (key == NULL) {
    t->err = "MAC_KEY_CREATE_ERROR";
    goto err;
...
static int mac_test_run_mac(EVP_TEST *t) {
  ...
  if (expected->alg == NULL)
    ...
    TEST_info("Trying the EVP_MAC %s test with %s", expected->mac_name,
              expected->alg);
  ...
  if (expected->alg != NULL) {
    ...
    if (OSSL_PARAM_locate_const(defined_params, OSSL_MAC_PARAM_CIPHER) !=
      ...
      if (is_cipher_disabled(expected->alg))
        ...
        OSSL_MAC_PARAM_CIPHER, expected->alg, 0);
      ...
      if (is_digest_disabled(expected->alg))
        ...
        params[params_n++] = OSSL_PARAM_construct_utf8_string(
        OSSL_MAC_PARAM_DIGEST, expected->alg, 0);
    } else {
  ...
  if (expected->salt != NULL)
    params[params_n++] = OSSL_PARAM_construct_octet_string(
    OSSL_MAC_PARAM_SALT, expected->salt, expected->salt_len);
  if (expected->iv != NULL)
    params[params_n++] = OSSL_PARAM_construct_octet_string(
  ...
  if (expected->block_size >= 0)
    *psizes++ =
    OSSL_PARAM_construct_int(OSSL_MAC_PARAM_BLOCK_SIZE, &block_size);
  if (psizes != sizes) {
...
retry:
  ...
  if (xof) {
    ...
    if (!EVP_MAC_finalXOF(ctx, got, expected->output_len) ||
        !memory_err_compare(t, "TEST_MAC_ERR", expected->output,
                            expected->output_len, got, expected->output_len)) {
  ...
  if (!xof) {
    OPENSSL_cleanse(got, got_len);
    if (!TEST_true(EVP_Q_mac(libctx, expected->mac_name, NULL, expected->alg,
                             params, expected->key, expected->key_len,
                             expected->input, expected->input_len, got, got_len,
                             &size)) ||
        !TEST_mem_eq(got, size, expected->output, expected->output_len)) {
...
"MAC", mac_test_init, mac_test_cleanup, mac_test_parse, mac_test_run};
...
static int encapsulate(EVP_TEST *t, EVP_PKEY_CTX *ctx, const char *op,
  ...
  if (EVP_PKEY_encapsulate(ctx, wrapped, &wrappedlen, secret, &secretlen) <=
      0) {
...
static int kem_test_run(EVP_TEST *t) {
  ...
  if (kdata->keyname != NULL) {
    ...
    if ((pkey = EVP_PKEY_new_raw_public_key_ex(
         libctx, kdata->algname, propquery, kdata->encoded_pub_key,
    ...
    if ((pkey = EVP_PKEY_new_raw_private_key_ex(
         libctx, kdata->algname, propquery, kdata->encoded_priv_key,
  ...
  if (kdata->input == NULL) {
    ...
    ret = encapsulate(t, kdata->ctx, kdata->op, &wrapped, &wrappedlen, &secret,
                      &secretlen);
    if (ret == 0 || t->err != NULL)
      goto err;
    ...
    if (found_key || kdata->encoded_priv_key != NULL)
      ret = decapsulate(t, kdata->ctx, kdata->op, wrapped, wrappedlen, secret,
                        secretlen);
  } else {
...
static int pkey_test_parse(EVP_TEST *t, const char *keyword,
                           const char *value) {
  PKEY_DATA *kdata = t->data;
  if (strcmp(keyword, "Input") == 0)
...
static int pkey_test_run_init(EVP_TEST *t) {
  ...
  if (sk_OPENSSL_STRING_num(data->init_controls) > 0) {
    if (!ctrl2params(t, data->init_controls, NULL, params, OSSL_NELEM(params),
                     &params_n))
      goto err;
    p = params;
  }
...
static int pkey_fromdata_test_run(EVP_TEST *t) {
  ...
  if (sk_OPENSSL_STRING_num(kdata->controls) > 0) {
    if (!ctrl2params(t, kdata->controls, key_settable_ctx_params, params,
                     OSSL_NELEM(params), &params_n))
      goto err;
    p = params;
  }
...
"Sign", sign_test_init, pkey_test_cleanup, pkey_test_parse, pkey_test_run};
...
"Sign-Message", sign_test_message_init, pkey_test_cleanup, pkey_test_parse,
pkey_test_run};

/*
 * "VerifyRecover" implies EVP_PKEY_verify_recover_init_ex2() if the argument is
 * a colon-separated pair, {algorithm}:{key}. If not, it implies
...
"VerifyRecover", verify_recover_test_init, pkey_test_cleanup, pkey_test_parse,
pkey_test_run};
...
"Verify", verify_test_init, pkey_test_cleanup, pkey_test_parse,
verify_test_run};
...
"Verify-Message-Public", verify_message_public_test_init, pkey_test_cleanup,
...
static int pbe_test_run(EVP_TEST *t) {
  ...
  if (expected->pbe_type == PBE_TYPE_PBKDF2) {
    if (PKCS5_PBKDF2_HMAC((char *)expected->pass, expected->pass_len,
                          expected->salt, expected->salt_len, expected->iter,
                          expected->md, expected->key_len, key) == 0) {
      t->err = "PBKDF2_ERROR";
      goto err;
    }
#ifndef OPENSSL_NO_SCRYPT
    ...
    if (EVP_PBE_scrypt((const char *)expected->pass, expected->pass_len,
                       ...
                       expected->r, expected->p, expected->maxmem, key,
                       expected->key_len) == 0) {
      t->err = "SCRYPT_ERROR";
      goto err;
    }
    ...
    fetched_digest =
    EVP_MD_fetch(libctx, EVP_MD_get0_name(expected->md), propquery);
    if (fetched_digest == NULL) {
      t->err = "PKCS12_ERROR";
      goto err;
    }
    if (PKCS12_key_gen_uni(expected->pass, expected->pass_len, expected->salt,
                           expected->salt_len, expected->id, expected->iter,
                           expected->key_len, key, fetched_digest) == 0) {
      t->err = "PKCS12_ERROR";
      goto err;
    }
  }
...
"PBE", pbe_test_init, pbe_test_cleanup, pbe_test_parse, pbe_test_run};
...
static int encode_test_run(EVP_TEST *t) {
  ...
  if (expected->encoding == BASE64_CANONICAL_ENCODING) {
    ...
    if (!memory_err_compare(t, "BAD_ENCODING", expected->output,
                            expected->output_len, encode_out, output_len))
      goto err;
  }
...
static int rand_test_parse(EVP_TEST *t, const char *keyword,
                           const char *value) {
  RAND_DATA *rdata = t->data;
  RAND_DATA_PASS *item;
  const char *p;
...
static int rand_test_run(EVP_TEST *t) {
  ...
  *p = OSSL_PARAM_construct_end();
  ...
  for (i = 0; i <= expected->n; i++) {
    ...
    *p++ = OSSL_PARAM_construct_octet_string(OSSL_RAND_PARAM_TEST_ENTROPY, z,
                                             item->entropy_len);
    z = item->nonce != NULL ? item->nonce : (unsigned char *)"";
    *p++ = OSSL_PARAM_construct_octet_string(OSSL_RAND_PARAM_TEST_NONCE, z,
                                             item->nonce_len);
    *p = OSSL_PARAM_construct_end();
    if (!TEST_true(
    ...
    if (item->reseed_entropy != NULL) {
      ...
      if (!TEST_true(
          EVP_RAND_reseed(expected->ctx, expected->prediction_resistance, NULL,
                          0, item->reseed_addin, item->reseed_addin_len)))
        goto err;
    }
    if (item->pr_entropyA != NULL) {
      params[0] = OSSL_PARAM_construct_octet_string(
      OSSL_RAND_PARAM_TEST_ENTROPY, item->pr_entropyA, item->pr_entropyA_len);
      params[1] = OSSL_PARAM_construct_end();
      if (!TEST_true(EVP_RAND_CTX_set_params(expected->parent, params)))
        goto err;
    }
    ...
    if (item->pr_entropyB != NULL) {
      params[0] = OSSL_PARAM_construct_octet_string(
      OSSL_RAND_PARAM_TEST_ENTROPY, item->pr_entropyB, item->pr_entropyB_len);
      params[1] = OSSL_PARAM_construct_end();
      if (!TEST_true(EVP_RAND_CTX_set_params(expected->parent, params)))
        goto err;
    }
...
"RAND", rand_test_init, rand_test_cleanup, rand_test_parse, rand_test_run};
...
"PKEYKDF", pkey_kdf_test_init, pkey_kdf_test_cleanup, pkey_kdf_test_parse,
pkey_kdf_test_run};
 ...
 */
static int void_test_parse(EVP_TEST *t, const char *keyword,
                           const char *value) {
  return 0;
}
...
"PrivPubKeyPair", keypair_test_init, keypair_test_cleanup, void_test_parse,
keypair_test_run};
...
static int check_pkey_expected_values(EVP_TEST *t, const EVP_PKEY *pkey,
  ...
  if (sk_OPENSSL_STRING_num(out_controls) > 0) {
    ...
    if (!ctrl2params(t, out_controls, EVP_PKEY_gettable_params(pkey),
    ...
    for (p = out_params; p->key != NULL; ++p) {
      if (p->data_type == OSSL_PARAM_OCTET_STRING) {
        ...
        if (data == NULL)
          goto err;
        ret = EVP_PKEY_get_octet_string_param(pkey, p->key, data, p->data_size,
                                              &len) &&
              len == p->data_size &&
...
static int keygen_test_run(EVP_TEST *t) {
  ...
  if (sk_OPENSSL_STRING_num(keygen->in_controls) > 0) {
    ...
    if (!ctrl2params(t, keygen->in_controls,
                     EVP_PKEY_CTX_settable_params(genctx), params, 4,
                     &params_n))
      goto err;
...
static int check_test_error(EVP_TEST *t) {
  ...
  if (t->err == NULL && t->expected_err != NULL) {
    TEST_info("%s:%d: Succeeded but was expecting %s", t->s.test_file,
              t->s.start, t->expected_err);
    return 0;
  }

  if (strcmp(t->err, t->expected_err) != 0) {
    TEST_info("%s:%d: Expected %s got %s", t->s.test_file, t->s.start,
              t->expected_err, t->err);
    return 0;
  }
  ...
  if (t->reason == NULL) {
    TEST_info("%s:%d: Test is missing function or reason code", t->s.test_file,
              t->s.start);
    return 0;
  }
  ...
  err = ERR_peek_error();
  if (err == 0) {
    TEST_info("%s:%d: Expected error \"%s\" not set", t->s.test_file,
              t->s.start, t->reason);
    return 0;
  }
...
static int securitycheck_enabled(void) {
  ...
  if (enabled == -1) {
    if (OSSL_PROVIDER_available(libctx, "fips")) {
      ...
      if (prov != NULL) {
        params[0] =
        OSSL_PARAM_construct_int(OSSL_PROV_PARAM_SECURITY_CHECKS, &check);
        params[1] = OSSL_PARAM_construct_end();
...
start:
  if (strcmp(pp->key, "PrivateKey") == 0) {
    ...
    if (klist == &private_keys)
      pkey =
      EVP_PKEY_new_raw_private_key_ex(libctx, strnid, NULL, keybin, keylen);
    else
      pkey =
      EVP_PKEY_new_raw_public_key_ex(libctx, strnid, NULL, keybin, keylen);
    if (pkey == NULL && !key_unsupported()) {
    ...
    if (!prov_available(pp->value)) {
      TEST_info("skipping, '%s' provider not available: %s:%d", pp->value,
  ...
  for (pp++, i = 1; i < (t->s.numpairs - skipped); pp++, i++) {
    if (strcmp(pp->key, "Securitycheck") == 0) {
...
#endif
      {
        TEST_info("skipping, Securitycheck is disabled: %s:%d", t->s.test_file,
                  t->s.start);
        t->skip = 1;
        return 0;
      }
      ...
      if (OSSL_set_max_threads(libctx, atoi(pp->value)) == 0) {
        TEST_info("skipping, '%s' threads not available: %s:%d", pp->value,
                  t->s.test_file, t->s.start);
        t->skip = 1;
      }
...
const OPTIONS *test_get_options(void) {
  ...
  {"provider", OPT_PROVIDER_NAME, 's',
   "The provider to load (when no configuration file, the default value is "
   "'default')"},
