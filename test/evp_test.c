                       ...
                       size_t params_sz, size_t *params_n)
{
    ...
    for (i = 0; i < sk_OPENSSL_STRING_num(controls); i++) {
        ...
        if (tmpval == NULL
            || !OSSL_PARAM_allocate_from_text(&params[*params_n], settables,
                                              tmpkey, tmpval, strlen(tmpval),
                                              NULL)) {
            OPENSSL_free(tmpkey);
            t->err = "ERR_PARAM_ERROR";
            goto err;
...
static void cipher_test_cleanup(EVP_TEST *t)
{
    ...
    for (i = 0; i < AAD_NUM; i++) OPENSSL_free(cdat->aad[i]);
...
static void kdf_test_cleanup(EVP_TEST *t)
{
    ...
    ctrlfree(kdata->init_controls);
    for (p = kdata->params; p->key != NULL; p++) OPENSSL_free(p->data);
    OPENSSL_free(kdata->output);
...
static int prov_available(char *providers)
{
    ...
    while (more) {
        for (; isspace((unsigned char)(*providers)); providers++) continue;
        if (*providers == '\0')
...
start:
    ...
    for (pp++, i = 1; i < (t->s.numpairs - skipped); pp++, i++) {
        if (strcmp(pp->key, "Securitycheck") == 0) {
            ...
            if (t->security_category >= 0) {
                TEST_info("Line %d: multiple security category lines",
                          t->s.curr);
                return 0;
            }
