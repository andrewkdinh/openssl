...
static int rsa_keygen(int bits, EVP_PKEY **pub, EVP_PKEY **priv)
{
    ...
    if (!TEST_ptr(*priv = EVP_PKEY_Q_keygen(libctx, NULL, "RSA", (size_t)bits))
        || !TEST_ptr(
            ectx = OSSL_ENCODER_CTX_new_for_pkey(*priv, EVP_PKEY_PUBLIC_KEY,
...
static int kem_rsa_gen_recover(void)
{
    ...
    ret = TEST_true(rsa_keygen(bits, &pub, &priv))
        ...
        && TEST_int_eq(
              EVP_PKEY_encapsulate(dctx, NULL, &ctlen, NULL, &secretlen), 1)
        && TEST_int_eq(ctlen, secretlen) && TEST_int_eq(ctlen, bits / 8)
        && TEST_int_eq(
              EVP_PKEY_encapsulate(dctx, ct, &ctlen, secret, &secretlen), 1)
        ...
        && TEST_int_eq(EVP_PKEY_decapsulate(rctx, NULL, &unwraplen, ct, ctlen),
                       1)
        && TEST_int_eq(
              EVP_PKEY_decapsulate(rctx, unwrap, &unwraplen, ct, ctlen), 1)
...
static int test_cipher_tdes_randkey(void)
{
    ...
    ret = TEST_ptr(aes_cipher = EVP_CIPHER_fetch(libctx, "AES-256-CBC", NULL))
        ...
        && TEST_ptr(tdes_cipher =
                        EVP_CIPHER_fetch(libctx, "DES-EDE3-CBC", NULL))
...
static int kem_rsa_params(void)
{
    ...
    ret =
        ...
        && (is_fips_lt_3_5
            || (TEST_int_eq(EVP_PKEY_encapsulate(pubctx, NULL, &ctlen, NULL,
                                                 &secretlen),
                            1)
                && TEST_true(ctlen <= sizeof(ct))
                && TEST_true(secretlen <= sizeof(secret))
                && TEST_int_eq(EVP_PKEY_encapsulate(pubctx, ct, &ctlen, secret,
                                                    &secretlen),
                               1)))
        ...
        && TEST_int_eq(EVP_PKEY_CTX_set_kem_op(pubctx, "RSASVE"), 1)
        && TEST_int_eq(EVP_PKEY_encapsulate(pubctx, NULL, &ctlen, NULL, NULL),
                       1)
        && TEST_int_eq(EVP_PKEY_encapsulate(pubctx, ct, &ctlen, secret, NULL),
                       1)
        && TEST_int_eq(EVP_PKEY_encapsulate(pubctx, NULL, &ctlen, NULL, NULL),
                       1)
        /* Test outlen is optional */
        && TEST_int_eq(
            EVP_PKEY_encapsulate(pubctx, NULL, NULL, NULL, &secretlen), 1)
        && TEST_int_eq(
            EVP_PKEY_encapsulate(pubctx, ct, NULL, secret, &secretlen), 1)
        ...
        && TEST_int_eq(
            EVP_PKEY_encapsulate(pubctx, NULL, NULL, NULL, &secretlen), 1)
        && TEST_int_eq(
            EVP_PKEY_encapsulate(pubctx, NULL, &ctlen, NULL, &secretlen), 1)
        ...
        && TEST_int_eq(
            EVP_PKEY_decapsulate(privctx, secret, NULL, ct, sizeof(ct)), 1)
        && TEST_int_eq(
            EVP_PKEY_decapsulate(privctx, NULL, &secretlen, ct, sizeof(ct)), 1)
        && TEST_int_eq(secretlen, 256)
        /* Fail if passed NULL arguments */
        && TEST_int_eq(
            EVP_PKEY_decapsulate(privctx, NULL, NULL, ct, sizeof(ct)), 0)
        && TEST_int_eq(
            EVP_PKEY_decapsulate(privctx, secret, &secretlen, NULL, 0), 0)
        && TEST_int_eq(
            EVP_PKEY_decapsulate(privctx, secret, &secretlen, NULL, sizeof(ct)),
            0)
        && TEST_int_eq(EVP_PKEY_decapsulate(privctx, secret, &secretlen, ct, 0),
                       0);
