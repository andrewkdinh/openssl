...
static int test_dsa_param_keygen(int tstid)
{
     ...
     */
    static const BIGNUM *bn[] = {&ossl_bignum_dh2048_256_p, &ossl_bignum_dh2048_256_q, &ossl_bignum_dh2048_256_g};
    ...
    TEST_note("Testing with (p, q, g) = (%s, %s, %s)\n", getname(pind), getname(qind), getname(gind));

    if (!TEST_ptr(pkey_parm = EVP_PKEY_new()) || !TEST_ptr(dsa = DSA_new()) || !TEST_ptr(p = BN_dup(bn[pind]))
        || !TEST_ptr(q = BN_dup(bn[qind])) || !TEST_ptr(g = BN_dup(bn[gind])) || !TEST_true(DSA_set0_pqg(dsa, p, q, g)))
        goto err;
    p = q = g = NULL;
    ...
    if (!TEST_ptr(gen_ctx = EVP_PKEY_CTX_new_from_pkey(libctx, pkey_parm, NULL))
        || !TEST_int_gt(EVP_PKEY_keygen_init(gen_ctx), 0) || !TEST_int_eq(EVP_PKEY_keygen(gen_ctx, &pkey), expected))
        goto err;
...
static int do_dh_param_keygen(int tstid, const BIGNUM **bn)
{
    ...
    TEST_note("Testing with (p, q, g) = (%s, %s, %s)", getname(pind), getname(qind), getname(gind));

    if (!TEST_ptr(pkey_parm = EVP_PKEY_new()) || !TEST_ptr(dh = DH_new()) || !TEST_ptr(p = BN_dup(bn[pind]))
        || !TEST_ptr(q = BN_dup(bn[qind])) || !TEST_ptr(g = BN_dup(bn[gind])) || !TEST_true(DH_set0_pqg(dh, p, q, g)))
        goto err;
    p = q = g = NULL;
...
{
    static const BIGNUM *bn[] = {&ossl_bignum_ffdhe2048_p, &ossl_bignum_ffdhe2048_q, &ossl_bignum_const_2};
...
static int dhx_cert_load(void)
{
    ...
    static const unsigned char dhx_cert[] = {
        };

    if (!TEST_ptr(bio = BIO_new_mem_buf(dhx_cert, sizeof(dhx_cert))) || !TEST_ptr(cert = X509_new_ex(libctx, NULL))
...
static int test_cipher_reinit(int test_id)
{
    ...
    unsigned char iv[48] = {0x0f, 0x0e, 0x0d, 0x0c, 0x0b, 0x0a, 0x09, 0x08, 0x07, 0x06, 0x05, 0x04,
     ...
     */
    no_null_key =
        EVP_CIPHER_is_a(cipher, "RC4") || EVP_CIPHER_is_a(cipher, "RC4-40") || EVP_CIPHER_is_a(cipher, "RC4-HMAC-MD5");
    ...
    if (!TEST_true(EVP_EncryptInit_ex(ctx, cipher, NULL, key, iv))
        ...
        || !TEST_int_eq(EVP_EncryptUpdate(ctx, out2, &out2_len, in, sizeof(in)), ccm ? 0 : 1)
        || (!no_null_key
            && (!TEST_true(EVP_EncryptInit_ex(ctx, NULL, NULL, NULL, iv))
                || !TEST_int_eq(EVP_EncryptUpdate(ctx, out3, &out3_len, in, sizeof(in)), ccm || siv ? 0 : 1))))
        goto err;

    if (ccm == 0) {
        if (diff) {
            if (!TEST_mem_ne(out1, out1_len, out2, out2_len) || !TEST_mem_ne(out1, out1_len, out3, out3_len)
...
static int test_cipher_reinit_partialupdate(int test_id)
{
    ...
    static const unsigned char in[32] = {
        0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0xba, 0xbe, 0xba, 0xbe, 0x00, 0x00, 0xba, 0xbe,
        0x01, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f,
    };
    ...
    static const unsigned char iv[48] = {0x0f, 0x0e, 0x0d, 0x0c, 0x0b, 0x0a, 0x09, 0x08, 0x07, 0x06, 0x05, 0x04,
                                         0x03, 0x02, 0x01, 0x00, 0x0f, 0x0e, 0x0d, 0x0c, 0x0b, 0x0a, 0x09, 0x08,
                                         0x07, 0x06, 0x05, 0x04, 0x03, 0x02, 0x01, 0x00, 0x0f, 0x0e, 0x0d, 0x0c,
                                         0x0b, 0x0a, 0x09, 0x08, 0x07, 0x06, 0x05, 0x04, 0x03, 0x02, 0x01, 0x00};
    const char *name = sk_OPENSSL_STRING_value(cipher_names, test_id);
    ...
    if (((EVP_CIPHER_get_flags(cipher)
         ...
         != 0)
        || EVP_CIPHER_get_mode(cipher) == EVP_CIPH_CCM_MODE || EVP_CIPHER_get_mode(cipher) == EVP_CIPH_XTS_MODE
...
static void collect_cipher_names(EVP_CIPHER *cipher, void *cipher_names_list)
{
    ...
    if (OSSL_PROVIDER_available(libctx, "fips") && strncmp(name, "DES", 3) == 0)
        return;
    assert(name != NULL);
...
static int rsa_keygen(int bits, EVP_PKEY **pub, EVP_PKEY **priv)
{
    ...
    if (!TEST_ptr(*priv = EVP_PKEY_Q_keygen(libctx, NULL, "RSA", (size_t)bits))
        || !TEST_ptr(ectx = OSSL_ENCODER_CTX_new_for_pkey(*priv, EVP_PKEY_PUBLIC_KEY, "DER", "type-specific", NULL))
...
static int kem_rsa_gen_recover(void)
{
    ...
    ret = TEST_true(rsa_keygen(bits, &pub, &priv)) && TEST_ptr(sctx = EVP_PKEY_CTX_new_from_pkey(libctx, pub, NULL))
        ...
        && TEST_int_eq(EVP_PKEY_encapsulate(dctx, NULL, &ctlen, NULL, &secretlen), 1) && TEST_int_eq(ctlen, secretlen)
        && TEST_int_eq(ctlen, bits / 8) && TEST_int_eq(EVP_PKEY_encapsulate(dctx, ct, &ctlen, secret, &secretlen), 1)
        ...
        && TEST_int_eq(EVP_PKEY_decapsulate(rctx, NULL, &unwraplen, ct, ctlen), 1)
        && TEST_int_eq(EVP_PKEY_decapsulate(rctx, unwrap, &unwraplen, ct, ctlen), 1)
    ...
    if (fips_provider_version_match(libctx, ">=3.4.0")) {
        ctlen = 1;
        if (!TEST_int_eq(EVP_PKEY_encapsulate(dctx, ct, &ctlen, secret, &secretlen), 0))
            ret = 0;
        unwraplen = 1;
        if (!TEST_int_eq(EVP_PKEY_decapsulate(rctx, unwrap, &unwraplen, ct, ctlen), 0))
            ret = 0;
    }
...
static int test_cipher_tdes_randkey(void)
{
    ...
    ret = TEST_ptr(aes_cipher = EVP_CIPHER_fetch(libctx, "AES-256-CBC", NULL))
        ...
        && TEST_int_ne(EVP_CIPHER_get_flags(tdes_cipher) & EVP_CIPH_RAND_KEY, 0) && TEST_ptr(ctx = EVP_CIPHER_CTX_new())
        && TEST_true(EVP_CipherInit_ex2(ctx, tdes_cipher, NULL, NULL, 1, params))
...
static int kem_rsa_params(void)
{
    ...
    ret = TEST_true(rsa_keygen(2048, &pub, &priv)) && TEST_ptr(pubctx = EVP_PKEY_CTX_new_from_pkey(libctx, pub, NULL))
        ...
        && TEST_int_eq(EVP_PKEY_decapsulate(pubctx, secret, &secretlen, ct, sizeof(ct)), 0)
        ...
        && (is_fips_lt_3_5
            || (TEST_int_eq(EVP_PKEY_encapsulate(pubctx, NULL, &ctlen, NULL, &secretlen), 1)
                && TEST_true(ctlen <= sizeof(ct)) && TEST_true(secretlen <= sizeof(secret))
...
static EVP_PKEY *gen_dh_key(void)
{
    ...
    if (!TEST_ptr(gctx = EVP_PKEY_CTX_new_from_name(libctx, "DH", NULL)) || !TEST_int_gt(EVP_PKEY_keygen_init(gctx), 0)
        || !TEST_true(EVP_PKEY_CTX_set_params(gctx, params)) || !TEST_true(EVP_PKEY_keygen(gctx, &pkey)))
        goto err;
err:
...
int setup_tests(void)
{
    ...
    ADD_ALL_TESTS(test_cipher_reinit_partialupdate, sk_OPENSSL_STRING_num(cipher_names));
