...
static int verify_accept_cb(X509_STORE_CTX *ctx, void *arg) { return 1; }
...
static int parse_protos(const char *protos, unsigned char **out,
                        size_t *outlen) {
  size_t len, i, prefix;
...
static int client_npn_cb(SSL *s, unsigned char **out, unsigned char *outlen,
  ...
  ret = SSL_select_next_proto(out, outlen, in, inlen, ctx_data->npn_protocols,
...
static int configure_handshake_ctx(SSL_CTX *server_ctx, SSL_CTX *server2_ctx,
  ...
  if (!TEST_int_eq(
      SSL_CTX_set_max_send_fragment(server_ctx, test->max_fragment_size), 1))
    goto err;
  if (server2_ctx != NULL) {
    if (!TEST_int_eq(
        SSL_CTX_set_max_send_fragment(server2_ctx, test->max_fragment_size), 1))
      goto err;
  }
  if (!TEST_int_eq(
      SSL_CTX_set_max_send_fragment(client_ctx, test->max_fragment_size), 1))
    goto err;
  ...
  case TLSEXT_max_fragment_length_DISABLED:
    SSL_CTX_set_tlsext_max_fragment_length(client_ctx,
                                           extra->client.max_fragment_len_mode);
    break;
  }
  ...
  if (extra->client.alpn_protocols != NULL) {
    ...
    if (!TEST_true(parse_protos(extra->client.alpn_protocols, &alpn_protos,
                                &alpn_protos_len))
        /* Reversed return value convention... */
        ||
        !TEST_int_eq(
        SSL_CTX_set_alpn_protos(client_ctx, alpn_protos, alpn_protos_len), 0))
      goto err;
  ...
  if (extra->server2.session_ticket_app_data != NULL) {
    ...
    if (!TEST_ptr(server2_ctx_data->session_ticket_app_data))
      goto err;
    SSL_CTX_set_session_ticket_cb(server2_ctx, NULL, decrypt_session_ticket_cb,
                                  server2_ctx_data);
  }
  ...
  if (!configure_handshake_ctx_for_srp(server_ctx, server2_ctx, client_ctx,
                                       extra, server_ctx_data, server2_ctx_data,
                                       client_ctx_data))
    goto err;
...
static void do_reneg_setup_step(const SSL_TEST_CTX *test_ctx, PEER *peer) {
  ...
  if (!TEST_int_eq(peer->status, PEER_RETRY) ||
      !TEST_true(
      test_ctx->handshake_mode == SSL_TEST_HANDSHAKE_RENEG_SERVER ||
...
static int set_sock_as_sctp(int sock) {
  ...
  (void)setsockopt(sock, IPPROTO_SCTP, SCTP_RTOINFO, (const void *)&rto_info,
  ...
  (void)setsockopt(sock, IPPROTO_SCTP, SCTP_ASSOCINFO,
                   (const void *)&assocparams, sizeof(struct sctp_assocparams));
...
static int create_sctp_socks(int *ssock, int *csock) {
  ...
  for (ai = res; ai != NULL; ai = BIO_ADDRINFO_next(ai)) {
    ...
    if (!set_sock_as_sctp(lsock) ||
        !BIO_listen(lsock, BIO_ADDRINFO_address(ai), BIO_SOCK_REUSEADDR)) {
...
do_handshake_internal(SSL_CTX *server_ctx, SSL_CTX *server2_ctx,
  ...
  if (!configure_handshake_ctx(server_ctx, server2_ctx, client_ctx, test_ctx,
                               extra, &server_ctx_data, &server2_ctx_data,
                               &client_ctx_data)) {
  ...
  if (session_in != NULL) {
    ...
    if ((sess_id_len > 0 &&
         !TEST_true(SSL_CTX_add_session(server_ctx, serv_sess_in))) ||
  ...
  if (!TEST_int_ge(ex_data_idx, 0) ||
      !TEST_int_eq(SSL_set_ex_data(server.ssl, ex_data_idx, &server_ex_data),
                   1) ||
      !TEST_int_eq(SSL_set_ex_data(client.ssl, ex_data_idx, &client_ex_data),
                   1))
    goto err;
  ...
  for (;;) {
    if (client_turn) {
      ...
      status =
      handshake_status(client.status, server.status, 1 /* client went last */);
      if (server.status == PEER_WAITING)
        server.status = PEER_RETRY;
    } else {
      do_connect_step(test_ctx, &server, phase);
      status =
      handshake_status(server.status, client.status, 0 /* server went last */);
    }
...
HANDSHAKE_RESULT *do_handshake(SSL_CTX *server_ctx, SSL_CTX *server2_ctx,
  ...
  result =
  do_handshake_internal(server_ctx, server2_ctx, client_ctx, test_ctx,
                        &test_ctx->extra, NULL, NULL, &session, &serv_sess);
  ...
  result = do_handshake_internal(resume_server_ctx, NULL, resume_client_ctx,
                                 test_ctx, &test_ctx->resume_extra, session,
                                 serv_sess, NULL, NULL);
end:
