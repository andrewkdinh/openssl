...
static int broken_session_ticket_cb(SSL            *s,
 ...
 */
static int client_npn_cb(SSL                 *s,
                         unsigned char      **out,
 ...
 */
static int server_alpn_cb(SSL                  *s,
                          ...
                          void                 *arg)
{
     ...
     */
    ret  = SSL_select_next_proto(&tmp_out,
                                outlen,
...
static int decrypt_session_ticket_cb(SSL                 *s,
                                     SSL_SESSION         *ss,
 ...
 */
static int configure_handshake_ctx(SSL_CTX                   *server_ctx,
                                   SSL_CTX                   *server2_ctx,
                                   SSL_CTX                   *client_ctx,
                                   ...
                                   CTX_DATA                  *client_ctx_data)
{
    ...
    if (extra->client.alpn_protocols != NULL) {
        ...
        if (!TEST_true(parse_protos(extra->client.alpn_protocols, &alpn_protos, &alpn_protos_len))
            ...
            || !TEST_int_eq(SSL_CTX_set_alpn_protos(client_ctx, alpn_protos, (unsigned int)alpn_protos_len), 0))
            goto err;
    ...
    if (!TEST_ptr(ticket_keys = OPENSSL_zalloc(ticket_key_len))
        || !TEST_long_eq(SSL_CTX_set_tlsext_ticket_keys(server_ctx, ticket_keys, ticket_key_len), 1)) {
    ...
    if (!configure_handshake_ctx_for_srp(server_ctx,
                                         server2_ctx,
                                         client_ctx,
                                         extra,
                                         server_ctx_data,
...
typedef enum { PEER_SUCCESS, PEER_RETRY, PEER_ERROR, PEER_WAITING, PEER_TEST_FAILURE } peer_status_t;
 ...
 */
static handshake_status_t
handshake_status(peer_status_t last_status, peer_status_t previous_status, int client_spoke_last)
{
    switch (last_status) {
 ...
 * parsing.)
 */
static HANDSHAKE_RESULT *do_handshake_internal(SSL_CTX                   *server_ctx,
                                               SSL_CTX                   *server2_ctx,
                                               SSL_CTX                   *client_ctx,
                                               const SSL_TEST_CTX        *test_ctx,
                                               const SSL_TEST_EXTRA_CONF *extra,
                                               SSL_SESSION               *session_in,
                                               SSL_SESSION               *serv_sess_in,
                                               ...
                                               SSL_SESSION              **serv_sess_out)
{
    ...
    if (!configure_handshake_ctx(server_ctx,
                                 server2_ctx,
                                 client_ctx,
                                 test_ctx,
                                 extra,
...
HANDSHAKE_RESULT *do_handshake(SSL_CTX            *server_ctx,
                               SSL_CTX            *server2_ctx,
                               SSL_CTX            *client_ctx,
                               SSL_CTX            *resume_server_ctx,
                               ...
                               const SSL_TEST_CTX *test_ctx)
{
    ...
    result = do_handshake_internal(server_ctx,
                                   server2_ctx,
                                   client_ctx,
                                   test_ctx,
                                   &test_ctx->extra,
    ...
    result = do_handshake_internal(resume_server_ctx,
                                   NULL,
                                   resume_client_ctx,
                                   test_ctx,
                                   &test_ctx->resume_extra,
                                   session,
                                   serv_sess,
                                   NULL,
                                   NULL);
