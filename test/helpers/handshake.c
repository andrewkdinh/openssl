...
static int configure_handshake_ctx(SSL_CTX *server_ctx, SSL_CTX *server2_ctx,
    ...
    if (server2_ctx != NULL) {
        if (!TEST_int_eq(
            SSL_CTX_set_max_send_fragment(server2_ctx, test->max_fragment_size),
            1))
            goto err;
    }
    if (!TEST_int_eq(
    ...
    if (extra->server2.session_ticket_app_data != NULL) {
        ...
        server2_ctx, NULL, decrypt_session_ticket_cb, server2_ctx_data);
    }

    /*
     * Use fixed session ticket keys so that we can decrypt a ticket created
     * with one CTX in another CTX. Don't address server2 for the moment.
     */
