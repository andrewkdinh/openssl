                                   ...
                                   CTX_DATA *client_ctx_data)
{
    ...
    if (!TEST_int_eq(
            SSL_CTX_set_max_send_fragment(server_ctx, test->max_fragment_size),
            1))
        goto err;
    if (server2_ctx != NULL) {
    ...
    if (!TEST_int_eq(
            SSL_CTX_set_max_send_fragment(client_ctx, test->max_fragment_size),
            1))
        goto err;
    ...
    if (!TEST_ptr(ticket_keys = OPENSSL_zalloc(ticket_key_len))
        || !TEST_int_eq(SSL_CTX_set_tlsext_ticket_keys(server_ctx, ticket_keys,
                                                       ticket_key_len),
                        1)) {
                      ...
                      SSL_SESSION **serv_sess_out)
{
    ...
    if (!TEST_int_ge(ex_data_idx, 0)
        || !TEST_int_eq(
            SSL_set_ex_data(server.ssl, ex_data_idx, &server_ex_data), 1)
        || !TEST_int_eq(
            SSL_set_ex_data(client.ssl, ex_data_idx, &client_ex_data), 1))
        goto err;
