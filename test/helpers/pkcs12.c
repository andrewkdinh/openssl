...
static void generate_p12(PKCS12_BUILDER *pb, const PKCS12_ENC *mac) {
  ...
  if (mac != NULL) {
    ...
    if (!TEST_true(PKCS12_set_mac(p12, mac->pass, strlen(mac->pass), NULL, 0,
                                  mac->iter, md))) {
      pb->success = 0;
      goto err;
    }
...
void end_contentinfo_encrypted(PKCS12_BUILDER *pb, const PKCS12_ENC *enc) {
  if (pb->success && pb->bags != NULL) {
    if (legacy) {
      if (!TEST_true(PKCS12_add_safe(&pb->safes, pb->bags, enc->nid, enc->iter,
                                     enc->pass)))
        pb->success = 0;
    } else {
...
void add_keybag(PKCS12_BUILDER *pb, const unsigned char *bytes, int len,
  ...
  if (legacy)
    bag = PKCS12_add_key(&pb->bags, pkey, 0 /*keytype*/, enc->iter, enc->nid,
                         enc->pass);
  else
    bag = PKCS12_add_key_ex(&pb->bags, pkey, 0 /*keytype*/, enc->iter, enc->nid,
                            enc->pass, test_ctx, test_propq);
  if (!TEST_ptr(bag)) {
    pb->success = 0;
    goto err;
  }
...
void add_secretbag(PKCS12_BUILDER *pb, int secret_nid, const char *secret,
  ...
  bag = PKCS12_add_secret(&pb->bags, secret_nid, (const unsigned char *)secret,
                          strlen(secret));
  if (!TEST_ptr(bag)) {
    pb->success = 0;
    return;
