...
static int add_attributes(PKCS12_SAFEBAG *bag, const PKCS12_ATTR *attr)
{
    ...
    while (p_attr->oid != NULL) {
        ...
        if (attr_nid == NID_friendlyName) {
            ...
            if (!TEST_true(PKCS12_add_localkeyid(bag,
            ...
            if (!TEST_true(PKCS12_add1_attr_by_txt(
                    bag, p_attr->oid, MBSTRING_ASC,
                    (unsigned char *)p_attr->value, strlen(p_attr->value))))
                goto err;
        }
        p_attr++;
    }
                     ...
                     const PKCS12_ATTR *attrs)
{
    ...
    if (!check_attrs(PKCS12_SAFEBAG_get0_attrs(bag), attrs)
        ...
        || !TEST_true(
            check_asn1_string(PKCS12_SAFEBAG_get0_bag_obj(bag), secret)))
        pb->success = 0;
}
