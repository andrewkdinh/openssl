...
static int check_attrs(const STACK_OF(X509_ATTRIBUTE) * bag_attrs,
...
void PKCS12_helper_set_write_files(int enable) { write_files = enable; }
...
void start_pkcs12(PKCS12_BUILDER *pb) { pb->safes = NULL; }
...
static void generate_p12(PKCS12_BUILDER *pb, const PKCS12_ENC *mac) {
    ...
    if (mac != NULL) {
        ...
        if (!TEST_true(PKCS12_set_mac(p12, mac->pass, strlen(mac->pass), NULL,
                                      0, mac->iter, md))) {
            pb->success = 0;
            goto err;
        }
...
void start_contentinfo(PKCS12_BUILDER *pb) { pb->bags = NULL; }
...
static STACK_OF(PKCS12_SAFEBAG) *
decode_contentinfo(STACK_OF(PKCS7) * safes, int idx, const PKCS12_ENC *enc) {
...
static int add_attributes(PKCS12_SAFEBAG *bag, const PKCS12_ATTR *attr) {
    ...
    while (p_attr->oid != NULL) {
        ...
        if (attr_nid == NID_friendlyName) {
            ...
            if (!TEST_true(PKCS12_add_localkeyid(
                bag, (unsigned char *)p_attr->value, strlen(p_attr->value))))
                goto err;
        } else if (attr_nid == NID_oracle_jdk_trustedkeyusage) {
            attrs = (STACK_OF(X509_ATTRIBUTE) *)PKCS12_SAFEBAG_get0_attrs(bag);
            x509_attr = X509_ATTRIBUTE_create(attr_nid, V_ASN1_OBJECT,
                                              OBJ_txt2obj(p_attr->value, 0));
            ...
            if (!TEST_true(PKCS12_add1_attr_by_txt(
                bag, p_attr->oid, MBSTRING_ASC, (unsigned char *)p_attr->value,
                strlen(p_attr->value))))
                goto err;
        }
        p_attr++;
...
void add_keybag(PKCS12_BUILDER *pb, const unsigned char *bytes, int len,
    ...
    if (legacy)
        bag = PKCS12_add_key(&pb->bags, pkey, 0 /*keytype*/, enc->iter,
                             enc->nid, enc->pass);
    else
        bag = PKCS12_add_key_ex(&pb->bags, pkey, 0 /*keytype*/, enc->iter,
                                enc->nid, enc->pass, test_ctx, test_propq);
    if (!TEST_ptr(bag)) {
        pb->success = 0;
        goto err;
    }
...
static int check_asn1_string(const ASN1_TYPE *av, const char *txt) {
    ...
    case V_ASN1_BMPSTRING:
        value =
        OPENSSL_uni2asc(av->value.bmpstring->data, av->value.bmpstring->length);
    ...
    case V_ASN1_OCTET_STRING:
        if (!TEST_mem_eq(txt, strlen(txt), (char *)av->value.octet_string->data,
...
void check_keybag(PKCS12_BUILDER *pb, const unsigned char *bytes, int len,
    ...
    case NID_pkcs8ShroudedKeyBag:
        if (legacy)
            ...
            p8 = PKCS12_decrypt_skey_ex(bag, enc->pass, strlen(enc->pass),
                                        test_ctx, test_propq);
        if (!TEST_ptr(p8)) {
            pb->success = 0;
            goto err;
