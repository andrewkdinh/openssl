...
static OSSL_TIME fake_now_cb(void *arg) { return qtest_get_time(); }
  ...
  *qtserv = NULL;
  ...
  if ((flags & QTEST_FLAG_NOISE) != 0) {
    ...
    if ((flags & QTEST_FLAG_FAKE_TIME) != 0) {
      if (!TEST_int_eq(BIO_ctrl(cbio, BIO_CTRL_NOISE_SET_NOW_CB, 0, &now_cb),
                       1))
        goto err;
    }
    ...
    if ((flags & QTEST_FLAG_FAKE_TIME) != 0) {
      if (!TEST_int_eq(BIO_ctrl(sbio, BIO_CTRL_NOISE_SET_NOW_CB, 0, &now_cb),
                       1))
        goto err;
    }
  ...
  if (!TEST_ptr(*qtserv =
                ossl_quic_tserver_new(&tserver_args, certfile, keyfile)))
    goto err;
...
void qtest_start_stopwatch(void) { start_time = qtest_get_time(); }
...
#if !defined(OPENSSL_NO_POSIX_IO) && defined(OPENSSL_THREADS) &&               \
!defined(CRYPTO_TDEBUG)
  return 1;
#else
  return 0;
#endif
...
int qtest_check_server_transport_err(QUIC_TSERVER *qtserv, uint64_t code) {
  ...
  if (!TEST_ptr(cause) || !TEST_true(cause->remote) ||
      !TEST_false(cause->app) || !TEST_uint64_t_eq(cause->error_code, code))
    return 0;
...
int qtest_check_server_protocol_err(QUIC_TSERVER *qtserv) {
  return qtest_check_server_transport_err(qtserv,
...
static int packet_plain_mutate(const QUIC_PKT_HDR *hdrin,
                               ...
                               QUIC_PKT_HDR **hdrout,
                               const OSSL_QTX_IOVEC **iovecout, size_t *numout,
                               void *arg) {
...
void *pplaincbarg) {
  ...
  return ossl_quic_tserver_set_plain_packet_mutator(
  fault->qtserv, packet_plain_mutate, packet_plain_finish, fault);
}
...
static int handshake_mutate(const unsigned char *msgin, size_t msginlen,
  ...
  if (!PACKET_buf_init(&pkt, buf, msginlen) || !PACKET_get_1(&pkt, &msgtype) ||
  ...
  if (fault->handshakecb != NULL &&
      !fault->handshakecb(fault, buf, fault->handbuflen, fault->handshakecbarg))
    return 0;
...
int qtest_fault_set_handshake_listener(QTEST_FAULT *fault,
  ...
  return ossl_quic_tserver_set_handshake_mutator(
  fault->qtserv, handshake_mutate, handshake_finish, fault);
}
...
static int pcipher_sendmmsg(BIO *b, BIO_MSG *msg, size_t stride, size_t num_msg,
  ...
  for (i = 0; i < num_msg; ++i) {
    ...
    if (bdata->fault->pciphercb != NULL) {
      if (!PACKET_buf_init(&pkt, bdata->fault->msg.data,
                           bdata->fault->msg.data_len))
        return 0;

      do {
        if (!ossl_quic_wire_decode_pkt_hdr(&pkt, bdata->short_conn_id_len, 1, 0,
                                           &hdr, NULL, NULL))
          goto out;
...
int qtest_fault_set_bw_limit(QTEST_FAULT *fault, size_t ctos_bw, size_t stoc_bw,
  ...
  if (!TEST_int_eq(
      BIO_ctrl(cbio, BIO_CTRL_NOISE_SEND_BANDWIDTH, (long)ctos_bw, NULL), 1))
    return 0;
  if (!TEST_int_eq(
      BIO_ctrl(sbio, BIO_CTRL_NOISE_SEND_BANDWIDTH, (long)stoc_bw, NULL), 1))
    return 0;
  return 1;
}
