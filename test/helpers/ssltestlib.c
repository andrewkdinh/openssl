...
#if (!defined(OPENSSL_NO_KTLS) || !defined(OPENSSL_NO_QUIC)) &&                \
!defined(OPENSSL_NO_POSIX_IO) && !defined(OPENSSL_NO_SOCK)
...
void bio_f_tls_dump_filter_free(void) { BIO_meth_free(method_tls_dump); }
...
static void dump_data(const char *data, int len) {
  ...
  while (rem > 0) {
    ...
    printf("** Record Version: %02x%02x\n", rec[RECORD_VERSION_HI],
           rec[RECORD_VERSION_LO]);
    ...
    if (content == SSL3_RT_HANDSHAKE) {
      ...
      if (epoch > 0) {
        ...
        msglen =
        (rec[MSG_LEN_HI] << 16) | (rec[MSG_LEN_MID] << 8) | rec[MSG_LEN_LO];
        printf("*** Message Length: %d\n", msglen);
        printf("*** Message sequence: %d\n",
               (rec[MSG_SEQ_HI] << 8) | rec[MSG_SEQ_LO]);
        fragoff = (rec[MSG_FRAG_OFF_HI] << 16) | (rec[MSG_FRAG_OFF_MID] << 8) |
                  rec[MSG_FRAG_OFF_LO];
        printf("*** Message Fragment offset: %d\n", fragoff);
        fraglen = (rec[MSG_FRAG_LEN_HI] << 16) | (rec[MSG_FRAG_LEN_MID] << 8) |
                  rec[MSG_FRAG_LEN_LO];
...
const BIO_METHOD *bio_s_mempacket_test(void) {
  if (meth_mem == NULL) {
    if (!TEST_ptr(meth_mem =
                  BIO_meth_new(BIO_TYPE_MEMPACKET_TEST, "Mem Packet Test")) ||
...
void bio_s_mempacket_test_free(void) { BIO_meth_free(meth_mem); }
...
static int mempacket_test_read(BIO *bio, char *out, int outl) {
  ...
  if (thispkt->type != INJECT_PACKET_IGNORE_REC_SEQ &&
    ...
    for (rem = thispkt->len, rec = thispkt->data; rem > 0; rem -= len) {
      ...
      len =
      ((rec[RECORD_LEN_HI] << 8) | rec[RECORD_LEN_LO]) + DTLS1_RT_HEADER_LENGTH;
      if (rem < (int)len)
        return -1;
...
int mempacket_swap_epoch(BIO *bio) {
  ...
  for (rem = thispkt->len, rec = thispkt->data; rem > 0;
       rem -= len, rec += len) {
    if (rem < DTLS1_RT_HEADER_LENGTH)
      return 0;
    epoch = (rec[EPOCH_HI] << 8) | rec[EPOCH_LO];
...
int mempacket_test_inject(BIO *bio, const char *in, int inl, int pktnum,
  ...
  size_t len =
  ((inu[RECORD_LEN_HI] << 8) | inu[RECORD_LEN_LO]) + DTLS1_RT_HEADER_LENGTH;
...
const BIO_METHOD *bio_s_always_retry(void) {
  if (meth_always_retry == NULL) {
    if (!TEST_ptr(meth_always_retry =
                  BIO_meth_new(BIO_TYPE_ALWAYS_RETRY, "Always Retry")) ||
...
void bio_s_always_retry_free(void) { BIO_meth_free(meth_always_retry); }
...
void set_always_retry_err_val(int err) { retry_err = err; }
...
const BIO_METHOD *bio_s_maybe_retry(void) {
  if (meth_maybe_retry == NULL) {
    if (!TEST_ptr(meth_maybe_retry =
                  BIO_meth_new(BIO_TYPE_MAYBE_RETRY, "Maybe Retry")) ||
...
void bio_s_maybe_retry_free(void) { BIO_meth_free(meth_maybe_retry); }
...
defined(OPENSSL_NO_DH)
  ...
  if (serverctx != NULL &&
      ((min_proto_version > 0 && !TEST_true(SSL_CTX_set_min_proto_version(
                                 serverctx, min_proto_version))) ||
       (max_proto_version > 0 && !TEST_true(SSL_CTX_set_max_proto_version(
                                 serverctx, max_proto_version)))))
    goto err;
  if (clientctx != NULL &&
      ((min_proto_version > 0 && !TEST_true(SSL_CTX_set_min_proto_version(
                                 clientctx, min_proto_version))) ||
       (max_proto_version > 0 && !TEST_true(SSL_CTX_set_max_proto_version(
                                 clientctx, max_proto_version)))))
    goto err;
...
int create_test_sockets(int *cfdp, int *sfdp, int socktype, BIO_ADDR *saddr) {
  ...
  if (saddr != NULL && !BIO_ADDR_rawmake(saddr, sin.sin_family, &sin.sin_addr,
  ...
  while (sfd == -1 || !cfd_connected) {
    ...
    if (!cfd_connected &&
        connect(cfd, (struct sockaddr *)&sin, sizeof(sin)) < 0)
      goto out;
    else
      cfd_connected = 1;
  }
...
int create_ssl_objects2(SSL_CTX *serverctx, SSL_CTX *clientctx, SSL **sssl,
  ...
  if (!TEST_true(SSL_get_rpoll_descriptor(clientssl, &rdesc)) ||
      ...
      !TEST_int_eq(rdesc.value.fd, cfd) || !TEST_int_eq(wdesc.value.fd, cfd))
    goto error;
  ...
  if (!TEST_true(SSL_get_rpoll_descriptor(serverssl, &rdesc)) ||
      ...
      !TEST_int_eq(rdesc.type, BIO_POLL_DESCRIPTOR_TYPE_SOCK_FD) ||
      !TEST_int_eq(wdesc.type, BIO_POLL_DESCRIPTOR_TYPE_SOCK_FD) ||
      !TEST_int_eq(rdesc.value.fd, sfd) || !TEST_int_eq(wdesc.value.fd, sfd))
    goto error;
...
int wait_until_sock_readable(int sock) { return 0; }
...
SSL_SESSION *create_a_psk(SSL *ssl, size_t mdsize) {
  ...
  if (!TEST_ptr(sess) || !TEST_ptr(cipher) ||
