...
const BIO_METHOD *bio_f_tls_dump_filter(void) {
    if (method_tls_dump == NULL) {
        method_tls_dump =
        BIO_meth_new(BIO_TYPE_TLS_DUMP_FILTER, "TLS dump filter");
        if (method_tls_dump == NULL ||
        ...
        if (content == SSL3_RT_HANDSHAKE) {
            ...
            if (epoch > 0) {
                ...
                fragoff = (rec[MSG_FRAG_OFF_HI] << 16) |
                          (rec[MSG_FRAG_OFF_MID] << 8) | rec[MSG_FRAG_OFF_LO];
                printf("*** Message Fragment offset: %d\n", fragoff);
                fraglen = (rec[MSG_FRAG_LEN_HI] << 16) |
                          (rec[MSG_FRAG_LEN_MID] << 8) | rec[MSG_FRAG_LEN_LO];
...
const BIO_METHOD *bio_s_maybe_retry(void) {
    if (meth_maybe_retry == NULL) {
        if (!TEST_ptr(meth_maybe_retry =
            ...
            !TEST_true(
            BIO_meth_set_write(meth_maybe_retry, maybe_retry_write)) ||
            !TEST_true(BIO_meth_set_ctrl(meth_maybe_retry, maybe_retry_ctrl)) ||
            !TEST_true(
            BIO_meth_set_create(meth_maybe_retry, maybe_retry_new)) ||
            !TEST_true(
            BIO_meth_set_destroy(meth_maybe_retry, maybe_retry_free)))
            return NULL;
    }
    ...
    if (sctx != NULL) {
        if (*sctx != NULL)
            serverctx = *sctx;
                 ...
                 serverctx, SSL_OP_ALLOW_CLIENT_RENEGOTIATION)))
            goto err;
    }
