...
static int server_srp_cb(SSL *s, int *ad, void *arg)
{
    ...
    if (SSL_set_srp_server_param_pw(s, ctx_data->srp_user, ctx_data->srp_password, "2048" /* known group */) < 0) {
                                    ...
                                    CTX_DATA *server2_ctx_data, CTX_DATA *client_ctx_data)
{
    ...
    if (extra->client.srp_user != NULL) {
        if (!TEST_true(SSL_CTX_set_srp_username(client_ctx, extra->client.srp_user)))
            return 0;
