...
static int test_invalid_template(void) {
  ...
  INVALIDTEMPLATE *tmp =
  d2i_INVALIDTEMPLATE(NULL, &p, sizeof(t_invalid_template));
...
static int test_reuse_asn1_object(void) {
  ...
  static unsigned char oid_der[] = {0x06, 0x06, 0x2a, 0x03,
                                    0x04, 0x05, 0x06, 0x07};
  int ret = 0;
  ASN1_OBJECT *obj;
  ...
  if (!TEST_ptr(obj = ASN1_OBJECT_create(NID_undef, cn_der, sizeof(cn_der), "C",
                                         "countryName")))
    goto err;
