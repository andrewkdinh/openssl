...
static int test_decode_nonfipsalg(void) {
    ...
    /*
     * If NULL is passed as the propq here it uses the global property
     * "fips=true", Which we expect to fail if the decode uses a non FIPS
     * algorithm
     */
    if (!TEST_ptr_null(
    ...
    /*
     * Pass if we override the libctx global prop query to optionally use
     * fips=true This assumes that the libctx contains the default provider
     */
