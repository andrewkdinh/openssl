...
static int equalBN(const char *op, const BIGNUM *expected,
                   const BIGNUM *actual) {
...
static int test_swap(void) {
  ...
  if (!TEST_ptr(a = BN_new()) || !TEST_ptr(b = BN_new()) ||
      !TEST_ptr(c = BN_new()) || !TEST_ptr(d = BN_new()))
    goto err;

  if (!(TEST_true(BN_bntest_rand(a, 1024, 1, 0)) &&
        TEST_true(BN_bntest_rand(b, 1024, 1, 0)) && TEST_ptr(BN_copy(c, a)) &&
  ...
  BN_swap(a, b);
  if (!equalBN("swap, flags", a, d) || !equalBN("swap, flags", b, c) ||
  ...
  for (i = 0; i < NUM0 + NUM1; i++) {
    if (i < NUM1) {
      if (!(TEST_true(BN_bntest_rand(a, 512, 0, 0))) &&
          TEST_ptr(BN_copy(b, a)) && TEST_int_ne(BN_set_bit(a, i), 0) &&
    ...
    if (!(TEST_true(BN_sub(c, a, b)) && TEST_true(BN_add(c, c, b)) &&
          TEST_true(BN_sub(c, c, a)) && TEST_BN_eq_zero(c)))
      goto err;
  }
  st = 1;
err:
  ...
  for (i = 0; i < NUM0 + NUM1; i++) {
    if (i < NUM1) {
      if (!(TEST_true(BN_bntest_rand(a, 400, 0, 0)) &&
            TEST_ptr(BN_copy(b, a)) && TEST_true(BN_lshift(a, a, i)) &&
    ...
    if (!(TEST_true(BN_RECP_CTX_set(recp, b, ctx)) &&
          ...
          TEST_true(BN_mul(e, d, b, ctx)) && TEST_true(BN_add(d, e, c)) &&
          TEST_true(BN_sub(d, d, a)) && TEST_BN_eq_zero(d)))
      goto err;
  }
  st = 1;
err:
...
static int test_signed_mod_replace_ab(int n) {
  ...
  if (TEST_true(BN_div(a, b, a, b, ctx)) && TEST_BN_eq(a, c) &&
      TEST_BN_eq(b, d))
    st = 1;
err:
  BN_free(a);
...
static int test_signed_mod_replace_ba(int n) {
  ...
  if (TEST_true(BN_div(b, a, a, b, ctx)) && TEST_BN_eq(b, c) &&
      TEST_BN_eq(a, d))
    st = 1;
err:
  BN_free(a);
...
static int test_mod(void) {
  ...
  for (i = 0; i < NUM0; i++) {
    ...
    if (!(TEST_true(BN_mod(c, a, b, ctx)) &&
          TEST_true(BN_div(d, e, a, b, ctx)) && TEST_BN_eq(e, c) &&
          TEST_true(BN_mul(c, d, b, ctx)) && TEST_true(BN_add(d, c, e)) &&
          TEST_BN_eq(d, a)))
      goto err;
  }
  st = 1;
...
static int test_modexp_mont5(void) {
  ...
  if (!TEST_ptr(a = BN_new()) || !TEST_ptr(p = BN_new()) ||
      !TEST_ptr(m = BN_new()) || !TEST_ptr(d = BN_new()) ||
      !TEST_ptr(e = BN_new()) || !TEST_ptr(b = BN_new()) ||
      !TEST_ptr(n = BN_new()) || !TEST_ptr(c = BN_new()) ||
  ...
  if (!(
      ...
      TEST_true(BN_hex2bn(
      &n, "D78AF684E71DB0C39CFF4E64FB9DB567132CB9C50CC98009FEB820B26F2DED9B"
...
static int test_gf2m_mod(void) {
  ...
  if (!TEST_ptr(a = BN_new()) || !TEST_ptr(b[0] = BN_new()) ||
      !TEST_ptr(b[1] = BN_new()) || !TEST_ptr(c = BN_new()) ||
...
static int test_gf2m_mul(void) {
  ...
  if (!TEST_ptr(a = BN_new()) || !TEST_ptr(b[0] = BN_new()) ||
      !TEST_ptr(b[1] = BN_new()) || !TEST_ptr(c = BN_new()) ||
      !TEST_ptr(d = BN_new()) || !TEST_ptr(e = BN_new()) ||
      !TEST_ptr(f = BN_new()) || !TEST_ptr(g = BN_new()) ||
      !TEST_ptr(h = BN_new()))
    goto err;
...
static int test_kronecker(void) {
  ...
  if (!TEST_ptr(a = BN_new()) || !TEST_ptr(b = BN_new()) ||
      !TEST_ptr(r = BN_new()) || !TEST_ptr(t = BN_new()))
    goto err;
...
static int file_sum(STANZA *s) {
  ...
  if (!TEST_ptr(a = getBN(s, "A")) || !TEST_ptr(b = getBN(s, "B")) ||
      !TEST_ptr(sum = getBN(s, "Sum")) || !TEST_ptr(ret = BN_new()))
    goto err;

  if (!TEST_true(BN_add(ret, a, b)) || !equalBN("A + B", sum, ret) ||
      !TEST_true(BN_sub(ret, sum, a)) || !equalBN("Sum - A", b, ret) ||
   ...
   */
  if (!TEST_true(BN_copy(ret, a)) || !TEST_true(BN_add(ret, ret, b)) ||
      !equalBN("A + B (r is a)", sum, ret) || !TEST_true(BN_copy(ret, b)) ||
      !TEST_true(BN_add(ret, a, ret)) || !equalBN("A + B (r is b)", sum, ret) ||
      !TEST_true(BN_copy(ret, sum)) || !TEST_true(BN_sub(ret, ret, a)) ||
      !equalBN("Sum - A (r is a)", b, ret) || !TEST_true(BN_copy(ret, a)) ||
      !TEST_true(BN_sub(ret, sum, ret)) ||
      !equalBN("Sum - A (r is b)", b, ret) || !TEST_true(BN_copy(ret, sum)) ||
      !TEST_true(BN_sub(ret, ret, b)) || !equalBN("Sum - B (r is a)", a, ret) ||
      !TEST_true(BN_copy(ret, b)) || !TEST_true(BN_sub(ret, sum, ret)) ||
  ...
  if (!BN_is_negative(a) && !BN_is_negative(b) && BN_cmp(a, b) >= 0) {
    if (!TEST_true(BN_uadd(ret, a, b)) || !equalBN("A +u B", sum, ret) ||
        !TEST_true(BN_usub(ret, sum, a)) || !equalBN("Sum -u A", b, ret) ||
     ...
     * BIGNUM.
     */
    if (!TEST_true(BN_copy(ret, a)) || !TEST_true(BN_uadd(ret, ret, b)) ||
        !equalBN("A +u B (r is a)", sum, ret) || !TEST_true(BN_copy(ret, b)) ||
  ...
  if (!BN_is_negative(b) && b_word != (BN_ULONG)-1) {
    if (!TEST_true(BN_copy(ret, a)) || !TEST_true(BN_add_word(ret, b_word)) ||
        !equalBN("A + B (word)", sum, ret) || !TEST_true(BN_copy(ret, sum)) ||
...
static int file_lshift1(STANZA *s) {
  ...
  if (!TEST_ptr(a = getBN(s, "A")) ||
      !TEST_ptr(lshift1 = getBN(s, "LShift1")) || !TEST_ptr(zero = BN_new()) ||
      !TEST_ptr(ret = BN_new()) || !TEST_ptr(two = BN_new()) ||
      !TEST_ptr(remainder = BN_new()))
    goto err;

  BN_zero(zero);

  if (!TEST_true(BN_set_word(two, 2)) || !TEST_true(BN_add(ret, a, a)) ||
      !equalBN("A + A", lshift1, ret) || !TEST_true(BN_mul(ret, a, two, ctx)) ||
      ...
      !TEST_true(BN_lshift1(ret, a)) || !equalBN("A << 1", lshift1, ret) ||
...
static int file_lshift(STANZA *s) {
  ...
  if (!TEST_ptr(a = getBN(s, "A")) || !TEST_ptr(lshift = getBN(s, "LShift")) ||
      !TEST_ptr(ret = BN_new()) || !getint(s, &n, "N"))
    goto err;

  if (!TEST_true(BN_lshift(ret, a, n)) || !equalBN("A << N", lshift, ret) ||
      !TEST_true(BN_rshift(ret, lshift, n)) || !equalBN("A >> N", a, ret))
    goto err;
...
static int file_rshift(STANZA *s) {
  ...
  if (!TEST_ptr(a = getBN(s, "A")) || !TEST_ptr(rshift = getBN(s, "RShift")) ||
      !TEST_ptr(ret = BN_new()) || !getint(s, &n, "N"))
    goto err;
...
static int file_square(STANZA *s) {
  ...
  if (!TEST_ptr(a = getBN(s, "A")) || !TEST_ptr(square = getBN(s, "Square")) ||
      !TEST_ptr(zero = BN_new()) || !TEST_ptr(ret = BN_new()) ||
      !TEST_ptr(remainder = BN_new()))
    goto err;

  BN_zero(zero);
  if (!TEST_true(BN_sqr(ret, a, ctx)) || !equalBN("A^2", square, ret) ||
      !TEST_true(BN_mul(ret, a, a, ctx)) || !equalBN("A * A", square, ret) ||
...
static int file_product(STANZA *s) {
  ...
  if (!TEST_ptr(a = getBN(s, "A")) || !TEST_ptr(b = getBN(s, "B")) ||
      !TEST_ptr(product = getBN(s, "Product")) || !TEST_ptr(ret = BN_new()) ||
      !TEST_ptr(remainder = BN_new()) || !TEST_ptr(zero = BN_new()))
    goto err;
...
static int file_quotient(STANZA *s) {
  ...
  if (!TEST_true(BN_div(ret, ret2, a, b, ctx)) ||
      !equalBN("A / B", quotient, ret) || !equalBN("A % B", remainder, ret2) ||
...
#ifdef BN_DEC_FMT1
      TEST_error("Got A %% B (word) = " BN_DEC_FMT1 ", wanted " BN_DEC_FMT1,
  ...
  if (!BN_is_negative(b)) {
    if (!TEST_true(BN_copy(nnmod, remainder)) ||
        (BN_is_negative(nnmod) && !TEST_true(BN_add(nnmod, nnmod, b))) ||
...
static int file_modmul(STANZA *s) {
  ...
  if (!TEST_ptr(a = getBN(s, "A")) || !TEST_ptr(b = getBN(s, "B")) ||
      !TEST_ptr(m = getBN(s, "M")) || !TEST_ptr(mod_mul = getBN(s, "ModMul")) ||
      !TEST_ptr(ret = BN_new()))
    goto err;
...
static int file_modexp(STANZA *s) {
  ...
  if (!TEST_ptr(a = getBN(s, "A")) || !TEST_ptr(e = getBN(s, "E")) ||
      !TEST_ptr(m = getBN(s, "M")) || !TEST_ptr(mod_exp = getBN(s, "ModExp")) ||
      !TEST_ptr(ret = BN_new()) || !TEST_ptr(d = BN_new()))
    goto err;
  ...
  if (!TEST_true(BN_mod_exp(d, a, b, c, ctx)) ||
      !TEST_true(BN_mul(e, a, a, ctx)) || !TEST_BN_eq(d, e))
    goto err;
...
static int file_exp(STANZA *s) {
  ...
  if (!TEST_ptr(a = getBN(s, "A")) || !TEST_ptr(e = getBN(s, "E")) ||
      !TEST_ptr(exp = getBN(s, "Exp")) || !TEST_ptr(ret = BN_new()))
    goto err;
...
static int file_modsqrt(STANZA *s) {
  ...
  if (!TEST_ptr(a = getBN(s, "A")) || !TEST_ptr(p = getBN(s, "P")) ||
      !TEST_ptr(mod_sqrt = getBN(s, "ModSqrt")) || !TEST_ptr(ret = BN_new()) ||
...
static int file_gcd(STANZA *s) {
  ...
  if (!TEST_ptr(a = getBN(s, "A")) || !TEST_ptr(b = getBN(s, "B")) ||
      !TEST_ptr(gcd = getBN(s, "GCD")) || !TEST_ptr(ret = BN_new()))
    goto err;
...
static int test_dec2bn(void) {
  ...
  if (!TEST_int_eq(parsedecBN(&bn, "0"), 1) || !TEST_BN_eq_word(bn, 0) ||
      !TEST_BN_eq_zero(bn) || !TEST_BN_le_zero(bn) || !TEST_BN_ge_zero(bn) ||
      !TEST_BN_even(bn))
    goto err;
  BN_free(bn);
  bn = NULL;

  if (!TEST_int_eq(parsedecBN(&bn, "256"), 3) || !TEST_BN_eq_word(bn, 256) ||
      !TEST_BN_ge_zero(bn) || !TEST_BN_gt_zero(bn) || !TEST_BN_ne_zero(bn) ||
      !TEST_BN_even(bn))
    goto err;
  BN_free(bn);
  bn = NULL;

  if (!TEST_int_eq(parsedecBN(&bn, "-42"), 3) || !TEST_BN_abs_eq_word(bn, 42) ||
      !TEST_BN_lt_zero(bn) || !TEST_BN_le_zero(bn) || !TEST_BN_ne_zero(bn) ||
      !TEST_BN_even(bn))
    goto err;
  BN_free(bn);
  bn = NULL;

  if (!TEST_int_eq(parsedecBN(&bn, "1"), 1) || !TEST_BN_eq_word(bn, 1) ||
      !TEST_BN_ne_zero(bn) || !TEST_BN_gt_zero(bn) || !TEST_BN_ge_zero(bn) ||
      !TEST_BN_eq_one(bn) || !TEST_BN_odd(bn))
    goto err;
  BN_free(bn);
  bn = NULL;

  if (!TEST_int_eq(parsedecBN(&bn, "-0"), 2) || !TEST_BN_eq_zero(bn) ||
      !TEST_BN_ge_zero(bn) || !TEST_BN_le_zero(bn) || !TEST_BN_even(bn))
    goto err;
  BN_free(bn);
  bn = NULL;

  if (!TEST_int_eq(parsedecBN(&bn, "42trailing garbage is ignored"), 2) ||
      !TEST_BN_abs_eq_word(bn, 42) || !TEST_BN_ge_zero(bn) ||
...
static int test_hex2bn(void) {
  ...
  if (!TEST_int_eq(parseBN(&bn, "0"), 1) || !TEST_BN_eq_zero(bn) ||
      !TEST_BN_ge_zero(bn) || !TEST_BN_even(bn))
    goto err;
  BN_free(bn);
  bn = NULL;

  if (!TEST_int_eq(parseBN(&bn, "256"), 3) || !TEST_BN_eq_word(bn, 0x256) ||
      !TEST_BN_ge_zero(bn) || !TEST_BN_gt_zero(bn) || !TEST_BN_ne_zero(bn) ||
      !TEST_BN_even(bn))
    goto err;
  BN_free(bn);
  ...
  if (!TEST_int_eq(parseBN(&bn, "-42"), 3) || !TEST_BN_abs_eq_word(bn, 0x42) ||
      !TEST_BN_lt_zero(bn) || !TEST_BN_le_zero(bn) || !TEST_BN_ne_zero(bn) ||
      !TEST_BN_even(bn))
    goto err;
  BN_free(bn);
  ...
  if (!TEST_int_eq(parseBN(&bn, "cb"), 2) || !TEST_BN_eq_word(bn, 0xCB) ||
      !TEST_BN_ge_zero(bn) || !TEST_BN_gt_zero(bn) || !TEST_BN_ne_zero(bn) ||
      !TEST_BN_odd(bn))
    goto err;
  BN_free(bn);
  ...
  if (!TEST_int_eq(parseBN(&bn, "-0"), 2) || !TEST_BN_eq_zero(bn) ||
      !TEST_BN_ge_zero(bn) || !TEST_BN_le_zero(bn) || !TEST_BN_even(bn))
    goto err;
  BN_free(bn);
  bn = NULL;
  ...
  if (!TEST_int_eq(parseBN(&bn, "abctrailing garbage is ignored"), 3) ||
      !TEST_BN_eq_word(bn, 0xabc) || !TEST_BN_ge_zero(bn) ||
      !TEST_BN_gt_zero(bn) || !TEST_BN_ne_zero(bn) || !TEST_BN_even(bn))
    goto err;
  st = 1;
...
static int test_asc2bn(void) {
  ...
  if (!TEST_true(BN_asc2bn(&bn, "0")) || !TEST_BN_eq_zero(bn) ||
      !TEST_BN_ge_zero(bn))
    goto err;

  if (!TEST_true(BN_asc2bn(&bn, "256")) || !TEST_BN_eq_word(bn, 256) ||
      !TEST_BN_ge_zero(bn))
    goto err;

  if (!TEST_true(BN_asc2bn(&bn, "-42")) || !TEST_BN_abs_eq_word(bn, 42) ||
      !TEST_BN_lt_zero(bn))
    goto err;

  if (!TEST_true(BN_asc2bn(&bn, "0x1234")) || !TEST_BN_eq_word(bn, 0x1234) ||
      !TEST_BN_ge_zero(bn))
    goto err;
  ...
  if (!TEST_true(BN_asc2bn(&bn, "-0xabcd")) ||
      !TEST_BN_abs_eq_word(bn, 0xabcd) || !TEST_BN_lt_zero(bn))
    goto err;
  ...
  if (!TEST_true(BN_asc2bn(&bn, "123trailing garbage is ignored")) ||
      !TEST_BN_eq_word(bn, 123) || !TEST_BN_ge_zero(bn))
    goto err;
...
#define zerotest(fn)                                                           \
  if (!TEST_ptr(fn(input, 1, zbn)) || !TEST_true(BN_is_zero(zbn)) ||           \
      !TEST_ptr(fn(input, 0, zbn)) || !TEST_true(BN_is_zero(zbn)) ||           \
      !TEST_ptr(fn(NULL, 0, zbn)) || !TEST_true(BN_is_zero(zbn)))              \
  goto err
...
static int test_bin2bn_lengths(void) {
  ...
  if (!TEST_ptr(bn_be = BN_new()) || !TEST_ptr(bn_expected_be = BN_new()) ||
      ...
      !TEST_ptr(bn_le = BN_new()) || !TEST_ptr(bn_expected_le = BN_new()) ||
      !TEST_true(BN_set_word(bn_expected_le, 0x201)))
    goto err;

#define lengthtest(fn, e)                                                      \
  if (!TEST_ptr_null(fn(input, -1, bn_##e)) ||                                 \
      !TEST_ptr(fn(input, 0, bn_##e)) || !TEST_true(BN_is_zero(bn_##e)) ||     \
...
static int test_rand_range_single(size_t n) {
  ...
  if (!TEST_ptr(counts = OPENSSL_zalloc(sizeof(*counts) * range)) ||
      !TEST_ptr(rng = BN_new()) || !TEST_ptr(val = BN_new()) ||
...
static int test_negzero(void) {
  ...
  for (consttime = 0; consttime < 2; consttime++) {
    ...
    if (!TEST_true(BN_div(a, b, numerator, denominator, ctx)) ||
        !TEST_BN_eq_zero(a) || !TEST_BN_ge_zero(a))
      goto err;
...
static int test_badmod(void) {
  ...
  if (!TEST_ptr(a = BN_new()) || !TEST_ptr(b = BN_new()) ||
      !TEST_ptr(zero = BN_new()) || !TEST_ptr(mont = BN_MONT_CTX_new()))
    goto err;
  BN_zero(zero);
...
static int test_expmodzero(void) {
  ...
  if (!TEST_ptr(zero = BN_new()) || !TEST_ptr(a = BN_new()) ||
      !TEST_ptr(r = BN_new()))
    goto err;
  BN_zero(zero);
...
static int test_expmodone(void) {
  ...
  if (!TEST_ptr(r) || !TEST_ptr(a) || !TEST_ptr(p) || !TEST_ptr(p) ||
      !TEST_ptr(m) || !TEST_true(BN_set_word(a, 1)) ||
  ...
  for (i = 0; i < 2; i++) {
    if (!TEST_true(BN_mod_exp(r, a, p, m, NULL)) || !TEST_BN_eq_zero(r) ||
        ...
        !TEST_BN_eq_zero(r) || !TEST_true(BN_mod_exp_recp(r, a, p, m, NULL)) ||
        !TEST_BN_eq_zero(r))
      goto err;
...
static int test_smallprime(int kBits) {
  ...
  if (kBits <= 1) {
    ...
    if (!TEST_true(BN_generate_prime_ex(r, kBits, 0, NULL, NULL, NULL)) ||
...
static int test_smallsafeprime(int kBits) {
  ...
  if (kBits <= 5 && kBits != 3) {
    ...
    if (!TEST_true(BN_generate_prime_ex(r, kBits, 1, NULL, NULL, NULL)) ||
        !TEST_int_eq(BN_num_bits(r), kBits))
      goto err;
  }
...
static int test_is_prime(int i) {
  ...
  for (trial = 0; trial <= 1; ++trial) {
    if (!TEST_true(BN_set_word(r, primes[i])) ||
        !TEST_int_eq(BN_check_prime(r, ctx, NULL), 1))
      goto err;
  }
...
static int test_coprime(void) {
  ...
  ret = TEST_ptr(a = BN_new()) && TEST_ptr(b = BN_new()) &&
        TEST_true(BN_set_word(a, 66)) && TEST_true(BN_set_word(b, 99)) &&
...
static int test_gcd_prime(void) {
  ...
  for (i = 0; i < NUM_PRIME_TESTS; i++) {
    if (!TEST_true(BN_generate_prime_ex(b, 1024, 0, NULL, NULL, NULL)) ||
        !TEST_true(BN_gcd(gcd, a, b, ctx)) || !TEST_true(BN_is_one(gcd)) ||
...
static int test_mod_exp2_mont(void) {
  ...
  if (!TEST_ptr(exp_result = BN_new()) || !TEST_ptr(exp_a1 = BN_new()) ||
      !TEST_ptr(exp_p1 = BN_new()) || !TEST_ptr(exp_a2 = BN_new()) ||
      !TEST_ptr(exp_p2 = BN_new()) || !TEST_ptr(exp_m = BN_new()))
    goto err;

  if (!TEST_true(BN_one(exp_a1)) || !TEST_true(BN_one(exp_p1)) ||
      !TEST_true(BN_one(exp_a2)) || !TEST_true(BN_one(exp_p2)))
    goto err;

  BN_zero(exp_m);

  /* input of 0 is even, so must fail */
  if (!TEST_int_eq(BN_mod_exp2_mont(exp_result, exp_a1, exp_p1, exp_a2, exp_p2,
                                    exp_m, ctx, NULL),
                   0))
    goto err;
...
static int test_mod_exp_alias(int idx) {
  ...
  if (!TEST_int_eq(
      (idx == 0 ? BN_mod_exp_simple : BN_mod_exp_recp)(r, a, b, c, ctx), 1))
    goto err;
  if (!TEST_ptr_ne(str = BN_bn2dec(r), NULL))
    goto err;
  ...
  if (!TEST_int_eq(
      (idx == 0 ? BN_mod_exp_simple : BN_mod_exp_recp)(r, a, r, c, ctx), 1))
    goto err;
  if (!TEST_ptr_ne(str = BN_bn2dec(r), NULL))
    goto err;
  if (!TEST_str_eq(str, "36"))
...
static int file_test_run(STANZA *s) {
  ...
  {"Sum", file_sum},           {"LShift1", file_lshift1},
  {"LShift", file_lshift},     {"RShift", file_rshift},
  {"Square", file_square},     {"Product", file_product},
  {"Quotient", file_quotient}, {"ModMul", file_modmul},
  {"ModExp", file_modexp},     {"Exp", file_exp},
  {"ModSqrt", file_modsqrt},   {"GCD", file_gcd},
  };
...
void cleanup_tests(void) { BN_CTX_free(ctx); }
