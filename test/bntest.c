...
static int file_lshift1(STANZA *s)
{
    ...
    if (!TEST_ptr(a = getBN(s, "A")) || !TEST_ptr(lshift1 = getBN(s, "LShift1"))
...
#define BOTTOM_BIT_NOTOUCH 0
        ...
        if (!TEST_int_eq(BN_bn2binpad(n, out, sizeof(out)), sizeof(out))
            ...
            || !TEST_mem_eq(out, sizeof(out) - bytes, zeros,
                            sizeof(out) - bytes))
            goto err;
    }
...
{
    for (dst += len - 1; len > 0; src++, dst--, len--) *dst = *src;
    return 1;
}
...
static int test_badmod(void)
{
    ...
    if (!TEST_false(BN_mod_exp_mont(a, BN_value_one(), BN_value_one(), zero,
                                    ctx, NULL)))
        goto err;
    ERR_clear_error();
