...
static int file_sum(STANZA *s) {
    ...
    if (!TEST_true(BN_copy(ret, a)) || !TEST_true(BN_add(ret, ret, b)) ||
        ...
        !equalBN("A + B (r is b)", sum, ret) || !TEST_true(BN_copy(ret, sum)) ||
        ...
        !equalBN("Sum - B (r is a)", a, ret) || !TEST_true(BN_copy(ret, b)) ||
        !TEST_true(BN_sub(ret, sum, ret)) ||
...
static int file_lshift1(STANZA *s) {
    ...
    if (!TEST_ptr(a = getBN(s, "A")) ||
        ...
        !TEST_ptr(two = BN_new()) || !TEST_ptr(remainder = BN_new()))
        goto err;
...
static int file_lshift(STANZA *s) {
    ...
    if (!TEST_ptr(a = getBN(s, "A")) ||
        !TEST_ptr(lshift = getBN(s, "LShift")) || !TEST_ptr(ret = BN_new()) ||
        !getint(s, &n, "N"))
        goto err;
...
static int file_rshift(STANZA *s) {
    ...
    if (!TEST_ptr(a = getBN(s, "A")) ||
        !TEST_ptr(rshift = getBN(s, "RShift")) || !TEST_ptr(ret = BN_new()) ||
        !getint(s, &n, "N"))
        goto err;
...
static int file_square(STANZA *s) {
    ...
    if (!TEST_ptr(a = getBN(s, "A")) ||
        !TEST_ptr(square = getBN(s, "Square")) || !TEST_ptr(zero = BN_new()) ||
...
#ifdef BN_DEC_FMT1
            TEST_error("Got A %% B (word) = " BN_DEC_FMT1
                       ", wanted " BN_DEC_FMT1,
...
static int file_modexp(STANZA *s) {
    ...
    if (!TEST_ptr(a = getBN(s, "A")) || !TEST_ptr(e = getBN(s, "E")) ||
        !TEST_ptr(m = getBN(s, "M")) ||
        !TEST_ptr(mod_exp = getBN(s, "ModExp")) || !TEST_ptr(ret = BN_new()) ||
        !TEST_ptr(d = BN_new()))
        goto err;
...
static int test_dec2bn(void) {
    ...
    if (!TEST_int_eq(parsedecBN(&bn, "-42"), 3) ||
        !TEST_BN_abs_eq_word(bn, 42) || !TEST_BN_lt_zero(bn) ||
...
static int test_hex2bn(void) {
    ...
    if (!TEST_int_eq(parseBN(&bn, "-42"), 3) ||
        !TEST_BN_abs_eq_word(bn, 0x42) || !TEST_BN_lt_zero(bn) ||
        !TEST_BN_le_zero(bn) || !TEST_BN_ne_zero(bn) || !TEST_BN_even(bn))
        goto err;
    BN_free(bn);
    bn = NULL;
...
static int file_test_run(STANZA *s) {
    ...
    for (; --numtests >= 0; tp++) {
        if (findattr(s, tp->name) != NULL) {
            if (!tp->func(s)) {
                TEST_info("%s:%d: Failed %s test", s->test_file, s->start,
                          tp->name);
                return 0;
            }
            return 1;
