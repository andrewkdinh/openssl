...
static const unsigned char simpleder[] = {0xfc, 0x04, 0x00, 0x01, 0x02,
                                          0x03, 0xff, 0xfe, 0xfd};
...
static const unsigned char quic4[] = {0xc0, 0x00, 0x00, 0x00, 0x00, 0x00,
                                      0x00, 0x04, 0x80, 0x01, 0x3c, 0x6a};
/* QUIC sub-packet with 8-byte length prefix, containing a 8-byte vlint */
static const unsigned char quic5[] = {0xc0, 0x00, 0x00, 0x00, 0x00, 0x00,
                                      0x00, 0x08, 0xef, 0x77, 0x21, 0x3f,
                                      0x3f, 0x50, 0x5b, 0xa5};
...
static const unsigned char quic7[] = {0x07, 0x80, 0x00, 0x00, 0x08, 0x65, 0x14,
                                      0x40, 0x01, 0x05, 0x40, 0x01, 0x11, 0x40,
                                      0x01, 0x12, 0x40, 0x01, 0x13};
...
static int test_WPACKET_start_sub_packet(void)
{
   ...
   /* Single sub-packet with length prefix */
    ...
    if (!TEST_true(WPACKET_init(&pkt, buf))
        ...
        || !TEST_true(WPACKET_close(&pkt))
        || !TEST_true(WPACKET_get_length(&pkt, &len)) || !TEST_size_t_eq(len, 3)
        || !TEST_true(WPACKET_close(&pkt)) || !TEST_true(WPACKET_finish(&pkt))
        || !TEST_true(WPACKET_get_total_written(&pkt, &written))
...
static int test_WPACKET_quic(void)
{
    ...
    if (!TEST_true(WPACKET_init(&pkt, buf))
        ...
        || !TEST_true(WPACKET_quic_write_vlint(&pkt, 0x09))
        || !TEST_false(WPACKET_finish(&pkt)) || !TEST_true(WPACKET_close(&pkt))
        || !TEST_false(WPACKET_close(&pkt)) || !TEST_true(WPACKET_finish(&pkt))
        || !TEST_true(WPACKET_get_total_written(&pkt, &written))
    ...
    if (!TEST_true(WPACKET_init(&pkt, buf))
        ...
        || !TEST_true(WPACKET_get_length(&pkt, &len)) || !TEST_size_t_eq(len, 2)
        || !TEST_true(
            WPACKET_start_quic_sub_packet_bound(&pkt, OSSL_QUIC_VLINT_2B_MIN))
        ...
        || !TEST_true(WPACKET_close(&pkt))
        || !TEST_true(WPACKET_get_length(&pkt, &len)) || !TEST_size_t_eq(len, 8)
        || !TEST_true(WPACKET_close(&pkt))
        || !TEST_true(
            WPACKET_start_quic_sub_packet_bound(&pkt, OSSL_QUIC_VLINT_2B_MIN))
