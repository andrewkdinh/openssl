...
{
    if (!TEST_int_eq(
            ossl_quic_wire_encode_frame_max_stream_data(pkt, 0x1234, 0x9781),
            1))
        return 0;
...
static int encode_case_11_dec(PACKET *pkt, ossl_ssize_t fail)
{
    ...
    if (!TEST_int_eq(ossl_quic_wire_decode_frame_max_stream_data(
...
{
    if (!TEST_int_eq(ossl_quic_wire_encode_frame_stream_data_blocked(
                         pkt, 0x1234, 0x9781),
                     1))
        return 0;
...
static const OSSL_QUIC_FRAME_NEW_CONN_ID encode_case_16_f = {
    ...
    {{0xde, 0x06, 0xcb, 0x76, 0x5d, 0xb1, 0xa7, 0x71, 0x78, 0x09, 0xbb, 0xe8,
      0x50, 0x19, 0x12, 0x9a}}};
...
{
    if (!TEST_int_eq(
            ossl_quic_wire_encode_frame_new_conn_id(pkt, &encode_case_16b_f),
            1))
        return 0;
...
{
    if (!TEST_int_eq(ossl_quic_wire_encode_frame_path_challenge(
                         pkt, encode_case_18_data),
                     1))
        return 0;
...
static int encode_case_18_dec(PACKET *pkt, ossl_ssize_t fail)
{
    ...
    if (!TEST_int_eq(
            ossl_quic_wire_decode_frame_path_challenge(pkt, &challenge),
            fail < 0))
        return 0;
...
{
    if (!TEST_int_eq(
            ossl_quic_wire_encode_frame_path_response(pkt, encode_case_19_data),
            1))
        return 0;
...
static const struct encode_test_case encode_cases[] = {
    ENCODE_CASE(1) ENCODE_CASE(2) ENCODE_CASE(3) ENCODE_CASE(4) ENCODE_CASE(5)
        ENCODE_CASE(6) ENCODE_CASE(7) ENCODE_CASE(8) ENCODE_CASE(9)
            ENCODE_CASE(10) ENCODE_CASE(11) ENCODE_CASE(12) ENCODE_CASE(13)
                ENCODE_CASE(14) ENCODE_CASE(15) ENCODE_CASE(16) ENCODE_CASE(16b)
                    ENCODE_CASE(17) ENCODE_CASE(18) ENCODE_CASE(19)
                        ENCODE_CASE(20) ENCODE_CASE(21) ENCODE_CASE(22)
                            ENCODE_CASE(23)};
...
static int ack_generic_decode(PACKET *pkt)
{
    ...
    if (!TEST_int_eq(
            ossl_quic_wire_peek_frame_ack_num_ranges(pkt, &peek_total_ranges),
            1))
        return 0;
...
static int test_wire_ack(int idx)
{
    ...
    if (!TEST_int_eq(PACKET_buf_init(&pkt, (unsigned char *)c->input_buf,
                                     c->input_buf_len),
                     1))
        goto err;
...
static const size_t non_minimal_len[] = {
    OSSL_NELEM(non_minimal_1), OSSL_NELEM(non_minimal_2),
    OSSL_NELEM(non_minimal_3), OSSL_NELEM(non_minimal_4),
    OSSL_NELEM(non_minimal_5), OSSL_NELEM(non_minimal_6),
};
