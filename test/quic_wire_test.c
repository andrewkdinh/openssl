...
static const unsigned char encode_case_1_expect[] = {0, 0, 0};
...
static const unsigned char encode_case_2_expect[] = {0x01};

/* 3. ACK */
static const OSSL_QUIC_ACK_RANGE encode_case_3_ranges[] = {{20, 30}, {0, 10}};

static const OSSL_QUIC_FRAME_ACK encode_case_3_f = {
    (OSSL_QUIC_ACK_RANGE *)encode_case_3_ranges, OSSL_NELEM(encode_case_3_ranges), {OSSL_TIME_MS}, 60, 70, 80, 1};
...
static int encode_case_3_dec(PACKET *pkt, ossl_ssize_t fail)
{
    ...
    if (!TEST_uint64_t_le(f.num_ack_ranges * sizeof(OSSL_QUIC_ACK_RANGE), SIZE_MAX)
        || !TEST_uint64_t_le(encode_case_3_f.num_ack_ranges * sizeof(OSSL_QUIC_ACK_RANGE), SIZE_MAX))
        return 0;

    if (!TEST_mem_eq(f.ack_ranges, (size_t)f.num_ack_ranges * sizeof(OSSL_QUIC_ACK_RANGE), encode_case_3_f.ack_ranges,
                     (size_t)encode_case_3_f.num_ack_ranges * sizeof(OSSL_QUIC_ACK_RANGE)))
        return 0;

    if (!TEST_uint64_t_eq(ossl_time2ticks(f.delay_time), ossl_time2ticks(encode_case_3_f.delay_time)))
        return 0;
...
/* 4. RESET_STREAM */
static const OSSL_QUIC_FRAME_RESET_STREAM encode_case_4_f = {0x1234, 0x9781, 0x11717};

static int encode_case_4_enc(WPACKET *pkt)
{
    if (!TEST_int_eq(ossl_quic_wire_encode_frame_reset_stream(pkt, &encode_case_4_f), 1))
        return 0;
...
/* 5. STOP_SENDING */
static const OSSL_QUIC_FRAME_STOP_SENDING encode_case_5_f = {0x1234, 0x9781};

static int encode_case_5_enc(WPACKET *pkt)
{
    if (!TEST_int_eq(ossl_quic_wire_encode_frame_stop_sending(pkt, &encode_case_5_f), 1))
        return 0;
...
/* 6. CRYPTO */
static const unsigned char encode_case_6_data[] = {93, 18, 17, 102, 33};

static const OSSL_QUIC_FRAME_CRYPTO encode_case_6_f = {0x1234, sizeof(encode_case_6_data), encode_case_6_data};

static int encode_case_6_enc(WPACKET *pkt)
{
    if (!TEST_ptr(ossl_quic_wire_encode_frame_crypto(pkt, &encode_case_6_f)))
        return 0;
...
/* 7. NEW_TOKEN */
static const unsigned char encode_case_7_token[] = {0xde, 0x06, 0xcb, 0x76, 0x5d, 0xb1, 0xa7, 0x71,
                                                    0x78, 0x09, 0xbb, 0xe8, 0x50, 0x19, 0x12, 0x9a};

static int encode_case_7_enc(WPACKET *pkt)
{
    if (!TEST_int_eq(ossl_quic_wire_encode_frame_new_token(pkt, encode_case_7_token, sizeof(encode_case_7_token)), 1))
        return 0;
...
static int encode_case_7_dec(PACKET *pkt, ossl_ssize_t fail)
{
    ...
    if (!TEST_int_eq(ossl_quic_wire_decode_frame_new_token(pkt, &token, &token_len), fail < 0))
        return 0;

    if (fail >= 0)
        return 1;

    if (!TEST_mem_eq(token, token_len, encode_case_7_token, sizeof(encode_case_7_token)))
        return 0;

    return 1;
}

static const unsigned char encode_case_7_expect[] = {0x07,                   /* Type */
                                                     0x10,                   /* Length */
                                                     0xde, 0x06, 0xcb, 0x76, 0x5d, 0xb1, 0xa7, 0x71, /* Token */
                                                     0x78, 0x09, 0xbb, 0xe8, 0x50, 0x19, 0x12, 0x9a};

/* 8. STREAM (no length, no offset, no fin) */
static const unsigned char encode_case_8_data[] = {0xde, 0x06, 0xcb, 0x76, 0x5d};
static const OSSL_QUIC_FRAME_STREAM encode_case_8_f = {0x1234, 0, 5, encode_case_8_data, 0, 0};

static int encode_case_8_enc(WPACKET *pkt)
{
    if (!TEST_ptr(ossl_quic_wire_encode_frame_stream(pkt, &encode_case_8_f)))
        return 0;
...
static int encode_case_8_dec(PACKET *pkt, ossl_ssize_t fail)
{
    ...
    if (!TEST_mem_eq(f.data, (size_t)f.len, encode_case_8_data, sizeof(encode_case_8_data)))
        return 0;
...
static const unsigned char encode_case_9_data[] = {0xde, 0x06, 0xcb, 0x76, 0x5d};
static const OSSL_QUIC_FRAME_STREAM encode_case_9_f = {0x1234, 0x39, 5, encode_case_9_data, 1, 1};

static int encode_case_9_enc(WPACKET *pkt)
{
    if (!TEST_ptr(ossl_quic_wire_encode_frame_stream(pkt, &encode_case_9_f)))
        return 0;
...
static int encode_case_9_dec(PACKET *pkt, ossl_ssize_t fail)
{
    ...
    if (!TEST_mem_eq(f.data, (size_t)f.len, encode_case_9_data, sizeof(encode_case_9_data)))
        return 0;
...
{
    if (!TEST_int_eq(ossl_quic_wire_encode_frame_max_stream_data(pkt, 0x1234, 0x9781), 1))
        return 0;
...
static int encode_case_11_dec(PACKET *pkt, ossl_ssize_t fail)
{
    ...
    if (!TEST_int_eq(ossl_quic_wire_decode_frame_max_stream_data(pkt, &stream_id, &max_data), fail < 0))
        return 0;
...
{
    uint64_t max_streams_1 = 0, max_streams_2 = 0, frame_type_1 = 0, frame_type_2 = 0;
    int is_minimal = 1, success_if;

    success_if = (fail < 0 || fail >= 1);
    if (!TEST_int_eq(ossl_quic_wire_peek_frame_header(pkt, &frame_type_1, &is_minimal), success_if))
        return 0;
    ...
    if (!TEST_int_eq(ossl_quic_wire_decode_frame_max_streams(pkt, &max_streams_1), success_if))
        return 0;

    success_if = (fail < 0 || fail >= 4);
    if (!TEST_int_eq(ossl_quic_wire_peek_frame_header(pkt, &frame_type_2, &is_minimal), success_if))
        return 0;
    ...
    success_if = (fail < 0);
    if (!TEST_int_eq(ossl_quic_wire_decode_frame_max_streams(pkt, &max_streams_2), success_if))
        return 0;

    if ((fail < 0 || fail >= 3) && !TEST_uint64_t_eq(frame_type_1, OSSL_QUIC_FRAME_TYPE_MAX_STREAMS_BIDI))
        return 0;

    if ((fail < 0 || fail >= 3) && !TEST_uint64_t_eq(max_streams_1, 0x1234))
        return 0;

    if ((fail < 0 || fail >= 8) && !TEST_uint64_t_eq(frame_type_2, OSSL_QUIC_FRAME_TYPE_MAX_STREAMS_UNI))
        return 0;

    if ((fail < 0 || fail >= 8) && !TEST_uint64_t_eq(max_streams_2, 0x9781))
        return 0;
...
static int encode_case_13_dec(PACKET *pkt, ossl_ssize_t fail)
{
    ...
    if (!TEST_int_eq(ossl_quic_wire_decode_frame_data_blocked(pkt, &max_data), fail < 0))
        return 0;
...
{
    if (!TEST_int_eq(ossl_quic_wire_encode_frame_stream_data_blocked(pkt, 0x1234, 0x9781), 1))
        return 0;
...
static int encode_case_14_dec(PACKET *pkt, ossl_ssize_t fail)
{
    ...
    if (!TEST_int_eq(ossl_quic_wire_decode_frame_stream_data_blocked(pkt, &stream_id, &max_data), fail < 0))
        return 0;
...
static int encode_case_15_dec(PACKET *pkt, ossl_ssize_t fail)
{
    ...
    success_if = (fail < 0 || fail >= 3);
    if (!TEST_int_eq(ossl_quic_wire_decode_frame_streams_blocked(pkt, &max_streams_1), success_if))
        return 0;
    ...
    if (!TEST_int_eq(ossl_quic_wire_decode_frame_streams_blocked(pkt, &max_streams_2), fail < 0 || fail >= 8))
        return 0;

    if ((fail < 0 || fail >= 1) && !TEST_uint64_t_eq(frame_type_1, OSSL_QUIC_FRAME_TYPE_STREAMS_BLOCKED_BIDI))
        return 0;
...
static const unsigned char encode_case_16_conn_id[] = {0x33, 0x44, 0x55, 0x66};

static const OSSL_QUIC_FRAME_NEW_CONN_ID encode_case_16_f = {
    0x9781,
    0x1234,
    {0x4, {0x33, 0x44, 0x55, 0x66}},
    {{0xde, 0x06, 0xcb, 0x76, 0x5d, 0xb1, 0xa7, 0x71, 0x78, 0x09, 0xbb, 0xe8, 0x50, 0x19, 0x12, 0x9a}}};

static int encode_case_16_enc(WPACKET *pkt)
{
    if (!TEST_int_eq(ossl_quic_wire_encode_frame_new_conn_id(pkt, &encode_case_16_f), 1))
        return 0;
...
static int encode_case_16_dec(PACKET *pkt, ossl_ssize_t fail)
{
    ...
    if (!TEST_mem_eq(f.stateless_reset.token, sizeof(f.stateless_reset.token), encode_case_16_f.stateless_reset.token,
...
{
    if (!TEST_int_eq(ossl_quic_wire_encode_frame_new_conn_id(pkt, &encode_case_16b_f), 1))
        return 0;
...
/* 18. PATH_CHALLENGE */
static const uint64_t encode_case_18_data = (((uint64_t)0x5f4b12) << 40) | (uint64_t)0x731834UL;

static int encode_case_18_enc(WPACKET *pkt)
{
    if (!TEST_int_eq(ossl_quic_wire_encode_frame_path_challenge(pkt, encode_case_18_data), 1))
        return 0;
...
/* 19. PATH_RESPONSE */
static const uint64_t encode_case_19_data = (((uint64_t)0x5f4b12) << 40) | (uint64_t)0x731834UL;

static int encode_case_19_enc(WPACKET *pkt)
{
    if (!TEST_int_eq(ossl_quic_wire_encode_frame_path_response(pkt, encode_case_19_data), 1))
        return 0;
...
static const char encode_case_20_reason[] = {
    /* "reason for closure" */
    0x72, 0x65, 0x61, 0x73, 0x6f, 0x6e, 0x20, 0x66, 0x6f, 0x72, 0x20, 0x63, 0x6c, 0x6f, 0x73, 0x75, 0x72, 0x65};

static const OSSL_QUIC_FRAME_CONN_CLOSE encode_case_20_f = {0, 0x1234, 0x9781, (char *)encode_case_20_reason,
                                                            sizeof(encode_case_20_reason)};

static int encode_case_20_enc(WPACKET *pkt)
{
    if (!TEST_int_eq(ossl_quic_wire_encode_frame_conn_close(pkt, &encode_case_20_f), 1))
        return 0;
...
static int encode_case_20_dec(PACKET *pkt, ossl_ssize_t fail)
{
    ...
    if (!TEST_mem_eq(f.reason, f.reason_len, encode_case_20_f.reason, encode_case_20_f.reason_len))
        return 0;

    return 1;
}

static const unsigned char encode_case_20_expect[] = {0x1C,                           /* Type */
                                                      0x52, 0x34,                     /* Sequence Number */
...
static const unsigned char encode_case_21_expect[] = {0x1E};
...
static int encode_case_22_enc(WPACKET *pkt)
{
    ...
    if (!TEST_ptr(p = ossl_quic_wire_encode_transport_param_bytes(pkt, 0x9781, NULL, 2)))
        return 0;
...
static int encode_case_22_dec(PACKET *pkt, ossl_ssize_t fail)
{
    ...
    if (!TEST_int_eq(ossl_quic_wire_peek_transport_param(pkt, &id), fail < 0 || fail >= 2))
        return 0;

    if ((fail < 0 || fail >= 2) && !TEST_uint64_t_eq(id, 0x1234))
        return 0;
    ...
    if ((fail < 0 || fail >= 8) && !TEST_uint64_t_eq(id, 0x1234))
        return 0;

    if ((fail < 0 || fail >= 8) && !TEST_mem_eq(p, len, encode_case_22_data, sizeof(encode_case_22_data)))
        return 0;

    if ((fail < 0 || fail >= 8) && !TEST_int_eq(ossl_quic_wire_peek_transport_param(pkt, &id), fail < 0 || fail >= 12))
        return 0;

    if ((fail < 0 || fail >= 12) && !TEST_uint64_t_eq(id, 0x9781))
        return 0;
    ...
    if ((fail < 0 || fail >= 15) && !TEST_uint64_t_eq(id, 0x9781))
        return 0;

    if ((fail < 0 || fail >= 15) && !TEST_mem_eq(p, len, data, sizeof(data)))
        return 0;
...
static int encode_case_23_dec(PACKET *pkt, ossl_ssize_t fail)
{
    ...
    if (!TEST_int_eq(ossl_quic_wire_decode_transport_param_int(pkt, &id, &value), fail < 0 || fail >= 7))
        return 0;

    if ((fail < 0 || fail >= 7) && !TEST_uint64_t_eq(id, 0x1234))
        return 0;

    if ((fail < 0 || fail >= 7) && !TEST_uint64_t_eq(value, 0x9781))
        return 0;

    if (!TEST_int_eq(ossl_quic_wire_decode_transport_param_int(pkt, &id, &value), fail < 0 || fail >= 14))
        return 0;

    if ((fail < 0 || fail >= 14) && !TEST_uint64_t_eq(id, 0x2233))
        return 0;

    if ((fail < 0 || fail >= 14) && !TEST_uint64_t_eq(value, 0x4545))
        return 0;
...
static const unsigned char encode_case_23_expect[] = {
    0x52, 0x34, 0x04, 0x80, 0x00, 0x97, 0x81,

    0x62, 0x33, 0x04, 0x80, 0x00, 0x45, 0x45,
};
...
static const struct encode_test_case encode_cases[] = {
    ENCODE_CASE(1) ENCODE_CASE(2) ENCODE_CASE(3) ENCODE_CASE(4) ENCODE_CASE(5) ENCODE_CASE(6) ENCODE_CASE(7)
        ENCODE_CASE(8) ENCODE_CASE(9) ENCODE_CASE(10) ENCODE_CASE(11) ENCODE_CASE(12) ENCODE_CASE(13) ENCODE_CASE(14)
            ENCODE_CASE(15) ENCODE_CASE(16) ENCODE_CASE(16b) ENCODE_CASE(17) ENCODE_CASE(18) ENCODE_CASE(19)
                ENCODE_CASE(20) ENCODE_CASE(21) ENCODE_CASE(22) ENCODE_CASE(23)};
...
static int ack_generic_decode(PACKET *pkt)
{
    ...
    if (!TEST_int_eq(ossl_quic_wire_peek_frame_ack_num_ranges(pkt, &peek_total_ranges), 1))
        return 0;
...
static const struct ack_test_case ack_cases[] = {
    ACK_CASE(1, 1, ack_generic_decode) ACK_CASE(2, 0, ack_generic_decode) ACK_CASE(3, 1, ack_generic_decode)
        ACK_CASE(4, 0, ack_generic_decode) ACK_CASE(5, 1, ack_generic_decode) ACK_CASE(6, 1, ack_generic_decode)
...
static int test_wire_ack(int idx)
{
    ...
    if (!TEST_int_eq(PACKET_buf_init(&pkt, (unsigned char *)c->input_buf, c->input_buf_len), 1))
        goto err;
...
static int test_wire_pkt_hdr_pn(int tidx)
{
    ...
    if (!TEST_true(ossl_quic_wire_decode_pkt_hdr_pn(buf, pn_len, t->rx_largest_pn, &res_pn)))
        goto err;
...
/* RFC 9001 s. A.4 */
static const QUIC_CONN_ID retry_orig_dcid = {8, {0x83, 0x94, 0xc8, 0xf0, 0x3e, 0x51, 0x57, 0x08}};

static const unsigned char retry_encoded[] = {0xff, /* Long Header, Retry */
...
static int test_wire_retry_integrity_tag(void)
{
    ...
    if (!TEST_true(ossl_quic_calculate_retry_integrity_tag(NULL, NULL, &hdr, &retry_orig_dcid, got_tag)))
        goto err;

    if (!TEST_mem_eq(got_tag, sizeof(got_tag), retry_encoded + sizeof(retry_encoded) - QUIC_RETRY_INTEGRITY_TAG_LEN,
                     QUIC_RETRY_INTEGRITY_TAG_LEN))
        goto err;

    if (!TEST_true(ossl_quic_validate_retry_integrity_tag(NULL, NULL, &hdr, &retry_orig_dcid)))
        goto err;
...
static const unsigned char non_minimal_5[] = {0xC0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

static const unsigned char non_minimal_6[] = {0xC0, 0x00, 0x00, 0x00, 0x3F, 0xFF, 0xFF, 0xFF};

static const unsigned char *const non_minimal[] = {
    non_minimal_1, non_minimal_2, non_minimal_3, non_minimal_4, non_minimal_5, non_minimal_6,
};

static const size_t non_minimal_len[] = {
    OSSL_NELEM(non_minimal_1), OSSL_NELEM(non_minimal_2), OSSL_NELEM(non_minimal_3),
    OSSL_NELEM(non_minimal_4), OSSL_NELEM(non_minimal_5), OSSL_NELEM(non_minimal_6),
};
...
static int test_wire_minimal(int idx)
{
    ...
    if (!TEST_true(PACKET_buf_init(&pkt, non_minimal[idx], non_minimal_len[idx])))
        goto err;

    if (!TEST_true(ossl_quic_wire_peek_frame_header(&pkt, &frame_type, &is_minimal)))
        goto err;
