...
static int encode_case_3_dec(PACKET *pkt, ossl_ssize_t fail) {
  ...
  if (!TEST_uint64_t_le(f.num_ack_ranges * sizeof(OSSL_QUIC_ACK_RANGE),
                        SIZE_MAX) ||
      !TEST_uint64_t_le(
      encode_case_3_f.num_ack_ranges * sizeof(OSSL_QUIC_ACK_RANGE), SIZE_MAX))
    return 0;

  if (!TEST_mem_eq(
      f.ack_ranges, (size_t)f.num_ack_ranges * sizeof(OSSL_QUIC_ACK_RANGE),
...
/* 4. RESET_STREAM */
static const OSSL_QUIC_FRAME_RESET_STREAM encode_case_4_f = {0x1234, 0x9781,
                                                             0x11717};

static int encode_case_4_enc(WPACKET *pkt) {
  if (!TEST_int_eq(
      ossl_quic_wire_encode_frame_reset_stream(pkt, &encode_case_4_f), 1))
    return 0;
...
static int encode_case_5_enc(WPACKET *pkt) {
  if (!TEST_int_eq(
      ossl_quic_wire_encode_frame_stop_sending(pkt, &encode_case_5_f), 1))
    return 0;
...
static int encode_case_6_dec(PACKET *pkt, ossl_ssize_t fail) {
  ...
  if (!TEST_mem_eq(f.data, (size_t)f.len, encode_case_6_data,
...
static int encode_case_7_enc(WPACKET *pkt) {
  if (!TEST_int_eq(ossl_quic_wire_encode_frame_new_token(
                   pkt, encode_case_7_token, sizeof(encode_case_7_token)),
                   1))
    return 0;
...
static int encode_case_7_dec(PACKET *pkt, ossl_ssize_t fail) {
  ...
  if (!TEST_mem_eq(token, token_len, encode_case_7_token,
...
static const unsigned char encode_case_8_data[] = {0xde, 0x06, 0xcb, 0x76,
                                                   0x5d};
...
static const unsigned char encode_case_9_data[] = {0xde, 0x06, 0xcb, 0x76,
                                                   0x5d};
...
static int encode_case_9_dec(PACKET *pkt, ossl_ssize_t fail) {
  ...
  if (!TEST_mem_eq(f.data, (size_t)f.len, encode_case_9_data,
...
static int encode_case_11_enc(WPACKET *pkt) {
  if (!TEST_int_eq(
      ossl_quic_wire_encode_frame_max_stream_data(pkt, 0x1234, 0x9781), 1))
    return 0;
...
static int encode_case_12_dec(PACKET *pkt, ossl_ssize_t fail) {
  uint64_t max_streams_1 = 0, max_streams_2 = 0, frame_type_1 = 0,
           frame_type_2 = 0;
  ...
  if (!TEST_int_eq(ossl_quic_wire_decode_frame_max_streams(pkt, &max_streams_1),
                   success_if))
    return 0;
  ...
  success_if = (fail < 0);
  if (!TEST_int_eq(ossl_quic_wire_decode_frame_max_streams(pkt, &max_streams_2),
                   success_if))
    return 0;
...
static int encode_case_13_dec(PACKET *pkt, ossl_ssize_t fail) {
  ...
  if (!TEST_int_eq(ossl_quic_wire_decode_frame_data_blocked(pkt, &max_data),
                   fail < 0))
    return 0;
...
static int encode_case_14_enc(WPACKET *pkt) {
  if (!TEST_int_eq(
      ossl_quic_wire_encode_frame_stream_data_blocked(pkt, 0x1234, 0x9781), 1))
    return 0;
...
static int encode_case_14_dec(PACKET *pkt, ossl_ssize_t fail) {
  ...
  if (!TEST_int_eq(ossl_quic_wire_decode_frame_stream_data_blocked(
...
static int encode_case_15_enc(WPACKET *pkt) {
  if (!TEST_int_eq(ossl_quic_wire_encode_frame_streams_blocked(pkt, 0, 0x1234),
                   1))
    return 0;

  if (!TEST_int_eq(ossl_quic_wire_encode_frame_streams_blocked(pkt, 1, 0x9781),
                   1))
    return 0;
...
{{0xde, 0x06, 0xcb, 0x76, 0x5d, 0xb1, 0xa7, 0x71, 0x78, 0x09, 0xbb, 0xe8, 0x50,
  0x19, 0x12, 0x9a}}};

static int encode_case_16_enc(WPACKET *pkt) {
  if (!TEST_int_eq(
      ossl_quic_wire_encode_frame_new_conn_id(pkt, &encode_case_16_f), 1))
    return 0;
...
static int encode_case_16_dec(PACKET *pkt, ossl_ssize_t fail) {
  ...
  if (!TEST_mem_eq(f.conn_id.id, f.conn_id.id_len, encode_case_16_conn_id,
                   sizeof(encode_case_16_conn_id)))
    return 0;

  if (!TEST_mem_eq(f.stateless_reset.token, sizeof(f.stateless_reset.token),
...
static int encode_case_16b_enc(WPACKET *pkt) {
  if (!TEST_int_eq(
      ossl_quic_wire_encode_frame_new_conn_id(pkt, &encode_case_16b_f), 1))
    return 0;
...
static int encode_case_18_enc(WPACKET *pkt) {
  if (!TEST_int_eq(
      ossl_quic_wire_encode_frame_path_challenge(pkt, encode_case_18_data), 1))
    return 0;
...
static int encode_case_18_dec(PACKET *pkt, ossl_ssize_t fail) {
  ...
  if (!TEST_int_eq(ossl_quic_wire_decode_frame_path_challenge(pkt, &challenge),
                   fail < 0))
    return 0;
...
static int encode_case_19_enc(WPACKET *pkt) {
  if (!TEST_int_eq(
      ossl_quic_wire_encode_frame_path_response(pkt, encode_case_19_data), 1))
    return 0;
...
0, 0x1234, 0x9781, (char *)encode_case_20_reason,
sizeof(encode_case_20_reason)};

static int encode_case_20_enc(WPACKET *pkt) {
  if (!TEST_int_eq(
      ossl_quic_wire_encode_frame_conn_close(pkt, &encode_case_20_f), 1))
    return 0;
...
static int encode_case_20_dec(PACKET *pkt, ossl_ssize_t fail) {
  ...
  if (!TEST_mem_eq(f.reason, f.reason_len, encode_case_20_f.reason,
                   encode_case_20_f.reason_len))
    return 0;
...
static const unsigned char encode_case_22_data[] = {0x55, 0x77, 0x32, 0x46,
                                                    0x99};
...
static int encode_case_22_enc(WPACKET *pkt) {
  ...
  if (!TEST_ptr(ossl_quic_wire_encode_transport_param_bytes(
      pkt, 0x1234, encode_case_22_data, sizeof(encode_case_22_data))))
    return 0;

  if (!TEST_ptr(
      p = ossl_quic_wire_encode_transport_param_bytes(pkt, 0x9781, NULL, 2)))
    return 0;
...
static int encode_case_23_enc(WPACKET *pkt) {
  if (!TEST_int_eq(
      ossl_quic_wire_encode_transport_param_int(pkt, 0x1234, 0x9781), 1))
    return 0;

  if (!TEST_int_eq(
      ossl_quic_wire_encode_transport_param_int(pkt, 0x2233, 0x4545), 1))
    return 0;
...
static int encode_case_23_dec(PACKET *pkt, ossl_ssize_t fail) {
  ...
  if (!TEST_int_eq(ossl_quic_wire_decode_transport_param_int(pkt, &id, &value),
                   fail < 0 || fail >= 7))
    return 0;
...
#define ENCODE_CASE(n)                                                         \
  {encode_case_##n##_enc, encode_case_##n##_expect,                            \
   OSSL_NELEM(encode_case_##n##_expect), encode_case_##n##_dec},

static const struct encode_test_case encode_cases[] = {
ENCODE_CASE(1) ENCODE_CASE(2) ENCODE_CASE(3) ENCODE_CASE(4) ENCODE_CASE(5)
ENCODE_CASE(6) ENCODE_CASE(7) ENCODE_CASE(8) ENCODE_CASE(9) ENCODE_CASE(10)
ENCODE_CASE(11) ENCODE_CASE(12) ENCODE_CASE(13) ENCODE_CASE(14) ENCODE_CASE(15)
ENCODE_CASE(16) ENCODE_CASE(16b) ENCODE_CASE(17) ENCODE_CASE(18) ENCODE_CASE(19)
...
static int test_wire_encode(int idx) {
  ...
  for (i = 0; i < c->expect_buf_len; ++i) {
     ...
     */
    if (!TEST_int_eq(PACKET_buf_init(&pkt2, (unsigned char *)c->expect_buf, i),
                     1))
      goto err;
...
static int ack_generic_decode(PACKET *pkt) {
  ...
  if (!TEST_int_eq(
      ossl_quic_wire_peek_frame_ack_num_ranges(pkt, &peek_total_ranges), 1))
    return 0;
...
#define ACK_CASE(n, expect_fail, dec)                                          \
  {ack_case_##n##_input, sizeof(ack_case_##n##_input), (dec), (expect_fail)},

static const struct ack_test_case ack_cases[] = {
ACK_CASE(1, 1, ack_generic_decode) ACK_CASE(2, 0, ack_generic_decode)
ACK_CASE(3, 1, ack_generic_decode) ACK_CASE(4, 0, ack_generic_decode)
ACK_CASE(5, 1, ack_generic_decode) ACK_CASE(6, 1, ack_generic_decode)
...
static int test_wire_ack(int idx) {
  ...
  if (!TEST_int_eq(
      PACKET_buf_init(&pkt, (unsigned char *)c->input_buf, c->input_buf_len),
      1))
    goto err;
...
static int test_wire_retry_integrity_tag(void) {
  ...
  if (!TEST_true(
      ossl_quic_wire_decode_pkt_hdr(&pkt, 0, 0, 0, &hdr, NULL, NULL)))
    goto err;

  if (!TEST_int_eq(hdr.type, QUIC_PKT_TYPE_RETRY))
    goto err;

  if (!TEST_true(ossl_quic_calculate_retry_integrity_tag(
      NULL, NULL, &hdr, &retry_orig_dcid, got_tag)))
    goto err;
...
static const unsigned char non_minimal_5[] = {0xC0, 0x00, 0x00, 0x00,
                                              0x00, 0x00, 0x00, 0x00};

static const unsigned char non_minimal_6[] = {0xC0, 0x00, 0x00, 0x00,
                                              0x3F, 0xFF, 0xFF, 0xFF};

static const unsigned char *const non_minimal[] = {
non_minimal_1, non_minimal_2, non_minimal_3,
non_minimal_4, non_minimal_5, non_minimal_6,
};
...
static int test_wire_minimal(int idx) {
  ...
  if (!TEST_true(
      ossl_quic_wire_peek_frame_header(&pkt, &frame_type, &is_minimal)))
    goto err;
