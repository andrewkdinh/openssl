...
static int encode_case_4_dec(PACKET *pkt, ossl_ssize_t fail) {
    ...
    if (!TEST_int_eq(ossl_quic_wire_decode_frame_reset_stream(pkt, &f),
                     fail < 0))
        return 0;
...
static int encode_case_5_dec(PACKET *pkt, ossl_ssize_t fail) {
    ...
    if (!TEST_int_eq(ossl_quic_wire_decode_frame_stop_sending(pkt, &f),
                     fail < 0))
        return 0;
...
static int encode_case_7_dec(PACKET *pkt, ossl_ssize_t fail) {
    ...
    if (!TEST_int_eq(
        ossl_quic_wire_decode_frame_new_token(pkt, &token, &token_len),
        fail < 0))
        return 0;
...
static int encode_case_12_dec(PACKET *pkt, ossl_ssize_t fail) {
    ...
    if (!TEST_int_eq(
        ossl_quic_wire_decode_frame_max_streams(pkt, &max_streams_1),
        success_if))
        return 0;
...
static int encode_case_14_enc(WPACKET *pkt) {
    if (!TEST_int_eq(
        ossl_quic_wire_encode_frame_stream_data_blocked(pkt, 0x1234, 0x9781),
        1))
        return 0;
...
static int encode_case_15_enc(WPACKET *pkt) {
    if (!TEST_int_eq(
        ossl_quic_wire_encode_frame_streams_blocked(pkt, 0, 0x1234), 1))
        return 0;

    if (!TEST_int_eq(
        ossl_quic_wire_encode_frame_streams_blocked(pkt, 1, 0x9781), 1))
        return 0;
...
static int encode_case_18_enc(WPACKET *pkt) {
    if (!TEST_int_eq(
        ossl_quic_wire_encode_frame_path_challenge(pkt, encode_case_18_data),
        1))
        return 0;
...
static int encode_case_18_dec(PACKET *pkt, ossl_ssize_t fail) {
    ...
    if (!TEST_int_eq(
        ossl_quic_wire_decode_frame_path_challenge(pkt, &challenge), fail < 0))
        return 0;
...
static int test_wire_encode(int idx) {
    ...
    for (i = 0; i < c->expect_buf_len; ++i) {
         ...
         */
        if (!TEST_int_eq(
            PACKET_buf_init(&pkt2, (unsigned char *)c->expect_buf, i), 1))
            goto err;
...
static int test_wire_minimal(int idx) {
    ...
    if (!TEST_true(
        PACKET_buf_init(&pkt, non_minimal[idx], non_minimal_len[idx])))
        goto err;
