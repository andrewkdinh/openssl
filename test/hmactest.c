...
# endif
    ...
    p = pt(HMAC(EVP_md5(), test[idx].key, test[idx].key_len, test[idx].data, test[idx].data_len, NULL, NULL),
           MD5_DIGEST_LENGTH);
...
static int test_hmac_bad(void)
{
    ...
    ctx = HMAC_CTX_new();
    if (!TEST_ptr(ctx) || !TEST_ptr_null(HMAC_CTX_get_md(ctx)) || !TEST_false(HMAC_Init_ex(ctx, NULL, 0, NULL, NULL))
...
static int test_hmac_run(void)
{
    ...
    if (!TEST_true(HMAC_Init_ex(ctx, test[4].key, test[4].key_len, EVP_sha1(), NULL))
        || !TEST_true(HMAC_Update(ctx, test[4].data, test[4].data_len)) || !TEST_true(HMAC_Final(ctx, buf, &len)))
        goto err;
    ...
    if (!TEST_true(HMAC_Init_ex(ctx, test[5].key, test[5].key_len, EVP_sha256(), NULL))
        ...
        || !TEST_true(HMAC_Update(ctx, test[5].data, test[5].data_len)) || !TEST_true(HMAC_Final(ctx, buf, &len)))
        goto err;
    ...
    if (!TEST_true(HMAC_Init_ex(ctx, test[6].key, test[6].key_len, NULL, NULL))
        || !TEST_true(HMAC_Update(ctx, test[6].data, test[6].data_len)) || !TEST_true(HMAC_Final(ctx, buf, &len)))
        goto err;
    p = pt(buf, len);
...
static int test_hmac_single_shot(void)
{
    ...
    p = pt(HMAC(EVP_sha1(), NULL, 0, test[4].data, test[4].data_len, NULL, NULL), SHA_DIGEST_LENGTH);
    if (!TEST_ptr(p) || !TEST_str_eq(p, test[4].digest))
        return 0;
...
static int test_hmac_final_update_fail(void)
{
    ...
    if (!TEST_true(HMAC_Init_ex(ctx, test[5].key, test[5].key_len, EVP_sha256(), NULL))
        || !TEST_true(HMAC_Update(ctx, test[5].data, test[5].data_len)) || !TEST_true(HMAC_Final(ctx, buf, &len))
        || !TEST_false(HMAC_Update(ctx, test[5].data, test[5].data_len)) || !TEST_false(HMAC_Final(ctx, buf, &len)))
        goto err;
...
static int test_hmac_copy(void)
{
    ...
    if (!TEST_true(HMAC_Init_ex(ctx, test[7].key, test[7].key_len, EVP_sha1(), NULL))
        || !TEST_true(HMAC_Update(ctx, test[7].data, test[7].data_len)) || !TEST_true(HMAC_CTX_copy(ctx2, ctx))
...
static int test_hmac_copy_uninited(void)
{
    ...
    if (!TEST_ptr(ctx = EVP_MD_CTX_new())
        ...
        || !TEST_true(EVP_DigestSignInit(ctx, NULL, EVP_sha1(), NULL, pkey)) || !TEST_ptr(ctx_tmp = EVP_MD_CTX_new())
...
#ifdef CHARSET_EBCDIC
    ebcdic2ascii(test_chunks[idx].key, test_chunks[idx].key, test_chunks[idx].key_len);
#endif

    if (!TEST_true(HMAC_Init_ex(ctx, test_chunks[idx].key, test_chunks[idx].key_len, md, NULL)))
        goto err;
...
int setup_tests(void)
{
    ...
    ADD_ALL_TESTS(test_hmac_chunks, sizeof(test_chunks) / sizeof(struct test_chunks_st));
    return 1;
}
