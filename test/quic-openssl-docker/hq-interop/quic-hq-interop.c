 ...
 * responses to disk. It demonstrates OpenSSL's QUIC API, including ALPN
 * protocol settings, peer address management, and SSL handshake and shutdown
 * processes.
 *
...
/**
 * @brief Static pointer to a BIO_ADDR structure representing the peer's
 * address.
 *
 * This variable is used to store the address of a peer for network
 * communication. It is statically allocated and should be initialized
 * appropriately.
 */
    ...
    while (this_poll_count != 0 && poll_done < this_poll_count) {
        ...
        for (poll_idx = 0; poll_idx < this_poll_count; poll_idx++) {
            ...
            if (poll_list[poll_idx].revents == SSL_POLL_EVENT_R) {
                ...
                /*
                 * Get up to sizeof(buf) bytes of the response. We keep reading
                ...
                if (!SSL_read_ex(poll_list[poll_idx].desc.value.ssl, buf,
                    ...
                    switch (
                    handle_io_failure(poll_list[poll_idx].desc.value.ssl, 0)) {
                    case 1:
                        eof = 0;
