...
{"secp384r1:secp521r1:X25519:prime256v1:X448", /* test 0 */
 "X25519:secp521r1:secp384r1:prime256v1:X448", CLIENT_PREFERENCE, "secp384r1",
 SH},
{"secp521r1:secp384r1:X25519:prime256v1:X448", /* test 1 */
 "X25519:secp521r1:secp384r1:prime256v1:X448", SERVER_PREFERENCE, "secp521r1",
 SH},
...
{"secp521r1:secp384r1:*X25519:prime256v1:*X448", /* test 6 */
 "secp384r1:secp521r1:prime256v1/X25519:X448", CLIENT_PREFERENCE, "secp521r1",
 HRR},
{"secp521r1:secp384r1:*X25519:prime256v1:*X448", /* test 7 */
 "secp384r1:secp521r1:prime256v1/X25519:X448", SERVER_PREFERENCE, "secp384r1",
 HRR},
...
{"X25519:secp256r1:X448:secp521r1:-X448:-secp256r1:-X25519:-secp521r1", "",
 CLIENT_PREFERENCE, NEGOTIATION_FAILURE},
...
static void server_response_check_cb(int write_p, int version, int content_type,
  ...
  if (write_p == 0 && /* Incoming data... */
    ...
    /* Check what it is: SH or HRR (compare the 'random' data field with HRR
     * magic number) */
...
const struct tls13groupselection_test_st *current_test_vector, int ssl_or_ctx) {
  int ok = 0;
  ...
  NULL, TLS_server_method(), TLS_client_method(), TLS1_VERSION, 0, &server_ctx,
  &client_ctx, cert, privkey));

  /* Customization of the contexts */
  if (ssl_or_ctx == WORK_ON_CONTEXT)
    TEST_false_or_end(
    SSL_CTX_set1_groups_list(client_ctx, current_test_vector->client_groups));
  ...
  if (ssl_or_ctx == WORK_ON_SSL_OBJECT)
    TEST_false_or_end(
    SSL_set1_groups_list(clientssl, current_test_vector->client_groups));
...
TEST_TYPE test_type) {
  ...
  if (ssl_or_ctx == WORK_ON_CONTEXT) {
    ...
    if (current_test_vector->server_groups != NULL) {
      TEST_true_or_end(
      SSL_CTX_set1_groups_list(server_ctx, current_test_vector->server_groups));
    }
  ...
  if (ssl_or_ctx == WORK_ON_SSL_OBJECT) {
    ...
    if (current_test_vector->server_groups != NULL)
      TEST_true_or_end(
      SSL_set1_groups_list(serverssl, current_test_vector->server_groups));
  ...
  SSL_set_msg_callback(clientssl,
...
static int tls13groupselection_test(int i) {
  ...
  if (!res)
    TEST_error("====> [ERROR] TLSv1.3 test %d with WORK_ON_SSL_OBJECT failed",
               i);
  testresult *= res;
  ...
  if (test_type == TEST_SYNTAX_FAILURE)
    ...
    res = test_groupnegotiation(&tls13groupselection_tests[i], WORK_ON_CONTEXT,
                                test_type);
