...
TEST_TYPE test_type) {
    ...
    if (ssl_or_ctx == WORK_ON_CONTEXT) {
        ...
        if (current_test_vector->server_groups != NULL) {
            TEST_true_or_end(SSL_CTX_set1_groups_list(
            server_ctx, current_test_vector->server_groups));
        }
        TEST_true_or_end(
...
static int tls13groupselection_test(int i) {
    ...
    if (test_type == TEST_SYNTAX_FAILURE)
        res =
        test_invalidsyntax(&tls13groupselection_tests[i], WORK_ON_SSL_OBJECT);
    else
    ...
    if (!res)
        TEST_error(
        "====> [ERROR] TLSv1.3 test %d with WORK_ON_SSL_OBJECT failed", i);
    testresult *= res;

    if (test_type == TEST_SYNTAX_FAILURE)
        res =
        test_invalidsyntax(&tls13groupselection_tests[i], WORK_ON_CONTEXT);
    else
        res = test_groupnegotiation(&tls13groupselection_tests[i],
                                    WORK_ON_CONTEXT, test_type);

    if (!res)
        TEST_error("====> [ERROR] TLSv1.3 test %d with WORK_ON_CONTEXT failed",
                   i);
    testresult *= res;
