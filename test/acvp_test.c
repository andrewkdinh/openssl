...
static int ecdsa_create_pkey(EVP_PKEY **pkey, const char *curve_name,
    ...
    if (!TEST_ptr(bld = OSSL_PARAM_BLD_new()) ||
        ...
        !TEST_true(OSSL_PARAM_BLD_push_octet_string(
                   bld, OSSL_PKEY_PARAM_PUB_KEY, pub, pub_len) > 0) ||
...
static int eddsa_keygen_test(int id) {
    ...
    if (!TEST_ptr(pkey = EVP_PKEY_Q_keygen(libctx, NULL, tst->curve_name)) ||
        !TEST_int_ge(self_test_args.called, 3) ||
        !TEST_true(pkey_get_octet_bytes(pkey, OSSL_PKEY_PARAM_PRIV_KEY, &priv,
                                        &priv_len)) ||
        !TEST_true(
...
static int dsa_keygen_test(int id) {
    ...
    for (i = 0; i < 2; ++i) {
        if (!TEST_int_gt(EVP_PKEY_keygen(keygen_ctx, &key), 0) ||
            !TEST_true(pkey_get_bn_bytes(key, OSSL_PKEY_PARAM_PRIV_KEY, &priv,
                                         &priv_len)) ||
            !TEST_true(
...
static int dsa_pqver_test(int id) {
    ...
    if (!TEST_ptr(bn_ctx = BN_CTX_new_ex(libctx)) ||
        ...
        &param_key, tst->p, tst->p_len, tst->q, tst->q_len, NULL, 0, tst->seed,
...
static int dsa_sigver_test(int id) {
    ...
    if (!TEST_ptr(bn_ctx = BN_CTX_new()) ||
        !TEST_true(dsa_create_pkey(&pkey, tst->p, tst->p_len, tst->q,
                                   tst->q_len, tst->g, tst->g_len, NULL, 0, 0,
                                   0, 0, tst->pub, tst->pub_len, bn_ctx)))
        goto err;
...
static int rsa_sigver_test(int id) {
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    if (!TEST_ptr(bn_ctx = BN_CTX_new()) ||
        ...
        !TEST_int_eq(EVP_DigestVerify(md_ctx, tst->sig, tst->sig_len, tst->msg,
                                      tst->msg_len),
                     tst->pass) ||
        !check_verify_message(pkey_ctx, 1) ||
        !TEST_true(EVP_PKEY_verify_init(pkey_ctx)) ||
...
static int rsa_decryption_primitive_test(int id) {
    ...
    if (!TEST_ptr(pkey =
                  EVP_PKEY_Q_keygen(libctx, NULL, "RSA", (size_t)2048)) ||
        !TEST_true(
        pkey_get_bn_bytes(pkey, OSSL_PKEY_PARAM_RSA_N, &n, &n_len)) ||
        !TEST_true(
        pkey_get_bn_bytes(pkey, OSSL_PKEY_PARAM_RSA_E, &e, &e_len)) ||
...
static int drbg_test(int id) {
    ...
    if (!TEST_true(EVP_RAND_instantiate(ctx, 0, 0, (void *)"", 0, NULL)) ||
        !TEST_true(EVP_RAND_generate(ctx, returned_bits, returned_bits_len, 0,
                                     0, NULL, 0)) ||
        !TEST_true(EVP_RAND_generate(ctx, returned_bits, returned_bits_len, 0,
                                     0, NULL, 0)))
        goto err;
