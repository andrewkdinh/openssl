...
static int sig_gen(EVP_PKEY *pkey, OSSL_PARAM *params, const char *digest_name,
  ...
  if (!TEST_ptr(sig = OPENSSL_malloc(sz)) ||
      ...
      !TEST_int_eq(
      EVP_DigestSignInit_ex(md_ctx, NULL, digest_name, libctx, NULL, pkey, p),
      1) ||
...
static int ecdsa_keygen_test(int id) {
  ...
  if (!TEST_ptr(pkey =
      ...
      !TEST_true(
      pkey_get_bn_bytes(pkey, OSSL_PKEY_PARAM_EC_PUB_Y, &puby, &puby_len)))
    goto err;
...
static int ecdsa_create_pkey(EVP_PKEY **pkey, const char *curve_name,
  ...
  if (!TEST_ptr(bld = OSSL_PARAM_BLD_new()) ||
      ...
      !TEST_true(OSSL_PARAM_BLD_push_octet_string(bld, OSSL_PKEY_PARAM_PUB_KEY,
                                                  pub, pub_len) > 0) ||
      !TEST_ptr(params = OSSL_PARAM_BLD_to_param(bld)) ||
      !TEST_ptr(ctx = EVP_PKEY_CTX_new_from_name(libctx, "EC", NULL)) ||
      !TEST_int_eq(EVP_PKEY_fromdata_init(ctx), 1) ||
      !TEST_int_eq(EVP_PKEY_fromdata(ctx, pkey, EVP_PKEY_PUBLIC_KEY, params),
                   expected))
    goto err;
...
static int ecdsa_sigver_test(int id) {
  ...
  if (!TEST_int_gt((sig_len = i2d_ECDSA_SIG(sign, &sig)), 0) ||
      !TEST_ptr(md_ctx = EVP_MD_CTX_new()) ||
      !TEST_true(EVP_DigestVerifyInit_ex(md_ctx, NULL, tst->digest_alg, libctx,
                                         NULL, pkey, NULL)) ||
...
static int eddsa_pub_verify_test(int id) {
  ...
  if (!TEST_true(
      eddsa_create_pkey(&pkey, tst->curve_name, tst->pub, tst->pub_len, 1)))
    goto err;
...
static int eddsa_keygen_test(int id) {
  ...
  if (!TEST_ptr(pkey = EVP_PKEY_Q_keygen(libctx, NULL, tst->curve_name)) ||
      !TEST_int_ge(self_test_args.called, 3) ||
      !TEST_true(
      pkey_get_octet_bytes(pkey, OSSL_PKEY_PARAM_PRIV_KEY, &priv, &priv_len)) ||
      !TEST_true(
      pkey_get_octet_bytes(pkey, OSSL_PKEY_PARAM_PUB_KEY, &pub, &pub_len)))
    goto err;
...
static int dsa_keygen_test(int id) {
  ...
  for (i = 0; i < 2; ++i) {
    if (!TEST_int_gt(EVP_PKEY_keygen(keygen_ctx, &key), 0) ||
        !TEST_true(
        pkey_get_bn_bytes(key, OSSL_PKEY_PARAM_PRIV_KEY, &priv, &priv_len)) ||
        !TEST_true(
        pkey_get_bn_bytes(key, OSSL_PKEY_PARAM_PUB_KEY, &pub, &pub_len)))
      goto err;
...
static int dsa_create_pkey(EVP_PKEY **pkey, const unsigned char *p,
  ...
  if (!TEST_ptr(bld = OSSL_PARAM_BLD_new()) ||
      ...
      !TEST_true(OSSL_PARAM_BLD_push_int(bld, OSSL_PKEY_PARAM_FFC_VALIDATE_PQ,
                                         validate_pq)) ||
      !TEST_true(OSSL_PARAM_BLD_push_int(bld, OSSL_PKEY_PARAM_FFC_VALIDATE_G,
                                         validate_g)) ||
...
static int dsa_pqver_test(int id) {
  ...
  if (!TEST_ptr(bn_ctx = BN_CTX_new_ex(libctx)) ||
      !TEST_true(dsa_create_pkey(&param_key, tst->p, tst->p_len, tst->q,
                                 tst->q_len, NULL, 0, tst->seed, tst->seed_len,
                                 tst->counter, 1, 0, NULL, 0, bn_ctx)) ||
      !TEST_ptr(key_ctx =
                EVP_PKEY_CTX_new_from_pkey(libctx, param_key, NULL)) ||
...
static int dsa_sigver_test(int id) {
  ...
  if (!TEST_ptr(bn_ctx = BN_CTX_new()) ||
      !TEST_true(dsa_create_pkey(&pkey, tst->p, tst->p_len, tst->q, tst->q_len,
                                 tst->g, tst->g_len, NULL, 0, 0, 0, 0, tst->pub,
                                 tst->pub_len, bn_ctx)))
    goto err;
...
static int aes_ccm_enc_dec(const char *alg, const unsigned char *pt,
  ...
  if (enc) {
    out_len += len;
    if (!TEST_int_gt(
        EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, tag_len, out + out_len),
        0) ||
...
static int aes_ccm_enc_dec_test(int id) {
  ...
  return aes_ccm_enc_dec(tst->alg, tst->pt, tst->pt_len, tst->key, tst->key_len,
                         tst->iv, tst->iv_len, tst->aad, tst->aad_len, tst->ct,
                         ct_len, tag, tag_len, enc, pass) &&
         aes_ccm_enc_dec(tst->alg, tst->ct, ct_len, tst->key, tst->key_len,
                         tst->iv, tst->iv_len, tst->aad, tst->aad_len, tst->pt,
                         tst->pt_len, tag, tag_len, !enc, pass)
         /* test that it fails if the tag is incorrect */
         &&
         aes_ccm_enc_dec(tst->alg, tst->ct, ct_len, tst->key, tst->key_len,
                         tst->iv, tst->iv_len, tst->aad, tst->aad_len, tst->pt,
                         tst->pt_len, tag - 1, tag_len, !enc, !pass);
}
...
static int aes_gcm_enc_dec(const char *alg, const unsigned char *pt,
  ...
  if (enc) {
    if ((ct != NULL && !TEST_mem_eq(out, olen, ct, ct_len)) ||
        !TEST_int_gt(
        EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, tag_len, out + olen),
        0) ||
...

  {
    ...
    if (outiv != NULL)
      *p =
      OSSL_PARAM_construct_octet_string(OSSL_CIPHER_PARAM_IV, outiv, iv_len);
...
static int aes_gcm_enc_dec_test(int id) {
  ...
  return aes_gcm_enc_dec(tst->alg, tst->pt, tst->pt_len, tst->key, tst->key_len,
                         tst->iv, tst->iv_len, tst->aad, tst->aad_len, tst->ct,
                         tst->ct_len, tst->tag, tst->tag_len, enc, pass, out,
                         NULL, NULL) &&
         aes_gcm_enc_dec(tst->alg, tst->ct, tst->ct_len, tst->key, tst->key_len,
                         tst->iv, tst->iv_len, tst->aad, tst->aad_len, tst->pt,
                         tst->pt_len, tst->tag, tst->tag_len, !enc, pass, out,
                         NULL, NULL)
...
static int aes_gcm_gen_iv_internal_test(void) {
  ...
  return aes_gcm_enc_dec(tst->alg, tst->pt, tst->pt_len, tst->key, tst->key_len,
                         NULL, tst->iv_len, tst->aad, tst->aad_len, NULL,
                         tst->ct_len, NULL, tst->tag_len, enc, pass, out,
                         &out_len, iv) &&
...
static int dh_safe_prime_keyver_test(int id) {
  ...
  if (!TEST_ptr(bn_ctx = BN_CTX_new_ex(libctx)) ||
      !TEST_true(dh_create_pkey(&pkey, tst->group_name, tst->pub, tst->pub_len,
...
static int rsa_siggen_test(int id) {
  ...
  *p++ = OSSL_PARAM_construct_utf8_string(OSSL_SIGNATURE_PARAM_DIGEST,
  ...
  if (salt_len >= 0)
    *p++ =
    OSSL_PARAM_construct_int(OSSL_SIGNATURE_PARAM_PSS_SALTLEN, &salt_len);
  *p++ = OSSL_PARAM_construct_end();

  if (!TEST_ptr(pkey = EVP_PKEY_Q_keygen(libctx, NULL, "RSA", tst->mod)) ||
      !TEST_true(pkey_get_bn_bytes(pkey, OSSL_PKEY_PARAM_RSA_N, &n, &n_len)) ||
      !TEST_true(pkey_get_bn_bytes(pkey, OSSL_PKEY_PARAM_RSA_E, &e, &e_len)) ||
      !TEST_true(sig_gen(pkey, params, tst->digest_alg, tst->msg, tst->msg_len,
                         &sig, &sig_len)))
    goto err;
  test_output_memory("n", n, n_len);
...
static int rsa_sigver_test(int id) {
  ...
  *p = OSSL_PARAM_construct_end();
  ...
  if (!TEST_ptr(bn_ctx = BN_CTX_new()) ||
      !TEST_true(rsa_create_pkey(&pkey, tst->n, tst->n_len, tst->e, tst->e_len,
                                 NULL, 0, bn_ctx)) ||
      !TEST_ptr(md_ctx = EVP_MD_CTX_new()) ||
      ...
      !TEST_int_eq(
      EVP_DigestVerify(md_ctx, tst->sig, tst->sig_len, tst->msg, tst->msg_len),
      tst->pass) ||
      !check_verify_message(pkey_ctx, 1) ||
      !TEST_true(EVP_PKEY_verify_init(pkey_ctx)) ||
      !check_verify_message(pkey_ctx, 0))
...
static int drbg_test(int id) {
  ...
  if (!TEST_true(EVP_RAND_instantiate(ctx, 0, 0, (void *)"", 0, NULL)) ||
      !TEST_true(EVP_RAND_generate(ctx, returned_bits, returned_bits_len, 0, 0,
                                   NULL, 0)) ||
      !TEST_true(
      EVP_RAND_generate(ctx, returned_bits, returned_bits_len, 0, 0, NULL, 0)))
    goto err;
...
static int aes_cfb1_bits_test(void) {
  ...
  if (!TEST_ptr(params = EVP_CIPHER_CTX_settable_params(ctx)) ||
      !TEST_ptr(p =
                OSSL_PARAM_locate_const(params, OSSL_CIPHER_PARAM_USE_BITS)))
    goto err;
