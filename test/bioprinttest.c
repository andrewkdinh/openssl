...
static pw pw_params[] = {{4, ""}, {5, ""}, {4, "12"}, {5, "12"}, {0, ""}, {-1, ""}, {4, "08"}};
...
static int dofptest(int test, int sub, double val, const char *width, int prec)
{
    ...
    for (i = 0; i < (int)OSSL_NELEM(fspecs); i++) {
        ...
        if (justprint) {
            ...
            TEST_info("test %d format=|%s| exp=|%s|, ret=|%s|", test, format, fpexpected[test][sub][i], result);
            ret = 0;
        }
    }
    if (justprint)
...
static int test_fp(int i)
{
    ...
    if (justprint)
        printf("    {\n");
    r = TEST_true(dofptest(i, t++, 0.0, pwp->w, pwp->p)) && TEST_true(dofptest(i, t++, 0.67, pwp->w, pwp->p))
        && TEST_true(dofptest(i, t++, frac, pwp->w, pwp->p)) && TEST_true(dofptest(i, t++, frac / 1000, pwp->w, pwp->p))
...
const OPTIONS *test_get_options(void)
{
    static const OPTIONS options[] = {
        OPT_TEST_OPTIONS_DEFAULT_USAGE, {"expected", OPT_PRINT, '-', "Output values"}, {NULL}};
    return options;
}
