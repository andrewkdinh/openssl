...
static int test_skey_cipher(void) {
  ...
  if (!TEST_ptr(ctx = EVP_CIPHER_CTX_new()) ||
      !TEST_int_gt(EVP_CipherInit_SKEY(ctx, fake_cipher, key, NULL, 0, 1, NULL),
                   0))
    goto end;
...
static int test_skey_skeymgmt(void) {
  ...
  if (!TEST_ptr(imp_params = EVP_SKEYMGMT_get0_imp_settable_params(skeymgmt)) ||
      !TEST_ptr(p =
                OSSL_PARAM_locate_const(imp_params, OSSL_SKEY_PARAM_RAW_BYTES)))
    goto end;
  ...
  if (!TEST_ptr(key = EVP_SKEY_import(libctx, EVP_SKEYMGMT_get0_name(skeymgmt),
  ...
  if (!TEST_int_gt(EVP_SKEY_export(key, OSSL_SKEYMGMT_SELECT_SECRET_KEY,
      ...
      !TEST_int_gt(OSSL_PARAM_get_octet_string_ptr(p, &export_key, &export_len),
                   0) ||
