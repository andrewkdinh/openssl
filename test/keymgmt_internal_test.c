...
static int get_ulong_via_BN(const OSSL_PARAM *p, unsigned long *goal) {
    ...
    if (!TEST_true(OSSL_PARAM_get_BN(p, &n)) ||
        !TEST_int_ge(BN_bn2nativepad(n, (unsigned char *)goal, sizeof(*goal)),
                     0))
        ret = 0;
    BN_free(n);
    return ret;
...
static int export_cb(const OSSL_PARAM *params, void *arg) {
    ...
    if (!TEST_ptr(p = OSSL_PARAM_locate_const(
                  params, OSSL_PKEY_PARAM_RSA_COEFFICIENT1)) ||
        !TEST_true(get_ulong_via_BN(p, &keydata[QINV])) ||
        !TEST_ptr(p = OSSL_PARAM_locate_const(
                  params, OSSL_PKEY_PARAM_RSA_COEFFICIENT2)) ||
...
static int test_pass_rsa(FIXTURE *fixture) {
    ...
    for (;;) {
        ...
        if (!TEST_ptr(provkey =
            ...
            !TEST_true(evp_keymgmt_export(
            km2, provkey, OSSL_KEYMGMT_SELECT_KEYPAIR, &export_cb, keydata)))
            goto err;
