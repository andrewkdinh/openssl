...
static FIXTURE *set_up(const char *testcase_name) {
  ...
  if (!TEST_ptr(fixture = OPENSSL_zalloc(sizeof(*fixture))) ||
      ...
      !TEST_ptr(fixture->prov1 =
                OSSL_PROVIDER_load(fixture->ctx1, "default")) ||
      !TEST_ptr(fixture->ctx2 = OSSL_LIB_CTX_new()) ||
      !TEST_ptr(fixture->prov2 =
                OSSL_PROVIDER_load(fixture->ctx2, "default"))) {
    tear_down(fixture);
    return NULL;
  }
  return fixture;
...
static int export_cb(const OSSL_PARAM *params, void *arg) {
  ...
  if (!TEST_ptr(p =
                OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_RSA_FACTOR1)) ||
      !TEST_true(get_ulong_via_BN(p, &keydata[P])) ||
      !TEST_ptr(p =
                OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_RSA_FACTOR2)) ||
      !TEST_true(get_ulong_via_BN(p, &keydata[Q])) ||
      !TEST_ptr(p =
                OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_RSA_FACTOR3)) ||
...
static int test_pass_rsa(FIXTURE *fixture) {
  ...
  if (!TEST_ptr(bn1 = BN_new()) || !TEST_true(BN_set_word(bn1, expected[N])) ||
      !TEST_ptr(bn2 = BN_new()) || !TEST_true(BN_set_word(bn2, expected[E])) ||
      !TEST_ptr(bn3 = BN_new()) || !TEST_true(BN_set_word(bn3, expected[D])) ||
      !TEST_true(RSA_set0_key(rsa, bn1, bn2, bn3)))
    goto err;

  if (!TEST_ptr(bn1 = BN_new()) || !TEST_true(BN_set_word(bn1, expected[P])) ||
      !TEST_ptr(bn2 = BN_new()) || !TEST_true(BN_set_word(bn2, expected[Q])) ||
      !TEST_true(RSA_set0_factors(rsa, bn1, bn2)))
    goto err;

  if (!TEST_ptr(bn1 = BN_new()) || !TEST_true(BN_set_word(bn1, expected[DP])) ||
      !TEST_ptr(bn2 = BN_new()) || !TEST_true(BN_set_word(bn2, expected[DQ])) ||
      !TEST_ptr(bn3 = BN_new()) ||
  ...
  for (;;) {
    ...
    if (!TEST_ptr_null(provkey2 =
                       evp_pkey_export_to_provider(pk, NULL, &km, NULL)))
      goto err;

    if (!TEST_ptr(provkey =
                  evp_pkey_export_to_provider(pk, NULL, &km1, NULL)) ||
        !TEST_true(evp_keymgmt_export(km2, provkey, OSSL_KEYMGMT_SELECT_KEYPAIR,
                                      &export_cb, keydata)))
      goto err;
...
static int test_evp_pkey_export_to_provider(int n) {
  ...
  if (n == 0) {
    if (!TEST_ptr(keydata =
                  evp_pkey_export_to_provider(pkey, NULL, NULL, NULL)))
      goto end;
  } else if (n == 1) {
    if (!TEST_ptr(keydata =
                  evp_pkey_export_to_provider(pkey, NULL, &keymgmt, NULL)))
      goto end;
  } else {
