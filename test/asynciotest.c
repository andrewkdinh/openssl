      ...
      while (PACKET_remaining(&pkt) > 0) {
        ...
        if (msgtype == SSL3_MT_SERVER_HELLO) {
          if (!PACKET_forward(&wholebody, SSL3_HM_HEADER_LENGTH - 1) ||
              ...
              !PACKET_get_length_prefixed_2(&wholebody, &extensions))
            return -1;
...
static int test_asyncio(int test) {
  ...
  for (j = 0; j < 2; j++) {
     ...
     */
    for (ret = -1, i = 0, len = 0; len != sizeof(testdata) && i < MAX_ATTEMPTS;
         i++) {
...
void cleanup_tests(void) { BIO_meth_free(methods_async); }
