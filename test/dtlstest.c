...
static int test_dtls_unprocessed(int testidx) {
  ...
  if (!TEST_true(create_ssl_objects(sctx, cctx, &serverssl1, &clientssl1, NULL,
                                    c_to_s_fbio)))
    goto end;
...
#define TOTAL_FULL_HAND_RECORDS                                                \
  (CLI_TO_SRV_COOKIE_EXCH + SRV_TO_CLI_COOKIE_EXCH + CLI_TO_SRV_EPOCH_0_RECS + \
   CLI_TO_SRV_EPOCH_1_RECS + SRV_TO_CLI_EPOCH_0_RECS +                         \
   SRV_TO_CLI_EPOCH_1_RECS)
...
static int test_dtls_drop_records(int idx) {
  ...
  if (!TEST_int_eq(
      (int)BIO_ctrl(mempackbio, MEMPACKET_CTRL_GET_DROP_REC, 0, NULL), -1))
    goto end;
 ...
 * Test 0: Test receiving a handshake record early from next epoch on server
 * side Test 1: Test receiving a handshake record early from next epoch on
 * client side Test 2: Test receiving an app data record early from next epoch
 * on client side Test 3: Test receiving an app data before Finished on client
 * side
 */
