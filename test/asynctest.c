...
static int test_ASYNC_callback_status(void) {
  ...
  if (!ASYNC_init_thread(1, 0) || (waitctx = ASYNC_WAIT_CTX_new()) == NULL ||
      ...
      1 ||
      ASYNC_WAIT_CTX_get_callback(waitctx, &get_callback, &get_arg) != 1 ||
...
static int test_ASYNC_start_job_ex(void) {
  ...
  if ((waitctx = ASYNC_WAIT_CTX_new()) == NULL ||
      ASYNC_start_job(&job, waitctx, &funcret, change_deflt_libctx, NULL, 0) !=
      ASYNC_PAUSE) {
...
static int test_ASYNC_set_mem_functions(void) {
  ...
  if ((alloc_fn != test_alloc_stack) || (free_fn != test_free_stack)) {
    fprintf(stderr, "test_ASYNC_set_mem_functions() - setting and retrieving "
                    "custom allocators failed\n");
    return 0;
  }
...
int main(int argc, char **argv) {
  if (!ASYNC_is_capable()) {
    ...
    if (!test_ASYNC_init_thread() || !test_ASYNC_callback_status() ||
        !test_ASYNC_start_job() || !test_ASYNC_get_current_job() ||
        !test_ASYNC_WAIT_CTX_get_all_fds() || !test_ASYNC_block_pause() ||
