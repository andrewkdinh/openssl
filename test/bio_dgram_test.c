...
static int do_sendmmsg(BIO *b, BIO_MSG *msg, size_t num_msg, uint64_t flags,
                       size_t *num_processed) {
  size_t done;

  for (done = 0; done < num_msg;) {
    if (!BIO_sendmmsg(b, msg + done, sizeof(BIO_MSG), num_msg - done, flags,
                      num_processed))
      return 0;
...
static int do_recvmmsg(BIO *b, BIO_MSG *msg, size_t num_msg, uint64_t flags,
                       size_t *num_processed) {
  size_t done;

  for (done = 0; done < num_msg;) {
    if (!BIO_recvmmsg(b, msg + done, sizeof(BIO_MSG), num_msg - done, flags,
                      num_processed))
      return 0;
...
static int test_bio_dgram_pair(int idx) {
  ...
  if (!TEST_int_eq(
      BIO_ADDR_rawmake(addr1, AF_INET, &in_local, sizeof(in_local), 1234), 1))
    goto err;
  ...
  if (!TEST_int_eq(
      BIO_ADDR_rawmake(addr2, AF_INET, &in_local, sizeof(in_local), 2345), 1))
    goto err;
  ...
  if (!TEST_false(BIO_sendmmsg(bio1, msgs, sizeof(BIO_MSG), OSSL_NELEM(msgs), 0,
                               &num_processed)) ||
      !TEST_size_t_eq(num_processed, 0))
    goto err;
  ...
  if (!TEST_mem_eq(rmsgs[0].data, rmsgs[0].data_len, msgs[0].data,
                   msgs[0].data_len))
    goto err;
