...
static const OSSL_PARAM *p_gettable_params(void *_) { return p_param_types; }
...
static int p_get_params(void *provctx, OSSL_PARAM params[]) {
    ...
    for (; ok && p->key != NULL; p++) {
        if (strcmp(p->key, "greeting") == 0) {
            ...
            {"openssl-version", OSSL_PARAM_UTF8_PTR, &opensslv,
             sizeof(&opensslv), 0},
            {"provider-name", OSSL_PARAM_UTF8_PTR, &provname, sizeof(&provname),
             0},
            ...
            /*
             * "default" has not been loaded into the parent libctx. We should
             * be able to explicitly load it as a non-child provider.
             */
             ...
             * available.
             */
            if (ok && OSSL_PROVIDER_available(ctx->libctx, "default") &&
                OSSL_PROVIDER_available(ctx->libctx, "base") &&
                OSSL_PROVIDER_available(ctx->libctx, "legacy") &&
                OSSL_PROVIDER_available(ctx->libctx, "p_test") && md4 != NULL &&
                mdctx != NULL) {
...
static const OSSL_ALGORITHM *p_query(OSSL_PROVIDER *prov, int operation_id,
                                     int *no_cache) {
    *no_cache = 1;
    return NULL;
}
