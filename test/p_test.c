...
static const OSSL_PARAM p_param_types[] = {{"greeting", OSSL_PARAM_UTF8_STRING, NULL, 0, 0},
...
static void p_set_error(int lib, int reason, const char *file, int line, const char *func, const char *fmt, ...)
{
    va_list ap;
    ...
    for (; ok && p->key != NULL; p++) {
        if (strcmp(p->key, "greeting") == 0) {
            ...
            static OSSL_PARAM counter_request[] = {
                ...
                {"provider-name", OSSL_PARAM_UTF8_PTR, &provname, sizeof(&provname), 0},

                /* This might be present, if there's such a configuration */
                {"greeting", OSSL_PARAM_UTF8_PTR, &greeting, sizeof(&greeting), 0},
            ...
            if (c_get_params(hand, counter_request)) {
                if (greeting) {
                    ...
                    local_snprintf(buf, sizeof(buf), "Hello OpenSSL %.20s, greetings from %s!", versionp, namep);
                }
            } else {
            ...
            */
            ...
            if (deflt == NULL || !OSSL_PROVIDER_available(ctx->libctx, "default")) {
                /* We set error "3" for a failure to load the default provider */
                p_set_error(ERR_LIB_PROV, 3, ctx->thisfile, OPENSSL_LINE, ctx->thisfunc, NULL);
                ok = 0;
            }
             ...
             * available.
             */
            if (ok && OSSL_PROVIDER_available(ctx->libctx, "default") && OSSL_PROVIDER_available(ctx->libctx, "base")
                && OSSL_PROVIDER_available(ctx->libctx, "legacy") && OSSL_PROVIDER_available(ctx->libctx, "p_test")
                && md4 != NULL && mdctx != NULL) {
...
{
    static const OSSL_ITEM reason_strings[] = {{1, "dummy reason string"},
...
static const OSSL_ALGORITHM *p_query(OSSL_PROVIDER *prov, int operation_id, int *no_cache)
{
    *no_cache = 1;
    return NULL;
}
...
static const OSSL_DISPATCH p_test_table[] = {
    ...
    {OSSL_FUNC_PROVIDER_GET_REASON_STRINGS, (void (*)(void))p_get_reason_strings},
    {OSSL_FUNC_PROVIDER_TEARDOWN, (void (*)(void))p_teardown},
    {OSSL_FUNC_PROVIDER_QUERY_OPERATION, (void (*)(void))p_query},
    OSSL_DISPATCH_END};

int OSSL_provider_init(const OSSL_CORE_HANDLE *handle, const OSSL_DISPATCH *oin, const OSSL_DISPATCH **out,
                       void **provctx)
{
    P_TEST_CTX *ctx;
    ...
    if (ctx->libctx == NULL) {
        ...
        p_set_error(ERR_LIB_PROV, 2, ctx->thisfile, OPENSSL_LINE, ctx->thisfunc, NULL);
        p_teardown(ctx);
        return 0;
    }
    /*
