...
static int test_mod_exp_zero(void) {
    ...
    if (!TEST_ptr(m = BN_new()) || !TEST_ptr(a = BN_new()) ||
        !TEST_ptr(p = BN_new()) || !TEST_ptr(r = BN_new()))
        goto err;
...
static int test_mod_exp(int round) {
    ...
    if (!TEST_ptr(r_mont = BN_new()) || !TEST_ptr(r_mont_const = BN_new()) ||
        !TEST_ptr(r_recp = BN_new()) || !TEST_ptr(r_simple = BN_new()) ||
    ...
    if (!TEST_BN_eq(r_simple, r_mont) || !TEST_BN_eq(r_simple, r_recp) ||
...
static int test_mod_exp_x2(int idx) {
    ...
    if (!TEST_ptr(r_mont_const_x2_1 = BN_new()) ||
        ...
        !TEST_ptr(r_simple1 = BN_new()) || !TEST_ptr(r_simple2 = BN_new()) ||
        !TEST_ptr(a1 = BN_new()) || !TEST_ptr(b1 = BN_new()) ||
        !TEST_ptr(m1 = BN_new()) || !TEST_ptr(a2 = BN_new()) ||
        !TEST_ptr(b2 = BN_new()) || !TEST_ptr(m2 = BN_new()))
        goto err;
    ...
    if (!TEST_true(BN_mod(a1, a1, m1, ctx)) ||
        ...
        !TEST_true(BN_mod_exp_mont_consttime_x2(r_mont_const_x2_1, a1, b1, m1,
                                                NULL, r_mont_const_x2_2, a2, b2,
                                                m2, NULL, ctx)))
        goto err;
