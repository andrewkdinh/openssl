...
#define DEF_SCRIPT(name, desc)                                                 \
  ...
  static SCRIPT_INFO script_info_##name = {#name, desc, __FILE__, __LINE__,    \
                                           script_gen_##name};                 \
  static void script_gen_##name(GEN_CTX *ctx)
...
static void opgen_END(GEN_CTX *ctx) { openc_opcode(ctx, OPK_END); }
...
static ossl_inline void SRDR_save(SRDR *srdr) { srdr->save_cur = srdr->cur; }

static ossl_inline void SRDR_restore(SRDR *srdr) { srdr->cur = srdr->save_cur; }
...
static void print_opc(BIO *bio, size_t op_num, size_t offset,
                      const char *name) {
  if (op_num != SIZE_MAX)
  ...
  spin_again:
    ...
    case OPK_FUNC: {
      ...
      if (terp->fctx.skip_rest) {
        ...
        if (ret != 1) {
          TEST_error("op %zu (FUNC %s) failed with return value %d", op_num,
                     (const char *)f_name, ret);
          goto err;
        }
      }
    } break;
