...
static int ssl_ctx_configure(SSL_CTX *ctx, int is_server) {
  ...
  if (!TEST_int_eq(
      SSL_CTX_use_certificate_file(ctx, cert_file, SSL_FILETYPE_PEM), 1) ||
      !TEST_int_eq(SSL_CTX_use_PrivateKey_file(ctx, key_file, SSL_FILETYPE_PEM),
                   1))
    return 0;
...
DEF_FUNC(hf_new_ssl) {
  ...
  if (!TEST_true(SSL_CTX_set_domain_flags(ctx, SSL_DOMAIN_FLAG_MULTI_THREAD |
...
DEF_FUNC(hf_connect_wait) {
  ...
  if (RT()->scratch0 == 0) {
    ...
    if (!TEST_false(
        SSL_set_alpn_protos(ssl, alpn_ossltest, sizeof(alpn_ossltest))))
      goto err;
  }
...
#define OP_SIMPLE_PAIR_CONN_D()                                                \
  (OP_NEW_SSL_D(Ds), OP_NEW_SSL_L_FROM_LISTEN(Ds, L, 0), OP_NEW_SSL_C(C),      \
   OP_SET_PEER_ADDR_FROM(C, L), OP_CONNECT_WAIT(C))
...
#define OP_NEW_STREAM(conn_name, stream_name, flags)                           \
  (OP_SELECT_SSL(0, conn_name), OP_PUSH_PZ(#stream_name), OP_PUSH_U64(flags),  \
   OP_PUSH_U64(0), OP_FUNC(hf_new_stream))

#define OP_ACCEPT_STREAM_WAIT(conn_name, stream_name, flags)                   \
  (OP_SELECT_SSL(0, conn_name), OP_PUSH_PZ(#stream_name), OP_PUSH_U64(flags),  \
   OP_PUSH_U64(1), OP_FUNC(hf_new_stream))
...
#define OP_WRITE_EX2(name, buf, buf_len, flags)                                \
  (OP_SELECT_SSL(0, name), OP_PUSH_BUFP(buf, buf_len), OP_PUSH_U64(flags),     \
...
#define OP_READ_EXPECT(name, buf, buf_len)                                     \
  (OP_SELECT_SSL(0, name), OP_PUSH_BUFP(buf, buf_len), OP_FUNC(hf_read_expect))
...
#define OP_DETACH(conn_name, stream_name)                                      \
  (OP_SELECT_SSL(0, conn_name), OP_PUSH_PZ(#stream_name), OP_FUNC(hf_detach))

#define OP_ATTACH(conn_name, stream_name)                                      \
  (OP_SELECT_SSL(0, conn_name), OP_PUSH_PZ(stream_name), OP_FUNC(hf_attach))
...
#define OP_EXPECT_SSL_ERR(name, expected)                                      \
  (OP_SELECT_SSL(0, name), OP_PUSH_U64(expected), OP_FUNC(hf_expect_ssl_err))
