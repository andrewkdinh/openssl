...
static void report_ssl(SSL *ssl, BIO *bio, const char *pfx) {
    ...
    if (is_quic && is_conn &&
        ...
        "%s, %s, reason: \"%s\"\n",
        pfx, e_str, (unsigned long long)cc_info.error_code, f_str,
...
static int RADIX_PROCESS_set_ssl(RADIX_PROCESS *rp, const char *name,
                                 SSL *ssl) {
    RADIX_OBJ *obj;
...
static int bindings_process_init(size_t node_idx, size_t process_idx) {
    ...
    if (!TEST_true(
        CRYPTO_THREAD_init_local(&radix_thread, radix_thread_cleanup_tl)))
        return 0;
...
static int bindings_process_finish(int testresult_main) {
    ...
    if (!TEST_true(
        RADIX_PROCESS_join_all_threads(&radix_process, &testresult_child)))
        return 0;
