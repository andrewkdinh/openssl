...
static int test_generic(INFO *info, int kind)
{
    ...
    uint32_t pn_space = (kind == TEST_KIND_DISCARD) ? QUIC_PN_SPACE_HANDSHAKE
                                                    : QUIC_PN_SPACE_APP;
    ...
    for (i = 0; i < 2; ++i) {
        ...
        if (!TEST_true(ossl_quic_sstream_get_stream_frame(info->sstream[i], 0,
            ...
            || !TEST_size_t_eq(
                ossl_quic_sstream_get_buffer_used(info->sstream[i]), 12)
            || !TEST_true(ossl_quic_sstream_mark_transmitted(
                info->sstream[i], hdr.offset, hdr.offset + hdr.len - 1)))
            goto err;

        if (i == 1
            && !TEST_true(ossl_quic_sstream_mark_transmitted_fin(
    ...
    if (!TEST_ptr(cfq_item = ossl_quic_cfq_add_frame(
                      info->cfq, 10, pn_space, OSSL_QUIC_FRAME_TYPE_NEW_CONN_ID,
                      0, placeholder_data, sizeof(placeholder_data),
                      cfq_free_cb_, NULL))
    ...
    switch (kind) {
    case TEST_KIND_ACK:
        if (!TEST_true(ossl_ackm_on_rx_ack_frame(info->ackm, &ack, pn_space,
                                                 cur_time)))
            goto err;

        for (i = 0; i < 2; ++i)
            if (!TEST_size_t_eq(
                    ossl_quic_sstream_get_buffer_used(info->sstream[i]), 0))
                goto err;
    ...
    case TEST_KIND_LOSS:
        ...
        if (!TEST_true(ossl_quic_fifd_pkt_commit(&info->fifd, pkt2))
            || !TEST_true(ossl_ackm_on_rx_ack_frame(info->ackm, &ack, pn_space,
                                                    cur_time)))
            goto err;
        ...
        for (i = 0; i < 2; ++i) {
            ...
            if (!TEST_true(ossl_quic_sstream_get_stream_frame(
                    info->sstream[i], 0, &hdr, iov, &num_iov))
                || !TEST_uint64_t_eq(hdr.offset, 0)
                || !TEST_uint64_t_eq(hdr.len, 12))
                goto err;
        }
...
static int test_fifd(int idx)
{
    ...
    if (!TEST_true(ossl_statm_init(&info.statm))
        ...
        || !TEST_true(ossl_quic_fifd_init(
            &info.fifd, info.cfq, info.ackm, info.txpim, get_sstream_by_id,
            NULL, regen_frame, NULL, confirm_frame, NULL, sstream_updated, NULL,
            NULL, NULL)))
        goto err;
