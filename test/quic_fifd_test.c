...
static QUIC_SSTREAM *(*get_sstream_by_id_p)(uint64_t stream_id, uint32_t pn_space, void *arg);
...
static void (*regen_frame_p)(uint64_t frame_type, uint64_t stream_id, QUIC_TXPIM_PKT *pkt, void *arg);

static void regen_frame(uint64_t frame_type, uint64_t stream_id, QUIC_TXPIM_PKT *pkt, void *arg)
{
    regen_frame_p(frame_type, stream_id, pkt, arg);
}

static void confirm_frame(uint64_t frame_type, uint64_t stream_id, QUIC_TXPIM_PKT *pkt, void *arg)
{
}
 ...
 */
static QUIC_SSTREAM *sstream_expect(uint64_t stream_id, uint32_t pn_space, void *arg)
{
...
static void regen_expect(uint64_t frame_type, uint64_t stream_id, QUIC_TXPIM_PKT *pkt, void *arg)
{
...
static int test_generic(INFO *info, int kind)
{
    ...
    uint32_t pn_space = (kind == TEST_KIND_DISCARD) ? QUIC_PN_SPACE_HANDSHAKE : QUIC_PN_SPACE_APP;
    ...
    for (i = 0; i < 2; ++i) {
        ...
        if (!TEST_true(ossl_quic_sstream_append(info->sstream[i], (unsigned char *)"Test message", 12, &consumed))
        ...
        if (!TEST_true(ossl_quic_sstream_get_stream_frame(info->sstream[i], 0, &hdr, iov, &num_iov))
            || !TEST_int_eq(hdr.is_fin, i == 1) || !TEST_uint64_t_eq(hdr.offset, 0) || !TEST_uint64_t_eq(hdr.len, 12)
            || !TEST_size_t_eq(ossl_quic_sstream_get_buffer_used(info->sstream[i]), 12)
            || !TEST_true(ossl_quic_sstream_mark_transmitted(info->sstream[i], hdr.offset, hdr.offset + hdr.len - 1)))
            goto err;
    ...
    cfq_freed = 0;
    if (!TEST_ptr(cfq_item = ossl_quic_cfq_add_frame(info->cfq, 10, pn_space, OSSL_QUIC_FRAME_TYPE_NEW_CONN_ID, 0,
                                                     placeholder_data, sizeof(placeholder_data), cfq_free_cb_, NULL))
    ...
    switch (kind) {
    case TEST_KIND_ACK:
        if (!TEST_true(ossl_ackm_on_rx_ack_frame(info->ackm, &ack, pn_space, cur_time)))
            goto err;
    ...
    case TEST_KIND_LOSS:
        ...
        if (!TEST_true(ossl_quic_fifd_pkt_commit(&info->fifd, pkt2))
            || !TEST_true(ossl_ackm_on_rx_ack_frame(info->ackm, &ack, pn_space, cur_time)))
            goto err;
        ...
        for (i = 0; i < 2; ++i) {
             ...
             */
            if (!TEST_true(ossl_quic_sstream_get_stream_frame(info->sstream[i], 0, &hdr, iov, &num_iov))
                || !TEST_uint64_t_eq(hdr.offset, 0) || !TEST_uint64_t_eq(hdr.len, 12))
                goto err;
        }

        /* FC frame should have regenerated for each stream */
        if (!TEST_size_t_eq(regen_count, 7) || !TEST_uint64_t_eq(regen_stream_id[0], 42)
        ...
        num_iov = OSSL_NELEM(iov);
        if (!TEST_true(ossl_quic_sstream_get_stream_frame(info->sstream[1], 1, &hdr, iov, &num_iov))
            || !TEST_true(hdr.is_fin) || !TEST_uint64_t_eq(hdr.len, 0))
            goto err;
...
static int test_fifd(int idx)
{
    ...
    if (!TEST_true(ossl_statm_init(&info.statm)) || !TEST_ptr(info.ccdata = ossl_cc_dummy_method.new(fake_now, NULL))
        || !TEST_ptr(info.ackm = ossl_ackm_new(fake_now, NULL, &info.statm, &ossl_cc_dummy_method, info.ccdata))
        || !TEST_true(ossl_ackm_on_handshake_confirmed(info.ackm)) || !TEST_ptr(info.cfq = ossl_quic_cfq_new())
        || !TEST_ptr(info.txpim = ossl_quic_txpim_new())
        || !TEST_true(ossl_quic_fifd_init(&info.fifd, info.cfq, info.ackm, info.txpim, get_sstream_by_id, NULL,
                                          regen_frame, NULL, confirm_frame, NULL, sstream_updated, NULL, NULL, NULL)))
        goto err;
    ...
    if (!TEST_true(test_generic(&info, idx)) || !TEST_false(cb_fail))
        goto err;
