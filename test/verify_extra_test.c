...
static int test_self_signed(const char *filename, int use_trusted,
                            int expected) {
...
static int test_self_signed_bad(void) { return test_self_signed(bad_f, 1, 0); }
...
static int do_test_purpose(int purpose, int expected) {
    ...
    if (!TEST_ptr(eecert) || !TEST_ptr(untrcert) || !TEST_ptr(trcert) ||
        !TEST_ptr(trusted) || !TEST_ptr(untrusted) || !TEST_ptr(ctx))
        goto err;
