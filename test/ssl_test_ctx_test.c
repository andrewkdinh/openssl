...
#define EXECUTE_SSL_TEST_CTX_TEST() EXECUTE_TEST(execute_test, tear_down)
...
static int test_bad_configuration(int idx) {
  ...
  if (!TEST_ptr_null(
      ctx = SSL_TEST_CTX_create(conf, bad_configurations[idx], NULL))) {
    SSL_TEST_CTX_free(ctx);
    return 0;
  }
...
void cleanup_tests(void) { NCONF_free(conf); }
