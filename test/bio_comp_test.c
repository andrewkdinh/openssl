...
static int do_bio_comp_test(const BIO_METHOD *meth, size_t size) {
  ...
  if (!TEST_int_eq(size, rsize) || !TEST_mem_eq(original, osize, result, rsize))
    goto err;
...
static int test_zstd(int n) { return do_bio_comp(BIO_f_zstd(), n); }
#endif
#ifndef OPENSSL_NO_BROTLI
static int test_brotli(int n) { return do_bio_comp(BIO_f_brotli(), n); }
#endif
#ifndef OPENSSL_NO_ZLIB
static int test_zlib(int n) { return do_bio_comp(BIO_f_zlib(), n); }
#endif
