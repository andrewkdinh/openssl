...
static int tmpmd_digest(void *provctx, const unsigned char *in, size_t inl, unsigned char *out, size_t *outl,
                        size_t outsz)
{
    return 0;
}

static const OSSL_DISPATCH testprovmd_functions[] = {{OSSL_FUNC_DIGEST_GET_PARAMS, (void (*)(void))tmpmd_get_params},
                                                     {OSSL_FUNC_DIGEST_DIGEST, (void (*)(void))tmpmd_digest},
                                                     OSSL_DISPATCH_END};

static const OSSL_ALGORITHM testprov_digests[] = {{"testprovmd", MYPROPERTIES, testprovmd_functions},
                                                  {NULL, NULL, NULL}};
...
static int testprov_provider_init(const OSSL_CORE_HANDLE *handle, const OSSL_DISPATCH *in, const OSSL_DISPATCH **out,
                                  void **provctx)
{
    *provctx = (void *)handle;
...
static int test_default_props_and_providers(int propsorder)
{
    ...
    if (propsorder == DEFAULT_PROPS_FIRST && !TEST_true(EVP_set_default_properties(libctx, MYPROPERTIES)))
        goto err;

    if (!TEST_ptr(testprov = OSSL_PROVIDER_load(libctx, "testprov")))
        goto err;

    if (propsorder == DEFAULT_PROPS_AFTER_LOAD && !TEST_true(EVP_set_default_properties(libctx, MYPROPERTIES)))
        goto err;
