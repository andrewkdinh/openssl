...
static int check_time(long offset)
{
    ...
    if (!TEST_true(OPENSSL_gmtime_adj(&tm1, 0, offset)) || !TEST_int_eq(tm1.tm_year, tm2.tm_year)
        ...
        || !TEST_int_eq(tm1.tm_sec, tm2.tm_sec) || !TEST_true(OPENSSL_gmtime_diff(&off_day, &off_sec, &o1, &tm1)))
        return 0;
...
{
    return check_time(offset) && check_time(-offset) && check_time(offset * 1000L) && check_time(-offset * 1000L)
