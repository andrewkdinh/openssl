...
static EVP_PKEY *make_template(const char *type, OSSL_PARAM *genparams) {
     ...
     */
    (void)((ctx = EVP_PKEY_CTX_new_from_name(keyctx, type, testpropq)) !=
           NULL &&
...
static EVP_PKEY *make_key(const char *type, EVP_PKEY *template,
    ...
    EVP_PKEY *pkey = NULL;
    EVP_PKEY_CTX *ctx =
    template != NULL ? EVP_PKEY_CTX_new_from_pkey(keyctx, template, testpropq)
...
static int test_encode_decode(const char *file, const int line,
     ...
     * dumping purposes.
     */
    if (!TEST_true(encode_cb(file, line, &encoded, &encoded_len, pkey,
...
static int decode_EVP_PKEY_prov(const char *file, const int line, void **object,
    ...
    for (i = 0; i < 3; i++) {
        const char *testtype =
        (i == 0) ? input_type : ((i == 1) ? NULL : badtype);

        if (!TEST_FL_ptr(dctx = OSSL_DECODER_CTX_new_for_pkey(
                         &testpkey, testtype, structure_type, keytype,
                         selection, testctx, testpropq)) ||
            (pass != NULL &&
...
static int encode_EVP_PKEY_legacy_PEM(const char *file, const int line,
    ...
    if (pcipher != NULL && pass != NULL) {
        passlen = strlen(pass);
        if (!TEST_FL_ptr(cipher =
                         EVP_CIPHER_fetch(testctx, pcipher, testpropq)))
            goto end;
    }
...
static int encode_EVP_PKEY_PVK(const char *file, const int line, void **encoded,
    ...
    if (!TEST_FL_true(
        ...
        !TEST_FL_ptr(mem_ser = BIO_new(BIO_s_mem())) ||
        !TEST_FL_int_ge(i2b_PVK_bio_ex(mem_ser, pkey, enc, pass_pw,
                                       (void *)pass, testctx, testpropq),
                        0) ||
        !TEST_FL_true(BIO_get_mem_ptr(mem_ser, &mem_buf) > 0) ||
        !TEST_FL_ptr(*encoded = mem_buf->data) ||
        !TEST_FL_long_gt(*encoded_len = mem_buf->length, 0))
...
static int test_unprotected_via_legacy_PEM(const char *type, EVP_PKEY *key) {
    ...
    "type-specific", NULL, NULL, encode_EVP_PKEY_legacy_PEM,
    decode_EVP_PKEY_prov, test_text, check_unprotected_legacy_PEM, dump_pem, 0);
}
...
static int test_unprotected_via_MSBLOB(const char *type, EVP_PKEY *key) {
    ...
    test_mem, check_MSBLOB, dump_der, 0);
}
...
static int create_ec_explicit_prime_params(OSSL_PARAM_BLD *bld) {
    ...
    0x04, 0xe4, 0x97, 0x08, 0xbe, 0x7d, 0xfa, 0xa2, 0x9a, 0xa3, 0x12,
    0x6f, 0xe4, 0xe7, 0xd0, 0x25, 0xe3, 0x4a, 0xc1, 0x03, 0x15, 0x8c,
    0xd9, 0x33, 0xc6, 0x97, 0x42, 0xf5, 0xdc, 0x97, 0xb9, 0xd7, 0x31,
    0xe9, 0x7d, 0x74, 0x3d, 0x67, 0x6a, 0x3b, 0x21, 0x08, 0x9c, 0x31,
    0x73, 0xf8, 0xc1, 0x27, 0xc9, 0xd2, 0xa0, 0xa0, 0x83, 0x66, 0xe0,
    0xc9, 0xda, 0xa8, 0xc6, 0x56, 0x2b, 0x94, 0xb1, 0xae, 0x55};
...
static int create_ec_explicit_trinomial_params_namedcurve(OSSL_PARAM_BLD *bld) {
    ...
    0x04, 0x01, 0x72, 0x32, 0xBA, 0x85, 0x3A, 0x7E, 0x73, 0x1A, 0xF1,
    0x29, 0xF2, 0x2F, 0xF4, 0x14, 0x95, 0x63, 0xA4, 0x19, 0xC2, 0x6B,
    0xF5, 0x0A, 0x4C, 0x9D, 0x6E, 0xEF, 0xAD, 0x61, 0x26, 0x01, 0xDB,
    0x53, 0x7D, 0xEC, 0xE8, 0x19, 0xB7, 0xF7, 0x0F, 0x55, 0x5A, 0x67,
    0xC4, 0x27, 0xA8, 0xCD, 0x9B, 0xF1, 0x8A, 0xEB, 0x9B, 0x56, 0xE0,
...
static int create_ec_explicit_trinomial_params(OSSL_PARAM_BLD *bld) {
    ...
    0x04, 0x00, 0xd7, 0xba, 0xd0, 0x26, 0x6c, 0x31, 0x6a, 0x78, 0x76,
    0x01, 0xd1, 0x32, 0x4b, 0x8f, 0x30, 0x29, 0x2d, 0x78, 0x30, 0xca,
    0x43, 0xaa, 0xf0, 0xa2, 0x5a, 0xd4, 0x0f, 0xb3, 0xf4, 0x00, 0x85,
    0x4b, 0x1b, 0x8d, 0x50, 0x10, 0xa5, 0x1c, 0x80, 0xf7, 0x86, 0x40,
    0x62, 0x4c, 0x87, 0xd1, 0x26, 0x7a, 0x9c, 0x5c, 0xe9, 0x82, 0x29,
...
int setup_tests(void) {
    ...
    MAKE_DOMAIN_KEYS(ECExplicitPrime2G, "EC",
