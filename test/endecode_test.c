                              ...
                              checker *check_cb, dumper *dump_cb, int flags)
{
    ...
    if (!TEST_true(check_cb(file, line, type, encoded, encoded_len))
        || !TEST_true(decode_cb(file, line, (void **)&pkey2, encoded,
                                encoded_len, output_type, output_structure,
                                (flags & FLAG_DECODE_WITH_TYPE ? type : NULL),
                                selection, pass))
        || ((output_structure == NULL
            ...
            && !TEST_true(decode_cb(
                file, line, (void **)&pkey3, encoded, encoded_len, output_type,
                output_structure, (flags & FLAG_DECODE_WITH_TYPE ? type : NULL),
                0, pass)))
                                ...
                                const char *pcipher)
{
    ...
    if (!TEST_FL_ptr(
        ...
        || (pass != NULL
            && !TEST_FL_true(
                OSSL_ENCODER_CTX_set_passphrase(ectx, upass, strlen(pass))))
        || (pcipher != NULL
...
static int test_unprotected_via_DER(const char *type, EVP_PKEY *key, int fips)
{
    return test_encode_decode(
        ...
        "PrivateKeyInfo", NULL, NULL, encode_EVP_PKEY_prov,
        decode_EVP_PKEY_prov, test_mem, check_unprotected_PKCS8_DER, dump_der,
...
static int test_unprotected_via_PEM(const char *type, EVP_PKEY *key, int fips)
{
    return test_encode_decode(
        ...
        "PrivateKeyInfo", NULL, NULL, encode_EVP_PKEY_prov,
        decode_EVP_PKEY_prov, test_text, check_unprotected_PKCS8_PEM, dump_pem,
        fips ? 0 : FLAG_FAIL_IF_FIPS);
}
...
static int test_params_via_DER(const char *type, EVP_PKEY *key)
{
    return test_encode_decode(__FILE__, __LINE__, type, key,
                              OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS, "DER",
                              "type-specific", NULL, NULL, encode_EVP_PKEY_prov,
                              decode_EVP_PKEY_prov, test_mem, check_params_DER,
...
{
    return test_encode_decode(
        __FILE__, __LINE__, type, key, OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS,
        "PEM", "type-specific", NULL, NULL, encode_EVP_PKEY_prov,
        decode_EVP_PKEY_prov, test_text, check_params_PEM, dump_pem, 0);
}
...
static int test_unprotected_via_legacy_PEM(const char *type, EVP_PKEY *key)
{
    ...
    return test_encode_decode(
        ...
        "PEM", "type-specific", NULL, NULL, encode_EVP_PKEY_legacy_PEM,
        decode_EVP_PKEY_prov, test_text, check_unprotected_legacy_PEM, dump_pem,
        0);
}
...
static int test_unprotected_via_MSBLOB(const char *type, EVP_PKEY *key)
{
    return test_encode_decode(
        __FILE__, __LINE__, type, key,
        OSSL_KEYMGMT_SELECT_KEYPAIR | OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS,
        "MSBLOB", NULL, NULL, NULL, encode_EVP_PKEY_MSBLOB,
        decode_EVP_PKEY_prov, test_mem, check_MSBLOB, dump_der, 0);
}
...
static int test_unprotected_via_PVK(const char *type, EVP_PKEY *key)
{
    return test_encode_decode(
        __FILE__, __LINE__, type, key,
        OSSL_KEYMGMT_SELECT_KEYPAIR | OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS,
        "PVK", NULL, NULL, NULL, encode_EVP_PKEY_PVK, decode_EVP_PKEY_prov,
...
static int test_protected_via_DER(const char *type, EVP_PKEY *key, int fips)
{
    return test_encode_decode(
        ...
        encode_EVP_PKEY_prov, decode_EVP_PKEY_prov, test_mem,
        check_protected_PKCS8_DER, dump_der, fips ? 0 : FLAG_FAIL_IF_FIPS);
}
...
static int test_protected_via_PEM(const char *type, EVP_PKEY *key, int fips)
{
    return test_encode_decode(
        ...
        encode_EVP_PKEY_prov, decode_EVP_PKEY_prov, test_text,
        check_protected_PKCS8_PEM, dump_pem, fips ? 0 : FLAG_FAIL_IF_FIPS);
}
...
static int test_protected_via_legacy_PEM(const char *type, EVP_PKEY *key)
{
    ...
    return test_encode_decode(
        __FILE__, __LINE__, type, key,
        OSSL_KEYMGMT_SELECT_KEYPAIR | OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS,
        "PEM", "type-specific", pass, pass_cipher, encode_EVP_PKEY_legacy_PEM,
        decode_EVP_PKEY_prov, test_text, check_protected_legacy_PEM, dump_pem,
        0);
}
...
static int test_protected_via_PVK(const char *type, EVP_PKEY *key)
{
    ...
    ret = test_encode_decode(
        __FILE__, __LINE__, type, key,
        OSSL_KEYMGMT_SELECT_KEYPAIR | OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS,
        "PVK", NULL, pass, NULL, encode_EVP_PKEY_PVK, decode_EVP_PKEY_prov,
        test_mem, check_PVK, dump_der, 0);
...
static int test_public_via_DER(const char *type, EVP_PKEY *key, int fips)
{
    return test_encode_decode(
        ...
        "DER", "SubjectPublicKeyInfo", NULL, NULL, encode_EVP_PKEY_prov,
...
static int test_public_via_PEM(const char *type, EVP_PKEY *key, int fips)
{
    return test_encode_decode(
        __FILE__, __LINE__, type, key,
        OSSL_KEYMGMT_SELECT_PUBLIC_KEY | OSSL_KEYMGMT_SELECT_ALL_PARAMETERS,
        "PEM", "SubjectPublicKeyInfo", NULL, NULL, encode_EVP_PKEY_prov,
...
static int test_public_via_MSBLOB(const char *type, EVP_PKEY *key)
{
    return test_encode_decode(
        ...
        "MSBLOB", NULL, NULL, NULL, encode_EVP_PKEY_MSBLOB,
        decode_EVP_PKEY_prov, test_mem, check_public_MSBLOB, dump_der, 0);
}
