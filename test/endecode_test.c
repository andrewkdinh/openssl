...
#define TEST_FL_strn2_eq(a, m, b, n)                                           \
  test_strn_eq(file, line, #a, #b, a, m, b, n)
...
typedef int(encoder)(const char *file, const int line, void **encoded,
                     long *encoded_len, void *object, int selection,
                     const char *output_type, const char *output_structure,
                     const char *pass, const char *pcipher);
typedef int(decoder)(const char *file, const int line, void **object,
                     void *encoded, long encoded_len, const char *input_type,
                     const char *structure_type, const char *keytype,
                     int selection, const char *pass);
typedef int(tester)(const char *file, const int line, const void *data1,
                    size_t data1_len, const void *data2, size_t data2_len);
typedef int(checker)(const char *file, const int line, const char *type,
...
static int test_encode_decode(const char *file, const int line,
                              const char *type, EVP_PKEY *pkey, int selection,
                              const char *output_type,
                              const char *output_structure, const char *pass,
                              const char *pcipher, encoder *encode_cb,
                              decoder *decode_cb, tester *test_cb,
  ...
  if (!TEST_true(check_cb(file, line, type, encoded, encoded_len)) ||
      ...
      ((output_structure == NULL ||
       ...
       !TEST_true(decode_cb(file, line, (void **)&pkey3, encoded, encoded_len,
                            output_type, output_structure,
                            (flags & FLAG_DECODE_WITH_TYPE ? type : NULL), 0,
                            pass))) ||
      !TEST_true(encode_cb(file, line, &encoded2, &encoded2_len, pkey2,
                           selection, output_type, output_structure, pass,
                           pcipher)))
    goto end;
  ...
  if (selection == OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) {
    ...
    if (!TEST_int_eq(EVP_PKEY_eq(pkey, pkey2), 1) ||
        (pkey3 != NULL && !TEST_int_eq(EVP_PKEY_eq(pkey, pkey3), 1)))
      goto end;
  }
...
static int encode_EVP_PKEY_prov(const char *file, const int line,
                                void **encoded, long *encoded_len, void *object,
  ...
  if (!TEST_FL_ptr(
      ectx = OSSL_ENCODER_CTX_new_for_pkey(pkey, selection, output_type,
                                           output_structure, testpropq)) ||
      !TEST_FL_int_gt(OSSL_ENCODER_CTX_get_num_encoders(ectx), 0) ||
      (pass != NULL && !TEST_FL_true(OSSL_ENCODER_CTX_set_passphrase(
                       ectx, upass, strlen(pass)))) ||
      (pcipher != NULL &&
...
static int encode_EVP_PKEY_i2d(const char *unused_file, const int unused_line,
                               void **encoded, long *encoded_len, void *object,
                               int unused_selection,
...
static int decode_EVP_PKEY_prov(const char *file, const int line, void **object,
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (!TEST_FL_ptr(dctx = OSSL_DECODER_CTX_new_for_pkey(
                     &testpkey, testtype, structure_type, keytype, selection,
                     testctx, testpropq)) ||
        (pass != NULL &&
...
static int encode_EVP_PKEY_legacy_PEM(const char *file, const int line,
  ...
  if (!TEST_FL_ptr(mem_ser = BIO_new(BIO_s_mem())) ||
      ...
      mem_ser, pkey, cipher, upass, passlen, NULL, NULL)) ||
      !TEST_FL_true(BIO_get_mem_ptr(mem_ser, &mem_buf) > 0) ||
      !TEST_FL_ptr(*encoded = mem_buf->data) ||
      !TEST_FL_long_gt(*encoded_len = mem_buf->length, 0))
    goto end;
...
static int encode_EVP_PKEY_PVK(const char *file, const int line, void **encoded,
                               long *encoded_len, void *object, int selection,
                               ossl_unused const char *output_type,
                               ossl_unused const char *output_structure,
                               const char *pass,
  ...
  if (!TEST_FL_true(
      ossl_assert((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0)) ||
      !TEST_FL_ptr(mem_ser = BIO_new(BIO_s_mem())) ||
      !TEST_FL_int_ge(i2b_PVK_bio_ex(mem_ser, pkey, enc, pass_pw, (void *)pass,
                                     testctx, testpropq),
                      0) ||
      !TEST_FL_true(BIO_get_mem_ptr(mem_ser, &mem_buf) > 0) ||
      !TEST_FL_ptr(*encoded = mem_buf->data) ||
...
static int test_text(const char *file, const int line, const void *data1,
                     size_t data1_len, const void *data2, size_t data2_len) {
  return TEST_FL_strn2_eq(data1, data1_len, data2, data2_len);
}

static int test_mem(const char *file, const int line, const void *data1,
                    size_t data1_len, const void *data2, size_t data2_len) {
...
static int test_unprotected_via_DER(const char *type, EVP_PKEY *key, int fips) {
  ...
  __FILE__, __LINE__, type, key,
  OSSL_KEYMGMT_SELECT_KEYPAIR | OSSL_KEYMGMT_SELECT_ALL_PARAMETERS, "DER",
  "PrivateKeyInfo", NULL, NULL, encode_EVP_PKEY_prov, decode_EVP_PKEY_prov,
...
static int test_unprotected_via_i2d(const char *type, EVP_PKEY *key, int fips) {
  ...
  __FILE__, __LINE__, type, key,
  OSSL_KEYMGMT_SELECT_KEYPAIR | OSSL_KEYMGMT_SELECT_ALL_PARAMETERS, "DER",
  "PrivateKeyInfo", NULL, NULL, encode_EVP_PKEY_i2d, decode_EVP_PKEY_prov,
  test_mem, check_unprotected_PKCS8_DER, dump_der,
  fips ? 0 : FLAG_FAIL_IF_FIPS);
}
...
static int test_unprotected_via_PEM(const char *type, EVP_PKEY *key, int fips) {
  return test_encode_decode(
  __FILE__, __LINE__, type, key,
  OSSL_KEYMGMT_SELECT_KEYPAIR | OSSL_KEYMGMT_SELECT_ALL_PARAMETERS, "PEM",
  "PrivateKeyInfo", NULL, NULL, encode_EVP_PKEY_prov, decode_EVP_PKEY_prov,
...
#ifndef OPENSSL_NO_KEYPARAMS
static int check_params_DER(const char *file, const int line, const char *type,
                            const void *data, size_t data_len) {
  const unsigned char *datap = data;
  int ok = 0;
  int itype = NID_undef;
...
static int check_params_PEM(const char *file, const int line, const char *type,
                            const void *data, size_t data_len) {
...
static int test_params_via_DER(const char *type, EVP_PKEY *key) {
  return test_encode_decode(
  __FILE__, __LINE__, type, key, OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS, "DER",
  "type-specific", NULL, NULL, encode_EVP_PKEY_prov, decode_EVP_PKEY_prov,
  test_mem, check_params_DER, dump_der, FLAG_DECODE_WITH_TYPE);
}

static int test_params_via_PEM(const char *type, EVP_PKEY *key) {
  return test_encode_decode(
  __FILE__, __LINE__, type, key, OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS, "PEM",
  "type-specific", NULL, NULL, encode_EVP_PKEY_prov, decode_EVP_PKEY_prov,
  test_text, check_params_PEM, dump_pem, 0);
}
...
static int test_unprotected_via_legacy_PEM(const char *type, EVP_PKEY *key) {
  if (!default_libctx || is_fips)
    return TEST_skip("Test not available if using a non-default library "
                     "context or FIPS provider");

  return test_encode_decode(
  __FILE__, __LINE__, type, key,
  OSSL_KEYMGMT_SELECT_KEYPAIR | OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS, "PEM",
  "type-specific", NULL, NULL, encode_EVP_PKEY_legacy_PEM, decode_EVP_PKEY_prov,
  test_text, check_unprotected_legacy_PEM, dump_pem, 0);
}

static int check_MSBLOB(const char *file, const int line, const char *type,
                        const void *data, size_t data_len) {
  const unsigned char *datap = data;
...
static int test_unprotected_via_MSBLOB(const char *type, EVP_PKEY *key) {
  return test_encode_decode(
  __FILE__, __LINE__, type, key,
  OSSL_KEYMGMT_SELECT_KEYPAIR | OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS, "MSBLOB",
  NULL, NULL, NULL, encode_EVP_PKEY_MSBLOB, decode_EVP_PKEY_prov, test_mem,
  check_MSBLOB, dump_der, 0);
}

static int check_PVK(const char *file, const int line, const char *type,
                     const void *data, size_t data_len) {
...
static int test_unprotected_via_PVK(const char *type, EVP_PKEY *key) {
  return test_encode_decode(
  __FILE__, __LINE__, type, key,
  OSSL_KEYMGMT_SELECT_KEYPAIR | OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS, "PVK",
  NULL, NULL, NULL, encode_EVP_PKEY_PVK, decode_EVP_PKEY_prov, test_mem,
  check_PVK, dump_der, 0);
}
...
static int test_protected_via_DER(const char *type, EVP_PKEY *key, int fips) {
  ...
  "EncryptedPrivateKeyInfo", pass, pass_cipher, encode_EVP_PKEY_prov,
  decode_EVP_PKEY_prov, test_mem, check_protected_PKCS8_DER, dump_der,
  fips ? 0 : FLAG_FAIL_IF_FIPS);
}
...
static int test_protected_via_PEM(const char *type, EVP_PKEY *key, int fips) {
  ...
  OSSL_KEYMGMT_SELECT_KEYPAIR | OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS, "PEM",
  "EncryptedPrivateKeyInfo", pass, pass_cipher, encode_EVP_PKEY_prov,
  decode_EVP_PKEY_prov, test_text, check_protected_PKCS8_PEM, dump_pem,
  fips ? 0 : FLAG_FAIL_IF_FIPS);
}
...
static int test_protected_via_legacy_PEM(const char *type, EVP_PKEY *key) {
  ...
  __FILE__, __LINE__, type, key,
  OSSL_KEYMGMT_SELECT_KEYPAIR | OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS, "PEM",
  "type-specific", pass, pass_cipher, encode_EVP_PKEY_legacy_PEM,
  decode_EVP_PKEY_prov, test_text, check_protected_legacy_PEM, dump_pem, 0);
}
...
static int test_protected_via_PVK(const char *type, EVP_PKEY *key) {
  ...
  ret = test_encode_decode(
  __FILE__, __LINE__, type, key,
  OSSL_KEYMGMT_SELECT_KEYPAIR | OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS, "PVK",
  NULL, pass, NULL, encode_EVP_PKEY_PVK, decode_EVP_PKEY_prov, test_mem,
  check_PVK, dump_der, 0);
...
static int check_public_DER(const char *file, const int line, const char *type,
                            const void *data, size_t data_len) {
  const unsigned char *datap = data;
...
static int test_public_via_DER(const char *type, EVP_PKEY *key, int fips) {
  ...
  "SubjectPublicKeyInfo", NULL, NULL, encode_EVP_PKEY_prov,
  decode_EVP_PKEY_prov, test_mem, check_public_DER, dump_der,
  fips ? 0 : FLAG_FAIL_IF_FIPS);
}

static int check_public_PEM(const char *file, const int line, const char *type,
                            const void *data, size_t data_len) {
  static const char expected_pem_header[] =
...
static int test_public_via_PEM(const char *type, EVP_PKEY *key, int fips) {
  ...
  "SubjectPublicKeyInfo", NULL, NULL, encode_EVP_PKEY_prov,
  decode_EVP_PKEY_prov, test_text, check_public_PEM, dump_pem,
  fips ? 0 : FLAG_FAIL_IF_FIPS);
}
...
static int test_public_via_MSBLOB(const char *type, EVP_PKEY *key) {
  ...
  "MSBLOB", NULL, NULL, NULL, encode_EVP_PKEY_MSBLOB, decode_EVP_PKEY_prov,
  test_mem, check_public_MSBLOB, dump_der, 0);
}

#define KEYS(KEYTYPE) static EVP_PKEY *key_##KEYTYPE = NULL
#define MAKE_KEYS(KEYTYPE, KEYTYPEstr, params)                                 \
  ok = ok && TEST_ptr(key_##KEYTYPE = make_key(KEYTYPEstr, NULL, params))
#define FREE_KEYS(KEYTYPE) EVP_PKEY_free(key_##KEYTYPE);
...
#define MAKE_DOMAIN_KEYS(KEYTYPE, KEYTYPEstr, params)                          \
  ok =                                                                         \
  ok && TEST_ptr(template_##KEYTYPE = make_template(KEYTYPEstr, params)) &&    \
...
static int do_create_ec_explicit_prime_params(OSSL_PARAM_BLD *bld,
  ...
  0x00, 0xff, 0xff, 0xff, 0xff, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00,
  ...
  static const unsigned char seed[] = {0xc4, 0x9d, 0x36, 0x08, 0x86, 0xe7, 0x04,
                                       0x93, 0x6a, 0x66, 0x78, 0xe1, 0x13, 0x9d,
                                       0x26, 0xb7, 0x81, 0x9f, 0x7e, 0x90};
  ...
  return TEST_ptr(a = BN_CTX_get(bnctx)) && TEST_ptr(b = BN_CTX_get(bnctx)) &&
         ...
         bld, OSSL_PKEY_PARAM_EC_GENERATOR, gen, gen_len)) &&
         TEST_true(OSSL_PARAM_BLD_push_octet_string(
         bld, OSSL_PKEY_PARAM_EC_SEED, seed, sizeof(seed))) &&
...
static int create_ec_explicit_prime_params(OSSL_PARAM_BLD *bld) {
  ...
  0x04, 0xe4, 0x97, 0x08, 0xbe, 0x7d, 0xfa, 0xa2, 0x9a, 0xa3, 0x12, 0x6f, 0xe4,
  0xe7, 0xd0, 0x25, 0xe3, 0x4a, 0xc1, 0x03, 0x15, 0x8c, 0xd9, 0x33, 0xc6, 0x97,
  0x42, 0xf5, 0xdc, 0x97, 0xb9, 0xd7, 0x31, 0xe9, 0x7d, 0x74, 0x3d, 0x67, 0x6a,
  0x3b, 0x21, 0x08, 0x9c, 0x31, 0x73, 0xf8, 0xc1, 0x27, 0xc9, 0xd2, 0xa0, 0xa0,
  0x83, 0x66, 0xe0, 0xc9, 0xda, 0xa8, 0xc6, 0x56, 0x2b, 0x94, 0xb1, 0xae, 0x55};
...
static int create_ec_explicit_trinomial_params_namedcurve(OSSL_PARAM_BLD *bld) {
  ...
  0x04, 0x01, 0x72, 0x32, 0xBA, 0x85, 0x3A, 0x7E, 0x73, 0x1A, 0xF1, 0x29, 0xF2,
...
static int create_ec_explicit_trinomial_params(OSSL_PARAM_BLD *bld) {
  ...
  0x04, 0x00, 0xd7, 0xba, 0xd0, 0x26, 0x6c, 0x31, 0x6a, 0x78, 0x76, 0x01, 0xd1,
...
int setup_tests(void) {
  ...
  OSSL_PARAM DSA_params[] = {OSSL_PARAM_size_t("pbits", &pbits),
  ...
  if (default_libctx) {
    ...
    if (!test_get_libctx(&testctx, &nullprov, config_file, &deflprov,
                         prov_name))
      return 0;
  }
  ...
  if (!TEST_ptr(bnctx = BN_CTX_new_ex(testctx)) ||
      ...
      !TEST_ptr(ec_explicit_prime_params_nc =
                OSSL_PARAM_BLD_to_param(bld_prime_nc)) ||
      !TEST_ptr(ec_explicit_prime_params_explicit =
      ...
      !TEST_ptr(ec_explicit_tri_params_nc =
                OSSL_PARAM_BLD_to_param(bld_tri_nc)) ||
      !TEST_ptr(ec_explicit_tri_params_explicit =
  ...
  MAKE_DOMAIN_KEYS(ECExplicitPrimeNamedCurve, "EC",
