...
static CMP_VFY_TEST_FIXTURE *set_up(const char *const test_case_name)
{
    ...
    if (ts == NULL || !TEST_ptr(fixture->cmp_ctx = OSSL_CMP_CTX_new(libctx, NULL))
...
/* chain */
static X509 *endentity1 = NULL, *endentity2 = NULL, *intermediate = NULL, *root = NULL;
/* INSTA chain */
...
static const unsigned char sec_1[] = {'9', 'p', 'p', '8', '-', 'b', '3', '5', 'i', '-',
                                      'X', 'd', '3', 'Q', '-', 'u', 'd', 'N', 'R'};
...
{
    int res = TEST_int_eq(fixture->expected, ossl_cmp_msg_check_update(fixture->cmp_ctx, fixture->msg, NULL, 0));
...
static int execute_validate_cert_path_test(CMP_VFY_TEST_FIXTURE *fixture)
{
    ...
    int res = TEST_int_eq(fixture->expected, OSSL_CMP_validate_cert_path(fixture->cmp_ctx, ts, fixture->cert));
...
static int test_validate_msg_mac_alg_protection(int miss, int wrong)
{
    ...
    if (!TEST_true(miss ? OSSL_CMP_CTX_set0_trusted(fixture->cmp_ctx, NULL)
                        : OSSL_CMP_CTX_set1_secretValue(fixture->cmp_ctx, sec_1, wrong ? 4 : sizeof(sec_1)))
...
{
    const unsigned char sec_bad[] = {'9', 'p', 'p', '8', '-', 'b', '3', '5', 'i', '-',
                                     'X', 'd', '3', 'Q', '-', 'u', 'd', 'N', 'r'};
    ...
    if (!TEST_true(OSSL_CMP_CTX_set1_secretValue(fixture->cmp_ctx, sec_bad, sizeof(sec_bad)))
        || !TEST_ptr(fixture->msg = load_pkimsg(ip_waiting_f, libctx))) {
        tear_down(fixture);
        fixture = NULL;
    }
...
{
    return X509_add_cert(OSSL_CMP_CTX_get0_untrusted(ctx), cert, X509_ADD_FLAG_UP_REF);
}
...
static int test_validate_msg_signature_partial_chain(int expired)
{
    ...
    if (ts == NULL || !TEST_ptr(fixture->msg = load_pkimsg(ir_protected_f, libctx))
...
static int test_validate_msg_signature_srvcert(int bad_sig, int miss, int wrong)
{
    ...
    if (!TEST_ptr(fixture->msg = load_pkimsg(ir_protected_f, libctx))
        || !TEST_true(miss ? OSSL_CMP_CTX_set1_secretValue(fixture->cmp_ctx, sec_1, sizeof(sec_1))
                           : OSSL_CMP_CTX_set1_srvCert(fixture->cmp_ctx, wrong ? clcert : srvcert))
...
static int test_validate_msg_signature_sender_cert_untrusted(void)
{
    ...
    if (!TEST_ptr(fixture->msg = load_pkimsg(ir_protected_0_extracerts, libctx))
        || !add_trusted(fixture->cmp_ctx, instaca_cert) || !add_untrusted(fixture->cmp_ctx, insta_cert)) {
        tear_down(fixture);
        fixture = NULL;
    }
    EXECUTE_TEST(execute_validate_msg_test, tear_down);
...
static int test_validate_msg_signature_sender_cert_trusted(void)
{
    ...
    fixture->expected = 1;
    if (!TEST_ptr(fixture->msg = load_pkimsg(ir_protected_0_extracerts, libctx))
        || !add_trusted(fixture->cmp_ctx, instaca_cert) || !add_trusted(fixture->cmp_ctx, insta_cert)) {
        tear_down(fixture);
        fixture = NULL;
    }
    EXECUTE_TEST(execute_validate_msg_test, tear_down);
...
static void setup_path(CMP_VFY_TEST_FIXTURE **fixture, X509 *wrong, int expired)
{
    ...
    if (!add_trusted((*fixture)->cmp_ctx, wrong == NULL ? root : wrong)
        || !add_untrusted((*fixture)->cmp_ctx, endentity1) || !add_untrusted((*fixture)->cmp_ctx, intermediate)) {
        tear_down((*fixture));
        (*fixture) = NULL;
    }
}
...
static int execute_msg_check_test(CMP_VFY_TEST_FIXTURE *fixture)
{
    ...
    if (!TEST_int_eq(fixture->expected,
                     ossl_cmp_msg_check_update(fixture->cmp_ctx, fixture->msg, fixture->allow_unprotected_cb,
    ...
    return TEST_int_eq(0, ASN1_OCTET_STRING_cmp(ossl_cmp_hdr_get0_senderNonce(hdr), fixture->cmp_ctx->recipNonce))
        && TEST_int_eq(0, ASN1_OCTET_STRING_cmp(tid, fixture->cmp_ctx->transactionID));
}

static int allow_unprotected(const OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *msg, int invalid_protection, int allow)
{
    return allow;
}

static void setup_check_update(CMP_VFY_TEST_FIXTURE **fixture, int expected, ossl_cmp_allow_unprotected_cb_t cb,
                               int arg, const unsigned char *trid_data, const unsigned char *nonce_data)
{
    ...
    if ((*fixture)->msg == NULL
        || (nonce_data != NULL && !ossl_cmp_asn1_octet_string_set1_bytes(&ctx->senderNonce, nonce_data, nonce_len))) {
        tear_down((*fixture));
        (*fixture) = NULL;
    } else if (trid_data != NULL) {
        ASN1_OCTET_STRING *trid = ASN1_OCTET_STRING_new();

        if (trid == NULL || !ASN1_OCTET_STRING_set(trid, trid_data, OSSL_CMP_TRANSACTIONID_LENGTH)
...
static int test_msg_check_transaction_id(void)
{
    ...
    const unsigned char trans_id[OSSL_CMP_TRANSACTIONID_LENGTH] = {0x39, 0xB6, 0x90, 0x28, 0xC4, 0xBC, 0x7A, 0xF6,
...
static int test_msg_check_recipient_nonce(void)
{
    ...
    const unsigned char rec_nonce[OSSL_CMP_SENDERNONCE_LENGTH] = {0x48, 0xF1, 0x71, 0x1F, 0xE5, 0xAF, 0x1C, 0x8B,
...
int setup_tests(void)
{
    ...
    if (!TEST_ptr(server_f = test_get_argument(0)) || !TEST_ptr(client_f = test_get_argument(1))
        || !TEST_ptr(endentity1_f = test_get_argument(2)) || !TEST_ptr(endentity2_f = test_get_argument(3))
        || !TEST_ptr(root_f = test_get_argument(4)) || !TEST_ptr(intermediate_f = test_get_argument(5))
        || !TEST_ptr(ir_protected_f = test_get_argument(6)) || !TEST_ptr(ir_unprotected_f = test_get_argument(7))
        || !TEST_ptr(ip_waiting_f = test_get_argument(8)) || !TEST_ptr(ir_rmprotection_f = test_get_argument(9))
        || !TEST_ptr(instacert_f = test_get_argument(10)) || !TEST_ptr(instaca_f = test_get_argument(11))
    ...
    if (!TEST_ptr(endentity1 = load_cert_pem(endentity1_f, libctx))
        || !TEST_ptr(endentity2 = load_cert_pem(endentity2_f, libctx)) || !TEST_ptr(root = load_cert_pem(root_f, NULL))
    ...
    if (!TEST_ptr(srvcert = load_cert_pem(server_f, libctx)) || !TEST_ptr(clcert = load_cert_pem(client_f, libctx)))
        goto err;
