...
/* chain */
static X509 *endentity1 = NULL, *endentity2 = NULL, *intermediate = NULL,
            *root = NULL;
/* INSTA chain */
...
static const unsigned char sec_1[] = {'9', 'p', 'p', '8', '-', 'b', '3',
                                      '5', 'i', '-', 'X', 'd', '3', 'Q',
                                      '-', 'u', 'd', 'N', 'R'};
...
static int test_validate_msg_mac_alg_protection(int miss, int wrong) {
    ...
    if (!TEST_true(miss
                   ? OSSL_CMP_CTX_set0_trusted(fixture->cmp_ctx, NULL)
                   : OSSL_CMP_CTX_set1_secretValue(
                     fixture->cmp_ctx, sec_1, wrong ? 4 : sizeof(sec_1))) ||
...
static int test_validate_msg_mac_alg_protection_bad(void) {
    const unsigned char sec_bad[] = {'9', 'p', 'p', '8', '-', 'b', '3',
                                     '5', 'i', '-', 'X', 'd', '3', 'Q',
                                     '-', 'u', 'd', 'N', 'r'};
...
static int test_validate_msg_signature_srvcert(int bad_sig, int miss,
                                               int wrong) {
    SETUP_TEST_FIXTURE(CMP_VFY_TEST_FIXTURE, set_up);
    fixture->cert = srvcert;
    fixture->expected = !bad_sig && !wrong && !miss;
    if (!TEST_ptr(fixture->msg = load_pkimsg(ir_protected_f, libctx)) ||
        !TEST_true(miss ? OSSL_CMP_CTX_set1_secretValue(fixture->cmp_ctx, sec_1,
                                                        sizeof(sec_1))
                        : OSSL_CMP_CTX_set1_srvCert(
                          fixture->cmp_ctx, wrong ? clcert : srvcert)) ||
...
static int test_validate_msg_signature_sender_cert_extracert(void) {
    ...
    fixture->expected = 1;
    if (!TEST_ptr(fixture->msg =
                  load_pkimsg(ir_protected_2_extracerts, libctx)) ||
        !add_trusted(fixture->cmp_ctx, instaca_cert)) {
        tear_down(fixture);
        fixture = NULL;
    } else {
        fixture->cert =
        sk_X509_value(fixture->msg->extraCerts, 1); /* Insta CA */
    }
    EXECUTE_TEST(execute_validate_msg_test, tear_down);
    return result;
}
...
static void setup_path(CMP_VFY_TEST_FIXTURE **fixture, X509 *wrong,
                       int expired) {
...
static int execute_msg_check_test(CMP_VFY_TEST_FIXTURE *fixture) {
    ...
    return TEST_int_eq(0,
           ...
           TEST_int_eq(
           0, ASN1_OCTET_STRING_cmp(tid, fixture->cmp_ctx->transactionID));
}
...
static void setup_check_update(CMP_VFY_TEST_FIXTURE **fixture, int expected,
    ...
    if ((*fixture)->msg == NULL ||
        (nonce_data != NULL && !ossl_cmp_asn1_octet_string_set1_bytes(
                               &ctx->senderNonce, nonce_data, nonce_len))) {
        tear_down((*fixture));
        (*fixture) = NULL;
