...
typedef void *PEM_read_bio_of_void(BIO *out, void **obj, pem_password_cb *cb,
                                   void *u);
typedef int EVP_PKEY_print_fn(BIO *out, const EVP_PKEY *pkey, int indent,
                              ASN1_PCTX *pctx);
...
static const OSSL_PARAM DSA_params[] = {OSSL_PARAM_size_t("pbits", &pbits),
...
static EVP_PKEY *make_key(const char *type,
     ...
     */
    ctx = template != NULL ? EVP_PKEY_CTX_new(template, NULL)
...
static int test_protected_PEM(const char *keytype, int evp_type,
    ...
    if (!TEST_ptr(ectx = OSSL_ENCODER_CTX_new_for_pkey(
                  provided_pkey, selection, "PEM", structure, NULL)) ||
    ...
    if (pem_read_bio != NULL) {
        ...
        if (!TEST_ptr(decoded_legacy_pkey = EVP_PKEY_new()) ||
            !TEST_ptr(dctx = OSSL_DECODER_CTX_new_for_pkey(
                      &decoded_provided_pkey, "PEM", structure, keytype,
                      selection, NULL, NULL)) ||
        ...
        if (!TEST_int_gt(
            evp_pkey_eq(decoded_provided_pkey, decoded_legacy_pkey), 0)) {
...
static int
test_unprotected_PEM(const char *keytype, int evp_type, const void *legacy_key,
                     PEM_write_bio_of_void_unprotected *pem_write_bio,
                     PEM_read_bio_of_void *pem_read_bio,
                     EVP_PKEY_eq_fn *evp_pkey_eq,
                     EVP_PKEY_print_fn *evp_pkey_print, EVP_PKEY *provided_pkey,
...
static int test_DER(const char *keytype, int evp_type, const void *legacy_key,
                    i2d_of_void *i2d, d2i_of_void *d2i,
                    EVP_PKEY_eq_fn *evp_pkey_eq,
                    EVP_PKEY_print_fn *evp_pkey_print, EVP_PKEY *provided_pkey,
                    int selection, const char *structure) {
    int ok = 0;
    ...
    if (!TEST_ptr(ectx = OSSL_ENCODER_CTX_new_for_pkey(
                  provided_pkey, selection, "DER", structure, NULL)) ||
        !TEST_true(
        OSSL_ENCODER_to_data(ectx, &der_provided, &der_provided_len)) ||
        !TEST_size_t_gt(der_legacy_len = i2d(legacy_key, &der_legacy), 0) ||
        !TEST_mem_eq(der_provided, der_provided_len, der_legacy,
                     der_legacy_len))
        goto end;
    ...
    if (d2i != NULL) {
        ...
        if (!TEST_ptr(decoded_legacy_pkey = EVP_PKEY_new()) ||
            !TEST_ptr(dctx = OSSL_DECODER_CTX_new_for_pkey(
                      &decoded_provided_pkey, "DER", structure, keytype,
                      selection, NULL, NULL)) ||
            !TEST_true(
            (pder_provided = der_provided, tmp_size = der_provided_len,
             OSSL_DECODER_from_data(dctx, &pder_provided, &tmp_size))) ||
            !TEST_ptr((pder_legacy = der_legacy,
                       decoded_legacy_key =
                       d2i(NULL, &pder_legacy, (long)der_legacy_len))) ||
            !TEST_true(
            EVP_PKEY_assign(decoded_legacy_pkey, evp_type, decoded_legacy_key)))
            goto end;
...
static int test_key(int idx) {
    ...
    if (test_stanza->pem_write_bio_PrivateKey != NULL) {
        ...
        for (i = 0; i < OSSL_NELEM(test_stanza->structure); i++) {
            ...
            TEST_info("Test OSSL_ENCODER against "
                      "PEM_write_bio_{TYPE}PrivateKey for %s, %s",
            ...
            if (!test_protected_PEM(key->keytype, key->evp_type, legacy_obj,
                                    ...
                                    EVP_PKEY_eq, EVP_PKEY_print_private, pkey,
                                    selection, structure))
                ok = 0;
        }
    }
    ...
    if (test_stanza->pem_write_bio_PublicKey != NULL) {
        ...
        for (i = 0; i < OSSL_NELEM(test_stanza->structure); i++) {
            ...
            TEST_info("Test OSSL_ENCODER against PEM_write_bio_{TYPE}PublicKey "
                      "for %s, %s",
                      test_stanza->keytype, structure);
            if (!test_unprotected_PEM(key->keytype, key->evp_type, legacy_obj,
                                      ...
                                      EVP_PKEY_eq, EVP_PKEY_print_public, pkey,
                                      selection, structure))
                ok = 0;
        }
    }
    ...
    if (test_stanza->pem_write_bio_params != NULL) {
        ...
        for (i = 0; i < OSSL_NELEM(test_stanza->structure); i++) {
            ...
            test_stanza->keytype, structure);
            if (!test_unprotected_PEM(
                key->keytype, key->evp_type, legacy_obj,
                test_stanza->pem_write_bio_params,
                test_stanza->pem_read_bio_params, EVP_PKEY_parameters_eq,
                EVP_PKEY_print_params, pkey, selection, structure))
                ok = 0;
        }
    }
    ...
    if (test_stanza->pem_write_bio_PUBKEY != NULL) {
        ...
        if (!test_unprotected_PEM(
            key->keytype, key->evp_type, legacy_obj,
            test_stanza->pem_write_bio_PUBKEY, test_stanza->pem_read_bio_PUBKEY,
    ...
    if (test_stanza->i2d_PrivateKey != NULL) {
        ...
        for (i = 0; i < OSSL_NELEM(test_stanza->structure); i++) {
            ...
            if (!test_DER(key->keytype, key->evp_type, legacy_obj,
                          ...
                          test_stanza->d2i_PrivateKey, EVP_PKEY_eq,
                          EVP_PKEY_print_private, pkey, selection, structure))
                ok = 0;
        }
    }
    ...
    if (test_stanza->i2d_PublicKey != NULL) {
        ...
        for (i = 0; i < OSSL_NELEM(test_stanza->structure); i++) {
            ...
            if (!test_DER(key->keytype, key->evp_type, legacy_obj,
                          ...
                          test_stanza->d2i_PublicKey, EVP_PKEY_eq,
                          EVP_PKEY_print_public, pkey, selection, structure))
                ok = 0;
        }
    }
    ...
    if (test_stanza->i2d_params != NULL) {
        ...
        for (i = 0; i < OSSL_NELEM(test_stanza->structure); i++) {
            ...
            if (!test_DER(key->keytype, key->evp_type, legacy_obj,
                          ...
                          EVP_PKEY_parameters_eq, EVP_PKEY_print_params, pkey,
                          selection, structure))
                ok = 0;
        }
    }
    ...
    if (test_stanza->i2d_PUBKEY != NULL) {
        ...
        if (!test_DER(key->keytype, key->evp_type, legacy_obj,
                      ...
                      EVP_PKEY_eq, EVP_PKEY_print_public, pkey, selection,
                      structure))
            ok = 0;
    }
end:
