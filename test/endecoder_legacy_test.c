...
typedef int PEM_write_bio_of_void_protected(BIO *out, const void *obj, const EVP_CIPHER *enc, unsigned char *kstr,
...
static const OSSL_PARAM DSA_params[] = {OSSL_PARAM_size_t("pbits", &pbits), OSSL_PARAM_size_t("qbits", &qbits),
                                        OSSL_PARAM_END};
#endif
#ifndef OPENSSL_NO_EC
static char groupname[] = "prime256v1";
static const OSSL_PARAM EC_params[] = {OSSL_PARAM_utf8_string("group", groupname, sizeof(groupname) - 1),
                                       OSSL_PARAM_END};
#endif
...
#ifndef OPENSSL_NO_DH
    {"DH", EVP_PKEY_DH, DH_params, NULL},    {"DHX", EVP_PKEY_DHX, DHX_params, NULL},
#endif
#ifndef OPENSSL_NO_DSA
...
static EVP_PKEY *make_key(const char *type, const OSSL_PARAM *gen_template_params)
{
    ...
    if (gen_template_params != NULL
        && ((ctx = EVP_PKEY_CTX_new_from_name(NULL, type, NULL)) == NULL || EVP_PKEY_paramgen_init(ctx) <= 0
            || (gen_template_params[0].key != NULL && EVP_PKEY_CTX_set_params(ctx, gen_template_params_noconst) <= 0)
     ...
     */
    ctx = template != NULL ? EVP_PKEY_CTX_new(template, NULL) : EVP_PKEY_CTX_new_from_name(NULL, type, NULL);
...
static int test_membio_str_eq(BIO *bio_provided, BIO *bio_legacy)
{
    ...
    return TEST_long_ge(len_legacy, 0) && TEST_long_ge(len_provided, 0)
        && TEST_strn2_eq(str_provided, len_provided, str_legacy, len_legacy);
}

static int test_protected_PEM(const char *keytype, int evp_type, const void *legacy_key,
                              PEM_write_bio_of_void_protected *pem_write_bio, PEM_read_bio_of_void *pem_read_bio,
                              EVP_PKEY_eq_fn *evp_pkey_eq, EVP_PKEY_print_fn *evp_pkey_print, EVP_PKEY *provided_pkey,
                              int selection, const char *structure)
{
    ...
    if (!TEST_ptr(membio_legacy = BIO_new(BIO_s_mem())) || !TEST_ptr(membio_provided = BIO_new(BIO_s_mem())))
        goto end;

    if (!TEST_ptr(ectx = OSSL_ENCODER_CTX_new_for_pkey(provided_pkey, selection, "PEM", structure, NULL))
        || !TEST_true(OSSL_ENCODER_to_bio(ectx, membio_provided))
        || !TEST_true(pem_write_bio(membio_legacy, legacy_key, NULL, NULL, 0, NULL, NULL))
    ...
    if (pem_read_bio != NULL) {
        ...
        if (!TEST_ptr(decoded_legacy_pkey = EVP_PKEY_new())
            || !TEST_ptr(dctx = OSSL_DECODER_CTX_new_for_pkey(&decoded_provided_pkey, "PEM", structure, keytype,
                                                              selection, NULL, NULL))
...
static int test_unprotected_PEM(const char *keytype, int evp_type, const void *legacy_key,
                                PEM_write_bio_of_void_unprotected *pem_write_bio, PEM_read_bio_of_void *pem_read_bio,
                                EVP_PKEY_eq_fn *evp_pkey_eq, EVP_PKEY_print_fn *evp_pkey_print, EVP_PKEY *provided_pkey,
                                int selection, const char *structure)
{
    int ok = 0;
    ...
    if (!TEST_ptr(ectx = OSSL_ENCODER_CTX_new_for_pkey(provided_pkey, selection, "PEM", structure, NULL))
        || !TEST_true(OSSL_ENCODER_to_bio(ectx, membio_provided))
        || !TEST_true(pem_write_bio(membio_legacy, legacy_key)) || !test_membio_str_eq(membio_provided, membio_legacy))
        goto end;
...
static int test_DER(const char *keytype, int evp_type, const void *legacy_key, i2d_of_void *i2d, d2i_of_void *d2i,
                    EVP_PKEY_eq_fn *evp_pkey_eq, EVP_PKEY_print_fn *evp_pkey_print, EVP_PKEY *provided_pkey,
                    int selection, const char *structure)
{
    int ok = 0;
    ...
    if (d2i != NULL) {
        ...
        if (!TEST_ptr(decoded_legacy_pkey = EVP_PKEY_new())
            || !TEST_ptr(dctx = OSSL_DECODER_CTX_new_for_pkey(&decoded_provided_pkey, "DER", structure, keytype,
                                                              selection, NULL, NULL))
            || !TEST_true((pder_provided = der_provided, tmp_size = der_provided_len,
                           OSSL_DECODER_from_data(dctx, &pder_provided, &tmp_size)))
...
static int test_key(int idx)
{
    ...
    /* Set up the keys */
    if (!TEST_ptr(pkey = key->key) || !TEST_true(evp_pkey_copy_downgraded(&downgraded_pkey, pkey))
        || !TEST_ptr(downgraded_pkey) || !TEST_int_eq(EVP_PKEY_get_id(downgraded_pkey), key->evp_type)
    ...
    if (test_stanza->pem_write_bio_PrivateKey != NULL) {
        ...
        for (i = 0; i < OSSL_NELEM(test_stanza->structure); i++) {
            ...
            TEST_info("Test OSSL_ENCODER against PEM_write_bio_{TYPE}PrivateKey for %s, %s", test_stanza->keytype,
                      structure);
            if (!test_protected_PEM(key->keytype, key->evp_type, legacy_obj, test_stanza->pem_write_bio_PrivateKey,
                                    test_stanza->pem_read_bio_PrivateKey, EVP_PKEY_eq, EVP_PKEY_print_private, pkey,
                                    selection, structure))
                ok = 0;
        }
    }
    ...
    if (test_stanza->pem_write_bio_PublicKey != NULL) {
        ...
        for (i = 0; i < OSSL_NELEM(test_stanza->structure); i++) {
            ...
            TEST_info("Test OSSL_ENCODER against PEM_write_bio_{TYPE}PublicKey for %s, %s", test_stanza->keytype,
                      structure);
            if (!test_unprotected_PEM(key->keytype, key->evp_type, legacy_obj, test_stanza->pem_write_bio_PublicKey,
                                      test_stanza->pem_read_bio_PublicKey, EVP_PKEY_eq, EVP_PKEY_print_public, pkey,
                                      selection, structure))
                ok = 0;
        }
    }
    ...
    if (test_stanza->pem_write_bio_params != NULL) {
        ...
        for (i = 0; i < OSSL_NELEM(test_stanza->structure); i++) {
            ...
            TEST_info("Test OSSL_ENCODER against PEM_write_bio_{TYPE}params for %s, %s", test_stanza->keytype,
                      structure);
            if (!test_unprotected_PEM(key->keytype, key->evp_type, legacy_obj, test_stanza->pem_write_bio_params,
                                      test_stanza->pem_read_bio_params, EVP_PKEY_parameters_eq, EVP_PKEY_print_params,
    ...
    if (test_stanza->pem_write_bio_PUBKEY != NULL) {
        ...
        if (!test_unprotected_PEM(key->keytype, key->evp_type, legacy_obj, test_stanza->pem_write_bio_PUBKEY,
                                  test_stanza->pem_read_bio_PUBKEY, EVP_PKEY_eq, EVP_PKEY_print_public, pkey, selection,
                                  structure))
            ok = 0;
    }
    ...
    if (test_stanza->i2d_PublicKey != NULL) {
        ...
        for (i = 0; i < OSSL_NELEM(test_stanza->structure); i++) {
            ...
            if (!test_DER(key->keytype, key->evp_type, legacy_obj, test_stanza->i2d_PublicKey,
                          test_stanza->d2i_PublicKey, EVP_PKEY_eq, EVP_PKEY_print_public, pkey, selection, structure))
                ok = 0;
        }
    }
    ...
    if (test_stanza->i2d_params != NULL) {
        ...
        for (i = 0; i < OSSL_NELEM(test_stanza->structure); i++) {
            ...
            if (!test_DER(key->keytype, key->evp_type, legacy_obj, test_stanza->i2d_params, test_stanza->d2i_params,
    ...
    if (test_stanza->i2d_PUBKEY != NULL) {
        ...
        if (!test_DER(key->keytype, key->evp_type, legacy_obj, test_stanza->i2d_PUBKEY, test_stanza->d2i_PUBKEY,
                      EVP_PKEY_eq, EVP_PKEY_print_public, pkey, selection, structure))
            ok = 0;
    }
end:
