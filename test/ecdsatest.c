...
static int test_builtin(int n, int as) {
  ...
  if (!TEST_int_ge(temp, 0) ||
      ...
      (as == EVP_PKEY_SM2 && !set_sm2_id(mctx, pkey)) ||
      !TEST_int_eq(EVP_DigestVerify(mctx, sig, sig_len - 1, tbs, sizeof(tbs)),
                   -1) ||
      !TEST_true(EVP_MD_CTX_reset(mctx))
...
static int test_ecdsa_sig_NULL(void) {
  ...
  ret =
  TEST_ptr(eckey = EC_KEY_new_by_curve_name(NID_X9_62_prime256v1)) &&
  TEST_int_eq(EC_KEY_generate_key(eckey), 1) &&
  TEST_int_eq(ECDSA_sign(0, dgst, sizeof(dgst), NULL, &siglen0, eckey), 1) &&
  TEST_int_gt(siglen0, 0) && TEST_ptr(sig = OPENSSL_malloc(siglen0)) &&
  TEST_int_eq(ECDSA_sign(0, dgst, sizeof(dgst), sig, &siglen, eckey), 1) &&
  TEST_int_gt(siglen, 0) && TEST_int_le(siglen, siglen0) &&
  TEST_int_eq(ECDSA_verify(0, dgst, sizeof(dgst), sig, siglen, eckey), 1) &&
  TEST_int_eq(ECDSA_sign_setup(eckey, NULL, &kinv, &rp), 1) &&
  TEST_int_eq(
  ECDSA_sign_ex(0, dgst, sizeof(dgst), NULL, &siglen, kinv, rp, eckey), 1) &&
  TEST_int_gt(siglen, 0) && TEST_int_le(siglen, siglen0) &&
  TEST_int_eq(
  ECDSA_sign_ex(0, dgst, sizeof(dgst), sig, &siglen0, kinv, rp, eckey), 1) &&
