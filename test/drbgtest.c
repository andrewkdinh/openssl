 ...
 *                |before_reseed| time.
 */
static int test_drbg_reseed(int expect_success, EVP_RAND_CTX *primary,
                            EVP_RAND_CTX *public, EVP_RAND_CTX *private,
                            unsigned char *public_random,
                            unsigned char *private_random,
                            int expect_primary_reseed, int expect_public_reseed,
  ...
  if (!TEST_int_eq(rand_bytes((unsigned char *)public_random, RANDOM_SIZE),
                   expect_success) ||
      !TEST_int_eq(
...
static int test_drbg_reseed_in_child(EVP_RAND_CTX *primary,
  ...
  if (!TEST_int_ge(pid = fork(), 0)) {
    ...
    if (TEST_true(test_drbg_reseed(1, primary, public, private, &random[0],
                                   &random[RANDOM_SIZE], 1, 1, 1, 0)) &&
...
static int test_rand_reseed_on_fork(EVP_RAND_CTX *primary, EVP_RAND_CTX *public,
                                    EVP_RAND_CTX *private) {
  unsigned int i;
  pid_t pid = getpid();
  ...
  for (i = 1; i <= DRBG_FORK_COUNT; ++i) {
    ...
    if (!TEST_true(
        test_drbg_reseed_in_child(primary, public, private, presult)))
      return 0;
  ...
  if (verbose || !success) {
    ...
    for (i = 0; i < DRBG_FORK_RESULT_COUNT; ++i) {
      ...
      TEST_note("    random: %s, pid: %d (%s, %s)", rand_hex, result[i].pid,
                result[i].name, result[i].private ? "private" : "public");
...
static int test_rand_fork_safety(int i) {
  ...
  if (!TEST_int_gt(RAND_bytes(random, 1), 0) ||
      !TEST_int_gt(RAND_priv_bytes(random, 1), 0))
    success = 0;
...
static int test_rand_reseed(void) {
  ...
  if (!TEST_ptr_ne(public, private) || !TEST_ptr_ne(public, primary) ||
  ...
  if (!TEST_true(test_drbg_reseed(1, primary, public, private, NULL, NULL, 1, 1,
                                  1, before_reseed)))
    goto error;
...
static int set_reseed_time_interval(EVP_RAND_CTX *drbg, int t) {
  ...
  params[0] =
  OSSL_PARAM_construct_int(OSSL_DRBG_PARAM_RESEED_TIME_INTERVAL, &t);
...
static EVP_RAND_CTX *new_drbg(EVP_RAND_CTX *parent) {
  ...
  params[0] =
  OSSL_PARAM_construct_utf8_string(OSSL_DRBG_PARAM_CIPHER, "AES-256-CTR", 0);
  params[1] = OSSL_PARAM_construct_end();
