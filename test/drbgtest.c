                                    ...
                                    EVP_RAND_CTX *private)
{
    ...
    if (verbose || !success) {
        ...
        for (i = 0; i < DRBG_FORK_RESULT_COUNT; ++i) {
            ...
            TEST_note("    random: %s, pid: %d (%s, %s)", rand_hex,
                      result[i].pid, result[i].name,
...
static int test_rand_reseed(void)
{
     ...
     */
    if (!TEST_true(test_drbg_reseed(1, primary, public, private, NULL, NULL, 1,
                                    1, 1, 0)))
        goto error;
     ...
     */
    if (!TEST_true(test_drbg_reseed(1, primary, public, private, NULL, NULL, 0,
                                    0, 0, 0)))
        goto error;
...
static int test_multi_thread(void)
{
    ...
    for (i = 0; i < THREADS; i++) run_thread(&t[i]);
    run_multi_thread_test();
    for (i = 0; i < THREADS; i++) wait_for_thread(t[i]);
