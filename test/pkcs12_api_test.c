...
static int changepass(PKCS12 *p12, EVP_PKEY *key, X509 *cert,
  ...
  if (!TEST_true(i2d_PKCS12_bio(bio, p12)))
    goto err;
  if (!TEST_ptr(p12new =
                PKCS12_init_ex(NID_pkcs7_data, testctx, "provider=default")))
    goto err;
...
static int pkcs12_create_ex2_test(int test) {
  ...
  if (test == 0) {
    ...
    ptr = PKCS12_create_ex2(NULL, NULL, NULL, NULL, NULL, NID_undef, NID_undef,
                            0, 0, 0, testctx, NULL, NULL, NULL);
    if (TEST_ptr(ptr))
      goto err;
    ...
    cb_ret = 1;
    ptr =
    PKCS12_create_ex2(NULL, NULL, NULL, cert, NULL, NID_undef, NID_undef, 0, 0,
                      0, testctx, NULL, pkcs12_create_cb, (void *)&cb_ret);
