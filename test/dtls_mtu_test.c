...
static unsigned int clnt_psk_callback(SSL *ssl, const char *hint, char *ident,
...
static int mtu_test(SSL_CTX *ctx, const char *cs, int no_etm) {
    ...
    for (i = 0; i < 30; i++) {
        ...
        if (debug)
            TEST_info("%s%s MTU for record mtu %d = %lu", cs,
                      no_etm ? "-noEtM" : "", 500 + i, (unsigned long)mtus[i]);
    ...
    for (s = mtus[0]; s <= mtus[29]; s++) {
        ...
        for (i = 0; i < 30; i++) {
            ...
            if (!TEST_false(s <= mtus[i] && reclen > (size_t)(500 + i))) {
                 ...
                 */
                TEST_error("%s: s=%lu, mtus[i]=%lu, reclen=%lu, i=%d", cs,
                           (unsigned long)s, (unsigned long)mtus[i],
...
void cleanup_tests(void) { bio_s_mempacket_test_free(); }
