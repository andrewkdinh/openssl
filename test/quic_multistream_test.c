...
#define C_SPIN_AGAIN()                                                         \
  ...
  for (;;) {
    ...
    case OPK_S_WRITE: {
      ...
      if (!TEST_true(ossl_quic_tserver_write(ACQUIRE_S(), s_stream_id, op->arg0,
                                             op->arg1, &bytes_written)) ||
          !TEST_size_t_eq(bytes_written, op->arg1))
        goto out;
    } break;
    ...
    case OPK_S_READ_FAIL: {
      ...
      ret = ossl_quic_tserver_read(ACQUIRE_S(), s_stream_id, buf, sizeof(buf),
                                   &bytes_read);
    ...
    case OPK_S_SET_INJECT_HANDSHAKE:
      ...
      if (!TEST_true(qtest_fault_set_handshake_listener(
          h->qtf,
          h->qtf_handshake_cb != NULL ? helper_handshake_listener : NULL, h)))
        goto out;
    ...
    case OPK_S_SET_INJECT_DATAGRAM:
      ...
      if (!TEST_true(qtest_fault_set_datagram_listener(
          h->qtf, h->qtf_datagram_cb != NULL ? helper_datagram_listener : NULL,
          h)))
        goto out;
  ...
  if (!testresult) {
    ...
    if (h->c_conn != NULL) {
      ...
      if (SSL_get_conn_close_info(h->c_conn, &cc_info, sizeof(cc_info))) {
        ...
        TEST_info(
        "client side is closed: %llu(%s)/%llu(%s), "
        "%s, %s, reason: \"%s\"",
        ...
        (cc_info.flags & SSL_CONN_CLOSE_FLAG_TRANSPORT) != 0 ? "transport"
                                                             : "app",
...
static CRYPTO_THREAD_RETVAL run_script_child_thread(void *arg) {
  ...
  testresult =
  run_script_worker(args->h, args->script, args->script_name, args->thread_idx);
...
static int script_39_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr,
  ...
  if (!TEST_true(
      ...
      || !TEST_true(
         WPACKET_quic_write_vlint(&wpkt, retire_prior_to)) /* retire prior to */
...
static int script_76_check(struct helper *h, struct helper_local *hl) {
  if (!TEST_false(SSL_shutdown_ex(
      h->c_conn, SSL_SHUTDOWN_FLAG_WAIT_PEER | SSL_SHUTDOWN_FLAG_NO_BLOCK, NULL,
      0)))
    return 0;
