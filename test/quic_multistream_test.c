...
static int check_rejected(struct helper *h, struct helper_local *hl)
{
    ...
    if (!ossl_quic_tserver_stream_has_peer_stop_sending(ACQUIRE_S(), stream_id,
                                                        NULL)
        || !ossl_quic_tserver_stream_has_peer_reset_stream(ACQUIRE_S(),
                                                           stream_id, NULL)) {
    ...
    for (;;) {
        ...
        case OPK_S_READ_EXPECT: {
            ...
            if (!TEST_true(ossl_quic_tserver_read(
                    ACQUIRE_S(), s_stream_id, tmp_buf + offset,
                    op->arg1 - offset, &bytes_read)))
                goto out;
        ...
        case OPK_C_EXPECT_CONN_CLOSE_INFO: {
            ...
            if (!TEST_int_eq(expect_app,
                ...
                || !TEST_int_eq(expect_remote,
                                (cc_info.flags & SSL_CONN_CLOSE_FLAG_LOCAL)
                                    == 0)
        ...
        case OPK_S_EXPECT_CONN_CLOSE_INFO: {
            ...
            if (!TEST_ptr(
                    tc = ossl_quic_tserver_get_terminate_cause(ACQUIRE_S())))
                goto out;
        ...
        case OPK_S_SET_INJECT_HANDSHAKE:
            ...
            if (!TEST_true(qtest_fault_set_handshake_listener(
                    h->qtf,
                    h->qtf_handshake_cb != NULL ? helper_handshake_listener
                                                : NULL,
                    h)))
                goto out;
        ...
        case OPK_S_SET_INJECT_DATAGRAM:
            ...
            if (!TEST_true(qtest_fault_set_datagram_listener(
                    h->qtf,
                    h->qtf_datagram_cb != NULL ? helper_datagram_listener
                                               : NULL,
                    h)))
                goto out;
    ...
    if (!testresult) {
        ...
        for (i = 0; i < repeat_stack_len; ++i)
            TEST_info(
                ...
                repeat_stack_done[i], repeat_stack_limit[i],
                repeat_stack_idx[i]);
...
static const struct script_op script_1[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        OP_C_WRITE(DEFAULT, "apple", 5) OP_C_CONCLUDE(DEFAULT)
            OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
                OP_S_EXPECT_FIN(a) OP_S_WRITE(a, "orange", 6) OP_S_CONCLUDE(a)
                    OP_C_READ_EXPECT(DEFAULT, "orange", 6)
                        OP_C_EXPECT_FIN(DEFAULT) OP_END};

/* 2. Multi-stream test */
static const struct script_op script_2[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_SET_INCOMING_STREAM_POLICY(
        SSL_INCOMING_STREAM_POLICY_ACCEPT) OP_C_WRITE(DEFAULT, "apple", 5)
        OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5) OP_S_WRITE(
            a, "orange",
            6) OP_C_READ_EXPECT(DEFAULT, "orange", 6)

            OP_C_NEW_STREAM_BIDI(
                b,
                C_BIDI_ID(
                    1)) OP_C_WRITE(b, "flamingo", 8) OP_C_CONCLUDE(b)
                OP_S_BIND_STREAM_ID(
                    b, C_BIDI_ID(1)) OP_S_READ_EXPECT(b, "flamingo", 8)
                    OP_S_EXPECT_FIN(b) OP_S_WRITE(b, "gargoyle", 8) OP_S_CONCLUDE(b) OP_C_READ_EXPECT(
                        b, "gargoyle", 8) OP_C_EXPECT_FIN(b)

                        OP_C_NEW_STREAM_UNI(
                            c,
                            C_UNI_ID(
                                0)) OP_C_WRITE(c, "elephant", 8) OP_C_CONCLUDE(c)
                            OP_S_BIND_STREAM_ID(
                                c,
                                C_UNI_ID(
                                    0)) OP_S_READ_EXPECT(c, "elephant", 8) OP_S_EXPECT_FIN(c) OP_S_WRITE_FAIL(c)

                                OP_C_ACCEPT_STREAM_NONE()

                                    OP_S_NEW_STREAM_BIDI(
                                        d,
                                        S_BIDI_ID(
                                            0)) OP_S_WRITE(d, "frog", 4) OP_S_CONCLUDE(d)

                                        OP_C_ACCEPT_STREAM_WAIT(d) OP_C_ACCEPT_STREAM_NONE() OP_C_READ_EXPECT(
                                            d, "frog", 4) OP_C_EXPECT_FIN(d)

                                            OP_S_NEW_STREAM_BIDI(e, S_BIDI_ID(1)) OP_S_WRITE(
                                                e, "mixture",
                                                7) OP_S_CONCLUDE(e)

                                                OP_C_ACCEPT_STREAM_WAIT(e) OP_C_READ_EXPECT(
                                                    e, "mixture",
                                                    7) OP_C_EXPECT_FIN(e)
                                                    OP_C_WRITE(e, "ramble", 6) OP_S_READ_EXPECT(
                                                        e,
                                                        "ramble", 6) OP_C_CONCLUDE(e) OP_S_EXPECT_FIN(e)

                                                        OP_S_NEW_STREAM_UNI(
                                                            f,
                                                            S_UNI_ID(
                                                                0)) OP_S_WRITE(f, "yonder", 6) OP_S_CONCLUDE(f)

                                                            OP_C_ACCEPT_STREAM_WAIT(f) OP_C_ACCEPT_STREAM_NONE() OP_C_READ_EXPECT(
                                                                f, "yonder",
                                                                6) OP_C_EXPECT_FIN(f) OP_C_WRITE_FAIL(f)
...
static const struct script_op script_3[] = {
    ...
    OP_C_WRITE_FAIL(DEFAULT)
        ...
        OP_C_WRITE(a, "by", 2)
            ...
            OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "appleby",
                                                                  7)

                OP_S_WRITE(a, "hello", 5) OP_C_READ_EXPECT(a, "hello", 5)

                    OP_C_WRITE_FAIL(DEFAULT) OP_C_ATTACH(a) OP_C_WRITE(
                        DEFAULT, "is here", 7) OP_S_READ_EXPECT(a, "is here", 7)
                    ...
                    OP_C_READ_FAIL(DEFAULT)
                        ...
                        OP_C_ACCEPT_STREAM_WAIT(a)
                            OP_C_READ_EXPECT(a, "apple", 5)
                                ...
                                OP_C_ATTACH(a) OP_C_WRITE(DEFAULT, "orange", 6)
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_AUTO_UNI)
            OP_C_WRITE(DEFAULT, "apple", 5)
                ...
                OP_S_BIND_STREAM_ID(a, C_UNI_ID(0))
                    OP_S_READ_EXPECT(a, "apple", 5) OP_S_WRITE_FAIL(a)
    ...
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_NEW_THREAD(5, script_11_child)
                ...
                OP_S_NEW_STREAM_BIDI(a, ANY_ID) OP_S_WRITE(a, "foo",
                                                           3) OP_S_CONCLUDE(a)

                    OP_S_NEW_STREAM_BIDI(b, ANY_ID) OP_S_WRITE(b, "foo", 3)
                        OP_S_CONCLUDE(b)

                            OP_S_NEW_STREAM_BIDI(c, ANY_ID)
                                OP_S_WRITE(c, "foo", 3) OP_S_CONCLUDE(c)

                                    OP_S_NEW_STREAM_BIDI(d, ANY_ID)
                                        OP_S_WRITE(d, "foo", 3) OP_S_CONCLUDE(d)
...
static const struct script_op script_12[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(
                ...
                OP_S_EXPECT_FIN(a) OP_S_BIND_STREAM_ID(b, C_BIDI_ID(1))
                    OP_S_READ_EXPECT(b, "foo", 3) OP_S_EXPECT_FIN(b)
                        OP_S_BIND_STREAM_ID(c, C_BIDI_ID(2))
                            OP_S_READ_EXPECT(c, "foo", 3) OP_S_EXPECT_FIN(c)
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_NEW_THREAD(5, script_13_child)
                ...
                OP_BEGIN_REPEAT(50)
                    ...
                    OP_S_NEW_STREAM_BIDI(a, ANY_ID) OP_S_WRITE(a, "foo", 3)
                        OP_S_CONCLUDE(a) OP_S_UNBIND_STREAM_ID(a)
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_NEW_THREAD(5, script_14_child)
                ...
                OP_BEGIN_REPEAT(50)
                    ...
                    OP_S_ACCEPT_STREAM_WAIT(a) OP_S_READ_EXPECT(a, "foo", 3)
                        OP_S_EXPECT_FIN(a) OP_S_UNBIND_STREAM_ID(a)
...
static const struct script_op script_15[] = {
    ...
    OP_S_NEW_STREAM_BIDI(a, S_BIDI_ID(0)) OP_S_WRITE(a, "bar", 3)
        OP_S_CONCLUDE(a)

            OP_C_ACCEPT_STREAM_WAIT(a) OP_C_READ_EXPECT(a, "bar", 3)
                OP_C_EXPECT_FIN(a)
    ...
    OP_BEGIN_REPEAT(200)
        ...
        OP_S_ACCEPT_STREAM_WAIT(b) OP_S_READ_EXPECT(b, "foo", 3)
            OP_S_EXPECT_FIN(b) OP_S_UNBIND_STREAM_ID(b)
...
static const struct script_op script_16[] = {
    ...
    OP_C_NEW_STREAM_BIDI(a, ANY_ID) OP_C_WRITE(a, "bar", 3) OP_C_CONCLUDE(a)
        ...
        OP_S_ACCEPT_STREAM_WAIT(b) OP_S_READ_EXPECT(b, "bar", 3)
            OP_S_EXPECT_FIN(b)

    /* Drain the queue of incoming streams. */
    OP_BEGIN_REPEAT(200)

        OP_C_ACCEPT_STREAM_WAIT(b) OP_C_READ_EXPECT(b, "foo", 3)
            OP_C_EXPECT_FIN(b) OP_C_FREE_STREAM(b)
    ...
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        ...
        OP_C_WRITE(DEFAULT, "apple", 5)
            ...
            OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
                ...
                OP_C_WRITE(DEFAULT, "orange", 6) OP_S_READ_EXPECT(a, "orange",
                                                                  6)

                    OP_S_WRITE(a, "strawberry",
                               10) OP_C_READ_EXPECT(DEFAULT, "strawberry", 10)
                    ...
                    OP_SET_INJECT_WORD(QUIC_PKT_TYPE_1RTT, OSSL_QUIC_VLINT_MAX)
                        ...
                        OP_S_WRITE(a, "orange", 6)
                            ...
                            OP_C_EXPECT_CONN_CLOSE_INFO(
                                OSSL_QUIC_ERR_FRAME_ENCODING_ERROR, 0, 0)
                ...
                OP_S_READ_EXPECT(a, "apple", 5)
                    ...
                    OP_SET_INJECT_WORD(1, 0)
                        ...
                        OP_S_WRITE(a, "orange", 6)
                            ...
                            OP_C_EXPECT_CONN_CLOSE_INFO(
                                OSSL_QUIC_ERR_PROTOCOL_VIOLATION, 0, 0)
                                  ...
                                  unsigned char *buf, size_t len)
{
    ...
    if (!TEST_true(WPACKET_quic_write_vlint(&wpkt, h->inject_word1))
        || !TEST_true(
            WPACKET_quic_write_vlint(&wpkt, (((uint64_t)1) << 60) + 1)))
        goto err;
...
static const struct script_op script_26[] = {
    OP_S_SET_INJECT_PLAIN(script_24_inject_plain) OP_C_SET_ALPN(
        "ossltest") OP_C_CONNECT_WAIT()
            ...
            OP_S_READ_EXPECT(a, "apple", 5)
                ...
                OP_SET_INJECT_WORD(1, OSSL_QUIC_FRAME_TYPE_STREAMS_BLOCKED_BIDI)
                    ...
                    OP_S_WRITE(a, "orange", 6)
                        ...
                        OP_C_EXPECT_CONN_CLOSE_INFO(
                            OSSL_QUIC_ERR_STREAM_LIMIT_ERROR, 0, 0)
...
static const struct script_op script_28[] = {
    OP_S_SET_INJECT_PLAIN(script_28_inject_plain) OP_C_SET_ALPN("ossltest")
        OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(
            SSL_DEFAULT_STREAM_MODE_NONE)

            OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "orange", 6)

                OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(
                    a, "orange", 6)

                    OP_C_NEW_STREAM_UNI(b, C_UNI_ID(0)) OP_C_WRITE(b, "apple",
                                                                   5)

                        OP_S_BIND_STREAM_ID(b, C_UNI_ID(0)) OP_S_READ_EXPECT(
                            b, "apple", 5)
                            ...
                            OP_SET_INJECT_WORD(
                                ...
                                OP_S_WRITE(a, "fruit", 5)
                                    ...
                                    OP_C_EXPECT_CONN_CLOSE_INFO(
                                        OSSL_QUIC_ERR_STREAM_STATE_ERROR, 0, 0)
            ...
            OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
                ...
                OP_S_NEW_STREAM_UNI(a, S_UNI_ID(0)) OP_S_WRITE(a, "apple", 5)
                    ...
                    OP_C_ACCEPT_STREAM_WAIT(a) OP_C_READ_EXPECT(a, "apple", 5)
                        ...
                        OP_SET_INJECT_WORD(S_UNI_ID(0) + 1,
            ...
            OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
                ...
                OP_S_NEW_STREAM_UNI(a, S_UNI_ID(0)) OP_S_WRITE(a, "apple", 5)
                    ...
                    OP_C_ACCEPT_STREAM_WAIT(a) OP_C_READ_EXPECT(a, "apple", 5)
                        ...
                        OP_SET_INJECT_WORD(C_UNI_ID(0) + 1,
                                           OSSL_QUIC_FRAME_TYPE_STOP_SENDING)
                            OP_S_WRITE(a, "orange", 6)
                ...
                OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5)
                    ...
                    OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0))
                        OP_S_READ_EXPECT(a, "apple", 5)
                            ...
                            OP_SET_INJECT_WORD(C_BIDI_ID(0) + 1,
                                               2) OP_S_WRITE(a, "orange", 6)
                        ...
                        OP_S_READ_EXPECT(a, "apple", 5)
                            ...
                            OP_SET_INJECT_WORD(C_BIDI_ID(0) + 1, 3)
                                OP_S_WRITE(a, "orange", 6)
                                    ...
                                    OP_C_EXPECT_CONN_CLOSE_INFO(
                                        OSSL_QUIC_ERR_FLOW_CONTROL_ERROR, 0, 0)
        ...
        OP_C_CONNECT_WAIT()
            OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
                ...
                OP_C_NEW_STREAM_UNI(a, C_UNI_ID(0)) OP_C_WRITE(a, "apple", 5)
                    ...
                    OP_S_BIND_STREAM_ID(a, C_UNI_ID(0)) OP_S_READ_EXPECT(
                        a, "apple", 5)

                        OP_S_NEW_STREAM_UNI(b, S_UNI_ID(0)) OP_SET_INJECT_WORD(
                            C_UNI_ID(0) + 1,
    ...
    OP_S_SET_INJECT_PLAIN(script_28_inject_plain) OP_C_SET_ALPN("ossltest")
        OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(
            ...
            OP_C_NEW_STREAM_UNI(a, C_UNI_ID(0)) OP_C_WRITE(a, "apple", 5)
                ...
                OP_S_BIND_STREAM_ID(a, C_UNI_ID(0)) OP_S_READ_EXPECT(a, "apple",
                                                                     5)
                    ...
                    OP_BEGIN_REPEAT(100)
                        ...
                        OP_C_WRITE(a, script_40_data, sizeof(script_40_data))
                            ...
                            OP_END_REPEAT()
                                ...
                                OP_C_CONCLUDE(a) OP_C_SHUTDOWN_WAIT(
                                    NULL, 0) /* disengages tick inhibition */
                            ...
                            const void *buf, size_t len, SSL *ssl, void *arg)
{
    ...
    if (version != OSSL_QUIC1_VERSION || content_type != SSL3_RT_QUIC_FRAME_FULL
        || len < 1)
        return;
                    ...
                    OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(
                        ...
                        OP_SET_INJECT_WORD(1, 0x100000 /* 1 MiB */)
                            OP_S_WRITE(a, "orange", 6)
                                ...
                                OP_C_EXPECT_CONN_CLOSE_INFO(
                                    OSSL_QUIC_ERR_CRYPTO_BUFFER_EXCEEDED, 0, 0)
                                  ...
                                  unsigned char *buf, size_t len)
{
    ...
    if (range_count > 0)
        if (!TEST_true(WPACKET_quic_write_vlint(&wpkt, /*range[0].gap=*/agap))
            || !TEST_true(
                WPACKET_quic_write_vlint(&wpkt, /*range[0].len=*/alen)))
            goto err;
...
static const struct script_op script_52[] = {
    OP_S_SET_INJECT_PLAIN(script_52_inject_plain) OP_C_SET_ALPN(
        "ossltest") OP_C_CONNECT_WAIT()

        OP_C_WRITE(DEFAULT, "apple", 5) OP_S_BIND_STREAM_ID(
            a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)

            OP_SET_INJECT_WORD(1, OSSL_QUIC_FRAME_TYPE_DATA_BLOCKED)

                OP_S_WRITE(a, "orange", 6) OP_C_READ_EXPECT(DEFAULT, "orange",
                                                            6)

                    OP_C_WRITE(DEFAULT, "Strawberry",
                               10) OP_S_READ_EXPECT(a, "Strawberry", 10)

                        OP_SET_INJECT_WORD(
                            1, OSSL_QUIC_FRAME_TYPE_STREAM_DATA_BLOCKED)

                            OP_S_WRITE(a, "orange",
                                       6) OP_C_READ_EXPECT(DEFAULT, "orange", 6)

                                OP_C_WRITE(DEFAULT, "Strawberry",
                                           10) OP_S_READ_EXPECT(a, "Strawberry",
                                                                10)
                                      ...
                                      size_t buf_len)
{
    ...
    for (i = 0; i < buf_len; ++i) buf[i] ^= 0xff;
...
static const struct script_op script_54[] = {
    OP_S_SET_INJECT_HANDSHAKE(script_54_inject_handshake)
        OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT_OR_FAIL()
            ...
            OP_C_EXPECT_CONN_CLOSE_INFO(OSSL_QUIC_ERR_CRYPTO_UNEXPECTED_MESSAGE,
                                        0, 0)
...
static int check_shutdown_reason(struct helper *h, struct helper_local *hl)
{
    ...
    if (!TEST_size_t_ge(tc->reason_len, 50)
        || !TEST_mem_eq(long_reason, tc->reason_len, tc->reason,
                        tc->reason_len))
        return 0;
            ...
            OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
                ...
                OP_S_NEW_STREAM_BIDI(a, S_BIDI_ID(0)) OP_S_WRITE(a, "apple", 5)
                    ...
                    OP_C_ACCEPT_STREAM_WAIT(a) OP_C_READ_EXPECT(a, "apple", 5)
                        ...
                        OP_SET_INJECT_WORD(S_BIDI_ID(0) + 1,
                                           OSSL_QUIC_FRAME_TYPE_MAX_STREAM_DATA)
                            OP_S_WRITE(a, "orange", 6)
                                OP_C_READ_EXPECT(a, "orange", 6)
                                    OP_C_WRITE(a, "Strawberry", 10)
                ...
                OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5)
                    OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0))
                        OP_S_READ_EXPECT(a, "apple", 5)
                            ...
                            OP_SET_INJECT_WORD(1, 0) OP_S_NEW_TICKET()
                                OP_S_WRITE(a, "orange", 6)
                ...
                OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5)
                    OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0))
                        OP_S_READ_EXPECT(a, "apple", 5)
                            ...
                            OP_SET_INJECT_WORD(2, 0) OP_S_NEW_TICKET()
                                OP_S_WRITE(a, "orange", 6)
                ...
                OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0))
                    OP_S_READ_EXPECT(a, "apple", 5)
                        ...
                        OP_CHECK(set_max_early_data, 0xfffffffe)
                            OP_S_NEW_TICKET() OP_S_WRITE(a, "orange", 6)
...
static const struct script_op script_72[] = {
    ...
    OP_BEGIN_REPEAT(200)
        ...
        OP_C_NEW_STREAM_BIDI_EX(a, ANY_ID,
                                ALLOW_FAIL | SSL_STREAM_FLAG_NO_BLOCK)
            OP_C_SKIP_IF_UNBOUND(a, 2) OP_C_WRITE(a, "apple", 5)
                OP_C_FREE_STREAM(a)
                ...
                OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(
                    ...
                    OP_S_WRITE(a, "orange", 6) OP_C_READ_EXPECT(a, "orange", 6)
                        ...
                        OP_CHECK(trigger_late_session_ticket, 0)
                            ...
                            OP_S_WRITE(a, "Strawberry",
                                       10) OP_C_READ_EXPECT(a, "Strawberry", 10)

                                OP_CHECK(check_got_session_ticket, 0) OP_CHECK2(
                                    check_idle_timeout,
...
static const struct script_op script_80[] = {
    OP_S_SET_INJECT_PLAIN(script_80_inject_pkt) OP_C_SET_ALPN("ossltest")
        OP_C_CONNECT_WAIT() OP_C_WRITE(DEFAULT, "apple", 5)
            OP_C_CONCLUDE(DEFAULT) OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0))
                OP_S_READ_EXPECT(a, "apple", 5) OP_SET_INJECT_WORD(1, 0)
                    OP_S_WRITE(a, "apple", 5)
                        OP_C_READ_EXPECT(DEFAULT, "apple", 5)
                            OP_SET_INJECT_WORD(0, 1) OP_S_WRITE(a, "apple", 5)
...
static const struct script_op script_82[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_CHECK2(check_idle_timeout, SSL_VALUE_CLASS_FEATURE_PEER_REQUEST,
                      30000)
                OP_CHECK2(check_idle_timeout,
                          SSL_VALUE_CLASS_FEATURE_NEGOTIATED, 30000)
...
static const struct script_op script_83[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_CHECK(cannot_change_idle_timeout, 0) OP_CHECK2(
                check_idle_timeout, SSL_VALUE_CLASS_FEATURE_PEER_REQUEST, 30000)
                OP_CHECK2(check_idle_timeout,
                          SSL_VALUE_CLASS_FEATURE_NEGOTIATED, 30000)
...
static const struct script_op script_84[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_CHECK2(check_avail_streams, 0, 100) OP_CHECK2(
                check_avail_streams, 1,
                100) OP_CHECK2(check_avail_streams, 2,
                               100) OP_CHECK2(check_avail_streams, 3, 100)
                ...
                OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0))
                    ...
                    OP_CHECK2(check_avail_streams, 0, 99) OP_CHECK2(
                        check_avail_streams, 1,
                        100) OP_CHECK2(check_avail_streams, 2,
                                       100) OP_CHECK2(check_avail_streams, 3,
                                                      100)
                        ...
                        OP_C_NEW_STREAM_UNI(b, C_UNI_ID(0))
                            ...
                            OP_CHECK2(check_avail_streams, 0, 99) OP_CHECK2(
                                check_avail_streams, 1,
                                100) OP_CHECK2(check_avail_streams, 2, 99) OP_CHECK2(check_avail_streams, 3, 100)

                                OP_S_NEW_STREAM_BIDI(
                                    c, S_BIDI_ID(0)) OP_S_WRITE(c, "x", 1)

                                    OP_C_ACCEPT_STREAM_WAIT(
                                        c) OP_C_READ_EXPECT(c, "x", 1)

                                        OP_CHECK2(check_avail_streams, 0, 99) OP_CHECK2(check_avail_streams, 1, 99) OP_CHECK2(
                                            check_avail_streams,
                                            2,
                                                    ...
                                                    OP_CHECK2(check_avail_streams, 0, 99) OP_CHECK2(
                                                        ...
                                                        OP_CHECK2(
                                                            ...
                                                            OP_CHECK2(
                                                                ...
                                                                0, 0) OP_CHECK(set_event_handling_mode_conn,
...
static const struct script_op script_85[] = {
                ...
                OP_C_NEW_STREAM_BIDI(b, C_BIDI_ID(1)) OP_C_WRITE(b, "orange", 6)
                    ...
                    OP_C_NEW_STREAM_BIDI(c, C_BIDI_ID(2))
                        OP_C_WRITE(c, "Strawberry", 10)
                            ...
                            OP_C_NEW_STREAM_BIDI(d, C_BIDI_ID(3)) OP_C_WRITE(
                                d, "sync", 4)
    ...
    /* Ensure sync. */
    OP_S_READ_EXPECT(d, "sync", 4) OP_S_WRITE(d, "x", 1)
...
static const struct script_op script_86[] = {
    ...
    OP_CHECK(set_event_handling_mode_stream,
             SSL_VALUE_EVENT_HANDLING_MODE_EXPLICIT) OP_C_WRITE(a, "orange", 6)
        OP_C_CONCLUDE(a)
    ...
    OP_CHECK(reenable_test_event_handling, 0)
        OP_S_READ_EXPECT(a, "appleorange", 11) OP_S_EXPECT_FIN(a)

    /* Back into explicit mode. */
    OP_CHECK(set_event_handling_mode_conn,
             SSL_VALUE_EVENT_HANDLING_MODE_EXPLICIT) OP_S_WRITE(a, "ok", 2)
        OP_C_READ_FAIL(a)
...
static const struct script_op script_87[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5)
            OP_C_CONCLUDE(a) OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0))
                OP_S_READ_EXPECT(a, "apple", 5) OP_S_EXPECT_FIN(a)
                    OP_S_WRITE(a, "orange", 6) OP_C_READ_EXPECT(a, "orange", 6)
                        OP_S_CONCLUDE(a) OP_C_EXPECT_FIN(a) OP_SLEEP(1000)
                            OP_C_STREAM_RESET_FAIL(a, 42) OP_END};
