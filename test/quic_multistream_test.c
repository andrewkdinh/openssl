...
struct helper {
    ...
    int (*qtf_packet_plain_cb)(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t buf_len);
    int (*qtf_handshake_cb)(struct helper *h, unsigned char *buf, size_t buf_len);
    int (*qtf_datagram_cb)(struct helper *h, BIO_MSG *m, size_t stride);
...
{
    return (thread_idx < 0) ? &h->s_checked_out : &h->threads[thread_idx].s_checked_out;
}
...
static int helper_init(struct helper *h, const char *script_name, int free_order, int blocking, int need_injector)
{
    ...
    if (!TEST_ptr(h->s_streams = lh_STREAM_INFO_new(stream_info_hash, stream_info_cmp)))
        goto err;

    if (!TEST_ptr(h->c_streams = lh_STREAM_INFO_new(stream_info_hash, stream_info_cmp)))
        goto err;
    ...
    if (!TEST_ptr(h->s_net_bio_orig_addr = BIO_ADDR_new()) || !TEST_ptr(h->s_net_bio_addr = BIO_ADDR_new()))
        goto err;

    if (!TEST_true(BIO_ADDR_rawmake(h->s_net_bio_orig_addr, AF_INET, &ina, sizeof(ina), 0)))
        goto err;
    ...
    if (h->blocking) {
        ...
        h->server_thread.t = ossl_crypto_thread_native_start(server_helper_thread, h, 1);
...
static int helper_local_init(struct helper_local *hl, struct helper *h, int thread_idx)
{
    hl->h = h;
    hl->c_streams = NULL;
    ...
    if (thread_idx < 0) {
        ...
        if (!TEST_ptr(hl->c_streams = lh_STREAM_INFO_new(stream_info_hash, stream_info_cmp)))
            return 0;
    }
...
static STREAM_INFO *get_stream_info(LHASH_OF(STREAM_INFO) * lh, const char *stream_name)
{
...
static int helper_local_set_c_stream(struct helper_local *hl, const char *stream_name, SSL *c_stream)
{
...
static int helper_set_s_stream(struct helper *h, const char *stream_name, uint64_t s_stream_id)
{
    STREAM_INFO *info;
...
static int helper_packet_plain_listener(QTEST_FAULT *qtf, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t buf_len,
                                        void *arg)
{
    struct helper *h = arg;
...
static int helper_handshake_listener(QTEST_FAULT *fault, unsigned char *buf, size_t buf_len, void *arg)
{
    struct helper *h = arg;
...
static int helper_datagram_listener(QTEST_FAULT *fault, BIO_MSG *msg, size_t stride, void *arg)
{
    struct helper *h = arg;
...
static int run_script_worker(struct helper *h, const struct script_op *script, const char *script_name, int thread_idx)
{
    int testresult = 0;
...
#define C_SPIN_AGAIN()                                  \
    ...
    for (;;) {
        ...
        if (!hl->explicit_event_handling && (thread_idx >= 0 || connect_started))
        ...
        if (thread_idx >= 0) {
            ...
            default:
                TEST_error("opcode %lu not allowed on child thread", (unsigned long)op->op);
                goto out;
            }
        }
        ...
        case OPK_END_REPEAT:
            ...
            if (++repeat_stack_done[repeat_stack_len - 1] == repeat_stack_limit[repeat_stack_len - 1]) {
                --repeat_stack_len;
            } else {
        ...
        case OPK_C_SET_ALPN: {
            ...
            if (!TEST_false(SSL_set_alpn_protos(h->c_conn, tmp_buf, alpn_len + 1)))
                goto out;
        ...
        case OPK_C_WRITE: {
            ...
            if (!TEST_true(r) || !check_consistent_want(c_tgt, r) || !TEST_size_t_eq(bytes_written, op->arg1))
                goto out;
        } break;
        ...
        case OPK_C_WRITE_EX2: {
            ...
            r = SSL_write_ex2(c_tgt, op->arg0, op->arg1, op->arg2, &bytes_written);
            if (!TEST_true(r) || !check_consistent_want(c_tgt, r) || !TEST_size_t_eq(bytes_written, op->arg1))
                goto out;
        } break;
        ...
        case OPK_S_WRITE: {
            ...
            if (!TEST_true(ossl_quic_tserver_write(ACQUIRE_S(), s_stream_id, op->arg0, op->arg1, &bytes_written))
        ...
        case OPK_C_WAIT_FOR_DATA: {
            ...
            if (!SSL_peek_ex(c_tgt, buf, sizeof(buf), &bytes_read) || bytes_read == 0)
                C_SPIN_AGAIN();
        } break;
        ...
        case OPK_C_READ_EXPECT: {
            ...
            if (op->arg1 > 0 && tmp_buf == NULL && !TEST_ptr(tmp_buf = OPENSSL_malloc(op->arg1)))
                goto out;

            r = SSL_read_ex(c_tgt, tmp_buf + offset, op->arg1 - offset, &bytes_read);
            ...
            if (op->arg1 > 0 && !TEST_mem_eq(tmp_buf, op->arg1, op->arg0, op->arg1))
                goto out;
        ...
        case OPK_C_EXPECT_FIN: {
            ...
            if (!check_consistent_want(c_tgt, r) || !TEST_false(r) || !TEST_size_t_eq(bytes_read, 0))
                goto out;

            if (is_want(c_tgt, 0))
                C_SPIN_AGAIN();

            if (!TEST_int_eq(SSL_get_error(c_tgt, 0), SSL_ERROR_ZERO_RETURN))
                goto out;
        ...
        case OPK_C_NEW_STREAM: {
            ...
            if (allow_fail && c_stream == NULL) {
                if (!TEST_size_t_eq(ERR_GET_REASON(ERR_get_error()), SSL_R_STREAM_COUNT_LIMITED))
                    goto out;
            ...
            if (op->arg2 != UINT64_MAX && !TEST_uint64_t_eq(SSL_get_stream_id(c_stream), op->arg2))
                goto out;
        ...
        case OPK_S_NEW_STREAM: {
            ...
            if (!TEST_true(ossl_quic_tserver_stream_new(ACQUIRE_S(), op->arg1 > 0, &stream_id)))
                goto out;

            if (op->arg2 != UINT64_MAX && !TEST_uint64_t_eq(stream_id, op->arg2))
                goto out;

            if (!TEST_true(helper_set_s_stream(h, op->stream_name, stream_id)))
                goto out;
        } break;
        ...
        case OPK_C_ACCEPT_STREAM_NONE: {
            ...
            if (!TEST_ptr_null(c_stream = SSL_accept_stream(h->c_conn, SSL_ACCEPT_STREAM_NO_BLOCK))) {
                SSL_free(c_stream);
                goto out;
            }
        } break;
        ...
        case OPK_C_FREE_STREAM: {
            if (!TEST_ptr(c_tgt) || !TEST_true(!SSL_is_connection(c_tgt)))
                goto out;
        ...
        case OPK_C_SET_INCOMING_STREAM_POLICY: {
            ...
            if (!TEST_true(SSL_set_incoming_stream_policy(c_tgt, op->arg1, 0)))
                goto out;
        } break;
        ...
        case OPK_C_EXPECT_CONN_CLOSE_INFO: {
            ...
            if (h->blocking && !TEST_true(SSL_shutdown_ex(c_tgt, SSL_SHUTDOWN_FLAG_WAIT_PEER, NULL, 0)))
                goto out;

            if (!SSL_get_conn_close_info(c_tgt, &cc_info, sizeof(cc_info)))
                C_SPIN_AGAIN();

            if (!TEST_int_eq(expect_app, (cc_info.flags & SSL_CONN_CLOSE_FLAG_TRANSPORT) == 0)
                || !TEST_int_eq(expect_remote, (cc_info.flags & SSL_CONN_CLOSE_FLAG_LOCAL) == 0)
        ...
        case OPK_S_EXPECT_CONN_CLOSE_INFO: {
            ...
            if (!TEST_uint64_t_eq(error_code, tc->error_code) || !TEST_int_eq(expect_app, tc->app)
        ...
        case OPK_C_WRITE_FAIL: {
            ...
            if (!TEST_false(r) || !check_consistent_want(c_tgt, r))
                goto out;
        } break;
        ...
        case OPK_S_WRITE_FAIL: {
            ...
            if (!TEST_false(ossl_quic_tserver_write(ACQUIRE_S(), s_stream_id, (const unsigned char *)"apple", 5,
                                                    &bytes_written)))
                goto out;
        } break;
        ...
        case OPK_S_READ_FAIL: {
            ...
            ret = ossl_quic_tserver_read(ACQUIRE_S(), s_stream_id, buf, sizeof(buf), &bytes_read);
        ...
        case OPK_NEW_THREAD: {
            ...
            for (i = 0; i < op->arg1; ++i) {
                ...
                h->threads[i].t = ossl_crypto_thread_native_start(run_script_child_thread, &h->threads[i], 1);
        ...
        case OPK_S_SET_INJECT_PLAIN:
            ...
            if (!TEST_true(qtest_fault_set_packet_plain_listener(
                    h->qtf, h->qtf_packet_plain_cb != NULL ? helper_packet_plain_listener : NULL, h)))
                goto out;
        ...
        case OPK_S_SET_INJECT_HANDSHAKE:
            ...
            if (!TEST_true(qtest_fault_set_handshake_listener(
                    h->qtf, h->qtf_handshake_cb != NULL ? helper_handshake_listener : NULL, h)))
                goto out;
        ...
        case OPK_S_SET_INJECT_DATAGRAM:
            ...
            if (!TEST_true(qtest_fault_set_datagram_listener(
                    h->qtf, h->qtf_datagram_cb != NULL ? helper_datagram_listener : NULL, h)))
                goto out;
        ...
        case OPK_C_INHIBIT_TICK: {
            ...
            ossl_quic_engine_set_inhibit_tick(ossl_quic_channel_get0_engine(ch), op->arg1);
        } break;
...
out:
    ...
    if (!testresult) {
        ...
        for (i = 0; i < repeat_stack_len; ++i)
            TEST_info("while repeating, iteration %zu of %zu, starting at script op %zu", repeat_stack_done[i],
                      repeat_stack_limit[i], repeat_stack_idx[i]);
        ...
        if (h->c_conn != NULL) {
            ...
            if (SSL_get_conn_close_info(h->c_conn, &cc_info, sizeof(cc_info))) {
                ...
                TEST_info("client side is closed: %llu(%s)/%llu(%s), "
                          "%s, %s, reason: \"%s\"",
                          (unsigned long long)cc_info.error_code, e_str, (unsigned long long)cc_info.frame_type, f_str,
        ...
        tcause = (h->s != NULL ? ossl_quic_tserver_get_terminate_cause(h->s) : NULL);
        if (tcause != NULL) {
            ...
            TEST_info("server side is closed: %llu(%s)/%llu(%s), "
                      "%s, %s, reason: \"%s\"",
                      (unsigned long long)tcause->error_code, e_str, (unsigned long long)tcause->frame_type, f_str,
                      tcause->remote ? "remote" : "local", tcause->app ? "app" : "transport",
...
static int run_script(const struct script_op *script, const char *script_name, int free_order, int blocking)
{
    int testresult = 0;
    struct helper h;

    if (!TEST_true(helper_init(&h, script_name, free_order, blocking, 1)))
        goto out;
...
static CRYPTO_THREAD_RETVAL run_script_child_thread(void *arg)
{
    ...
    testresult = run_script_worker(args->h, args->script, args->script_name, args->thread_idx);
...
static const struct script_op script_1[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_WRITE(DEFAULT, "apple", 5) OP_C_CONCLUDE(DEFAULT)
        OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5) OP_S_EXPECT_FIN(a)
            OP_S_WRITE(a, "orange", 6) OP_S_CONCLUDE(a) OP_C_READ_EXPECT(DEFAULT, "orange", 6) OP_C_EXPECT_FIN(DEFAULT)
                OP_END};

/* 2. Multi-stream test */
static const struct script_op script_2[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_SET_INCOMING_STREAM_POLICY(SSL_INCOMING_STREAM_POLICY_ACCEPT)
        OP_C_WRITE(DEFAULT, "apple", 5) OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5) OP_S_WRITE(
            a, "orange", 6) OP_C_READ_EXPECT(DEFAULT, "orange", 6)

            OP_C_NEW_STREAM_BIDI(b, C_BIDI_ID(1)) OP_C_WRITE(b, "flamingo", 8) OP_C_CONCLUDE(b) OP_S_BIND_STREAM_ID(
                b, C_BIDI_ID(1)) OP_S_READ_EXPECT(b, "flamingo", 8) OP_S_EXPECT_FIN(b) OP_S_WRITE(b, "gargoyle", 8)
                OP_S_CONCLUDE(b) OP_C_READ_EXPECT(b, "gargoyle", 8) OP_C_EXPECT_FIN(b)

                    OP_C_NEW_STREAM_UNI(c, C_UNI_ID(0)) OP_C_WRITE(c, "elephant", 8) OP_C_CONCLUDE(c)
                        OP_S_BIND_STREAM_ID(c, C_UNI_ID(0)) OP_S_READ_EXPECT(c, "elephant",
                                                                             8) OP_S_EXPECT_FIN(c) OP_S_WRITE_FAIL(c)

                            OP_C_ACCEPT_STREAM_NONE()

                                OP_S_NEW_STREAM_BIDI(d, S_BIDI_ID(0)) OP_S_WRITE(d, "frog", 4) OP_S_CONCLUDE(d)

                                    OP_C_ACCEPT_STREAM_WAIT(d) OP_C_ACCEPT_STREAM_NONE() OP_C_READ_EXPECT(
                                        d, "frog", 4) OP_C_EXPECT_FIN(d)

                                        OP_S_NEW_STREAM_BIDI(e, S_BIDI_ID(1)) OP_S_WRITE(e, "mixture",
                                                                                         7) OP_S_CONCLUDE(e)

                                            OP_C_ACCEPT_STREAM_WAIT(e) OP_C_READ_EXPECT(e, "mixture", 7)
                                                OP_C_EXPECT_FIN(e) OP_C_WRITE(e, "ramble", 6) OP_S_READ_EXPECT(
                                                    e, "ramble", 6) OP_C_CONCLUDE(e) OP_S_EXPECT_FIN(e)

                                                    OP_S_NEW_STREAM_UNI(
                                                        f, S_UNI_ID(0)) OP_S_WRITE(f, "yonder", 6) OP_S_CONCLUDE(f)

                                                        OP_C_ACCEPT_STREAM_WAIT(f) OP_C_ACCEPT_STREAM_NONE()
                                                            OP_C_READ_EXPECT(f, "yonder", 6) OP_C_EXPECT_FIN(
                                                                f) OP_C_WRITE_FAIL(f)

                                                                OP_C_SET_INCOMING_STREAM_POLICY(
                                                                    SSL_INCOMING_STREAM_POLICY_REJECT)
                                                                    OP_S_NEW_STREAM_BIDI(g, S_BIDI_ID(2)) OP_S_WRITE(
                                                                        g, "unseen", 6) OP_S_CONCLUDE(g)
     ...
     */
    OP_CHECK(check_rejected, S_BIDI_ID(2)) OP_CHECK(check_rejected, S_BIDI_ID(3))

        OP_END};

/* 3. Default stream detach/reattach test */
static const struct script_op script_3[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()

        OP_C_WRITE(DEFAULT, "apple", 5) OP_C_DETACH(a)             /* DEFAULT becomes stream 'a' */
    OP_C_WRITE_FAIL(DEFAULT)

        OP_C_WRITE(a, "by", 2)

            OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "appleby", 7)

                OP_S_WRITE(a, "hello", 5) OP_C_READ_EXPECT(a, "hello", 5)

                    OP_C_WRITE_FAIL(DEFAULT) OP_C_ATTACH(a) OP_C_WRITE(DEFAULT, "is here", 7)
                        OP_S_READ_EXPECT(a, "is here", 7)

                            OP_C_DETACH(a) OP_C_CONCLUDE(a) OP_S_EXPECT_FIN(a)
...
static const struct script_op script_4[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE) OP_C_WRITE_FAIL(DEFAULT)

            OP_S_NEW_STREAM_BIDI(a, S_BIDI_ID(0)) OP_S_WRITE(a, "apple", 5)

                OP_C_READ_FAIL(DEFAULT)

                    OP_C_ACCEPT_STREAM_WAIT(a) OP_C_READ_EXPECT(a, "apple", 5)

                        OP_C_ATTACH(a) OP_C_WRITE(DEFAULT, "orange", 6) OP_S_READ_EXPECT(a, "orange", 6)
...
static const struct script_op script_5[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE) OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0))
            OP_C_NEW_STREAM_BIDI(b, C_BIDI_ID(1))

                OP_C_WRITE(a, "apple", 5) OP_C_STREAM_RESET(a, 42)

                    OP_C_WRITE(b, "strawberry", 10)

                        OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_BIND_STREAM_ID(b, C_BIDI_ID(1))
                            OP_S_READ_EXPECT(b, "strawberry", 10)
    /* Reset disrupts read of already sent data */
    OP_S_READ_FAIL(a, 0) OP_CHECK(check_stream_reset, C_BIDI_ID(0))

        OP_END};

/* 6. Test STOP_SENDING functionality */
static const struct script_op script_6[] = {OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()

                                                OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
                                                    OP_S_NEW_STREAM_BIDI(a, S_BIDI_ID(0)) OP_S_WRITE(a, "apple", 5)

                                                        OP_C_ACCEPT_STREAM_WAIT(a) OP_C_FREE_STREAM(a)
                                                            OP_C_ACCEPT_STREAM_NONE()
...
static const struct script_op script_7[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_AUTO_UNI) OP_C_WRITE(DEFAULT, "apple", 5)

            OP_S_BIND_STREAM_ID(a, C_UNI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5) OP_S_WRITE_FAIL(a)

                OP_END};

/* 8. Unidirectional default stream mode test (server sends first) */
static const struct script_op script_8[] = {OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()

                                                OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_AUTO_UNI)
                                                    OP_S_NEW_STREAM_UNI(a, S_UNI_ID(0)) OP_S_WRITE(a, "apple", 5)
                                                        OP_C_READ_EXPECT(DEFAULT, "apple", 5) OP_C_WRITE_FAIL(DEFAULT)
...
static const struct script_op script_9[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_AUTO_UNI) OP_S_NEW_STREAM_BIDI(a, S_BIDI_ID(0))
            OP_S_WRITE(a, "apple", 5) OP_C_READ_EXPECT(DEFAULT, "apple", 5) OP_C_WRITE(DEFAULT, "orange", 6)
...
static const struct script_op script_10[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        ...
        OP_C_WRITE(DEFAULT, "apple", 5) OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)

            OP_C_SHUTDOWN_WAIT(NULL, 0) OP_C_EXPECT_CONN_CLOSE_INFO(0, 1, 0) OP_S_EXPECT_CONN_CLOSE_INFO(0, 1, 1)

                OP_END};

/* 11. Many threads accepted on the same client connection */
static const struct script_op script_11_child[] = {OP_C_ACCEPT_STREAM_WAIT(a) OP_C_READ_EXPECT(a, "foo", 3) OP_SLEEP(10)
                                                       OP_C_EXPECT_FIN(a)

                                                           OP_END};

static const struct script_op script_11[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)

        OP_NEW_THREAD(5, script_11_child)

            OP_S_NEW_STREAM_BIDI(a, ANY_ID) OP_S_WRITE(a, "foo", 3) OP_S_CONCLUDE(a)

                OP_S_NEW_STREAM_BIDI(b, ANY_ID) OP_S_WRITE(b, "foo", 3) OP_S_CONCLUDE(b)

                    OP_S_NEW_STREAM_BIDI(c, ANY_ID) OP_S_WRITE(c, "foo", 3) OP_S_CONCLUDE(c)

                        OP_S_NEW_STREAM_BIDI(d, ANY_ID) OP_S_WRITE(d, "foo", 3) OP_S_CONCLUDE(d)

                            OP_S_NEW_STREAM_BIDI(e, ANY_ID) OP_S_WRITE(e, "foo", 3) OP_S_CONCLUDE(e)

                                OP_END};

/* 12. Many threads initiated on the same client connection */
static const struct script_op script_12_child[] = {OP_C_NEW_STREAM_BIDI(a, ANY_ID) OP_C_WRITE(a, "foo", 3)
                                                       OP_C_CONCLUDE(a) OP_C_FREE_STREAM(a)
...
static const struct script_op script_12[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
        ...
        OP_NEW_THREAD(5, script_12_child)
            ...
            OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "foo", 3) OP_S_EXPECT_FIN(a)
                OP_S_BIND_STREAM_ID(b, C_BIDI_ID(1)) OP_S_READ_EXPECT(b, "foo", 3) OP_S_EXPECT_FIN(b)
                    OP_S_BIND_STREAM_ID(c, C_BIDI_ID(2)) OP_S_READ_EXPECT(c, "foo", 3) OP_S_EXPECT_FIN(c)
                        OP_S_BIND_STREAM_ID(d, C_BIDI_ID(3)) OP_S_READ_EXPECT(d, "foo", 3) OP_S_EXPECT_FIN(d)
                            OP_S_BIND_STREAM_ID(e, C_BIDI_ID(4)) OP_S_READ_EXPECT(e, "foo", 3) OP_S_EXPECT_FIN(e)

                                OP_END};

/* 13. Many threads accepted on the same client connection (stress test) */
static const struct script_op script_13_child[] = {OP_BEGIN_REPEAT(10)

                                                       OP_C_ACCEPT_STREAM_WAIT(a) OP_C_READ_EXPECT(a, "foo", 3)
                                                           OP_C_EXPECT_FIN(a) OP_C_FREE_STREAM(a)
    ...
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
        ...
        OP_NEW_THREAD(5, script_13_child)
            ...
            OP_BEGIN_REPEAT(50)
                ...
                OP_S_NEW_STREAM_BIDI(a, ANY_ID) OP_S_WRITE(a, "foo", 3) OP_S_CONCLUDE(a) OP_S_UNBIND_STREAM_ID(a)
...
static const struct script_op script_14_child[] = {OP_BEGIN_REPEAT(10)

                                                       OP_C_NEW_STREAM_BIDI(a, ANY_ID) OP_C_WRITE(a, "foo", 3)
                                                           OP_C_CONCLUDE(a) OP_C_FREE_STREAM(a)
    ...
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
        ...
        OP_NEW_THREAD(5, script_14_child)
            ...
            OP_BEGIN_REPEAT(50)
                ...
                OP_S_ACCEPT_STREAM_WAIT(a) OP_S_READ_EXPECT(a, "foo", 3) OP_S_EXPECT_FIN(a) OP_S_UNBIND_STREAM_ID(a)
...
static const struct script_op script_15[] = {
    ...
    OP_BEGIN_REPEAT(200)
        ...
        OP_C_NEW_STREAM_BIDI_EX(a, ANY_ID, SSL_STREAM_FLAG_ADVANCE) OP_C_WRITE(a, "foo", 3) OP_C_CONCLUDE(a)
            OP_C_FREE_STREAM(a)

                OP_END_REPEAT()

    /* Prove the connection is still good. */
    OP_S_NEW_STREAM_BIDI(a, S_BIDI_ID(0)) OP_S_WRITE(a, "bar", 3) OP_S_CONCLUDE(a)

        OP_C_ACCEPT_STREAM_WAIT(a) OP_C_READ_EXPECT(a, "bar", 3) OP_C_EXPECT_FIN(a)
    ...
    OP_BEGIN_REPEAT(200)
        ...
        OP_S_ACCEPT_STREAM_WAIT(b) OP_S_READ_EXPECT(b, "foo", 3) OP_S_EXPECT_FIN(b) OP_S_UNBIND_STREAM_ID(b)
...
static const struct script_op script_16[] = {
    ...
    OP_C_NEW_STREAM_BIDI(a, ANY_ID) OP_C_WRITE(a, "bar", 3) OP_C_CONCLUDE(a)

        OP_S_ACCEPT_STREAM_WAIT(b) OP_S_READ_EXPECT(b, "bar", 3) OP_S_EXPECT_FIN(b)

    /* Drain the queue of incoming streams. */
    OP_BEGIN_REPEAT(200)

        OP_C_ACCEPT_STREAM_WAIT(b) OP_C_READ_EXPECT(b, "foo", 3) OP_C_EXPECT_FIN(b) OP_C_FREE_STREAM(b)
...
static const struct script_op script_17[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        ...
        OP_C_WRITE(DEFAULT, "apple", 5)
            ...
            OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)

                OP_CHECK(override_key_update, 1)

                    OP_BEGIN_REPEAT(200)

                        OP_C_WRITE(DEFAULT, "apple", 5) OP_S_READ_EXPECT(a, "apple", 5)
     ...
     * directions.
     */
    OP_C_WRITE(DEFAULT, "xyzzy", 5) OP_S_READ_EXPECT(a, "xyzzy", 5)

        OP_S_WRITE(a, "plugh", 5) OP_C_READ_EXPECT(DEFAULT, "plugh", 5)
            ...
            OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
                ...
                OP_CHECK(override_key_update, 1)
                    ...
                    OP_BEGIN_REPEAT(200)
                        ...
                        OP_C_WRITE(DEFAULT, "apple", 5) OP_S_READ_EXPECT(a, "apple", 5) OP_CHECK(skip_time_ms, 8)
    ...
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        ...
        OP_C_WRITE(DEFAULT, "apple", 5)
            ...
            OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
                ...
                OP_C_WRITE(DEFAULT, "orange", 6) OP_S_READ_EXPECT(a, "orange", 6)

                    OP_S_WRITE(a, "strawberry", 10) OP_C_READ_EXPECT(DEFAULT, "strawberry", 10)

                        OP_CHECK(check_key_update_lt, 1) OP_CHECK(trigger_key_update, 0)

                            OP_C_WRITE(DEFAULT, "orange", 6) OP_S_READ_EXPECT(a, "orange", 6) OP_S_WRITE(a, "ok", 2)

                                OP_C_READ_EXPECT(DEFAULT, "ok", 2) OP_CHECK(check_key_update_ge, 1)
...
static const struct script_op script_20_child[] = {
    OP_C_ACCEPT_STREAM_WAIT(a) OP_C_READ_EXPECT(a, "foo", 3)
        ...
        OP_CHECK(script_20_trigger1, 0) OP_CHECK(script_20_wait2, 1)

            OP_C_READ_FAIL_WAIT(a) OP_C_EXPECT_SSL_ERR(a, SSL_ERROR_SYSCALL)

                OP_EXPECT_ERR_LIB(ERR_LIB_SSL) OP_EXPECT_ERR_REASON(SSL_R_PROTOCOL_IS_SHUTDOWN)

                    OP_POP_ERR() OP_EXPECT_ERR_LIB(ERR_LIB_SSL) OP_EXPECT_ERR_REASON(SSL_R_QUIC_NETWORK_ERROR)
    ...
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
        ...
        OP_NEW_THREAD(5, script_20_child)
            ...
            OP_BEGIN_REPEAT(5)
                ...
                OP_S_NEW_STREAM_BIDI(a, ANY_ID) OP_S_WRITE(a, "foo", 3) OP_S_UNBIND_STREAM_ID(a)

                    OP_END_REPEAT()

                        OP_CHECK(script_20_wait1, 5)

                            OP_C_CLOSE_SOCKET() OP_CHECK(script_20_trigger2, 0)

                                OP_END};

/* 21. Fault injection - unknown frame in 1-RTT packet */
static int script_21_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    WPACKET wpkt;
...
static const struct script_op script_21[] = {
    OP_S_SET_INJECT_PLAIN(script_21_inject_plain) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
...
static int script_22_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    if (h->inject_word0 == 0)
        return 1;
...
static const struct script_op script_22[] = {
    OP_S_SET_INJECT_PLAIN(script_22_inject_plain) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
...
static int script_23_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    WPACKET wpkt;
...
static const struct script_op script_23[] = {
    OP_S_SET_INJECT_PLAIN(script_23_inject_plain) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
...
static int script_24_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    WPACKET wpkt;
    unsigned char frame_buf[16];
...
static const struct script_op script_24[] = {
    OP_S_SET_INJECT_PLAIN(script_24_inject_plain) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
...
static int script_28_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    WPACKET wpkt;
...
static const struct script_op script_28[] = {
    OP_S_SET_INJECT_PLAIN(script_28_inject_plain) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)

            OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "orange", 6)

                OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "orange", 6)

                    OP_C_NEW_STREAM_UNI(b, C_UNI_ID(0)) OP_C_WRITE(b, "apple", 5)

                        OP_S_BIND_STREAM_ID(b, C_UNI_ID(0)) OP_S_READ_EXPECT(b, "apple", 5)
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_S_NEW_STREAM_UNI(a, S_UNI_ID(0)) OP_S_WRITE(a, "apple", 5)
                ...
                OP_C_ACCEPT_STREAM_WAIT(a) OP_C_READ_EXPECT(a, "apple", 5)
                    ...
                    OP_SET_INJECT_WORD(S_UNI_ID(0) + 1, OSSL_QUIC_FRAME_TYPE_STOP_SENDING) OP_S_WRITE(a, "orange", 6)
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_S_NEW_STREAM_UNI(a, S_UNI_ID(0)) OP_S_WRITE(a, "apple", 5)
                ...
                OP_C_ACCEPT_STREAM_WAIT(a) OP_C_READ_EXPECT(a, "apple", 5)
                    ...
                    OP_SET_INJECT_WORD(C_UNI_ID(0) + 1, OSSL_QUIC_FRAME_TYPE_STOP_SENDING) OP_S_WRITE(a, "orange", 6)
...
static int script_32_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    WPACKET wpkt;
    ...
    if (!TEST_true(WPACKET_quic_write_vlint(&wpkt, type))
        || !TEST_true(WPACKET_quic_write_vlint(&wpkt, /* stream ID */
                                               h->inject_word0 - 1))
        || !TEST_true(WPACKET_quic_write_vlint(&wpkt, offset)) || !TEST_true(WPACKET_quic_write_vlint(&wpkt, flen)))
        goto err;
...
static const struct script_op script_32[] = {
    OP_S_SET_INJECT_PLAIN(script_32_inject_plain) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_S_NEW_STREAM_UNI(a, S_UNI_ID(0)) OP_S_WRITE(a, "apple", 5)
                ...
                OP_C_ACCEPT_STREAM_WAIT(a) OP_C_READ_EXPECT(a, "apple", 5)
                    ...
                    OP_SET_INJECT_WORD(C_UNI_ID(0) + 1, 1) OP_S_WRITE(a, "orange", 6)
...
static const struct script_op script_33[] = {
    OP_S_SET_INJECT_PLAIN(script_32_inject_plain) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5)

                OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)

                    OP_SET_INJECT_WORD(C_BIDI_ID(0) + 1, 2) OP_S_WRITE(a, "orange", 6)
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5)
                ...
                OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
                    ...
                    OP_SET_INJECT_WORD(C_BIDI_ID(0) + 1, 3) OP_S_WRITE(a, "orange", 6)
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_S_NEW_STREAM_UNI(a, S_UNI_ID(0)) OP_S_WRITE(a, "apple", 5)
                ...
                OP_C_ACCEPT_STREAM_WAIT(a) OP_C_READ_EXPECT(a, "apple", 5)
                    ...
                    OP_SET_INJECT_WORD(S_UNI_ID(0) + 1, OSSL_QUIC_FRAME_TYPE_MAX_STREAM_DATA) OP_S_WRITE(a, "orange", 6)
...
static const struct script_op script_37[] = {
    OP_S_SET_INJECT_PLAIN(script_28_inject_plain) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_C_NEW_STREAM_UNI(a, C_UNI_ID(0)) OP_C_WRITE(a, "apple", 5)

                OP_S_BIND_STREAM_ID(a, C_UNI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
            ...
            OP_C_NEW_STREAM_UNI(a, C_UNI_ID(0)) OP_C_WRITE(a, "apple", 5)
                ...
                OP_S_BIND_STREAM_ID(a, C_UNI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
                    ...
                    OP_SET_INJECT_WORD(C_BIDI_ID(0) + 1, OSSL_QUIC_FRAME_TYPE_STREAM_DATA_BLOCKED)
                        ...
                        OP_S_NEW_STREAM_UNI(b, S_UNI_ID(0)) OP_S_WRITE(b, "orange", 5)
...
static int script_39_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    WPACKET wpkt;
    unsigned char frame_buf[64];
...
static const struct script_op script_39[] = {
    OP_S_SET_INJECT_PLAIN(script_39_inject_plain) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)

            OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5) OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0))
                OP_S_READ_EXPECT(a, "apple", 5)

                    OP_SET_INJECT_WORD(0, 1) OP_S_WRITE(a, "orange", 5)
...
static const struct script_op script_40[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
        ...
        OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5)
            ...
            OP_C_INHIBIT_TICK(1) OP_C_SET_WRITE_BUF_SIZE(a, 1024 * 100 * 3)
                ...
                OP_BEGIN_REPEAT(100)
                    ...
                    OP_C_WRITE(a, script_40_data, sizeof(script_40_data))
                        ...
                        OP_END_REPEAT()
                            ...
                            OP_C_CONCLUDE(a) OP_C_SHUTDOWN_WAIT(NULL, 0) /* disengages tick inhibition */
            ...
            OP_S_READ_EXPECT(a, script_40_data, sizeof(script_40_data))
                ...
                OP_END_REPEAT()
                    ...
                    OP_S_EXPECT_FIN(a)
                        ...
                        OP_C_EXPECT_CONN_CLOSE_INFO(0, 1, 0) OP_S_EXPECT_CONN_CLOSE_INFO(0, 1, 1)
...
static int script_41_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    WPACKET wpkt;
    unsigned char frame_buf[16];
    ...
    if (!TEST_true(WPACKET_get_total_written(&wpkt, &written)) || !TEST_size_t_eq(written, 9))
        goto err;
...
static void script_41_trace(int write_p, int version, int content_type, const void *buf, size_t len, SSL *ssl,
                            void *arg)
{
    ...
    if (version != OSSL_QUIC1_VERSION || content_type != SSL3_RT_QUIC_FRAME_FULL || len < 1)
        return;
    ...
    if (!TEST_true(ossl_quic_wire_peek_frame_header(&pkt, &frame_type, &was_minimal))) {
        ++h->scratch1;
        return;
    }
...
static const struct script_op script_41[] = {
    OP_S_SET_INJECT_PLAIN(script_41_inject_plain) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        OP_CHECK(script_41_setup, 0)
            ...
            OP_C_WRITE(DEFAULT, "apple", 5) OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
                ...
                OP_SET_INJECT_WORD(1, OSSL_QUIC_FRAME_TYPE_PATH_CHALLENGE)
                    ...
                    OP_S_WRITE(a, "orange", 6) OP_C_READ_EXPECT(DEFAULT, "orange", 6)

                        OP_C_WRITE(DEFAULT, "strawberry", 10) OP_S_READ_EXPECT(a, "strawberry", 10)

                            OP_CHECK(script_41_check, 0) OP_END};

/* 42. Fault injection - CRYPTO frame with illegal offset */
static int script_42_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    unsigned char frame_buf[64];
    size_t written;
    ...
    if (!TEST_true(WPACKET_quic_write_vlint(&wpkt, OSSL_QUIC_FRAME_TYPE_CRYPTO))
        ...
        || !TEST_true(WPACKET_quic_write_vlint(&wpkt, 1)) || !TEST_true(WPACKET_put_bytes_u8(&wpkt, 0x42)))
        goto err;
...
static const struct script_op script_42[] = {
    OP_S_SET_INJECT_PLAIN(script_42_inject_plain) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5)
                ...
                OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
                    ...
                    OP_SET_INJECT_WORD(1, (((uint64_t)1) << 62) - 1) OP_S_WRITE(a, "orange", 6)
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5)
                ...
                OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
                    ...
                    OP_SET_INJECT_WORD(1, 0x100000 /* 1 MiB */) OP_S_WRITE(a, "orange", 6)
...
static int script_44_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    WPACKET wpkt;
    unsigned char frame_buf[16];
...
static const struct script_op script_44[] = {
    OP_S_SET_INJECT_PLAIN(script_44_inject_plain) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
    ...
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        ...
        OP_C_WRITE(DEFAULT, "apple", 5) OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
            ...
            OP_BEGIN_REPEAT(2)
                ...
                OP_CHECK(force_ping, 0) OP_CHECK(wait_incoming_acks_increased, 0)
...
static int script_46_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    WPACKET wpkt;
    unsigned char frame_buf[16];
    ...
    if (!TEST_true(WPACKET_quic_write_vlint(&wpkt, type)) || !TEST_true(WPACKET_quic_write_vlint(&wpkt, largest_acked))
    ...
    if (type == OSSL_QUIC_FRAME_TYPE_ACK_WITH_ECN)
        if (!TEST_true(WPACKET_quic_write_vlint(&wpkt, ect0)) || !TEST_true(WPACKET_quic_write_vlint(&wpkt, ect1))
...
static const struct script_op script_46[] = {
    OP_S_SET_INJECT_PLAIN(script_46_inject_plain) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
...
static int script_52_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    unsigned char frame_buf[64];
    size_t written;
...
static const struct script_op script_52[] = {
    OP_S_SET_INJECT_PLAIN(script_52_inject_plain) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
...
static int script_53_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    size_t written;
    WPACKET wpkt;
    ...
    if (!TEST_true(WPACKET_quic_write_vlint(&wpkt, OSSL_QUIC_FRAME_TYPE_CRYPTO))
        || !TEST_true(WPACKET_quic_write_vlint(&wpkt, offset)) || !TEST_true(WPACKET_quic_write_vlint(&wpkt, data_len)))
        goto err;
...
static const struct script_op script_53[] = {
    OP_S_SET_INJECT_PLAIN(script_53_inject_plain) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()

        OP_C_WRITE(DEFAULT, "apple", 5) OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)

            OP_SET_INJECT_WORD(1, 0) OP_S_WRITE(a, "Strawberry", 10)
...
static int script_54_inject_handshake(struct helper *h, unsigned char *buf, size_t buf_len)
{
    size_t i;
...
static const struct script_op script_54[] = {
    OP_S_SET_INJECT_HANDSHAKE(script_54_inject_handshake) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT_OR_FAIL()
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5) OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0))
                OP_S_READ_EXPECT(a, "apple", 5)
                    ...
                    OP_SET_INJECT_WORD(0, 2) OP_S_WRITE(a, "orange", 5)
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5) OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0))
                OP_S_READ_EXPECT(a, "apple", 5)
                    ...
                    OP_SET_INJECT_WORD(0, 3) OP_S_WRITE(a, "orange", 5)
...
static const struct script_op script_57[] = {
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5) OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0))
                OP_S_READ_EXPECT(a, "apple", 5)
                    ...
                    OP_SET_INJECT_WORD(0, 4) OP_S_WRITE(a, "orange", 5) OP_C_READ_EXPECT(a, "orange", 5)

                        OP_C_WRITE(a, "Strawberry", 10) OP_S_READ_EXPECT(a, "Strawberry", 10)
     ...
     * communicate.
     */
    OP_SET_INJECT_WORD(0, 5) OP_S_WRITE(a, "raspberry", 9) OP_C_READ_EXPECT(a, "raspberry", 9)

        OP_C_WRITE(a, "peach", 5) OP_S_READ_EXPECT(a, "peach", 5)

            OP_END};

/* 58. Fault injection - repeated HANDSHAKE_DONE */
static int script_58_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    unsigned char frame_buf[64];
    size_t written;
    ...
    if (h->inject_word0 == 1) {
        ...
        if (!TEST_true(WPACKET_put_bytes_u8(&wpkt, 0x40)) || !TEST_true(WPACKET_put_bytes_u8(&wpkt, 0x1E)))
            goto err;
    }
...
static const struct script_op script_58[] = {
    OP_S_SET_INJECT_PLAIN(script_58_inject_plain) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
...
static int check_shutdown_reason(struct helper *h, struct helper_local *hl)
{
    ...
    if (!TEST_size_t_ge(tc->reason_len, 50) || !TEST_mem_eq(long_reason, tc->reason_len, tc->reason, tc->reason_len))
        return 0;
...
static const struct script_op script_60[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        ...
        OP_C_WRITE(DEFAULT, "apple", 5) OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
            ...
            OP_CHECK(init_reason, 0) OP_C_SHUTDOWN_WAIT(long_reason, 0) OP_CHECK(check_shutdown_reason, 0)

                OP_END};

/* 61. Fault injection - RESET_STREAM exceeding stream count FC */
static int script_61_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    WPACKET wpkt;
    unsigned char frame_buf[32];
...
static const struct script_op script_61[] = {
    OP_S_SET_INJECT_PLAIN(script_61_inject_plain) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5)
                ...
                OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
                    ...
                    OP_SET_INJECT_WORD(S_BIDI_ID(5000) + 1, 4) OP_S_WRITE(a, "orange", 6)
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_S_NEW_STREAM_UNI(a, S_UNI_ID(0)) OP_S_WRITE(a, "apple", 5)
                ...
                OP_C_ACCEPT_STREAM_WAIT(a) OP_C_READ_EXPECT(a, "apple", 5)
                    ...
                    OP_SET_INJECT_WORD(S_BIDI_ID(20) + 1, 1) OP_S_WRITE(a, "orange", 6) OP_C_READ_EXPECT(a, "orange", 6)

                        OP_END};

/* 65. Fault injection - CRYPTO - zero-length is accepted */
static int script_65_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    unsigned char frame_buf[64];
    size_t written;
    ...
    if (!TEST_true(WPACKET_quic_write_vlint(&wpkt, OSSL_QUIC_FRAME_TYPE_CRYPTO))
        || !TEST_true(WPACKET_quic_write_vlint(&wpkt, 0)) || !TEST_true(WPACKET_quic_write_vlint(&wpkt, 0)))
        goto err;
...
static const struct script_op script_65[] = {
    OP_S_SET_INJECT_PLAIN(script_65_inject_plain) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5)
                ...
                OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
                    ...
                    OP_SET_INJECT_WORD(1, 0) OP_S_WRITE(a, "orange", 6) OP_C_READ_EXPECT(a, "orange", 6)

                        OP_END};

/* 66. Fault injection - large MAX_STREAM_DATA */
static int script_66_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int ok = 0;
    WPACKET wpkt;
    unsigned char frame_buf[64];
...
static const struct script_op script_66[] = {
    OP_S_SET_INJECT_PLAIN(script_66_inject_plain) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_S_NEW_STREAM_BIDI(a, S_BIDI_ID(0)) OP_S_WRITE(a, "apple", 5)
                ...
                OP_C_ACCEPT_STREAM_WAIT(a) OP_C_READ_EXPECT(a, "apple", 5)
                    ...
                    OP_SET_INJECT_WORD(S_BIDI_ID(0) + 1, OSSL_QUIC_FRAME_TYPE_MAX_STREAM_DATA)
                        OP_S_WRITE(a, "orange", 6) OP_C_READ_EXPECT(a, "orange", 6) OP_C_WRITE(a, "Strawberry", 10)
                            OP_S_READ_EXPECT(a, "Strawberry", 10)
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_S_NEW_STREAM_BIDI(a, S_BIDI_ID(0)) OP_S_WRITE(a, "apple", 5)
                ...
                OP_C_ACCEPT_STREAM_WAIT(a) OP_C_READ_EXPECT(a, "apple", 5)
                    ...
                    OP_SET_INJECT_WORD(1, OSSL_QUIC_FRAME_TYPE_MAX_DATA) OP_S_WRITE(a, "orange", 6)
                        OP_C_READ_EXPECT(a, "orange", 6) OP_C_WRITE(a, "Strawberry", 10)
                            OP_S_READ_EXPECT(a, "Strawberry", 10)

                                OP_END};

/* 68. Fault injection - Unexpected TLS messages */
static int script_68_inject_handshake(struct helper *h, unsigned char *msg, size_t msglen)
{
    ...
    const unsigned char certreq[] = {
        ...
        1,
        1,                                /* certificate_request_context */
        0,
        8,                                /* Extensions block length */
        0,
        TLSEXT_TYPE_signature_algorithms, /* sig_algs extension*/
        0,
        4,                                /* 4 bytes of sig algs extension*/
        0,
        2,                                /* sigalgs list is 2 bytes long */
        8,
    ...
    if (!TEST_true(qtest_fault_resize_message(h->qtf, datalen - SSL3_HM_HEADER_LENGTH)))
        return 0;
...
static const struct script_op script_68[] = {
    OP_S_SET_INJECT_HANDSHAKE(script_68_inject_handshake) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)

            OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5) OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0))
                OP_S_READ_EXPECT(a, "apple", 5)

                    OP_SET_INJECT_WORD(1, 0) OP_S_NEW_TICKET() OP_S_WRITE(a, "orange", 6)
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5) OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0))
                OP_S_READ_EXPECT(a, "apple", 5)
                    ...
                    OP_SET_INJECT_WORD(2, 0) OP_S_NEW_TICKET() OP_S_WRITE(a, "orange", 6)
...
static int set_max_early_data(struct helper *h, struct helper_local *hl)
{
    ...
    if (!TEST_true(ossl_quic_tserver_set_max_early_data(ACQUIRE_S(), (uint32_t)hl->check_op->arg2)))
        return 0;
    ...
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
        ...
        OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5) OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0))
            OP_S_READ_EXPECT(a, "apple", 5)
                ...
                OP_CHECK(set_max_early_data, 0xfffffffe) OP_S_NEW_TICKET() OP_S_WRITE(a, "orange", 6)
    ...
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
        ...
        OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5) OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0))
            OP_S_READ_EXPECT(a, "apple", 5)
                ...
                OP_CHECK(set_max_early_data, 0xffffffff) OP_S_NEW_TICKET() OP_S_WRITE(a, "orange", 6)
...
static const struct script_op script_72[] = {
    ...
    OP_BEGIN_REPEAT(200)
        ...
        OP_C_NEW_STREAM_BIDI_EX(a, ANY_ID, ALLOW_FAIL | SSL_STREAM_FLAG_NO_BLOCK) OP_C_SKIP_IF_UNBOUND(a, 2)
            OP_C_WRITE(a, "apple", 5) OP_C_FREE_STREAM(a)
...
static const struct script_op script_73[] = {
    ...
    OP_BEGIN_REPEAT(200)
        ...
        OP_C_NEW_STREAM_UNI_EX(a, ANY_ID, ALLOW_FAIL | SSL_STREAM_FLAG_NO_BLOCK) OP_C_SKIP_IF_UNBOUND(a, 2)
            OP_C_WRITE(a, "apple", 5) OP_C_FREE_STREAM(a)
 ...
 * armed again
 */
static int script_74_alter_version(const QUIC_PKT_HDR *hdrin, const OSSL_QTX_IOVEC *iovecin, size_t numin,
                                   QUIC_PKT_HDR **hdrout, const OSSL_QTX_IOVEC **iovecout, size_t *numout, void *arg)
{
 ...
 */
static int script_74_arm_packet_mutator(struct helper *h, struct helper_local *hl)
{
    QUIC_CHANNEL *ch = ossl_quic_conn_get_channel(h->c_conn);

    do_mutation = 1;
    if (!ossl_quic_channel_set_mutator(ch, script_74_alter_version, script_74_finish_mutation, NULL))
        return 0;
    return 1;
}

static const struct script_op script_74[] = {
    OP_CHECK(script_74_arm_packet_mutator, 0) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
...
static const struct script_op script_75[] = {OP_S_SET_INJECT_DATAGRAM(server_gen_version_neg) OP_SET_INJECT_WORD(2, 0)
...
{
    if (!TEST_false(SSL_shutdown_ex(h->c_conn, SSL_SHUTDOWN_FLAG_WAIT_PEER | SSL_SHUTDOWN_FLAG_NO_BLOCK, NULL, 0)))
        return 0;
...
static const struct script_op script_76[] = {
    ...
    OP_CHECK(script_76_check, 0) OP_S_SHUTDOWN(42)

        OP_C_SHUTDOWN_WAIT(NULL, SSL_SHUTDOWN_FLAG_WAIT_PEER) OP_C_EXPECT_CONN_CLOSE_INFO(42, 1, 1)
...
static const struct script_op script_77[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        ...
        OP_C_SET_INCOMING_STREAM_POLICY(SSL_INCOMING_STREAM_POLICY_ACCEPT)
            ...
            OP_S_NEW_STREAM_BIDI(a, S_BIDI_ID(0)) OP_S_WRITE(a, "Strawberry", 10)

                OP_C_READ_EXPECT(DEFAULT, "Strawberry", 10)

                    OP_S_NEW_STREAM_BIDI(b, S_BIDI_ID(1)) OP_S_WRITE(b, "xyz", 3)

                        OP_C_ACCEPT_STREAM_WAIT(b) OP_C_READ_EXPECT(b, "xyz", 3)
...
static const struct script_op script_78[] = {
    OP_C_SET_ALPN("ossltest") OP_CHECK(setup_session, 0) OP_C_CONNECT_WAIT()
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5)
                ...
                OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
                    ...
                    OP_S_WRITE(a, "orange", 6) OP_C_READ_EXPECT(a, "orange", 6)

                        OP_CHECK(trigger_late_session_ticket, 0)

                            OP_S_WRITE(a, "Strawberry", 10) OP_C_READ_EXPECT(a, "Strawberry", 10)
...
static const struct script_op script_79[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_WRITE_EX2(DEFAULT, "apple", 5, SSL_WRITE_FLAG_CONCLUDE)
        OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5) OP_S_EXPECT_FIN(a)
            OP_S_WRITE(a, "orange", 6) OP_S_CONCLUDE(a) OP_C_READ_EXPECT(DEFAULT, "orange", 6) OP_C_EXPECT_FIN(DEFAULT)
                OP_END};
 ...
 *  }
 */
static int script_80_send_stateless_reset(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    ...
    databuf[0] = 0x40;
    memcpy(&databuf[48], test_reset_token.token, sizeof(test_reset_token.token));

    if (!TEST_int_eq(SSL_inject_net_dgram(h->c_conn, databuf, sizeof(databuf), NULL, h->s_net_bio_addr), 1))
        return 0;

    return 1;
}

static int script_80_gen_new_conn_id(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    int rc = 0;
    size_t l;
    unsigned char frame_buf[64];
    ...
    memcpy(ncid.stateless_reset.token, test_reset_token.token, sizeof(test_reset_token.token));
...
static int script_80_inject_pkt(struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t len)
{
    if (h->inject_word1 == 1)
...
static const struct script_op script_80[] = {
    OP_S_SET_INJECT_PLAIN(script_80_inject_pkt) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        OP_C_WRITE(DEFAULT, "apple", 5) OP_C_CONCLUDE(DEFAULT) OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0))
            OP_S_READ_EXPECT(a, "apple", 5) OP_SET_INJECT_WORD(1, 0) OP_S_WRITE(a, "apple", 5)
                OP_C_READ_EXPECT(DEFAULT, "apple", 5) OP_SET_INJECT_WORD(0, 1) OP_S_WRITE(a, "apple", 5)
                    OP_C_EXPECT_CONN_CLOSE_INFO(0, 0, 1) OP_END};
...
static int modify_idle_timeout(struct helper *h, struct helper_local *hl)
{
    ...
    if (!TEST_false(SSL_set_feature_request_uint(h->c_conn, SSL_VALUE_QUIC_IDLE_TIMEOUT, (1ULL << 62))))
        return 0;

    /* Set value. */
    if (!TEST_true(SSL_set_feature_request_uint(h->c_conn, SSL_VALUE_QUIC_IDLE_TIMEOUT, hl->check_op->arg2)))
        return 0;

    if (!TEST_true(SSL_get_feature_request_uint(h->c_conn, SSL_VALUE_QUIC_IDLE_TIMEOUT, &v)))
        return 0;
...
static int check_idle_timeout(struct helper *h, struct helper_local *hl)
{
    ...
    if (!TEST_true(SSL_get_value_uint(h->c_conn, hl->check_op->arg1, SSL_VALUE_QUIC_IDLE_TIMEOUT, &v)))
        return 0;
...
static const struct script_op script_81[] = {
    OP_C_SET_ALPN("ossltest") OP_CHECK(modify_idle_timeout, 25000) OP_C_CONNECT_WAIT()
...
static int cannot_change_idle_timeout(struct helper *h, struct helper_local *hl)
{
    ...
    if (!TEST_false(SSL_set_feature_request_uint(h->c_conn, SSL_VALUE_QUIC_IDLE_TIMEOUT, 5000)))
        return 0;
...
static int check_write_buf_stat(struct helper *h, struct helper_local *hl)
{
    ...
    if (!TEST_true(SSL_get_stream_write_buf_size(c_a, &size)) || !TEST_true(SSL_get_stream_write_buf_used(c_a, &used))
        || !TEST_true(SSL_get_stream_write_buf_avail(c_a, &avail)) || !TEST_uint64_t_ge(size, avail)
        || !TEST_uint64_t_ge(size, used) || !TEST_uint64_t_eq(avail + used, size))
        return 0;
...
static const struct script_op script_84[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_CHECK2(check_avail_streams, 0, 100) OP_CHECK2(check_avail_streams, 1, 100) OP_CHECK2(
                check_avail_streams, 2, 100) OP_CHECK2(check_avail_streams, 3, 100)

                OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0))

                    OP_CHECK2(check_avail_streams, 0, 99) OP_CHECK2(check_avail_streams, 1, 100) OP_CHECK2(
                        check_avail_streams, 2, 100) OP_CHECK2(check_avail_streams, 3, 100)

                        OP_C_NEW_STREAM_UNI(b, C_UNI_ID(0))

                            OP_CHECK2(check_avail_streams, 0, 99) OP_CHECK2(check_avail_streams, 1, 100) OP_CHECK2(
                                check_avail_streams, 2, 99) OP_CHECK2(check_avail_streams, 3, 100)

                                OP_S_NEW_STREAM_BIDI(c, S_BIDI_ID(0)) OP_S_WRITE(c, "x", 1)

                                    OP_C_ACCEPT_STREAM_WAIT(c) OP_C_READ_EXPECT(c, "x", 1)

                                        OP_CHECK2(check_avail_streams, 0, 99) OP_CHECK2(check_avail_streams, 1, 99)
                                            OP_CHECK2(check_avail_streams, 2, 99) OP_CHECK2(check_avail_streams, 3, 100)

                                                OP_S_NEW_STREAM_UNI(d, S_UNI_ID(0)) OP_S_WRITE(d, "x", 1)

                                                    OP_C_ACCEPT_STREAM_WAIT(d) OP_C_READ_EXPECT(d, "x", 1)

                                                        OP_CHECK2(check_avail_streams, 0,
                                                                  99) OP_CHECK2(check_avail_streams, 1, 99)
                                                            OP_CHECK2(check_avail_streams, 2,
                                                                      99) OP_CHECK2(check_avail_streams, 3, 99)

                                                                OP_CHECK2(check_write_buf_stat, 0, 0) OP_CHECK(
                                                                    set_event_handling_mode_conn,
                                                                    SSL_VALUE_EVENT_HANDLING_MODE_EXPLICIT)
                                                                    OP_C_WRITE(a, "apple",
                                                                               5) OP_CHECK2(check_write_buf_stat, 5, 0)
...
ossl_unused static int script_85_poll(struct helper *h, struct helper_local *hl)
{
    ...
    if (!TEST_ptr(c_a = helper_local_get_c_stream(hl, "a")) || !TEST_ptr(c_b = helper_local_get_c_stream(hl, "b"))
        || !TEST_ptr(c_c = helper_local_get_c_stream(hl, "c")) || !TEST_ptr(c_d = helper_local_get_c_stream(hl, "d")))
        return 0;
    ...
    ret = SSL_poll(items, OSSL_NELEM(items), sizeof(SSL_POLL_ITEM), &timeout, 0, &result_count);
    ...
    if (!TEST_int_eq(ret, expected_ret) || !TEST_size_t_eq(result_count, expected_result_count))
        ok = 0;

    for (i = 0; i < OSSL_NELEM(items); ++i)
        if (!TEST_uint64_t_eq(items[i].revents, expected_revents[i])) {
            TEST_error("mismatch at index %zu in poll results, mode %d", i, (int)mode);
            ok = 0;
        }
...
static const struct script_op script_85[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "flamingo", 8)

                OP_C_NEW_STREAM_BIDI(b, C_BIDI_ID(1)) OP_C_WRITE(b, "orange", 6)

                    OP_C_NEW_STREAM_BIDI(c, C_BIDI_ID(2)) OP_C_WRITE(c, "Strawberry", 10)

                        OP_C_NEW_STREAM_BIDI(d, C_BIDI_ID(3)) OP_C_WRITE(d, "sync", 4)

                            OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_BIND_STREAM_ID(b, C_BIDI_ID(1))
                                OP_S_BIND_STREAM_ID(c, C_BIDI_ID(2)) OP_S_BIND_STREAM_ID(d, C_BIDI_ID(3))

    /* Check nothing readable yet. */
    OP_CHECK(script_85_poll, 0)

    /* Send something that will make client sockets readable. */
    OP_S_READ_EXPECT(a, "flamingo", 8) OP_S_WRITE(a, "herringbone", 11)
    ...
    /* Ensure sync. */
    OP_S_READ_EXPECT(d, "sync", 4) OP_S_WRITE(d, "x", 1) OP_C_READ_EXPECT(d, "x", 1)

    /* Send something that will make 'c' reset. */
    OP_S_SET_INJECT_PLAIN(script_28_inject_plain)
        OP_SET_INJECT_WORD(C_BIDI_ID(2) + 1, OSSL_QUIC_FRAME_TYPE_STOP_SENDING)

            OP_S_NEW_STREAM_BIDI(z, S_BIDI_ID(0)) OP_S_WRITE(z, "z", 1)

    /* Ensure sync. */
    OP_S_WRITE(d, "x", 1) OP_C_READ_EXPECT(d, "x", 1)
...
static const struct script_op script_86[] = {
    ...
    OP_CHECK(set_event_handling_mode_stream, SSL_VALUE_EVENT_HANDLING_MODE_EXPLICIT) OP_C_WRITE(a, "orange", 6)
        OP_C_CONCLUDE(a)
    ...
    OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0))
        ...
        OP_BEGIN_REPEAT(20) OP_S_READ_FAIL(a, 1) OP_SLEEP(10) OP_END_REPEAT()

    /* Now let the data arrive and confirm it arrives. */
    OP_CHECK(reenable_test_event_handling, 0) OP_S_READ_EXPECT(a, "appleorange", 11) OP_S_EXPECT_FIN(a)

    /* Back into explicit mode. */
    OP_CHECK(set_event_handling_mode_conn, SSL_VALUE_EVENT_HANDLING_MODE_EXPLICIT) OP_S_WRITE(a, "ok", 2)
        OP_C_READ_FAIL(a)

    /* Works once event handling is done. */
    OP_CHECK(reenable_test_event_handling, 0) OP_C_READ_EXPECT(a, "ok", 2)

        OP_END};

/* 87. Test stream reset functionality */
static const struct script_op script_87[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5)
        OP_C_CONCLUDE(a) OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5) OP_S_EXPECT_FIN(a)
            OP_S_WRITE(a, "orange", 6) OP_C_READ_EXPECT(a, "orange", 6) OP_S_CONCLUDE(a) OP_C_EXPECT_FIN(a)
                OP_SLEEP(1000) OP_C_STREAM_RESET_FAIL(a, 42) OP_END};
...
static const struct script_op *const scripts[] = {
    ...
    script_21, script_22, script_23, script_24, script_25, script_26, script_27, script_28, script_29, script_30,
    script_31, script_32, script_33, script_34, script_35, script_36, script_37, script_38, script_39, script_40,
    script_41, script_42, script_43, script_44, script_45, script_46, script_47, script_48, script_49, script_50,
    script_51, script_52, script_53, script_54, script_55, script_56, script_57, script_58, script_59, script_60,
    script_61, script_62, script_63, script_64, script_65, script_66, script_67, script_68, script_69, script_70,
    script_71, script_72, script_73, script_74, script_75, script_76, script_77, script_78, script_79, script_80,
    script_81, script_82, script_83, script_84, script_85, script_86, script_87};
...
static int test_script(int idx)
{
    ...
    TEST_info("Running script %d (order=%d, blocking=%d)", script_idx + 1, free_order, blocking);
    return run_script(scripts[script_idx], script_name, free_order, blocking);
}

/* Dynamically generated tests. */
static struct script_op dyn_frame_types_script[] = {
    OP_S_SET_INJECT_PLAIN(script_21_inject_plain) OP_SET_INJECT_WORD(0, 0) /* dynamic */
...
static ossl_unused int test_dyn_frame_types(int idx)
{
    ...
    BIO_snprintf(script_name, sizeof(script_name), "dyn script %d", idx);
...
int setup_tests(void)
{
    ...
    if (!TEST_ptr(certfile = test_get_argument(0)) || !TEST_ptr(keyfile = test_get_argument(1)))
        return 0;
