...
struct helper {
    ...
    int (*qtf_handshake_cb)(struct helper *h, unsigned char *buf,
                            size_t buf_len);
...
#define OP_END {OPK_END}
#define OP_CHECK(func, arg2) {OPK_CHECK, NULL, 0, (func), NULL, (arg2)},
#define OP_C_SET_ALPN(alpn) {OPK_C_SET_ALPN, (alpn), 0, NULL, NULL},
#define OP_C_CONNECT_WAIT() {OPK_C_CONNECT_WAIT, NULL, 0, NULL, NULL},
#define OP_C_CONNECT_WAIT_OR_FAIL() {OPK_C_CONNECT_WAIT, NULL, 1, NULL, NULL},
...
#define OP_C_DETACH(stream_name) {OPK_C_DETACH, NULL, 0, NULL, #stream_name},
#define OP_C_ATTACH(stream_name) {OPK_C_ATTACH, NULL, 0, NULL, #stream_name},
...
#define OP_C_NEW_STREAM_UNI(stream_name, expect_id)                            \
    {OPK_C_NEW_STREAM, NULL,       SSL_STREAM_FLAG_UNI, NULL,                  \
     #stream_name,     (expect_id)},
#define OP_C_NEW_STREAM_UNI_EX(stream_name, expect_id, flags)                  \
    {OPK_C_NEW_STREAM, NULL,       (flags) | SSL_STREAM_FLAG_UNI, NULL,        \
     #stream_name,     (expect_id)},
...
#define OP_BEGIN_REPEAT(n) {OPK_BEGIN_REPEAT, NULL, (n)},
#define OP_END_REPEAT() {OPK_END_REPEAT},
...
#define OP_C_CLOSE_SOCKET() {OPK_C_CLOSE_SOCKET},
#define OP_C_EXPECT_SSL_ERR(stream_name, err)                                  \
    {OPK_C_EXPECT_SSL_ERR, NULL, (err), NULL, #stream_name},
#define OP_EXPECT_ERR_REASON(err) {OPK_EXPECT_ERR_REASON, NULL, (err)},
#define OP_EXPECT_ERR_LIB(lib) {OPK_EXPECT_ERR_LIB, NULL, (lib)},
#define OP_SLEEP(ms) {OPK_SLEEP, NULL, 0, NULL, NULL, (ms)},
...
#define OP_S_NEW_TICKET() {OPK_S_NEW_TICKET},
...
#define OP_S_SHUTDOWN(error_code) {OPK_S_SHUTDOWN, NULL, (error_code)},
#define OP_POP_ERR() {OPK_POP_ERR},
...
#define OP_SKIP_IF_BLOCKING(n) {OPK_SKIP_IF_BLOCKING, NULL, (n), NULL, 0},
...
static int skip_time_ms(struct helper *h, struct helper_local *hl) {
    ...
    h->time_slip =
    ossl_time_add(h->time_slip, ossl_ms2time(hl->check_op->arg2));
...
static int check_rejected(struct helper *h, struct helper_local *hl) {
    ...
    if (!ossl_quic_tserver_stream_has_peer_stop_sending(ACQUIRE_S(), stream_id,
                                                        NULL) ||
        !ossl_quic_tserver_stream_has_peer_reset_stream(ACQUIRE_S(), stream_id,
                                                        NULL)) {
...
static int check_stream_reset(struct helper *h, struct helper_local *hl) {
    ...
    if (!ossl_quic_tserver_stream_has_peer_reset_stream(ACQUIRE_S(), stream_id,
                                                        &aec)) {
        h->check_spin_again = 1;
        return 0;
    }
...
static int helper_init(struct helper *h, const char *script_name,
                       int free_order, int blocking, int need_injector) {
    ...
    if (!TEST_ptr(h->s_streams =
                  lh_STREAM_INFO_new(stream_info_hash, stream_info_cmp)))
        goto err;

    if (!TEST_ptr(h->c_streams =
                  lh_STREAM_INFO_new(stream_info_hash, stream_info_cmp)))
        goto err;
    ...
    if (!TEST_true(BIO_ADDR_rawmake(h->s_net_bio_orig_addr, AF_INET, &ina,
                                    sizeof(ina), 0)))
        goto err;
    ...
    if (!TEST_ptr(h->s_priv =
                  ossl_quic_tserver_new(&s_args, certfile, keyfile)))
        goto err;
    ...
    BIO_snprintf(title, sizeof(title), "quic_multistream_test: %s",
                 script_name);
...
static int helper_local_set_c_stream(struct helper_local *hl,
                                     const char *stream_name, SSL *c_stream) {
...
static int helper_set_s_stream(struct helper *h, const char *stream_name,
                               uint64_t s_stream_id) {
    STREAM_INFO *info;
...
static int helper_handshake_listener(QTEST_FAULT *fault, unsigned char *buf,
...
static int helper_datagram_listener(QTEST_FAULT *fault, BIO_MSG *msg,
                                    size_t stride, void *arg) {
    struct helper *h = arg;
...
static int run_script_worker(struct helper *h, const struct script_op *script,
                             const char *script_name, int thread_idx) {
    int testresult = 0;
...
#define C_SPIN_AGAIN()                                                         \
    ...
    for (;;) {
        ...
        case OPK_END:
            ...
            if (thread_idx < 0) {
                ...
                for (i = 0; i < h->num_threads; ++i) {
                    ...
                    if (!done) {
                        if (!end_wait_warning) {
                            TEST_info(
                            "still waiting for other threads to finish (%zu)",
                            i);
                            end_wait_warning = 1;
                        }
            ...
            TEST_info("script \"%s\" finished on thread %d", script_name,
                      thread_idx);
            testresult = 1;
            goto out;
        ...
        case OPK_C_SET_ALPN: {
            ...
            if (!TEST_size_t_le(alpn_len, UINT8_MAX) ||
                !TEST_ptr(tmp_buf =
                          (unsigned char *)OPENSSL_malloc(alpn_len + 1)))
                goto out;
            ...
            if (!TEST_false(
                SSL_set_alpn_protos(h->c_conn, tmp_buf, alpn_len + 1)))
                goto out;
        ...
        case OPK_C_WRITE: {
            ...
            if (!TEST_true(r) || !check_consistent_want(c_tgt, r) ||
        ...
        case OPK_C_WRITE_EX2: {
            ...
            r =
            SSL_write_ex2(c_tgt, op->arg0, op->arg1, op->arg2, &bytes_written);
            if (!TEST_true(r) || !check_consistent_want(c_tgt, r) ||
                !TEST_size_t_eq(bytes_written, op->arg1))
                goto out;
        } break;
        ...
        case OPK_S_READ_EXPECT: {
            ...
            if (!TEST_true(ossl_quic_tserver_read(
                ACQUIRE_S(), s_stream_id, tmp_buf + offset, op->arg1 - offset,
                &bytes_read)))
                goto out;
        ...
        case OPK_C_EXPECT_FIN: {
            ...
            if (!check_consistent_want(c_tgt, r) || !TEST_false(r) ||
        ...
        case OPK_C_DETACH: {
            ...
            if (!TEST_true(
                helper_local_set_c_stream(hl, op->stream_name, c_stream)))
                goto out;
        } break;
        ...
        case OPK_C_ATTACH: {
            ...
            if (!TEST_true(
                helper_local_set_c_stream(hl, op->stream_name, NULL)))
                goto out;
        } break;
        ...
        case OPK_C_NEW_STREAM: {
            ...
            if (op->arg2 != UINT64_MAX &&
                !TEST_uint64_t_eq(SSL_get_stream_id(c_stream), op->arg2))
                goto out;
        ...
        case OPK_S_ACCEPT_STREAM_WAIT: {
            ...
            if (!TEST_true(
                helper_set_s_stream(h, op->stream_name, new_stream_id)))
                goto out;
        } break;
        ...
        case OPK_C_ACCEPT_STREAM_NONE: {
            ...
            if (!TEST_ptr_null(c_stream = SSL_accept_stream(
                               h->c_conn, SSL_ACCEPT_STREAM_NO_BLOCK))) {
                SSL_free(c_stream);
                goto out;
            }
        } break;
        ...
        case OPK_C_EXPECT_CONN_CLOSE_INFO: {
            ...
            if (h->blocking && !TEST_true(SSL_shutdown_ex(
                               c_tgt, SSL_SHUTDOWN_FLAG_WAIT_PEER, NULL, 0)))
                goto out;
            ...
            if (!TEST_int_eq(expect_app,
                             ...
                             0) ||
                !TEST_int_eq(expect_remote, (cc_info.flags &
        ...
        case OPK_S_WRITE_FAIL: {
            ...
            if (!TEST_false(ossl_quic_tserver_write(
                ACQUIRE_S(), s_stream_id, (const unsigned char *)"apple", 5,
                &bytes_written)))
                goto out;
        } break;
        ...
        case OPK_S_READ_FAIL: {
            ...
            ret = ossl_quic_tserver_read(ACQUIRE_S(), s_stream_id, buf,
        ...
        case OPK_NEW_THREAD: {
            ...
            h->threads =
            OPENSSL_zalloc(op->arg1 * sizeof(struct child_thread_args));
            ...
            for (i = 0; i < op->arg1; ++i) {
                ...
                h->threads[i].t = ossl_crypto_thread_native_start(
                run_script_child_thread, &h->threads[i], 1);
        ...
        case OPK_S_SET_INJECT_PLAIN:
            ...
            if (!TEST_true(qtest_fault_set_packet_plain_listener(
                h->qtf,
                h->qtf_packet_plain_cb != NULL ? helper_packet_plain_listener
                                               : NULL,
                h)))
                goto out;
...
out:
    ...
    if (!testresult) {
        ...
        for (i = 0; i < repeat_stack_len; ++i)
            TEST_info(
            "while repeating, iteration %zu of %zu, starting at script op %zu",
            repeat_stack_done[i], repeat_stack_limit[i], repeat_stack_idx[i]);
        ...
        if (h->c_conn != NULL) {
            ...
            if (SSL_get_conn_close_info(h->c_conn, &cc_info, sizeof(cc_info))) {
                ...
                TEST_info("client side is closed: %llu(%s)/%llu(%s), "
                          "%s, %s, reason: \"%s\"",
                          (unsigned long long)cc_info.error_code, e_str,
                          (unsigned long long)cc_info.frame_type, f_str,
        ...
        tcause =
        (h->s != NULL ? ossl_quic_tserver_get_terminate_cause(h->s) : NULL);
        if (tcause != NULL) {
            ...
            TEST_info("server side is closed: %llu(%s)/%llu(%s), "
                      "%s, %s, reason: \"%s\"",
                      (unsigned long long)tcause->error_code, e_str,
                      (unsigned long long)tcause->frame_type, f_str,
...
static int run_script(const struct script_op *script, const char *script_name,
...
static CRYPTO_THREAD_RETVAL run_script_child_thread(void *arg) {
    ...
    testresult = run_script_worker(args->h, args->script, args->script_name,
                                   args->thread_idx);
...
OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_WRITE(DEFAULT, "apple", 5)
OP_C_CONCLUDE(DEFAULT) OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0))
OP_S_READ_EXPECT(a, "apple", 5) OP_S_EXPECT_FIN(a) OP_S_WRITE(a, "orange", 6)
OP_S_CONCLUDE(a) OP_C_READ_EXPECT(DEFAULT, "orange", 6) OP_C_EXPECT_FIN(DEFAULT)
OP_END};
...
OP_C_WRITE(DEFAULT, "apple", 5) OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0))
OP_S_READ_EXPECT(a, "apple", 5) OP_S_WRITE(a, "orange", 6)
OP_C_READ_EXPECT(DEFAULT, "orange", 6)

OP_C_NEW_STREAM_BIDI(b, C_BIDI_ID(1)) OP_C_WRITE(b, "flamingo", 8)
OP_C_CONCLUDE(b) OP_S_BIND_STREAM_ID(b, C_BIDI_ID(1))
OP_S_READ_EXPECT(b, "flamingo", 8) OP_S_EXPECT_FIN(b)
OP_S_WRITE(b, "gargoyle", 8) OP_S_CONCLUDE(b) OP_C_READ_EXPECT(b, "gargoyle", 8)
OP_C_EXPECT_FIN(b)

OP_C_NEW_STREAM_UNI(c, C_UNI_ID(0)) OP_C_WRITE(c, "elephant", 8)
OP_C_CONCLUDE(c) OP_S_BIND_STREAM_ID(c, C_UNI_ID(0))
OP_S_READ_EXPECT(c, "elephant", 8) OP_S_EXPECT_FIN(c) OP_S_WRITE_FAIL(c)

OP_C_ACCEPT_STREAM_NONE()

OP_S_NEW_STREAM_BIDI(d, S_BIDI_ID(0)) OP_S_WRITE(d, "frog", 4) OP_S_CONCLUDE(d)

OP_C_ACCEPT_STREAM_WAIT(d) OP_C_ACCEPT_STREAM_NONE()
OP_C_READ_EXPECT(d, "frog", 4) OP_C_EXPECT_FIN(d)

OP_S_NEW_STREAM_BIDI(e, S_BIDI_ID(1)) OP_S_WRITE(e, "mixture", 7)
OP_S_CONCLUDE(e)

OP_C_ACCEPT_STREAM_WAIT(e) OP_C_READ_EXPECT(e, "mixture", 7) OP_C_EXPECT_FIN(e)
OP_C_WRITE(e, "ramble", 6) OP_S_READ_EXPECT(e, "ramble", 6) OP_C_CONCLUDE(e)
OP_S_EXPECT_FIN(e)

OP_S_NEW_STREAM_UNI(f, S_UNI_ID(0)) OP_S_WRITE(f, "yonder", 6) OP_S_CONCLUDE(f)

OP_C_ACCEPT_STREAM_WAIT(f) OP_C_ACCEPT_STREAM_NONE()
OP_C_READ_EXPECT(f, "yonder", 6) OP_C_EXPECT_FIN(f) OP_C_WRITE_FAIL(f)

OP_C_SET_INCOMING_STREAM_POLICY(SSL_INCOMING_STREAM_POLICY_REJECT)
OP_S_NEW_STREAM_BIDI(g, S_BIDI_ID(2)) OP_S_WRITE(g, "unseen", 6)
OP_S_CONCLUDE(g)

OP_C_ACCEPT_STREAM_NONE()

OP_C_SET_INCOMING_STREAM_POLICY(SSL_INCOMING_STREAM_POLICY_AUTO)
OP_S_NEW_STREAM_BIDI(h, S_BIDI_ID(3)) OP_S_WRITE(h, "UNSEEN", 6)
OP_S_CONCLUDE(h)
...
OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_NEW_STREAM_BIDI(b, C_BIDI_ID(1))
...
OP_C_ACCEPT_STREAM_WAIT(a) OP_C_FREE_STREAM(a) OP_C_ACCEPT_STREAM_NONE()
...
OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_AUTO_UNI)
OP_S_NEW_STREAM_BIDI(a, S_BIDI_ID(0)) OP_S_WRITE(a, "apple", 5)
OP_C_READ_EXPECT(DEFAULT, "apple", 5) OP_C_WRITE(DEFAULT, "orange", 6)
...
OP_C_SHUTDOWN_WAIT(NULL, 0) OP_C_EXPECT_CONN_CLOSE_INFO(0, 1, 0)
OP_S_EXPECT_CONN_CLOSE_INFO(0, 1, 1)

OP_END};

/* 11. Many threads accepted on the same client connection */
static const struct script_op script_11_child[] = {OP_C_ACCEPT_STREAM_WAIT(
a) OP_C_READ_EXPECT(a, "foo", 3) OP_SLEEP(10) OP_C_EXPECT_FIN(a)
...
static const struct script_op script_12_child[] = {OP_C_NEW_STREAM_BIDI(
a, ANY_ID) OP_C_WRITE(a, "foo", 3) OP_C_CONCLUDE(a) OP_C_FREE_STREAM(a)
...
OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "foo", 3)
OP_S_EXPECT_FIN(a) OP_S_BIND_STREAM_ID(b, C_BIDI_ID(1))
OP_S_READ_EXPECT(b, "foo", 3) OP_S_EXPECT_FIN(b)
OP_S_BIND_STREAM_ID(c, C_BIDI_ID(2)) OP_S_READ_EXPECT(c, "foo", 3)
OP_S_EXPECT_FIN(c) OP_S_BIND_STREAM_ID(d, C_BIDI_ID(3))
OP_S_READ_EXPECT(d, "foo", 3) OP_S_EXPECT_FIN(d)
OP_S_BIND_STREAM_ID(e, C_BIDI_ID(4)) OP_S_READ_EXPECT(e, "foo", 3)
OP_S_EXPECT_FIN(e)
...
OP_C_ACCEPT_STREAM_WAIT(a) OP_C_READ_EXPECT(a, "foo", 3) OP_C_EXPECT_FIN(a)
OP_C_FREE_STREAM(a)
...
OP_C_NEW_STREAM_BIDI(a, ANY_ID) OP_C_WRITE(a, "foo", 3) OP_C_CONCLUDE(a)
OP_C_FREE_STREAM(a)
...
OP_S_ACCEPT_STREAM_WAIT(a) OP_S_READ_EXPECT(a, "foo", 3) OP_S_EXPECT_FIN(a)
OP_S_UNBIND_STREAM_ID(a)
...
OP_C_WRITE(a, "foo", 3) OP_C_CONCLUDE(a) OP_C_FREE_STREAM(a)
...
OP_S_ACCEPT_STREAM_WAIT(b) OP_S_READ_EXPECT(b, "foo", 3) OP_S_EXPECT_FIN(b)
...
OP_C_ACCEPT_STREAM_WAIT(b) OP_C_READ_EXPECT(b, "foo", 3) OP_C_EXPECT_FIN(b)
OP_C_FREE_STREAM(b)
...
static int script_20_wait(struct helper *h, volatile uint64_t *counter,
                          uint64_t threshold) {
#if defined(OPENSSL_THREADS)
    int stop = 0;
...
OP_POP_ERR() OP_EXPECT_ERR_LIB(ERR_LIB_SSL)
...
static int script_21_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr,
    ...
    if (!TEST_true(
        WPACKET_init_static_len(&wpkt, frame_buf, sizeof(frame_buf), 0)))
        return 0;
...
OP_S_SET_INJECT_PLAIN(script_21_inject_plain) OP_C_SET_ALPN("ossltest")
OP_C_CONNECT_WAIT()
...
OP_S_SET_INJECT_PLAIN(script_22_inject_plain) OP_C_SET_ALPN("ossltest")
OP_C_CONNECT_WAIT()
...
static int script_23_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr,
    ...
    if (!TEST_true(
        WPACKET_quic_write_vlint(&wpkt, OSSL_QUIC_FRAME_TYPE_NEW_TOKEN)) ||
...
OP_S_SET_INJECT_PLAIN(script_23_inject_plain) OP_C_SET_ALPN("ossltest")
OP_C_CONNECT_WAIT()
...
OP_S_SET_INJECT_PLAIN(script_24_inject_plain) OP_C_SET_ALPN("ossltest")
OP_C_CONNECT_WAIT()
...
OP_S_SET_INJECT_PLAIN(script_28_inject_plain) OP_C_SET_ALPN("ossltest")
OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
...
/* 31. Fault injection - received STOP_SENDING for nonexistent receive-only
 * stream */
...
OP_S_SET_INJECT_PLAIN(script_32_inject_plain) OP_C_SET_ALPN("ossltest")
OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
...
static int script_39_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr,
    ...
    if (!TEST_true(
        WPACKET_quic_write_vlint(&wpkt, OSSL_QUIC_FRAME_TYPE_NEW_CONN_ID)) ||
        !TEST_true(WPACKET_quic_write_vlint(&wpkt, seq_no)) /* seq no */
        || !TEST_true(WPACKET_quic_write_vlint(
           &wpkt, retire_prior_to)) /* retire prior to */
...
OP_S_SET_INJECT_PLAIN(script_39_inject_plain) OP_C_SET_ALPN("ossltest")
OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
...
static void script_41_trace(int write_p, int version, int content_type,
    ...
    if (version != OSSL_QUIC1_VERSION ||
        content_type != SSL3_RT_QUIC_FRAME_FULL || len < 1)
        return;
    ...
    if (!TEST_true(
        ossl_quic_wire_peek_frame_header(&pkt, &frame_type, &was_minimal))) {
        ++h->scratch1;
        return;
    }
    ...
    if (!TEST_true(
        ossl_quic_wire_decode_frame_path_response(&pkt, &frame_data)) ||
...
OP_S_SET_INJECT_PLAIN(script_41_inject_plain) OP_C_SET_ALPN("ossltest")
OP_C_CONNECT_WAIT() OP_CHECK(script_41_setup, 0)
...
static int script_42_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr,
    ...
    if (!TEST_true(
        WPACKET_quic_write_vlint(&wpkt, OSSL_QUIC_FRAME_TYPE_CRYPTO)) ||
...
OP_S_SET_INJECT_PLAIN(script_42_inject_plain) OP_C_SET_ALPN("ossltest")
OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
...
OP_S_SET_INJECT_PLAIN(script_44_inject_plain) OP_C_SET_ALPN("ossltest")
OP_C_CONNECT_WAIT()
...
static int wait_incoming_acks_increased(struct helper *h,
...
static int script_46_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr,
    ...
    if (!TEST_true(WPACKET_quic_write_vlint(&wpkt, type)) ||
        ...
        !TEST_true(
        WPACKET_quic_write_vlint(&wpkt, /*ack_range_count=*/range_count)) ||
        !TEST_true(
        WPACKET_quic_write_vlint(&wpkt, /*first_ack_range=*/first_range)))
        goto err;

    if (range_count > 0)
        if (!TEST_true(
            WPACKET_quic_write_vlint(&wpkt, /*range[0].gap=*/agap)) ||
...
OP_S_SET_INJECT_PLAIN(script_46_inject_plain) OP_C_SET_ALPN("ossltest")
OP_C_CONNECT_WAIT()
...
OP_S_SET_INJECT_PLAIN(script_52_inject_plain) OP_C_SET_ALPN("ossltest")
OP_C_CONNECT_WAIT()
...
OP_S_SET_INJECT_PLAIN(script_53_inject_plain) OP_C_SET_ALPN("ossltest")
OP_C_CONNECT_WAIT()
...
static int script_54_inject_handshake(struct helper *h, unsigned char *buf,
                                      size_t buf_len) {
    size_t i;
...
OP_S_SET_INJECT_HANDSHAKE(script_54_inject_handshake) OP_C_SET_ALPN("ossltest")
...
OP_SET_INJECT_WORD(0, 4) OP_S_WRITE(a, "orange", 5)
 ...
 * communicate.
 */
OP_SET_INJECT_WORD(0, 5) OP_S_WRITE(a, "raspberry", 9)
...
static int script_58_inject_plain(struct helper *h, QUIC_PKT_HDR *hdr,
    ...
    if (h->inject_word0 == 1) {
        if (!TEST_true(WPACKET_quic_write_vlint(
            &wpkt, OSSL_QUIC_FRAME_TYPE_HANDSHAKE_DONE)))
            goto err;
    } else {
...
OP_S_SET_INJECT_PLAIN(script_58_inject_plain) OP_C_SET_ALPN("ossltest")
OP_C_CONNECT_WAIT()
...
static int check_shutdown_reason(struct helper *h, struct helper_local *hl) {
    ...
    if (!TEST_size_t_ge(tc->reason_len, 50) ||
        !TEST_mem_eq(long_reason, tc->reason_len, tc->reason, tc->reason_len))
        return 0;
...
OP_CHECK(init_reason, 0) OP_C_SHUTDOWN_WAIT(long_reason, 0)
...
OP_S_SET_INJECT_PLAIN(script_61_inject_plain) OP_C_SET_ALPN("ossltest")
OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
...
OP_SET_INJECT_WORD(OSSL_QUIC_FRAME_TYPE_RESET_STREAM,
                   S_BIDI_ID(OSSL_QUIC_VLINT_MAX / 4)) OP_S_WRITE(a, "fruit", 5)
...
OP_SET_INJECT_WORD(OSSL_QUIC_FRAME_TYPE_STOP_SENDING,
                   C_BIDI_ID(OSSL_QUIC_VLINT_MAX / 4)) OP_S_WRITE(a, "fruit", 5)
...
OP_SET_INJECT_WORD(S_BIDI_ID(20) + 1, 1) OP_S_WRITE(a, "orange", 6)
...
OP_S_SET_INJECT_PLAIN(script_65_inject_plain) OP_C_SET_ALPN("ossltest")
OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
...
OP_SET_INJECT_WORD(1, 0) OP_S_WRITE(a, "orange", 6)
OP_C_READ_EXPECT(a, "orange", 6)
...
OP_S_SET_INJECT_PLAIN(script_66_inject_plain) OP_C_SET_ALPN("ossltest")
OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
...
static int script_68_inject_handshake(struct helper *h, unsigned char *msg,
    ...
    if (!TEST_true(
        qtest_fault_resize_message(h->qtf, datalen - SSL3_HM_HEADER_LENGTH)))
        return 0;
...
OP_S_SET_INJECT_HANDSHAKE(script_68_inject_handshake) OP_C_SET_ALPN("ossltest")
OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
...
OSSL_QUIC_ERR_CRYPTO_ERR_BEGIN + SSL_AD_UNEXPECTED_MESSAGE, 0, 0)

OP_END};

static int set_max_early_data(struct helper *h, struct helper_local *hl) {

    if (!TEST_true(ossl_quic_tserver_set_max_early_data(
        ACQUIRE_S(), (uint32_t)hl->check_op->arg2)))
        return 0;
...
OP_CHECK(set_max_early_data, 0xfffffffe) OP_S_NEW_TICKET()
OP_S_WRITE(a, "orange", 6)
...
OP_CHECK(set_max_early_data, 0xffffffff) OP_S_NEW_TICKET()
OP_S_WRITE(a, "orange", 6) OP_C_READ_EXPECT(a, "orange", 6)
...
OP_C_SKIP_IF_UNBOUND(a, 2) OP_C_WRITE(a, "apple", 5) OP_C_FREE_STREAM(a)
...
static int server_gen_version_neg(struct helper *h, BIO_MSG *msg,
                                  size_t stride) {
...
static void script_74_finish_mutation(void *arg) { OPENSSL_free(hdr_to_free); }
...
OP_CHECK(script_74_arm_packet_mutator, 0) OP_C_SET_ALPN("ossltest")
OP_C_CONNECT_WAIT()
...
OP_S_SET_INJECT_DATAGRAM(server_gen_version_neg) OP_SET_INJECT_WORD(2, 0)
...
static int script_76_check(struct helper *h, struct helper_local *hl) {
    if (!TEST_false(SSL_shutdown_ex(
        h->c_conn, SSL_SHUTDOWN_FLAG_WAIT_PEER | SSL_SHUTDOWN_FLAG_NO_BLOCK,
        NULL, 0)))
        return 0;
...
static int trigger_late_session_ticket(struct helper *h,
                                       struct helper_local *hl) {
    new_session_count = 0;
...
OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
OP_S_EXPECT_FIN(a) OP_S_WRITE(a, "orange", 6) OP_S_CONCLUDE(a)
OP_C_READ_EXPECT(DEFAULT, "orange", 6) OP_C_EXPECT_FIN(DEFAULT) OP_END};
...
OP_S_SET_INJECT_PLAIN(script_80_inject_pkt) OP_C_SET_ALPN("ossltest")
OP_C_CONNECT_WAIT() OP_C_WRITE(DEFAULT, "apple", 5) OP_C_CONCLUDE(DEFAULT)
OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
OP_SET_INJECT_WORD(1, 0) OP_S_WRITE(a, "apple", 5)
OP_C_READ_EXPECT(DEFAULT, "apple", 5) OP_SET_INJECT_WORD(0, 1)
OP_S_WRITE(a, "apple", 5) OP_C_EXPECT_CONN_CLOSE_INFO(0, 0, 1) OP_END};
...
static int modify_idle_timeout(struct helper *h, struct helper_local *hl) {
    ...
    if (!TEST_false(SSL_set_feature_request_uint(
        h->c_conn, SSL_VALUE_QUIC_IDLE_TIMEOUT, (1ULL << 62))))
        return 0;

    /* Set value. */
    if (!TEST_true(SSL_set_feature_request_uint(
        h->c_conn, SSL_VALUE_QUIC_IDLE_TIMEOUT, hl->check_op->arg2)))
        return 0;

    if (!TEST_true(SSL_get_feature_request_uint(
        h->c_conn, SSL_VALUE_QUIC_IDLE_TIMEOUT, &v)))
        return 0;
...
OP_C_SET_ALPN("ossltest") OP_CHECK(modify_idle_timeout, 25000)
OP_C_CONNECT_WAIT()
...
static int cannot_change_idle_timeout(struct helper *h,
                                      struct helper_local *hl) {
    uint64_t v = 0;
    ...
    if (!TEST_false(SSL_set_feature_request_uint(
        h->c_conn, SSL_VALUE_QUIC_IDLE_TIMEOUT, 5000)))
        return 0;
...
static int set_event_handling_mode_conn(struct helper *h,
                                        struct helper_local *hl);
static int reenable_test_event_handling(struct helper *h,
                                        struct helper_local *hl);
...
static int check_write_buf_stat(struct helper *h, struct helper_local *hl) {
    ...
    if (!TEST_true(SSL_get_stream_write_buf_size(c_a, &size)) ||
        ...
        !TEST_uint64_t_ge(size, avail) || !TEST_uint64_t_ge(size, used) ||
...
OP_CHECK2(check_avail_streams, 0, 99) OP_CHECK2(check_avail_streams, 1, 99)
OP_CHECK2(check_avail_streams, 2, 99) OP_CHECK2(check_avail_streams, 3, 99)

OP_CHECK2(check_write_buf_stat, 0, 0)
OP_CHECK(set_event_handling_mode_conn, SSL_VALUE_EVENT_HANDLING_MODE_EXPLICIT)
OP_C_WRITE(a, "apple", 5) OP_CHECK2(check_write_buf_stat, 5, 0)
...
ossl_unused static int script_85_poll(struct helper *h,
                                      struct helper_local *hl) {
    ...
    ret = SSL_poll(items, OSSL_NELEM(items), sizeof(SSL_POLL_ITEM), &timeout, 0,
                   &result_count);
    ...
    for (i = 0; i < OSSL_NELEM(items); ++i)
        if (!TEST_uint64_t_eq(items[i].revents, expected_revents[i])) {
            TEST_error("mismatch at index %zu in poll results, mode %d", i,
                       (int)mode);
            ok = 0;
        }
...
static ossl_unused int set_event_handling_mode_stream(struct helper *h,
                                                      struct helper_local *hl) {
...
OP_C_WRITE(a, "orange", 6) OP_C_CONCLUDE(a)
...
OP_CHECK(reenable_test_event_handling, 0) OP_S_READ_EXPECT(a, "appleorange", 11)
OP_S_EXPECT_FIN(a)

/* Back into explicit mode. */
OP_CHECK(set_event_handling_mode_conn, SSL_VALUE_EVENT_HANDLING_MODE_EXPLICIT)
OP_S_WRITE(a, "ok", 2) OP_C_READ_FAIL(a)
...
OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5) OP_C_CONCLUDE(a)
OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
OP_S_EXPECT_FIN(a) OP_S_WRITE(a, "orange", 6) OP_C_READ_EXPECT(a, "orange", 6)
OP_S_CONCLUDE(a) OP_C_EXPECT_FIN(a) OP_SLEEP(1000) OP_C_STREAM_RESET_FAIL(a, 42)
OP_END};

static const struct script_op *const scripts[] = {
script_1,  script_2,  script_3,  script_4,  script_5,  script_6,  script_7,
script_8,  script_9,  script_10, script_11, script_12, script_13, script_14,
script_15, script_16, script_17, script_18, script_19, script_20, script_21,
script_22, script_23, script_24, script_25, script_26, script_27, script_28,
script_29, script_30, script_31, script_32, script_33, script_34, script_35,
script_36, script_37, script_38, script_39, script_40, script_41, script_42,
script_43, script_44, script_45, script_46, script_47, script_48, script_49,
script_50, script_51, script_52, script_53, script_54, script_55, script_56,
script_57, script_58, script_59, script_60, script_61, script_62, script_63,
script_64, script_65, script_66, script_67, script_68, script_69, script_70,
script_71, script_72, script_73, script_74, script_75, script_76, script_77,
script_78, script_79, script_80, script_81, script_82, script_83, script_84,
script_85, script_86, script_87};
...
{QUIC_PKT_TYPE_INITIAL, OSSL_QUIC_VLINT_MAX,
 OSSL_QUIC_ERR_FRAME_ENCODING_ERROR},
{QUIC_PKT_TYPE_HANDSHAKE, OSSL_QUIC_VLINT_MAX,
 OSSL_QUIC_ERR_FRAME_ENCODING_ERROR},
{QUIC_PKT_TYPE_1RTT, OSSL_QUIC_VLINT_MAX, OSSL_QUIC_ERR_FRAME_ENCODING_ERROR},

{QUIC_PKT_TYPE_INITIAL, OSSL_QUIC_FRAME_TYPE_STREAM,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_INITIAL, OSSL_QUIC_FRAME_TYPE_RESET_STREAM,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_INITIAL, OSSL_QUIC_FRAME_TYPE_STOP_SENDING,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_INITIAL, OSSL_QUIC_FRAME_TYPE_NEW_TOKEN,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_INITIAL, OSSL_QUIC_FRAME_TYPE_MAX_DATA,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_INITIAL, OSSL_QUIC_FRAME_TYPE_MAX_STREAM_DATA,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_INITIAL, OSSL_QUIC_FRAME_TYPE_MAX_STREAMS_BIDI,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_INITIAL, OSSL_QUIC_FRAME_TYPE_MAX_STREAMS_UNI,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_INITIAL, OSSL_QUIC_FRAME_TYPE_DATA_BLOCKED,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_INITIAL, OSSL_QUIC_FRAME_TYPE_STREAM_DATA_BLOCKED,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_INITIAL, OSSL_QUIC_FRAME_TYPE_STREAMS_BLOCKED_BIDI,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_INITIAL, OSSL_QUIC_FRAME_TYPE_STREAMS_BLOCKED_UNI,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_INITIAL, OSSL_QUIC_FRAME_TYPE_NEW_CONN_ID,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_INITIAL, OSSL_QUIC_FRAME_TYPE_RETIRE_CONN_ID,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_INITIAL, OSSL_QUIC_FRAME_TYPE_PATH_CHALLENGE,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_INITIAL, OSSL_QUIC_FRAME_TYPE_PATH_RESPONSE,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_INITIAL, OSSL_QUIC_FRAME_TYPE_CONN_CLOSE_APP,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_INITIAL, OSSL_QUIC_FRAME_TYPE_HANDSHAKE_DONE,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},

{QUIC_PKT_TYPE_HANDSHAKE, OSSL_QUIC_FRAME_TYPE_STREAM,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_HANDSHAKE, OSSL_QUIC_FRAME_TYPE_RESET_STREAM,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_HANDSHAKE, OSSL_QUIC_FRAME_TYPE_STOP_SENDING,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_HANDSHAKE, OSSL_QUIC_FRAME_TYPE_NEW_TOKEN,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_HANDSHAKE, OSSL_QUIC_FRAME_TYPE_MAX_DATA,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_HANDSHAKE, OSSL_QUIC_FRAME_TYPE_MAX_STREAM_DATA,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_HANDSHAKE, OSSL_QUIC_FRAME_TYPE_MAX_STREAMS_BIDI,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_HANDSHAKE, OSSL_QUIC_FRAME_TYPE_MAX_STREAMS_UNI,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_HANDSHAKE, OSSL_QUIC_FRAME_TYPE_DATA_BLOCKED,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_HANDSHAKE, OSSL_QUIC_FRAME_TYPE_STREAM_DATA_BLOCKED,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_HANDSHAKE, OSSL_QUIC_FRAME_TYPE_STREAMS_BLOCKED_BIDI,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_HANDSHAKE, OSSL_QUIC_FRAME_TYPE_STREAMS_BLOCKED_UNI,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_HANDSHAKE, OSSL_QUIC_FRAME_TYPE_NEW_CONN_ID,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_HANDSHAKE, OSSL_QUIC_FRAME_TYPE_RETIRE_CONN_ID,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_HANDSHAKE, OSSL_QUIC_FRAME_TYPE_PATH_CHALLENGE,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_HANDSHAKE, OSSL_QUIC_FRAME_TYPE_PATH_RESPONSE,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_HANDSHAKE, OSSL_QUIC_FRAME_TYPE_CONN_CLOSE_APP,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
{QUIC_PKT_TYPE_HANDSHAKE, OSSL_QUIC_FRAME_TYPE_HANDSHAKE_DONE,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},

/* Client uses a zero-length CID so this is not allowed. */
{QUIC_PKT_TYPE_1RTT, OSSL_QUIC_FRAME_TYPE_RETIRE_CONN_ID,
 OSSL_QUIC_ERR_PROTOCOL_VIOLATION},
};
