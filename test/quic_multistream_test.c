...
static int
helper_packet_plain_listener(QTEST_FAULT *qtf, QUIC_PKT_HDR *hdr, unsigned char *buf, size_t buf_len, void *arg)
{
    struct helper *h = arg;
...
#define C_SPIN_AGAIN()                                  \
    ...
    for (;;) {
        ...
        case OPK_C_SET_ALPN: {
            ...
            if (!TEST_false(SSL_set_alpn_protos(h->c_conn, tmp_buf, (unsigned int)(alpn_len + 1))))
                goto out;
        ...
        case OPK_C_SET_INCOMING_STREAM_POLICY: {
            ...
            if (!TEST_true(SSL_set_incoming_stream_policy(c_tgt, (int)op->arg1, 0)))
                goto out;
        } break;
        ...
        case OPK_S_WRITE_FAIL: {
            ...
            if (!TEST_false(ossl_quic_tserver_write(ACQUIRE_S(),
                                                    s_stream_id,
        ...
        case OPK_S_SET_INJECT_HANDSHAKE:
            ...
            if (!TEST_true(
                    qtest_fault_set_handshake_listener(h->qtf,
        ...
        case OPK_S_SET_INJECT_DATAGRAM:
            ...
            if (!TEST_true(
                    qtest_fault_set_datagram_listener(h->qtf,
        ...
        case OPK_C_INHIBIT_TICK: {
            ...
            ossl_quic_engine_set_inhibit_tick(ossl_quic_channel_get0_engine(ch), (int)op->arg1);
        } break;
...
static const struct script_op script_2[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_SET_INCOMING_STREAM_POLICY(
        SSL_INCOMING_STREAM_POLICY_ACCEPT) OP_C_WRITE(DEFAULT, "apple", 5) OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0))
        OP_S_READ_EXPECT(a, "apple", 5) OP_S_WRITE(a, "orange", 6) OP_C_READ_EXPECT(DEFAULT, "orange", 6)

            OP_C_NEW_STREAM_BIDI(b, C_BIDI_ID(1)) OP_C_WRITE(b, "flamingo", 8) OP_C_CONCLUDE(
                b) OP_S_BIND_STREAM_ID(b, C_BIDI_ID(1)) OP_S_READ_EXPECT(b, "flamingo", 8) OP_S_EXPECT_FIN(b)
                OP_S_WRITE(b, "gargoyle", 8) OP_S_CONCLUDE(b) OP_C_READ_EXPECT(b, "gargoyle", 8) OP_C_EXPECT_FIN(b)

                    OP_C_NEW_STREAM_UNI(c, C_UNI_ID(0)) OP_C_WRITE(c, "elephant", 8) OP_C_CONCLUDE(
                        c) OP_S_BIND_STREAM_ID(c, C_UNI_ID(0)) OP_S_READ_EXPECT(c,
                                                                                "elephant",
                        ...
                        OP_C_ACCEPT_STREAM_NONE()
                            ...
                            OP_S_NEW_STREAM_BIDI(d, S_BIDI_ID(0)) OP_S_WRITE(d, "frog", 4) OP_S_CONCLUDE(d)
                                ...
                                OP_C_ACCEPT_STREAM_WAIT(
                                    d) OP_C_ACCEPT_STREAM_NONE() OP_C_READ_EXPECT(d, "frog", 4) OP_C_EXPECT_FIN(d)

                                    OP_S_NEW_STREAM_BIDI(e, S_BIDI_ID(1)) OP_S_WRITE(e, "mixture", 7) OP_S_CONCLUDE(e)

                                        OP_C_ACCEPT_STREAM_WAIT(e) OP_C_READ_EXPECT(e, "mixture", 7) OP_C_EXPECT_FIN(e)
                                            OP_C_WRITE(e, "ramble", 6) OP_S_READ_EXPECT(e, "ramble", 6) OP_C_CONCLUDE(
                                                e) OP_S_EXPECT_FIN(e)

                                                OP_S_NEW_STREAM_UNI(
                                                    f,
                                                    S_UNI_ID(0)) OP_S_WRITE(f, "yonder", 6) OP_S_CONCLUDE(f)
                                                        ...
                                                        OP_C_READ_EXPECT(f, "yonder", 6) OP_C_EXPECT_FIN(
                                                            ...
                                                            OP_C_SET_INCOMING_STREAM_POLICY(
                                                                ...
                                                                OP_S_NEW_STREAM_BIDI(g, S_BIDI_ID(2))
                                                                    OP_S_WRITE(g, "unseen", 6) OP_S_CONCLUDE(g)
...
static void
script_41_trace(int write_p, int version, int content_type, const void *buf, size_t len, SSL *ssl, void *arg)
{
...
static int check_idle_timeout(struct helper *h, struct helper_local *hl)
{
    ...
    if (!TEST_true(SSL_get_value_uint(h->c_conn, (uint32_t)hl->check_op->arg1, SSL_VALUE_QUIC_IDLE_TIMEOUT, &v)))
        return 0;
...
static const struct script_op script_84[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
            ...
            OP_CHECK2(check_avail_streams, 0, 100) OP_CHECK2(check_avail_streams, 1, 100)
                OP_CHECK2(check_avail_streams, 2, 100) OP_CHECK2(check_avail_streams, 3, 100)

                    OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0))

                        OP_CHECK2(check_avail_streams, 0, 99) OP_CHECK2(check_avail_streams, 1, 100)
                            OP_CHECK2(check_avail_streams, 2, 100) OP_CHECK2(check_avail_streams, 3, 100)
                                        ...
                                        OP_CHECK2(check_avail_streams, 2, 99) OP_CHECK2(check_avail_streams, 3, 100)
                                            ...
                                            OP_S_NEW_STREAM_BIDI(c, S_BIDI_ID(0)) OP_S_WRITE(c, "x", 1)
                                                ...
                                                OP_C_ACCEPT_STREAM_WAIT(c) OP_C_READ_EXPECT(c, "x", 1)
                                                    ...
                                                    OP_CHECK2(check_avail_streams, 0, 99) OP_CHECK2(
                                                        check_avail_streams,
                                                        1,
                                                        99) OP_CHECK2(check_avail_streams,
                                                                      2,
                                                                      99) OP_CHECK2(check_avail_streams, 3, 100)
                                                        ...
                                                        OP_S_NEW_STREAM_UNI(d, S_UNI_ID(0)) OP_S_WRITE(d, "x", 1)
                                                            ...
                                                            OP_C_ACCEPT_STREAM_WAIT(d) OP_C_READ_EXPECT(d, "x", 1)
                                                                ...
                                                                OP_CHECK2(check_avail_streams, 0, 99) OP_CHECK2(
                                                                    check_avail_streams,
                                                                    1,
                                                                    99) OP_CHECK2(check_avail_streams, 2, 99)
                                                                    OP_CHECK2(check_avail_streams, 3, 99)
                                                                        ...
                                                                        OP_CHECK2(check_write_buf_stat, 0, 0) OP_CHECK(
                                                                            ...
                                                                            OP_C_WRITE(a, "apple", 5) OP_CHECK2(
                                                                                check_write_buf_stat,
                                                                                5,
                                                                                0)

                                                                                OP_CHECK(reenable_test_event_handling,
                                                                                         0)
