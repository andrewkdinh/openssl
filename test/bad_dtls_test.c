...
#define SS_SESSID_OFS 15  /* Session ID goes here */
  ...
  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04, 0x30, /* OCTET_STRING,
                                                                 master secret
                                                               */
...
#define SH_SESSID_OFS 60 /* Session ID goes here */
  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x2f, /* Cipher suite
                                                                 AES128-SHA */
...
NODROP(1),         NODROP(3),         NODROP(2),        NODROP(0x1234),
NODROP(0x1230),    NODROP(0x1235),    NODROP(0xffff),   NODROP(0x10001),
NODROP(0xfffe),    NODROP(0x10000),   DROP(0x10001),    DROP(0xff),
NODROP(0x100000),  NODROP(0x800000),  NODROP(0x7fffe1), NODROP(0xffffff),
NODROP(0x1000000), NODROP(0xfffffe),  DROP(0xffffff),   NODROP(0x1000010),
  ...
  for (i = 0; i < (int)OSSL_NELEM(tests); i++) {
    ...
    if (!TEST_true(send_record(rbio, SSL3_RT_APPLICATION_DATA, tests[i].seq,
      ...
      TEST_error("Failed to send data seq #0x%x%08x (%d)\n",
                 (unsigned int)(tests[i].seq >> 32), (unsigned int)tests[i].seq,
                 i);
      goto end;
    }
