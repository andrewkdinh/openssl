...
static APK_DATA keydata_v2[] = {
    {kExampleRSAKeyPKCS8_v2, sizeof(kExampleRSAKeyPKCS8_v2), "RSA",
     EVP_PKEY_RSA},
#ifndef OPENSSL_NO_EC
...
static EVP_PKEY *load_example_hmac_key(void)
{
    ...
    pkey = EVP_PKEY_new_raw_private_key_ex(testctx, "HMAC", NULL, key,
                                           sizeof(key));
    if (!TEST_ptr(pkey))
        return NULL;
...
static unsigned char ec_pub[] = {
    0x04, 0xad, 0x11, 0x90, 0x77, 0x4b, 0x46, 0xee, 0x72, 0x51, 0x15,
    0x97, 0x4a, 0x6a, 0xa7, 0xaf, 0x59, 0xfa, 0x4b, 0xf2, 0x41, 0xc8,
    0x3a, 0x81, 0x23, 0xb6, 0x90, 0x04, 0x6c, 0x67, 0x66, 0xd0, 0xdc,
    0xf2, 0x15, 0x1d, 0x41, 0x61, 0xb7, 0x95, 0x85, 0x38, 0x5a, 0x84,
    0x56, 0xe8, 0xb3, 0x0e, 0xf5, 0xc6, 0x5d, 0xa4, 0x54, 0x26, 0xb0,
    0xf7, 0xa5, 0x4a, 0x33, 0xf1, 0x08, 0x09, 0xb8, 0xdb, 0x03};
...
static int test_EC_priv_pub(void)
{
    ...
    if (!TEST_int_eq(EVP_PKEY_get_octet_string_param(
                         params_and_pub, OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY,
...
static int test_EVP_PKEY_sign(int tst)
{
    ...
    if (!TEST_ptr(ctx) || !TEST_int_gt(EVP_PKEY_sign_init(ctx), 0)
        || !TEST_int_gt(EVP_PKEY_sign(ctx, NULL, &sig_len, tbs, sizeof(tbs)),
                        0))
        goto out;
...
static struct keys_st {
    ...
    {EVP_PKEY_POLY1305, (const uint8_t *)"01234567890123456789012345678901",
     NULL, NULL, 32, 0
#endif
#ifndef OPENSSL_NO_SIPHASH
    },
    ...
    {NID_undef, ml_kem_seed, ml_kem_1024_pubkey, "ML-KEM-1024",
     ML_KEM_SEED_BYTES, sizeof(ml_kem_1024_pubkey)},
#endif
};

#ifndef OPENSSL_NO_ML_KEM
static int ml_kem_seed_to_priv(const char *alg, const unsigned char *seed,
                               int seedlen, unsigned char **ret, size_t *retlen)
{
...
static int test_set_get_raw_keys_int(int tst, int pub, int uselibctx)
{
    ...
    if (tst != 1) {
        ...
        if ((!pub
             && !TEST_false(
                 EVP_PKEY_get_raw_private_key(pkey, shortbuf, &shortlen)))
            || (pub
                && !TEST_false(
...
static int test_HKDF(void)
{
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (!TEST_int_gt(EVP_PKEY_derive_init(pctx), 0)
            ...
            || !TEST_int_gt(
                EVP_PKEY_CTX_set1_hkdf_key(pctx, key, sizeof(key) - 1), 0)
            || !TEST_int_gt(
...
static int test_empty_salt_info_HKDF(void)
{
    ...
    if (!TEST_int_gt(EVP_PKEY_derive_init(pctx), 0)
        || !TEST_int_gt(EVP_PKEY_CTX_set_hkdf_md(pctx, EVP_sha256()), 0)
        || !TEST_int_gt(
            EVP_PKEY_CTX_set1_hkdf_salt(pctx, fake, sizeof(fake) - 1), 0)
        || !TEST_int_gt(
            EVP_PKEY_CTX_set1_hkdf_salt(pctx, salt, sizeof(salt) - 1), 0)
        || !TEST_int_gt(EVP_PKEY_CTX_set1_hkdf_key(pctx, key, sizeof(key) - 1),
                        0)
...
static int test_EVP_PKEY_CTX_get_set_params(EVP_PKEY *pkey)
{
    ...
    if (!TEST_ptr(params)
        || !TEST_ptr(
            OSSL_PARAM_locate_const(params, OSSL_SIGNATURE_PARAM_DIGEST)))
        goto err;
...
static int test_EVP_rsa_pss_set_saltlen(void)
{
    ...
    ret = TEST_ptr(pkey = load_example_rsa_key())
        ...
        && TEST_true(
              EVP_DigestSignInit(sha256_ctx, &pkey_ctx, sha256, NULL, pkey))
        && TEST_true(
              EVP_PKEY_CTX_set_rsa_padding(pkey_ctx, RSA_PKCS1_PSS_PADDING))
        && TEST_int_gt(EVP_PKEY_CTX_set_rsa_pss_saltlen(pkey_ctx, test_value),
                       0)
...
static const EVP_INIT_TEST_st evp_init_tests[] = {
    {"aes-128-cfb", kCFBDefaultKey, iCFBIV, cfbPlaintext, cfbCiphertext, NULL,
     0, sizeof(cfbPlaintext), sizeof(cfbCiphertext), 0, 1, 0, 1},
    ...
    {"aes-128-cfb", kCFBDefaultKey, iCFBIV, cfbPlaintext, cfbCiphertext, NULL,
     0, sizeof(cfbPlaintext), sizeof(cfbCiphertext), 0, 0, 0, 1},
    {"aes-256-gcm", kGCMDefaultKey, iGCMDefaultIV, gcmDefaultPlaintext,
     gcmDefaultCiphertext, gcmDefaultTag, sizeof(iGCMDefaultIV),
     sizeof(gcmDefaultPlaintext), sizeof(gcmDefaultCiphertext),
     sizeof(gcmDefaultTag), 0, 0, 1},
    {"aes-128-cfb", kCFBDefaultKey, iCFBIV, cfbCiphertext, cfbPlaintext, NULL,
     0, sizeof(cfbCiphertext), sizeof(cfbPlaintext), 0, 1, 1, 0},
    ...
    {"aes-128-cfb", kCFBDefaultKey, iCFBIV, cfbCiphertext, cfbPlaintext, NULL,
     0, sizeof(cfbCiphertext), sizeof(cfbPlaintext), 0, 0, 1, 0},
    {"aes-256-gcm", kGCMDefaultKey, iGCMDefaultIV, gcmDefaultCiphertext,
     gcmDefaultPlaintext, gcmDefaultTag, sizeof(iGCMDefaultIV),
     sizeof(gcmDefaultCiphertext), sizeof(gcmDefaultPlaintext),
...
static const EVP_RESET_TEST_st evp_reset_tests[] = {
    {cfbPlaintext, cfbCiphertext, sizeof(cfbPlaintext), sizeof(cfbCiphertext),
     1},
    {cfbCiphertext, cfbPlaintext, sizeof(cfbCiphertext), sizeof(cfbPlaintext),
     0}};
...
static int test_evp_reset(int idx)
{
    ...
    if (!TEST_true(EVP_CipherInit_ex(ctx, type, NULL, kCFBDefaultKey, iCFBIV,
                                     t->enc))) {
        errmsg = "CIPHER_INIT";
        goto err;
    }
...
static int test_evp_updated_iv(int idx)
{
    ...
    if (!TEST_true(EVP_CIPHER_CTX_get_updated_iv(ctx, updated_iv,
                                                 sizeof(updated_iv)))) {
...
static int test_custom_md_meth(void)
{
    ...
    if (!TEST_true(EVP_MD_meth_set_init(tmp, custom_md_init))
        ...
        || !TEST_true(
            EVP_MD_meth_set_app_datasize(tmp, sizeof(custom_dgst_ctx))))
        goto err;
...
static int test_sign_continuation(void)
{
    ...
    if (!TEST_ptr(mctx = EVP_MD_CTX_new())
        || !TEST_true(EVP_DigestSignInit_ex(mctx, NULL, NULL, testctx, NULL,
                                            pkey, nodup_params))
        || !TEST_true(EVP_DigestSignUpdate(mctx, sigbuf, sizeof(sigbuf)))
        || !TEST_true(EVP_DigestSignFinal(mctx, signature, &siglen))
                           ...
                           const unsigned char *gcm_tag, size_t gcm_tag_s)
{
    ...
    if (!TEST_true(EVP_EncryptInit_ex2(ctx, cipher, gcm_key, gcm_iv, params))
        || (gcm_aad != NULL
            && !TEST_true(
                EVP_EncryptUpdate(ctx, NULL, &outlen, gcm_aad, gcm_aad_s)))
                           ...
                           const unsigned char *gcm_tag, size_t gcm_tag_s)
{
    ...
    if (!TEST_true(EVP_DecryptInit_ex2(ctx, cipher, gcm_key, gcm_iv, params))
        || (gcm_aad != NULL
            && !TEST_true(
                EVP_DecryptUpdate(ctx, NULL, &outlen, gcm_aad, gcm_aad_s)))
    ...
    for (numpipes = 1; numpipes <= EVP_MAX_PIPES; numpipes++) {
        for (plaintextlen = 1; plaintextlen <= 256; plaintextlen++) {
            ...
            params[0] = OSSL_PARAM_construct_octet_ptr(
                OSSL_CIPHER_PARAM_PIPELINE_AEAD_TAG, (void **)&aead_tags,
                taglen);
            if (!TEST_true(EVP_CIPHER_CTX_get_params(ctx, params)))
                goto err;
            ...
            for (i = 0; i < numpipes; i++) outsize_array[i] = plaintextlen;
