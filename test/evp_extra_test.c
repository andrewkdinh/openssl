...
static EVP_PKEY *make_key_fromdata(char *keytype, OSSL_PARAM *params) {
  ...
  if (!TEST_int_eq(EVP_PKEY_CTX_set_params(pctx, params), 0) ||
      ...
      !TEST_int_gt(EVP_PKEY_fromdata(pctx, &tmp_pkey, EVP_PKEY_KEYPAIR, params),
                   0))
    goto err;
...
static int test_evp_get_ec_pub_legacy(void) {
  ...
  if (!TEST_ptr(
      eckey = EC_KEY_new_by_curve_name_ex(libctx, NULL, NID_X9_62_prime256v1)))
    goto err;
...
static int test_EVP_PKEY_sign(int tst) {
  ...
  if (!TEST_ptr(sig)
      ...
      || !TEST_int_le(EVP_PKEY_sign(ctx, sig, &shortsig_len, tbs, sizeof(tbs)),
                      0) ||
...
static int test_siphash_digestsign(void) {
  ...
  memset(key, 1, 16);
  if (!TEST_ptr(pkey =
                EVP_PKEY_new_raw_private_key(EVP_PKEY_SIPHASH, NULL, key, 16)))
    goto out;
...
static int test_set_get_raw_keys_int(int tst, int pub, int uselibctx) {
  ...
  if (tst != 1) {
    ...
    if ((!pub && !TEST_false(
        ...
        (pub &&
         !TEST_false(EVP_PKEY_get_raw_public_key(pkey, shortbuf, &shortlen))))
      goto done;
  }
...
static int test_EVP_PKEY_CTX_get_set_params(EVP_PKEY *pkey) {
  ...
  *param_md = OSSL_PARAM_construct_utf8_string(OSSL_SIGNATURE_PARAM_DIGEST,
  ...
  if (!TEST_ptr(mdctx) || !TEST_true(EVP_DigestSignInit_ex(
                          mdctx, NULL, "SHA1", testctx, testpropq, pkey, NULL)))
    goto err;
...
static int test_EVP_PKEY_set1_DH(void) {
  ...
  if (!TEST_ptr(p = BN_new()) || !TEST_ptr(g = BN_new()) ||
      ...
      !TEST_true(BN_set_word(g, 2)) || !TEST_true(BN_set_word(pubkey, 4321)) ||
  ...
  if (!TEST_true(EVP_PKEY_get_octet_string_param(pkey2, OSSL_PKEY_PARAM_PUB_KEY,
                                                 pub, sizeof(pub), &len)) ||
...
static int test_evp_iv_aes(int idx) {
  ...
  case 6:
    type =
    (type != NULL) ? type : EVP_CIPHER_fetch(testctx, "aes-128-cbc", testpropq);
    ref_iv = cbc_state;
  ...
  case 7:
    type =
    (type != NULL) ? type : EVP_CIPHER_fetch(testctx, "aes-128-ofb", testpropq);
    ref_iv = ofb_state;
  ...
  case 8:
    type =
    (type != NULL) ? type : EVP_CIPHER_fetch(testctx, "aes-128-cfb", testpropq);
    ref_iv = cfb_state;
  ...
  case 9:
    type =
    (type != NULL) ? type : EVP_CIPHER_fetch(testctx, "aes-128-gcm", testpropq);
    ref_iv = gcm_state;
  ...
  case 10:
    type =
    (type != NULL) ? type : EVP_CIPHER_fetch(testctx, "aes-128-ccm", testpropq);
    ref_iv = ccm_state;
  ...
  case 11:
    type =
    (type != NULL) ? type : EVP_CIPHER_fetch(testctx, "aes-128-ocb", testpropq);
    ref_iv = ocb_state;
...
static int evp_init_seq_set_iv(EVP_CIPHER_CTX *ctx, const EVP_INIT_TEST_st *t) {
  ...
  if (t->ivlen != 0) {
    if (!TEST_int_gt(
        EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_SET_IVLEN, t->ivlen, NULL), 0))
      goto err;
  }
...
static int test_evp_init_seq(int idx) {
  ...
  if (t->finalenc == 0 && t->tag != NULL) {
    /* Set expected tag */
    if (!TEST_int_gt(EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG, t->taglen,
                                         (void *)t->tag),
                     0)) {
      errmsg = "SET_TAG";
      goto err;
...
static int test_gcm_reinit(int idx) {
  ...
  if (!TEST_int_gt(EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, taglen, tag),
                   0)) {
    errmsg = "GET_TAG1";
    goto err;
  }
...
static int test_signatures_with_engine(int tst) {
  ...
  case 1:
    pkey =
    EVP_PKEY_new_CMAC_key(e, badcmackey, sizeof(badcmackey), EVP_aes_128_cbc());
    break;
  case 2:
...
static int test_sign_continuation(void) {
  ...
  static const OSSL_PARAM nodup_params[] = {OSSL_PARAM_int("NO_DUP", &nodupnum),
                                            OSSL_PARAM_END};
  ...
  if (!TEST_ptr(
      pctx = EVP_PKEY_CTX_new_from_name(testctx, "RSA", "provider=fake-rsa")) ||
...
static int aes_gcm_encrypt(const unsigned char *gcm_key, size_t gcm_key_s,
  ...
  if (!TEST_true(EVP_EncryptInit_ex2(ctx, cipher, gcm_key, gcm_iv, params)) ||
      (gcm_aad != NULL &&
       !TEST_true(EVP_EncryptUpdate(ctx, NULL, &outlen, gcm_aad, gcm_aad_s))) ||
...
static int aes_gcm_decrypt(const unsigned char *gcm_key, size_t gcm_key_s,
  ...
  if (!TEST_true(EVP_DecryptInit_ex2(ctx, cipher, gcm_key, gcm_iv, params)) ||
      (gcm_aad != NULL &&
       !TEST_true(EVP_DecryptUpdate(ctx, NULL, &outlen, gcm_aad, gcm_aad_s))) ||
...
static int test_aes_gcm_ivlen_change_cve_2023_5363(void) {
  ...
  ,
  };
    ...
    for (plaintextlen = 1; plaintextlen <= 256; plaintextlen++) {
      ...
      for (i = 0; i < numpipes; i++) {
        if (!TEST_true(EVP_EncryptInit(ctx, cipher, key, iv_array[i])) ||
            !TEST_true(EVP_EncryptUpdate(ctx, NULL, &outlen, plaintext_array[i],
                                         plaintextlen)) ||
            !TEST_true(EVP_EncryptUpdate(ctx, ciphertext, &outlen,
                                         plaintext_array[i], plaintextlen)))
          goto err;
      ...
      if (!TEST_true(EVP_CIPHER_CTX_reset(ctx)) ||
          ...
          (const unsigned char **)ciphertext_array_p, ciphertextlen_array)))
        goto err;
      ...
      for (i = 0; i < numpipes; i++) {
        ...
        if (!TEST_mem_eq(plaintext_array[i], plaintextlen, exp_plaintext,
                         plaintextlen))
          goto err;
      }
