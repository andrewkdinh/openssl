...
static void free_checker(ossl_unused size_t *p) { num_rec_freed++; }
...
static int test_size_t_priority_queue_int(int reserve, int order, int count,
  ...
  TEST_info("testing count %d, %s, %s, values %s, remove %d, %sfree", count,
            ...
            random ? "random" : "deterministic", remove, popfree ? "pop " : "");
...
static int test_large_priority_queue(void) {
  return test_size_t_priority_queue_int(0, 0, MAX_SAMPLES, MAX_SAMPLES / 100, 1,
                                        1);
}
