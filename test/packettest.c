...
static int test_PACKET_get_1(void)
{
    ...
    if (!TEST_true(PACKET_buf_init(&pkt, smbuf, BUF_LEN))
        || !TEST_true(PACKET_get_1(&pkt, &i)) || !TEST_uint_eq(i, 0x02)
        || !TEST_true(PACKET_forward(&pkt, BUF_LEN - 2))
        || !TEST_true(PACKET_get_1(&pkt, &i)) || !TEST_uint_eq(i, 0xfe)
...
static int test_PACKET_get_4(void)
{
    ...
    if (!TEST_true(PACKET_buf_init(&pkt, smbuf, BUF_LEN))
        || !TEST_true(PACKET_get_4(&pkt, &i)) || !TEST_ulong_eq(i, 0x08060402UL)
        || !TEST_true(PACKET_forward(&pkt, BUF_LEN - 8))
        || !TEST_true(PACKET_get_4(&pkt, &i)) || !TEST_ulong_eq(i, 0xfefcfaf8UL)
...
static int test_PACKET_get_net_2(void)
{
    ...
    if (!TEST_true(PACKET_buf_init(&pkt, smbuf, BUF_LEN))
        || !TEST_true(PACKET_get_net_2(&pkt, &i)) || !TEST_uint_eq(i, 0x0204)
...
static int test_PACKET_get_bytes(void)
{
    ...
    if (!TEST_true(PACKET_buf_init(&pkt, smbuf, BUF_LEN))
        ...
        || !TEST_uchar_eq(bytes[0], 2) || !TEST_uchar_eq(bytes[1], 4)
        || !TEST_uchar_eq(bytes[2], 6) || !TEST_uchar_eq(bytes[3], 8)
        || !TEST_size_t_eq(PACKET_remaining(&pkt), BUF_LEN - 4)
        || !TEST_true(PACKET_forward(&pkt, BUF_LEN - 8))
        || !TEST_true(PACKET_get_bytes(&pkt, &bytes, 4))
        || !TEST_uchar_eq(bytes[0], 0xf8) || !TEST_uchar_eq(bytes[1], 0xfa)
        || !TEST_uchar_eq(bytes[2], 0xfc) || !TEST_uchar_eq(bytes[3], 0xfe)
...
static int test_PACKET_copy_bytes(void)
{
    ...
    if (!TEST_true(PACKET_buf_init(&pkt, smbuf, BUF_LEN))
        ...
        || !TEST_char_eq(bytes[0], 2) || !TEST_char_eq(bytes[1], 4)
        || !TEST_char_eq(bytes[2], 6) || !TEST_char_eq(bytes[3], 8)
        || !TEST_size_t_eq(PACKET_remaining(&pkt), BUF_LEN - 4)
        || !TEST_true(PACKET_forward(&pkt, BUF_LEN - 8))
        || !TEST_true(PACKET_copy_bytes(&pkt, bytes, 4))
        || !TEST_uchar_eq(bytes[0], 0xf8) || !TEST_uchar_eq(bytes[1], 0xfa)
...
static int test_PACKET_get_length_prefixed_1(void)
{
    ...
    for (i = 1; i < BUF_LEN; i++) buf1[i] = (i * 2) & 0xff;
...
static int test_PACKET_get_length_prefixed_2(void)
{
    ...
    memset(&subpkt, 0, sizeof(subpkt));
    for (i = 1; i <= 1024; i++) buf1[i - 1] = (i * 2) & 0xff;
...
static int test_PACKET_get_length_prefixed_3(void)
{
    ...
    memset(&subpkt, 0, sizeof(subpkt));
    for (i = 0; i < 1024; i++) buf1[i] = (i * 2) & 0xff;
...
static int test_PACKET_as_length_prefixed_2(void)
{
    ...
    memset(&subpkt, 0, sizeof(subpkt));
    for (i = 1; i <= 1024; i++) buf[i - 1] = (i * 2) & 0xff;
...
static int test_PACKET_get_quic_vlint(void)
{
    ...
    static const struct quic_test_case cases[] = {
        ...
        {{0xfd, 0x1f, 0x59, 0x8d, 0xc9, 0xf8, 0x71, 0x8a},
         8,
         4404337426105397642},
    };
...
static int test_PACKET_get_quic_length_prefixed(void)
{
    ...
    for (i = 0; i < OSSL_NELEM(cases); ++i) {
        ...
        if (!TEST_int_eq(PACKET_get_quic_length_prefixed(&pkt, &subpkt),
                         !cases[i].fail))
            return 0;
...
int setup_tests(void)
{
    ...
    for (i = 1; i <= BUF_LEN; i++) smbuf[i - 1] = (i * 2) & 0xff;
