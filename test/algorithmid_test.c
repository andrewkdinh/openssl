...
static int test_spki_aid(X509_PUBKEY *pubkey, const char *filename)
{
    ...
    OSSL_PARAM params[] = {OSSL_PARAM_octet_string(ALGORITHMID_NAME, &algid_prov, sizeof(algid_prov)), OSSL_PARAM_END};
    int ret = 0;
...
static int test_x509_sig_aid(X509 *eecert, const char *ee_filename, X509 *cacert, const char *ca_filename)
{
    ...
    if (!TEST_int_ne(sig_nid = OBJ_obj2nid(sig_oid), NID_undef)
        || !TEST_true(OBJ_find_sigid_algs(sig_nid, &dig_nid, &pkey_nid)) || !TEST_ptr(pkey = X509_get0_pubkey(cacert)))
        goto end;
    ...
    if (!TEST_true(EVP_PKEY_is_a(pkey, OBJ_nid2sn(pkey_nid)))) {
        ...
        TEST_info("Signature algorithm is %s (pkey type %s, hash type %s)", OBJ_nid2sn(sig_nid), OBJ_nid2sn(pkey_nid),
                  OBJ_nid2sn(dig_nid));
    ...
    if (!TEST_ptr(mdctx = EVP_MD_CTX_new())
        || !TEST_true(EVP_DigestVerifyInit_ex(mdctx, &pctx, OBJ_nid2sn(dig_nid), NULL, NULL, pkey, NULL))) {
...
static int test_x509_files(void)
{
    ...
    ret = test_x509_sig_aid(eecert, eecert_filename, cacert, cacert_filename)
        & test_x509_spki_aid(eecert, eecert_filename) & test_x509_spki_aid(cacert, cacert_filename);
end:
    BIO_free(bee);
    BIO_free(bca);
    X509_free(eecert);
