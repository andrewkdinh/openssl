...
static int test_spki_aid(X509_PUBKEY *pubkey, const char *filename)
{
     ...
     * in this function.
     */
    if ((keydata = evp_pkey_export_to_provider(pkey, NULL, &keymgmt, NULL))
        == NULL) {
    ...
    if (!TEST_ptr(gettable_params = EVP_KEYMGMT_gettable_params(keymgmt))
        || !TEST_ptr(
            OSSL_PARAM_locate_const(gettable_params, ALGORITHMID_NAME))) {
        TEST_info(
                             ...
                             X509 *cacert, const char *ca_filename)
{
    ...
    size_t algid_prov_len = 0;
    const OSSL_PARAM *gettable_params = NULL;
    OSSL_PARAM params[] = {OSSL_PARAM_octet_string("algorithm-id", &algid_prov,
                                                   sizeof(algid_prov)),
                           OSSL_PARAM_END};
    int ret = 0;
    ...
    if (!TEST_ptr(mdctx = EVP_MD_CTX_new())
        || !TEST_true(EVP_DigestVerifyInit_ex(mdctx, &pctx, OBJ_nid2sn(dig_nid),
                                              NULL, NULL, pkey, NULL))) {
