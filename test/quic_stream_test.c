...
static int test_sstream_simple(void) {
  ...
  if (!TEST_true(
      ...
      !TEST_uint64_t_eq(hdr.len, sizeof(data_1)) || !TEST_false(hdr.is_fin))
    goto err;
  ...
  if (!TEST_true(
      ...
      !TEST_uint64_t_eq(hdr.len, sizeof(data_1) - 8) || !TEST_false(hdr.is_fin))
    goto err;
  ...
  for (i = 0; i < 2; ++i) {
    ...
    if (!TEST_true(
        ossl_quic_sstream_get_stream_frame(sstream, 0, &hdr, iov, &num_iov)) ||
        !TEST_uint64_t_eq(hdr.offset, 16) || !TEST_uint64_t_eq(hdr.len, 0) ||
        !TEST_true(hdr.is_fin) || !TEST_size_t_eq(num_iov, 0))
      goto err;
  }
  ...
  for (r = 0; r < 100; ++r) {
    ...
    if (idx % 2 == 0) {
      if (!TEST_true(test_single_copy_read(rstream, read_buf, data_size,
                                           &readbytes, &fin)))
        goto err;
    } else if (!TEST_true(ossl_quic_rstream_read(rstream, read_buf, data_size,
                                                 &readbytes, &fin))) {
      goto err;
    }
    if (!TEST_size_t_ge(readbytes, queued_min - read_off) ||
        ...
        (idx % 3 != 0 &&
         !TEST_mem_eq(read_buf, readbytes, bulk_data + read_off, readbytes)))
      goto err;
    read_off += readbytes;
    queued_min = read_off;
    if (test_random() % 50 == 0)
