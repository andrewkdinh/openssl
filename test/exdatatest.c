...
static void MYOBJ_allochello3(MYOBJ *obj, char *cp) {
    ...
    if (TEST_ptr_null(ex_data =
                      CRYPTO_get_ex_data(&obj->ex_data, saved_idx3)) &&
        TEST_true(CRYPTO_alloc_ex_data(CRYPTO_EX_INDEX_APP, obj, &obj->ex_data,
                                       saved_idx3)) &&
...
static MYOBJ *MYOBJ_dup(MYOBJ *in) {
    ...
    if (obj != NULL)
        obj->st |=
        CRYPTO_dup_ex_data(CRYPTO_EX_INDEX_APP, &obj->ex_data, &in->ex_data);
    return obj;
}
...
static int test_exdata(void) {
    ...
    saved_idx2 = CRYPTO_get_ex_new_index(CRYPTO_EX_INDEX_APP, saved_argl,
                                         saved_argp, exnew2, exdup2, exfree2);
    t1 = MYOBJ_new();
    t2 = MYOBJ_new();
