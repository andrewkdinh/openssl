...
static const unsigned char pbe_ciphertext_des_sha1[] = {
    0xce, 0x4b, 0xb0, 0x0a, 0x7b, 0x48, 0xd7, 0xe3, 0x9a, 0x9f, 0x46,
    0xd6, 0x41, 0x42, 0x4b, 0x44, 0x36, 0x45, 0x5f, 0x60, 0x8f, 0x3c,
    0xd0, 0x55, 0xd0, 0x8d, 0xa9, 0xab, 0x78, 0x5b, 0x63, 0xaf,
};
#endif
                          ...
                          const unsigned char *exp, const int exp_len)
{
    ...
    if (!TEST_true(PKCS5_pbe_set0_algor(algor, EVP_CIPHER_nid(cipher), pbe_iter,
        ...
        || !TEST_true(PKCS5_PBE_keyivgen(ctx, pbe_password,
                                         strlen(pbe_password), algor->parameter,
                                         cipher, md, 1))
