...
static void swap_copy(unsigned char *out, const void *in, size_t len)
{
    ...
    for (j = 0; j < len; j++) out[j] = ((unsigned char *)in)[len - j - 1];
}
...
static const struct {
    ...
    {16,
     {0x61, 0xe8, 0x7e, 0x31, 0xe9, 0x33, 0x83, 0x3d, 0x87, 0x99, 0xc7, 0xd8,
      0x5d, 0xa9, 0x8b, 0x42}},
    {16,
     {0xee, 0x6e, 0x8b, 0xc3, 0xec, 0xcf, 0x37, 0xcc, 0x89, 0x67, 0xf2, 0x68,
      0x33, 0xa0, 0x14, 0xb0}},
};
...
static int test_param_int32(int n)
{
    ...
    const size_t len = raw_values[n].len >= sizeof(int32_t) ? sizeof(int32_t)
                                                            : raw_values[n].len;
...
static int test_param_int64(int n)
{
    ...
    const size_t len = raw_values[n].len >= sizeof(int64_t) ? sizeof(int64_t)
                                                            : raw_values[n].len;
...
static int test_param_size_t(int n)
{
    ...
    const size_t len = raw_values[n].len >= sizeof(size_t) ? sizeof(size_t)
                                                           : raw_values[n].len;
...
static int test_param_time_t(int n)
{
    ...
    const size_t len = raw_values[n].len >= sizeof(time_t) ? sizeof(time_t)
                                                           : raw_values[n].len;
