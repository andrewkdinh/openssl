...
{16,
 {0x61, 0xe8, 0x7e, 0x31, 0xe9, 0x33, 0x83, 0x3d, 0x87, 0x99, 0xc7, 0xd8, 0x5d,
  0xa9, 0x8b, 0x42}},
{16,
 {0xee, 0x6e, 0x8b, 0xc3, 0xec, 0xcf, 0x37, 0xcc, 0x89, 0x67, 0xf2, 0x68, 0x33,
  0xa0, 0x14, 0xb0}},
};
...
static int test_param_int(int n) {
  ...
  const size_t len =
  raw_values[n].len >= sizeof(int) ? sizeof(int) : raw_values[n].len;
...
static int test_param_long(int n) {
  ...
  const size_t len =
  raw_values[n].len >= sizeof(long int) ? sizeof(long int) : raw_values[n].len;
...
static int test_param_int32(int n) {
  ...
  const size_t len =
  raw_values[n].len >= sizeof(int32_t) ? sizeof(int32_t) : raw_values[n].len;
...
static int test_param_uint32(int n) {
  ...
  const size_t len =
  raw_values[n].len >= sizeof(uint32_t) ? sizeof(uint32_t) : raw_values[n].len;
...
static int test_param_int64(int n) {
  ...
  const size_t len =
  raw_values[n].len >= sizeof(int64_t) ? sizeof(int64_t) : raw_values[n].len;
...
static int test_param_uint64(int n) {
  ...
  const size_t len =
  raw_values[n].len >= sizeof(uint64_t) ? sizeof(uint64_t) : raw_values[n].len;
...
static int test_param_size_t(int n) {
  ...
  const size_t len =
  raw_values[n].len >= sizeof(size_t) ? sizeof(size_t) : raw_values[n].len;
...
static int test_param_time_t(int n) {
  ...
  const size_t len =
  raw_values[n].len >= sizeof(time_t) ? sizeof(time_t) : raw_values[n].len;
...
static int test_param_construct(int tstid) {
  ...
  static const char *uint_names[] = {"uint", "ulong", "uint32", "uint64",
                                     "size_t"};
  static const unsigned char bn_val[16] = {0xac, 0x75, 0x22, 0x7d, 0x81, 0x06,
                                           0x7a, 0x23, 0xa6, 0xed, 0x87, 0xc7,
                                           0xab, 0xf4, 0x73, 0x22};
  ...
  if (!TEST_true(OSSL_PARAM_get_octet_ptr(cp, (const void **)&vp2, &k)) ||
      !TEST_size_t_eq(k, sizeof(ul)) || (tstid <= 1 && !TEST_ptr_eq(vp2, vp)))
    goto err;
...
static int test_param_modified(void) {
  OSSL_PARAM param[3] = {OSSL_PARAM_int("a", NULL), OSSL_PARAM_int("b", NULL),
                         OSSL_PARAM_END};
  int a, b;
