...
static int _torture_rw(void) {
  ...
  TEST_info("performed %d reads and %d writes over 2 read and 2 write threads "
            "in %e seconds",
...
static void free_old_rcu_data(void *data) { CRYPTO_free(data, NULL, 0); }
  ...
  while (lw1 != 1 || lw2 != 1) {
    ...
    if (oldval > val) {
      TEST_info("rcu torture value went backwards! %llu : %llu",
                (unsigned long long)oldval, (unsigned long long)val);
...
static void once_do_run(void) { once_run_count++; }
...
static void thread_local_thread_cb(void) {
  ...
  if (!TEST_ptr_null(ptr) || !TEST_true(CRYPTO_THREAD_set_local(
                             &thread_local_key, &destructor_run_count)))
    return;
...
static int thread_setup_libctx(int libctx, const char *providers[]) {
  ...
  if (libctx &&
      !TEST_true(test_get_libctx(&multi_libctx, NULL, config_file, NULL, NULL)))
    return 0;

  if (providers != NULL)
    for (n = 0; providers[n] != NULL; n++)
      if (!TEST_size_t_lt(n, MAXIMUM_PROVIDERS) ||
          !TEST_ptr(multi_provider[n] =
                    OSSL_PROVIDER_load(multi_libctx, providers[n]))) {
...
static int start_threads(size_t n, void (*thread_func)(void)) {
  ...
  for (i = 0; i < n; i++)
    if (!TEST_true(
        run_thread(multi_threads + multi_num_threads++, thread_func)))
      return 0;
  return 1;
}
...
static void thread_general_worker(void) {
  ...
  const unsigned char key[AES_BLOCK_SIZE] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05,
                                             0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,
                                             0x0c, 0x0d, 0x0e, 0x0f};
  const unsigned char iv[AES_BLOCK_SIZE] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05,
                                            0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,
                                            0x0c, 0x0d, 0x0e, 0x0f};
  unsigned int mdoutl;
  int ciphoutl;
   ...
   */
  pkey =
  EVP_PKEY_Q_keygen(multi_libctx, NULL, "RSA", (size_t)(isfips ? 2048 : 512));
  if (!TEST_ptr(pkey))
    goto err;
...
static void thread_shared_evp_pkey(void) {
  ...
  for (i = 0; i < 1 + do_fips; i++) {
    ...
    ctx =
    EVP_PKEY_CTX_new_from_pkey(multi_libctx, shared_evp_pkey,
                               i == 0 ? "provider=default" : "provider=fips");
    if (!TEST_ptr(ctx))
      goto err;
...
static int test_multi_general_worker_default_provider(void) {
  return thread_run_test(&thread_general_worker, 2, &thread_general_worker, 1,
                         default_provider);
}
...
static int test_multi_default(void) {
  ...
  return thread_run_test(&thread_multi_simple_fetch, 2,
                         &thread_multi_simple_fetch, 0, default_provider);
}
...
static void test_bio_dgram_pair_worker(void) {
   ...
   */
  if (ch & 2)
    r = BIO_sendmmsg(ch & 1 ? multi_bio2 : multi_bio1, &msg, sizeof(BIO_MSG), 1,
                     0, &num_processed);
  else
    r = BIO_recvmmsg(ch & 1 ? multi_bio2 : multi_bio1, &msg, sizeof(BIO_MSG), 1,
                     0, &num_processed);
