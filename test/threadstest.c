...
static void thread_local_thread_cb(void)
{
    ...
    if (!TEST_ptr_null(ptr)
        || !TEST_true(
            CRYPTO_THREAD_set_local(&thread_local_key, &destructor_run_count)))
        return;
...
static void thead_teardown_libctx(void)
{
    ...
    for (p = multi_provider; *p != NULL; p++) OSSL_PROVIDER_unload(*p);
...
static int thread_setup_libctx(int libctx, const char *providers[])
{
    ...
    if (libctx
        && !TEST_true(
            test_get_libctx(&multi_libctx, NULL, config_file, NULL, NULL)))
        return 0;
...
static int test_multi_shared_pkey_common(void (*worker)(void))
{
    ...
    multi_intialise();
    if (!thread_setup_libctx(
            1, do_fips ? fips_and_default_providers : default_provider)
