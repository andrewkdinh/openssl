...
static int mock_http_server(BIO *in, BIO *out, char version, int keep_alive,
  ...
  /* first line should contain "(GET|POST) (/<suggested status>)?/<path>
   * HTTP/1.x" */
  if (!is_get &&
...
static long http_bio_cb_ex(BIO *bio, int oper, const char *argp, size_t len,
  ...
  if (oper == (BIO_CB_CTRL | BIO_CB_RETURN) && cmd == BIO_CTRL_FLUSH)
    ret = mock_http_server(bio, args->out, args->version, args->keep_alive,
                           args->content_type, args->txt, (ASN1_VALUE *)x509,
                           x509_it);
  return ret;
}
...
static int test_http_method(int do_get, int do_txt, int suggested_status) {
  ...
  rsp = do_get
        ? OSSL_HTTP_get(real_server ? REAL_SERVER_URL : path, NULL /* proxy */,
                        NULL /* no_proxy */, real_server ? NULL : wbio,
                        real_server ? NULL : rbio, NULL /* bio_update_fn */,
                        NULL /* arg */, 0 /* buf_size */, headers,
                        real_server ? "text/html; charset=utf-8" : content_type,
                        !do_txt /* expect_asn1 */,
                        OSSL_HTTP_DEFAULT_MAX_RESP_LEN, 0 /* timeout */)
        : OSSL_HTTP_transfer(
          NULL, NULL /* host */, NULL /* port */, path, 0 /* use_ssl */,
          NULL /* proxy */, NULL /* no_pr */, wbio, rbio, NULL /* bio_fn */,
          NULL /* arg */, 0 /* buf_size */, headers, content_type, req,
          content_type, !do_txt /* expect_asn1 */,
          OSSL_HTTP_DEFAULT_MAX_RESP_LEN, 0 /* timeout */, 0 /* keep_alive */);
  ...
  if (rsp != NULL) {
    if (do_get && real_server) {
      ...
      res =
      TEST_int_eq(BIO_gets(rsp, rtext, sizeof(rtext)), sizeof(text1) - 1) &&
...
static int test_http_keep_alive(char version, int keep_alive, int kept_alive) {
  ...
  for (res = 1, i = 1; res && i <= 2; i++) {
    ...
    &rctx, NULL /* server */, NULL /* port */, RPATH, 0 /* use_ssl */,
    NULL /* proxy */, NULL /* no_proxy */, wbio, rbio, NULL /* bio_update_fn */,
    NULL, 0 /* buf_size */, NULL /* headers */, NULL /* content_type */,
    NULL /* req => GET */, content_type, 0 /* ASN.1 not expected */,
    0 /* max_resp_len */, 0 /* timeout */, keep_alive);
    if (keep_alive == 2 && kept_alive == 0)
      res =
      res && TEST_ptr_null(rsp) && TEST_int_eq(OSSL_HTTP_is_alive(rctx), 0);
    else
...
static int test_http_url_path_query_ok(const char *url,
                                       const char *exp_path_qu) {
  char *host, *path;
  int res;

  res = TEST_true(OSSL_HTTP_parse_url(url, NULL, NULL, &host, NULL, NULL, &path,
                                      NULL, NULL)) &&
...
static int test_http_url_timestamp(void) {
  return test_http_url_ok("host/p/2017-01-03T00:00:00", 0, "host", "80",
         ...
         test_http_url_ok("http://host/p/2017-01-03T00:00:00", 0, "host", "80",
                          "/p/2017-01-03T00:00:00") &&
...
static int test_http_resp_hdr_limit(size_t limit) {
  ...
  if (!TEST_true(
      OSSL_HTTP_REQ_CTX_set_request_line(rctx, 0 /* GET */, NULL, NULL, RPATH)))
    goto err;
...
void cleanup_tests(void) { X509_free(x509); }
