...
0xd3, 0x6b, 0x54, 0xec, 0x5b, 0xac, 0x89, 0x96, 0x8c, 0x2c, 0x66,
0xa5, 0x67, 0x0d, 0xe3, 0xdd, 0x43, 0x69, 0xbc, 0x83, 0x3d, 0x60,
0xc7, 0xb8, 0x2b, 0x1c, 0x5a, 0xfd, 0xb5, 0xcd, 0xd0, 0xf8};
...
OSSL_PARAM_utf8_string(OSSL_CAPABILITY_TLS_GROUP_NAME, XORGROUP_NAME,
                       sizeof(XORGROUP_NAME)),
OSSL_PARAM_utf8_string(OSSL_CAPABILITY_TLS_GROUP_NAME_INTERNAL,
                       XORGROUP_NAME_INTERNAL, sizeof(XORGROUP_NAME_INTERNAL)),
...
OSSL_PARAM_utf8_string(OSSL_CAPABILITY_TLS_GROUP_NAME, XORKEMGROUP_NAME,
...
OSSL_PARAM_utf8_string(OSSL_CAPABILITY_TLS_SIGALG_IANA_NAME, XORSIGALG_NAME,
                       sizeof(XORSIGALG_NAME)),
OSSL_PARAM_utf8_string(OSSL_CAPABILITY_TLS_SIGALG_NAME, XORSIGALG_NAME,
                       sizeof(XORSIGALG_NAME)),
OSSL_PARAM_utf8_string(OSSL_CAPABILITY_TLS_SIGALG_OID, XORSIGALG_OID,
...
OSSL_PARAM_utf8_string(OSSL_CAPABILITY_TLS_SIGALG_NAME, XORSIGALG_HASH_NAME,
                       sizeof(XORSIGALG_HASH_NAME)),
OSSL_PARAM_utf8_string(OSSL_CAPABILITY_TLS_SIGALG_HASH_NAME, XORSIGALG_HASH,
                       sizeof(XORSIGALG_HASH)),
OSSL_PARAM_utf8_string(OSSL_CAPABILITY_TLS_SIGALG_OID, XORSIGALG_HASH_OID,
...
OSSL_PARAM_utf8_string(OSSL_CAPABILITY_TLS_SIGALG_NAME, XORSIGALG12_NAME,
                       sizeof(XORSIGALG12_NAME)),
OSSL_PARAM_utf8_string(OSSL_CAPABILITY_TLS_SIGALG_OID, XORSIGALG12_OID,
  ...
  if (strcmp(capability, "TLS-GROUP") == 0) {
    ...
    for (i = 0; i < NUM_DUMMY_GROUPS; i++) {
      ...
      if (dummy_group_names[i] == NULL) {
        ...
        if (dummy_group_names[i] == NULL)
          return 0;
        BIO_snprintf(dummy_group_names[i], dummy_name_max_size, "%s%d",
                     dummy_base, i);
      }
...
static void xor_freectx(void *pxorctx) { OPENSSL_free(pxorctx); }
...
static int xor_encapsulate(void *vpxorctx, unsigned char *ct, size_t *ctlen,
  ...
  *ctlen = XOR_KEY_SIZE;
  ...
  if (derivectx == NULL || !xor_init(derivectx, ourkey, NULL) ||
...
static int xor_decapsulate(void *vpxorctx, unsigned char *ss, size_t *sslen,
  ...
  /* Derive ss via KEX */
  derivectx = xor_newkemkexctx(pxorctx->provctx);
  if (derivectx == NULL || !xor_init(derivectx, pxorctx->key, NULL) ||
...
static int xor_match(const void *keydata1, const void *keydata2,
                     int selection) {
  ...
  if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0) {
    if (key1->hasprivkey) {
      if (key2->hasprivkey)
        ok =
        ok & (CRYPTO_memcmp(key1->privkey, key2->privkey, XOR_KEY_SIZE) == 0);
      else
  ...
  if ((selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) != 0) {
    if (key1->haspubkey) {
      if (key2->haspubkey)
        ok =
        ok & (CRYPTO_memcmp(key1->pubkey, key2->pubkey, XOR_KEY_SIZE) == 0);
      else
...
static const OSSL_PARAM *xor_gen_settable_params(ossl_unused void *genctx,
  ...
  OSSL_PARAM_utf8_string(OSSL_PKEY_PARAM_GROUP_NAME, NULL, 0), OSSL_PARAM_END};
  return settable;
}
...
static int xor_import(void *vkey, int select, const OSSL_PARAM params[]) {
  ...
  if ((param_priv_key != NULL &&
       !OSSL_PARAM_get_octet_string(param_priv_key, &pprivkey, sizeof(privkey),
                                    &priv_len)) ||
      (param_pub_key != NULL &&
       !OSSL_PARAM_get_octet_string(param_pub_key, &ppubkey, sizeof(pubkey),
                                    &pub_len)))
    goto err;
...
static int xor_export(void *vkey, int select, OSSL_CALLBACK *param_cb,
  ...
  *p++ = OSSL_PARAM_construct_octet_string(OSSL_PKEY_PARAM_PUB_KEY, key->pubkey,
                                           sizeof(key->pubkey));
...
static void xor_gen_cleanup(void *genctx) { OPENSSL_free(genctx); }
...
static void *xor_xorhmacsig_gen(void *genctx, OSSL_CALLBACK *osslcb,
                                void *cbarg) {
...
static void *xor_xorhmacsha2sig_gen(void *genctx, OSSL_CALLBACK *osslcb,
                                    void *cbarg) {
  XORKEY *k = xor_gen(genctx, osslcb, cbarg);
...
typedef enum { KEY_OP_PUBLIC, KEY_OP_PRIVATE, KEY_OP_KEYGEN } xor_key_op_t;

/* Re-create XORKEY from encoding(s): Same end-state as after key-gen */
static XORKEY *xor_key_op(const X509_ALGOR *palg, const unsigned char *p,
                          int plen, xor_key_op_t op, OSSL_LIB_CTX *libctx,
                          const char *propq) {
  XORKEY *key = NULL;
  int nid = NID_undef;
...
static PKCS8_PRIV_KEY_INFO *key_to_p8info(const void *key, int key_nid,
  ...
  if ((p8info = PKCS8_PRIV_KEY_INFO_new()) == NULL ||
      ...
      !PKCS8_pkey_set0(p8info, OBJ_nid2obj(key_nid), 0, V_ASN1_UNDEF, NULL, der,
                       derlen)) {
    ERR_raise(ERR_LIB_USER, ERR_R_MALLOC_FAILURE);
...
static X509_SIG *p8info_to_encp8(PKCS8_PRIV_KEY_INFO *p8info,
  ...
  p8 = PKCS8_encrypt_ex(-1, ctx->cipher, kstr, klen, NULL, 0, 0, p8info, libctx,
                        NULL);
...
static X509_PUBKEY *xorx_key_to_pubkey(const void *key, int key_nid,
  ...
  if ((xpk = X509_PUBKEY_new()) == NULL || (derlen = k2d(key, &der)) <= 0 ||
      !X509_PUBKEY_set0_param(xpk, OBJ_nid2obj(key_nid), V_ASN1_UNDEF, NULL,
                              der, derlen)) {
    ERR_raise(ERR_LIB_USER, ERR_R_MALLOC_FAILURE);
 ...
 * xorx_type_specific_[priv,pub,params]_to_der.
 */
...
#define DO_ENC_PRIVATE_KEY(impl, type, kind, output)                           \
  if ((selection & DO_ENC_PRIVATE_KEY_selection_mask) != 0)                    \
    ...
    key_to_##kind##_##output##_priv_bio, cb, cbarg, prepare_##type##_params,   \
...
#define DO_ENC_PUBLIC_KEY(impl, type, kind, output)                            \
  if ((selection & DO_ENC_PUBLIC_KEY_selection_mask) != 0)                     \
    ...
    key_to_##kind##_##output##_pub_bio, cb, cbarg, prepare_##type##_params,    \
...
#define DO_ENC_PARAMETERS(impl, type, kind, output)                            \
  if ((selection & DO_ENC_PARAMETERS_selection_mask) != 0)                     \
    return key2any_encode(ctx, cout, key, impl##_pem_type,                     \
                          ...
                          key_to_##kind##_##output##_param_bio, NULL, NULL,    \
                          NULL, type##_##kind##_params_to_der);
...
#define DO_ENC_EncryptedPrivateKeyInfo_selection_mask                          \
  DO_ENC_PRIVATE_KEY_selection_mask
#define DO_ENC_EncryptedPrivateKeyInfo(impl, type, output)                     \
  DO_ENC_PRIVATE_KEY(impl, type, epki, output)

/* SubjectPublicKeyInfo is a structure for public keys only */
#define DO_ENC_SubjectPublicKeyInfo_selection_mask                             \
  DO_ENC_PUBLIC_KEY_selection_mask
...
#define MAKE_ENCODER(impl, type, kind, output)                                 \
  ...
  void *vctx, int selection, const OSSL_PARAM params[]) {                      \
    struct key2any_ctx_st *ctx = vctx;                                         \
                                                                               \
    return xor_prov_import_key(xor_##impl##_keymgmt_functions, ctx->provctx,   \
                               selection, params);                             \
  }                                                                            \
  ...
  static int impl##_to_##kind##_##output##_does_selection(void *ctx,           \
                                                          int selection) {     \
    return key2any_check_selection(selection, DO_ENC_##kind##_selection_mask); \
  }                                                                            \
...
#define ENCODER_TEXT(_name, _sym)                                              \
  {_name, "provider=" ENCODER_PROVIDER ",fips=yes,output=text",                \
   (xor_##_sym##_to_text_encoder_functions)}
#define ENCODER(_name, _sym, _fips, _output)                                   \
  {_name, "provider=" ENCODER_PROVIDER ",fips=yes,output=" #_output,           \
...
ENCODER_w_structure(XORSIGALG_HASH_NAME, xorhmacsha2sig, der,
                    EncryptedPrivateKeyInfo),
ENCODER_w_structure(XORSIGALG_HASH_NAME, xorhmacsha2sig, pem,
                    EncryptedPrivateKeyInfo),
ENCODER_w_structure(XORSIGALG_HASH_NAME, xorhmacsha2sig, der,
...
static X509_PUBKEY
* xorx_d2i_X509_PUBKEY_INTERNAL(const unsigned char **pp, long len,
                                OSSL_LIB_CTX *libctx) {
...
static XORKEY *xor_d2i_PUBKEY(XORKEY **a, const unsigned char **pp,
                              long length) {
  XORKEY *key = NULL;
  X509_PUBKEY *xpk;
...
#define DO_PrivateKeyInfo(keytype)                                             \
  ...
  xorx_d2i_PKCS8, NULL, NULL, xorx_key_adjust, (free_key_fn *)xor_freekey

#define DO_SubjectPublicKeyInfo(keytype)                                       \
  "SubjectPublicKeyInfo", 0, (OSSL_KEYMGMT_SELECT_PUBLIC_KEY), NULL, NULL,     \
  NULL, NULL, (d2i_of_void *)xor_d2i_PUBKEY, NULL, xorx_key_adjust,            \
...
#define MAKE_DECODER(keytype_name, keytype, type, kind)                        \
  ...
  keytype_name, xor_##keytype##_keymgmt_functions, DO_##kind(keytype)};        \
                                                                               \
  ...
  {OSSL_FUNC_DECODER_DECODE, (void (*)(void))xor_der2key_decode},              \
...
#define DECODER(_name, _input, _output)                                        \
  {_name, "provider=" DECODER_PROVIDER ",fips=yes,input=" #_input,             \
...
DECODER_w_structure(XORSIGALG_HASH_NAME, der, SubjectPublicKeyInfo,
                    xorhmacsha2sig),
...
static int xor_sig_setup_md(PROV_XORSIG_CTX *ctx, const char *mdname,
                            const char *mdprops) {
  EVP_MD *md;
...
static int xor_sig_sign(void *vpxor_sigctx, unsigned char *sig, size_t *siglen,
  ...
  if (!EVP_Q_mac(pxor_sigctx->libctx, "HMAC", NULL, "sha1", NULL,
                 xorkey->privkey, XOR_KEY_SIZE, tbs, tbslen, &sig[0],
                 EVP_MAX_MD_SIZE, &xor_sig_len)) {
...
static int xor_sig_verify(void *vpxor_sigctx, const unsigned char *sig,
  ...
  if (!EVP_Q_mac(pxor_sigctx->libctx, "HMAC", NULL, "sha1", NULL,
                 xorkey->privkey, XOR_KEY_SIZE, tbs, tbslen, &resignature[0],
                 EVP_MAX_MD_SIZE, &resiglen)) {
...
static int xor_sig_digest_signverify_init(void *vpxor_sigctx,
                                          const char *mdname, void *vxorsig,
                                          int operation) {
  PROV_XORSIG_CTX *pxor_sigctx = (PROV_XORSIG_CTX *)vpxor_sigctx;
...
static int xor_sig_digest_verify_init(void *vpxor_sigctx, const char *mdname,
                                      void *vxorsig,
                                      const OSSL_PARAM params[]) {
  return xor_sig_digest_signverify_init(vpxor_sigctx, mdname, vxorsig,
                                        EVP_PKEY_OP_VERIFY);
...
int xor_sig_digest_sign_final(void *vpxor_sigctx, unsigned char *sig,
...
static int xor_sig_get_ctx_params(void *vpxor_sigctx, OSSL_PARAM *params) {
  ...
  if (pxor_sigctx->aid == NULL)
    pxor_sigctx->aid_len =
    xor_get_aid(&(pxor_sigctx->aid), pxor_sigctx->sig->tls_name);
...
static const OSSL_PARAM *
xor_sig_gettable_ctx_params(ossl_unused void *vpxor_sigctx,
                            ossl_unused void *vctx) {
  return known_gettable_ctx_params;
}

static int xor_sig_set_ctx_params(void *vpxor_sigctx,
                                  const OSSL_PARAM params[]) {
  PROV_XORSIG_CTX *pxor_sigctx = (PROV_XORSIG_CTX *)vpxor_sigctx;
  const OSSL_PARAM *p;
...
static const OSSL_PARAM *
xor_sig_settable_ctx_params(ossl_unused void *vpsm2ctx,
                            ossl_unused void *provctx) {
...
static int xor_sig_set_ctx_md_params(void *vpxor_sigctx,
                                     const OSSL_PARAM params[]) {
  PROV_XORSIG_CTX *pxor_sigctx = (PROV_XORSIG_CTX *)vpxor_sigctx;
...
int tls_provider_init(const OSSL_CORE_HANDLE *handle, const OSSL_DISPATCH *in,
  ...
  *provctx = xor_prov_ctx;
  ...
  if (!c_obj_add_sigid(handle, XORSIGALG_HASH_OID, XORSIGALG_HASH,
                       XORSIGALG_HASH_OID)) {
    ERR_raise(ERR_LIB_USER, XORPROV_R_OBJ_CREATE_ERR);
    goto err;
  }
