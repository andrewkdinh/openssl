...
static int tls_prov_get_capabilities(void *provctx, const char *capability,
    ...
    if (strcmp(capability, "TLS-GROUP") == 0) {
        ...
        /*
         * Now register some dummy groups > GROUPLIST_INCREMENT (== 40) as
         * defined in ssl/t1_lib.c, to make sure we exercise the code paths for
         * registering large numbers of groups.
         */
...
static int xor_match(const void *keydata1, const void *keydata2,
    ...
    if ((selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) != 0) {
        if (key1->haspubkey) {
            if (key2->haspubkey)
                ok = ok & (CRYPTO_memcmp(key1->pubkey, key2->pubkey,
                                         XOR_KEY_SIZE) == 0);
            else
...
static int xor_export(void *vkey, int select, OSSL_CALLBACK *param_cb,
    ...
    *p++ = OSSL_PARAM_construct_octet_string(
    OSSL_PKEY_PARAM_PRIV_KEY, key->privkey, sizeof(key->privkey));
...
static PKCS8_PRIV_KEY_INFO *key_to_p8info(const void *key, int key_nid,
    ...
    if ((p8info = PKCS8_PRIV_KEY_INFO_new()) == NULL ||
        ...
        !PKCS8_pkey_set0(p8info, OBJ_nid2obj(key_nid), 0, V_ASN1_UNDEF, NULL,
                         der, derlen)) {
        ERR_raise(ERR_LIB_USER, ERR_R_MALLOC_FAILURE);
...
int xor_sig_digest_sign_final(void *vpxor_sigctx, unsigned char *sig,
    ...
    return xor_sig_sign(vpxor_sigctx, sig, siglen, sigsize, digest,
                        (size_t)dlen);
}
