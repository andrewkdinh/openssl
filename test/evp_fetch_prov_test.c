...
const OPTIONS *test_get_options(void)
{
    static const OPTIONS test_options[] = {
        ...
        {"property", OPT_FETCH_PROPERTY, 's',
         "The fetch property e.g. provider=fips"},
...
static X509_ALGOR *make_algor(int nid)
{
    ...
    if (!TEST_ptr(algor = X509_ALGOR_new())
        || !TEST_true(
            X509_ALGOR_set0(algor, OBJ_nid2obj(nid), V_ASN1_UNDEF, NULL))) {
...
static int test_md(const EVP_MD *md)
{
    ...
    const unsigned char exptd[] = {
        0x27, 0x51, 0x8b, 0xa9, 0x68, 0x30, 0x11, 0xf6, 0xb3, 0x96, 0x07,
...
int setup_tests(void)
{
    ...
    if (strcmp(alg, "digest") == 0) {
        ...
        ADD_ALL_TESTS_NOSUBTEST(test_explicit_EVP_CIPHER_fetch_by_X509_ALGOR,
                                2);
    }
    return 1;
}
