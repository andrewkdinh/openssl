...
static int test_EVP_KDF_fetch_freeze(void)
{
    ...
    if (!TEST_ptr(ctx)
        || !TEST_ptr(kdf = EVP_KDF_fetch(ctx, OSSL_KDF_NAME_PBKDF2, NULL))
    ...
    if (!TEST_ptr(kdf = EVP_KDF_fetch(ctx, OSSL_KDF_NAME_PBKDF2, "?fips=true"))
        || !TEST_int_eq(kdf->origin, EVP_ORIG_FROZEN))
        goto err;
    EVP_KDF_free(kdf);

    if (!TEST_ptr(kdf = EVP_KDF_fetch(ctx, OSSL_KDF_NAME_PBKDF2, "?provider=default"))
        || !TEST_int_ne(kdf->origin, EVP_ORIG_FROZEN))
        goto err;
    EVP_KDF_free(kdf);

    if (!TEST_ptr(kdf = EVP_KDF_fetch(NULL, OSSL_KDF_NAME_PBKDF2, NULL))
        || !TEST_int_ne(kdf->origin, EVP_ORIG_FROZEN))
        goto err;
