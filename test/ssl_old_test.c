...
static int cb_server_npn(SSL *s, const unsigned char **data, unsigned int *len,
                         void *arg) {
...
int doit_localhost(SSL *s_ssl, SSL *c_ssl, int family, long bytes,
...
static void sv_usage(void) {
    ...
    fprintf(stderr, " -dhe2048      - use 2048 bit key (safe prime) for DHE "
                    "(default, no-op)\n");
    ...
    fprintf(stderr, " -server_min_proto <string> - Minimum version the server "
                    "should support\n");
    fprintf(stderr, " -server_max_proto <string> - Maximum version the server "
                    "should support\n");
    fprintf(stderr, " -client_min_proto <string> - Minimum version the client "
                    "should support\n");
    fprintf(stderr, " -client_max_proto <string> - Maximum version the client "
                    "should support\n");
    fprintf(stderr, " -should_negotiate <string> - The version that should be "
                    "negotiated, fail-client or fail-server\n");
#ifndef OPENSSL_NO_CT
    ...
    fprintf(stderr, " -sn_server1 <string> - have server context 1 respond to "
                    "this servername\n");
    fprintf(stderr, " -sn_server2 <string> - have server context 2 respond to "
                    "this servername\n");
    ...
    fprintf(stderr, " -should_reuse <number>     - The expected state of "
                    "reusing the session\n");
    fprintf(stderr, " -no_ticket    - do not issue TLS session ticket\n");
    fprintf(stderr, " -client_ktls  - try to enable client KTLS\n");
    fprintf(stderr, " -server_ktls  - try to enable server KTLS\n");
    fprintf(stderr, " -provider <name>    - Load the given provider into the "
                    "library context\n");
    fprintf(stderr, " -config <cnf>    - Load the given config file into the "
                    "library context\n");
}
...
static void print_details(SSL *c_ssl, const char *prefix) {
    ...
    BIO_printf(bio_stdout, "%s%s, cipher %s %s", prefix, SSL_get_version(c_ssl),
...
static int protocol_from_string(const char *value) {
    ...
    {"ssl3", SSL3_VERSION},      {"tls1", TLS1_VERSION},
    {"tls1.1", TLS1_1_VERSION},  {"tls1.2", TLS1_2_VERSION},
    {"tls1.3", TLS1_3_VERSION},  {"dtls1", DTLS1_VERSION},
...
int main(int argc, char *argv[]) {
    ...
    s_cctx, SSL_CONF_FLAG_CMDLINE | SSL_CONF_FLAG_SERVER |
            SSL_CONF_FLAG_CERTIFICATE | SSL_CONF_FLAG_REQUIRE_PRIVATE);
    SSL_CONF_CTX_set_flags(
    s_cctx2, SSL_CONF_FLAG_CMDLINE | SSL_CONF_FLAG_SERVER |
             SSL_CONF_FLAG_CERTIFICATE | SSL_CONF_FLAG_REQUIRE_PRIVATE);
    ...
    SSL_CONF_CTX_set_flags(
    c_cctx, SSL_CONF_FLAG_CMDLINE | SSL_CONF_FLAG_CLIENT |
            SSL_CONF_FLAG_CERTIFICATE | SSL_CONF_FLAG_REQUIRE_PRIVATE);
    ...
    if (ssl3 + tls1 + tls1_1 + tls1_2 + dtls + dtls1 + dtls12 > 1) {
        fprintf(stderr, "At most one of -ssl3, -tls1, -tls1_1, -tls1_2, -dtls, "
                        "-dtls1 or -dtls12 should "
                        "be requested.\n");
        goto end;
    }
    ...
    if (!ssl3 && !tls1 && !tls1_1 && !tls1_2 && !dtls && !dtls1 && !dtls12 &&
        ...
        fprintf(stderr, "This case cannot work.  Use -f to perform "
                        ...
                        "or add one of -ssl3, -tls1, -tls1_1, -tls1_2, -dtls, "
                        "-dtls1, -dtls12, -reuse\n"
    ...
    if (print_time) {
        ...
        if (number < 50 && !force)
            fprintf(stderr, "Warning: For accurate timings, use more "
    ...
    if (serverinfo_sct) {
        if (!SSL_CTX_add_client_custom_ext(
            c_ctx, TLSEXT_TYPE_signed_certificate_timestamp, NULL, NULL, NULL,
    ...
    if (serverinfo_tack) {
        if (!SSL_CTX_add_client_custom_ext(c_ctx, TACK_EXT_TYPE, NULL, NULL,
                                           NULL, serverinfo_cli_parse_cb,
                                           NULL)) {
...
#ifndef OPENSSL_NO_SOCK
        case BIO_IPV4:
            ret = doit_localhost(s_ssl, c_ssl, BIO_FAMILY_IPV4, bytes, &s_time,
                                 &c_time);
            break;
        case BIO_IPV6:
            ret = doit_localhost(s_ssl, c_ssl, BIO_FAMILY_IPV6, bytes, &s_time,
                                 &c_time);
            break;
#else
        case BIO_IPV4:
    ...
    if (should_reuse != -1) {
        if (SSL_session_reused(s_ssl) != should_reuse ||
            ...
            BIO_printf(bio_err,
                       ...
                       should_reuse, SSL_session_reused(s_ssl),
...
int doit_localhost(SSL *s_ssl, SSL *c_ssl, int family, long count,
    ...
    acpt =
    BIO_new_accept(family == BIO_FAMILY_IPV4 ? "127.0.0.1:0" : "[::1]:0");
    if (acpt == NULL)
        goto err;
