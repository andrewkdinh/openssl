...
static EC_GROUP *create_EC_group(const char *p_hex, const char *a_hex,
    ...
    if (!TEST_true(BN_hex2bn(&p, p_hex)) || !TEST_true(BN_hex2bn(&a, a_hex)) ||
...
static int test_sm2_crypt(const EC_GROUP *group, const EVP_MD *digest,
    ...
    key = EC_KEY_new();
    if (!TEST_ptr(key) || !TEST_true(EC_KEY_set_group(key, group)) ||
    ...
    if (!TEST_true(ossl_sm2_encrypt(key, digest, (const uint8_t *)message,
                                    msg_len, ctext, &ctext_len))) {
        restore_rand();
        goto done;
    }
    restore_rand();
    ...
    if (!TEST_ptr(recovered) ||
        !TEST_true(ossl_sm2_decrypt(key, digest, ctext, ctext_len, recovered,
                                    &recovered_len)) ||
...
static int test_sm2_sign(const EC_GROUP *group, const char *userid,
                         ...
                         const char *s_hex, int omit_pubkey) {
    const size_t msg_len = strlen(message);
    int ok = 0;
    BIGNUM *priv = NULL;
    EC_POINT *pt = NULL;
    ...
    if (!TEST_true(BN_hex2bn(&r, r_hex)) || !TEST_true(BN_hex2bn(&s, s_hex)) ||
        !TEST_BN_eq(r, sig_r) || !TEST_BN_eq(s, sig_s))
        goto done;
