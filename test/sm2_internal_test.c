...
static int get_faked_bytes(unsigned char *buf, size_t num, ossl_unused const char *name, ossl_unused EVP_RAND_CTX *ctx)
{
                                 ...
                                 const char *y_hex, const char *order_hex, const char *cof_hex)
{
    ...
    if (!TEST_true(BN_hex2bn(&g_x, x_hex)) || !TEST_true(BN_hex2bn(&g_y, y_hex))
        || !TEST_true(EC_POINT_set_affine_coordinates(group, generator, g_x, g_y, NULL)))
        goto done;

    if (!TEST_true(BN_hex2bn(&order, order_hex)) || !TEST_true(BN_hex2bn(&cof, cof_hex))
...
static int test_sm2_crypt(const EC_GROUP *group, const EVP_MD *digest, const char *privkey_hex, const char *message,
                          const char *k_hex, const char *ctext_hex)
{
    ...
    if (!TEST_ptr(expected) || !TEST_true(BN_hex2bn(&priv, privkey_hex)))
        goto done;
    ...
    if (!TEST_ptr(pt) || !TEST_true(EC_POINT_mul(group, pt, priv, NULL, NULL, NULL))
        || !TEST_true(EC_KEY_set_public_key(key, pt))
        || !TEST_true(ossl_sm2_ciphertext_size(key, digest, msg_len, &ctext_len)))
        goto done;
    ...
    if (!TEST_ptr(recovered) || !TEST_true(ossl_sm2_decrypt(key, digest, ctext, ctext_len, recovered, &recovered_len))
        || !TEST_int_eq(recovered_len, msg_len) || !TEST_mem_eq(recovered, recovered_len, message, msg_len))
        goto done;
...
static int sm2_crypt_test(void)
{
    ...
    if (!test_sm2_crypt(test_group, EVP_sm3(), "1649AB77A00637BD5E2EFE283FBF353534AA7F7CB89463F208DDBC2920BB0DA0",
                        "encryption standard",
    ...
    if (!test_sm2_crypt(test_group, EVP_sha256(), "1649AB77A00637BD5E2EFE283FBF353534AA7F7CB89463F208DDBC2920BB0DA0",
                        "encryption standard",
                        "004C62EEFD6ECFC2B95B92FD6C3D9575148AFA17425546D49018E5388D49DD7B4F"
    ...
    gm_group = create_EC_group("fffffffeffffffffffffffffffffffffffffffff00000000ffffffffffffffff",
    ...
    if (!test_sm2_crypt(gm_group, EVP_sm3(),
...
static int test_sm2_sign(const EC_GROUP *group, const char *userid, const char *privkey_hex, const char *message,
                         const char *k_hex, const char *r_hex, const char *s_hex, int omit_pubkey)
{
    const size_t msg_len = strlen(message);
    int ok = 0;
    BIGNUM *priv = NULL;
    ...
    if (!TEST_true(BN_hex2bn(&r, r_hex)) || !TEST_true(BN_hex2bn(&s, s_hex)) || !TEST_BN_eq(r, sig_r)
        || !TEST_BN_eq(s, sig_s))
        goto done;

    ok = ossl_sm2_do_verify(key, EVP_sm3(), sig, (const uint8_t *)userid, strlen(userid), (const uint8_t *)message,
                            msg_len);
...
static int sm2_sig_test(void)
{
    ...
    if (!TEST_true(test_sm2_sign(test_group, "ALICE123@YAHOO.COM",
                                 "128B2FA8BD433C6C068C8D803DFF79792A519A55171B1B650C23661D15897263", "message digest",
    ...
    if (!TEST_true(test_sm2_sign(gm_group,
    ...
    if (!TEST_false(test_sm2_sign(gm_group,
                     /* the default ID specified in GM/T 0009-2012 (Sec. 10).*/
                                  SM2_DEFAULT_USERID,
                     /* privkey */
                                  "3945208F7B2144B13F36E38AC6D39F95889393692860B51A42FB81EF4DF7C5B8",
