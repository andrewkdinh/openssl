...
static struct c2_curve_test {
    ...
    {"NIST curve K-163", "0800000000000000000000000000000000000000C9", "1", "1",
     "02FE13C0537BBC11ACAA07D793DE4E6D5E5C94EEE8", "0289070FB05D38FF58321F2E800536D538CCDAA3D9", 1,
     "04000000000000000000020108A2E0CC0D99F8A5EF", "2", 163},
    /* Curve B-163 (FIPS PUB 186-2, App. 6) */
    {"NIST curve B-163", "0800000000000000000000000000000000000000C9", "1",
     "020A601907B8C953CA1481EB10512F78744A3205FD", "03F0EBA16286A2D57EA0991168D4994637E8343E36",
     "00D51FBC6C71A0094FA2CDD545B11C5C0C797324F1", 1, "040000000000000000000292FE77E70C12A4234C33", "2", 163},
    /* Curve K-233 (FIPS PUB 186-2, App. 6) */
    {"NIST curve K-233", "020000000000000000000000000000000000000004000000000000000001", "0", "1",
     "017232BA853A7E731AF129F22FF4149563A419C26BF50A4C9D6EEFAD6126",
     "01DB537DECE819B7F70F555A67C427A8CD9BF18AEB9B56E0C11056FAE6A3", 0,
     "008000000000000000000000000000069D5BB915BCD46EFB1AD5F173ABDF", "4", 233},
    /* Curve B-233 (FIPS PUB 186-2, App. 6) */
    {"NIST curve B-233", "020000000000000000000000000000000000000004000000000000000001",
     "000000000000000000000000000000000000000000000000000000000001",
     "0066647EDE6C332C7F8C0923BB58213B333B20E9CE4281FE115F7D8F90AD",
     "00FAC9DFCBAC8313BB2139F1BB755FEF65BC391F8B36F8F8EB7371FD558B",
     "01006A08A41903350678E58528BEBF8A0BEFF867A7CA36716F7E01F81052", 1,
     "01000000000000000000000000000013E974E72F8A6922031D2603CFE0D7", "2", 233},
...
static int check_named_curve_test(int id)
{
    ...
    if (!TEST_int_eq(EC_GROUP_set_seed(group, invalid_seed, invalid_seed_len), invalid_seed_len))
        goto err;
...
static int check_named_curve_from_ecparameters(int id)
{
     ...
     * group.
     */
    if (!TEST_int_eq(EC_GROUP_set_seed(tmpg, invalid_seed, invalid_seed_len), invalid_seed_len)
...
static int ossl_parameter_test(void)
{
    ...
    if (EC_GROUP_get0_seed(group_nmd) != NULL) {
        if (!TEST_true(OSSL_PARAM_BLD_push_octet_string(bld, OSSL_PKEY_PARAM_EC_SEED, EC_GROUP_get0_seed(group_nmd),
    ...
    if (!TEST_ptr(group_gen = EC_GROUP_get0_generator(group_nmd))
        || !TEST_size_t_gt(bsize = EC_POINT_point2oct(group_nmd, EC_GROUP_get0_generator(group_nmd),
                                                      POINT_CONVERSION_UNCOMPRESSED, NULL, 0, bn_ctx),
                           0)
        || !TEST_ptr(buf2 = OPENSSL_malloc(bsize))
        || !TEST_size_t_eq(EC_POINT_point2oct(group_nmd, EC_GROUP_get0_generator(group_nmd),
...
static ossl_inline int ec_point_hex2point_test_helper(const EC_GROUP *group, const EC_POINT *P,
                                                      point_conversion_form_t form, BN_CTX *bnctx)
{
    int ret = 0;
...
static int do_test_custom_explicit_fromdata(EC_GROUP *group, BN_CTX *ctx, unsigned char *gen, int gen_size)
{
    int ret = 0, i_out;
    ...
    if (EC_GROUP_get0_seed(group) != NULL) {
        if (!TEST_true(OSSL_PARAM_BLD_push_octet_string(bld, OSSL_PKEY_PARAM_EC_SEED, EC_GROUP_get0_seed(group),
    ...
    if (EC_GROUP_get_field_type(group) == NID_X9_62_prime_field) {
        ...
        if (!TEST_false(EVP_PKEY_get_int_param(pkeyparam, OSSL_PKEY_PARAM_EC_CHAR2_M, &i_out))
            ...
            || !TEST_false(EVP_PKEY_get_utf8_string_param(pkeyparam, OSSL_PKEY_PARAM_EC_CHAR2_TYPE, name, sizeof(name),
                                                          &name_len)))
            goto err;
    } else {
#ifndef OPENSSL_NO_EC2M
...
static int custom_generator_test(int id)
{
    ...
    if (!TEST_ptr(k = BN_CTX_get(ctx))
        ...
        || !TEST_int_eq(EC_POINT_point2oct(group, Q1, POINT_CONVERSION_UNCOMPRESSED, NULL, 0, ctx), bsize)
        || !TEST_ptr(b1 = OPENSSL_malloc(bsize))
        || !TEST_int_eq(EC_POINT_point2oct(group, Q1, POINT_CONVERSION_UNCOMPRESSED, b1, bsize, ctx), bsize)
        ...
        || !TEST_int_eq(EC_POINT_point2oct(group, Q2, POINT_CONVERSION_UNCOMPRESSED, NULL, 0, ctx), bsize)
        || !TEST_ptr(b2 = OPENSSL_malloc(bsize))
        || !TEST_int_eq(EC_POINT_point2oct(group, Q2, POINT_CONVERSION_UNCOMPRESSED, b2, bsize, ctx), bsize)
...
static int custom_params_test(int id)
{
    ...
    if (!TEST_true(EC_GROUP_get_curve(group, p, a, b, ctx))
        ...
        || !TEST_true(EC_POINT_dbl(group, G2, EC_GROUP_get0_generator(group), ctx))
            /* pull out the bytes of that */
        || !TEST_int_eq(EC_POINT_point2oct(group, G2, POINT_CONVERSION_UNCOMPRESSED, NULL, 0, ctx), bsize)
        || !TEST_ptr(buf1 = OPENSSL_malloc(bsize))
        || !TEST_int_eq(EC_POINT_point2oct(group, G2, POINT_CONVERSION_UNCOMPRESSED, buf1, bsize, ctx), bsize)
    ...
    if (/* allocate temporary points on group and altgroup */
        ...
        || !TEST_true(EC_POINT_mul(group, Q1, k, NULL, NULL, ctx))
            /* pull out the bytes of that */
        || !TEST_int_eq(EC_POINT_point2oct(group, Q1, POINT_CONVERSION_UNCOMPRESSED, NULL, 0, ctx), bsize)
        || !TEST_int_eq(EC_POINT_point2oct(group, Q1, POINT_CONVERSION_UNCOMPRESSED, buf1, bsize, ctx), bsize)
            /* k := k/2 */
        || !TEST_true(BN_rshift1(k, k))
            /* Q2 := k/2 G2 on altgroup */
        || !TEST_true(EC_POINT_mul(altgroup, Q2, k, NULL, NULL, ctx))
            /* pull out the bytes of that */
        || !TEST_int_eq(EC_POINT_point2oct(altgroup, Q2, POINT_CONVERSION_UNCOMPRESSED, NULL, 0, ctx), bsize)
        || !TEST_ptr(buf2 = OPENSSL_malloc(bsize))
        || !TEST_int_eq(EC_POINT_point2oct(altgroup, Q2, POINT_CONVERSION_UNCOMPRESSED, buf2, bsize, ctx), bsize)
            /* Q1 = kG = k/2 G2 = Q2 should hold */
    ...
    if (!TEST_true(EC_POINT_mul(group, Q1, priv1, NULL, NULL, ctx))
        ...
        || !TEST_int_eq(EC_POINT_point2oct(group, Q1, POINT_CONVERSION_UNCOMPRESSED, pub1, bsize, ctx), bsize))
        goto err;

    /* retrieve bytes for pub2 for later */
    if (!TEST_ptr(Q = EC_KEY_get0_public_key(eckey2))
        || !TEST_int_eq(EC_POINT_point2oct(altgroup, Q, POINT_CONVERSION_UNCOMPRESSED, NULL, 0, ctx), bsize)
        || !TEST_ptr(pub2 = OPENSSL_malloc(bsize))
        || !TEST_int_eq(EC_POINT_point2oct(altgroup, Q, POINT_CONVERSION_UNCOMPRESSED, pub2, bsize, ctx), bsize))
        goto err;
    ...
    if (!TEST_ptr(pctx1 = EVP_PKEY_CTX_new(pkey1, NULL)) || !TEST_int_eq(EVP_PKEY_derive_init(pctx1), 1)
        ...
        || !TEST_int_eq(EVP_PKEY_derive(pctx1, NULL, &sslen), 1) || !TEST_int_gt(bsize, sslen)
    ...
    if (!TEST_ptr(pctx2 = EVP_PKEY_CTX_new(pkey2, NULL)) || !TEST_int_eq(EVP_PKEY_derive_init(pctx2), 1)
        ...
        || !TEST_int_gt(bsize, t) || !TEST_int_le(sslen, t) || !TEST_int_eq(EVP_PKEY_derive(pctx2, buf2, &t), 1))
        goto err;
    ...
    if (!TEST_ptr(pctx1 = EVP_PKEY_CTX_new(pkey1, NULL)) || !TEST_int_eq(EVP_PKEY_derive_init(pctx1), 1)
        ...
        || !TEST_int_eq(EVP_PKEY_derive(dctx, NULL, &t), 1) || !TEST_int_gt(bsize, t) || !TEST_int_le(sslen, t)
