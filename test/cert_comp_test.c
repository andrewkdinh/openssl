 ...
 * Test 3 = app pre-compresses certificate in SSL_CTX, but it's unused due to
 * prefs
 */
/* Compression helper */
static int ssl_comp_cert(SSL *ssl, int alg) {
  ...
  if (!TEST_size_t_gt(
      comp_len = SSL_get1_compressed_cert(ssl, alg, &comp_data, &orig_len), 0))
    goto err;

  if (!TEST_true(
      SSL_set1_compressed_cert(ssl, alg, comp_data, comp_len, orig_len)))
    goto err;
  retval = alg;
