...
static int int_filter_all(HT_VALUE *v, void *arg) { return 1; }
...
static int test_int_hashtable(void) {
    ...
    for (i = 0; i < n_int_tests; i++) {
        ...
        if (!TEST_int_eq(
            ossl_ht_test_int_insert(ht, TO_HT_KEY(&key), &int_tests[i], NULL),
            1)) {
...
static unsigned long int stress_hash(const int *p) { return *p; }
...
static void hashtable_intfree(HT_VALUE *v) { OPENSSL_free(v->value); }
...
static int test_hashtable_stress(int idx) {
    ...
    for (i = 0; i < n; i++) {
        ...
        *p = 3 * i + 1;
        ...
        if (!TEST_int_eq(ossl_ht_test_int_insert(h, TO_HT_KEY(&key), p, NULL),
                         1)) {
    ...
    for (iter = 0; iter < TEST_THREAD_ITERATIONS; iter++) {
        ...
        case DO_LOOKUP:
            ...
            if (m != NULL && m != expected_m) {
                ...
                TEST_info(
                "Iteration %d Read unexpected value %p when %p expected", giter,
        ...
        case DO_DELETE:
            ...
            if (expected_rc == 1) {
                ...
                CRYPTO_atomic_add(&expected_m->pending_delete, 1, &ret,
                                  worker_lock);
            }
            if (expected_rc != ossl_ht_delete(m_ht, TO_HT_KEY(&key))) {
                TEST_info("Iteration %d Expected rc %d on delete of element %u "
                          "which is %s\n",
