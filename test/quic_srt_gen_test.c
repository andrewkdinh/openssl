...
static const unsigned char key_2[] = {0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01, 0x01,
...
static int test_srt_gen(int idx)
{
    ...
    if (!TEST_ptr(srt_gen = ossl_quic_srt_gen_new(NULL, NULL, t->key, t->key_len)))
        goto err;

    for (i = 0; i < 2; ++i) {
        memset(&token, 0xff, sizeof(token));

        if (!TEST_true(ossl_quic_srt_gen_calculate_token(srt_gen, &t->dcid, &token)))
            goto err;

        if (!TEST_mem_eq(token.token, sizeof(token.token), &t->expected, sizeof(t->expected)))
            goto err;
    }
