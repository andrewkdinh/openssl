...
static const char  *kInvalidCRL[]           = {"-----BEGIN X509 CRL-----\n",
 ...
 * extension.
 */
static const char  *kKnownCriticalCRL[]     = {"-----BEGIN X509 CRL-----\n",
 ...
 */
static const char  *kUnknownCriticalCRL[]   = {"-----BEGIN X509 CRL-----\n",
 ...
 */
static const char  *kUnknownCriticalCRL2[]  = {"-----BEGIN X509 CRL-----\n",
...
static int test_basic_crl(void)
{
    ...
    r = TEST_ptr(basic_crl) && TEST_ptr(revoked_crl)
        ...
        && TEST_int_eq(verify(test_leaf, test_root, make_CRL_stack(basic_crl, revoked_crl), X509_V_FLAG_CRL_CHECK),
...
static int test_bad_issuer_crl(void)
{
    ...
    r = TEST_ptr(bad_issuer_crl)
        && TEST_int_eq(verify(test_leaf, test_root, make_CRL_stack(bad_issuer_crl, NULL), X509_V_FLAG_CRL_CHECK),
...
static int test_known_critical_crl(void)
{
    ...
    r = TEST_ptr(known_critical_crl)
        && TEST_int_eq(verify(test_leaf, test_root, make_CRL_stack(known_critical_crl, NULL), X509_V_FLAG_CRL_CHECK),
                       X509_V_OK);
...
static int test_unknown_critical_crl(int n)
{
    ...
    r = TEST_ptr(unknown_critical_crl)
        && TEST_int_eq(verify(test_leaf, test_root, make_CRL_stack(unknown_critical_crl, NULL), X509_V_FLAG_CRL_CHECK),
...
static int test_reuse_crl(int idx)
{
    ...
    case 1: /* invalid PEM */
        ...
        break;
    case 2:
        if (!TEST_ptr(result) || !TEST_ptr(reused_crl) || !TEST_ptr_eq(result, reused_crl))
            goto err;
        break;
    }
