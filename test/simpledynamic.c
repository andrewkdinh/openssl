...
int sd_close(SD lib) { return dlclose(lib) != 0 ? 0 : 1; }

const char *sd_error(void) { return dlerror(); }
...
int sd_close(SD lib) { return FreeLibrary(lib) == 0 ? 0 : 1; }

const char *sd_error(void) {
    static char buffer[255];

    buffer[0] = '\0';
    FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(), 0, buffer,
                   sizeof(buffer), NULL);
    return buffer;
}
