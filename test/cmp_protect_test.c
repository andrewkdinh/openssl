...
static int
execute_calc_protection_fails_test(CMP_PROTECT_TEST_FIXTURE *fixture) {
...
static int
execute_calc_protection_pbmac_test(CMP_PROTECT_TEST_FIXTURE *fixture) {
  ASN1_BIT_STRING *protection =
  ossl_cmp_calc_protection(fixture->cmp_ctx, fixture->msg);
  int res =
  TEST_ptr(protection) &&
...
static int verify_signature(OSSL_CMP_MSG *msg, ASN1_BIT_STRING *protection,
  ...
  return ASN1_item_verify_ex(ASN1_ITEM_rptr(OSSL_CMP_PROTECTEDPART),
                             msg->header->protectionAlg, protection, &prot_part,
                             NULL, pkey, libctx, NULL) > 0;
}
...
execute_calc_protection_signature_test(CMP_PROTECT_TEST_FIXTURE *fixture) {
  ...
  int ret =
  (TEST_ptr(protection) &&
   TEST_true(verify_signature(fixture->msg, protection, fixture->pubkey,
...
static int test_cmp_calc_protection_pbmac(void) {
  ...
  SETUP_TEST_FIXTURE(CMP_PROTECT_TEST_FIXTURE, set_up);
  if (!TEST_true(OSSL_CMP_CTX_set1_secretValue(fixture->cmp_ctx, sec_insta,
                                               sizeof(sec_insta))) ||
...
static int
execute_cmp_build_cert_chain_test(CMP_PROTECT_TEST_FIXTURE *fixture) {
  int ret = 0;
  OSSL_CMP_CTX *ctx = fixture->cmp_ctx;
  X509_STORE *store;
  STACK_OF(X509) *chain =
  X509_build_chain(fixture->cert, fixture->certs, NULL, fixture->with_ss,
