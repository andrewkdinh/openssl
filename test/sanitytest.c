...
static int test_sanity_enum_size(void)
{
    ...
    /* Enum size */
    if (!TEST_size_t_eq(sizeof(enum smallchoices), sizeof(int)) || !TEST_size_t_eq(sizeof(enum medchoices), sizeof(int))
...
static int test_sanity_range(void)
{
    ...
    if (!TEST_size_t_eq(sizeof(int8_t), 1) || !TEST_size_t_eq(sizeof(uint8_t), 1) || !TEST_size_t_eq(sizeof(int16_t), 2)
        || !TEST_size_t_eq(sizeof(uint16_t), 2) || !TEST_size_t_eq(sizeof(int32_t), 4)
        || !TEST_size_t_eq(sizeof(uint32_t), 4) || !TEST_size_t_eq(sizeof(int64_t), 8)
        || !TEST_size_t_eq(sizeof(uint64_t), 8)
#ifdef UINT128_MAX
        || !TEST_size_t_eq(sizeof(int128_t), 16) || !TEST_size_t_eq(sizeof(uint128_t), 16)
#endif
        || !TEST_size_t_eq(sizeof(char), 1) || !TEST_size_t_eq(sizeof(unsigned char), 1))
        return 0;
     ...
     */
    if (!TEST_size_t_ge(sizeof(ossl_intmax_t), 8) || !TEST_size_t_ge(sizeof(ossl_uintmax_t), 8)
