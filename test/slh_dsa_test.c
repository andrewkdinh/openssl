...
static EVP_PKEY *slh_dsa_key_from_data(const char *alg,
  ...
  OSSL_PARAM params[2];
  const char *keytype =
  public ? OSSL_PKEY_PARAM_PUB_KEY : OSSL_PKEY_PARAM_PRIV_KEY;
  int selection = public ? EVP_PKEY_PUBLIC_KEY : EVP_PKEY_KEYPAIR;

  params[0] =
  OSSL_PARAM_construct_octet_string(keytype, (uint8_t *)data, datalen);
...
static int slh_dsa_create_keypair(EVP_PKEY **pkey, const char *name,
  ...
  if (!TEST_ptr(bld = OSSL_PARAM_BLD_new()) ||
      !TEST_true(OSSL_PARAM_BLD_push_octet_string(bld, OSSL_PKEY_PARAM_PRIV_KEY,
                                                  priv, priv_len) > 0) ||
      !TEST_true(OSSL_PARAM_BLD_push_octet_string(bld, pub_name, pub, pub_len) >
                 0) ||
...
static int slh_dsa_key_eq_test(void) {
  ...
  if (!TEST_ptr(key[0] =
                slh_dsa_key_from_data(td1->alg, td1->pub, td1->pub_len, 1)) ||
      !TEST_ptr(key[1] =
                slh_dsa_key_from_data(td1->alg, td1->pub, td1->pub_len, 1)) ||
      !TEST_ptr(key[2] =
                slh_dsa_key_from_data(td2->alg, td2->pub, td2->pub_len, 1)) ||
...
static int slh_dsa_key_validate_failure_test(void) {
  ...
  if (!TEST_ptr(key = slh_dsa_key_from_data(
                "SLH-DSA-SHA2-128f", slh_dsa_sha2_128s_0_keygen_priv,
...
static int do_slh_dsa_verify(const SLH_DSA_SIG_TEST_DATA *td, uint8_t *sig,
  ...
  *p = OSSL_PARAM_construct_end();
  ...
  if (!TEST_int_eq(EVP_PKEY_verify_message_init(vctx, sig_alg, params), 1) ||
      !TEST_int_eq(EVP_PKEY_verify(vctx, sig, sig_len, td->msg, td->msg_len),
                   1))
    goto err;
  ret = 1;
err:
...
static int slh_dsa_sign_verify_test(int tst_id) {
  ...
  *p = OSSL_PARAM_construct_end();
   ...
   */
  if (!slh_dsa_create_keypair(&pkey, td->alg, td->priv, td->priv_len, td->pub,
                              td->pub_len))
    goto err;
  ...
  if (!TEST_int_eq(EVP_PKEY_sign_message_init(sctx, sig_alg, params), 1) ||
      ...
      !TEST_int_eq(EVP_PKEY_sign(sctx, psig, &psig_len, td->msg, td->msg_len),
                   1))
    goto err;
  if (!TEST_int_eq(EVP_Q_digest(lib_ctx, "SHA256", NULL, psig, psig_len, digest,
                                &digest_len),
                   1))
    goto err;
...
static int slh_dsa_usage_test(void) {
  ...
  /* Generate a key */
  if (!TEST_ptr(
      gctx = EVP_PKEY_CTX_new_from_name(lib_ctx, "SLH-DSA-SHA2-128s", NULL)) ||
  ...
  if (!TEST_ptr(pub_bio = BIO_new(BIO_s_mem())) ||
      ...
      !TEST_true(PEM_write_bio_PrivateKey_ex(
      priv_bio, gkey, cipher, NULL, 0, NULL, (void *)pass, lib_ctx, NULL)))
    goto err;
  ...
  if (!TEST_ptr(
      ...
      !TEST_ptr(sig_alg =
                EVP_SIGNATURE_fetch(lib_ctx, "SLH-DSA-SHA2-128s", NULL)) ||
...
static int slh_dsa_deterministic_usage_test(void) {
  ...
  *p = OSSL_PARAM_construct_end();
  ...
  if (!TEST_ptr(sig = OPENSSL_zalloc(sig_len * 2)) ||
      ...
      !TEST_int_eq(EVP_PKEY_sign(dupctx, sig + sig_len, &len, msg, msg_len),
                   1) ||
      !TEST_size_t_eq(sig_len, len))
    goto err;
  /* Read the public key and add to a verify ctx */
  ...
  if (!TEST_int_eq(EVP_PKEY_verify_message_init(vctx, sig_alg, NULL), 1) ||
      ...
      !TEST_int_eq(EVP_PKEY_verify(vctx, sig, sig_len, msg, msg_len), 1) ||
      !TEST_int_eq(
      EVP_PKEY_verify(dupctx, sig + sig_len, sig_len, msg, msg_len), 1))
    goto err;
  ret = 1;
err:
  EVP_CIPHER_free(cipher);
...
static int slh_dsa_keygen_invalid_test(void) {
  ...
  p[0] =
  OSSL_PARAM_construct_octet_string(OSSL_PKEY_PARAM_SLH_DSA_SEED, seed, 97);
  p[1] = OSSL_PARAM_construct_end();
  if (!TEST_int_eq(EVP_PKEY_CTX_set_params(ctx, params), 0))
    goto err;

  /* Test the generate fails if the seed is not the correct size */
  p[0] = OSSL_PARAM_construct_octet_string(OSSL_PKEY_PARAM_SLH_DSA_SEED, seed,
                                           n * 3 - 1);
  p[1] = OSSL_PARAM_construct_end();
