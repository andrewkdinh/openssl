...
static EVP_PKEY *slh_dsa_key_from_data(const char *alg, const unsigned char *data, size_t datalen, int public)
{
    int ret;
    ...
    ret = TEST_ptr(ctx = EVP_PKEY_CTX_new_from_name(lib_ctx, alg, NULL)) && TEST_int_eq(EVP_PKEY_fromdata_init(ctx), 1)
...
static int slh_dsa_create_keypair(EVP_PKEY **pkey, const char *name, const uint8_t *priv, size_t priv_len,
                                  const uint8_t *pub, size_t pub_len)
{
    ...
    if (!TEST_ptr(bld = OSSL_PARAM_BLD_new())
        || !TEST_true(OSSL_PARAM_BLD_push_octet_string(bld, OSSL_PKEY_PARAM_PRIV_KEY, priv, priv_len) > 0)
        || !TEST_true(OSSL_PARAM_BLD_push_octet_string(bld, pub_name, pub, pub_len) > 0)
...
static int slh_dsa_bad_pub_len_test(void)
{
    ...
    if (!TEST_ptr_null(pkey = slh_dsa_key_from_data(td->alg, pubdata, td->pub_len - 1, 1))
        || !TEST_ptr_null(pkey = slh_dsa_key_from_data(td->alg, pubdata, td->pub_len + 1, 1)))
        goto end;

    ret = 1;
end:
    if (ret == 0)
        TEST_note("Incorrectly accepted public key of length %u (expected %u)", (unsigned)pub_len,
                  (unsigned)td->pub_len);
    EVP_PKEY_free(pkey);
    return ret == 1;
}
...
static int slh_dsa_key_eq_test(void)
{
    ...
    if (!TEST_int_eq(EVP_PKEY_eq(key[0], key[1]), 1) || !TEST_int_ne(EVP_PKEY_eq(key[0], key[2]), 1)
...
static int slh_dsa_key_validate_failure_test(void)
{
     ...
     * public key.
     */
    if (!TEST_ptr(key = slh_dsa_key_from_data("SLH-DSA-SHA2-128f", slh_dsa_sha2_128s_0_keygen_priv,
 ...
 */
static int do_slh_dsa_verify(const SLH_DSA_SIG_TEST_DATA *td, uint8_t *sig, size_t sig_len)
{
    int ret = 0;
    EVP_PKEY_CTX *vctx = NULL;
    EVP_PKEY *key = NULL;
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    if (!TEST_int_eq(EVP_PKEY_verify_message_init(vctx, sig_alg, params), 1)
        || !TEST_int_eq(EVP_PKEY_verify(vctx, sig, sig_len, td->msg, td->msg_len), 1))
        goto err;
    ret = 1;
err:
...
static int slh_dsa_sign_verify_test(int tst_id)
{
    ...
    *p = OSSL_PARAM_construct_end();
     ...
     */
    if (!slh_dsa_create_keypair(&pkey, td->alg, td->priv, td->priv_len, td->pub, td->pub_len))
        goto err;
    ...
    if (!TEST_int_eq(EVP_PKEY_sign_message_init(sctx, sig_alg, params), 1)
        ...
        goto err;
    if (!TEST_int_eq(EVP_Q_digest(lib_ctx, "SHA256", NULL, psig, psig_len, digest, &digest_len), 1))
        goto err;
...
static EVP_PKEY *do_gen_key(const char *alg, const uint8_t *seed, size_t seed_len)
{
    ...
    if (seed_len != 0)
        *p++ = OSSL_PARAM_construct_octet_string(OSSL_PKEY_PARAM_SLH_DSA_SEED, (char *)seed, seed_len);
    *p = OSSL_PARAM_construct_end();
...
static int slh_dsa_keygen_test(int tst_id)
{
    ...
    if (!TEST_true(EVP_PKEY_get_octet_string_param(pkey, OSSL_PKEY_PARAM_PRIV_KEY, priv, sizeof(priv), &priv_len)))
        goto err;
    if (!TEST_true(EVP_PKEY_get_octet_string_param(pkey, OSSL_PKEY_PARAM_PUB_KEY, pub, sizeof(pub), &pub_len)))
        goto err;
    if (!TEST_true(EVP_PKEY_get_int_param(pkey, OSSL_PKEY_PARAM_BITS, &bits)) || !TEST_int_eq(bits, 8 * 2 * n)
        || !TEST_true(EVP_PKEY_get_int_param(pkey, OSSL_PKEY_PARAM_SECURITY_BITS, &sec_bits))
        || !TEST_int_eq(sec_bits, 8 * n) || !TEST_true(EVP_PKEY_get_int_param(pkey, OSSL_PKEY_PARAM_MAX_SIZE, &sig_len))
        || !TEST_int_ge(sig_len, 7856) || !TEST_int_le(sig_len, 49856))
        goto err;

    if (!TEST_size_t_eq(priv_len, key_len) || !TEST_size_t_eq(pub_len, key_len / 2))
        goto err;
...
static int slh_dsa_usage_test(void)
{
    ...
    if (!TEST_ptr(gctx = EVP_PKEY_CTX_new_from_name(lib_ctx, "SLH-DSA-SHA2-128s", NULL))
        || !TEST_int_eq(EVP_PKEY_keygen_init(gctx), 1) || !TEST_int_eq(EVP_PKEY_keygen(gctx, &gkey), 1))
        goto err;

    /* Save it to a BIO - it uses a mem bio for testing */
    if (!TEST_ptr(pub_bio = BIO_new(BIO_s_mem())) || !TEST_ptr(priv_bio = BIO_new(BIO_s_mem()))
        || !TEST_ptr(cipher = EVP_CIPHER_fetch(lib_ctx, "AES-256-CBC", NULL))
        || !TEST_true(PEM_write_bio_PUBKEY_ex(pub_bio, gkey, lib_ctx, NULL))
        || !TEST_true(PEM_write_bio_PrivateKey_ex(priv_bio, gkey, cipher, NULL, 0, NULL, (void *)pass, lib_ctx, NULL)))
        goto err;
    ...
    if (!TEST_int_eq(EVP_PKEY_sign(sctx, NULL, &sig_len, msg, msg_len), 1) || !TEST_ptr(sig = OPENSSL_malloc(sig_len))
        || !TEST_int_eq(EVP_PKEY_sign(sctx, sig, &sig_len, msg, msg_len), 1))
        goto err;
    if (!TEST_true(EVP_PKEY_pairwise_check(sctx)) || !TEST_true(EVP_PKEY_public_check(sctx))
...
static int slh_dsa_deterministic_usage_test(void)
{
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    if (!TEST_int_eq(EVP_PKEY_sign(sctx, NULL, &sig_len, msg, msg_len), 1))
        goto err;
    len = sig_len;
    if (!TEST_ptr(sig = OPENSSL_zalloc(sig_len * 2)) || !TEST_int_eq(EVP_PKEY_sign(sctx, sig, &len, msg, msg_len), 1)
        || !TEST_size_t_eq(sig_len, len) || !TEST_int_eq(EVP_PKEY_sign(dupctx, sig + sig_len, &len, msg, msg_len), 1)
    ...
    /* verify the signature */
    if (!TEST_int_eq(EVP_PKEY_verify_message_init(vctx, sig_alg, NULL), 1) || !TEST_ptr(dupctx = EVP_PKEY_CTX_dup(vctx))
        || !TEST_int_eq(EVP_PKEY_verify(vctx, sig, sig_len, msg, msg_len), 1)
        || !TEST_int_eq(EVP_PKEY_verify(dupctx, sig + sig_len, sig_len, msg, msg_len), 1))
        goto err;
    ret = 1;
err:
    EVP_CIPHER_free(cipher);
...
static int slh_dsa_digest_sign_verify_test(void)
{
    ...
    *p++ = OSSL_PARAM_construct_octet_string(OSSL_SIGNATURE_PARAM_CONTEXT_STRING, context, sizeof(context));
    *p++ = OSSL_PARAM_construct_end();
    ...
    if (!TEST_ptr(mctx = EVP_MD_CTX_new())
        ...
        goto err;
    if (!TEST_int_eq(EVP_DigestSign(mctx, NULL, &sig_len, msg, msg_len), 1) || !TEST_ptr(sig = OPENSSL_zalloc(sig_len)))
        goto err;
    sig_len--;
...
static int slh_dsa_keygen_invalid_test(void)
{
    ...
    p[0] = OSSL_PARAM_construct_octet_string(OSSL_PKEY_PARAM_SLH_DSA_SEED, seed, n * 3 - 1);
    p[1] = OSSL_PARAM_construct_end();

    if (!TEST_int_eq(EVP_PKEY_CTX_set_params(ctx, params), 1) || !TEST_int_eq(EVP_PKEY_generate(ctx, &pkey), 0))
        goto err;
