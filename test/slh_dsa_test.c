...
static int slh_dsa_create_keypair(EVP_PKEY **pkey, const char *name,
    ...
    if (!TEST_ptr(bld = OSSL_PARAM_BLD_new()) ||
        !TEST_true(OSSL_PARAM_BLD_push_octet_string(
                   bld, OSSL_PKEY_PARAM_PRIV_KEY, priv, priv_len) > 0) ||
        !TEST_true(
        OSSL_PARAM_BLD_push_octet_string(bld, pub_name, pub, pub_len) > 0) ||
...
static int slh_dsa_key_validate_test(void) {
    ...
    if (!TEST_ptr(key =
                  slh_dsa_key_from_data(td->alg, td->pub, td->pub_len, 1)))
        return 0;
...
static int slh_dsa_key_validate_failure_test(void) {
    ...
    /*
     * Loading 128s private key data into a 128f algorithm will have an
