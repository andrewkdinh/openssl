...
static int slh_dsa_key_eq_test(void)
{
    ...
    if (!TEST_ptr(
            key[0] = slh_dsa_key_from_data(td1->alg, td1->pub, td1->pub_len, 1))
        || !TEST_ptr(
            key[1] = slh_dsa_key_from_data(td1->alg, td1->pub, td1->pub_len, 1))
        || !TEST_ptr(
            key[2] = slh_dsa_key_from_data(td2->alg, td2->pub, td2->pub_len, 1))
    ...
    for (i = 0; i < OSSL_NELEM(key); ++i) EVP_PKEY_free(key[i]);
    return ret;
}
                             ...
                             size_t sig_len)
{
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    if (!TEST_int_eq(EVP_PKEY_verify_message_init(vctx, sig_alg, params), 1)
        || !TEST_int_eq(
            EVP_PKEY_verify(vctx, sig, sig_len, td->msg, td->msg_len), 1))
        goto err;
    ret = 1;
err:
...
static int slh_dsa_usage_test(void)
{
    ...
    /* Generate a key */
    if (!TEST_ptr(gctx = EVP_PKEY_CTX_new_from_name(lib_ctx,
                                                    "SLH-DSA-SHA2-128s", NULL))
...
static int slh_dsa_deterministic_usage_test(void)
{
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    if (!TEST_ptr(sig = OPENSSL_zalloc(sig_len * 2))
        ...
        || !TEST_int_eq(
            EVP_PKEY_sign(dupctx, sig + sig_len, &len, msg, msg_len), 1)
        || !TEST_size_t_eq(sig_len, len))
        goto err;
    /* Read the public key and add to a verify ctx */
    if (!TEST_ptr(
