...
static int test_bio_tfo(int idx) {
    ...
    /* SECOND ACCEPT: if TFO is supported, this will still fail until data is
     * sent
     */
    if (BIO_do_accept(abio) <= 0) {
        if (!BIO_should_retry(abio)) {
        ...
        if (idx == 0)
            ...
            BIO_printf(bio_err,
                       "Ignore: connection accepted before data, possibly "
                       "no TFO cookie, or TFO may not be enabled\n");
        else if (idx == 4)
            BIO_printf(bio_err, "Success: connection accepted before data, "
                                "client TFO is disabled\n");
        else
            BIO_printf(bio_err, "Warning: connection accepted before data, TFO "
                                "may not be enabled\n");
        sbio = BIO_pop(abio);
        goto success;
    }
    ...
    /* FINAL ACCEPT: if TFO is enabled, socket should be accepted at *this*
     * point
     */
    if (BIO_do_accept(abio) <= 0) {
        sockerr = get_last_socket_error();
    ...
    if (!TEST_ptr(sbio = BIO_pop(abio)) ||
        !TEST_true(
        BIO_read_ex(sbio, read_buffer, sizeof(read_buffer), &bytes)) ||
...
static int test_fd_tfo(int idx) {
    ...
    if (!TEST_int_ge(
        afd = BIO_socket(ai->ai_family, ai->ai_socktype, ai->ai_protocol, 0),
        0) ||
    ...
    if (!TEST_true(
        BIO_ADDR_rawmake(baddr, sstorage.ss_family, addr, addrlen, port)))
        goto err;

    /* CLIENT SOCKET */
    if (!TEST_int_ge(
        cfd = BIO_socket(ai->ai_family, ai->ai_socktype, ai->ai_protocol, 0),
        0))
        goto err;
