...
static void test_fail_string_common(const char *prefix, const char *file, int line, const char *type, const char *left,
                                    const char *right, const char *op, const char *m1, size_t l1, const char *m2,
                                    size_t l2)
{
 ...
 */
void test_fail_string_message(const char *prefix, const char *file, int line, const char *type, const char *left,
                              const char *right, const char *op, const char *m1, size_t l1, const char *m2, size_t l2)
{
...
{
    test_fail_string_common("string", NULL, 0, NULL, NULL, NULL, name, m, l, m, l);
}
 ...
 * every so often.
 */
static void hex_convert_memory(const unsigned char *m, size_t n, char *b, size_t width)
{
    size_t i;
 ...
 */
static const int bn_bytes = (MAX_STRING_WIDTH - 9) / (BN_OUTPUT_SIZE * 2 + 1) * BN_OUTPUT_SIZE;
static const int bn_chars = (MAX_STRING_WIDTH - 9) / (BN_OUTPUT_SIZE * 2 + 1) * (BN_OUTPUT_SIZE * 2 + 1) - 1;
 ...
 */
static int convert_bn_memory(const unsigned char *in, size_t bytes, char *out, int *lz, const BIGNUM *bn)
{
    int n = bytes * 2, i;
    char *p = out, *q = NULL;
    const char *r;
 ...
 */
static void test_fail_bignum_common(const char *prefix, const char *file, int line, const char *type, const char *left,
                                    const char *right, const char *op, const BIGNUM *bn1, const BIGNUM *bn2)
{
        ...
        *p++ = '\0';
        if (!diff) {
            ...
            if (cnt == 0 && bn2 == NULL)
                ...
                test_printf_stderr("+%s:% 5d\n", b2, cnt);
            if (real_diff && (cnt == 0 || (n1 > 0 && n2 > 0)) && bn1 != NULL && bn2 != NULL)
                test_printf_stderr(" %s\n", bdiff);
        }
        if (m1 != NULL)
            m1 += bytes;
 ...
 */
void test_fail_bignum_message(const char *prefix, const char *file, int line, const char *type, const char *left,
                              const char *right, const char *op, const BIGNUM *bn1, const BIGNUM *bn2)
{
...
void test_fail_bignum_mono_message(const char *prefix, const char *file, int line, const char *type, const char *left,
                                   const char *right, const char *op, const BIGNUM *bn)
{
...
void test_output_bignum(const char *name, const BIGNUM *bn)
{
    if (bn == NULL || BN_is_zero(bn)) {
        ...
        test_fail_bignum_common("bignum", NULL, 0, NULL, NULL, NULL, name, bn, bn);
    }
}
 ...
 */
static void test_fail_memory_common(const char *prefix, const char *file, int line, const char *type, const char *left,
                                    const char *right, const char *op, const unsigned char *m1, size_t l1,
 ...
 */
void test_fail_memory_message(const char *prefix, const char *file, int line, const char *type, const char *left,
                              const char *right, const char *op, const unsigned char *m1, size_t l1,
                              const unsigned char *m2, size_t l2)
{
...
{
    test_fail_memory_common("memory", NULL, 0, NULL, NULL, NULL, name, m, l, m, l);
}
