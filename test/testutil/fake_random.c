...
static void fake_rand_freectx(void *vrng) { OPENSSL_free(vrng); }
...
static int fake_rand_enable_locking(void *vrng) { return 1; }

static int fake_rand_get_ctx_params(ossl_unused void *vrng,
                                    OSSL_PARAM params[]) {
    FAKE_RAND *frng = (FAKE_RAND *)vrng;
    OSSL_PARAM *p;
...
fake_rand_gettable_ctx_params(ossl_unused void *vrng,
...
{"FAKE", "provider=fake", fake_rand_functions}, {NULL, NULL, NULL}};
...
void fake_rand_finish(OSSL_PROVIDER *p) { OSSL_PROVIDER_unload(p); }
