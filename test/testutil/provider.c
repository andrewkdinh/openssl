...
static int fips_provider_version(OSSL_LIB_CTX *libctx, FIPS_VERSION *vers)
{
    ...
    *params = OSSL_PARAM_construct_utf8_ptr(OSSL_PROV_PARAM_VERSION, &vs, 0);
    ...
    if (!OSSL_PROVIDER_get_params(fips_prov, params)
        || sscanf(vs, "%d.%d.%d", &vers->major, &vers->minor, &vers->patch)
            != 3)
        goto err;
...
int fips_provider_version_match(OSSL_LIB_CTX *libctx, const char *versions)
{
    ...
    enum { MODE_EQ, MODE_NE, MODE_LE, MODE_LT, MODE_GT, MODE_GE } mode;

    while (*versions != '\0') {
        for (; isspace((unsigned char)(*versions)); versions++) continue;
        if (*versions == '\0')
            break;
        for (p = versions;
             *versions != '\0' && !isspace((unsigned char)(*versions));
             versions++)
            continue;
        if (*p == '!') {
            mode = MODE_NE;
