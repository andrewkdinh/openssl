...
static const unsigned char gcm_key[] = {0xee, 0xbc, 0x1f, 0x57, 0x48, 0x7f, 0x51, 0x92, 0x1c, 0x04, 0x65,
...
static const unsigned char gcm_pt[] = {0xf5, 0x6e, 0x87, 0x05, 0x5b, 0xc3, 0x2d, 0x0e,
                                       0xeb, 0x31, 0xb2, 0xea, 0xcc, 0x2b, 0xf2, 0xa5};
static const unsigned char gcm_aad[] = {0x4d, 0x23, 0xc3, 0xce, 0xc3, 0x34, 0xb4, 0x9b,
                                        0xdb, 0x37, 0x0c, 0x43, 0x7f, 0xec, 0x78, 0xde};
static const unsigned char gcm_ct[] = {0xf7, 0x26, 0x44, 0x13, 0xa8, 0x4c, 0x0e, 0x7c,
                                       0xd5, 0x36, 0x86, 0x7e, 0xb9, 0xf2, 0x17, 0x36};
static const unsigned char gcm_tag[] = {0x67, 0xba, 0x05, 0x10, 0x26, 0x2a, 0xe4, 0x87,
...
static int do_encrypt(unsigned char *iv_gen, unsigned char *ct, int *ct_len, unsigned char *tag, int *tag_len)
{
    ...
    *tag_len = 16;
    ret = TEST_ptr(ctx = EVP_CIPHER_CTX_new())
        && TEST_true(EVP_EncryptInit_ex(ctx, EVP_aes_256_gcm(), NULL, NULL, NULL) > 0)
        && TEST_true(EVP_EncryptInit_ex(ctx, NULL, NULL, gcm_key, iv_gen != NULL ? NULL : gcm_iv) > 0)
        && TEST_true(EVP_EncryptUpdate(ctx, NULL, &outlen, gcm_aad, sizeof(gcm_aad)) > 0)
        && TEST_true(EVP_EncryptUpdate(ctx, ct, ct_len, gcm_pt, sizeof(gcm_pt)) > 0)
...
static int do_decrypt(const unsigned char *iv, const unsigned char *ct, int ct_len, const unsigned char *tag,
                      int tag_len)
{
    int ret = 0;
    EVP_CIPHER_CTX *ctx = NULL;
    ...
    ret = TEST_ptr(ctx = EVP_CIPHER_CTX_new())
        ...
        && TEST_int_eq(EVP_CIPHER_CTX_get_tag_length(ctx), 16)
        && TEST_true(EVP_DecryptUpdate(ctx, NULL, &outlen, gcm_aad, sizeof(gcm_aad)) > 0)
        ...
        && TEST_true(EVP_DecryptFinal_ex(ctx, outbuf, &outlen) > 0) && TEST_mem_eq(gcm_pt, sizeof(gcm_pt), pt, ptlen);
...
static int kat_test(void)
{
    ...
    return do_encrypt(NULL, ct, &ctlen, tag, &taglen) && TEST_mem_eq(gcm_ct, sizeof(gcm_ct), ct, ctlen)
        && TEST_mem_eq(gcm_tag, sizeof(gcm_tag), tag, taglen) && do_decrypt(gcm_iv, ct, ctlen, tag, taglen);
}
...
static int badkeylen_test(void)
{
    ...
    ret = TEST_ptr(cipher = EVP_aes_192_gcm()) && TEST_ptr(ctx = EVP_CIPHER_CTX_new())
