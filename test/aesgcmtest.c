                      ...
                      unsigned char *tag, int *tag_len)
{
    ...
    *tag_len = 16;
    ret =
        ...
        && TEST_true(
            EVP_EncryptInit_ex(ctx, EVP_aes_256_gcm(), NULL, NULL, NULL) > 0)
        ...
        && TEST_true(EVP_EncryptUpdate(ctx, ct, ct_len, gcm_pt, sizeof(gcm_pt))
                     > 0)
        && TEST_true(EVP_EncryptFinal_ex(ctx, outbuf, &outlen) > 0)
        && TEST_int_eq(EVP_CIPHER_CTX_get_tag_length(ctx), 16)
        && TEST_true(EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, 16, tag)
                     > 0)
