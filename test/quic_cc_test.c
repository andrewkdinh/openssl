...
static void do_free(NET_PKT *pkt) { OPENSSL_free(pkt); }
...
static int net_sim_send(struct net_sim *s, size_t sz) {
  ...
  if (success) {
    ...
    pkt->determination_time =
    ossl_time_add(pkt->arrive_time, ossl_ms2time(s->latency));
     ...
     */
    pkt->determination_time =
    ossl_time_add(pkt->tx_time, ossl_ms2time(3 * s->latency));
...
static int dump_state(const OSSL_CC_METHOD *ccm, OSSL_CC_DATA *cc,
  ...
  if (!TEST_true(
      ccm->get_option_uint(cc, OSSL_CC_OPTION_CUR_CWND_SIZE, &cwnd_size)))
    return 0;

  if (!TEST_true(
      ccm->get_option_uint(cc, OSSL_CC_OPTION_CUR_BYTES_IN_FLIGHT, &cur_bytes)))
    return 0;
...
static int test_simulate(void) {
  ...
  *p++ =
  OSSL_PARAM_construct_size_t(OSSL_CC_OPTION_MAX_DGRAM_PAYLOAD_LEN, &mdpl);
...
static int test_sanity(void) {
  ...
  *p++ = OSSL_PARAM_construct_end();
  ...
  p = params;
  *p++ =
  OSSL_PARAM_construct_size_t(OSSL_CC_OPTION_MAX_DGRAM_PAYLOAD_LEN, &diag_mdpl);
  *p++ = OSSL_PARAM_construct_uint64(OSSL_CC_OPTION_CUR_BYTES_IN_FLIGHT,
                                     &diag_cur_bytes_in_flight);
  *p++ = OSSL_PARAM_construct_end();
