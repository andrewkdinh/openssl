...
int pkcs12_main(int argc, char **argv) {
  ...
  if (cpass != NULL) {
    ...
    if (twopass) {
      if (export_pkcs12)
        BIO_printf(
        bio_err, "Option -twopass cannot be used with -passout or -password\n");
      else
        BIO_printf(
        bio_err, "Option -twopass cannot be used with -passin or -password\n");
      goto end;
    }
  } else {
    cpass = pass;
  ...
  if (export_pkcs12) {
    ...
    p12 = PKCS12_create_ex2(cpass, name, key, ee_cert, certs, key_pbe, cert_pbe,
                            iter, -1, keytype, app_get0_libctx(),
    ...
    if (maciter != -1) {
      if (pbmac1_pbkdf2 == 1) {
        if (!PKCS12_set_pbmac1_pbkdf2(p12, mpass, -1, NULL, macsaltlen, maciter,
