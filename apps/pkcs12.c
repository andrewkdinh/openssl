...
int pkcs12_main(int argc, char **argv) {
    ...
    if (export_pkcs12) {
        ...
        if (!(options & NOKEYS)) {
            key = load_key(keyname ? keyname : infile, FORMAT_PEM, 1, passin, e,
                           keyname ? "private key from -inkey file"
        ...
        if (chain) {
            ...
            if (vret == X509_V_OK) {
                ...
                certs, chain2, X509_ADD_FLAG_UP_REF | X509_ADD_FLAG_NO_DUP);
        ...
        p12 = PKCS12_create_ex2(cpass, name, key, ee_cert, certs, key_pbe,
                                cert_pbe, iter, -1, keytype, app_get0_libctx(),
                                app_get0_propq(), jdk_trust, (void *)obj);
        ...
        if (maciter != -1) {
            if (pbmac1_pbkdf2 == 1) {
                if (!PKCS12_set_pbmac1_pbkdf2(p12, mpass, -1, NULL, macsaltlen,
                                              maciter, macmd,
                                              pbmac1_pbkdf2_md)) {
                ...
                if (!PKCS12_set_mac(p12, mpass, -1, NULL, macsaltlen, maciter,
                                    macmd)) {
                    BIO_printf(
                    bio_err,
                    "Error creating PKCS12 MAC; no PKCS12KDF support?\n");
                    BIO_printf(bio_err, "Use -nomac or -pbmac1_pbkdf2 if "
                                        "PKCS12KDF support not available\n");
                    goto export_end;
                }
            }
        }
    ...
    if ((options & INFO) && PKCS12_mac_present(p12)) {
        ...
        if (OBJ_obj2nid(macobj) == NID_pbmac1) {
            ...
            if (pbkdf2_param == NULL) {
                ...
                BIO_printf(
                bio_err, "Key length: %ld, Salt length: %d\n",
    ...
    if (macver) {
        ...
        if (OBJ_obj2nid(macobj) != NID_pbmac1) {
            ...
            pkcs12kdf =
            EVP_KDF_fetch(app_get0_libctx(), "PKCS12KDF", app_get0_propq());
            if (pkcs12kdf == NULL) {
                BIO_printf(
                bio_err, "Error verifying PKCS12 MAC; no PKCS12KDF support.\n");
                BIO_printf(
                bio_err,
...
int dump_certs_pkeys_bag(BIO *out, const PKCS12_SAFEBAG *bag, const char *pass,
    ...
    case NID_safeContentsBag:
        ...
        print_attribs(out, attrs, "Bag Attributes");
        return dump_certs_pkeys_bags(out, PKCS12_SAFEBAG_get0_safes(bag), pass,
                                     passlen, options, pempass, enc);
