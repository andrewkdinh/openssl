...
{"maxfraglen", OPT_MAXFRAGLEN, 'p',
 "Enable Maximum Fragment Length Negotiation (len values: 512, 1024, 2048 and "
 "4096)"},
...
{"name", OPT_PROTOHOST, 's',
 "Hostname to use for \"-starttls lmtp\", \"-starttls smtp\" or \"-starttls "
 "xmpp[-server]\""},
...
{"nextprotoneg", OPT_NEXTPROTONEG, 's',
 "Enable NPN extension, considering named protocols supported (comma-separated "
 "list)"},
#endif
...
/* Known false-positive of MemorySanitizer. */
    ...
    if (bindstr != NULL) {
        int res;
        res =
        BIO_parse_hostserv(bindstr, &bindhost, &bindport, BIO_PARSE_PRIO_HOST);
        if (!res) {
            BIO_printf(bio_err,
    ...
    if (cert_file != NULL) {
        cert =
        load_cert_pass(cert_file, cert_format, 1, pass, "client certificate");
        if (cert == NULL)
            goto end;
    }

    if (chain_file != NULL) {
        if (!load_certs(chain_file, 0, &chain, pass,
    ...
    if (isquic && alpn_in == NULL) {
        BIO_printf(bio_err,
                   "%s: QUIC requires ALPN to be specified (e.g. \"h3\" for "
                   "HTTP/3) via the -alpn option\n",
                   prog);
        goto end;
    }
#endif
...
re_start:
    ...
    case PROTO_NNTP: {
        ...
        (void)BIO_flush(fbio);
        BIO_gets(fbio, mbuf, BUFSIZZ);
        /* no point in trying to parse the CAPABILITIES response if there is
         * none */
    ...
    for (;;) {
        ...
        if ((isdtls || isquic) &&
            SSL_get_event_timeout(con, &timeout, &is_infinite) && !is_infinite)
            timeoutp = &timeout;
        else
            timeoutp = NULL;
...
#else
            i =
            select(width, (void *)&readfds, (void *)&writefds, NULL, timeoutp);
#endif
            if (i < 0) {
        ...
        if (timeoutp != NULL) {
            ...
            if (isdtls && !FD_ISSET(SSL_get_fd(con), &readfds) &&
...
static void print_stuff(BIO *bio, SSL *s, int full) {
    ...
    if (full) {
        ...
        if (sk != NULL) {
            ...
            for (i = 0; i < sk_X509_num(sk); i++) {
                ...
                X509_NAME_print_ex(bio, X509_get_subject_name(chain_cert), 0,
                                   get_nameopt());
                BIO_puts(bio, "\n");
                BIO_printf(bio, "   i:");
                X509_NAME_print_ex(bio, X509_get_issuer_name(chain_cert), 0,
                                   get_nameopt());
                BIO_puts(bio, "\n");
    ...
    if (istls13) {
         ...
         * on when we get a NewSessionTicket. Therefore, we print it now as
         * well.
         */
    ...
    if (SSL_get_session(s) != NULL && keymatexportlabel != NULL) {
        ...
        if (SSL_export_keying_material(
            ...
            strlen(keymatexportlabel), NULL, 0, 0) <= 0) {
...
static int user_data_execute(struct user_data_st *user_data, int cmd,
                             char *arg) {
    switch (cmd) {
    case USER_COMMAND_HELP:
        ...
        if (SSL_is_quic(user_data->con)) {
            BIO_printf(bio_err, "  {fin}: Send FIN on the stream. No further "
                                "writing is possible\n");
    ...
    if (user_data->mode == USER_DATA_MODE_BASIC) {
        ...
        if (cmd_start == buf_start) {
            ...
            if (cmd_end == NULL) {
                ...
                BIO_printf(
                bio_err, "ERROR PROCESSING COMMAND. REST OF LINE IGNORED: %s\n",
                cmd_start);
                user_data->buflen = user_data->bufoff = *len = *off = 0;
                return USER_DATA_PROCESS_NO_DATA;
            }
