...
/* Known false-positive of MemorySanitizer. */
  ...
  while ((o = opt_next()) != OPT_EOF) {
    ...
    case OPT_MAXFRAGLEN:
      ...
      default:
        BIO_printf(bio_err,
                   "%s: Max Fragment Len %u is out of permitted values", prog,
                   len);
        goto opthelp;
      }
      break;
  ...
  if (opt_num_rest() == 1) {
    ...
    if (connectstr != NULL) {
      BIO_printf(
      bio_err, "%s: cannot provide both -connect option and target parameter\n",
      prog);
      goto opthelp;
    }
    connect_type = use_inet;
  ...
  if (noservername) {
    if (servername != NULL) {
      BIO_printf(
      bio_err, "%s: Can't use -servername and -noservername together\n", prog);
      goto opthelp;
    }
  ...
  if (proxystr != NULL) {
    ...
    /* Retain the original target host:port for use in the HTTP proxy connect
     * string */
...
re_start:
  ...
  for (;;) {
    ...
    if (!ssl_pending && ((!isquic && FD_ISSET(SSL_get_fd(con), &writefds)) ||
