...
static void user_data_init(struct user_data_st *user_data, SSL *con, char *buf, size_t bufmax, int mode);
static int user_data_add(struct user_data_st *user_data, size_t i);
static int user_data_process(struct user_data_st *user_data, size_t *len, size_t *off);
...
static unsigned int psk_client_cb(SSL *ssl, const char *hint, char *identity, unsigned int max_identity_len,
...
static int psk_use_session_cb(SSL *s, const EVP_MD *md, const unsigned char **id, size_t *idlen, SSL_SESSION **sess)
{
    ...
    if (psksess != NULL) {
        ...
        if (usesess == NULL || !SSL_SESSION_set1_master_key(usesess, key, key_len)
...
static int next_proto_cb(SSL *s, unsigned char **out, unsigned char *outlen, const unsigned char *in,
...
# endif
    ...
    {"connect", OPT_CONNECT, 's', "TCP/IP where to connect; default: " PORT ")"},
    {"bind", OPT_BIND, 's', "bind local address for connection"},
    {"proxy", OPT_PROXY, 's', "Connect to via specified proxy to the real server"},
...
# endif
    ...
    {"no-interactive", OPT_NO_INTERACTIVE, '-', "Don't run the client in the interactive mode"},

    OPT_SECTION("Debug"),
    {"showcerts", OPT_SHOWCERTS, '-', "Show all certificates sent by the server"},
    {"debug", OPT_DEBUG, '-', "Extra output"},
    {"msg", OPT_MSG, '-', "Show protocol messages"},
    {"msgfile", OPT_MSGFILE, '>', "File to send output of -msg or -trace, instead of stdout"},
...
# ifndef OPENSSL_NO_DTLS
    ...
    {"timeout", OPT_TIMEOUT, '-', "Enable send/receive timeout on DTLS connections"},
...
# endif
    ...
    {"verifyCAstore", OPT_VERIFYCASTORE, ':', "CA store URI for certificate verification"},
    OPT_X_OPTIONS,
    OPT_PROV_OPTIONS,
...
static const OPT_PAIR services[] = {
    {"smtp", PROTO_SMTP},         {"pop3", PROTO_POP3}, {"imap", PROTO_IMAP},
    {"ftp", PROTO_FTP},           {"xmpp", PROTO_XMPP}, {"xmpp-server", PROTO_XMPP_SERVER},
    {"telnet", PROTO_TELNET},     {"irc", PROTO_IRC},   {"mysql", PROTO_MYSQL},
    {"postgres", PROTO_POSTGRES}, {"lmtp", PROTO_LMTP}, {"nntp", PROTO_NNTP},
    {"sieve", PROTO_SIEVE},       {"ldap", PROTO_LDAP}, {NULL, 0}};
...
# else
        BIO_printf(bio_err, "%s: -proxy not supported in no-http build\n", prog);
        goto end;
# endif
    }
...
re_start:
    ...
    peer_addr = NULL;
    if (init_client(&sock, host, port, bindhost, bindport, socket_family, socket_type, protocol, tfo, !isquic,
                    &peer_addr)
        == 0) {
...
# endif
    ...
    case PROTO_FTP: {
        ...
        do {
            mbuf_len = BIO_gets(fbio, mbuf, BUFSIZZ);
                 ...
                 && (!isdigit((unsigned char)mbuf[0]) || !isdigit((unsigned char)mbuf[1])
                     || !isdigit((unsigned char)mbuf[2]) || mbuf[3] != ' '));
        (void)BIO_flush(fbio);
        BIO_pop(fbio);
        BIO_free(fbio);
    ...
    case PROTO_IRC: {
        ...
        do {
            ...
            timeout.tv_usec = 0;
                 ...
                 */
            if (!BIO_get_buffer_num_lines(fbio) && !BIO_pending(fbio) && !BIO_pending(sbio)
    ...
    for (;;) {
        ...
        if ((isdtls || isquic) && SSL_get_event_timeout(con, &timeout, &is_infinite) && !is_infinite)
            timeoutp = &timeout;
        else
            timeoutp = NULL;
        ...
        if (!write_ssl) {
            ...
            } while (!write_ssl && cbuf_len == 0 && user_data_has_data(&user_data));
            if (cbuf_len > 0) {
                read_tty = 0;
                timeout.tv_sec = 0;
                timeout.tv_usec = 0;
...
#  endif
             ...
             * the right thing.
             */
            if ((!isquic && read_ssl) || (isquic && SSL_net_read_desired(con)))
                openssl_fdset(SSL_get_fd(con), &readfds);
            if ((!isquic && write_ssl) || (isquic && (first_loop || SSL_net_write_desired(con))))
...
# endif
        ...
        if (timeoutp != NULL) {
            ...
            if (isdtls && !FD_ISSET(SSL_get_fd(con), &readfds) && !FD_ISSET(SSL_get_fd(con), &writefds))
                BIO_printf(bio_err, "TIMEOUT occurred\n");
        }

        if (!ssl_pending
            && ((!isquic && FD_ISSET(SSL_get_fd(con), &writefds)) || (isquic && (cbuf_len > 0 || first_loop)))) {
...
# endif
        ...
        } else if (ssl_pending || (!isquic && FD_ISSET(SSL_get_fd(con), &readfds))) {
# ifdef RENEG
            {
                static int iiii;
                if (++iiii == 52) {
...
static void print_cert_key_info(BIO *bio, X509 *cert)
{
    ...
    if (curve != NULL)
        BIO_printf(bio, "   a:PKEY: %s, (%s); sigalg: %s\n", keyalg, curve, OBJ_nid2ln(X509_get_signature_nid(cert)));
    else
...
# endif
    ...
    if (SSL_get_session(s) != NULL && keymatexportlabel != NULL) {
        ...
        if (SSL_export_keying_material(s, exportedkeymat, keymatexportlen, keymatexportlabel, strlen(keymatexportlabel),
                                       NULL, 0, 0)
            <= 0) {
...
static int user_data_process(struct user_data_st *user_data, size_t *len, size_t *off)
{
    ...
    if (user_data->mode == USER_DATA_MODE_BASIC) {
        ...
        case 'k':
            user_data->buflen = user_data->bufoff = *len = *off = 0;
            return user_data_execute(user_data, USER_COMMAND_KEY_UPDATE, buf_start[0] == 'K' ? "req" : "noreq");
        default:
            break;
        }
        ...
        if (cmd_start == buf_start) {
            ...
            if (cmd_end == NULL) {
                ...
                BIO_printf(bio_err, "ERROR PROCESSING COMMAND. REST OF LINE IGNORED: %s\n", cmd_start);
                user_data->buflen = user_data->bufoff = *len = *off = 0;
                return USER_DATA_PROCESS_NO_DATA;
            }
            *cmd_end = '\0';
            ...
            if (user_data->buf + 1 == cmd_start && user_data->buflen == 1
