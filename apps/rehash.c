...
static int handle_symlink(const char *filename, const char *fullpath)
{
    ...
    for (type = OSSL_NELEM(suffixes) - 1; type > 0; type--)
        if (OPENSSL_strncasecmp(&filename[i], suffixes[type],
                                strlen(suffixes[type]))
            == 0)
            break;
    ...
    if (name != NULL) {
        if (h == HASH_NEW || h == HASH_BOTH) {
            ...
            unsigned long hash_value = X509_NAME_hash_ex(
                name, app_get0_libctx(), app_get0_propq(), &ok);
        ...
        for (bp = hash_table[i]; bp; bp = nextbp) {
            ...
            for (ep = bp->first_entry; ep; ep = nextep) {
                ...
                if (ep->old_id < bp->num_needed) {
                    ...
                    while (bit_isset(idmask, nextid)) nextid++;

                    BIO_snprintf(buf, buflen, "%s%s%08x.%s%d", dirname, pathsep,
                                 bp->hash, suffixes[bp->type], nextid);
                    if (verbose)
                        BIO_printf(bio_out, "link %s -> %s\n", ep->filename,
                                   &buf[dirlen]);
                    ...
                    /* Link to be deleted */
                    BIO_snprintf(buf, buflen, "%s%s%08x.%s%d", dirname, pathsep,
                                 bp->hash, suffixes[bp->type], ep->old_id);
                    if (verbose)
...
int rehash_main(int argc, char **argv)
{
    ...
    if (*argv != NULL) {
        while (*argv != NULL) errs += do_dir(*argv++, h);
