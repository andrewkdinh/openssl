...
int ts_main(int argc, char **argv) {
  ...
  if (mode == OPT_QUERY) {
    ...
    if ((data != NULL) && (digest != NULL))
      goto opthelp;
    ret =
    !query_command(data, digest, md, policy, no_nonce, cert, in, out, text);
    ...
    ret =
    !reply_command(conf, section, engine, queryfile, password, inkey, md,
                   signer, chain, policy, in, token_in, out, token_out, text);

  } else if (mode == OPT_VERIFY) {
    if ((in == NULL) || !EXACTLY_ONE(queryfile, data, digest))
      goto opthelp;
    ret = !verify_command(data, digest, queryfile, in, token_in, CApath, CAfile,
                          CAstore, untrusted, vpmtouched ? vpm : NULL);
  } else {
    goto opthelp;
  }
...
static int reply_command(CONF *conf, const char *section, const char *engine,
  ...
  if (in != NULL) {
    ...
    response = create_response(conf, section, engine, queryfile, passin, inkey,
...
static TS_VERIFY_CTX *create_verify_ctx(const char *data, const char *digest,
  ...
  if (!TS_VERIFY_CTX_set0_store(
      ctx, create_cert_store(CApath, CAfile, CAstore, vpm)))
    goto err;
...
static int verify_cb(int ok, X509_STORE_CTX *ctx) { return ok; }
