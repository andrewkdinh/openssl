...
{"numqbits", 0, 0,
 "Number of bits in the subprime parameter q if generating parameters or key "
 "(optional)"},
{NULL}};
...
int dsaparam_main(int argc, char **argv) {
    ...
    if (ctx == NULL) {
        BIO_printf(
        bio_err, "Error, DSA parameter generation context allocation failed\n");
        goto end;
    }
    if (numbits > 0) {
        ...
        if (numqbits > 0) {
            if (EVP_PKEY_CTX_set_dsa_paramgen_q_bits(ctx, numqbits) <= 0) {
                BIO_printf(bio_err, "Error, DSA key generation setting "
                                    "subprime bit length failed\n");
                goto end;
            }
        }
    ...
    if (genkey) {
        ...
        ctx =
        EVP_PKEY_CTX_new_from_pkey(app_get0_libctx(), params, app_get0_propq());
        if (ctx == NULL) {
            BIO_printf(bio_err,
