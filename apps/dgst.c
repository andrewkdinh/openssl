...
int dgst_main(int argc, char **argv) {
    ...
    if (sigkey != NULL) {
        ...
        if (do_verify)
            ...
            res =
            EVP_DigestSignInit_ex(mctx, &pctx, digestname, app_get0_libctx(),
                                  app_get0_propq(), sigkey, NULL);
        else
    ...
    if (argc == 0) {
        ...
        if (oneshot_sign)
            ...
            ret = do_fp(out, buf, inp, separator, out_bin, xoflen, sigkey,
                        sigbuf, siglen, NULL, md_name, "stdin");
    } else {
        ...
        for (i = 0; i < argc; i++) {
            if (BIO_read_filename(in, argv[i]) <= 0) {
                ...
                if (oneshot_sign) {
                    ...
                    if (do_fp(out, buf, inp, separator, out_bin, xoflen, sigkey,
                              sigbuf, siglen, sig_name, md_name, argv[i]))
                        ret = EXIT_FAILURE;
                }
            }
            (void)BIO_reset(bmd);
