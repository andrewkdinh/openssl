...
static int do_fp_oneshot_sign(BIO *out, EVP_MD_CTX *ctx, BIO *in, int sep,
                              int binout, EVP_PKEY *key, unsigned char *sigin,
                              int siglen, const char *sig_name,
                              const char *file);
int do_fp(BIO *out, unsigned char *buf, BIO *bp, int sep, int binout,
          int xoflen, EVP_PKEY *key, unsigned char *sigin, int siglen,
...
{"xoflen", OPT_XOFLEN, 'p',
 "Output length for XOF algorithms. To obtain the maximum security strength "
 "set this to 32 (or greater) for SHAKE128, and 64 (or greater) for SHAKE256"},
...
int dgst_main(int argc, char **argv) {
  ...
  if (keyfile != NULL) {
    ...
    }
    {
      ...
      if (EVP_PKEY_get_default_digest_name(sigkey, def_md, sizeof(def_md)) ==
          2 &&
  ...
  if (hmac_key != NULL) {
    ...
    sigkey = EVP_PKEY_new_raw_private_key(
    EVP_PKEY_HMAC, impl, (unsigned char *)hmac_key, strlen(hmac_key));
    if (sigkey == NULL)
      goto end;
  }
  ...
  if (sigkey != NULL) {
    ...
    if (do_verify)
      if (impl == NULL)
        res =
        EVP_DigestVerifyInit_ex(mctx, &pctx, digestname, app_get0_libctx(),
                                app_get0_propq(), sigkey, NULL);
      else
        res = EVP_DigestVerifyInit(mctx, &pctx, md, impl, sigkey);
    else if (impl == NULL)
      res = EVP_DigestSignInit_ex(mctx, &pctx, digestname, app_get0_libctx(),
                                  app_get0_propq(), sigkey, NULL);
    else
  ...
  if (argc == 0) {
    ...
    if (oneshot_sign)
      ret = do_fp_oneshot_sign(out, signctx, in, separator, out_bin, sigkey,
                               sigbuf, siglen, NULL, "stdin");
    else
    ...
    for (i = 0; i < argc; i++) {
      if (BIO_read_filename(in, argv[i]) <= 0) {
        ...
        if (oneshot_sign) {
          if (do_fp_oneshot_sign(out, signctx, in, separator, out_bin, sigkey,
                                 sigbuf, siglen, sig_name, argv[i]))
            ret = EXIT_FAILURE;
        } else {
          if (do_fp(out, buf, inp, separator, out_bin, xoflen, sigkey, sigbuf,
...
static void print_out(BIO *out, unsigned char *buf, size_t len, int sep,
                      int binout, const char *sig_name, const char *md_name,
                      const char *file) {
  int i, backslash = 0;
