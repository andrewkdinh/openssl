...
static void pkey_print_message(const char *str, const char *str2, unsigned int bits, int sec);
...
const OPTIONS speed_options[] = {
    ...
    {"mb", OPT_MB, '-', "Enable (tls1>=1) multi-block mode on EVP-named cipher"},
    ...
    {"signature-algorithms", OPT_SIG, '-', "Benchmark signature algorithms"},

    OPT_SECTION("Timing"),
    {"elapsed", OPT_ELAPSED, '-', "Use wall-clock time instead of CPU user time as divisor"},
...
static const OPT_PAIR dsa_choices[DSA_NUM] = {{"dsa1024", R_DSA_1024}, {"dsa2048", R_DSA_2048}};
...
static const OPT_PAIR rsa_choices[RSA_NUM] = {
    {"rsa512", R_RSA_512},   {"rsa1024", R_RSA_1024}, {"rsa2048", R_RSA_2048},  {"rsa3072", R_RSA_3072},
...
static const OPT_PAIR ffdh_choices[FFDH_NUM] = {
    {"ffdh2048", R_FFDH_2048}, {"ffdh3072", R_FFDH_3072}, {"ffdh4096", R_FFDH_4096},
    {"ffdh6144", R_FFDH_6144}, {"ffdh8192", R_FFDH_8192},
};
...
static const OPT_PAIR ecdsa_choices[ECDSA_NUM] = {
    {"ecdsap160", R_EC_P160},         {"ecdsap192", R_EC_P192},         {"ecdsap224", R_EC_P224},
    {"ecdsap256", R_EC_P256},         {"ecdsap384", R_EC_P384},         {"ecdsap521", R_EC_P521},
#ifndef OPENSSL_NO_EC2M
    {"ecdsak163", R_EC_K163},         {"ecdsak233", R_EC_K233},         {"ecdsak283", R_EC_K283},
    {"ecdsak409", R_EC_K409},         {"ecdsak571", R_EC_K571},         {"ecdsab163", R_EC_B163},
    {"ecdsab233", R_EC_B233},         {"ecdsab283", R_EC_B283},         {"ecdsab409", R_EC_B409},
    {"ecdsab571", R_EC_B571},
#endif
    {"ecdsabrp256r1", R_EC_BRP256R1}, {"ecdsabrp256t1", R_EC_BRP256T1}, {"ecdsabrp384r1", R_EC_BRP384R1},
    {"ecdsabrp384t1", R_EC_BRP384T1}, {"ecdsabrp512r1", R_EC_BRP512R1}, {"ecdsabrp512t1", R_EC_BRP512T1}};
enum {
...
static const OPT_PAIR ecdh_choices[EC_NUM] = {
    {"ecdhp160", R_EC_P160},         {"ecdhp192", R_EC_P192},         {"ecdhp224", R_EC_P224},
    {"ecdhp256", R_EC_P256},         {"ecdhp384", R_EC_P384},         {"ecdhp521", R_EC_P521},
#ifndef OPENSSL_NO_EC2M
    {"ecdhk163", R_EC_K163},         {"ecdhk233", R_EC_K233},         {"ecdhk283", R_EC_K283},
    {"ecdhk409", R_EC_K409},         {"ecdhk571", R_EC_K571},         {"ecdhb163", R_EC_B163},
    {"ecdhb233", R_EC_B233},         {"ecdhb283", R_EC_B283},         {"ecdhb409", R_EC_B409},
    {"ecdhb571", R_EC_B571},
#endif
    {"ecdhbrp256r1", R_EC_BRP256R1}, {"ecdhbrp256t1", R_EC_BRP256T1}, {"ecdhbrp384r1", R_EC_BRP384R1},
    {"ecdhbrp384t1", R_EC_BRP384T1}, {"ecdhbrp512r1", R_EC_BRP512R1}, {"ecdhbrp512t1", R_EC_BRP512T1},
#ifndef OPENSSL_NO_ECX
...
static int EVP_Digest_loop(const char *mdname, ossl_unused int algindex, void *args)
{
    ...
    if (EVP_MD_xof(md)) {
        ...
        for (count = 0; COND(c[algindex][testnum]); count++) {
            if (!EVP_DigestInit_ex2(ctx, md, NULL) || !EVP_DigestUpdate(ctx, buf, (size_t)lengths[testnum])
...
static int mac_setup(const char *name, EVP_MAC **mac, OSSL_PARAM params[], loopargs_t *loopargs,
...
static void mac_teardown(EVP_MAC **mac, loopargs_t *loopargs, unsigned int loopargs_len)
{
    unsigned int i;
...
static int EVP_MAC_loop(ossl_unused int algindex, void *args)
{
    ...
    for (count = 0; COND(c[algindex][testnum]); count++) {
        ...
        if (!EVP_MAC_init(mctx, NULL, 0, NULL) || !EVP_MAC_update(mctx, buf, lengths[testnum])
...
static int EVP_Update_loop_aead_enc(void *args)
{
    ...
    for (count = 0; COND(c[D_EVP][testnum]); count++) {
        ...
        if (mode_op != EVP_CIPH_SIV_MODE) {
            if (!EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_IVLEN, sizeof(aead_iv), NULL)) {
        ...
        if (mode_op != EVP_CIPH_GCM_MODE && mode_op != EVP_CIPH_SIV_MODE && mode_op != EVP_CIPH_GCM_SIV_MODE) {
...
static int EVP_Update_loop_aead_dec(void *args)
{
    ...
    for (count = 0; COND(c[D_EVP][testnum]); count++) {
        ...
        if (mode_op != EVP_CIPH_SIV_MODE && mode_op != EVP_CIPH_GCM_MODE && mode_op != EVP_CIPH_GCM_SIV_MODE) {
            if (!EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG, TAG_LEN, NULL)) {
                BIO_printf(bio_err, "\nFailed to set tag length\n");
                dofail();
                exit(1);
        ...
        if (mode_op == EVP_CIPH_CCM_MODE) {
            if (!EVP_DecryptUpdate(ctx, NULL, &outl, NULL, lengths[testnum])) {
...
static int SIG_sign_loop(void *args)
{
    ...
    unsigned char *sig = app_malloc(tempargs->sig_max_sig_len[testnum], "sig sign loop");
...
static int run_benchmark(int async_jobs, int (*loop_function)(void *), loopargs_t *loopargs)
{
    ...
    for (i = 0; i < async_jobs && !error; i++) {
        ...
        ret = ASYNC_start_job(&loopargs[i].inprogress_job, loopargs[i].wait_ctx, &job_op_count, loop_function,
...
static EVP_PKEY *get_ecdsa(const EC_CURVE *curve)
{
    ...
    if (kctx == NULL) {
        ...
        if (error == ERR_peek_last_error() /* oldest and latest errors match */
            ...
            && (ERR_GET_REASON(error) == EVP_R_UNSUPPORTED_ALGORITHM || ERR_GET_REASON(error) == ERR_R_UNSUPPORTED))
...
static void collect_kem(EVP_KEM *kem, void *stack)
{
    ...
    if (is_kem_fetchable(kem) && EVP_KEM_up_ref(kem) && sk_EVP_KEM_push(kem_stack, kem) <= 0)
...
static int signatures_cmp(const EVP_SIGNATURE *const *a, const EVP_SIGNATURE *const *b)
{
...
static void collect_signatures(EVP_SIGNATURE *sig, void *stack)
{
    ...
    if (is_signature_fetchable(sig) && EVP_SIGNATURE_up_ref(sig) && sk_EVP_SIGNATURE_push(sig_stack, sig) <= 0)
...
int speed_main(int argc, char **argv)
{
...
#endif
    long op_count = 1;
    openssl_speed_sec_t seconds = {SECONDS,       RSA_SECONDS, DSA_SECONDS,  ECDSA_SECONDS, ECDH_SECONDS,
                                   EdDSA_SECONDS, SM2_SECONDS, FFDH_SECONDS, KEM_SECONDS,   SIG_SECONDS};

    static const unsigned char key32[32] = {0x12, 0x34, 0x56, 0x78, 0x9a, 0xbc, 0xde, 0xf0, 0x34, 0x56, 0x78,
    ...
    static const struct {
        ...
        unsigned int length;
        unsigned int bits;
    } rsa_keys[] = {{test512, sizeof(test512), 512},      {test1024, sizeof(test1024), 1024},
                    {test2048, sizeof(test2048), 2048},   {test3072, sizeof(test3072), 3072},
                    {test4096, sizeof(test4096), 4096},   {test7680, sizeof(test7680), 7680},
    ...
    static const FFDH_PARAMS ffdh_params[FFDH_NUM] = {{"ffdh2048", NID_ffdhe2048, 2048},
...
opterr:
        ...
        case OPT_PRIMES:
            ...
            break;
        case OPT_SECONDS:
            seconds.sym = seconds.rsa = seconds.dsa = seconds.ecdsa = seconds.ecdh = seconds.eddsa = seconds.sm2 =
                seconds.ffdh = seconds.kem = seconds.sig = opt_int_arg();
            break;
        case OPT_BYTES:
    ...
    for (idx = 0; idx < (unsigned int)sk_EVP_SIGNATURE_num(sig_stack); idx++) {
        ...
        else if (strcmp(sig_name, "ED25519") && strcmp(sig_name, "ED448") && strcmp(sig_name, "ECDSA")
                 && strcmp(sig_name, "HMAC") && strcmp(sig_name, "SIPHASH") && strcmp(sig_name, "POLY1305")
                 && strcmp(sig_name, "CMAC") && strcmp(sig_name, "SM2")) { /* skip alg */
    ...
    if (argc == 0 && !doit[D_EVP] && !doit[D_HMAC] && !doit[D_EVP_CMAC] && !do_kems && !do_sigs) {
    ...
    if (doit[D_GHASH]) {
        ...
        params[2] = OSSL_PARAM_construct_octet_string(OSSL_MAC_PARAM_KEY, (void *)key32, 16);
    ...
    if (doit[D_EVP]) {
        if (evp_cipher != NULL) {
            ...
            if (mode_op == EVP_CIPH_GCM_MODE || mode_op == EVP_CIPH_CCM_MODE || mode_op == EVP_CIPH_OCB_MODE
                || mode_op == EVP_CIPH_SIV_MODE || mode_op == EVP_CIPH_GCM_SIV_MODE) {
                ae_mode = 1;
                if (decrypt)
            ...
            for (testnum = 0; testnum < size_num; testnum++) {
                ...
                for (k = 0; k < loopargs_len; k++) {
                     ...
                     */
                    if (!EVP_CipherInit_ex(loopargs[k].ctx, evp_cipher, NULL, NULL, NULL, ae_mode ? 1 : !decrypt)) {
                    ...
                    if (!ae_mode) {
                        if (!EVP_CipherInit_ex(loopargs[k].ctx, NULL, NULL, loopargs[k].key, iv, -1)) {
                            BIO_printf(bio_err, "\nFailed to set the key\n");
                            dofail();
                            exit(1);
                        }
                    } else if (mode_op == EVP_CIPH_SIV_MODE || mode_op == EVP_CIPH_GCM_SIV_MODE) {
                        EVP_CIPHER_CTX_ctrl(loopargs[k].ctx, EVP_CTRL_SET_SPEED, 1, NULL);
                    }
                    if (ae_mode && decrypt) {
                        /* Set length of iv (Doesn't apply to SIV mode) */
                        if (mode_op != EVP_CIPH_SIV_MODE) {
                            if (!EVP_CIPHER_CTX_ctrl(loopargs[k].ctx, EVP_CTRL_AEAD_SET_IVLEN, sizeof(aead_iv), NULL)) {
                                BIO_printf(bio_err, "\nFailed to set iv length\n");
                                dofail();
                                exit(1);
                            }
                        ...
                        if (mode_op != EVP_CIPH_GCM_MODE && mode_op != EVP_CIPH_SIV_MODE
                            ...
                            if (!EVP_CIPHER_CTX_ctrl(loopargs[k].ctx, EVP_CTRL_AEAD_SET_TAG, TAG_LEN, NULL)) {
                                BIO_printf(bio_err, "\nFailed to set tag length\n");
                                dofail();
                                exit(1);
                            }
                        }
                        if (!EVP_CipherInit_ex(loopargs[k].ctx, NULL, NULL, loopargs[k].key, aead_iv, -1)) {
                            BIO_printf(bio_err, "\nFailed to set the key\n");
                            dofail();
                            exit(1);
                        }
                        /* Set total length of input. Only required for CCM */
                        if (mode_op == EVP_CIPH_CCM_MODE) {
                            if (!EVP_EncryptUpdate(loopargs[k].ctx, NULL, &outlen, NULL, lengths[testnum])) {
                                BIO_printf(bio_err, "\nCouldn't set input text length\n");
                                dofail();
                                exit(1);
                            }
                        ...
                        if (aead) {
                            if (!EVP_EncryptUpdate(loopargs[k].ctx, NULL, &outlen, aad, sizeof(aad))) {
                                BIO_printf(bio_err, "\nCouldn't insert AAD when encrypting\n");
                                dofail();
                                exit(1);
                            }
                        }
                        if (!EVP_EncryptUpdate(loopargs[k].ctx, loopargs[k].buf, &outlen, loopargs[k].buf,
                                               lengths[testnum])) {
                        ...
                        if (!EVP_EncryptFinal_ex(loopargs[k].ctx, loopargs[k].buf, &outlen)) {
                            BIO_printf(bio_err, "\nFailed finalize the encryption\n");
                            dofail();
                            exit(1);
                        }

                        if (!EVP_CIPHER_CTX_ctrl(loopargs[k].ctx, EVP_CTRL_AEAD_GET_TAG, TAG_LEN, &loopargs[k].tag)) {
                        ...
                        if (!EVP_CipherInit_ex(loopargs[k].ctx, evp_cipher, NULL, NULL, NULL, 0)) {
                        ...
                        if (mode_op == EVP_CIPH_SIV_MODE || mode_op == EVP_CIPH_GCM_SIV_MODE)
                            EVP_CIPHER_CTX_ctrl(loopargs[k].ctx, EVP_CTRL_SET_SPEED, 1, NULL);
                    }
                }
    ...
    if (doit[D_KMAC128]) {
        ...
        params[0] = OSSL_PARAM_construct_octet_string(OSSL_MAC_PARAM_KEY, (void *)key32, 16);
    ...
    if (doit[D_KMAC256]) {
        ...
        params[0] = OSSL_PARAM_construct_octet_string(OSSL_MAC_PARAM_KEY, (void *)key32, 32);
        params[1] = OSSL_PARAM_construct_end();
    ...
    for (testnum = 0; testnum < RSA_NUM; testnum++) {
        ...
        if (primes > RSA_DEFAULT_PRIME_NUM) {
            ...
            st = bn != NULL && BN_set_word(bn, RSA_F4) && init_gen_str(&genctx, "RSA", NULL, 0, NULL, NULL)
                ...
                && EVP_PKEY_CTX_set_rsa_keygen_primes(genctx, primes) > 0 && EVP_PKEY_keygen(genctx, &rsa_key) > 0;
            BN_free(bn);
            bn = NULL;
        ...
        for (i = 0; st && i < loopargs_len; i++) {
            ...
            if (loopargs[i].rsa_sign_ctx[testnum] == NULL || EVP_PKEY_sign_init(loopargs[i].rsa_sign_ctx[testnum]) <= 0
                || EVP_PKEY_sign(loopargs[i].rsa_sign_ctx[testnum], loopargs[i].buf2, &loopargs[i].sigsize,
                                 loopargs[i].buf, 36)
                    <= 0)
                st = 0;
        }
        if (!st) {
            ...
            pkey_print_message("private", "rsa sign", rsa_keys[testnum].bits, seconds.rsa);
            /* RSA_blinding_on(rsa_key[testnum],NULL); */
            Time_F(START);
            count = run_benchmark(async_jobs, RSA_sign_loop, loopargs);
            d = Time_F(STOP);
            BIO_printf(bio_err, mr ? "+R1:%ld:%d:%.2f\n" : "%ld %u bits private RSA sign ops in %.2fs\n", count,
                       rsa_keys[testnum].bits, d);
        ...
        for (i = 0; st && i < loopargs_len; i++) {
            ...
            if (loopargs[i].rsa_verify_ctx[testnum] == NULL
                ...
                || EVP_PKEY_verify(loopargs[i].rsa_verify_ctx[testnum], loopargs[i].buf2, loopargs[i].sigsize,
                                   loopargs[i].buf, 36)
                    <= 0)
                st = 0;
        }
        if (!st) {
            ...
            pkey_print_message("public", "rsa verify", rsa_keys[testnum].bits, seconds.rsa);
            Time_F(START);
        ...
        for (i = 0; st && i < loopargs_len; i++) {
            ...
            if (loopargs[i].rsa_encrypt_ctx[testnum] == NULL
                ...
                || EVP_PKEY_encrypt(loopargs[i].rsa_encrypt_ctx[testnum], loopargs[i].buf2, &loopargs[i].encsize,
                                    loopargs[i].buf, 36)
                    <= 0)
                st = 0;
        }
        if (!st) {
            BIO_printf(bio_err, "RSA encrypt setup failure.  No RSA encrypt will be done.\n");
            dofail();
            op_count = 1;
        } else {
            pkey_print_message("public", "rsa encrypt", rsa_keys[testnum].bits, seconds.rsa);
            /* RSA_blinding_on(rsa_key[testnum],NULL); */
            Time_F(START);
        ...
        for (i = 0; st && i < loopargs_len; i++) {
            ...
            if (loopargs[i].rsa_decrypt_ctx[testnum] == NULL
                ...
                || EVP_PKEY_decrypt(loopargs[i].rsa_decrypt_ctx[testnum], loopargs[i].buf, &declen, loopargs[i].buf2,
                                    loopargs[i].encsize)
                    <= 0)
                st = 0;
        }
        if (!st) {
            BIO_printf(bio_err, "RSA decrypt setup failure.  No RSA decrypt will be done.\n");
            dofail();
            op_count = 1;
        } else {
            pkey_print_message("private", "rsa decrypt", rsa_keys[testnum].bits, seconds.rsa);
            /* RSA_blinding_on(rsa_key[testnum],NULL); */
            Time_F(START);
    ...
    for (testnum = 0; testnum < DSA_NUM; testnum++) {
        ...
        for (i = 0; st && i < loopargs_len; i++) {
            ...
            if (loopargs[i].dsa_sign_ctx[testnum] == NULL || EVP_PKEY_sign_init(loopargs[i].dsa_sign_ctx[testnum]) <= 0
                || EVP_PKEY_sign(loopargs[i].dsa_sign_ctx[testnum], loopargs[i].buf2, &loopargs[i].sigsize,
                                 loopargs[i].buf, 20)
                    <= 0)
                st = 0;
        }
        if (!st) {
            ...
            pkey_print_message("sign", "dsa", dsa_bits[testnum], seconds.dsa);
            Time_F(START);
        ...
        for (i = 0; st && i < loopargs_len; i++) {
            ...
            if (loopargs[i].dsa_verify_ctx[testnum] == NULL
                ...
                || EVP_PKEY_verify(loopargs[i].dsa_verify_ctx[testnum], loopargs[i].buf2, loopargs[i].sigsize,
                                   loopargs[i].buf, 36)
                    <= 0)
                st = 0;
        }
        if (!st) {
            ...
            pkey_print_message("verify", "dsa", dsa_bits[testnum], seconds.dsa);
            Time_F(START);
    ...
    for (testnum = 0; testnum < ECDSA_NUM; testnum++) {
        ...
        for (i = 0; st && i < loopargs_len; i++) {
            ...
            if (loopargs[i].ecdsa_sign_ctx[testnum] == NULL
                ...
                || EVP_PKEY_sign(loopargs[i].ecdsa_sign_ctx[testnum], loopargs[i].buf2, &loopargs[i].sigsize,
                                 loopargs[i].buf, 20)
                    <= 0)
                st = 0;
        }
        if (!st) {
            ...
            pkey_print_message("sign", "ecdsa", ec_curves[testnum].bits, seconds.ecdsa);
            Time_F(START);
            count = run_benchmark(async_jobs, ECDSA_sign_loop, loopargs);
            d = Time_F(STOP);
            BIO_printf(bio_err, mr ? "+R7:%ld:%u:%.2f\n" : "%ld %u bits ECDSA sign ops in %.2fs\n", count,
        ...
        for (i = 0; st && i < loopargs_len; i++) {
            ...
            if (loopargs[i].ecdsa_verify_ctx[testnum] == NULL
                ...
                || EVP_PKEY_verify(loopargs[i].ecdsa_verify_ctx[testnum], loopargs[i].buf2, loopargs[i].sigsize,
                                   loopargs[i].buf, 20)
                    <= 0)
                st = 0;
        }
        if (!st) {
            ...
            pkey_print_message("verify", "ecdsa", ec_curves[testnum].bits, seconds.ecdsa);
            Time_F(START);
            count = run_benchmark(async_jobs, ECDSA_verify_loop, loopargs);
            d = Time_F(STOP);
            BIO_printf(bio_err, mr ? "+R8:%ld:%u:%.2f\n" : "%ld %u bits ECDSA verify ops in %.2fs\n", count,
                       ec_curves[testnum].bits, d);
    ...
    for (testnum = 0; testnum < EC_NUM; testnum++) {
        ...
        if (ecdh_checks != 0) {
            pkey_print_message("", "ecdh", ec_curves[testnum].bits, seconds.ecdh);
            Time_F(START);
    ...
    for (testnum = 0; testnum < EdDSA_NUM; testnum++) {
        ...
        for (i = 0; i < loopargs_len; i++) {
            ...
            if ((ed_pctx = EVP_PKEY_CTX_new_id(ed_curves[testnum].nid, NULL)) == NULL
                || EVP_PKEY_keygen_init(ed_pctx) <= 0 || EVP_PKEY_keygen(ed_pctx, &ed_pkey) <= 0) {
                st = 0;
        ...
        if (st == 0) {
            ...
            for (i = 0; i < loopargs_len; i++) {
                ...
                st = EVP_DigestSign(loopargs[i].eddsa_ctx[testnum], loopargs[i].buf2, &loopargs[i].sigsize,
                                    loopargs[i].buf, 20);
                if (st == 0)
                    break;
            }
            if (st == 0) {
                ...
                pkey_print_message("sign", ed_curves[testnum].name, ed_curves[testnum].bits, seconds.eddsa);
                Time_F(START);
                count = run_benchmark(async_jobs, EdDSA_sign_loop, loopargs);
                d = Time_F(STOP);

                BIO_printf(bio_err, mr ? "+R10:%ld:%u:%s:%.2f\n" : "%ld %u bits %s sign ops in %.2fs \n", count,
                           ed_curves[testnum].bits, ed_curves[testnum].name, d);
                eddsa_results[testnum][0] = (double)count / d;
                op_count = count;
            }
            /* Perform EdDSA verification test */
            for (i = 0; i < loopargs_len; i++) {
                st = EVP_DigestVerify(loopargs[i].eddsa_ctx2[testnum], loopargs[i].buf2, loopargs[i].sigsize,
                                      loopargs[i].buf, 20);
                if (st != 1)
                    break;
            }
            if (st != 1) {
                ...
                pkey_print_message("verify", ed_curves[testnum].name, ed_curves[testnum].bits, seconds.eddsa);
                Time_F(START);
    ...
    for (testnum = 0; testnum < SM2_NUM; testnum++) {
        ...
        for (i = 0; i < loopargs_len; i++) {
            ...
            st = !((pctx = EVP_PKEY_CTX_new_id(EVP_PKEY_SM2, NULL)) == NULL || EVP_PKEY_keygen_init(pctx) <= 0
                   || EVP_PKEY_CTX_set_ec_paramgen_curve_nid(pctx, sm2_curves[testnum].nid) <= 0
        ...
        if (st == 0) {
            ...
            for (i = 0; i < loopargs_len; i++) {
                ...
                st = EVP_DigestSign(loopargs[i].sm2_ctx[testnum], loopargs[i].buf2, &loopargs[i].sigsize,
                                    loopargs[i].buf, 20);
                if (st == 0)
                    break;
            }
            if (st == 0) {
                ...
                pkey_print_message("sign", sm2_curves[testnum].name, sm2_curves[testnum].bits, seconds.sm2);
                Time_F(START);
                count = run_benchmark(async_jobs, SM2_sign_loop, loopargs);
                d = Time_F(STOP);

                BIO_printf(bio_err, mr ? "+R12:%ld:%u:%s:%.2f\n" : "%ld %u bits %s sign ops in %.2fs \n", count,
                           sm2_curves[testnum].bits, sm2_curves[testnum].name, d);
            ...
            for (i = 0; i < loopargs_len; i++) {
                st = EVP_DigestVerify(loopargs[i].sm2_vfy_ctx[testnum], loopargs[i].buf2, loopargs[i].sigsize,
                                      loopargs[i].buf, 20);
                if (st != 1)
                    break;
            }
            if (st != 1) {
                ...
                pkey_print_message("verify", sm2_curves[testnum].name, sm2_curves[testnum].bits, seconds.sm2);
                Time_F(START);
                count = run_benchmark(async_jobs, SM2_verify_loop, loopargs);
                d = Time_F(STOP);
                BIO_printf(bio_err, mr ? "+R13:%ld:%u:%s:%.2f\n" : "%ld %u bits %s verify ops in %.2fs\n", count,
                           sm2_curves[testnum].bits, sm2_curves[testnum].name, d);
    ...
    for (testnum = 0; testnum < FFDH_NUM; testnum++) {
        ...
        for (i = 0; i < loopargs_len; i++) {
            ...
            if (EVP_PKEY_derive_init(test_ctx) <= 0 || EVP_PKEY_derive_set_peer(test_ctx, pkey_A) <= 0
        ...
        if (ffdh_checks != 0) {
            pkey_print_message("", "ffdh", ffdh_params[testnum].bits, seconds.ffdh);
            Time_F(START);
    ...
    for (testnum = 0; testnum < kems_algs_len; testnum++) {
        ...
        for (i = 0; i < loopargs_len; i++) {
            ...
            if ((!kem_gen_ctx || EVP_PKEY_keygen_init(kem_gen_ctx) <= 0)
                || (use_params && EVP_PKEY_CTX_set_params(kem_gen_ctx, params) <= 0)) {
            ...
            kem_encaps_ctx = EVP_PKEY_CTX_new_from_pkey(app_get0_libctx(), pkey, app_get0_propq());
            if (kem_encaps_ctx == NULL || EVP_PKEY_encapsulate_init(kem_encaps_ctx, NULL) <= 0
                || (kem_type == KEM_RSA && EVP_PKEY_CTX_set_kem_op(kem_encaps_ctx, "RSASVE") <= 0)
                || ((kem_type == KEM_EC || kem_type == KEM_X25519 || kem_type == KEM_X448)
                    && EVP_PKEY_CTX_set_kem_op(kem_encaps_ctx, "DHKEM") <= 0)
                || EVP_PKEY_encapsulate(kem_encaps_ctx, NULL, &out_len, NULL, &send_secret_len) <= 0) {
                BIO_printf(bio_err, "Error while initializing encaps data structs for %s.\n", kem_name);
                goto kem_err_break;
            }
            ...
            if (EVP_PKEY_encapsulate(kem_encaps_ctx, out, &out_len, send_secret, &send_secret_len) <= 0) {
                BIO_printf(bio_err, "Encaps error for %s.\n", kem_name);
                goto kem_err_break;
            }
            /* Now prepare decaps data structs */
            kem_decaps_ctx = EVP_PKEY_CTX_new_from_pkey(app_get0_libctx(), pkey, app_get0_propq());
            if (kem_decaps_ctx == NULL || EVP_PKEY_decapsulate_init(kem_decaps_ctx, NULL) <= 0
                || (kem_type == KEM_RSA && EVP_PKEY_CTX_set_kem_op(kem_decaps_ctx, "RSASVE") <= 0)
                || ((kem_type == KEM_EC || kem_type == KEM_X25519 || kem_type == KEM_X448)
                    && EVP_PKEY_CTX_set_kem_op(kem_decaps_ctx, "DHKEM") <= 0)
                || EVP_PKEY_decapsulate(kem_decaps_ctx, NULL, &rcv_secret_len, out, out_len) <= 0) {
                BIO_printf(bio_err, "Error while initializing decaps data structs for %s.\n", kem_name);
                goto kem_err_break;
            }
            ...
            if (EVP_PKEY_decapsulate(kem_decaps_ctx, rcv_secret, &rcv_secret_len, out, out_len) <= 0
...
kem_err_break:
        ...
        if (kem_checks != 0) {
            ...
            d = Time_F(STOP);
            BIO_printf(bio_err, mr ? "+R17:%ld:%s:%.2f\n" : "%ld %s KEM decaps ops in %.2fs\n", count, kem_name, d);
    ...
    for (testnum = 0; testnum < sigs_algs_len; testnum++) {
        ...
        for (i = 0; i < loopargs_len; i++) {
            ...
            if (strncmp(sig_name, "dsa", 3) == 0) {
                ...
                if (ctx_params == NULL || EVP_PKEY_paramgen_init(ctx_params) <= 0
                    || EVP_PKEY_CTX_set_dsa_paramgen_bits(ctx_params, atoi(sig_name + 3)) <= 0
                    ...
                    BIO_printf(bio_err, "Error initializing classic keygen ctx for %s.\n", sig_name);
                    goto sig_err_break;
                }
            }
            ...
            if (sig_gen_ctx == NULL)
                sig_gen_ctx =
                    EVP_PKEY_CTX_new_from_name(app_get0_libctx(), use_params == 1 ? "RSA" : sig_name, app_get0_propq());
            ...
            if (EVP_PKEY_keygen(sig_gen_ctx, &pkey) <= 0) {
                BIO_printf(bio_err, "Error while generating signature EVP_PKEY for %s.\n", sig_name);
                goto sig_err_break;
            }
            /* Now prepare signature data structs */
            sig_sign_ctx = EVP_PKEY_CTX_new_from_pkey(app_get0_libctx(), pkey, app_get0_propq());
            if (sig_sign_ctx == NULL || EVP_PKEY_sign_init(sig_sign_ctx) <= 0
                || (use_params == 1 && (EVP_PKEY_CTX_set_rsa_padding(sig_sign_ctx, RSA_PKCS1_PADDING) <= 0))
                || EVP_PKEY_sign(sig_sign_ctx, NULL, &max_sig_len, md, md_len) <= 0) {
                BIO_printf(bio_err, "Error while initializing signing data structs for %s.\n", sig_name);
                goto sig_err_break;
            }
            ...
            memset(md, 0, SHA256_DIGEST_LENGTH);
            sig_verify_ctx = EVP_PKEY_CTX_new_from_pkey(app_get0_libctx(), pkey, app_get0_propq());
            if (sig_verify_ctx == NULL || EVP_PKEY_verify_init(sig_verify_ctx) <= 0
                || (use_params == 1 && (EVP_PKEY_CTX_set_rsa_padding(sig_verify_ctx, RSA_PKCS1_PADDING) <= 0))) {
                BIO_printf(bio_err, "Error while initializing verify data structs for %s.\n", sig_name);
                goto sig_err_break;
            }
...
sig_err_break:
        ...
        if (sig_checks != 0) {
            ...
            d = Time_F(STOP);
            BIO_printf(bio_err, mr ? "+R19:%ld:%s:%.2f\n" : "%ld %s signature sign ops in %.2fs\n", count, sig_name, d);
            ...
            d = Time_F(STOP);
            BIO_printf(bio_err, mr ? "+R20:%ld:%s:%.2f\n" : "%ld %s signature verify ops in %.2fs\n", count, sig_name,
                       d);
...
show_res:
    ...
    for (k = 0; k < RSA_NUM; k++) {
        ...
        if (mr)
            ...
            printf("rsa %5u bits %8.6fs %8.6fs %8.6fs %8.6fs %8.1f %8.1f %8.1f %8.1f\n", rsa_keys[k].bits,
                   1.0 / rsa_results[k][0], 1.0 / rsa_results[k][1], 1.0 / rsa_results[k][2], 1.0 / rsa_results[k][3],
                   rsa_results[k][0], rsa_results[k][1], rsa_results[k][2], rsa_results[k][3]);
    }
    testnum = 1;
#ifndef OPENSSL_NO_DSA
    for (k = 0; k < DSA_NUM; k++) {
        ...
        if (mr)
            ...
            printf("dsa %4u bits %8.6fs %8.6fs %8.1f %8.1f\n", dsa_bits[k], 1.0 / dsa_results[k][0],
                   1.0 / dsa_results[k][1], dsa_results[k][0], dsa_results[k][1]);
    }
#endif /* OPENSSL_NO_DSA */
    testnum = 1;
    for (k = 0; k < OSSL_NELEM(ecdsa_doit); k++) {
        ...
        if (mr)
            ...
            printf("%4u bits ecdsa (%s) %8.4fs %8.4fs %8.1f %8.1f\n", ec_curves[k].bits, ec_curves[k].name,
                   1.0 / ecdsa_results[k][0], 1.0 / ecdsa_results[k][1], ecdsa_results[k][0], ecdsa_results[k][1]);
    }
    ...
    for (k = 0; k < EC_NUM; k++) {
        ...
        if (mr)
            ...
            printf("%4u bits ecdh (%s) %8.4fs %8.1f\n", ec_curves[k].bits, ec_curves[k].name, 1.0 / ecdh_results[k][0],
                   ecdh_results[k][0]);
    }
    ...
    for (k = 0; k < OSSL_NELEM(eddsa_doit); k++) {
        ...
        if (mr)
            printf("+F6:%u:%u:%s:%f:%f\n", k, ed_curves[k].bits, ed_curves[k].name, eddsa_results[k][0],
                   eddsa_results[k][1]);
        else
            printf("%4u bits EdDSA (%s) %8.4fs %8.4fs %8.1f %8.1f\n", ed_curves[k].bits, ed_curves[k].name,
                   1.0 / eddsa_results[k][0], 1.0 / eddsa_results[k][1], eddsa_results[k][0], eddsa_results[k][1]);
    }
#endif /* OPENSSL_NO_ECX */
    ...
    for (k = 0; k < OSSL_NELEM(sm2_doit); k++) {
        ...
        if (mr)
            printf("+F7:%u:%u:%s:%f:%f\n", k, sm2_curves[k].bits, sm2_curves[k].name, sm2_results[k][0],
                   sm2_results[k][1]);
        else
            printf("%4u bits SM2 (%s) %8.4fs %8.4fs %8.1f %8.1f\n", sm2_curves[k].bits, sm2_curves[k].name,
                   1.0 / sm2_results[k][0], 1.0 / sm2_results[k][1], sm2_results[k][0], sm2_results[k][1]);
    }
#endif
#ifndef OPENSSL_NO_DH
    testnum = 1;
    ...
    for (k = 0; k < kems_algs_len; k++) {
        ...
        if (mr)
            printf("+F9:%u:%f:%f:%f\n", k, kems_results[k][0], kems_results[k][1], kems_results[k][2]);
        else
            printf("%27s %8.6fs %8.6fs %8.6fs %9.1f %9.1f %9.1f\n", kem_name, 1.0 / kems_results[k][0],
                   1.0 / kems_results[k][1], 1.0 / kems_results[k][2], kems_results[k][0], kems_results[k][1],
                   kems_results[k][2]);
    }
    ret = 0;
    ...
    for (k = 0; k < sigs_algs_len; k++) {
        ...
        if (mr)
            printf("+F10:%u:%f:%f:%f\n", k, sigs_results[k][0], sigs_results[k][1], sigs_results[k][2]);
        else
            printf("%27s %8.6fs %8.6fs %8.6fs %9.1f %9.1f %9.1f\n", sig_name, 1.0 / sigs_results[k][0],
                   1.0 / sigs_results[k][1], 1.0 / sigs_results[k][2], sigs_results[k][0], sigs_results[k][1],
                   sigs_results[k][2]);
    }
    ret = 0;
...
static void pkey_print_message(const char *str, const char *str2, unsigned int bits, int tm)
{
...
static int strtoint(const char *str, const int min_val, const int upper_val, int *res)
{
    char *end = NULL;
    long int val = 0;
    ...
    if (errno == 0 && end != str && *end == 0 && min_val <= val && val < upper_val) {
        *res = (int)val;
        return 1;
    } else {
        return 0;
