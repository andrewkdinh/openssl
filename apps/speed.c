...
static double rsa_results[RSA_NUM]
...
static const OPT_PAIR ecdh_choices[EC_NUM] = {{"ecdhp160", R_EC_P160},
                                              {"ecdhp192", R_EC_P192},
                                              {"ecdhp224", R_EC_P224},
                                              {"ecdhp256", R_EC_P256},
                                              {"ecdhp384", R_EC_P384},
...
static int MD5_loop(void *args) { return EVP_Digest_loop("md5", D_MD5, args); }
...
static int HMAC_loop(void *args) { return EVP_MAC_loop(D_HMAC, args); }

static int CMAC_loop(void *args) { return EVP_MAC_loop(D_EVP_CMAC, args); }

static int KMAC128_loop(void *args) { return EVP_MAC_loop(D_KMAC128, args); }

static int KMAC256_loop(void *args) { return EVP_MAC_loop(D_KMAC256, args); }
...
static int EVP_Update_loop_aead_enc(void *args) {
  ...
  for (count = 0; COND(c[D_EVP][testnum]); count++) {
    ...
    if (mode_op != EVP_CIPH_SIV_MODE) {
      if (!EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_IVLEN, sizeof(aead_iv),
                               NULL)) {
...
static int RSA_decrypt_loop(void *args) {
  ...
  for (count = 0; COND(rsa_c[testnum][3]); count++) {
    rsa_num = tempargs->buflen;
    ret = EVP_PKEY_decrypt(rsa_decrypt_ctx[testnum], buf, &rsa_num, buf2,
                           tempargs->encsize);
    if (ret <= 0) {
...
static int SM2_verify_loop(void *args) {
  ...
  for (count = 0; COND(sm2_c[testnum][1]); count++) {
    if (!EVP_DigestVerifyInit(sm2ctx[testnum], NULL, EVP_sm3(), NULL,
                              sm2_pkey[testnum])) {
...
static int run_benchmark(int async_jobs, int (*loop_function)(void *),
  ...
  for (i = 0; i < async_jobs && !error; i++) {
    ...
    ret = ASYNC_start_job(&loopargs[i].inprogress_job, loopargs[i].wait_ctx,
                          &job_op_count, loop_function, (void *)&looparg_item,
                          sizeof(looparg_item));
    switch (ret) {
    case ASYNC_PAUSE:
      ++num_inprogress;
...
static EVP_PKEY *get_ecdsa(const EC_CURVE *curve) {
  ...
  if (ERR_peek_error()) {
    BIO_printf(
    bio_err, "WARNING: the error queue contains previous unhandled errors.\n");
    dofail();
  }
  ...
  if (kctx == NULL) {
    ...
    if ((pctx = EVP_PKEY_CTX_new_from_name(NULL, "EC", NULL)) == NULL ||
        ...
        EVP_PKEY_CTX_set_ec_paramgen_curve_nid(pctx, curve->nid) <= 0 ||
...
int speed_main(int argc, char **argv) {
  ...
  SECONDS,       RSA_SECONDS, DSA_SECONDS,  ECDSA_SECONDS, ECDH_SECONDS,
  ...
  {test512, sizeof(test512), 512},      {test1024, sizeof(test1024), 1024},
  ...
  OPENSSL_assert(strcmp(ecdsa_choices[ECDSA_NUM - 1].name, "ecdsabrp512t1") ==
                 0);
#endif /* OPENSSL_NO_ECX */
  ...
  while ((o = opt_next()) != OPT_EOF) {
    ...
    case OPT_PRIMES:
      ...
      break;
    case OPT_SECONDS:
      seconds.sym = seconds.rsa = seconds.dsa = seconds.ecdsa = seconds.ecdh =
      seconds.eddsa = seconds.sm2 = seconds.ffdh = seconds.kem = seconds.sig =
      opt_int_arg();
      break;
    case OPT_BYTES:
  ...
  for (idx = 0; idx < (unsigned int)sk_EVP_SIGNATURE_num(sig_stack); idx++) {
    ...
    else if (strcmp(sig_name, "ED25519") && strcmp(sig_name, "ED448") &&
             strcmp(sig_name, "ECDSA") && strcmp(sig_name, "HMAC") &&
             strcmp(sig_name, "SIPHASH") && strcmp(sig_name, "POLY1305") &&
  ...
  if (argc == 0 && !doit[D_EVP] && !doit[D_HMAC] && !doit[D_EVP_CMAC] &&
    ...
    if ((mac = EVP_MAC_fetch(app_get0_libctx(), "GMAC", app_get0_propq())) !=
        NULL) {
      EVP_MAC_free(mac);
      mac = NULL;
    } else {
    ...
    if ((mac = EVP_MAC_fetch(app_get0_libctx(), "HMAC", app_get0_propq())) !=
        NULL) {
      EVP_MAC_free(mac);
      mac = NULL;
    } else {
  ...
  if (doit[D_CBC_DES]) {
    ...
    for (i = 0; st && i < loopargs_len; i++) {
      loopargs[i].ctx =
      init_evp_cipher_ctx("des-cbc", deskey, sizeof(deskey) / 3);
  ...
  if (doit[D_EDE3_DES]) {
    ...
    for (i = 0; st && i < loopargs_len; i++) {
      loopargs[i].ctx =
      init_evp_cipher_ctx("des-ede3-cbc", deskey, sizeof(deskey));
      st = loopargs[i].ctx != NULL;
    }
    algindex = D_EDE3_DES;
    for (testnum = 0; st && testnum < size_num; testnum++) {
  ...
  if (doit[D_GHASH]) {
    ...
    OSSL_MAC_PARAM_IV, (char *)gmac_iv, sizeof(gmac_iv) - 1);
    params[2] =
    OSSL_PARAM_construct_octet_string(OSSL_MAC_PARAM_KEY, (void *)key32, 16);
  ...
  if (doit[D_EVP]) {
    if (evp_cipher != NULL) {
      ...
      if (mode_op == EVP_CIPH_GCM_MODE || mode_op == EVP_CIPH_CCM_MODE ||
          mode_op == EVP_CIPH_OCB_MODE || mode_op == EVP_CIPH_SIV_MODE ||
          mode_op == EVP_CIPH_GCM_SIV_MODE) {
        ae_mode = 1;
        if (decrypt)
      ...
      for (testnum = 0; testnum < size_num; testnum++) {
        ...
        for (k = 0; k < loopargs_len; k++) {
           ...
           */
          if (!EVP_CipherInit_ex(loopargs[k].ctx, evp_cipher, NULL, NULL, NULL,
                                 ae_mode ? 1 : !decrypt)) {
          ...
          if (!ae_mode) {
            if (!EVP_CipherInit_ex(loopargs[k].ctx, NULL, NULL, loopargs[k].key,
                                   iv, -1)) {
          ...
          if (ae_mode && decrypt) {
            ...
            if (mode_op != EVP_CIPH_GCM_MODE && mode_op != EVP_CIPH_SIV_MODE &&
                mode_op != EVP_CIPH_GCM_SIV_MODE) {
              if (!EVP_CIPHER_CTX_ctrl(loopargs[k].ctx, EVP_CTRL_AEAD_SET_TAG,
                                       TAG_LEN, NULL)) {
                BIO_printf(bio_err, "\nFailed to set tag length\n");
                dofail();
                exit(1);
            ...
            /* Set total length of input. Only required for CCM */
            if (mode_op == EVP_CIPH_CCM_MODE) {
              if (!EVP_EncryptUpdate(loopargs[k].ctx, NULL, &outlen, NULL,
                                     lengths[testnum])) {
                BIO_printf(bio_err, "\nCouldn't set input text length\n");
                dofail();
                exit(1);
            ...
            if (aead) {
              if (!EVP_EncryptUpdate(loopargs[k].ctx, NULL, &outlen, aad,
                                     sizeof(aad))) {
                BIO_printf(bio_err, "\nCouldn't insert AAD when encrypting\n");
                dofail();
                exit(1);
            ...
            if (!EVP_EncryptUpdate(loopargs[k].ctx, loopargs[k].buf, &outlen,
                                   loopargs[k].buf, lengths[testnum])) {
              BIO_printf(bio_err, "\nFailed to to encrypt the data\n");
              dofail();
              exit(1);
            }

            if (!EVP_EncryptFinal_ex(loopargs[k].ctx, loopargs[k].buf,
                                     &outlen)) {
            ...
            if (!EVP_CipherInit_ex(loopargs[k].ctx, evp_cipher, NULL, NULL,
                                   NULL, 0)) {
  ...
  if (doit[D_KMAC256]) {
    ...
    params[0] =
    OSSL_PARAM_construct_octet_string(OSSL_MAC_PARAM_KEY, (void *)key32, 32);
    params[1] = OSSL_PARAM_construct_end();
  ...
  for (testnum = 0; testnum < RSA_NUM; testnum++) {
    ...
    for (i = 0; st && i < loopargs_len; i++) {
      ...
      if (loopargs[i].rsa_sign_ctx[testnum] == NULL ||
          ...
          EVP_PKEY_sign(loopargs[i].rsa_sign_ctx[testnum], loopargs[i].buf2,
                        &loopargs[i].sigsize, loopargs[i].buf, 36) <= 0)
        st = 0;
    }
    if (!st) {
      BIO_printf(bio_err,
      ...
      pkey_print_message("private", "rsa sign", rsa_keys[testnum].bits,
                         seconds.rsa);
    ...
    for (i = 0; st && i < loopargs_len; i++) {
      ...
      if (loopargs[i].rsa_verify_ctx[testnum] == NULL ||
          ...
          EVP_PKEY_verify(loopargs[i].rsa_verify_ctx[testnum], loopargs[i].buf2,
                          loopargs[i].sigsize, loopargs[i].buf, 36) <= 0)
        st = 0;
    }
    if (!st) {
      ...
      pkey_print_message("public", "rsa verify", rsa_keys[testnum].bits,
                         seconds.rsa);
      Time_F(START);
    ...
    if (!st) {
      ...
      pkey_print_message("public", "rsa encrypt", rsa_keys[testnum].bits,
                         seconds.rsa);
      /* RSA_blinding_on(rsa_key[testnum],NULL); */
      Time_F(START);
    ...
    if (!st) {
      ...
      pkey_print_message("private", "rsa decrypt", rsa_keys[testnum].bits,
                         seconds.rsa);
      /* RSA_blinding_on(rsa_key[testnum],NULL); */
      Time_F(START);
  ...
  for (testnum = 0; testnum < DSA_NUM; testnum++) {
    ...
    for (i = 0; st && i < loopargs_len; i++) {
      ...
      if (loopargs[i].dsa_sign_ctx[testnum] == NULL ||
          ...
          EVP_PKEY_sign(loopargs[i].dsa_sign_ctx[testnum], loopargs[i].buf2,
                        &loopargs[i].sigsize, loopargs[i].buf, 20) <= 0)
        st = 0;
    }
    if (!st) {
      BIO_printf(bio_err,
    ...
    for (i = 0; st && i < loopargs_len; i++) {
      ...
      if (loopargs[i].dsa_verify_ctx[testnum] == NULL ||
          ...
          EVP_PKEY_verify(loopargs[i].dsa_verify_ctx[testnum], loopargs[i].buf2,
                          loopargs[i].sigsize, loopargs[i].buf, 36) <= 0)
        st = 0;
    }
    if (!st) {
  ...
  for (testnum = 0; testnum < ECDSA_NUM; testnum++) {
    ...
    for (i = 0; st && i < loopargs_len; i++) {
      ...
      if (loopargs[i].ecdsa_sign_ctx[testnum] == NULL ||
          ...
          EVP_PKEY_sign(loopargs[i].ecdsa_sign_ctx[testnum], loopargs[i].buf2,
                        &loopargs[i].sigsize, loopargs[i].buf, 20) <= 0)
        st = 0;
    }
    if (!st) {
      ...
      pkey_print_message("sign", "ecdsa", ec_curves[testnum].bits,
                         seconds.ecdsa);
      Time_F(START);
    ...
    if (!st) {
      BIO_printf(
      bio_err, "ECDSA verify setup failure.  No ECDSA verify will be done.\n");
      dofail();
      ecdsa_doit[testnum] = 0;
    } else {
      pkey_print_message("verify", "ecdsa", ec_curves[testnum].bits,
                         seconds.ecdsa);
      Time_F(START);
  ...
  for (testnum = 0; testnum < EdDSA_NUM; testnum++) {
    ...
    for (i = 0; i < loopargs_len; i++) {
      ...
      if (!EVP_DigestSignInit(loopargs[i].eddsa_ctx[testnum], NULL, NULL, NULL,
                              ed_pkey)) {
        st = 0;
        EVP_PKEY_free(ed_pkey);
        break;
      }
      if (!EVP_DigestVerifyInit(loopargs[i].eddsa_ctx2[testnum], NULL, NULL,
                                NULL, ed_pkey)) {
        st = 0;
        EVP_PKEY_free(ed_pkey);
        break;
    ...
    if (st == 0) {
      ...
      for (i = 0; i < loopargs_len; i++) {
        ...
        st = EVP_DigestSign(loopargs[i].eddsa_ctx[testnum], loopargs[i].buf2,
      ...
      for (i = 0; i < loopargs_len; i++) {
        st = EVP_DigestVerify(loopargs[i].eddsa_ctx2[testnum], loopargs[i].buf2,
                              loopargs[i].sigsize, loopargs[i].buf, 20);
        if (st != 1)
          break;
      }
  ...
  for (testnum = 0; testnum < SM2_NUM; testnum++) {
    ...
    for (i = 0; i < loopargs_len; i++) {
      ...
      if (!EVP_DigestSignInit(loopargs[i].sm2_ctx[testnum], NULL, EVP_sm3(),
                              NULL, sm2_pkey))
        break;
    ...
    if (st == 0) {
      ...
      for (i = 0; i < loopargs_len; i++) {
        ...
        st = EVP_DigestSign(loopargs[i].sm2_ctx[testnum], loopargs[i].buf2,
                            &loopargs[i].sigsize, loopargs[i].buf, 20);
        if (st == 0)
          break;
      }
      ...
      for (i = 0; i < loopargs_len; i++) {
        st =
        EVP_DigestVerify(loopargs[i].sm2_vfy_ctx[testnum], loopargs[i].buf2,
                         loopargs[i].sigsize, loopargs[i].buf, 20);
        if (st != 1)
          break;
      }
  ...
  for (testnum = 0; testnum < FFDH_NUM; testnum++) {
    ...
    for (i = 0; i < loopargs_len; i++) {
      ...
      if (EVP_PKEY_derive(ffdh_ctx, loopargs[i].secret_ff_a, &secret_size) <=
          0) {
      ...
      if (CRYPTO_memcmp(loopargs[i].secret_ff_a, loopargs[i].secret_ff_b,
                        secret_size)) {
  ...
  for (testnum = 0; testnum < kems_algs_len; testnum++) {
    ...
    for (i = 0; i < loopargs_len; i++) {
      ...
      if (kem_type == KEM_RSA) {
        ...
        params[0] =
        OSSL_PARAM_construct_utf8_string(OSSL_PKEY_PARAM_GROUP_NAME, name, 0);
        use_params = 1;
      }
      ...
      kem_encaps_ctx =
      EVP_PKEY_CTX_new_from_pkey(app_get0_libctx(), pkey, app_get0_propq());
      if (kem_encaps_ctx == NULL ||
          ...
          EVP_PKEY_encapsulate(kem_encaps_ctx, NULL, &out_len, NULL,
                               &send_secret_len) <= 0) {
        BIO_printf(bio_err,
      ...
      if (EVP_PKEY_encapsulate(kem_encaps_ctx, out, &out_len, send_secret,
                               &send_secret_len) <= 0) {
      ...
      if (kem_decaps_ctx == NULL ||
          ...
          EVP_PKEY_decapsulate(kem_decaps_ctx, NULL, &rcv_secret_len, out,
                               out_len) <= 0) {
        BIO_printf(bio_err,
      ...
      if (EVP_PKEY_decapsulate(kem_decaps_ctx, rcv_secret, &rcv_secret_len, out,
                               out_len) <= 0 ||
  ...
  for (testnum = 0; testnum < sigs_algs_len; testnum++) {
    ...
    for (i = 0; i < loopargs_len; i++) {
      ...
      if (sig_gen_ctx == NULL)
        ...
        app_get0_libctx(), use_params == 1 ? "RSA" : sig_name,
        app_get0_propq());
      ...
      if (sig_sign_ctx == NULL || EVP_PKEY_sign_init(sig_sign_ctx) <= 0 ||
          (use_params == 1 && (EVP_PKEY_CTX_set_rsa_padding(
      ...
      if (sig_verify_ctx == NULL || EVP_PKEY_verify_init(sig_verify_ctx) <= 0 ||
          (use_params == 1 && (EVP_PKEY_CTX_set_rsa_padding(
                               sig_verify_ctx, RSA_PKCS1_PADDING) <= 0))) {
        BIO_printf(bio_err,
...
show_res:
  ...
  for (k = 0; k < RSA_NUM; k++) {
    if (!rsa_doit[k])
      continue;
    if (testnum && !mr) {
      printf("%19ssign    verify    encrypt   decrypt   sign/s verify/s  "
             "encr./s  decr./s\n",
             " ");
      testnum = 0;
    }
    if (mr)
      printf("+F2:%u:%u:%f:%f:%f:%f\n", k, rsa_keys[k].bits, rsa_results[k][0],
             rsa_results[k][1], rsa_results[k][2], rsa_results[k][3]);
    else
      printf(
      "rsa %5u bits %8.6fs %8.6fs %8.6fs %8.6fs %8.1f %8.1f %8.1f %8.1f\n",
      rsa_keys[k].bits, 1.0 / rsa_results[k][0], 1.0 / rsa_results[k][1],
      1.0 / rsa_results[k][2], 1.0 / rsa_results[k][3], rsa_results[k][0],
      rsa_results[k][1], rsa_results[k][2], rsa_results[k][3]);
  }
  testnum = 1;
#ifndef OPENSSL_NO_DSA
  ...
  for (k = 0; k < OSSL_NELEM(ecdsa_doit); k++) {
    ...
    if (mr)
      printf("+F4:%u:%u:%f:%f\n", k, ec_curves[k].bits, ecdsa_results[k][0],
             ecdsa_results[k][1]);
    else
  ...
  for (k = 0; k < EC_NUM; k++) {
    ...
    if (mr)
      printf("+F5:%u:%u:%f:%f\n", k, ec_curves[k].bits, ecdh_results[k][0],
             1.0 / ecdh_results[k][0]);

    else
      printf("%4u bits ecdh (%s) %8.4fs %8.1f\n", ec_curves[k].bits,
             ec_curves[k].name, 1.0 / ecdh_results[k][0], ecdh_results[k][0]);
  }
  ...
  for (k = 0; k < OSSL_NELEM(sm2_doit); k++) {
    ...
    if (mr)
      ...
      printf("%4u bits SM2 (%s) %8.4fs %8.4fs %8.1f %8.1f\n",
             sm2_curves[k].bits, sm2_curves[k].name, 1.0 / sm2_results[k][0],
             1.0 / sm2_results[k][1], sm2_results[k][0], sm2_results[k][1]);
  }
#endif
#ifndef OPENSSL_NO_DH
  testnum = 1;
  for (k = 0; k < FFDH_NUM; k++) {
    ...
    if (mr)
      printf("+F8:%u:%u:%f:%f\n", k, ffdh_params[k].bits, ffdh_results[k][0],
             1.0 / ffdh_results[k][0]);
  ...
  for (k = 0; k < kems_algs_len; k++) {
    ...
    if (mr)
      ...
      printf("%27s %8.6fs %8.6fs %8.6fs %9.1f %9.1f %9.1f\n", kem_name,
             1.0 / kems_results[k][0], 1.0 / kems_results[k][1],
             1.0 / kems_results[k][2], kems_results[k][0], kems_results[k][1],
             kems_results[k][2]);
  }
  ret = 0;
  ...
  for (k = 0; k < sigs_algs_len; k++) {
    ...
    if (mr)
      ...
      printf("%27s %8.6fs %8.6fs %8.6fs %9.1f %9.1f %9.1f\n", sig_name,
             ...
             1.0 / sigs_results[k][2], sigs_results[k][0], sigs_results[k][1],
             sigs_results[k][2]);
  }
  ret = 0;
  ...
  for (j = 0; j < num; j++) {
    ...
    for (count = 0; run && COND(count); count++) {
      ...
      if (packlen > 0) {
        ...
        mb_param.inp = inp;
        mb_param.len = len;
        (void)EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_TLS1_1_MULTIBLOCK_ENCRYPT,
                                  sizeof(mb_param), &mb_param);
      } else {
        int pad;
