...
static int
mac_setup(const char *name, EVP_MAC **mac, OSSL_PARAM params[], loopargs_t *loopargs, unsigned int loopargs_len)
{
    unsigned int i;
...
#endif
    long                       op_count  = 1;
    openssl_speed_sec_t        seconds   = {SECONDS,
                                            RSA_SECONDS,
                                            DSA_SECONDS,
                                            ECDSA_SECONDS,
                                            ECDH_SECONDS,
            ...
            for (testnum = 0; testnum < size_num; testnum++) {
                ...
                for (k = 0; k < loopargs_len; k++) {
                    ...
                    if (ae_mode && decrypt) {
                        ...
                        if (!EVP_EncryptUpdate(loopargs[k].ctx,
                                               loopargs[k].buf,
                                               &outlen,
                                               loopargs[k].buf,
                                               lengths[testnum])) {
...
show_res:
    ...
    for (k = 0; k < sigs_algs_len; k++) {
        ...
        if (mr)
            ...
            printf("%27s %8.6fs %8.6fs %8.6fs %9.1f %9.1f %9.1f\n",
                   sig_name,
                   1.0 / sigs_results[k][0],
                   1.0 / sigs_results[k][1],
                   1.0 / sigs_results[k][2],
                   sigs_results[k][0],
                   sigs_results[k][1],
                   sigs_results[k][2]);
    }
