...
int dhparam_main(int argc, char **argv) {
    ...
    if (num) {
        ...
        ctx =
        EVP_PKEY_CTX_new_from_name(app_get0_libctx(), alg, app_get0_propq());
        if (ctx == NULL) {
            BIO_printf(bio_err,
        ...
        do {
             ...
             * if we're going to get DH or DHX (or DSA in the event of
             * dsaparam). We check that we got one of those key types
             * afterwards.
             */
