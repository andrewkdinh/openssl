...
0x2E, 0x2F, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x41,
...
static char *md5crypt(const char *passwd, const char *magic, const char *salt) {
  ...
  for (i = 0; i < 1000; i++) {
    if (!EVP_DigestInit_ex(md2, EVP_md5(), NULL))
      goto err;
    if (!EVP_DigestUpdate(md2, (i & 1) ? (const unsigned char *)passwd : buf,
    ...
    if (!EVP_DigestUpdate(md2, (i & 1) ? buf : (const unsigned char *)passwd,
...

  {
    ...
    *output++ = ascii_dollar[0];
    ...
    for (i = 0; i < 15; i += 3) {
      ...
      *output++ =
      cov_2char[((buf_perm[i + 1] & 0xf) << 2) | (buf_perm[i + 2] >> 6)];
...
#define ROUNDS_MAX 999999999
  ...
  n = passwd_len;
  while (n) {
    if (!EVP_DigestUpdate(md, (n & 1) ? buf : (const unsigned char *)passwd,
  ...
  for (n = 0; n < rounds; n++) {
    if (!EVP_DigestInit_ex(md2, sha, NULL))
      goto err;
    if (!EVP_DigestUpdate(md2, (n & 1) ? (const unsigned char *)p_bytes : buf,
    ...
    if (!EVP_DigestUpdate(md2, (n & 1) ? buf : (const unsigned char *)p_bytes,
                          (n & 1) ? buf_size : passwd_len))
      goto err;
    if (!EVP_DigestFinal_ex(md2, buf, NULL))
      goto err;
