...
int ssl_print_tmp_key(BIO *out, SSL *s) {
  ...
  case EVP_PKEY_EC: {
    ...
    if (!EVP_PKEY_get_utf8_string_param(key, OSSL_PKEY_PARAM_GROUP_NAME, name,
  ...
  case (BIO_CB_RECVMMSG | BIO_CB_RETURN):
    ...
    if (ret > 0) {
      for (i = 0; i < *(mmsgargs->msgs_processed); i++) {
        BIO_MSG *msg =
        (BIO_MSG *)((char *)mmsgargs->msg + (i * mmsgargs->stride));

        BIO_printf(out, "read from %p [%p] (%zu bytes => %zu (0x%zX))\n",
                   (void *)bio, (void *)msg->data, msg->data_len, msg->data_len,
                   msg->data_len);
      ...
      BIO_printf(out, "read from %p [%p] (%zu bytes => %d)\n", (void *)bio,
                 (void *)msg->data, msg->data_len, ret);
    }
    break;
...
void msg_cb(int write_p, int version, int content_type, const void *buf,
  ...
  if (version == SSL3_VERSION || version == TLS1_VERSION ||
    ...
    case SSL3_RT_INNER_CONTENT_TYPE:
      ...
      break;
    default:
      BIO_snprintf(tmpbuf, sizeof(tmpbuf) - 1, ", Unknown (content_type=%d)",
                   content_type);
...
/*  {"rsa_pkcs1_sha224",       0x0301    TLSEXT_SIGALG_rsa_pkcs1_sha224}, not in
   rfc8446 */
/*  {"ecdsa_sha224",           0x0303    TLSEXT_SIGALG_ecdsa_sha224}      not in
   rfc8446 */
...
int generate_stateless_cookie_callback(SSL *ssl, unsigned char *cookie,
  ...
  if (EVP_Q_mac(NULL, "HMAC", NULL, "SHA1", NULL, cookie_secret,
                COOKIE_SECRET_LENGTH, buffer, length, cookie,
                DTLS1_COOKIE_LENGTH, cookie_len) == NULL) {
...
int load_excert(SSL_EXCERT **pexc) {
  ...
  for (; exc; exc = exc->next) {
    ...
    if (exc->keyfile != NULL) {
      exc->key =
      load_key(exc->keyfile, exc->keyform, 0, NULL, NULL, "server key");
    } else {
      exc->key =
      load_key(exc->certfile, exc->certform, 0, NULL, NULL, "server key");
    }
    if (exc->key == NULL)
      return 0;
...
#define TLSA_TAIL_SIZE 12
    ...
    BIO_printf(bio, "DANE TLSA %d %d %d %s%s ", usage, selector, mtype,
...
static int security_callback_debug(const SSL *s, const SSL_CTX *ctx, int op,
  ...
  case SSL_SECOP_OTHER_SIGALG: {
    ...
    /* raw_sig_code: signature_scheme from tls1.3, or signature_and_hash from
     * tls1.2 */
