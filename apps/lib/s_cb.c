    ...
    case (BIO_CB_RECVMMSG | BIO_CB_RETURN):
        ...
        if (ret > 0) {
            for (i = 0; i < *(mmsgargs->msgs_processed); i++) {
                BIO_MSG *msg = (BIO_MSG *)((char *)mmsgargs->msg + (i * mmsgargs->stride));

                BIO_printf(out, "read from %p [%p] (%zu bytes => %zu (0x%zX))\n", (void *)bio, (void *)msg->data,
                           msg->data_len, msg->data_len, msg->data_len);
...
static STRINT_PAIR ssl_versions[] = {{"SSL 3.0", SSL3_VERSION},         {"TLS 1.0", TLS1_VERSION},
...
static STRINT_PAIR handshakes[] = {{", HelloRequest", SSL3_MT_HELLO_REQUEST},
...
void msg_cb(int write_p, int version, int content_type, const void *buf, size_t len, SSL *ssl, void *arg)
{
    ...
    if (version == SSL3_VERSION || version == TLS1_VERSION || version == TLS1_1_VERSION || version == TLS1_2_VERSION
        || version == TLS1_3_VERSION || version == DTLS1_VERSION || version == DTLS1_BAD_VER) {
        ...
        BIO_snprintf(tmpbuf, sizeof(tmpbuf) - 1, "Not TLS data or unknown version (version=%d, content_type=%d)",
...
static STRINT_PAIR signature_tls12_alg_list[] = {{"anonymous", TLSEXT_signature_anonymous /* 0 */},
...
static STRINT_PAIR signature_tls12_hash_list[] = {
    {"none", TLSEXT_hash_none /* 0 */},     {"MD5", TLSEXT_hash_md5 /* 1 */},
    {"SHA1", TLSEXT_hash_sha1 /* 2 */},     {"SHA224", TLSEXT_hash_sha224 /* 3 */},
    {"SHA256", TLSEXT_hash_sha256 /* 4 */}, {"SHA384", TLSEXT_hash_sha384 /* 5 */},
    {"SHA512", TLSEXT_hash_sha512 /* 6 */}, {NULL}};
...
void tlsext_cb(SSL *s, int client_server, int type, const unsigned char *data, int len, void *arg)
{
    ...
    BIO_printf(bio, "TLS %s extension \"%s\" (id=%d), len=%d\n", client_server ? "server" : "client", extname, type,
               len);
...
int generate_stateless_cookie_callback(SSL *ssl, unsigned char *cookie, size_t *cookie_len)
{
    ...
    if (EVP_Q_mac(NULL, "HMAC", NULL, "SHA1", NULL, cookie_secret, COOKIE_SECRET_LENGTH, buffer, length, cookie,
                  DTLS1_COOKIE_LENGTH, cookie_len)
        == NULL) {
...
static STRINT_PAIR chain_flags[] = {{"Overall Validity", CERT_PKEY_VALID},
...
#define TLSA_TAIL_SIZE 12
        ...
        BIO_printf(bio, "DANE TLSA %d %d %d %s%s ", usage, selector, mtype, (dlen > TLSA_TAIL_SIZE) ? "..." : "",
                   hexdata);
...
int ssl_load_stores(SSL_CTX *ctx, const char *vfyCApath, const char *vfyCAfile, const char *vfyCAstore,
                    const char *chCApath, const char *chCAfile, const char *chCAstore, STACK_OF(X509_CRL) *crls,
                    int crl_download)
{
...
static STRINT_PAIR callback_types[] = {{"Supported Ciphersuite", SSL_SECOP_CIPHER_SUPPORTED},
