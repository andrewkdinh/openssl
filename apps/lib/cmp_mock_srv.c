...
static int delayed_delivery(OSSL_CMP_SRV_CTX *srv_ctx, const OSSL_CMP_MSG *req)
{
     ...
     */
    if (req_type == OSSL_CMP_IR || req_type == OSSL_CMP_CR || req_type == OSSL_CMP_P10CR
...
static int refcert_cmp(const X509 *refcert, const X509_NAME *issuer, const ASN1_INTEGER *serial)
{
...
static int clean_transaction(OSSL_CMP_SRV_CTX *srv_ctx, ossl_unused const ASN1_OCTET_STRING *id)
{
    mock_srv_ctx *ctx = OSSL_CMP_SRV_CTX_get0_custom_ctx(srv_ctx);
...
static OSSL_CMP_PKISI *process_cert_request(OSSL_CMP_SRV_CTX *srv_ctx, const OSSL_CMP_MSG *cert_req,
                                            ossl_unused int certReqId, const OSSL_CRMF_MSG *crm, const X509_REQ *p10cr,
                                            X509 **certOut, STACK_OF(X509) **chainOut, STACK_OF(X509) **caPubs)
{
    ...
    *caPubs = NULL;
    ...
    if (bodytype == OSSL_CMP_KUR && crm != NULL /* thus not p10cr */ && ctx->refCert != NULL) {
        ...
        if (!refcert_cmp(ctx->refCert, OSSL_CRMF_CERTID_get0_issuer(cid), OSSL_CRMF_CERTID_get0_serialNumber(cid))) {
    ...
    if (central_keygen < 0)
        goto err;
    if (central_keygen == 1
        && (ctx->keyOut == NULL || (keyOut = EVP_PKEY_dup(ctx->keyOut)) == NULL
            || !OSSL_CMP_CTX_set0_newPkey(OSSL_CMP_SRV_CTX_get0_cmp_ctx(srv_ctx), 1 /* priv */, keyOut))) {
...
static OSSL_CMP_PKISI *process_rr(OSSL_CMP_SRV_CTX *srv_ctx, const OSSL_CMP_MSG *rr, const X509_NAME *issuer,
                                  const ASN1_INTEGER *serial)
{
    ...
    if (ctx->sendError == 1 || ctx->sendError == OSSL_CMP_MSG_get_bodytype(rr)) {
        ERR_raise(ERR_LIB_CMP, CMP_R_ERROR_PROCESSING_MESSAGE);
        return NULL;
    }
...
static int check_client_crl(const STACK_OF(OSSL_CMP_CRLSTATUS) *crlStatusList, const X509_CRL *crl)
{
    ...
    return thisupd == NULL || ASN1_TIME_compare(thisupd, X509_CRL_get0_lastUpdate(crl)) < 0;
}

static OSSL_CMP_ITAV *process_genm_itav(mock_srv_ctx *ctx, int req_nid, const OSSL_CMP_ITAV *req)
{
    ...
    case NID_id_it_rootCaCert: {
        ...
        if (rootcacert != NULL
            && X509_NAME_cmp(X509_get_subject_name(rootcacert), X509_get_subject_name(ctx->newWithNew)) != 0)
                /* The subjects do not match */
            rsp = OSSL_CMP_ITAV_new_rootCaKeyUpdate(NULL, NULL, NULL);
        else
            rsp = OSSL_CMP_ITAV_new_rootCaKeyUpdate(ctx->newWithNew, ctx->newWithOld, ctx->oldWithNew);
    } break;
    ...
    case NID_id_it_certReqTemplate: {
        ...
        if (!OSSL_CRMF_CERTTEMPLATE_fill(reqtemp, NULL, NULL, X509_get_issuer_name(ctx->refCert), NULL))
            goto crt_err;

        if ((keyalg = X509_ALGOR_new()) == NULL)
            goto crt_err;

        (void)X509_ALGOR_set0(keyalg, OBJ_nid2obj(NID_X9_62_id_ecPublicKey), V_ASN1_UNDEF, NULL); /* cannot fail */

        eckeyalg = OSSL_CMP_ATAV_new_algId(keyalg);
        rsakeylen = OSSL_CMP_ATAV_new_rsaKeyLen(4096);
        ok = OSSL_CMP_ATAV_push1(&keyspec, eckeyalg) && OSSL_CMP_ATAV_push1(&keyspec, rsakeylen);
...
static int process_genm(OSSL_CMP_SRV_CTX *srv_ctx, const OSSL_CMP_MSG *genm, const STACK_OF(OSSL_CMP_ITAV) *in,
...
static void process_error(OSSL_CMP_SRV_CTX *srv_ctx, const OSSL_CMP_MSG *error, const OSSL_CMP_PKISI *statusInfo,
                          const ASN1_INTEGER *errorCode, const OSSL_CMP_PKIFREETEXT *errorDetails)
{
    mock_srv_ctx *ctx = OSSL_CMP_SRV_CTX_get0_custom_ctx(srv_ctx);
    ...
    if (sk_ASN1_UTF8STRING_num(errorDetails) <= 0) {
        ...
        for (i = 0; i < sk_ASN1_UTF8STRING_num(errorDetails); i++) {
            if (i > 0)
                BIO_printf(bio_err, ", ");
            ASN1_STRING_print_ex(bio_err, sk_ASN1_UTF8STRING_value(errorDetails, i), ASN1_STRFLGS_ESC_QUOTE);
        }
        BIO_printf(bio_err, "\n");
    }
}

static int process_certConf(OSSL_CMP_SRV_CTX *srv_ctx, const OSSL_CMP_MSG *certConf, ossl_unused int certReqId,
...
static int process_pollReq(OSSL_CMP_SRV_CTX *srv_ctx, const OSSL_CMP_MSG *pollReq, ossl_unused int certReqId,
                           OSSL_CMP_MSG **req, int64_t *check_after)
{
    ...
    if (req != NULL)
        *req = NULL;
    if (ctx == NULL || pollReq == NULL || req == NULL || check_after == NULL) {
        ERR_raise(ERR_LIB_CMP, CMP_R_NULL_ARGUMENT);
        return 0;
    }
...
OSSL_CMP_SRV_CTX *ossl_cmp_mock_srv_new(OSSL_LIB_CTX *libctx, const char *propq)
{
    ...
    if (srv_ctx != NULL && ctx != NULL
        && OSSL_CMP_SRV_CTX_init(srv_ctx, ctx, process_cert_request, process_rr, process_genm, process_error,
                                 process_certConf, process_pollReq)
        && OSSL_CMP_SRV_CTX_init_trans(srv_ctx, delayed_delivery, clean_transaction))
        return srv_ctx;
