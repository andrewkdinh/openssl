...
#define DEFINE_OSSL_SET1_CERT(FIELD)                                           \
  int ossl_cmp_mock_srv_set1_##FIELD(OSSL_CMP_SRV_CTX *srv_ctx, X509 *cert) {  \
...
process_cert_request(OSSL_CMP_SRV_CTX *srv_ctx, const OSSL_CMP_MSG *cert_req,
  ...
  if (ctx == NULL || cert_req == NULL || certOut == NULL || chainOut == NULL ||
      caPubs == NULL) {
    ERR_raise(ERR_LIB_CMP, CMP_R_NULL_ARGUMENT);
    return NULL;
  }
  ...
  *caPubs = NULL;
  ...
  if (bodytype == OSSL_CMP_KUR && crm != NULL /* thus not p10cr */ &&
    ...
    if (!refcert_cmp(ctx->refCert, OSSL_CRMF_CERTID_get0_issuer(cid),
...
static OSSL_CMP_ITAV *process_genm_itav(mock_srv_ctx *ctx, int req_nid,
  ...
  case NID_id_it_rootCaCert: {
    ...
    if (rootcacert != NULL &&
      ...
      rsp = OSSL_CMP_ITAV_new_rootCaKeyUpdate(ctx->newWithNew, ctx->newWithOld,
                                              ctx->oldWithNew);
  } break;
...
static void process_error(OSSL_CMP_SRV_CTX *srv_ctx, const OSSL_CMP_MSG *error,
  ...
  if (sk_ASN1_UTF8STRING_num(errorDetails) <= 0) {
    ...
    for (i = 0; i < sk_ASN1_UTF8STRING_num(errorDetails); i++) {
      if (i > 0)
        BIO_printf(bio_err, ", ");
      ASN1_STRING_print_ex(bio_err, sk_ASN1_UTF8STRING_value(errorDetails, i),
...
OSSL_CMP_SRV_CTX *ossl_cmp_mock_srv_new(OSSL_LIB_CTX *libctx,
  ...
  if (srv_ctx != NULL && ctx != NULL &&
      OSSL_CMP_SRV_CTX_init(srv_ctx, ctx, process_cert_request, process_rr,
                            process_genm, process_error, process_certConf,
                            process_pollReq) &&
