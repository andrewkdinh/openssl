...
int TerminalSocket(int FunctionCode, int *ReturnSocket)
{
    ...
    */
    ...
    case TERM_SOCK_CREATE:
        ...
        */
        status = sys$qio(EFN$C_ENF, TerminalDeviceChan, IO$_READVBLK, &TerminalDeviceIosb, TerminalDeviceAst, 0,
                         TerminalDeviceBuff, sizeof(TerminalDeviceBuff) - 2, 0, 0, 0, 0);
        if (!(status & 1)) {
...
static int TerminalDeviceAst(int astparm)
{
    ...
    */
    status = sys$qio(EFN$C_ENF, TerminalDeviceChan, IO$_READVBLK, &TerminalDeviceIosb, TerminalDeviceAst, 0,
                     TerminalDeviceBuff, sizeof(TerminalDeviceBuff) - 1, 0, 0, 0, 0);
...
static void LogMessage(char *msg, ...)
{
    ...
    */
    BIO_snprintf(MsgBuff, sizeof(MsgBuff), "%02d-%s-%04d %02d:%02d:%02d [%08X] %s\n", LocTime->tm_mday,
                 Month[LocTime->tm_mon], (LocTime->tm_year + 1900), LocTime->tm_hour, LocTime->tm_min, LocTime->tm_sec,
                 pid, msg);
