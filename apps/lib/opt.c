...
char *opt_progname(const char *argv0)
{
    ...
    if (n > sizeof(prog) - 1)
        n = sizeof(prog) - 1;
    for (q = prog, i = 0; i < n; i++, p++) *q++ = tolower((unsigned char)*p);
    *q = '\0';
    return prog;
}
...
int opt_pair(const char *name, const OPT_PAIR *pairs, int *result)
{
    ...
    for (pp = pairs; pp->name; pp++) opt_printf_stderr("\t%s\n", pp->name);
    return 0;
}
...
int opt_string(const char *name, const char **options)
{
    ...
    for (p = options; *p != NULL; p++)
        if (strcmp(*p, name) == 0)
            return 1;
    opt_printf_stderr("%s: Value must be one of:\n", prog);
    for (p = options; *p != NULL; p++) opt_printf_stderr("\t%s\n", *p);
    return 0;
}
...
int opt_num_rest(void)
{
    ...
    for (pp = opt_rest(); *pp; pp++, i++) continue;
    return i;
}
...
#  endif
        for (i = 0; i < len_0; i++) tempname[i] = (WCHAR)name[i];
