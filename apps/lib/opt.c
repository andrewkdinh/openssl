...
char *opt_getprog(void) { return prog; }
...
char *opt_init(int ac, char **av, const OPTIONS *o) {
    ...
    for (; o->name != NULL && o->name != OPT_PARAM_STR; ++o) {
        ...
        for (next = o + 1; next->name; ++next) {
             ...
             */
            duplicated =
            next->retval != OPT_DUP && strcmp(o->name, next->name) == 0;
            if (duplicated) {
...
void opt_set_unknown_name(const char *name) { unknown_name = name; }
...
int opt_format(const char *s, unsigned long flags, int *result) {
    ...
    case 'B':
    case 'b':
        if (s[1] == '\0' || strcmp(s, "B64") == 0 || strcmp(s, "b64") == 0 ||
...
defined(INTMAX_MAX) && defined(UINTMAX_MAX) && !defined(OPENSSL_NO_INTTYPES_H)
