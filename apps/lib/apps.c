...
#ifndef APP_INIT
int app_init(long mesgwin) { return 1; }
#endif

int ctx_set_verify_locations(SSL_CTX *ctx, const char *CAfile, int noCAfile,
...
char *app_conf_try_string(const CONF *conf, const char *group,
                          const char *name) {
  char *res;
...
X509 *load_cert_pass(const char *uri, int format, int maybe_stdin,
  ...
  if (IS_HTTPS(uri)) {
    ...
    (void)load_key_certs_crls(uri, format, maybe_stdin, pass, desc, 0, NULL,
...
X509_CRL *load_crl(const char *uri, int format, int maybe_stdin,
  ...
  if (IS_HTTPS(uri)) {
    ...
    (void)load_key_certs_crls(uri, format, maybe_stdin, NULL, desc, 0, NULL,
...
EVP_PKEY *load_key(const char *uri, int format, int may_stdin, const char *pass,
  ...
  (void)load_key_certs_crls(uri, format, may_stdin, pass, desc, 0, &pkey, NULL,
...
EVP_PKEY *load_pubkey(const char *uri, int format, int maybe_stdin,
  ...
  if (format == FORMAT_ENGINE)
    uri = allocated_uri = make_engine_uri(e, uri, desc);
  (void)load_key_certs_crls(uri, format, maybe_stdin, pass, desc, 1, NULL,
                            &pkey, NULL, NULL, NULL, NULL, NULL);
  if (pkey == NULL)
    (void)load_key_certs_crls(uri, format, maybe_stdin, pass, desc, 0, &pkey,
...
EVP_PKEY *load_keyparams_suppress(const char *uri, int format, int maybe_stdin,
  ...
  (void)load_key_certs_crls(uri, format, maybe_stdin, NULL, desc,
                            suppress_decode_errors, NULL, NULL, &params, NULL,
                            NULL, NULL, NULL);
...
static void warn_cert(const char *uri, X509 *cert, int warn_EE,
  ...
  int res =
  X509_cmp_timeframe(vpm, X509_get0_notBefore(cert), X509_get0_notAfter(cert));
...
int load_cert_certs(const char *uri, X509 **pcert, STACK_OF(X509) * *pcerts,
  ...
  if (exclude_http &&
      (HAS_CASE_PREFIX(uri, "http://") || HAS_CASE_PREFIX(uri, "https://"))) {
    BIO_printf(bio_err, "error: HTTP retrieval not allowed for %s\n", desc);
    return ret;
  }
  pass_string = get_passwd(pass, desc);
  ret = load_key_certs_crls(uri, FORMAT_UNDEF, 0, pass_string, desc, 0, NULL,
                            NULL, NULL, pcert, pcerts, NULL, NULL);
...
int load_certs(const char *uri, int maybe_stdin, STACK_OF(X509) * *certs,
  ...
  if (desc == NULL)
    desc = "certificates";
  ret = load_key_certs_crls(uri, FORMAT_UNDEF, maybe_stdin, pass, desc, 0, NULL,
                            NULL, NULL, NULL, certs, NULL, NULL);
...
int load_crls(const char *uri, STACK_OF(X509_CRL) * *crls, const char *pass,
  ...
  if (desc == NULL)
    desc = "CRLs";
  ret = load_key_certs_crls(uri, FORMAT_UNDEF, 0, pass, desc, 0, NULL, NULL,
                            NULL, NULL, NULL, NULL, crls);
...
int load_key_certs_crls(const char *uri, int format, int maybe_stdin,
  ...
  if (uri == NULL) {
    ...
    if (bio != NULL) {
      ctx = OSSL_STORE_attach(bio, "file", libctx, propq, get_ui_method(),
                              &uidata, params, NULL, NULL);
      BIO_free(bio);
    }
  } else {
  ...
  while ((ppkey != NULL || ppubkey != NULL || pparams != NULL ||
    ...
    case OSSL_STORE_INFO_CERT:
      if (pcert != NULL) {
        ...
        ok = X509_add_cert(*pcerts, OSSL_STORE_INFO_get1_CERT(info),
  ...
  if (CAfile != NULL || !noCAfile) {
    ...
    if (CAfile != NULL) {
      if (X509_LOOKUP_load_file_ex(lookup, CAfile, X509_FILETYPE_PEM, libctx,
                                   propq) <= 0) {
        ERR_clear_error();
        if (X509_LOOKUP_load_file_ex(lookup, CAfile, X509_FILETYPE_ASN1, libctx,
                                     propq) <= 0) {
...
X509_NAME *parse_name(const char *cp, int chtype, int canmulti,
  ...
  while (*cp != '\0') {
    ...
    *bp++ = '\0';
    if (*cp == '\0') {
      BIO_printf(
      bio_err, "%s: Missing '=' after RDN type string '%s' in %s name string\n",
    ...
    *bp++ = '\0';
    ...
    if (nid == NID_undef) {
      ...
      if (ismulti)
        BIO_printf(bio_err,
                   "%s hint: a '+' in a value string needs be escaped using "
                   "'\\' else a new member of a multi-valued RDN is expected\n",
                   opt_getprog());
      continue;
    }
    if (*valstr == '\0') {
...
static int do_pkey_ctx_init(EVP_PKEY_CTX *pkctx,
...
static int do_sign_init(EVP_MD_CTX *ctx, EVP_PKEY *pkey, const char *md,
...
static int adapt_keyid_ext(X509 *cert, X509V3_CTX *ext_ctx, const char *name,
 ...
 */
int do_X509_verify(X509 *x, EVP_PKEY *pkey,
                   STACK_OF(OPENSSL_STRING) * vfyopts) {
  int rv = 0;
...
crls_http_cb(const X509_STORE_CTX *ctx, const X509_NAME *nm) {
  X509 *x;
...
static const char *tls_error_hint(void) {
  ...
  case SSL_R_CERTIFICATE_VERIFY_FAILED:
    return "Cannot authenticate server via its TLS certificate, likely due to "
           "mismatch with our trusted TLS certs or missing revocation status";
  case SSL_AD_REASON_OFFSET + TLS1_AD_UNKNOWN_CA:
    return "Server did not accept our TLS certificate, likely due to mismatch "
           "with server's trust anchor or missing revocation status";
  case SSL_AD_REASON_OFFSET + SSL3_AD_HANDSHAKE_FAILURE:
    return "TLS handshake failure. Possibly the server requires our TLS "
           "certificate but did not receive it";
  default:
...
BIO *app_http_tls_cb(BIO *bio, void *arg, int connect, int detail) {
  ...
  if (connect) {
    ...
    if ((info->use_proxy &&
         !OSSL_HTTP_proxy_connect(bio, info->server, info->port, NULL,
                                  NULL, /* no proxy credentials */
...
ASN1_VALUE *app_http_post_asn1(
const char *host, const char *port, const char *path, const char *proxy,
...
const char *expected_content_type, long timeout, const ASN1_ITEM *rsp_it) {
  ...
  NULL, host, port, path, use_ssl, proxy, no_proxy, NULL /* bio */,
  NULL /* rbio */, app_http_tls_cb, &info, 0 /* buf_size */, headers,
  content_type, req_mem, expected_content_type, 1 /* expect_asn1 */,
  OSSL_HTTP_DEFAULT_MAX_RESP_LEN, timeout, 0 /* keep_alive */);
  BIO_free(req_mem);
...
int app_isdir(const char *name) { return opt_isdir(name); }
...
#else
int fileno_stdin(void) { return fileno(stdin); }
#endif

int fileno_stdout(void) { return fileno(stdout); }
...
BIO *dup_bio_out(int format) {
  BIO *b =
  BIO_new_fp(stdout, BIO_NOCLOSE | (FMT_istext(format) ? BIO_FP_TEXT : 0));
  void *prefix = NULL;
...
BIO *dup_bio_err(int format) {
  BIO *b =
  BIO_new_fp(stderr, BIO_NOCLOSE | (FMT_istext(format) ? BIO_FP_TEXT : 0));
...
OSSL_PARAM *app_params_new_from_opts(STACK_OF(OPENSSL_STRING) * opts,
  ...
  for (params_n = 0; params_n < sz; params_n++) {
    ...
    *vtmp = 0;
    ...
    vtmp++;
    if (!OSSL_PARAM_allocate_from_text(&params[params_n], paramdefs, stmp, vtmp,
                                       strlen(vtmp), &found))
      goto err;
    OPENSSL_free(stmp);
  }
...
EVP_PKEY *app_keygen(EVP_PKEY_CTX *ctx, const char *alg, int bits,
                     int verbose) {
  EVP_PKEY *res = NULL;
