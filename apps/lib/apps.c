    ...
    if (!keepbio || pwdbio == NULL) {
        if (CHECK_AND_SKIP_PREFIX(arg, "file:")) {
            ...
            if (tmp == NULL || tmp - arg > PASS_SOURCE_SIZE_MAX)
                BIO_printf(bio_err,
                           "Invalid password argument, missing ':' within the "
                           "first %d chars\n",
                           PASS_SOURCE_SIZE_MAX + 1);
            else
                BIO_printf(
                bio_err, "Invalid password argument, starting with \"%.*s\"\n",
                (int)(tmp - arg + 1), arg);
            return NULL;
        }
    }
...
EVP_PKEY *load_key(const char *uri, int format, int may_stdin, const char *pass,
    ...
    (void)load_key_certs_crls(uri, format, may_stdin, pass, desc, 0, &pkey,
...
int load_key_certs_crls(const char *uri, int format, int maybe_stdin,
     ...
     * If expect == 0, more than one of them is nonzero (multiple types
     * expected).
     */
...
#define X509V3_EXT_PARSE_UNKNOWN                                               \
    (2L << 16)                             /* ASN1 parse unknown extensions    \
                                            */
...
X509_NAME *parse_name(const char *cp, int chtype, int canmulti,
    ...
    while (*cp != '\0') {
        ...
        *bp++ = '\0';
        ...
        if (nid == NID_undef) {
            BIO_printf(
            bio_err, "%s warning: Skipping unknown %s name attribute \"%s\"\n",
        ...
        if (*valstr == '\0') {
            BIO_printf(bio_err,
                       "%s warning: No value provided for %s name attribute "
                       "\"%s\", skipped\n",
                       opt_getprog(), desc, typestr);
            continue;
        }
...
static const char *tls_error_hint(void) {
    ...
    case SSL_R_CERTIFICATE_VERIFY_FAILED:
        return "Cannot authenticate server via its TLS certificate, likely due "
               "to "
               "mismatch with our trusted TLS certs or missing revocation "
               "status";
    case SSL_AD_REASON_OFFSET + TLS1_AD_UNKNOWN_CA:
        return "Server did not accept our TLS certificate, likely due to "
               "mismatch "
