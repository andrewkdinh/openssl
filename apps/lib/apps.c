 ...
 */
int load_key_certs_crls(const char          *uri,
                        int                  format,
                        int                  maybe_stdin,
                        const char          *pass,
                        const char          *desc,
...
X509_STORE *
setup_verify(const char *CAfile, int noCAfile, const char *CApath, int noCApath, const char *CAstore, int noCAstore)
{
...
X509_NAME *parse_name(const char *cp, int chtype, int canmulti, const char *desc)
{
    ...
    while (*cp != '\0') {
        ...
        *bp++ = '\0';
        ...
        if (!X509_NAME_add_entry_by_NID(n, nid, chtype, valstr, (int)strlen((char *)valstr), -1, ismulti ? -1 : 0)) {
            ERR_print_errors(bio_err);
            BIO_printf(bio_err,
...
int check_cert_attributes(BIO        *bio,
                          X509       *x,
...
ASN1_VALUE *app_http_get_asn1(const char                 *url,
                              const char                 *proxy,
                              const char                 *no_proxy,
                              SSL_CTX                    *ssl_ctx,
...
ASN1_VALUE *app_http_post_asn1(const char                 *host,
                               const char                 *port,
                               const char                 *path,
                               const char                 *proxy,
                               const char                 *no_proxy,
