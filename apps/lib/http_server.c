...
static void noteterm(int sig) { termsig = sig; }
...
void spawn_loop(const char *prog) {
  ...
  if (setpgid(0, 0)) {
    log_HTTP1(prog, LOG_CRIT, "error detaching from parent process group: %s",
              strerror(errno));
    exit(1);
  }
  ...
  if ((accept_get && CHECK_AND_SKIP_PREFIX(url, "GET ")) ||
    ...
    if (*url != '/') {
      log_HTTP2(prog, LOG_WARNING,
                "invalid %s -- URL does not begin with '/': %s", meth, url);
      (void)http_server_send_status(prog, cbio, 400, "Bad Request");
      goto out;
    }
    url++;
    ...
    if (!HAS_PREFIX(end, HTTP_VERSION_STR)) {
      log_HTTP2(prog, LOG_WARNING, "invalid %s -- bad HTTP/version string: %s",
                meth, end + 1);
      (void)http_server_send_status(prog, cbio, 400, "Bad Request");
      goto out;
    }
    *end = '\0';
    ...
    len = urldecode(url);
    if (len < 0) {
      log_HTTP2(prog, LOG_WARNING, "invalid %s request -- bad URL encoding: %s",
                meth, url);
      (void)http_server_send_status(prog, cbio, 400, "Bad Request");
      goto out;
    }
    if (strlen(meth) == 3) { /* GET */
      if ((getbio = BIO_new_mem_buf(url, len)) == NULL ||
        ...
        log_HTTP1(prog, LOG_ERR, "could not allocate base64 bio with size = %d",
                  len);
        goto fatal;
      }
...
int http_server_send_asn1_resp(const char *prog, BIO *cbio, int keep_alive,
  ...
  if (log_get_verbosity() < LOG_TRACE && (p = strchr(buf, '\r')) != NULL)
    trace_log_message(-1, prog, LOG_INFO, "sending response, 1st line: %.*s",
                      (int)(p - buf), buf);
