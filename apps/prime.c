...
/* Consistent with RSA modulus size limit and the size of plausible individual
 * primes */
#define BUFSIZE 4098
...
{"hex", OPT_HEX, '-',
 "Enables hex format for output from prime generation or input to primality "
 "checking"},
{"in", OPT_IN_FILE, '-',
    ...
    if (generate) {
        ...
        for (; *argv; argv++) {
            ...
            if (!in_file) {
                ...
                while ((bytes_read = BIO_get_line(in, file_read_buf, BUFSIZE)) >
                       0) {
                    ...
                    if (bytes_read == BUFSIZE - 1 &&
                        ...
                        argv[0], BUFSIZE - 2);
                        while (BIO_get_line(in, file_read_buf, BUFSIZE) ==
                               BUFSIZE - 1)
                            ;
                        continue;
                    }
                    ...
                    valid_digits_length =
                    strspn(file_read_buf, "1234567890abcdefABCDEF");
