...
end_of_options:
    ...
    if (req) {
        ...
        if (extfile_conf != NULL) {
            ...
            if (!X509V3_EXT_add_nconf(extfile_conf, &ctx, extensions, NULL)) {
                BIO_printf(bio_err,
                           "Error checking certificate extensions from extfile "
                           "section %s\n",
                           extensions);
                ret = 1;
                goto end;
        ...
        if (ss_cert_file != NULL) {
            total++;
            j = certify_cert(&x, ss_cert_file, certformat, passin, pkey, x509,
                             dgst, sigopts, vfyopts, attribs, db, serial, subj,
                             chtype, multirdn, email_dn, startdate, enddate,
                             days, batch, extensions, conf, verbose, certopt,
        ...
        if (infile != NULL) {
            total++;
            j = certify(&x, infile, informat, pkey, x509p, dgst, sigopts,
                        vfyopts, attribs, db, serial, subj, chtype, multirdn,
                        email_dn, startdate, enddate, days, batch, extensions,
                        conf, verbose, certopt, get_nameopt(), default_op,
        ...
        for (i = 0; i < argc; i++) {
            total++;
            j = certify(&x, argv[i], informat, pkey, x509p, dgst, sigopts,
                        vfyopts, attribs, db, serial, subj, chtype, multirdn,
                        email_dn, startdate, enddate, days, batch, extensions,
                        conf, verbose, certopt, get_nameopt(), default_op,
                        ext_copy, selfsign, dateopt);
    ...
    if (gencrl) {
        ...
        if (!crldays && !crlhours && !crlsec) {
            if (!app_conf_try_number(conf, section, ENV_DEFAULT_CRL_DAYS,
                                     &crldays))
                crldays = 0;
            if (!app_conf_try_number(conf, section, ENV_DEFAULT_CRL_HOURS,
                                     &crlhours))
                crlhours = 0;
        }
...
static char *make_revocation_str(REVINFO_TYPE rev_type, const char *rev_arg) {
    ...
    case REV_CA_COMPROMISE:
        ...
        if (!ASN1_GENERALIZEDTIME_set_string(NULL, rev_arg)) {
            BIO_printf(
            bio_err, "Invalid time format %s. Need YYYYMMDDHHMMSSZ\n", rev_arg);
            return NULL;
        }
        other = rev_arg;
