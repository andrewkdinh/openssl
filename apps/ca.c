...
X509 **xret, const char *infile, EVP_PKEY *pkey, X509 *x509, const char *dgst,
STACK_OF(OPENSSL_STRING) * sigopts, STACK_OF(CONF_VALUE) * policy, CA_DB *db,
BIGNUM *serial, const char *subj, unsigned long chtype, int multirdn,
...
{"key", OPT_KEY, 's',
 "Key to decrypt the private key or cert files if encrypted. Better use "
 "-passin"},
...
end_of_options:
  ...
  for (i = 0; i < sk_OPENSSL_PSTRING_num(db->db->data); i++) {
    ...
    if ((pp[DB_type][0] != DB_TYPE_REV) && (pp[DB_rev_date][0] != '\0')) {
      BIO_printf(
      bio_err, "entry %d: not revoked yet, but has a revocation date\n", i + 1);
      goto end;
    }
  ...
  if (req) {
    ...
    if (spkac_file != NULL) {
      total++;
      j = certify_spkac(&x, spkac_file, pkey, x509, dgst, sigopts, attribs, db,
                        serial, subj, chtype, multirdn, email_dn, startdate,
                        enddate, days, extensions, conf, verbose, certopt,
                        get_nameopt(), default_op, ext_copy, dateopt);
      if (j < 0)
        goto end;
      if (j > 0) {
        total_done++;
    ...
    if (ss_cert_file != NULL) {
      total++;
      j = certify_cert(&x, ss_cert_file, certformat, passin, pkey, x509, dgst,
                       sigopts, vfyopts, attribs, db, serial, subj, chtype,
    ...
    if (infile != NULL) {
      total++;
      j =
      certify(&x, infile, informat, pkey, x509p, dgst, sigopts, vfyopts,
              attribs, db, serial, subj, chtype, multirdn, email_dn, startdate,
    ...
    for (i = 0; i < argc; i++) {
      total++;
      j =
      certify(&x, argv[i], informat, pkey, x509p, dgst, sigopts, vfyopts,
              attribs, db, serial, subj, chtype, multirdn, email_dn, startdate,
              enddate, days, batch, extensions, conf, verbose, certopt,
              get_nameopt(), default_op, ext_copy, selfsign, dateopt);
      if (j < 0)
    ...
    for (i = 0; i < sk_X509_num(cert_sk); i++) {
      ...
      *n = '\0'; /* closing new_cert */
      ...
      Sout =
      bio_open_default(outfile, 'w', output_der ? FORMAT_ASN1 : FORMAT_TEXT);
      if (Sout == NULL)
        goto end;
  ...
  if (gencrl) {
    ...
    if ((crl_nextupdate == NULL) && (crldays == 0) && (crlhours == 0) &&
        (crlsec == 0)) {
      BIO_printf(bio_err,
  ...
  if (dorevoke) {
    if (infile == NULL) {
      ...
      revcert =
      load_cert_pass(infile, informat, 1, passin, "certificate to be revoked");
      if (revcert == NULL)
        goto end;
      if (dorevoke == 2)
        rev_type = REV_VALID;
...
static char *lookup_conf(const CONF *conf, const char *section,
                         const char *tag) {
...
static int certify_cert(X509 **xret, const char *infile, int certformat,
  ...
  ok =
  do_body(xret, pkey, x509, dgst, sigopts, policy, db, serial, subj, chtype,
          multirdn, email_dn, startdate, enddate, days, batch, verbose, rreq,
...
static int do_body(X509 **xret, EVP_PKEY *pkey, X509 *x509, const char *dgst,
  ...
  X509V3_set_ctx(&ext_ctx, selfsign ? ret : x509, ret,
                 NULL /* no need to give req, needed info is in ret */, NULL,
                 X509V3_CTX_REPLACE);
  ...
  if (selfsign) {
    ...
    if (!cert_matches_key(ret, pkey))
      BIO_printf(
      bio_err, "Warning: Signature key and public key of cert do not match\n");
  }
  ...
  if (ext_sect) {
    if (extfile_conf != NULL) {
      ...
      if (!X509V3_EXT_add_nconf(lconf, &ext_ctx, ext_sect, ret)) {
        BIO_printf(
        bio_err, "Error adding certificate extensions from config section %s\n",
        ext_sect);
        goto end;
      }
  ...
  if (!email_dn) {
    ...
    i = -1;
    while ((i = X509_NAME_get_index_by_NID(dn_subject, NID_pkcs9_emailAddress,
                                           i)) >= 0) {
