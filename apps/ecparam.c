...
int ecparam_main(int argc, char **argv) {
  ...
  if (curve_name != NULL) {
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    if (OPENSSL_strcasecmp(curve_name, "SM2") == 0)
      gctx_params =
      EVP_PKEY_CTX_new_from_name(app_get0_libctx(), "sm2", app_get0_propq());
    else
      gctx_params =
      EVP_PKEY_CTX_new_from_name(app_get0_libctx(), "ec", app_get0_propq());
    ...
    params_key =
    load_keyparams_suppress(infile, informat, 1, "EC", "EC parameters", 1);
    if (params_key == NULL)
      params_key =
      load_keyparams_suppress(infile, informat, 1, "SM2", "SM2 parameters", 1);
    ...
    if (point_format &&
        ...
        params_key, OSSL_PKEY_PARAM_EC_POINT_CONVERSION_FORMAT, point_format)) {
      BIO_printf(bio_err, "unable to set point conversion format\n");
      goto end;
    }

    if (asn1_encoding != NULL &&
        !EVP_PKEY_set_utf8_string_param(params_key, OSSL_PKEY_PARAM_EC_ENCODING,
                                        asn1_encoding)) {
  ...
  if (no_seed && !EVP_PKEY_set_octet_string_param(
                 params_key, OSSL_PKEY_PARAM_EC_SEED, NULL, 0)) {
  ...
  if (check || check_named) {
    ...
    if (check_named && !EVP_PKEY_set_utf8_string_param(
    ...
    pctx =
    EVP_PKEY_CTX_new_from_pkey(app_get0_libctx(), params_key, app_get0_propq());
