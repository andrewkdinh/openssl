...
opthelp:
    ...
    if (curve_name != NULL) {
        ...
        *p = OSSL_PARAM_construct_end();
        ...
        if (gctx_params == NULL || EVP_PKEY_keygen_init(gctx_params) <= 0
            || EVP_PKEY_CTX_set_params(gctx_params, params) <= 0 || EVP_PKEY_keygen(gctx_params, &params_key) <= 0) {
            BIO_printf(bio_err, "unable to generate key\n");
            goto end;
        }
    } else {
        params_key = load_keyparams_suppress(infile, informat, 1, "EC", "EC parameters", 1);
        if (params_key == NULL)
            params_key = load_keyparams_suppress(infile, informat, 1, "SM2", "SM2 parameters", 1);
        ...
        if (point_format
            && !EVP_PKEY_set_utf8_string_param(params_key, OSSL_PKEY_PARAM_EC_POINT_CONVERSION_FORMAT, point_format)) {
            BIO_printf(bio_err, "unable to set point conversion format\n");
            goto end;
        }

        if (asn1_encoding != NULL
            && !EVP_PKEY_set_utf8_string_param(params_key, OSSL_PKEY_PARAM_EC_ENCODING, asn1_encoding)) {
    ...
    if (check || check_named) {
        ...
        if (check_named
            && !EVP_PKEY_set_utf8_string_param(params_key, OSSL_PKEY_PARAM_EC_GROUP_CHECK_TYPE,
    ...
    if (!noout) {
        ectx_params = OSSL_ENCODER_CTX_new_for_pkey(params_key, OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS,
