...
make_receipt_request(STACK_OF(OPENSSL_STRING) *rr_to, int rr_allorfirst, STACK_OF(OPENSSL_STRING) *rr_from);
...
const OPTIONS cms_options[] = {
    ...
    {"pwri_password", OPT_PWRI_PASSWORD, 's', "Specific password for recipient"},
    ...
    {"cert...", OPT_PARAM, '.', "Recipient certs (optional; used only when encrypting)"},
    {"", OPT_CIPHER, '-', "The encryption algorithm to use (any supported cipher)"},
    {"kekcipher", OPT_KEKCIPHER, 's', "The key encryption algorithm to use"},
    ...
    {"cades", OPT_CADES, '-', "Include signingCertificate attribute (CAdES-BES)"},
    {"nodetach", OPT_NODETACH, '-', "Use opaque signing"},
    {"nocerts", OPT_NOCERTS, '-', "Don't include signer's certificate when signing"},
    {"noattr", OPT_NOATTR, '-', "Don't include any signed attributes"},
    {"nosmimecap", OPT_NOSMIMECAP, '-', "Omit the SMIMECapabilities attribute"},
    {"no_signing_time", OPT_NO_SIGNING_TIME, '-', "Omit the signing time attribute"},
    ...
    {"no_attr_verify", OPT_NO_ATTR_VERIFY, '-', "Do not verify signed attribute signatures"},
    ...
    {"verify_retcode", OPT_VERIFY_RETCODE, '-', "Exit non-zero on verification failure"},
...
opthelp:
    ...
    if (certfile != NULL && !load_certs(certfile, 0, &other, NULL, "certificate file"))
        goto end;

    if (recipfile != NULL && (operation == SMIME_DECRYPT)
        && (recip = load_cert(recipfile, FORMAT_UNDEF, "recipient certificate file")) == NULL)
        goto end;

    if (originatorfile != NULL
        && (originator = load_cert(originatorfile, FORMAT_UNDEF, "originator certificate file")) == NULL)
        goto end;

    if (operation == SMIME_SIGN_RECEIPT
        && (signer = load_cert(signerfile, FORMAT_UNDEF, "receipt signer certificate file")) == NULL)
        goto end;
    ...
    if (digesthex != NULL) {
        if (operation != SMIME_SIGN) {
            BIO_printf(bio_err, "Cannot use -digest for non-signing operation\n");
            goto end;
        }
        if (infile != NULL || (flags & CMS_DETACHED) == 0 || (flags & CMS_STREAM) != 0) {
            BIO_printf(bio_err, "Cannot use -digest when -in, -nodetach or streaming is used\n");
            goto end;
        }
        digestbin = OPENSSL_hexstr2buf(digesthex, &digestlen);
        if (digestbin == NULL) {
            BIO_printf(bio_err, "Invalid hex value after -digest\n");
            goto end;
        }
    } else {
    ...
    if (operation == SMIME_DATA_CREATE) {
        ...
        if (pwri_pass != NULL) {
            ...
            if (pwri_tmp == NULL)
                goto end;
            if (CMS_add0_recipient_password(cms, -1, NID_undef, NID_undef, pwri_tmp, -1, kekcipher) == NULL)
                goto end;
            pwri_tmp = NULL;
        }
        if (!(flags & CMS_STREAM)) {
            if (!CMS_final(cms, in, NULL, flags)) {
                if (originator != NULL
                    && ERR_GET_REASON(ERR_peek_error()) == CMS_R_ERROR_UNSUPPORTED_STATIC_KEY_AGREEMENT) {
