...
int cms_main(int argc, char **argv) {
  ...
  if ((operation & SMIME_IP) == 0 && contfile != NULL)
    BIO_printf(
    bio_err, "Warning: -contfile option is ignored for the given operation\n");
  if (operation != SMIME_ENCRYPT && *argv != NULL)
    BIO_printf(bio_err, "Warning: recipient certificate file parameters "
                        "ignored for operation other than -encrypt\n");
  ...
  if (digesthex != NULL) {
    ...
    if (infile != NULL || (flags & CMS_DETACHED) == 0 ||
        (flags & CMS_STREAM) != 0) {
      BIO_printf(
      bio_err, "Cannot use -digest when -in, -nodetach or streaming is used\n");
      goto end;
    }
  ...
  out =
  bio_open_default(outfile, 'w', binary_files ? FORMAT_BINARY : outformat);
  if (out == NULL)
    goto end;
  ...
  if (operation == SMIME_DATA_CREATE) {
    ...
    for (i = 0; i < sk_X509_num(encerts); i++) {
      ...
      res = EVP_PKEY_CTX_ctrl(pctx, -1, -1, EVP_PKEY_CTRL_CIPHER,
    ...
    if (secret_key != NULL) {
      if (!CMS_add0_recipient_key(cms, NID_undef, secret_key, secret_keylen,
                                  secret_keyid, secret_keyidlen, NULL, NULL,
                                  NULL))
        goto end;
    ...
    if (pwri_pass != NULL) {
      ...
      if (pwri_tmp == NULL)
        goto end;
      if (CMS_add0_recipient_password(cms, -1, NID_undef, NID_undef, pwri_tmp,
                                      -1, NULL) == NULL)
        goto end;
      pwri_tmp = NULL;
    }
    ...
    if (operation == SMIME_SIGN) {
      ...
      if (rr_to != NULL && ((rr = make_receipt_request(rr_to, rr_allorfirst,
                                                       rr_from)) == NULL)) {
  ...
  if (operation == SMIME_DECRYPT) {
    ...
    if (secret_key != NULL) {
      if (!CMS_decrypt_set1_key(cms, secret_key, secret_keylen, secret_keyid,
                                secret_keyidlen)) {
    ...
    if (!CMS_EncryptedData_decrypt(cms, secret_key, secret_keylen, indata, out,
                                   flags))
      goto end;
...
make_receipt_request(STACK_OF(OPENSSL_STRING) * rr_to, int rr_allorfirst,
  ...
  rr = CMS_ReceiptRequest_create0_ex(NULL, -1, rr_allorfirst, rct_from, rct_to,
                                     app_get0_libctx());
  if (rr == NULL)
    goto err;
  return rr;
