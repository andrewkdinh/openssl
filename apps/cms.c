...
int cms_main(int argc, char **argv) {
    ...
    if (digesthex != NULL) {
        ...
        in =
        bio_open_default(infile, 'r', binary_files ? FORMAT_BINARY : informat);
        if (in == NULL)
            goto end;
    }
    ...
    if (operation == SMIME_DATA_CREATE) {
        ...
        cms =
        CMS_digest_create_ex(in, sign_md, flags, libctx, app_get0_propq());
        ...
        for (i = 0; i < sk_X509_num(encerts); i++) {
            ...
            /* This flag enforces allocating the EVP_PKEY_CTX for the recipient
             * here
             */
            EVP_PKEY_CTX *pctx;
            ...
            if (CMS_RecipientInfo_type(ri) == CMS_RECIPINFO_AGREE &&
                ...
                if (EVP_EncryptInit_ex(wctx, wrap_cipher, NULL, NULL, NULL) !=
                    1)
                    goto end;
            }
        }
        ...
        if (!(flags & CMS_STREAM)) {
            if (!CMS_final(cms, in, NULL, flags)) {
                if (originator != NULL &&
                    ...
                    BIO_printf(bio_err,
                               "Cannot use originator for encryption\n");
                    goto end;
                }
                goto end;
            }
        ...
        cms = CMS_EncryptedData_encrypt_ex(
        in, cipher, secret_key, secret_keylen, flags, libctx, app_get0_propq());
        ...
        if (operation == SMIME_SIGN) {
            ...
            flags |= CMS_PARTIAL;
            cms =
            CMS_sign_ex(NULL, NULL, other, in, flags, libctx, app_get0_propq());
            if (cms == NULL)
                goto end;
            if (econtent_type != NULL)
            ...
            if (rr_to != NULL && ((rr = make_receipt_request(
                                   rr_to, rr_allorfirst, rr_from)) == NULL)) {
