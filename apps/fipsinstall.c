...
static int load_fips_prov_and_run_self_test(const char *prov_name,
  ...
  if (!quiet) {
    *p++ =
    OSSL_PARAM_construct_utf8_ptr(OSSL_PROV_PARAM_NAME, &name, sizeof(name));
    *p++ =
    OSSL_PARAM_construct_utf8_ptr(OSSL_PROV_PARAM_VERSION, &vers, sizeof(vers));
...
int fipsinstall_main(int argc, char **argv) {
  ...
  mem_bio =
  BIO_new_mem_buf((const void *)INSTALL_STATUS_VAL, strlen(INSTALL_STATUS_VAL));
  if (mem_bio == NULL) {
