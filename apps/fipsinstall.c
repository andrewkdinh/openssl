...
static int load_fips_prov_and_run_self_test(const char *prov_name,
    ...
    if (!quiet) {
        *p++ = OSSL_PARAM_construct_utf8_ptr(OSSL_PROV_PARAM_NAME, &name,
                                             sizeof(name));
        *p++ = OSSL_PARAM_construct_utf8_ptr(OSSL_PROV_PARAM_VERSION, &vers,
                                             sizeof(vers));
        *p++ = OSSL_PARAM_construct_utf8_ptr(OSSL_PROV_PARAM_BUILDINFO, &build,
                                             sizeof(build));
 ...
 */
static int write_config_fips_section(
BIO *out, const char *section, unsigned char *module_mac, size_t module_mac_len,
...
static CONF *generate_config_and_load(const char *prov_name,
    ...
    if (!write_config_header(mem_bio, prov_name, section) ||
        !write_config_fips_section(mem_bio, section, module_mac, module_mac_len,
                                   opts, NULL, 0))
        goto end;
...
static int verify_config(const char *infile, const char *section,
    ...
    if (buf1 == NULL || (size_t)len != module_mac_len ||
    ...
    if (install_mac != NULL && install_mac_len > 0) {
        s =
        NCONF_get_string(conf, section, OSSL_PROV_FIPS_PARAM_INSTALL_STATUS);
        ...
        if (buf2 == NULL || (size_t)len != install_mac_len ||
...
int fipsinstall_main(int argc, char **argv) {
    ...
    while ((o = opt_next()) != OPT_EOF) {
        ...
        case OPT_NO_PBKDF2_LOWER_BOUND_CHECK:
            if (!check_non_pedantic_fips(pedantic,
    ...
    if (self_test_log || self_test_corrupt_desc != NULL ||
    ...
    if (verify) {
        ...
        if (!write_config_fips_section(fout, section_name, module_mac,
                                       module_mac_len, &fips_opts, install_mac,
                                       install_mac_len))
            goto end;
        if (!quiet)
