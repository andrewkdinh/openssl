...
{OPT_MORE_STR, 0, 0,
 "of form <OID>:int:<n> or <OID>:str:<s>, e.g. \'1.2.3.4:int:56789, "
 "id-kp:str:name'"},
{"template", OPT_TEMPLATE, 's',
 "File to save certTemplate received in genp of type certReqTemplate"},
{"keyspec", OPT_KEYSPEC, 's',
 "Optional file to save Key specification received in genp of type "
 "certReqTemplate"},

OPT_SECTION("Certificate enrollment"),
{"newkey", OPT_NEWKEY, 's',
 "Private or public key for the requested cert. Default: CSR key or client "
 "key"},
...
#if defined(OPENSSL_NO_SOCK) || defined(OPENSSL_NO_HTTP)
{OPT_MORE_STR, 0, 0,
 "NOTE: -server, -proxy, and -no_proxy not supported due to no-sock/no-http "
 "build"},
#else
{"server", OPT_SERVER, 's',
...
{"cert", OPT_CERT, 's',
 "Client's CMP signer certificate; its public key must match the -key "
 "argument"},
{OPT_MORE_STR, 0, 0,
...
#if defined(OPENSSL_NO_SOCK) || defined(OPENSSL_NO_HTTP)
{OPT_MORE_STR, 0, 0,
 "NOTE: -tls_used and all other TLS options not supported due to "
 "no-sock/no-http build"},
#else
...
#define FUNC                                                                   \
    (strcmp(OPENSSL_FUNC, "(unknown function)") == 0 ? "CMP" : OPENSSL_FUNC)
#define CMP_print(bio, level, prefix, msg, a1, a2, a3)                         \
    ((void)(level > opt_verbosity                                              \
            ? 0                                                                \
            : (BIO_printf(bio, "%s:%s:%d:CMP %s: " msg "\n", FUNC,             \
                          OPENSSL_FILE, OPENSSL_LINE, prefix, a1, a2, a3))))
...
static EVP_PKEY *load_key_pwd(const char *uri, int format, const char *pass,
                              ENGINE *eng, const char *desc) {
...
static X509 *load_cert_pwd(const char *uri, const char *pass,
                           const char *desc) {
    X509 *cert;
    char *pass_string = get_passwd(pass, desc);
...
static OSSL_CMP_MSG *read_write_req_resp(OSSL_CMP_CTX *ctx,
    ...
    if (opt_rspin != NULL) {
        ...
        if (opt_use_mock_srv) {
            if (rspin_in_use)
                CMP_warn("too few -rspin filename arguments; resorting to "
                         "using mock server");
            ...
            if (opt_server == NULL) {
                CMP_err(
                "missing -server or -use_mock_srv option, or too few -rspin "
                "filename arguments");
                goto err;
            }
            if (rspin_in_use)
...
#else
            CMP_err(
            "-server not supported on no-sock/no-http build; missing "
            "-use_mock_srv option or too few -rspin filename arguments");
#endif
        }
        rspin_in_use = 0;
    }
...
static int set_gennames(OSSL_CMP_CTX *ctx, char *names, const char *desc) {
    ...
    for (; names != NULL; names = next) {
        ...
        if (strcmp(names, "critical") == 0) {
            (void)OSSL_CMP_CTX_set_option(
            ctx, OSSL_CMP_OPT_SUBJECTALTNAME_CRITICAL, 1);
            continue;
        }
...
static X509_STORE *load_trusted(char *input, int for_new_cert,
                                const char *desc) {
...
static OSSL_CMP_SRV_CTX *setup_srv_ctx(ENGINE *engine) {
    ...
    ossl_cmp_mock_srv_new(app_get0_libctx(), app_get0_propq());
    ...
    if (opt_srv_secret != NULL) {
        int res;
        char *pass_str =
        get_passwd(opt_srv_secret, "PBMAC secret of mock server");
        ...
        CMP_err(
        "server credentials (-srv_secret or -srv_cert) must be given if "
        "-use_mock_srv or -port is used");
        goto err;
    } else {
        CMP_warn(
        "server will not be able to handle PBM-protected requests since "
        "-srv_secret is not given");
    }
    ...
    if (opt_srv_key != NULL) {
        EVP_PKEY *pkey =
        load_key_pwd(opt_srv_key, opt_keyform, opt_srv_keypass, engine,
    ...
    if (opt_srv_trusted != NULL) {
        ...
        CMP_warn("mock server will not be able to handle signature-protected "
                 "requests since -srv_trusted is not given");
    }
    ...
    if (opt_rsp_key != NULL) {
        EVP_PKEY *pkey =
        load_key_pwd(opt_rsp_key, opt_keyform, opt_rsp_keypass, engine,
...
static int setup_verification_ctx(OSSL_CMP_CTX *ctx) {
    ...
    if (opt_srvcert != NULL || opt_trusted != NULL) {
        if (opt_srvcert != NULL) {
            ...
            if (opt_recipient != NULL) {
                CMP_warn("-recipient option is ignored since -srvcert option "
                         "is present");
                opt_recipient = NULL;
            }
    ...
    if (opt_out_trusted != NULL) { /* for use in OSSL_CMP_certConf_cb() */
        ...
        X509_STORE *out_trusted = load_trusted(
        opt_out_trusted, 1, "trusted certs for verifying newly enrolled cert");
...
static SSL_CTX *setup_ssl_ctx(OSSL_CMP_CTX *ctx, const char *host,
    ...
    if (opt_tls_trusted != NULL) {
        ...
        CMP_warn(
        "-tls_used given without -tls_trusted; will not authenticate the "
        "TLS server");
    }
    ...
    if (opt_tls_cert != NULL && opt_tls_key != NULL) {
        ...
        for (i = 0; i < sk_X509_num(untrusted); i++) {
            ...
            if (!SSL_CTX_add1_chain_cert(ssl_ctx, cert)) {
                CMP_err(
                "could not add untrusted cert to TLS client cert chain");
                goto err;
            }
        }
        ...
        if (opt_tls_extra != NULL) {
            STACK_OF(X509) *tls_extra = load_certs_multifile(
        ...
        pkey = load_key_pwd(opt_tls_key, opt_keyform, opt_tls_keypass, engine,
        ...
        if (!X509_check_private_key(SSL_CTX_get0_certificate(ssl_ctx), pkey)) {
            CMP_err2(
            "TLS private key '%s' does not match the TLS certificate '%s'\n",
    ...
    if (trust_store != NULL) {
        ...
        if (!truststore_set_host_etc(
            trust_store, opt_tls_host != NULL ? opt_tls_host : host))
            goto err;
    }
    return ssl_ctx;
err:
...
static int setup_protection_ctx(OSSL_CMP_CTX *ctx, ENGINE *engine) {
    if (!opt_unprotected_requests && opt_secret == NULL && opt_key == NULL) {
        CMP_err(
        "must give -key or -secret unless -unprotected_requests is used");
        return 0;
    }
    ...
    if (opt_secret != NULL) {
        ...
        if (pass_string != NULL) {
            ...
            ctx, (unsigned char *)pass_string, strlen(pass_string));
            clear_free(pass_string);
            if (res == 0)
                return 0;
        }
        if (opt_cert != NULL || opt_key != NULL)
            CMP_warn(
            "-cert and -key not used for protection since -secret is given");
    }
    if (opt_ref != NULL && !OSSL_CMP_CTX_set1_referenceValue(
                           ctx, (unsigned char *)opt_ref, strlen(opt_ref)))
        return 0;
    ...
    if (opt_secret == NULL && opt_srvcert == NULL && opt_trusted == NULL)
        CMP_warn(
        "will not authenticate server due to missing -secret, -trusted, "
        "or -srvcert");
    ...
    if (opt_cert != NULL) {
        ...
        if (!ok) {
            ...
            if (opt_own_trusted != NULL) {
                own_trusted =
                load_trusted(opt_own_trusted, 0,
...
static int set_fallback_pubkey(OSSL_CMP_CTX *ctx) {
    ...
    *end = bak;
    ...
    if (req == NULL) {
        CMP_err1("failed to load ir/cr/kur file '%s' attempting to get "
                 "fallback public key",
                 file);
        return 0;
    }
    ...
    if (pkey == NULL || !OSSL_CMP_CTX_set0_newPkey(ctx, 0 /* priv */, pkey1)) {
        EVP_PKEY_free(pkey1);
        CMP_err1(
        "failed to get fallback public key obtained from ir/cr/kur file '%s'",
        file);
        goto err;
    }
    res = 1;
...
static int setup_request_ctx(OSSL_CMP_CTX *ctx, ENGINE *engine) {
    ...
    if (opt_subject == NULL && opt_csr == NULL && opt_oldcert == NULL &&
        opt_cert == NULL && opt_cmd != CMP_RR && opt_cmd != CMP_GENM)
        CMP_warn("no -subject given; no -csr or -oldcert or -cert available "
                 "for fallback");
    ...
    if (opt_cmd == CMP_IR || opt_cmd == CMP_CR || opt_cmd == CMP_KUR) {
        if (opt_reqin == NULL && opt_newkey == NULL && !opt_centralkeygen &&
            ...
            CMP_err(
            "missing -newkey (or -key) to be certified and no -csr, -oldcert, "
            "-cert, "
            "or -reqin option given, which could provide fallback public key."
        ...
        if (opt_centralkeygen) {
            if (opt_popo > OSSL_CRMF_POPO_NONE) {
                CMP_err1("-popo value %d is inconsistent with -centralkeygen",
                         opt_popo);
                return 0;
            }
            if (opt_newkeyout == NULL) {
                CMP_err("-newkeyout not given, nowhere to save centrally "
                        "generated key");
                return 0;
            }
            opt_popo = OSSL_CRMF_POPO_NONE;
        }
        if (opt_newkey == NULL && opt_popo != OSSL_CRMF_POPO_NONE &&
            opt_popo != OSSL_CRMF_POPO_RAVERIFIED) {
            if (opt_csr != NULL) {
                CMP_err1(
                "no -newkey option given with private key for POPO, -csr "
                "option provides just public key%s",
                opt_key == NULL ? "" : ", and -key option superseded by -csr");
                if (opt_reqin != NULL)
                    CMP_info(
                    "since -reqin is used, may use -popo -1 or -popo 0 to "
                    "disable the needless generation of a POPO");
                return 0;
            }
            if (opt_key == NULL) {
                CMP_err(
                "missing -newkey (or -key) option for key to be certified and "
                "for POPO");
                return 0;
            }
        }
        if (opt_certout == NULL && opt_reqout_only == NULL) {
            CMP_err(
            "-certout not given, nowhere to save newly enrolled certificate");
            return 0;
        }
        ...
        if (opt_subject != NULL) {
            if (opt_ref == NULL && opt_cert == NULL) {
                ...
                CMP_warn1(
                "-subject %s since sender is taken from -ref or -cert", msg);
            }
        }
    ...
    if (opt_cmd == CMP_KUR) {
        ...
        if (ref_cert == NULL && opt_csr == NULL) {
            CMP_err(
            "missing -oldcert for certificate to be updated and no -csr given");
            return 0;
        }
        if (opt_subject != NULL)
            CMP_warn2(
            "given -subject '%s' overrides the subject of '%s' for KUR",
    ...
    if (opt_cmd == CMP_RR) {
        if (opt_issuer == NULL && opt_serial == NULL) {
            if (opt_oldcert == NULL && opt_csr == NULL) {
                CMP_err(
                "missing -oldcert or -issuer and -serial for certificate to be "
                "revoked and no -csr given");
                return 0;
            }
            if (opt_oldcert != NULL && opt_csr != NULL)
                CMP_warn(
                "ignoring -csr since certificate to be revoked is given");
        } else {
#define OSSL_CMP_RR_MSG "since -issuer and -serial is given for command 'rr'"
            if (opt_issuer == NULL || opt_serial == NULL) {
                CMP_err(
                "Must give both -issuer and -serial options or neither");
                return 0;
            }
            if (opt_oldcert != NULL)
    ...
    if (opt_recipient == NULL && opt_srvcert == NULL && opt_issuer == NULL &&
        ...
        CMP_warn(
        "missing -recipient, -srvcert, -issuer, -oldcert or -cert; recipient "
        "for "
        "any requests not covered by -reqin will be set to \"NULL-DN\"");

    if (opt_cmd == CMP_P10CR || opt_cmd == CMP_RR || opt_cmd == CMP_GENM) {
        const char *msg =
        "option is ignored for 'p10cr', 'rr', and 'genm' commands";
    ...
    } else if (opt_reqin != NULL && opt_key == NULL && opt_csr == NULL &&
               opt_oldcert == NULL && !opt_centralkeygen) {
    ...
    if (OSSL_CMP_CTX_reqExtensions_have_SAN(ctx) && opt_sans != NULL) {
        CMP_err("cannot have Subject Alternative Names both via -reqexts and "
                "via -sans");
        return 0;
    }
    ...
    if (opt_policy_oids_critical) {
        if (opt_policy_oids == NULL)
            CMP_warn("-opt_policy_oids_critical has no effect unless "
                     "-policy_oids is given");
        ...
        *ptr++ = '\0';
        ...
        if (CHECK_AND_SKIP_CASE_PREFIX(ptr, "int:")) {
            ...
            if (*ptr != '\0') {
                if (*ptr != ',') {
                    CMP_err1(
                    "Missing ',' or end of -geninfo arg after int at %.40s",
                    ptr);
                    goto err;
                }
                ptr++;
...
static int setup_client_ctx(OSSL_CMP_CTX *ctx, ENGINE *engine) {
...
#else
        CMP_err("missing -use_mock_srv or -rspin option; -server option is not "
                "supported due to no-sock build");
        goto err;
#endif
    }
#if !defined(OPENSSL_NO_SOCK) && !defined(OPENSSL_NO_HTTP)
    ...
    if (!OSSL_HTTP_parse_url(opt_server, &use_ssl, NULL /* user */, &host,
                             &port, &portnum, &path, NULL /* q */,
                             NULL /* frag */)) {
...
set_path:
    ...
    if (opt_cmd != CMP_GENM || opt_infotype != NID_id_it_rootCaCert) {
        const char *msg = "option is ignored unless -cmd 'genm' and -infotype "
                          "rootCaCert is given";
    ...
    if (opt_cmd != CMP_GENM || opt_infotype != NID_id_it_certReqTemplate) {
        ...
        if (opt_template == NULL)
            CMP_err(
            "missing -template option for genm with infotype certReqTemplate");
    }
    ...
    if (opt_reqin != NULL || opt_reqout != NULL || opt_rspin != NULL ||
        opt_rspout != NULL || opt_use_mock_srv)
    ...
    if (!set_name(opt_recipient, OSSL_CMP_CTX_set1_recipient, ctx,
                  "recipient") ||
 ...
 */
static int save_free_certs(STACK_OF(X509) * certs, const char *file,
                           const char *desc) {
    BIO *bio = NULL;
    int i;
    ...
    if (file == NULL)
        goto end;
    if (certs != NULL)
        CMP_info3("received %d %s certificate(s), saving to file '%s'", n, desc,
                  file);
    ...
    if ((bio = BIO_new(BIO_s_file())) == NULL ||
        ...
        CMP_err3("could not open file '%s' for %s %s certificate(s)", file,
                 certs == NULL ? "deleting" : "writing", desc);
        n = -1;
        goto end;
    }
...
static int delete_file(const char *file, const char *desc) {
    ...
    if (unlink(file) != 0 && errno != ENOENT) {
        CMP_err2(
        "Failed to delete %s, which should be done to indicate there is no %s",
        file, desc);
        return 0;
    }
    return 1;
...
static int save_crl_or_delete(X509_CRL *crl, const char *file,
                              const char *desc) {
    if (file == NULL)
        return 1;
...
static int read_config(void) {
    ...
    for (opt = &cmp_options[start_opt], i = start_idx; opt->name != NULL;
         i++, opt++)
        if (!strcmp(opt->name, OPT_SECTION_STR) ||
            !strcmp(opt->name, OPT_MORE_STR))
            n_options--;
    OPENSSL_assert(OSSL_NELEM(cmp_vars) == n_options + OPT_PROV__FIRST + 1 -
                                           OPT_PROV__LAST + OPT_R__FIRST + 1 -
                                           OPT_R__LAST + OPT_V__FIRST + 1 -
                                           OPT_V__LAST);
    for (opt = &cmp_options[start_opt], i = start_idx; opt->name != NULL;
         i++, opt++) {
        int provider_option =
        (OPT_PROV__FIRST <= opt->retval && opt->retval < OPT_PROV__LAST);
        int rand_state_option =
        (OPT_R__FIRST <= opt->retval && opt->retval < OPT_R__LAST);
        int verification_option =
        (OPT_V__FIRST <= opt->retval && opt->retval < OPT_V__LAST);
        ...
        case 'l':
            ...
            if (opt->valtype == 'N' && num < 0) {
                opt_printf_stderr(
                "Negative number \"%ld\" for config option -%s\n", num,
                opt->name);
                return -1;
            }
            break;
        ...
        if (provider_option || verification_option) {
            ...
            if (conf_argc > 1) {
                ...
                if (provider_option ? !opt_provider(opt_next())
...
static int cmp_server(OSSL_CMP_CTX *srv_cmp_ctx) {
    ...
    while (opt_max_msgs <= 0 || msgs < opt_max_msgs) {
        ...
        ret = http_server_get_asn1_req(ASN1_ITEM_rptr(OSSL_CMP_MSG),
                                       (ASN1_VALUE **)&req, &path, &cbio, acbio,
                                       &keep_alive, prog, 0, 0);
        ...
        if (req != NULL) {
            ...
            OSSL_CMP_MSG_free(req);
            if (resp == NULL) {
                (void)http_server_send_status(prog, cbio, 500,
                                              "Internal Server Error");
                break; /* treated as fatal error */
            }
            ret = http_server_send_asn1_resp(
            prog, cbio, keep_alive, "application/pkixcmp",
    ...
    for (i = 0; i < sk_OSSL_CMP_ATAV_num(keySpec); i++) {
        ...
        case NID_id_regCtrl_algId: {
            ...
            if (paramtype == V_ASN1_UNDEF || alg->parameter == NULL) {
                ...
                BIO_printf(mem, " - ");
                ASN1_item_print(mem, (ASN1_VALUE *)alg, 0,
                                ASN1_ITEM_rptr(X509_ALGOR), NULL);
            }
        } break;
...
static int do_genm(OSSL_CMP_CTX *ctx) {
    if (opt_infotype == NID_id_it_caCerts) {
        ...
        if (opt_oldwithold == NULL) {
            CMP_warn(
            "No -oldwithold given, will use all certs given with -trusted "
            "as trust anchors for verifying the newWithNew cert");
        } else {
            oldwithold =
            load_cert_pwd(opt_oldwithold, opt_otherpass,
        ...
        if (newwithnew == NULL)
            ...
            CMP_warn("oldWithNew certificate received in genp for verifying "
                     "oldWithOld, but oldWithOld was not provided");
    ...
    end_upd:
        ...
        if (opt_oldcrl == NULL && opt_crlcert == NULL) {
            CMP_err("Missing -oldcrl and no -crlcert given for -infotype "
                    "crlStatusList");
            return 0;
        }
        if (opt_crlout == NULL) {
        ...
        if (opt_crlcert != NULL) {
            crlcert =
            load_cert_pwd(opt_crlcert, opt_otherpass,
        ...
        if (opt_oldcrl != NULL && opt_crlcert != NULL) {
            if (X509_NAME_cmp(X509_CRL_get_issuer(oldcrl),
                              X509_get_issuer_name(crlcert)) != 0)
...
int cmp_main(int argc, char **argv) {
            ...
            if (strcmp(opt_section, CMP_SECTION) == 0) { /* default */
                if (!NCONF_get_section(conf, opt_section))
                    CMP_info2("no [%s] section found in config file '%s';"
                              " will thus use just [default] and unnamed "
                              "section if present",
                              opt_section, configfile);
            } else {
    ...
    if (opt_engine != NULL) {
        engine =
        setup_engine_methods(opt_engine, 0 /* not: ENGINE_METHOD_ALL */, 0);
        if (engine == NULL) {
    ...
    if (opt_tls_used && (opt_use_mock_srv || opt_server == NULL)) {
        CMP_warn("ignoring -tls_used option since -use_mock_srv is given or "
                 "-server is not given");
        opt_tls_used = 0;
    }
    ...
    if (opt_reqout_only != NULL) {
        const char *msg =
        "option is ignored since -reqout_only option is given";
    ...
    if (opt_rspin != NULL) {
        if (opt_server != NULL)
            CMP_warn(
            "-server option is not used if enough filenames given for -rspin");
        if (opt_use_mock_srv)
            CMP_warn("-use_mock_srv option is not used if enough filenames "
                     "given for -rspin");
    }
#endif
    ...
    for (i = 0; i < opt_repeat; i++) {
        ...
        if (newcert != NULL && (opt_cmd == CMP_IR || opt_cmd == CMP_CR ||
            ...
            if (newcert != NULL && newchain != NULL /* NULL is on error only */
                ...
                if (!X509_add_cert(newchain, newcert,
                                   X509_ADD_FLAG_PREPEND |
                                   X509_ADD_FLAG_UP_REF)) {
                    sk_X509_pop_free(newchain, X509_free);
                    goto err;
                }
                if (!save_free_certs(newchain, opt_chainout,
                                     "newly enrolled cert and chain"))
                    goto err;
            } else {
                if (save_free_certs(newchain, opt_chainout, "chain") < 0 ||
                    !save_cert_or_delete(newcert, opt_certout,
                                         "newly enrolled"))
                    goto err;
            }
            ...
            if (opt_centralkeygen) {
                ...
                int result = 1;
                EVP_PKEY *new_key =
                OSSL_CMP_CTX_get0_newPkey(cmp_ctx, 1 /* priv */);

                if (new_key == NULL)
                    goto err;
                if ((out = bio_open_owner(opt_newkeyout, FORMAT_PEM, 1)) ==
                    NULL)
                    goto err;
                if (opt_newkeypass != NULL) {
                    pass_string = get_passwd(
                    opt_newkeypass, "Centrally generated private key password");
                    cipher = EVP_CIPHER_fetch(app_get0_libctx(), SN_aes_256_cbc,
                                              app_get0_propq());
                }

                CMP_info1("saving centrally generated key to file '%s'",
                          opt_newkeyout);
                if (PEM_write_bio_PrivateKey(out, new_key, cipher, NULL, 0,
                                             NULL, (void *)pass_string) <= 0)
                    result = 0;
