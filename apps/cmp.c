...
const OPTIONS cmp_options[] = {
    ...
    {"infotype", OPT_INFOTYPE, 's', "InfoType name for requesting specific info in genm, with specific support"},
    {OPT_MORE_STR, 0, 0, "for 'caCerts' and 'rootCaCert'"},
    {"profile", OPT_PROFILE, 's', "Certificate profile name to place in generalInfo field of request PKIHeader"},
    {"geninfo", OPT_GENINFO, 's', "Comma-separated list of OID and value to place in generalInfo PKIHeader"},
    {OPT_MORE_STR, 0, 0, "of form <OID>:int:<n> or <OID>:str:<s>, e.g. \'1.2.3.4:int:56789, id-kp:str:name'"},
    {"template", OPT_TEMPLATE, 's', "File to save certTemplate received in genp of type certReqTemplate"},
    {"keyspec", OPT_KEYSPEC, 's', "Optional file to save Key specification received in genp of type certReqTemplate"},

    OPT_SECTION("Certificate enrollment"),
    {"newkey", OPT_NEWKEY, 's', "Private or public key for the requested cert. Default: CSR key or client key"},
    ...
    {"newkeyout", OPT_NEWKEYOUT, 's', "File to save centrally generated key, in PEM format"},
    ...
    {"issuer", OPT_ISSUER, 's', "DN of the issuer to place in the certificate template of ir/cr/kur/rr;"},
    {OPT_MORE_STR, 0, 0, "also used as recipient if neither -recipient nor -srvcert are given"},
    {"serial", OPT_SERIAL, 's', "Serial number of certificate to be revoked in revocation request (rr)"},
...
#if defined(OPENSSL_NO_SOCK) || defined(OPENSSL_NO_HTTP)
    {OPT_MORE_STR, 0, 0, "NOTE: -server, -proxy, and -no_proxy not supported due to no-sock/no-http build"},
#else
    ...
    {"trusted", OPT_TRUSTED, 's', "Certificates to use as trust anchors when verifying signed CMP responses"},
    ...
    {"srvcertout", OPT_SRVCERTOUT, 's', "File to save the server cert used and validated for CMP response protection"},
    {"extracertsout", OPT_EXTRACERTSOUT, 's', "File to save extra certificates received in the extraCerts field"},
    {"cacertsout", OPT_CACERTSOUT, 's', "File to save CA certs received in caPubs field or genp with id-it-caCerts"},
    {"oldwithold", OPT_OLDWITHOLD, 's', "Root CA certificate to request update for in genm of type rootCaCert"},
    {"newwithnew", OPT_NEWWITHNEW, 's', "File to save NewWithNew cert received in genp of type rootCaKeyUpdate"},
    {"newwithold", OPT_NEWWITHOLD, 's', "File to save NewWithOld cert received in genp of type rootCaKeyUpdate"},
    {"oldwithnew", OPT_OLDWITHNEW, 's', "File to save OldWithNew cert received in genp of type rootCaKeyUpdate"},
    {"crlcert", OPT_CRLCERT, 's', "certificate to request a CRL for in genm of type crlStatusList"},
    {"oldcrl", OPT_OLDCRL, 's', "CRL to request update for in genm of type crlStatusList"},
    {"crlout", OPT_CRLOUT, 's', "File to save new CRL received in genp of type 'crls'"},

    OPT_SECTION("Client authentication"),
    {"ref", OPT_REF, 's', "Reference value to use as senderKID in case no -cert is given"},
    ...
    {"certform", OPT_CERTFORM, 's', "Format (PEM or DER) to use when saving a certificate to a file. Default PEM"},
    {"crlform", OPT_CRLFORM, 's', "Format (PEM or DER) to use when saving a CRL to a file. Default DER"},
...
#if defined(OPENSSL_NO_SOCK) || defined(OPENSSL_NO_HTTP)
    {OPT_MORE_STR, 0, 0, "NOTE: -tls_used and all other TLS options not supported due to no-sock/no-http build"},
#else
    ...
    {"tls_host", OPT_TLS_HOST, 's', "Address to be checked (rather than -server) during TLS hostname validation"},
#endif

    OPT_SECTION("Client-side debugging"),
    {"batch", OPT_BATCH, '-', "Do not interactively prompt for input when a password is required etc."},
    ...
    {"reqout_only", OPT_REQOUT_ONLY, 's', "Save first CMP request created by the client to file and exit"},
...
#if defined(OPENSSL_NO_SOCK) || defined(OPENSSL_NO_HTTP)
    {OPT_MORE_STR, 0, 0, "NOTE: -port and -max_msgs not supported due to no-sock/no-http build"},
#else
    ...
    {"srv_key", OPT_SRV_KEY, 's', "Private key used by the server for signing messages"},
    ...
    {"ref_cert", OPT_RSP_CERT, 's', "Certificate to be expected for rr and any oldCertID in kur messages"},
    {"rsp_cert", OPT_RSP_CERT, 's', "Certificate to be returned as mock enrollment result"},
    {"rsp_key", OPT_RSP_KEY, 's', "Private key for the certificate to be returned as mock enrollment result"},
    {OPT_MORE_STR, 0, 0, "Key to be returned for central key pair generation"},
    {"rsp_keypass", OPT_RSP_KEYPASS, 's', "Response private key (and cert) pass phrase source"},
    {"rsp_crl", OPT_RSP_CRL, 's', "CRL to be returned in genp of type crls"},
    ...
    {"rsp_newwithnew", OPT_RSP_NEWWITHNEW, 's', "New root CA certificate to include in genp of type rootCaKeyUpdate"},
...
static int truststore_set_host_etc(X509_STORE *ts, const char *host)
{
    ...
    if (!X509_VERIFY_PARAM_set1_host(ts_vpm, NULL, 0) || !X509_VERIFY_PARAM_set1_ip(ts_vpm, NULL, 0)
...
static OSSL_CMP_MSG *read_write_req_resp(OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *req)
{
    ...
    if (opt_reqout_only != NULL) {
        if (OSSL_CMP_MSG_write(opt_reqout_only, req) < 0)
            CMP_err1("cannot write request PKIMessage to file '%s'", opt_reqout_only);
        else
            reqout_only_done = 1;
...
#else
            CMP_err(
                "-server not supported on no-sock/no-http build; missing -use_mock_srv option or too few -rspin filename arguments");
#endif
        }
        rspin_in_use = 0;
    }
...
static int set_name(const char *str, int (*set_fn)(OSSL_CMP_CTX *ctx, const X509_NAME *name), OSSL_CMP_CTX *ctx,
                    const char *desc)
{
    if (str != NULL) {
...
static int setup_cert(void *ctx, const char *file, const char *pass, const char *desc, add_X509_fn_t set1_fn)
{
    X509 *cert;
    int ok;
...
#undef FORMAT_OPTIONS
    ...
    if (opt_crlform_s != NULL && !opt_format(opt_crlform_s, OPT_FMT_PEMDER, &opt_crlform)) {
...
static OSSL_CMP_SRV_CTX *setup_srv_ctx(ENGINE *engine)
{
    ...
    if (!setup_cert(ctx, opt_srv_cert, opt_srv_keypass, "signer certificate of the mock server",
                    (add_X509_fn_t)OSSL_CMP_CTX_set1_cert))
        goto err;
    if (opt_srv_key != NULL) {
        EVP_PKEY *pkey =
            load_key_pwd(opt_srv_key, opt_keyform, opt_srv_keypass, engine, "private key for mock server cert");
    ...
    if (!setup_certs(opt_srv_untrusted, "untrusted certificates for mock server", ctx,
                     (add_X509_stack_fn_t)OSSL_CMP_CTX_set1_untrusted))
        goto err;

    if (!setup_cert(srv_ctx, opt_ref_cert, opt_otherpass, "reference cert to be expected by the mock server",
                    (add_X509_fn_t)ossl_cmp_mock_srv_set1_refCert))
        goto err;
    if (opt_rsp_cert == NULL) {
        CMP_warn("no -rsp_cert given for mock server");
    } else {
        if (!setup_cert(srv_ctx, opt_rsp_cert, opt_rsp_keypass, "cert the mock server returns on certificate requests",
                        (add_X509_fn_t)ossl_cmp_mock_srv_set1_certOut))
            goto err;
    }
    if (opt_rsp_key != NULL) {
        EVP_PKEY *pkey =
            load_key_pwd(opt_rsp_key, opt_keyform, opt_rsp_keypass, engine, "private key for enrollment cert");

        if (pkey == NULL || !ossl_cmp_mock_srv_set1_keyOut(srv_ctx, pkey)) {
            EVP_PKEY_free(pkey);
            goto err;
        }
        EVP_PKEY_free(pkey);
    ...
    if (!setup_mock_crlout(srv_ctx, opt_rsp_crl, "CRL to be returned by the mock server"))
        goto err;
    if (!setup_certs(opt_rsp_extracerts, "CMP extra certificates for mock server", srv_ctx,
...
static int setup_verification_ctx(OSSL_CMP_CTX *ctx)
{
    ...
    if (opt_srvcert != NULL || opt_trusted != NULL) {
        if (opt_srvcert != NULL) {
            ...
            if (!setup_cert(ctx, opt_srvcert, opt_otherpass, "directly trusted CMP server certificate",
    ...
    if (opt_tls_cert != NULL && opt_tls_key != NULL) {
            ...
            if (trust_store != NULL) {
                ...
                X509_VERIFY_PARAM_clear_flags(tls_vpm,
                                              ~(X509_V_FLAG_USE_CHECK_TIME | X509_V_FLAG_NO_CHECK_TIME
                                                | X509_V_FLAG_PARTIAL_CHAIN | X509_V_FLAG_POLICY_CHECK));
            }
        ...
        if (opt_tls_extra != NULL) {
            STACK_OF(X509) *tls_extra =
                load_certs_multifile(opt_tls_extra, opt_otherpass, "extra certificates for TLS", vpm);
            int res = 1;
...
static int setup_protection_ctx(OSSL_CMP_CTX *ctx, ENGINE *engine)
{
    ...
    if (opt_key != NULL) {
        EVP_PKEY *pkey =
            load_key_pwd(opt_key, opt_keyform, opt_keypass, engine, "private key for CMP client certificate");
    ...
    if (opt_cert != NULL) {
        ...
        if (!load_cert_certs(opt_cert, &cert, &certs, 0, opt_keypass, "CMP client certificate (optionally with chain)",
                             vpm))
...
static int set_fallback_pubkey(OSSL_CMP_CTX *ctx)
{
    ...
    *end = bak;
    ...
    if ((pubkey = OSSL_CMP_MSG_get0_certreq_publickey(req)) == NULL || (pkey = X509_PUBKEY_get0(pubkey)) == NULL) {
...
static int setup_request_ctx(OSSL_CMP_CTX *ctx, ENGINE *engine)
{
    ...
    if (opt_subject == NULL && opt_csr == NULL && opt_oldcert == NULL && opt_cert == NULL && opt_cmd != CMP_RR
        && opt_cmd != CMP_GENM)
    ...
    if (opt_cmd == CMP_IR || opt_cmd == CMP_CR || opt_cmd == CMP_KUR) {
        if (opt_reqin == NULL && opt_newkey == NULL && !opt_centralkeygen && opt_key == NULL && opt_csr == NULL
            && opt_oldcert == NULL) {
            CMP_err(
                "missing -newkey (or -key) to be certified and no -csr, -oldcert, -cert, or -reqin option given, which could provide fallback public key."
        ...
        if (opt_newkey == NULL && opt_popo != OSSL_CRMF_POPO_NONE && opt_popo != OSSL_CRMF_POPO_RAVERIFIED) {
            if (opt_csr != NULL) {
                ...
                if (opt_reqin != NULL)
                    CMP_info(
                        "since -reqin is used, may use -popo -1 or -popo 0 to disable the needless generation of a POPO");
                return 0;
            }
            if (opt_key == NULL) {
    ...
    if (opt_cmd == CMP_KUR) {
        ...
        if (opt_subject != NULL)
            CMP_warn2("given -subject '%s' overrides the subject of '%s' for KUR", opt_subject,
                      ref_cert != NULL ? ref_cert : opt_csr);
    }
    if (opt_cmd == CMP_RR) {
...
#define OSSL_CMP_RR_MSG "since -issuer and -serial is given for command 'rr'"
    ...
    if (opt_recipient == NULL && opt_srvcert == NULL && opt_issuer == NULL && opt_oldcert == NULL && opt_cert == NULL)
        CMP_warn(
            "missing -recipient, -srvcert, -issuer, -oldcert or -cert; recipient for any requests not covered by -reqin will be set to \"NULL-DN\"");
    ...
    if (opt_cmd == CMP_P10CR || opt_cmd == CMP_RR || opt_cmd == CMP_GENM) {
        ...
        if (pkey == NULL) {
            ERR_clear_error();
            desc = opt_csr == NULL ? "fallback public key for cert to be enrolled"
    ...
    } else if (opt_reqin != NULL && opt_key == NULL && opt_csr == NULL && opt_oldcert == NULL && !opt_centralkeygen) {
    ...
    do {
        ...
        *ptr++ = '\0';
        ...
        if (CHECK_AND_SKIP_CASE_PREFIX(ptr, "int:")) {
            ...
            if (*ptr != '\0') {
                if (*ptr != ',') {
                    CMP_err1("Missing ',' or end of -geninfo arg after int at %.40s", ptr);
                    goto err;
                }
                ptr++;
            }

            if ((aint = ASN1_INTEGER_new()) == NULL || !ASN1_INTEGER_set(aint, value))
                goto oom;
            ...
            if (end == NULL)
                ...
                *end++ = '\0';
            if ((text = ASN1_UTF8STRING_new()) == NULL || !ASN1_STRING_set(text, ptr, -1))
                goto oom;
            ptr = end;
...
static int setup_client_ctx(OSSL_CMP_CTX *ctx, ENGINE *engine)
{
    ...
    if (!OSSL_HTTP_parse_url(opt_server, &use_ssl, NULL /* user */, &host, &port, &portnum, &path, NULL /* q */,
                             NULL /* frag */)) {
    ...
    if (opt_no_proxy != NULL && !OSSL_CMP_CTX_set1_no_proxy(ctx, opt_no_proxy))
        goto oom;
    (void)BIO_snprintf(server_buf, sizeof(server_buf), "http%s://%s:%s/%s", opt_tls_used ? "s" : "", host, port,
...
static int write_crl(BIO *bio, X509_CRL *crl)
{
    ...
    return opt_crlform == FORMAT_PEM ? PEM_write_bio_X509_CRL(bio, crl) : i2d_X509_CRL_bio(bio, crl);
}
...
static int save_free_certs(STACK_OF(X509) *certs, const char *file, const char *desc)
{
    ...
    if ((bio = BIO_new(BIO_s_file())) == NULL || !BIO_write_filename(bio, (char *)file)) {
        CMP_err3("could not open file '%s' for %s %s certificate(s)", file, certs == NULL ? "deleting" : "writing",
                 desc);
        n = -1;
        goto end;
    }
...
static int save_crl(X509_CRL *crl, const char *file, const char *desc)
{
    BIO *bio = NULL;
    int res = 0;
...
static int delete_file(const char *file, const char *desc)
{
    ...
    if (unlink(file) != 0 && errno != ENOENT) {
        CMP_err2("Failed to delete %s, which should be done to indicate there is no %s", file, desc);
        return 0;
    }
    return 1;
}
...
static const char *prev_item(const char *opt, const char *end)
{
    ...
    if (len > SECTION_NAME_MAX) {
        CMP_warn3("using only first %d characters of section name starting with \"%.*s\"", SECTION_NAME_MAX,
                  SECTION_NAME_MAX, beg);
...
static int read_config(void)
{
    ...
    for (opt = &cmp_options[start_opt], i = start_idx; opt->name != NULL; i++, opt++)
        if (!strcmp(opt->name, OPT_SECTION_STR) || !strcmp(opt->name, OPT_MORE_STR))
            n_options--;
    OPENSSL_assert(OSSL_NELEM(cmp_vars)
                   == n_options + OPT_PROV__FIRST + 1 - OPT_PROV__LAST + OPT_R__FIRST + 1 - OPT_R__LAST + OPT_V__FIRST
                       + 1 - OPT_V__LAST);
    for (opt = &cmp_options[start_opt], i = start_idx; opt->name != NULL; i++, opt++) {
...
static int cmp_server(OSSL_CMP_CTX *srv_cmp_ctx)
{
    ...
    while (opt_max_msgs <= 0 || msgs < opt_max_msgs) {
        ...
        ret = http_server_get_asn1_req(ASN1_ITEM_rptr(OSSL_CMP_MSG), (ASN1_VALUE **)&req, &path, &cbio, acbio,
                                       &keep_alive, prog, 0, 0);
        ...
        if (req != NULL) {
            ...
            ret = http_server_send_asn1_resp(prog, cbio, keep_alive, "application/pkixcmp",
                                             ASN1_ITEM_rptr(OSSL_CMP_MSG), (const ASN1_VALUE *)resp);
    ...
    for (i = 0; i < sk_OSSL_CMP_ATAV_num(keySpec); i++) {
        ...
        case NID_id_regCtrl_algId: {
            ...
            if (paramtype == V_ASN1_UNDEF || alg->parameter == NULL) {
                ...
                BIO_printf(mem, " - ");
                ASN1_item_print(mem, (ASN1_VALUE *)alg, 0, ASN1_ITEM_rptr(X509_ALGOR), NULL);
            }
        } break;
...
static void print_status(void)
{
    ...
    CMP_print(bio_err,
              status == OSSL_CMP_PKISTATUS_accepted ? OSSL_CMP_LOG_INFO
                  : status == OSSL_CMP_PKISTATUS_rejection || status == OSSL_CMP_PKISTATUS_waiting
                  ? OSSL_CMP_LOG_ERR
...
static int do_genm(OSSL_CMP_CTX *ctx)
{
    if (opt_infotype == NID_id_it_caCerts) {
        ...
        if (cacerts == NULL) {
            ...
            CMP_err1("Failed to store CA certificates from genp in %s", opt_cacertsout);
            return 0;
        }
        return 1;
        ...
        if (opt_oldwithold == NULL) {
            CMP_warn(
                "No -oldwithold given, will use all certs given with -trusted as trust anchors for verifying the newWithNew cert");
        } else {
            oldwithold =
                load_cert_pwd(opt_oldwithold, opt_otherpass, "OldWithOld cert for genm with -infotype rootCaCert");
            if (oldwithold == NULL)
                goto end_upd;
        }
        if (!OSSL_CMP_get1_rootCaKeyUpdate(ctx, oldwithold, &newwithnew, &newwithold, &oldwithnew))
            goto end_upd;
        ...
        if (newwithnew == NULL)
            ...
            CMP_warn(
                "oldWithNew certificate received in genp for verifying oldWithOld, but oldWithOld was not provided");

        if (save_cert_or_delete(newwithnew, opt_newwithnew, "NewWithNew cert from genp")
            && save_cert_or_delete(newwithold, opt_newwithold, "NewWithOld cert from genp")
            && save_cert_or_delete(oldwithnew, opt_oldwithnew, "OldWithNew cert from genp"))
            res = 1;
...
end_upd:
        ...
        if (opt_crlcert != NULL) {
            crlcert = load_cert_pwd(opt_crlcert, opt_otherpass, "Cert for genm with -infotype crlStatusList");
            if (crlcert == NULL)
                goto end_crlupd;
        }

        if (opt_oldcrl != NULL) {
            oldcrl = load_crl(opt_oldcrl, FORMAT_UNDEF, 0, "CRL for genm with -infotype crlStatusList");
            if (oldcrl == NULL)
                goto end_crlupd;
        }

        if (opt_oldcrl != NULL && opt_crlcert != NULL) {
            if (X509_NAME_cmp(X509_CRL_get_issuer(oldcrl), X509_get_issuer_name(crlcert)) != 0)
...
end_crlupd:
        ...
        if (certTemplate == NULL) {
            ...
            if (!delete_file(opt_template, "certTemplate from genp"))
                return 0;
            if (opt_keyspec != NULL && !delete_file(opt_keyspec, "keySpec from genp"))
                return 0;
            return 1;
        }
...
tmpl_end:
        ...
        if (opt_infotype != NID_undef) {
            CMP_warn1("No specific support for -infotype %s available", opt_infotype_s);

            req = OSSL_CMP_ITAV_create(OBJ_nid2obj(opt_infotype), NULL);
            if (req == NULL || !OSSL_CMP_CTX_push0_genm_ITAV(ctx, req)) {
                CMP_err1("Failed to create genm for -infotype %s", opt_infotype_s);
                return 0;
            }
        }
    ...
    for (i = 1; i < argc - 1; i++) {
        if (*argv[i] == '-') {
            if (!strcmp(argv[i] + 1, cmp_options[OPT_CONFIG - OPT_HELP].name))
                ...
                opt_section = argv[++i];
            else if (strcmp(argv[i] + 1, cmp_options[OPT_VERBOSITY - OPT_HELP].name) == 0
...
#if !defined(OPENSSL_NO_SOCK) && !defined(OPENSSL_NO_HTTP)
    if (opt_tls_cert == NULL && opt_tls_key == NULL && opt_tls_keypass == NULL && opt_tls_extra == NULL
...
# endif
    ...
    for (i = 0; i < opt_repeat; i++) {
        ...
        print_status();
        if (!save_cert_or_delete(OSSL_CMP_CTX_get0_validatedSrvCert(cmp_ctx), opt_srvcertout, "validated server"))
            ret = 0;
        if (!ret)
            goto err;
        ret = 0;
        if (save_free_certs(OSSL_CMP_CTX_get1_extraCertsIn(cmp_ctx), opt_extracertsout, "extra") < 0)
            goto err;
        if (newcert != NULL && (opt_cmd == CMP_IR || opt_cmd == CMP_CR || opt_cmd == CMP_KUR || opt_cmd == CMP_P10CR)) {
            ...
            if (newcert != NULL && newchain != NULL /* NULL is on error only */
                && opt_certout != NULL && opt_chainout != NULL && strcmp(opt_certout, opt_chainout) == 0) {
            ...
            if (save_free_certs(OSSL_CMP_CTX_get1_caPubs(cmp_ctx), opt_cacertsout, "CA") < 0)
                goto err;
            if (opt_centralkeygen) {
                ...
                if ((out = bio_open_owner(opt_newkeyout, FORMAT_PEM, 1)) == NULL)
                    goto err;
                if (opt_newkeypass != NULL) {
                    pass_string = get_passwd(opt_newkeypass, "Centrally generated private key password");
                ...
                if (PEM_write_bio_PrivateKey(out, new_key, cipher, NULL, 0, NULL, (void *)pass_string) <= 0)
                    result = 0;
