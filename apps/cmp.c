...
#if !defined(OPENSSL_NO_SOCK) && !defined(OPENSSL_NO_HTTP)
      if (opt_server == NULL) {
        CMP_err("missing -server or -use_mock_srv option, or too few -rspin "
                "filename arguments");
        goto err;
      }
      if (rspin_in_use)
...
#else
      CMP_err("-server not supported on no-sock/no-http build; missing "
...
static int set_gennames(OSSL_CMP_CTX *ctx, char *names, const char *desc) {
  ...
  for (; names != NULL; names = next) {
    ...
    if (strcmp(names, "critical") == 0) {
      (void)OSSL_CMP_CTX_set_option(ctx, OSSL_CMP_OPT_SUBJECTALTNAME_CRITICAL,
                                    1);
      continue;
    }
...
static OSSL_CMP_SRV_CTX *setup_srv_ctx(ENGINE *engine) {
  ...
  if (opt_srv_secret != NULL) {
    ...
    CMP_err("server credentials (-srv_secret or -srv_cert) must be given if "
            "-use_mock_srv or -port is used");
    goto err;
  } else {
    CMP_warn("server will not be able to handle PBM-protected requests since "
  ...
  if (!setup_cert(ctx, opt_srv_cert, opt_srv_keypass,
    ...
    goto err;
  if (opt_srv_key != NULL) {
    EVP_PKEY *pkey = load_key_pwd(opt_srv_key, opt_keyform, opt_srv_keypass,
  ...
  if (!setup_certs(opt_srv_untrusted, "untrusted certificates for mock server",
                   ctx, (add_X509_stack_fn_t)OSSL_CMP_CTX_set1_untrusted))
    goto err;
  ...
  if (opt_rsp_key != NULL) {
    EVP_PKEY *pkey = load_key_pwd(opt_rsp_key, opt_keyform, opt_rsp_keypass,
                                  engine, "private key for enrollment cert");
  ...
  if (!setup_mock_crlout(srv_ctx, opt_rsp_crl,
    ...
    goto err;
  if (!setup_certs(opt_rsp_extracerts, "CMP extra certificates for mock server",
                   srv_ctx,
  ...
  if (!ossl_cmp_mock_srv_set_statusInfo(srv_ctx, opt_pkistatus, opt_failurebits,
                                        opt_statusstring))
    goto err;
...
static SSL_CTX *setup_ssl_ctx(OSSL_CMP_CTX *ctx, const char *host,
  ...
  if (opt_tls_trusted != NULL) {
    ...
    CMP_warn("-tls_used given without -tls_trusted; will not authenticate the "
             "TLS server");
  }
  ...
  if (opt_tls_cert != NULL && opt_tls_key != NULL) {
...

    {
      ...
      if (trust_store != NULL) {
        ...
        tls_vpm, ~(X509_V_FLAG_USE_CHECK_TIME | X509_V_FLAG_NO_CHECK_TIME |
                   X509_V_FLAG_PARTIAL_CHAIN | X509_V_FLAG_POLICY_CHECK));
      }
      CMP_debug("trying to build cert chain for own TLS cert");
      if (SSL_CTX_build_cert_chain(ssl_ctx, SSL_BUILD_CHAIN_FLAG_UNTRUSTED |
    ...
    CMP_warn(
    "-tls_used given without -tls_key; cannot authenticate to the TLS server");
  }
  if (trust_store != NULL) {
    /*
...
static int setup_protection_ctx(OSSL_CMP_CTX *ctx, ENGINE *engine) {
  ...
  if (opt_secret != NULL) {
    ...
    if (pass_string != NULL) {
      cleanse(opt_secret);
      res = OSSL_CMP_CTX_set1_secretValue(ctx, (unsigned char *)pass_string,
                                          strlen(pass_string));
      clear_free(pass_string);
      if (res == 0)
        return 0;
  ...
  if (opt_secret == NULL && opt_srvcert == NULL && opt_trusted == NULL)
    CMP_warn("will not authenticate server due to missing -secret, -trusted, "
             "or -srvcert");
  ...
  if (opt_cert != NULL) {
    ...
    if (!load_cert_certs(opt_cert, &cert, &certs, 0, opt_keypass,
                         "CMP client certificate (optionally with chain)", vpm))
    ...
    if (!ok) {
      ...
      if (opt_own_trusted != NULL) {
        own_trusted = load_trusted(
        opt_own_trusted, 0, "trusted certs for verifying own CMP signer cert");
        ok = own_trusted != NULL;
      }
...
static int set_fallback_pubkey(OSSL_CMP_CTX *ctx) {
  ...
  *end = bak;
  ...
  if (req == NULL) {
    CMP_err1(
    "failed to load ir/cr/kur file '%s' attempting to get fallback public key",
    file);
    return 0;
  }
...
static int setup_request_ctx(OSSL_CMP_CTX *ctx, ENGINE *engine) {
  ...
  if (opt_subject == NULL && opt_csr == NULL && opt_oldcert == NULL &&
    ...
    CMP_warn(
    "no -subject given; no -csr or -oldcert or -cert available for fallback");
  ...
  if (opt_cmd == CMP_IR || opt_cmd == CMP_CR || opt_cmd == CMP_KUR) {
    if (opt_reqin == NULL && opt_newkey == NULL && !opt_centralkeygen &&
      ...
      CMP_err(
      "missing -newkey (or -key) to be certified and no -csr, -oldcert, -cert, "
    ...
    if (opt_newkey == NULL && opt_popo != OSSL_CRMF_POPO_NONE &&
      ...
      if (opt_csr != NULL) {
        CMP_err1("no -newkey option given with private key for POPO, -csr "
                 "option provides just public key%s",
                 opt_key == NULL ? "" : ", and -key option superseded by -csr");
        if (opt_reqin != NULL)
          CMP_info("since -reqin is used, may use -popo -1 or -popo 0 to "
                   "disable the needless generation of a POPO");
        return 0;
      }
      if (opt_key == NULL) {
        CMP_err("missing -newkey (or -key) option for key to be certified and "
                "for POPO");
        return 0;
      }
    }
  ...
  if (opt_cmd == CMP_RR) {
    if (opt_issuer == NULL && opt_serial == NULL) {
      if (opt_oldcert == NULL && opt_csr == NULL) {
        CMP_err("missing -oldcert or -issuer and -serial for certificate to be "
...
#define OSSL_CMP_RR_MSG "since -issuer and -serial is given for command 'rr'"
  ...
  if (opt_recipient == NULL && opt_srvcert == NULL && opt_issuer == NULL &&
    ...
    CMP_warn(
    "missing -recipient, -srvcert, -issuer, -oldcert or -cert; recipient for "
  ...
  if (opt_san_nodefault) {
    ...
    (void)OSSL_CMP_CTX_set_option(ctx, OSSL_CMP_OPT_SUBJECTALTNAME_NODEFAULT,
                                  1);
  }
  ...
  if (opt_policy_oids_critical) {
    if (opt_policy_oids == NULL)
      CMP_warn(
      "-opt_policy_oids_critical has no effect unless -policy_oids is given");
...
static int save_template(const char *file, const OSSL_CRMF_CERTTEMPLATE *tmpl) {
  ...
  if (!ASN1_i2d_bio_of(OSSL_CRMF_CERTTEMPLATE, i2d_OSSL_CRMF_CERTTEMPLATE, bio,
                       tmpl)) {
...
static int read_config(void) {
  ...
  for (opt = &cmp_options[start_opt], i = start_idx; opt->name != NULL;
    ...
    case 'M':
      ...
      break;
    default:
      CMP_err2("internal: unsupported type '%c' for option '%s'", opt->valtype,
               opt->name);
      return 0;
      break;
    }
    if (provider_option || verification_option) {
      ...
      if (conf_argc > 1) {
        ...
        if (provider_option ? !opt_provider(opt_next())
          ...
          CMP_err2("for option '%s' in config file section '%s'", opt->name,
                   opt_section);
          return 0;
        }
      }
...
static int get_opts(int argc, char **argv) {
  ...
  while ((o = opt_next()) != OPT_EOF) {
    ...
    case OPT_REVREASON:
      ...
      if (opt_revreason < CRL_REASON_NONE ||
          opt_revreason > CRL_REASON_AA_COMPROMISE || opt_revreason == 7) {
...
static int cmp_server(OSSL_CMP_CTX *srv_cmp_ctx) {
  ...
  while (opt_max_msgs <= 0 || msgs < opt_max_msgs) {
    ...
    ret =
    http_server_get_asn1_req(ASN1_ITEM_rptr(OSSL_CMP_MSG), (ASN1_VALUE **)&req,
                             &path, &cbio, acbio, &keep_alive, prog, 0, 0);
  ...
  for (i = 0; i < sk_OSSL_CMP_ATAV_num(keySpec); i++) {
    ...
    case NID_id_regCtrl_algId: {
      ...
      if (paramtype == V_ASN1_UNDEF || alg->parameter == NULL) {
        ...
        BIO_printf(mem, " - ");
        ASN1_item_print(mem, (ASN1_VALUE *)alg, 0, ASN1_ITEM_rptr(X509_ALGOR),
                        NULL);
      }
    } break;
...
static int do_genm(OSSL_CMP_CTX *ctx) {
  if (opt_infotype == NID_id_it_caCerts) {
    ...
    if (opt_oldwithold == NULL) {
      CMP_warn("No -oldwithold given, will use all certs given with -trusted "
  ...
  end_upd:
    ...
    if (opt_oldcrl == NULL && opt_crlcert == NULL) {
      CMP_err(
      "Missing -oldcrl and no -crlcert given for -infotype crlStatusList");
      return 0;
    }
    if (opt_crlout == NULL) {
...
int cmp_main(int argc, char **argv) {
      ...
      if (strcmp(opt_section, CMP_SECTION) == 0) { /* default */
        ...
        while ((end = prev_item(opt_section, end)) != NULL) {
          if (!NCONF_get_section(conf, opt_item)) {
            CMP_err2("no [%s] section found in config file '%s'", opt_item,
                     configfile);
            goto err;
          }
        }
  ...
  if (opt_rspin != NULL) {
    ...
    if (opt_use_mock_srv)
      CMP_warn(
      "-use_mock_srv option is not used if enough filenames given for -rspin");
  }
#endif
    ...
    if (newcert != NULL && (opt_cmd == CMP_IR || opt_cmd == CMP_CR ||
      ...
      if (newcert != NULL && newchain != NULL /* NULL is on error only */
        ...
        if (!X509_add_cert(newchain, newcert,
                           X509_ADD_FLAG_PREPEND | X509_ADD_FLAG_UP_REF)) {
      ...
      if (save_free_certs(OSSL_CMP_CTX_get1_caPubs(cmp_ctx), opt_cacertsout,
                          "CA") < 0)
        goto err;
      if (opt_centralkeygen) {
        ...
        if (opt_newkeypass != NULL) {
          ...
          cipher =
          EVP_CIPHER_fetch(app_get0_libctx(), SN_aes_256_cbc, app_get0_propq());
        }
