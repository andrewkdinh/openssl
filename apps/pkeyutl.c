...
static EVP_PKEY *get_pkey(const char *kdfalg, const char *keyfile, int keyform,
                          int key_type, char *passinarg, int pkey_op,
                          ENGINE *e);
static EVP_PKEY_CTX *init_ctx(const char *kdfalg, int *pkeysize, int pkey_op,
                              ENGINE *e, const int engine_impl, int rawin,
                              EVP_PKEY *pkey /* ownership is passed to ctx */,
                              EVP_MD_CTX *mctx, const char *digestname,
                              const char *kemop, OSSL_LIB_CTX *libctx,
                              const char *propq);
...
static int do_raw_keyop(int pkey_op, EVP_MD_CTX *mctx, EVP_PKEY *pkey, BIO *in,
...
{"derive", OPT_DERIVE, '-',
 "Derive shared secret from own and peer (EC)DH keys"},
...
{"digest", OPT_DIGEST, 's',
 "The digest algorithm to use for signing/verifying raw input data. Implies "
 "-rawin"},
...
int pkeyutl_main(int argc, char **argv) {
  ...
  EVP_PKEY *pkey = NULL;
  char *infile = NULL, *outfile = NULL, *secoutfile = NULL, *sigfile = NULL,
       *passinarg = NULL;
  ...
  if (pkey_op == EVP_PKEY_OP_SIGN || pkey_op == EVP_PKEY_OP_VERIFY) {
    ...
    BIO_printf(
    bio_err, "%s: -digest and -rawin can only be used with -sign or -verify\n",
    prog);
    goto opthelp;
  }
  ...
  ctx = init_ctx(kdfalg, &keysize, pkey_op, e, engine_impl, rawin, pkey, mctx,
                 digestname, kemop, libctx, app_get0_propq());
  if (ctx == NULL) {
  ...
  if (pkeyopts_passin != NULL) {
    ...
    for (i = 0; i < num; i++) {
      ...
      if (passin == NULL) {
        ...
        r =
        EVP_read_pw_string(passwd_buf, sizeof(passwd_buf) - 1, passwd_buf, 0);
        if (r < 0) {
          if (r == -2)
  ...
  if (!rawin &&
    ...
    if (buf_inlen > EVP_MAX_MD_SIZE) {
      BIO_printf(bio_err,
                 "Error: The non-raw input data length %d is too long - max "
                 "supported hashed size is %d\n",
  ...
  if (pkey_op == EVP_PKEY_OP_VERIFY) {
    if (rawin) {
      rv =
      do_raw_keyop(pkey_op, mctx, pkey, in, filesize, sig, siglen, NULL, 0);
    } else {
  ...
  if (rawin) {
    ...
    rv = do_raw_keyop(pkey_op, mctx, pkey, in, filesize, NULL, 0, &buf_out,
                      (size_t *)&buf_outlen);
  } else {
    if (kdflen != 0) {
      buf_outlen = kdflen;
      ...
      rv = do_keyop(ctx, pkey_op, NULL, (size_t *)&buf_outlen, buf_in,
                    (size_t)buf_inlen, NULL, (size_t *)&secretlen);
    }
    if (rv > 0 &&
      ...
      rv = do_keyop(ctx, pkey_op, buf_out, (size_t *)&buf_outlen, buf_in,
                    (size_t)buf_inlen, secret, (size_t *)&secretlen);
    }
  }
  if (rv <= 0) {
...
static EVP_PKEY_CTX *init_ctx(const char *kdfalg, int *pkeysize, int pkey_op,
  ...
  if (rawin) {
    ...
    case EVP_PKEY_OP_SIGN:
      rv =
      EVP_DigestSignInit_ex(mctx, NULL, digestname, libctx, propq, pkey, NULL);
      break;
...
static int setup_peer(EVP_PKEY_CTX *ctx, int peerform, const char *file,
  ...
  if (strcmp(EVP_PKEY_get0_type_name(peer), EVP_PKEY_get0_type_name(pkey)) !=
      0) {
    BIO_printf(
    bio_err,
...
static int do_raw_keyop(int pkey_op, EVP_MD_CTX *mctx, EVP_PKEY *pkey, BIO *in,
  ...
  if (only_nomd(pkey)) {
    if (filesize < 0) {
      BIO_printf(
      bio_err, "Error: unable to determine file size for oneshot operation\n");
      goto end;
    }
