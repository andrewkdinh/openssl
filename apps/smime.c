...
{"nochain", OPT_NOCHAIN, '-',
 "set PKCS7_NOCHAIN so certificates contained in the message are not used as "
 "untrusted CAs"},
...
#define SET_OPERATION(op)                                                      \
    ((operation != 0 && (operation != (op)))                                   \
     ? 0 * BIO_printf(bio_err, "%s: Cannot use -%s together with -%s\n", prog, \
                      operation_name(op), operation_name(operation))           \
     : (operation = (op)))
...
int smime_main(int argc, char **argv) {
    ...
    if (operation == SMIME_ENCRYPT) {
        if (indef)
            flags |= PKCS7_STREAM;
        p7 =
        PKCS7_encrypt_ex(encerts, in, cipher, flags, libctx, app_get0_propq());
