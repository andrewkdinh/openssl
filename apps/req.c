...
static int prompt_info(X509_REQ *req, STACK_OF(CONF_VALUE) *dn_sk, const char *dn_sect, STACK_OF(CONF_VALUE) *attr_sk,
                       const char *attr_sect, int attribs, unsigned long chtype);
static int auto_info(X509_REQ *req, STACK_OF(CONF_VALUE) *sk, STACK_OF(CONF_VALUE) *attr, int attribs,
                     unsigned long chtype);
static int add_attribute_object(X509_REQ *req, char *text, const char *def, char *value, int nid, int n_min, int n_max,
                                unsigned long chtype);
static int add_DN_object(X509_NAME *n, char *text, const char *def, char *value, int nid, int n_min, int n_max,
                         unsigned long chtype, int mval);
static int build_data(char *text, const char *def, char *value, int n_min, int n_max, char *buf, const int buf_size,
...
const OPTIONS req_options[] = {
    ...
    {"x509", OPT_X509, '-', "Output an X.509 certificate structure instead of a cert request"},
    ...
    {"CAkey", OPT_CAKEY, 's', "Issuer private key to use with -CA; default is -CA arg"},
    {OPT_MORE_STR, 1, 1, "(Required by some CA's)"},
    {"subj", OPT_SUBJ, 's', "Set or modify subject of request or cert"},
    {"subject", OPT_SUBJECT, '-', "Print the subject of the output request or cert"},
    {"multivalue-rdn", OPT_MULTIVALUE_RDN, '-', "Deprecated; multi-valued RDNs support is always on."},
    {"not_before", OPT_NOT_BEFORE, 's', "[CC]YYMMDDHHMMSSZ value for notBefore certificate field"},
    {"not_after", OPT_NOT_AFTER, 's', "[CC]YYMMDDHHMMSSZ value for notAfter certificate field, overrides -days"},
    ...
    {"extensions", OPT_EXTENSIONS, 's', "Cert or request extension section (override value in config file)"},
...
opthelp:
    ...
    if (extsect == NULL)
        extsect = app_conf_try_string(req_conf, section, gen_x509 ? V3_EXTENSIONS : REQ_EXTENSIONS);
    if (extsect != NULL) {
        ...
        if (!X509V3_EXT_add_nconf(req_conf, &ctx, extsect, NULL)) {
            BIO_printf(bio_err, "Error checking %s extension section %s\n", gen_x509 ? "x509" : "request", extsect);
            goto end;
        }
    }
    if (addext_conf != NULL) {
    ...
    if (keyalg != NULL && pkey != NULL) {
        BIO_printf(bio_err, "Warning: Not generating key via given -newkey option since -key is given\n");
    ...
    if (newreq && pkey == NULL) {
        ...
        if (newkey_len < MIN_KEY_LENGTH
            && (EVP_PKEY_CTX_is_a(genctx, "RSA") || EVP_PKEY_CTX_is_a(genctx, "RSA-PSS")
...
loop:
    ...
    if (!newreq) {
        if (keyfile != NULL)
            BIO_printf(bio_err, "Warning: Not placing -key in cert or request since request is used\n");
        req = load_csr_autofmt(infile /* if NULL, reads from stdin */, informat, vfyopts, "X509 request");
        if (req == NULL)
            goto end;
    } else if (infile != NULL) {
        BIO_printf(bio_err, "Warning: Ignoring -in option since -new or -newkey or -precert is given\n");
    ...
    if (CAkeyfile != NULL) {
        if (CAfile == NULL) {
            ...
            if ((CAkey = load_key(CAkeyfile, FORMAT_UNDEF, 0, passin, e,
                                  CAkeyfile != CAfile ? "issuer private key from -CAkey arg"
    ...
    if (newreq || gen_x509) {
        ...
        if (gen_x509) {
            ...
            if (CAcert != NULL && keyfile != NULL)
                BIO_printf(bio_err, "Warning: Not using -key or -newkey for signing since -CA option is given\n");
            ...
            /* Add extensions */
            if (extsect != NULL && !X509V3_EXT_add_nconf(req_conf, &ext_ctx, extsect, new_x509)) {
            ...
            i = do_X509_sign(new_x509, x509v1, issuer_key, digest, sigopts, &ext_ctx);
            if (!i)
                goto end;
        } else {
            X509V3_CTX ext_ctx;
            ...
            if (precert) {
                BIO_printf(bio_err, "Warning: Ignoring -precert flag since no cert is produced\n");
            }
            /* Set up V3 context struct */
            ...
            /* Add extensions */
            if (extsect != NULL && !X509V3_EXT_REQ_add_nconf(req_conf, &ext_ctx, extsect, req)) {
    ...
    out = bio_open_default(outfile, keyout != NULL && outfile != NULL && strcmp(keyout, outfile) == 0 ? 'a' : 'w',
                           outformat);
    if (out == NULL)
        goto end;
...
start:
        for (;;) {
            ...
            p = q = NULL;
            type = v->name;
            if (!check_end(type, "_min") || !check_end(type, "_max") || !check_end(type, "_default")
...
static int auto_info(X509_REQ *req, STACK_OF(CONF_VALUE) *dn_sk, STACK_OF(CONF_VALUE) *attr_sk, int attribs,
                     unsigned long chtype)
{
