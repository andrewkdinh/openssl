...
static void exts_cleanup(OPENSSL_STRING *x) { OPENSSL_free((char *)x); }
...
int req_main(int argc, char **argv) {
    ...
    if (!gen_x509) {
        if (days != UNSET_DAYS)
            BIO_printf(bio_err, "Warning: Ignoring -days without -x509; not "
                                "generating a certificate\n");
        if (not_before != NULL)
            BIO_printf(bio_err,
                       "Warning: Ignoring -not_before without -x509; not "
                       "generating a certificate\n");
        if (not_after != NULL)
            BIO_printf(bio_err,
                       "Warning: Ignoring -not_after without -x509; not "
                       "generating a certificate\n");
        if (ext_copy == EXT_COPY_NONE)
            BIO_printf(bio_err, "Warning: Ignoring -copy_extensions 'none' "
                                "when -x509 is not given\n");
    }
    if (infile == NULL) {
        if (gen_x509)
            newreq = 1;
            ...
            BIO_printf(bio_err,
                       "Warning: Will read cert request from stdin since no "
                       "-in option is given\n");
    }
    ...
    if (extsect == NULL)
        extsect = app_conf_try_string(
        req_conf, section, gen_x509 ? V3_EXTENSIONS : REQ_EXTENSIONS);
    if (extsect != NULL) {
    ...
    if (keyalg != NULL && pkey != NULL) {
        BIO_printf(bio_err,
                   "Warning: Not generating key via given -newkey option "
                   "since -key is given\n");
    ...
    if (newreq && pkey == NULL) {
        ...
        if (newkey_len < MIN_KEY_LENGTH &&
            ...
            BIO_printf(bio_err,
                       "Private key length too short, needs to be at least %d "
                       "bits, not %ld.\n",
        ...
        if (newkey_len > OPENSSL_RSA_MAX_MODULUS_BITS &&
            ...
            BIO_printf(
            bio_err,
            "Warning: It is not recommended to use more than %d bit for RSA "
            "keys.\n"
            "         Your key size is %ld! Larger key size may behave not as "
            "expected.\n",
        ...
        if (EVP_PKEY_CTX_is_a(genctx, "DSA") &&
            ...
            BIO_printf(
            bio_err,
            "Warning: It is not recommended to use more than %d bit for DSA "
            "keys.\n"
            "         Your key size is %ld! Larger key size may behave not as "
            "expected.\n",
    ...
    if (!newreq) {
        if (keyfile != NULL)
            BIO_printf(bio_err, "Warning: Not placing -key in cert or request "
                                "since request is used\n");
        req = load_csr_autofmt(infile /* if NULL, reads from stdin */, informat,
                               vfyopts, "X509 request");
        if (req == NULL)
            goto end;
    } else if (infile != NULL) {
        BIO_printf(bio_err,
                   "Warning: Ignoring -in option since -new or -newkey or "
                   "-precert is given\n");
    ...
    if (newreq || gen_x509) {
        ...
        if (gen_x509) {
            ...
            if (CAcert != NULL && keyfile != NULL)
                BIO_printf(bio_err,
                           "Warning: Not using -key or -newkey for signing "
                           "since -CA option is given\n");

            if ((new_x509 = X509_new_ex(app_get0_libctx(), app_get0_propq())) ==
                NULL)
                goto end;
            ...
            if (ext_copy == EXT_COPY_UNSET) {
                if (infile != NULL)
                    BIO_printf(bio_err,
                               "Warning: No -copy_extensions given; ignoring "
            ...
            if (CAcert == NULL) {
                ...
                if (!cert_matches_key(new_x509, issuer_key))
                    BIO_printf(bio_err, "Warning: Signature key and public key "
                                        "of cert do not match\n");
            }
            ...
            if (addext_conf != NULL &&
                !X509V3_EXT_add_nconf(addext_conf, &ext_ctx, "default",
                                      new_x509)) {
                BIO_printf(
                bio_err, "Error adding x509 extensions defined via -addext\n");
                goto end;
            }

            /* If a pre-cert was requested, we need to add a poison extension */
            if (precert) {
                if (X509_add1_ext_i2d(new_x509, NID_ct_precert_poison, NULL, 1,
                                      0) != 1) {
            ...
            if (addext_conf != NULL && !X509V3_EXT_REQ_add_nconf(
                                       addext_conf, &ext_ctx, "default", req)) {
                BIO_printf(
                bio_err,
...
static int make_REQ(X509_REQ *req, EVP_PKEY *pkey, X509_NAME *fsubj,
    ...
    if (fsubj != NULL)
        ...
        i =
        prompt_info(req, dn_sk, dn_sect, attr_sk, attr_sect, attribs, chtype);
    if (!i)
        goto err;
...
static int prompt_info(X509_REQ *req, STACK_OF(CONF_VALUE) * dn_sk,
    ...
    if (!batch) {
        ...
        BIO_printf(
        bio_err, "There are quite a few fields but you can leave some blank\n");
...
static int auto_info(X509_REQ *req, STACK_OF(CONF_VALUE) * dn_sk,
    ...
    for (i = 0; i < sk_CONF_VALUE_num(dn_sk); i++) {
        ...
        if (!X509_NAME_add_entry_by_txt(
            subj, type, chtype, (unsigned char *)v->value, -1, -1, mval))
            return 0;
    }
...
static EVP_PKEY_CTX *set_keygen_ctx(const char *gstr, char **pkeytype,
    ...
    if (param != NULL) {
        ...
        if (keygen_engine != NULL)
            ...
            gctx = EVP_PKEY_CTX_new_from_pkey(app_get0_libctx(), param,
                                              app_get0_propq());
        *pkeylen = EVP_PKEY_get_bits(param);
        ...
        if (keygen_engine != NULL) {
            int pkey_id =
            get_legacy_pkey_id(app_get0_libctx(), *pkeytype, keygen_engine);

            if (pkey_id != NID_undef)
                gctx = EVP_PKEY_CTX_new_id(pkey_id, keygen_engine);
        } else {
            gctx = EVP_PKEY_CTX_new_from_name(app_get0_libctx(), *pkeytype,
                                              app_get0_propq());
        }
    }
    ...
    if (keylen == -1 &&
        (EVP_PKEY_CTX_is_a(gctx, "RSA") || EVP_PKEY_CTX_is_a(gctx, "RSA-PSS")))
        keylen = *pkeylen;
