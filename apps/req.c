...
int req_main(int argc, char **argv) {
  ...
  if (!gen_x509) {
    ...
    if (not_before != NULL)
      BIO_printf(bio_err, "Warning: Ignoring -not_before without -x509; not "
                          "generating a certificate\n");
    if (not_after != NULL)
      BIO_printf(bio_err, "Warning: Ignoring -not_after without -x509; not "
                          "generating a certificate\n");
  ...
  if (infile == NULL) {
    if (gen_x509)
      ...
      BIO_printf(bio_err, "Warning: Will read cert request from stdin since no "
                          "-in option is given\n");
  }
  ...
  if (keyalg != NULL && pkey != NULL) {
    BIO_printf(bio_err, "Warning: Not generating key via given -newkey option "
                        "since -key is given\n");
  ...
  if (newreq && pkey == NULL) {
    ...
    if (newkey_len < MIN_KEY_LENGTH && (EVP_PKEY_CTX_is_a(genctx, "RSA") ||
  ...
  if (!newreq) {
    ...
    BIO_printf(bio_err, "Warning: Ignoring -in option since -new or -newkey or "
                        "-precert is given\n");
  ...
  if (newreq || gen_x509) {
    ...
    if (gen_x509) {
      ...
      if (CAcert != NULL && keyfile != NULL)
        BIO_printf(bio_err, "Warning: Not using -key or -newkey for signing "
      ...
      if (ext_copy == EXT_COPY_UNSET) {
        if (infile != NULL)
          BIO_printf(bio_err, "Warning: No -copy_extensions given; ignoring "
      ...
      X509V3_set_ctx(&ext_ctx, CAcert != NULL ? CAcert : new_x509, new_x509,
      ...
      if (addext_conf != NULL &&
          !X509V3_EXT_add_nconf(addext_conf, &ext_ctx, "default", new_x509)) {
        BIO_printf(bio_err,
      ...
      if (precert) {
        if (X509_add1_ext_i2d(new_x509, NID_ct_precert_poison, NULL, 1, 0) !=
            1) {
      ...
      if (precert) {
        BIO_printf(
        bio_err, "Warning: Ignoring -precert flag since no cert is produced\n");
      }
      /* Set up V3 context struct */
      ...
      if (addext_conf != NULL &&
          !X509V3_EXT_REQ_add_nconf(addext_conf, &ext_ctx, "default", req)) {
        BIO_printf(bio_err,
...
static int prompt_info(X509_REQ *req, STACK_OF(CONF_VALUE) * dn_sk,
  ...
  if (!batch) {
    BIO_printf(bio_err, "You are about to be asked to enter information that "
  ...
  start:
    for (;;) {
      ...
      if (!add_DN_object(subj, v->value, def, value, nid, n_min, n_max, chtype,
                         mval))
        return 0;
    }
    ...
    start2:
      for (;;) {
        ...
        if (!add_attribute_object(req, v->value, def, value, nid, n_min, n_max,
                                  chtype))
          return 0;
      }
    }
...
#else
      spec_char =
      (*p == os_toascii[':'] || *p == os_toascii[','] || *p == os_toascii['.']);
#endif
      if (spec_char) {
        p++;
        if (*p)
...
static int add_DN_object(X509_NAME *n, char *text, const char *def, char *value,
  ...
  ret = build_data(text, def, value, n_min, n_max, buf, sizeof(buf), "DN value",
                   "DN default");
  if ((ret == 0) || (ret == 1))
    return ret;
  ret = 1;
...
static EVP_PKEY_CTX *set_keygen_ctx(const char *gstr, char **pkeytype,
  ...
  if (param != NULL) {
    ...
    if (keygen_engine != NULL)
      ...
      gctx =
      EVP_PKEY_CTX_new_from_pkey(app_get0_libctx(), param, app_get0_propq());
