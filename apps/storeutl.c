...
static int process(const char *uri, const UI_METHOD *uimeth, PW_CB_DATA *uidata, int expected, int criterion,
                   OSSL_STORE_SEARCH *search, int text, int noout, int recursive, int indent, const char *outfile,
...
opthelp:
    ...
    ret = process(argv[0], get_ui_method(), &pw_cb_data, expected, criterion, search, text, noout, recursive, 0,
                  outfile, prog, libctx);
    ...
    for (;;) {
        ...
        case OSSL_STORE_INFO_NAME:
            if (recursive) {
                ...
                ret += process(suburi, uimeth, uidata, expected, criterion, search, text, noout, recursive, indent + 2,
                               outfile, prog, libctx);
            }
            break;
