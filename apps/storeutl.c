...
static int process(const char *uri, const UI_METHOD *uimeth, PW_CB_DATA *uidata,
                   ...
                   int text, int noout, int recursive, int indent,
                   const char *outfile, const char *prog, OSSL_LIB_CTX *libctx);
...
static int process(const char *uri, const UI_METHOD *uimeth, PW_CB_DATA *uidata,
                   int expected, int criterion, OSSL_STORE_SEARCH *search,
                   int text, int noout, int recursive, int indent,
                   const char *outfile, const char *prog,
                   OSSL_LIB_CTX *libctx) {
  OSSL_STORE_CTX *store_ctx = NULL;
  int ret = 1, items = 0;

  if ((store_ctx = OSSL_STORE_open_ex(uri, libctx, app_get0_propq(), uimeth,
                                      uidata, NULL, NULL, NULL)) == NULL) {
  ...
  for (;;) {
    ...
    if (info == NULL) {
      ...
      BIO_printf(bio_err, "ERROR: OSSL_STORE_load() returned NULL without "
    ...
    case OSSL_STORE_INFO_NAME:
      if (recursive) {
        ...
        ret +=
        process(suburi, uimeth, uidata, expected, criterion, search, text,
                noout, recursive, indent + 2, outfile, prog, libctx);
      }
      break;
    ...
    case OSSL_STORE_INFO_PKEY:
      ...
      if (!noout)
        PEM_write_bio_PrivateKey(out, OSSL_STORE_INFO_get0_PKEY(info), NULL,
                                 NULL, 0, NULL, NULL);
      break;
