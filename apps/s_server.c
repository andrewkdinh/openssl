...
static int get_ocsp_resp_from_responder(SSL *s, tlsextstatusctx *srctx,
  ...
  if (aia != NULL) {
    if (!OSSL_HTTP_parse_url(sk_OPENSSL_STRING_value(aia, 0), &use_ssl, NULL,
                             &host, &port, NULL, &path, NULL, NULL)) {
  ...
  if (id == NULL) {
    ...
    if (inctx == NULL)
      goto err;
    if (!X509_STORE_CTX_init(inctx, SSL_CTX_get_cert_store(ssl_ctx), NULL,
                             NULL))
      goto err;
  ...
  *resp = process_responder(req, host, port, path, proxy, no_proxy, use_ssl,
                            NULL /* headers */, srctx->timeout);
  if (*resp == NULL) {
...
int s_server_main(int argc, char *argv[]) {
  ...
  if (use_zc_sendfile && !use_sendfile) {
    BIO_printf(bio_out, "Warning: -zerocopy_sendfile depends on -sendfile, "
                        "enabling -sendfile now.\n");
    use_sendfile = 1;
  }
  ...
  if (nocert == 0) {
    ...
    s_cert =
    load_cert_pass(s_cert_file, s_cert_format, 1, pass, "server certificate");
  ...
  if (ctx2 != NULL) {
    ...
    if (!ctx_set_verify_locations(ctx2, CAfile, noCAfile, CApath, noCApath,
                                  CAstore, noCAstore)) {
      ERR_print_errors(bio_err);
      goto end;
    }
  ...
  if (!no_dhe) {
    ...
    if (ctx2 != NULL) {
      if (dhfile != NULL) {
        ...
        s_cert_file2, FORMAT_UNDEF, 0, "DH", "DH parameters", 1);
...
static long int count_reads_callback(BIO *bio, int cmd, const char *argp,
  ...
  if (s_debug) {
    ...
    ret =
    (int)bio_dump_callback(bio, cmd, argp, len, argi, argl, ret, processed);
  ...
  for (;;) {
    ...
    if (read_from_terminal) {
      ...
      if (!s_quiet && !s_brief) {
        ...
        if ((buf[0] == 'K' || buf[0] == 'k') &&
            ((buf[1] == '\n') || (buf[1] == '\r'))) {
          SSL_key_update(con, buf[0] == 'K' ? SSL_KEY_UPDATE_REQUESTED
