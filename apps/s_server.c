...
int s_server_main(int argc, char *argv[]) {
    ...
    while ((o = opt_next()) != OPT_EOF) {
        ...
        case OPT_ACCEPT:
            ...
            if (BIO_parse_hostserv(opt_arg(), &host, &port,
                                   BIO_PARSE_PRIO_SERV) < 1) {
                BIO_printf(
                bio_err, "%s: -accept argument malformed or ambiguous\n", port);
                goto end;
            }
            break;
#ifdef AF_UNIX
    ...
    if (use_sendfile && enable_ktls == 0) {
        BIO_printf(
        bio_out, "Warning: -sendfile depends on -ktls, enabling -ktls now.\n");
        enable_ktls = 1;
    }
    ...
    if (!no_dhe) {
        ...
        if (dhfile != NULL)
            dhpkey =
            load_keyparams(dhfile, FORMAT_UNDEF, 0, "DH", "DH parameters");
...
static int generate_session_id(SSL *ssl, unsigned char *id,
    ...
    do {
        ...
        memcpy(id, session_id_prefix,
               (session_id_prefix_len < *id_len) ? session_id_prefix_len
                                                 : *id_len);
