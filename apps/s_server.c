...
static int psk_find_session_cb(SSL *ssl, const unsigned char *identity, size_t identity_len, SSL_SESSION **sess)
{
    ...
    if (tmpsess == NULL || !SSL_SESSION_set1_master_key(tmpsess, key, key_len)
...
static const BIO_METHOD *BIO_f_ebcdic_filter(void)
{
    if (methods_ebcdic == NULL) {
        ...
        if (methods_ebcdic == NULL || !BIO_meth_set_write(methods_ebcdic, ebcdic_write)
            || !BIO_meth_set_read(methods_ebcdic, ebcdic_read) || !BIO_meth_set_puts(methods_ebcdic, ebcdic_puts)
            || !BIO_meth_set_gets(methods_ebcdic, ebcdic_gets) || !BIO_meth_set_ctrl(methods_ebcdic, ebcdic_ctrl)
            || !BIO_meth_set_create(methods_ebcdic, ebcdic_new) || !BIO_meth_set_destroy(methods_ebcdic, ebcdic_free))
            return NULL;
    }
...
static int get_ocsp_resp_from_responder(SSL *s, tlsextstatusctx *srctx, OCSP_RESPONSE **resp)
{
    ...
    if (aia != NULL) {
        if (!OSSL_HTTP_parse_url(sk_OPENSSL_STRING_value(aia, 0), &use_ssl, NULL, &host, &port, NULL, &path, NULL,
                                 NULL)) {
    ...
    if (id == NULL) {
        ...
        if (inctx == NULL)
            goto err;
        if (!X509_STORE_CTX_init(inctx, SSL_CTX_get_cert_store(ssl_ctx), NULL, NULL))
            goto err;
...
static int alpn_cb(SSL *s, const unsigned char **out, unsigned char *outlen, const unsigned char *in,
...
# endif
    ...
    OPT_SECTION("Network"),
    {"port", OPT_PORT, 'p', "TCP/IP port to listen on for connections (default is " PORT ")"},
...
# endif
    ...
    {"ext_cache", OPT_EXT_CACHE, '-', "Disable internal cache, set up and use external cache"},
...
# ifndef OPENSSL_NO_OCSP
    ...
    {"status_timeout", OPT_STATUS_TIMEOUT, 'n', "Status request responder timeout"},
...
# endif
    ...
    OPT_SECTION("Debug"),
    {"security_debug", OPT_SECURITY_DEBUG, '-', "Print output from SSL/TLS security framework"},
    ...
    {"rev", OPT_REV, '-', "act as an echo server that sends back received text reversed"},
...
# endif
    ...
    {"max_early_data", OPT_MAX_EARLY, 'n', "The maximum number of bytes of early data as advertised in tickets"},
...
# ifndef OPENSSL_NO_DTLS
    ...
    {"listen", OPT_LISTEN, '-', "Listen for a DTLS ClientHello with a cookie and then connect"},
# endif
...
# ifndef OPENSSL_NO_OCSP
            s_tlsextstatus = 1;
            if (!OSSL_HTTP_parse_url(opt_arg(), &tlscstatp.use_ssl, NULL, &tlscstatp.host, &tlscstatp.port, NULL,
...
# endif
    if (early_data && rev) {
        BIO_printf(bio_err, "Can't use -early_data in combination with -rev\n");
        goto end;
    }
...
# endif
    ...
    if (!SSL_CTX_set_session_id_context(ctx, (void *)&s_server_session_id_context,
    ...
    if (ctx2 != NULL) {
        ...
        if (!SSL_CTX_set_session_id_context(ctx2, (void *)&s_server_session_id_context,
                                            sizeof(s_server_session_id_context))) {
            BIO_printf(bio_err, "error setting session id context\n");
            ERR_print_errors(bio_err);
            goto end;
...
# endif
        ...
        /* else we have data */
        if ((www == 1 && HAS_PREFIX(buf, "GET ")) || (www == 2 && HAS_PREFIX(buf, "GET /stats "))) {
            X509 *peer = NULL;
    ...
    *do_copy = 0;
    for (sess = first; sess; sess = sess->next) {
        if (idlen == (int)sess->idlen && !memcmp(sess->id, id, idlen)) {
            ...
            return d2i_SSL_SESSION_ex(NULL, &p, sess->derlen, app_get0_libctx(), app_get0_propq());
        }
    }
