...
# endif
    ...
    if (cipher != NULL) {
        ...
        if (rawkey_set > 0 && skeyopts != NULL) {
            BIO_printf(
                bio_err,
                "Either a raw key or the 'skeyopt' args must be used.\n");
            goto end;
        }
        ...
        if (rawkey_set) {
            ...
            skey = EVP_SKEY_import(
                app_get0_libctx(), EVP_SKEYMGMT_get0_name(mgmt),
        ...
        if (printkey) {
            if (!nosalt) {
                printf("salt=");
                for (i = 0; i < (int)saltlen; i++) printf("%02X", salt[i]);
                printf("\n");
            }
