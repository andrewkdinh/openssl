  ...
  if ((str == NULL) && (cipher != NULL) && (hkey == NULL) &&
    ...
    if (1) {
      ...
      for (;;) {
        ...
        BIO_snprintf(prompt, sizeof(prompt),
                     "enter %s %s password:", EVP_CIPHER_get0_name(cipher),
  ...
  if (cipher != NULL) {
    if (str != NULL) { /* a passphrase is available */
      ...
      if (pbkdf2 == 1) {
        ...
        if (!EVP_BytesToKey(cipher, dgst, sptr, (unsigned char *)str, str_len,
                            1, key, iv)) {
    ...
    if (rawkey_set) {
      ...
      app_get0_libctx(), skeymgmt != NULL ? skeymgmt : EVP_CIPHER_name(cipher),
      app_get0_propq());
      if (mgmt == NULL)
        goto end;
