...
static int check(X509_STORE *ctx, const char *file, STACK_OF(X509) * uchain,
                 ...
                 int show_chain, STACK_OF(OPENSSL_STRING) * opts);
...
int verify_main(int argc, char **argv) {
    ...
    if (trusted != NULL &&
        ...
        BIO_printf(
        bio_err, "%s: Cannot use -trusted with -CAfile, -CApath or -CAstore\n",
        prog);
        goto end;
    }
    ...
    ret = 0;
    if (argc < 1) {
        if (check(store, NULL, untrusted, trusted, crls, show_chain, vfyopts) !=
            1)
            ret = -1;
    } else {
        for (i = 0; i < argc; i++)
