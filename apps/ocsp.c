...
int ocsp_main(int argc, char **argv) {
    ...
    while ((o = opt_next()) != OPT_EOF) {
        ...
        case OPT_MD:
            if (trailing_md) {
                BIO_printf(
                bio_err, "%s: Digest must be before -cert or -serial\n", prog);
                goto opthelp;
            }
...
redo_accept:
    ...
    if (signfile != NULL) {
        ...
        key =
        load_key(keyfile, FORMAT_UNDEF, 0, NULL, NULL, "signer private key");
        if (key == NULL)
            goto end;
...
OCSP_RESPONSE *process_responder(OCSP_REQUEST *req, const char *host,
    ...
    "application/ocsp-response", req_timeout, ASN1_ITEM_rptr(OCSP_RESPONSE));
