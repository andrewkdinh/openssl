...
static int print_ocsp_summary(BIO *out, OCSP_BASICRESP *bs, OCSP_REQUEST *req,
                              ...
                              long maxage);
static void
make_ocsp_response(BIO *err, OCSP_RESPONSE **resp, OCSP_REQUEST *req, CA_DB *db,
                   STACK_OF(X509) * ca, X509 *rcert, EVP_PKEY *rkey,
                   const EVP_MD *md, STACK_OF(OPENSSL_STRING) * sigopts,
                   STACK_OF(X509) * rother, unsigned long flags, int nmin,
                   int ndays, int badsig, const EVP_MD *resp_md);
...
int ocsp_main(int argc, char **argv) {
  ...
  while ((o = opt_next()) != OPT_EOF) {
    ...
    case OPT_URL:
      ...
      if (!OSSL_HTTP_parse_url(opt_arg(), &use_ssl, NULL /* userinfo */, &host,
  ...
  if (rsignfile != NULL) {
    ...
    rkey =
    load_key(rkeyfile, FORMAT_UNDEF, 0, passin, NULL, "responder private key");
    if (rkey == NULL)
      goto end;
  }
    ...
    if (index_changed(rdb)) {
      ...
      if (newrdb != NULL && index_index(newrdb) > 0) {
        ...
        trace_log_message(-1, prog, LOG_ERR,
                          "error reloading updated index: %s", ridx_filename);
      }
    }
#endif
...
done_resp:
  ...
  if (store == NULL) {
    store =
    setup_verify(CAfile, noCAfile, CApath, noCApath, CAstore, noCAstore);
    if (!store)
      goto end;
  }
  if (vpmtouched)
...
static int index_changed(CA_DB *rdb) {
  ...
  if (rdb != NULL && stat(rdb->dbfname, &sb) != -1) {
    if (rdb->dbst.st_mtime != sb.st_mtime ||
        rdb->dbst.st_ctime != sb.st_ctime || rdb->dbst.st_ino != sb.st_ino ||
...
make_ocsp_response(BIO *err, OCSP_RESPONSE **resp, OCSP_REQUEST *req, CA_DB *db,
                   STACK_OF(X509) * ca, X509 *rcert, EVP_PKEY *rkey,
                   const EVP_MD *rmd, STACK_OF(OPENSSL_STRING) * sigopts,
                   STACK_OF(X509) * rother, unsigned long flags, int nmin,
                   int ndays, int badsig, const EVP_MD *resp_md) {
  ...
  for (i = 0; i < id_count; i++) {
    ...
    if (inf == NULL) {
      ...
      OCSP_basic_add1_status(bs, cid, V_OCSP_CERTSTATUS_GOOD, 0, NULL, thisupd,
                             nextupd);
      ...
      if (invtm != NULL)
        ...
        OCSP_SINGLERESP_add1_ext_i2d(single, NID_hold_instruction_code, inst, 0,
                                     0);
...
static int do_responder(OCSP_REQUEST **preq, BIO **pcbio, BIO *acbio,
  ...
  ASN1_ITEM_rptr(OCSP_REQUEST), (ASN1_VALUE **)preq, NULL, pcbio, acbio,
...
static int send_ocsp_response(BIO *cbio, const OCSP_RESPONSE *resp) {
#ifndef OPENSSL_NO_SOCK
  return http_server_send_asn1_resp(
  prog, cbio, 0 /* no keep-alive */, "application/ocsp-response",
...
OCSP_RESPONSE *process_responder(OCSP_REQUEST *req, const char *host,
  ...
  host, port, path, proxy, no_proxy, ctx, headers, "application/ocsp-request",
  (ASN1_VALUE *)req, ASN1_ITEM_rptr(OCSP_REQUEST), "application/ocsp-response",
