...
static char *srp_verify_user(const char *user, const char *srp_verifier,
  ...
  if (len > 0) {
    ...
    if ((gNid = SRP_create_verifier(user, password, &srp_usersalt, &verifier, N,
                                    g)) == NULL) {
    ...
    if (mode == OPT_LIST) {
      ...
      if (userindex >= 0) {
        ...
        if ((row[DB_srpid] == NULL) || (row[DB_srpgN] == NULL) ||
            (row[DB_srptype] == NULL) || (row[DB_srpverifier] == NULL) ||
            (row[DB_srpsalt] == NULL) ||
            (userinfo &&
      ...
      if (userindex < 0) {
        ...
        if (type == 'v') {
          ...
          if (!(gNid = srp_create_user(
                ...
                gNrow ? gNrow[DB_srpverifier] : NULL, passout, verbose))) {
            BIO_printf(bio_err,
                       "Cannot create srp verifier for user \"%s\", operation "
                       "abandoned.\n",
                       user);
            errors++;
            goto end;
