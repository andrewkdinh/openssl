...
static void warn_copying(ASN1_OBJECT *excluded, const char *names) {
    ...
    if (names != NULL && strstr(names, sn) != NULL)
        BIO_printf(
        bio_err,
        "Warning: -ext should not specify copying %s extension to CSR; "
        "ignoring this\n",
        sn);
}
...
int x509_main(int argc, char **argv) {
    ...
    if (preserve_dates && not_before != NULL) {
        BIO_printf(bio_err,
                   "Cannot use -preserve_dates with -not_before option\n");
        goto err;
    }
    if (preserve_dates && not_after != NULL) {
        BIO_printf(bio_err,
                   "Cannot use -preserve_dates with -not_after option\n");
        goto err;
    }
    ...
    if (newcert) {
        ...
        if (privkeyfile == NULL && pubkeyfile == NULL) {
            BIO_printf(bio_err, "The -new option requires using the -key or "
                                "-force_pubkey option\n");
            goto err;
        }
    }
...
#define WARN_NO_CA(opt)                                                        \
    BIO_printf(bio_err, "Warning: ignoring " opt                               \
                        " option since -CA option is not given\n");
        if (CAkeyfile != NULL)
            WARN_NO_CA("-CAkey");
    ...
    if (extfile == NULL) {
        if (extsect != NULL)
            BIO_printf(
            bio_err, "Warning: ignoring -extensions option without -extfile\n");
    } else {
        X509V3_CTX ctx2;
    ...
    if (reqfile) {
        if (infile == NULL && isatty(fileno_stdin()))
            BIO_printf(bio_err,
                       "Warning: Reading cert request from stdin since no "
                       "-in option is given\n");
    ...
    if (reqfile || newcert) {
        ...
        if (privkeyfile == NULL && CAkeyfile == NULL) {
            BIO_printf(bio_err,
                       "We need a private key to sign with, use -key or "
                       "-CAkey or -CA with private key\n");
            goto err;
        }
        ...
        if (infile == NULL && isatty(fileno_stdin()))
            BIO_printf(bio_err,
                       "Warning: Reading certificate from stdin since no "
                       "-in or -new option is given\n");
    ...
    if ((fsubj != NULL || req != NULL) &&
        ...
        x, fsubj != NULL ? fsubj : X509_REQ_get_subject_name(req)))
        goto end;
    ...
    if (reqfile || newcert || privkey != NULL || CAfile != NULL) {
        if (!preserve_dates &&
            !set_cert_times(x, not_before, not_after, days, 1))
            goto end;
        if (fissu != NULL) {
    ...
    if (x509toreq) { /* also works in conjunction with -req */
        ...
        if ((CAkey = load_key(CAkeyfile, CAkeyformat, 0, passin, e,
                              "CA private key")) == NULL)
            goto end;
