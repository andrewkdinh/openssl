...
static int get_peer_public_key(PEER_DATA *peer, OSSL_LIB_CTX *libctx) {
    ...
    ret = EVP_PKEY_fromdata_init(ctx) > 0 &&
          (EVP_PKEY_fromdata(ctx, &peer->pub, EVP_PKEY_PUBLIC_KEY, params) > 0);
...
static int generate_secret(PEER_DATA *peerA, EVP_PKEY *peerBpub,
     ...
     *   params[0] =
     * OSSL_PARAM_construct_utf8_string(OSSL_EXCHANGE_PARAM_KDF_TYPE, "X963KDF",
     * 0); params[1] =
     * OSSL_PARAM_construct_utf8_string(OSSL_EXCHANGE_PARAM_KDF_DIGEST,
     * "SHA256", 0); params[2] =
     * OSSL_PARAM_construct_size_t(OSSL_EXCHANGE_PARAM_KDF_OUTLEN, &outlen);
     *   params[3] =
     * OSSL_PARAM_construct_octet_string(OSSL_EXCHANGE_PARAM_KDF_UKM, ukm,
     * sizeof(ukm)); params[4] = OSSL_PARAM_construct_end(); if
     * (!EVP_PKEY_CTX_set_params(derivectx, params)) goto cleanup;
     *
