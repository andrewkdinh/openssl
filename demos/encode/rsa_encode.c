 ...
 * supplied passphrase.
 */
static EVP_PKEY *load_key(OSSL_LIB_CTX *libctx, FILE *f,
     ...
     */
    dctx = OSSL_DECODER_CTX_new_for_pkey(&pkey, "PEM", NULL, "RSA", selection,
                                         libctx, propq);
    if (dctx == NULL) {
    ...
    if (passphrase != NULL) {
        if (OSSL_DECODER_CTX_set_passphrase(
            dctx, (const unsigned char *)passphrase, strlen(passphrase)) == 0) {
...
static int store_key(EVP_PKEY *pkey, FILE *f, const char *passphrase) {
    ...
    if (passphrase != NULL) {
        ...
        if (OSSL_ENCODER_CTX_set_passphrase(
            ectx, (const unsigned char *)passphrase, strlen(passphrase)) == 0) {
