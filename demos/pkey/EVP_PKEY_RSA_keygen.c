 ...
 */
static EVP_PKEY *generate_rsa_key_long(OSSL_LIB_CTX *libctx,
                                       unsigned int bits) {
 ...
 */
static EVP_PKEY *generate_rsa_key_short(OSSL_LIB_CTX *libctx,
                                        unsigned int bits) {
    EVP_PKEY *pkey = NULL;
...
static int dump_key(const EVP_PKEY *pkey) {
    ...
    /*
     * Retrieve value of the second prime factor, commonly known as q. This
     * value is secret and forms part of the private key. It must not be
     * published.
     *
