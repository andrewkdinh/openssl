...
int main(int argc, char **argv) {
   ...
   */
  params[0] =
  OSSL_PARAM_construct_utf8_string(OSSL_PKEY_PARAM_FFC_TYPE, "fips186_4", 0);
  ...
  params[3] =
  OSSL_PARAM_construct_int(OSSL_PKEY_PARAM_FFC_PCOUNTER, (int *)&pcounter);
  params[4] = OSSL_PARAM_construct_end();

  /* generate a new key that is the combination of the existing key and the new
   * params */
  ctx1 = create_merged_key(dsaparamskey, params, libctx, propq);
  if (ctx1 == NULL)
    goto cleanup;
  /* This will fail since not all the parameters used for key generation are
   * added */
