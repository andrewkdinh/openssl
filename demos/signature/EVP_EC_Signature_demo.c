...
static EVP_PKEY *get_key(OSSL_LIB_CTX *libctx, const char *propq, int public)
{
    ...
    dctx = OSSL_DECODER_CTX_new_for_pkey(&pkey, "DER", NULL, "EC", selection,
                                         libctx, propq);
                     ...
                     size_t *sig_out_len, unsigned char **sig_out_value)
{
     ...
     * sign provider.
     */
    if (!EVP_DigestSignInit_ex(sign_context, NULL, sig_name, libctx, NULL,
                               priv_key, NULL)) {
                       ...
                       size_t sig_len, unsigned char *sig_value)
{
    ...
    /* Verify */
    if (!EVP_DigestVerifyInit_ex(verify_context, NULL, sig_name, libctx, NULL,
                                 pub_key, NULL)) {
