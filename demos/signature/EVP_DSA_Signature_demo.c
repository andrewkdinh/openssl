...
static int generate_dsa_key(OSSL_LIB_CTX *libctx, EVP_PKEY *params,
                            EVP_PKEY **p_pkey)
{
    int ret = 0;
...
static int demo_sign(OSSL_LIB_CTX *libctx, size_t *p_sig_len,
                     unsigned char **p_sig_value, OSSL_PARAM keypair[])
{
    int ret = 0;
    size_t sig_len = 0;
...
static int demo_verify(OSSL_LIB_CTX *libctx, size_t sig_len,
                       unsigned char *sig_value, OSSL_PARAM public_key[])
{
    int ret = 0;
    EVP_MD_CTX *ctx = NULL;
    EVP_PKEY_CTX *pkey_ctx = NULL;
    ...
    if (EVP_PKEY_fromdata_init(pkey_ctx) != 1)
        goto end;
    if (EVP_PKEY_fromdata(pkey_ctx, &pkey, EVP_PKEY_PUBLIC_KEY, public_key)
        != 1)
        goto end;
    ...
    if (EVP_DigestVerifyInit_ex(ctx, NULL, DIGEST, libctx, NULL, pkey, NULL)
        != 1)
        goto end;
