...
static OSSL_DEMO_H3_STREAM *h3_conn_create_stream(OSSL_DEMO_H3_CONN *conn, int type)
{
    ...
    if ((s->s = SSL_new_stream(conn->qconn, flags)) == NULL) {
        ERR_raise_data(ERR_LIB_USER, ERR_R_INTERNAL_ERROR, "could not create QUIC stream object");
        goto err;
    }
...
static int h3_conn_recv_data(nghttp3_conn *h3conn, int64_t stream_id, const uint8_t *data, size_t datalen,
                             void *conn_user_data, void *stream_user_data)
{
    ...
    return conn->recv_data_cb(h3conn, stream_id, data, datalen, conn_user_data, stream_user_data);
}

static int h3_conn_stream_close(nghttp3_conn *h3conn, int64_t stream_id, uint64_t app_error_code, void *conn_user_data,
                                void *stream_user_data)
{
    ...
    if (conn->stream_close_cb != NULL)
        ret = conn->stream_close_cb(h3conn, stream_id, app_error_code, conn_user_data, stream_user_data);

    h3_conn_remove_stream(conn, stream);
    return ret;
}

static int h3_conn_stop_sending(nghttp3_conn *h3conn, int64_t stream_id, uint64_t app_error_code, void *conn_user_data,
                                void *stream_user_data)
{
    int ret = 0;
    OSSL_DEMO_H3_CONN *conn = conn_user_data;
    ...
    if (conn->stop_sending_cb != NULL)
        ret = conn->stop_sending_cb(h3conn, stream_id, app_error_code, conn_user_data, stream_user_data);
...
static int h3_conn_reset_stream(nghttp3_conn *h3conn, int64_t stream_id, uint64_t app_error_code, void *conn_user_data,
                                void *stream_user_data)
{
    int ret = 0;
    OSSL_DEMO_H3_CONN *conn = conn_user_data;
    ...
    if (conn->reset_stream_cb != NULL)
        ret = conn->reset_stream_cb(h3conn, stream_id, app_error_code, conn_user_data, stream_user_data);
...
static int h3_conn_deferred_consume(nghttp3_conn *h3conn, int64_t stream_id, size_t consumed, void *conn_user_data,
                                    void *stream_user_data)
{
    int ret = 0;
    OSSL_DEMO_H3_CONN *conn = conn_user_data;
    ...
    if (conn->deferred_consume_cb != NULL)
        ret = conn->deferred_consume_cb(h3conn, stream_id, consumed, conn_user_data, stream_user_data);

    conn->consumed_app_data += consumed;
    return ret;
}

OSSL_DEMO_H3_CONN *OSSL_DEMO_H3_CONN_new_for_conn(BIO *qconn_bio, const nghttp3_callbacks *callbacks,
                                                  const nghttp3_settings *settings, void *user_data)
{
    ...
    if (qconn_bio == NULL) {
        ERR_raise_data(ERR_LIB_USER, ERR_R_PASSED_NULL_PARAMETER, "QUIC connection BIO must be provided");
        return NULL;
    }
    ...
    if (BIO_get_ssl(qconn_bio, &conn->qconn) == 0) {
        ERR_raise_data(ERR_LIB_USER, ERR_R_PASSED_INVALID_ARGUMENT, "BIO must be an SSL BIO");
        goto err;
    }
    ...
    if (SSL_in_before(conn->qconn))
        if (SSL_set_alpn_protos(conn->qconn, alpn, sizeof(alpn))) {
            ...
            ERR_raise_data(ERR_LIB_USER, ERR_R_INTERNAL_ERROR, "failed to configure ALPN");
            goto err;
        }
    ...
    if (!SSL_set_default_stream_mode(conn->qconn, SSL_DEFAULT_STREAM_MODE_NONE)) {
        ERR_raise_data(ERR_LIB_USER, ERR_R_INTERNAL_ERROR, "failed to configure default stream mode");
        goto err;
    }
     ...
     */
    if ((s_ctl_send = h3_conn_create_stream(conn, OSSL_DEMO_H3_STREAM_TYPE_CTRL_SEND)) == NULL)
        goto err;

    if ((s_qpenc_send = h3_conn_create_stream(conn, OSSL_DEMO_H3_STREAM_TYPE_QPACK_ENC_SEND)) == NULL)
        goto err;

    if ((s_qpdec_send = h3_conn_create_stream(conn, OSSL_DEMO_H3_STREAM_TYPE_QPACK_DEC_SEND)) == NULL)
        goto err;
    ...
    if (ec < 0) {
        ERR_raise_data(ERR_LIB_USER, ERR_R_INTERNAL_ERROR, "cannot create nghttp3 connection: %s (%d)",
    ...
    if (ec < 0) {
        ERR_raise_data(ERR_LIB_USER, ERR_R_INTERNAL_ERROR, "cannot bind nghttp3 control stream: %s (%d)",
                       nghttp3_strerror(ec), ec);
        goto err;
    }

    ec = nghttp3_conn_bind_qpack_streams(conn->h3conn, s_qpenc_send->id, s_qpdec_send->id);
    if (ec < 0) {
        ERR_raise_data(ERR_LIB_USER, ERR_R_INTERNAL_ERROR, "cannot bind nghttp3 QPACK streams: %s (%d)",
                       nghttp3_strerror(ec), ec);
        goto err;
    }
...
OSSL_DEMO_H3_CONN *OSSL_DEMO_H3_CONN_new_for_addr(SSL_CTX *ctx, const char *addr, const nghttp3_callbacks *callbacks,
                                                  const nghttp3_settings *settings, void *user_data)
{
    BIO *qconn_bio = NULL;
    SSL *qconn = NULL;
    ...
    conn = OSSL_DEMO_H3_CONN_new_for_conn(qconn_bio, callbacks, settings, user_data);
    if (conn == NULL)
        goto err;
...
static void h3_conn_pump_stream(OSSL_DEMO_H3_STREAM *s, void *conn_)
{
    ...
    for (;;) {
            ...
            if (ec <= 0) {
                ...
                if (SSL_get_error(s->s, ec) == SSL_ERROR_ZERO_RETURN) {
                    ...
                    ec = nghttp3_conn_read_stream(conn->h3conn, s->id, NULL, 0,
                                                  /*fin=*/1);
                    if (ec < 0) {
                        ERR_raise_data(ERR_LIB_USER, ERR_R_INTERNAL_ERROR, "cannot pass FIN to nghttp3: %s (%d)",
                                       nghttp3_strerror(ec), ec);
                        goto err;
                    }
                    ...
                    if (ec < 0) {
                        ERR_raise_data(ERR_LIB_USER, ERR_R_INTERNAL_ERROR, "cannot mark stream as reset: %s (%d)",
                                       nghttp3_strerror(ec), ec);
                        goto err;
                    }
        ...
        ec = nghttp3_conn_read_stream(conn->h3conn, s->id, s->buf + s->buf_cur, s->buf_total - s->buf_cur, /*fin=*/0);
        if (ec < 0) {
            ERR_raise_data(ERR_LIB_USER, ERR_R_INTERNAL_ERROR, "nghttp3 failed to process incoming data: %s (%d)",
                           nghttp3_strerror(ec), ec);
            goto err;
        }
...
int OSSL_DEMO_H3_CONN_handle_events(OSSL_DEMO_H3_CONN *conn)
{
    ...
    for (;;) {
         ...
         */
        ec = nghttp3_conn_writev_stream(conn->h3conn, &stream_id, &fin, vecs, ARRAY_LEN(vecs));
        if (ec < 0)
            return 0;
        if (ec == 0)
            break;
	 ...
	 * after all data are written.
        ...
        if ((s = lh_OSSL_DEMO_H3_STREAM_retrieve(conn->streams, &key)) == NULL) {
            ERR_raise_data(ERR_LIB_USER, ERR_R_INTERNAL_ERROR, "no stream for ID %zd", stream_id);
            return 0;
        }
        ...
        for (i = 0; i < num_vecs; ++i) {
            ...
            if (s->s == NULL) {
                ...
                if (SSL_get_error(s->s, 0) == SSL_ERROR_WANT_WRITE) {
                    ...
                    ERR_raise_data(ERR_LIB_USER, ERR_R_INTERNAL_ERROR, "writing HTTP/3 data to network failed");
                    return 0;
                }
            } else {
                /*
...
int OSSL_DEMO_H3_CONN_submit_request(OSSL_DEMO_H3_CONN *conn, const nghttp3_nv *nva, size_t nvlen,
                                     const nghttp3_data_reader *dr, void *user_data)
{
    int ec;
    OSSL_DEMO_H3_STREAM *s_req = NULL;

    if (conn == NULL) {
        ERR_raise_data(ERR_LIB_USER, ERR_R_PASSED_NULL_PARAMETER, "connection must be specified");
        return 0;
    }
    ...
    ec = nghttp3_conn_submit_request(conn->h3conn, s_req->id, nva, nvlen, dr, s_req);
    if (ec < 0) {
        ERR_raise_data(ERR_LIB_USER, ERR_R_INTERNAL_ERROR, "cannot submit HTTP/3 request: %s (%d)",
                       nghttp3_strerror(ec), ec);
        goto err;
    }
