...
static int h3_conn_recv_data(nghttp3_conn  *h3conn,
                             int64_t        stream_id,
                             const uint8_t *data,
                             size_t         datalen,
                             void          *conn_user_data,
...
static int h3_conn_stream_close(nghttp3_conn *h3conn,
                                int64_t       stream_id,
...
static int h3_conn_stop_sending(nghttp3_conn *h3conn,
                                int64_t       stream_id,
                                uint64_t      app_error_code,
                                void         *conn_user_data,
                                void         *stream_user_data)
...
static int h3_conn_reset_stream(nghttp3_conn *h3conn,
                                int64_t       stream_id,
                                uint64_t      app_error_code,
                                void         *conn_user_data,
                                void         *stream_user_data)
...
static int h3_conn_deferred_consume(nghttp3_conn *h3conn,
                                    int64_t       stream_id,
                                    size_t        consumed,
                                    void         *conn_user_data,
                                    void         *stream_user_data)
        ...
        if (s->buf_cur == s->buf_total) {
            ...
            if (ec <= 0) {
                ...
                if (SSL_get_error(s->s, ec) == SSL_ERROR_ZERO_RETURN) {
                    ...
                    ec = nghttp3_conn_read_stream(conn->h3conn,
                                                  s->id,
                                                  NULL,
                                                  0,
                                                  /*fin=*/1);
