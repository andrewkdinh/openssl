    ...
    const nghttp3_settings *settings, void *user_data)
{
    ...
    if ((conn->streams =
             lh_OSSL_DEMO_H3_STREAM_new(h3_stream_hash, h3_stream_eq))
        == NULL)
        goto err;
    ...
    if ((s_qpdec_send = h3_conn_create_stream(
             conn, OSSL_DEMO_H3_STREAM_TYPE_QPACK_DEC_SEND))
        == NULL)
        goto err;
    ...
    const nghttp3_settings *settings, void *user_data)
{
    ...
    conn = OSSL_DEMO_H3_CONN_new_for_conn(qconn_bio, callbacks, settings,
                                          user_data);
    if (conn == NULL)
        goto err;
	 ...
	 * after all data are written.
        ...
        for (i = 0; i < num_vecs; ++i) {
            ...
            if (written > 0) {
                 ...
                 */
                ec = nghttp3_conn_add_write_offset(conn->h3conn, stream_id,
                                                   written);
                if (ec < 0)
                    return 0;
                 ...
                 */
                ec = nghttp3_conn_add_ack_offset(conn->h3conn, stream_id,
                                                 written);
                if (ec < 0)
                    return 0;
            }
                                     ...
                                     void *user_data)
{
    ...
    ec = nghttp3_conn_submit_request(conn->h3conn, s_req->id, nva, nvlen, dr,
                                     s_req);
    if (ec < 0) {
        ERR_raise_data(ERR_LIB_USER, ERR_R_INTERNAL_ERROR,
