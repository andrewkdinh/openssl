...
static int h3_stream_eq(const OSSL_DEMO_H3_STREAM *a,
                        const OSSL_DEMO_H3_STREAM *b) {
    if (a->id < b->id)
        return -1;
    if (a->id > b->id)
        return 1;
...
static OSSL_DEMO_H3_STREAM *h3_conn_create_stream(OSSL_DEMO_H3_CONN *conn,
                                                  int type) {
...
static OSSL_DEMO_H3_STREAM *h3_conn_accept_stream(OSSL_DEMO_H3_CONN *conn,
                                                  SSL *qstream) {
    OSSL_DEMO_H3_STREAM *s;
...
static void h3_conn_remove_stream(OSSL_DEMO_H3_CONN *conn,
                                  OSSL_DEMO_H3_STREAM *s) {
    if (s == NULL)
        return;
...
static int h3_conn_recv_data(nghttp3_conn *h3conn, int64_t stream_id,
    ...
    return conn->recv_data_cb(h3conn, stream_id, data, datalen, conn_user_data,
                              stream_user_data);
}
...
const nghttp3_settings *settings, void *user_data) {
    ...
    if ((conn->streams =
         lh_OSSL_DEMO_H3_STREAM_new(h3_stream_hash, h3_stream_eq)) == NULL)
        goto err;
     ...
     */
    if (!SSL_set_default_stream_mode(conn->qconn,
     ...
     */
    if ((s_ctl_send = h3_conn_create_stream(
         conn, OSSL_DEMO_H3_STREAM_TYPE_CTRL_SEND)) == NULL)
        goto err;

    if ((s_qpenc_send = h3_conn_create_stream(
         conn, OSSL_DEMO_H3_STREAM_TYPE_QPACK_ENC_SEND)) == NULL)
        goto err;

    if ((s_qpdec_send = h3_conn_create_stream(
         conn, OSSL_DEMO_H3_STREAM_TYPE_QPACK_DEC_SEND)) == NULL)
        goto err;
    ...
    ec = nghttp3_conn_client_new(&conn->h3conn, &intl_callbacks, settings, NULL,
                                 conn);
    if (ec < 0) {
        ERR_raise_data(ERR_LIB_USER, ERR_R_INTERNAL_ERROR,
    ...
    ec = nghttp3_conn_bind_qpack_streams(conn->h3conn, s_qpenc_send->id,
                                         s_qpdec_send->id);
    if (ec < 0) {
        ERR_raise_data(ERR_LIB_USER, ERR_R_INTERNAL_ERROR,
...
const nghttp3_settings *settings, void *user_data) {
    ...
    conn =
    OSSL_DEMO_H3_CONN_new_for_conn(qconn_bio, callbacks, settings, user_data);
    if (conn == NULL)
        goto err;
...
static void h3_conn_pump_stream(OSSL_DEMO_H3_STREAM *s, void *conn_) {
    ...
    for (;;) {
        if (s->s ==
            NULL /* If we already did STOP_SENDING, ignore this stream. */
...
int OSSL_DEMO_H3_CONN_handle_events(OSSL_DEMO_H3_CONN *conn) {
    ...
    for (;;) {
        if ((snew = SSL_accept_stream(conn->qconn,
                                      SSL_ACCEPT_STREAM_NO_BLOCK)) == NULL)
            break;

        /*
         * Each new incoming stream gets wrapped into an OSSL_DEMO_H3_STREAM
         * object and added into our stream ID map.
         */
    ...
    for (;;) {
         ...
         */
        ec = nghttp3_conn_writev_stream(conn->h3conn, &stream_id, &fin, vecs,
                                        ARRAY_LEN(vecs));
        if (ec < 0)
            return 0;
        if (ec == 0)
        ...
        key.id = stream_id;
        if ((s = lh_OSSL_DEMO_H3_STREAM_retrieve(conn->streams, &key)) ==
            NULL) {
            ERR_raise_data(ERR_LIB_USER, ERR_R_INTERNAL_ERROR,
        ...
        for (i = 0; i < num_vecs; ++i) {
            ...
            if (s->s == NULL) {
                ...
                written = vecs[i].len;
            } else if (!SSL_write_ex2(s->s, vecs[i].base, vecs[i].len, flags,
                                      &written)) {
            ...
            if (written > 0) {
                 ...
                 */
                ec =
                nghttp3_conn_add_write_offset(conn->h3conn, stream_id, written);
                if (ec < 0)
                    return 0;
                 ...
                 */
                ec =
                nghttp3_conn_add_ack_offset(conn->h3conn, stream_id, written);
                if (ec < 0)
                    return 0;
            }
        }
...
int OSSL_DEMO_H3_CONN_submit_request(OSSL_DEMO_H3_CONN *conn,
    ...
    if ((s_req = h3_conn_create_stream(conn, OSSL_DEMO_H3_STREAM_TYPE_REQ)) ==
        NULL)
        goto err;
    ...
    ec =
    nghttp3_conn_submit_request(conn->h3conn, s_req->id, nva, nvlen, dr, s_req);
    if (ec < 0) {
        ERR_raise_data(ERR_LIB_USER, ERR_R_INTERNAL_ERROR,
