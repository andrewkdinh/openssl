...
#define CLIENTUNIOPEN                                                          \
    0x01                  /* unidirectional open by the client (2, 6 and 10)   \
                           */
#define CLIENTCLOSED 0x02 /* closed by the client */
#define CLIENTBIDIOPEN                                                         \
    0x04 /* bidirectional open by the client (something like 0, 4, 8 ...) */
#define SERVERUNIOPEN                                                          \
    0x08                  /* unidirectional open by the server (3, 7 and 11)   \
                           */
...
#define ISCONNECTION                                                           \
    0x80 /* the stream is a connection from SSL_accept_connection() */
...
struct h3ssl {
    ...
    int received_from_two; /* workaround for -607 on nghttp3_conn_read_stream on
                              stream 2 */
    int restart;           /* new request/response cycle started */
    uint64_t
    id_bidi; /* the id of the stream used to read request and send response */
...
static void add_id_status(uint64_t id, SSL *ssl, struct h3ssl *h3ssl,
                          int status) {
    struct ssl_id *ssl_ids;
    int i;
...
static void replace_ids_connection(struct h3ssl *h3ssl, SSL *oldstream,
                                   SSL *newstream) {
    struct ssl_id *ssl_ids;
    int i;
...
static int quic_server_read(nghttp3_conn *h3conn, SSL *stream, uint64_t id,
    ...
    if (r != ret) {
        ...
        if (!nghttp3_err_is_fatal(r)) {
            printf(
            "nghttp3_conn_read_stream used %d of %d (not fatal) on %llu\n", r,
...
static int read_from_ssl_ids(nghttp3_conn **curh3conn, struct h3ssl *h3ssl) {
    ...
    for (i = 0; i < MAXSSL_IDS; i++) {
        if (ssl_ids[i].s != NULL) {
            ...
            item->events =
            UINT64_MAX; /* TODO adjust to the event we need process */
            item->revents =
            UINT64_MAX; /* TODO adjust to the event we need process */
            numitem++;
            item++;
    ...
    for (i = 0, item = items; i < numitem; i++, item++) {
        ...
        if (item->revents & SSL_POLL_EVENT_IC) {
            ...
            if (!SSL_set_incoming_stream_policy(
                ...
                fprintf(stderr,
                        "error while setting inccoming stream policy\n");
                ret = -1;
                goto err;
            }
        ...
        if ((item->revents & SSL_POLL_EVENT_ISB) ||
            ...
            printf("=> Received connection on %lld %d\n",
                   (unsigned long long)new_id, SSL_get_stream_type(stream));
...
static int quic_server_write(struct h3ssl *h3ssl, uint64_t streamid,
    ...
    printf("quic_server_write %lld on %lld (NOT FOUND!)\n",
           (unsigned long long)len, (unsigned long long)streamid);
    return 0;
}
...
static int run_quic_server(SSL_CTX *ctx, int fd) {
    ...
    restart:
        ...
        if (!h3ssl.has_uni) {
            /* time to create those otherwise we can't push anything to the
             * client */
            printf("Create uni\n");
        ...
        if (h3ssl.ldata == 0) {
            ...
            if (strstr(h3ssl.url, ".png"))
                ...
                make_nv(&resp[num_nv++], "content-type",
        ...
        if (nghttp3_conn_submit_response(h3conn, h3ssl.id_bidi, resp, num_nv,
                                         &dr)) {
        ...
        for (;;) {
            ...
            if (sveccnt <= 0) {
                printf(
                "nghttp3_conn_writev_stream done: %ld stream: %llu fin %d\n",
