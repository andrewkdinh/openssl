...
#define OSSL_LIST_FOREACH_FROM(p, name, init)                                  \
  for ((p) = (init); (p) != NULL; (p) = ossl_list_##name##_next(p))
...
#define OSSL_LIST_FOREACH_REV_FROM(p, name, init)                              \
  for ((p) = (init); (p) != NULL; (p) = ossl_list_##name##_prev(p))
...
#define OSSL_LIST_FOREACH_DELSAFE_FROM(p, pn, name, init)                      \
  for ((p) = (init); (p) != NULL && (((pn) = ossl_list_##name##_next(p)), 1);  \
       (p) = (pn))
...
#define OSSL_LIST_FOREACH_REV_DELSAFE_FROM(p, pn, name, init)                  \
  for ((p) = (init); (p) != NULL && (((pn) = ossl_list_##name##_prev(p)), 1);  \
       (p) = (pn))
...
#define DEFINE_LIST_OF_IMPL(name, type)                                        \
  static ossl_unused ossl_inline void ossl_list_##name##_init(                 \
  OSSL_LIST(name) * list) {                                                    \
    memset(list, 0, sizeof(*list));                                            \
  }                                                                            \
  static ossl_unused ossl_inline void ossl_list_##name##_init_elem(            \
  type *elem) {                                                                \
