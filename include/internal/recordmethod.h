 ...
 * There could be some complexity in finding suitable record layer
 * implementations e.g. we need to find one that matches the negotiated
 * protocol, cipher, extensions, etc. The selection_cb approach given above
 * doesn't work so well if unknown third party providers with OSSL_RECORD_METHOD
...
struct ossl_record_method_st {
    ...
    void *rlarg, OSSL_RECORD_LAYER **ret);
    ...
    /*
     * Returns 1 if we have processed data buffered that can be read or 0
     * otherwise
