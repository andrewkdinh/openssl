...
static ossl_inline int ring_buf_write_at(struct ring_buf *r, uint64_t logical_offset, const unsigned char *buf,
                                         size_t buf_len)
{
    size_t avail, idx, l;
    ...
    if (logical_offset < r->ctail_offset
        || safe_add_u64(logical_offset, buf_len, &err) > safe_add_u64(r->head_offset, avail, &err)
        || safe_add_u64(r->head_offset, buf_len, &err) > MAX_OFFSET || err)
        return 0;
...
static ossl_inline size_t ring_buf_push(struct ring_buf *r, const unsigned char *buf, size_t buf_len)
{
...
static ossl_inline const unsigned char *ring_buf_get_ptr(const struct ring_buf *r, uint64_t logical_offset,
                                                         size_t *max_len)
{
    unsigned char *start = r->start;
    size_t idx;
 ...
 */
static ossl_inline int ring_buf_get_buf_at(const struct ring_buf *r, uint64_t logical_offset, const unsigned char **buf,
                                           size_t *buf_len)
{
...
static ossl_inline void ring_buf_cpop_range(struct ring_buf *r, uint64_t start, uint64_t end, int cleanse)
{
    assert(end >= start);
...
static ossl_inline int ring_buf_resize(struct ring_buf *r, size_t num_bytes, int cleanse)
{
