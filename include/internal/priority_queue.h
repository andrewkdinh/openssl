...
#define DEFINE_PRIORITY_QUEUE_OF_INTERNAL(type, ctype)                         \
  ...
  static ossl_unused ossl_inline void ossl_pqueue_##type##_pop_free(           \
  PRIORITY_QUEUE_OF(type) * pq, void (*freefunc)(ctype *)) {                   \
  ...
  static ossl_unused ossl_inline ctype *ossl_pqueue_##type##_remove(           \
  PRIORITY_QUEUE_OF(type) * pq, size_t elem) {                                 \
