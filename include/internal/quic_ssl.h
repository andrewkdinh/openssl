...
__owur int ossl_quic_write_flags(SSL *s, const void *buf, size_t len, uint64_t flags, size_t *written);
...
__owur int ossl_quic_get_event_timeout(SSL *s, struct timeval *tv, int *is_infinite);
...
__owur int ossl_quic_conn_shutdown(SSL *s, uint64_t flags, const SSL_SHUTDOWN_EX_ARGS *args, size_t args_len);
...
__owur int ossl_quic_conn_set_initial_peer_addr(SSL *s, const BIO_ADDR *peer_addr);
...
__owur int ossl_quic_set_incoming_stream_policy(SSL *s, int policy, uint64_t aec);
__owur SSL *ossl_quic_accept_stream(SSL *s, uint64_t flags);
__owur size_t ossl_quic_get_accept_stream_queue_len(SSL *s);
__owur int ossl_quic_get_value_uint(SSL *s, uint32_t class_, uint32_t id, uint64_t *value);
__owur int ossl_quic_set_value_uint(SSL *s, uint32_t class_, uint32_t id, uint64_t value);
...
__owur int ossl_quic_stream_reset(SSL *ssl, const SSL_STREAM_RESET_ARGS *args, size_t args_len);

__owur int ossl_quic_get_stream_read_state(SSL *ssl);
__owur int ossl_quic_get_stream_write_state(SSL *ssl);
__owur int ossl_quic_get_stream_read_error_code(SSL *ssl, uint64_t *app_error_code);
__owur int ossl_quic_get_stream_write_error_code(SSL *ssl, uint64_t *app_error_code);
__owur int ossl_quic_get_conn_close_info(SSL *ssl, SSL_CONN_CLOSE_INFO *info, size_t info_len);
...
int ossl_quic_conn_poll_events(SSL *ssl, uint64_t events, int do_tick, uint64_t *revents);
