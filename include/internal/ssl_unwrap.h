...
#define SSL_CONNECTION_FROM_SSL_ONLY_int(ssl, c)                               \
  ((ssl) == NULL                                                               \
   ? NULL                                                                      \
   : ((ssl)->type == SSL_TYPE_SSL_CONNECTION ? (c SSL_CONNECTION *)(ssl)       \
                                             : NULL))
  ...
  ((ssl) == NULL                                                               \
   ...
   : ((ssl)->type == SSL_TYPE_SSL_CONNECTION                                   \
      ...
      : (SSL_TYPE_IS_QUIC((ssl)->type)                                         \
         ? (c SSL_CONNECTION *)ossl_quic_obj_get0_handshake_layer(             \
           (QUIC_OBJ *)(ssl))                                                  \
         : NULL)))
...
#define QUIC_CONNECTION_FROM_SSL_int(ssl, c)                                   \
  ((ssl) == NULL                                                               \
   ? NULL                                                                      \
   : ((ssl)->type == SSL_TYPE_QUIC_CONNECTION ? (c QUIC_CONNECTION *)(ssl)     \
                                              : NULL))
...
#define QUIC_LISTENER_FROM_SSL_int(ssl, c)                                     \
  ...
  (ssl) == NULL                                                                \
  ? NULL                                                                       \
  : ((ssl)->type == SSL_TYPE_QUIC_LISTENER ? (c QUIC_LISTENER *)(ssl) : NULL))

#define QUIC_DOMAIN_FROM_SSL_int(ssl, c)                                       \
  ((ssl) == NULL                                                               \
   ? NULL                                                                      \
   : ((ssl)->type == SSL_TYPE_QUIC_DOMAIN ? (c QUIC_DOMAIN *)(ssl) : NULL))

#define IS_QUIC_CS(ssl)                                                        \
  ((ssl) != NULL && ((ssl)->type == SSL_TYPE_QUIC_CONNECTION ||                \
                     (ssl)->type == SSL_TYPE_QUIC_XSO))

#define IS_QUIC(ssl) ((ssl) != NULL && SSL_TYPE_IS_QUIC((ssl)->type))
...
#define QUIC_XSO_FROM_CONST_SSL(ssl) QUIC_XSO_FROM_SSL_int(ssl, const)
#define QUIC_LISTENER_FROM_SSL(ssl)                                            \
  QUIC_LISTENER_FROM_SSL_int(ssl, SSL_CONNECTION_NO_CONST)
#define QUIC_LISTENER_FROM_CONST_SSL(ssl) QUIC_LISTENER_FROM_SSL_int(ssl, const)
