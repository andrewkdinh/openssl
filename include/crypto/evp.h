...
#define BLOCK_CIPHER_func_ofb(cname, cprefix, cbits, kstruct, ksched)          \
  static int cname##_ofb_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out,       \
    ...
    if (inl) {                                                                 \
      ...
      in, out, (long)inl, &EVP_C_DATA(kstruct, ctx)->ksched, ctx->iv, &num);   \
      EVP_CIPHER_CTX_set_num(ctx, num);                                        \
    }                                                                          \
    return 1;                                                                  \
  }
...
#define BLOCK_CIPHER_defs(cname, kstruct, nid, block_size, key_len, iv_len,    \
  ...
  BLOCK_CIPHER_def_cfb(cname, kstruct, nid, key_len, iv_len, cbits, flags,     \
                       init_key, cleanup, set_asn1, get_asn1, ctrl)            \
  BLOCK_CIPHER_def_ofb(cname, kstruct, nid, key_len, iv_len, cbits, flags,     \
                       init_key, cleanup, set_asn1, get_asn1, ctrl)            \
...
#define IMPLEMENT_BLOCK_CIPHER(cname, ksched, cprefix, kstruct, nid,           \
  ...
  BLOCK_CIPHER_defs(cname, kstruct, nid, block_size, key_len, iv_len, cbits,   \
                    flags, init_key, cleanup, set_asn1, get_asn1, ctrl)

#define IMPLEMENT_CFBR(cipher, cprefix, kstruct, ksched, keysize, cbits,       \
                       iv_len, fl)                                             \
  BLOCK_CIPHER_func_cfb(cipher##_##keysize, cprefix, cbits, kstruct, ksched)   \
  BLOCK_CIPHER_def_cfb(cipher##_##keysize, kstruct, NID_##cipher##_##keysize,  \
                       keysize / 8, iv_len, cbits,                             \
                       (fl) | EVP_CIPH_FLAG_DEFAULT_ASN1, cipher##_init_key,   \
                       NULL, NULL, NULL, NULL)
