 ...
 *
 * IMPORTANT (QUIC): If buf_len is inadequate to hold the datagram, it is
 * truncated (similar to read(2)). A buffer size of at least 1472 must be used
 * by default to guarantee this does not occur.
 */
 ...
 * TX BIO.
 */
size_t net_tx_avail(APP_CONN *conn) { return BIO_ctrl_pending(conn->net_bio); }

/*
 * These functions returns zero or more of:
 *
 *   POLLIN:    The SSL state machine is interested in socket readability
 * events.
 *
 *   POLLOUT:   The SSL state machine is interested in socket writeability
 * events.
 *
 ...
 * created SSL_CTX.
 */
void teardown_ctx(SSL_CTX *ctx) { SSL_CTX_free(ctx); }
