...
static OSSL_PARAM *fuzz_params(OSSL_PARAM *param, const uint8_t **buf,
    ...
    for (; param != NULL && param->key != NULL; param++) {
        ...
        case OSSL_PARAM_UTF8_STRING:
            if (*use_param &&
                (data_len = read_utf8_string(buf, len, &p_value_utf8_str)) < 0)
                data_len = 0;
            *p = *param;
        ...
        case OSSL_PARAM_OCTET_STRING:
            if (*use_param && (data_len = read_octet_string(
        ...
        case OSSL_PARAM_OCTET_PTR:
            if (*use_param &&
                (data_len = read_octet_ptr(buf, len, &p_value_octet_ptr)) < 0)
                data_len = 0;
            *p = *param;
...
static int do_evp_cipher(const EVP_CIPHER *evp_cipher,
...
static int do_evp_kem(EVP_KEM *evp_kem, const OSSL_PARAM params[]) { return 0; }
...
#define EVP_FUZZ(source, evp, f)                                               \
    do {                                                                       \
        evp *alg =                                                             \
        sk_##evp##_value(source, *algorithm % sk_##evp##_num(source));         \
        OSSL_PARAM *fuzzed_params;                                             \
                                                                               \
        if (alg == NULL)                                                       \
            break;                                                             \
        fuzzed_params =                                                        \
        fuzz_params((OSSL_PARAM *)evp##_settable_ctx_params(alg), &buf, &len); \
...
int FuzzerTestOneInput(const uint8_t *buf, size_t len) {
    ...
    case 9:
        /*
        Implement and call:
        static int do_evp_keymgmt(EVP_KEYMGMT *evp_kdf, const OSSL_PARAM
        params[])
        {
            return 0;
        }
