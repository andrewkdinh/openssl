...
static OSSL_TIME fake_now_cb(void *arg) { return fake_now; }
...
int FuzzerTestOneInput(const uint8_t *buf, size_t len) {
  ...
  if (SSL_set_alpn_protos(client, (const unsigned char *)"\x08ossltest", 9) !=
      0)
    goto end;
  ...
  if (!SSL_set_incoming_stream_policy(client, SSL_INCOMING_STREAM_POLICY_ACCEPT,
                                      0))
    goto end;
