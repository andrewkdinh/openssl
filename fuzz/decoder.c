...
int FuzzerInitialize(int *argc, char ***argv)
{
    ...
    ASN1_PCTX_set_flags(pctx,
                        ASN1_PCTX_FLAGS_SHOW_ABSENT | ASN1_PCTX_FLAGS_SHOW_SEQUENCE | ASN1_PCTX_FLAGS_SHOW_SSOF
...
int FuzzerTestOneInput(const uint8_t *buf, size_t len)
{
    ...
    dctx = OSSL_DECODER_CTX_new_for_pkey(&pkey, NULL, NULL, NULL, 0, NULL, NULL);
    if (dctx == NULL) {
        return 0;
    }
    if (OSSL_DECODER_from_data(dctx, &buf, &len)) {
         ...
         * Skip it.
         */
        if ((!EVP_PKEY_is_a(pkey, "DH") && !EVP_PKEY_is_a(pkey, "DHX")) || EVP_PKEY_get_bits(pkey) <= 2048)
