...
int ossl_prov_set_macctx(EVP_MAC_CTX *macctx, const OSSL_PARAM params[],
  ...
  if (engine != NULL)
    *mp++ =
    OSSL_PARAM_construct_utf8_string(OSSL_ALG_PARAM_ENGINE, (char *)engine, 0);
#endif
...
EVP_MAC_CTX **macctx, const OSSL_PARAM params[], const char *macname,
const char *ciphername, const char *mdname, OSSL_LIB_CTX *libctx) {
  const OSSL_PARAM *p;
