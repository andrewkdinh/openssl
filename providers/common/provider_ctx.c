...
PROV_CTX *ossl_prov_ctx_new(void) { return OPENSSL_zalloc(sizeof(PROV_CTX)); }

void ossl_prov_ctx_free(PROV_CTX *ctx) { OPENSSL_free(ctx); }
...
void ossl_prov_ctx_set0_core_bio_method(PROV_CTX *ctx,
...
ossl_prov_ctx_get0_core_get_params(PROV_CTX *ctx) {
  if (ctx == NULL)
    return NULL;
...
const char *ossl_prov_ctx_get_param(PROV_CTX *ctx, const char *name,
  ...
  if (ctx->core_get_params(ctx->handle, param) && OSSL_PARAM_modified(param) &&
      val != NULL)
    return val;
  return defval;
}
