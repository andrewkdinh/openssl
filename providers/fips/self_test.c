 ...
 */
static const unsigned char hmac_kat_pt[] = {0xdd, 0x0c, 0x30, 0x33, 0x35, 0xf9, 0xe4, 0x2e,
 ...
 */
static int verify_integrity(OSSL_CORE_BIO *bio, OSSL_FUNC_BIO_read_ex_fn read_ex_cb, unsigned char *expected,
                            size_t expected_len, OSSL_LIB_CTX *libctx, OSSL_SELF_TEST *ev, const char *event_type)
{
    int ret = 0, status;
...
int SELF_TEST_post(SELF_TEST_POST_PARAMS *st, int on_demand_test)
{
    ...
    if (bio_module == NULL
        || !verify_integrity(bio_module, st->bio_read_ex_cb, module_checksum, checksum_len, st->libctx, ev,
