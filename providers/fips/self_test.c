 ...
 * Ones not required or incorrect need to be undefined or redefined
 * respectively.
 */
 ...
 */
static const unsigned char hmac_kat_pt[] = {0xdd, 0x0c, 0x30, 0x33, 0x35, 0xf9,
                                            0xe4, 0x2e, 0xc2, 0xef, 0xcc, 0xbf,
                                            0x07, 0x95, 0xee, 0xa2};
static const unsigned char hmac_kat_key[] = {
0xf4, 0x55, 0x66, 0x50, 0xac, 0x31, 0xd3, 0x54, 0x61, 0x61, 0x0b,
...
0xf5, 0xf5, 0xe5, 0xf2, 0x66, 0x49, 0xe2, 0x40, 0xfc, 0x9e, 0x85,
...
static void set_fips_state(int state) { tsan_store(&FIPS_state, state); }
...
int SELF_TEST_post(SELF_TEST_POST_PARAMS *st, int on_demand_test) {
    ...
    module_checksum =
    OPENSSL_hexstr2buf(st->module_checksum_data, &checksum_len);
    ...
    if (bio_module == NULL ||
        !verify_integrity(bio_module, st->bio_read_ex_cb, module_checksum,
                          checksum_len, st->libctx, ev,
    ...
    if (rng != NULL)
        if ((testrand = EVP_RAND_fetch(st->libctx, "TEST-RAND", NULL)) ==
            NULL ||
