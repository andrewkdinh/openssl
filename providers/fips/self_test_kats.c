...
static int set_kat_drbg(OSSL_LIB_CTX *ctx, const unsigned char *entropy, size_t entropy_len, const unsigned char *nonce,
                        size_t nonce_len, const unsigned char *persstr, size_t persstr_len);
...
static int cipher_init(EVP_CIPHER_CTX *ctx, const EVP_CIPHER *cipher, const ST_KAT_CIPHER *t, int enc)
{
    ...
    return EVP_CipherInit_ex(ctx, cipher, NULL, NULL, NULL, enc)
        ...
        && (in_tag == NULL || EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG, t->tag_len, in_tag) > 0)
...
static int self_test_cipher(const ST_KAT_CIPHER *t, OSSL_SELF_TEST *st, OSSL_LIB_CTX *libctx)
{
    ...
    if ((t->mode & CIPHER_MODE_ENCRYPT) != 0) {
        ...
        if (t->tag != NULL) {
            ...
            if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, t->tag_len, tag) <= 0
    ...
    if ((t->mode & CIPHER_MODE_DECRYPT) != 0) {
        if (!(cipher_init(ctx, cipher, t, !encrypt)
              && EVP_CipherUpdate(ctx, pt_buf, &len, t->base.expected, t->base.expected_len)
...
static int self_test_drbg(const ST_KAT_DRBG *t, OSSL_SELF_TEST *st, OSSL_LIB_CTX *libctx)
{
    ...
    if (!EVP_RAND_generate(drbg, out, t->expectedlen, strength, prediction_resistance, t->entropyaddin1,
                           t->entropyaddin1len))
        goto err;
     ...
     */
    if (!EVP_RAND_generate(drbg, out, t->expectedlen, strength, prediction_resistance, t->entropyaddin2,
                           t->entropyaddin2len))
        goto err;
 ...
 */
static int self_test_kem_decapsulate(const ST_KAT_KEM *t, OSSL_SELF_TEST *st, OSSL_LIB_CTX *libctx, EVP_PKEY *pkey,
                                     int reject)
{
    int ret = 0;
    EVP_PKEY_CTX *ctx = NULL;
