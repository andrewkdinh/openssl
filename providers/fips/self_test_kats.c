...
static int set_kat_drbg(OSSL_LIB_CTX *ctx, const unsigned char *entropy,
                        size_t entropy_len, const unsigned char *nonce,
                        size_t nonce_len, const unsigned char *persstr,
                        size_t persstr_len);
...
static int cipher_init(EVP_CIPHER_CTX *ctx, const EVP_CIPHER *cipher,
    ...
    return EVP_CipherInit_ex(ctx, cipher, NULL, NULL, NULL, enc) &&
           (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_IVLEN, t->iv_len, NULL) >
            0) &&
           (in_tag == NULL || EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG,
                                                  t->tag_len, in_tag) > 0) &&
...
static int self_test_cipher(const ST_KAT_CIPHER *t, OSSL_SELF_TEST *st,
    ...
    if ((t->mode & CIPHER_MODE_DECRYPT) != 0) {
        if (!(cipher_init(ctx, cipher, t, !encrypt) &&
              EVP_CipherUpdate(ctx, pt_buf, &len, t->base.expected,
                               t->base.expected_len) &&
...
static int self_test_drbg(const ST_KAT_DRBG *t, OSSL_SELF_TEST *st,
    ...
    OSSL_PARAM drbg_params[3] = {OSSL_PARAM_END, OSSL_PARAM_END,
                                 OSSL_PARAM_END};
    ...
    drbg_params[0] =
    OSSL_PARAM_construct_utf8_string(t->param_name, t->param_value, 0);
    ...
    drbg_params[0] = OSSL_PARAM_construct_octet_string(
    OSSL_RAND_PARAM_TEST_ENTROPY, (void *)t->entropyin, t->entropyinlen);
    drbg_params[1] = OSSL_PARAM_construct_octet_string(
    OSSL_RAND_PARAM_TEST_NONCE, (void *)t->nonce, t->noncelen);
    ...
    drbg_params[0] = OSSL_PARAM_construct_octet_string(
    OSSL_RAND_PARAM_TEST_ENTROPY, (void *)t->entropyinpr1, t->entropyinpr1len);
    if (!EVP_RAND_CTX_set_params(test, drbg_params))
        goto err;

    if (!EVP_RAND_generate(drbg, out, t->expectedlen, strength,
                           prediction_resistance, t->entropyaddin1,
                           t->entropyaddin1len))
        goto err;

    drbg_params[0] = OSSL_PARAM_construct_octet_string(
    OSSL_RAND_PARAM_TEST_ENTROPY, (void *)t->entropyinpr2, t->entropyinpr2len);
    if (!EVP_RAND_CTX_set_params(test, drbg_params))
        goto err;
     ...
     */
    if (!EVP_RAND_generate(drbg, out, t->expectedlen, strength,
                           prediction_resistance, t->entropyaddin2,
                           t->entropyaddin2len))
        goto err;
...
static int self_test_ka(const ST_KAT_KAS *t, OSSL_SELF_TEST *st,
                        OSSL_LIB_CTX *libctx) {
    int ret = 0;
    ...
    params =
    kat_params_to_ossl_params(libctx, t->key_group, t->key_host_data, NULL);
    params_peer =
    kat_params_to_ossl_params(libctx, t->key_group, t->key_peer_data, NULL);
...
static int digest_signature(const uint8_t *sig, size_t sig_len, uint8_t *out,
                            size_t *out_len, OSSL_LIB_CTX *lib_ctx) {
    int ret;
    unsigned int len = 0;
    EVP_MD_CTX *ctx = EVP_MD_CTX_new();
    EVP_MD *md = EVP_MD_fetch(lib_ctx, "SHA256", NULL);

    ret = ctx != NULL && md != NULL && EVP_DigestInit_ex(ctx, md, NULL) == 1 &&
...
static int self_test_digest_sign(const ST_KAT_SIGN *t, OSSL_SELF_TEST *st,
                                 OSSL_LIB_CTX *libctx) {
    int ret = 0;
    ...
    if (t->entropy != NULL) {
        if (!set_kat_drbg(libctx, t->entropy, t->entropy_len, t->nonce,
                          t->nonce_len, t->persstr, t->persstr_len))
            goto err;
    }
    ...
    if (fromctx == NULL || paramskey == NULL || paramsinit == NULL ||
        paramsverify == NULL)
        goto err;
        ...
        if (t->sig_expected != NULL) {
            if ((t->mode & SIGNATURE_MODE_SIG_DIGESTED) != 0) {
                ...
                if (!digest_signature(psig, siglen, digested_sig,
                    ...
                    memcmp(digested_sig, t->sig_expected,
                           t->sig_expected_len) != 0)
                    goto err;
            } else {
 ...
 */
static int self_test_asym_keygen(const ST_KAT_ASYM_KEYGEN *t,
    ...
    for (expected = t->expected_params; expected->data != NULL; ++expected) {
        if (expected->type != OSSL_PARAM_OCTET_STRING ||
            !EVP_PKEY_get_octet_string_param(key, expected->name, out,
                                             sizeof(out), &out_len))
            goto err;
        OSSL_SELF_TEST_oncorrupt_byte(st, out);
        /* Check the KAT */
...
static int self_test_kem_encapsulate(const ST_KAT_KEM *t, OSSL_SELF_TEST *st,
    ...
    *params = OSSL_PARAM_construct_octet_string(
    OSSL_KEM_PARAM_IKME, (unsigned char *)t->entropy, t->entropy_len);
 ...
 *
 * In FIPS 140-3, the asymmetric POST must be a KAT, not a PCT.  For DSA and
 * ECDSA, the sign operation includes the random value 'k'.  For a KAT to work,
 * we have to have control of the DRBG to make sure it is in a "test" state,
 * where its output is truly deterministic.
 *
 */
...
static int set_kat_drbg(OSSL_LIB_CTX *ctx, const unsigned char *entropy,
    ...
    drbg_params[0] = OSSL_PARAM_construct_octet_string(
    OSSL_RAND_PARAM_TEST_ENTROPY, (void *)entropy, entropy_len);
    drbg_params[1] = OSSL_PARAM_construct_octet_string(
    OSSL_RAND_PARAM_TEST_NONCE, (void *)nonce, nonce_len);
    ...
    if (!EVP_RAND_instantiate(kat_rand, strength, 0, persstr, persstr_len,
                              NULL))
        goto err;
...
static int setup_main_random(OSSL_LIB_CTX *libctx) {
    ...
    drbg_params[0] =
    OSSL_PARAM_construct_uint(OSSL_RAND_PARAM_GENERATE, &generate);
    drbg_params[1] =
    OSSL_PARAM_construct_uint(OSSL_RAND_PARAM_STRENGTH, &strength);
