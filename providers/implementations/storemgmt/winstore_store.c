...
static void winstore_win_advance(struct winstore_ctx_st *ctx) {
  ...
  (name.cbData == 0 ? NULL
                    : CertFindCertificateInStore(
                      ctx->win_store, X509_ASN_ENCODING | PKCS_7_ASN_ENCODING,
                      0, CERT_FIND_SUBJECT_NAME, &name, ctx->win_ctx));
...
winstore_settable_ctx_params(void *loaderctx, const OSSL_PARAM params[]) {
  static const OSSL_PARAM known_settable_ctx_params[] = {
  OSSL_PARAM_octet_string(OSSL_STORE_PARAM_SUBJECT, NULL, 0),
  OSSL_PARAM_utf8_string(OSSL_STORE_PARAM_PROPERTIES, NULL, 0), OSSL_PARAM_END};
...
static void load_cleanup(void *construct_data) { /* No-op. */ }
...
static int winstore_load(void *loaderctx, OSSL_CALLBACK *object_cb,
  ...
  ret =
  winstore_load_using(ctx, object_cb, object_cbarg, pw_cb, pw_cbarg,
