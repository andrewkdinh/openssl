...
static void winstore_win_advance(struct winstore_ctx_st *ctx)
{
    ...
    ctx->win_ctx =
        (name.cbData == 0 ? NULL
                          : CertFindCertificateInStore(ctx->win_store, X509_ASN_ENCODING | PKCS_7_ASN_ENCODING, 0,
                                                       CERT_FIND_SUBJECT_NAME, &name, ctx->win_ctx));
...
{
    static const OSSL_PARAM known_settable_ctx_params[] = {OSSL_PARAM_octet_string(OSSL_STORE_PARAM_SUBJECT, NULL, 0),
...
static int load_construct(OSSL_DECODER_INSTANCE *decoder_inst, const OSSL_PARAM *params, void *construct_data)
{
...
static int setup_decoder(struct winstore_ctx_st *ctx)
{
    ...
    for (to_algo = ossl_any_to_obj_algorithm; to_algo->algorithm_names != NULL; to_algo++) {
        ...
        if (!ossl_decoder_ctx_add_decoder_inst(ctx->dctx, to_obj_inst)) {
            ossl_decoder_instance_free(to_obj_inst);
            ERR_raise(ERR_LIB_PROV, ERR_R_OSSL_DECODER_LIB);
            goto err;
        }
...
static int winstore_load_using(struct winstore_ctx_st *ctx, OSSL_CALLBACK *object_cb, void *object_cbarg,
                               OSSL_PASSPHRASE_CALLBACK *pw_cb, void *pw_cbarg, const void *der, size_t der_len)
{
...
static int winstore_load(void *loaderctx, OSSL_CALLBACK *object_cb, void *object_cbarg, OSSL_PASSPHRASE_CALLBACK *pw_cb,
                         void *pw_cbarg)
{
    int ret = 0;
    struct winstore_ctx_st *ctx = loaderctx;
    ...
    ret = winstore_load_using(ctx, object_cb, object_cbarg, pw_cb, pw_cbarg, ctx->win_ctx->pbCertEncoded,
