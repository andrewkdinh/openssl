...
static void any2obj_freectx(void *ctx) { OPENSSL_free(ctx); }
...
any2obj_settable_ctx_params(ossl_unused void *provctx) {
...
static int any2obj_decode_final(void *vctx, int objtype, const char *input_type,
  ...
  if (mem != NULL) {
    ...
    *p++ = OSSL_PARAM_construct_octet_string(OSSL_OBJECT_PARAM_DATA, mem->data,
                                             mem->length);
...
next:
  ...
  return any2obj_decode_final(ctx, OSSL_OBJECT_PKEY, "msblob",
                              isdss ? "DSA" : "RSA", mem, data_cb, data_cbarg);
...
next:
  ...
  return any2obj_decode_final(ctx, OSSL_OBJECT_PKEY, "pvk",
                              ok ? (isdss ? "DSA" : "RSA") : NULL, mem, data_cb,
                              data_cbarg);
