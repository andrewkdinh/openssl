...
static int rsa_encrypt(void *vprsactx, unsigned char *out, size_t *outlen,
    ...
    if ((prsactx->pad_mode == RSA_PKCS1_PADDING ||
        ...
        prsactx, OSSL_FIPS_IND_SETTABLE1, prsactx->libctx, "RSA Encrypt",
    ...
    if (prsactx->pad_mode == RSA_PKCS1_OAEP_PADDING) {
        ...
        ret =
        RSA_public_encrypt(rsasize, tbuf, out, prsactx->rsa, RSA_NO_PADDING);
        OPENSSL_free(tbuf);
    } else {
        ret =
        RSA_public_encrypt(inlen, in, out, prsactx->rsa, prsactx->pad_mode);
    }
...
static int rsa_decrypt(void *vprsactx, unsigned char *out, size_t *outlen,
    ...
    if (prsactx->pad_mode == RSA_PKCS1_OAEP_PADDING ||
        ...
        if ((tbuf = OPENSSL_malloc(len)) == NULL)
            return 0;
        ret =
        RSA_private_decrypt(inlen, in, tbuf, prsactx->rsa, RSA_NO_PADDING);
        /*
        ...
        if (prsactx->pad_mode == RSA_PKCS1_OAEP_PADDING) {
            ...
            out, outsize, tbuf, len, len, prsactx->oaep_label,
            prsactx->oaep_labellen, prsactx->oaep_md, prsactx->mgf1_md);
        } else {
            ...
            prsactx->libctx, out, outsize, tbuf, len, prsactx->client_version,
            prsactx->alt_version);
        }
        OPENSSL_free(tbuf);
    } else {
...
static int rsa_get_ctx_params(void *vprsactx, OSSL_PARAM *params) {
    ...
    if (p != NULL &&
        ...
        p, prsactx->oaep_md == NULL ? "" : EVP_MD_get0_name(prsactx->oaep_md)))
        return 0;

    p = OSSL_PARAM_locate(params, OSSL_ASYM_CIPHER_PARAM_MGF1_DIGEST);
    if (p != NULL) {
        EVP_MD *mgf1_md =
        prsactx->mgf1_md == NULL ? prsactx->oaep_md : prsactx->mgf1_md;
    ...
    if (p != NULL && !OSSL_PARAM_set_octet_ptr(p, prsactx->oaep_label,
    ...
    p =
    OSSL_PARAM_locate(params, OSSL_ASYM_CIPHER_PARAM_TLS_NEGOTIATED_VERSION);
...
OSSL_PARAM_DEFN(OSSL_ASYM_CIPHER_PARAM_OAEP_LABEL, OSSL_PARAM_OCTET_PTR, NULL,
                0),
...
static int rsa_set_ctx_params(void *vprsactx, const OSSL_PARAM params[]) {
    ...
    if (!OSSL_FIPS_IND_SET_CTX_PARAM(
        prsactx, OSSL_FIPS_IND_SETTABLE1, params,
    ...
    p =
    OSSL_PARAM_locate_const(params, OSSL_ASYM_CIPHER_PARAM_TLS_CLIENT_VERSION);
    if (p != NULL) {
        unsigned int client_version;
    ...
    p =
    OSSL_PARAM_locate_const(params, OSSL_ASYM_CIPHER_PARAM_IMPLICIT_REJECTION);
    if (p != NULL) {
        unsigned int implicit_rejection;
...
OSSL_ASYM_CIPHER_PARAM_FIPS_RSA_PKCS15_PAD_DISABLED) OSSL_PARAM_END};
