...
static int rsa_decrypt(void *vprsactx, unsigned char *out, size_t *outlen,
  ...
  if (prsactx->pad_mode == RSA_PKCS1_OAEP_PADDING ||
    ...
    if (prsactx->pad_mode == RSA_PKCS1_OAEP_PADDING) {
      ...
      out, outsize, tbuf, len, len, prsactx->oaep_label, prsactx->oaep_labellen,
      prsactx->oaep_md, prsactx->mgf1_md);
    } else {
...
static int rsa_get_ctx_params(void *vprsactx, OSSL_PARAM *params) {
  ...
  if (p != NULL &&
      !OSSL_PARAM_set_octet_ptr(p, prsactx->oaep_label, prsactx->oaep_labellen))
    return 0;
...
static int rsa_set_ctx_params(void *vprsactx, const OSSL_PARAM params[]) {
  ...
  if (p != NULL) {
    ...
    p =
    OSSL_PARAM_locate_const(params, OSSL_ASYM_CIPHER_PARAM_OAEP_DIGEST_PROPS);
    if (p != NULL) {
      str = mdprops;
  ...
  if (p != NULL) {
    ...
    p =
    OSSL_PARAM_locate_const(params, OSSL_ASYM_CIPHER_PARAM_MGF1_DIGEST_PROPS);
    if (p != NULL) {
      str = mdprops;
      if (!OSSL_PARAM_get_utf8_string(p, &str, sizeof(mdprops)))
        return 0;
