...
static int ecdh_set_peer(void *vpecdhctx, void *vecdh)
{
    ...
    if (!ossl_fips_ind_ec_key_check(
            OSSL_FIPS_IND_GET(pecdhctx), OSSL_FIPS_IND_SETTABLE0,
            pecdhctx->libctx, EC_KEY_get0_group(vecdh), "ECDH Set Peer", 1))
        return 0;
#endif
...
static int ecdh_set_ctx_params(void *vpecdhctx, const OSSL_PARAM params[])
{
    ...
    if (p != NULL) {
        ...
        if (!ossl_fips_ind_digest_exch_check(
                OSSL_FIPS_IND_GET(pectx), OSSL_FIPS_IND_SETTABLE1,
                pectx->libctx, pectx->kdf_md, "ECDH Set Ctx")) {
            EVP_MD_free(pectx->kdf_md);
            pectx->kdf_md = NULL;
            return 0;
        }
    ...
    OSSL_FIPS_IND_SETTABLE_CTX_PARAM(OSSL_EXCHANGE_PARAM_FIPS_KEY_CHECK)
        OSSL_FIPS_IND_SETTABLE_CTX_PARAM(OSSL_EXCHANGE_PARAM_FIPS_DIGEST_CHECK)
            OSSL_FIPS_IND_SETTABLE_CTX_PARAM(
                OSSL_EXCHANGE_PARAM_FIPS_ECDH_COFACTOR_CHECK) OSSL_PARAM_END};
