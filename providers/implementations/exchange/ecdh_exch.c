...
static int ecdh_init(void *vpecdhctx, void *vecdh, const OSSL_PARAM params[]) {
    ...
    if (!ossl_prov_is_running() || pecdhctx == NULL || vecdh == NULL ||
        (EC_KEY_get0_group(vecdh) == NULL) || !EC_KEY_up_ref(vecdh))
        return 0;
...
static int ecdh_match_params(const EC_KEY *priv, const EC_KEY *peer) {
    ...
    ret = group_priv != NULL && group_peer != NULL &&
...
static int ecdh_set_peer(void *vpecdhctx, void *vecdh) {
    ...
    if (!ossl_fips_ind_ec_key_check(
        OSSL_FIPS_IND_GET(pecdhctx), OSSL_FIPS_IND_SETTABLE0, pecdhctx->libctx,
...
static int ecdh_set_ctx_params(void *vpecdhctx, const OSSL_PARAM params[]) {
    ...
    if (!OSSL_FIPS_IND_SET_CTX_PARAM(
        pectx, OSSL_FIPS_IND_SETTABLE2, params,
        OSSL_EXCHANGE_PARAM_FIPS_ECDH_COFACTOR_CHECK))
        return 0;

    p =
    OSSL_PARAM_locate_const(params, OSSL_EXCHANGE_PARAM_EC_ECDH_COFACTOR_MODE);
    if (p != NULL) {
        int mode;
    ...
    if (p != NULL) {
        ...
        str = mdprops;
        p =
        OSSL_PARAM_locate_const(params, OSSL_EXCHANGE_PARAM_KDF_DIGEST_PROPS);
...
#ifdef FIPS_MODULE
        if (!ossl_fips_ind_digest_exch_check(
            OSSL_FIPS_IND_GET(pectx), OSSL_FIPS_IND_SETTABLE1, pectx->libctx,
...
static int ecdh_get_ctx_params(void *vpecdhctx, OSSL_PARAM params[]) {
    ...
    if (p != NULL &&
        ...
        p, pectx->kdf_md == NULL ? "" : EVP_MD_get0_name(pectx->kdf_md))) {
        return 0;
    }
...
static ossl_inline int ecdh_plain_derive(void *vpecdhctx, unsigned char *secret,
    ...
    if (pecdhctx->cofactor_mode != -1 &&
        pecdhctx->cofactor_mode != key_cofactor_mode && has_cofactor) {
     ...
     * This applies to the 'B' and 'K' curves that have cofactors that are
     * not 1.
     */
    if (has_cofactor && !cofactor_approved) {
        if (!OSSL_FIPS_IND_ON_UNAPPROVED(
            pecdhctx, OSSL_FIPS_IND_SETTABLE2, pecdhctx->libctx, "ECDH",
            "Cofactor", ossl_fips_config_ecdh_cofactor_check)) {
...
static ossl_inline int ecdh_X9_63_kdf_derive(void *vpecdhctx,
    ...
    /* Do KDF stuff */
    if (!ossl_ecdh_kdf_X9_63(secret, pecdhctx->kdf_outlen, stmp, stmplen,
                             pecdhctx->kdf_ukm, pecdhctx->kdf_ukmlen,
