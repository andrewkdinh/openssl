...
static int ecdh_init(void *vpecdhctx, void *vecdh, const OSSL_PARAM params[])
{
    ...
    if (!ossl_prov_is_running() || pecdhctx == NULL || vecdh == NULL || (EC_KEY_get0_group(vecdh) == NULL)
        || !EC_KEY_up_ref(vecdh))
        return 0;
    ...
    if (!ecdh_set_ctx_params(pecdhctx, params))
        return 0;
#ifdef FIPS_MODULE
    if (!ossl_fips_ind_ec_key_check(OSSL_FIPS_IND_GET(pecdhctx), OSSL_FIPS_IND_SETTABLE0, pecdhctx->libctx,
...
static int ecdh_set_peer(void *vpecdhctx, void *vecdh)
{
    ...
    if (!ossl_prov_is_running() || pecdhctx == NULL || vecdh == NULL || !ecdh_match_params(pecdhctx->k, vecdh))
        return 0;
#ifdef FIPS_MODULE
    if (!ossl_fips_ind_ec_key_check(OSSL_FIPS_IND_GET(pecdhctx), OSSL_FIPS_IND_SETTABLE0, pecdhctx->libctx,
                                    EC_KEY_get0_group(vecdh), "ECDH Set Peer", 1))
        return 0;
#endif
...
static int ecdh_set_ctx_params(void *vpecdhctx, const OSSL_PARAM params[])
{
    ...
    if (!OSSL_FIPS_IND_SET_CTX_PARAM(pectx, OSSL_FIPS_IND_SETTABLE0, params, OSSL_EXCHANGE_PARAM_FIPS_KEY_CHECK))
        return 0;
    if (!OSSL_FIPS_IND_SET_CTX_PARAM(pectx, OSSL_FIPS_IND_SETTABLE1, params, OSSL_EXCHANGE_PARAM_FIPS_DIGEST_CHECK))
        return 0;
...
#ifdef FIPS_MODULE
        if (!ossl_fips_ind_digest_exch_check(OSSL_FIPS_IND_GET(pectx), OSSL_FIPS_IND_SETTABLE1, pectx->libctx,
...
static const OSSL_PARAM known_settable_ctx_params[] = {
    ...
    OSSL_FIPS_IND_SETTABLE_CTX_PARAM(OSSL_EXCHANGE_PARAM_FIPS_KEY_CHECK)
        OSSL_FIPS_IND_SETTABLE_CTX_PARAM(OSSL_EXCHANGE_PARAM_FIPS_DIGEST_CHECK)
            OSSL_FIPS_IND_SETTABLE_CTX_PARAM(OSSL_EXCHANGE_PARAM_FIPS_ECDH_COFACTOR_CHECK) OSSL_PARAM_END};
...
static ossl_inline int ecdh_plain_derive(void *vpecdhctx, unsigned char *secret, size_t *psecretlen, size_t outlen)
{
    ...
    if (pecdhctx->cofactor_mode != -1 && pecdhctx->cofactor_mode != key_cofactor_mode && has_cofactor) {
    ...
    if (has_cofactor && !cofactor_approved) {
        if (!OSSL_FIPS_IND_ON_UNAPPROVED(pecdhctx, OSSL_FIPS_IND_SETTABLE2, pecdhctx->libctx, "ECDH", "Cofactor",
...
static ossl_inline int ecdh_X9_63_kdf_derive(void *vpecdhctx, unsigned char *secret, size_t *psecretlen, size_t outlen)
{
    ...
    /* Do KDF stuff */
    if (!ossl_ecdh_kdf_X9_63(secret, pecdhctx->kdf_outlen, stmp, stmplen, pecdhctx->kdf_ukm, pecdhctx->kdf_ukmlen,
                             pecdhctx->kdf_md, pecdhctx->libctx, NULL))
        goto err;
    *psecretlen = pecdhctx->kdf_outlen;
    ret = 1;
