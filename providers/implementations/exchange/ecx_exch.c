...
static int x25519_init(void *vecxctx, void *vkey, ossl_unused const OSSL_PARAM params[])
{
    return ecx_init(vecxctx, vkey, "X25519");
}

static int x448_init(void *vecxctx, void *vkey, ossl_unused const OSSL_PARAM params[])
{
...
static int ecx_derive(void *vecxctx, unsigned char *secret, size_t *secretlen, size_t outlen)
{
    ...
    if (!ossl_prov_is_running())
        return 0;
    return ossl_ecx_compute_key(ecxctx->peerkey, ecxctx->key, ecxctx->keylen, secret, secretlen, outlen);
}
...
static const OSSL_PARAM *ecx_gettable_ctx_params(ossl_unused void *vctx, ossl_unused void *provctx)
{
    static const OSSL_PARAM known_gettable_ctx_params[] = {OSSL_FIPS_IND_GETTABLE_CTX_PARAM() OSSL_PARAM_END};
...
const OSSL_DISPATCH ossl_x25519_keyexch_functions[] = {
    ...
    {OSSL_FUNC_KEYEXCH_GETTABLE_CTX_PARAMS, (void (*)(void))ecx_gettable_ctx_params},
    OSSL_DISPATCH_END
};
