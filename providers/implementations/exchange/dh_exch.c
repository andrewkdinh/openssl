...
static int dh_check_key(PROV_DH_CTX *ctx) {
    ...
    if (!key_approved) {
        if (!OSSL_FIPS_IND_ON_UNAPPROVED(
            ctx, OSSL_FIPS_IND_SETTABLE0, ctx->libctx, "DH Init", "DH Key",
...
static int dh_init(void *vpdhctx, void *vdh, const OSSL_PARAM params[]) {
    ...
    if (!ossl_prov_is_running() || pdhctx == NULL || vdh == NULL ||
        !DH_up_ref(vdh))
        return 0;
    DH_free(pdhctx->dh);
    pdhctx->dh = vdh;
...
static int dh_set_peer(void *vpdhctx, void *vdh) {
    ...
    if (!ossl_prov_is_running() || pdhctx == NULL || vdh == NULL ||
        !dh_match_params(vdh, pdhctx->dh) || !DH_up_ref(vdh))
        return 0;
...
static int dh_plain_derive(void *vpdhctx, unsigned char *secret,
...
static int dh_X9_42_kdf_derive(void *vpdhctx, unsigned char *secret,
    ...
    if (pdhctx->kdf_type == PROV_DH_KDF_X9_42_ASN1) {
        if (!ossl_dh_kdf_X9_42_asn1(secret, pdhctx->kdf_outlen, stmp, stmplen,
                                    pdhctx->kdf_cekalg, pdhctx->kdf_ukm,
                                    pdhctx->kdf_ukmlen, pdhctx->kdf_md,
                                    pdhctx->libctx, NULL))
            goto err;
    }
    *secretlen = pdhctx->kdf_outlen;
...
static int dh_get_ctx_params(void *vpdhctx, OSSL_PARAM params[]) {
    ...
    if (p != NULL &&
        ...
        p, pdhctx->kdf_md == NULL ? "" : EVP_MD_get0_name(pdhctx->kdf_md))) {
        return 0;
    }
    ...
    if (p != NULL && !OSSL_PARAM_set_utf8_string(
                     p, pdhctx->kdf_cekalg == NULL ? "" : pdhctx->kdf_cekalg))
        return 0;
