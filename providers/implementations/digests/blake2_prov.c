...
#define IMPLEMENT_BLAKE_functions(variant, VARIANT, variantsize)               \
    ...
    OSSL_PARAM_END};                                                           \
                                                                               \
    const OSSL_PARAM *ossl_blake##variant##_gettable_ctx_params(               \
    ossl_unused void *ctx, ossl_unused void *pctx) {                           \
        return known_blake##variant##_ctx_params;                              \
    }                                                                          \
                                                                               \
    const OSSL_PARAM *ossl_blake##variant##_settable_ctx_params(               \
    ossl_unused void *ctx, ossl_unused void *pctx) {                           \
        return known_blake##variant##_ctx_params;                              \
    }                                                                          \
                                                                               \
    int ossl_blake##variant##_get_ctx_params(void *vctx,                       \
                                             OSSL_PARAM params[]) {            \
        ...
        if (p != NULL && !OSSL_PARAM_set_uint(                                 \
                         p, (unsigned int)mdctx->params.digest_length)) {      \
        ...
        return 1;                                                              \
                                                                               ...
                                                                               \
    int ossl_blake##variant##_set_ctx_params(void *vctx,                       \
        ...
        if (p != NULL) {                                                       \
            ...
            ossl_blake##variant##_param_set_digest_length(&mdctx->params,      \
                                                          (uint8_t)size);      \
        }                                                                      \
                                                                               \
        return 1;                                                              \
    ...
    static OSSL_FUNC_digest_get_params_fn blake##variantsize##_get_params;     \
                                                                               \
    static int blake##variantsize##_internal_init(void *ctx,                   \
                                                  const OSSL_PARAM params[]) { \
        return ossl_prov_is_running() &&                                       \
               ossl_blake##variant##_set_ctx_params(ctx, params) &&            \
    ...
    static void blake##variantsize##_copyctx(void *voutctx, void *vinctx) {    \
        ...
        *outctx = *inctx;                                                      \
                                                                               ...
                                                                               \
    static int blake##variantsize##_internal_final(                            \
    void *ctx, unsigned char *out, size_t *outl, size_t outsz) {               \
    ...
    static int blake##variantsize##_get_params(OSSL_PARAM params[]) {          \
        return ossl_digest_default_get_params(                                 \
        params, BLAKE##VARIANT##_BLOCKBYTES, BLAKE##VARIANT##_OUTBYTES, 0);    \
    }                                                                          \
                                                                               \
    ...
    {OSSL_FUNC_DIGEST_FINAL,                                                   \
     (void (*)(void))blake##variantsize##_internal_final},                     \
    ...
    {OSSL_FUNC_DIGEST_GET_PARAMS,                                              \
     (void (*)(void))blake##variantsize##_get_params},                         \
    {OSSL_FUNC_DIGEST_GETTABLE_PARAMS,                                         \
     (void (*)(void))ossl_digest_default_gettable_params},                     \
    {OSSL_FUNC_DIGEST_INIT,                                                    \
     (void (*)(void))blake##variantsize##_internal_init},                      \
