...
/*
 * Forward declaration of any unique methods implemented here. This is not
 * strictly necessary for the compiler, but provides an assurance that the
 * signatures of the functions in the dispatch table are correct.
 */
...
static PROV_SHA3_METHOD sha3_generic_md = {generic_sha3_absorb,
                                           generic_sha3_final, NULL};

static PROV_SHA3_METHOD shake_generic_md = {
generic_sha3_absorb, generic_sha3_final, generic_sha3_squeeze};
...
static PROV_SHA3_METHOD sha3_ARMSHA3_md = {armsha3_sha3_absorb,
                                           generic_sha3_final};
static PROV_SHA3_METHOD shake_ARMSHA3_md = {
armsha3_sha3_absorb, generic_sha3_final, generic_sha3_squeeze};
#define SHAKE_SET_MD(uname, typ)                                               \
...
#define SHA3_newctx(typ, uname, name, bitlen, pad)                             \
    ...
    static void *name##_newctx(void *provctx) {                                \
        KECCAK1600_CTX *ctx =                                                  \
        ossl_prov_is_running() ? OPENSSL_zalloc(sizeof(*ctx)) : NULL;          \
                                                                               \
        if (ctx == NULL)                                                       \
            return NULL;                                                       \
...
static void *keccak_dupctx(void *ctx) {
    ...
    KECCAK1600_CTX *ret =
    ossl_prov_is_running() ? OPENSSL_malloc(sizeof(*ret)) : NULL;
...
#define IMPLEMENT_SHA3_functions(bitlen)                                       \
    ...
    PROV_FUNC_SHA3_DIGEST(sha3_##bitlen, bitlen, SHA3_BLOCKSIZE(bitlen),       \
                          SHA3_MDSIZE(bitlen), SHA3_FLAGS)

#define IMPLEMENT_KECCAK_functions(bitlen)                                     \
    SHA3_newctx(keccak, KECCAK_##bitlen, keccak_##bitlen, bitlen, '\x01')      \
    PROV_FUNC_SHA3_DIGEST(keccak_##bitlen, bitlen, SHA3_BLOCKSIZE(bitlen),     \
                          SHA3_MDSIZE(bitlen), SHA3_FLAGS)

#define IMPLEMENT_SHAKE_functions(bitlen)                                      \
    SHAKE_newctx(shake, SHAKE_##bitlen, shake_##bitlen, bitlen,                \
                 0 /* no default md length */, '\x1f')                         \
    PROV_FUNC_SHAKE_DIGEST(shake_##bitlen, bitlen, SHA3_BLOCKSIZE(bitlen), 0,  \
                           SHAKE_FLAGS)

#define IMPLEMENT_KMAC_functions(bitlen)                                       \
    KMAC_newctx(keccak_kmac_##bitlen, bitlen, '\x04') PROV_FUNC_SHAKE_DIGEST(  \
    keccak_kmac_##bitlen, bitlen, SHA3_BLOCKSIZE(bitlen), KMAC_MDSIZE(bitlen), \
    KMAC_FLAGS)
