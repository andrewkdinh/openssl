...
static void blake2b_compress(BLAKE2B_CTX *S, const uint8_t *blocks, size_t len)
{
    ...
    do {
        for (i = 0; i < 16; ++i) { m[i] = load64(blocks + i * sizeof(m[i])); }
...
#define ROUND(r)  \
        ...
        for (i = 0; i < 8; ++i) { S->h[i] = v[i] ^= v[i + 8] ^ S->h[i]; }
#undef G
#undef ROUND
        blocks += increment;
        len -= increment;
...
int ossl_blake2b_final(unsigned char *md, BLAKE2B_CTX *c)
{
    ...
    for (i = 0; i < iter; ++i) store64(target + sizeof(c->h[i]) * i, c->h[i]);
