...
void ossl_chacha20_initctx(PROV_CHACHA20_CTX *ctx) {
  ...
  ctx, CHACHA20_KEYLEN * 8, CHACHA20_BLKLEN * 8, CHACHA20_IVLEN * 8, 0,
  CHACHA20_FLAGS, ossl_prov_cipher_hw_chacha20(CHACHA20_KEYLEN * 8), NULL);
}
...
static void *chacha20_dupctx(void *vctx) {
  ...
  if (ctx != NULL) {
    ...
    if (dupctx != NULL && dupctx->base.tlsmac != NULL && dupctx->base.alloced) {
      dupctx->base.tlsmac =
      OPENSSL_memdup(dupctx->base.tlsmac, dupctx->base.tlsmacsize);
...
static int chacha20_get_params(OSSL_PARAM params[]) {
  ...
  params, 0, CHACHA20_FLAGS, CHACHA20_KEYLEN * 8, CHACHA20_BLKLEN * 8,
  CHACHA20_IVLEN * 8);
}
