...
static int aes_einit(void *ctx, const unsigned char *key, size_t keylen, const unsigned char *iv, size_t ivlen,
...
static int aes_dinit(void *ctx, const unsigned char *key, size_t keylen, const unsigned char *iv, size_t ivlen,
                     const OSSL_PARAM params[])
{
...
static int aes_get_ctx_params(void *vctx, OSSL_PARAM params[])
{
    ...
    if (p != NULL && !OSSL_PARAM_set_octet_string(p, ctx->base.oiv, ctx->base.ivlen)
        && !OSSL_PARAM_set_octet_ptr(p, &ctx->base.oiv, ctx->base.ivlen)) {
        ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_SET_PARAMETER);
        return 0;
    }
    p = OSSL_PARAM_locate(params, OSSL_CIPHER_PARAM_UPDATED_IV);
    if (p != NULL && !OSSL_PARAM_set_octet_string(p, ctx->base.iv, ctx->base.ivlen)
    ...
    if (p != NULL) {
        sz = p->data_size;
        if (sz == 0 || sz > AES_CBC_MAX_HMAC_SIZE || !ctx->base.enc || ctx->taglen == UNINITIALISED_SIZET) {
            ERR_raise(ERR_LIB_PROV, PROV_R_INVALID_TAG);
            return 0;
        }
...
static void base_ctx_init(void *provctx, PROV_AES_HMAC_SHA_ETM_CTX *ctx, const PROV_CIPHER_HW_AES_HMAC_SHA_ETM *meths,
                          size_t kbits, size_t blkbits, size_t ivbits, uint64_t flags)
{
...
static void *aes_cbc_hmac_sha1_etm_newctx(void *provctx, size_t kbits, size_t blkbits, size_t ivbits, uint64_t flags)
{
    ...
    if (ctx != NULL)
        base_ctx_init(provctx, &ctx->base_ctx, ossl_prov_cipher_hw_aes_cbc_hmac_sha1_etm(), kbits, blkbits, ivbits,
                      flags);
    return ctx;
}
...
static void *aes_cbc_hmac_sha256_etm_newctx(void *provctx, size_t kbits, size_t blkbits, size_t ivbits, uint64_t flags)
{
    ...
    ctx = OPENSSL_zalloc(sizeof(*ctx));
    if (ctx != NULL)
        base_ctx_init(provctx, &ctx->base_ctx, ossl_prov_cipher_hw_aes_cbc_hmac_sha256_etm(), kbits, blkbits, ivbits,
                      flags);
    return ctx;
}
...
static void *aes_cbc_hmac_sha512_etm_newctx(void *provctx, size_t kbits, size_t blkbits, size_t ivbits, uint64_t flags)
{
    ...
    ctx = OPENSSL_zalloc(sizeof(*ctx));
    if (ctx != NULL)
        base_ctx_init(provctx, &ctx->base_ctx, ossl_prov_cipher_hw_aes_cbc_hmac_sha512_etm(), kbits, blkbits, ivbits,
                      flags);
    return ctx;
}
