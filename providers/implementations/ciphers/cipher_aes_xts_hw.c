...
#define XTS_SET_KEY_FN(fn_set_enc_key, fn_set_dec_key, fn_block_enc,           \
                       fn_block_dec, fn_stream_enc, fn_stream_dec)             \
  {                                                                            \
...
static int cipher_hw_aes_xts_generic_initkey(PROV_CIPHER_CTX *ctx,
  ...
  if (HWAES_CAPABLE) {
    ...
    XTS_SET_KEY_FN(HWAES_set_encrypt_key, HWAES_set_decrypt_key, HWAES_encrypt,
                   HWAES_decrypt, stream_enc, stream_dec);
    return 1;
  } else
#endif /* HWAES_CAPABLE */
...
#ifdef VPAES_CAPABLE
  if (VPAES_CAPABLE) {
    XTS_SET_KEY_FN(vpaes_set_encrypt_key, vpaes_set_decrypt_key, vpaes_encrypt,
                   vpaes_decrypt, stream_enc, stream_dec);
    return 1;
  } else
#endif /* VPAES_CAPABLE */
  {
  ...
  {
    XTS_SET_KEY_FN(AES_set_encrypt_key, AES_set_decrypt_key, AES_encrypt,
                   AES_decrypt, stream_enc, stream_dec);
  }
  return 1;
}
...
static int cipher_hw_aesni_xts_initkey(PROV_CIPHER_CTX *ctx,
  ...
  void (*aesni_xts_enc)(const unsigned char *in, unsigned char *out,
                        size_t length, const AES_KEY *key1, const AES_KEY *key2,
                        const unsigned char iv[16]);
  void (*aesni_xts_dec)(const unsigned char *in, unsigned char *out,
                        size_t length, const AES_KEY *key1, const AES_KEY *key2,
                        const unsigned char iv[16]);
  ...
  XTS_SET_KEY_FN(aesni_set_encrypt_key, aesni_set_decrypt_key, aesni_encrypt,
                 aesni_decrypt, aesni_xts_enc, aesni_xts_dec);
  return 1;
}

#define PROV_CIPHER_HW_declare_xts()                                           \
  static const PROV_CIPHER_HW aesni_xts = {cipher_hw_aesni_xts_initkey, NULL,  \
...
static int cipher_hw_aes_xts_t4_initkey(PROV_CIPHER_CTX *ctx,
  ...
  XTS_SET_KEY_FN(aes_t4_set_encrypt_key, aes_t4_set_decrypt_key, aes_t4_encrypt,
                 aes_t4_decrypt, stream_enc, stream_dec);
  return 1;
}

#define PROV_CIPHER_HW_declare_xts()                                           \
  static const PROV_CIPHER_HW aes_xts_t4 = {cipher_hw_aes_xts_t4_initkey,      \
...
static int cipher_hw_aes_xts_rv64i_zknd_zkne_initkey(PROV_CIPHER_CTX *ctx,
  ...
  XTS_SET_KEY_FN(rv64i_zkne_set_encrypt_key, rv64i_zknd_set_decrypt_key,
                 rv64i_zkne_encrypt, rv64i_zknd_decrypt, stream_enc,
                 stream_dec);
  return 1;
}
...
PROV_CIPHER_CTX *ctx, const unsigned char *key, size_t keylen) {
  ...
  if (keylen * 8 == 128 * 2 || keylen * 8 == 256 * 2) {
    XTS_SET_KEY_FN(rv64i_zvkned_set_encrypt_key, rv64i_zvkned_set_decrypt_key,
    ...
    XTS_SET_KEY_FN(AES_set_encrypt_key, AES_set_encrypt_key,
                   rv64i_zvkned_encrypt, rv64i_zvkned_decrypt, stream_enc,
                   stream_dec);
  }
  return 1;
}
...
#define PROV_CIPHER_HW_declare_xts()                                           \
  ...
  cipher_hw_aes_xts_rv64i_zvkned_initkey, NULL, cipher_hw_aes_xts_copyctx};    \
  static const PROV_CIPHER_HW aes_xts_rv64i_zvbb_zvkg_zvkned = {               \
  cipher_hw_aes_xts_rv64i_zvbb_zvkg_zvkned_initkey, NULL,                      \
  cipher_hw_aes_xts_copyctx};
...
static int cipher_hw_aes_xts_rv32i_zbkb_zknd_zkne_initkey(
PROV_CIPHER_CTX *ctx, const unsigned char *key, size_t keylen) {
  PROV_AES_XTS_CTX *xctx = (PROV_AES_XTS_CTX *)ctx;

  XTS_SET_KEY_FN(rv32i_zbkb_zkne_set_encrypt_key,
                 rv32i_zbkb_zknd_zkne_set_decrypt_key, rv32i_zkne_encrypt,
...
#define PROV_CIPHER_HW_declare_xts()                                           \
  ...
  cipher_hw_aes_xts_rv32i_zbkb_zknd_zkne_initkey, NULL,                        \
  cipher_hw_aes_xts_copyctx};
#define PROV_CIPHER_HW_select_xts()                                            \
...
cipher_hw_aes_xts_generic_initkey, NULL, cipher_hw_aes_xts_copyctx};
PROV_CIPHER_HW_declare_xts() const PROV_CIPHER_HW *ossl_prov_cipher_hw_aes_xts(
size_t keybits) {
