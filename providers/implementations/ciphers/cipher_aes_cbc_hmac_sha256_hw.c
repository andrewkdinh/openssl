...
int ossl_cipher_capable_aes_cbc_hmac_sha256(void) { return 0; }
...
static int aesni_cbc_hmac_sha256_cipher(PROV_CIPHER_CTX *vctx,
  ...
  if (ctx->base.enc) {
    ...
    if (((OPENSSL_ia32cap_P[2] & (1 << 29)) ||         /* SHAEXT? */
      ...
      (void)aesni_cbc_sha256_enc(in, out, blocks, &ctx->ks, ctx->base.iv,
    ...
    if (plen != len) { /* "TLS" mode of operation */
      ...
      aesni_cbc_encrypt(out + aes_off, out + aes_off, len - aes_off, &ctx->ks,
                        ctx->base.iv, 1);
    } else {
      aesni_cbc_encrypt(in + aes_off, out + aes_off, len - aes_off, &ctx->ks,
                        ctx->base.iv, 1);
    }
  } else {
    union {
...
void *ctx, EVP_CTRL_TLS1_1_MULTIBLOCK_PARAM *param) {
  return (int)tls1_multi_block_encrypt(ctx, param->out, param->inp, param->len,
                                       param->interleave / 4);
}
#endif
