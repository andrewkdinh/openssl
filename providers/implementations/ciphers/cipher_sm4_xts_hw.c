...
static int cipher_hw_sm4_xts_generic_initkey(PROV_CIPHER_CTX *ctx,
...
#ifdef HWSM4_CAPABLE
  if (HWSM4_CAPABLE) {
    XTS_SET_KEY_FN(HWSM4_set_encrypt_key, HWSM4_set_decrypt_key, HWSM4_encrypt,
                   HWSM4_decrypt, stream, stream_gb);
    return 1;
  } else
#endif /* HWSM4_CAPABLE */
  ...
  if (VPSM4_CAPABLE) {
    ...
    XTS_SET_KEY_FN(vpsm4_set_encrypt_key, vpsm4_set_decrypt_key, vpsm4_encrypt,
                   vpsm4_decrypt, stream, stream_gb);
    return 1;
  } else
#endif /* VPSM4_CAPABLE */
  {
