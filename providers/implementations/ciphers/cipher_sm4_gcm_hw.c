...
static int sm4_gcm_initkey(PROV_GCM_CTX *ctx, const unsigned char *key, size_t keylen)
{
...
# ifdef HWSM4_ctr32_encrypt_blocks
        SM4_GCM_HW_SET_KEY_CTR_FN(ks, HWSM4_set_encrypt_key, HWSM4_encrypt, HWSM4_ctr32_encrypt_blocks);
...
# endif
        ...
        if (VPSM4_EX_CAPABLE) {
        SM4_GCM_HW_SET_KEY_CTR_FN(ks, vpsm4_ex_set_encrypt_key, vpsm4_ex_encrypt, vpsm4_ex_ctr32_encrypt_blocks);
    } else
#endif /* VPSM4_EX_CAPABLE */

#ifdef VPSM4_CAPABLE
        if (VPSM4_CAPABLE) {
        SM4_GCM_HW_SET_KEY_CTR_FN(ks, vpsm4_set_encrypt_key, vpsm4_encrypt, vpsm4_ctr32_encrypt_blocks);
    } else
#endif /* VPSM4_CAPABLE */
    {
...
static int hw_gcm_cipher_update(PROV_GCM_CTX *ctx, const unsigned char *in, size_t len, unsigned char *out)
{
    if (ctx->enc) {
        if (ctx->ctr != NULL) {
...
static const PROV_GCM_HW sm4_gcm = {sm4_gcm_initkey,      ossl_gcm_setiv,        ossl_gcm_aad_update,
                                    hw_gcm_cipher_update, ossl_gcm_cipher_final, ossl_gcm_one_shot};
