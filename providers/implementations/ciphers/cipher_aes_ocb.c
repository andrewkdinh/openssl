...
PROV_CIPHER_FUNC(int, ocb_cipher,
                 (PROV_AES_OCB_CTX * ctx, const unsigned char *in,
                  unsigned char *out, size_t nextblock));
...
static ossl_inline int aes_generic_ocb_copy_ctx(PROV_AES_OCB_CTX *dst,
  ...
  return CRYPTO_ocb128_copy_ctx(&dst->ocb, &src->ocb, &dst->ksenc.ks,
                                &dst->ksdec.ks);
}
...
static int aes_ocb_block_update(void *vctx, unsigned char *out, size_t *outl,
  ...
  return aes_ocb_block_update_internal(ctx, buf, buflen, out, outl, outsize, in,
                                       inl, fn);
}
...
cipher_ocb_gettable_ctx_params(ossl_unused void *cctx,
...
static const OSSL_PARAM *
cipher_ocb_settable_ctx_params(ossl_unused void *cctx,
                               ossl_unused void *p_ctx) {
