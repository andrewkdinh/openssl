...
void *ossl_tdes_newctx(void *provctx, int mode, size_t kbits, size_t blkbits,
  ...
  if (tctx != NULL) {
    ...
    ossl_cipher_generic_initkey(tctx, kbits, blkbits, ivbits, mode, flags, hw,
                                provctx);
  }
  return tctx;
}
...
static int tdes_encrypt_check_approved(PROV_TDES_CTX *ctx, int enc) {
  ...
  if (enc && !OSSL_FIPS_IND_ON_UNAPPROVED(
             ctx, OSSL_FIPS_IND_SETTABLE0, ctx->base.libctx, "Triple-DES",
             "Encryption", ossl_fips_config_tdes_encrypt_disallowed))
    return 0;
  return 1;
}
#endif
...
static int tdes_init(void *vctx, const unsigned char *key, size_t keylen,
  ...
  if (iv != NULL) {
    if (!ossl_cipher_generic_initiv(ctx, iv, ivlen))
      return 0;
  } else if (ctx->iv_set && (ctx->mode == EVP_CIPH_CBC_MODE ||
...
int ossl_tdes_get_params(OSSL_PARAM params[], unsigned int md, uint64_t flags,
  ...
  return ossl_cipher_generic_get_params(params, md, flags, kbits, blkbits,
                                        ivbits);
}
