...
static void *aes_wrap_newctx(size_t kbits, size_t blkbits, size_t ivbits,
  ...
  if (ctx != NULL) {
    ossl_cipher_generic_initkey(ctx, kbits, blkbits, ivbits, mode, flags, NULL,
                                NULL);
...
static int aes_wrap_cipher_internal(void *vctx, unsigned char *out,
  ...
  rv = wctx->wrapfn(&wctx->ks.ks, ctx->iv_set ? ctx->iv : NULL, out, in, inlen,
                    ctx->block);
  if (!rv) {
...
#define IMPLEMENT_cipher(mode, fname, UCMODE, flags, kbits, blkbits, ivbits)   \
  ...
  {OSSL_FUNC_CIPHER_NEWCTX, (void (*)(void))aes_##kbits##fname##_newctx},      \
