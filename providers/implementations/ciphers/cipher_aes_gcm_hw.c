  ...
  if (ctx->enc) {
    if (ctx->ctr != NULL) {
      ...
      if (len >= AES_GCM_ENC_BYTES && AES_GCM_ASM(ctx)) {
        ...
        bulk = AES_gcm_encrypt(in + res, out + res, len - res, ctx->gcm.key,
    ...
    if (ctx->ctr != NULL) {
      ...
      if (len >= AES_GCM_DEC_BYTES && AES_GCM_ASM(ctx)) {
        ...
        bulk = AES_gcm_decrypt(in + res, out + res, len - res, ctx->gcm.key,
                               ctx->gcm.Yi.c, ctx->gcm.Xi.u);
...
#else
const PROV_GCM_HW *ossl_prov_aes_hw_gcm(size_t keybits) { return &aes_gcm; }
#endif
