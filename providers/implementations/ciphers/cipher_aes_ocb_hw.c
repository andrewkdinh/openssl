...
#define OCB_SET_KEY_FN(fn_set_enc_key, fn_set_dec_key, fn_block_enc,           \
...
#ifdef HWAES_CAPABLE
  if (HWAES_CAPABLE) {
    OCB_SET_KEY_FN(HWAES_set_encrypt_key, HWAES_set_decrypt_key, HWAES_encrypt,
                   HWAES_decrypt, HWAES_ocb_encrypt, HWAES_ocb_decrypt);
  } else
#endif
#ifdef VPAES_CAPABLE
  if (VPAES_CAPABLE) {
    OCB_SET_KEY_FN(vpaes_set_encrypt_key, vpaes_set_decrypt_key, vpaes_encrypt,
                   vpaes_decrypt, NULL, NULL);
  } else
#endif
  {
    OCB_SET_KEY_FN(AES_set_encrypt_key, AES_set_decrypt_key, AES_encrypt,
...
static int cipher_hw_aes_ocb_aesni_initkey(PROV_CIPHER_CTX *vctx,
  ...
  OCB_SET_KEY_FN(aesni_set_encrypt_key, aesni_set_decrypt_key, aesni_encrypt,
                 aesni_decrypt, aesni_ocb_encrypt, aesni_ocb_decrypt);
  return 1;
}
...
static int cipher_hw_aes_ocb_t4_initkey(PROV_CIPHER_CTX *vctx,
  ...
  OCB_SET_KEY_FN(aes_t4_set_encrypt_key, aes_t4_set_decrypt_key, aes_t4_encrypt,
                 aes_t4_decrypt, NULL, NULL);
  return 1;
}

#define PROV_CIPHER_HW_declare()                                               \
  static const PROV_CIPHER_HW aes_t4_ocb = {cipher_hw_aes_ocb_t4_initkey, NULL};
#define PROV_CIPHER_HW_select()                                                \
...
static int cipher_hw_aes_ocb_rv64i_zvkned_initkey(PROV_CIPHER_CTX *vctx,
  ...
  if (keylen * 8 == 128 || keylen * 8 == 256) {
    OCB_SET_KEY_FN(rv64i_zvkned_set_encrypt_key, rv64i_zvkned_set_decrypt_key,
...
#define PROV_CIPHER_HW_declare()                                               \
  ...
  cipher_hw_aes_ocb_rv64i_zknd_zkne_initkey, NULL};                            \
  static const PROV_CIPHER_HW aes_rv64i_zvkned_ocb = {                         \
  cipher_hw_aes_ocb_rv64i_zvkned_initkey, NULL};
#define PROV_CIPHER_HW_select()                                                \
...
static int cipher_hw_aes_ocb_rv32i_zbkb_zknd_zkne_initkey(
PROV_CIPHER_CTX *vctx, const unsigned char *key, size_t keylen) {
  PROV_AES_OCB_CTX *ctx = (PROV_AES_OCB_CTX *)vctx;

  OCB_SET_KEY_FN(rv32i_zbkb_zkne_set_encrypt_key,
...
#define PROV_CIPHER_HW_declare()                                               \
  ...
  cipher_hw_aes_ocb_rv32i_zknd_zkne_initkey, NULL};                            \
  static const PROV_CIPHER_HW aes_rv32i_zbkb_zknd_zkne_ocb = {                 \
  cipher_hw_aes_ocb_rv32i_zbkb_zknd_zkne_initkey, NULL};
#define PROV_CIPHER_HW_select()                                                \
...
cipher_hw_aes_ocb_generic_initkey, NULL};
PROV_CIPHER_HW_declare() const PROV_CIPHER_HW *ossl_prov_cipher_hw_aes_ocb(
size_t keybits) {
