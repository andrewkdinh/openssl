...
static void *sm4_xts_newctx(void *provctx, unsigned int mode, uint64_t flags,
  ...
  if (ctx != NULL) {
    ossl_cipher_generic_initkey(&ctx->base, kbits, blkbits, ivbits, mode, flags,
                                ossl_prov_cipher_hw_sm4_xts(kbits), NULL);
  }
  return ctx;
}
...
static int sm4_xts_cipher(void *vctx, unsigned char *out, size_t *outl,
  ...
  if (!ossl_prov_is_running() || ctx->xts.key1 == NULL ||
      ctx->xts.key2 == NULL || !ctx->base.iv_set || out == NULL || in == NULL ||
      inl < SM4_BLOCK_SIZE)
    return 0;
  ...
  if (ctx->xts_standard) {
    ...
    else if (ossl_crypto_xts128gb_encrypt(&ctx->xts, ctx->base.iv, in, out, inl,
                                          ctx->base.enc))
      return 0;
  }
  *outl = inl;
...
sm4_xts_settable_ctx_params(ossl_unused void *cctx, ossl_unused void *provctx) {
...
#define IMPLEMENT_cipher(lcmode, UCMODE, kbits, flags)                         \
  ...
  static int sm4_##kbits##_##lcmode##_get_params(OSSL_PARAM params[]) {        \
    ...
    params, EVP_CIPH_##UCMODE##_MODE, flags, 2 * kbits, SM4_XTS_BLOCK_BITS,    \
    SM4_XTS_IV_BITS);                                                          \
  }                                                                            \
  ...
  {OSSL_FUNC_CIPHER_SET_CTX_PARAMS, (void (*)(void))sm4_xts_set_ctx_params},   \
