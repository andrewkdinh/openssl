...
static int rc2_get_ctx_params(void *vctx, OSSL_PARAM params[])
{
    ...
    if (p1 != NULL || p2 != NULL) {
        ...
        if (!ASN1_TYPE_set_int_octetstring(type, num, ctx->base.iv, (int)ctx->base.ivlen)) {
            ASN1_TYPE_free(type);
            ERR_raise(ERR_LIB_PROV, ERR_R_ASN1_LIB);
            return 0;
        }
...
static int rc2_set_ctx_params(void *vctx, const OSSL_PARAM params[])
{
    ...
    if (p != NULL) {
        ...
        if (p->data_type != OSSL_PARAM_OCTET_STRING || ctx->base.ivlen > sizeof(iv)
            || (type = d2i_ASN1_TYPE(NULL, &d, (long)p->data_size)) == NULL
            || ((size_t)ASN1_TYPE_get_int_octetstring(type, &num, iv, (int)ctx->base.ivlen) != ctx->base.ivlen)
