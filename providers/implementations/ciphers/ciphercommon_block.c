 ...
 * of data remaining in the input that is a multiple of the blocksize. The
 * buffer is only filled if it already has some data in it, isn't full already
 * or we don't have at least one block in the input.
 *
...
void ossl_cipher_padblock(unsigned char *buf, size_t *buflen,
                          size_t blocksize) {
    size_t i;
...
int ossl_cipher_unpadblock(unsigned char *buf, size_t *buflen,
                           size_t blocksize) {
    size_t pad, i;
    size_t len = *buflen;
...
/*-
 * ossl_cipher_tlsunpadblock removes the CBC padding from the decrypted, TLS,
 * CBC record in constant time. Also removes the MAC from the record in constant
 * time.
 *
...
int ossl_cipher_tlsunpadblock(OSSL_LIB_CTX *libctx, unsigned int tlsversion,
    ...
    case SSL3_VERSION:
        ...
        buflen, *buflen, buf, mac, alloced, blocksize, macsize, libctx);
    ...
    case DTLS1_BAD_VER:
        ...
        *buflen -= blocksize;
        /* Fall through */
    case TLS1_VERSION:
        ret = tls1_cbc_remove_padding_and_mac(
        buflen, *buflen, buf, mac, alloced, blocksize, macsize, aead, libctx);
        return ret;
