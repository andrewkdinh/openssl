 ...
 */
int ossl_cipher_var_keylen_set_ctx_params(void *vctx,
...
OSSL_PARAM_END};
const OSSL_PARAM *
ossl_cipher_aead_gettable_ctx_params(ossl_unused void *cctx,
...
OSSL_PARAM_END};
const OSSL_PARAM *
ossl_cipher_aead_settable_ctx_params(ossl_unused void *cctx,
                                     ossl_unused void *provctx) {
...
int ossl_cipher_generic_skey_einit(void *vctx, void *skeydata,
    ...
    return cipher_generic_init_internal((PROV_CIPHER_CTX *)vctx, key->data,
                                        key->length, iv, ivlen, params, 1);
}
...
int ossl_cipher_generic_block_update(void *vctx, unsigned char *out,
    ...
    if (ctx->tlsversion > 0) {
        ...
        *outl = inl;
        if (!ctx->enc && !ossl_cipher_tlsunpadblock(
                         ctx->libctx, ctx->tlsversion, out, outl, blksz,
                         &ctx->tlsmac, &ctx->alloced, ctx->tlsmacsize, 0)) {
            ERR_raise(ERR_LIB_PROV, PROV_R_CIPHER_OPERATION_FAILED);
            return 0;
        }
        return 1;
    ...
    if (ctx->bufsz != 0)
        nextblocks =
        ossl_cipher_fillblock(ctx->buf, &ctx->bufsz, blksz, &in, &inl);
    else
