...
#define IMPLEMENT_tdes_cipher(type, UCTYPE, lcmode, UCMODE, flags, kbits,      \
  ...
  static int tdes_##type##_##lcmode##_get_params(OSSL_PARAM params[]) {        \
    return ossl_tdes_get_params(params, EVP_CIPH_##UCMODE##_MODE, flags,       \
  ...
  {OSSL_FUNC_CIPHER_NEWCTX, (void (*)(void))tdes_##type##_##lcmode##_newctx},  \
  ...
  {OSSL_FUNC_CIPHER_SET_CTX_PARAMS, (void (*)(void))ossl_tdes_set_ctx_params}, \
...
#define PROV_CIPHER_HW_tdes_mode(type, mode)                                   \
  ...
  ossl_cipher_hw_tdes_##type##_initkey, ossl_cipher_hw_tdes_##mode,            \
