...
static int cipher_hw_aes_initkey(PROV_CIPHER_CTX *dat, const unsigned char *key,
                                 size_t keylen) {
    int ret;
    ...
    if ((dat->mode == EVP_CIPH_ECB_MODE || dat->mode == EVP_CIPH_CBC_MODE) &&
...
#endif
        {
            ...
            dat->block = (block128_f)AES_decrypt;
            dat->stream.cbc =
            (dat->mode == EVP_CIPH_CBC_MODE) ? (cbc128_f)AES_cbc_encrypt : NULL;
        }
    } else
#ifdef HWAES_CAPABLE
    if (HWAES_CAPABLE) {
    ...
    if (VPAES_CAPABLE) {
        ...
        dat->stream.cbc =
        (dat->mode == EVP_CIPH_CBC_MODE) ? (cbc128_f)vpaes_cbc_encrypt : NULL;
    } else
#endif
    {
        ret = AES_set_encrypt_key(key, keylen * 8, ks);
...
#define PROV_CIPHER_HW_aes_mode(mode)                                          \
    static const PROV_CIPHER_HW aes_##mode = {cipher_hw_aes_initkey,           \
...
PROV_CIPHER_HW_aes_mode(cbc) PROV_CIPHER_HW_aes_mode(ecb)
PROV_CIPHER_HW_aes_mode(ofb128) PROV_CIPHER_HW_aes_mode(cfb128)
PROV_CIPHER_HW_aes_mode(cfb1) PROV_CIPHER_HW_aes_mode(cfb8)
