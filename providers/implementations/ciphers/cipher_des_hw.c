...
static int cipher_hw_des_initkey(PROV_CIPHER_CTX *ctx, const unsigned char *key,
                                 size_t keylen) {
...
static int cipher_hw_des_ecb_cipher(PROV_CIPHER_CTX *ctx, unsigned char *out,
  ...
  for (i = 0, len -= bl; i <= len; i += bl)
    DES_ecb_encrypt((const_DES_cblock *)(in + i), (const_DES_cblock *)(out + i),
                    key, ctx->enc);
  return 1;
}
...
static int cipher_hw_des_cfb64_cipher(PROV_CIPHER_CTX *ctx, unsigned char *out,
  ...
  while (len > 0 && len >= chunk) {
    DES_cfb64_encrypt(in, out, (long)chunk, key, (DES_cblock *)ctx->iv, &num,
                      ctx->enc);
    len -= chunk;
    in += chunk;
    out += chunk;
...
static int cipher_hw_des_cfb8_cipher(PROV_CIPHER_CTX *ctx, unsigned char *out,
  ...
  while (inl >= MAXCHUNK) {
    DES_cfb_encrypt(in, out, 8, (long)MAXCHUNK, key, (DES_cblock *)ctx->iv,
                    ctx->enc);
    inl -= MAXCHUNK;
    in += MAXCHUNK;
    out += MAXCHUNK;
  ...
  if (inl > 0)
    DES_cfb_encrypt(in, out, 8, (long)inl, key, (DES_cblock *)ctx->iv,
                    ctx->enc);
  return 1;
}

#define PROV_CIPHER_HW_des_mode(mode)                                          \
  static const PROV_CIPHER_HW des_##mode = {cipher_hw_des_initkey,             \
...
PROV_CIPHER_HW_des_mode(ecb) PROV_CIPHER_HW_des_mode(cbc)
PROV_CIPHER_HW_des_mode(ofb64) PROV_CIPHER_HW_des_mode(cfb64)
PROV_CIPHER_HW_des_mode(cfb1) PROV_CIPHER_HW_des_mode(cfb8)
