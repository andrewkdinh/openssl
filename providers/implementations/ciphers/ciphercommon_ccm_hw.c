...
int ossl_ccm_generic_gettag(PROV_CCM_CTX *ctx, unsigned char *tag,
                            size_t tlen) {
...
int ossl_ccm_generic_auth_encrypt(PROV_CCM_CTX *ctx, const unsigned char *in,
    ...
    if (ctx->str != NULL)
        rv =
        CRYPTO_ccm128_encrypt_ccm64(&ctx->ccm_ctx, in, out, len, ctx->str) == 0;
    else
...
int ossl_ccm_generic_auth_decrypt(PROV_CCM_CTX *ctx, const unsigned char *in,
    ...
    if (ctx->str != NULL)
        rv =
        CRYPTO_ccm128_decrypt_ccm64(&ctx->ccm_ctx, in, out, len, ctx->str) == 0;
    else
