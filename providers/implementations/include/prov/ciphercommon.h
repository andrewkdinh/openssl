...
struct prov_cipher_ctx_st {
    ...
    unsigned int
    iv_set : 1; /* Set when the iv is copied to the iv/oiv buffers */
    ...
    unsigned int
    use_bits : 1; /* Set to 0 for cfb1 to use bits instead of bytes */

    unsigned int
    tlsversion;            /* If TLS padding is in use the TLS version number */
...
#define IMPLEMENT_generic_cipher_func(alg, UCALG, lcmode, UCMODE, flags,       \
    ...
    {OSSL_FUNC_CIPHER_ENCRYPT_INIT,                                            \
     (void (*)(void))ossl_cipher_generic_einit},                               \
    {OSSL_FUNC_CIPHER_DECRYPT_INIT,                                            \
     (void (*)(void))ossl_cipher_generic_dinit},                               \
    ...
    {OSSL_FUNC_CIPHER_ENCRYPT_SKEY_INIT,                                       \
     (void (*)(void))ossl_cipher_generic_skey_einit},                          \
    {OSSL_FUNC_CIPHER_DECRYPT_SKEY_INIT,                                       \
     (void (*)(void))ossl_cipher_generic_skey_dinit},                          \
    OSSL_DISPATCH_END};
...
#define IMPLEMENT_generic_cipher_genfn(alg, UCALG, lcmode, UCMODE, flags,      \
    ...
    static void *alg##_##kbits##_##lcmode##_newctx(void *provctx) {            \
        ...
        if (ctx != NULL) {                                                     \
            ...
            ctx, kbits, blkbits, ivbits, EVP_CIPH_##UCMODE##_MODE, flags,      \
            ossl_prov_cipher_hw_##alg##_##lcmode(kbits), provctx);             \
        }                                                                      \
        return ctx;                                                            \
    }
...
#define IMPLEMENT_CIPHER_HW_OFB(MODE, NAME, CTX_NAME, KEY_NAME, FUNC_PREFIX)   \
    ...
    PROV_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in,         \
    size_t len) {                                                              \
        int num = ctx->num;                                                    \
