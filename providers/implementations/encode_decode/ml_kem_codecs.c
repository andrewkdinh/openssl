...
{"seed-priv", 0x06aa, 0, 0x308206a6, 0x0440, 6, 0x40, 0x04820660, 0x4a, 0x0660,
 0, 0},
...
{"seed-priv", 0x0caa, 0, 0x30820ca6, 0x0440, 6, 0x40, 0x04820c60, 0x4a, 0x0c60,
 0, 0},
...
ML_KEM_KEY *ossl_ml_kem_d2i_PKCS8(const uint8_t *prvenc, int prvlen,
  ...
  formats =
  ossl_prov_ctx_get_param(provctx, OSSL_PKEY_PARAM_ML_KEM_INPUT_FORMATS, NULL);
  ...
  if (p8fmt->seed_length > 0) {
    if (!ossl_ml_kem_set_seed(buf + p8fmt->seed_offset, ML_KEM_SEED_BYTES,
                              key)) {
      ERR_raise_data(ERR_LIB_OSSL_DECODER, ERR_R_INTERNAL_ERROR,
                     "error storing %s private key seed", v->algorithm_name);
      goto end;
    }
  }
  if (p8fmt->priv_length > 0) {
    if ((key->encoded_dk = OPENSSL_malloc(p8fmt->priv_length)) == NULL) {
      ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_KEY,
                     "error parsing %s private key", v->algorithm_name);
      goto end;
    }
...
int ossl_ml_kem_i2d_pubkey(const ML_KEM_KEY *key, unsigned char **out) {
  ...
  if (!ossl_ml_kem_encode_public_key(*out, publen, key)) {
    ERR_raise_data(ERR_LIB_OSSL_ENCODER, ERR_R_INTERNAL_ERROR,
                   "error encoding %s public key", key->vinfo->algorithm_name);
    OPENSSL_free(*out);
    return 0;
  }
...
int ossl_ml_kem_i2d_prvkey(const ML_KEM_KEY *key, uint8_t **out,
  ...
  formats =
  ossl_prov_ctx_get_param(provctx, OSSL_PKEY_PARAM_ML_KEM_OUTPUT_FORMATS, NULL);
  fmt_slots = ossl_ml_common_pkcs8_fmt_order(v->algorithm_name, codec->p8fmt,
                                             "output", formats);
  if (fmt_slots == NULL)
    return 0;
  ...
  default:
    ERR_raise_data(ERR_LIB_PROV, ERR_R_INTERNAL_ERROR,
                   "error encoding %s private key", v->algorithm_name);
    goto end;
  }
