...
static int read_pem(PROV_CTX *provctx, OSSL_CORE_BIO *cin, char **pem_name,
                    char **pem_header, unsigned char **data, long *len) {
...
pem2der_settable_ctx_params(ossl_unused void *provctx) {
...
static int pem2der_pass_helper(char *buf, int num, int w, void *data) {
    ...
    if (pass_data == NULL || pass_data->cb == NULL ||
...
#define SPKI_LAST_IDX 2
    ...
    ok =
    read_pem(ctx->provctx, cin, &pem_name, &pem_header, &der, &der_len) > 0;
    ...
    if (strlen(pem_header) > 10) {
        ...
        if (!PEM_get_EVP_CIPHER_INFO(pem_header, &cipher) ||
            !PEM_do_header(&cipher, der, &der_len, pem2der_pass_helper,
                           &pass_data))
            goto end;
    }
    ...
    if (i < OSSL_NELEM(pem_name_map)) {
        ...
        if (i <= PKCS8_LAST_IDX &&
            ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) ||
             OPENSSL_strcasecmp(ctx->data_structure,
                                "EncryptedPrivateKeyInfo") == 0 ||
             OPENSSL_strcasecmp(ctx->data_structure, "PrivateKeyInfo") == 0)) {
            ok = ossl_epki2pki_der_decode(
            der, der_len, selection, data_cb, data_cbarg, pw_cb, pw_cbarg,
        ...
        if (i <= SPKI_LAST_IDX &&
            ((selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) ||
             OPENSSL_strcasecmp(ctx->data_structure, "SubjectPublicKeyInfo") ==
             0)) {
        ...
        if (data_structure != NULL)
            *p++ = OSSL_PARAM_construct_utf8_string(
            OSSL_OBJECT_PARAM_DATA_STRUCTURE, data_structure, 0);
        *p++ =
        OSSL_PARAM_construct_octet_string(OSSL_OBJECT_PARAM_DATA, der, der_len);
...
{OSSL_FUNC_DECODER_SET_CTX_PARAMS, (void (*)(void))pem2der_set_ctx_params},
OSSL_DISPATCH_END};
