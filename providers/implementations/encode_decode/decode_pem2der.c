...
static int read_pem(PROV_CTX *provctx, OSSL_CORE_BIO *cin, char **pem_name, char **pem_header, unsigned char **data,
                    long *len)
{
...
static int pem2der_decode(void *vctx, OSSL_CORE_BIO *cin, int selection, OSSL_CALLBACK *data_cb, void *data_cbarg,
...
#define SPKI_LAST_IDX 2
    ...
    if (i < OSSL_NELEM(pem_name_map)) {
        ...
        if (i <= SPKI_LAST_IDX
            ...
            ok = ossl_spki2typespki_der_decode(der, der_len, selection, data_cb, data_cbarg, pw_cb, pw_cbarg,
                                               PROV_LIBCTX_OF(ctx->provctx), ctx->propq);
            goto end;
        }
...
const OSSL_DISPATCH ossl_pem_to_der_decoder_functions[] = {
    ...
    {OSSL_FUNC_DECODER_SETTABLE_CTX_PARAMS, (void (*)(void))pem2der_settable_ctx_params},
