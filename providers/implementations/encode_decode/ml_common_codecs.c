...
ML_COMMON_PKCS8_FMT_PREF *ossl_ml_common_pkcs8_fmt_order(const char *algorithm_name, const ML_COMMON_PKCS8_FMT *p8fmt,
                                                         const char *direction, const char *formats)
{
    ...
    do {
        ...
        for (i = 0; i < NUM_PKCS8_FORMATS; ++i) {
            ...
            if (ret[i].pref > 0 || OPENSSL_strncasecmp(ret[i].fmt->p8_name, fmt, (end - fmt)) != 0)
                continue;
            /* First time match */
            ret[i].pref = ++count;
            break;
    ...
    if (count == 0) {
        OPENSSL_free(ret);
        ERR_raise_data(ERR_LIB_PROV, PROV_R_ML_DSA_NO_FORMAT, "no %s private key %s formats are enabled",
