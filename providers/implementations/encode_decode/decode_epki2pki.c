...
epki2pki_settable_ctx_params(ossl_unused void *provctx) {
...
static int epki2pki_decode(void *vctx, OSSL_CORE_BIO *cin, int selection,
  ...
  ok =
  ossl_epki2pki_der_decode(der, der_len, selection, data_cb, data_cbarg, pw_cb,
                           pw_cbarg, PROV_LIBCTX_OF(ctx->provctx), ctx->propq);
  OPENSSL_free(der);
  return ok;
}
...
int ossl_epki2pki_der_decode(unsigned char *der, long der_len, int selection,
  ...
  if ((p8 = d2i_X509_SIG(NULL, &pder, der_len)) != NULL) {
    ...
    if (!pw_cb(pbuf, sizeof(pbuf), &plen, NULL, pw_cbarg)) {
      ...
      if (!PKCS12_pbe_crypt_ex(alg, pbuf, plen, oct->data, oct->length,
                               &new_der, &new_der_len, 0, libctx, propq)) {
        ok = 0;
      } else {
        der = new_der;
        der_len = new_der_len;
  ...
  if (p8inf != NULL && PKCS8_pkey_get0(NULL, NULL, NULL, &alg, p8inf)) {
    ...
    *p++ =
    OSSL_PARAM_construct_utf8_string(OSSL_OBJECT_PARAM_DATA_TYPE, keytype, 0);
    *p++ =
    OSSL_PARAM_construct_utf8_string(OSSL_OBJECT_PARAM_INPUT_TYPE, "DER", 0);
