...
key2pvk_settable_ctx_params(ossl_unused void *provctx) {
...
#define MAKE_MS_ENCODER(impl, output, type)                                    \
  ...
  static OSSL_FUNC_encoder_encode_fn impl##2##output##_encode;                 \
                                                                               \
  static void *impl##2##output##_import_object(void *ctx, int selection,       \
  ...
  OSSL_PASSPHRASE_CALLBACK *cb, void *cbarg) {                                 \
    ...
    return key2##output##_encode(vctx, key, selection, cout, type##_set1, cb,  \
                                 cbarg);                                       \
  }                                                                            \
  const OSSL_DISPATCH ossl_##impl##_to_##output##_encoder_functions[] = {      \
  {OSSL_FUNC_ENCODER_NEWCTX, (void (*)(void))key2ms_newctx},                   \
  {OSSL_FUNC_ENCODER_FREECTX, (void (*)(void))key2ms_freectx},                 \
  ...
  {OSSL_FUNC_ENCODER_ENCODE, (void (*)(void))impl##2##output##_encode},        \
  OSSL_DISPATCH_END}
