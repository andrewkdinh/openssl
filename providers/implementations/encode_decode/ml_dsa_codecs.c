...
ML_DSA_KEY *ossl_ml_dsa_d2i_PUBKEY(const uint8_t *pk, int pk_len, int evp_type,
  ...
  if (!ossl_ml_dsa_pk_decode(ret, pk, (size_t)pk_len)) {
    ERR_raise_data(ERR_LIB_PROV, PROV_R_BAD_ENCODING,
                   "errror parsing %s public key from input SPKI", params->alg);
...
ML_DSA_KEY *ossl_ml_dsa_d2i_PKCS8(const uint8_t *prvenc, int prvlen,
  ...
  formats =
  ossl_prov_ctx_get_param(provctx, OSSL_PKEY_PARAM_ML_DSA_INPUT_FORMATS, NULL);
  fmt_slots =
  ossl_ml_common_pkcs8_fmt_order(v->alg, codec->p8fmt, "input", formats);
  if (fmt_slots == NULL)
    goto end;
  ...
  if (p8fmt == NULL ||
    ...
    ERR_raise_data(ERR_LIB_PROV, PROV_R_ML_DSA_NO_FORMAT,
                   "no matching enabled %s private key input formats", v->alg);
    goto end;
  }
  ...
  if (ossl_ml_dsa_set_prekey(key, 0, 0, seed, ML_DSA_SEED_BYTES, priv,
                             v->sk_len))
    ret = key;
...
int ossl_ml_dsa_i2d_prvkey(const ML_DSA_KEY *key, uint8_t **out,
  ...
  if (sk == NULL) {
    ERR_raise_data(ERR_LIB_PROV, PROV_R_NOT_A_PRIVATE_KEY,
                   "no %s private key data available", params->alg);
    return 0;
  }

  formats =
  ossl_prov_ctx_get_param(provctx, OSSL_PKEY_PARAM_ML_DSA_OUTPUT_FORMATS, NULL);
  fmt_slots =
  ossl_ml_common_pkcs8_fmt_order(params->alg, codec->p8fmt, "output", formats);
  if (fmt_slots == NULL)
    return 0;
...
int ossl_ml_dsa_key_to_text(BIO *out, const ML_DSA_KEY *key, int selection) {
  ...
  if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0) {
    ...
    if (BIO_printf(out, "%s Private-Key:\n", params->alg) <= 0)
      return 0;
    if (seed != NULL &&
        !ossl_bio_print_labeled_buf(out, "seed:", seed, ML_DSA_SEED_BYTES))
      return 0;
