 ...
 */
static const ML_COMMON_CODEC codecs[3] = {{&ml_dsa_44_spkifmt, ml_dsa_44_p8fmt},
...
ML_DSA_KEY *ossl_ml_dsa_d2i_PUBKEY(const uint8_t *pk, int pk_len, int evp_type, PROV_CTX *provctx, const char *propq)
{
    ...
    if ((params = ossl_ml_dsa_params_get(evp_type)) == NULL || (codec = ml_dsa_get_codec(evp_type)) == NULL)
        return NULL;
    ...
    if (!ossl_ml_dsa_pk_decode(ret, pk, (size_t)pk_len)) {
        ERR_raise_data(ERR_LIB_PROV, PROV_R_BAD_ENCODING, "errror parsing %s public key from input SPKI", params->alg);
...
ML_DSA_KEY *ossl_ml_dsa_d2i_PKCS8(const uint8_t *prvenc, int prvlen, int evp_type, PROV_CTX *provctx, const char *propq)
{
    ...
    if ((v = ossl_ml_dsa_params_get(evp_type)) == NULL || (codec = ml_dsa_get_codec(evp_type)) == NULL)
        return 0;
    ...
    formats = ossl_prov_ctx_get_param(provctx, OSSL_PKEY_PARAM_ML_DSA_INPUT_FORMATS, NULL);
    fmt_slots = ossl_ml_common_pkcs8_fmt_order(v->alg, codec->p8fmt, "input", formats);
    if (fmt_slots == NULL)
        goto end;
    ...
    if (ptype != V_ASN1_UNDEF) {
        ERR_raise_data(ERR_LIB_PROV, PROV_R_UNEXPECTED_KEY_PARAMETERS,
                       "unexpected parameters with a PKCS#8 %s private key", v->alg);
        goto end;
    }
    ...
    if (p8fmt == NULL || (p8fmt->seed_length > 0 && p8fmt->seed_length != ML_DSA_SEED_BYTES)
        || (p8fmt->priv_length > 0 && p8fmt->priv_length != v->sk_len)
        || (p8fmt->pub_length > 0 && p8fmt->pub_length != v->pk_len)) {
        ERR_raise_data(ERR_LIB_PROV, PROV_R_ML_DSA_NO_FORMAT, "no matching enabled %s private key input formats",
                       v->alg);
        goto end;
    }
    ...
    if (ossl_ml_dsa_set_prekey(key, 0, 0, seed, ML_DSA_SEED_BYTES, priv, v->sk_len))
        ret = key;
...
int ossl_ml_dsa_i2d_pubkey(const ML_DSA_KEY *key, unsigned char **out)
{
    ...
    if (pk == NULL) {
        ERR_raise_data(ERR_LIB_PROV, PROV_R_NOT_A_PUBLIC_KEY, "no %s public key data available", params->alg);
        return 0;
    }
    if (out != NULL && (*out = OPENSSL_memdup(pk, params->pk_len)) == NULL)
        return 0;
...
int ossl_ml_dsa_i2d_prvkey(const ML_DSA_KEY *key, uint8_t **out, PROV_CTX *provctx)
{
    ...
    if (sk == NULL) {
        ERR_raise_data(ERR_LIB_PROV, PROV_R_NOT_A_PRIVATE_KEY, "no %s private key data available", params->alg);
        return 0;
    }

    formats = ossl_prov_ctx_get_param(provctx, OSSL_PKEY_PARAM_ML_DSA_OUTPUT_FORMATS, NULL);
    fmt_slots = ossl_ml_common_pkcs8_fmt_order(params->alg, codec->p8fmt, "output", formats);
    if (fmt_slots == NULL)
        return 0;
    ...
    if (p8fmt == NULL || (p8fmt->seed_length > 0 && p8fmt->seed_length != ML_DSA_SEED_BYTES)
        ...
        ERR_raise_data(ERR_LIB_PROV, PROV_R_ML_DSA_NO_FORMAT, "no matching enabled %s private key output formats",
                       params->alg);
        goto end;
    }
    len = p8fmt->p8_bytes;
    ...
    case 4:
        break;
    default:
        ERR_raise_data(ERR_LIB_PROV, ERR_R_INTERNAL_ERROR, "error encoding %s private key", params->alg);
        goto end;
    }
...
int ossl_ml_dsa_key_to_text(BIO *out, const ML_DSA_KEY *key, int selection)
{
    ...
    if (pk == NULL) {
        ...
        ERR_raise_data(ERR_LIB_PROV, PROV_R_MISSING_KEY, "no %s key material available", params->alg);
        return 0;
    }
