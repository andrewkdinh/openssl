...
static int pvk2key_decode(void *vctx, OSSL_CORE_BIO *cin, int selection,
    ...
    if ((selection == 0 ||
        ...
        key =
        ctx->desc->read_private_key(in, ossl_pw_pvk_password, &pwdata,
        ...
        if (lib == ERR_LIB_PEM && (reason == PEM_R_BAD_PASSWORD_READ ||
...
next:
    ...
    if (key != NULL) {
        ...
        OSSL_OBJECT_PARAM_DATA_TYPE, (char *)ctx->desc->name, 0);
