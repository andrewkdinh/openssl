...
pvk2key_settable_ctx_params(ossl_unused void *provctx) {
...
static int pvk2key_decode(void *vctx, OSSL_CORE_BIO *cin, int selection,
  ...
  if ((selection == 0 || (selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0) &&
    ...
    if (lib == ERR_LIB_PEM &&
        (reason == PEM_R_BAD_PASSWORD_READ || reason == PEM_R_BAD_DECRYPT)) {
      ERR_clear_last_mark();
      goto end;
    }
...
static int pvk2key_export_object(void *vctx, const void *reference,
...
#define IMPLEMENT_MS(KEYTYPE, keytype)                                         \
  ...
  {OSSL_FUNC_DECODER_DOES_SELECTION, (void (*)(void))pvk2key_does_selection},  \
  ...
  {OSSL_FUNC_DECODER_SET_CTX_PARAMS, (void (*)(void))pvk2key_set_ctx_params},  \
  OSSL_DISPATCH_END}
