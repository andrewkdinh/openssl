 ...
 */
        ...
        struct der2key_ctx_st *ctx, key_from_pkcs8_t *key_from_pkcs8)
    {
        ...
        if ((p8inf = d2i_PKCS8_PRIV_KEY_INFO(NULL, input_der, input_der_len))
            ...
            key =
                key_from_pkcs8(p8inf, PROV_LIBCTX_OF(ctx->provctx), ctx->propq);
    ...
    static struct der2key_ctx_st *der2key_newctx(
        void *provctx, const struct keytype_desc_st *desc)
    {
    ...
    static const OSSL_PARAM *der2key_settable_ctx_params(
...
# endif
    ...
    static ossl_inline void *ml_kem_d2i_PUBKEY(
        const uint8_t **der, long der_len, struct der2key_ctx_st *ctx)
    {
        ML_KEM_KEY *key;
...
# define ml_kem_1024_adjust                 NULL
    ...
    static ossl_inline void *slh_dsa_d2i_PUBKEY(
        const uint8_t **der, long der_len, struct der2key_ctx_st *ctx)
    {
        int ok = 0;
        OSSL_LIB_CTX *libctx = PROV_LIBCTX_OF(ctx->provctx);
     ...
     */
        ...
        if (!ossl_slh_dsa_set_pub(ret, spki->pubkey->data,
                                  spki->pubkey->length)) {
            ERR_raise_data(ERR_LIB_PROV, PROV_R_BAD_ENCODING,
...
#define rsapss_adjust                   rsa_adjust
    ...
    static ossl_inline void *ml_dsa_d2i_PUBKEY(
        const uint8_t **der, long der_len, struct der2key_ctx_st *ctx)
    {
        ML_DSA_KEY *key;
