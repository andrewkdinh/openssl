...
ASN1_SEQUENCE(BARE_ALGOR)
    =
        {
            ...
            ASN1_SEQUENCE(
                BARE_PUBKEY) = {ASN1_EMBED(BARE_PUBKEY, algor, BARE_ALGOR),
                                ASN1_SIMPLE(BARE_PUBKEY, pubkey, ASN1_BIT_STRING)} static_ASN1_SEQUENCE_END(BARE_PUBKEY)
    ...
    typedef void *d2i_PKCS8_fn(const unsigned char **, long, struct der2key_ctx_st *);
    typedef void *d2i_PUBKEY_fn(const unsigned char **, long, struct der2key_ctx_st *);
 ...
 */
    ...
    static void *der2key_decode_p8(const unsigned char **input_der, long input_der_len, struct der2key_ctx_st *ctx,
                                   key_from_pkcs8_t *key_from_pkcs8)
    {
        ...
        if ((p8inf = d2i_PKCS8_PRIV_KEY_INFO(NULL, input_der, input_der_len)) != NULL
            ...
            /* Allow decoding sm2 private key with id_ecPublicKey */
                || (OBJ_obj2nid(alg->algorithm) == NID_X9_62_id_ecPublicKey && ctx->desc->evp_type == NID_sm2)))
...
/* ---------------------------------------------------------------------- */
    ...
    static int der2key_decode(void *vctx, OSSL_CORE_BIO *cin, int selection, OSSL_CALLBACK *data_cb, void *data_cbarg,
...
next:
     ...
     */
        ...
        if (key != NULL) {
            ...
            if (strcmp(ctx->desc->keytype_name, "EC") == 0 && (EC_KEY_get_flags(key) & EC_FLAG_SM2_RANGE) != 0)
                params[1] = OSSL_PARAM_construct_utf8_string(OSSL_OBJECT_PARAM_DATA_TYPE, "SM2", 0);
            else
#endif
                params[1] =
...
# define dh_check                       NULL
    ...
    static void *dh_d2i_PKCS8(const unsigned char **der, long der_len, struct der2key_ctx_st *ctx)
    {
...
# define dsa_check                      NULL
    ...
    static void *dsa_d2i_PKCS8(const unsigned char **der, long der_len, struct der2key_ctx_st *ctx)
    {
...
# define ec_free                        (free_key_fn *)EC_KEY_free
    ...
    static void *ec_d2i_PKCS8(const unsigned char **der, long der_len, struct der2key_ctx_st *ctx)
    {
    ...
    /* We're trying to be clever by comparing two truths */
        ...
        if (sm2)
            ret = ctx->desc->evp_type == EVP_PKEY_SM2 || ctx->desc->evp_type == NID_X9_62_id_ecPublicKey;
        else
 ...
 */
    ...
    static void *ecx_d2i_PKCS8(const unsigned char **der, long der_len, struct der2key_ctx_st *ctx)
    {
...
#  define sm2_adjust                    ec_adjust
    ...
    static void *sm2_d2i_PKCS8(const unsigned char **der, long der_len, struct der2key_ctx_st *ctx)
    {
        return der2key_decode_p8(der, der_len, ctx, (key_from_pkcs8_t *)ossl_ec_key_from_pkcs8);
    }
# endif
...
/* ---------------------------------------------------------------------- */
    ...
    static void *ml_kem_d2i_PKCS8(const uint8_t **der, long der_len, struct der2key_ctx_st *ctx)
    {
        ML_KEM_KEY *key;

        key = ossl_ml_kem_d2i_PKCS8(*der, der_len, ctx->desc->evp_type, ctx->provctx, ctx->propq);
        if (key != NULL)
            *der += der_len;
        return key;
    }

    static ossl_inline void *ml_kem_d2i_PUBKEY(const uint8_t **der, long der_len, struct der2key_ctx_st *ctx)
    {
        ML_KEM_KEY *key;

        key = ossl_ml_kem_d2i_PUBKEY(*der, der_len, ctx->desc->evp_type, ctx->provctx, ctx->propq);
        if (key != NULL)
            *der += der_len;
        return key;
    }
...
# define ml_kem_1024_adjust                 NULL
...
#ifndef OPENSSL_NO_SLH_DSA
    static void *slh_dsa_d2i_PKCS8(const uint8_t **der, long der_len, struct der2key_ctx_st *ctx)
    {
    ...
    /* Algorithm parameters must be absent. */
        if ((X509_ALGOR_get0(NULL, &ptype, NULL, alg), ptype != V_ASN1_UNDEF)) {
            ERR_raise_data(ERR_LIB_PROV, PROV_R_UNEXPECTED_KEY_PARAMETERS,
                           "unexpected parameters with a PKCS#8 %s private key", ctx->desc->keytype_name);
            goto end;
        }
        if (OBJ_obj2nid(alg->algorithm) != ctx->desc->evp_type)
            goto end;
        if ((key = ossl_slh_dsa_key_new(libctx, ctx->propq, ctx->desc->keytype_name)) == NULL)
            goto end;
    ...
    static ossl_inline void *slh_dsa_d2i_PUBKEY(const uint8_t **der, long der_len, struct der2key_ctx_st *ctx)
    {
        int ok = 0;
        OSSL_LIB_CTX *libctx = PROV_LIBCTX_OF(ctx->provctx);
     ...
     */
        if (der_len != 18 + (long)len) {
            ERR_raise_data(ERR_LIB_PROV, PROV_R_BAD_ENCODING, "unexpected %s public key length: %ld != %ld",
                           ctx->desc->keytype_name, der_len, 18 + (long)len);
            goto err;
        }
    ...
    /* The spki storage is freed on error */
        if (ASN1_item_d2i_ex((ASN1_VALUE **)&spki, &end, der_len, ASN1_ITEM_rptr(BARE_PUBKEY), NULL, NULL) == NULL) {
            ERR_raise_data(ERR_LIB_PROV, PROV_R_BAD_ENCODING, "malformed %s public key ASN.1 encoding",
    ...
    /* The spki structure now owns some memory */
        ...
        if (OBJ_cmp(OBJ_nid2obj(ctx->desc->evp_type), spki->algor.oid) != 0) {
            ERR_raise_data(ERR_LIB_PROV, PROV_R_BAD_ENCODING, "unexpected algorithm OID for an %s public key",
                           ossl_slh_dsa_key_get_name(ret));
            goto err;
        }

        if (!ossl_slh_dsa_set_pub(ret, spki->pubkey->data, spki->pubkey->length)) {
            ERR_raise_data(ERR_LIB_PROV, PROV_R_BAD_ENCODING, "failed to parse %s public key from the input data",
                           ossl_slh_dsa_key_get_name(ret));
            goto err;
        }
        ok = 1;
...
#define rsa_free                        (free_key_fn *)RSA_free
    ...
    static void *rsa_d2i_PKCS8(const unsigned char **der, long der_len, struct der2key_ctx_st *ctx)
    {
        return der2key_decode_p8(der, der_len, ctx, (key_from_pkcs8_t *)ossl_rsa_key_from_pkcs8);
    }

    static void *rsa_d2i_PUBKEY(const unsigned char **der, long der_len, ossl_unused struct der2key_ctx_st *ctx)
    {
...
/* ---------------------------------------------------------------------- */
    ...
    static void *ml_dsa_d2i_PKCS8(const uint8_t **der, long der_len, struct der2key_ctx_st *ctx)
    {
        ML_DSA_KEY *key;

        key = ossl_ml_dsa_d2i_PKCS8(*der, der_len, ctx->desc->evp_type, ctx->provctx, ctx->propq);
        if (key != NULL)
            *der += der_len;
        return key;
    }

    static ossl_inline void *ml_dsa_d2i_PUBKEY(const uint8_t **der, long der_len, struct der2key_ctx_st *ctx)
    {
        ML_DSA_KEY *key;

        key = ossl_ml_dsa_d2i_PUBKEY(*der, der_len, ctx->desc->evp_type, ctx->provctx, ctx->propq);
        if (key != NULL)
            *der += der_len;
        return key;
    }
