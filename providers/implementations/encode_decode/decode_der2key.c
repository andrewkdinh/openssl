...
{ASN1_EMBED(BARE_PUBKEY, algor, BARE_ALGOR),
 ASN1_SIMPLE(BARE_PUBKEY, pubkey,
             ASN1_BIT_STRING)} static_ASN1_SEQUENCE_END(BARE_PUBKEY)
...
der2key_settable_ctx_params(ossl_unused void *provctx) {
...
next:
  ...
  if (key != NULL) {
    ...
    if (strcmp(ctx->desc->keytype_name, "EC") == 0 &&
      ...
      params[1] =
      OSSL_PARAM_construct_utf8_string(OSSL_OBJECT_PARAM_DATA_TYPE, "SM2", 0);
    else
#endif
...
#define D2I_PUBKEY_NOCTX(n, f)                                                 \
  static void *n##_d2i_PUBKEY(const unsigned char **der, long der_len,         \
...
static void *ml_kem_d2i_PKCS8(const uint8_t **der, long der_len,
                              struct der2key_ctx_st *ctx) {
  ML_KEM_KEY *key;

  key = ossl_ml_kem_d2i_PKCS8(*der, der_len, ctx->desc->evp_type, ctx->provctx,
                              ctx->propq);
  if (key != NULL)
    *der += der_len;
  return key;
...
static ossl_inline void *ml_kem_d2i_PUBKEY(const uint8_t **der, long der_len,
                                           struct der2key_ctx_st *ctx) {
  ML_KEM_KEY *key;

  key = ossl_ml_kem_d2i_PUBKEY(*der, der_len, ctx->desc->evp_type, ctx->provctx,
                               ctx->propq);
  if (key != NULL)
    *der += der_len;
  return key;
...
#ifndef OPENSSL_NO_SLH_DSA
static void *slh_dsa_d2i_PKCS8(const uint8_t **der, long der_len,
                               struct der2key_ctx_st *ctx) {
...
static void *rsa_d2i_PUBKEY(const unsigned char **der, long der_len,
                            ossl_unused struct der2key_ctx_st *ctx) {
...
static void *ml_dsa_d2i_PKCS8(const uint8_t **der, long der_len,
                              struct der2key_ctx_st *ctx) {
  ML_DSA_KEY *key;

  key = ossl_ml_dsa_d2i_PKCS8(*der, der_len, ctx->desc->evp_type, ctx->provctx,
                              ctx->propq);
  if (key != NULL)
    *der += der_len;
  return key;
...
static ossl_inline void *ml_dsa_d2i_PUBKEY(const uint8_t **der, long der_len,
  ...
  key = ossl_ml_dsa_d2i_PUBKEY(*der, der_len, ctx->desc->evp_type, ctx->provctx,
                               ctx->propq);
  if (key != NULL)
    *der += der_len;
  return key;
...
#define DO_type_specific_keypair(keytype)                                      \
  "type-specific", keytype##_evp_type, (OSSL_KEYMGMT_SELECT_KEYPAIR),          \
  keytype##_d2i_private_key, keytype##_d2i_public_key, NULL, NULL, NULL,       \
  keytype##_check, keytype##_adjust, keytype##_free

#define DO_type_specific_pub(keytype)                                          \
  "type-specific", keytype##_evp_type, (OSSL_KEYMGMT_SELECT_PUBLIC_KEY), NULL, \
  keytype##_d2i_public_key, NULL, NULL, NULL, keytype##_check,                 \
  keytype##_adjust, keytype##_free

#define DO_type_specific_priv(keytype)                                         \
  "type-specific", keytype##_evp_type, (OSSL_KEYMGMT_SELECT_PRIVATE_KEY),      \
  keytype##_d2i_private_key, NULL, NULL, NULL, NULL, keytype##_check,          \
  keytype##_adjust, keytype##_free

#define DO_type_specific_params(keytype)                                       \
  "type-specific", keytype##_evp_type, (OSSL_KEYMGMT_SELECT_ALL_PARAMETERS),   \
  NULL, NULL, keytype##_d2i_key_params, NULL, NULL, keytype##_check,           \
  keytype##_adjust, keytype##_free

#define DO_type_specific(keytype)                                              \
  "type-specific", keytype##_evp_type, (OSSL_KEYMGMT_SELECT_ALL),              \
  keytype##_d2i_private_key, keytype##_d2i_public_key,                         \
  keytype##_d2i_key_params, NULL, NULL, keytype##_check, keytype##_adjust,     \
  keytype##_free

#define DO_type_specific_no_pub(keytype)                                       \
  "type-specific", keytype##_evp_type,                                         \
  (OSSL_KEYMGMT_SELECT_PRIVATE_KEY | OSSL_KEYMGMT_SELECT_ALL_PARAMETERS),      \
  keytype##_d2i_private_key, NULL, keytype##_d2i_key_params, NULL, NULL,       \
  keytype##_check, keytype##_adjust, keytype##_free

#define DO_PrivateKeyInfo(keytype)                                             \
  "PrivateKeyInfo", keytype##_evp_type, (OSSL_KEYMGMT_SELECT_PRIVATE_KEY),     \
  NULL, NULL, NULL, keytype##_d2i_PKCS8, NULL, keytype##_check,                \
  keytype##_adjust, keytype##_free

#define DO_SubjectPublicKeyInfo(keytype)                                       \
  "SubjectPublicKeyInfo", keytype##_evp_type,                                  \
  (OSSL_KEYMGMT_SELECT_PUBLIC_KEY), NULL, NULL, NULL, NULL,                    \
  keytype##_d2i_PUBKEY, keytype##_check, keytype##_adjust, keytype##_free

#define DO_DH(keytype)                                                         \
  "DH", keytype##_evp_type, (OSSL_KEYMGMT_SELECT_ALL_PARAMETERS), NULL, NULL,  \
  keytype##_d2i_key_params, NULL, NULL, keytype##_check, keytype##_adjust,     \
  keytype##_free

#define DO_DHX(keytype)                                                        \
  "DHX", keytype##_evp_type, (OSSL_KEYMGMT_SELECT_ALL_PARAMETERS), NULL, NULL, \
  keytype##_d2i_key_params, NULL, NULL, keytype##_check, keytype##_adjust,     \
  keytype##_free

#define DO_DSA(keytype)                                                        \
  "DSA", keytype##_evp_type, (OSSL_KEYMGMT_SELECT_ALL),                        \
  keytype##_d2i_private_key, keytype##_d2i_public_key,                         \
  keytype##_d2i_key_params, NULL, NULL, keytype##_check, keytype##_adjust,     \
  keytype##_free
...
#define DO_RSA(keytype)                                                        \
  "RSA", keytype##_evp_type, (OSSL_KEYMGMT_SELECT_KEYPAIR),                    \
  keytype##_d2i_private_key, keytype##_d2i_public_key, NULL, NULL, NULL,       \
  keytype##_check, keytype##_adjust, keytype##_free
...
#define MAKE_DECODER(keytype_name, keytype, type, kind)                        \
  ...
  {OSSL_FUNC_DECODER_SET_CTX_PARAMS, (void (*)(void))der2key_set_ctx_params},  \
  OSSL_DISPATCH_END}
...
MAKE_DECODER("SLH-DSA-SHA2-128s", slh_dsa_sha2_128s, slh_dsa,
             SubjectPublicKeyInfo);
MAKE_DECODER("SLH-DSA-SHA2-128f", slh_dsa_sha2_128f, slh_dsa,
             SubjectPublicKeyInfo);
MAKE_DECODER("SLH-DSA-SHA2-192s", slh_dsa_sha2_192s, slh_dsa,
             SubjectPublicKeyInfo);
MAKE_DECODER("SLH-DSA-SHA2-192f", slh_dsa_sha2_192f, slh_dsa,
             SubjectPublicKeyInfo);
MAKE_DECODER("SLH-DSA-SHA2-256s", slh_dsa_sha2_256s, slh_dsa,
             SubjectPublicKeyInfo);
MAKE_DECODER("SLH-DSA-SHA2-256f", slh_dsa_sha2_256f, slh_dsa,
             SubjectPublicKeyInfo);
MAKE_DECODER("SLH-DSA-SHAKE-128s", slh_dsa_shake_128s, slh_dsa,
             SubjectPublicKeyInfo);
MAKE_DECODER("SLH-DSA-SHAKE-128f", slh_dsa_shake_128f, slh_dsa,
             SubjectPublicKeyInfo);
MAKE_DECODER("SLH-DSA-SHAKE-192s", slh_dsa_shake_192s, slh_dsa,
             SubjectPublicKeyInfo);
MAKE_DECODER("SLH-DSA-SHAKE-192f", slh_dsa_shake_192f, slh_dsa,
             SubjectPublicKeyInfo);
MAKE_DECODER("SLH-DSA-SHAKE-256s", slh_dsa_shake_256s, slh_dsa,
             SubjectPublicKeyInfo);
MAKE_DECODER("SLH-DSA-SHAKE-256f", slh_dsa_shake_256f, slh_dsa,
             SubjectPublicKeyInfo);
#endif /* OPENSSL_NO_SLH_DSA */
