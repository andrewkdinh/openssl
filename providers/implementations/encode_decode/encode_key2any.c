...
static PKCS8_PRIV_KEY_INFO *key_to_p8info(const void *key, int key_nid,
  ...
  if ((p8info = PKCS8_PRIV_KEY_INFO_new()) == NULL ||
      ...
      !PKCS8_pkey_set0(p8info, OBJ_nid2obj(key_nid), 0, params_type, params,
                       der, derlen)) {
...
static X509_SIG *key_to_encp8(const void *key, int key_nid, void *params,
                              int params_type, OSSL_i2d_of_void_ctx *k2d,
                              KEY2ANY_CTX *ctx) {
...
static X509_PUBKEY *key_to_pubkey(const void *key, int key_nid, void *params,
                                  int params_type, OSSL_i2d_of_void_ctx *k2d,
                                  KEY2ANY_CTX *ctx) {
  /* der, derlen store the key DER output and its length */
  unsigned char *der = NULL;
  ...
  if ((xpk = X509_PUBKEY_new()) == NULL ||
      (derlen = k2d(key, &der, (void *)ctx)) <= 0 ||
      !X509_PUBKEY_set0_param(xpk, OBJ_nid2obj(key_nid), params_type, params,
                              der, derlen)) {
 ...
 */
static int key_to_type_specific_der_bio(BIO *out, const void *key, int key_nid,
                                        ossl_unused const char *pemname,
                                        key_to_paramstring_fn *p2s,
                                        OSSL_i2d_of_void_ctx *k2d,
                                        KEY2ANY_CTX *ctx) {
...
static int key_to_type_specific_pem_priv_bio(BIO *out, const void *key,
                                             ...
                                             OSSL_i2d_of_void_ctx *k2d,
                                             KEY2ANY_CTX *ctx) {
  return key_to_type_specific_pem_bio_cb(out, key, key_nid, pemname, p2s, k2d,
                                         ctx, ossl_pw_pem_password,
                                         &ctx->pwdata);
}
...
#define k2d_NOCTX(n, f)                                                        \
  static int n##_k2d(const void *key, unsigned char **pder,                    \
...
static int prepare_dh_params(const void *dh, int nid, int save, void **pstr,
                             int *pstrtype) {
...
static int encode_dsa_params(const void *dsa, int nid, void **pstr,
                             int *pstrtype) {
  ASN1_STRING *params = ASN1_STRING_new();
...
k2d_NOCTX(dsa_prv, i2d_DSAPrivateKey) k2d_NOCTX(dsa_pub, i2d_DSAPublicKey)
 ...
 * ecx_type_specific_[priv,pub,params]_to_der.
 */
...
static int key2any_encode(KEY2ANY_CTX *ctx, OSSL_CORE_BIO *cout,
                          ...
                          check_key_type_fn *checker, key_to_der_fn *writer,
  ...
  if (key == NULL) {
    ...
    if (out != NULL && (pwcb == NULL || ossl_pw_set_ossl_passphrase_cb(
                                        &ctx->pwdata, pwcb, pwcbarg)))
...
#define DO_PRIVATE_KEY(impl, type, kind, output)                               \
  if ((selection & DO_PRIVATE_KEY_selection_mask) != 0)                        \
    ...
    ctx, cout, key, impl##_evp_type, impl##_pem_type " PRIVATE KEY",           \
    type##_check_key_type, key_to_##kind##_##output##_priv_bio, cb, cbarg,     \
...
#define DO_PUBLIC_KEY(impl, type, kind, output)                                \
  if ((selection & DO_PUBLIC_KEY_selection_mask) != 0)                         \
    return key2any_encode(                                                     \
    ctx, cout, key, impl##_evp_type, impl##_pem_type " PUBLIC KEY",            \
    type##_check_key_type, key_to_##kind##_##output##_pub_bio, cb, cbarg,      \
...
#define DO_PARAMETERS(impl, type, kind, output)                                \
  if ((selection & DO_PARAMETERS_selection_mask) != 0)                         \
    return key2any_encode(                                                     \
    ctx, cout, key, impl##_evp_type, impl##_pem_type " PARAMETERS",            \
    type##_check_key_type, key_to_##kind##_##output##_param_bio, NULL, NULL,   \
...
#define MAKE_ENCODER(impl, type, kind, output)                                 \
  ...
  void *vctx, int selection, const OSSL_PARAM params[]) {                      \
    KEY2ANY_CTX *ctx = vctx;                                                   \
                                                                               \
    return ossl_prov_import_key(ossl_##impl##_keymgmt_functions, ctx->provctx, \
                                selection, params);                            \
  }                                                                            \
