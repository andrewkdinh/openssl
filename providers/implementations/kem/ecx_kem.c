 ...
 */
static int
generate_ecxdhkm(const ECX_KEY *sender, const ECX_KEY *peer, unsigned char *out, size_t maxout, unsigned int secretsz)
{
    size_t len = 0;
 ...
 */
static int derive_secret(PROV_ECX_CTX        *ctx,
                         unsigned char       *secret,
                         const ECX_KEY       *privkey1,
                         const ECX_KEY       *peerkey1,
                         const ECX_KEY       *privkey2,
                         const ECX_KEY       *peerkey2,
                         ...
                         const unsigned char *recipient_pub)
{
    ...
    if (!generate_ecxdhkm(privkey1, peerkey1, dhkm, sizeof(dhkm), (unsigned int)encodedkeylen))
        goto err;
    dhkmlen = encodedkeylen;

    /* Concat the optional second ECXDH (used for Auth) */
    if (auth) {
        if (!generate_ecxdhkm(privkey2, peerkey2, dhkm + dhkmlen, sizeof(dhkm) - dhkmlen, (unsigned int)encodedkeylen))
            goto err;
 ...
 */
static int
dhkem_decap(PROV_ECX_CTX *ctx, unsigned char *secret, size_t *secretlen, const unsigned char *enc, size_t enclen)
{
    int                       ret                = 0;
