                                    ...
                                    size_t               kemctxlen)
{
    ...
    ret        = ossl_hpke_labeled_extract(kctx,
                                    prk,
                                    prklen,
                                    NULL,
                                    0,
...
int ossl_ec_dhkem_derive_private(EC_KEY *ec, BIGNUM *priv, const unsigned char *ikm, size_t ikmlen)
{
    ...
    if (!ossl_hpke_labeled_extract(kdfctx,
                                   prk,
                                   info->Nsecret,
                                   NULL,
                                   0,
    ...
    do {
        if (!ossl_hpke_labeled_expand(kdfctx,
                                      privbuf,
                                      info->Nsk,
                                      prk,
                                      info->Nsecret,
 ...
 */
static int
generate_ecdhkm(const EC_KEY *sender, const EC_KEY *peer, unsigned char *out, size_t maxout, unsigned int secretsz)
{
 ...
 */
static int derive_secret(PROV_EC_CTX         *ctx,
                         unsigned char       *secret,
                         const EC_KEY        *privkey1,
                         const EC_KEY        *peerkey1,
                         const EC_KEY        *privkey2,
                         const EC_KEY        *peerkey2,
                         ...
                         const unsigned char *recipient_pub)
{
    ...
    if (!generate_ecdhkm(privkey1, peerkey1, dhkm, sizeof(dhkm), (unsigned int)encodedprivlen))
        goto err;
    ...
    if (auth) {
        ...
        if (!generate_ecdhkm(privkey2, peerkey2, dhkm + dhkmlen, sizeof(dhkm) - dhkmlen, (unsigned int)encodedprivlen))
            goto err;
        dhkmlen   += encodedprivlen;
 ...
 */
static int
dhkem_decap(PROV_EC_CTX *ctx, unsigned char *secret, size_t *secretlen, const unsigned char *enc, size_t enclen)
{
    int                       ret                = 0;
