 ...
 */
static EC_KEY *eckey_frompub(EC_KEY *in, const unsigned char *pubbuf,
                             size_t pubbuflen) {
  EC_KEY *key;
...
static int ecpubkey_todata(const EC_KEY *ec, unsigned char *out, size_t *outlen,
  ...
  *outlen = EC_POINT_point2oct(group, pub, POINT_CONVERSION_UNCOMPRESSED, out,
                               maxoutlen, NULL);
  return *outlen != 0;
}
...
static int ossl_ec_match_params(const EC_KEY *key1, const EC_KEY *key2) {
  ...
  ret =
  group1 != NULL && group2 != NULL && EC_GROUP_cmp(group1, group2, ctx) == 0;
  if (!ret)
 ...
 */
static int dhkem_extract_and_expand(EVP_KDF_CTX *kctx, unsigned char *okm,
                                    size_t okmlen, uint16_t kemid,
  ...
  ret = ossl_hpke_labeled_extract(kctx, prk, prklen, NULL, 0, LABEL_KEM,
                                  suiteid, sizeof(suiteid),
                                  OSSL_DHKEM_LABEL_EAE_PRK, dhkm, dhkmlen) &&
        ossl_hpke_labeled_expand(
        kctx, okm, okmlen, prk, prklen, LABEL_KEM, suiteid, sizeof(suiteid),
...
int ossl_ec_dhkem_derive_private(EC_KEY *ec, BIGNUM *priv,
  ...
  kdfctx = ossl_kdf_ctx_create("HKDF", info->mdname, ossl_ec_key_get_libctx(ec),
  ...
  if (ikmlen < info->Nsk) {
    ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_INPUT_LENGTH,
                   "ikm length is :%zu, should be at least %zu", ikmlen,
                   info->Nsk);
    goto err;
  }
  ...
  if (!ossl_hpke_labeled_extract(kdfctx, prk, info->Nsecret, NULL, 0, LABEL_KEM,
                                 suiteid, sizeof(suiteid),
  ...
  do {
    if (!ossl_hpke_labeled_expand(
        kdfctx, privbuf, info->Nsk, prk, info->Nsecret, LABEL_KEM, suiteid,
 ...
 */
static EC_KEY *derivekey(PROV_EC_CTX *ctx, const unsigned char *ikm,
                         size_t ikmlen) {
  int ret = 0;
  EC_KEY *key;
...
static int generate_ecdhkm(const EC_KEY *sender, const EC_KEY *peer,
  ...
  if (!check_publickey(peer))
    return 0;
  return ECDH_compute_key(out, secretlen, EC_KEY_get0_public_key(peer), sender,
                          NULL) > 0;
}
...
static int derive_secret(PROV_EC_CTX *ctx, unsigned char *secret,
  ...
  if (auth) {
    ...
    if (!generate_ecdhkm(privkey2, peerkey2, dhkm + dhkmlen,
                         sizeof(dhkm) - dhkmlen, encodedprivlen))
      goto err;
    dhkmlen += encodedprivlen;
  ...
  kdfctx =
  ossl_kdf_ctx_create(ctx->kdfname, info->mdname, ctx->libctx, ctx->propq);
  if (kdfctx == NULL)
    goto err;
  if (!dhkem_extract_and_expand(kdfctx, secret, info->Nsecret, info->kem_id,
                                dhkm, dhkmlen, kemctx, kemctxlen))
    goto err;
  ret = 1;
err:
...
static int dhkem_encap(PROV_EC_CTX *ctx, unsigned char *enc, size_t *enclen,
  ...
  if (!ecpubkey_todata(sender_ephemkey, sender_pub, &sender_publen,
                       sizeof(sender_pub)) ||
      !ecpubkey_todata(ctx->recipient_key, recipient_pub, &recipient_publen,
  ...
  if (!derive_secret(ctx, secret, sender_ephemkey, ctx->recipient_key,
                     ctx->sender_authkey, ctx->recipient_key, sender_pub,
                     recipient_pub))
    goto err;
 ...
 *     secretlen Passes in the max size of the secret buffer and returns the
 *               secret length.
 *     enc A buffer containing the senders ephemeral public key that was
 * returned from dhkem_encap(). enclen The length in bytes of enc. Returns: 1 If
 * the shared secret is returned or 0 on error.
 */
static int dhkem_decap(PROV_EC_CTX *ctx, unsigned char *secret,
                       size_t *secretlen, const unsigned char *enc,
                       size_t enclen) {
  int ret = 0;
  ...
  if (!derive_secret(ctx, secret, ctx->recipient_key, sender_ephempubkey,
                     ctx->recipient_key, ctx->sender_authkey, enc,
                     recipient_pub))
    goto err;
  *secretlen = info->Nsecret;
  ret = 1;
