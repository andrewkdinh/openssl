...
static int dsa_check_key(PROV_DSA_CTX *ctx, int sign, const char *desc) {
    ...
    if (!approved) {
        if (!OSSL_FIPS_IND_ON_UNAPPROVED(
            ctx, OSSL_FIPS_IND_SETTABLE0, ctx->libctx, desc, "DSA Key",
            ossl_fips_config_signature_digest_check)) {
...
static int dsa_digest_signverify_init(void *vpdsactx, const char *mdname,
    ...
    if (mdname != NULL
        ...
        && (mdname[0] == '\0' ||
            OPENSSL_strcasecmp(pdsactx->mdname, mdname) != 0) &&
...
#define IMPL_DSA_SIGALG(md, MD)                                                \
    ...
    static int dsa_##md##_sign_init(void *vpdsactx, void *vdsa,                \
        ...
        return dsa_sigalg_signverify_init(vpdsactx, vdsa,                      \
                                          dsa_sigalg_set_ctx_params, params,   \
                                          MD, EVP_PKEY_OP_SIGN, desc);         \
    }                                                                          \
                                                                               \
