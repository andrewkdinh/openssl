...
dsa_sigalg_settable_ctx_params;
...
static int dsa_setup_md(PROV_DSA_CTX *ctx, const char *mdname,
...
static int dsa_sign_check_approved(PROV_DSA_CTX *ctx, int signing,
  ...
  if (signing && !OSSL_FIPS_IND_ON_UNAPPROVED(
                 ctx, OSSL_FIPS_IND_SETTABLE2, ctx->libctx, desc, "DSA",
...
static int dsa_check_key(PROV_DSA_CTX *ctx, int sign, const char *desc) {
  ...
  if (!approved) {
    if (!OSSL_FIPS_IND_ON_UNAPPROVED(ctx, OSSL_FIPS_IND_SETTABLE0, ctx->libctx,
                                     desc, "DSA Key",
 ...
 */
static int dsa_sign_directly(void *vpdsactx, unsigned char *sig, size_t *siglen,
  ...
  ret = ossl_dsa_sign_int(0, tbs, tbslen, sig, &sltmp, pdsactx->dsa,
                          pdsactx->nonce_type, pdsactx->mdname, pdsactx->libctx,
                          pdsactx->propq);
  if (ret <= 0)
    return 0;
...
static int dsa_verify_directly(void *vpdsactx, const unsigned char *sig,
...
static int dsa_verify_set_sig(void *vpdsactx, const unsigned char *sig,
                              size_t siglen) {
  PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
  OSSL_PARAM params[2];
...
static int dsa_verify_message_final(void *vpdsactx) {
  ...
  return dsa_verify_directly(vpdsactx, pdsactx->sig, pdsactx->siglen, digest,
                             dlen);
}
...
static int dsa_digest_sign_init(void *vpdsactx, const char *mdname, void *vdsa,
                                const OSSL_PARAM params[]) {
  return dsa_digest_signverify_init(
  vpdsactx, mdname, vdsa, params, EVP_PKEY_OP_SIGNMSG, "DSA Digest Sign Init");
}

static int dsa_digest_signverify_update(void *vpdsactx,
                                        const unsigned char *data,
                                        size_t datalen) {
  PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
...
static int dsa_get_ctx_params(void *vpdsactx, OSSL_PARAM *params) {
  ...
  if (p != NULL &&
      ...
      p, pdsactx->aid_len == 0 ? NULL : pdsactx->aid_buf, pdsactx->aid_len))
    return 0;
...
/**
 * @brief Setup common params for dsa_set_ctx_params and
 * dsa_sigalg_set_ctx_params The caller is responsible for checking |vpdsactx|
 * is not NULL and |params| is not empty.
 */
static int dsa_common_set_ctx_params(void *vpdsactx,
                                     const OSSL_PARAM params[]) {
  PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
  const OSSL_PARAM *p;
...
void *vpdsactx, void *vdsa,
OSSL_FUNC_signature_set_ctx_params_fn *set_ctx_params,
const OSSL_PARAM params[], const char *mdname, int operation,
const char *desc) {
  PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
...
dsa_sigalg_settable_ctx_params(void *vpdsactx, ossl_unused void *provctx) {
  PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
...
static int dsa_sigalg_set_ctx_params(void *vpdsactx,
                                     const OSSL_PARAM params[]) {
  PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
  const OSSL_PARAM *p;
  int ret;
  ...
  if (pdsactx->operation == EVP_PKEY_OP_VERIFYMSG) {
    ...
    if (p != NULL) {
      ...
      pdsactx->sig = NULL;
      pdsactx->siglen = 0;
      if (!OSSL_PARAM_get_octet_string(p, (void **)&pdsactx->sig, 0,
                                       &pdsactx->siglen))
        return 0;
    }
  }
...
#define IMPL_DSA_SIGALG(md, MD)                                                \
  ...
  dsa_##md##_verify_message_init;                                              \
                                                                               \
  static int dsa_##md##_sign_init(void *vpdsactx, void *vdsa,                  \
                                  const OSSL_PARAM params[]) {                 \
    static const char desc[] = "DSA-" MD " Sign Init";                        \
                                                                               \
    return dsa_sigalg_signverify_init(vpdsactx, vdsa,                          \
                                      dsa_sigalg_set_ctx_params, params, MD,  \
                                      EVP_PKEY_OP_SIGN, desc);                 \
  }                                                                            \
                                                                               \
  static int dsa_##md##_sign_message_init(void *vpdsactx, void *vdsa,          \
                                          const OSSL_PARAM params[]) {         \
    static const char desc[] = "DSA-" MD " Sign Message Init";                \
                                                                               \
    return dsa_sigalg_signverify_init(vpdsactx, vdsa,                          \
                                      dsa_sigalg_set_ctx_params, params, MD,  \
                                      EVP_PKEY_OP_SIGNMSG, desc);              \
  }                                                                            \
                                                                               \
  static int dsa_##md##_verify_init(void *vpdsactx, void *vdsa,                \
                                    const OSSL_PARAM params[]) {               \
    static const char desc[] = "DSA-" MD " Verify Init";                      \
                                                                               \
    return dsa_sigalg_signverify_init(vpdsactx, vdsa,                          \
                                      dsa_sigalg_set_ctx_params, params, MD,  \
                                      EVP_PKEY_OP_VERIFY, desc);               \
  }                                                                            \
                                                                               \
  static int dsa_##md##_verify_message_init(void *vpdsactx, void *vdsa,        \
                                            const OSSL_PARAM params[]) {       \
    static const char desc[] = "DSA-" MD " Verify Message Init";              \
                                                                               \
    return dsa_sigalg_signverify_init(vpdsactx, vdsa,                          \
                                      dsa_sigalg_set_ctx_params, params, MD,  \
                                      EVP_PKEY_OP_VERIFYMSG, desc);            \
  }                                                                            \
                                                                               \
  const OSSL_DISPATCH ossl_dsa_##md##_signature_functions[] = {                \
  {OSSL_FUNC_SIGNATURE_NEWCTX, (void (*)(void))dsa_newctx},                    \
  {OSSL_FUNC_SIGNATURE_SIGN_INIT, (void (*)(void))dsa_##md##_sign_init},       \
  {OSSL_FUNC_SIGNATURE_SIGN, (void (*)(void))dsa_sign},                        \
  ...
  {OSSL_FUNC_SIGNATURE_VERIFY_INIT, (void (*)(void))dsa_##md##_verify_init},   \
  {OSSL_FUNC_SIGNATURE_VERIFY, (void (*)(void))dsa_verify},                    \
