...
static int dsa_setup_md(PROV_DSA_CTX *ctx, const char *mdname, const char *mdprops, const char *desc)
{
    EVP_MD *md = NULL;
    ...
    if (mdname != NULL) {
        ...
        if (WPACKET_init_der(&pkt, ctx->aid_buf, sizeof(ctx->aid_buf))
            && ossl_DER_w_algorithmIdentifier_DSA_with_MD(&pkt, -1, ctx->dsa, md_nid) && WPACKET_finish(&pkt)) {
...
static int dsa_sign_check_approved(PROV_DSA_CTX *ctx, int signing, const char *desc)
{
    /* DSA Signing is not approved in FIPS 140-3 */
    if (signing
        && !OSSL_FIPS_IND_ON_UNAPPROVED(ctx, OSSL_FIPS_IND_SETTABLE2, ctx->libctx, desc, "DSA",
...
static int dsa_check_key(PROV_DSA_CTX *ctx, int sign, const char *desc)
{
    ...
    if (!approved) {
        if (!OSSL_FIPS_IND_ON_UNAPPROVED(ctx, OSSL_FIPS_IND_SETTABLE0, ctx->libctx, desc, "DSA Key",
...
static int dsa_signverify_init(void *vpdsactx, void *vdsa, OSSL_FUNC_signature_set_ctx_params_fn *set_ctx_params,
...
#ifdef FIPS_MODULE
    {
        int operation_is_sign = (operation & (EVP_PKEY_OP_SIGN | EVP_PKEY_OP_SIGNMSG)) != 0;
...
{
    return dsa_signverify_init(vpdsactx, vdsa, dsa_set_ctx_params, params, EVP_PKEY_OP_SIGN, "DSA Sign Init");
}
 ...
 */
static int dsa_sign_directly(void *vpdsactx, unsigned char *sig, size_t *siglen, size_t sigsize,
                             const unsigned char *tbs, size_t tbslen)
{
    ...
    ret = ossl_dsa_sign_int(0, tbs, tbslen, sig, &sltmp, pdsactx->dsa, pdsactx->nonce_type, pdsactx->mdname,
...
static int dsa_signverify_message_update(void *vpdsactx, const unsigned char *data, size_t datalen)
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
...
static int dsa_sign_message_final(void *vpdsactx, unsigned char *sig, size_t *siglen, size_t sigsize)
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
 ...
 */
static int dsa_sign(void *vpdsactx, unsigned char *sig, size_t *siglen, size_t sigsize, const unsigned char *tbs,
                    size_t tbslen)
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
...
{
    return dsa_signverify_init(vpdsactx, vdsa, dsa_set_ctx_params, params, EVP_PKEY_OP_VERIFY, "DSA Verify Init");
}

static int dsa_verify_directly(void *vpdsactx, const unsigned char *sig, size_t siglen, const unsigned char *tbs,
                               size_t tbslen)
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
    size_t mdsize = dsa_get_md_size(pdsactx);
...
static int dsa_verify_set_sig(void *vpdsactx, const unsigned char *sig, size_t siglen)
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
    OSSL_PARAM params[2];
...
static int dsa_verify_message_final(void *vpdsactx)
{
    ...
    return dsa_verify_directly(vpdsactx, pdsactx->sig, pdsactx->siglen, digest, dlen);
}
...
static int dsa_digest_signverify_init(void *vpdsactx, const char *mdname, void *vdsa, const OSSL_PARAM params[],
                                      int operation, const char *desc)
{
    ...
    if (!dsa_signverify_init(vpdsactx, vdsa, dsa_set_ctx_params, params, operation, desc))
        return 0;
...
{
    return dsa_digest_signverify_init(vpdsactx, mdname, vdsa, params, EVP_PKEY_OP_SIGNMSG, "DSA Digest Sign Init");
}

static int dsa_digest_signverify_update(void *vpdsactx, const unsigned char *data, size_t datalen)
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
...
static int dsa_digest_sign_final(void *vpdsactx, unsigned char *sig, size_t *siglen, size_t sigsize)
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
    int ok = 0;
...
{
    return dsa_digest_signverify_init(vpdsactx, mdname, vdsa, params, EVP_PKEY_OP_VERIFYMSG, "DSA Digest Verify Init");
}
...
static int dsa_get_ctx_params(void *vpdsactx, OSSL_PARAM *params)
{
    ...
    if (p != NULL && !OSSL_PARAM_set_octet_string(p, pdsactx->aid_len == 0 ? NULL : pdsactx->aid_buf, pdsactx->aid_len))
        return 0;
...
static const OSSL_PARAM known_gettable_ctx_params[] = {
    ...
    OSSL_PARAM_uint(OSSL_SIGNATURE_PARAM_NONCE_TYPE, NULL), OSSL_FIPS_IND_GETTABLE_CTX_PARAM() OSSL_PARAM_END};
...
static int dsa_common_set_ctx_params(void *vpdsactx, const OSSL_PARAM params[])
{
    ...
    if (!OSSL_FIPS_IND_SET_CTX_PARAM(pdsactx, OSSL_FIPS_IND_SETTABLE0, params, OSSL_SIGNATURE_PARAM_FIPS_KEY_CHECK))
        return 0;
    if (!OSSL_FIPS_IND_SET_CTX_PARAM(pdsactx, OSSL_FIPS_IND_SETTABLE1, params, OSSL_SIGNATURE_PARAM_FIPS_DIGEST_CHECK))
        return 0;
    if (!OSSL_FIPS_IND_SET_CTX_PARAM(pdsactx, OSSL_FIPS_IND_SETTABLE2, params, OSSL_SIGNATURE_PARAM_FIPS_SIGN_CHECK))
        return 0;

    p = OSSL_PARAM_locate_const(params, OSSL_SIGNATURE_PARAM_NONCE_TYPE);
    if (p != NULL && !OSSL_PARAM_get_uint(p, &pdsactx->nonce_type))
        return 0;
    return 1;
}
 ...
 */
static int dsa_sigalg_signverify_init(void *vpdsactx, void *vdsa, OSSL_FUNC_signature_set_ctx_params_fn *set_ctx_params,
                                      const OSSL_PARAM params[], const char *mdname, int operation, const char *desc)
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
    ...
    if (!dsa_signverify_init(vpdsactx, vdsa, set_ctx_params, params, operation, desc))
        return 0;
...
static const OSSL_PARAM settable_sigalg_ctx_params[] = {
    OSSL_PARAM_octet_string(OSSL_SIGNATURE_PARAM_SIGNATURE, NULL, 0), DSA_COMMON_SETTABLE_CTX_PARAMS};

static const OSSL_PARAM *dsa_sigalg_settable_ctx_params(void *vpdsactx, ossl_unused void *provctx)
{
    PROV_DSA_CTX *pdsactx = (PROV_DSA_CTX *)vpdsactx;
...
static int dsa_sigalg_set_ctx_params(void *vpdsactx, const OSSL_PARAM params[])
{
    ...
    if (pdsactx->operation == EVP_PKEY_OP_VERIFYMSG) {
        ...
        if (p != NULL) {
            ...
            pdsactx->sig = NULL;
            pdsactx->siglen = 0;
            if (!OSSL_PARAM_get_octet_string(p, (void **)&pdsactx->sig, 0, &pdsactx->siglen))
                return 0;
        }
    }
    return 1;
