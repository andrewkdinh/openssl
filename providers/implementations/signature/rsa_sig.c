...
static OSSL_ITEM padding_item[] = {{RSA_PKCS1_PADDING, OSSL_PKEY_RSA_PAD_MODE_PKCSV15},
    ...
    case RSA_PKCS1_PSS_PADDING:
        if (rsa_pss_restricted(prsactx))
            if ((mdname != NULL && !EVP_MD_is_a(prsactx->md, mdname))
                || (mgf1_mdname != NULL && !EVP_MD_is_a(prsactx->mgf1_md, mgf1_mdname))) {
...
static int rsa_pss_compute_saltlen(PROV_RSA_CTX *ctx)
{
    ...
    if (saltlen < 0) {
        ...
        ERR_raise_data(ERR_LIB_PROV, PROV_R_PSS_SALTLEN_TOO_SMALL, "minimum salt length: %d, actual salt length: %d",
...
static unsigned char *rsa_generate_signature_aid(PROV_RSA_CTX *ctx, unsigned char *aid_buf, size_t buf_len,
                                                 size_t *aid_len)
{
    WPACKET pkt;
    ...
    case RSA_PKCS1_PSS_PADDING:
        ...
        if (!ossl_rsa_pss_params_30_set_defaults(&pss_params)
            ...
            || !ossl_rsa_pss_params_30_set_maskgenhashalg(&pss_params, ctx->mgf1_mdnid)
...
static int rsa_setup_md(PROV_RSA_CTX *ctx, const char *mdname, const char *mdprops, const char *desc)
{
    EVP_MD *md = NULL;
...
#ifdef FIPS_MODULE
        {
            int sha1_allowed = ((ctx->operation & (EVP_PKEY_OP_SIGN | EVP_PKEY_OP_SIGNMSG)) == 0);

            if (!ossl_fips_ind_digest_sign_check(OSSL_FIPS_IND_GET(ctx), OSSL_FIPS_IND_SETTABLE1, ctx->libctx, md_nid,
                                                 sha1_allowed, desc, ossl_fips_config_signature_digest_check))
                goto err;
        }
#endif
...
static int rsa_setup_mgf1_md(PROV_RSA_CTX *ctx, const char *mdname, const char *mdprops)
{
    ...
    if ((mdnid = ossl_digest_rsa_sign_get_md_nid(md)) <= 0 || !rsa_check_padding(ctx, NULL, mdname, mdnid)) {
        if (mdnid <= 0)
            ERR_raise_data(ERR_LIB_PROV, PROV_R_DIGEST_NOT_ALLOWED, "digest=%s", mdname);
        EVP_MD_free(md);
        return 0;
...
static int rsa_signverify_init(PROV_RSA_CTX *prsactx, void *vrsa, OSSL_FUNC_signature_set_ctx_params_fn *set_ctx_params,
                               const OSSL_PARAM params[], int operation, const char *desc)
{
    int protect;
    ...
    if (!set_ctx_params(prsactx, params))
        return 0;
#ifdef FIPS_MODULE
    if (!ossl_fips_ind_rsa_key_check(OSSL_FIPS_IND_GET(prsactx), OSSL_FIPS_IND_SETTABLE0, prsactx->libctx, prsactx->rsa,
                                     desc, protect))
        return 0;
#endif
    return 1;
...
static int rsa_pss_saltlen_check_passed(PROV_RSA_CTX *ctx, const char *algoname, int saltlen)
{
    ...
    if (!approved) {
        if (!OSSL_FIPS_IND_ON_UNAPPROVED(ctx, OSSL_FIPS_IND_SETTABLE3, ctx->libctx, algoname, "PSS Salt Length",
...
static int rsa_sign_init(void *vprsactx, void *vrsa, const OSSL_PARAM params[])
{
    ...
    return rsa_signverify_init(prsactx, vrsa, rsa_set_ctx_params, params, EVP_PKEY_OP_SIGN, "RSA Sign Init");
}
 ...
 */
static int rsa_sign_directly(PROV_RSA_CTX *prsactx, unsigned char *sig, size_t *siglen, size_t sigsize,
    ...
    if (mdsize != 0) {
        ...
        case RSA_X931_PADDING:
            if ((size_t)RSA_size(prsactx->rsa) < tbslen + 1) {
                ERR_raise_data(ERR_LIB_PROV, PROV_R_KEY_SIZE_TOO_SMALL, "RSA key size = %d, expected minimum = %d",
        ...
        case RSA_PKCS1_PSS_PADDING: {
            ...
            if (!ossl_rsa_padding_add_PKCS1_PSS_mgf1(prsactx->rsa, prsactx->tbuf, tbs, prsactx->md, prsactx->mgf1_md,
                                                     &saltlen)) {
                ERR_raise(ERR_LIB_PROV, ERR_R_RSA_LIB);
                return 0;
            }
...
static int rsa_signverify_message_update(void *vprsactx, const unsigned char *data, size_t datalen)
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
...
static int rsa_sign_message_final(void *vprsactx, unsigned char *sig, size_t *siglen, size_t sigsize)
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
 ...
 */
static int rsa_sign(void *vprsactx, unsigned char *sig, size_t *siglen, size_t sigsize, const unsigned char *tbs,
                    size_t tbslen)
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
...
static int rsa_verify_recover_init(void *vprsactx, void *vrsa, const OSSL_PARAM params[])
{
    ...
    return rsa_signverify_init(prsactx, vrsa, rsa_set_ctx_params, params, EVP_PKEY_OP_VERIFYRECOVER,
                               "RSA VerifyRecover Init");
}
 ...
 */
static int rsa_verify_recover(void *vprsactx, unsigned char *rout, size_t *routlen, size_t routsize,
    ...
    if (prsactx->md != NULL) {
        ...
        case RSA_X931_PADDING:
            ...
            if (ret != EVP_MD_get_size(prsactx->md)) {
                ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_DIGEST_LENGTH, "Should be %d, but got %d",
            ...
            *routlen = ret;
            if (rout != prsactx->tbuf) {
                if (routsize < (size_t)ret) {
                    ERR_raise_data(ERR_LIB_PROV, PROV_R_OUTPUT_BUFFER_TOO_SMALL, "buffer size is %d, should be %d",
                                   routsize, ret);
                    return 0;
                }
...
static int rsa_verify_init(void *vprsactx, void *vrsa, const OSSL_PARAM params[])
{
    ...
    return rsa_signverify_init(prsactx, vrsa, rsa_set_ctx_params, params, EVP_PKEY_OP_VERIFY, "RSA Verify Init");
}

static int rsa_verify_directly(PROV_RSA_CTX *prsactx, const unsigned char *sig, size_t siglen, const unsigned char *tbs,
                               size_t tbslen)
{
    size_t rslen;
    ...
    if (prsactx->md != NULL) {
        ...
        case RSA_PKCS1_PSS_PADDING: {
            ...
            saltlen = prsactx->saltlen;
            ret = ossl_rsa_verify_PKCS1_PSS_mgf1(prsactx->rsa, tbs, prsactx->md, prsactx->mgf1_md, prsactx->tbuf,
                                                 &saltlen);
            if (ret <= 0) {
                ERR_raise(ERR_LIB_PROV, ERR_R_RSA_LIB);
                return 0;
...
static int rsa_verify_set_sig(void *vprsactx, const unsigned char *sig, size_t siglen)
{
    ...
    params[0] = OSSL_PARAM_construct_octet_string(OSSL_SIGNATURE_PARAM_SIGNATURE, (unsigned char *)sig, siglen);
...
static int rsa_verify_message_final(void *vprsactx)
{
    ...
    return rsa_verify_directly(prsactx, prsactx->sig, prsactx->siglen, digest, dlen);
}
...
static int rsa_verify(void *vprsactx, const unsigned char *sig, size_t siglen, const unsigned char *tbs, size_t tbslen)
{
    ...
    if (prsactx->operation == EVP_PKEY_OP_VERIFYMSG)
        return rsa_verify_set_sig(prsactx, sig, siglen) && rsa_signverify_message_update(prsactx, tbs, tbslen)
...
static int rsa_digest_signverify_init(void *vprsactx, const char *mdname, void *vrsa, const OSSL_PARAM params[],
                                      int operation, const char *desc)
{
    ...
    if (!rsa_signverify_init(prsactx, vrsa, rsa_set_ctx_params, params, operation, desc))
        return 0;
...
static int rsa_digest_sign_init(void *vprsactx, const char *mdname, void *vrsa, const OSSL_PARAM params[])
{
    if (!ossl_prov_is_running())
        return 0;
    return rsa_digest_signverify_init(vprsactx, mdname, vrsa, params, EVP_PKEY_OP_SIGNMSG, "RSA Digest Sign Init");
}
...
static int rsa_digest_verify_init(void *vprsactx, const char *mdname, void *vrsa, const OSSL_PARAM params[])
{
    if (!ossl_prov_is_running())
        return 0;
    return rsa_digest_signverify_init(vprsactx, mdname, vrsa, params, EVP_PKEY_OP_VERIFYMSG, "RSA Digest Verify Init");
}

static int rsa_digest_verify_update(void *vprsactx, const unsigned char *data, size_t datalen)
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
...
int rsa_digest_verify_final(void *vprsactx, const unsigned char *sig, size_t siglen)
{
    ...
    if (rsa_verify_set_sig(prsactx, sig, siglen) && rsa_verify_message_final(vprsactx))
        ok = 1;
...
static int rsa_x931_padding_allowed(PROV_RSA_CTX *ctx)
{
    ...
    if (!approved) {
        if (!OSSL_FIPS_IND_ON_UNAPPROVED(ctx, OSSL_FIPS_IND_SETTABLE2, ctx->libctx, "RSA Sign set ctx", "X931 Padding",
...
static int rsa_set_ctx_params(void *vprsactx, const OSSL_PARAM params[])
{
    ...
    if (!OSSL_FIPS_IND_SET_CTX_PARAM(prsactx, OSSL_FIPS_IND_SETTABLE0, params, OSSL_SIGNATURE_PARAM_FIPS_KEY_CHECK))
        return 0;

    if (!OSSL_FIPS_IND_SET_CTX_PARAM(prsactx, OSSL_FIPS_IND_SETTABLE1, params, OSSL_SIGNATURE_PARAM_FIPS_DIGEST_CHECK))
        return 0;
...
bad_pad:
    ...
    if (p != NULL) {
        ...
        if (rsa_pss_restricted(prsactx)) {
            ...
            case RSA_PSS_SALTLEN_AUTO:
            case RSA_PSS_SALTLEN_AUTO_DIGEST_MAX:
                if ((prsactx->operation & (EVP_PKEY_OP_VERIFY | EVP_PKEY_OP_VERIFYMSG)) == 0) {
    ...
    OSSL_FIPS_IND_SETTABLE_CTX_PARAM(OSSL_SIGNATURE_PARAM_FIPS_KEY_CHECK)
        OSSL_FIPS_IND_SETTABLE_CTX_PARAM(OSSL_SIGNATURE_PARAM_FIPS_DIGEST_CHECK)
            OSSL_FIPS_IND_SETTABLE_CTX_PARAM(OSSL_SIGNATURE_PARAM_FIPS_RSA_PSS_SALTLEN_CHECK)
                OSSL_FIPS_IND_SETTABLE_CTX_PARAM(OSSL_SIGNATURE_PARAM_FIPS_SIGN_X931_PAD_CHECK) OSSL_PARAM_END};
...
const OSSL_DISPATCH ossl_rsa_signature_functions[] = {
    ...
    {OSSL_FUNC_SIGNATURE_DIGEST_VERIFY_UPDATE, (void (*)(void))rsa_digest_verify_update},
    {OSSL_FUNC_SIGNATURE_DIGEST_VERIFY_FINAL, (void (*)(void))rsa_digest_verify_final},
    ...
    {OSSL_FUNC_SIGNATURE_SETTABLE_CTX_MD_PARAMS, (void (*)(void))rsa_settable_ctx_md_params},
    OSSL_DISPATCH_END};
 ...
 */
static int rsa_sigalg_signverify_init(void *vprsactx, void *vrsa, OSSL_FUNC_signature_set_ctx_params_fn *set_ctx_params,
                                      const OSSL_PARAM params[], const char *mdname, int operation, int pad_mode,
                                      const char *desc)
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
    ...
    if (!rsa_signverify_init(prsactx, vrsa, set_ctx_params, params, operation, desc))
        return 0;
...
static const OSSL_PARAM settable_sigalg_ctx_params[] = {
    OSSL_PARAM_octet_string(OSSL_SIGNATURE_PARAM_SIGNATURE, NULL, 0), OSSL_PARAM_END};

static const OSSL_PARAM *rsa_sigalg_settable_ctx_params(void *vprsactx, ossl_unused void *provctx)
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
...
static int rsa_sigalg_set_ctx_params(void *vprsactx, const OSSL_PARAM params[])
{
    ...
    if (prsactx->operation == EVP_PKEY_OP_VERIFYMSG) {
        ...
        if (p != NULL) {
            ...
            prsactx->sig = NULL;
            prsactx->siglen = 0;
            if (!OSSL_PARAM_get_octet_string(p, (void **)&prsactx->sig, 0, &prsactx->siglen))
                return 0;
        }
    }
    return 1;
