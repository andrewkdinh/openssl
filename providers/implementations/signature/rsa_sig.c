...
rsa_generate_signature_aid(PROV_RSA_CTX *ctx, unsigned char *aid_buf, size_t buf_len, size_t *aid_len)
{
    WPACKET           pkt;
...
static int rsa_signverify_init(PROV_RSA_CTX                          *prsactx,
                               void                                  *vrsa,
...
static int rsa_pss_saltlen_check_passed(PROV_RSA_CTX *ctx, const char *algoname, int saltlen)
{
    ...
    if (!approved) {
        if (!OSSL_FIPS_IND_ON_UNAPPROVED(ctx,
                                         OSSL_FIPS_IND_SETTABLE3,
                                         ctx->libctx,
                                         algoname,
                                         "PSS Salt Length",
                             ...
                             size_t               tbslen)
{
    ...
    if (mdsize != 0) {
        ...
        if (EVP_MD_is_a(prsactx->md, OSSL_DIGEST_NAME_MDC2)) {
            ...
            ret = RSA_sign_ASN1_OCTET_STRING(0, tbs, (unsigned int)tbslen, sig, &sltmp, prsactx->rsa);
        ...
        case RSA_X931_PADDING:
            ...
            ret = RSA_private_encrypt((int)(tbslen + 1), prsactx->tbuf, sig, prsactx->rsa, RSA_X931_PADDING);
            clean_tbuf(prsactx);
            break;
        case RSA_PKCS1_PADDING: {
            unsigned int sltmp;

            ret = RSA_sign(prsactx->mdnid, tbs, (unsigned int)tbslen, sig, &sltmp, prsactx->rsa);
            if (ret <= 0) {
                ERR_raise(ERR_LIB_PROV, ERR_R_RSA_LIB);
                return 0;
            }
        ...
        ret = RSA_private_encrypt((int)tbslen, tbs, sig, prsactx->rsa, prsactx->pad_mode);
    }
 ...
 */
static int
rsa_sign(void *vprsactx, unsigned char *sig, size_t *siglen, size_t sigsize, const unsigned char *tbs, size_t tbslen)
{
    PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
...
static int rsa_verify_recover_init(void *vprsactx, void *vrsa, const OSSL_PARAM params[])
{
    ...
    return rsa_signverify_init(prsactx,
                               vrsa,
                               rsa_set_ctx_params,
                               params,
                              ...
                              size_t               siglen)
{
    ...
    if (prsactx->md != NULL) {
        ...
        case RSA_X931_PADDING:
            if (!setup_tbuf(prsactx))
                return 0;
            ret = RSA_public_decrypt((int)siglen, sig, prsactx->tbuf, prsactx->rsa, RSA_X931_PADDING);
            if (ret < 1) {
                ERR_raise(ERR_LIB_PROV, ERR_R_RSA_LIB);
                return 0;
            }
        ...
        ret = RSA_public_decrypt((int)siglen, sig, rout, prsactx->rsa, prsactx->pad_mode);
        if (ret < 0) {
            ERR_raise(ERR_LIB_PROV, ERR_R_RSA_LIB);
            return 0;
        }
                               ...
                               size_t               tbslen)
{
    ...
    if (prsactx->md != NULL) {
        switch (prsactx->pad_mode) {
        case RSA_PKCS1_PADDING:
            if (!RSA_verify(prsactx->mdnid, tbs, (unsigned int)tbslen, sig, (unsigned int)siglen, prsactx->rsa)) {
                ERR_raise(ERR_LIB_PROV, ERR_R_RSA_LIB);
                return 0;
            }
            return 1;
        ...
        case RSA_PKCS1_PSS_PADDING: {
            ...
            if (!setup_tbuf(prsactx))
                return 0;
            ret = RSA_public_decrypt((int)siglen, sig, prsactx->tbuf, prsactx->rsa, RSA_NO_PADDING);
            if (ret <= 0) {
                ERR_raise(ERR_LIB_PROV, ERR_R_RSA_LIB);
                return 0;
            }
            saltlen = prsactx->saltlen;
            ret     = ossl_rsa_verify_PKCS1_PSS_mgf1(prsactx->rsa,
                                                 tbs,
                                                 prsactx->md,
                                                 prsactx->mgf1_md,
                                                 prsactx->tbuf,
        ...
        if (!setup_tbuf(prsactx))
            return 0;
        ret = RSA_public_decrypt((int)siglen, sig, prsactx->tbuf, prsactx->rsa, prsactx->pad_mode);
        if (ret <= 0) {
            ERR_raise(ERR_LIB_PROV, ERR_R_RSA_LIB);
            return 0;
        }
...
static int rsa_digest_signverify_init(void            *vprsactx,
                                      const char      *mdname,
                                      void            *vrsa,
                                      const OSSL_PARAM params[],
                                      int              operation,
...
static int rsa_set_ctx_params(void *vprsactx, const OSSL_PARAM params[])
{
    ...
    if (!OSSL_FIPS_IND_SET_CTX_PARAM(prsactx,
                                     OSSL_FIPS_IND_SETTABLE2,
                                     params,
 ...
 */
static int                                        rsa_sigalg_signverify_init(void                                  *vprsactx,
                                                                             void                                  *vrsa,
                                                                             OSSL_FUNC_signature_set_ctx_params_fn *set_ctx_params,
                                                                             const OSSL_PARAM                       params[],
                                                                             const char                            *mdname,
