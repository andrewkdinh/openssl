    ...
    if (mdname != NULL) {
...
#ifdef FIPS_MODULE
        {
            ...
            if (!ossl_fips_ind_digest_sign_check(
                OSSL_FIPS_IND_GET(ctx), OSSL_FIPS_IND_SETTABLE1, ctx->libctx,
    ...
    case RSA_FLAG_TYPE_RSASSAPSS:
...

        {
            ...
            if (!ossl_rsa_pss_params_30_is_unrestricted(pss)) {
                ...
                /* call rsa_setup_mgf1_md before rsa_setup_md to avoid
                 * duplication */
...
static int rsa_pss_saltlen_check_passed(PROV_RSA_CTX *ctx, const char *algoname,
    ...
    if (!approved) {
        if (!OSSL_FIPS_IND_ON_UNAPPROVED(
            ctx, OSSL_FIPS_IND_SETTABLE3, ctx->libctx, algoname,
            "PSS Salt Length", ossl_fips_config_rsa_pss_saltlen_check)) {
            ERR_raise(ERR_LIB_PROV, PROV_R_INVALID_SALT_LENGTH);
            return 0;
        }
    }
...
static int rsa_sign_directly(PROV_RSA_CTX *prsactx, unsigned char *sig,
    ...
    if (mdsize != 0) {
        ...
        case RSA_X931_PADDING:
            ...
            ret = RSA_private_encrypt(tbslen + 1, prsactx->tbuf, sig,
                                      prsactx->rsa, RSA_X931_PADDING);
            clean_tbuf(prsactx);
            break;
        case RSA_PKCS1_PADDING: {
            unsigned int sltmp;
            ...
            ret =
            RSA_sign(prsactx->mdnid, tbs, tbslen, sig, &sltmp, prsactx->rsa);
            if (ret <= 0) {
                ERR_raise(ERR_LIB_PROV, ERR_R_RSA_LIB);
                return 0;
            }
        ...
        case RSA_PKCS1_PSS_PADDING: {
            ...
            if (!ossl_rsa_padding_add_PKCS1_PSS_mgf1(
                prsactx->rsa, prsactx->tbuf, tbs, prsactx->md, prsactx->mgf1_md,
                &saltlen)) {
                ERR_raise(ERR_LIB_PROV, ERR_R_RSA_LIB);
                return 0;
            }
...
static int rsa_verify_recover(void *vprsactx, unsigned char *rout,
    ...
    if (prsactx->md != NULL) {
            ...
            if (rout != prsactx->tbuf) {
                if (routsize < (size_t)ret) {
                    ERR_raise_data(ERR_LIB_PROV, PROV_R_OUTPUT_BUFFER_TOO_SMALL,
                                   "buffer size is %d, should be %d", routsize,
                                   ret);
                    return 0;
                }
        ...
        case RSA_PKCS1_PADDING: {
            ...
            ret = ossl_rsa_verify(prsactx->mdnid, NULL, 0, rout, &sltmp, sig,
                                  siglen, prsactx->rsa);
            if (ret <= 0) {
                ERR_raise(ERR_LIB_PROV, ERR_R_RSA_LIB);
                return 0;
...
static int rsa_verify_directly(PROV_RSA_CTX *prsactx, const unsigned char *sig,
    ...
    if (prsactx->md != NULL) {
        ...
        case RSA_X931_PADDING:
            if (!setup_tbuf(prsactx))
                return 0;
            if (rsa_verify_recover(prsactx, prsactx->tbuf, &rslen, 0, sig,
                                   siglen) <= 0)
                return 0;
            break;
        case RSA_PKCS1_PSS_PADDING: {
            ...
            saltlen = prsactx->saltlen;
            ret = ossl_rsa_verify_PKCS1_PSS_mgf1(prsactx->rsa, tbs, prsactx->md,
                                                 prsactx->mgf1_md,
...
static int rsa_verify_set_sig(void *vprsactx, const unsigned char *sig,
    ...
    OSSL_SIGNATURE_PARAM_SIGNATURE, (unsigned char *)sig, siglen);
...
static int rsa_get_ctx_params(void *vprsactx, OSSL_PARAM *params) {
    ...
    if (p != NULL) {
        if (p->data_type == OSSL_PARAM_INTEGER) {
            ...
            default: {
                int len =
                BIO_snprintf(p->data, p->data_size, "%d", prsactx->saltlen);
...
static int rsa_set_ctx_params(void *vprsactx, const OSSL_PARAM params[]) {
    ...
    if (p != NULL) {
        ...
        cont:
            if (RSA_test_flags(prsactx->rsa, RSA_FLAG_TYPE_MASK) ==
                RSA_FLAG_TYPE_RSA)
                break;
            /* FALLTHRU */
        default:
...
#define IMPL_RSA_SIGALG(md, MD)                                                \
    ...
    static int rsa_##md##_sign_init(void *vprsactx, void *vrsa,                \
        ...
        EVP_PKEY_OP_SIGN, RSA_PKCS1_PADDING, desc);                            \
    }                                                                          \
                                                                               \
