...
static OSSL_FUNC_signature_verify_message_update_fn
...
rsa_sigalg_settable_ctx_params;
...
static int rsa_check_padding(const PROV_RSA_CTX *prsactx, const char *mdname,
...
static unsigned char *rsa_generate_signature_aid(PROV_RSA_CTX *ctx,
  ...
  case RSA_PKCS1_PADDING:
    ret =
    ossl_DER_w_algorithmIdentifier_MDWithRSAEncryption(&pkt, -1, ctx->mdnid);
    ...
    ERR_raise_data(ERR_LIB_PROV, ERR_R_UNSUPPORTED,
                   "Algorithm ID generation - md NID: %d", ctx->mdnid);
    goto cleanup;
  case RSA_PKCS1_PSS_PADDING:
    ...
    if (!ossl_rsa_pss_params_30_set_defaults(&pss_params) ||
        ...
        &pkt, -1, RSA_FLAG_TYPE_RSASSAPSS, &pss_params)) {
      ERR_raise(ERR_LIB_PROV, ERR_R_INTERNAL_ERROR);
      goto cleanup;
    }
    break;
  default:
    ERR_raise_data(ERR_LIB_PROV, ERR_R_UNSUPPORTED,
                   "Algorithm ID generation - pad mode: %d", ctx->pad_mode);
    goto cleanup;
  }
...
static int rsa_setup_md(PROV_RSA_CTX *ctx, const char *mdname,
  ...
  if (mdname != NULL) {
    ...
    if (md_nid == NID_undef) {
      ERR_raise_data(ERR_LIB_PROV, PROV_R_DIGEST_NOT_ALLOWED, "digest=%s",
                     mdname);
      goto err;
    }
    /*
    ...
    {
      int sha1_allowed =
      ((ctx->operation & (EVP_PKEY_OP_SIGN | EVP_PKEY_OP_SIGNMSG)) == 0);

      if (!ossl_fips_ind_digest_sign_check(
          OSSL_FIPS_IND_GET(ctx), OSSL_FIPS_IND_SETTABLE1, ctx->libctx, md_nid,
...
static int rsa_pss_saltlen_check_passed(PROV_RSA_CTX *ctx, const char *algoname,
  ...
  if (!approved) {
    if (!OSSL_FIPS_IND_ON_UNAPPROVED(ctx, OSSL_FIPS_IND_SETTABLE3, ctx->libctx,
 ...
 */
static int rsa_sign_directly(PROV_RSA_CTX *prsactx, unsigned char *sig,
  ...
  if (mdsize != 0) {
    ...
    if (EVP_MD_is_a(prsactx->md, OSSL_DIGEST_NAME_MDC2)) {
      ...
      ret =
      RSA_sign_ASN1_OCTET_STRING(0, tbs, tbslen, sig, &sltmp, prsactx->rsa);
    ...
    case RSA_X931_PADDING:
      ...
      ret = RSA_private_encrypt(tbslen + 1, prsactx->tbuf, sig, prsactx->rsa,
                                RSA_X931_PADDING);
      clean_tbuf(prsactx);
      break;
    case RSA_PKCS1_PADDING: {
    ...
    case RSA_PKCS1_PSS_PADDING: {
      ...
      if (!ossl_rsa_padding_add_PKCS1_PSS_mgf1(prsactx->rsa, prsactx->tbuf, tbs,
      ...
      if (!rsa_pss_saltlen_check_passed(prsactx, "RSA Sign", saltlen))
        return 0;
#endif
      ret = RSA_private_encrypt(RSA_size(prsactx->rsa), prsactx->tbuf, sig,
    ...
    ret =
    RSA_private_encrypt(tbslen, tbs, sig, prsactx->rsa, prsactx->pad_mode);
  }
 ...
 */
static int rsa_verify_recover(void *vprsactx, unsigned char *rout,
  ...
  if (prsactx->md != NULL) {
    ...
    case RSA_X931_PADDING:
      ...
      if (ret != EVP_MD_get_size(prsactx->md)) {
        ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_DIGEST_LENGTH,
                       "Should be %d, but got %d", EVP_MD_get_size(prsactx->md),
                       ret);
        return 0;
      }
      ...
      if (rout != prsactx->tbuf) {
        if (routsize < (size_t)ret) {
          ERR_raise_data(ERR_LIB_PROV, PROV_R_OUTPUT_BUFFER_TOO_SMALL,
                         "buffer size is %d, should be %d", routsize, ret);
          return 0;
        }
    ...
    case RSA_PKCS1_PADDING: {
      ...
      ret = ossl_rsa_verify(prsactx->mdnid, NULL, 0, rout, &sltmp, sig, siglen,
                            prsactx->rsa);
      if (ret <= 0) {
        ERR_raise(ERR_LIB_PROV, ERR_R_RSA_LIB);
        return 0;
    ...
    ret =
    RSA_public_decrypt(siglen, sig, rout, prsactx->rsa, prsactx->pad_mode);
    if (ret < 0) {
      ERR_raise(ERR_LIB_PROV, ERR_R_RSA_LIB);
      return 0;
    }
...
static int rsa_verify_directly(PROV_RSA_CTX *prsactx, const unsigned char *sig,
  ...
  if (prsactx->md != NULL) {
    ...
    case RSA_X931_PADDING:
      if (!setup_tbuf(prsactx))
        return 0;
      if (rsa_verify_recover(prsactx, prsactx->tbuf, &rslen, 0, sig, siglen) <=
          0)
        return 0;
      break;
    case RSA_PKCS1_PSS_PADDING: {
      ...
      if (tbslen != mdsize) {
        ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_DIGEST_LENGTH,
                       "Should be %d, but got %d", mdsize, tbslen);
        return 0;
      }
      ...
      saltlen = prsactx->saltlen;
      ret =
      ossl_rsa_verify_PKCS1_PSS_mgf1(prsactx->rsa, tbs, prsactx->md,
                                     prsactx->mgf1_md, prsactx->tbuf, &saltlen);
      if (ret <= 0) {
        ERR_raise(ERR_LIB_PROV, ERR_R_RSA_LIB);
        return 0;
      }
...
static int rsa_verify_message_final(void *vprsactx) {
  ...
  return rsa_verify_directly(prsactx, prsactx->sig, prsactx->siglen, digest,
                             dlen);
}
...
static int rsa_digest_signverify_init(void *vprsactx, const char *mdname,
  ...
  if (!rsa_signverify_init(prsactx, vrsa, rsa_set_ctx_params, params, operation,
                           desc))
    return 0;
...
static int rsa_digest_sign_init(void *vprsactx, const char *mdname, void *vrsa,
                                const OSSL_PARAM params[]) {
  if (!ossl_prov_is_running())
    return 0;
  return rsa_digest_signverify_init(
  vprsactx, mdname, vrsa, params, EVP_PKEY_OP_SIGNMSG, "RSA Digest Sign Init");
}
...
static int rsa_digest_verify_init(void *vprsactx, const char *mdname,
  ...
  if (!ossl_prov_is_running())
    return 0;
  return rsa_digest_signverify_init(vprsactx, mdname, vrsa, params,
...
static int rsa_get_ctx_params(void *vprsactx, OSSL_PARAM *params) {
  ...
  if (p != NULL) {
    ...
    aid =
    rsa_generate_signature_aid(prsactx, aid_buf, sizeof(aid_buf), &aid_len);
...
static int rsa_x931_padding_allowed(PROV_RSA_CTX *ctx) {
  ...
  if (!approved) {
    if (!OSSL_FIPS_IND_ON_UNAPPROVED(
        ctx, OSSL_FIPS_IND_SETTABLE2, ctx->libctx, "RSA Sign set ctx",
        "X931 Padding", ossl_fips_config_rsa_sign_x931_disallowed)) {
...
static int rsa_set_ctx_params(void *vprsactx, const OSSL_PARAM params[]) {
  ...
  if (!OSSL_FIPS_IND_SET_CTX_PARAM(
      prsactx, OSSL_FIPS_IND_SETTABLE2, params,
      OSSL_SIGNATURE_PARAM_FIPS_SIGN_X931_PAD_CHECK))
    return 0;

  if (!OSSL_FIPS_IND_SET_CTX_PARAM(
      prsactx, OSSL_FIPS_IND_SETTABLE3, params,
  ...
  if (p != NULL) {
    ...
    case RSA_PKCS1_OAEP_PADDING:
      ...
      goto bad_pad;
    case RSA_PKCS1_PSS_PADDING:
      if ((prsactx->operation & (EVP_PKEY_OP_SIGN | EVP_PKEY_OP_SIGNMSG |
    ...
    bad_pad:
      if (err_extra_text == NULL)
        ...
        ERR_raise_data(ERR_LIB_PROV, PROV_R_ILLEGAL_OR_UNSUPPORTED_PADDING_MODE,
                       err_extra_text);
      return 0;
    }
  }
  ...
  if (p != NULL) {
    ...
    if (propsp != NULL) {
      ...
      if (!OSSL_PARAM_get_utf8_string(propsp, &pmgf1mdprops,
                                      sizeof(mgf1mdprops)))
        return 0;
    }
 ...
 */
static int rsa_sigalg_signverify_init(
void *vprsactx, void *vrsa,
OSSL_FUNC_signature_set_ctx_params_fn *set_ctx_params,
...
rsa_sigalg_settable_ctx_params(void *vprsactx, ossl_unused void *provctx) {
  PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
...
static int rsa_sigalg_set_ctx_params(void *vprsactx,
                                     const OSSL_PARAM params[]) {
  PROV_RSA_CTX *prsactx = (PROV_RSA_CTX *)vprsactx;
  const OSSL_PARAM *p;
  ...
  if (prsactx->operation == EVP_PKEY_OP_VERIFYMSG) {
    ...
    if (p != NULL) {
      ...
      prsactx->sig = NULL;
      prsactx->siglen = 0;
      if (!OSSL_PARAM_get_octet_string(p, (void **)&prsactx->sig, 0,
                                       &prsactx->siglen))
        return 0;
    }
  }
...
#define IMPL_RSA_SIGALG(md, MD)                                                \
  ...
  rsa_##md##_verify_message_init;                                              \
                                                                               \
  static int rsa_##md##_sign_init(void *vprsactx, void *vrsa,                  \
                                  const OSSL_PARAM params[]) {                 \
    static const char desc[] = "RSA Sigalg Sign Init";                         \
                                                                               \
    return rsa_sigalg_signverify_init(                                         \
    vprsactx, vrsa, rsa_sigalg_set_ctx_params, params, MD, EVP_PKEY_OP_SIGN,  \
    RSA_PKCS1_PADDING, desc);                                                  \
  }                                                                            \
                                                                               \
  static int rsa_##md##_sign_message_init(void *vprsactx, void *vrsa,          \
                                          const OSSL_PARAM params[]) {         \
    static const char desc[] = "RSA Sigalg Sign Message Init";                 \
                                                                               \
    return rsa_sigalg_signverify_init(                                         \
    vprsactx, vrsa, rsa_sigalg_set_ctx_params, params, MD,                    \
    EVP_PKEY_OP_SIGNMSG, RSA_PKCS1_PADDING, desc);                             \
  }                                                                            \
                                                                               \
  static int rsa_##md##_verify_init(void *vprsactx, void *vrsa,                \
                                    const OSSL_PARAM params[]) {               \
    static const char desc[] = "RSA Sigalg Verify Init";                       \
                                                                               \
    return rsa_sigalg_signverify_init(                                         \
    vprsactx, vrsa, rsa_sigalg_set_ctx_params, params, MD,                    \
    EVP_PKEY_OP_VERIFY, RSA_PKCS1_PADDING, desc);                              \
  }                                                                            \
                                                                               \
  static int rsa_##md##_verify_recover_init(void *vprsactx, void *vrsa,        \
                                            const OSSL_PARAM params[]) {       \
    static const char desc[] = "RSA Sigalg Verify Recover Init";               \
                                                                               \
    return rsa_sigalg_signverify_init(                                         \
    vprsactx, vrsa, rsa_sigalg_set_ctx_params, params, MD,                    \
    EVP_PKEY_OP_VERIFYRECOVER, RSA_PKCS1_PADDING, desc);                       \
  }                                                                            \
                                                                               \
  static int rsa_##md##_verify_message_init(void *vprsactx, void *vrsa,        \
                                            const OSSL_PARAM params[]) {       \
    static const char desc[] = "RSA Sigalg Verify Message Init";               \
                                                                               \
    return rsa_sigalg_signverify_init(                                         \
    vprsactx, vrsa, rsa_sigalg_set_ctx_params, params, MD,                    \
    EVP_PKEY_OP_VERIFYMSG, RSA_PKCS1_PADDING, desc);                           \
  }                                                                            \
                                                                               \
  const OSSL_DISPATCH ossl_rsa_##md##_signature_functions[] = {                \
  {OSSL_FUNC_SIGNATURE_NEWCTX, (void (*)(void))rsa_newctx},                    \
  {OSSL_FUNC_SIGNATURE_SIGN_INIT, (void (*)(void))rsa_##md##_sign_init},       \
  {OSSL_FUNC_SIGNATURE_SIGN, (void (*)(void))rsa_sign},                        \
  ...
  {OSSL_FUNC_SIGNATURE_VERIFY_INIT, (void (*)(void))rsa_##md##_verify_init},   \
  {OSSL_FUNC_SIGNATURE_VERIFY, (void (*)(void))rsa_verify},                    \
