                        ...
                        const char *mdprops, const char *desc)
{
    ...
    if (mdname != NULL) {
        ...
        {
            int sha1_allowed =
                ((ctx->operation & (EVP_PKEY_OP_SIGN | EVP_PKEY_OP_SIGNMSG))
                 == 0);

            if (!ossl_fips_ind_digest_sign_check(
                    OSSL_FIPS_IND_GET(ctx), OSSL_FIPS_IND_SETTABLE1,
                    ctx->libctx, md_nid, sha1_allowed, desc,
    ...
    if (mdsize != 0) {
        ...
        case RSA_PKCS1_PSS_PADDING: {
            ...
            if (!ossl_rsa_padding_add_PKCS1_PSS_mgf1(
                    prsactx->rsa, prsactx->tbuf, tbs, prsactx->md,
                    prsactx->mgf1_md, &saltlen)) {
                ERR_raise(ERR_LIB_PROV, ERR_R_RSA_LIB);
                return 0;
            }
#ifdef FIPS_MODULE
...
static int rsa_get_ctx_params(void *vprsactx, OSSL_PARAM *params)
{
    ...
    if (p != NULL) {
        ...
        aid = rsa_generate_signature_aid(prsactx, aid_buf, sizeof(aid_buf),
                                         &aid_len);
...
bad_pad:
    ...
    if (p != NULL) {
        ...
        case OSSL_PARAM_UTF8_STRING:
            ...
            else if (strcmp(p->data, OSSL_PKEY_RSA_PSS_SALT_LEN_AUTO_DIGEST_MAX)
                     == 0)
    ...
    if (p != NULL) {
        const OSSL_PARAM *propsp = OSSL_PARAM_locate_const(
            params, OSSL_SIGNATURE_PARAM_MGF1_PROPERTIES);
