...
static int ml_dsa_sign_msg_init(void *vctx, void *vkey,
                                const OSSL_PARAM params[]) {
  return ml_dsa_signverify_msg_init(vctx, vkey, params, EVP_PKEY_OP_SIGNMSG,
                                    "ML_DSA Sign Init");
}
...
static int ml_dsa_digest_signverify_init(void *vctx, const char *mdname,
  ...
  return ml_dsa_signverify_msg_init(vctx, vkey, params, EVP_PKEY_OP_SIGN,
                                    "ML_DSA Sign Init");
}
...
static int ml_dsa_signverify_msg_update(void *vctx, const unsigned char *data,
  ...
  if (ctx->md_ctx == NULL) {
    ...
    ctx->key, ctx->msg_encode, ctx->context_string, ctx->context_string_len);
    if (ctx->md_ctx == NULL)
      return 0;
  }
...
static int ml_dsa_sign(void *vctx, uint8_t *sig, size_t *siglen, size_t sigsize,
  ...
  ret = ossl_ml_dsa_sign(ctx->key, ctx->mu, msg, msg_len, ctx->context_string,
                         ctx->context_string_len, rnd, sizeof(rand_tmp),
...
static int ml_dsa_digest_sign(void *vctx, uint8_t *sig, size_t *siglen,
                              size_t sigsize, const uint8_t *tbs,
                              size_t tbslen) {
  return ml_dsa_sign(vctx, sig, siglen, sigsize, tbs, tbslen);
}

static int ml_dsa_verify_msg_init(void *vctx, void *vkey,
                                  const OSSL_PARAM params[]) {
...
static int ml_dsa_verify_msg_final(void *vctx) {
  ...
  return ossl_ml_dsa_verify(ctx->key, 1, mu, sizeof(mu), NULL, 0, 0, ctx->sig,
                            ctx->siglen);
}
...
static int ml_dsa_set_ctx_params(void *vctx, const OSSL_PARAM params[]) {
  ...
  if (pctx->operation == EVP_PKEY_OP_VERIFYMSG) {
    ...
    if (p != NULL) {
      ...
      pctx->sig = NULL;
      pctx->siglen = 0;
      if (!OSSL_PARAM_get_octet_string(p, (void **)&pctx->sig, 0,
                                       &pctx->siglen))
        return 0;
    }
  }
...
#define MLDSA_COMMON_SETTABLE_CTX_PARAMS                                       \
  ...
  OSSL_PARAM_int(OSSL_SIGNATURE_PARAM_MESSAGE_ENCODING, 0), OSSL_PARAM_END
...
#define MAKE_SIGNATURE_FUNCTIONS(alg)                                          \
  ...
  {OSSL_FUNC_SIGNATURE_DIGEST_SIGN, (void (*)(void))ml_dsa_digest_sign},       \
  {OSSL_FUNC_SIGNATURE_DIGEST_VERIFY_INIT,                                     \
   (void (*)(void))ml_dsa_digest_signverify_init},                             \
  {OSSL_FUNC_SIGNATURE_DIGEST_VERIFY, (void (*)(void))ml_dsa_digest_verify},   \
