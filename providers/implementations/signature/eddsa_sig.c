...
#define S390X_CAN_SIGN(edtype)                                                 \
    ((OPENSSL_s390xcap_P.pcc[1] &                                              \
      S390X_CAPBIT(S390X_SCALAR_MULTIPLY_##edtype)) &&                         \
...
static OSSL_FUNC_signature_sign_message_init_fn
ed25519ctx_signverify_message_init;
...
eddsa_settable_variant_ctx_params;
...
typedef struct {
    ...
    /* for ph instances, this indicates whether the caller is expected to
     * prehash
     */
...
static int eddsa_signverify_init(void *vpeddsactx, void *vedkey) {
    ...
    peddsactx->aid_len = 0;
    ret =
    WPACKET_init_der(&pkt, peddsactx->aid_buf, sizeof(peddsactx->aid_buf));
    switch (edkey->type) {
...
static int ed25519_sign(void *vpeddsactx, unsigned char *sigret, size_t *siglen,
     ...
     * is provided.
     */
    if (S390X_CAN_SIGN(ED25519) && !peddsactx->dom2_flag &&
        !peddsactx->context_string_flag && peddsactx->context_string_len == 0 &&
        !peddsactx->prehash_flag && !peddsactx->prehash_by_caller_flag) {
    ...
    if (peddsactx->prehash_flag) {
        if (!peddsactx->prehash_by_caller_flag) {
            if (!EVP_Q_digest(peddsactx->libctx, SN_sha512, NULL, tbs, tbslen,
                              md, &mdlen) ||
    ...
    if (ossl_ed25519_sign(
        sigret, tbs, tbslen, edkey->pubkey, edkey->privkey,
        peddsactx->dom2_flag, peddsactx->prehash_flag,
        peddsactx->context_string_flag, peddsactx->context_string,
        peddsactx->context_string_len, peddsactx->libctx, NULL) == 0) {
        ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_SIGN);
        return 0;
    }
    *siglen = ED25519_SIGSIZE;
...
static int ed448_shake256(OSSL_LIB_CTX *libctx, const char *propq,
...
static int ed448_sign(void *vpeddsactx, unsigned char *sigret, size_t *siglen,
     ...
     */
    if (S390X_CAN_SIGN(ED448) && peddsactx->context_string_len == 0 &&
        !peddsactx->prehash_flag && !peddsactx->prehash_by_caller_flag) {
    ...
    if (peddsactx->prehash_flag) {
        if (!peddsactx->prehash_by_caller_flag) {
            if (!ed448_shake256(peddsactx->libctx, NULL, tbs, tbslen, md,
                                mdlen))
                return 0;
            tbs = md;
            tbslen = mdlen;
    ...
    if (ossl_ed448_sign(peddsactx->libctx, sigret, tbs, tbslen, edkey->pubkey,
                        edkey->privkey, peddsactx->context_string,
                        peddsactx->context_string_len, peddsactx->prehash_flag,
                        edkey->propq) == 0) {
        ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_SIGN);
        return 0;
    }
...
static int ed25519_verify(void *vpeddsactx, const unsigned char *sig,
...
#ifdef S390X_EC_ASM
    /*
     * s390x_ed25519_digestverify() does not yet support dom2 or
     * context-strings. fall back to non-accelerated verify if those options are
     * set, or pre-hasing is provided.
     */
    if (S390X_CAN_SIGN(ED25519) && !peddsactx->dom2_flag &&
        !peddsactx->context_string_flag && peddsactx->context_string_len == 0 &&
        !peddsactx->prehash_flag && !peddsactx->prehash_by_caller_flag)
    ...
    return ossl_ed25519_verify(
    tbs, tbslen, sig, edkey->pubkey, peddsactx->dom2_flag,
    peddsactx->prehash_flag, peddsactx->context_string_flag,
    peddsactx->context_string, peddsactx->context_string_len, peddsactx->libctx,
    edkey->propq);
}
...
static int ed25519_digest_verify(void *vpeddsactx, const unsigned char *sigret,
                                 size_t siglen, const unsigned char *tbs,
                                 size_t tbslen) {
...
static int ed448_digest_verify(void *vpeddsactx, const unsigned char *sigret,
                               size_t siglen, const unsigned char *tbs,
                               size_t tbslen) {
...
static int eddsa_get_ctx_params(void *vpeddsactx, OSSL_PARAM *params) {
    ...
    if (p != NULL && !OSSL_PARAM_set_octet_string(
                     p, peddsactx->aid_len == 0 ? NULL : peddsactx->aid_buf,
                     peddsactx->aid_len))
        return 0;
...
static int eddsa_set_ctx_params(void *vpeddsactx, const OSSL_PARAM params[]) {
    ...
    if (p != NULL) {
        ...
        if (peddsactx->instance_id_preset_flag) {
            ...
            ERR_raise_data(
            ERR_LIB_PROV, PROV_R_NO_INSTANCE_ALLOWED,
            "the EdDSA instance is preset, you may not try to specify it",
            NULL);
            return 0;
        }

        if (!OSSL_PARAM_get_utf8_string(p, &pinstance_name,
    ...
    if (p != NULL) {
        ...
        if (!OSSL_PARAM_get_octet_string(p, &vp_context_string,
