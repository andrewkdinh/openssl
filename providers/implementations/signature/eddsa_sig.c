...
static int ed25519_sign(void *vpeddsactx, unsigned char *sigret, size_t *siglen,
  ...
  if (peddsactx->prehash_flag) {
    if (!peddsactx->prehash_by_caller_flag) {
      if (!EVP_Q_digest(peddsactx->libctx, SN_sha512, NULL, tbs, tbslen, md,
                        &mdlen) ||
  ...
  if (ossl_ed25519_sign(
      sigret, tbs, tbslen, edkey->pubkey, edkey->privkey, peddsactx->dom2_flag,
...
static int ed448_verify(void *vpeddsactx, const unsigned char *sig,
  ...
  return ossl_ed448_verify(
  peddsactx->libctx, tbs, tbslen, sig, edkey->pubkey, peddsactx->context_string,
  peddsactx->context_string_len, peddsactx->prehash_flag, edkey->propq);
}
