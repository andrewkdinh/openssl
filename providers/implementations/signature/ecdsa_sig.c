...
static int ecdsa_get_ctx_params(void *vctx, OSSL_PARAM *params) {
    ...
    if (p != NULL &&
        ...
        p, ctx->md == NULL ? ctx->mdname : EVP_MD_get0_name(ctx->md)))
        return 0;
...
static int ecdsa_sigalg_set_ctx_params(void *vctx, const OSSL_PARAM params[]) {
    ...
    if (ctx->operation == EVP_PKEY_OP_VERIFYMSG) {
        ...
        if (p != NULL) {
            ...
            ctx->sig = NULL;
            ctx->siglen = 0;
            if (!OSSL_PARAM_get_octet_string(p, (void **)&ctx->sig, 0,
                                             &ctx->siglen))
                return 0;
        }
    }
...
#define IMPL_ECDSA_SIGALG(md, MD)                                              \
    ...
    static int ecdsa_##md##_sign_init(void *vctx, void *vec,                   \
        ...
        static const char desc[] = "ECDSA-" MD " Sign Init";                   \
                                                                               \
        return ecdsa_sigalg_signverify_init(                                   \
        vctx, vec, ecdsa_sigalg_set_ctx_params, params, MD, EVP_PKEY_OP_SIGN,  \
        desc);                                                                 \
    }                                                                          \
                                                                               \
    static int ecdsa_##md##_sign_message_init(void *vctx, void *vec,           \
        ...
        static const char desc[] = "ECDSA-" MD " Sign Message Init";           \
                                                                               \
        return ecdsa_sigalg_signverify_init(                                   \
        vctx, vec, ecdsa_sigalg_set_ctx_params, params, MD,                    \
