...
static int ecdsa_setup_md(PROV_ECDSA_CTX *ctx, const char *mdname, const char *mdprops, const char *desc)
{
    EVP_MD *md = NULL;
    size_t mdname_len;
    int md_nid, md_size;
    ...
    if (md_size <= 0) {
        ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_DIGEST, "%s has invalid md size %d", mdname, md_size);
        goto err;
    }
...
#else
    {
        ...
        if (WPACKET_init_der(&pkt, ctx->aid_buf, sizeof(ctx->aid_buf))
            && ossl_DER_w_algorithmIdentifier_ECDSA_with_MD(&pkt, -1, ctx->ec, md_nid) && WPACKET_finish(&pkt)) {
...
static int ecdsa_signverify_init(PROV_ECDSA_CTX *ctx, void *ec, OSSL_FUNC_signature_set_ctx_params_fn *set_ctx_params,
                                 const OSSL_PARAM params[], int operation, const char *desc)
{
    ...
    if (!set_ctx_params(ctx, params))
        return 0;
#ifdef FIPS_MODULE
    if (!ossl_fips_ind_ec_key_check(OSSL_FIPS_IND_GET(ctx), OSSL_FIPS_IND_SETTABLE0, ctx->libctx,
...
static int ecdsa_sign_init(void *vctx, void *ec, const OSSL_PARAM params[])
{
    ...
    ctx->verify_message = 1;
#endif
    return ecdsa_signverify_init(ctx, ec, ecdsa_set_ctx_params, params, EVP_PKEY_OP_SIGN, "ECDSA Sign Init");
}
 ...
 */
static int ecdsa_sign_directly(void *vctx, unsigned char *sig, size_t *siglen, size_t sigsize, const unsigned char *tbs,
                               size_t tbslen)
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
    int ret;
    ...
    if (ctx->nonce_type != 0) {
        ...
        if (ctx->mdname[0] != '\0')
            mdname = ctx->mdname;
        ret = ossl_ecdsa_deterministic_sign(tbs, tbslen, sig, &sltmp, ctx->ec, ctx->nonce_type, mdname, ctx->libctx,
                                            ctx->propq);
    } else {
...
static int ecdsa_signverify_message_update(void *vctx, const unsigned char *data, size_t datalen)
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
...
static int ecdsa_sign_message_final(void *vctx, unsigned char *sig, size_t *siglen, size_t sigsize)
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
 ...
 */
static int ecdsa_sign(void *vctx, unsigned char *sig, size_t *siglen, size_t sigsize, const unsigned char *tbs,
                      size_t tbslen)
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
...
#endif
    return ecdsa_signverify_init(ctx, ec, ecdsa_set_ctx_params, params, EVP_PKEY_OP_VERIFY, "ECDSA Verify Init");
}

static int ecdsa_verify_directly(void *vctx, const unsigned char *sig, size_t siglen, const unsigned char *tbs,
                                 size_t tbslen)
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
...
static int ecdsa_verify_set_sig(void *vctx, const unsigned char *sig, size_t siglen)
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
    OSSL_PARAM params[2];
...
static int ecdsa_verify_message_final(void *vctx)
{
    ...
    return ecdsa_verify_directly(vctx, ctx->sig, ctx->siglen, digest, dlen);
}
...
static int ecdsa_digest_signverify_init(void *vctx, const char *mdname, void *ec, const OSSL_PARAM params[],
                                        int operation, const char *desc)
{
    ...
    ctx->verify_message = 1;
#endif
    if (!ecdsa_signverify_init(vctx, ec, ecdsa_set_ctx_params, params, operation, desc))
        return 0;
...
{
    return ecdsa_digest_signverify_init(vctx, mdname, ec, params, EVP_PKEY_OP_SIGNMSG, "ECDSA Digest Sign Init");
}

static int ecdsa_digest_signverify_update(void *vctx, const unsigned char *data, size_t datalen)
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
...
{
    return ecdsa_digest_signverify_init(vctx, mdname, ec, params, EVP_PKEY_OP_VERIFYMSG, "ECDSA Digest Verify Init");
}
...
static int ecdsa_common_set_ctx_params(void *vctx, const OSSL_PARAM params[])
{
    ...
    if (!OSSL_FIPS_IND_SET_CTX_PARAM(ctx, OSSL_FIPS_IND_SETTABLE0, params, OSSL_SIGNATURE_PARAM_FIPS_KEY_CHECK))
        return 0;
    if (!OSSL_FIPS_IND_SET_CTX_PARAM(ctx, OSSL_FIPS_IND_SETTABLE1, params, OSSL_SIGNATURE_PARAM_FIPS_DIGEST_CHECK))
        return 0;
    ...
    if (p != NULL && !OSSL_PARAM_get_uint(p, &ctx->nonce_type))
        return 0;
    return 1;
}
 ...
 */
static int ecdsa_sigalg_signverify_init(void *vctx, void *vec, OSSL_FUNC_signature_set_ctx_params_fn *set_ctx_params,
                                        const OSSL_PARAM params[], const char *mdname, int operation, const char *desc)
{
    ...
    if (!ecdsa_signverify_init(vctx, vec, set_ctx_params, params, operation, desc))
        return 0;
...
static const OSSL_PARAM settable_sigalg_ctx_params[] = {
    OSSL_PARAM_octet_string(OSSL_SIGNATURE_PARAM_SIGNATURE, NULL, 0), ECDSA_COMMON_SETTABLE_CTX_PARAMS};

static const OSSL_PARAM *ecdsa_sigalg_settable_ctx_params(void *vctx, ossl_unused void *provctx)
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
...
static int ecdsa_sigalg_set_ctx_params(void *vctx, const OSSL_PARAM params[])
{
    ...
    if (ctx->operation == EVP_PKEY_OP_VERIFYMSG) {
        ...
        if (p != NULL) {
            ...
            ctx->sig = NULL;
            ctx->siglen = 0;
            if (!OSSL_PARAM_get_octet_string(p, (void **)&ctx->sig, 0, &ctx->siglen))
                return 0;
        }
    }
    return 1;
