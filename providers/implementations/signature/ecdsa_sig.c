...
static int ecdsa_signverify_init(PROV_ECDSA_CTX                        *ctx,
                                 void                                  *ec,
                               ...
                               size_t               tbslen)
{
    ...
    if (ctx->nonce_type != 0) {
        ...
        if (ctx->mdname[0] != '\0')
            mdname = ctx->mdname;
        ret = ossl_ecdsa_deterministic_sign(tbs,
                                            (int)tbslen,
                                            sig,
                                            &sltmp,
                                            ctx->ec,
        ...
        ret = ECDSA_sign_ex(0, tbs, (int)tbslen, sig, &sltmp, ctx->kinv, ctx->r, ctx->ec);
    }
    if (ret <= 0)
        return 0;
 ...
 */
static int
ecdsa_sign(void *vctx, unsigned char *sig, size_t *siglen, size_t sigsize, const unsigned char *tbs, size_t tbslen)
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
...
static int
ecdsa_verify_directly(void *vctx, const unsigned char *sig, size_t siglen, const unsigned char *tbs, size_t tbslen)
{
    PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
...
static int ecdsa_digest_signverify_init(void            *vctx,
                                        const char      *mdname,
                                        void            *ec,
                                        const OSSL_PARAM params[],
                                        int              operation,
 ...
 */
static int                                        ecdsa_sigalg_signverify_init(void                                  *vctx,
                                                                               void                                  *vec,
                                                                               OSSL_FUNC_signature_set_ctx_params_fn *set_ctx_params,
                                                                               const OSSL_PARAM                       params[],
                                                                               const char                            *mdname,
