...
static int ecdsa_setup_md(PROV_ECDSA_CTX *ctx, const char *mdname,
  ...
  if (!ctx->flag_allow_md) {
    if (ctx->mdname[0] != '\0' && !EVP_MD_is_a(md, ctx->mdname)) {
      ERR_raise_data(ERR_LIB_PROV, PROV_R_DIGEST_NOT_ALLOWED, "digest %s != %s",
                     mdname, ctx->mdname);
      goto err;
    }
    EVP_MD_free(md);
...
ecdsa_signverify_init(PROV_ECDSA_CTX *ctx, void *ec,
  ...
  if (!set_ctx_params(ctx, params))
    return 0;
#ifdef FIPS_MODULE
  if (!ossl_fips_ind_ec_key_check(
      OSSL_FIPS_IND_GET(ctx), OSSL_FIPS_IND_SETTABLE0, ctx->libctx,
 ...
 */
static int ecdsa_sign_directly(void *vctx, unsigned char *sig, size_t *siglen,
  ...
  if (ctx->nonce_type != 0) {
    ...
    if (ctx->mdname[0] != '\0')
      mdname = ctx->mdname;
    ret = ossl_ecdsa_deterministic_sign(tbs, tbslen, sig, &sltmp, ctx->ec,
                                        ctx->nonce_type, mdname, ctx->libctx,
                                        ctx->propq);
  } else {
    ret =
    ECDSA_sign_ex(0, tbs, tbslen, sig, &sltmp, ctx->kinv, ctx->r, ctx->ec);
  }
  if (ret <= 0)
    return 0;
...
static int ecdsa_verify_directly(void *vctx, const unsigned char *sig,
...
static int ecdsa_verify_set_sig(void *vctx, const unsigned char *sig,
                                size_t siglen) {
  PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
  OSSL_PARAM params[2];
...
static int ecdsa_digest_signverify_init(void *vctx, const char *mdname,
  ...
  ctx->verify_message = 1;
#endif
  if (!ecdsa_signverify_init(vctx, ec, ecdsa_set_ctx_params, params, operation,
                             desc))
    return 0;
...
static int ecdsa_digest_sign_init(void *vctx, const char *mdname, void *ec,
  ...
  return ecdsa_digest_signverify_init(
  vctx, mdname, ec, params, EVP_PKEY_OP_SIGNMSG, "ECDSA Digest Sign Init");
}
...
static int ecdsa_digest_verify_init(void *vctx, const char *mdname, void *ec,
                                    const OSSL_PARAM params[]) {
  return ecdsa_digest_signverify_init(
  vctx, mdname, ec, params, EVP_PKEY_OP_VERIFYMSG, "ECDSA Digest Verify Init");
}
...
static int ecdsa_get_ctx_params(void *vctx, OSSL_PARAM *params) {
  ...
  if (p != NULL && !OSSL_PARAM_set_octet_string(
 ...
 */
static int ecdsa_sigalg_signverify_init(
void *vctx, void *vec, OSSL_FUNC_signature_set_ctx_params_fn *set_ctx_params,
...
ecdsa_sigalg_settable_ctx_params(void *vctx, ossl_unused void *provctx) {
  PROV_ECDSA_CTX *ctx = (PROV_ECDSA_CTX *)vctx;
...
#define IMPL_ECDSA_SIGALG(md, MD)                                              \
  ...
  ecdsa_##md##_verify_message_init;                                            \
                                                                               \
  static int ecdsa_##md##_sign_init(void *vctx, void *vec,                     \
                                    const OSSL_PARAM params[]) {               \
    static const char desc[] = "ECDSA-" MD " Sign Init";                      \
                                                                               \
    return ecdsa_sigalg_signverify_init(vctx, vec,                             \
                                        ecdsa_sigalg_set_ctx_params, params,   \
                                        MD, EVP_PKEY_OP_SIGN, desc);          \
  }                                                                            \
                                                                               \
  static int ecdsa_##md##_sign_message_init(void *vctx, void *vec,             \
                                            const OSSL_PARAM params[]) {       \
    ...
    return ecdsa_sigalg_signverify_init(vctx, vec,                             \
                                        ...
                                        MD, EVP_PKEY_OP_SIGNMSG, desc);       \
                                                                               ...
                                                                               \
  static int ecdsa_##md##_verify_init(void *vctx, void *vec,                   \
                                      const OSSL_PARAM params[]) {             \
    ...
    return ecdsa_sigalg_signverify_init(vctx, vec,                             \
                                        ...
                                        MD, EVP_PKEY_OP_VERIFY, desc);        \
                                                                               ...
                                                                               \
  static int ecdsa_##md##_verify_message_init(void *vctx, void *vec,           \
                                              const OSSL_PARAM params[]) {     \
  ...
  {OSSL_FUNC_SIGNATURE_NEWCTX, (void (*)(void))ecdsa_newctx},                  \
  {OSSL_FUNC_SIGNATURE_SIGN_INIT, (void (*)(void))ecdsa_##md##_sign_init},     \
  {OSSL_FUNC_SIGNATURE_SIGN, (void (*)(void))ecdsa_sign},                      \
  ...
  {OSSL_FUNC_SIGNATURE_VERIFY_INIT, (void (*)(void))ecdsa_##md##_verify_init}, \
  {OSSL_FUNC_SIGNATURE_VERIFY, (void (*)(void))ecdsa_verify},                  \
