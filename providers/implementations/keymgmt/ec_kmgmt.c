...
static ossl_inline int key_to_params(const EC_KEY *eckey, OSSL_PARAM_BLD *tmpl,
    ...
    if (pub_point != NULL) {
        ...
        if (p != NULL || tmpl != NULL) {
            /* convert pub_point to a octet string according to the SECG
             * standard */
            point_conversion_form_t format = EC_KEY_get_conv_form(eckey);

            if ((pub_key_len = EC_POINT_point2buf(ecg, pub_point, format,
                                                  pub_key, bnctx)) == 0 ||
                !ossl_param_build_set_octet_string(
                tmpl, p, OSSL_PKEY_PARAM_PUB_KEY, *pub_key, pub_key_len))
                goto err;
        }
        if (px != NULL || py != NULL) {
            if (px != NULL) {
...
static int ec_get_ecm_params(const EC_GROUP *group, OSSL_PARAM params[]) {
    ...
    if (basis_nid == NID_X9_62_tpBasis) {
        if (!EC_GROUP_get_trinomial_basis(group, &k1) ||
            ...
            NULL, params, OSSL_PKEY_PARAM_EC_CHAR2_TP_BASIS, (int)k1))
            goto err;
    } else {
        if (!EC_GROUP_get_pentanomial_basis(group, &k1, &k2, &k3) ||
            !ossl_param_build_set_int(
            NULL, params, OSSL_PKEY_PARAM_EC_CHAR2_PP_K1, (int)k1) ||
            !ossl_param_build_set_int(
            NULL, params, OSSL_PKEY_PARAM_EC_CHAR2_PP_K2, (int)k2) ||
...
static int common_get_params(void *key, OSSL_PARAM params[], int sm2) {
    ...
    if ((p = OSSL_PARAM_locate(params, OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY)) !=
        ...
        ecg, ecp, POINT_CONVERSION_UNCOMPRESSED, p->data, p->data_size, bnctx);
        if (p->return_size == 0)
            goto err;
    }
...
static void *ec_gen(void *genctx, OSSL_CALLBACK *osslcb, void *cbarg) {
    ...
    if (gctx->group_check != NULL)
        ret =
        ret && ossl_ec_set_check_group_type_from_name(ec, gctx->group_check);
    if (ret)
        return ec;
err:
