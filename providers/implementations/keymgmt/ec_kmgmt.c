 ...
 */
static ossl_inline int key_to_params(const EC_KEY *eckey, OSSL_PARAM_BLD *tmpl, OSSL_PARAM params[],
...
static ossl_inline int otherparams_to_params(const EC_KEY *ec, OSSL_PARAM_BLD *tmpl, OSSL_PARAM params[])
{
    ...
    if (name != NULL
        && !ossl_param_build_set_utf8_string(tmpl, params, OSSL_PKEY_PARAM_EC_POINT_CONVERSION_FORMAT, name))
        return 0;
    ...
    if ((EC_KEY_get_enc_flags(ec) & EC_PKEY_NO_PUBKEY) != 0
        && !ossl_param_build_set_int(tmpl, params, OSSL_PKEY_PARAM_EC_INCLUDE_PUBLIC, 0))
        return 0;
...
static int ec_export(void *keydata, int selection, OSSL_CALLBACK *param_cb, void *cbarg)
{
    ...
    if ((selection & OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) != 0) {
        ...
        BN_CTX_start(bnctx);
        ok = ok
            && ossl_ec_group_todata(EC_KEY_get0_group(ec), tmpl, NULL, ossl_ec_key_get_libctx(ec),
                                    ossl_ec_key_get0_propq(ec), bnctx, &genbuf);
    }
...
static int ec_get_ecm_params(const EC_GROUP *group, OSSL_PARAM params[])
{
    ...
    if (basis_nid == NID_X9_62_tpBasis) {
        if (!EC_GROUP_get_trinomial_basis(group, &k1)
            || !ossl_param_build_set_int(NULL, params, OSSL_PKEY_PARAM_EC_CHAR2_TP_BASIS, (int)k1))
            goto err;
    } else {
        if (!EC_GROUP_get_pentanomial_basis(group, &k1, &k2, &k3)
            || !ossl_param_build_set_int(NULL, params, OSSL_PKEY_PARAM_EC_CHAR2_PP_K1, (int)k1)
...
static int ec_validate(const void *keydata, int selection, int checktype)
{
    ...
    if ((selection & OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) != 0) {
        ...
        if ((flags & EC_FLAG_CHECK_NAMED_GROUP) != 0)
            ok = ok
                && EC_GROUP_check_named_curve(EC_KEY_get0_group(eck), (flags & EC_FLAG_CHECK_NAMED_GROUP_NIST) != 0,
                                              ctx)
                    > 0;
        else
            ok = ok && EC_GROUP_check(EC_KEY_get0_group(eck), ctx);
...
static int ec_gen_set_params(void *genctx, const OSSL_PARAM params[])
{
    ...
    if (!OSSL_FIPS_IND_SET_CTX_PARAM(gctx, OSSL_FIPS_IND_SETTABLE0, params, OSSL_PKEY_PARAM_FIPS_KEY_CHECK))
        goto err;
    ...
    COPY_OCTET_PARAM(params, OSSL_PKEY_PARAM_DHKEM_IKM, gctx->dhkem_ikm, gctx->dhkem_ikmlen);
...
static int ec_gen_set_group_from_params(struct ec_gen_ctx *gctx)
{
    ...
    if (gctx->pt_format != NULL
        && !OSSL_PARAM_BLD_push_utf8_string(bld, OSSL_PKEY_PARAM_EC_POINT_CONVERSION_FORMAT, gctx->pt_format, 0))
        goto err;
    ...
    if (gctx->seed != NULL
        && !OSSL_PARAM_BLD_push_octet_string(bld, OSSL_PKEY_PARAM_EC_SEED, gctx->seed, gctx->seed_len))
        goto err;

    if (gctx->gen == NULL
        || !OSSL_PARAM_BLD_push_octet_string(bld, OSSL_PKEY_PARAM_EC_GENERATOR, gctx->gen, gctx->gen_len))
        goto err;
build:
...
static const OSSL_PARAM *ec_gen_settable_params(ossl_unused void *genctx, ossl_unused void *provctx)
{
    static OSSL_PARAM settable[] = {OSSL_PARAM_utf8_string(OSSL_PKEY_PARAM_GROUP_NAME, NULL, 0),
                                    ...
                                    OSSL_FIPS_IND_SETTABLE_CTX_PARAM(OSSL_PKEY_PARAM_FIPS_KEY_CHECK) OSSL_PARAM_END};
    return settable;
}

static const OSSL_PARAM *ec_gen_gettable_params(ossl_unused void *genctx, ossl_unused void *provctx)
{
    static const OSSL_PARAM known_ec_gen_gettable_ctx_params[] = {OSSL_FIPS_IND_GETTABLE_CTX_PARAM() OSSL_PARAM_END};
...
#ifdef FIPS_MODULE
    if (!ossl_fips_ind_ec_key_check(OSSL_FIPS_IND_GET(gctx), OSSL_FIPS_IND_SETTABLE0, gctx->libctx, gctx->gen_group,
                                    "EC KeyGen", 1))
        goto err;
#endif
...
const OSSL_DISPATCH ossl_ec_keymgmt_functions[] = {
    ...
    {OSSL_FUNC_KEYMGMT_GEN_SETTABLE_PARAMS, (void (*)(void))ec_gen_settable_params},
    {OSSL_FUNC_KEYMGMT_GEN_GET_PARAMS, (void (*)(void))ec_gen_get_params},
    {OSSL_FUNC_KEYMGMT_GEN_GETTABLE_PARAMS, (void (*)(void))ec_gen_gettable_params},
    ...
    {OSSL_FUNC_KEYMGMT_QUERY_OPERATION_NAME, (void (*)(void))ec_query_operation_name},
...
const OSSL_DISPATCH ossl_sm2_keymgmt_functions[] = {
    ...
    {OSSL_FUNC_KEYMGMT_EXPORT, (void (*)(void))ec_export},
    {OSSL_FUNC_KEYMGMT_EXPORT_TYPES, (void (*)(void))ec_export_types},
    {OSSL_FUNC_KEYMGMT_QUERY_OPERATION_NAME, (void (*)(void))sm2_query_operation_name},
    {OSSL_FUNC_KEYMGMT_DUP, (void (*)(void))ec_dup},
    OSSL_DISPATCH_END};
# endif
#endif
