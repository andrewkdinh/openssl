...
static ossl_inline int key_to_params(const EC_KEY *eckey, OSSL_PARAM_BLD *tmpl,
  ...
  if (pub_point != NULL) {
    ...
    if (p != NULL || tmpl != NULL) {
      ...
      if ((pub_key_len =
           EC_POINT_point2buf(ecg, pub_point, format, pub_key, bnctx)) == 0 ||
          !ossl_param_build_set_octet_string(tmpl, p, OSSL_PKEY_PARAM_PUB_KEY,
                                             *pub_key, pub_key_len))
        goto err;
    }
    if (px != NULL || py != NULL) {
      ...
      if (!EC_POINT_get_affine_coordinates(ecg, pub_point, x, y, bnctx))
        goto err;
      if (px != NULL &&
          !ossl_param_build_set_bn(tmpl, px, OSSL_PKEY_PARAM_EC_PUB_X, x))
        goto err;
      if (py != NULL &&
          !ossl_param_build_set_bn(tmpl, py, OSSL_PKEY_PARAM_EC_PUB_Y, y))
        goto err;
    }
  }
  ...
  if (priv_key != NULL && include_private) {
    ...
    if (!ossl_param_build_set_bn_pad(tmpl, params, OSSL_PKEY_PARAM_PRIV_KEY,
                                     priv_key, sz))
      goto err;
  }
  ret = 1;
...
static void ec_freedata(void *keydata) { EC_KEY_free(keydata); }
  ...
  if (basis_nid == NID_X9_62_tpBasis) {
    if (!EC_GROUP_get_trinomial_basis(group, &k1) ||
        !ossl_param_build_set_int(NULL, params,
                                  OSSL_PKEY_PARAM_EC_CHAR2_TP_BASIS, (int)k1))
      goto err;
  } else {
    if (!EC_GROUP_get_pentanomial_basis(group, &k1, &k2, &k3) ||
        !ossl_param_build_set_int(NULL, params, OSSL_PKEY_PARAM_EC_CHAR2_PP_K1,
                                  (int)k1) ||
        !ossl_param_build_set_int(NULL, params, OSSL_PKEY_PARAM_EC_CHAR2_PP_K2,
                                  (int)k2) ||
        !ossl_param_build_set_int(NULL, params, OSSL_PKEY_PARAM_EC_CHAR2_PP_K3,
                                  (int)k3))
      goto err;
  }
  ret = 1;
...
static int common_get_params(void *key, OSSL_PARAM params[], int sm2) {
  ...
  if ((p = OSSL_PARAM_locate(
       params, OSSL_PKEY_PARAM_EC_DECODED_FROM_EXPLICIT_PARAMS)) != NULL) {
  ...
  ret =
  ec_get_ecm_params(ecg, params) &&
  ossl_ec_group_todata(ecg, NULL, params, libctx, propq, bnctx, &genbuf) &&
...
static int ec_validate(const void *keydata, int selection, int checktype) {
  ...
  if ((selection & OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) != 0) {
    ...
    if ((flags & EC_FLAG_CHECK_NAMED_GROUP) != 0)
      ok = ok && EC_GROUP_check_named_curve(
...
static int ec_gen_set_group_from_params(struct ec_gen_ctx *gctx) {
  ...
  if (gctx->pt_format != NULL &&
      ...
      bld, OSSL_PKEY_PARAM_EC_POINT_CONVERSION_FORMAT, gctx->pt_format, 0))
    goto err;
