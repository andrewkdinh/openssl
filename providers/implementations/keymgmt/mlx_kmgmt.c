...
static void *mlx_kem_key_new(unsigned int v, OSSL_LIB_CTX *libctx,
                             char *propq) {
  MLX_KEY *key = NULL;
  unsigned int ml_kem_variant;

  if (!ossl_prov_is_running() || v >= OSSL_NELEM(hybrid_vtable) ||
...
static int mlx_kem_export(void *vkey, int selection, OSSL_CALLBACK *param_cb,
  ...
  if (sub_arg.pubenc != NULL && sub_arg.pubcount == 2 &&
      !ossl_param_build_set_octet_string(tmpl, NULL, OSSL_PKEY_PARAM_PUB_KEY,
                                         sub_arg.pubenc, publen))
    goto err;

  if (sub_arg.prvenc != NULL && sub_arg.prvcount == 2 &&
      !ossl_param_build_set_octet_string(tmpl, NULL, OSSL_PKEY_PARAM_PRIV_KEY,
                                         sub_arg.prvenc, prvlen))
    goto err;
...
static int mlx_kem_key_fromdata(MLX_KEY *key, const OSSL_PARAM params[],
  ...
  if (param_pub_key != NULL &&
    ...
    return 0;
  if (include_private)
    param_prv_key = OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_PRIV_KEY);
...
static int mlx_kem_import(void *vkey, int selection,
  ...
  if (pub != NULL) {
    ...
    if (pub->data == NULL) {
      ...
      ERR_raise_data(ERR_LIB_PROV, PROV_R_OUTPUT_BUFFER_TOO_SMALL,
                     ...
                     (unsigned long)pub->data_size, (unsigned long)publen);
      return 0;
    } else {
    ...
    if (prv != NULL) {
      ...
      if (prv->data == NULL) {
        ...
        ERR_raise_data(ERR_LIB_PROV, PROV_R_OUTPUT_BUFFER_TOO_SMALL,
                       ...
                       (unsigned long)prv->data_size, (unsigned long)prvlen);
        return 0;
      } else {
...
mlx_kem_gen_settable_params(ossl_unused void *vgctx,
                            ossl_unused void *provctx) {
  static OSSL_PARAM settable[] = {
  OSSL_PARAM_octet_string(OSSL_PKEY_PARAM_PROPERTIES, NULL, 0), OSSL_PARAM_END};
...
static void *mlx_kem_gen(void *vgctx, OSSL_CALLBACK *osslcb, void *cbarg) {
  ...
  key->mkey =
  EVP_PKEY_Q_keygen(key->libctx, key->propq, key->minfo->algorithm_name);
  key->xkey = EVP_PKEY_Q_keygen(
  key->libctx, key->propq, key->xinfo->algorithm_name, key->xinfo->group_name);
...
static void *mlx_kem_dup(const void *vkey, int selection) {
  ...
  if (ret->propq != NULL && (ret->propq = OPENSSL_strdup(ret->propq)) == NULL) {
    OPENSSL_free(ret);
    return NULL;
  }
...
#define DECLARE_DISPATCH(name, variant)                                        \
  ...
  {OSSL_FUNC_KEYMGMT_GEN_SETTABLE_PARAMS,                                      \
   (OSSL_FUNC)mlx_kem_gen_settable_params},                                    \
