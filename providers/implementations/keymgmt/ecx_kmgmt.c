...
static int ecx_match(const void *keydata1, const void *keydata2,
                     int selection) {
    ...
    if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
        ...
        if ((selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) != 0) {
            ...
            if (pa != NULL && pb != NULL) {
                ok = ok && key1->type == key2->type && pal == pbl &&
...
static int key_to_params(ECX_KEY *key, OSSL_PARAM_BLD *tmpl,
    ...
    if (!ossl_param_build_set_octet_string(
        tmpl, params, OSSL_PKEY_PARAM_PUB_KEY, key->pubkey, key->keylen))
        return 0;

    if (include_private && key->privkey != NULL &&
        !ossl_param_build_set_octet_string(
        tmpl, params, OSSL_PKEY_PARAM_PRIV_KEY, key->privkey, key->keylen))
        return 0;
...
static int ecx_set_params(void *key, const OSSL_PARAM params[]) {
    ...
    if (p != NULL) {
        ...
        if (p->data_size != ecxkey->keylen ||
            !OSSL_PARAM_get_octet_string(p, &buf, sizeof(ecxkey->pubkey), NULL))
            return 0;
...
static int ed448_set_params(void *key, const OSSL_PARAM params[]) { return 1; }
 ...
 *
 * The parameter `self_test` is used to indicate whether to create
 * OSSL_SELF_TEST instance.
 */
static int ecd_fips140_pairwise_test(const ECX_KEY *ecx, int type,
                                     int self_test) {
    int ret = 0;
    ...
    if (is_ed25519)
        operation_result =
        ossl_ed25519_sign(sig, msg, msg_len, ecx->pubkey, ecx->privkey, 0, 0, 0,
                          NULL, 0, ecx->libctx, ecx->propq);
    else
        operation_result =
        ossl_ed448_sign(ecx->libctx, sig, msg, msg_len, ecx->pubkey,
                        ecx->privkey, NULL, 0, 0, ecx->propq);
    ...
    if (is_ed25519)
        operation_result =
        ossl_ed25519_verify(msg, msg_len, sig, ecx->pubkey, 0, 0, 0, NULL, 0,
                            ecx->libctx, ecx->propq);
    else
        operation_result = ossl_ed448_verify(
        ecx->libctx, msg, msg_len, sig, ecx->pubkey, NULL, 0, 0, ecx->propq);
    if (operation_result != 1)
        goto err;
...
static void *ecx_gen(struct ecx_gen_ctx *gctx) {
    ...
    if (gctx == NULL)
        return NULL;
    if ((key = ossl_ecx_key_new(gctx->libctx, gctx->type, 0, gctx->propq)) ==
        NULL) {
    ...
    if (gctx->dhkem_ikm != NULL && gctx->dhkem_ikmlen != 0) {
        if (gctx->type == ECX_KEY_TYPE_ED25519 ||
            ...
            goto err;
        if (!ossl_ecx_dhkem_derive_private(key, privkey, gctx->dhkem_ikm,
                                           gctx->dhkem_ikmlen))
            goto err;
    } else
#endif
    ...
    case ECX_KEY_TYPE_ED25519:
        if (!ossl_ed25519_public_from_private(gctx->libctx, key->pubkey,
                                              privkey, gctx->propq))
            goto err;
        break;
...
#define MAKE_KEYMGMT_FUNCTIONS(alg)                                            \
    ...
    {OSSL_FUNC_KEYMGMT_GETTABLE_PARAMS,                                        \
     (void (*)(void))alg##_gettable_params},                                   \
    {OSSL_FUNC_KEYMGMT_SET_PARAMS, (void (*)(void))alg##_set_params},          \
    {OSSL_FUNC_KEYMGMT_SETTABLE_PARAMS,                                        \
     (void (*)(void))alg##_settable_params},                                   \
...
static void *s390x_ecx_keygen25519(struct ecx_gen_ctx *gctx) {
    ...
    ECX_KEY *key =
    ossl_ecx_key_new(gctx->libctx, ECX_KEY_TYPE_X25519, 1, gctx->propq);
...
static void *s390x_ecx_keygen448(struct ecx_gen_ctx *gctx) {
    ...
    ECX_KEY *key =
    ossl_ecx_key_new(gctx->libctx, ECX_KEY_TYPE_X448, 1, gctx->propq);
    unsigned char *privkey = NULL, *pubkey;
...
static void *s390x_ecd_keygen25519(struct ecx_gen_ctx *gctx) {
    ...
    ECX_KEY *key =
    ossl_ecx_key_new(gctx->libctx, ECX_KEY_TYPE_ED25519, 1, gctx->propq);
    unsigned char *privkey = NULL, *pubkey;
    unsigned int sz;
    EVP_MD *sha = NULL;
    int j;
...
static void *s390x_ecd_keygen448(struct ecx_gen_ctx *gctx) {
    ...
    ECX_KEY *key =
    ossl_ecx_key_new(gctx->libctx, ECX_KEY_TYPE_ED448, 1, gctx->propq);
    unsigned char *privkey = NULL, *pubkey;
