...
static int key_to_params(ECX_KEY *key, OSSL_PARAM_BLD *tmpl,
  ...
  if (include_private && key->privkey != NULL &&
      !ossl_param_build_set_octet_string(tmpl, params, OSSL_PKEY_PARAM_PRIV_KEY,
...
static const OSSL_PARAM *ecx_gen_settable_params(ossl_unused void *genctx,
  ...
  OSSL_PARAM_octet_string(OSSL_PKEY_PARAM_DHKEM_IKM, NULL, 0), OSSL_PARAM_END};
  return settable;
}
...
static int ecd_fips140_pairwise_test(const ECX_KEY *ecx, int type,
  ...
  if (is_ed25519)
    ...
    operation_result =
    ossl_ed448_sign(ecx->libctx, sig, msg, msg_len, ecx->pubkey, ecx->privkey,
                    NULL, 0, 0, ecx->propq);
  ...
  if (is_ed25519)
    operation_result = ossl_ed25519_verify(msg, msg_len, sig, ecx->pubkey, 0, 0,
                                           0, NULL, 0, ecx->libctx, ecx->propq);
  else
