...
static void mac_free(void *mackey) { ossl_mac_key_free(mackey); }
...
static int mac_match(const void *keydata1, const void *keydata2,
                     int selection) {
    ...
    if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0) {
        if ((key1->priv_key == NULL && key2->priv_key != NULL) ||
            ...
            ok =
            ok && (key1->priv_key == NULL /* implies key2->privkey == NULL */
                   || CRYPTO_memcmp(key1->priv_key, key2->priv_key,
                                    key1->priv_key_len) == 0);
        if (key1->cipher.cipher != NULL)
            ok =
            ok && EVP_CIPHER_is_a(key1->cipher.cipher,
...
static int mac_key_fromdata(MAC_KEY *key, const OSSL_PARAM params[]) {
    ...
    if (p != NULL) {
         ...
         */
        key->priv_key =
        OPENSSL_secure_malloc(p->data_size > 0 ? p->data_size : 1);
    ...
    if (key->cmac &&
        !ossl_prov_cipher_load_from_params(&key->cipher, params, key->libctx)) {
        ERR_raise(ERR_LIB_PROV, ERR_R_PASSED_INVALID_ARGUMENT);
        return 0;
    }
...
static int key_to_params(MAC_KEY *key, OSSL_PARAM_BLD *tmpl,
    ...
    if (key->priv_key != NULL && !ossl_param_build_set_octet_string(
                                 tmpl, params, OSSL_PKEY_PARAM_PRIV_KEY,
                                 key->priv_key, key->priv_key_len))
        return 0;

    if (key->cipher.cipher != NULL && !ossl_param_build_set_utf8_string(
...
static const OSSL_PARAM *cmac_gen_settable_params(ossl_unused void *genctx,
    ...
    static OSSL_PARAM settable[] = {
    OSSL_PARAM_octet_string(OSSL_PKEY_PARAM_PRIV_KEY, NULL, 0),
    OSSL_PARAM_utf8_string(OSSL_PKEY_PARAM_CIPHER, NULL, 0), OSSL_PARAM_END};
    return settable;
}
