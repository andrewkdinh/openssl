...
static int ml_kem_pairwise_test(const ML_KEM_KEY *key, int key_flags) {
  ...
  if (key_flags & ML_KEM_KEY_RANDOM_PCT) {
    operation_result =
    ossl_ml_kem_encap_rand(ctext, v->ctext_bytes, secret, sizeof(secret), key);
  } else {
    memset(entropy, 0125, sizeof(entropy));
    operation_result =
    ossl_ml_kem_encap_seed(ctext, v->ctext_bytes, secret, sizeof(secret),
                           entropy, sizeof(entropy), key);
  }
  ...
  operation_result =
  ossl_ml_kem_decap(out, sizeof(out), ctext, v->ctext_bytes, key);
...
ML_KEM_KEY *ossl_prov_ml_kem_new(PROV_CTX *ctx, const char *propq,
                                 int evp_type) {
  ML_KEM_KEY *key;
   ...
   */
  if ((key = ossl_ml_kem_key_new(PROV_LIBCTX_OF(ctx), propq, evp_type)) !=
      NULL) {
    const char *pct_type = ossl_prov_ctx_get_param(
    ctx, OSSL_PKEY_PARAM_ML_KEM_IMPORT_PCT_TYPE, "random");

    if (ossl_prov_ctx_get_bool_param(ctx, OSSL_PKEY_PARAM_ML_KEM_RETAIN_SEED,
                                     1))
      key->prov_flags |= ML_KEM_KEY_RETAIN_SEED;
    else
      key->prov_flags &= ~ML_KEM_KEY_RETAIN_SEED;
    if (ossl_prov_ctx_get_bool_param(ctx, OSSL_PKEY_PARAM_ML_KEM_PREFER_SEED,
                                     1))
...
static int ml_kem_export(void *vkey, int selection, OSSL_CALLBACK *param_cb,
  ...
  if (prvenc != NULL && !ossl_param_build_set_octet_string(
                        tmpl, params, OSSL_PKEY_PARAM_PRIV_KEY, prvenc, prvlen))
    goto err;

  /* The public key on request; it is always available when either is */
  if (pubenc != NULL &&
      !ossl_param_build_set_octet_string(tmpl, params, OSSL_PKEY_PARAM_PUB_KEY,
...
static int check_prvenc(const uint8_t *prvenc, ML_KEM_KEY *key) {
  ...
  if (buf != NULL && ossl_ml_kem_encode_private_key(buf, len, key))
...
static int ml_kem_key_fromdata(ML_KEM_KEY *key, const OSSL_PARAM params[],
  ...
  if (include_private) {
    ...
    if (p != NULL && OSSL_PARAM_get_octet_string_ptr(p, &prvenc, &prvlen) != 1)
      return 0;
  ...
  if (p != NULL && OSSL_PARAM_get_octet_string_ptr(p, &pubenc, &publen) != 1)
    return 0;
...
void *ml_kem_load(const void *reference, size_t reference_sz) {
  ...
  if (ossl_prov_is_running() && reference_sz == sizeof(key)) {
    ...
    *(ML_KEM_KEY **)reference = NULL;
    ...
    if (ossl_ml_kem_have_seed(key) &&
        (encoded_dk == NULL || (key->prov_flags & ML_KEM_KEY_PREFER_SEED))) {
      if (!ossl_ml_kem_genkey(NULL, 0, key) ||
          (encoded_dk != NULL && !check_prvenc(encoded_dk, key)))
        goto err;
    } else if (encoded_dk != NULL) {
      if (!ossl_ml_kem_parse_private_key(encoded_dk, key->vinfo->prvkey_bytes,
                                         key)) {
        ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_KEY,
...
static int ml_kem_get_params(void *vkey, OSSL_PARAM params[]) {
  ...
  const ML_KEM_VINFO *v = ossl_ml_kem_key_vinfo(key);
  OSSL_PARAM *p;
  const char *pubparams[] = {OSSL_PKEY_PARAM_PUB_KEY,
...
static void *ml_kem_gen_init(void *provctx, int selection,
   ...
   * appropriate.
   */
  if (!ossl_prov_is_running() || (selection & minimal_selection) == 0 ||
...
#ifndef FIPS_MODULE
#define DISPATCH_LOAD_FN {OSSL_FUNC_KEYMGMT_LOAD, (OSSL_FUNC)ml_kem_load},
#else
...
#define DECLARE_VARIANT(bits)                                                  \
  ...
  {OSSL_FUNC_KEYMGMT_GEN_SETTABLE_PARAMS,                                      \
   (OSSL_FUNC)ml_kem_gen_settable_params},                                     \
