...
struct dsa_gen_ctx {
    ...
    int
    gen_type; /* DSA_PARAMGEN_TYPE_FIPS_186_2 or DSA_PARAMGEN_TYPE_FIPS_186_4 */
    int pcounter;
    int hindex;
    char *mdname;
    char *mdprops;
...
static void dsa_freedata(void *keydata) { DSA_free(keydata); }
...
static int dsa_match(const void *keydata1, const void *keydata2,
                     int selection) {
...
static int dsa_export(void *keydata, int selection, OSSL_CALLBACK *param_cb,
    ...
    if ((selection & OSSL_KEYMGMT_SELECT_ALL_PARAMETERS) != 0)
        ok =
        ok && ossl_ffc_params_todata(ossl_dsa_get0_params(dsa), tmpl, NULL);
    if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
        int include_private =
        selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY ? 1 : 0;
...
DSA_IMEXPORTABLE_PARAMETERS, DSA_IMEXPORTABLE_PUBLIC_KEY,
...
static int dsa_gen_set_params(void *genctx, const OSSL_PARAM params[]) {
    ...
    if (p != NULL) {
         ...
         * must be in range:
         * DSA_PARAMGEN_TYPE_FIPS_186_4 <= gen_type <=
         * DSA_PARAMGEN_TYPE_FIPS_DEFAULT
         */
        if (gen_type != -1)
...
static void *dsa_gen(void *genctx, OSSL_CALLBACK *osslcb, void *cbarg) {
     ...
     * DSA_PARAMGEN_TYPE_FIPS_186_4 <= gen_type <=
     * DSA_PARAMGEN_TYPE_FIPS_DEFAULT Noted here as this needs to be adjusted if
     * a new type is added.
     */
    ...
    if ((gctx->selection & OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) != 0) {
        ...
        if (ossl_dsa_generate_ffc_parameters(dsa, gctx->gen_type, gctx->pbits,
                                             gctx->qbits, gencb) <= 0)
            goto end;
    }
