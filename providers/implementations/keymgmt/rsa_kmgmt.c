...
static int pss_params_fromdata(RSA_PSS_PARAMS_30 *pss_params, int *defaults_set,
                               ...
                               OSSL_LIB_CTX *libctx) {
  if (!ossl_rsa_pss_params_30_fromdata(pss_params, defaults_set, params,
                                       libctx))
    return 0;
...
static void rsa_freedata(void *keydata) { RSA_free(keydata); }
...
static int rsa_match(const void *keydata1, const void *keydata2,
                     int selection) {
...
#define RSA_KEY_TYPES()                                                        \
  ...
  OSSL_PARAM_BN(OSSL_PKEY_PARAM_RSA_D, NULL, 0), RSA_KEY_MP_TYPES()
...
static void *rsa_gen(void *genctx, OSSL_CALLBACK *osslcb, void *cbarg) {
  ...
  if (!RSA_generate_multi_prime_key(rsa_tmp, (int)gctx->nbits,
                                    (int)gctx->primes, gctx->pub_exp, gencb))
    goto err;
...
static const char *rsa_query_operation_name(int operation_id) { return "RSA"; }
