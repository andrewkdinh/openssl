...
static void dh_freedata(void *keydata) { DH_free(keydata); }
...
static const OSSL_PARAM *dh_gettable_params(void *provctx) { return dh_params; }
...
static void *dh_gen_init_base(void *provctx, int selection,
  ...
  if ((gctx = OPENSSL_zalloc(sizeof(*gctx))) != NULL) {
    ...
    gctx->mdname = NULL;
#ifdef FIPS_MODULE
    gctx->gen_type = (type == DH_FLAG_TYPE_DHX) ? DH_PARAMGEN_TYPE_FIPS_186_4
                                                : DH_PARAMGEN_TYPE_GROUP;
#else
    gctx->gen_type = (type == DH_FLAG_TYPE_DHX) ? DH_PARAMGEN_TYPE_FIPS_186_2
...
static int dh_gen_common_set_params(void *genctx, const OSSL_PARAM params[]) {
  ...
  if (p != NULL) {
    if (p->data_type != OSSL_PARAM_UTF8_STRING ||
        ((gen_type = dh_gen_type_name2id_w_default(p->data, gctx->dh_type)) ==
         -1)) {
  ...
  if (gctx->gen_type == DH_PARAMGEN_TYPE_GROUP && gctx->ffc_params == NULL) {
    ...
    if ((gctx->selection & OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) != 0) {
      ...
      if (gctx->gen_type == DH_PARAMGEN_TYPE_GENERATOR)
        ret =
        DH_generate_parameters_ex(dh, gctx->pbits, gctx->generator, gencb);
      else
        ret = ossl_dh_generate_ffc_parameters(dh, gctx->gen_type, gctx->pbits,
                                              gctx->qbits, gencb);
      if (ret <= 0)
        goto end;
    }
...
static const char *dhx_query_operation_name(int operation_id) { return "DH"; }
