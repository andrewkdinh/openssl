...
static int slh_dsa_match(const void *keydata1, const void *keydata2,
                         int selection) {
...
static int slh_dsa_validate(const void *key_data, int selection,
                            int check_type) {
...
static int slh_dsa_import(void *keydata, int selection,
...
static const OSSL_PARAM slh_dsa_key_types[] = {SLH_DSA_IMEXPORTABLE_PARAMETERS,
                                               OSSL_PARAM_END};
...
static int key_to_params(SLH_DSA_KEY *key, OSSL_PARAM_BLD *tmpl,
  ...
  if (include_private && ossl_slh_dsa_key_get_priv(key) != NULL)
    return ossl_param_build_set_octet_string(
    tmpl, NULL, OSSL_PKEY_PARAM_PRIV_KEY, ossl_slh_dsa_key_get_priv(key),
    ossl_slh_dsa_key_get_priv_len(key));
  /* Otherwise write out the public key element */
  return ossl_param_build_set_octet_string(tmpl, NULL, OSSL_PKEY_PARAM_PUB_KEY,
...
static int slh_dsa_get_params(void *keydata, OSSL_PARAM params[]) {
  ...
  if (priv != NULL) {
    ...
    if (p != NULL && !OSSL_PARAM_set_octet_string(
                     p, priv, ossl_slh_dsa_key_get_priv_len(key)))
      return 0;
  }
  ...
  if (pub != NULL) {
    ...
    if (p != NULL &&
        !OSSL_PARAM_set_octet_string(p, pub, ossl_slh_dsa_key_get_pub_len(key)))
      return 0;
  }
  /*
...
static int slh_dsa_fips140_pairwise_test(SLH_DSA_HASH_CTX *ctx,
  ...
  if (ossl_slh_dsa_sign(ctx, msg, msg_len, NULL, 0, NULL, 0, sig, &sig_len,
                        sig_len) != 1)
    goto err;
...
static void *slh_dsa_gen(void *genctx, const char *alg) {
  ...
  if (ctx == NULL)
    return NULL;
  if (!ossl_slh_dsa_generate_key(ctx, key, gctx->libctx, gctx->entropy,
                                 gctx->entropy_len))
    goto err;
#ifdef FIPS_MODULE
...
slh_dsa_gen_settable_params(ossl_unused void *genctx,
...
#define MAKE_KEYMGMT_FUNCTIONS(alg, fn)                                        \
  ...
  static void *slh_dsa_##fn##_gen(void *genctx, OSSL_CALLBACK *osslcb,         \
                                  void *cbarg) {                               \
  ...
  {OSSL_FUNC_KEYMGMT_GETTABLE_PARAMS,                                          \
   (void (*)(void))slh_dsa_gettable_params},                                   \
  ...
  {OSSL_FUNC_KEYMGMT_GEN_SET_PARAMS, (void (*)(void))slh_dsa_gen_set_params},  \
