...
static int ml_dsa_pairwise_test(const ML_DSA_KEY *key) {
  ...
  if (!ml_dsa_has(key, OSSL_KEYMGMT_SELECT_KEYPAIR) || ossl_fips_self_testing())
    return 1;
  ...
  if (ossl_ml_dsa_verify(key, 0, msg, sizeof(msg), NULL, 0, 0, sig, sig_len) <=
      0)
    goto err;
...
ML_DSA_KEY *ossl_prov_ml_dsa_new(PROV_CTX *ctx, const char *propq,
                                 int evp_type) {
  ML_DSA_KEY *key;
  ...
  if (key != NULL) {
    ...
    if (ossl_prov_ctx_get_bool_param(ctx, OSSL_PKEY_PARAM_ML_DSA_RETAIN_SEED,
                                     1))
      flags_set |= ML_DSA_KEY_RETAIN_SEED;
    else
      flags_clr = ML_DSA_KEY_RETAIN_SEED;

    if (ossl_prov_ctx_get_bool_param(ctx, OSSL_PKEY_PARAM_ML_DSA_PREFER_SEED,
                                     1))
...
static int ml_dsa_match(const void *keydata1, const void *keydata2,
                        int selection) {
...
static int ml_dsa_validate(const void *key_data, int selection,
                           int check_type) {
...
static int ml_dsa_key_fromdata(ML_DSA_KEY *key, const OSSL_PARAM params[],
  ...
  if (include_private) {
    ...
    if (p != NULL &&
        !OSSL_PARAM_get_octet_string_ptr(p, (const void **)&seed, &seed_len))
      return 0;
  ...
  if (seed_len != 0 && (sk_len == 0 || (ossl_ml_dsa_key_get_prov_flags(key) &
...
static int ml_dsa_import(void *keydata, int selection,
...
static const OSSL_PARAM ml_dsa_key_types[] = {ML_DSA_IMEXPORTABLE_PARAMETERS,
                                              OSSL_PARAM_END};
...
static int ml_dsa_get_params(void *keydata, OSSL_PARAM params[]) {
  ...
  if (priv != NULL &&
      ...
      !OSSL_PARAM_set_octet_string(p, priv, ossl_ml_dsa_key_get_priv_len(key)))
    return 0;
  if (pub != NULL &&
      (p = OSSL_PARAM_locate(params, OSSL_PKEY_PARAM_PUB_KEY)) != NULL &&
      !OSSL_PARAM_set_octet_string(p, pub, ossl_ml_dsa_key_get_pub_len(key)))
    return 0;
  /*
...
static int ml_dsa_export(void *keydata, int selection, OSSL_CALLBACK *param_cb,
  ...
  if (include_private) {
    ...
    if ((buf = ossl_ml_dsa_key_get_priv(key)) != NULL) {
      params[pnum++] = OSSL_PARAM_construct_octet_string(
      OSSL_PKEY_PARAM_PRIV_KEY, (void *)buf, ossl_ml_dsa_key_get_priv_len(key));
    }
  }
  if (pnum == 0 && (buf = ossl_ml_dsa_key_get_pub(key)) != NULL) {
    params[pnum++] = OSSL_PARAM_construct_octet_string(
    OSSL_PKEY_PARAM_PUB_KEY, (void *)buf, ossl_ml_dsa_key_get_pub_len(key));
  }
  if (pnum == 0)
    return 0;
...
static void *ml_dsa_load(const void *reference, size_t reference_sz) {
  ...
  if (ossl_prov_is_running() && reference_sz == sizeof(key)) {
    ...
    *(ML_DSA_KEY **)reference = NULL;
    ...
    seed = ossl_ml_dsa_key_get_seed(key);
    if (seed != NULL && (sk == NULL || (ossl_ml_dsa_key_get_prov_flags(key) &
                                        ML_DSA_KEY_PREFER_SEED))) {
      if (ossl_ml_dsa_generate_key(key))
        return key;
    } else if (sk != NULL) {
      if (ossl_ml_dsa_sk_decode(key, sk, ossl_ml_dsa_key_get_priv_len(key)))
        return key;
      key_params = ossl_ml_dsa_key_params(key);
      ERR_raise_data(ERR_LIB_PROV, PROV_R_INVALID_KEY,
                     "error parsing %s private key", key_params->alg);
    } else {
      return key;
    }
  }
...
static void *ml_dsa_gen(void *genctx, int evp_type) {
  ...
  if (gctx->entropy_len != 0 &&
      !ossl_ml_dsa_set_prekey(key, 0, 0, gctx->entropy, gctx->entropy_len, NULL,
                              0))
    goto err;
  if (!ossl_ml_dsa_generate_key(key)) {
    ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_GENERATE_KEY);
...
#ifndef FIPS_MODULE
#define DISPATCH_LOAD_FN {OSSL_FUNC_KEYMGMT_LOAD, (OSSL_FUNC)ml_dsa_load},
#else
...
#define MAKE_KEYMGMT_FUNCTIONS(alg)                                            \
  ...
  static void *ml_dsa_##alg##_gen(void *genctx, OSSL_CALLBACK *osslcb,         \
                                  void *cbarg) {                               \
  ...
  DISPATCH_LOAD_FN{OSSL_FUNC_KEYMGMT_GET_PARAMS,                               \
                   (void (*)(void))ml_dsa_get_params},                         \
