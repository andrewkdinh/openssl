...
static int ml_dsa_pairwise_test(const ML_DSA_KEY *key) {
    ...
    if (ossl_ml_dsa_verify(key, 0, msg, sizeof(msg), NULL, 0, 0, sig,
                           sig_len) <= 0)
        goto err;
...
ML_DSA_KEY *ossl_prov_ml_dsa_new(PROV_CTX *ctx, const char *propq,
    ...
    if (key != NULL) {
        ...
        if (ossl_prov_ctx_get_bool_param(ctx,
                                         OSSL_PKEY_PARAM_ML_DSA_RETAIN_SEED, 1))
            flags_set |= ML_DSA_KEY_RETAIN_SEED;
        else
            flags_clr = ML_DSA_KEY_RETAIN_SEED;

        if (ossl_prov_ctx_get_bool_param(ctx,
                                         OSSL_PKEY_PARAM_ML_DSA_PREFER_SEED, 1))
...
static int ml_dsa_get_params(void *keydata, OSSL_PARAM params[]) {
    ...
    if ((p = OSSL_PARAM_locate(params, OSSL_PKEY_PARAM_SECURITY_BITS)) !=
        ...
        !OSSL_PARAM_set_int(p,
                            ossl_ml_dsa_key_get_collision_strength_bits(key)))
        return 0;
...
static void *ml_dsa_gen(void *genctx, int evp_type) {
    ...
    if (gctx->entropy_len != 0 &&
        !ossl_ml_dsa_set_prekey(key, 0, 0, gctx->entropy, gctx->entropy_len,
                                NULL, 0))
        goto err;
    if (!ossl_ml_dsa_generate_key(key)) {
        ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_GENERATE_KEY);
...
#define MAKE_KEYMGMT_FUNCTIONS(alg)                                            \
    ...
    {OSSL_FUNC_KEYMGMT_GETTABLE_PARAMS,                                        \
     (void (*)(void))ml_dsa_gettable_params},                                  \
