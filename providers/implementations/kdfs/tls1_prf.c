...
static int fips_ems_check_passed(TLS1_PRF *ctx) {
     ...
     * We do the check this way since the PRF is used for other purposes, as
     * well as "extended master secret".
     */
...
static int fips_digest_check_passed(TLS1_PRF *ctx, const EVP_MD *md) {
    ...
    if (digest_unapproved) {
        if (!OSSL_FIPS_IND_ON_UNAPPROVED(
            ctx, OSSL_FIPS_IND_SETTABLE1, libctx, "TLS_PRF", "Digest",
...
static int kdf_tls1_prf_set_ctx_params(void *vctx, const OSSL_PARAM params[]) {
    ...
    if ((p = OSSL_PARAM_locate_const(params, OSSL_KDF_PARAM_SECRET)) != NULL) {
        OPENSSL_clear_free(ctx->sec, ctx->seclen);
        ctx->sec = NULL;
        if (!OSSL_PARAM_get_octet_string(p, (void **)&ctx->sec, 0,
                                         &ctx->seclen))
            return 0;
...
static int tls1_prf_alg(EVP_MAC_CTX *mdctx, EVP_MAC_CTX *sha1ctx,
    ...
    if (sha1ctx != NULL) {
        ...
        if (!tls1_prf_P_hash(sha1ctx, sec + slen - L_S2, L_S2, seed, seed_len,
                             tmp, olen)) {
