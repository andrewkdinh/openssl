...
static int fips_key_check_passed(TLS1_PRF *ctx) {
  ...
  if (!key_approved) {
    if (!OSSL_FIPS_IND_ON_UNAPPROVED(ctx, OSSL_FIPS_IND_SETTABLE2, libctx,
                                     "TLS_PRF", "Key size",
...
static int kdf_tls1_prf_derive(void *vctx, unsigned char *key, size_t keylen,
  ...
  return tls1_prf_alg(ctx->P_hash, ctx->P_sha1, ctx->sec, ctx->seclen,
                      ctx->seed, ctx->seedlen, key, keylen);
}
  ...
  if ((p = OSSL_PARAM_locate_const(params, OSSL_KDF_PARAM_DIGEST)) != NULL) {
    ...
    if (OPENSSL_strcasecmp(p->data, SN_md5_sha1) == 0) {
      if (!ossl_prov_macctx_load_from_params(
          &ctx->P_hash, params, OSSL_MAC_NAME_HMAC, NULL, SN_md5, libctx) ||
          !ossl_prov_macctx_load_from_params(
          &ctx->P_sha1, params, OSSL_MAC_NAME_HMAC, NULL, SN_sha1, libctx))
        return 0;
    } else {
      EVP_MAC_CTX_free(ctx->P_sha1);
      if (!ossl_prov_macctx_load_from_params(
          &ctx->P_hash, params, OSSL_MAC_NAME_HMAC, NULL, NULL, libctx))
        return 0;
    }
...
kdf_tls1_prf_settable_ctx_params(ossl_unused void *ctx,
...
static const OSSL_PARAM *
kdf_tls1_prf_gettable_ctx_params(ossl_unused void *ctx,
                                 ossl_unused void *provctx) {
 ...
 *     A(i) = HMAC_<hash>(secret, A(i-1))
 */
static int tls1_prf_P_hash(EVP_MAC_CTX *ctx_init, const unsigned char *sec,
...
static int tls1_prf_alg(EVP_MAC_CTX *mdctx, EVP_MAC_CTX *sha1ctx,
  ...
  if (sha1ctx != NULL) {
    ...
    if (!tls1_prf_P_hash(sha1ctx, sec + slen - L_S2, L_S2, seed, seed_len, tmp,
                         olen)) {
