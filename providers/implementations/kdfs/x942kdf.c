...
#ifndef FIPS_MODULE
{"DES3-WRAP", ossl_der_oid_id_alg_CMS3DESwrap, DER_OID_SZ_id_alg_CMS3DESwrap,
 24},
#endif
};
...
static int DER_w_keyinfo(WPACKET *pkt, const unsigned char *der_oid,
                         size_t der_oidlen, unsigned char **pcounter) {
...
static int der_encode_sharedinfo(
WPACKET *pkt, unsigned char *buf, size_t buflen, const unsigned char *der_oid,
...
unsigned char **pcounter) {
    return (buf != NULL ? WPACKET_init_der(pkt, buf, buflen)
           ...
           ossl_DER_w_end_sequence(pkt, -1) && WPACKET_finish(pkt);
}
 ...
 */
static int x942_encode_otherinfo(
size_t keylen, const unsigned char *cek_oid, size_t cek_oid_len,
...
const unsigned char *supp_priv, size_t supp_priv_len, unsigned char **der,
size_t *der_len, unsigned char **out_ctr) {
    int ret = 0;
    ...
    if (!der_encode_sharedinfo(&pkt, NULL, 0, cek_oid, cek_oid_len, acvp,
                               acvp_len, partyu, partyu_len, partyv, partyv_len,
    ...
    if (!der_encode_sharedinfo(&pkt, der_buf, der_buflen, cek_oid, cek_oid_len,
                               acvp, acvp_len, partyu, partyu_len, partyv,
                               partyv_len, supp_pub, supp_pub_len, supp_priv,
                               supp_priv_len, keylen_bits, &pcounter))
        goto err;
    /*
     * Since we allocated the exact size required, the buffer should point to
     * the start of the allocated buffer at this point.
     */
...
static int x942kdf_hash_kdm(const EVP_MD *kdf_md, const unsigned char *z,
                            size_t z_len, const unsigned char *other,
                            size_t other_len, unsigned char *ctr,
    ...
    if (z_len > X942KDF_MAX_INLEN || other_len > X942KDF_MAX_INLEN ||
        derived_key_len > X942KDF_MAX_INLEN || derived_key_len == 0) {
...
static void *x942kdf_dup(void *vctx) {
    ...
    if (dest != NULL) {
        if (!ossl_prov_memdup(src->secret, src->secret_len, &dest->secret,
                              &dest->secret_len) ||
            !ossl_prov_memdup(src->acvpinfo, src->acvpinfo_len, &dest->acvpinfo,
                              &dest->acvpinfo_len) ||
            ...
            !ossl_prov_memdup(src->supp_pubinfo, src->supp_pubinfo_len,
                              &dest->supp_pubinfo, &dest->supp_pubinfo_len) ||
            !ossl_prov_memdup(src->supp_privinfo, src->supp_privinfo_len,
                              &dest->supp_privinfo, &dest->supp_privinfo_len) ||
...
static int x942kdf_derive(void *vctx, unsigned char *key, size_t keylen,
    ...
    if (ctx->acvpinfo != NULL &&
        (ctx->partyuinfo != NULL || ctx->partyvinfo != NULL ||
         ctx->supp_pubinfo != NULL || ctx->supp_privinfo != NULL)) {
    ...
    if (!x942_encode_otherinfo(
        ctx->use_keybits ? ctx->dkm_len : 0, ctx->cek_oid, ctx->cek_oid_len,
        ctx->acvpinfo, ctx->acvpinfo_len, ctx->partyuinfo, ctx->partyuinfo_len,
        ctx->partyvinfo, ctx->partyvinfo_len, ctx->supp_pubinfo,
        ctx->supp_pubinfo_len, ctx->supp_privinfo, ctx->supp_privinfo_len, &der,
        &der_len, &ctr)) {
        ERR_raise(ERR_LIB_PROV, PROV_R_BAD_ENCODING);
        return 0;
    }
    ret = x942kdf_hash_kdm(md, ctx->secret, ctx->secret_len, der, der_len, ctr,
                           key, keylen);
    OPENSSL_free(der);
    return ret;
}
...
static int x942kdf_set_ctx_params(void *vctx, const OSSL_PARAM params[]) {
    ...
    if (p != NULL) {
        ...
        /*
         * We already grab the properties during
         * ossl_prov_digest_load_from_params() so there is no need to check the
         * validity again..
         */
        if (pq != NULL)
            propq = p->data;
...
x942kdf_settable_ctx_params(ossl_unused void *ctx, ossl_unused void *provctx) {
...
static const OSSL_PARAM *
x942kdf_gettable_ctx_params(ossl_unused void *ctx, ossl_unused void *provctx) {
