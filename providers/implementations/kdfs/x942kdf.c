...
static int der_encode_sharedinfo(WPACKET             *pkt,
                                 unsigned char       *buf,
                                 size_t               buflen,
                                 const unsigned char *der_oid,
                                 size_t               der_oidlen,
                                 const unsigned char *acvp,
                                 size_t               acvplen,
                                 const unsigned char *partyu,
                                 size_t               partyulen,
                                 const unsigned char *partyv,
                                 size_t               partyvlen,
                                 ...
                                 unsigned char      **pcounter)
{
    return (buf != NULL ? WPACKET_init_der(pkt, buf, buflen) : WPACKET_init_null_der(pkt))
           ...
           && DER_w_keyinfo(pkt, der_oid, der_oidlen, pcounter) && ossl_DER_w_end_sequence(pkt, -1)
           && WPACKET_finish(pkt);
}
...
static void *x942kdf_dup(void *vctx)
{
    ...
    if (dest != NULL) {
        if (!ossl_prov_memdup(src->secret, src->secret_len, &dest->secret, &dest->secret_len)
            ...
            || !ossl_prov_memdup(src->supp_privinfo,
