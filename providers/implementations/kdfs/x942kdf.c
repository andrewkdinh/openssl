...
static int DER_w_keyinfo(WPACKET *pkt, const unsigned char *der_oid, size_t der_oidlen, unsigned char **pcounter)
{
    return ossl_DER_w_begin_sequence(pkt, -1)
        ...
        && ossl_DER_w_precompiled(pkt, -1, der_oid, der_oidlen) && ossl_DER_w_end_sequence(pkt, -1);
}

static int der_encode_sharedinfo(WPACKET *pkt, unsigned char *buf, size_t buflen, const unsigned char *der_oid,
                                 size_t der_oidlen, const unsigned char *acvp, size_t acvplen,
                                 const unsigned char *partyu, size_t partyulen, const unsigned char *partyv,
                                 size_t partyvlen, const unsigned char *supp_pub, size_t supp_publen,
                                 const unsigned char *supp_priv, size_t supp_privlen, uint32_t keylen_bits,
                                 unsigned char **pcounter)
{
    return (buf != NULL ? WPACKET_init_der(pkt, buf, buflen) : WPACKET_init_null_der(pkt))
        ...
        && (keylen_bits == 0 || ossl_DER_w_octet_string_uint32(pkt, 2, keylen_bits))
 ...
 */
static int x942_encode_otherinfo(size_t keylen, const unsigned char *cek_oid, size_t cek_oid_len,
                                 const unsigned char *acvp, size_t acvp_len, const unsigned char *partyu,
                                 size_t partyu_len, const unsigned char *partyv, size_t partyv_len,
                                 const unsigned char *supp_pub, size_t supp_pub_len, const unsigned char *supp_priv,
                                 size_t supp_priv_len, unsigned char **der, size_t *der_len, unsigned char **out_ctr)
{
    int ret = 0;
    ...
    if (!der_encode_sharedinfo(&pkt, NULL, 0, cek_oid, cek_oid_len, acvp, acvp_len, partyu, partyu_len, partyv,
                               partyv_len, supp_pub, supp_pub_len, supp_priv, supp_priv_len, keylen_bits, NULL)
    ...
    if (!der_encode_sharedinfo(&pkt, der_buf, der_buflen, cek_oid, cek_oid_len, acvp, acvp_len, partyu, partyu_len,
                               partyv, partyv_len, supp_pub, supp_pub_len, supp_priv, supp_priv_len, keylen_bits,
                               &pcounter))
        goto err;
    /*
...
static int x942kdf_hash_kdm(const EVP_MD *kdf_md, const unsigned char *z, size_t z_len, const unsigned char *other,
                            size_t other_len, unsigned char *ctr, unsigned char *derived_key, size_t derived_key_len)
{
    int ret = 0, hlen;
    ...
    if (z_len > X942KDF_MAX_INLEN || other_len > X942KDF_MAX_INLEN || derived_key_len > X942KDF_MAX_INLEN
        || derived_key_len == 0) {
    ...
    for (counter = 1;; counter++) {
        ...
        if (!EVP_MD_CTX_copy_ex(ctx, ctx_init) || !EVP_DigestUpdate(ctx, z, z_len)
...
static void *x942kdf_dup(void *vctx)
{
    ...
    if (dest != NULL) {
        if (!ossl_prov_memdup(src->secret, src->secret_len, &dest->secret, &dest->secret_len)
            || !ossl_prov_memdup(src->acvpinfo, src->acvpinfo_len, &dest->acvpinfo, &dest->acvpinfo_len)
            || !ossl_prov_memdup(src->partyuinfo, src->partyuinfo_len, &dest->partyuinfo, &dest->partyuinfo_len)
            || !ossl_prov_memdup(src->partyvinfo, src->partyvinfo_len, &dest->partyvinfo, &dest->partyvinfo_len)
            || !ossl_prov_memdup(src->supp_pubinfo, src->supp_pubinfo_len, &dest->supp_pubinfo, &dest->supp_pubinfo_len)
            || !ossl_prov_memdup(src->supp_privinfo, src->supp_privinfo_len, &dest->supp_privinfo,
...
static int fips_x942kdf_key_check_passed(KDF_X942 *ctx)
{
    ...
    if (!key_approved) {
        if (!OSSL_FIPS_IND_ON_UNAPPROVED(ctx, OSSL_FIPS_IND_SETTABLE0, libctx, "X942KDF", "Key size",
...
static int x942kdf_derive(void *vctx, unsigned char *key, size_t keylen, const OSSL_PARAM params[])
{
    ...
    if (ctx->acvpinfo != NULL
        && (ctx->partyuinfo != NULL || ctx->partyvinfo != NULL || ctx->supp_pubinfo != NULL
    ...
    if (!x942_encode_otherinfo(ctx->use_keybits ? ctx->dkm_len : 0, ctx->cek_oid, ctx->cek_oid_len, ctx->acvpinfo,
                               ctx->acvpinfo_len, ctx->partyuinfo, ctx->partyuinfo_len, ctx->partyvinfo,
                               ctx->partyvinfo_len, ctx->supp_pubinfo, ctx->supp_pubinfo_len, ctx->supp_privinfo,
                               ctx->supp_privinfo_len, &der, &der_len, &ctr)) {
...
const OSSL_DISPATCH ossl_kdf_x942_kdf_functions[] = {
    ...
    {OSSL_FUNC_KDF_SETTABLE_CTX_PARAMS, (void (*)(void))x942kdf_settable_ctx_params},
