...
static void *krb5kdf_dup(void *vctx)
{
    ...
    if (dest != NULL) {
        if (!ossl_prov_memdup(src->key, src->key_len, &dest->key, &dest->key_len)
            || !ossl_prov_memdup(src->constant, src->constant_len, &dest->constant, &dest->constant_len)
 ...
 */
static void
n_fold(unsigned char *block, unsigned int blocksize, const unsigned char *constant, unsigned int constant_len)
{
...
static int
cipher_init(EVP_CIPHER_CTX *ctx, const EVP_CIPHER *cipher, ENGINE *engine, const unsigned char *key, size_t key_len)
{
    int klen, ret;
                   ...
                   size_t               okey_len)
{
    ...
    for (osize = 0; osize < okey_len; osize += cipherlen) {
        ...
        ret = EVP_EncryptUpdate(ctx, cipherblock, &olen, plainblock, (int)blocksize);
        if (!ret)
            goto out;
        cipherlen = olen;
