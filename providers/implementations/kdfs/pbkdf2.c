...
static void kdf_pbkdf2_init(KDF_PBKDF2 *ctx) {
  ...
  params[0] =
  OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_DIGEST, SN_sha1, 0);
...
static int fips_lower_bound_check_passed(KDF_PBKDF2 *ctx, size_t keylen) {
  ...
  if (!approved) {
    if (!OSSL_FIPS_IND_ON_UNAPPROVED(
        ctx, OSSL_FIPS_IND_SETTABLE0, libctx, "PBKDF2", desc,
...
static int kdf_pbkdf2_derive(void *vctx, unsigned char *key, size_t keylen,
  ...
  return pbkdf2_derive(ctx, (char *)ctx->pass, ctx->pass_len, ctx->salt,
                       ctx->salt_len, ctx->iter, md, key, keylen,
...
#ifdef FIPS_MODULE
    ossl_FIPS_IND_set_settable(OSSL_FIPS_IND_GET(ctx), OSSL_FIPS_IND_SETTABLE0,
                               ctx->lower_bound_checks);
#endif
  }
...
kdf_pbkdf2_settable_ctx_params(ossl_unused void *ctx, ossl_unused void *p_ctx) {
...
static const OSSL_PARAM *
kdf_pbkdf2_gettable_ctx_params(ossl_unused void *ctx, ossl_unused void *p_ctx) {
...
static int pbkdf2_derive(KDF_PBKDF2 *ctx, const char *pass, size_t passlen,
  ...
  if (lower_bound_checks) {
    int error = 0;
    int passed =
    pbkdf2_lower_bound_check_passed(saltlen, iter, keylen, &error, NULL);
