...
static int sskdf_derive(void *vctx, unsigned char *key, size_t keylen,
    ...
    if (ctx->macctx != NULL) {
        ...
        ret = SSKDF_mac_kdm(
        ctx->macctx, custom, custom_len, ctx->out_len, ctx->salt, ctx->salt_len,
        ctx->secret, ctx->secret_len, ctx->info, ctx->info_len, key, keylen);
        return ret;
    } else {
        /* H(x) = hash */
        if (md == NULL) {
...
static int fips_x963kdf_digest_check_passed(KDF_SSKDF *ctx, const EVP_MD *md) {
    ...
    if (digest_unapproved) {
        if (!OSSL_FIPS_IND_ON_UNAPPROVED(
            ctx, OSSL_FIPS_IND_SETTABLE0, libctx, "X963KDF", "Digest",
...
static int sskdf_common_set_ctx_params(KDF_SSKDF *ctx,
    ...
    if (!ossl_prov_macctx_load_from_params(&ctx->macctx, params, NULL, NULL,
                                           NULL, libctx))
        return 0;
    if (ctx->macctx != NULL) {
