...
#define SSKDF_COMMON_GETTABLES OSSL_PARAM_size_t(OSSL_KDF_PARAM_SIZE, NULL)
 ...
 */
static int SSKDF_hash_kdm(const EVP_MD *kdf_md, const unsigned char *z,
                          size_t z_len, const unsigned char *info,
                          size_t info_len, unsigned int append_ctr,
...
static int kmac_init(EVP_MAC_CTX *ctx, const unsigned char *custom,
  ...
  else if (!(kmac_out_len == derived_key_len || kmac_out_len == 20 ||
             kmac_out_len == 28 || kmac_out_len == 32 || kmac_out_len == 48 ||
             kmac_out_len == 64))
    return 0;
...
static int SSKDF_mac_kdm(EVP_MAC_CTX *ctx_init,
  ...
  for (counter = 1;; counter++) {
    ...
    if (!(ctx != NULL && EVP_MAC_update(ctx, c, sizeof(c)) &&
          EVP_MAC_update(ctx, z, z_len) && EVP_MAC_update(ctx, info, info_len)))
      goto end;
    if (len >= out_len) {
...
static void *sskdf_dup(void *vctx) {
  ...
  if (dest != NULL) {
    ...
    if (!ossl_prov_memdup(src->info, src->info_len, &dest->info,
                          &dest->info_len) ||
        !ossl_prov_memdup(src->salt, src->salt_len, &dest->salt,
                          &dest->salt_len) ||
        !ossl_prov_memdup(src->secret, src->secret_len, &dest->secret,
                          &dest->secret_len) ||
...
static int sskdf_derive(void *vctx, unsigned char *key, size_t keylen,
  ...
  if (ctx->macctx != NULL) {
    ...
    ret = SSKDF_mac_kdm(ctx->macctx, custom, custom_len, ctx->out_len,
                        ctx->salt, ctx->salt_len, ctx->secret, ctx->secret_len,
    ...
    return SSKDF_hash_kdm(md, ctx->secret, ctx->secret_len, ctx->info,
                          ctx->info_len, 0, key, keylen);
  }
}
...
static int x963kdf_derive(void *vctx, unsigned char *key, size_t keylen,
  ...
  return SSKDF_hash_kdm(md, ctx->secret, ctx->secret_len, ctx->info,
                        ctx->info_len, 1, key, keylen);
}

static int sskdf_common_set_ctx_params(KDF_SSKDF *ctx,
                                       const OSSL_PARAM params[]) {
  const OSSL_PARAM *p;
  OSSL_LIB_CTX *libctx = PROV_LIBCTX_OF(ctx->provctx);
  const EVP_MD *md = NULL;
  ...
  if (!ossl_prov_macctx_load_from_params(&ctx->macctx, params, NULL, NULL, NULL,
                                         libctx))
    return 0;
  if (ctx->macctx != NULL) {
  ...
  r = ossl_param_get1_octet_string(params, OSSL_KDF_PARAM_SECRET, &ctx->secret,
                                   &ctx->secret_len);
  if (r == -1)
    r = ossl_param_get1_octet_string(params, OSSL_KDF_PARAM_KEY, &ctx->secret,
                                     &ctx->secret_len);
  if (r == 0)
    return 0;
  ...
  if (ossl_param_get1_octet_string(params, OSSL_KDF_PARAM_SALT, &ctx->salt,
                                   &ctx->salt_len) == 0)
    return 0;
...
static const OSSL_PARAM *sskdf_settable_ctx_params(ossl_unused void *ctx,
  ...
  SSKDF_COMMON_SETTABLES, OSSL_FIPS_IND_SETTABLE_CTX_PARAM(
                          OSSL_KDF_PARAM_FIPS_KEY_CHECK) OSSL_PARAM_END};
...
static const OSSL_PARAM *sskdf_gettable_ctx_params(ossl_unused void *ctx,
  ...
  SSKDF_COMMON_GETTABLES, OSSL_FIPS_IND_GETTABLE_CTX_PARAM() OSSL_PARAM_END};
...
x963kdf_settable_ctx_params(ossl_unused void *ctx, ossl_unused void *provctx) {
  static const OSSL_PARAM known_settable_ctx_params[] = {
...
static const OSSL_PARAM *
x963kdf_gettable_ctx_params(ossl_unused void *ctx, ossl_unused void *provctx) {
  static const OSSL_PARAM known_gettable_ctx_params[] = {
  SSKDF_COMMON_GETTABLES, OSSL_FIPS_IND_GETTABLE_CTX_PARAM() OSSL_PARAM_END};
  return known_gettable_ctx_params;
}
