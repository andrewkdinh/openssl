...
static int kdf_hkdf_derive(void *vctx, unsigned char *key, size_t keylen,
  ...
  default:
    return HKDF(libctx, md, ctx->salt, ctx->salt_len, ctx->key, ctx->key_len,
...
static int fips_tls1_3_key_check_passed(KDF_HKDF *ctx) {
  ...
  if (!key_approved) {
    if (!OSSL_FIPS_IND_ON_UNAPPROVED(ctx, OSSL_FIPS_IND_SETTABLE1, libctx,
                                     "TLS13 KDF", "Key size",
...
static int kdf_tls1_3_derive(void *vctx, unsigned char *key, size_t keylen,
  ...
  case EVP_KDF_HKDF_MODE_EXTRACT_ONLY:
    ...
    ctx->key_len, ctx->prefix, ctx->prefix_len, ctx->label, ctx->label_len, key,
    keylen);

  case EVP_KDF_HKDF_MODE_EXPAND_ONLY:
    return prov_tls13_hkdf_expand(md, ctx->key, ctx->key_len, ctx->prefix,
                                  ctx->prefix_len, ctx->label, ctx->label_len,
                                  ctx->data, ctx->data_len, key, keylen);
  }
}
