...
static int HKDF_Expand(const EVP_MD *evp_md, const unsigned char *prk,
...
static void *kdf_hkdf_dup(void *vctx) {
    ...
    if (dest != NULL) {
        if (!ossl_prov_memdup(src->salt, src->salt_len, &dest->salt,
                              &dest->salt_len) ||
            !ossl_prov_memdup(src->key, src->key_len, &dest->key,
                              &dest->key_len) ||
            !ossl_prov_memdup(src->prefix, src->prefix_len, &dest->prefix,
                              &dest->prefix_len) ||
            !ossl_prov_memdup(src->label, src->label_len, &dest->label,
                              &dest->label_len) ||
            !ossl_prov_memdup(src->data, src->data_len, &dest->data,
                              &dest->data_len) ||
            !ossl_prov_memdup(src->info, src->info_len, &dest->info,
                              &dest->info_len) ||
...
static int kdf_hkdf_derive(void *vctx, unsigned char *key, size_t keylen,
    ...
    default:
        return HKDF(libctx, md, ctx->salt, ctx->salt_len, ctx->key,
                    ctx->key_len, ctx->info, ctx->info_len, key, keylen);

    case EVP_KDF_HKDF_MODE_EXTRACT_ONLY:
        return HKDF_Extract(libctx, md, ctx->salt, ctx->salt_len, ctx->key,
                            ctx->key_len, key, keylen);

    case EVP_KDF_HKDF_MODE_EXPAND_ONLY:
        return HKDF_Expand(md, ctx->key, ctx->key_len, ctx->info, ctx->info_len,
                           key, keylen);
    }
}

static int hkdf_common_set_ctx_params(KDF_HKDF *ctx,
                                      const OSSL_PARAM params[]) {
    OSSL_LIB_CTX *libctx = PROV_LIBCTX_OF(ctx->provctx);
    const OSSL_PARAM *p;
    int n;
...
kdf_hkdf_settable_ctx_params(ossl_unused void *ctx, ossl_unused void *provctx) {
...
static int hkdf_common_get_ctx_params(KDF_HKDF *ctx, OSSL_PARAM params[]) {
    ...
    /* OSSL_KDF_PARAM_MODE has multiple parameter types, so look for all
     * instances
     */
    p = params;
...
static const OSSL_PARAM *
kdf_hkdf_gettable_ctx_params(ossl_unused void *ctx, ossl_unused void *provctx) {
    static const OSSL_PARAM known_gettable_ctx_params[] = {
    HKDF_COMMON_GETTABLES, OSSL_FIPS_IND_GETTABLE_CTX_PARAM() OSSL_PARAM_END};
...
static int HKDF(OSSL_LIB_CTX *libctx, const EVP_MD *evp_md,
    ...
    if (!HKDF_Extract(libctx, evp_md, salt, salt_len, ikm, ikm_len, prk,
                      prk_len))
        return 0;
...
static int HKDF_Extract(OSSL_LIB_CTX *libctx, const EVP_MD *evp_md,
    ...
    return EVP_Q_mac(libctx, "HMAC", NULL, EVP_MD_get0_name(evp_md), NULL, salt,
                     salt_len, ikm, ikm_len, prk, EVP_MD_get_size(evp_md),
                     NULL) != NULL;
}
...
static int HKDF_Expand(const EVP_MD *evp_md, const unsigned char *prk,
    ...
    for (i = 1; i <= n; i++) {
        ...
        copy_len =
        (dig_len > okm_len - done_len) ? okm_len - done_len : dig_len;
 ...
 */
static int prov_tls13_hkdf_expand(const EVP_MD *md, const unsigned char *key,
                                  size_t keylen, const unsigned char *prefix,
                                  size_t prefixlen, const unsigned char *label,
...
OSSL_LIB_CTX *libctx, const EVP_MD *md, const unsigned char *prevsecret,
...
size_t labellen, unsigned char *out, size_t outlen) {
    ...
    if (prevsecret == NULL) {
        ...
        if (!prov_tls13_hkdf_expand(md, prevsecret, prevsecretlen, prefix,
                                    prefixlen, label, labellen, hash, mdlen,
                                    preextractsec, mdlen))
            return 0;
    ...
    ret = HKDF_Extract(libctx, md, prevsecret, prevsecretlen, insecret,
                       insecretlen, out, outlen);
...
static int fips_tls1_3_key_check_passed(KDF_HKDF *ctx) {
    ...
    if (!key_approved) {
        if (!OSSL_FIPS_IND_ON_UNAPPROVED(
            ctx, OSSL_FIPS_IND_SETTABLE1, libctx, "TLS13 KDF", "Key size",
...
static int kdf_tls1_3_derive(void *vctx, unsigned char *key, size_t keylen,
    ...
    case EVP_KDF_HKDF_MODE_EXTRACT_ONLY:
        ...
        PROV_LIBCTX_OF(ctx->provctx), md, ctx->salt, ctx->salt_len, ctx->key,
        ctx->key_len, ctx->prefix, ctx->prefix_len, ctx->label, ctx->label_len,
        key, keylen);

    case EVP_KDF_HKDF_MODE_EXPAND_ONLY:
        return prov_tls13_hkdf_expand(
        md, ctx->key, ctx->key_len, ctx->prefix, ctx->prefix_len, ctx->label,
...
static const OSSL_PARAM *
kdf_tls1_3_settable_ctx_params(ossl_unused void *ctx,
...
static const OSSL_PARAM *
kdf_tls1_3_gettable_ctx_params(ossl_unused void *ctx,
                               ossl_unused void *provctx) {
    static const OSSL_PARAM known_gettable_ctx_params[] = {
    HKDF_COMMON_GETTABLES, OSSL_FIPS_IND_GETTABLE_CTX_PARAM() OSSL_PARAM_END};
    return known_gettable_ctx_params;
