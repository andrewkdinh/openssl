...
static void *kbkdf_dup(void *vctx) {
    ...
    if (dest != NULL) {
        ...
        if (dest->ctx_init == NULL ||
            !ossl_prov_memdup(src->ki, src->ki_len, &dest->ki, &dest->ki_len) ||
            !ossl_prov_memdup(src->label, src->label_len, &dest->label,
                              &dest->label_len) ||
            !ossl_prov_memdup(src->context, src->context_len, &dest->context,
                              &dest->context_len) ||
...
static int kbkdf_derive(void *vctx, unsigned char *key, size_t keylen,
    ...
    if (ctx->is_kmac) {
        ret =
        kmac_derive(ctx->ctx_init, key, keylen, ctx->context, ctx->context_len);
        goto done;
    }
...
static int kbkdf_set_ctx_params(void *vctx, const OSSL_PARAM params[]) {
    ...
    if (!ossl_prov_macctx_load_from_params(&ctx->ctx_init, params, NULL, NULL,
                                           NULL, libctx))
        return 0;
    if (ctx->ctx_init != NULL) {
        ctx->is_kmac = 0;
    ...
    if (p != NULL) {
        if (ossl_param_get1_octet_string(p, OSSL_KDF_PARAM_KEY, &ctx->ki,
                                         &ctx->ki_len) == 0)
            return 0;
#ifdef FIPS_MODULE
    ...
    if (ossl_param_get1_octet_string(params, OSSL_KDF_PARAM_SALT, &ctx->label,
                                     &ctx->label_len) == 0)
        return 0;

    if (ossl_param_get1_concat_octet_string(
        params, OSSL_KDF_PARAM_INFO, &ctx->context, &ctx->context_len, 0) == 0)
        return 0;

    if (ossl_param_get1_octet_string(params, OSSL_KDF_PARAM_SEED, &ctx->iv,
                                     &ctx->iv_len) == 0)
        return 0;
    ...
    if (ctx->ctx_init != NULL && ctx->ki_len != 0) {
        if ((ctx->is_kmac &&
             !kmac_init(ctx->ctx_init, ctx->label, ctx->label_len)) ||
