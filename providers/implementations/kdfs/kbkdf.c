...
static void *kbkdf_dup(void *vctx)
{
    ...
    if (dest != NULL) {
        ...
        if (dest->ctx_init == NULL || !ossl_prov_memdup(src->ki, src->ki_len, &dest->ki, &dest->ki_len)
            || !ossl_prov_memdup(src->label, src->label_len, &dest->label, &dest->label_len)
            || !ossl_prov_memdup(src->context, src->context_len, &dest->context, &dest->context_len)
            || !ossl_prov_memdup(src->iv, src->iv_len, &dest->iv, &dest->iv_len))
            goto err;
        dest->mode = src->mode;
        dest->r = src->r;
...
static int fips_kbkdf_key_check_passed(KBKDF *ctx)
{
    ...
    if (!key_approved) {
        if (!OSSL_FIPS_IND_ON_UNAPPROVED(ctx, OSSL_FIPS_IND_SETTABLE0, libctx, "KBKDF", "Key size",
...
static int derive(EVP_MAC_CTX *ctx_init, kbkdf_mode mode, unsigned char *iv, size_t iv_len, unsigned char *label,
                  size_t label_len, unsigned char *context, size_t context_len, unsigned char *k_i, size_t h,
                  uint32_t l, int has_separator, unsigned char *ko, size_t ko_len, int r)
{
    int ret = 0;
    ...
    for (counter = 1; written < ko_len; counter++) {
        ...
        if (!EVP_MAC_update(ctx, 4 - (r / 8) + (unsigned char *)&i, r / 8) || !EVP_MAC_update(ctx, label, label_len)
            || (has_separator && !EVP_MAC_update(ctx, &zero, 1)) || !EVP_MAC_update(ctx, context, context_len)
            || (has_l && !EVP_MAC_update(ctx, (unsigned char *)&l, 4)) || !EVP_MAC_final(ctx, k_i, NULL, h))
            goto done;
...
static int kmac_init(EVP_MAC_CTX *ctx, const unsigned char *custom, size_t customlen)
{
    ...
    if (custom == NULL || customlen == 0)
        return 1;
    params[0] = OSSL_PARAM_construct_octet_string(OSSL_MAC_PARAM_CUSTOM, (void *)custom, customlen);
    params[1] = OSSL_PARAM_construct_end();
    return EVP_MAC_CTX_set_params(ctx, params) > 0;
}

static int kmac_derive(EVP_MAC_CTX *ctx, unsigned char *out, size_t outlen, const unsigned char *context,
                       size_t contextlen)
{
    OSSL_PARAM params[2];
    ...
    params[1] = OSSL_PARAM_construct_end();
    return EVP_MAC_CTX_set_params(ctx, params) > 0 && EVP_MAC_update(ctx, context, contextlen)
...
static int kbkdf_derive(void *vctx, unsigned char *key, size_t keylen, const OSSL_PARAM params[])
{
    ...
    if (ctx->is_kmac) {
        ret = kmac_derive(ctx->ctx_init, key, keylen, ctx->context, ctx->context_len);
        goto done;
    }
    ...
    ret = derive(ctx->ctx_init, ctx->mode, ctx->iv, ctx->iv_len, ctx->label, ctx->label_len, ctx->context,
                 ctx->context_len, k_i, h, l, ctx->use_separator, key, keylen, ctx->r);
done:
    if (ret != 1)
...
static int kbkdf_set_ctx_params(void *vctx, const OSSL_PARAM params[])
{
    ...
    if (ctx->ctx_init != NULL) {
        ctx->is_kmac = 0;
        if (EVP_MAC_is_a(EVP_MAC_CTX_get0_mac(ctx->ctx_init), OSSL_MAC_NAME_KMAC128)
            || EVP_MAC_is_a(EVP_MAC_CTX_get0_mac(ctx->ctx_init), OSSL_MAC_NAME_KMAC256)) {
            ctx->is_kmac = 1;
        } else if (!EVP_MAC_is_a(EVP_MAC_CTX_get0_mac(ctx->ctx_init), OSSL_MAC_NAME_HMAC)
    ...
    if (p != NULL) {
        if (ossl_param_get1_octet_string(p, OSSL_KDF_PARAM_KEY, &ctx->ki, &ctx->ki_len) == 0)
            return 0;
#ifdef FIPS_MODULE
    ...
    if (ossl_param_get1_octet_string(params, OSSL_KDF_PARAM_SALT, &ctx->label, &ctx->label_len) == 0)
        return 0;

    if (ossl_param_get1_concat_octet_string(params, OSSL_KDF_PARAM_INFO, &ctx->context, &ctx->context_len, 0) == 0)
        return 0;

    if (ossl_param_get1_octet_string(params, OSSL_KDF_PARAM_SEED, &ctx->iv, &ctx->iv_len) == 0)
        return 0;
