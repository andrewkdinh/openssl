...
static int                                  scrypt_alg(const char          *pass,
                                                       size_t               passlen,
...
static int kdf_scrypt_derive(void *vctx, unsigned char *key, size_t keylen, const OSSL_PARAM params[])
{
    ...
    return scrypt_alg((char *)ctx->pass,
                      ctx->pass_len,
                      ctx->salt,
                      ctx->salt_len,
                      ctx->N,
                      ...
                      const char          *propq)
{
    ...
    T = X + 32 * r;
    V = T + 32 * r;
    if (ossl_pkcs5_pbkdf2_hmac_ex(pass, (int)passlen, salt, (int)saltlen, 1, sha256, (int)Blen, B, libctx, propq) == 0)
        goto err;

    for (i = 0; i < p; i++)
        scryptROMix(B + 128 * r * i, r, N, X, T, V);

    if (ossl_pkcs5_pbkdf2_hmac_ex(pass, (int)passlen, B, (int)Blen, 1, sha256, (int)keylen, key, libctx, propq) == 0)
        goto err;
    rv = 1;
err:
    if (rv == 0)
