...
static int pkcs12kdf_derive(const unsigned char *pass, size_t passlen,
  ...
  for (;;) {
    ...
    for (iter_cnt = 1; iter_cnt < iter; iter_cnt++) {
      if (!EVP_DigestInit_ex(ctx, md_type, NULL) ||
          !EVP_DigestUpdate(ctx, Ai, u) || !EVP_DigestFinal_ex(ctx, Ai, NULL))
        goto end;
    }
...
kdf_pkcs12_settable_ctx_params(ossl_unused void *ctx,
...
static const OSSL_PARAM *
kdf_pkcs12_gettable_ctx_params(ossl_unused void *ctx,
                               ossl_unused void *provctx) {
