  ...
  if (!kctx->internal) {
    ...
    if (!approved) {
      if (!OSSL_FIPS_IND_ON_UNAPPROVED(
          kctx, OSSL_FIPS_IND_SETTABLE1, PROV_LIBCTX_OF(kctx->provctx), "KMAC",
...
static int kmac_init(void *vmacctx, const unsigned char *key, size_t keylen,
  ...
  if (kctx->custom_len == 0) {
    ...
    OSSL_PARAM_octet_string(OSSL_MAC_PARAM_CUSTOM, "", 0), OSSL_PARAM_END};
...
static int kmac_set_ctx_params(void *vmacctx, const OSSL_PARAM *params) {
  ...
  if ((p = OSSL_PARAM_locate_const(params, OSSL_MAC_PARAM_SIZE)) != NULL) {
    ...
    if (sz < 32 / 8) {
      if (!OSSL_FIPS_IND_ON_UNAPPROVED(
          kctx, OSSL_FIPS_IND_SETTABLE0, PROV_LIBCTX_OF(kctx->provctx), "KMAC",
