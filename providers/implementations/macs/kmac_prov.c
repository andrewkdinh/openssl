...
static int
encode_string(unsigned char *out, size_t out_max_len, size_t *out_len, const unsigned char *in, size_t in_len);
static int  right_encode(unsigned char *out, size_t out_max_len, size_t *out_len, size_t bits);
static int  bytepad(unsigned char       *out,
                    size_t              *out_len,
                    ...
                    size_t               in2_len,
                    size_t               w);
static int  kmac_bytepad_encode_key(unsigned char       *out,
                                    size_t               out_max_len,
                                    size_t              *out_len,
...
static int kmac_setkey(struct kmac_data_st *kctx, const unsigned char *key, size_t keylen)
{
    ...
    if (!kctx->internal) {
        ...
        if (!approved) {
            if (!OSSL_FIPS_IND_ON_UNAPPROVED(kctx,
