    ...
    if (!kctx->internal) {
        ...
        if (!approved) {
            if (!OSSL_FIPS_IND_ON_UNAPPROVED(
                kctx, OSSL_FIPS_IND_SETTABLE1, PROV_LIBCTX_OF(kctx->provctx),
                "KMAC", "Key size", ossl_fips_config_kmac_key_check)) {
                ERR_raise(ERR_LIB_PROV, PROV_R_INVALID_KEY_LENGTH);
                return 0;
            }
        }
...
static int kmac_set_ctx_params(void *vmacctx, const OSSL_PARAM *params) {
    ...
    if ((p = OSSL_PARAM_locate_const(params, OSSL_MAC_PARAM_SIZE)) != NULL) {
        ...
        if (sz < 32 / 8) {
            if (!OSSL_FIPS_IND_ON_UNAPPROVED(
                kctx, OSSL_FIPS_IND_SETTABLE0, PROV_LIBCTX_OF(kctx->provctx),
                "KMAC", "length", ossl_fips_config_no_short_mac)) {
                ERR_raise(ERR_LIB_PROV, PROV_R_INVALID_OUTPUT_LENGTH);
                return 0;
            }
        }
