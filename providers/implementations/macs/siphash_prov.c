...
static void siphash_free(void *vmacctx) { OPENSSL_free(vmacctx); }
...
siphash_gettable_ctx_params(ossl_unused void *ctx, ossl_unused void *provctx) {
...
static int siphash_set_params(void *vmacctx, const OSSL_PARAM *params) {
    ...
    if ((p = OSSL_PARAM_locate_const(params, OSSL_MAC_PARAM_C_ROUNDS)) !=
        NULL &&
        !OSSL_PARAM_get_uint(p, &ctx->crounds))
        return 0;
    if ((p = OSSL_PARAM_locate_const(params, OSSL_MAC_PARAM_D_ROUNDS)) !=
        NULL &&
