...
static size_t gmac_size(void) { return EVP_GCM_TLS_TAG_LEN; }
...
static int gmac_final(void *vmacctx, unsigned char *out, size_t *outl,
  ...
  hlen = gmac_size();
  params[0] = OSSL_PARAM_construct_octet_string(OSSL_CIPHER_PARAM_AEAD_TAG, out,
                                                (size_t)hlen);
...
static int gmac_set_ctx_params(void *vmacctx, const OSSL_PARAM params[]) {
  ...
  if ((p = OSSL_PARAM_locate_const(params, OSSL_MAC_PARAM_IV)) != NULL) {
    ...
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_IVLEN, p->data_size, NULL) <=
        0 ||
