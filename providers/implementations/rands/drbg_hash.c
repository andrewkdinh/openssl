...
static int drbg_hash_set_ctx_params_locked(void *vctx,
...
static int hash_df(PROV_DRBG *drbg, unsigned char *out,
                   ...
                   size_t inlen, const unsigned char *in2, size_t in2len,
...
static int hash_df1(PROV_DRBG *drbg, unsigned char *out,
                    const unsigned char in_byte, const unsigned char *in1,
                    size_t in1len) {
 ...
 */
static int add_bytes(PROV_DRBG *drbg, unsigned char *dst, unsigned char *in,
                     size_t inlen) {
  size_t i;
  int result;
 ...
 * Returns zero if an error occurs otherwise it returns 1.
 */
static int drbg_hash_instantiate(PROV_DRBG *drbg, const unsigned char *ent,
  ...
  return hash->ctx != NULL &&
         hash_df(drbg, hash->V, INBYTE_IGNORE, ent, ent_len, nonce, nonce_len,
                 pstr, pstr_len)
 ...
 * Returns zero if an error occurs otherwise it returns 1.
 */
static int drbg_hash_reseed(PROV_DRBG *drbg, const unsigned char *ent,
  ...
  if (!hash_df(drbg, hash->C, 0x01, hash->V, drbg->seedlen, ent, ent_len, adin,
               adin_len))
    return 0;
 ...
 * Returns zero if an error occurs otherwise it returns 1.
 */
static int drbg_hash_generate(PROV_DRBG *drbg, unsigned char *out,
...
static int drbg_hash_generate_wrapper(void *vdrbg, unsigned char *out,
...
static void *drbg_hash_new_wrapper(void *provctx, void *parent,
  ...
  return ossl_rand_drbg_new(provctx, parent, parent_dispatch, &drbg_hash_new,
                            &drbg_hash_free, &drbg_hash_instantiate,
                            &drbg_hash_uninstantiate, &drbg_hash_reseed,
                            &drbg_hash_generate);
}
...
drbg_hash_gettable_ctx_params(ossl_unused void *vctx, ossl_unused void *p_ctx) {
...
static int drbg_fetch_digest_from_prov(const OSSL_PARAM params[],
                                       OSSL_LIB_CTX *libctx, EVP_MD **digest) {
...
static const OSSL_PARAM *
drbg_hash_settable_ctx_params(ossl_unused void *vctx, ossl_unused void *p_ctx) {
