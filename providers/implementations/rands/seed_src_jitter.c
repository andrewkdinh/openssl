 ...
 */
static size_t ossl_prov_acquire_entropy_from_jitter(PROV_JITTER *s, RAND_POOL *pool)
{
    size_t bytes_needed;
...
static size_t get_jitter_random_value(PROV_JITTER *s, unsigned char *buf, size_t len)
{
    ...
    ERR_raise_data(ERR_LIB_RAND, RAND_R_ERROR_RETRIEVING_ENTROPY, "jent_read_entropy (%d)", result);
    return 0;
}

static void *jitter_new(void *provctx, void *parent, const OSSL_DISPATCH *parent_dispatch)
{
    PROV_JITTER *s;
...
static int jitter_instantiate(void *vseed, unsigned int strength, int prediction_resistance, const unsigned char *pstr,
                              size_t pstr_len, ossl_unused const OSSL_PARAM params[])
{
    PROV_JITTER *s = (PROV_JITTER *)vseed;
    int ret;

    if ((ret = jent_entropy_init_ex(0, JENT_FORCE_FIPS)) != 0) {
        ERR_raise_data(ERR_LIB_RAND, RAND_R_ERROR_RETRIEVING_ENTROPY, "jent_entropy_init_ex (%d)", ret);
        s->state = EVP_RAND_STATE_ERROR;
        return 0;
    }
...
static int jitter_generate(void *vseed, unsigned char *out, size_t outlen, unsigned int strength,
                           ossl_unused int prediction_resistance, ossl_unused const unsigned char *adin,
...
static int jitter_reseed(void *vseed, ossl_unused int prediction_resistance, ossl_unused const unsigned char *ent,
                         ossl_unused size_t ent_len, ossl_unused const unsigned char *adin, ossl_unused size_t adin_len)
{
    PROV_JITTER *s = (PROV_JITTER *)vseed;
...
static const OSSL_PARAM *jitter_gettable_ctx_params(ossl_unused void *vseed, ossl_unused void *provctx)
{
    static const OSSL_PARAM known_gettable_ctx_params[] = {
        OSSL_PARAM_int(OSSL_RAND_PARAM_STATE, NULL), OSSL_PARAM_uint(OSSL_RAND_PARAM_STRENGTH, NULL),
...
static size_t jitter_get_seed(void *vseed, unsigned char **pout, int entropy, size_t min_len, size_t max_len,
                              int prediction_resistance, const unsigned char *adin, size_t adin_len)
{
    ...
    if (entropy_available > 0 && ossl_rand_pool_adin_mix_in(pool, adin, adin_len)) {
...
static void jitter_clear_seed(ossl_unused void *vdrbg, unsigned char *out, size_t outlen)
{
...
const OSSL_DISPATCH ossl_jitter_functions[] = {
    ...
    {OSSL_FUNC_RAND_INSTANTIATE, (void (*)(void))jitter_instantiate},
    {OSSL_FUNC_RAND_UNINSTANTIATE, (void (*)(void))jitter_uninstantiate},
    ...
    {OSSL_FUNC_RAND_GETTABLE_CTX_PARAMS, (void (*)(void))jitter_gettable_ctx_params},
    {OSSL_FUNC_RAND_GET_CTX_PARAMS, (void (*)(void))jitter_get_ctx_params},
    {OSSL_FUNC_RAND_VERIFY_ZEROIZATION, (void (*)(void))jitter_verify_zeroization},
