 ...
 */
static int do_hmac(PROV_DRBG_HMAC *hmac, unsigned char inbyte, const unsigned char *in1, size_t in1len,
 ...
 * Returns zero if an error occurs otherwise it returns 1.
 */
static int drbg_hmac_update(PROV_DRBG_HMAC *hmac, const unsigned char *in1, size_t in1len, const unsigned char *in2,
 ...
 * Returns zero if an error occurs otherwise it returns 1.
 */
int ossl_drbg_hmac_init(PROV_DRBG_HMAC *hmac, const unsigned char *ent, size_t ent_len, const unsigned char *nonce,
                        size_t nonce_len, const unsigned char *pstr, size_t pstr_len)
{
    if (hmac->ctx == NULL) {
    ...
    return drbg_hmac_update(hmac, ent, ent_len, nonce, nonce_len, pstr, pstr_len);
}
static int drbg_hmac_instantiate(PROV_DRBG *drbg, const unsigned char *ent, size_t ent_len, const unsigned char *nonce,
                                 size_t nonce_len, const unsigned char *pstr, size_t pstr_len)
{
    return ossl_drbg_hmac_init((PROV_DRBG_HMAC *)drbg->data, ent, ent_len, nonce, nonce_len, pstr, pstr_len);
}

static int drbg_hmac_instantiate_wrapper(void *vdrbg, unsigned int strength, int prediction_resistance,
                                         const unsigned char *pstr, size_t pstr_len, const OSSL_PARAM params[])
{
    ...
    if (!ossl_prov_is_running() || !drbg_hmac_set_ctx_params_locked(drbg, params))
        goto err;
 ...
 * Returns zero if an error occurs otherwise it returns 1.
 */
static int drbg_hmac_reseed(PROV_DRBG *drbg, const unsigned char *ent, size_t ent_len, const unsigned char *adin,
                            size_t adin_len)
{
...
static int drbg_hmac_reseed_wrapper(void *vdrbg, int prediction_resistance, const unsigned char *ent, size_t ent_len,
 ...
 * Returns zero if an error occurs otherwise it returns 1.
 */
int ossl_drbg_hmac_generate(PROV_DRBG_HMAC *hmac, unsigned char *out, size_t outlen, const unsigned char *adin,
                            size_t adin_len)
{
    EVP_MAC_CTX *ctx = hmac->ctx;
    ...
    if (adin != NULL && adin_len > 0 && !drbg_hmac_update(hmac, adin, adin_len, NULL, 0, NULL, 0))
        return 0;
...
static int drbg_hmac_generate(PROV_DRBG *drbg, unsigned char *out, size_t outlen, const unsigned char *adin,
                              size_t adin_len)
{
    return ossl_drbg_hmac_generate((PROV_DRBG_HMAC *)drbg->data, out, outlen, adin, adin_len);
}
...
{
    return ossl_rand_drbg_new(provctx, parent, parent_dispatch, &drbg_hmac_new, &drbg_hmac_free, &drbg_hmac_instantiate,
                              &drbg_hmac_uninstantiate, &drbg_hmac_reseed, &drbg_hmac_generate);
}
...
static const OSSL_PARAM *drbg_hmac_gettable_ctx_params(ossl_unused void *vctx, ossl_unused void *p_ctx)
{
    static const OSSL_PARAM known_gettable_ctx_params[] = {
        OSSL_PARAM_utf8_string(OSSL_DRBG_PARAM_MAC, NULL, 0), OSSL_PARAM_utf8_string(OSSL_DRBG_PARAM_DIGEST, NULL, 0),
        OSSL_PARAM_DRBG_GETTABLE_CTX_COMMON, OSSL_FIPS_IND_GETTABLE_CTX_PARAM() OSSL_PARAM_END};
    return known_gettable_ctx_params;
}

static int drbg_fetch_algs_from_prov(const OSSL_PARAM params[], OSSL_LIB_CTX *libctx, EVP_MAC_CTX **macctx,
                                     EVP_MD **digest)
{
    ...
    if ((p = OSSL_PARAM_locate_const(params, OSSL_PROV_PARAM_CORE_PROV_NAME)) == NULL)
        return 0;
...
static int drbg_hmac_set_ctx_params_locked(void *vctx, const OSSL_PARAM params[])
{
    ...
    if (!OSSL_FIPS_IND_SET_CTX_PARAM(ctx, OSSL_FIPS_IND_SETTABLE0, params, OSSL_DRBG_PARAM_FIPS_DIGEST_CHECK))
        return 0;
...
static const OSSL_PARAM *drbg_hmac_settable_ctx_params(ossl_unused void *vctx, ossl_unused void *p_ctx)
{
    static const OSSL_PARAM known_settable_ctx_params[] = {
        ...
        OSSL_PARAM_utf8_string(OSSL_DRBG_PARAM_DIGEST, NULL, 0), OSSL_PARAM_utf8_string(OSSL_DRBG_PARAM_MAC, NULL, 0),
        OSSL_PARAM_DRBG_SETTABLE_CTX_COMMON,
        OSSL_FIPS_IND_SETTABLE_CTX_PARAM(OSSL_DRBG_PARAM_FIPS_DIGEST_CHECK) OSSL_PARAM_END};
...
const OSSL_DISPATCH ossl_drbg_ossl_hmac_functions[] = {
    ...
    {OSSL_FUNC_RAND_INSTANTIATE, (void (*)(void))drbg_hmac_instantiate_wrapper},
    {OSSL_FUNC_RAND_UNINSTANTIATE, (void (*)(void))drbg_hmac_uninstantiate_wrapper},
    ...
    {OSSL_FUNC_RAND_VERIFY_ZEROIZATION, (void (*)(void))drbg_hmac_verify_zeroization},
