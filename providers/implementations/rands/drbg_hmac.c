 ...
 * Returns zero if an error occurs otherwise it returns 1.
 */
static int drbg_hmac_update(PROV_DRBG_HMAC *hmac, const unsigned char *in1,
                            size_t in1len, const unsigned char *in2,
                            size_t in2len, const unsigned char *in3,
                            size_t in3len) {
 ...
 * Returns zero if an error occurs otherwise it returns 1.
 */
int ossl_drbg_hmac_init(PROV_DRBG_HMAC *hmac, const unsigned char *ent,
...
static int drbg_hmac_instantiate(PROV_DRBG *drbg, const unsigned char *ent,
                                 size_t ent_len, const unsigned char *nonce,
                                 size_t nonce_len, const unsigned char *pstr,
                                 size_t pstr_len) {
  return ossl_drbg_hmac_init((PROV_DRBG_HMAC *)drbg->data, ent, ent_len, nonce,
                             nonce_len, pstr, pstr_len);
}
 ...
 * Returns zero if an error occurs otherwise it returns 1.
 */
static int drbg_hmac_reseed(PROV_DRBG *drbg, const unsigned char *ent,
...
static int drbg_hmac_generate(PROV_DRBG *drbg, unsigned char *out,
                              size_t outlen, const unsigned char *adin,
                              size_t adin_len) {
...
static int drbg_hmac_generate_wrapper(void *vdrbg, unsigned char *out,
...
static void *drbg_hmac_new_wrapper(void *provctx, void *parent,
  ...
  return ossl_rand_drbg_new(provctx, parent, parent_dispatch, &drbg_hmac_new,
                            &drbg_hmac_free, &drbg_hmac_instantiate,
                            &drbg_hmac_uninstantiate, &drbg_hmac_reseed,
                            &drbg_hmac_generate);
}
...
drbg_hmac_gettable_ctx_params(ossl_unused void *vctx, ossl_unused void *p_ctx) {
...
static int drbg_fetch_algs_from_prov(const OSSL_PARAM params[],
                                     OSSL_LIB_CTX *libctx, EVP_MAC_CTX **macctx,
                                     EVP_MD **digest) {
...
static int drbg_hmac_set_ctx_params_locked(void *vctx,
  ...
  if (!drbg_fetch_algs_from_prov(params, libctx, &hmac->ctx, &prov_md)) {
    ...
    if (!ossl_prov_macctx_load_from_params(&hmac->ctx, params, NULL, NULL, NULL,
                                           libctx))
      return 0;
  } else {
...
static const OSSL_PARAM *
drbg_hmac_settable_ctx_params(ossl_unused void *vctx, ossl_unused void *p_ctx) {
