...
# endif
    ...
    for (i = 0; i < bytes_needed; i++) {
         ...
         * interference, etc.
         */
        for (k = 0; k < 99; k++) ts.tv_nsec = random();
...
int ossl_rand_pool_init(void)
{
    ...
    for (i = 0; i < OSSL_NELEM(random_devices); i++) random_devices[i].fd = -1;
...
void ossl_rand_pool_cleanup(void)
{
    ...
    for (i = 0; i < OSSL_NELEM(random_devices); i++) close_random_device(i);
}
